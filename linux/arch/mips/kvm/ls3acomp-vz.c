					      ...
					      struct kvm_vcpu *vcpu)
{
	...
	unsigned long curr_pc, any_send_data;
	void *data = run->mmio.data;
	int ret = 0;
	unsigned long any_send_addr = 0;
	u8 tmp_any_send_data;
	...
	switch (inst.loongson3_lscsr_format.fr) {
		...
		case 0x1:
			...
			if ((vcpu->arch.gprs[rs] == 0x1004) || (vcpu->arch.gprs[rs] == 0x100c)) {
				...
				*(u32 *)data = (1 << action);
				...
				run->mmio.phys_addr = ((unsigned long)(cpu / 4) << vcpu->kvm->arch.node_shift) | LOONGSON3_REG_BASE |
					...
					/*write ext int ctl reg*/
					*(u32 *)data = vcpu->arch.gprs[rd] & 0xffffffff;
					run->mmio.phys_addr = ((unsigned long)(vcpu->vcpu_id / 4) << vcpu->kvm->arch.node_shift)
								| LOONGSON_REG_BASE | vcpu->arch.gprs[rs];
			} else if (vcpu->arch.gprs[rs] >= 0x0420 && vcpu->arch.gprs[rs] < 0x0428) {
				/*other dev reg for ext int en, simulation is not required*/
				vcpu->mmio_needed = 0;
				er = EMULATE_DONE;
				break;
			} else {
			...
			if (!ret) {
				...
				kvm_err(KERN_ERR"%s(%d):address access err phys_addr 0x%llx id %d ret %d\n",
								__FUNCTION__, __LINE__, run->mmio.phys_addr, vcpu->vcpu_id, ret);
				er = EMULATE_DO_MMIO;
				run->mmio.is_write = 1;
				vcpu->mmio_needed = 1;
		...
		case 0x2:
			...
			if (vcpu->arch.gprs[rs] == 0x8) {
				...
				/*Then get guest phys*/
				run->mmio.phys_addr = ((unsigned long)(vcpu->vcpu_id / 4) << vcpu->kvm->arch.node_shift) |
							(LOONGSON_REG_BASE) | (vcpu->arch.gprs[rs] & 0xffff) | ((vcpu->vcpu_id % 4) << 8);
				ret = kvm_io_bus_read(vcpu, KVM_MMIO_BUS, run->mmio.phys_addr, run->mmio.len, run->mmio.data);
				run->mmio.is_write = 0;
				vcpu->mmio_is_write = 0;
				if (!ret) {
					...
					kvm_err(KERN_ERR"%s(%d): address access error run->mmio.phys_addr %llx size %d ret %d\n",
									__FUNCTION__, __LINE__, run->mmio.phys_addr, run->mmio.len, ret);
					er = EMULATE_DO_MMIO;
				}
				kvm_debug("vcpu id:%d,DRDCSR addr=0x%lx \n", vcpu->vcpu_id, vcpu->arch.gprs[rs] & 0xffff);
			} else {
		...
		case 0x3:
			...
			*(u64 *)data = vcpu->arch.gprs[rd];
			...
			if (vcpu->arch.gprs[rs] == 0x1020) {
				...
				run->mmio.phys_addr = ((unsigned long)(vcpu->vcpu_id / 4)
							<< vcpu->kvm->arch.node_shift)
							| LOONGSON_REG_BASE | (vcpu->arch.gprs[rs] & 0xffff) | ((vcpu->vcpu_id % 4) << 8);
				...
				if(ls3a_extirq_in_kernel(vcpu->kvm)) {
					...
					if (!ret) {
						...
						kvm_debug(KERN_ERR "%s(%d):--address access err  %llx size %d ret %d\n",
									__FUNCTION__, __LINE__, run->mmio.phys_addr, run->mmio.len, ret);
						er = EMULATE_DO_MMIO;
						run->mmio.is_write = 1;
						vcpu->mmio_needed = 1;
				...
				kvm_debug("%s:%d any_send dwrcsr addr %x data %lx data_mask 0x%llx\n",
								__FUNCTION__, __LINE__, offset, any_send_data, data_mask);
				if ((((offset & 0xff00) == 0x1600) || ((offset & 0xff00) == 0x1C00) ||
					...
					if (data_mask == 0) {
						ret = kvm_io_bus_write(vcpu, KVM_MMIO_BUS, any_send_addr, 4, (void *)&any_send_data);
					} else {
						for (tmp_data = 0 ; tmp_data < 4 ; tmp_data++) {
							if (!(data_mask & (0x1UL << tmp_data))) {
								tmp_any_send_data = (any_send_data >> (8 * tmp_data)) & 0xff;
								ret = kvm_io_bus_write(vcpu, KVM_MMIO_BUS, any_send_addr + tmp_data,
										1, (void *)&tmp_any_send_data);
							}
						}
					}
					...
					if (!ret) {
						...
						kvm_err("%s:%d any_send dwrcsr err offset %x data %lx data_mask 0x%llx\n",
								__FUNCTION__, __LINE__, offset, any_send_data, data_mask);
						er = EMULATE_DO_MMIO;
						run->mmio.is_write = 1;
						vcpu->mmio_needed = 1;
					...
					kvm_err("err any send,offset = 0x%x,data = 0x%lx\n", offset, any_send_data);
				...
				if (mail_hi) {
					...
					*(u64 *)data = vcpu->kvm->arch.mval[cpu][mailbox].mailval;
					...
					ret = kvm_io_bus_write(vcpu, KVM_MMIO_BUS,
								run->mmio.phys_addr, 8, data);
					if (!ret) {
						vcpu->mmio_needed = 0;
						er = EMULATE_DONE;
					} else {
						kvm_err("%s(%d): address access phys_addr %llx ret %d\n", __FUNCTION__,
												__LINE__, run->mmio.phys_addr, ret);
						er = EMULATE_DO_MMIO;
						run->mmio.is_write = 1;
						vcpu->mmio_needed = 1;
						vcpu->mmio_is_write = 1;
	...
	if (er == EMULATE_FAIL) {
		kvm_err("[%#lx]%s: unsupported lwc2 instruction 0x%08x 0x%08x\n",
			curr_pc, __FUNCTION__, inst.word, inst.loongson3_lscsr_format.fr);
					      ...
					      struct kvm_vcpu *vcpu)
{
	...
	if (er == EMULATE_FAIL) {
		kvm_err("[%#lx]%s: unsupported cop0 instruction 0x%08x\n",
			curr_pc, __FUNCTION__, inst.word);
