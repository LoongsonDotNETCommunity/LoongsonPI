...
static irqreturn_t pcie_pme_irq(int irq, void *context)
{
	...
	unsigned long flags;
	void *base = NULL;
	unsigned int pcie_irq_status = 0;
	...
	if (port->vendor == PCI_VENDOR_ID_LOONGSON) {
		base = (void *)TO_UNCAC(pci_resource_start(port, 0));
		if (base != NULL) {
			/*pcie port interrupt status reg*/
			pcie_irq_status = readl(base + 0x18);
			/*pcie port interrupt status clear reg, w1c*/
			writel(pcie_irq_status, base + 0x1c);
		} else {
			pci_warn(port, "bar0 is null\n");
		}
	}
	spin_unlock_irqrestore(&data->lock, flags);
