...
#endif /* CONFIG_FILE_FWIMG */
		err = rtw_halmac_init_hal_fw(d, array_mp_8822b_fw_nic,
					     array_length_mp_8822b_fw_nic);
...
void rtl8822b_init_misc(PADAPTER adapter)
{
	...
	rtw_write16(adapter, REG_CR, (rtw_read16(adapter,
				      REG_CR) | BIT_MAC_SEC_EN));

#ifdef CONFIG_XMIT_ACK
	/* ack for xmit mgmt frames. */
	rtw_write32(adapter, REG_FWHW_TXQ_CTRL_8822B,
		    rtw_read32(adapter, REG_FWHW_TXQ_CTRL_8822B) | BIT_EN_QUEUE_RPT_8822B(BIT(
					    4)));
#endif /* CONFIG_XMIT_ACK */

	rtw_write8(adapter, REG_TIMER0_SRC_SEL_8822B, rtw_read8(adapter,
			REG_TIMER0_SRC_SEL_8822B) & ~BIT(6));
}
...
void rtl8822b_init_default_value(PADAPTER adapter)
{
	...
	_rtw_memset(hal->EfuseHal.BTEfuseContent, 0xFF,
	...
	_rtw_memset(hal->EfuseHal.fakeBTEfuseContent, 0xFF,
		    EFUSE_MAX_BT_BANK * EFUSE_MAX_HW_SIZE);
	_rtw_memset(hal->EfuseHal.fakeBTEfuseInitMap, 0xFF, EFUSE_BT_MAX_MAP_LEN);
	_rtw_memset(hal->EfuseHal.fakeBTEfuseModifiedMap, 0xFF,
