...
static void rtl8822bu_interrupt_handler(PADAPTER padapter, u16 pkt_len,
					u8 *pbuf)
{
}
...
static u8 sethwreg(PADAPTER padapter, u8 variable, u8 *val)
{
	...
	case HW_VAR_RXDMA_AGG_PG_TH:
		...
		if (pdvobjpriv->traffic_stat.cur_tx_tp < 1
		    && pdvobjpriv->traffic_stat.cur_rx_tp < 1) {
	...
	case HW_VAR_USB_MODE:
		...
		if (registry_par->switch_usb_mode == 1) {
			if (IS_HIGH_SPEED_USB(padapter)) {
				status = rtw_halmac_switch_usb_mode(adapter_to_dvobj(padapter),
								    RTW_USB_SPEED_3);
				if (status)
					*val = _TRUE;
			}
...
*/
static u8 sethaldefvar(PADAPTER padapter, HAL_DEF_VARIABLE eVariable,
		       PVOID pValue)
{
	HAL_DATA_TYPE *pHalData = GET_HAL_DATA(padapter);
	u8 bResult = _SUCCESS;
...
*/
static u8 gethaldefvar(PADAPTER	padapter, HAL_DEF_VARIABLE eVariable,
		       PVOID pValue)
{
	HAL_DATA_TYPE *pHalData = GET_HAL_DATA(padapter);
	u8 bResult = _SUCCESS;
...
static u8 rtl8822bu_ps_func(PADAPTER padapter, HAL_INTF_PS_FUNC efunc_id,
			    u8 *val)
{
	u8 bResult = _TRUE;
