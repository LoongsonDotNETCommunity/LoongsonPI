...
static u8 _init_bb_reg(PADAPTER Adapter)
{
	...
	ret = _FALSE;
#ifdef CONFIG_LOAD_PHY_PARA_FROM_FILE
	res = phy_ConfigBBWithParaFile(Adapter, PHY_FILE_PHY_REG,
				       CONFIG_BB_PHY_REG);
	if (_SUCCESS == res)
		ret = _TRUE;
#endif
...
#endif
		if (_FALSE == ret) {
			status = odm_config_bb_with_header_file(&hal->odmpriv,
	...
	ret = _FALSE;
#ifdef CONFIG_LOAD_PHY_PARA_FROM_FILE
	res = phy_ConfigBBWithParaFile(Adapter, PHY_FILE_AGC_TAB,
				       CONFIG_BB_AGC_TAB);
	if (_SUCCESS == res)
		ret = _TRUE;
#endif
...
#endif
		if (_FALSE == ret) {
			status = odm_config_rf_with_header_file(&hal->odmpriv, CONFIG_RF_RADIO,
								phydm_path);
			if (HAL_STATUS_SUCCESS != status)
				goto exit;
			ret = _TRUE;
...
static void dm_CheckPbcGPIO(PADAPTER adapter)
{
	...
	rtw_write8(adapter, GPIO_IO_SEL,
		   tmp1byte); /* enable GPIO[2] as output mode */

	tmp1byte &= ~(HAL_8192C_HW_GPIO_WPS_BIT);
	rtw_write8(adapter, GPIO_IN,
		   tmp1byte); /* reset the floating voltage level */

	tmp1byte = rtw_read8(adapter, GPIO_IO_SEL);
	tmp1byte &= ~(HAL_8192C_HW_GPIO_WPS_BIT);
	rtw_write8(adapter, GPIO_IO_SEL,
		   tmp1byte); /* enable GPIO[2] as input mode */
...
static void check_rxfifo_full(PADAPTER adapter)
{
	...
	if (regsty->check_hw_status == 1) {
		...
		pdbgpriv->dbg_rx_fifo_curr_overflow = rtw_read16(adapter,
						      REG_RXERR_RPT_8822B);
...
u32 rtl8822b_read_rf_reg(PADAPTER adapter, enum rf_path path, u32 addr,
			 u32 mask)
{
...
void rtl8822b_write_rf_reg(PADAPTER adapter, enum rf_path path, u32 addr,
			   u32 mask, u32 val)
{
	struct dm_struct *phydm = adapter_to_phydm(adapter);
	u8 ret;
...
static void set_tx_power_level_by_path(PADAPTER adapter, u8 channel,
				       u8 path)
{
 ...
 */
void rtl8822b_set_tx_power_index(PADAPTER adapter, u32 powerindex,
 ...
 */
u8 rtl8822b_get_tx_power_index(PADAPTER adapter, enum rf_path rfpath,
			       u8 rate, u8 bandwidth, u8 channel, struct txpwr_idx_comp *tic)
{
	PHAL_DATA_TYPE hal = GET_HAL_DATA(adapter);
	s16 power_idx;
	u8 base_idx = 0;
	...
	base_idx = PHY_GetTxPowerIndexBase(adapter, rfpath, rate, ntx_idx,
...
static void mac_switch_bandwidth(PADAPTER adapter, u8 pri_ch_idx)
{
	...
	err = rtw_halmac_set_bandwidth(adapter_to_dvobj(adapter), channel,
				       pri_ch_idx, bw);
	if (err) {
...
static void switch_chnl_and_set_bw_by_drv(PADAPTER adapter, u8 switch_band)
{
	...
	if (hal->bSetChnlBW) {
		...
		ret = config_phydm_switch_bandwidth_8822b(p_dm_odm, pri_ch_idx,
	...
	u8 ChnlOffsetOf80MHz, u8 CenterFrequencyIndex1)
{
	...
	if (bSetBandWidth) {
		...
		} else if ((hal->current_channel_bw != ChnlWidth)
			   || /* check whether need set band or not */
 ...
 */
void rtl8822b_set_channel_bw(PADAPTER adapter, u8 center_ch,
			     enum channel_width bw, u8 offset40, u8 offset80)
{
	rtl8822b_handle_sw_chnl_and_set_bw(adapter, _TRUE, _TRUE, center_ch, bw,
					   offset40, offset80, center_ch);
}
...
static void _sounding_config_su(PADAPTER adapter,
				struct beamformee_entry *bfee, enum _HW_CFG_SOUNDING_TYPE cfg_type)
{
	...
	case 0:
		new_ctrl &= ~(BIT_R_TXBF0_20M_8822B | BIT_R_TXBF0_40M_8822B |
			      BIT_R_TXBF0_80M_8822B);
		break;
	case 1:
		new_ctrl &= ~(BIT_R_TXBF1_20M_8822B | BIT_R_TXBF1_40M_8822B |
			      BIT_R_TXBF1_80M_8822B);
		break;
	}
...
static void _sounding_config_mu(PADAPTER adapter,
				struct beamformee_entry *bfee, enum _HW_CFG_SOUNDING_TYPE cfg_type)
{
	struct beamforming_info *info;
...
static void _config_sounding(PADAPTER adapter,
			     struct beamformee_entry *bfee, u8 mu_sounding,
			     enum _HW_CFG_SOUNDING_TYPE cfg_type)
{
...
static void _config_beamformer_su(PADAPTER adapter,
...
static void _config_beamformer_mu(PADAPTER adapter,
				  struct beamformer_entry *bfer)
{
	/* General */
	PHAL_DATA_TYPE hal;
...
static void _config_beamformee_su(PADAPTER adapter,
				  struct beamformee_entry *bfee)
{
	/* General */
	...
	phydm_8822btxbf_rfmode(GET_PDM_ODM(adapter), info->beamformee_su_cnt,
			       info->beamformee_mu_cnt);

	/* P_AID of Beamformee & enable NDPA transmission & enable NDPA interrupt */
	val32 = rtw_read32(adapter, REG_TXBF_CTRL_8822B);
	if (idx == 0) {
		val32 = BIT_SET_R_TXBF0_AID_8822B(val32, p_aid);
		val32 &= ~(BIT_R_TXBF0_20M_8822B | BIT_R_TXBF0_40M_8822B |
			   BIT_R_TXBF0_80M_8822B);
	} else {
		val32 = BIT_SET_R_TXBF1_AID_8822B(val32, p_aid);
		val32 &= ~(BIT_R_TXBF1_20M_8822B | BIT_R_TXBF1_40M_8822B |
			   BIT_R_TXBF1_80M_8822B);
	}
	val32 |= BIT_R_EN_NDPA_INT_8822B | BIT_USE_NDPA_PARAMETER_8822B |
		 BIT_R_ENABLE_NDPA_8822B;
...
static void _config_beamformee_mu(PADAPTER adapter,
				  struct beamformee_entry *bfee)
{
	/* General */
	PHAL_DATA_TYPE hal;
	...
	val32 = BIT_SET_R_MU_TABLE_VALID_8822B(val32,
					       info->beamformee_mu_reg_maping);
	rtw_write32(adapter, REG_MU_TX_CTL_8822B, val32);
	...
	rtw_bf_send_vht_gid_mgnt_packet(adapter, bfee->mac_addr, bfee->gid_valid,
					bfee->user_position);
}

static void _reset_beamformer_su(PADAPTER adapter,
				 struct beamformer_entry *bfer)
{
	/* Beamforming */
	struct beamforming_info *info;
...
static void _reset_beamformer_mu(PADAPTER adapter,
				 struct beamformer_entry *bfer)
{
	struct beamforming_info *bf_info;
...
static void _reset_beamformee_su(PADAPTER adapter,
				 struct beamformee_entry *bfee)
{
	/* Beamforming */
	struct beamforming_info *info;
	...
	if (idx == 0) {
		...
		txbf_ctrl &= ~(BIT_R_TXBF0_20M_8822B | BIT_R_TXBF0_40M_8822B |
			       BIT_R_TXBF0_80M_8822B);
		...
		txbf_ctrl &= ~(BIT_R_TXBF1_20M_8822B | BIT_R_TXBF1_40M_8822B |
			       BIT_R_TXBF1_80M_8822B);

		bfmee_sel = BIT_SET_AID1_8822B(bfmee_sel, 0);
		bfmee_sel &= ~BIT_TXUSER_ID1_8822B;
	}
	txbf_ctrl |= BIT_R_EN_NDPA_INT_8822B | BIT_USE_NDPA_PARAMETER_8822B |
		     BIT_R_ENABLE_NDPA_8822B;
...
static void _reset_beamformee_mu(PADAPTER adapter,
				 struct beamformee_entry *bfee)
{
	/* Beamforming */
	struct beamforming_info *info;
				   ...
				   struct beamformer_entry	*bfer_info)
{
	...
	rtw_halmac_bf_cfg_mu_bfee(adapter_to_dvobj(adapter), gid_valid,
				  user_position);

	info->bSetBFHwConfigInProgess = _FALSE;
}

void rtl8822b_phy_bf_set_csi_report(PADAPTER adapter,
				    struct _RT_CSI_INFO *csi)
{
	PHAL_DATA_TYPE hal;
	struct beamforming_info *info;
...
void rtl8822b_phy_bf_sounding_status(PADAPTER adapter, u8 status)
{
	...
	if (sounding->state == SOUNDING_STATE_SU_SOUNDDOWN) {
		...
		RTW_INFO("%s: is_sounding_success STA1:%d, STA2:%d, STA3:%d, STA4:%d, STA5:%d, STA6:%d\n",
			 __FUNCTION__, is_sounding_success[0], is_sounding_success[1],
