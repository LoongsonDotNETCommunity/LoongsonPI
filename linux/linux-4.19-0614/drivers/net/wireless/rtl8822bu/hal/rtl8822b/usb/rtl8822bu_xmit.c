...
static void update_txdesc_h2c_pkt(struct xmit_frame *pxmitframe, u8 *pmem,
				  s32 sz)
{
	u8 *ptxdesc =  pmem;
...
s32 rtl8822bu_xmit_buf_handler(PADAPTER padapter)
{
	...
	do {
		...
		rtw_write_port_and_wait(padapter, pxmitbuf->ff_hwaddr, pxmitbuf->len,
					(unsigned char *)pxmitbuf, 500);
	} while (1);
...
static s32 rtw_dump_xframe(PADAPTER padapter,
...
#ifdef RTW_HALMAC
		pxmitbuf->bulkout_id = rtw_halmac_usb_get_bulkout_id(pdvobj, mem_addr,
				       w_sz);
#endif
...
static s32 rtl8822bu_xmitframe_complete(PADAPTER padapter,
					struct xmit_priv *pxmitpriv, struct xmit_buf *pxmitbuf)
{
	...
	do {
		...
		if (rtw_xmitframe_coalesce(padapter, pxmitframe->pkt,
					   pxmitframe) == _FALSE) {
	...
	len = rtw_wlan_pkt_size(pfirstframe) + TXDESC_SIZE +
	...
	single_sta_in_queue = rtw_end_of_queue_search(sta_phead,
			      get_next(sta_plist));
	...
	while (rtw_end_of_queue_search(xmitframe_phead,
		...
		len = rtw_wlan_pkt_size(pxmitframe) + TXDESC_SIZE + (pxmitframe->pkt_offset
				* PACKET_OFFSET_SZ);
	...
	if (_rtw_queue_empty(&ptxservq->sta_pending) == _TRUE)
		...
		rtw_list_insert_tail(&ptxservq->tx_pending,
				     get_list_head(phwxmit->sta_queue));
	}
...
agg_end:
	...
	pxmitpriv->agg_pkt[0].offset = _RND8(pfirstframe->attrib.last_txcmdsz +
					     TXDESC_SIZE + (pfirstframe->pkt_offset * PACKET_OFFSET_SZ));
...
#ifdef RTW_HALMAC
	pxmitbuf->bulkout_id = rtw_halmac_usb_get_bulkout_id(pdvobj,
			       pfirstframe->buf_addr, pfirstframe->attrib.last_txcmdsz);
#endif
		...
		if (pxmitframe) {
			...
			if ((pxmitframe->frame_tag & 0x0f) == DATA_FRAMETAG) {
				...
				rtw_os_xmit_complete(padapter,
						     pxmitframe);/* always return ndis_packet after rtw_xmitframe_coalesce */
			}
...
static s32 xmitframe_direct(PADAPTER padapter,
			    struct xmit_frame *pxmitframe)
{
	s32 res = _SUCCESS;
...
s32 rtl8822bu_hal_xmitframe_enqueue(PADAPTER padapter,
				    struct xmit_frame *pxmitframe)
{
	struct xmit_priv *pxmitpriv = &padapter->xmitpriv;
	s32 err;
