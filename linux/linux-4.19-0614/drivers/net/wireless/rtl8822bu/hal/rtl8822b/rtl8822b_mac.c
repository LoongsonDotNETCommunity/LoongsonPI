...
inline u8 rtl8822b_rcr_config(PADAPTER p, u32 rcr)
{
	...
	if (v32) {
		...
		if (v32) {
			err = rtw_halmac_config_rx_info(adapter_to_dvobj(p),
...
#ifdef CONFIG_WOWLAN
		if (_TRUE == wowlan)
			err = rtw_halmac_dlfw(d, array_mp_8822b_fw_wowlan,
					      array_length_mp_8822b_fw_wowlan);
		else
#endif /* CONFIG_WOWLAN */
			err = rtw_halmac_dlfw(d, array_mp_8822b_fw_nic,
