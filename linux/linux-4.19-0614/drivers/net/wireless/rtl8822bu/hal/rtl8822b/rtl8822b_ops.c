...
static void read_chip_version(PADAPTER adapter)
{
	...
	hal->version_id.ChipType = ((value32 & BIT_RTL_ID_8822B) ? TEST_CHIP :
				    NORMAL_CHIP);
	...
	hal->version_id.RFType = ((value32 & BIT_RF_TYPE_ID_8822B) ? RF_TYPE_2T2R :
				  RF_TYPE_1T1R);
	if (adapter->registrypriv.special_rf_path == 1)
		hal->version_id.RFType = RF_TYPE_1T1R;	/* RF_1T1R; */

	hal->RegulatorMode = ((value32 & BIT_SPSLDO_SEL_8822B) ? RT_LDO_REGULATOR :
	...
	hal->MultiFunc |= ((value32 & BIT_WL_FUNC_EN_8822B) ? RT_MULTI_FUNC_WIFI :
			   0);
	hal->MultiFunc |= ((value32 & BIT_BT_FUNC_EN_8822B) ? RT_MULTI_FUNC_BT :
			   0);
	hal->PolarityCtl = ((value32 & BIT_WL_HWPDN_SL_8822B) ?
...
static void Hal_EfuseParseTxPowerInfo(PADAPTER adapter, u8 *map,
				      u8 mapvalid)
{
	PHAL_DATA_TYPE hal = GET_HAL_DATA(adapter);
	...
	if ((_TRUE == mapvalid) && (map[EEPROM_RF_BOARD_OPTION_8822B] != 0xFF))
		hal->EEPROMRegulatory = map[EEPROM_RF_BOARD_OPTION_8822B] &
					0x7; /* bit0~2 */
	else
...
static void Hal_EfuseParseBTCoexistInfo(PADAPTER adapter, u8 *map,
					u8 mapvalid)
{
	PHAL_DATA_TYPE hal = GET_HAL_DATA(adapter);
	u8 setting;
...
static void Hal_EfuseParseChnlPlan(PADAPTER adapter, u8 *map,
				   u8 autoloadfail)
{
...
static void Hal_EfuseParseThermalMeter(PADAPTER adapter, u8 *map,
				       u8 mapvalid)
{
	PHAL_DATA_TYPE hal = GET_HAL_DATA(adapter);
...
static void Hal_EfuseParseAntennaDiversity(PADAPTER adapter, u8 *map,
		u8 mapvalid)
{
...
static void Hal_EfuseParseCustomerID(PADAPTER adapter, u8 *map,
				     u8 mapvalid)
{
	PHAL_DATA_TYPE hal = GET_HAL_DATA(adapter);
...
static void hal_ReadPAType(PADAPTER adapter, u8 *map, u8 mapvalid)
{
	...
	if (mapvalid) {
		...
		if (GetRegAmplifierType2G(adapter) == 0) {
			...
			hal->ExternalPA_2G  = (GetRegAmplifierType2G(adapter) & ODM_BOARD_EXT_PA)
					      ? 1 : 0;
			hal->ExternalLNA_2G = (GetRegAmplifierType2G(adapter) & ODM_BOARD_EXT_LNA)
					      ? 1 : 0;
		}
		...
		if (GetRegAmplifierType5G(adapter) == 0) {
			...
			hal->external_pa_5g  = (GetRegAmplifierType5G(adapter) &
						ODM_BOARD_EXT_PA_5G)  ? 1 : 0;
			hal->external_lna_5g = (GetRegAmplifierType5G(adapter) &
...
{
	PHAL_DATA_TYPE hal = GET_HAL_DATA(adapter);
	u8 extTypePA_2G_A = (map[EEPROM_2G_LNA_TYPE_GAIN_SEL_AB_8822B] & BIT2) >>
			    2;
	u8 extTypePA_2G_B = (map[EEPROM_2G_LNA_TYPE_GAIN_SEL_AB_8822B] & BIT6) >>
			    6;
	u8 extTypePA_5G_A = (map[EEPROM_5G_LNA_TYPE_GAIN_SEL_AB_8822B] & BIT2) >>
			    2;
	u8 extTypePA_5G_B = (map[EEPROM_5G_LNA_TYPE_GAIN_SEL_AB_8822B] & BIT6) >>
			    6;
	u8 extTypeLNA_2G_A = (map[EEPROM_2G_LNA_TYPE_GAIN_SEL_AB_8822B] &
			      (BIT1 | BIT0)) >> 0;
	u8 extTypeLNA_2G_B = (map[EEPROM_2G_LNA_TYPE_GAIN_SEL_AB_8822B] &
			      (BIT5 | BIT4)) >> 4;
	u8 extTypeLNA_5G_A = (map[EEPROM_5G_LNA_TYPE_GAIN_SEL_AB_8822B] &
			      (BIT1 | BIT0)) >> 0;
	u8 extTypeLNA_5G_B = (map[EEPROM_5G_LNA_TYPE_GAIN_SEL_AB_8822B] &
			      (BIT5 | BIT4)) >> 4;
...
static void Hal_EfuseParsePackageType(PADAPTER adapter, u8 *map,
				      u8 mapvalid)
{
}
...
u8 rtl8822b_read_efuse(PADAPTER adapter)
{
	...
	hal->bautoload_fail_flag = (val8 & BIT_AUTOLOAD_SUS_8822B) ? _FALSE :
				   _TRUE;
	/*
...
static void InitBeaconParameters(PADAPTER adapter)
{
	...
	rtw_write8(adapter, REG_TBTT_PROHIBIT_8822B + 2,
		   (rtw_read8(adapter, REG_TBTT_PROHIBIT_8822B + 2) & 0xF0) |
		   (TBTT_PROHIBIT_HOLD_TIME_STOP_BCN >> 8));

	rtw_write8(adapter, REG_DRVERLYINT_8822B,
		   DRIVER_EARLY_INT_TIME_8822B); /* 5ms */
	rtw_write8(adapter, REG_BCNDMATIM_8822B,
		   BCN_DMA_ATIME_INT_TIME_8822B); /* 2ms */
...
static void xmit_status_check(PADAPTER p)
{
	...
	if (0 == pxmitpriv->free_xmitbuf_cnt
	    || 0 == pxmitpriv->free_xmit_extbuf_cnt) {
...
static void set_opmode_monitor(PADAPTER adapter)
{
	...
	rtw_hal_get_hwreg(adapter, HW_VAR_RCR,
			  (u8 *)&GET_HAL_DATA(adapter)->rcr_backup);
...
static void hw_port0_tsf_sync_sel(_adapter *adapter, u8 hw_port,
				  u8 benable, u16 tr_offset)
{
...
static void set_opmode_port0(PADAPTER adapter, u8 mode)
{
	...
	case _HW_STATE_AP_:
		...
		val8 = rtw_read8(adapter, REG_BCN_CTRL_8822B);
		val8 |= BIT_EN_BCN_FUNCTION_8822B | BIT_DIS_TSF_UDT_8822B |
			BIT_P0_EN_TXBCN_RPT_8822B;
		...
		*/
		...
		rtw_write16(adapter, REG_TSFTR_SYN_OFFSET_8822B,
			    0x7fff); /* +32767 (~32ms) */
...
#ifdef CONFIG_CONCURRENT_MODE
		{
			...
			for (i = 0; i < dvobj->iface_nums; i++) {
				...
				if (check_fwstate(&iface->mlmepriv,
					...
					hw_port0_tsf_sync_sel(adapter, iface->hw_port, _TRUE,
							      50);/*the offset = 50ms.*/
					break;
				}
			}
	...
	if (((pmlmeinfo->state & 0x03) == WIFI_FW_STATION_STATE)
		...
		for (i = 0; i < dvobj->iface_nums; i++) {
			...
			if ((MLME_IS_AP(iface) || MLME_IS_MESH(iface))
				...
				hw_port0_tsf_sync_sel(iface, adapter->hw_port, _TRUE,
						      50);/* the offset = 50ms.*/
		}
...
static void hw_var_set_mlme_join(PADAPTER adapter, u8 type)
{
	...
	if (type == 0) {
		...
		rtl8822b_rx_tsf_addr_filter_config(adapter,
						   BIT_CHK_TSF_EN_8822B | BIT_CHK_TSF_CBSSID_8822B);
	} else if (type == 1) {
		...
		if (rtw_mi_get_ap_num(adapter) || rtw_mi_get_mesh_num(adapter)) {
			...
			/* reset TSF 1/2 after resume_tx_beacon */
			rtw_write8(adapter, REG_DUAL_TSF_RST_8822B,
				   BIT_TSFTR_RST_8822B | BIT_TSFTR_CLI0_RST_8822B);
		}
	}
...
static void hw_var_set_sec_dk_cfg(PADAPTER adapter, u8 enable)
{
	...
	if (enable) {
		...
		reg_scr &= ~(BIT_RXBCUSEDK_8822B | BIT_TXBCUSEDK_8822B |
			     BIT_RXUHUSEDK_8822B | BIT_TXUHUSEDK_8822B);
	}
...
void hw_var_set_dl_rsvd_page(PADAPTER adapter, u8 mstatus)
{
	...
	if (mstatus == RT_MEDIA_CONNECT) {
		...
		rtw_write16(adapter, port_cfg[get_hw_port(adapter)].ps_aid,
			    (0xF800 | pmlmeinfo->aid));
...
static void hw_var_ap_port_switch(_adapter *adapter, u8 mode)
{
	...
	if (!rtw_p2p_chk_state(&adapter->wdinfo, P2P_STATE_NONE)) {
		RTW_INFO("%s, role=%d, p2p_state=%d, pre_p2p_state=%d\n", __func__,
			 rtw_p2p_role(&adapter->wdinfo), rtw_p2p_state(&adapter->wdinfo),
			 rtw_p2p_pre_state(&adapter->wdinfo));
	}
#endif
#endif
...
#define HAL_NAV_UPPER_UNIT	128	/* micro-second */
		...
		if (usNavUpper > HAL_NAV_UPPER_UNIT * 0xFF) {
			RTW_INFO(FUNC_ADPT_FMT
				 ": [HW_VAR_NAV_UPPER] value(0x%08X us) is larger than (%d * 0xFF)!!!\n",
...
static void dump_qinfo(void *sel, struct qinfo *info, u32 pkt_num,
		       const char *tag)
{
	RTW_PRINT_SEL(sel,
...
static void dump_bcn_qinfo(void *sel, struct bcn_qinfo *info, u32 pkt_num,
			   const char *tag)
{
...
static void dump_mac_qinfo(void *sel, _adapter *adapter)
{
	...
	dump_qinfo(sel, (struct qinfo *)&q1_info, (q0_q1_info >> 15) & 0xFFF,
		   "Q1 ");
	dump_qinfo(sel, (struct qinfo *)&q2_info, q2_q3_info & 0xFFF, "Q2 ");
	dump_qinfo(sel, (struct qinfo *)&q3_info, (q2_q3_info >> 15) & 0xFFF,
		   "Q3 ");
	dump_qinfo(sel, (struct qinfo *)&q4_info, q4_q5_info & 0xFFF, "Q4 ");
	dump_qinfo(sel, (struct qinfo *)&q5_info, (q4_q5_info >> 15) & 0xFFF,
		   "Q5 ");
	dump_qinfo(sel, (struct qinfo *)&q6_info, q6_q7_info & 0xFFF, "Q6 ");
	dump_qinfo(sel, (struct qinfo *)&q7_info, (q6_q7_info >> 15) & 0xFFF,
		   "Q7 ");
	dump_qinfo(sel, (struct qinfo *)&mg_q_info, mg_hi_q_info & 0xFFF, "MG ");
	dump_qinfo(sel, (struct qinfo *)&hi_q_info, (mg_hi_q_info >> 15) & 0xFFF,
		   "HI ");
	dump_bcn_qinfo(sel, (struct bcn_qinfo *)&bcn_q_info,
		       cmd_bcn_q_info & 0xFFF, "BCN ");
...
void rtl8822b_read_wmmedca_reg(PADAPTER adapter, u16 *vo_params,
 ...
 */
u8 rtl8822b_sethaldefvar(PADAPTER adapter, HAL_DEF_VARIABLE variable,
			 void *pval)
{
	PHAL_DATA_TYPE hal;
	u8 bResult;
...
void rtl8822b_ra_info_dump(_adapter *padapter, void *sel)
{
	...
	for (mac_id = 0; mac_id < macid_ctl->num; mac_id++) {
		if (rtw_macid_is_used(macid_ctl, mac_id)
		    && !rtw_macid_is_bmc(macid_ctl, mac_id)) {
 ...
 */
u8 rtl8822b_gethaldefvar(PADAPTER adapter, HAL_DEF_VARIABLE variable,
			 void *pval)
{
	PHAL_DATA_TYPE hal;
	struct dvobj_priv *d;
...
void rtl8822b_fill_txdesc_vcs(PADAPTER adapter, struct pkt_attrib *pattrib,
			      u8 *ptxdesc)
{
	struct mlme_ext_priv *pmlmeext = &adapter->mlmeextpriv;
	struct mlme_ext_info *pmlmeinfo = &(pmlmeext->mlmext_info);
...
u8 rtl8822b_sc_mapping(PADAPTER adapter, struct pkt_attrib *pattrib)
{
	...
	if (hal->current_channel_bw == CHANNEL_WIDTH_80) {
		if (pattrib->bwmode == CHANNEL_WIDTH_80)
			...
			if ((hal->nCur40MhzPrimeSC == HAL_PRIME_CHNL_OFFSET_LOWER)
			    && (hal->nCur80MhzPrimeSC == HAL_PRIME_CHNL_OFFSET_LOWER))
				SCSettingOfDesc = VHT_DATA_SC_20_LOWEST_OF_80MHZ;
			else if ((hal->nCur40MhzPrimeSC == HAL_PRIME_CHNL_OFFSET_UPPER)
				 && (hal->nCur80MhzPrimeSC == HAL_PRIME_CHNL_OFFSET_LOWER))
				SCSettingOfDesc = VHT_DATA_SC_20_LOWER_OF_80MHZ;
			else if ((hal->nCur40MhzPrimeSC == HAL_PRIME_CHNL_OFFSET_LOWER)
				 && (hal->nCur80MhzPrimeSC == HAL_PRIME_CHNL_OFFSET_UPPER))
...
void rtl8822b_fill_txdesc_phy(PADAPTER adapter, struct pkt_attrib *pattrib,
			      u8 *ptxdesc)
{
	if (pattrib->ht_en) {
...
#ifdef CONFIG_CONCURRENT_MODE
void rtl8822b_fill_txdesc_force_bmc_camid(struct pkt_attrib *pattrib,
		u8 *ptxdesc)
{
	if ((pattrib->encrypt > 0) && (!pattrib->bswenc)
...
void rtl8822b_fill_txdesc_bmc_tx_rate(struct pkt_attrib *pattrib,
				      u8 *ptxdesc)
{
...
static void fill_default_txdesc(struct xmit_frame *pxmitframe, u8 *pbuf)
{
	...
	if (pxmitframe->frame_tag == DATA_FRAMETAG) {
		...
		if ((pattrib->ether_type != 0x888e) &&
			...
			RTW_INFO(FUNC_ADPT_FMT ": SP Packet(0x%04X) rate=0x%x\n",
				 FUNC_ADPT_ARG(adapter), pattrib->ether_type,
		...
		if (pxmitframe->ack_report) {
			...
			SET_TX_DESC_SW_DEFINE_8822B(pbuf,
						    (u8)(GET_PRIMARY_ADAPTER(adapter)->xmitpriv.seq_no));
		}
	...
	if (!bmcst && pattrib->psta)
		odm_set_tx_ant_by_tx_info(adapter_to_phydm(adapter), pbuf,
...
void rtl8822b_dbg_dump_tx_desc(PADAPTER adapter, int frame_tag,
			       u8 *ptxdesc)
{
	u8 bDumpTxPkt;
