...
static inline u8 is_valid_id_status(enum halmac_feature_id id,
...
static int init_halmac_event_with_waittime(struct dvobj_priv *d,
		enum halmac_feature_id id, u8 *buf, u32 size, u32 time)
{
...
static inline int init_halmac_event(struct dvobj_priv *d,
				    enum halmac_feature_id id, u8 *buf, u32 size)
{
	return init_halmac_event_with_waittime(d, id, buf, size,
					       DEFAULT_INDICATOR_TIMELMT);
}

static void free_halmac_event(struct dvobj_priv *d,
...
static int wait_halmac_event(struct dvobj_priv *d,
			     enum halmac_feature_id id)
{
 ...
 */
static u8 _halmac_event_indication(void *p,
				   enum halmac_feature_id feature_id,
				   enum halmac_cmd_process_status process_status, u8 *buf, u32 size)
{
	struct dvobj_priv *d;
	PADAPTER adapter;
	PHAL_DATA_TYPE hal;
...
int rtw_halmac_init_adapter(struct dvobj_priv *d,
			    struct halmac_platform_api *pf_api)
{
...
int rtw_halmac_get_hw_value(struct dvobj_priv *d, enum halmac_hw_id hw_id,
			    void *pvalue)
{
	struct halmac_adapter *mac;
	struct halmac_api *api;
 ...
 */
int rtw_halmac_config_rx_info(struct dvobj_priv *d,
...
static inline enum halmac_rx_fifo_expanding_mode
_trx_share_mode_drv2halmac(u8 trx_share_mode)
{
	if (0 == trx_share_mode)
...
static enum halmac_rx_fifo_expanding_mode _rtw_get_trx_share_mode(
...
static u8 _rsvd_page_num_halmac2drv(enum halmac_drv_rsvd_pg_num
				    rsvd_page_number)
{
	u8 num = 0;
...
int rtw_halmac_p2pps(struct dvobj_priv *dvobj,
		     struct hal_p2p_ps_para *pp2p_ps_para)
{
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;
	struct halmac_adapter *halmac = dvobj_to_halmac(dvobj);
	struct halmac_api *api = HALMAC_GET_API(halmac);
...
static inline u32 _phy_parameter_val_drv2halmac(u32 val, u8 msk_en,
		u32 msk)
{
	if (!msk_en)
		return val;
...
static inline enum halmac_usb_mode _usb_mode_drv2halmac(
...
int rtw_halmac_bf_cfg_mu_mimo(struct dvobj_priv *d,
			      enum halmac_snd_role role,
