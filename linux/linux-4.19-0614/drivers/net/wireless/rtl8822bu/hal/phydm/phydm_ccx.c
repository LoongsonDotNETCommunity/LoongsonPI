...
)
{
	...
	if (dm->support_ic_type & ODM_IC_11AC_SERIES) {
		...
		odm_set_bb_reg(dm, R_0x1c38, 0xffffff00,
			       ((fahm_th[2] << 24) | (fahm_th[1] << 16) | (fahm_th[0] << 8)));
		odm_set_bb_reg(dm, R_0x1c78, 0xffffff00,
			       ((fahm_th[5] << 24) | (fahm_th[4] << 16) | (fahm_th[3] << 8)));
		odm_set_bb_reg(dm, R_0x1c7c, 0xffffff00,
			       ((fahm_th[7] << 24) | (fahm_th[6] << 16)));
		odm_set_bb_reg(dm, R_0x1cb8, 0xffffff00,
			       ((fahm_th[10] << 24) | (fahm_th[9] << 16) | (fahm_th[8] << 8)));
	} else {
		odm_set_bb_reg(dm, R_0x970, MASKDWORD,
			       ((fahm_th[3] << 24) | (fahm_th[2] << 16) | (fahm_th[1] << 8) |
				fahm_th[0]));
		odm_set_bb_reg(dm, R_0x974, MASKDWORD,
			       ((fahm_th[7] << 24) | (fahm_th[6] << 16) | (fahm_th[5] << 8) |
				fahm_th[4]));
		odm_set_bb_reg(dm, R_0x978, MASKDWORD,
...
)
{
	...
	if (!((ccx->nhm_app == NHM_BACKGROUND) || (ccx->nhm_app == NHM_ACS)))
		phydm_pause_func(dm, F00_DIG, PHYDM_RESUME, PHYDM_PAUSE_LEVEL_1, 1,
				 &value32);

	ccx->nhm_app = NHM_BACKGROUND;
}

u8 phydm_nhm_racing_ctrl(
	void			*dm_void,
...
)
{
	...
	if (ccx->nhm_rpt_sum >= ccx->nhm_result[0])
		ccx->nhm_ratio = (u8)(((ccx->nhm_rpt_sum - ccx->nhm_result[0]) * 100) >>
				      8);
	else {
...
)
{
	...
	pdm_set_reg(dm, reg2, MASKDWORD, BYTE_2_DWORD(ccx->nhm_th[3],
			ccx->nhm_th[2], ccx->nhm_th[1], ccx->nhm_th[0]));
	pdm_set_reg(dm, reg3, MASKDWORD, BYTE_2_DWORD(ccx->nhm_th[7],
			ccx->nhm_th[6], ccx->nhm_th[5], ccx->nhm_th[4]));
	pdm_set_reg(dm, reg4, MASKBYTE0, ccx->nhm_th[8]);
	pdm_set_reg(dm, reg1, 0xffff0000, BYTE_2_DWORD(0, 0, ccx->nhm_th[10],
			ccx->nhm_th[9]));
...
boolean phydm_nhm_th_update_chk(
	void		*dm_void,
...
)
{
	...
	if (phydm_nhm_th_update_chk(dm, nhm_app, &(nhm_th[0]), &igi)) {
		...
		} else if (phydm_pause_func(dm, F00_DIG, PHYDM_PAUSE, PHYDM_PAUSE_LEVEL_1,
					    1, &igi) == PAUSE_FAIL) {
	...
	if (dm->support_ic_type & ODM_IC_11AC_SERIES) {
		if (nhm_setting == SET_NHM_SETTING) {
			...
			odm_set_bb_reg(dm, ODM_REG_NHM_TH9_TH10_11AC, BIT(9),
				       ccx_info->nhm_include_cca);
			odm_set_bb_reg(dm, ODM_REG_NHM_TH9_TH10_11AC, BIT(10),
				       ccx_info->nhm_include_txon);
			odm_set_bb_reg(dm, ODM_REG_NHM_TH9_TH10_11AC, BIT(11),
				       ccx_info->nhm_divider_opt);
			...
			/*Set NHM threshold*/ /*Unit: PWdB U(8,1)*/
			odm_set_bb_reg(dm, ODM_REG_NHM_TH3_TO_TH0_11AC, MASKBYTE0,
				       ccx_info->nhm_th[0]);
			odm_set_bb_reg(dm, ODM_REG_NHM_TH3_TO_TH0_11AC, MASKBYTE1,
				       ccx_info->nhm_th[1]);
			odm_set_bb_reg(dm, ODM_REG_NHM_TH3_TO_TH0_11AC, MASKBYTE2,
				       ccx_info->nhm_th[2]);
			odm_set_bb_reg(dm, ODM_REG_NHM_TH3_TO_TH0_11AC, MASKBYTE3,
				       ccx_info->nhm_th[3]);
			odm_set_bb_reg(dm, ODM_REG_NHM_TH7_TO_TH4_11AC, MASKBYTE0,
				       ccx_info->nhm_th[4]);
			odm_set_bb_reg(dm, ODM_REG_NHM_TH7_TO_TH4_11AC, MASKBYTE1,
				       ccx_info->nhm_th[5]);
			odm_set_bb_reg(dm, ODM_REG_NHM_TH7_TO_TH4_11AC, MASKBYTE2,
				       ccx_info->nhm_th[6]);
			odm_set_bb_reg(dm, ODM_REG_NHM_TH7_TO_TH4_11AC, MASKBYTE3,
				       ccx_info->nhm_th[7]);
			...
			ccx_info->nhm_inexclude_cca_restore = (enum nhm_inexclude_cca_all)
							      odm_get_bb_reg(dm, ODM_REG_NHM_TH9_TH10_11AC, BIT(9));
			ccx_info->nhm_inexclude_txon_restore = (enum nhm_inexclude_txon_all)
							       odm_get_bb_reg(dm, ODM_REG_NHM_TH9_TH10_11AC, BIT(10));

			/*Store pervious NHM period*/
			ccx_info->nhm_period_restore = (u16)odm_get_bb_reg(dm,
						       ODM_REG_CCX_PERIOD_11AC, MASKHWORD);

			/*Store NHM threshold*/
			ccx_info->nhm_th_restore[0] = (u8)odm_get_bb_reg(dm,
						      ODM_REG_NHM_TH3_TO_TH0_11AC, MASKBYTE0);
			ccx_info->nhm_th_restore[1] = (u8)odm_get_bb_reg(dm,
						      ODM_REG_NHM_TH3_TO_TH0_11AC, MASKBYTE1);
			ccx_info->nhm_th_restore[2] = (u8)odm_get_bb_reg(dm,
						      ODM_REG_NHM_TH3_TO_TH0_11AC, MASKBYTE2);
			ccx_info->nhm_th_restore[3] = (u8)odm_get_bb_reg(dm,
						      ODM_REG_NHM_TH3_TO_TH0_11AC, MASKBYTE3);
			ccx_info->nhm_th_restore[4] = (u8)odm_get_bb_reg(dm,
						      ODM_REG_NHM_TH7_TO_TH4_11AC, MASKBYTE0);
			ccx_info->nhm_th_restore[5] = (u8)odm_get_bb_reg(dm,
						      ODM_REG_NHM_TH7_TO_TH4_11AC, MASKBYTE1);
			ccx_info->nhm_th_restore[6] = (u8)odm_get_bb_reg(dm,
						      ODM_REG_NHM_TH7_TO_TH4_11AC, MASKBYTE2);
			ccx_info->nhm_th_restore[7] = (u8)odm_get_bb_reg(dm,
						      ODM_REG_NHM_TH7_TO_TH4_11AC, MASKBYTE3);
			ccx_info->nhm_th_restore[8] = (u8)odm_get_bb_reg(dm, ODM_REG_NHM_TH8_11AC,
						      MASKBYTE0);
			ccx_info->nhm_th_restore[9] = (u8)odm_get_bb_reg(dm,
						      ODM_REG_NHM_TH9_TH10_11AC, MASKBYTE2);
			ccx_info->nhm_th_restore[10] = (u8)odm_get_bb_reg(dm,
		...
		if (nhm_setting == SET_NHM_SETTING) {
			...
			odm_set_bb_reg(dm, ODM_REG_NHM_TH9_TH10_11N, BIT(10),
				       ccx_info->nhm_include_txon);
			odm_set_bb_reg(dm, ODM_REG_NHM_TH9_TH10_11N, BIT(11),
				       ccx_info->nhm_divider_opt);
			...
			/*Set NHM threshold*/
			odm_set_bb_reg(dm, ODM_REG_NHM_TH3_TO_TH0_11N, MASKBYTE0,
				       ccx_info->nhm_th[0]);
			odm_set_bb_reg(dm, ODM_REG_NHM_TH3_TO_TH0_11N, MASKBYTE1,
				       ccx_info->nhm_th[1]);
			odm_set_bb_reg(dm, ODM_REG_NHM_TH3_TO_TH0_11N, MASKBYTE2,
				       ccx_info->nhm_th[2]);
			odm_set_bb_reg(dm, ODM_REG_NHM_TH3_TO_TH0_11N, MASKBYTE3,
				       ccx_info->nhm_th[3]);
			odm_set_bb_reg(dm, ODM_REG_NHM_TH7_TO_TH4_11N, MASKBYTE0,
				       ccx_info->nhm_th[4]);
			odm_set_bb_reg(dm, ODM_REG_NHM_TH7_TO_TH4_11N, MASKBYTE1,
				       ccx_info->nhm_th[5]);
			odm_set_bb_reg(dm, ODM_REG_NHM_TH7_TO_TH4_11N, MASKBYTE2,
				       ccx_info->nhm_th[6]);
			odm_set_bb_reg(dm, ODM_REG_NHM_TH7_TO_TH4_11N, MASKBYTE3,
				       ccx_info->nhm_th[7]);
			...
			/*Store pervious disable_ignore_cca, disable_ignore_txon*/
			ccx_info->nhm_inexclude_cca_restore = (enum nhm_inexclude_cca_all)
							      odm_get_bb_reg(dm, ODM_REG_NHM_TH9_TH10_11N, BIT(9));
			ccx_info->nhm_inexclude_txon_restore = (enum nhm_inexclude_txon_all)
							       odm_get_bb_reg(dm, ODM_REG_NHM_TH9_TH10_11N, BIT(10));
			...
			ccx_info->nhm_th_restore[8] = (u8)odm_get_bb_reg(dm, ODM_REG_NHM_TH8_11N,
						      MASKBYTE0);
			ccx_info->nhm_th_restore[9] = (u8)odm_get_bb_reg(dm,
...
)
{
	...
	if (phydm_check_nhm_rdy(dm)) {
		...
		if (ccx_info->nhm_rpt_sum != 0)
			ccx_info->nhm_ratio  = (u8)(((ccx_info->nhm_rpt_sum -
...
)
{
	...
	PHYDM_DBG(dm, DBG_ENV_MNTR,
		  ...
		  ccx->nhm_igi, rpt->nhm_ratio, rpt->clm_ratio, rpt->nhm_rpt_stamp,
		  rpt->clm_rpt_stamp);
...
)
{
	...
	if ((strcmp(input[1], help) == 0)) {
		...
		PDM_SNPF(out_len, used, output + used, out_len - used,
			 ...
			 set_result, rpt.nhm_ratio, rpt.clm_ratio, rpt.nhm_rpt_stamp,
			 rpt.clm_rpt_stamp);
