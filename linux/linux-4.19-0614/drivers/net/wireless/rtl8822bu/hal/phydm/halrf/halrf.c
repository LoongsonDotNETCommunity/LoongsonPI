...
void halrf_get_fw_version(void	*dm_void)
{
...
#if (DM_ODM_SUPPORT_TYPE & ODM_WIN)
	{
		...
		rf->fw_ver = (((PADAPTER)adapter)->MgntInfo.FirmwareVersion << 16) | ((
...
#elif (DM_ODM_SUPPORT_TYPE & ODM_AP)
	{
		...
		rf->fw_ver = (priv->pshare->fw_version << 16) |
...
#else
	{
		...
		rf->fw_ver = (hal_data->firmware_version << 16) |
		...
		for (idx = 0; idx < 2; idx++) {
			...
			for (i = 0; i < 8; i++) {
				odm_write_4byte(dm, 0x1bd8,
						...
						(iqk_info->iqk_cfir_real[2][path][idx][i] << 9));
				odm_write_4byte(dm, 0x1bd8,
						((0xc0000000 >> idx) + 0x1) + (i * 4) +
						(iqk_info->iqk_cfir_imag[2][path][idx][i] << 9));
	...
	for (path = 0; path < 2; path++) {
		for (idx = 0; idx < 2; idx++) {
			...
			for (i = 0; i < 8; i++) {
				...
				odm_write_4byte(dm, 0x1bd8,
						((0xc0000000 >> idx) + 0x3) + (i * 4) +
						(iqk_info->iqk_cfir_real[0][path][idx][i] << 9));
				odm_write_4byte(dm, 0x1bd8,
						((0xc0000000 >> idx) + 0x1) + (i * 4) +
						(iqk_info->iqk_cfir_imag[0][path][idx][i] << 9));
			}
		}
		odm_set_bb_reg(dm, 0x1bd8, MASKDWORD, 0x0);
		odm_set_bb_reg(dm, 0x1b0c, BIT(13) | BIT(12), 0x0);
...
)
{
	...
	if (dm_value[0] == 0x0)
		...
		halrf_iqk_dbg_cfir_write(dm, 0, dm_value[1], dm_value[2], dm_value[3],
					 dm_value[4]);
	else if (dm_value[0] == 0x11)
		halrf_iqk_dbg_cfir_write(dm, 1, dm_value[1], dm_value[2], dm_value[3],
					 dm_value[4]);
...
)
{
	...
	if ((dm->mp_mode != NULL) && (rf->is_con_tx != NULL)
	    && (rf->is_single_tone != NULL) && (rf->is_carrier_suppresion != NULL))
...
)
{
	...
	if (!dm->rf_calibrate_info.is_lck_in_progress) {
		...
		dm->rf_calibrate_info.lck_progressing_time = odm_get_progressing_time(dm,
				start_time);
		PHYDM_DBG(dm, ODM_COMP_CALIBRATION,
