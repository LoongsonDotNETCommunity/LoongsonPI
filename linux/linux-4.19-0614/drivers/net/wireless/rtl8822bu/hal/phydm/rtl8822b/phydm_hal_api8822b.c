...
)
{
	...
	u32		debug_level =
		dm->debug_level;/*no use, just prevent FW 3081 compile warning*/

	debug_level = 5; /*no use, just prevent FW 3081 compile warning*/

	PHYDM_DBG(dm, ODM_PHY_CONFIG,
		  "8822B RFE[%d]:{Path=0x%x}{inv_en=%d}{source=0x%x}\n",
	...
	/*[Path_mux_sel]*/
	odm_set_bb_reg(dm, 0x1990, BIT(rfe_num),
		       ((path_mux_sel == BB_PATH_A) ? 0 : 1));
...
__iram_odm_func__
void phydm_ccapar_by_rfe_8822b(
	struct dm_struct				*dm
)
{
...
#endif
	{
		...
		if ((dm->rfe_type == 3) || (dm->rfe_type == 5) || (dm->rfe_type == 12)
		    || (dm->rfe_type == 15) || (dm->rfe_type == 16) || (dm->rfe_type == 17)) {
...
__iram_odm_func__
void phydm_rxdfirpar_by_bw_8822b(
	struct dm_struct				*dm,
...
__iram_odm_func__
boolean phydm_write_txagc_1byte_8822b(
	struct dm_struct				*dm,
	u32					power_index,
	enum rf_path		path,
	u8					hw_rate
)
{
	...
	if ((path > RF_PATH_B) || (hw_rate > 0x53)) {
		PHYDM_DBG(dm, ODM_PHY_CONFIG,
			  "phydm_write_txagc_1byte_8822b(): unsupported path (%d)\n", path);
		return false;
	}
	...
	for (i = 0; i < 4; i++) {
		if (i != rate_offset)
			txagc_content = txagc_content | (config_phydm_read_txagc_8822b(dm, path,
							 rate_idx + i) << (i << 3));
		else
	...
	PHYDM_DBG(dm, ODM_PHY_CONFIG,
		  "phydm_write_txagc_1byte_8822b(): path-%d rate index 0x%x (0x%x) = 0x%x\n",
...
__iram_odm_func__
void phydm_init_hw_info_by_rfe_type_8822b(
	struct dm_struct				*dm
)
{
#if (PHYDM_FW_API_FUNC_ENABLE_8822B == 1)
	...
	if ((dm->rfe_type == 1) || (dm->rfe_type == 6) || (dm->rfe_type == 7)) {
		odm_cmn_info_init(dm, ODM_CMNINFO_BOARD_TYPE,
				  (ODM_BOARD_EXT_LNA | ODM_BOARD_EXT_LNA_5G | ODM_BOARD_EXT_PA |
				   ODM_BOARD_EXT_PA_5G));
	...
	PHYDM_DBG(dm, ODM_PHY_CONFIG,
		  ...
		  dm->rfe_type, dm->board_type, dm->package_type);
	PHYDM_DBG(dm, ODM_PHY_CONFIG,
		  "%s: 5G ePA (%d), 5G eLNA (%d), 2G ePA (%d), 2G eLNA (%d)\n", __func__,
		  dm->ext_pa_5g, dm->ext_lna_5g, dm->ext_pa, dm->ext_lna);
	PHYDM_DBG(dm, ODM_PHY_CONFIG,
...
__iram_odm_func__
s32 phydm_get_condition_number_8822B(
	struct dm_struct				*dm
)
{
	s32	ret_val;
...
__iram_odm_func__
u32 config_phydm_read_rf_reg_8822b(
	struct dm_struct		*dm,
	enum rf_path		path,
	u32					reg_addr,
	u32					bit_mask
)
{
	...
	if (path > RF_PATH_B) {
		PHYDM_DBG(dm, ODM_PHY_CONFIG,
			  "config_phydm_read_rf_reg_8822b(): unsupported path (%d)\n", path);
	...
	PHYDM_DBG(dm, ODM_PHY_CONFIG,
		  "config_phydm_read_rf_reg_8822b(): RF-%d 0x%x = 0x%x, bit mask = 0x%x\n",
		  path, reg_addr, readback_value, bit_mask);
	return readback_value;
}

__iram_odm_func__
boolean config_phydm_write_rf_reg_8822b(
	struct dm_struct				*dm,
	enum rf_path		path,
	u32					reg_addr,
	u32					bit_mask,
...
)
{
	...
	if (path > RF_PATH_B) {
		PHYDM_DBG(dm, ODM_PHY_CONFIG,
			  "config_phydm_write_rf_reg_8822b(): unsupported path (%d)\n", path);
		return false;
	}
	...
	if (bit_mask != RFREGOFFSETMASK) {
		data_original = config_phydm_read_rf_reg_8822b(dm, path, reg_addr,
				RFREGOFFSETMASK);

		/* Error handling. RF is disabled */
		if (config_phydm_read_rf_check_8822b(data_original) == false) {
			PHYDM_DBG(dm, ODM_PHY_CONFIG,
				  "config_phydm_write_rf_reg_8822b(): Write fail, RF is disable\n");
			return false;
		}
	...
	PHYDM_DBG(dm, ODM_PHY_CONFIG,
		  "config_phydm_write_rf_reg_8822b(): RF-%d 0x%x = 0x%x (original: 0x%x), bit mask = 0x%x\n",
		  path, reg_addr, data, data_original, bit_mask);
	return true;
}

__iram_odm_func__
boolean config_phydm_write_txagc_8822b(
	struct dm_struct				*dm,
	u32					power_index,
	enum rf_path		path,
	u8					hw_rate
)
{
	...
	if (dm->is_disable_phy_api) {
		PHYDM_DBG(dm, ODM_PHY_CONFIG,
			  "config_phydm_write_txagc_8822b(): disable PHY API for debug!!\n");
		return true;
	}
	...
	if ((path > RF_PATH_B) || (hw_rate > 0x53)) {
		PHYDM_DBG(dm, ODM_PHY_CONFIG,
			  "config_phydm_write_txagc_8822b(): unsupported path (%d)\n", path);
		return false;
	}
	...
	PHYDM_DBG(dm, ODM_PHY_CONFIG,
		  "config_phydm_write_txagc_8822b(): path-%d rate index 0x%x (0x%x) = 0x%x\n",
		  path, hw_rate, (offset_txagc[path] + hw_rate), power_index);
	return true;
...
__iram_odm_func__
u8 config_phydm_read_txagc_8822b(
	struct dm_struct				*dm,
	enum rf_path		path,
	u8					hw_rate
)
{
	...
	if ((path > RF_PATH_B) || (hw_rate > 0x53)) {
		PHYDM_DBG(dm, ODM_PHY_CONFIG,
			  "config_phydm_read_txagc_8822b(): unsupported path (%d)\n", path);
	...
	PHYDM_DBG(dm, ODM_PHY_CONFIG,
		  "config_phydm_read_txagc_8822b(): path-%d rate index 0x%x = 0x%x\n", path,
...
__iram_odm_func__
void phydm_dynamic_spur_det_eliminate(
	struct dm_struct				*dm
)
{
#if (PHYDM_FW_API_FUNC_ENABLE_8822B == 1)
	...
	u32		freq_pt_2g_final = 0, freq_pt_5g_final = 0, freq_pt_2g_b_final = 0,
			freq_pt_5g_b_final = 0;
	...
	u32		rank_psd_index_in[PSD_VAL_NUM] = {0},
			rank_sample_index_in[PSD_SMP_NUM] = {0};
	...
	u8		idx = 0, set_result_nbi = PHYDM_SET_NO_NEED,
			set_result_csi = PHYDM_SET_NO_NEED;
	boolean	s_dopsd = false, s_donbi_a = false, s_docsi = false,
		s_donbi_b = false;
	...
	for (k = 0; k < PSD_SMP_NUM; k++) {
		...
		for (j = 0; j < PSD_VAL_NUM; j++) {
			...
			reg_910_15_12 = odm_get_bb_reg(dm, 0x910,
						       (BIT(15) | BIT(14) | BIT(13) | BIT(12)));

			if (dm->rx_ant_status & BB_PATH_A) {
				odm_set_bb_reg(dm, 0x808, MASKBYTE0,
					       (((BB_PATH_A) << 4) | BB_PATH_A)); /*path-A*/

				if ((*dm->channel >= 1) && (*dm->channel <= 14))
					odm_set_bb_reg(dm, 0x910, MASKDWORD,
						       BIT(22) | freq_pt_2g_final);/* Start PSD */
				else
					odm_set_bb_reg(dm, 0x910, MASKDWORD,
						       BIT(22) | freq_pt_5g_final);/* Start PSD */
			...
			if (dm->rx_ant_status & BB_PATH_B) {
				odm_set_bb_reg(dm, 0x808, MASKBYTE0,
					       (((BB_PATH_B) << 4) | BB_PATH_B)); /*path-B*/

				if ((*dm->channel > 0) && (*dm->channel <= 14))
					odm_set_bb_reg(dm, 0x910, MASKDWORD,
						       BIT(22) | freq_pt_2g_b_final);/* Start PSD */
				else
					odm_set_bb_reg(dm, 0x910, MASKDWORD,
						       BIT(22) | freq_pt_5g_b_final);/* Start PSD */
			...
			odm_set_bb_reg(dm, 0x910, (BIT(15) | BIT(14) | BIT(13) | BIT(12)),
				       reg_910_15_12);

			odm_set_bb_reg(dm, 0x808, MASKBYTE0,
				       (((dm->rx_ant_status) << 4) | dm->rx_ant_status));
		...
		if (dm->rx_ant_status & BB_PATH_A) {
			phydm_seq_sorting(dm, psd_set, rank_psd_index_in, rank_psd_index_out,
					  PSD_VAL_NUM);
			max_ret_psd_2nd[k] = psd_set[0];
		}
		if (dm->rx_ant_status & BB_PATH_B) {
			phydm_seq_sorting(dm, psd_set_B, rank_psd_index_in, rank_psd_index_out,
					  PSD_VAL_NUM);
	...
	if (s_donbi_a == true || s_donbi_b == true) {
		if (*dm->band_width == CHANNEL_WIDTH_20) {
			if (*dm->channel == 153)
				set_result_nbi = phydm_nbi_setting(dm, FUNC_ENABLE, *dm->channel, 20, 5760,
								   PHYDM_DONT_CARE);
			else if (*dm->channel == 161)
				set_result_nbi = phydm_nbi_setting(dm, FUNC_ENABLE, *dm->channel, 20, 5800,
								   PHYDM_DONT_CARE);
			else if (*dm->channel >= 5 && *dm->channel <= 8)
				set_result_nbi = phydm_nbi_setting(dm, FUNC_ENABLE, *dm->channel, 20, 2440,
								   PHYDM_DONT_CARE);
			else if (*dm->channel == 13)
				set_result_nbi = phydm_nbi_setting(dm, FUNC_ENABLE, *dm->channel, 20, 2480,
								   PHYDM_DONT_CARE);
			else
				set_result_nbi = PHYDM_SET_NO_NEED;
		} else if (*dm->band_width == CHANNEL_WIDTH_40) {
			if (*dm->channel == 54) {
				set_result_nbi = phydm_nbi_setting(dm, FUNC_ENABLE, *dm->channel, 40, 5280,
								   PHYDM_DONT_CARE);
			} else if (*dm->channel == 118) {
				set_result_nbi = phydm_nbi_setting(dm, FUNC_ENABLE, *dm->channel, 40, 5600,
								   PHYDM_DONT_CARE);
			} else if (*dm->channel == 151) {
				set_result_nbi = phydm_nbi_setting(dm, FUNC_ENABLE, *dm->channel, 40, 5760,
								   PHYDM_DONT_CARE);
			} else if (*dm->channel == 159) {
				set_result_nbi = phydm_nbi_setting(dm, FUNC_ENABLE, *dm->channel, 40, 5800,
								   PHYDM_DONT_CARE);
				/* 2.4G */
			} else if ((*dm->channel >= 4) && (*dm->channel <= 6)) {
				set_result_nbi = phydm_nbi_setting(dm, FUNC_ENABLE, *dm->channel, 40, 2440,
								   PHYDM_DONT_CARE);
			} else if (*dm->channel == 11) {
				set_result_nbi = phydm_nbi_setting(dm, FUNC_ENABLE, *dm->channel, 40, 2480,
								   PHYDM_DONT_CARE);
			} else
				set_result_nbi = PHYDM_SET_NO_NEED;
		} else if (*dm->band_width == CHANNEL_WIDTH_80) {
			if (*dm->channel == 58) {
				set_result_nbi = phydm_nbi_setting(dm, FUNC_ENABLE, *dm->channel, 80, 5280,
								   PHYDM_DONT_CARE);
			} else if (*dm->channel == 122) {
				set_result_nbi = phydm_nbi_setting(dm, FUNC_ENABLE, *dm->channel, 80, 5600,
								   PHYDM_DONT_CARE);
			} else if (*dm->channel == 155) {
				set_result_nbi = phydm_nbi_setting(dm, FUNC_ENABLE, *dm->channel, 80, 5760,
								   PHYDM_DONT_CARE);
			} else
				set_result_nbi = PHYDM_SET_NO_NEED;
		} else
	...
	if (s_docsi == true) {
		if (*dm->band_width == CHANNEL_WIDTH_20) {
			if (*dm->channel == 153)
				set_result_csi = phydm_csi_mask_setting(dm, FUNC_ENABLE, *dm->channel, 20,
									5760, PHYDM_DONT_CARE);
			else if (*dm->channel == 161)
				set_result_csi = phydm_csi_mask_setting(dm, FUNC_ENABLE, *dm->channel, 20,
									5800, PHYDM_DONT_CARE);
			else if (*dm->channel >= 5 && *dm->channel <= 8)
				set_result_csi = phydm_csi_mask_setting(dm, FUNC_ENABLE, *dm->channel, 20,
									2440, PHYDM_DONT_CARE);
			else if (*dm->channel == 13)
				set_result_csi = phydm_csi_mask_setting(dm, FUNC_ENABLE, *dm->channel, 20,
									2480, PHYDM_DONT_CARE);
			else
				set_result_csi = PHYDM_SET_NO_NEED;
		} else if (*dm->band_width == CHANNEL_WIDTH_40) {
			if (*dm->channel == 54)
				set_result_csi = phydm_csi_mask_setting(dm, FUNC_ENABLE, *dm->channel, 40,
									5280, PHYDM_DONT_CARE);
			else if (*dm->channel == 118)
				set_result_csi = phydm_csi_mask_setting(dm, FUNC_ENABLE, *dm->channel, 40,
									5600, PHYDM_DONT_CARE);
			else if (*dm->channel == 151)
				set_result_csi = phydm_csi_mask_setting(dm, FUNC_ENABLE, *dm->channel, 40,
									5760, PHYDM_DONT_CARE);
			else if (*dm->channel == 159)
			...
			if (*dm->channel == 58)
				set_result_csi = phydm_csi_mask_setting(dm, FUNC_ENABLE, *dm->channel, 80,
									5280, PHYDM_DONT_CARE);
			else if (*dm->channel == 122)
				set_result_csi = phydm_csi_mask_setting(dm, FUNC_ENABLE, *dm->channel, 80,
									5600, PHYDM_DONT_CARE);
...
__iram_odm_func__
boolean config_phydm_switch_band_8822b(
	struct dm_struct				*dm,
	u8					central_ch
)
{
	...
	if (dm->is_disable_phy_api) {
		PHYDM_DBG(dm, ODM_PHY_CONFIG,
			  "config_phydm_switch_band_8822b(): disable PHY API for debug!!\n");
		return true;
	}
	...
	if (central_ch <= 14) {
		...
		if (reg_8 == 0x1) {
			...
			if ((dm->rfe_type == 3) || (dm->rfe_type == 5) || (dm->rfe_type == 8)
			    || (dm->rfe_type == 17)) {
		...
		PHYDM_DBG(dm, ODM_PHY_CONFIG,
			  "config_phydm_switch_band_8822b(): Fail to switch band (ch: %d)\n",
			  central_ch);
		return false;
	}
	...
	if (rf_reg_status == false) {
		PHYDM_DBG(dm, ODM_PHY_CONFIG,
			  "config_phydm_switch_band_8822b(): Fail to switch band (ch: %d), because writing RF register is fail\n",
			  central_ch);
		return false;
	}
	...
	PHYDM_DBG(dm, ODM_PHY_CONFIG,
		  "config_phydm_switch_band_8822b(): Success to switch band (ch: %d)\n",
		  central_ch);
	return true;
}
...
)
{
	...
	if (dm->is_disable_phy_api) {
		PHYDM_DBG(dm, ODM_PHY_CONFIG,
			  "config_phydm_switch_channel_8822b(): disable PHY API for debug!!\n");
		return true;
	}
	...
	if (central_ch <= 14) {
		...
		if (!((dm->rfe_type == 15) || (dm->rfe_type == 16))) {
			if ((central_ch >= 36) && (central_ch <= 64)) {
				...
				PHYDM_DBG(dm, ODM_PHY_CONFIG,
					  "config_phydm_switch_channel_8822b(): Fail to switch channel (AGC) (ch: %d)\n",
					  central_ch);
				return false;
			}
		...
		if ((central_ch >= 36) && (central_ch <= 48))
			...
			PHYDM_DBG(dm, ODM_PHY_CONFIG,
				  "config_phydm_switch_channel_8822b(): Fail to switch channel (fc_area) (ch: %d)\n",
				  central_ch);
			return false;
		}
		...
		PHYDM_DBG(dm, ODM_PHY_CONFIG,
			  "config_phydm_switch_channel_8822b(): Fail to switch channel (ch: %d)\n",
			  central_ch);
		return false;
	}
	...
	if (rf_reg_be != 0xff)
		odm_set_rf_reg(dm, RF_PATH_A, 0xbe, (BIT(17) | BIT(16) | BIT(15)),
			       rf_reg_be);
	else {
		PHYDM_DBG(dm, ODM_PHY_CONFIG,
			  "config_phydm_switch_channel_8822b(): Fail to switch channel (ch: %d, Phase noise)\n",
			  central_ch);
		return false;
	}
	...
	if (rf_reg_status == false) {
		PHYDM_DBG(dm, ODM_PHY_CONFIG,
			  "config_phydm_switch_channel_8822b(): Fail to switch channel (ch: %d), because writing RF register is fail\n",
			  central_ch);
		return false;
	}
	...
	PHYDM_DBG(dm, ODM_PHY_CONFIG,
		  "config_phydm_switch_channel_8822b(): Success to switch channel (ch: %d)\n",
		  central_ch);
	return true;
}

__iram_odm_func__
boolean config_phydm_switch_bandwidth_8822b(
	struct dm_struct				*dm,
	u8					primary_ch_idx,
	enum channel_width				bandwidth
) {
	...
	{
		PHYDM_DBG(dm, ODM_PHY_CONFIG,
			  "config_phydm_switch_bandwidth_8822b(): disable PHY API for debug!!\n");
		return true;
	}
	...
	{
		PHYDM_DBG(dm, ODM_PHY_CONFIG,
			  "config_phydm_switch_bandwidth_8822b(): Fail to switch bandwidth (bw: %d, primary ch: %d)\n",
	...
	{
		PHYDM_DBG(dm, ODM_PHY_CONFIG,
			  "config_phydm_switch_bandwidth_8822b(): Fail to switch bandwidth (bw: %d, primary ch: %d), because writing RF register is fail\n",
			  bandwidth, primary_ch_idx);
		return false;
	}
...
__iram_odm_func__
boolean config_phydm_switch_channel_bw_8822b(
	struct dm_struct				*dm,
	u8					central_ch,
	u8					primary_ch_idx,
	enum channel_width				bandwidth
...
__iram_odm_func__
boolean config_phydm_trx_mode_8822b(
	struct dm_struct				*dm,
...
)
{
	...
	odm_set_bb_reg(dm, 0x80c, MASKBYTE0,
		       ((tx_path << 4) | tx_path));/* TX path HW block enable */
	...
	while (1) {
		...
		if ((rf_reg33 == 0x00001) && (config_phydm_read_rf_check_8822b(rf_reg33)))
			...
			PHYDM_DBG(dm, ODM_PHY_CONFIG,
				  "Fail to set TRx mode setting, because writing RF mode table is fail\n");
			return false;
		}
	}
	...
	if (*dm->mp_mode || (*dm->antenna_test) || (dm->normal_rx_path)) {
		...
		PHYDM_DBG(dm, ODM_PHY_CONFIG,
			  "MP mode or Antenna test mode!! support path-B TX and RX\n");
	} else {
		...
		odm_set_rf_reg(dm, RF_PATH_A, 0xef, RFREGOFFSETMASK, 0x00000);
		PHYDM_DBG(dm, ODM_PHY_CONFIG,
			  "Normal mode!! Do not support path-B TX and RX\n");
	}
...
__iram_odm_func__
boolean config_phydm_parameter_init_8822b(
	struct dm_struct				*dm,
