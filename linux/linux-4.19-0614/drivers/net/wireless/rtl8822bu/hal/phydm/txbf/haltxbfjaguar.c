...
)
{
	...
	odm_set_rf_reg(dm, RF_PATH_A, 0xef, 0x80000,
		       0x1);	/*RF mode table write enable*/
	odm_set_rf_reg(dm, RF_PATH_B, 0xef, 0x80000,
		       0x1);	/*RF mode table write enable*/
	...
	if (beam_info->beamformee_su_cnt > 0) {
		...
		odm_set_rf_reg(dm, RF_PATH_A, 0x32, 0xfffff,
		...
		odm_set_rf_reg(dm, RF_PATH_B, 0x32, 0xfffff,
			       0xE26BF);	/*Enable TXIQGEN in RX mode*/
	} else {
		/* Paath_A */
		odm_set_rf_reg(dm, RF_PATH_A, 0x30, 0x78000, 0x3);		/*Select RX mode*/
...
)
{
	...
	if (dm->rf_type == RF_2T2R)
		odm_set_bb_reg(dm, ODM_REG_CSI_CONTENT_VALUE, MASKDWORD,
			       0x00000000);	/*nc =2*/
	else
		odm_set_bb_reg(dm, ODM_REG_CSI_CONTENT_VALUE, MASKDWORD,
			       0x01081008);	/*nc =1*/
	...
	if ((beamforming_info->beamformer_su_cnt > 0)
		...
		if (bfer_idx == 0) {
			for (i = 0; i < 6 ; i++)
				odm_write_1byte(dm, (REG_BFMER0_INFO_8812A + i),
			...
			for (i = 0; i < 6 ; i++)
				odm_write_1byte(dm, (REG_BFMER1_INFO_8812A + i),
						beamformer_entry.mac_addr[i]);
			/*CSI report use legacy ofdm so don't need to fill P_AID.*/
	...
	if ((beamforming_info->beamformee_su_cnt > 0)
		...
		if (bfee_idx == 0) {
			...
			odm_write_1byte(dm, REG_TXBF_CTRL_8812A + 3, odm_read_1byte(dm,
					REG_TXBF_CTRL_8812A + 3) | BIT(4) | BIT(6) | BIT(7));
		} else
			odm_write_2byte(dm, REG_TXBF_CTRL_8812A + 2,
...
)
{
	...
	if (beamformee_entry.beamform_entry_cap == BEAMFORMING_CAP_NONE) {
		...
		if (idx == 0) {
			...
			odm_write_2byte(dm, REG_TXBF_CTRL_8812A + 2, odm_read_2byte(dm,
					REG_TXBF_CTRL_8812A + 2) & 0xF000);
			odm_write_2byte(dm, REG_BFMEE_SEL_8812A + 2, odm_read_2byte(dm,
