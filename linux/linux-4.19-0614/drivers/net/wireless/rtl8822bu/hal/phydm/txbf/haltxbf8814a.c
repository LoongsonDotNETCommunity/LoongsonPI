...
boolean phydm_beamforming_set_iqgen_8814A(
	void			*dm_void
)
{
	...
	for (i = RF_PATH_A ; i < MAX_RF_PATH ; i++)
		odm_set_rf_reg(dm, i, RF_WE_LUT, 0x80000,
	...
	while (1) {
		...
		if ((rf_mode[0] == 0x18000) && (rf_mode[1] == 0x18000)
		    && (rf_mode[2] == 0x18000) && (rf_mode[3] == 0x18000))
			break;
		else if (counter == 100) {
	...
	for (i = RF_PATH_A ; i < MAX_RF_PATH ; i++) {
		...
		odm_set_rf_reg(dm, i, RF_TXPA_G2, 0xfffff,
			       0x226BF); /*Enable TXIQGEN in Rx mode*/
	}
	odm_set_rf_reg(dm, RF_PATH_A, RF_TXPA_G2, 0xfffff,
		       0xE26BF); /*Enable TXIQGEN in Rx mode*/
...
void hal_txbf_8814a_set_ndpa_rate(
	void			*dm_void,
	u8	BW,
	u8	rate
)
...
)
{
	...
	x_read_data_addr = PHYDM_MEMORY_MAP_BUF_READ + mac_id *
			   32; /*Ctrl Info: 32Bytes for each macid(n)*/

	if ((x_read_data_addr < PHYDM_MEMORY_MAP_BUF_READ)
	    || (x_read_data_addr > 0x8FFF)) {
...
void hal_txbf_8814a_get_tx_rate(
	void			*dm_void
)
{
	struct dm_struct	*dm = (struct dm_struct *)dm_void;
...
void hal_txbf_8814a_reset_tx_path(
	void			*dm_void,
	u8				idx
)
{
	...
	if ((beamforming_info->last_usb_hub) != (*dm->hub_usb_mode)) {
		nr_index = tx_bf_nr(hal_txbf_8814a_get_ntx(dm),
				    beamformee_entry.comp_steering_num_of_bfer);
		...
		if (tx_ss == 0xf) {
			odm_set_bb_reg(dm, REG_BB_TX_PATH_SEL_1_8814A,
				       MASKBYTE3 | MASKBYTE2HIGHNIBBLE, 0x93f);
			odm_set_bb_reg(dm, REG_BB_TX_PATH_SEL_1_8814A, MASKDWORD, 0x93f93f0);
		} else if (tx_ss == 0xe) {
			odm_set_bb_reg(dm, REG_BB_TX_PATH_SEL_1_8814A,
				       MASKBYTE3 | MASKBYTE2HIGHNIBBLE, 0x93e);
			odm_set_bb_reg(dm, REG_BB_TX_PATH_SEL_2_8814A, MASKDWORD, 0x93e93e0);
		} else if (tx_ss == 0x6) {
			odm_set_bb_reg(dm, REG_BB_TX_PATH_SEL_1_8814A,
				       MASKBYTE3 | MASKBYTE2HIGHNIBBLE, 0x936);
		...
		if (idx == 0) {
			...
			case 1:			/*Nsts = 2	BC*/
				odm_set_bb_reg(dm, REG_BB_TXBF_ANT_SET_BF0_8814A,
					       MASKBYTE3LOWNIBBLE | MASKL3BYTES, 0x9366);		/*tx2path, BC*/
				break;

			case 2:			/*Nsts = 3	BCD*/
				odm_set_bb_reg(dm, REG_BB_TXBF_ANT_SET_BF0_8814A,
					       MASKBYTE3LOWNIBBLE | MASKL3BYTES, 0x93e93ee);	/*tx3path, BCD*/
				break;

			default:			/*nr>3, same as Case 3*/
				odm_set_bb_reg(dm, REG_BB_TXBF_ANT_SET_BF0_8814A,
					       MASKBYTE3LOWNIBBLE | MASKL3BYTES, 0x93f93ff);	/*tx4path, ABCD*/
				break;
			}
		} else	{
			switch (nr_index) {
			...
			case 1:			/*Nsts = 2	BC*/
				odm_set_bb_reg(dm, REG_BB_TXBF_ANT_SET_BF1_8814A,
					       MASKBYTE3LOWNIBBLE | MASKL3BYTES, 0x9366);		/*tx2path, BC*/
				break;
...
)
{
	...
	if (beamforming_info->beamformee_su_cnt > 0) {
		...
		odm_set_bb_reg(dm, REG_BB_TXBF_ANT_SET_BF1_8814A, BIT(28) | BIT29,
			       0x2);			/*enable BB TxBF ant mapping register*/
		odm_set_bb_reg(dm, REG_BB_TXBF_ANT_SET_BF1_8814A, BIT30,
			       0x1);			/*if Nsts > Nc don't apply V matrix*/
	...
	if ((beamforming_info->beamformee_su_cnt == 0)
	    && (beamforming_info->beamformer_su_cnt == 0)) {
		odm_set_bb_reg(dm, REG_BB_TX_PATH_SEL_1_8814A,
			       MASKBYTE3 | MASKBYTE2HIGHNIBBLE,
			       0x932);	/*set tx_path selection for 8814a BFer bug refine*/
		odm_set_bb_reg(dm, REG_BB_TX_PATH_SEL_2_8814A, MASKDWORD, 0x93e9360);
	}
}
#if 0
void hal_txbf_8814a_download_ndpa(
	void			*dm_void,
	u8				idx
)
{
	...
	*dm->is_fw_dw_rsvd_page_in_progress = true;
	...
	do {
		...
		odm_write_1byte(dm, REG_FIFOPAGE_CTRL_2_8814A + 1,
				(bcn_valid_reg | BIT(7)));
...
void hal_txbf_8814a_fw_txbf_cmd(
	void			*dm_void
)
{
	struct dm_struct	*dm = (struct dm_struct *)dm_void;
...
)
{
	...
	PHYDM_DBG(dm, DBG_TXBF, "[%s] bfer_idx=%d, bfee_idx=%d\n", __func__,
		  bfer_idx, bfee_idx);
	odm_set_mac_reg(dm, REG_SND_PTCL_CTRL_8814A, MASKBYTE1 | MASKBYTE2,
			0x0202);
	...
	if ((beamforming_info->beamformer_su_cnt > 0)
		...
		if (bfer_idx == 0) {
			for (i = 0; i < 6 ; i++)
				odm_write_1byte(dm, (REG_ASSOCIATED_BFMER0_INFO_8814A + i),
						beamformer_entry.mac_addr[i]);
		} else {
			for (i = 0; i < 6 ; i++)
				odm_write_1byte(dm, (REG_ASSOCIATED_BFMER1_INFO_8814A + i),
						beamformer_entry.mac_addr[i]);
		}

		/*CSI report parameters of Beamformer*/
		nc_index = hal_txbf_8814a_get_nrx(
	...
	if ((beamforming_info->beamformee_su_cnt > 0)
		...
		if (bfee_idx == 0) {
			...
			odm_write_1byte(dm, REG_TXBF_CTRL_8814A + 3, odm_read_1byte(dm,
					REG_TXBF_CTRL_8814A + 3) | BIT(4) | BIT(6) | BIT(7));
		} else
			odm_write_2byte(dm, REG_TXBF_CTRL_8814A + 2,
		...
		if (bfee_idx == 0) {
			...
			odm_write_2byte(dm, REG_ASSOCIATED_BFMEE_SEL_8814A + 2,
...
)
{
	...
	if (beamformee_entry.beamform_entry_cap == BEAMFORMING_CAP_NONE) {
		...
		if (idx == 0) {
			...
			odm_write_2byte(dm, REG_ASSOCIATED_BFMEE_SEL_8814A + 2, odm_read_2byte(dm,
					REG_ASSOCIATED_BFMEE_SEL_8814A + 2) & 0x60);
		}
	}
}

void hal_txbf_8814a_status(
	void			*dm_void,
	u8				idx
)
{
