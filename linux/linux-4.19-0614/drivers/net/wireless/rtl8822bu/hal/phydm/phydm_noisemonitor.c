...
s16 odm_inband_noise_monitor_n_series(struct dm_struct	*dm,
				      u8 is_pause_dig, u8 igi_value, u32 max_time)
{
	...
	reg_c50 &= ~BIT(7);
	dm->noise_level.noise[RF_PATH_A] = (s8)(-110 + reg_c50 +
	...
	if (max_rf_path == 2) {
		...
		reg_c58 &= ~BIT(7);
		dm->noise_level.noise[RF_PATH_B] = (s8)(-110 + reg_c58 +
...
)
{
	...
	if (max_rf_path == 2) {
		...
		reg_e50 &= ~BIT(7);
		dm->noise_level.noise[RF_PATH_B] = (s8)(-110 + reg_e50 +
							noise_data.sum[RF_PATH_B]);
		dm->noise_level.noise_all += dm->noise_level.noise[RF_PATH_B];
	}
	dm->noise_level.noise_all /= max_rf_path;
...
)
{
	...
	if (dm->support_ic_type & (ODM_RTL8822B | ODM_RTL8821C))
		return phydm_idle_noise_measurement_ac(dm, is_pause_dig, igi_value,
						       max_time);
...
)
{
	...
	if (dm->support_ic_type & ODM_IC_11AC_SERIES)
		return odm_inband_noise_monitor_ac_series(dm, is_pause_dig, igi_value,
				max_time);
	else
		return odm_inband_noise_monitor_n_series(dm, is_pause_dig, igi_value,
				max_time);
}
...
)
{
	...
	score_smooth = (total_cca_cnt >= 300) ? ((dm->noisy_decision_smooth + 3) >>
			3) : 0;
