...
)
{
	...
	if (dm->support_ic_type & ODM_IC_11N_SERIES) {
		if (curr_mf_state == MF_USC_LSC) {
			...
			odm_set_bb_reg(dm, 0xc84, 0xf0000000,
...
)
{
	...
	if (ACTING_AS_AP(
		    dm->adapter))   /* primary cca process only do at AP mode */
#endif
	{
#if (DM_ODM_SUPPORT_TYPE == ODM_WIN)
...
)
{
	...
	sec_ch_offset = *
			(dm->sec_ch_offset);		/* NIC: 2: sec is below,  1: sec is above */
...
)
{
	...
	sec_ch_offset = *
			(dm->sec_ch_offset);		/* AP: 1: sec is below,  2: sec is above */
