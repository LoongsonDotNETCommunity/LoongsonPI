...
void phydm_dynamic_switch_htstf_mumimo_8822b(
	struct dm_struct		*dm
)
{
...
void phydm_dynamic_parameters_ota(
	struct dm_struct		*dm
)
{
	u8	rssi_l2h = 40, rssi_h2l = 35;
...
static
void _txa_bias_cali_4_each_path(
	struct dm_struct		*dm,
	u8	 rf_path,
	u8	 efuse_value
)
...
void phydm_txcurrentcalibration(
	struct dm_struct		*dm
)
{
...
void phydm_dynamic_select_cck_path_8822b(
	struct 	dm_struct		*dm
)
{
	struct phydm_fa_struct	*fa_cnt = (struct phydm_fa_struct *)
	...
	if (dm->ap_total_num > 10) {
		if (drp_8822b->path_judge & BIT(2))
			odm_set_bb_reg(dm, 0xa04, 0x0f000000,
	...
	if (drp_8822b->path_judge & BB_PATH_A) {
		...
		drp_8822b->path_judge = (enum bb_path)(drp_8822b->path_judge | BIT(
				2));	/*it means we have already choosed cck rx path*/
	}
...
)
{
	...
	if (!switch_soml && ((dm->rfe_type == 1) || (dm->rfe_type == 6)
	...
	if (*dm->channel <= 14) {
		if (switch_soml && (!((dm->rfe_type == 3) || (dm->rfe_type == 5)
	...
	if (!((dm->rfe_type == 1) || (dm->rfe_type == 6) || (dm->rfe_type == 7)
		...
		if (*dm->channel <= 14) {
			...
			if ((switch_soml == true) && (!((dm->rfe_type == 3)
							|| (dm->rfe_type == 5)))) {
				if (switch_soml == true) {
					odm_set_bb_reg(dm, 0x8cc, MASKDWORD, 0x08108000);
					odm_set_bb_reg(dm, 0x8d8, BIT(27), 0x0);
...
void phydm_config_tx2path_8822b(
	struct dm_struct		*dm,
...
void phydm_dynamic_ant_weighting_8822b(
	void		*dm_void
)
{
...
)
{
	...
	if ((dm->p_advance_ota & PHYDM_HP_OTA_SETTING_A)
	    || (dm->p_advance_ota & PHYDM_HP_OTA_SETTING_B)) {
	...
	if ((dm->p_advance_ota & PHYDM_ASUS_OTA_SETTING_CCK_PATH)
	    || (dm->p_advance_ota & PHYDM_HP_OTA_SETTING_CCK_PATH)) {
		if (dm->is_linked)
			phydm_dynamic_select_cck_path_8822b(dm);
		else
			drp_8822b->path_judge = (enum bb_path)(drp_8822b->path_judge | ((~ BIT(
					2)) | BB_PATH_A | BB_PATH_B));
	}
