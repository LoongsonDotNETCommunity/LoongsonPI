...
#if (DM_ODM_SUPPORT_TYPE & ODM_WIN)
		if (PlatformAllocateMemoryWithZero(adapter, (void **)&adc_smp_buf->octet,
...
)
{
	...
	if (dm->support_ic_type & ODM_RTL8197F) {
		...
		/*move LA mode content from IMEM to TxPktBuffer
			...
			Length : 64K*/
		GET_HAL_INTERFACE(dm->priv)->init_ddma_handler(dm->priv, OCPBASE_IMEM,
...
)
{
	...
	if (adc_smp->la_trig_mode == PHYDM_MAC_TRIG) {
		...
		odm_set_mac_reg(dm, 0x7c0, BIT(4) | BIT(3),
		...
		if (adc_smp->la_trig_mode == PHYDM_ADC_MAC_TRIG) {
			odm_set_mac_reg(dm, 0x7c0, BIT(3),
...
)
{
	...
	if (information_buffer_length != adc_smp_buf->buffer_size) {
		...
		*bytes_written = 0;
		...
		odm_move_memory(dm, information_buffer, adc_smp_buf->octet,
				adc_smp_buf->buffer_size);
...
)
{
	...
	if (dm->support_ic_type & ODM_IC_11AC_SERIES) {
		...
		odm_set_bb_reg(dm, 0x95C, 0x1f,
			       trig_sig_sel);	/*0x95C[4:0], BB debug port bit*/
		odm_set_bb_reg(dm, 0x95C, BIT(31),
		...
		*/
		...
		odm_set_bb_reg(dm, 0x9a0, 0x1f,
			       trig_sig_sel);	/*0x9A0[4:0], BB debug port bit*/
		odm_set_bb_reg(dm, 0x9A0, BIT(31),
			       is_trigger_edge); /*0: posedge, 1: negedge*/
...
)
{
	...
	if (dm->support_ic_type & PHYDM_IC_SUPPORT_LA_MODE) {
		...
		if ((strcmp(input[1], help) == 0)) {
			...
			adc_smp_set(dm, trig_mode, trig_sig_sel, dma_data_sig_sel,
