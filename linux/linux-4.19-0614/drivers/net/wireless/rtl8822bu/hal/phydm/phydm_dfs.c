...
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	struct _DFS_STATISTICS	*dfs = (struct _DFS_STATISTICS *)
				       phydm_get_structure(dm, PHYDM_DFS);
	...
	} else if (dm->support_ic_type & (ODM_RTL8814A | ODM_RTL8822B |
					  ODM_RTL8821C)) {
		odm_set_bb_reg(dm, 0x814, 0x3fffffff, 0x04cc4d10);
		odm_set_bb_reg(dm, 0x834, MASKBYTE0, 0x06);
	...
	if ((dfs->idle_mode == 1)) { /*idle (no traffic)*/
		...
		if (region_domain == PHYDM_DFS_DOMAIN_MKK) {
			...
			if (phydm_dfs_is_meteorology_channel(
				    dm)) {/*need to add check cac end condition*/
				peak_th_cur = 2;
				nb2wb_th_cur = 3;
				three_peak_opt_cur = 1;
				three_peak_th2_cur = 0;
...
)
{
	...
	if (index == 0)
		pre_post_now_acc_fa_in_hist = dfs->fa_inc_hist[index] +
					      dfs->fa_inc_hist[index + 1] + dfs->fa_inc_hist[4];
	else if (index == 4)
		pre_post_now_acc_fa_in_hist = dfs->fa_inc_hist[index] + dfs->fa_inc_hist[0]
					      + dfs->fa_inc_hist[index - 1];
	else
		pre_post_now_acc_fa_in_hist = dfs->fa_inc_hist[index] +
					      dfs->fa_inc_hist[index + 1] + dfs->fa_inc_hist[index - 1];
	...
	if (dfs->det_print) {
		...
		PHYDM_DBG(dm, DBG_DFS,
			  ...
			  dfs->igi_cur, dfs->st_l2h_cur, regf98_value, short_pulse_cnt_inc,
			  long_pulse_cnt_inc);
	...
	if (max_fa_in_hist >= fa_mask_th || total_fa_in_hist >= fa_mask_th
	    || pre_post_now_acc_fa_in_hist >= fa_mask_th || (dfs->igi_cur >= 0x30)) {
	...
	if (dfs->det_print) {
		...
		PHYDM_DBG(dm, DBG_DFS,
			  ...
			  fa_mask_th, max_fa_in_hist, total_fa_in_hist, pre_post_now_acc_fa_in_hist);
	}
	...
	if ((st_l2h_new != dfs->st_l2h_cur)) {
		...
		dfs->pwdb_th = MAX_2(dfs->pwdb_th,
				     (int)dfs->pwdb_th); /*limit the pwdb value to absoulte lower bound 8*/
		dfs->pwdb_th = MIN_2(dfs->pwdb_th,
...
boolean phydm_radar_detect(void *dm_void)
{
	...
	if (dfs->igi_pre != dfs->igi_cur) {
		dfs->pwdb_th = ((int)dfs->st_l2h_cur - (int)dfs->igi_cur) / 2 +
			       dfs->pwdb_scalar_factor;
		dfs->pwdb_th = MAX_2(dfs->pwdb_th_cur,
				     (int)dfs->pwdb_th); /* limit the pwdb value to absoulte lower bound 0xa */
		dfs->pwdb_th = MIN_2(dfs->pwdb_th_cur,
				     0x1f);    /* limit the pwdb value to absoulte upper bound 0x1f */
		odm_set_bb_reg(dm,  0x918, 0x00001f00, dfs->pwdb_th);
	}
