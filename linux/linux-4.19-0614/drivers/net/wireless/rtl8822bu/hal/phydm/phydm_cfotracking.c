...
)
{
	...
	if (dm->support_ic_type & (ODM_RTL8188E | ODM_RTL8188F)) {
	...
	else if ((dm->support_ic_type & (ODM_RTL8703B | ODM_RTL8723B | ODM_RTL8192E
					 | ODM_RTL8821 | ODM_RTL8723D))) {
		...
		HAL_SetSYSOnReg((PADAPTER)dm->adapter, REG_SYS_XTAL_CTRL0, 0x3FFC0000,
				(crystal_cap | (crystal_cap << 6)));
#endif
	}
#endif
...
u8 phydm_get_default_crytaltal_cap(
	void					*dm_void
)
{
	struct dm_struct	*dm = (struct dm_struct *)dm_void;
...
)
{
	...
	atc_status = (boolean)odm_get_bb_reg(dm, ODM_REG(BB_ATC, dm),
					     ODM_BIT(BB_ATC, dm));
	return atc_status;
}

void phydm_cfo_tracking_reset(
	void					*dm_void
)
{
	struct dm_struct					*dm = (struct dm_struct *)dm_void;
...
)
{
	...
	cfo_track->def_x_cap = cfo_track->crystal_cap =
				       phydm_get_default_crytaltal_cap(dm);
...
)
{
	...
	if (!dm->is_linked || !dm->is_one_entry_only) {
		...
		for (i = 0; i < dm->num_rf_path; i++) {
			...
			valid_path_cnt++;
			cfo_rpt_sum = (u32)CFO_HW_RPT_2_KHZ(((cfo_track->CFO_tail[i] < 0) ?
							     (0 - cfo_track->CFO_tail[i]) :  cfo_track->CFO_tail[i]));
			cfo_khz_avg[i] = cfo_rpt_sum / cfo_track->CFO_cnt[i];

			PHYDM_DBG(dm, DBG_CFO_TRK,
				  "[Path-%d] CFO_sum = (( %d )), cnt = (( %d )) , CFO_avg= (( %s%d )) kHz\n",
				  i, cfo_rpt_sum, cfo_track->CFO_cnt[i],
				  ((cfo_track->CFO_tail[i] < 0) ? "-" : " "), cfo_khz_avg[i]);
		}
...
)
{
	...
	struct phydm_perpkt_info_struct		*pktinfo = (struct
			phydm_perpkt_info_struct *)pktinfo_void;
	struct phydm_cfo_track_struct			*cfo_track = (struct phydm_cfo_track_struct
			*)phydm_get_structure(dm, PHYDM_CFOTRACK);
	u8					i;
