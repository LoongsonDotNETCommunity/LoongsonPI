...
)
{
	...
	if (dm->support_ic_type & ODM_IC_11AC_SERIES) {
		...
		odm_set_bb_reg(dm, 0x8f8, (BIT(25) | BIT(24) | BIT(23) | BIT(22)),
			       header_idx);
		...
		if (dbg->low_bw_20_occur) {
			for (i = 0; i < rate_num; i++) {
				...
				PHYDM_DBG(dm, ODM_COMP_COMMON,
					  ...
					  (rate_ss_shift), (rate_ss_shift + 7),
					  dbg->num_qry_pkt_sc_20m[rate_ss_shift + 0],
					  dbg->num_qry_pkt_sc_20m[rate_ss_shift + 1],
					  dbg->num_qry_pkt_sc_20m[rate_ss_shift + 2],
					  dbg->num_qry_pkt_sc_20m[rate_ss_shift + 3],
					  dbg->num_qry_pkt_sc_20m[rate_ss_shift + 4],
					  dbg->num_qry_pkt_sc_20m[rate_ss_shift + 5],
					  dbg->num_qry_pkt_sc_20m[rate_ss_shift + 6],
					  dbg->num_qry_pkt_sc_20m[rate_ss_shift + 7]);
			}
		}
	}
		...
		if (dbg->low_bw_20_occur) {
			for (i = 0; i < rate_num; i++) {
				...
				PHYDM_DBG(dm, ODM_COMP_COMMON,
					  ...
					  dbg->num_qry_pkt_sc_20m[rate_ss_shift + 6],
					  dbg->num_qry_pkt_sc_20m[rate_ss_shift + 7],
					  dbg->num_qry_pkt_sc_20m[rate_ss_shift + 8],
					  dbg->num_qry_pkt_sc_20m[rate_ss_shift + 9]);
			}
		}
		...
		if (dbg->low_bw_40_occur) {
			for (i = 0; i < rate_num; i++) {
				...
				PHYDM_DBG(dm, ODM_COMP_COMMON,
					  ...
					  (i + 1),
					  dbg->num_qry_pkt_sc_40m[rate_ss_shift + 0],
					  dbg->num_qry_pkt_sc_40m[rate_ss_shift + 1],
					  dbg->num_qry_pkt_sc_40m[rate_ss_shift + 2],
					  dbg->num_qry_pkt_sc_40m[rate_ss_shift + 3],
					  dbg->num_qry_pkt_sc_40m[rate_ss_shift + 4],
					  dbg->num_qry_pkt_sc_40m[rate_ss_shift + 5],
					  dbg->num_qry_pkt_sc_40m[rate_ss_shift + 6],
					  dbg->num_qry_pkt_sc_40m[rate_ss_shift + 7],
					  dbg->num_qry_pkt_sc_40m[rate_ss_shift + 8],
					  dbg->num_qry_pkt_sc_40m[rate_ss_shift + 9]);
			}
		}
	}
#endif
...
{
	struct dm_struct		*dm = (struct dm_struct *)dm_void;
	struct phydm_phystatus_statistic		*dbg_statistic =
			&dm->phy_dbg_info.phystatus_statistic_info;
	struct phydm_phystatus_avg		*dbg_avg =
			&dm->phy_dbg_info.phystatus_statistic_avg;
	...
	/*CCK*/
	dbg_avg->rssi_cck_avg = (u8)((dbg_statistic->rssi_cck_cnt != 0) ?
				     (dbg_statistic->rssi_cck_sum / dbg_statistic->rssi_cck_cnt) : 0);
	...
	if (dbg_statistic->rssi_ofdm_cnt != 0) {
		dbg_avg->rssi_ofdm_avg = (u8)(dbg_statistic->rssi_ofdm_sum /
					      dbg_statistic->rssi_ofdm_cnt);
		dbg_avg->evm_ofdm_avg = (u8)(dbg_statistic->evm_ofdm_sum /
					     dbg_statistic->rssi_ofdm_cnt);
		dbg_avg->snr_ofdm_avg = (u8)(dbg_statistic->snr_ofdm_sum /
					     dbg_statistic->rssi_ofdm_cnt);
	}
	...
	if (dbg_statistic->rssi_1ss_cnt != 0) {
		dbg_avg->rssi_1ss_avg = (u8)(dbg_statistic->rssi_1ss_sum /
					     dbg_statistic->rssi_1ss_cnt);
		dbg_avg->evm_1ss_avg = (u8)(dbg_statistic->evm_1ss_sum /
					    dbg_statistic->rssi_1ss_cnt);
		dbg_avg->snr_1ss_avg = (u8)(dbg_statistic->snr_1ss_sum /
					    dbg_statistic->rssi_1ss_cnt);
	}

	PHYDM_DBG(dm, ODM_COMP_COMMON,
		  "* 1-ss Cnt= ((%d)) RSSI:{%d} EVM:{%d} SNR:{%d}\n",
		  dbg_statistic->rssi_1ss_cnt, dbg_avg->rssi_1ss_avg, dbg_avg->evm_1ss_avg,
		  dbg_avg->snr_1ss_avg);

#if (defined(PHYDM_COMPILE_ABOVE_2SS))
	if (dm->support_ic_type & (PHYDM_IC_ABOVE_2SS)) {
		if (dbg_statistic->rssi_2ss_cnt != 0) {
			dbg_avg->rssi_2ss_avg[0] = (u8)(dbg_statistic->rssi_2ss_sum[0] /
							dbg_statistic->rssi_2ss_cnt);
			dbg_avg->rssi_2ss_avg[1] = (u8)(dbg_statistic->rssi_2ss_sum[1] /
							dbg_statistic->rssi_2ss_cnt);

			dbg_avg->evm_2ss_avg[0] = (u8)(dbg_statistic->evm_2ss_sum[0] /
						       dbg_statistic->rssi_2ss_cnt);
			dbg_avg->evm_2ss_avg[1] = (u8)(dbg_statistic->evm_2ss_sum[1] /
						       dbg_statistic->rssi_2ss_cnt);

			dbg_avg->snr_2ss_avg[0] = (u8)(dbg_statistic->snr_2ss_sum[0] /
						       dbg_statistic->rssi_2ss_cnt);
			dbg_avg->snr_2ss_avg[1] = (u8)(dbg_statistic->snr_2ss_sum[1] /
						       dbg_statistic->rssi_2ss_cnt);
		}
	...
	if (dm->support_ic_type & (PHYDM_IC_ABOVE_3SS)) {
		if (dbg_statistic->rssi_3ss_cnt != 0) {
			dbg_avg->rssi_3ss_avg[0] = (u8)(dbg_statistic->rssi_3ss_sum[0] /
							dbg_statistic->rssi_3ss_cnt);
			dbg_avg->rssi_3ss_avg[1] = (u8)(dbg_statistic->rssi_3ss_sum[1] /
							dbg_statistic->rssi_3ss_cnt);
			dbg_avg->rssi_3ss_avg[2] = (u8)(dbg_statistic->rssi_3ss_sum[2] /
							dbg_statistic->rssi_3ss_cnt);

			dbg_avg->evm_3ss_avg[0] = (u8)(dbg_statistic->evm_3ss_sum[0] /
						       dbg_statistic->rssi_3ss_cnt);
			dbg_avg->evm_3ss_avg[1] = (u8)(dbg_statistic->evm_3ss_sum[1] /
						       dbg_statistic->rssi_3ss_cnt);
			dbg_avg->evm_3ss_avg[2] = (u8)(dbg_statistic->evm_3ss_sum[2] /
						       dbg_statistic->rssi_3ss_cnt);

			dbg_avg->snr_3ss_avg[0] = (u8)(dbg_statistic->snr_3ss_sum[0] /
						       dbg_statistic->rssi_3ss_cnt);
			dbg_avg->snr_3ss_avg[1] = (u8)(dbg_statistic->snr_3ss_sum[1] /
						       dbg_statistic->rssi_3ss_cnt);
			dbg_avg->snr_3ss_avg[2] = (u8)(dbg_statistic->snr_3ss_sum[2] /
						       dbg_statistic->rssi_3ss_cnt);
		}
		...
		PHYDM_DBG(dm, ODM_COMP_COMMON,
			  ...
			  dbg_statistic->rssi_3ss_cnt,
			  dbg_avg->rssi_3ss_avg[0], dbg_avg->rssi_3ss_avg[1],
			  dbg_avg->rssi_3ss_avg[2],
	...
	if (dm->support_ic_type & PHYDM_IC_ABOVE_4SS) {
		if (dbg_statistic->rssi_4ss_cnt != 0) {
			dbg_avg->rssi_4ss_avg[0] = (u8)(dbg_statistic->rssi_4ss_sum[0] /
							dbg_statistic->rssi_4ss_cnt);
			dbg_avg->rssi_4ss_avg[1] = (u8)(dbg_statistic->rssi_4ss_sum[1] /
							dbg_statistic->rssi_4ss_cnt);
			dbg_avg->rssi_4ss_avg[2] = (u8)(dbg_statistic->rssi_4ss_sum[2] /
							dbg_statistic->rssi_4ss_cnt);
			dbg_avg->rssi_4ss_avg[3] = (u8)(dbg_statistic->rssi_4ss_sum[3] /
							dbg_statistic->rssi_4ss_cnt);

			dbg_avg->evm_4ss_avg[0] = (u8)(dbg_statistic->evm_4ss_sum[0] /
						       dbg_statistic->rssi_4ss_cnt);
			dbg_avg->evm_4ss_avg[1] = (u8)(dbg_statistic->evm_4ss_sum[1] /
						       dbg_statistic->rssi_4ss_cnt);
			dbg_avg->evm_4ss_avg[2] = (u8)(dbg_statistic->evm_4ss_sum[2] /
						       dbg_statistic->rssi_4ss_cnt);
			dbg_avg->evm_4ss_avg[3] = (u8)(dbg_statistic->evm_4ss_sum[3] /
						       dbg_statistic->rssi_4ss_cnt);

			dbg_avg->snr_4ss_avg[0] = (u8)(dbg_statistic->snr_4ss_sum[0] /
						       dbg_statistic->rssi_4ss_cnt);
			dbg_avg->snr_4ss_avg[1] = (u8)(dbg_statistic->snr_4ss_sum[1] /
						       dbg_statistic->rssi_4ss_cnt);
			dbg_avg->snr_4ss_avg[2] = (u8)(dbg_statistic->snr_4ss_sum[2] /
						       dbg_statistic->rssi_4ss_cnt);
			dbg_avg->snr_4ss_avg[3] = (u8)(dbg_statistic->snr_4ss_sum[3] /
						       dbg_statistic->rssi_4ss_cnt);
		}
		...
		PHYDM_DBG(dm, ODM_COMP_COMMON,
			  ...
			  dbg_statistic->rssi_4ss_cnt,
			  dbg_avg->rssi_4ss_avg[0], dbg_avg->rssi_4ss_avg[1],
			  dbg_avg->rssi_4ss_avg[2], dbg_avg->rssi_4ss_avg[3],
			  dbg_avg->evm_4ss_avg[0], dbg_avg->evm_4ss_avg[1], dbg_avg->evm_4ss_avg[2],
			  dbg_avg->evm_4ss_avg[3],
			  dbg_avg->snr_4ss_avg[0], dbg_avg->snr_4ss_avg[1], dbg_avg->snr_4ss_avg[2],
...
)
{
	...
	struct phydm_fa_struct *false_alm_cnt = (struct phydm_fa_struct *)
						phydm_get_structure(dm, PHYDM_FALSEALMCNT);
	struct phydm_cfo_track_struct				*cfo_track = (struct
			phydm_cfo_track_struct *)phydm_get_structure(dm, PHYDM_CFOTRACK);
	...
	if (dm->is_linked) {
		...
		if ((dm->support_ic_type & PHYSTS_2ND_TYPE_IC)
			...
			PHYDM_DBG(dm, ODM_COMP_COMMON, "[AGC Idx] {0x%x, 0x%x, 0x%x, 0x%x}\n",
				  dm->ofdm_agc_idx[0], dm->ofdm_agc_idx[1], dm->ofdm_agc_idx[2],
				  dm->ofdm_agc_idx[3]);
		} else {
		...
		tmp_val_u1 = (cfo_track->crystal_cap > cfo_track->def_x_cap) ?
			     (cfo_track->crystal_cap - cfo_track->def_x_cap) : (cfo_track->def_x_cap -
					     cfo_track->crystal_cap);
		PHYDM_DBG(dm, ODM_COMP_COMMON,
			  "CFO_avg = ((%d kHz)) , CFO_tracking = ((%s%d))\n",
			  cfo_track->CFO_ave_pre,
			  ((cfo_track->crystal_cap > cfo_track->def_x_cap) ? "+" : "-"), tmp_val_u1);
	...
	PHYDM_DBG(dm, ODM_COMP_COMMON,
		  ...
		  dm->is_linked, dm->number_linked_client, dm->rssi_min, dig_t->cur_ig_value,
		  dm->noisy_decision);
}
...
)
{
	...
	if (dm_value[1] >= dm->num_rf_path) {
		...
		if (phydm_api_set_txagc(dm, dm_value[3], (enum rf_path) dm_value[1],
					(u8)dm_value[2], true))
			PDM_SNPF(out_len, used, output + used,
		...
		if (dm->support_ic_type & (ODM_RTL8822B | ODM_RTL8821C)) {
			...
			for (i = 0; i < ODM_RATEVHTSS2MCS9; i += 4)
				status = (status & phydm_api_set_txagc(dm, power_index,
...
)
{
	...
	if (dm_value[0] == 100) {
		...
		if (dm->fw_debug_components == 0) {
			...
			phydm_fw_trace_en_h2c(dm, false, dm->fw_debug_components, dm_value[2],
					      dm_value[3]); /*H2C to enable C2H Msg*/
		} else {
			dm->debug_components |= DBG_FW_TRACE;
			phydm_fw_trace_en_h2c(dm, true, dm->fw_debug_components, dm_value[2],
					      dm_value[3]); /*H2C to enable C2H Msg*/
		}
	}
}
...
)
{
	...
	if (state == false) {
		...
		odm_set_bb_reg(dm, 0x8c8, 0xfe,
			       ((dig_t->big_jump_step3 << 5) | (dig_t->big_jump_step2 << 3) |
			...
			if (*dm->band_width == CHANNEL_WIDTH_20) {
				for (tone_num = 228; tone_num <= 255; tone_num++) {
					...
					pr_debug("Tone((-%-3d)) RXEVM (1ss/2ss) =%d , %d\n", (256 - tone_num),
						 evm_tone_0[tone_num], evm_tone_1[tone_num]);
				}
...
)
{
	...
	case PHYDM_SMART_ANT:
		...
		if (input_idx >= 1) {
			...
			phydm_hl_smart_ant_debug_type2(dm, &input[0], &used, output, &out_len,
						       input_num);
#elif (defined(CONFIG_HL_SMART_ANTENNA_TYPE1))
			phydm_hl_smart_ant_debug(dm, &input[0], &used, output, &out_len,
						 input_num);
#endif
			...
			phydm_cumitek_smt_ant_debug(dm, &input[0], &used, output, &out_len,
						    input_num);
#endif
		}
	...
	case PHYDM_TRX_PATH:
		...
		if (dm->support_ic_type & (ODM_RTL8822B | ODM_RTL8197F)) {
			...
			if (is_enable_dbg_mode) {
				dm->is_disable_phy_api = false;
				phydm_api_trx_mode(dm, (enum bb_path) tx_path, (enum bb_path) rx_path,
						   is_tx2_path);
				dm->is_disable_phy_api = true;
				PDM_SNPF(out_len, used, output + used,
					 out_len - used,
	...
	case PHYDM_LA_MODE:
		...
		phydm_lamode_trigger_setting(dm, &input[0], &used, output, &out_len,
					     input_num);
#else
		PDM_SNPF(out_len, used, output + used, out_len - used,
	...
	case PHYDM_CSI_MASK_EN:
		...
		if (input_idx >= 1) {
			phydm_api_debug(dm, PHYDM_API_CSI_MASK, (u32 *)var1, &used, output,
					&out_len);
			/**/
		}
	...
	case PHYDM_DIS_DYM_ANT_WEIGHTING:
		...
		phydm_dyn_ant_weight_dbg(dm, &input[0], &used, output, &out_len,
					 input_num);
#endif
		break;
	...
	case PHYDM_PAUSE_FUNC:
		phydm_pause_func_console(dm, &input[0], &used, output, &out_len,
					 input_num);
		break;
