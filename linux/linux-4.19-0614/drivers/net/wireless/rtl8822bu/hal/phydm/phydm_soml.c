...
)
{
	...
	if (dm->support_interface == ODM_ITRF_PCIE)
		...
		rtw_run_in_thread_cmd(padapter, phydm_adaptive_soml_workitem_callback,
				      padapter);
	}
}
...
)
{
	...
	if (dm_soml_table->soml_state_cnt < ((dm_soml_table->soml_train_num) <<
					     1)) {
	...
	if (dm->support_ic_type == ODM_RTL8197F) {
		if (on_off_state == SOML_ON) {
			for (i = 0; i < HT_RATE_IDX; i++) {
				num_bytes_diff = dm_soml_table->num_ht_bytes[i] -
						 dm_soml_table->pre_num_ht_bytes[i];
		...
		if (on_off_state == SOML_ON) {
			for (i = 0; i < VHT_RATE_IDX; i++) {
				num_bytes_diff = dm_soml_table->num_vht_bytes[i] -
						 dm_soml_table->pre_num_vht_bytes[i];
...
)
{
	...
	if ((dm->support_ic_type & ODM_ADAPTIVE_SOML_SUPPORT_IC)) {
		if (TRUE) {
			if ((dm->rssi_min >= SOML_RSSI_TH_HIGH)
			    || (dm_soml_table->is_soml_method_enable == 1)) {
				PHYDM_DBG(dm, DBG_ADPTV_SOML, "> TH_H || is_soml_method_enable==1\n");
				PHYDM_DBG(dm, DBG_ADPTV_SOML,
					  "soml_state_cnt =((%d)),  soml_on_off =((%s))\n",
					  dm_soml_table->soml_state_cnt,
					  (dm_soml_table->soml_on_off == SOML_ON) ? "SOML_ON" : "SOML_OFF");
				/*Traning state: 0(alt) 1(ori) 2(alt) 3(ori)============================================================*/
				if (dm_soml_table->soml_state_cnt < ((dm_soml_table->soml_train_num) <<
								     1)) {
					if (dm_soml_table->soml_state_cnt == 0) {
						if (dm->support_ic_type == ODM_RTL8197F) {
							odm_move_memory(dm, dm_soml_table->num_ht_bytes, ht_reset,
									HT_RATE_IDX * size);
							odm_move_memory(dm, dm_soml_table->num_ht_bytes_on, ht_reset,
									HT_RATE_IDX * size);
							odm_move_memory(dm, dm_soml_table->num_ht_bytes_off, ht_reset,
									HT_RATE_IDX * size);
						} else if (dm->support_ic_type == ODM_RTL8822B) {
							odm_move_memory(dm, dm_soml_table->num_vht_bytes, vht_reset,
									VHT_RATE_IDX * size);
							odm_move_memory(dm, dm_soml_table->num_vht_bytes_on, vht_reset,
									VHT_RATE_IDX * size);
							odm_move_memory(dm, dm_soml_table->num_vht_bytes_off, vht_reset,
									VHT_RATE_IDX * size);
							...
							PHYDM_DBG(dm, DBG_ADPTV_SOML,
								  ...
								  dm_soml_table->cfo_counter, dm_soml_table->cfo_diff_a,
								  dm_soml_table->cfo_diff_b);
						...
						odm_set_timer(dm, &dm_soml_table->phydm_adaptive_soml_timer,
							      dm_soml_table->soml_delay_time); /*ms*/
					} else if ((dm_soml_table->soml_state_cnt % 2) != 0) {
						dm_soml_table->soml_state_cnt++;
						if (dm->support_ic_type == ODM_RTL8197F)
							odm_move_memory(dm, dm_soml_table->pre_num_ht_bytes,
									dm_soml_table->num_ht_bytes, HT_RATE_IDX * size);
						else if (dm->support_ic_type == ODM_RTL8822B) {
							odm_move_memory(dm, dm_soml_table->pre_num_vht_bytes,
									dm_soml_table->num_vht_bytes, VHT_RATE_IDX * size);
							dm_soml_table->cfo_counter++;
							phydm_soml_cfo_process(dm,
									       &dm_soml_table->cfo_diff_a,
									       &dm_soml_table->cfo_diff_b);
				...
				/*Decision state: ==============================================================*/
					...
					phydm_soml_statistics(dm, dm_soml_table->soml_on_off);
					dm_soml_table->cfo_diff_avg_a = (dm_soml_table->cfo_counter != 0) ?
									(dm_soml_table->cfo_diff_sum_a  / dm_soml_table->cfo_counter) : 0;
					dm_soml_table->cfo_diff_avg_b = (dm_soml_table->cfo_counter != 0) ?
									(dm_soml_table->cfo_diff_sum_b / dm_soml_table->cfo_counter) : 0;
					PHYDM_DBG(dm, DBG_ADPTV_SOML,
					...
					if (dm->support_ic_type == ODM_RTL8197F) {
						for (i = 0; i < rate_num; i++) {
							rate_ss_shift = (i << 3);
							PHYDM_DBG(dm, DBG_ADPTV_SOML,
								  "*num_ht_bytes_on  HT MCS[%d :%d ] = {%d, %d, %d, %d, %d, %d, %d, %d}\n",
								  (rate_ss_shift), (rate_ss_shift + 7),
								  dm_soml_table->num_ht_bytes_on[rate_ss_shift + 0],
								  dm_soml_table->num_ht_bytes_on[rate_ss_shift + 1],
								  dm_soml_table->num_ht_bytes_on[rate_ss_shift + 2],
								  dm_soml_table->num_ht_bytes_on[rate_ss_shift + 3],
								  dm_soml_table->num_ht_bytes_on[rate_ss_shift + 4],
								  dm_soml_table->num_ht_bytes_on[rate_ss_shift + 5],
								  dm_soml_table->num_ht_bytes_on[rate_ss_shift + 6],
								  dm_soml_table->num_ht_bytes_on[rate_ss_shift + 7]);
						}

						for (i = 0; i < rate_num; i++) {
							rate_ss_shift = (i << 3);
							PHYDM_DBG(dm, DBG_ADPTV_SOML,
								  "*num_ht_bytes_off  HT MCS[%d :%d ] = {%d, %d, %d, %d, %d, %d, %d, %d}\n",
								  (rate_ss_shift), (rate_ss_shift + 7),
								  dm_soml_table->num_ht_bytes_off[rate_ss_shift + 0],
								  dm_soml_table->num_ht_bytes_off[rate_ss_shift + 1],
								  dm_soml_table->num_ht_bytes_off[rate_ss_shift + 2],
								  dm_soml_table->num_ht_bytes_off[rate_ss_shift + 3],
								  dm_soml_table->num_ht_bytes_off[rate_ss_shift + 4],
								  dm_soml_table->num_ht_bytes_off[rate_ss_shift + 5],
								  dm_soml_table->num_ht_bytes_off[rate_ss_shift + 6],
								  dm_soml_table->num_ht_bytes_off[rate_ss_shift + 7]);
						}
						...
						PHYDM_DBG(dm, DBG_ADPTV_SOML,
							  ...
							  dm_soml_table->num_vht_qam[BPSK_QPSK], dm_soml_table->num_vht_qam[QAM16],
							  dm_soml_table->num_vht_qam[QAM64], dm_soml_table->num_vht_qam[QAM256],
							  num_total_qam);
						if (((dm_soml_table->num_vht_qam[QAM256] * 100) > (num_total_qam *
								dm_soml_table->qam256_dist_th))
						    && (dm_soml_table->cfo_diff_avg_a > dm_soml_table->cfo_qam256_th)
						    && (dm_soml_table->cfo_diff_avg_b > dm_soml_table->cfo_qam256_th)) {
							PHYDM_DBG(dm, DBG_ADPTV_SOML,
							...
							return;
						} else if (((dm_soml_table->num_vht_qam[QAM64] * 100) >
							    (num_total_qam * dm_soml_table->qam64_dist_th))
							   && (dm_soml_table->cfo_diff_avg_a > dm_soml_table->cfo_qam64_th)
							   && (dm_soml_table->cfo_diff_avg_b > dm_soml_table->cfo_qam64_th)) {
							PHYDM_DBG(dm, DBG_ADPTV_SOML,
							...
							phydm_soml_on_off(dm, SOML_OFF);
							return;
						} else if (((dm_soml_table->num_vht_qam[QAM16] * 100) >
							    (num_total_qam * dm_soml_table->qam16_dist_th))
							   && (dm_soml_table->cfo_diff_avg_a > dm_soml_table->cfo_qam16_th)
							   && (dm_soml_table->cfo_diff_avg_b > dm_soml_table->cfo_qam16_th)) {
							PHYDM_DBG(dm, DBG_ADPTV_SOML,
							...
							phydm_soml_on_off(dm, SOML_OFF);
							return;
						} else if (((dm_soml_table->num_vht_qam[BPSK_QPSK] * 100) >
							    (num_total_qam * dm_soml_table->bpsk_qpsk_dist_th))
							   && (dm_soml_table->cfo_diff_avg_a > dm_soml_table->cfo_qpsk_th)
							   && (dm_soml_table->cfo_diff_avg_b > dm_soml_table->cfo_qpsk_th)) {
							PHYDM_DBG(dm, DBG_ADPTV_SOML,
						...
						for (i = 0; i < rate_num; i++) {
							rate_ss_shift = 10 * i;
							PHYDM_DBG(dm, DBG_ADPTV_SOML,
								  "[  num_vht_bytes_on  VHT-%d ss MCS[0:9] = {%d, %d, %d, %d, %d, %d, %d, %d, %d, %d} ]\n",
								  (i + 1),
								  dm_soml_table->num_vht_bytes_on[rate_ss_shift + 0],
								  dm_soml_table->num_vht_bytes_on[rate_ss_shift + 1],
								  dm_soml_table->num_vht_bytes_on[rate_ss_shift + 2],
								  dm_soml_table->num_vht_bytes_on[rate_ss_shift + 3],
								  dm_soml_table->num_vht_bytes_on[rate_ss_shift + 4],
								  dm_soml_table->num_vht_bytes_on[rate_ss_shift + 5],
								  dm_soml_table->num_vht_bytes_on[rate_ss_shift + 6],
								  dm_soml_table->num_vht_bytes_on[rate_ss_shift + 7],
								  dm_soml_table->num_vht_bytes_on[rate_ss_shift + 8],
								  dm_soml_table->num_vht_bytes_on[rate_ss_shift + 9]);
						}

						for (i = 0; i < rate_num; i++) {
							rate_ss_shift = 10 * i;
							PHYDM_DBG(dm, DBG_ADPTV_SOML,
								  "[  num_vht_bytes_off  VHT-%d ss MCS[0:9] = {%d, %d, %d, %d, %d, %d, %d, %d, %d, %d} ]\n",
								  (i + 1),
								  dm_soml_table->num_vht_bytes_off[rate_ss_shift + 0],
								  dm_soml_table->num_vht_bytes_off[rate_ss_shift + 1],
								  dm_soml_table->num_vht_bytes_off[rate_ss_shift + 2],
								  dm_soml_table->num_vht_bytes_off[rate_ss_shift + 3],
								  dm_soml_table->num_vht_bytes_off[rate_ss_shift + 4],
								  dm_soml_table->num_vht_bytes_off[rate_ss_shift + 5],
								  dm_soml_table->num_vht_bytes_off[rate_ss_shift + 6],
								  dm_soml_table->num_vht_bytes_off[rate_ss_shift + 7],
								  dm_soml_table->num_vht_bytes_off[rate_ss_shift + 8],
								  dm_soml_table->num_vht_bytes_off[rate_ss_shift + 9]);
						}
				...
				PHYDM_DBG(dm, DBG_ADPTV_SOML,
					  "[escape from > TH_H || is_soml_method_enable==1]\n");
