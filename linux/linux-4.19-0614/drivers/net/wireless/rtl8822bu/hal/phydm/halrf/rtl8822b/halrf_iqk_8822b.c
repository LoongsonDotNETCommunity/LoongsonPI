...
void phydm_get_read_counter(struct dm_struct *dm)
{
	...
	while (1) {
		if ((odm_get_rf_reg(dm, RF_PATH_A, 0x8, RFREGOFFSETMASK) == 0xabcde)
		    || (counter > 300))
			break;
		counter++;
		ODM_delay_ms(1);
...
)
{
	...
	for (i = 0; i < SS_8822B; i++) {
		...
		tmp2 = tmp2 + ((iqk_info->iqk_fail_report[channel][i][RX_IQK] & 0x1) <<
			       (i + 4));
...
void _iqk_iqk_fail_report_8822b(
	struct dm_struct	*dm
)
{
	u32		tmp1bf0 = 0x0;
...
)
{
	...
	for (i = 0; i < RF_REG_NUM_8822B; i++) {
		RF_backup[i][RF_PATH_A] = odm_get_rf_reg(dm, RF_PATH_A, backup_rf_reg[i],
					  RFREGOFFSETMASK);
		RF_backup[i][RF_PATH_B] = odm_get_rf_reg(dm, RF_PATH_B, backup_rf_reg[i],
					  RFREGOFFSETMASK);
	}
...
)
{
	...
	while (1) {
		...
		if (cca_ing) {
			...
			odm_set_bb_reg(dm, 0xa04, BIT(27) | BIT(26) | BIT(25) | BIT(24),
				       0x0);		/*CCK RX path off*/
...
void _iqk_restore_mac_bb_8822b(
	struct dm_struct		*dm,
	u32		*MAC_backup,
	u32		*BB_backup,
	u32		*backup_mac_reg,
...
)
{
	...
	/*0xdf[4]=0*/
	_iqk_rf_set_check(dm, RF_PATH_A, 0xdf,
			  RF_backup[0][RF_PATH_A] & (~BIT(4)));
	_iqk_rf_set_check(dm, RF_PATH_B, 0xdf,
	...
	for (i = 1; i < RF_REG_NUM_8822B; i++) {
		...
		odm_set_rf_reg(dm, RF_PATH_B, backup_rf_reg[i], RFREGOFFSETMASK,
...
)
{
	...
	case 0:
		...
		break;
	case 1: /*LOK backup*/
		iqk_info->lok_idac[0][path] = odm_get_rf_reg(dm, (enum rf_path)path, 0x58,
					      RFREGOFFSETMASK);
		break;
	case 2:	/*TXIQK backup*/
	case 3: /*RXIQK backup*/
...
)
{
	...
	for (path = 0; path < 2; path++) {
		if (reload_idx == 2) {
			...
			tmp = odm_get_rf_reg(dm, (enum rf_path)path, 0xdf,
					     RFREGOFFSETMASK) | BIT(4);
			_iqk_rf_set_check(dm, (enum rf_path)path, 0xdf, tmp);
			odm_set_rf_reg(dm, (enum rf_path)path, 0x58, RFREGOFFSETMASK,
		...
		for (idx = 0; idx < reload_idx; idx++) {
			...
			odm_set_bb_reg(dm, 0x1bd4, BIT(20) | BIT(19) | BIT(18) | BIT(17) | BIT(16),
				       0x10);
			for (i = 0; i < 8; i++) {
				odm_write_4byte(dm, 0x1bd8,
						((0xc0000000 >> idx) + 0x3) + (i * 4) +
						(iqk_info->iqk_cfir_real[channel][path][idx][i] << 9));
				odm_write_4byte(dm, 0x1bd8,
						((0xc0000000 >> idx) + 0x1) + (i * 4) +
						(iqk_info->iqk_cfir_imag[channel][path][idx][i] << 9));
			}
			if (idx == 0)
				odm_set_bb_reg(dm, iqk_apply[path], BIT(0),
					       !(iqk_info->iqk_fail_report[channel][path][idx]));
			else
				odm_set_bb_reg(dm, iqk_apply[path], BIT(10),
					       !(iqk_info->iqk_fail_report[channel][path][idx]));
		}
...
void _iqk_configure_macbb_8822b(
	struct dm_struct		*dm
)
{
...
void halrf_iqk_check_if_reload(
	struct dm_struct	*dm
)
{
	struct dm_iqk_info	*iqk_info = &dm->IQK_info;
...
)
{
	...
	while (notready) {
		if (odm_get_rf_reg(dm, (enum rf_path)path, 0x8,
...
boolean _iqk_rx_iqk_gain_search_fail_8822b(
	struct dm_struct			*dm,
	u8		path,
	u8		step
)
{
	...
	if (step == RXIQK1) {
		...
		if (fail) {
			odm_write_4byte(dm, 0x1b00, 0xf8000008 | path << 1);
			odm_write_4byte(dm, 0x1b24, (odm_read_4byte(dm,
						     0x1b24) & 0xffffe3ff) | (iqk_info->lna_idx << 10));
		}
	}
	return fail;
}

boolean _lok_one_shot_8822b(
	void		*dm_void,
	u8			path
)
{
...
)
{
	...
	if (idx == TXIQK) {
		...
		odm_write_4byte(dm, 0x1b00, 0xf8000008 | path << 1);
		odm_write_4byte(dm, 0x1b24, (odm_read_4byte(dm,
					     0x1b24) & 0xffffe3ff) | ((iqk_info->lna_idx & 0x7) << 10));
	}
	odm_write_4byte(dm, 0x1b00, IQK_CMD);
	odm_write_4byte(dm, 0x1b00, IQK_CMD + 0x1);
	...
	if (idx == RXIQK2) {
		iqk_info->rxiqk_agc[0][path] =
			(u16)(((odm_get_rf_reg(dm, (enum rf_path)path, 0x0,
					       RFREGOFFSETMASK) >> 5) & 0xff) |
			      (iqk_info->tmp1bcc << 8));
...
boolean _iqk_rx_iqk_by_path_8822b(
	void		*dm_void,
	u8		path
)
{
...
boolean _iqk_rximr_rxk1_test_8822b(
	struct dm_struct			*dm,
	u8		path,
	u32      tone_index
)
{
	...
	odm_write_4byte(dm, 0x1b00, 0xf8000008 | path << 1);
	odm_write_4byte(dm, 0x1b20, (odm_read_4byte(dm,
				     0x1b20) & 0x000fffff) | ((tone_index & 0xfff) << 20));
	odm_write_4byte(dm, 0x1b24, (odm_read_4byte(dm,
				     0x1b24) & 0x000fffff) | ((tone_index & 0xfff) << 20));
...
u32 _iqk_tximr_selfcheck_8822b(
	void *dm_void,
	u8 tone_index,
	u8 path
)
...
void _iqk_start_tximr_test_8822b(
	struct	dm_struct	*dm,
	u8 imr_limit
)
{
...
u32 _iqk_rximr_selfcheck_8822b(
	void *dm_void,
	u32 tone_index,
	u8 path,
	u32 tmp1b38
					...
					while (count < 3) {
						...
						if (kfail) {
							...
							if (count == 3) {
								...
								PHYDM_DBG(dm, ODM_COMP_CALIBRATION,
									  "[IQK]path = %x, toneindex = %x rxk1 fail\n", path, tone_index);
							}
						} else {
							odm_write_4byte(dm, 0x1b00, 0xf8000008 | path << 1);
							odm_write_4byte(dm, 0x1b1c, 0xa2193c32);
							...
							PHYDM_DBG(dm, ODM_COMP_CALIBRATION,
								  "[IQK]path = 0x%x, tone_idx = 0x%x, tmp1b38 = 0x%x\n", path, tone_index,
								  temp1b38[side][i]);
							break;
						}
					}
			...
			for (side = 0; side < 2; side++) {
				for (i = 0x0; i < imr_limit; i++) {
					if (side == 0)
						...
						tone_index = 0x08 | (i << 4);
					_iqk_rxk2_setting_8822b(dm, path, false);
					imr_result = _iqk_rximr_selfcheck_8822b(dm, tone_index, path,
										temp1b38[side][i]);
					PHYDM_DBG(dm, ODM_COMP_CALIBRATION,
						  "[IQK]tone_idx = 0x%5x, freq = %s%10s, RXIMR = %5d dB\n", tone_index,
						  (side == 0) ? "-" : " ", freq[i], imr_result);
				}
				odm_write_4byte(dm, 0x1b00, 0xf8000008 | path << 1);
...
void _iqk_start_rximr_test_8822b(
	struct	dm_struct	*dm,
	u8 imr_limit
)
{
...
)
{
	...
	PHYDM_DBG(dm, ODM_COMP_CALIBRATION,
		  ...
		  (*dm->band_type == ODM_BAND_5G) ? "5G" : "2G", *dm->band_width, dm->ext_pa,
		  dm->ext_pa_5g);
	PHYDM_DBG(dm, ODM_COMP_CALIBRATION,
		  "[IQK]Interface = %d, cut_version = %x\n", dm->support_interface,
		  dm->cut_version);
	...
	while (1) {
		...
		_iqk_restore_mac_bb_8822b(dm, MAC_backup, BB_backup, backup_mac_reg,
					  backup_bb_reg);
