...
)
{
	...
	if (!(dm->support_ic_type & (ODM_RTL8814A | ODM_IC_11N_SERIES |
				     ODM_RTL8822B)))
		return;
#endif
...
)
{
	...
	if (dm->support_ic_type == ODM_RTL8188E
		...
		bb_swing = PHY_GetTxBBSwing_8812A((PADAPTER)adapter,
						  hal_data->CurrentBandType, RF_PATH_A);
...
)
{
	...
	if (cali_info->default_bb_swing_index_flag != true) {
		...
		if (dm->support_ic_type == ODM_RTL8188E
		    ...
		    dm->support_ic_type == ODM_RTL8192E
		    || dm->support_ic_type == ODM_RTL8703B) {
			cali_info->default_ofdm_index = (default_swing_index >= OFDM_TABLE_SIZE) ?
							30 : default_swing_index;
			cali_info->default_cck_index = (default_cck_swing_index >= CCK_TABLE_SIZE)
						       ? 20 : default_cck_swing_index;
			...
			cali_info->default_ofdm_index = (default_swing_index >= TXSCALE_TABLE_SIZE)
...
void odm_txpowertracking_direct_call(
	void		*adapter
)
{
	HAL_DATA_TYPE		*hal_data	= GET_HAL_DATA(((PADAPTER)adapter));
...
)
{
	...
	if (!tm_trigger) {
		if (IS_HARDWARE_TYPE_8188E(adapter) || IS_HARDWARE_TYPE_JAGUAR(adapter)
		    ...
		    || IS_HARDWARE_TYPE_8188F(adapter) || IS_HARDWARE_TYPE_8703B(adapter)
		    || IS_HARDWARE_TYPE_8822B(adapter) || IS_HARDWARE_TYPE_8723D(adapter)
		    || IS_HARDWARE_TYPE_8821C(adapter)
		    || IS_HARDWARE_TYPE_8710B(adapter))/* JJ ADD 20161014 */
			PHY_SetRFReg((PADAPTER)adapter, RF_PATH_A, RF_T_METER_88E,
				     BIT(17) | BIT(16), 0x03);
		else
			PHY_SetRFReg((PADAPTER)adapter, RF_PATH_A, RF_T_METER, RFREGOFFSETMASK,
				     0x60);
