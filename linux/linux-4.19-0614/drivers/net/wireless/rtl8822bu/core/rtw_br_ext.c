...
static __inline__ unsigned char *__nat25_find_pppoe_tag(
	struct pppoe_hdr *ph, unsigned short type)
{
...
static int skb_pull_and_merge(struct sk_buff *skb, unsigned char *src,
			      int len)
{
	int tail_len;
...
static __inline__ void __nat25_generate_ipv4_network_addr(
...
static __inline__ void __nat25_generate_ipx_network_addr_with_node(
	unsigned char *networkAddr,
...
static __inline__ void __nat25_generate_ipx_network_addr_with_socket(
	unsigned char *networkAddr,
...
static __inline__ void __nat25_generate_apple_network_addr(
	unsigned char *networkAddr,
...
static __inline__ void __nat25_generate_pppoe_network_addr(
	unsigned char *networkAddr,
...
static unsigned char *scan_tlv(unsigned char *data, int len,
			       unsigned char tag, unsigned char len8b)
{
	while (len > 0) {
		...
		if (len >= 8) {
			...
			if (mac) {
				RTW_INFO("Router Solicitation, replace MAC From: %02x:%02x:%02x:%02x:%02x:%02x, To: %02x:%02x:%02x:%02x:%02x:%02x\n",
					 ...
					 replace_mac[0], replace_mac[1], replace_mac[2], replace_mac[3],
...
static __inline__ int __nat25_network_hash(unsigned char *networkAddr)
{
	if (networkAddr[0] == NAT25_IPV4) {
		...
		x = networkAddr[1] ^ networkAddr[2] ^ networkAddr[3] ^ networkAddr[4] ^
		    networkAddr[5] ^
		    networkAddr[6] ^ networkAddr[7] ^ networkAddr[8] ^ networkAddr[9] ^
		    networkAddr[10];
		...
		x = networkAddr[0] ^ networkAddr[1] ^ networkAddr[2] ^ networkAddr[3] ^
		    networkAddr[4] ^ networkAddr[5] ^ networkAddr[6] ^ networkAddr[7] ^
		    networkAddr[8];
		...
		x = networkAddr[1] ^ networkAddr[2] ^ networkAddr[3] ^ networkAddr[4] ^
		    ...
		    networkAddr[6] ^ networkAddr[7] ^ networkAddr[8] ^ networkAddr[9] ^
		    networkAddr[10] ^
		    networkAddr[11] ^ networkAddr[12] ^ networkAddr[13] ^ networkAddr[14] ^
		    networkAddr[15] ^
		    networkAddr[16];
...
static __inline__ void __network_hash_unlink(struct nat25_network_db_entry
		*ent)
{
	/* Caller must _enter_critical_bh already! */
	/* _irqL irqL; */
...
static int checkIPMcAndReplace(_adapter *priv, struct sk_buff *skb,
			       unsigned int *dst_ip)
{
	...
	while (plist != phead) {
		...
		for (i = 0; i < MAX_IP_MC_ENTRY; i++) {
			if (pstat->ipmc[i].used
			    && !memcmp(&pstat->ipmc[i].mcmac[3], ((unsigned char *)dst_ip) + 1, 3)) {
...
int nat25_db_handle(_adapter *priv, struct sk_buff *skb, int method)
{
	...
	if (protocol == __constant_htons(ETH_P_IP)) {
		...
		if (((unsigned char *)(iph) + (iph->ihl << 2)) >= (skb->data + ETH_HLEN +
				skb->len)) {
	...
	/*---------------------------------------------------*/
		...
		if (ipx != NULL) {
			...
			case NAT25_INSERT: {
				...
				if (!memcmp(skb->data + ETH_ALEN, ipx->ipx_source.node, ETH_ALEN)) {
					...
					__nat25_generate_ipx_network_addr_with_socket(networkAddr,
							&ipx->ipx_source.net, &ipx->ipx_source.sock);

					/* change IPX source node addr to wlan STA address */
					memcpy(ipx->ipx_source.node, GET_MY_HWADDR(priv), ETH_ALEN);
				} else
					__nat25_generate_ipx_network_addr_with_node(networkAddr,
							&ipx->ipx_source.net, ipx->ipx_source.node);
			...
			case NAT25_LOOKUP: {
				if (!memcmp(GET_MY_HWADDR(priv), ipx->ipx_dest.node, ETH_ALEN)) {
					...
					__nat25_generate_ipx_network_addr_with_socket(networkAddr,
							&ipx->ipx_dest.net, &ipx->ipx_dest.sock);
					...
					__nat25_generate_ipx_network_addr_with_node(networkAddr,
							&ipx->ipx_dest.net, ipx->ipx_dest.node);
		...
		/*   AARP  */
			...
			case NAT25_INSERT: {
				...
				__nat25_generate_apple_network_addr(networkAddr, &ea->pa_src_net,
								    &ea->pa_src_node);
			...
			case NAT25_LOOKUP: {
				...
				__nat25_generate_apple_network_addr(networkAddr, &ea->pa_dst_net,
								    &ea->pa_dst_node);
		...
		/*   DDP  */
			...
			case NAT25_INSERT: {
				...
				__nat25_generate_apple_network_addr(networkAddr, &ddp->deh_snet,
								    &ddp->deh_snode);
			...
			case NAT25_LOOKUP: {
				...
				__nat25_generate_apple_network_addr(networkAddr, &ddp->deh_dnet,
								    &ddp->deh_dnode);
			...
			if (ph->sid == 0) {	/* Discovery phase according to tag */
				if (ph->code == PADI_CODE || ph->code == PADR_CODE) {
					if (priv->ethBrExtInfo.addPPPoETag) {
						...
						pOldTag = (struct pppoe_tag *)__nat25_find_pppoe_tag(ph,
								ntohs(PTT_RELAY_SID));
						if (pOldTag) { /* if SID existed, copy old value and delete it */
							old_tag_len = ntohs(pOldTag->tag_len);
							if (old_tag_len + TAG_HDR_LEN + MAGIC_CODE_LEN + RTL_RELAY_TAG_LEN >
							    sizeof(tag_buf)) {
			...
			if (ph->code == PADO_CODE || ph->code == PADS_CODE) {
				if (priv->ethBrExtInfo.addPPPoETag) {
					...
					if ((tagType != ntohs(PTT_RELAY_SID))
					    || (tagLen < (MAGIC_CODE_LEN + RTL_RELAY_TAG_LEN))) {
					...
					if (skb_pull_and_merge(skb, ptr + offset,
							       TAG_HDR_LEN + MAGIC_CODE_LEN + RTL_RELAY_TAG_LEN - offset) < 0) {
						DEBUG_ERR("call skb_pull_and_merge() failed in PADO packet!\n");
						return -1;
					}
					ph->length = htons(ntohs(ph->length) - (TAG_HDR_LEN + MAGIC_CODE_LEN +
				...
				if (ph->sid != 0) {
					RTW_INFO("NAT25: Lookup PPPoE, lookup session packet from %s\n",
						 skb->dev->name);
					__nat25_generate_pppoe_network_addr(networkAddr, skb->data + ETH_ALEN,
									    &(ph->sid));
	...
	/*---------------------------------------------------*/
		...
		case NAT25_INSERT: {
			RTW_INFO("NAT25: Insert IP, SA=%4x:%4x:%4x:%4x:%4x:%4x:%4x:%4x,"
				 ...
				 iph->saddr.s6_addr16[0], iph->saddr.s6_addr16[1], iph->saddr.s6_addr16[2],
				 iph->saddr.s6_addr16[3],
				 iph->saddr.s6_addr16[4], iph->saddr.s6_addr16[5], iph->saddr.s6_addr16[6],
				 iph->saddr.s6_addr16[7],
				 iph->daddr.s6_addr16[0], iph->daddr.s6_addr16[1], iph->daddr.s6_addr16[2],
				 iph->daddr.s6_addr16[3],
				 iph->daddr.s6_addr16[4], iph->daddr.s6_addr16[5], iph->daddr.s6_addr16[6],
				 iph->daddr.s6_addr16[7]);

			if (memcmp(&iph->saddr, "\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0",
				   16)) {
				__nat25_generate_ipv6_network_addr(networkAddr,
				...
				if (iph->nexthdr == IPPROTO_ICMPV6 &&
					...
					if (update_nd_link_layer_addr(skb->data + ETH_HLEN + sizeof(*iph),
						...
						struct icmp6hdr  *hdr = (struct icmp6hdr *)(skb->data + ETH_HLEN + sizeof(
										*iph));
						hdr->icmp6_cksum = 0;
...
int nat25_handle_frame(_adapter *priv, struct sk_buff *skb)
{
	...
	if (!(skb->data[0] & 1)) {
		...
		if (*((unsigned short *)(skb->data + ETH_ALEN * 2)) == __constant_htons(
			    ETH_P_8021Q)) {
			is_vlan_tag = 1;
		...
		if (!priv->ethBrExtInfo.nat25_disable) {
			...
			if (!priv->ethBrExtInfo.nat25sc_disable &&
			    (*((unsigned short *)(skb->data + ETH_ALEN * 2)) == __constant_htons(
				     ETH_P_IP)) &&
