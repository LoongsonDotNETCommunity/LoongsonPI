...
static const struct country_chplan
	RTL8821AE_HMC_M2_country_chplan_exc_map[] = {
...
static const struct country_chplan
	RTL8812AENF_NGFF_country_chplan_exc_map[] = {
	COUNTRY_CHPLAN_ENT("TW", 0x39, 1, 0), /* Taiwan */
	COUNTRY_CHPLAN_ENT("US", 0x34, 1, 0), /* United States of America (USA) */
};
#endif
...
static const struct country_chplan
	RTL8188EE_HMC_M2_country_chplan_exc_map[] = {
...
static const struct country_chplan
	RTL8723BE_HMC_M2_country_chplan_exc_map[] = {
	COUNTRY_CHPLAN_ENT("CA", 0x20, 1, 0), /* Canada */
	COUNTRY_CHPLAN_ENT("MX", 0x34, 1, 0), /* Mexico */
	COUNTRY_CHPLAN_ENT("TW", 0x39, 1, 0), /* Taiwan */
	COUNTRY_CHPLAN_ENT("US", 0x34, 1, 0), /* United States of America (USA) */
...
static const struct country_chplan
	RTL8723BS_NGFF1216_country_chplan_exc_map[] = {
	COUNTRY_CHPLAN_ENT("CA", 0x20, 1, 0), /* Canada */
	COUNTRY_CHPLAN_ENT("MX", 0x34, 1, 0), /* Mexico */
	COUNTRY_CHPLAN_ENT("TW", 0x39, 1, 0), /* Taiwan */
	COUNTRY_CHPLAN_ENT("US", 0x34, 1, 0), /* United States of America (USA) */
...
static const struct country_chplan
	RTL8192EEBT_HMC_M2_country_chplan_exc_map[] = {
	COUNTRY_CHPLAN_ENT("CA", 0x20, 1, 0), /* Canada */
	COUNTRY_CHPLAN_ENT("MX", 0x34, 1, 0), /* Mexico */
	COUNTRY_CHPLAN_ENT("TW", 0x39, 1, 0), /* Taiwan */
	COUNTRY_CHPLAN_ENT("US", 0x34, 1, 0), /* United States of America (USA) */
...
static const struct country_chplan
	RTL8723DE_NGFF1630_country_chplan_exc_map[] = {
 ...
 */
static const struct country_chplan *rtw_def_module_get_chplan_from_country(
...
*/
const struct country_chplan *rtw_get_chplan_from_country(
	const char *country_code)
{
#if RTW_DEF_MODULE_REGULATORY_CERT
	...
	map_sz = sizeof(CUSTOMIZED_country_chplan_map) / sizeof(
			 struct country_chplan);
#else
#if RTW_DEF_MODULE_REGULATORY_CERT
...
exit:
#if RTW_DEF_MODULE_REGULATORY_CERT
	if (!ent || !(COUNTRY_CHPLAN_DEF_MODULE_FALGS(ent) &
...
void rtw_regd_exc_add_with_nlen(struct rf_ctl_t *rfctl,
				const char *country, u8 domain, const char *regd_name, u32 nlen)
{
	struct regd_exc_ent *ent;
	_irqL irqL;
	...
	ent = (struct regd_exc_ent *)rtw_zmalloc(sizeof(struct regd_exc_ent) + nlen
			+ 1);
	if (!ent)
		goto exit;
...
inline void rtw_regd_exc_add(struct rf_ctl_t *rfctl, const char *country,
			     u8 domain, const char *regd_name)
{
	rtw_regd_exc_add_with_nlen(rfctl, country, domain, regd_name,
				   strlen(regd_name));
}
...
inline struct regd_exc_ent *rtw_regd_exc_search(struct rf_ctl_t *rfctl,
		const char *country, u8 domain)
{
	struct regd_exc_ent *ent;
	_irqL irqL;
...
void rtw_regd_exc_list_free(struct rf_ctl_t *rfctl)
{
	...
	while ((rtw_end_of_queue_search(head, cur)) == _FALSE) {
		...
		rtw_mfree((u8 *)ent, sizeof(struct regd_exc_ent) + strlen(
				  ent->regd_name) + 1);
	}
...
#define TMP_STR_LEN 16
	...
	for (band = BAND_ON_2_4G; band <= BAND_ON_5G; band++) {
		...
		rfpath_num = (band == BAND_ON_2_4G ? hal_spec->rfpath_num_2g :
			...
			for (tlrs = TXPWR_LMT_RS_CCK; tlrs < TXPWR_LMT_RS_NUM; tlrs++) {
				...
				for (ntx_idx = RF_1TX; ntx_idx < MAX_TX_COUNT; ntx_idx++) {
					...
					while ((rtw_end_of_queue_search(head, cur)) == _FALSE) {
						...
						sprintf(fmt, "%%%zus%%s ",
							strlen(ent->regd_name) < 4 ? 5 - strlen(ent->regd_name) : 1);
					...
					sprintf(fmt, "%%%zus%%s ",
						strlen(regd_str(TXPWR_LMT_WW)) < 4 ? 5 - strlen(regd_str(
									TXPWR_LMT_WW)) : 1);
					snprintf(tmp_str, TMP_STR_LEN, fmt
					...
					for (path = 0; path < RF_PATH_MAX; path++) {
						...
						while ((rtw_end_of_queue_search(head, cur)) == _FALSE) {
							...
							_RTW_PRINT_SEL(sel, "%3c "
								       , strcmp(ent->regd_name,
										rfctl->regd_name) == 0 ? rf_path_char(path) : ' ');
						}
						_RTW_PRINT_SEL(sel, "%3c "
							       , strcmp(rfctl->regd_name,
									regd_str(TXPWR_LMT_WW)) == 0 ? rf_path_char(path) : ' ');
					}
					_RTW_PRINT_SEL(sel, "\n");
					...
					for (n = 0; n < ch_num; n++) {
						...
						while ((rtw_end_of_queue_search(head, cur)) == _FALSE) {
							ent = LIST_CONTAINOR(cur, struct txpwr_lmt_ent, list);
							cur = get_next(cur);
							lmt = phy_get_txpwr_lmt_abs(adapter, ent->regd_name, band, bw, tlrs,
										    ntx_idx, ch, 0);
							if (lmt == MAX_POWER_INDEX) {
								sprintf(fmt, "%%%zus ",
									strlen(ent->regd_name) >= 5 ? strlen(ent->regd_name) + 1 : 5);
								...
								if (lmt == -1) { /* -0.5 */
									...
									sprintf(fmt, "%%%zud.5 ",
										strlen(ent->regd_name) >= 5 ? strlen(ent->regd_name) - 1 : 3);
						...
						lmt = phy_get_txpwr_lmt_abs(adapter, regd_str(TXPWR_LMT_WW), band, bw,
									    tlrs, ntx_idx, ch, 0);
						if (lmt == MAX_POWER_INDEX) {
							sprintf(fmt, "%%%zus ",
								strlen(regd_str(TXPWR_LMT_WW)) >= 5 ? strlen(regd_str(
											TXPWR_LMT_WW)) + 1 : 5);
							snprintf(tmp_str, TMP_STR_LEN, fmt, "NA");
							_RTW_PRINT_SEL(sel, "%s", tmp_str);
						} else {
						...
						for (path = RF_PATH_A; path < RF_PATH_MAX; path++) {
							...
							while ((rtw_end_of_queue_search(head, cur)) == _FALSE) {
								ent = LIST_CONTAINOR(cur, struct txpwr_lmt_ent, list);
								cur = get_next(cur);
								lmt_offset = phy_get_txpwr_lmt(adapter, ent->regd_name, band, bw, path, rs,
											       ntx_idx, ch, 0);
							...
							lmt_offset = phy_get_txpwr_lmt(adapter, regd_str(TXPWR_LMT_WW), band, bw,
										       path, rs, ntx_idx, ch, 0);
							if (lmt_offset == MAX_POWER_INDEX)
								_RTW_PRINT_SEL(sel, "%3s ", "NA");
							else
								_RTW_PRINT_SEL(sel, "%3d ", lmt_offset);
						...
						if (rfpath_num > 1) {
							for (i = 0; i < rfctl->txpwr_regd_num; i++) {
								for (path = 0; path < RF_PATH_MAX; path++) {
									if (path >= rfpath_num)
										break;
									if (*(lmt_idx + i * RF_PATH_MAX + path) != *(lmt_idx + i * RF_PATH_MAX + ((
												path + 1) % rfpath_num)))
										break;
								}
								if (path >= rfpath_num)
...
void rtw_txpwr_lmt_add_with_nlen(struct rf_ctl_t *rfctl,
				 const char *regd_name, u32 nlen
				 , u8 band, u8 bw, u8 tlrs, u8 ntx_idx, u8 ch_idx, s8 lmt)
{
	...
	/* alloc new one */
	ent = (struct txpwr_lmt_ent *)rtw_zvmalloc(sizeof(struct txpwr_lmt_ent) +
			nlen + 1);
	if (!ent)
		goto release_lock;
...
chk_lmt_val:
	...
	if (pre_lmt != MAX_POWER_INDEX)
		RTW_PRINT("duplicate txpwr_lmt for [%s][%s][%s][%s][%uT][%d]\n"
			  , regd_name, band_str(band), ch_width_str(bw), txpwr_lmt_rs_str(tlrs),
			  ntx_idx + 1
...
inline struct txpwr_lmt_ent *rtw_txpwr_lmt_get_by_name(
	struct rf_ctl_t *rfctl, const char *regd_name)
{
	struct txpwr_lmt_ent *ent;
	_irqL irqL;
...
void rtw_txpwr_lmt_list_free(struct rf_ctl_t *rfctl)
{
	...
	while ((rtw_end_of_queue_search(head, cur)) == _FALSE) {
		...
		rtw_list_delete(&ent->list);
		rtw_vmfree((u8 *)ent, sizeof(struct txpwr_lmt_ent) + strlen(
				   ent->regd_name) + 1);
	}
...
void rtw_rf_set_tx_gain_offset(_adapter *adapter, u8 path, s8 offset)
{
	...
	if (IS_HARDWARE_TYPE_8723D(adapter)) {
		...
		target_path = path;
		RTW_INFO("kfree gain_offset 0x55:0x%x ", rtw_hal_read_rfreg(adapter,
				target_path, 0x55, 0xffffffff));
	}
...
void rtw_rf_apply_tx_gain_offset(_adapter *adapter, u8 ch)
{
	...
	s8 kfree_offset = 0;
	s8 tx_pwr_track_offset =
		0; /* TODO: 8814A should consider tx pwr track when setting tx gain offset */
	s8 total_offset;
	int i, total = 0;
...
u8 rtw_is_dfs_ch(u8 ch)
{
	...
	if (!rtw_chbw_to_freq_range(ch, CHANNEL_WIDTH_20,
				    HAL_PRIME_CHNL_OFFSET_DONT_CARE, &hi, &lo))
		return 0;
...
{
	return (dfs_region == PHYDM_DFS_DOMAIN_ETSI
		&& rtw_is_range_overlap(hi, lo, 5650, 5600)) ? _TRUE : _FALSE;
}
