...
s32	_rtw_init_xmit_priv(struct xmit_priv *pxmitpriv, _adapter *padapter)
{
	...
	*/
	...
	pxmitpriv->xframe_ext_alloc_addr = rtw_zvmalloc(NR_XMIT_EXTBUFF * sizeof(
			struct xmit_frame) + 4);
	...
	pxmitpriv->pallocated_xmit_extbuf = rtw_zvmalloc(NR_XMIT_EXTBUFF * sizeof(
			struct xmit_buf) + 4);
	...
	for (i = 0; i < NR_XMIT_EXTBUFF; i++) {
		...
		res = rtw_os_xmit_resource_alloc(padapter, pxmitbuf,
						 MAX_XMIT_EXTBUF_SZ + XMITBUF_ALIGN_SZ, _TRUE);
		if (res == _FAIL) {
			res = _FAIL;
			goto exit;
		}
...
void _rtw_free_xmit_priv(struct xmit_priv *pxmitpriv)
{
	...
	if (pxmitpriv->xframe_ext_alloc_addr)
		rtw_vmfree(pxmitpriv->xframe_ext_alloc_addr,
			   NR_XMIT_EXTBUFF * sizeof(struct xmit_frame) + 4);
...
void rtw_get_adapter_tx_rate_bmp_by_bw(_adapter *adapter, u8 bw,
				       u16 *r_bmp_cck_ofdm, u32 *r_bmp_ht, u32 *r_bmp_vht)
{
	...
	for (i = 0; i < macid_ctl->num; i++) {
		...
		if ((fix_bw != 0xFF && fix_bw != bw) || (fix_bw == 0xFF
				&& macid_ctl->bw[i] != bw))
			continue;

		if (macid_ctl->vht_en[i])
			bmp_vht |= (macid_ctl->rate_bmp0[i] >> 12) | (macid_ctl->rate_bmp1[i] <<
					20);
		else
			bmp_ht |= (macid_ctl->rate_bmp0[i] >> 12) | (macid_ctl->rate_bmp1[i] <<
					20);
	}
...
void rtw_get_shared_macid_tx_rate_bmp_by_bw(struct dvobj_priv *dvobj,
		u8 bw, u16 *r_bmp_cck_ofdm, u32 *r_bmp_ht, u32 *r_bmp_vht)
{
	struct macid_ctl_t *macid_ctl = dvobj_to_macidctl(dvobj);
	u16 bmp_cck_ofdm = 0;
	u32 bmp_ht = 0;
		...
		if (hal_is_bw_support(dvobj_get_primary_adapter(dvobj), bw)) {
			for (i = 0; i < dvobj->iface_nums; i++) {
				if (!dvobj->padapters[i])
					continue;
				rtw_get_adapter_tx_rate_bmp_by_bw(dvobj->padapters[i], bw, &tmp_cck_ofdm,
								  &tmp_ht, &tmp_vht);
...

	{
		...
		for (bw = CHANNEL_WIDTH_20; bw <= CHANNEL_WIDTH_40; bw++) {
			if (rf_ctl->rate_bmp_ht_by_bw[highest_rate_bw] <
			    rf_ctl->rate_bmp_ht_by_bw[bw]) {
				highest_rate_bw_bmp = ch_width_to_bw_cap(bw);
				highest_rate_bw = bw;
			} else if (rf_ctl->rate_bmp_ht_by_bw[highest_rate_bw] ==
				   rf_ctl->rate_bmp_ht_by_bw[bw])
				highest_rate_bw_bmp |= ch_width_to_bw_cap(bw);
		}
		...
		if (ori_highest_ht_rate_bw_bmp != rf_ctl->highest_ht_rate_bw_bmp
		    || largest_bit(ori_bmp_ht[highest_rate_bw]) != largest_bit(
			    rf_ctl->rate_bmp_ht_by_bw[highest_rate_bw])
		   ) {
			if (DBG_HIGHEST_RATE_BMP_BW_CHANGE) {
				RTW_INFO("highest_ht_rate_bw_bmp:0x%02x=>0x%02x\n",
					 ori_highest_ht_rate_bw_bmp, rf_ctl->highest_ht_rate_bw_bmp);
				RTW_INFO("rate_bmp_ht_by_bw[%u]:0x%08x=>0x%08x\n", highest_rate_bw,
					 ori_bmp_ht[highest_rate_bw], rf_ctl->rate_bmp_ht_by_bw[highest_rate_bw]);
			}
			update_ht_rs = _TRUE;
		}
		...
		for (bw = CHANNEL_WIDTH_20; bw <= CHANNEL_WIDTH_160; bw++) {
			if (rf_ctl->rate_bmp_vht_by_bw[highest_rate_bw] <
			    rf_ctl->rate_bmp_vht_by_bw[bw]) {
				highest_rate_bw_bmp = ch_width_to_bw_cap(bw);
				highest_rate_bw = bw;
			} else if (rf_ctl->rate_bmp_vht_by_bw[highest_rate_bw] ==
				   rf_ctl->rate_bmp_vht_by_bw[bw])
				highest_rate_bw_bmp |= ch_width_to_bw_cap(bw);
		}
		...
		if (ori_highest_vht_rate_bw_bmp != rf_ctl->highest_vht_rate_bw_bmp
		    || largest_bit(ori_bmp_vht[highest_rate_bw]) != largest_bit(
			    rf_ctl->rate_bmp_vht_by_bw[highest_rate_bw])
		   ) {
			if (DBG_HIGHEST_RATE_BMP_BW_CHANGE) {
				RTW_INFO("highest_vht_rate_bw_bmp:0x%02x=>0x%02x\n",
					 ori_highest_vht_rate_bw_bmp, rf_ctl->highest_vht_rate_bw_bmp);
				RTW_INFO("rate_bmp_vht_by_bw[%u]:0x%08x=>0x%08x\n", highest_rate_bw,
					 ori_bmp_vht[highest_rate_bw], rf_ctl->rate_bmp_vht_by_bw[highest_rate_bw]);
			}
			update_vht_rs = _TRUE;
		}
		...
		if (update_ht_rs == _TRUE || update_vht_rs == _TRUE)
			rtw_hal_set_tx_power_level(dvobj_get_primary_adapter(dvobj),
						   hal_data->current_channel);
	}
}
...
u8 rtw_get_tx_bw_bmp_of_ht_rate(struct dvobj_priv *dvobj, u8 rate,
				u8 max_bw)
{
	struct rf_ctl_t *rf_ctl = dvobj_to_rfctl(dvobj);
	u8 bw;
...
u8 rtw_get_tx_bw_bmp_of_vht_rate(struct dvobj_priv *dvobj, u8 rate,
				 u8 max_bw)
{
	struct rf_ctl_t *rf_ctl = dvobj_to_rfctl(dvobj);
	u8 bw;
 ...
 * Date: 2017/06/05
 */
static void update_attrib_trigger_frame_info(_adapter *padapter,
		struct pkt_attrib *pattrib)
{
	...
	if (check_fwstate(pmlmepriv, WIFI_STATION_STATE) == _TRUE) {
		if ((pwrpriv->pwr_mode == PS_MODE_MIN)
		    || (pwrpriv->pwr_mode == PS_MODE_MAX)) {
...
static void update_attrib_phy_info(_adapter *padapter,
				  ...
				  struct pkt_attrib *pattrib, struct sta_info *psta)
{
	...
	if (pattrib->encrypt && bmcast
	    && _rtw_camctl_chk_flags(padapter,
...
#ifdef CONFIG_TDLS
u8 rtw_check_tdls_established(_adapter *padapter,
			      struct pkt_attrib *pattrib)
{
			 ...
			 struct pkt_attrib *pattrib)
{
	...
	if ((check_fwstate(pmlmepriv, WIFI_ADHOC_STATE) == _TRUE) ||
		...
		if (rtw_check_tdls_established(padapter, pattrib) == _TRUE)
			_rtw_memcpy(pattrib->ra, pattrib->dst,
				    ETH_ALEN);	/* For TDLS direct link Tx, set ra to be same to dst */
		else
#endif
...
get_sta_info:
	...
	if (ETH_P_IP == pattrib->ether_type) {
		...
		if (GET_IPV4_PROTOCOL(ip) == 0x01) { /* ICMP */
			...
			if (rtw_st_ctl_chk_reg_rule(&psta->st_ctl, padapter, IPV4_SRC(ip),
						    TCP_SRC(tcp), IPV4_DST(ip), TCP_DST(tcp)) == _TRUE) {
...
int rtw_build_tdls_ies(_adapter *padapter, struct xmit_frame *pxmitframe,
		       u8 *pframe, struct tdls_txmgmt *ptxmgmt)
{
	struct pkt_attrib *pattrib = &pxmitframe->attrib;
...
s32 rtw_make_tdls_wlanhdr(_adapter *padapter, u8 *hdr,
			  struct pkt_attrib *pattrib, struct tdls_txmgmt *ptxmgmt)
{
	u16 *qc;
	struct rtw_ieee80211_hdr *pwlanhdr = (struct rtw_ieee80211_hdr *)hdr;
	struct mlme_priv *pmlmepriv = &padapter->mlmepriv;
...
s32 rtw_xmit_tdls_coalesce(_adapter *padapter,
			   struct xmit_frame *pxmitframe, struct tdls_txmgmt *ptxmgmt)
{
	s32 llc_sz;
	...
	if (rtw_make_tdls_wlanhdr(padapter, mem_start, pattrib,
				  ptxmgmt) == _FAIL) {
		res = _FAIL;
		goto exit;
	}
...
s32 check_amsdu_tx_support(_adapter *padapter)
{
	...
	if (tx_amsdu == 1)
		ret = _TRUE;
	else if (tx_amsdu == 2 && (tx_amsdu_rate == 0
				   || current_tx_rate > tx_amsdu_rate))
		ret = _TRUE;
	else
		ret = _FALSE;
...
s32 rtw_xmitframe_coalesce_amsdu(_adapter *padapter,
				 struct xmit_frame *pxmitframe, struct xmit_frame *pxmitframe_queue)
{
	...
	if (pxmitframe_queue) {
		...
		*len = htons(XATTRIB_GET_MCTRL_LEN(pattrib_queue) + llc_sz + mem_sz);
		...
		//calc padding
		padding = 4 - ((ETH_HLEN + XATTRIB_GET_MCTRL_LEN(pattrib_queue) + llc_sz +
				mem_sz) & (4 - 1));
		if (padding == 4)
			padding = 0;
		...
		pattrib->last_txcmdsz += ETH_HLEN + XATTRIB_GET_MCTRL_LEN(
						 pattrib_queue) + llc_sz + mem_sz + padding ;
	}
 ...
 */
s32 rtw_mgmt_xmitframe_coalesce(_adapter *padapter, _pkt *pkt,
				struct xmit_frame *pxmitframe)
{
...
#define DBG_MGMT_XMIT_ENC_DUMP 0
	...
	_rtw_memcpy(pframe, tmp_buf + pattrib->hdrlen,
...
static struct xmit_buf *__rtw_alloc_cmd_xmitbuf(struct xmit_priv
		*pxmitpriv,
...
static struct xmit_frame *get_one_xmitframe(struct xmit_priv *pxmitpriv,
		struct hw_xmit *phwxmit, struct tx_servq *ptxservq, _queue *pframe_queue)
{
	_list	*xmitframe_plist, *xmitframe_phead;
	struct	xmit_frame	*pxmitframe = NULL;
...
struct xmit_frame *rtw_get_xframe(struct xmit_priv *pxmitpriv,
				  int *num_frame)
{
	_irqL irqL0;
			...
			/* if (!priv->pmib->ethBrExtInfo.nat25_disable)		 */
		{
			...
			if (*((unsigned short *)(skb->data + MACADDRLEN * 2)) == __constant_htons(
				...
				if (memcmp(padapter->scdb_mac, skb->data + MACADDRLEN, MACADDRLEN)) {
					...
					padapter->scdb_entry = (struct nat25_network_db_entry *)scdb_findEntry(
								       padapter,
...
s32 rtw_monitor_xmit_entry(struct sk_buff *skb, struct net_device *ndev)
{
	...
	_rtw_pktfile_read(&pktfile, (u8 *)(&rtap_hdr),
	...
	_rtw_pktfile_read(&pktfile, dummybuf,
...
//	_rtw_memcpy(pframe, (void *)checking, len);
	...
	if ((frame_ctl & RTW_IEEE80211_FCTL_FTYPE) == RTW_IEEE80211_FTYPE_DATA) {
		...
		if (is_broadcast_mac_addr(pwlanhdr->addr3)
		    || is_broadcast_mac_addr(pwlanhdr->addr1))
...
s32 rtw_xmit(_adapter *padapter, _pkt **ppkt)
{
	...
	pxmitframe->pkt =
		NULL; /* let rtw_xmit_posthandle not to free pkt inside */
					...
					struct xmit_frame *pxmitframe)
{
	...
	*/
	...
	if (bmcst) {
		...
		if (rtw_tim_map_anyone_be_set(padapter,
					      pstapriv->sta_dz_bitmap)) { /* if anyone sta is in ps mode */
			/* pattrib->qsel = QSLT_HIGH; */ /* HIQ */
...
void wakeup_sta_to_xmit(_adapter *padapter, struct sta_info *psta)
{
	...
	if (!(rtw_tim_map_anyone_be_set_exclude_aid0(padapter,
			pstapriv->sta_dz_bitmap))) { /* no any sta in ps mode */
...
void dump_xmit_block_info(void *sel, const char *fun_name,
			  _adapter *padapter)
{
	struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);
...
#ifdef DBG_TXBD_DESC_DUMP
static struct rtw_tx_desc_backup
	tx_backup[HW_QUEUE_ENTRY][TX_BAK_FRMAE_CNT];
static u8 backup_idx[HW_QUEUE_ENTRY];

void rtw_tx_desc_backup(_adapter *padapter, struct xmit_frame *pxmitframe,
			u8 desc_size, u8 hwq)
{
	u16 reg_rp;
	u16 reg_wp;
	...
	_rtw_memcpy(tx_backup[hwq][backup_idx[hwq]].tx_bak_desc, pxmit_buf,
		    desc_size);
	_rtw_memcpy(tx_backup[hwq][backup_idx[hwq]].tx_bak_data_hdr,
		    pxmit_buf + desc_size, TX_BAK_DATA_LEN);
...
u8 rtw_get_tx_desc_backup(_adapter *padapter, u8 hwq,
			  struct rtw_tx_desc_backup **pbak)
{
