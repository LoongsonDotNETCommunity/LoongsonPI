...
void rtw_init_mlme_timer(_adapter *padapter)
{
	...
	rtw_init_timer(&(pmlmepriv->assoc_timer), padapter,
		       rtw_join_timeout_handler, padapter);
	rtw_init_timer(&(pmlmepriv->scan_to_timer), padapter,
		       rtw_scan_timeout_handler, padapter);

#ifdef CONFIG_DFS_MASTER
	rtw_init_timer(&(pmlmepriv->dfs_master_timer), padapter,
		       rtw_dfs_master_timer_hdl, padapter);
#endif

#ifdef CONFIG_SET_SCAN_DENY_TIMER
	rtw_init_timer(&(pmlmepriv->set_scan_deny_timer), padapter,
		       rtw_set_scan_deny_timer_hdl, padapter);
#endif

#ifdef RTK_DMP_PLATFORM
	_init_workitem(&(pmlmepriv->Linkup_workitem), Linkup_workitem_callback,
		       padapter);
	_init_workitem(&(pmlmepriv->Linkdown_workitem), Linkdown_workitem_callback,
		       padapter);
#endif
}
...
sint	_rtw_init_mlme_priv(_adapter *padapter)
{
	...
	init_fwstate(pmlmepriv,
		     WIFI_NULL_STATE);/*assigned interface role(STA/AP) must after execute set_opmode*/

	/* wdev->iftype = NL80211_IFTYPE_STATION*/
	pmlmepriv->cur_network.network.InfrastructureMode = Ndis802_11AutoUnknown;
	pmlmepriv->scan_mode =
		SCAN_ACTIVE; /* 1: active, 0: pasive. Maybe someday we should rename this varable to "active_mode" (Jeff) */
...
void rtw_free_mlme_priv_ie_data(struct mlme_priv *pmlmepriv)
{
	...
	rtw_free_mlme_ie_data(&pmlmepriv->p2p_assoc_resp_ie,
			      &pmlmepriv->p2p_assoc_resp_ie_len);
#endif
	...
	rtw_free_mlme_ie_data(&pmlmepriv->wfd_assoc_resp_ie,
			      &pmlmepriv->wfd_assoc_resp_ie_len);
#endif
...
#if defined(CONFIG_WFD) && defined(CONFIG_IOCTL_CFG80211)
int rtw_mlme_update_wfd_ie_data(struct mlme_priv *mlme, u8 type, u8 *ie,
				u32 ie_len)
{
	...
	if (*t_ie && *t_ie_len) {
		...
		attr_content = rtw_get_wfd_attr_content(*t_ie, *t_ie_len,
...
int	rtw_init_mlme_priv(_adapter
			   *padapter) /* (struct	mlme_priv *pmlmepriv) */
{
	int	res;
...
struct wlan_network *_rtw_find_network(_queue *scanned_queue,
				       const u8 *addr)
{
	_list	*phead, *plist;
...
struct wlan_network *rtw_find_network(_queue *scanned_queue,
				      const u8 *addr)
{
	struct	wlan_network *pnetwork;
	_irqL irqL;
...
int is_same_network(WLAN_BSSID_EX *src, WLAN_BSSID_EX *dst, u8 feature)
{
	...
	if (((_rtw_memcmp(src->MacAddress, dst->MacAddress, ETH_ALEN)) == _TRUE) &&
		...
		if ((src->Ssid.SsidLength == dst->Ssid.SsidLength) &&
		    (((_rtw_memcmp(src->Ssid.Ssid, dst->Ssid.Ssid,
				   src->Ssid.SsidLength)) == _TRUE) || //Case of normal AP
		     (is_all_null(src->Ssid.Ssid, src->Ssid.SsidLength) == _TRUE
		      || is_all_null(dst->Ssid.Ssid,
				     dst->Ssid.SsidLength) == _TRUE))) //Case of hidden AP
			return _TRUE;
		else if ((src->Ssid.SsidLength == 0
			  || dst->Ssid.SsidLength == 0)) //Case of hidden AP
			return _TRUE;
		else
			return _FALSE;
	} else {
...
struct	wlan_network	*rtw_get_oldest_wlan_network(_queue *scanned_queue)
{
	...
	while (1) {
		...
		if (pwlan->fixed != _TRUE) {
			if (oldest == NULL
			    || rtw_time_after(oldest->last_scanned, pwlan->last_scanned))
				oldest = pwlan;
		}
		    ...
		    _adapter *padapter, bool update_ie)
{
	...
	rtw_hal_antdiv_rssi_compared(padapter, dst,
				     src); /* this will update src.Rssi, need consider again */
#endif
	...
	if (check_fwstate(&padapter->mlmepriv, _FW_LINKED)
		...
		if (sq_smp != 101) { /* from the right channel */
			ss_final = ((u32)(src->PhyInfo.SignalStrength) + (u32)(
					    dst->PhyInfo.SignalStrength) * 4) / 5;
			sq_final = ((u32)(src->PhyInfo.SignalQuality) + (u32)(
					    dst->PhyInfo.SignalQuality) * 4) / 5;
			rssi_final = (src->Rssi + dst->Rssi * 4) / 5;
		} else {
	...
	if (strcmp(dst->Ssid.Ssid, DBG_RX_SIGNAL_DISPLAY_SSID_MONITORED) == 0) {
		RTW_INFO(FUNC_ADPT_FMT" %s("MAC_FMT"), SignalStrength:%u, SignalQuality:%u, RawRSSI:%ld\n"
			 , FUNC_ADPT_ARG(padapter)
			 , dst->Ssid.Ssid, MAC_ARG(dst->MacAddress), dst->PhyInfo.SignalStrength,
	...
	if (check_fwstate(&padapter->mlmepriv, _FW_LINKED)
		...
		if (padapter->recvpriv.signal_qual_data.total_num++ >=
			...
			padapter->recvpriv.signal_qual_data.total_num =
				PHY_LINKQUALITY_SLID_WIN_MAX;
			last_evm =
				padapter->recvpriv.signal_qual_data.elements[padapter->recvpriv.signal_qual_data.index];
			padapter->recvpriv.signal_qual_data.total_val -= last_evm;
		}
		padapter->recvpriv.signal_qual_data.total_val += query_rx_pwr_percentage(
					src->Rssi);

		padapter->recvpriv.signal_qual_data.elements[padapter->recvpriv.signal_qual_data.index++]
			= query_rx_pwr_percentage(src->Rssi);
...
static void update_current_network(_adapter *adapter,
...
bool rtw_update_scanned_network(_adapter *adapter, WLAN_BSSID_EX *target)
{
	...
	while (1) {
		...
		if ((oldest == NULL)
			...
			oldest->network.PhyInfo.SignalStrength))
#ifdef CONFIG_RTW_MESH
			if (!MLME_IS_MESH(adapter)
			    || check_fwstate(pmlmepriv, WIFI_ASOC_STATE) == _FALSE
			    || !rtw_bss_is_same_mbss(&pmlmepriv->cur_network.network,
						     &pnetwork->network))
#endif
				oldest = pnetwork;
#else
		if (oldest == NULL
		    || rtw_time_after(oldest->last_scanned, pnetwork->last_scanned))
#ifdef CONFIG_RTW_MESH
			if (!MLME_IS_MESH(adapter)
			    || check_fwstate(pmlmepriv, WIFI_ASOC_STATE) == _FALSE
			    || !rtw_bss_is_same_mbss(&pmlmepriv->cur_network.network,
	...
	if (!target_find) {
		if (_rtw_queue_empty(&(pmlmepriv->free_bss_pool)) == _TRUE) {
			...
			RTW_DBG("%s => ssid:%s ,bssid:"MAC_FMT"  will be deleted from scanned_queue (rssi:%ld , ss:%d)\n",
				...
				MAC_ARG(pnetwork->network.MacAddress), pnetwork->network.Rssi,
				pnetwork->network.PhyInfo.SignalStrength);
#else
			...
			rtw_hal_get_odm_var(adapter, HAL_ODM_ANTDIV_SELECT,
					    &(target->PhyInfo.Optimum_antenna), NULL);
#endif
		...
		if ((pnetwork->network.IELength > target->IELength)
		    && (target->Reserved[0] == BSS_TYPE_BCN))
			update_ie = _FALSE;
 ...
 *			   (4) HT
 * (5) others */
int rtw_is_desired_network(_adapter *adapter,
			   struct wlan_network *pnetwork)
{
	struct security_priv *psecuritypriv = &adapter->securitypriv;
	struct mlme_priv *pmlmepriv = &adapter->mlmepriv;
...
void rtw_survey_event_callback(_adapter	*adapter, u8 *pbuf)
{
	...
	pnetwork->Configuration.BeaconPeriod = le32_to_cpu(
	...
	pnetwork->Configuration.Length = le32_to_cpu(
	...
	if ((check_fwstate(pmlmepriv, WIFI_ADHOC_MASTER_STATE)) == _TRUE) {
		if (_rtw_memcmp(&(pmlmepriv->cur_network.network.MacAddress),
			...
			ibss_wlan = _rtw_find_network(&pmlmepriv->scanned_queue,
						      pnetwork->MacAddress);
			if (ibss_wlan) {
...
void rtw_surveydone_event_callback(_adapter	*adapter, u8 *pbuf)
{
		...
		if ((check_fwstate(pmlmepriv, WIFI_ADHOC_STATE) == _TRUE)) {
			...
			if (_SUCCESS == s_ret)
				...
				if (rtw_to_roam(adapter) != 0) {
					...
					_rtw_memcpy(&parm.ssid[0], &pmlmepriv->assoc_ssid,
						    sizeof(NDIS_802_11_SSID));
					parm.ssid_num = 1;
	...
	if (rtw_mesh_offch_candidate_accepted(adapter)) {
		...
		if (ch && pmlmepriv->cur_network.network.Configuration.DSConfig != ch) {
			...
			rtw_change_bss_chbw_cmd(adapter, RTW_CMDF_DIRECTLY, ch,
						adapter->mlmepriv.ori_bw, -1);
			issue_probereq_ex(adapter, &pmlmepriv->cur_network.network.mesh_id, NULL,
					  0, 0, 0, 0);
		}
	}
#endif
...
*/
void rtw_indicate_disconnect(_adapter *padapter, u16 reason,
			     u8 locally_generated)
{
	...
	beamforming_wk_cmd(padapter, BEAMFORMING_CTRL_LEAVE,
			   cur_network->MacAddress, ETH_ALEN, 1);
	...
	if (psta) { /* update ptarget_sta */
		...
		if (padapter->securitypriv.dot11AuthAlgrthm == dot11AuthAlgrthm_8021X) {
			...
			ie = rtw_get_ie(pnetwork->network.IEs + _BEACON_IE_OFFSET_, WLAN_EID_RSN
					, &ie_len, (pnetwork->network.IELength - _BEACON_IE_OFFSET_));
			if (ie && ie_len > 0
			    && rtw_parse_wpa2_ie(ie, ie_len + 2, NULL, NULL, NULL,
						 &mfp_opt) == _SUCCESS
			   ) {
				if (padapter->securitypriv.mfp_opt >= MFP_OPTIONAL
				    && mfp_opt >= MFP_OPTIONAL)
...
void rtw_joinbss_event_prehandle(_adapter *adapter, u8 *pbuf)
{
	...
	pnetwork->network.Ssid.SsidLength = le32_to_cpu(
			pnetwork->network.Ssid.SsidLength);
	pnetwork->network.Privacy = le32_to_cpu(pnetwork->network.Privacy);
	pnetwork->network.Rssi = le32_to_cpu(pnetwork->network.Rssi);
	pnetwork->network.NetworkTypeInUse = le32_to_cpu(
			pnetwork->network.NetworkTypeInUse) ;
	pnetwork->network.Configuration.ATIMWindow = le32_to_cpu(
				pnetwork->network.Configuration.ATIMWindow);
	pnetwork->network.Configuration.BeaconPeriod = le32_to_cpu(
				pnetwork->network.Configuration.BeaconPeriod);
	pnetwork->network.Configuration.DSConfig = le32_to_cpu(
				pnetwork->network.Configuration.DSConfig);
	pnetwork->network.Configuration.FHConfig.DwellTime = le32_to_cpu(
				pnetwork->network.Configuration.FHConfig.DwellTime);
	pnetwork->network.Configuration.FHConfig.HopPattern = le32_to_cpu(
				pnetwork->network.Configuration.FHConfig.HopPattern);
	pnetwork->network.Configuration.FHConfig.HopSet = le32_to_cpu(
				pnetwork->network.Configuration.FHConfig.HopSet);
	pnetwork->network.Configuration.FHConfig.Length = le32_to_cpu(
				pnetwork->network.Configuration.FHConfig.Length);
	pnetwork->network.Configuration.Length = le32_to_cpu(
				pnetwork->network.Configuration.Length);
	pnetwork->network.InfrastructureMode = le32_to_cpu(
		...
		if (check_fwstate(pmlmepriv, _FW_UNDER_LINKING)) {
			...
			if (check_fwstate(pmlmepriv, _FW_LINKED)) {
				if (the_same_macaddr == _TRUE)
					ptarget_wlan = _rtw_find_network(&pmlmepriv->scanned_queue,
									 cur_network->network.MacAddress);
				else {
					pcur_wlan = _rtw_find_network(&pmlmepriv->scanned_queue,
								      cur_network->network.MacAddress);
					if (pcur_wlan)
...
void rtw_sta_media_status_rpt(_adapter *adapter, struct sta_info *sta,
			      bool connected)
{
	...
	if (connected && !rtw_macid_is_bmc(macid_ctl, sta->cmn.mac_id)) {
		miracast_enabled = STA_OP_WFD_MODE(sta) != 0
				   && is_miracast_enabled(adapter);
...
u8 rtw_sta_media_status_rpt_cmd(_adapter *adapter, struct sta_info *sta,
				bool connected)
{
	...
	cmd_parm = (struct drvextra_cmd_parm *)rtw_zmalloc(sizeof(
	...
	rpt_parm = (struct sta_media_status_rpt_cmd_parm *)rtw_zmalloc(sizeof(
...
inline void rtw_sta_media_status_rpt_cmd_hdl(_adapter *adapter,
		struct sta_media_status_rpt_cmd_parm *parm)
{
...
void rtw_sta_timeout_event_callback(_adapter *adapter, u8 *pbuf)
{
	...
	if (psta) {
		...
		if (rtw_is_list_empty(&psta->asoc_list) == _FALSE) {
			...
			updated = ap_free_sta(adapter, psta, _TRUE,
...
void rtw_sta_mstatus_disc_rpt(_adapter *adapter, u8 mac_id)
{
	...
	if (mac_id >= 0 && mac_id < macid_ctl->num) {
		u8 id_is_shared = mac_id ==
				  RTW_DEFAULT_MGMT_MACID; /* TODO: real shared macid judgment */
	...
	if (check_fwstate(pmlmepriv, WIFI_STATION_STATE)
		...
		if (psta)
			...
			RTW_INFO("%s "ADPT_FMT" - mac_addr: "MAC_FMT" psta == NULL\n", __func__,
				 ADPT_ARG(adapter), MAC_ARG(tgt_network->network.MacAddress));
			rtw_warn_on(1);
		}
	}
}
...
void rtw_stadel_event_callback(_adapter *adapter, u8 *pbuf)
{
	...
	if (psta == NULL) {
		RTW_INFO("%s(mac_id=%d)=" MAC_FMT " psta == NULL\n", __func__,
			 pstadel->mac_id, MAC_ARG(pstadel->macaddr));
		/*rtw_warn_on(1);*/
	}
	...
	if (check_fwstate(pmlmepriv, WIFI_STATION_STATE)) {
		...
		pwlan = _rtw_find_network(&pmlmepriv->scanned_queue,
		...
		rtw_indicate_disconnect(adapter, *(u16 *)pstadel->rsvd,
					pstadel->locally_generated);
#ifdef CONFIG_INTEL_WIDI
...
void rtw_cpwm_event_callback(PADAPTER padapter, u8 *pbuf)
{
	...
	preportpwrstate->state |= (u8)(adapter_to_pwrctl(padapter)->cpwm_tog +
				       0x80);
...
void rtw_mlme_reset_auto_scan_int(_adapter *adapter, u8 *reason)
{
	...
	u8 u_ch;
	u32 interval_ms =
		0xffffffff; /* 0xffffffff: special value to make min() works well, also means no auto scan */
...
#ifdef CONFIG_DFS_MASTER
	    && (!rfctl->radar_detect_ch || (IS_CH_WAITING(rfctl)
					    && !IS_UNDER_CAC(rfctl)))
#endif
	   ) {
		interval_ms = rtw_min(interval_ms,
				      adapter->mesh_cfg.peer_sel_policy.offch_find_int_ms);
...
static void collect_sta_traffic_statistics(_adapter *adapter)
{
	...
	for (i = 0; i < MACID_NUM_SW_LIMIT; i++) {
		...
		if (sta && !is_broadcast_mac_addr(sta->cmn.mac_addr)) {
			...
			curr_tx_mbytes = ((sta->sta_stats.tx_bytes - sta->sta_stats.last_tx_bytes)
					  >> 20) / 2; /*MBps*/
			curr_rx_mbytes = ((sta->sta_stats.rx_bytes - sta->sta_stats.last_rx_bytes)
					  >> 20) / 2; /*MBps*/
...
static void collect_traffic_statistics(_adapter *padapter)
{
	...
	pdvobjpriv->traffic_stat.cur_tx_tp = (u32)(
			pdvobjpriv->traffic_stat.cur_tx_bytes * 8 / 2 / 1024 / 1024);
	pdvobjpriv->traffic_stat.cur_rx_tp = (u32)(
			pdvobjpriv->traffic_stat.cur_rx_bytes * 8 / 2 / 1024 / 1024);
...
void rtw_dynamic_check_timer_handlder(void *ctx)
{
	...
	if (adapter->registrypriv.mp_mode == 1
	    && adapter->mppriv.mp_dm == 0) { /* for MP ODM dynamic Tx power tracking */
				       ...
				       , struct wlan_network **candidate, struct wlan_network *competitor)
{
	...
	if (rtw_chset_search_ch(adapter_to_chset(adapter),
				competitor->network.Configuration.DSConfig) < 0)
		goto exit;
	...
	if (is_same_ess(&competitor->network,
			&mlme->cur_network.network) == _FALSE)
		goto exit;
...
#if 1
	if (rtw_get_passing_time_ms(competitor->last_scanned) >=
...
int rtw_select_roaming_candidate(struct mlme_priv *mlme)
{
	...
	if (candidate == NULL) {
		...
		rson_score = rtw_cal_rson_score(&rson_curr,
						mlme->cur_network_scanned->network.Rssi);
...
candidate_exist:
	...
	if (_TRUE == bSupportAntDiv) {
		u8 CurrentAntenna;
		rtw_hal_get_odm_var(adapter, HAL_ODM_ANTDIV_SELECT, &(CurrentAntenna),
				    NULL);
		RTW_INFO("#### Opt_Ant_(%s) , cur_Ant(%s)\n",
			 (MAIN_ANT == candidate->network.PhyInfo.Optimum_antenna) ? "MAIN_ANT" :
			 "AUX_ANT",
...
sint rtw_set_auth(_adapter *adapter, struct security_priv *psecuritypriv)
{
	...
	psetauthparm = (struct setauth_parm *)rtw_zmalloc(sizeof(
				struct setauth_parm));
		 ...
		 sint keyid, u8 set_tx, bool enqueue)
{
	...
	psetkeyparm = (struct setkey_parm *)rtw_zmalloc(sizeof(
				struct setkey_parm));
	if (psetkeyparm == NULL) {
		res = _FAIL;
		goto exit;
...
bool rtw_is_wmmps_mode(_adapter *padapter)
{
	...
	if ((pqospriv->uapsd_ap_supported)
	    && ((pqospriv->uapsd_tid & BIT_MASK_TID_TC)  != 0))
		return _TRUE;
...
int rtw_restruct_wmm_ie(_adapter *adapter, u8 *in_ie, u8 *out_ie,
			uint in_len, uint initial_out_len)
{
	...
	while (i < in_len) {
		...
		if (in_ie[i] == 0xDD && in_ie[i + 2] == 0x00 && in_ie[i + 3] == 0x50
			...
			out_ie[initial_out_len + 8] =
				0x00; /* force the QoS Info Field to be zero */
#endif
...
static int rtw_rsn_sync_pmkid(_adapter *adapter, u8 *ie, uint ie_len,
			      int i_ent)
{
...
void	rtw_ht_use_default_setting(_adapter *padapter)
{
	...
	phtpriv->sgi_40m = TEST_FLAG(pregistrypriv->short_gi,
				     BIT1) ? _TRUE : _FALSE;
	phtpriv->sgi_20m = TEST_FLAG(pregistrypriv->short_gi,
				     BIT0) ? _TRUE : _FALSE;
...
unsigned int rtw_restructure_ht_ie(_adapter *padapter, u8 *in_ie,
				   u8 *out_ie, uint in_len, uint *pout_len, u8 channel)
{
	...
	if (cbw40_enable) {
		...
		if (oper_bw == CHANNEL_WIDTH_40
		    && oper_offset !=
		    HAL_PRIME_CHNL_OFFSET_DONT_CARE /* check this because TDLS has no info to set offset */
		    && !rtw_chset_is_chbw_valid(adapter_to_chset(padapter), channel, oper_bw,
						oper_offset)
		   ) {
			oper_bw = CHANNEL_WIDTH_20;
			oper_offset = HAL_PRIME_CHNL_OFFSET_DONT_CARE;
			rtw_warn_on(!rtw_chset_is_chbw_valid(adapter_to_chset(padapter), channel,
							     oper_bw, oper_offset));
		}
	...
	if (TEST_FLAG(phtpriv->stbc_cap, STBC_HT_ENABLE_RX)) {
		if ((pregistrypriv->rx_stbc == 0x3) ||							/* enable for 2.4/5 GHz */
		    ...
		    ||		/* enable for 2.4GHz */
		    ((channel > 14) && (pregistrypriv->rx_stbc == 0x2))
		    ||		/* enable for 5GHz */
	...
	_rtw_memcpy(ht_capie.supp_mcs_set, pmlmeext->default_supported_mcs_set,
		    16);
	...
	*/
	...
	if (padapter->driver_rx_ampdu_factor != 0xFF)
		max_rx_ampdu_factor = (HT_CAP_AMPDU_FACTOR)
	...
	if (padapter->driver_rx_ampdu_spacing != 0xFF)
		ht_capie.ampdu_params_info |= ((padapter->driver_rx_ampdu_spacing & 0x07)
					       << 2);
	else {
		if (padapter->securitypriv.dot11PrivacyAlgrthm == _AES_) {
			...
			*	By yiwei.sun
			*/
			rtw_hal_get_def_var(padapter, HW_VAR_BEST_AMPDU_DENSITY,
					    &best_ampdu_density);
		       ...
		       u8 channel)
{
	...
	len = 0;
	p = rtw_get_ie(pie + sizeof(NDIS_802_11_FIXED_IEs), _HT_CAPABILITY_IE_,
		       &len, ie_len - sizeof(NDIS_802_11_FIXED_IEs));
	if (p && len > 0) {
...
void rtw_issue_addbareq_cmd(_adapter *padapter,
			    struct xmit_frame *pxmitframe)
{
	u8 issued;
	int priority;
...
#ifdef CONFIG_WFD
static bool wfd_st_match_rule(_adapter *adapter, u8 *local_naddr,
