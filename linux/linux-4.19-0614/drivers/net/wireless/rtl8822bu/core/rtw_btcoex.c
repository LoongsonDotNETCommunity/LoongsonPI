...
void rtw_btcoex_rx_rate_change_notify(PADAPTER padapter, u8 is_data_frame,
				      u8 rate_id)
{
...
void rtw_btcoex_SetBtPatchVersion(PADAPTER padapter, u16 btHciVer,
				  u16 btPatchVer)
{
...
u8 rtw_btcoex_send_event_to_BT(_adapter *padapter, u8 status,
			       u8 event_code, u8 opcode_low, u8 opcode_high, u8 *dbg_msg)
{
	u8 localBuf[6] = "";
	u8 *pRetPar;
...
#endif
	status = rtw_btcoex_sendmsgbysocket(padapter, (u8 *)pEvent,
					    tx_event_length, _FALSE);
...
u8 rtw_btcoex_parse_BT_info_notify_cmd(_adapter *padapter, u8 *pcmd,
				       u16 cmdlen)
{
...
u8 rtw_btcoex_parse_BT_patch_ver_info_cmd(_adapter *padapter, u8 *pcmd,
		u16 cmdlen)
{
	RTW_HCI_STATUS status = HCI_STATUS_SUCCESS;
	...
	/* send complete event to BT */
	{
		...
		pEvent->Data[2] = HCIOPCODEHIGHT(HCI_BT_PATCH_VERSION_NOTIFY,
						 OGF_EXTENSION);
		len = len + 3;
...
u8 rtw_btcoex_parse_HCI_Ver_notify_cmd(_adapter *padapter, u8 *pcmd,
				       u16 cmdlen)
{
	RTW_HCI_STATUS status = HCI_STATUS_SUCCESS;
	...
	/* send complete event to BT */
	{
		...
		pEvent->EventCode = HCI_EVENT_COMMAND_COMPLETE;
		pEvent->Data[0] = 0x1;	/* packet # */
		pEvent->Data[1] = HCIOPCODELOW(HCI_EXTENSION_VERSION_NOTIFY,
					       OGF_EXTENSION);
		pEvent->Data[2] = HCIOPCODEHIGHT(HCI_EXTENSION_VERSION_NOTIFY,
						 OGF_EXTENSION);
		len = len + 3;
...
u8 rtw_btcoex_parse_WIFI_scan_notify_cmd(_adapter *padapter, u8 *pcmd,
		u16 cmdlen)
{
	RTW_HCI_STATUS status = HCI_STATUS_SUCCESS;
	...
	/* send complete event to BT */
	{
		...
		pEvent->Data[2] = HCIOPCODEHIGHT(HCI_ENABLE_WIFI_SCAN_NOTIFY,
						 OGF_EXTENSION);
		len = len + 3;
...
u8 rtw_btcoex_parse_HCI_link_status_notify_cmd(_adapter *padapter,
		u8 *pcmd, u16 cmdlen)
{
	RTW_HCI_STATUS	status = HCI_STATUS_SUCCESS;
	struct bt_coex_info	*pcoex_info = &padapter->coex_info;
...
u8 rtw_btcoex_parse_HCI_BT_coex_notify_cmd(_adapter *padapter, u8 *pcmd,
		u16 cmdlen)
{
	u8 localBuf[6] = "";
	u8 *pRetPar;
...
u8 rtw_btcoex_parse_HCI_BT_operation_notify_cmd(_adapter *padapter,
		u8 *pcmd, u16 cmdlen)
{
	u8 localBuf[6] = "";
	u8 *pRetPar;
...
u8 rtw_btcoex_parse_BT_AFH_MAP_notify_cmd(_adapter *padapter, u8 *pcmd,
		u16 cmdlen)
{
	u8 localBuf[6] = "";
	u8 *pRetPar;
...
u8 rtw_btcoex_parse_BT_register_val_notify_cmd(_adapter *padapter,
		u8 *pcmd, u16 cmdlen)
{
...

	{
		...
		pEvent->EventCode = HCI_EVENT_COMMAND_COMPLETE;
		pEvent->Data[0] = 0x1;	/* packet # */
		pEvent->Data[1] = HCIOPCODELOW(HCI_BT_REGISTER_VALUE_NOTIFY,
					       OGF_EXTENSION);
		pEvent->Data[2] = HCIOPCODEHIGHT(HCI_BT_REGISTER_VALUE_NOTIFY,
						 OGF_EXTENSION);
		len = len + 3;
...
u8 rtw_btcoex_parse_HCI_BT_abnormal_notify_cmd(_adapter *padapter,
		u8 *pcmd, u16 cmdlen)
{
	u8 localBuf[6] = "";
	u8 *pRetPar;
...
u8 rtw_btcoex_parse_HCI_query_RF_status_cmd(_adapter *padapter, u8 *pcmd,
		u16 cmdlen)
{
	u8 localBuf[6] = "";
	u8 *pRetPar;
 ...
 *	 |	OCF			             |	   OGF       | */
void rtw_btcoex_parse_hci_extend_cmd(_adapter *padapter, u8 *pcmd, u16 len,
				     const u16 hci_OCF)
{
...
void rtw_btcoex_recvmsgbysocket(void *data)
{
	...
	u16 parse_res = 0;
#if 0
	u8 para_len = 0, polling_enable = 0, poling_interval = 0, reason = 0,
	   btinfo_len = 0;
	...
	while (len > 0) {
		...
		if (RX_ATTEND_ACK == parse_res) {
			...
			RTW_INFO("RX_BT_ATTEND_REQ!,sock_open:%d, BT_attend:%d\n",
				 pcoex_info->sock_open, pcoex_info->BT_attend);
			rtw_btcoex_sendmsgbysocket(pbtcoexadapter, attend_ack, sizeof(attend_ack),
						   _FALSE);
			...
			RTW_INFO("RX_INVITE_REQ!,sock_open:%d, BT_attend:%d\n",
				 pcoex_info->sock_open, pcoex_info->BT_attend);
			rtw_btcoex_sendmsgbysocket(pbtcoexadapter, invite_rsp, sizeof(invite_rsp),
						   _FALSE);
			...
			/* BT leave */
			rtw_btcoex_sendmsgbysocket(pbtcoexadapter, leave_ack, sizeof(leave_ack),
						   _FALSE); /*  no ack */
			pcoex_info->BT_attend = _FALSE;
...
u8 rtw_btcoex_create_kernel_socket(_adapter *padapter)
{
	...
	if (kernel_socket_err < 0) {
		...
		_rtw_memset(&(pcoex_info->wifi_sockaddr), 0,
		...
		_rtw_memset(&(pcoex_info->bt_sockaddr), 0,
			    sizeof(pcoex_info->bt_sockaddr));
		...
		kernel_socket_err = pcoex_info->udpsock->ops->bind(pcoex_info->udpsock,
				    (struct sockaddr *)&pcoex_info->wifi_sockaddr,
				    sizeof(pcoex_info->wifi_sockaddr));
		if (kernel_socket_err == 0) {
			...
			rtw_btcoex_sendmsgbysocket(padapter, attend_req, sizeof(attend_req),
						   _TRUE);
			status = _SUCCESS;
		} else {
			pcoex_info->BT_attend = _FALSE;
...
void rtw_btcoex_init_socket(_adapter *padapter)
{
	...
	if (_FALSE == pcoex_info->is_exist) {
		...
		RTW_INFO("%s: pbtcoexadapter:%p, coex_info->is_exist: %s\n"
			 , __func__, pbtcoexadapter,
			 pcoex_info->is_exist == _TRUE ? "TRUE" : "FALSE");
	}
}
...
void rtw_btcoex_close_socket(_adapter *padapter)
{
	...
	RTW_INFO("%s--coex_info->is_exist: %s, pcoex_info->BT_attend:%s\n"
		 , __func__, pcoex_info->is_exist == _TRUE ? "TRUE" : "FALSE",
		 pcoex_info->BT_attend == _TRUE ? "TRUE" : "FALSE");

	if (_TRUE == pcoex_info->is_exist) {
		if (_TRUE == pcoex_info->BT_attend) {
			/*inform BT wifi leave*/
			rtw_btcoex_sendmsgbysocket(padapter, wifi_leave, sizeof(wifi_leave),
						   _FALSE);
			msleep(50);
		}
...
void rtw_btcoex_SendEventExtBtCoexControl(PADAPTER padapter,
...
/* Porting from Windows team */
void rtw_btcoex_SendEventExtBtInfoControl(PADAPTER padapter, u8 dataLen,
		void *pData)
{
	rtw_HCI_event *pEvent;
	u8			*pRetPar;
	...
	pEvent->EventCode = HCI_EVENT_EXTENSION_RTK;
	pEvent->Data[0] =
		HCI_EVENT_EXT_BT_INFO_CONTROL;		/* extension event code */
	len++;
...
void rtw_btcoex_SendScanNotify(PADAPTER padapter, u8 scanType)
{
	...
	pEvent->EventCode = HCI_EVENT_EXTENSION_RTK;
	pEvent->Data[0] =
		HCI_EVENT_EXT_WIFI_SCAN_NOTIFY;		/* extension event code */
	len++;
	...
	tx_event_length = pEvent->Length + 2;
#if 0
	rtw_btcoex_dump_tx_msg((u8 *)pEvent, tx_event_length,
			       "WIFI SCAN OPERATION");
#endif
	rtw_btcoex_sendmsgbysocket(padapter, (u8 *)pEvent, tx_event_length,
				   _FALSE);
