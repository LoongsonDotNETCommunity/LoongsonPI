...
static u32 go_add_group_info_attr(struct wifidirect_info *pwdinfo,
				  u8 *pbuf)
{
	_irqL irqL;
	_list	*phead, *plist;
	...
	if (attr_len > 0)
		len = rtw_set_p2p_attr_content(pbuf, P2P_ATTR_GROUP_INFO, attr_len,
					       pdata_attr);
...
static void issue_p2p_devdisc_resp(struct wifidirect_info *pwdinfo, u8 *da,
				   u8 status, u8 dialogToken)
{
	struct xmit_frame			*pmgntframe;
	struct pkt_attrib			*pattrib;
	unsigned char					*pframe;
	...
	*(fctrl) = 0;
	...
	/* P2P_ATTR_STATUS */
	p2pielen += rtw_set_p2p_attr_content(&p2pie[p2pielen], P2P_ATTR_STATUS, 1,
					     &status);
...
static void issue_p2p_provision_resp(struct wifidirect_info *pwdinfo,
				     u8 *raddr, u8 *frame_body, u16 config_method)
{
	_adapter *padapter = pwdinfo->padapter;
	unsigned char category = RTW_WLAN_CATEGORY_PUBLIC;
	u8			action = P2P_PUB_ACTION_ACTION;
...
static void issue_p2p_presence_resp(struct wifidirect_info *pwdinfo,
				    u8 *da, u8 status, u8 dialogToken)
{
	struct xmit_frame			*pmgntframe;
	struct pkt_attrib			*pattrib;
	unsigned char					*pframe;
	...
	*(fctrl) = 0;
	...
	p2pielen += rtw_set_p2p_attr_content(&p2pie[p2pielen], P2P_ATTR_NOA, 2,
					     noa_attr_content);
...
u32 build_beacon_p2p_ie(struct wifidirect_info *pwdinfo, u8 *pbuf)
{
	...
	p2pielen += rtw_set_p2p_attr_content(&p2pie[p2pielen], P2P_ATTR_CAPABILITY,
					     2, (u8 *)&capability);


	/* P2P Device ID ATTR */
	p2pielen += rtw_set_p2p_attr_content(&p2pie[p2pielen], P2P_ATTR_DEVICE_ID,
					     ETH_ALEN, pwdinfo->device_addr);
	...
	pbuf = rtw_set_ie(pbuf, _VENDOR_SPECIFIC_IE_, p2pielen,
...
u32 build_beacon_wfd_ie(struct wifidirect_info *pwdinfo, u8 *pbuf)
{
	...
	rtw_set_ie(pbuf, _VENDOR_SPECIFIC_IE_, wfdielen, (unsigned char *) wfdie,
		   &len);
			    ...
			    u8 tunneled)
{
		...
		if (P2P_ROLE_GO == pwdinfo->role) {
			if (is_any_client_associated(pwdinfo->padapter)) {
				if (pwdinfo->wfd_tdls_enable) {
					...
					v16 = pwfd_info->wfd_device_type | WFD_DEVINFO_WSD | WFD_DEVINFO_PC_TDLS |
					      WFD_DEVINFO_HDCP_SUPPORT;
					RTW_PUT_BE16(wfdie + wfdielen, v16);
				} else {
					/*	WiFi Direct mode + WSD ( WFD Service Discovery ) */
					v16 =  pwfd_info->wfd_device_type | WFD_DEVINFO_WSD |
					       WFD_DEVINFO_HDCP_SUPPORT;
					RTW_PUT_BE16(wfdie + wfdielen, v16);
				}
			} else {
	...
	pbuf = rtw_set_ie(pbuf, _VENDOR_SPECIFIC_IE_, wfdielen,
...
u32 build_probe_resp_p2p_ie(struct wifidirect_info *pwdinfo, u8 *pbuf)
{
	...
	if (widi_version == 35)
		...
		RTW_PUT_LE16(p2pie + p2pielen,
			     21 + 8 * pmlmepriv->num_p2p_sdt + pwdinfo->device_name_len);
	else
#endif /* CONFIG_INTEL_WIDI */
...
u32 build_prov_disc_request_p2p_ie(struct wifidirect_info *pwdinfo,
				   u8 *pbuf, u8 *pssid, u8 ussidlen, u8 *pdev_raddr)
{
	u8 p2pie[MAX_P2P_IE_LEN] = { 0x00 };
	u32 len = 0, p2pielen = 0;
			     ...
			     uint len)
{
	...
	p = rtw_get_ie(pframe + WLAN_HDR_A3_LEN + _PROBEREQ_IE_OFFSET_,
		       _SUPPORTEDRATES_IE_, (int *)&rate_cnt,
	...
	if (rtw_p2p_chk_role(pwdinfo, P2P_ROLE_DEVICE)
		...
		p2pie = rtw_get_p2p_ie(pframe + WLAN_HDR_A3_LEN + _PROBEREQ_IE_OFFSET_,
				       len - WLAN_HDR_A3_LEN - _PROBEREQ_IE_OFFSET_, NULL, &p2pielen);
		if (p2pie) {
			if ((p != NULL)
			    && _rtw_memcmp((void *)(p + 2), (void *) pwdinfo->p2p_wildcard_ssid, 7)) {
				/* todo: */
			     ...
			     uint len, struct sta_info *psta)
{
	...
	while (p2p_ie) {
		...
		if (rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_DEVICE_INFO, NULL,
			...
			if (pattr_content) {
				...
				rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_DEVICE_INFO,
							 pattr_content, (uint *)&attr_contentlen);

				_rtw_memcpy(psta->dev_addr, 	pattr_content,
				...
				if (WPS_ATTR_DEVICE_NAME == be16_to_cpu(*(u16 *)pattr_content)) {
					...
					psta->dev_name_len = (sizeof(psta->dev_name) < dev_name_len) ? sizeof(
								     psta->dev_name) : dev_name_len;
		...
		/* Get the next P2P IE */
		p2p_ie = rtw_get_p2p_ie(p2p_ie + p2p_ielen,
					ies_len - (p2p_ie - ies + p2p_ielen), NULL, &p2p_ielen);
			    ...
			    uint len)
{
	...
	if (p2p_ie) {
		...
		if (rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_GROUP_ID, groupid,
					     &attr_contentlen)) {
			if (_rtw_memcmp(pwdinfo->device_addr, groupid, ETH_ALEN) &&
			    _rtw_memcmp(pwdinfo->p2p_group_ssid, groupid + ETH_ALEN,
					pwdinfo->p2p_group_ssid_len)) {
				attr_contentlen = 0;
				if (rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_DEVICE_ID,
							     dev_addr, &attr_contentlen)) {
					_irqL irqL;
					_list	*phead, *plist;
	...
	issue_p2p_devdisc_resp(pwdinfo, get_addr2_ptr(pframe), status,
			       dialogToken);
			    ...
			    uint len)
{
	...
	wpsie = rtw_get_wps_ie(frame_body + _PUBLIC_ACTION_IE_OFFSET_,
			       len - _PUBLIC_ACTION_IE_OFFSET_, NULL, &wps_ielen);
	if (wpsie) {
		if (rtw_get_wps_attr_content(wpsie, wps_ielen, WPS_ATTR_CONF_METHOD,
			...
			case WPS_CM_LABEL: {
				_rtw_memcpy(
					pwdinfo->rx_prov_disc_info.strconfig_method_desc_of_prov_disc_req, "lab",
					3);
				break;
			}
			...
			issue_p2p_provision_resp(pwdinfo, get_addr2_ptr(pframe), frame_body,
						 uconfig_method);
		}
	}
...
u8 rtw_p2p_get_peer_ch_list(struct wifidirect_info *pwdinfo,
			    u8 *ch_content, u8 ch_cnt, u8 *peer_ch_list)
{
	u8 i = 0, j = 0;
	u8 temp = 0;
	u8 ch_no = 0;
...
u8 rtw_p2p_ch_inclusion(_adapter *adapter, u8 *peer_ch_list,
			u8 peer_ch_num, u8 *ch_list_inclusioned)
{
				    ...
				    u8 *pframe, uint len)
{
...
#endif /* CONFIG_TDLS	 */
#endif /* CONFIG_WFD */
	wpsie = rtw_get_wps_ie(pframe + _PUBLIC_ACTION_IE_OFFSET_,
			       len - _PUBLIC_ACTION_IE_OFFSET_, NULL, &wps_ielen);
	if (wpsie) {
		...
		if (_rtw_memcmp(
			    ...
			    3)) {
			rtw_get_wps_attr_content(wpsie, wps_ielen, WPS_ATTR_DEVICE_PWID,
	...
	while (p2p_ie) {
		...
		if (rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_GO_INTENT,
					     &attr_content, &attr_contentlen)) {
			RTW_INFO("[%s] GO Intent = %d, tie = %d\n", __FUNCTION__,
				 attr_content >> 1, attr_content & 0x01);
			pwdinfo->peer_intent =
				attr_content;	/*	include both intent and tie breaker values. */
			...
			if (rtw_p2p_chk_role(pwdinfo, P2P_ROLE_GO)) {
				...
				_rtw_memcpy(pwdinfo->groupid_info.go_device_addr, pwdinfo->device_addr,
					    ETH_ALEN);
				_rtw_memcpy(pwdinfo->groupid_info.ssid, pwdinfo->nego_ssid,
					    pwdinfo->nego_ssidlen);
			}
		}

		if (rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_LISTEN_CH,
					     (u8 *)listen_ch_attr, (uint *) &attr_contentlen) && attr_contentlen == 5)
		...
		attr_contentlen = 0;
		if (rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_INTENDED_IF_ADDR,
					     pwdinfo->p2p_peer_interface_addr, &attr_contentlen)) {
			if (attr_contentlen != ETH_ALEN)
				_rtw_memset(pwdinfo->p2p_peer_interface_addr, 0x00, ETH_ALEN);
		}

		if (rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_CH_LIST,
					     ch_content, &ch_cnt)) {
			peer_ch_num = rtw_p2p_get_peer_ch_list(pwdinfo, ch_content, ch_cnt,
							       peer_ch_list);
			ch_num_inclusioned = rtw_p2p_ch_inclusion(padapter, peer_ch_list,
					     peer_ch_num, ch_list_inclusioned);
...
#ifdef CONFIG_WFD
	rtw_process_wfd_ies(padapter, pframe + _PUBLIC_ACTION_IE_OFFSET_,
			    len - _PUBLIC_ACTION_IE_OFFSET_, __func__);
#endif
	...
	if (!p2p_ie) {
		...
		while (p2p_ie) {	/*	Found the P2P IE. */
			...
			if (rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_CH_LIST,
				...
				peer_ch_num = rtw_p2p_get_peer_ch_list(pwdinfo, pwdinfo->channel_list_attr,
								       pwdinfo->channel_list_attr_len, peer_ch_list);
				ch_num_inclusioned = rtw_p2p_ch_inclusion(padapter, peer_ch_list,
						     peer_ch_num, ch_list_inclusioned);
			...
			if (rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_GROUP_ID, groupid,
				...
				_rtw_memcpy(pwdinfo->groupid_info.ssid, &groupid[6],
					    attr_contentlen - ETH_ALEN);
			}
	...
	while (p2p_ie) {	/*	Found the P2P IE. */
		...
		if (rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_GROUP_ID, groupid,
			...
			RTW_INFO("[%s] Ssid = %s, ssidlen = %zu\n", __FUNCTION__,
				 &groupid[ETH_ALEN], strlen(&groupid[ETH_ALEN]));
			_rtw_memcpy(pwdinfo->groupid_info.go_device_addr, &groupid[0], ETH_ALEN);
			_rtw_memcpy(pwdinfo->groupid_info.ssid, &groupid[6],
				    attr_contentlen - ETH_ALEN);
		}
			    ...
			    uint len)
{
	...
	issue_p2p_presence_resp(pwdinfo, get_addr2_ptr(pframe), status,
				dialogToken);
...
void find_phase_handler(_adapter	*padapter)
{
	...
	_rtw_memcpy(&parm.ssid[0].Ssid, pwdinfo->p2p_wildcard_ssid,
...
void restore_p2p_state_handler(_adapter	*padapter)
{
	...
	if (rtw_mi_check_status(padapter, MI_LINKED)) {
		...
		if (rtw_p2p_chk_state(pwdinfo, P2P_STATE_TX_PROVISION_DIS_REQ)
		    || rtw_p2p_chk_state(pwdinfo, P2P_STATE_RX_PROVISION_DIS_RSP)) {
...
void pre_tx_invitereq_handler(_adapter	*padapter)
{
	...
	set_channel_bwmode(padapter, pwdinfo->invitereq_info.peer_ch,
			   HAL_PRIME_CHNL_OFFSET_DONT_CARE, CHANNEL_WIDTH_20);
...
void pre_tx_negoreq_handler(_adapter	*padapter)
{
	...
	set_channel_bwmode(padapter, pwdinfo->nego_req_info.peer_channel_num[0],
			   HAL_PRIME_CHNL_OFFSET_DONT_CARE, CHANNEL_WIDTH_20);
	rtw_hal_set_hwreg(padapter, HW_VAR_MLME_SITESURVEY, (u8 *)(&val8));
	issue_probereq_p2p(padapter, NULL);
	...
	for (i = 0; i < dvobj->iface_nums; i++) {
		...
		if (iface) {
			...
			if (check_fwstate(pmlmepriv,
					  _FW_UNDER_LINKING | WIFI_UNDER_WPS) == _TRUE) {
...
static int ro_ch_handler(_adapter *adapter, u8 *buf)
{
	...
	RTW_INFO(FUNC_ADPT_FMT" ch:%u duration:%d, cookie:0x%llx\n"
		 , FUNC_ADPT_ARG(adapter), remain_ch, roch_parm->duration,
		 roch_parm->cookie);
	...
	if (roch_parm->wdev && roch_parm->cookie) {
		...
		if (pcfg80211_wdinfo->remain_on_ch_cookie != roch_parm->cookie) {
			RTW_WARN(FUNC_ADPT_FMT" ongoing cookie:0x%llx, cookie:0x%llx\n"
				 , FUNC_ADPT_ARG(adapter), pcfg80211_wdinfo->remain_on_ch_cookie,
				 roch_parm->cookie);
			rtw_warn_on(1);
		}
	}
	...
	if (rtw_mi_check_status(adapter, MI_LINKED)
	    && (0 != rtw_mi_get_union_chan(adapter))) {
	...
	if (ready_on_channel == _TRUE) {
...
#endif
		{
...
#endif
			{
				...
				set_channel_bwmode(adapter, remain_ch, HAL_PRIME_CHNL_OFFSET_DONT_CARE,
						   CHANNEL_WIDTH_20);
			}
		}
	}
...
static int cancel_ro_ch_handler(_adapter *padapter, u8 *buf)
{
	...
	if (roch_parm->wdev && roch_parm->cookie) {
		...
		if (pcfg80211_wdinfo->remain_on_ch_cookie != roch_parm->cookie) {
			RTW_WARN(FUNC_ADPT_FMT" ongoing cookie:0x%llx, cookie:0x%llx\n"
				 , FUNC_ADPT_ARG(padapter), pcfg80211_wdinfo->remain_on_ch_cookie,
				 roch_parm->cookie);
			rtw_warn_on(1);
		}
	}
	...
	} else if (adapter_wdev_data(padapter)->p2p_enabled
...
static void rtw_change_p2pie_op_ch(_adapter *padapter,
				   const u8 *frame_body, u32 len, u8 ch)
{
	u8 *ies, *p2p_ie;
	...
	while (p2p_ie) {
		...
		attr_contentlen = 0;
		pattr = NULL;
		pattr = rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_OPERATING_CH,
						 NULL, (uint *)&attr_contentlen);
		if (pattr != NULL)
			*(pattr + 4) = ch;
...
static void rtw_change_p2pie_ch_list(_adapter *padapter,
				     const u8 *frame_body, u32 len, u8 ch)
{
	u8 *ies, *p2p_ie;
	u32 ies_len, p2p_ielen;
	...
	while (p2p_ie) {
		...
		pattr = rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_CH_LIST, NULL,
						 (uint *)&attr_contentlen);
		if (pattr != NULL) {
			int i;
			u32 num_of_ch;
...
static bool rtw_chk_p2pie_ch_list_with_buddy(_adapter *padapter,
		const u8 *frame_body, u32 len)
{
	bool fit = _FALSE;
#ifdef CONFIG_CONCURRENT_MODE
	u8 *ies, *p2p_ie;
...
static bool rtw_chk_p2pie_op_ch_with_buddy(_adapter *padapter,
		const u8 *frame_body, u32 len)
{
	bool fit = _FALSE;
#ifdef CONFIG_CONCURRENT_MODE
...
static void rtw_cfg80211_adjust_p2pie_channel(_adapter *padapter,
		const u8 *frame_body, u32 len)
{
#ifdef CONFIG_CONCURRENT_MODE
	u8 *ies, *p2p_ie;
...
#define DBG_XFRAME_DEL_WFD_IE 0
	...
	if (ies && ies_len_ori) {
		ies_len = rtw_del_wfd_ie(ies, ies_len_ori,
					 DBG_XFRAME_DEL_WFD_IE ? __func__ : NULL);
...
int rtw_p2p_check_frames(_adapter *padapter, const u8 *buf, u32 len, u8 tx)
{
	...
	frame_body = (unsigned char *)(buf + sizeof(struct
				       rtw_ieee80211_hdr_3addr));
	category = frame_body[0];
	/* just for check */
	if (category == RTW_WLAN_CATEGORY_PUBLIC) {
		...
		if (action == ACT_PUBLIC_VENDOR
			...
			p2p_ie = rtw_get_p2p_ie(
					 (u8 *)buf + sizeof(struct rtw_ieee80211_hdr_3addr) +
			...
			case P2P_GO_NEGO_REQ: {
				...
				if (tx) {
					...
					if (rtw_mi_check_status(padapter, MI_LINKED)
					    && padapter->registrypriv.full_ch_in_p2p_handshake == 0)
						rtw_cfg80211_adjust_p2pie_channel(padapter, frame_body,
										  len - sizeof(struct rtw_ieee80211_hdr_3addr));
#endif
				}

				cont = rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_OPERATING_CH,
								NULL, &cont_len);
				if (cont)
					op_ch = *(cont + 4);
				cont = rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_LISTEN_CH,
								NULL, &cont_len);
				if (cont)
					listen_ch = *(cont + 4);
				cont = rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_GO_INTENT,
								NULL, &cont_len);
				if (cont)
					intent = *cont;
				cont = rtw_get_p2p_attr_content(p2p_ie, p2p_ielen,
								P2P_ATTR_INTENDED_IF_ADDR, NULL, &cont_len);
				if (cont && cont_len == 6)
					iaddr = cont;
				...
				_rtw_memcpy(nego_info->peer_mac,
					    tx ? GetAddr1Ptr(buf) : get_addr2_ptr(buf), ETH_ALEN);
				if (iaddr)
					_rtw_memcpy(tx ? nego_info->iface_addr : nego_info->peer_iface_addr, iaddr,
						    ETH_ALEN);
				...
				RTW_INFO("RTW_%s:P2P_GO_NEGO_REQ, dialogToken=%d, intent:%u%s, listen_ch:%d, op_ch:%d, ch_list:%s"
					 , (tx == _TRUE) ? "Tx" : "Rx", dialogToken, (intent >> 1),
					 intent & 0x1 ? "+" : "-", listen_ch, op_ch, ch_list_buf);
				if (iaddr)
...
#if defined(CONFIG_CONCURRENT_MODE) && defined(CONFIG_CFG80211_ONECHANNEL_UNDER_CONCURRENT)
					if (rtw_mi_check_status(padapter, MI_LINKED)
					    && rtw_chk_p2pie_ch_list_with_buddy(padapter, frame_body,
										len - sizeof(struct rtw_ieee80211_hdr_3addr)) == _FALSE
					    && padapter->registrypriv.full_ch_in_p2p_handshake == 0) {
						RTW_INFO(FUNC_ADPT_FMT" ch_list has no intersect with buddy\n",
							 FUNC_ADPT_ARG(padapter));
						rtw_change_p2pie_ch_list(padapter, frame_body,
									 len - sizeof(struct rtw_ieee80211_hdr_3addr), 0);
					}
#endif
				}
			...
			case P2P_GO_NEGO_RESP: {
				...
				if (cont)
					intent = *cont;
				cont = rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_STATUS, NULL,
								&cont_len);
				if (cont)
					status = *cont;
				cont = rtw_get_p2p_attr_content(p2p_ie, p2p_ielen,
				...
				if (nego_info->token == dialogToken && nego_info->state == 0
				    && _rtw_memcmp(nego_info->peer_mac,
						   tx ? GetAddr1Ptr(buf) : get_addr2_ptr(buf), ETH_ALEN) == _TRUE
				   ) {
					if (iaddr)
						_rtw_memcpy(tx ? nego_info->iface_addr : nego_info->peer_iface_addr, iaddr,
							    ETH_ALEN);
				...
				RTW_INFO("RTW_%s:P2P_GO_NEGO_RESP, dialogToken=%d, intent:%u%s, status:%d, op_ch:%d, ch_list:%s"
					 , (tx == _TRUE) ? "Tx" : "Rx", dialogToken, (intent >> 1),
					 intent & 0x1 ? "+" : "-", status, op_ch, ch_list_buf);
				if (iaddr)
					_RTW_INFO(", iaddr:"MAC_FMT, MAC_ARG(iaddr));
				_RTW_INFO("\n");
			...
			case P2P_GO_NEGO_CONF: {
				...
				if (nego_info->token == dialogToken && nego_info->state == 1
					...
					if (status == 0) {
						if (rtw_p2p_nego_intent_compare(nego_info->req_intent,
										nego_info->rsp_intent) ^ !tx)
							is_go = _TRUE;
					}
			...
			case P2P_INVIT_REQ: {
				...
				cont = rtw_get_p2p_attr_content(p2p_ie, p2p_ielen,
								P2P_ATTR_INVITATION_FLAGS, NULL, &cont_len);
				if (cont)
					flags = *cont;
				cont = rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_OPERATING_CH,
								NULL, &cont_len);
				if (cont)
					op_ch = *(cont + 4);
				cont = rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_GROUP_BSSID,
								NULL, &cont_len);
				if (cont && cont_len == 6)
					gbssid = cont;
				...
				_rtw_memcpy(invit_info->peer_mac,
					    tx ? GetAddr1Ptr(buf) : get_addr2_ptr(buf), ETH_ALEN);
				if (gbssid)
				...
				if (!tx) {
					...
					if (rtw_mi_check_status(padapter, MI_LINKED)
						...
						if (op_ch != -1
						    && rtw_chk_p2pie_op_ch_with_buddy(padapter, frame_body,
										      len - sizeof(struct rtw_ieee80211_hdr_3addr)) == _FALSE) {
...
#endif
							if (rtw_chk_p2pie_ch_list_with_buddy(padapter, frame_body,
											     len - sizeof(struct rtw_ieee80211_hdr_3addr)) == _FALSE) {
								RTW_INFO(FUNC_ADPT_FMT" ch_list has no intersect with buddy\n",
									 FUNC_ADPT_ARG(padapter));
								rtw_change_p2pie_ch_list(padapter, frame_body,
			...
			case P2P_INVIT_RESP: {
				...
				if (invit_info->token == dialogToken && invit_info->state == 0
				    && _rtw_memcmp(invit_info->peer_mac,
						   tx ? GetAddr1Ptr(buf) : get_addr2_ptr(buf), ETH_ALEN) == _TRUE
				   ) {
			...
			case P2P_PROVISION_DISC_REQ: {
				...
				/* if(tx) */
				{
					...
					p2p_ie = rtw_get_p2p_ie(frame_body + _PUBLIC_ACTION_IE_OFFSET_,
								frame_body_len - _PUBLIC_ACTION_IE_OFFSET_, NULL, &p2p_ielen);
					if (p2p_ie) {

						if (rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_GROUP_ID, NULL,
									     &contentlen)) {
							...
							pwdev_priv->provdisc_req_issued =
								_TRUE;/* case: p2p_devices connection before Nego req. */
						}
...
void rtw_init_cfg80211_wifidirect_info(_adapter	*padapter)
{
	...
	_rtw_memset(pcfg80211_wdinfo, 0x00,
		    sizeof(struct cfg80211_wifidirect_info));

	rtw_init_timer(&pcfg80211_wdinfo->remain_on_ch_timer, padapter,
		       ro_ch_timer_process, padapter);
}
...
int process_p2p_cross_connect_ie(PADAPTER padapter, u8 *IEs, u32 IELength)
{
	...
	while (p2p_ie) {
		...
		if (rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_MANAGEABILITY,
					     p2p_attr, &attr_contentlen)) {
...
void process_p2p_ps_ie(PADAPTER padapter, u8 *IEs, u32 IELength)
{
	...
	while (p2p_ie) {
		...
		if (rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_NOA, noa_attr,
					     &attr_contentlen)) {
			find_p2p_ps = _TRUE;
			...
			if ((pwdinfo->p2p_ps_mode == P2P_PS_NONE) ||
			    (noa_index !=
			     pwdinfo->noa_index)) { /* if index change, driver should reconfigure related setting. */
...
void p2p_ps_wk_hdl(_adapter *padapter, u8 p2p_ps_state)
{
	...
	case P2P_PS_DISABLE:
		...
		rtw_hal_set_hwreg(padapter, HW_VAR_H2C_FW_P2P_PS_OFFLOAD,
				  (u8 *)(&p2p_ps_state));
...
static void pre_tx_scan_timer_process(void *FunctionContext)
{
	...
	if (rtw_p2p_chk_state(pwdinfo, P2P_STATE_TX_PROVISION_DIS_REQ)) {
		if (_TRUE ==
		    pwdinfo->tx_prov_disc_info.benable) {	/*	the provision discovery request frame is trigger to send or not */
...
int rtw_init_wifi_display_info(_adapter *padapter)
{
	...
	pwfd_info->rtsp_ctrlport =
		pwfd_info->init_rtsp_ctrlport; /* set non-zero value for legacy wfd */
#endif
...
void rtw_init_wifidirect_timers(_adapter *padapter)
{
	...
	rtw_init_timer(&pwdinfo->find_phase_timer, padapter,
		       find_phase_timer_process, padapter);
	rtw_init_timer(&pwdinfo->restore_p2p_state_timer, padapter,
		       restore_p2p_state_timer_process, padapter);
	rtw_init_timer(&pwdinfo->pre_tx_scan_timer, padapter,
		       pre_tx_scan_timer_process, padapter);
	rtw_init_timer(&pwdinfo->reset_ch_sitesurvey, padapter,
		       reset_ch_sitesurvey_timer_process, padapter);
	rtw_init_timer(&pwdinfo->reset_ch_sitesurvey2, padapter,
		       reset_ch_sitesurvey_timer_process2, padapter);
#ifdef CONFIG_CONCURRENT_MODE
	rtw_init_timer(&pwdinfo->ap_p2p_switch_timer, padapter,
		       ap_p2p_switch_timer_process, padapter);
#endif
}

void rtw_init_wifidirect_addrs(_adapter *padapter, u8 *dev_addr,
			       u8 *iface_addr)
{
#ifdef CONFIG_P2P
	struct wifidirect_info *pwdinfo = &padapter->wdinfo;
...
void init_wifidirect_info(_adapter *padapter, enum P2P_ROLE role)
{
	...
	pwdinfo->social_chan[3] =
		0;	/*	channel 0 for scanning ending in site survey function. */
	...
	_rtw_memset(&pwdinfo->invitereq_info, 0x00,
		    sizeof(struct tx_invite_req_info));
	pwdinfo->invitereq_info.token =
		3;	/*	Token used for P2P invitation request frame. */

	_rtw_memset(&pwdinfo->inviteresp_info, 0x00,
		    sizeof(struct tx_invite_resp_info));
	...
	_rtw_memset(&pwdinfo->tx_prov_disc_info, 0x00,
		    sizeof(struct tx_provdisc_req_info));
	pwdinfo->tx_prov_disc_info.wps_config_method_request = WPS_CM_NONE;

	_rtw_memset(&pwdinfo->nego_req_info, 0x00,
		    sizeof(struct tx_nego_req_info));
...
#else
	pwdinfo->supported_wps_cm = WPS_CONFIG_METHOD_DISPLAY |
				    WPS_CONFIG_METHOD_PBC | WPS_CONFIG_METHOD_KEYPAD;
#endif /* CONFIG_WFD */
	pwdinfo->channel_list_attr_len = 0;
	_rtw_memset(pwdinfo->channel_list_attr, 0x00, 100);

	_rtw_memset(
		pwdinfo->rx_prov_disc_info.strconfig_method_desc_of_prov_disc_req, 0x00,
		4);
	_rtw_memset(
		pwdinfo->rx_prov_disc_info.strconfig_method_desc_of_prov_disc_req, '0', 3);
	_rtw_memset(&pwdinfo->groupid_info, 0x00, sizeof(struct group_id_info));
#ifdef CONFIG_CONCURRENT_MODE
#ifdef CONFIG_IOCTL_CFG80211
	pwdinfo->ext_listen_interval =
		1000; /* The interval to be available with legacy AP during p2p0-find/scan */
	pwdinfo->ext_listen_period =
		3000; /* The time period to be available for P2P during nego */
	...
	pwdinfo->rx_invitereq_info.operation_ch[1] =
		0;	/*	Used to indicate the scan end in site survey function */
#ifdef CONFIG_P2P_OP_CHK_SOCIAL_CH
	pwdinfo->rx_invitereq_info.operation_ch[2] = 0;
	pwdinfo->rx_invitereq_info.operation_ch[3] = 0;
...
void dbg_rtw_p2p_set_state(struct wifidirect_info *wdinfo,
			   enum P2P_STATE state, const char *caller, int line)
{
...
void dbg_rtw_p2p_set_pre_state(struct wifidirect_info *wdinfo,
			       enum P2P_STATE state, const char *caller, int line)
{
...
#if 0
void dbg_rtw_p2p_restore_state(struct wifidirect_info *wdinfo,
...
#endif
void dbg_rtw_p2p_set_role(struct wifidirect_info *wdinfo,
			  enum P2P_ROLE role, const char *caller, int line)
{
	if (wdinfo->role != role) {
...
int rtw_p2p_enable(_adapter *padapter, enum P2P_ROLE role)
{
	...
	if (role == P2P_ROLE_DEVICE || role == P2P_ROLE_CLIENT
		...
		if (!rtw_p2p_chk_state(pwdinfo, P2P_STATE_NONE)) {
			...
			_rtw_memset(&pwdinfo->rx_prov_disc_info, 0x00,
				    sizeof(struct rx_provdisc_req_info));
