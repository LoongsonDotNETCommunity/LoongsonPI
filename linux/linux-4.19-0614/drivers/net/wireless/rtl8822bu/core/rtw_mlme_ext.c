...
static RT_CHANNEL_PLAN_MAP RTW_CHANNEL_PLAN_MAP_REALTEK_DEFINE =
	CHPLAN_ENT(RTW_RD_2G_WORLD,		RTW_RD_5G_FCC1,
		   TXPWR_LMT_FCC);		/* 0x7F, Realtek Define */
...
static u8 init_channel_set(_adapter *padapter, u8 ChannelPlan,
			   RT_CHANNEL_INFO *channel_set)
{
	...
	if (IsSupported24G(regsty->wireless_mode)
	    && hal_chk_band_cap(padapter, BAND_CAP_2G))
		b2_4GBand = _TRUE;

	if (is_supported_5g(regsty->wireless_mode)
	    && hal_chk_band_cap(padapter, BAND_CAP_5G))
		b5GBand = _TRUE;
	...
	if (b2_4GBand) {
		...
		for (index = 0; index < CH_LIST_LEN(RTW_ChannelPlan2G[Index2G]); index++) {
			if (rtw_regsty_is_excl_chs(regsty, CH_LIST_CH(RTW_ChannelPlan2G[Index2G],
						   index)) == _TRUE)
				continue;
			...
			channel_set[chanset_size].ChannelNum = CH_LIST_CH(
					RTW_ChannelPlan2G[Index2G], index);
	...
	if (b5GBand) {
		...
		for (index = 0; index < CH_LIST_LEN(RTW_ChannelPlan5G[Index5G]); index++) {
			if (rtw_regsty_is_excl_chs(regsty, CH_LIST_CH(RTW_ChannelPlan5G[Index5G],
						   index)) == _TRUE)
				continue;
#ifndef CONFIG_DFS
			...
			channel_set[chanset_size].ChannelNum = CH_LIST_CH(
					RTW_ChannelPlan5G[Index5G], index);
			...
			if ((ChannelPlan == RTW_CHPLAN_WORLD_WIDE_5G) /* all channels passive */
			    ...
			    || (rtw_is_dfs_ch(
					channel_set[chanset_size].ChannelNum)) /* DFS channel(band2, 3) passive */
			   )
				channel_set[chanset_size].ScanType = SCAN_PASSIVE;
			else
				channel_set[chanset_size].ScanType = SCAN_ACTIVE;
...
void rtw_txpwr_init_regd(struct rf_ctl_t *rfctl)
{
	...
	exc = _rtw_regd_exc_search(rfctl
				   ...
				   , rfctl->ChannelPlan);
	if (exc) {
		u8 has_country = (exc->country[0] == '\0'
				  && exc->country[1] == '\0') ? 0 : 1;
...
void rtw_rfctl_init(_adapter *adapter)
{
	...
	rfctl->max_chan_nums = init_channel_set(adapter, rfctl->ChannelPlan,
						rfctl->channel_set);
...
bool rtw_is_cac_reset_needed(_adapter *adapter, u8 ch, u8 bw, u8 offset)
{
	...
	if (rtw_chbw_to_freq_range(rfctl->radar_detect_ch, rfctl->radar_detect_bw,
				   rfctl->radar_detect_offset, &cur_hi, &cur_lo) == _FALSE) {
		RTW_ERR("cur detection range ch:%u, bw:%u, offset:%u\n",
			rfctl->radar_detect_ch, rfctl->radar_detect_bw,
	...
	if (!rtw_is_range_overlap(hi, lo, cur_hi, cur_lo)) {
		...
		if ((hi != cur_hi && rtw_is_dfs_range(hi, cur_hi)) || (lo != cur_lo
				&& rtw_is_dfs_range(cur_lo, lo)))
			needed = _TRUE;
	} else {
		/* request is not supper set of current, but has overlap */
		if ((lo < cur_lo && rtw_is_dfs_range(cur_lo, lo)) || (hi > cur_hi
				&& rtw_is_dfs_range(hi, cur_hi)))
			needed = _TRUE;
	}
...
bool _rtw_rfctl_overlap_radar_detect_ch(struct rf_ctl_t *rfctl, u8 ch,
					u8 bw, u8 offset)
{
	bool ret = _FALSE;
	u32 hi = 0, lo = 0;
...
bool rtw_chset_is_ch_non_ocp(RT_CHANNEL_INFO *ch_set, u8 ch, u8 bw,
			     u8 offset)
{
	bool ret = _FALSE;
	u32 hi = 0, lo = 0;
 ...
 */
static void _rtw_chset_update_non_ocp(RT_CHANNEL_INFO *ch_set, u8 ch,
				      u8 bw, u8 offset, int ms)
{
	u32 hi = 0, lo = 0;
	int i;
	...
	for (i = 0; i < MAX_CHANNEL_NUM && ch_set[i].ChannelNum != 0; i++) {
		...
		if (lo <= rtw_ch2freq(ch_set[i].ChannelNum)
			...
			if (ms >= 0)
				ch_set[i].non_ocp_end_time = rtw_get_current_time() + rtw_ms_to_systime(
								     ms);
			else
				ch_set[i].non_ocp_end_time = rtw_get_current_time() + rtw_ms_to_systime(
								     NON_OCP_TIME_MS);
...
inline void rtw_chset_update_non_ocp(RT_CHANNEL_INFO *ch_set, u8 ch, u8 bw,
				     u8 offset)
{
	_rtw_chset_update_non_ocp(ch_set, ch, bw, offset, -1);
}

inline void rtw_chset_update_non_ocp_ms(RT_CHANNEL_INFO *ch_set, u8 ch,
					u8 bw, u8 offset, int ms)
{
			  ...
			  u32 *r_non_ocp_ms, u32 *r_cac_ms)
{
	...
	if (rtw_chset_is_ch_non_ocp(rfctl->channel_set, ch, bw, offset))
		non_ocp_ms = rtw_chset_get_ch_non_ocp_ms(rfctl->channel_set, ch, bw,
				offset);
	else
		non_ocp_ms = 0;
	...
	if (!rtw_is_dfs_chbw(ch, bw, offset))
		...
		if (IS_CH_WAITING(rfctl))
			...
			cac_ms = 0;
	} else if (rtw_is_long_cac_ch(ch, bw, offset,
...
void rtw_reset_cac(_adapter *adapter, u8 ch, u8 bw, u8 offset)
{
	...
	rfctl->cac_start_time = rtw_get_current_time() + rtw_ms_to_systime(
					non_ocp_ms);
...
#define DBG_CHOOSE_SHORTEST_WAITING_CH 0
	...
	for (bw = CHANNEL_WIDTH_20; bw <= max_bw; bw++) {
		...
		for (i = 0; i < rfctl->max_chan_nums; i++) {
			...
			if ((d_flags & RTW_CHF_NON_OCP)
			    && rtw_chset_is_ch_non_ocp(rfctl->channel_set, ch, bw, offset))
				continue;

			if ((d_flags & RTW_CHF_DFS) && rtw_is_dfs_chbw(ch, bw, offset))
				continue;

			if ((d_flags & RTW_CHF_LONG_CAC)
			    && rtw_is_long_cac_ch(ch, bw, offset, rtw_odm_get_dfs_domain(adapter)))
				continue;

			if ((d_flags & RTW_CHF_NON_DFS) && !rtw_is_dfs_chbw(ch, bw, offset))
				continue;

			if ((d_flags & RTW_CHF_NON_LONG_CAC)
			    && !rtw_is_long_cac_ch(ch, bw, offset, rtw_odm_get_dfs_domain(adapter)))
				continue;

#ifdef CONFIG_DFS_MASTER
			waiting_ms = rtw_get_ch_waiting_ms(adapter, ch, bw, offset, &non_ocp_ms,
							   &cac_ms);
#endif
 ...
 */
u8 rtw_chset_is_chbw_valid(RT_CHANNEL_INFO *ch_set, u8 ch, u8 bw,
			   u8 offset)
{
	u8 cch;
	u8 *op_chs;
...
bool rtw_mlme_band_check(_adapter *adapter, const u32 ch)
{
	if (adapter->setband == WIFI_FREQUENCY_BAND_AUTO /* 2.4G and 5G */
	    || (adapter->setband == WIFI_FREQUENCY_BAND_2GHZ
		&& ch < 35) /* 2.4G only */
...
{
	if (RTW_IS_SCAN_BAND_SKIP(adapter, BAND_24G)
	    && ch < 35) /* SKIP 2.4G Band channel */
		return _TRUE;
	if (RTW_IS_SCAN_BAND_SKIP(adapter, BAND_5G)
	    && ch > 35) /* SKIP 5G Band channel */
		return _TRUE;
...
static void init_mlme_ext_priv_value(_adapter *padapter)
{
	...
	mlmeext_assign_scan_backop_flags_sta(
		pmlmeext, /*SS_BACKOP_EN|*/SS_BACKOP_PS_ANNC | SS_BACKOP_TX_RESUME);
	mlmeext_assign_scan_backop_flags_ap(pmlmeext,
...
void init_mlme_ext_timer(_adapter *padapter)
{
	...
	rtw_init_timer(&pmlmeext->ft_link_timer, padapter, rtw_ft_link_timer_hdl,
		       padapter);
	rtw_init_timer(&pmlmeext->ft_roam_timer, padapter, rtw_ft_roam_timer_hdl,
		       padapter);
#endif

#ifdef CONFIG_RTW_REPEATER_SON
	rtw_init_timer(&pmlmeext->rson_scan_timer, padapter, rson_timer_hdl,
		       padapter);
#endif
}
...
static u8 cmp_pkt_chnl_diff(_adapter *padapter, u8 *pframe,
			    uint packet_len)
{
	...
	u8 channel;
	u8 *p;
	p = rtw_get_ie(pframe + WLAN_HDR_A3_LEN + _BEACON_IE_OFFSET_, _DSSET_IE_,
		       &len, packet_len - _BEACON_IE_OFFSET_);
	if (p) {
		channel = *(p + 2);
...
static void _mgt_dispatcher(_adapter *padapter,
			    struct mlme_handler *ptable, union recv_frame *precv_frame)
{
	...
	if (ptable->func) {
		...
		if (!_rtw_memcmp(GetAddr1Ptr(pframe), adapter_mac_addr(padapter),
				 ETH_ALEN) &&
...
void mgt_dispatcher(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if ((index << 4) == WIFI_ACTION) {
		...
		if (*(pframe + 24) == RTW_WLAN_CATEGORY_PUBLIC
		    && *(pframe + 25) == TDLS_DISCOVERY_RESPONSE) {
			RTW_INFO("[TDLS] Recv %s from "MAC_FMT"\n",
				 rtw_tdls_action_txt(TDLS_DISCOVERY_RESPONSE),
...
_continue:
	...
	if (check_fwstate(pmlmepriv, _FW_LINKED) == _FALSE &&
	    check_fwstate(pmlmepriv,
			  WIFI_ADHOC_MASTER_STATE | WIFI_AP_STATE | WIFI_MESH_STATE) == _FALSE)
		return _SUCCESS;
	...
	wps_ie = rtw_get_wps_ie(
			 ...
			 NULL, &wps_ielen);
	if (wps_ie)
		target_ie = rtw_get_wps_attr_content(wps_ie, wps_ielen,
						     WPS_ATTR_MANUFACTURER, NULL, &target_ielen);
	if ((target_ie && (target_ielen == 4))
	    && (_TRUE == _rtw_memcmp((void *)target_ie, "Ozmo", 4))) {
		/* psta->flag_atmel_rc = 1; */
		unsigned char *sa_addr = get_sa(pframe);
		printk("%s: Find Ozmo RC -- %02x:%02x:%02x:%02x:%02x:%02x  \n\n",
		       __func__, *sa_addr, *(sa_addr + 1), *(sa_addr + 2), *(sa_addr + 3),
		       *(sa_addr + 4), *(sa_addr + 5));
	...
	if (check_fwstate(pmlmepriv, _FW_LINKED) == _TRUE &&
		...
		p = rtw_get_ie(pframe + WLAN_HDR_A3_LEN + _PROBEREQ_IE_OFFSET_,
			       _VENDOR_SPECIFIC_IE_, (int *)&ielen,
			       len - WLAN_HDR_A3_LEN - _PROBEREQ_IE_OFFSET_);
		...
		if (psta == NULL) {
			...
			psta->pid = (u16)(((mac_addr[4] << 8) + mac_addr[5]) + ((
						  peer_addr[4] << 8) + peer_addr[5]));
...
_non_rc_device:
	...
	if (((pmlmeinfo->state & 0x03) == WIFI_FW_AP_STATE) &&
	    rtw_mi_buddy_check_fwstate(padapter,
				       _FW_UNDER_LINKING | _FW_UNDER_SURVEY)) {
		/* don't process probe req */
		return _SUCCESS;
	}
#endif

	p = rtw_get_ie(pframe + WLAN_HDR_A3_LEN + _PROBEREQ_IE_OFFSET_, _SSID_IE_,
		       (int *)&ielen,
		       len - WLAN_HDR_A3_LEN - _PROBEREQ_IE_OFFSET_);
	...
	if (p != NULL) {
		...
		if ((ielen != 0
		     && _FALSE == _rtw_memcmp((void *)(p + 2), (void *)cur->Ssid.Ssid,
					      cur->Ssid.SsidLength))
		...
		if (MLME_IS_MESH(padapter)) {
			p = rtw_get_ie(pframe + WLAN_HDR_A3_LEN + _PROBEREQ_IE_OFFSET_,
				       WLAN_EID_MESH_ID, (int *)&ielen,
				       len - WLAN_HDR_A3_LEN - _PROBEREQ_IE_OFFSET_);

			if (!p)
				goto exit;
			if (ielen != 0
			    && _rtw_memcmp((void *)(p + 2), (void *)cur->mesh_id.Ssid,
					   cur->mesh_id.SsidLength) == _FALSE)
				goto exit;
		}
#endif
...
unsigned int OnProbeRsp(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if (rtw_p2p_chk_state(pwdinfo, P2P_STATE_TX_PROVISION_DIS_REQ)) {
		if (_TRUE == pwdinfo->tx_prov_disc_info.benable) {
			if (_rtw_memcmp(pwdinfo->tx_prov_disc_info.peerIFAddr,
					get_addr2_ptr(pframe), ETH_ALEN)) {
		...
		if (_TRUE == pwdinfo->nego_req_info.benable) {
			...
			if (_rtw_memcmp(pwdinfo->nego_req_info.peerDevAddr, get_addr2_ptr(pframe),
					ETH_ALEN)) {
		...
		if (_TRUE == pwdinfo->invitereq_info.benable) {
			...
			if (_rtw_memcmp(pwdinfo->invitereq_info.peer_macaddr,
					get_addr2_ptr(pframe), ETH_ALEN)) {
				pwdinfo->invitereq_info.benable = _FALSE;
				issue_p2p_invitation_request(padapter,
	...
	if (_rtw_memcmp(GetAddr3Ptr(pframe), get_my_bssid(&pmlmeinfo->network),
			ETH_ALEN)) {
...
unsigned int OnBeacon(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if (_rtw_memcmp(GetAddr3Ptr(pframe), get_my_bssid(&pmlmeinfo->network),
		...
		if ((pmlmeinfo->state & WIFI_FW_AUTH_NULL)
			...
			pmlmeinfo->assoc_AP_vendor = check_assoc_AP(pframe + sizeof(
			...
			_rtw_memset(pmlmeext->bcn_delay_ratio, 0,
			...
			if (padapter->registrypriv.wifi_spec) {
				if (process_p2p_cross_connect_ie(padapter, (pframe + WLAN_HDR_A3_LEN),
								 (len - WLAN_HDR_A3_LEN)) == _FALSE) {
		...
		if (((pmlmeinfo->state & 0x03) == WIFI_FW_STATION_STATE)
			...
			if (psta != NULL) {
				...
				if (cmp_pkt_chnl_diff(padapter, pframe, len) != 0) {
					...
					issue_deauth(padapter, (&(pmlmeinfo->network))->MacAddress,
						     WLAN_REASON_DEAUTH_LEAVING);

					report_del_sta_event(padapter, (&(pmlmeinfo->network))->MacAddress,
							     WLAN_REASON_JOIN_WRONG_CHANNEL, _TRUE, _FALSE);
					pmlmeinfo->state &= (~WIFI_FW_ASSOC_SUCCESS);
					return _SUCCESS;
				}
				...
				pmlmepriv->cur_network_scanned->network.Rssi =
					precv_frame->u.hdr.attrib.phy_info.recv_signal_power;
				...
				if (rtw_tdls_is_chsw_allowed(padapter) == _TRUE) {
					...
					if ((padapter->tdlsinfo.chsw_info.ch_sw_state & TDLS_CH_SW_INITIATOR_STATE)
						...
						ptdls_sta = rtw_get_stainfo(&padapter->stapriv,
									    padapter->tdlsinfo.chsw_info.addr);
						if (ptdls_sta != NULL) {
							if (ptdls_sta->tdls_sta_state | TDLS_LINKED_STATE)
								_set_timer(&ptdls_sta->stay_on_base_chnl_timer,
									   TDLS_CH_SW_STAY_ON_BASE_CHNL_TIMEOUT);
						}
					}
				}
#endif
...
#ifdef CONFIG_P2P_PS
				process_p2p_ps_ie(padapter, (pframe + WLAN_HDR_A3_LEN),
						  (len - WLAN_HDR_A3_LEN));
#endif /* CONFIG_P2P_PS */
			...
			if (psta != NULL) {
				...
				*/
				...
				rtw_ies_get_supported_rate(pframe + WLAN_HDR_A3_LEN + _BEACON_IE_OFFSET_,
							   len - WLAN_HDR_A3_LEN - _BEACON_IE_OFFSET_, rate_set, &rate_num);
				if (rate_num == 0) {
...
unsigned int OnAuth(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if (MLME_IS_MESH(padapter)) {
		...
		if (!rtw_mesh_plink_get(padapter, get_addr2_ptr(pframe))) {
			if (adapter_to_rfctl(padapter)->offch_state == OFFCHS_NONE)
				issue_probereq(padapter, &padapter->mlmepriv.cur_network.network.mesh_id,
					       get_addr2_ptr(pframe));
	...
	algorithm = le16_to_cpu(*(u16 *)((SIZE_PTR)pframe + WLAN_HDR_A3_LEN +
					 offset));
	seq	= le16_to_cpu(*(u16 *)((SIZE_PTR)pframe + WLAN_HDR_A3_LEN + offset +
				       2));
	...
	if ((algorithm > 0 && auth_mode == 0)
	    ...
	    (algorithm == 0
	     && auth_mode == 1)) {	/* rx a open-system auth but shared-key is enabled */
...
unsigned int OnAuthClient(_adapter *padapter,
			  union recv_frame *precv_frame)
{
	...
	algthm	= le16_to_cpu(*(unsigned short *)((SIZE_PTR)pframe + WLAN_HDR_A3_LEN
			      + offset));
	seq	= le16_to_cpu(*(unsigned short *)((SIZE_PTR)pframe + WLAN_HDR_A3_LEN +
			      offset + 2));
	status	= le16_to_cpu(*(unsigned short *)((SIZE_PTR)pframe + WLAN_HDR_A3_LEN
			      + offset + 4));
	...
	if (seq == 2) {
		if (pmlmeinfo->auth_algo == dot11AuthAlgrthm_Shared) {
			...
			p = rtw_get_ie(pframe + WLAN_HDR_A3_LEN + _AUTH_IE_OFFSET_, _CHLGETXT_IE_,
				       (int *)&len,
...
unsigned int OnAssocReq(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if (rtw_p2p_chk_role(pwdinfo, P2P_ROLE_GO)) {
		p2pie = rtw_get_p2p_ie(pframe + WLAN_HDR_A3_LEN + ie_offset,
				       pkt_len - WLAN_HDR_A3_LEN - ie_offset, NULL, &p2pielen);
		if (p2pie) {
			pstat->is_p2p_device = _TRUE;
			p2p_status_code = (u8)process_assoc_req_p2p_ie(pwdinfo, pframe, pkt_len,
					  pstat);
...
#ifdef CONFIG_WFD
		rtw_process_wfd_ies(padapter, pframe + WLAN_HDR_A3_LEN + ie_offset,
				    pkt_len - WLAN_HDR_A3_LEN - ie_offset, __func__);
#endif
	}
	pstat->p2p_status_code = p2p_status_code;
#endif /* CONFIG_P2P */
...
#ifdef CONFIG_IEEE80211W
		if (pstat->bpairwise_key_installed == _TRUE
		    && SEC_IS_BIP_KEY_INSTALLED(&padapter->securitypriv) == _TRUE) {
			RTW_INFO(MAC_FMT"\n", MAC_ARG(pstat->cmn.mac_addr));
			issue_action_SA_Query(padapter, pstat->cmn.mac_addr, 0, 0,
...
unsigned int OnAssocRsp(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	/* get capabilities */
	pmlmeinfo->capability = le16_to_cpu(*(unsigned short *)(
	...
	/* AID */
	res = pmlmeinfo->aid = (int)(le16_to_cpu(*(unsigned short *)(
					     pframe + WLAN_HDR_A3_LEN + 4)) & 0x3fff);
...
report_assoc_result:
	if (res > 0)
		rtw_buf_update(&pmlmepriv->assoc_rsp, &pmlmepriv->assoc_rsp_len, pframe,
			       pkt_len);
	else
...
unsigned int on_action_spct_ch_switch(_adapter *padapter,
				      struct sta_info *psta, u8 *ies, uint ies_len)
{
...
unsigned int on_action_spct(_adapter *padapter,
			    union recv_frame *precv_frame)
{
	unsigned int ret = _FAIL;
...
unsigned int OnAction_qos(_adapter *padapter,
			  union recv_frame *precv_frame)
{
	return _SUCCESS;
}

unsigned int OnAction_dls(_adapter *padapter,
			  union recv_frame *precv_frame)
{
	return _SUCCESS;
}
...
u8 rtw_rx_ampdu_size(_adapter *adapter)
{
	...
	if (!mlmeext_chk_scan_state(&adapter->mlmeextpriv, SCAN_DISABLE)
	    ...
	    && adapter->mlmeextpriv.sitesurvey_res.rx_ampdu_size !=
...
bool rtw_rx_ampdu_is_accept(_adapter *adapter)
{
	...
	if (!mlmeext_chk_scan_state(&adapter->mlmeextpriv, SCAN_DISABLE)
	    && !mlmeext_chk_scan_state(&adapter->mlmeextpriv, SCAN_COMPLETE)
	    && adapter->mlmeextpriv.sitesurvey_res.rx_ampdu_accept !=
 ...
 */
u8 rx_ampdu_apply_sta_tid(_adapter *adapter, struct sta_info *sta, u8 tid,
			  u8 accept, u8 size)
{
	u8 ret = 0;
...
u8 rx_ampdu_size_sta_limit(_adapter *adapter, struct sta_info *sta)
{
	...
	if (is_supported_vht(sta->wireless_mode)) {
		...
		if (is_supported_ht(sta->wireless_mode)) {
			nss = rtw_min(rtw_ht_mcsset_to_nss(
					      mlmeinfo->HT_caps.u.HT_cap_element.MCS_rate)
 ...
 * Blocking function, may sleep
 */
u8 rx_ampdu_apply_sta(_adapter *adapter, struct sta_info *sta, u8 accept,
		      u8 size)
{
	u8 change_cnt = 0;
	int i;
...
unsigned int OnAction_back(_adapter *padapter,
			   union recv_frame *precv_frame)
{
	u8 *addr;
	struct sta_info *psta = NULL;
	...
	if (!_rtw_memcmp(adapter_mac_addr(padapter), GetAddr1Ptr(pframe),
			 ETH_ALEN))
		return _SUCCESS;
	...
	if (category == RTW_WLAN_CATEGORY_BACK) { /* representing Block Ack */
		...
		case RTW_WLAN_ACTION_ADDBA_REQ: /* ADDBA request */
			...
			_rtw_memcpy(&(pmlmeinfo->ADDBA_req), &(frame_body[2]),
...
void issue_p2p_GO_request(_adapter *padapter, u8 *raddr)
{
	...
	*(u16 *)(p2pie + p2pielen) = cpu_to_be16(pwdinfo->device_name_len);
	...
	/*	Value: */
	_rtw_memcpy(p2pie + p2pielen, pwdinfo->device_name,
	...
	*(u16 *)(p2pie + p2pielen) = cpu_to_le16(0x0005);
	...
	/*	Channel Number */
	p2pie[p2pielen++] =
		pwdinfo->operating_channel;	/*	operating channel number */
			   ...
			   uint len, u8 result)
{
	...
	*(fctrl) = 0;
	...
	pwdinfo->negotiation_dialog_token =
		frame_body[7];	/*	The Dialog Token of provisioning discovery request frame. */
	pframe = rtw_set_fixed_ie(pframe, 1, &(pwdinfo->negotiation_dialog_token),
				  &(pattrib->pktlen));
	...
	rtw_get_wps_attr_content(wpsie, wpsielen, WPS_ATTR_DEVICE_PWID,
				 (u8 *) &wps_devicepassword_id, &wps_devicepassword_id_len);
	...
	*(u16 *)(wpsie + wpsielen) = cpu_to_be16(0x0002);
	...
	if (_rtw_memcmp(
		    pwdinfo->rx_prov_disc_info.strconfig_method_desc_of_prov_disc_req, "000",
		    3)) {
		if (wps_devicepassword_id == WPS_DPID_USER_SPEC)
			_rtw_memcpy(
				pwdinfo->rx_prov_disc_info.strconfig_method_desc_of_prov_disc_req, "dis",
				3);
		else if (wps_devicepassword_id == WPS_DPID_REGISTRAR_SPEC)
			_rtw_memcpy(
				pwdinfo->rx_prov_disc_info.strconfig_method_desc_of_prov_disc_req, "pad",
				3);
		else
			_rtw_memcpy(
				pwdinfo->rx_prov_disc_info.strconfig_method_desc_of_prov_disc_req, "pbc",
				3);
	}
...
void issue_p2p_GO_confirm(_adapter *padapter, u8 *raddr, u8 result)
{
	...
	*(u16 *)(p2pie + p2pielen) = cpu_to_le16(0x0005);
	...
	if (rtw_p2p_chk_role(pwdinfo, P2P_ROLE_CLIENT)) {
		...
		/*	Channel Number */
		p2pie[p2pielen++] =
			pwdinfo->operating_channel;		/*	Use the listen channel as the operating channel */
	}
...
void issue_p2p_invitation_request(_adapter *padapter, u8 *raddr)
{
	...
	*(u16 *)(p2pie + p2pielen) = cpu_to_le16(0x0005);
	...
	/*	Channel Number */
	p2pie[p2pielen++] =
		pwdinfo->invitereq_info.operating_ch;	/*	operating channel number */
	...
	*(u16 *)(p2pie + p2pielen) = cpu_to_le16(len_channellist_attr);
	...
	/*	Length: */
	*(u16 *)(p2pie + p2pielen) = cpu_to_le16(6 +
	...
	/*	SSID */
	_rtw_memcpy(p2pie + p2pielen, pwdinfo->invitereq_info.go_ssid,
		    pwdinfo->invitereq_info.ssidlen);
...
void issue_p2p_invitation_response(_adapter *padapter, u8 *raddr,
...
void issue_p2p_provision_request(_adapter *padapter, u8 *pssid,
				 u8 ussidlen, u8 *pdev_raddr)
{
	...
	*(fctrl) = 0;
	...
	p2pielen = build_prov_disc_request_p2p_ie(pwdinfo, pframe, pssid, ussidlen,
			pdev_raddr);
	...
	*(u16 *)(wpsie + wpsielen) = cpu_to_be16(0x0002);
	...
	/*	Value: */
	*(u16 *)(wpsie + wpsielen) = cpu_to_be16(
...
void issue_probersp_p2p(_adapter *padapter, unsigned char *da)
{
	...
	*(fctrl) = 0;
	...
	/* SSID */
	pframe = rtw_set_ie(pframe, _SSID_IE_, 7, pwdinfo->p2p_wildcard_ssid,
			    &pattrib->pktlen);

	/* supported rates... */
	/*	Use the OFDM rate in the P2P probe response frame. ( 6(B), 9(B), 12, 18, 24, 36, 48, 54 ) */
	pframe = rtw_set_ie(pframe, _SUPPORTEDRATES_IE_, 8, pwdinfo->support_rate,
			    &pattrib->pktlen);
	...
	if (adapter_wdev_data(padapter)->p2p_enabled
		...
		if (pmlmepriv->wps_probe_resp_ie != NULL
			...
			/* WPS IE */
			_rtw_memcpy(pframe, pmlmepriv->wps_probe_resp_ie,
				    pmlmepriv->wps_probe_resp_ie_len);
			pattrib->pktlen += pmlmepriv->wps_probe_resp_ie_len;
			pframe += pmlmepriv->wps_probe_resp_ie_len;

			/* P2P IE */
			_rtw_memcpy(pframe, pmlmepriv->p2p_probe_resp_ie,
...
#endif /* CONFIG_IOCTL_CFG80211		 */
	{
		...
		*(u16 *)(wpsie + wpsielen) = cpu_to_be16(0x0001);
		...
		if (_rtw_memcmp(pmlmepriv->sa_ext, zero_array_check,
				L2SDTA_SERVICE_VE_LEN) == _FALSE
		...
		*(u16 *)(wpsie + wpsielen) = cpu_to_be16(pwdinfo->device_name_len);
		...
		/*	Value: */
		_rtw_memcpy(wpsie + wpsielen, pwdinfo->device_name,
			    pwdinfo->device_name_len);
	...
	*(fctrl) = 0;
	...
	if (rtw_p2p_chk_state(pwdinfo, P2P_STATE_TX_PROVISION_DIS_REQ))
		pframe = rtw_set_ie(pframe, _SSID_IE_,
				    pwdinfo->tx_prov_disc_info.ssid.SsidLength,
				    pwdinfo->tx_prov_disc_info.ssid.Ssid, &(pattrib->pktlen));
	else
		pframe = rtw_set_ie(pframe, _SSID_IE_, P2P_WILDCARD_SSID_LEN,
	...
	if (adapter_wdev_data(padapter)->p2p_enabled
		...
		if (pmlmepriv->wps_probe_req_ie != NULL
			...
			/* WPS IE */
			_rtw_memcpy(pframe, pmlmepriv->wps_probe_req_ie,
				    pmlmepriv->wps_probe_req_ie_len);
			pattrib->pktlen += pmlmepriv->wps_probe_req_ie_len;
			pframe += pmlmepriv->wps_probe_req_ie_len;

			/* P2P IE */
			_rtw_memcpy(pframe, pmlmepriv->p2p_probe_req_ie,
...
#endif /* CONFIG_IOCTL_CFG80211 */
	{
		...
		*(u16 *)(wpsie + wpsielen) = cpu_to_be16(0x0002);
		...
		/*	Value: */
		*(u16 *)(wpsie + wpsielen) = cpu_to_be16(
						     WPS_DPID_REGISTRAR_SPEC);	/*	Registrar-specified */
		wpsielen += 2;
 ...
 */
int issue_probereq_p2p_ex(_adapter *adapter, u8 *da, int try_cnt,
			  int wait_ms)
{
	int ret;
	int i = 0;
...
s32 rtw_action_public_decache(union recv_frame *rframe, u8 token_offset)
{
	...
	struct mlme_ext_priv *mlmeext = &(adapter->mlmeextpriv);
	u8 *frame = rframe->u.hdr.rx_data;
	u16 seq_ctrl = ((rframe->u.hdr.attrib.seq_num & 0xffff) << 4) |
		       (rframe->u.hdr.attrib.frag_num & 0xf);
	u8 token = *(rframe->u.hdr.rx_data + sizeof(struct rtw_ieee80211_hdr_3addr)
		     + token_offset);
...
unsigned int on_action_public_p2p(union recv_frame *precv_frame)
{
...
#endif /* CONFIG_IOCTL_CFG80211 */
	{
		...
		case P2P_GO_NEGO_REQ: {
			...
			if (_rtw_memcmp(pwdinfo->rx_prov_disc_info.peerDevAddr, empty_addr,
					ETH_ALEN))
				_rtw_memcpy(pwdinfo->rx_prov_disc_info.peerDevAddr, get_addr2_ptr(pframe),
					    ETH_ALEN);

			result = process_p2p_group_negotation_req(pwdinfo, frame_body, len);
			issue_p2p_GO_response(padapter, get_addr2_ptr(pframe), frame_body, len,
					      result);
#ifdef CONFIG_INTEL_WIDI
			...
			if (rtw_p2p_chk_state(pwdinfo, P2P_STATE_GONEGO_ING)) {
				...
				if (P2P_STATUS_SUCCESS == result) {
					if (rtw_p2p_role(pwdinfo) == P2P_ROLE_CLIENT) {
						...
						pwdinfo->p2p_info.operation_ch[1] =
							1;	/* Check whether GO is operating in channel 1; */
						pwdinfo->p2p_info.operation_ch[2] =
							6;	/* Check whether GO is operating in channel 6; */
						pwdinfo->p2p_info.operation_ch[3] =
							11;	/* Check whether GO is operating in channel 11; */
		...
		case P2P_INVIT_REQ: {
			...
			p2p_ie = rtw_get_p2p_ie(frame_body + _PUBLIC_ACTION_IE_OFFSET_,
						len - _PUBLIC_ACTION_IE_OFFSET_, NULL, &p2p_ielen);
			if (p2p_ie) {
				...
				merged_p2p_ielen = rtw_get_p2p_merged_ies_len(frame_body +
						   _PUBLIC_ACTION_IE_OFFSET_, len - _PUBLIC_ACTION_IE_OFFSET_);

				merged_p2pie = rtw_zmalloc(merged_p2p_ielen +
							   2);	/* 2 is for EID and Length */
				...
				merged_p2p_ielen = rtw_p2p_merge_ies(frame_body +
								     _PUBLIC_ACTION_IE_OFFSET_, len - _PUBLIC_ACTION_IE_OFFSET_, merged_p2pie);

				rtw_get_p2p_attr_content(merged_p2pie, merged_p2p_ielen,
							 P2P_ATTR_INVITATION_FLAGS, &invitation_flag, &attr_contentlen);
				if (attr_contentlen) {

					rtw_get_p2p_attr_content(merged_p2pie, merged_p2p_ielen,
								 P2P_ATTR_GROUP_BSSID, pwdinfo->p2p_peer_interface_addr, &attr_contentlen);
					...
					if (invitation_flag & P2P_INVITATION_FLAGS_PERSISTENT) {
						...
						rtw_get_p2p_attr_content(merged_p2pie, merged_p2p_ielen, P2P_ATTR_GROUP_ID,
									 (u8 *) &group_id, &attr_contentlen);
						if (attr_contentlen) {
							if (_rtw_memcmp(group_id.go_device_addr, adapter_mac_addr(padapter),
									ETH_ALEN)) {
								...
								if (is_matched_in_profilelist(pwdinfo->p2p_peer_interface_addr,
									...
									if (rtw_get_p2p_attr_content(merged_p2pie, merged_p2p_ielen,
												     P2P_ATTR_OPERATING_CH, operatingch_info,
												     &attr_contentlen)) {
										if (rtw_chset_search_ch(adapter_to_chset(padapter),
													(u32)operatingch_info[4]) >= 0) {
											/*	The operating channel is acceptable for this device. */
											pwdinfo->rx_invitereq_info.operation_ch[0] = operatingch_info[4];
#ifdef CONFIG_P2P_OP_CHK_SOCIAL_CH
											pwdinfo->rx_invitereq_info.operation_ch[1] =
												1;		/* Check whether GO is operating in channel 1; */
											pwdinfo->rx_invitereq_info.operation_ch[2] =
												6;		/* Check whether GO is operating in channel 6; */
											pwdinfo->rx_invitereq_info.operation_ch[3] =
												11;		/* Check whether GO is operating in channel 11; */
#endif /* CONFIG_P2P_OP_CHK_SOCIAL_CH */
									...
									rtw_p2p_set_state(pwdinfo, P2P_STATE_RECV_INVITE_REQ_DISMATCH);
#ifdef CONFIG_INTEL_WIDI
									_rtw_memcpy(pwdinfo->p2p_peer_device_addr, group_id.go_device_addr,
										    ETH_ALEN);
									rtw_p2p_set_role(pwdinfo, P2P_ROLE_CLIENT);
#endif /* CONFIG_INTEL_WIDI */
				...
				issue_p2p_invitation_response(padapter, get_addr2_ptr(pframe),
							      pwdinfo->inviteresp_info.token, status_code);
				_set_timer(&pwdinfo->restore_p2p_state_timer, 3000);
			}
#ifdef CONFIG_INTEL_WIDI
			if (padapter->mlmepriv.widi_state == INTEL_WIDI_STATE_LISTEN) {
		...
		case P2P_INVIT_RESP: {
			...
			p2p_ie = rtw_get_p2p_ie(frame_body + _PUBLIC_ACTION_IE_OFFSET_,
						len - _PUBLIC_ACTION_IE_OFFSET_, NULL, &p2p_ielen);
			if (p2p_ie) {
				rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_STATUS, &attr_content,
							 &attr_contentlen);
				...
				if (attr_contentlen == 1) {
					...
					if (attr_content == P2P_STATUS_SUCCESS) {
						if (_rtw_memcmp(pwdinfo->invitereq_info.go_bssid,
...
unsigned int on_action_public_default(union recv_frame *precv_frame,
				      u8 action)
{
	unsigned int ret = _FAIL;
	u8 *pframe = precv_frame->u.hdr.rx_data;
...
unsigned int on_action_public(_adapter *padapter,
			      union recv_frame *precv_frame)
{
	unsigned int ret = _FAIL;
	u8 *pframe = precv_frame->u.hdr.rx_data;
...
unsigned int OnAction_ft(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	case RTW_WLAN_ACTION_FT_RSP:
		...
		status_code = le16_to_cpu(*(u16 *)((SIZE_PTR)pframe +  sizeof(
				struct rtw_ieee80211_hdr_3addr) + 14));
		if (status_code != 0) {
			RTW_ERR("FT: WLAN ACTION FT RESPONSE fail, status: %d\n", status_code);
			goto exit;
		}

		if (is_zero_mac_addr(&pframe_body[8])
		    || is_broadcast_mac_addr(&pframe_body[8])) {
		...
		_rtw_memcpy(pft_roam->ft_action, pframe, rtw_min(frame_len,
				RTW_FT_MAX_IE_SZ));
		ret = _SUCCESS;
		break;
...
static void rtw_wnm_btm_req_hdr_parsing(u8 *pframe,
					struct btm_req_hdr *phdr)
{
	u8 *pos = pframe;
	u32 offset = 0;
...
void rtw_wnm_issue_action(_adapter *padapter, u8 action, u8 reason)
{
	...
	*(fctrl) = 0;
	...
	case RTW_WLAN_ACTION_WNM_BTM_RSP:
		...
		pframe = rtw_set_fixed_ie(pframe, 1, &(dialog_token), &(pattrib->pktlen));
		pframe = rtw_set_fixed_ie(pframe, 1, &(reason), &(pattrib->pktlen));
		pframe = rtw_set_fixed_ie(pframe, 1, &(termination_delay),
					  &(pattrib->pktlen));
...
#ifdef CONFIG_IEEE80211W
unsigned int OnAction_sa_query(_adapter *padapter,
			       union recv_frame *precv_frame)
{
	u8 *pframe = precv_frame->u.hdr.rx_data;
	struct rx_pkt_attrib *pattrib = &precv_frame->u.hdr.attrib;
	...
	case 0: /* SA Query req */
		...
		RTW_INFO("OnAction_sa_query request,action=%d, tid=%04x, pframe=%02x-%02x\n"
			 , pframe[WLAN_HDR_A3_LEN + 1], tid, pframe[WLAN_HDR_A3_LEN + 2],
			 pframe[WLAN_HDR_A3_LEN + 3]);
		issue_action_SA_Query(padapter, get_addr2_ptr(pframe), 1, tid,
				      IEEE80211W_RIGHT_KEY);
		break;
...
unsigned int on_action_rm(_adapter *padapter,
			  union recv_frame *precv_frame)
{
#ifdef CONFIG_RTW_80211K
...
unsigned int OnAction_wmm(_adapter *padapter,
			  union recv_frame *precv_frame)
{
	return _SUCCESS;
}

unsigned int OnAction_vht(_adapter *padapter,
			  union recv_frame *precv_frame)
{
#ifdef CONFIG_80211AC_VHT
	struct rx_pkt_attrib *prxattrib = &precv_frame->u.hdr.attrib;
	u8 *pframe = precv_frame->u.hdr.rx_data;
	uint frame_len = precv_frame->u.hdr.len;
	struct rtw_ieee80211_hdr_3addr *whdr = (struct rtw_ieee80211_hdr_3addr *)
					       pframe;
	u8 *frame_body = pframe + sizeof(struct rtw_ieee80211_hdr_3addr);
	u8 category, action;
	struct sta_info *psta = NULL;
...
unsigned int OnAction_p2p(_adapter *padapter,
			  union recv_frame *precv_frame)
{
#ifdef CONFIG_P2P
	u8 *frame_body;
...
struct xmit_frame *_alloc_mgtxmitframe(struct xmit_priv *pxmitpriv,
				       bool once)
{
	struct xmit_frame *pmgntframe;
...
inline struct xmit_frame *alloc_mgtxmitframe_once(struct xmit_priv
		*pxmitpriv)
{
...
void update_monitor_frame_attrib(_adapter *padapter,
				 struct pkt_attrib *pattrib)
{
...
void update_mgntframe_attrib(_adapter *padapter,
			     struct pkt_attrib *pattrib)
{
	u8	wireless_mode;
	struct mlme_ext_priv	*pmlmeext = &(padapter->mlmeextpriv);
...
void update_mgntframe_attrib_addr(_adapter *padapter,
				  struct xmit_frame *pmgntframe)
{
	u8	*pframe;
...
s32 dump_mgntframe_and_wait(_adapter *padapter,
...
s32 dump_mgntframe_and_wait_ack(_adapter *padapter,
				struct xmit_frame *pmgntframe)
{
...
u32 rtw_build_vendor_ie(_adapter *padapter, unsigned char *pframe,
			u8 mgmt_frame_tyte)
{
	int vendor_ie_num = 0;
	struct mlme_priv *pmlmepriv = &(padapter->mlmepriv);
	...
	for (vendor_ie_num = 0 ; vendor_ie_num < WLAN_MAX_VENDOR_IE_NUM ;
	     vendor_ie_num++) {
		if (pmlmepriv->vendor_ielen[vendor_ie_num] > 0
		    && pmlmepriv->vendor_ie_mask[vendor_ie_num] & mgmt_frame_tyte) {
			_rtw_memcpy(pframe, pmlmepriv->vendor_ie[vendor_ie_num],
				    pmlmepriv->vendor_ielen[vendor_ie_num]);
...
void issue_beacon(_adapter *padapter, int timeout_ms)
{
	...
	*(fctrl) = 0;
	...
	if (MLME_IS_AP(padapter) || MLME_IS_MESH(padapter)) {
		...
		u8 *wpsie = NULL;
		wpsie = rtw_get_wps_ie(cur_network->IEs + _FIXED_IE_LENGTH_,
				       cur_network->IELength - _FIXED_IE_LENGTH_, NULL, &wpsielen);
		...
		if (rtw_p2p_chk_role(pwdinfo, P2P_ROLE_GO) && wpsie && wpsielen > 0) {
			...
			if (adapter_wdev_data(padapter)->p2p_enabled
				...
				if (pmlmepriv->wps_beacon_ie && pmlmepriv->wps_beacon_ie_len > 0) {
					...
					_rtw_memcpy(pframe, pmlmepriv->wps_beacon_ie,
...
#endif /* CONFIG_IOCTL_CFG80211 */
			{
				...
				*(u16 *)(pframe + insert_len) = cpu_to_be16(pwdinfo->device_name_len);
				...
				/*	Value: */
				_rtw_memcpy(pframe + insert_len, pwdinfo->device_name,
					    pwdinfo->device_name_len);
...

		{
			...
			uint wps_ielen;
			u8 sr = 0;
			wps_ie = rtw_get_wps_ie(pmgntframe->buf_addr + TXDESC_OFFSET + sizeof(
							struct rtw_ieee80211_hdr_3addr) + _BEACON_IE_OFFSET_,
						pattrib->pktlen - sizeof(struct rtw_ieee80211_hdr_3addr) -
						_BEACON_IE_OFFSET_, NULL, &wps_ielen);
			if (wps_ie && wps_ielen > 0)
				rtw_get_wps_attr_content(wps_ie,  wps_ielen, WPS_ATTR_SELECTED_REGISTRAR,
							 (u8 *)(&sr), NULL);
			if (sr != 0)
...
#endif
#ifdef CONFIG_APPEND_VENDOR_IE_ENABLE
		pattrib->pktlen += rtw_build_vendor_ie(padapter, pframe,
						       WIFI_BEACON_VENDOR_IE_BIT);
#endif
		goto _issue_bcn;
	...
	pframe = rtw_set_ie(pframe, _SUPPORTEDRATES_IE_,
			    ((rate_len > 8) ? 8 : rate_len), cur_network->SupportedRates,
			    &pattrib->pktlen);
	...
	if (rate_len > 8)
		pframe = rtw_set_ie(pframe, _EXT_SUPPORTEDRATES_IE_, (rate_len - 8),
				    (cur_network->SupportedRates + 8), &pattrib->pktlen);
	...
	*(fctrl) = 0;
	...
	if ((pmlmeinfo->state & 0x03) == WIFI_FW_AP_STATE) {
		...
		/* retrieve SSID IE from cur_network->Ssid */
		{
			...
			u8 buf[MAX_IE_SZ];
			u8 *ies = pmgntframe->buf_addr + TXDESC_OFFSET + sizeof(
					  struct rtw_ieee80211_hdr_3addr);
			...
			if (ssid_ie &&  cur_network->Ssid.SsidLength) {
				...
				*(ssid_ie + 1) = cur_network->Ssid.SsidLength;
				_rtw_memcpy(ssid_ie + 2, cur_network->Ssid.Ssid,
...
#ifdef CONFIG_AUTO_AP_MODE
	{
		...
		if (psta && psta->isrc && psta->pid > 0) {
			...
			pframe = rtw_set_ie(pframe, _VENDOR_SPECIFIC_IE_, sizeof(RC_INFO), RC_INFO,
					    &pattrib->pktlen);
		}
	}
#endif /* CONFIG_AUTO_AP_MODE */
...
int _issue_probereq(_adapter *padapter, const NDIS_802_11_SSID *pssid,
		    const u8 *da, u8 ch, bool append_wps, int wait_ack)
{
	int ret = _FAIL;
	struct xmit_frame		*pmgntframe;
	struct pkt_attrib		*pattrib;
	...
	*(fctrl) = 0;
	...
	if (pssid && !MLME_IS_MESH(padapter))
		pframe = rtw_set_ie(pframe, _SSID_IE_, pssid->SsidLength, pssid->Ssid,
				    &(pattrib->pktlen));
	else
	...
	if (bssrate_len > 8) {
		pframe = rtw_set_ie(pframe, _SUPPORTEDRATES_IE_, 8, bssrate,
				    &(pattrib->pktlen));
		pframe = rtw_set_ie(pframe, _EXT_SUPPORTEDRATES_IE_, (bssrate_len - 8),
				    (bssrate + 8), &(pattrib->pktlen));
	} else
	...
	if (MLME_IS_MESH(padapter)) {
		if (pssid)
			pframe = rtw_set_ie_mesh_id(pframe, &pattrib->pktlen, pssid->Ssid,
						    pssid->SsidLength);
		else
...
inline void issue_probereq(_adapter *padapter,
 ...
 * try_cnt means the maximal TX count to try
 */
int issue_probereq_ex(_adapter *padapter, const NDIS_802_11_SSID *pssid,
		      const u8 *da, u8 ch, bool append_wps,
...
void issue_auth(_adapter *padapter, struct sta_info *psta,
		unsigned short status)
{
	struct xmit_frame			*pmgntframe;
	struct pkt_attrib			*pattrib;
	...
	*(fctrl) = 0;
	...
	if (psta) { /* for AP mode */
		...
		pframe = rtw_set_fixed_ie(pframe, _AUTH_ALGM_NUM_, (unsigned char *)&val16,
					  &(pattrib->pktlen));

		/* setting auth seq number */
		val16 = (u16)psta->auth_seq;
		val16 = cpu_to_le16(val16);
		pframe = rtw_set_fixed_ie(pframe, _AUTH_SEQ_NUM_, (unsigned char *)&val16,
					  &(pattrib->pktlen));

		/* setting status code... */
		val16 = status;
		val16 = cpu_to_le16(val16);
		pframe = rtw_set_fixed_ie(pframe, _STATUS_CODE_, (unsigned char *)&val16,
					  &(pattrib->pktlen));

		/* added challenging text... */
		if ((psta->auth_seq == 2) && (psta->state & WIFI_FW_AUTH_STATE)
		    && (use_shared_key == 1))
			pframe = rtw_set_ie(pframe, _CHLGETXT_IE_, 128, psta->chg_txt,
					    &(pattrib->pktlen));
#endif
	} else {
		_rtw_memcpy(pwlanhdr->addr1, get_my_bssid(&pmlmeinfo->network), ETH_ALEN);
		...
		{
			/* setting auth algo number */
			val16 = (pmlmeinfo->auth_algo == dot11AuthAlgrthm_Shared) ? 1 :
		...
		if ((pmlmeinfo->auth_seq == 3) && (pmlmeinfo->state & WIFI_FW_AUTH_STATE)
			...
			pframe = rtw_set_fixed_ie(pframe, 4, (unsigned char *)&val32,
						  &(pattrib->pktlen));
		...
		if ((pmlmeinfo->auth_seq == 3) && (pmlmeinfo->state & WIFI_FW_AUTH_STATE)
		    && (use_shared_key == 1)) {
			pframe = rtw_set_ie(pframe, _CHLGETXT_IE_, 128, pmlmeinfo->chg_txt,
					    &(pattrib->pktlen));
...
void issue_asocrsp(_adapter *padapter, unsigned short status,
		   struct sta_info *pstat, int pkt_type)
{
	...
	*(fctrl) = 0;
	...
	_rtw_memcpy((void *)get_addr2_ptr(pwlanhdr), adapter_mac_addr(padapter),
		    ETH_ALEN);
	_rtw_memcpy((void *)GetAddr3Ptr(pwlanhdr),
	...
	pframe = rtw_set_fixed_ie(pframe, _STATUS_CODE_,
				  (unsigned char *)&ie_status, &(pattrib->pktlen));

	val = cpu_to_le16(pstat->cmn.aid | BIT(14) | BIT(15));
	pframe = rtw_set_fixed_ie(pframe, _ASOC_ID_, (unsigned char *)&val,
				  &(pattrib->pktlen));

	if (pstat->bssratelen <= 8)
		pframe = rtw_set_ie(pframe, _SUPPORTEDRATES_IE_, pstat->bssratelen,
				    pstat->bssrateset, &(pattrib->pktlen));
	else {
		pframe = rtw_set_ie(pframe, _SUPPORTEDRATES_IE_, 8, pstat->bssrateset,
				    &(pattrib->pktlen));
		pframe = rtw_set_ie(pframe, _EXT_SUPPORTEDRATES_IE_,
				    (pstat->bssratelen - 8), pstat->bssrateset + 8, &(pattrib->pktlen));
	}
	...
	if (status == _STATS_REFUSED_TEMPORARILY_) {
		...
		pframe = rtw_set_ie(pframe, _TIMEOUT_ITVL_IE_, 5, timeout_itvl,
				    &(pattrib->pktlen));
	}
#endif /* CONFIG_IEEE80211W */
	...
	if ((pstat->flags & WLAN_STA_HT) && (pmlmepriv->htpriv.ht_option)) {
		...
		pbuf = rtw_get_ie(ie + _BEACON_IE_OFFSET_, _HT_CAPABILITY_IE_, &ie_len,
		...
		pbuf = rtw_get_ie(ie + _BEACON_IE_OFFSET_, _HT_ADD_INFO_IE_, &ie_len,
				  (pnetwork->IELength - _BEACON_IE_OFFSET_));
		if (pbuf && ie_len > 0) {
			_rtw_memcpy(pframe, pbuf, ie_len + 2);
			pframe += (ie_len + 2);
	...
	if (pmlmepriv->ext_capab_ie_len > 0) {
		...
		pbuf = rtw_get_ie(ie + _BEACON_IE_OFFSET_, _EXT_CAP_IE_, &ie_len,
				  (pnetwork->IELength - _BEACON_IE_OFFSET_));
		if (pbuf && ie_len > 0) {
			_rtw_memcpy(pframe, pbuf, ie_len + 2);
			pframe += (ie_len + 2);
	...
	if ((pstat->flags & WLAN_STA_VHT) && (pmlmepriv->vhtpriv.vht_option)
		...
		/* FILL VHT CAP IE */
		pbuf = rtw_get_ie(ie + _BEACON_IE_OFFSET_, EID_VHTCapability, &ie_len,
				  (pnetwork->IELength - _BEACON_IE_OFFSET_));
		if (pbuf && ie_len > 0) {
			_rtw_memcpy(pframe, pbuf, ie_len + 2);
			pframe += (ie_len + 2);
		...
		pbuf = rtw_get_ie(ie + _BEACON_IE_OFFSET_, EID_VHTOperation, &ie_len,
				  (pnetwork->IELength - _BEACON_IE_OFFSET_));
		if (pbuf && ie_len > 0) {
			_rtw_memcpy(pframe, pbuf, ie_len + 2);
			pframe += (ie_len + 2);
	...
	if ((pstat->flags & WLAN_STA_WME) && (pmlmepriv->qospriv.qos_option)) {
		...
		for (pbuf = ie + _BEACON_IE_OFFSET_; ; pbuf += (ie_len + 2)) {
			pbuf = rtw_get_ie(pbuf, _VENDOR_SPECIFIC_IE_, &ie_len,
					  (pnetwork->IELength - _BEACON_IE_OFFSET_ - (ie_len + 2)));
	...
	if (pmlmeinfo->assoc_AP_vendor == HT_IOT_PEER_REALTEK)
		pframe = rtw_set_ie(pframe, _VENDOR_SPECIFIC_IE_, 6, REALTEK_96B_IE,
				    &(pattrib->pktlen));

	/* add WPS IE ie for wps 2.0 */
	if (pmlmepriv->wps_assoc_resp_ie && pmlmepriv->wps_assoc_resp_ie_len > 0) {
		_rtw_memcpy(pframe, pmlmepriv->wps_assoc_resp_ie,
...
void _issue_assocreq(_adapter *padapter, u8 is_reassoc)
{
	...
	u8 sup_ch[30 * 2] = {0x00 }, sup_ch_idx = 0,
				     idx_5g = 2;	/* For supported channel */
#endif /* CONFIG_DFS */
	...
	*(fctrl) = 0;
	...
	if (pmlmeext->cur_channel > 14) {
		...
		pframe = rtw_set_ie(pframe, EID_PowerCap, 2, pow_cap_ele,
				    &(pattrib->pktlen));

		/* supported channels */
		while (sup_ch_idx < rfctl->max_chan_nums
		       && rfctl->channel_set[sup_ch_idx].ChannelNum != 0) {
			if (rfctl->channel_set[sup_ch_idx].ChannelNum <= 14) {
				...
				sup_ch[1] =
					rfctl->channel_set[sup_ch_idx].ChannelNum;	/* Number of channel */
			} else {
		...
		pframe = rtw_set_ie(pframe, EID_SupportedChannels, idx_5g, sup_ch,
				    &(pattrib->pktlen));
	}
#endif /* CONFIG_DFS */
	...
	for (bssrate_len = 0; bssrate_len < NumRates; bssrate_len++) {
		...
		if (pmlmeinfo->network.SupportedRates[bssrate_len] ==
		    0x2C) /* Avoid the proprietary data rate (22Mbps) of Handlink WSG-4000 AP */
			break;
	...
	for (i = sizeof(NDIS_802_11_FIXED_IEs); i < pmlmeinfo->network.IELength;) {
		...
		case _VENDOR_SPECIFIC_IE_:
			if ((_rtw_memcmp(pIE->data, RTW_WPA_OUI, 4)) ||
				...
				pframe = rtw_set_ie(pframe, _VENDOR_SPECIFIC_IE_, vs_ie_length, pIE->data,
						    &(pattrib->pktlen));
			}
			break;
...
#endif
				pframe = rtw_set_ie(pframe, EID_WPA2, pIE->Length, pIE->data,
						    &(pattrib->pktlen));
			break;
#ifdef CONFIG_80211N_HT
		case EID_HTCapability:
			if (padapter->mlmepriv.htpriv.ht_option == _TRUE) {
				if (!(is_ap_in_tkip(padapter))) {
					_rtw_memcpy(&(pmlmeinfo->HT_caps), pIE->data,
						    sizeof(struct HT_caps_element));

					pmlmeinfo->HT_caps.u.HT_cap_element.HT_caps_info = cpu_to_le16(
								pmlmeinfo->HT_caps.u.HT_cap_element.HT_caps_info);

					pframe = rtw_set_ie(pframe, EID_HTCapability, pIE->Length,
		...
		case EID_EXTCapability:
			if (padapter->mlmepriv.htpriv.ht_option == _TRUE)
				pframe = rtw_set_ie(pframe, EID_EXTCapability, pIE->Length, pIE->data,
						    &(pattrib->pktlen));
			break;
#endif /* CONFIG_80211N_HT */
#ifdef CONFIG_80211AC_VHT
		case EID_VHTCapability:
			if (padapter->mlmepriv.vhtpriv.vht_option == _TRUE)
				pframe = rtw_set_ie(pframe, EID_VHTCapability, pIE->Length, pIE->data,
						    &(pattrib->pktlen));
			break;

		case EID_OpModeNotification:
			if (padapter->mlmepriv.vhtpriv.vht_option == _TRUE)
				pframe = rtw_set_ie(pframe, EID_OpModeNotification, pIE->Length, pIE->data,
						    &(pattrib->pktlen));
			break;
#endif /* CONFIG_80211AC_VHT */
		default:
	...
	if (adapter_wdev_data(padapter)->p2p_enabled
		...
		if (pmlmepriv->p2p_assoc_req_ie && pmlmepriv->p2p_assoc_req_ie_len > 0) {
			_rtw_memcpy(pframe, pmlmepriv->p2p_assoc_req_ie,
...
#endif /* CONFIG_IOCTL_CFG80211 */
	{
		if (!rtw_p2p_chk_state(pwdinfo, P2P_STATE_NONE)
			...
			*(u16 *)(p2pie + p2pielen) = cpu_to_le16(0x000D);
			...
			/*	Value: */
			_rtw_memcpy(p2pie + p2pielen, pwdinfo->device_addr,
...
static int _issue_nulldata(_adapter *padapter, unsigned char *da,
 ...
 */
int issue_nulldata(_adapter *padapter, unsigned char *da,
		   unsigned int power_mode, int try_cnt, int wait_ms)
{
	...
	do {
		ret = _issue_nulldata(padapter, da, power_mode,
				      wait_ms > 0 ? _TRUE : _FALSE);
...
/* when wait_ack is ture, this function shoule be called at process context */
static int _issue_qos_nulldata(_adapter *padapter, unsigned char *da,
			       u16 tid, u8 ps, int wait_ack)
{
	int ret = _FAIL;
	struct xmit_frame			*pmgntframe;
	struct pkt_attrib			*pattrib;
 ...
 * da == NULL for station mode
 */
int issue_qos_nulldata(_adapter *padapter, unsigned char *da, u16 tid,
		       u8 ps, int try_cnt, int wait_ms)
{
	int ret = _FAIL;
	int i = 0;
	systime start = rtw_get_current_time();
	...
	do {
		ret = _issue_qos_nulldata(padapter, da, tid, ps,
					  wait_ms > 0 ? _TRUE : _FALSE);
...
static int _issue_deauth(_adapter *padapter, unsigned char *da,
			 unsigned short reason, u8 wait_ack, u8 key_type)
{
	struct xmit_frame			*pmgntframe;
	struct pkt_attrib			*pattrib;
	unsigned char					*pframe;
	...
	*(fctrl) = 0;
	...
	pframe = rtw_set_fixed_ie(pframe, _RSON_CODE_, (unsigned char *)&reason,
				  &(pattrib->pktlen));
...
int issue_deauth(_adapter *padapter, unsigned char *da,
		 unsigned short reason)
{
	RTW_INFO("%s to "MAC_FMT"\n", __func__, MAC_ARG(da));
...
#ifdef CONFIG_IEEE80211W
int issue_deauth_11w(_adapter *padapter, unsigned char *da,
 ...
 * try_cnt means the maximal TX count to try
 */
int issue_deauth_ex(_adapter *padapter, u8 *da, unsigned short reason,
		    int try_cnt,
		    int wait_ms)
{
	int ret = _FAIL;
...
void issue_action_spct_ch_switch(_adapter *padapter, u8 *ra, u8 new_ch,
				 u8 ch_offset)
{
	_irqL	irqL;
	_list		*plist, *phead;
	...
	*(fctrl) = 0;
	...
	_rtw_memcpy(pwlanhdr->addr2, adapter_mac_addr(padapter),
		    ETH_ALEN); /* TA */
...
#ifdef CONFIG_IEEE80211W
void issue_action_SA_Query(_adapter *padapter, unsigned char *raddr,
			   unsigned char action, unsigned short tid, u8 key_type)
{
	...
	*(fctrl) = 0;
	...
	case 0: /* SA Query req */
		pframe = rtw_set_fixed_ie(pframe, 2,
					  (unsigned char *)&pmlmeext->sa_query_seq, &pattrib->pktlen);
	...
	case 1: /* SA Query rsp */
		...
		pframe = rtw_set_fixed_ie(pframe, 2, (unsigned char *)&tid,
					  &pattrib->pktlen);
		break;
	default:
		break;
	...
	*(fctrl) = 0;
	...
	if (category == 3) {
		...
		case RTW_WLAN_ACTION_ADDBA_REQ:
			...
			pframe = rtw_set_fixed_ie(pframe, 1, &(pmlmeinfo->dialogToken),
						  &(pattrib->pktlen));

#if defined(CONFIG_RTL8188E) && defined(CONFIG_SDIO_HCI)
			BA_para_set = (0x0802 | ((tid & 0xf) <<
						 2)); /* immediate ack & 16 buffer size */
#else
			BA_para_set = (0x1002 | ((tid & 0xf) <<
			...
			pframe = rtw_set_fixed_ie(pframe, 2, (unsigned char *)(&(BA_para_set)),
						  &(pattrib->pktlen));
			...
			pframe = rtw_set_fixed_ie(pframe, 2,
						  (unsigned char *)(&(BA_timeout_value)), &(pattrib->pktlen));
			...
			pframe = rtw_set_fixed_ie(pframe, 2,
						  (unsigned char *)(&(BA_starting_seqctrl)), &(pattrib->pktlen));
			break;

		case RTW_WLAN_ACTION_ADDBA_RESP:
			pframe = rtw_set_fixed_ie(pframe, 1, &(pmlmeinfo->ADDBA_req.dialog_token),
						  &(pattrib->pktlen));
			status = cpu_to_le16(status);
			pframe = rtw_set_fixed_ie(pframe, 2, (unsigned char *)(&status),
						  &(pattrib->pktlen));
			...
			pframe = rtw_set_fixed_ie(pframe, 2, (unsigned char *)(&(BA_para_set)),
						  &(pattrib->pktlen));
			pframe = rtw_set_fixed_ie(pframe, 2,
						  (unsigned char *)(&(pmlmeinfo->ADDBA_req.BA_timeout_value)),
						  &(pattrib->pktlen));
			break;
 ...
 */
inline u8 issue_addba_rsp_wait_ack(_adapter *adapter, unsigned char *ra,
				   u8 tid, u16 status, u8 size, int try_cnt, int wait_ms)
{
	int ret = _FAIL;
	int i = 0;
	systime start = rtw_get_current_time();
 ...
 * @initiator: if we are the initiator of AMPDU association. used by DELBA
 */
inline void issue_del_ba(_adapter *adapter, unsigned char *ra, u8 tid,
			 u16 reason, u8 initiator)
{
 ...
 *           > 0 means you want to wait ack through C2H_CCX_TX_RPT, and the value of wait_ms means the interval between each TX
 */
int issue_del_ba_ex(_adapter *adapter, unsigned char *ra, u8 tid,
		    u16 reason, u8 initiator
		    , int try_cnt, int wait_ms)
{
	int ret = _FAIL;
	...
	*(fctrl) = 0;
	...
	if (pmlmepriv->num_sta_no_ht > 0) {
		...
		while (1) {
			...
			p = rtw_get_ie(pbss_network->IEs + _FIXED_IE_LENGTH_, _HT_CAPABILITY_IE_,
				       &len, pbss_network->IELength - _FIXED_IE_LENGTH_);
		...
		for (i = 0; i < 8; i++) {
			if (ICS[i][0] == 1) {
				...
				pframe = rtw_set_ie(pframe, EID_BSSIntolerantChlReport, k, InfoContent,
						    &(pattrib->pktlen));
...
int _issue_action_SM_PS(_adapter *padapter,  unsigned char *raddr,
			u8 NewMimoPsMode,  u8 wait_ack)
{
	...
	if (NewMimoPsMode == WLAN_HT_CAP_SM_PS_DISABLED) {
		sm_power_control = sm_power_control  & ~(BIT(
					   0)); /* SM Power Save Enable = 0 SM Power Save Disable */
	} else if (NewMimoPsMode == WLAN_HT_CAP_SM_PS_STATIC) {
		sm_power_control = sm_power_control | BIT(
					   0);    /* SM Power Save Enable = 1 SM Power Save Enable  */
		sm_power_control = sm_power_control & ~(BIT(
				1)); /* SM Mode = 0 Static Mode */
	} else if (NewMimoPsMode == WLAN_HT_CAP_SM_PS_DYNAMIC) {
		sm_power_control = sm_power_control | BIT(
					   0); /* SM Power Save Enable = 1 SM Power Save Enable  */
		sm_power_control = sm_power_control | BIT(
					   1); /* SM Mode = 1 Dynamic Mode */
	} else
		return ret;
	...
	RTW_INFO("%s, sm_power_control=%u, NewMimoPsMode=%u\n", __FUNCTION__,
		 sm_power_control, NewMimoPsMode);
	...
	*(fctrl) = 0;
	...
	pframe = rtw_set_fixed_ie(pframe, 1, &(sm_power_control),
				  &(pattrib->pktlen));
 ...
 * try_cnt means the maximal TX count to try
 */
int issue_action_SM_PS_wait_ack(_adapter *padapter, unsigned char *raddr,
				u8 NewMimoPsMode, int try_cnt, int wait_ms)
{
	...
	do {
		ret = _issue_action_SM_PS(padapter, raddr, NewMimoPsMode,
					  wait_ms > 0 ? _TRUE : _FALSE);
...
int issue_action_SM_PS(_adapter *padapter,  unsigned char *raddr,
		       u8 NewMimoPsMode)
{
 ...
 */
static unsigned int _send_delba_sta_tid(_adapter *adapter, u8 initiator,
	...
	if (initiator == 0) {
		...
		if (force || sta->recvreorder_ctrl[tid].enable == _TRUE) {
			...
			if (rtw_del_rx_ampdu_test_trigger_no_tx_fail())
				...
				ret = issue_del_ba_ex(adapter, sta->cmn.mac_addr, tid, 37, initiator, 3,
						      1);
			else
...
inline unsigned int send_delba_sta_tid(_adapter *adapter, u8 initiator,
				       struct sta_info *sta, u8 tid
				       , u8 force)
{
...
inline unsigned int send_delba_sta_tid_wait_ack(_adapter *adapter,
		u8 initiator, struct sta_info *sta, u8 tid
		, u8 force)
{
	...
	if (_FALSE == bxmitok) {
		...
		if (passing_time > 100 || issue > 3)
			RTW_INFO("%s success, issue:%d, poll:%d, %u ms\n", __FUNCTION__, issue,
				 poll, rtw_get_passing_time_ms(start));
		else if (0)
			RTW_INFO("%s success, issue:%d, poll:%d, %u ms\n", __FUNCTION__, issue,
				 poll, rtw_get_passing_time_ms(start));
...
u8 collect_bss_info(_adapter *padapter, union recv_frame *precv_frame,
		    WLAN_BSSID_EX *bssid)
{
	int	i;
	sint len;
	...
	bssid->IELength = len;
	_rtw_memcpy(bssid->IEs, (pframe + sizeof(struct rtw_ieee80211_hdr_3addr)),
		    bssid->IELength);

	/* get the signal strength */
	/* bssid->Rssi = precv_frame->u.hdr.attrib.SignalStrength; */ /* 0-100 index. */
	bssid->Rssi =
		precv_frame->u.hdr.attrib.phy_info.recv_signal_power; /* in dBM.raw data */
	bssid->PhyInfo.SignalQuality =
		precv_frame->u.hdr.attrib.phy_info.signal_quality;/* in percentage */
	bssid->PhyInfo.SignalStrength =
		precv_frame->u.hdr.attrib.phy_info.signal_strength;/* in percentage */
	...
	rtw_hal_get_odm_var(padapter, HAL_ODM_ANTDIV_SELECT,
			    &(bssid->PhyInfo.Optimum_antenna), NULL);
#endif

	/* checking SSID */
	p = rtw_get_ie(bssid->IEs + ie_offset, _SSID_IE_, &len,
	...
	/* checking rate info... */
	i = 0;
	p = rtw_get_ie(bssid->IEs + ie_offset, _SUPPORTEDRATES_IE_, &len,
		       bssid->IELength - ie_offset);
	if (p != NULL) {
	...
	p = rtw_get_ie(bssid->IEs + ie_offset, _EXT_SUPPORTEDRATES_IE_, &len,
		       bssid->IELength - ie_offset);
	if (p != NULL) {
	...
	if (subtype == WIFI_PROBEREQ) {
		...
		if (p2p_ie) {
			...
			rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_LISTEN_CH, listen_ch,
						 &attr_contentlen);
	...
	if (p)
		...
		p = rtw_get_ie(bssid->IEs + ie_offset, _HT_ADD_INFO_IE_, &len,
			       bssid->IELength - ie_offset);
		if (p) {
	...
	_rtw_memcpy(&bssid->Configuration.BeaconPeriod,
		    rtw_get_beacon_interval_from_ie(bssid->IEs), 2);
	bssid->Configuration.BeaconPeriod = le32_to_cpu(
	...
	if ((val16 & 0x03) == cap_ESS) {
		...
		mesh_id_ie = rtw_get_ie(bssid->IEs + ie_offset, WLAN_EID_MESH_ID,
					&mesh_id_ie_len, bssid->IELength - ie_offset);
		mesh_conf_ie = rtw_get_ie(bssid->IEs + ie_offset, WLAN_EID_MESH_CONFIG,
					  &mesh_conf_ie_len, bssid->IELength - ie_offset);
	...
	if ((pregistrypriv->wifi_spec == 1)
	    && (_FALSE == pmlmeinfo->bwmode_updated)) {
		struct mlme_priv *pmlmepriv = &padapter->mlmepriv;
#ifdef CONFIG_80211N_HT
		p = rtw_get_ie(bssid->IEs + ie_offset, _HT_CAPABILITY_IE_, &len,
			       bssid->IELength - ie_offset);
		if (p && len > 0) {
	...
	if (strcmp(bssid->Ssid.Ssid, DBG_RX_SIGNAL_DISPLAY_SSID_MONITORED) == 0) {
		RTW_INFO("Receiving %s("MAC_FMT", DSConfig:%u) from ch%u with ss:%3u, sq:%3u, RawRSSI:%3ld\n"
			 , bssid->Ssid.Ssid, MAC_ARG(bssid->MacAddress),
...
#ifdef CONFIG_RTW_80211K
	p = rtw_get_ie(bssid->IEs + ie_offset, _EID_RRM_EN_CAP_IE_, &len,
		       bssid->IELength - ie_offset);
	if (p)
...
void start_clnt_join(_adapter *padapter)
{
	...
	if (caps & cap_ESS) {
		...
		if (padapter->wapiInfo.bWapiEnable
		    && pmlmeinfo->auth_algo == dot11AuthAlgrthm_WAPI) {
		...
		/*	For the Win8 P2P connection, it will be hard to have a successful connection if this Wi-Fi doesn't connect to it. */
		{
			...
			for (pos = get_next(head); !rtw_end_of_queue_search(head, pos);
			     pos = get_next(pos)) {

				scanned = LIST_CONTAINOR(pos, struct wlan_network, list);

				if (_rtw_memcmp(&(scanned->network.Ssid), &(pnetwork->Ssid),
						sizeof(NDIS_802_11_SSID)) == _TRUE
				    && _rtw_memcmp(scanned->network.MacAddress, pnetwork->MacAddress,
						   sizeof(NDIS_802_11_MAC_ADDRESS)) == _TRUE
				   ) {
					ie_offset = (scanned->network.Reserved[0] == BSS_TYPE_PROB_REQ ? 0 : 12);
					if (rtw_get_p2p_ie(scanned->network.IEs + ie_offset,
							   scanned->network.IELength - ie_offset, NULL, NULL))
						has_p2p_ie = _TRUE;
					break;
				}
			}
			...
			if (scanned == NULL || rtw_end_of_queue_search(head, pos)
			    || has_p2p_ie == _FALSE)
#endif /* CONFIG_P2P */
				/* To avoid connecting to AP fail during resume process, change retry count from 5 to 1 */
				issue_deauth_ex(padapter, pnetwork->MacAddress, WLAN_REASON_DEAUTH_LEAVING,
						1, 100);
		}
				...
				unsigned short reason, u8 locally_generated)
{
	...
	if ((pmlmeinfo->state & 0x03) == WIFI_FW_STATION_STATE) {
		if (pmlmeinfo->state & WIFI_FW_ASSOC_SUCCESS) {
			if (report_del_sta_event(padapter, MacAddr, reason, _TRUE,
						 locally_generated) != _FAIL)
				pmlmeinfo->state = WIFI_FW_NULL_STATE;
	...
	if (pregistrypriv->enable80211d
		...
		while ((i < MAX_CHANNEL_NUM) && (chplan_sta[i].ChannelNum != 0)) {
			_RTW_INFO("%02d(%c),", chplan_sta[i].ChannelNum,
				  chplan_sta[i].ScanType == SCAN_PASSIVE ? 'p' : 'a');
			i++;
		}
		_RTW_INFO("}\n");
#endif
...
void report_survey_event(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	psurvey_evt = (struct survey_event *)(pevtcmd + sizeof(
			struct C2HEvent_Header));

	if (collect_bss_info(padapter, precv_frame,
			     (WLAN_BSSID_EX *)&psurvey_evt->bss) == _FAIL) {
		rtw_mfree((u8 *)pcmd_obj, sizeof(struct cmd_obj));
	...
	ch_set_idx = rtw_chset_search_ch(chset,
					 psurvey_evt->bss.Configuration.DSConfig);
	if (ch_set_idx >= 0) {
		if (psurvey_evt->bss.InfrastructureMode == Ndis802_11Infrastructure) {
			...
			if (psurvey_evt->bss.Ssid.SsidLength == 0
			    || is_all_null(psurvey_evt->bss.Ssid.Ssid,
					   psurvey_evt->bss.Ssid.SsidLength) == _TRUE)
...
void report_surveydone_event(_adapter *padapter)
{
	...
	psurveydone_evt = (struct surveydone_event *)(pevtcmd + sizeof(
				  struct C2HEvent_Header));
...
u32 report_join_res(_adapter *padapter, int res)
{
	...
	pjoinbss_evt = (struct joinbss_event *)(pevtcmd + sizeof(
			struct C2HEvent_Header));
	_rtw_memcpy((unsigned char *)(&(pjoinbss_evt->network.network)),
		    &(pmlmeinfo->network), sizeof(WLAN_BSSID_EX));
...
u32 report_del_sta_event(_adapter *padapter, unsigned char *MacAddr,
			 unsigned short reason, bool enqueue, u8 locally_generated)
{
	struct cmd_obj *pcmd_obj;
	u8	*pevtcmd;
	u32 cmdsz;
	...
	pdel_sta_evt = (struct stadel_event *)(pevtcmd + sizeof(
			struct C2HEvent_Header));
	_rtw_memcpy((unsigned char *)(&(pdel_sta_evt->macaddr)), MacAddr,
		    ETH_ALEN);
	_rtw_memcpy((unsigned char *)(pdel_sta_evt->rsvd),
...
void report_add_sta_event(_adapter *padapter, unsigned char *MacAddr)
{
	...
	padd_sta_evt = (struct stassoc_event *)(pevtcmd + sizeof(
			struct C2HEvent_Header));
	_rtw_memcpy((unsigned char *)(&(padd_sta_evt->macaddr)), MacAddr,
		    ETH_ALEN);
...
bool rtw_port_switch_chk(_adapter *adapter)
{
	...
	RTW_INFO(FUNC_ADPT_FMT" wowlan_mode:%u\n"
		 ...
		 rtw_p2p_state(&if_port0->wdinfo), rtw_p2p_chk_state(&if_port0->wdinfo,
				 P2P_STATE_NONE),
		 ADPT_ARG(if_port1), if_port1_mlmeinfo->state,
		 rtw_p2p_state(&if_port1->wdinfo), rtw_p2p_chk_state(&if_port1->wdinfo,
				 P2P_STATE_NONE));
...
void update_sta_info(_adapter *padapter, struct sta_info *psta)
{
	...
	if (pmlmepriv->htpriv.ht_option) {
		...
		_rtw_memcpy(&psta->htpriv.ht_cap, &pmlmeinfo->HT_caps,
	...
	psta->cmn.ra_info.is_support_sgi = query_ra_short_GI(psta,
					   rtw_get_tx_bw_mode(padapter, psta));
...
static void rtw_mlmeext_disconnect(_adapter *padapter)
{
...
#ifdef CONFIG_LPS
			{
				_adapter *port0_iface = dvobj_get_port0_adapter(adapter_to_dvobj(
								padapter));
				if (port0_iface)
...

	{
		...
		if (rtw_mi_get_ch_setting_union_no_self(padapter, &ch, &bw,
							&offset) != 0) {
...
#ifdef CONFIG_BEAMFORMING
	if (psta)
		beamforming_wk_cmd(padapter, BEAMFORMING_CTRL_ENTER, (u8 *)psta,
...
void mlmeext_sta_add_event_callback(_adapter *padapter,
				    struct sta_info *psta)
{
	struct mlme_ext_priv	*pmlmeext = &(padapter->mlmeextpriv);
	struct mlme_ext_info	*pmlmeinfo = &(pmlmeext->mlmext_info);
	...
	psta->wireless_mode = rtw_check_network_type(psta->bssrateset,
			      psta->bssratelen, pmlmeext->cur_channel);
	rtw_hal_set_odm_var(padapter, HAL_ODM_STA_INFO, psta, _TRUE);
...
void linked_status_chk(_adapter *padapter, u8 from_timer)
{
	...
	if (is_client_associated_to_ap(padapter)) {
		...
		if (psta != NULL) {
			...
			if (pmlmeext->active_keep_alive_check && (rx_chk == _FAIL
					|| tx_chk == _FAIL)
			   ) {
				...
				if (switch_channel_by_drv) {
					if (!rtw_mi_get_ch_setting_union(padapter, &union_ch, &union_bw,
									 &union_offset)
				...
				if (rx_chk != _SUCCESS)
					issue_probereq_ex(padapter, &pmlmeinfo->network.Ssid, psta->cmn.mac_addr,
							  0, 0, 3, 1);
...
bypass_active_keep_alive:
				...
				if (rx_chk != _SUCCESS) {
					if (pmlmeext->retry == 0) {
						...
						RTW_INFO("issue_probereq to trigger probersp, retry=%d\n",
							 pmlmeext->retry);
#endif
						issue_probereq_ex(padapter, &pmlmeinfo->network.Ssid,
								  pmlmeinfo->network.MacAddress, 0, 0, 0, (from_timer ? 0 : 1));
						issue_probereq_ex(padapter, &pmlmeinfo->network.Ssid,
								  pmlmeinfo->network.MacAddress, 0, 0, 0, (from_timer ? 0 : 1));
						issue_probereq_ex(padapter, &pmlmeinfo->network.Ssid,
								  pmlmeinfo->network.MacAddress, 0, 0, 0, (from_timer ? 0 : 1));
		...
		while (rtw_end_of_queue_search(&dlist, plist) == _FALSE) {
			...
			report_del_sta_event(padapter, psta->cmn.mac_addr,
					     WLAN_REASON_EXPIRATION_CHK, from_timer ? _TRUE : _FALSE, _FALSE);
		}
	}
...
void survey_timer_hdl(void *ctx)
{
	...
	if (mlmeext_scan_state(pmlmeext) > SCAN_DISABLE) {
		...
		psurveyPara = (struct sitesurvey_parm *)rtw_zmalloc(sizeof(
...
#ifdef CONFIG_IEEE80211W
void report_sta_timeout_event(_adapter *padapter, u8 *MacAddr,
			      unsigned short reason)
{
	struct cmd_obj *pcmd_obj;
	u8	*pevtcmd;
...
void clnt_sa_query_timeout(_adapter *padapter)
{
	...
	receive_disconnect(padapter, get_my_bssid(&(mlmeinfo->network)),
			   WLAN_REASON_SA_QUERY_TIMEOUT, _FALSE);
}
...
void sa_query_timer_hdl(void *ctx)
{
	...
	if (check_fwstate(pmlmepriv, WIFI_STATION_STATE) == _TRUE &&
		...
		report_sta_timeout_event(padapter, psta->cmn.mac_addr,
	...
	_adapter *padapter, struct pkt_attrib *pattrib, u8 **pframe)
{
	...
	*pframe = rtw_set_ie(((u8 *)*pframe), _MDIE_, len, pie,
			     &(pattrib->pktlen));
	return _SUCCESS;
}
...
void rtw_ft_report_evt(_adapter *padapter)
{
	...
	if (ft_evt_parms.ies)
		_rtw_memcpy((void *)ft_evt_parms.ies, pft_roam->ft_event.ies,
			    ft_evt_parms.ies_len);
	else
		goto err_2;

	ft_evt_parms.target_ap = rtw_zmalloc(ETH_ALEN);
	if (ft_evt_parms.target_ap)
		_rtw_memcpy((void *)ft_evt_parms.target_ap, pnetwork->MacAddress,
			    ETH_ALEN);
	else
		goto err_1;
...
void rtw_ft_report_reassoc_evt(_adapter *padapter, u8 *pMacAddr)
{
	...
	passoc_sta_evt = (struct stassoc_event *)(pevtcmd + sizeof(
				 struct C2HEvent_Header));
	_rtw_memcpy((unsigned char *)(&(passoc_sta_evt->macaddr)), pMacAddr,
		    ETH_ALEN);
	rtw_enqueue_cmd(pcmdpriv, pcmd_obj);
}
...
#ifdef CONFIG_AUTO_AP_MODE
void rtw_auto_ap_rx_msg_dump(_adapter *padapter,
			     union recv_frame *precv_frame, u8 *ehdr_pos)
{
	struct rx_pkt_attrib *pattrib = &precv_frame->u.hdr.attrib;
...
u8 setopmode_hdl(_adapter *padapter, u8 *pbuf)
{
	...
	if (rtw_port_switch_chk(padapter) == _TRUE) {
		...
		if (psetop->mode == Ndis802_11APMode)
			adapter_to_pwrctl(padapter)->fw_psmode_iface_id =
				0xff; /* ap mode won't dowload rsvd pages */
		else if (psetop->mode == Ndis802_11Infrastructure) {
#ifdef CONFIG_LPS
			_adapter *port0_iface = dvobj_get_port0_adapter(adapter_to_dvobj(
							padapter));
...
u8 join_cmd_hdl(_adapter *padapter, u8 *pbuf)
{
...
#ifdef CONFIG_ANTENNA_DIVERSITY
	rtw_antenna_select_cmd(padapter, pparm->network.PhyInfo.Optimum_antenna,
			       _FALSE);
#endif
	...
	_rtw_memcpy(pnetwork->IEs, ((WLAN_BSSID_EX *)pbuf)->IEs,
		    pnetwork->IELength);
...
u8 rtw_scan_sparse(_adapter *adapter, struct rtw_ieee80211_channel *ch,
		   u8 ch_num)
{
...
#define SCAN_SPARSE_CH_NUM_INVALID 255
	...
	if (rtw_chk_roam_flags(adapter, RTW_ROAM_ACTIVE)) {
		if (busy_traffic == _TRUE && adapter->mlmepriv.need_to_roam == _TRUE)
			max_allow_ch = rtw_min(max_allow_ch,
					       RTW_SCAN_SPARSE_CH_NUM_ROAMING_ACTIVE);
	}
#endif
	...
	if (max_allow_ch != SCAN_SPARSE_CH_NUM_INVALID) {
		...
		scan_division_num = (ch_num / max_allow_ch) + ((ch_num % max_allow_ch) ?
				    1 : 0);
...
u32 rtw_scan_timeout_decision(_adapter *padapter)
{
	...
	ss->scan_timeout_ms = (scan_ms * max_chan_num) + back_op_times +
			      SCANNING_TIMEOUT_EX;
...
static int rtw_scan_ch_decision(_adapter *padapter,
...
static void sitesurvey_res_reset(_adapter *adapter,
				      ...
				      RT_SCAN_TYPE *type)
{
...
#endif /* CONFIG_P2P */
	{
		...
		if (ss->channel_idx != 0 && ss->dfs_ch_ssid_scan == 0
			...
			ch_set_idx = rtw_chset_search_ch(rfctl->channel_set,
							 ss->ch[ss->channel_idx - 1].hw_value);
...
#endif /*CONFIG_RTW_ACS*/
				scan_type = (ch->flags & RTW_IEEE80211_CHAN_PASSIVE_SCAN) ? SCAN_PASSIVE :
					    SCAN_ACTIVE;
		}
	}
	...
	if (scan_ch != 0) {
...
#ifdef CONFIG_SCAN_BACKOP
		{
			...
			if ((MSTATE_STA_LD_NUM(&mstate)
			     && mlmeext_chk_scan_backop_flags_sta(pmlmeext, SS_BACKOP_EN))
			    || (MSTATE_STA_NUM(&mstate)
				&& mlmeext_chk_scan_backop_flags_sta(pmlmeext, SS_BACKOP_EN_NL)))
				backop_flags |= mlmeext_scan_backop_flags_sta(pmlmeext);

			if ((MSTATE_AP_LD_NUM(&mstate)
			     && mlmeext_chk_scan_backop_flags_ap(pmlmeext, SS_BACKOP_EN))
			    || (MSTATE_AP_NUM(&mstate)
				&& mlmeext_chk_scan_backop_flags_ap(pmlmeext, SS_BACKOP_EN_NL)))
			...
			for (i = 0; i < RTW_SSID_SCAN_AMOUNT; i++) {
				if (pmlmeext->sitesurvey_res.ssid[i].SsidLength) {
					...
					if (padapter->registrypriv.wifi_spec)
						...
						issue_probereq_ex(padapter, &(pmlmeext->sitesurvey_res.ssid[i]), NULL, 0,
								  0, 0, 0);
					issue_probereq(padapter, &(pmlmeext->sitesurvey_res.ssid[i]), NULL);
				}
			}
...
void survey_done_set_ch_bw(_adapter *padapter)
{
...
#ifdef CONFIG_MCC_MODE
	if (!rtw_hal_mcc_change_scan_flag(padapter, &cur_channel, &cur_bwmode,
					  &cur_ch_offset)) {
		if (0)
	...
	if (rtw_mi_get_ch_setting_union(padapter, &cur_channel, &cur_bwmode,
					&cur_ch_offset) != 0) {
		if (0)
...
#ifdef CONFIG_IOCTL_CFG80211
			if (iface->wdinfo.driver_interface == DRIVER_CFG80211
			    && !adapter_wdev_data(iface)->p2p_enabled)
				continue;
#endif
	...
	for (i = 0; i < dvobj->iface_nums; i++) {
		...
		if (MLME_IS_STA(iface)) {
			if (is_client_associated_to_ap(iface) == _TRUE) {
				...
				if (MCC_EN(adapter)
				    && rtw_hal_check_mcc_status(adapter, MCC_STATUS_NEED_MCC)) {
...
void sitesurvey_set_igi(_adapter *adapter)
{
	...
	case SCAN_ENTER:
...
#ifdef CONFIG_IOCTL_CFG80211
		if (pwdinfo->driver_interface == DRIVER_CFG80211
		    && rtw_cfg80211_is_p2p_scan(adapter))
			igi = 0x30;
		else
#endif /* CONFIG_IOCTL_CFG80211 */
			if (!rtw_p2p_chk_state(pwdinfo, P2P_STATE_NONE))
		...
		/* record IGI status */
		ss->igi_scan = igi;
		rtw_hal_get_odm_var(adapter, HAL_ODM_INITIAL_GAIN, &ss->igi_before_scan,
				    NULL);
...
void sitesurvey_set_msr(_adapter *adapter, bool enter)
{
	...
	if (enter) {
		...
		rtw_hal_get_hwreg(adapter, HW_VAR_MEDIA_STATUS,
				  (u8 *)(&pmlmeinfo->hw_media_state));
#endif
...
u8 setkey_hdl(_adapter *padapter, u8 *pbuf)
{
	...
	if (used == _TRUE && rtw_camid_is_gk(padapter, cam_id) == _FALSE) {
		...
		RTW_PRINT(
			FUNC_ADPT_FMT" group key with "MAC_FMT" id:%u the same key id as pairwise key\n"
	...
	if (is_wep_enc(pparm->algorithm)
	    && check_mlmeinfo_state(pmlmeext, WIFI_FW_STATION_STATE) &&
...
void rtw_ap_wep_pk_setting(_adapter *adapter, struct sta_info *psta)
{
	...
	for (keyid = 0; keyid < 4; keyid++) {
		if ((psecuritypriv->key_mask & BIT(keyid))
		    && (keyid == psecuritypriv->dot11PrivacyKeyIndex)) {
			sta_pparm.algorithm = psecuritypriv->dot11PrivacyAlgrthm;
			sta_pparm.keyid = keyid;
			sta_pparm.gk = 0;
			_rtw_memcpy(sta_pparm.key, &(psecuritypriv->dot11DefKey[keyid].skey[0]),
				    16);
...
u8 set_stakey_hdl(_adapter *padapter, u8 *pbuf)
{
	...
	if (!pparm->gk && used == _TRUE
	    && rtw_camid_is_gk(padapter, cam_id) == _TRUE) {
		s16 camid_clr;

		RTW_PRINT(
			FUNC_ADPT_FMT" pairwise key with "MAC_FMT" id:%u the same key id as group key\n"
...
u8 add_ba_rsp_hdl(_adapter *padapter, unsigned char *pbuf)
{
	...
	if (pparm->status == 0) {
		...
		RTW_INFO("DBG_RX_SEQ "FUNC_ADPT_FMT" tid:%u SN_UPDATE indicate_seq:%d, start_seq:%d\n"
			 , FUNC_ADPT_ARG(padapter), preorder_ctrl->tid, preorder_ctrl->indicate_seq,
			 pparm->start_seq);
#endif
	}
#else
...
u8 set_tx_beacon_cmd(_adapter *padapter)
{
	...
	ptxBeacon_parm = (struct Tx_Beacon_param *)rtw_zmalloc(sizeof(
			   ...
			   u8 ch)
{
	...
	rtw_add_bcn_ie(padapter, pnetwork, _SUPPORTEDRATES_IE_,
		       pnetwork->SupportedRates, rate_len);

	if (remainder_rate_len)
		rtw_add_bcn_ie(padapter, pnetwork, _EXT_SUPPORTEDRATES_IE_,
			       (pnetwork->SupportedRates + 8), remainder_rate_len);
	else
		...
		for (i = 0; i < dvobj->iface_nums; i++) {
			...
			if ((MLME_IS_AP(iface) || MLME_IS_MESH(iface))
				...
				if (is_grouped == _FALSE) {
					...
					_rtw_memcpy(&(mlme->cur_network.network), &(mlmeext->mlmext_info.network),
						    sizeof(WLAN_BSSID_EX));
...
int rtw_chk_start_clnt_join(_adapter *adapter, u8 *ch, u8 *bw, u8 *offset)
{
...
#ifdef CONFIG_CONCURRENT_MODE
	{
		...
		if (rtw_mi_get_ch_setting_union_no_self(adapter, &u_ch, &u_bw,
							&u_offset) <= 0) {
			dump_adapters_status(RTW_DBGDUMP, dvobj);
			rtw_warn_on(1);
		}
		...
		/* chbw_allow? */
		chbw_allow = rtw_is_chbw_grouped(pmlmeext->cur_channel,
		...
		if (rtw_hal_set_mcc_setting_chk_start_clnt_join(adapter, &u_ch, &u_bw,
				&u_offset, chbw_allow))
			goto exit;
#endif
...
#ifdef CONFIG_SPCT_CH_SWITCH
					if (1)
						rtw_ap_inform_ch_switch(iface, pmlmeext->cur_channel,
									pmlmeext->cur_ch_offset);
					else
#endif
...
u8 set_chplan_hdl(_adapter *padapter, unsigned char *pbuf)
{
	...
	rfctl->max_chan_nums = init_channel_set(padapter, rfctl->ChannelPlan,
						rfctl->channel_set);
...
u8 set_csa_hdl(_adapter *padapter, unsigned char *pbuf)
{
	...
	set_channel_bwmode(padapter, new_ch_no, HAL_PRIME_CHNL_OFFSET_DONT_CARE,
			   CHANNEL_WIDTH_20);
...
u8 tdls_hdl(_adapter *padapter, unsigned char *pbuf)
{
	...
	struct mlme_ext_priv *pmlmeext = &padapter->mlmeextpriv;
	struct mlme_ext_info *pmlmeinfo = &pmlmeext->mlmext_info;
	struct sta_info *ap_sta = rtw_get_stainfo(&padapter->stapriv,
	...
	case TDLS_CH_SW_RESP:
		...
		if (padapter->mlmeextpriv.cur_channel == pchsw_info->off_ch_num) {
			rtw_tdls_cmd(padapter, ptdls_sta->cmn.mac_addr,
	...
	case TDLS_CH_SW_PREPARE:
		...
		doiqk = _TRUE;
		rtw_hal_set_hwreg(padapter, HW_VAR_DO_IQK, &doiqk);
		set_channel_bwmode(padapter, pchsw_info->off_ch_num, pchsw_info->ch_offset,
	...
	case TDLS_CH_SW_TO_OFF_CHNL:
		...
		if (padapter->registrypriv.wifi_spec == 0) {
			if (!(pchsw_info->ch_sw_state & TDLS_CH_SW_INITIATOR_STATE))
				_set_timer(&ptdls_sta->ch_sw_timer,
					   (u32)(ptdls_sta->ch_switch_timeout) / 1000);
		}

		if (rtw_tdls_do_ch_sw(padapter, ptdls_sta, TDLS_CH_SW_OFF_CHNL,
				      pchsw_info->off_ch_num,
				      pchsw_info->ch_offset, (pchsw_info->ch_offset) ? CHANNEL_WIDTH_40 :
				      CHANNEL_WIDTH_20, ptdls_sta->ch_switch_time) == _SUCCESS) {
			pchsw_info->ch_sw_state &= ~(TDLS_PEER_AT_OFF_STATE);
			if (pchsw_info->ch_sw_state & TDLS_CH_SW_INITIATOR_STATE) {
				if (issue_nulldata_to_TDLS_peer_STA(ptdls_sta->padapter,
								    ptdls_sta->cmn.mac_addr, 0, 1,
...
#if 0
		_rtw_memset(pHalData->tdls_ch_sw_iqk_info_base_chnl, 0x00,
			    sizeof(pHalData->tdls_ch_sw_iqk_info_base_chnl));
		_rtw_memset(pHalData->tdls_ch_sw_iqk_info_off_chnl, 0x00,
			    sizeof(pHalData->tdls_ch_sw_iqk_info_off_chnl));
#endif
	...
	case TDLS_CH_SW_TO_BASE_CHNL:
		...
		if (rtw_tdls_do_ch_sw(padapter, ptdls_sta, TDLS_CH_SW_BASE_CHNL,
				      pmlmeext->cur_channel,
				      pmlmeext->cur_ch_offset, pmlmeext->cur_bwmode,
				      ptdls_sta->ch_switch_time) == _SUCCESS) {
			if (ap_sta)
	...
	case TDLS_TEARDOWN_STA_NO_WAIT:
		...
		issue_tdls_teardown(padapter, &txmgmt,
				    (option == TDLS_TEARDOWN_STA) ? _TRUE : _FALSE);
