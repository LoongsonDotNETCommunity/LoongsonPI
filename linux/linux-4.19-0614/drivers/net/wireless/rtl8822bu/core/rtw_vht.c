...
void dump_vht_cap_ie_content(void *sel, const u8 *buf, u32 buf_len)
{
	...
	RTW_PRINT_SEL(sel,
		      "cap_info:%02x %02x %02x %02x: MAX_MPDU_LEN:%u %s%s%s%s%s RX-STBC:%u MAX_AMPDU_LEN:%u\n"
...
void dump_vht_cap_ie(void *sel, const u8 *ie, u32 ie_len)
{
	...
	vht_cap_ie = rtw_get_ie(ie, WLAN_EID_VHT_CAPABILITY, &vht_cap_ielen,
				ie_len);
	if (!ie || vht_cap_ie != ie)
		return;
...
void	rtw_vht_use_default_setting(_adapter *padapter)
{
	...
	pvhtpriv->sgi_80m = TEST_FLAG(pregistrypriv->short_gi,
				      BIT2) ? _TRUE : _FALSE;
	...
	rtw_vht_nss_to_mcsmap(rx_nss, pvhtpriv->vht_mcs_map,
			      pregistrypriv->vht_rx_mcs_map);
	pvhtpriv->vht_highest_rate = rtw_get_vht_highest_rate(
...
#ifdef CONFIG_BEAMFORMING
void update_sta_vht_info_apmode_bf_cap(_adapter *padapter,
				       struct sta_info *psta)
{
	...
	if (TEST_FLAG(pvhtpriv_ap->beamform_cap,
		...
		SET_FLAG(cur_beamform_cap,
			 GET_VHT_CAPABILITY_ELE_SU_BFEE_STS_CAP(pvhtpriv_sta->vht_cap) << 8);
	}
	...
	if (TEST_FLAG(pvhtpriv_ap->beamform_cap,
		...
		SET_FLAG(cur_beamform_cap,
			 GET_VHT_CAPABILITY_ELE_SU_BFER_SOUND_DIM_NUM(pvhtpriv_sta->vht_cap) << 12);
	}
...
void	update_sta_vht_info_apmode(_adapter *padapter, PVOID sta)
{
	...
	if (pvhtpriv_sta->notify_present)
		bw_mode = GET_VHT_OPERATING_MODE_FIELD_CHNL_WIDTH(
				  &pvhtpriv_sta->vht_op_mode_notify);
	...
	if (bw_mode != -1)
		psta->cmn.bw_mode =
			bw_mode; /* update bw_mode only if get value from VHT IEs */
	...
	if (psta->cmn.bw_mode == CHANNEL_WIDTH_80) {
		...
		pvhtpriv_sta->sgi_80m = (GET_VHT_CAPABILITY_ELE_SHORT_GI80M(
						 pvhtpriv_sta->vht_cap) & pvhtpriv_ap->sgi_80m) ? _TRUE : _FALSE;
		/* RTW_INFO("Current STA ShortGI80MHz = %d\n", pvhtpriv_sta->sgi_80m); */
	} else if (psta->cmn.bw_mode >= CHANNEL_WIDTH_160) {
		/* B5 Short GI for 80 MHz */
		pvhtpriv_sta->sgi_80m = (GET_VHT_CAPABILITY_ELE_SHORT_GI160M(
						 pvhtpriv_sta->vht_cap) & pvhtpriv_ap->sgi_80m) ? _TRUE : _FALSE;
	...
	pvhtpriv_sta->vht_highest_rate = rtw_get_vht_highest_rate(
...
void VHT_caps_handler(_adapter *padapter, PNDIS_802_11_VARIABLE_IEs pIE)
{
	...
	/* B5 Short GI for 80 MHz */
	pvhtpriv->sgi_80m = (GET_VHT_CAPABILITY_ELE_SHORT_GI80M(
				     pIE->data) & pvhtpriv->sgi_80m) ? _TRUE : _FALSE;
	...
	if (TEST_FLAG(pvhtpriv->beamform_cap, BEAMFORMING_VHT_BEAMFORMEE_ENABLE)
		...
		SET_FLAG(cur_beamform_cap,
			 GET_VHT_CAPABILITY_ELE_SU_BFEE_STS_CAP(pIE->data) << 8);
	...
	if (TEST_FLAG(pvhtpriv->beamform_cap, BEAMFORMING_VHT_BEAMFORMER_ENABLE)
		...
		SET_FLAG(cur_beamform_cap,
			 GET_VHT_CAPABILITY_ELE_SU_BFER_SOUND_DIM_NUM(pIE->data) << 12);
...
void rtw_process_vht_op_mode_notify(_adapter *padapter, u8 *pframe,
				    PVOID sta)
{
	struct sta_info		*psta = (struct sta_info *)sta;
	struct mlme_priv		*pmlmepriv = &padapter->mlmepriv;
...
u32	rtw_build_vht_operation_ie(_adapter *padapter, u8 *pbuf, u8 channel)
{
	...
	bw_mode = REGSTY_BW_5G(
			  pregistrypriv); /* TODO: control op bw with other info */

	if (hal_chk_bw_cap(padapter, BW_CAP_80M | BW_CAP_160M)
	    && REGSTY_BW_5G(pregistrypriv) >= CHANNEL_WIDTH_80
	   ) {
		center_freq = rtw_get_center_ch(channel, bw_mode,
...
u32	rtw_build_vht_cap_ie(_adapter *padapter, u8 *pbuf)
{
	...
	RTW_DBG("%s, line%d, Available RX buf size = %d bytes\n.", __FUNCTION__,
		__LINE__, max_recvbuf_sz - rx_packet_offset);
	...
	SET_VHT_CAPABILITY_ELE_SHORT_GI80M(pcap,
					   pvhtpriv->sgi_80m ? 1 : 0); /* We can receive Short GI of 80M */
	if (pvhtpriv->sgi_80m)
	...
	if (TEST_FLAG(pvhtpriv->beamform_cap, BEAMFORMING_VHT_BEAMFORMEE_ENABLE)) {
		...
		if (TEST_FLAG(pvhtpriv->beamform_cap,
			   ...
			   uint in_len, uint *pout_len)
{
	u32	ielen;
	u8 max_bw;
	u8 oper_ch, oper_bw = CHANNEL_WIDTH_20,
	...
	ht_op_ie = rtw_get_ie(in_ie + 12, WLAN_EID_HT_OPERATION, &ielen,
			      in_len - 12);
	if (!ht_op_ie || ielen != HT_OP_IE_LEN)
		goto exit;
	vht_cap_ie = rtw_get_ie(in_ie + 12, EID_VHTCapability, &ielen,
				in_len - 12);
	...
	*pout_len += rtw_build_vht_cap_ie(padapter, out_ie + *pout_len);
	...
	rtw_set_ie(out_vht_op_ie, EID_VHTOperation, VHT_OP_IE_LEN, vht_op_ie + 2,
		   pout_len);
	...
	if (max_bw >= CHANNEL_WIDTH_40) {
		...
		if (oper_bw == CHANNEL_WIDTH_40) {
			...
			while (!rtw_chset_is_chbw_valid(adapter_to_chset(padapter), oper_ch,
							oper_bw, oper_offset)) {
				oper_bw--;
	...
	rtw_warn_on(!rtw_chset_is_chbw_valid(adapter_to_chset(padapter), oper_ch,
					     oper_bw, oper_offset));
	...
	*pout_len += rtw_build_vht_op_mode_notify_ie(padapter, out_ie + *pout_len,
			oper_bw);
...
void rtw_vht_ies_attach(_adapter *padapter, WLAN_BSSID_EX *pnetwork)
{
	...
	p = rtw_get_ie(pnetwork->IEs + _BEACON_IE_OFFSET_, EID_VHTCapability,
		       &ie_len,
