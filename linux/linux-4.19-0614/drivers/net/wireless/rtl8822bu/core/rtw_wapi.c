...
void WapiFreeAllStaInfo(_adapter *padapter)
{
	...
	while (!list_empty(&(pWapiInfo->wapiSTAIdleList))) {
		pWapiStaInfo = (PRT_WAPI_STA_INFO)list_entry(
				       pWapiInfo->wapiSTAIdleList.next, RT_WAPI_STA_INFO, list);
		list_del_init(&pWapiStaInfo->list);
	}

	/* BKID List */
	while (!list_empty(&(pWapiInfo->wapiBKIDIdleList))) {
		pWapiBkid = (PRT_WAPI_BKID)list_entry(pWapiInfo->wapiBKIDIdleList.next,
						      RT_WAPI_BKID, list);
...
u8 WapiGetEntryForCamWrite(_adapter *padapter, u8 *pMacAddr, u8 KID,
			   BOOLEAN IsMsk)
{
...
u8 WapiGetEntryForCamClear(_adapter *padapter, u8 *pPeerMac, u8 keyid,
			   u8 IsMsk)
{
	PRT_WAPI_T		pWapiInfo = NULL;
	u8		i = 0;
...
u8 rtw_wapi_is_wai_packet(_adapter *padapter, u8 *pkt_data)
{
	...
	if ((!padapter->WapiSupport) || (!pWapiInfo->bWapiEnable)) {
		WAPI_TRACE(WAPI_MLME,
			   "<========== %s, WAPI not supported or not enabled!\n", __FUNCTION__);
		return 0;
	}
	...
	/* 88b4? */
	if ((pkt_data[Offset_TypeWAI] == 0x88)
	    && (pkt_data[Offset_TypeWAI + 1] == 0xb4)) {
...
void rtw_wapi_update_info(_adapter *padapter,
...
)
{
	...
	if (list_empty(&pWapiInfo->wapiSTAUsedList))
		...
		list_for_each_entry(pWapiSta, &pWapiInfo->wapiSTAUsedList, list) {
			if (_rtw_memcmp(precv_hdr->WapiSrcAddr, pWapiSta->PeerMacAddr,
					ETH_ALEN) == _TRUE) {
				bFind = true;
				break;
			}
...
void rtw_build_probe_resp_wapi_ie(_adapter *padapter,
				  unsigned char *pframe, struct pkt_attrib *pattrib)
{
	PRT_WAPI_T pWapiInfo = &(padapter->wapiInfo);
	u8 WapiIELength = 0;
...
void rtw_build_beacon_wapi_ie(_adapter *padapter, unsigned char *pframe,
...
void rtw_build_assoc_req_wapi_ie(_adapter *padapter, unsigned char *pframe,
				 struct pkt_attrib *pattrib)
{
	PRT_WAPI_BKID		pWapiBKID;
	u16					bkidNum;
			  ...
			  PNDIS_802_11_VARIABLE_IEs pIE)
{
	...
	pWapiSta = (PRT_WAPI_STA_INFO)list_entry(pWapiInfo->wapiSTAIdleList.next,
			RT_WAPI_STA_INFO, list);
	list_del_init(&pWapiSta->list);
	list_add_tail(&pWapiSta->list, &pWapiInfo->wapiSTAUsedList);
	_rtw_memcpy(pWapiSta->PeerMacAddr,
		    padapter->mlmeextpriv.mlmext_info.network.MacAddress, 6);
...
void rtw_wapi_return_one_sta_info(_adapter *padapter, u8 *MacAddr)
{
	...
	if (check_fwstate(pmlmepriv, WIFI_STATION_STATE)) {
		while (!list_empty(&(pWapiInfo->wapiBKIDStoreList))) {
			pWapiBkid = (PRT_WAPI_BKID)list_entry(pWapiInfo->wapiBKIDStoreList.next,
							      RT_WAPI_BKID, list);
			list_del_init(&pWapiBkid->list);
	...
	if (list_empty(&(pWapiInfo->wapiSTAUsedList))) {
...
#if 0
		pWapiStaInfo = (PRT_WAPI_STA_INFO)list_entry((
					pWapiInfo->wapiSTAUsedList.next), RT_WAPI_STA_INFO, list);
		...
		list_for_each_entry(pWapiStaInfo, &(pWapiInfo->wapiSTAUsedList), list) {
			...
			RTW_INFO("peer Addr %02x-%02x-%02x-%02x-%02x-%02x\n",
				 pWapiStaInfo->PeerMacAddr[0], pWapiStaInfo->PeerMacAddr[1],
				 pWapiStaInfo->PeerMacAddr[2], pWapiStaInfo->PeerMacAddr[3],
...
void rtw_wapi_set_key(_adapter *padapter, RT_WAPI_KEY *pWapiKey,
		      RT_WAPI_STA_INFO *pWapiSta, u8 bGroupKey, u8 bUseDefaultKey)
{
	PRT_WAPI_T		pWapiInfo =  &padapter->wapiInfo;
	...
	if (bUseDefaultKey) {
		...
		EntryId = WapiGetEntryForCamWrite(padapter, pMacAddr, pWapiKey->keyId,
						  bGroupKey);
	}
			...
			if (!memcmp(pWapiSta->PeerMacAddr, PeerAddr, 6)) {
				...
				case 1:              /* usk */
					if (bAuthenticator) {       /* authenticator */
						...
						if (!bUpdate) {    /* first */
							...
							WAPI_DATA(WAPI_INIT, "SetKey - AE USK Data Key", pWapiSta->wapiUsk.dataKey,
								  16);
							WAPI_DATA(WAPI_INIT, "SetKey - AE USK Mic Key", pWapiSta->wapiUsk.micKey,
								  16);
						} else {           /* update */
				...
				case 2:		/* msk */
					if (bAuthenticator) {        /* authenticator */
						...
						WAPI_DATA(WAPI_INIT, "SetKey - AE MSK Data Key",
							  pWapiInfo->wapiTxMsk.dataKey, 16);
						WAPI_DATA(WAPI_INIT, "SetKey - AE MSK Mic Key",
							  pWapiInfo->wapiTxMsk.micKey, 16);
					} else {
						if (!bUpdate) {
							...
							if (!pWapiSta->bSetkeyOk)
								pWapiSta->bSetkeyOk = true;
							pWapiInfo->bFirstAuthentiateInProgress = false;
							WAPI_DATA(WAPI_INIT, "SetKey - ASUE MSK Data Key",
								  pWapiSta->wapiMsk.dataKey, 16);
							WAPI_DATA(WAPI_INIT, "SetKey - ASUE MSK Mic Key", pWapiSta->wapiMsk.micKey,
								  16);
						} else {
...
void rtw_wapi_get_iv(_adapter *padapter, u8 *pRA, u8 *IV)
{
	...
	if (IS_MCAST(pRA)) {
		...
		if (list_empty(&pWapiInfo->wapiSTAUsedList)) {
			...
			list_for_each_entry(pWapiSta, &pWapiInfo->wapiSTAUsedList, list) {
				WAPI_DATA(WAPI_RX, "rtw_wapi_get_iv: peermacaddr ", pWapiSta->PeerMacAddr,
					  6);
	...
	if (psecuritypriv->dot11PrivacyAlgrthm == _SMS4_) {
		...
		if (IS_MCAST(pRA)) {
			if (!pWapiInfo->wapiTxMsk.bTxEnable) {
				bDrop = true;
				WAPI_TRACE(WAPI_RX,
					   "rtw_wapi_drop_for_key_absent: multicast key is absent\n");
				return bDrop;
			}
		} else {
			if (!list_empty(&pWapiInfo->wapiSTAUsedList)) {
				list_for_each_entry(pWapiSta, &pWapiInfo->wapiSTAUsedList, list) {
					WAPI_DATA(WAPI_RX, "rtw_wapi_drop_for_key_absent: pWapiSta->PeerMacAddr ",
						  pWapiSta->PeerMacAddr, 6);
				...
				if (bFindMatchPeer)	{
					if (!pWapiSta->wapiUsk.bTxEnable) {
						bDrop = true;
						WAPI_TRACE(WAPI_RX,
							   "rtw_wapi_drop_for_key_absent: unicast key is absent\n");
						return bDrop;
					}
				} else {
					bDrop = true;
...
void rtw_wapi_set_set_encryption(_adapter *padapter,
