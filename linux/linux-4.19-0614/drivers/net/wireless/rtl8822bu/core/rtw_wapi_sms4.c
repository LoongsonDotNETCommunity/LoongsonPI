...
static void xor_block(void *dst, void *src1, void *src2)
			...
			u8 *Output, u16 *OutputLength)
{
	...
	WapiSMS4Cryption(Key, IV, Input, InputLength, Output, OutputLength,
			 ENCRYPT);
}
		...
		if (pWapiInfo->wapiTxMsk.keyId <= 1) {
			...
			if (bPNOverflow) {
				...
				WAPI_TRACE(WAPI_ERR, "===============>%s():multicast PN overflow\n",
					   __FUNCTION__);
				rtw_wapi_app_event_handler(padapter, NULL, 0, pRA, false, false, true, 0,
							   false);
			}
		} else {
			...
			if (pWapiSta->wapiUsk.keyId <= 1) {
				...
				if (bPNOverflow) {
					...
					WAPI_TRACE(WAPI_ERR, "===============>%s():unicast PN overflow\n",
						   __FUNCTION__);
					rtw_wapi_app_event_handler(padapter, NULL, 0, pWapiSta->PeerMacAddr, false,
								   true, false, 0, false);
				}
			} else {
...
)
{
	...
	SecPtr = pframe;
	SecCalculateMicSMS4(KeyIdx, pMicKey, SecPtr, (SecPtr + DataOffset),
			    pattrib->pktlen, MicBuffer);

	WAPI_DATA(WAPI_TX, "Encryption - MIC", MicBuffer,
		  padapter->wapiInfo.extra_postfix_len);

	memcpy(pframe + pattrib->hdrlen + pattrib->iv_len + pattrib->pktlen -
	       pattrib->icv_len,
	       (u8 *)MicBuffer,
	       padapter->wapiInfo.extra_postfix_len
	      );
	...
	WapiSMS4Encryption(pDataKey, pIV, (SecPtr + DataOffset),
			   pattrib->pktlen + pattrib->icv_len, (SecPtr + DataOffset), &OutputLength);

	WAPI_DATA(WAPI_TX, "Encryption - After SMS4 encryption", pframe,
		  pattrib->hdrlen + pattrib->iv_len + pattrib->pktlen);
...
)
{
	...
	u8 *pRA, *pTA, *pMicKey, *pDataKey, *pLastRxPN, *pRecvPN, *pSecData,
	*pRecvMic, *pos;
	u8 TID = 0;
	...
	WAPI_TRACE(WAPI_RX, "=========>%s: check PN  %d\n", __FUNCTION__,
		   precv_hdr->bWapiCheckPNInDecrypt);
	WAPI_DATA(WAPI_RX, "Decryption - Before decryption", pskb->data,
		  pskb->len);
	...
	if (!bFindMatchPeer) {
		WAPI_TRACE(WAPI_ERR,
			   "%s: Can not find Peer Sta "MAC_FMT" for Key Info!!!\n", __FUNCTION__,
			   MAC_ARG(pTA));
		return false;
	}
	...
	if (IS_MCAST(pRA)) {
		...
		if (pWapiSta->wapiMsk.keyId == KeyIdx && pWapiSta->wapiMsk.bSet) {
		...
		} else if (pWapiSta->wapiMskUpdate.keyId == KeyIdx
			   && pWapiSta->wapiMskUpdate.bSet) {
			...
			WAPI_TRACE(WAPI_ERR,
				   "%s: Can not find MSK with matched KeyIdx(%d), Dropped !!!\n",
				   __FUNCTION__, KeyIdx);
			return false;
		}
	} else {
		...
		if (pWapiSta->wapiUsk.keyId == KeyIdx && pWapiSta->wapiUsk.bSet) {
			...
			pMicKey = pWapiSta->wapiUsk.micKey;
			pDataKey = pWapiSta->wapiUsk.dataKey;
		} else if (pWapiSta->wapiUskUpdate.keyId == KeyIdx
			   && pWapiSta->wapiUskUpdate.bSet) {
	...
	WapiSMS4Decryption(pDataKey, pRecvPN, pSecData, DataLen, pSecData,
			   &OutputLength);
	...
	SecCalculateMicSMS4(KeyIdx, pMicKey, pskb->data, pSecData, DataLen,
			    MicBuffer);

	WAPI_DATA(WAPI_RX, "Decryption - MIC received", pRecvMic, SMS4_MIC_LEN);
	WAPI_DATA(WAPI_RX, "Decryption - MIC calculated", MicBuffer, SMS4_MIC_LEN);

	if (0 == memcmp(MicBuffer, pRecvMic,
			padapter->wapiInfo.extra_postfix_len)) {
...
u32	rtw_sms4_decrypt(_adapter *padapter, u8 *precvframe)
{
	...
	* return tempraily */
	...
	if (false == SecSWSMS4Decryption(padapter, precvframe,
					 &padapter->recvpriv)) {
