 ...
 */
u8 *rtw_get_ie_ex(const u8 *in_ie, uint in_len, u8 eid, const u8 *oui,
...
int rtw_generate_ie(struct registry_priv *pregistrypriv)
{
	...
	*(u16 *)ie = cpu_to_le16((u16)
				 pdev_network->Configuration.BeaconPeriod); /* BCN_INTERVAL; */
	sz += 2;
	ie += 2;
	...
	*(u16 *)ie |= cpu_to_le16(cap_IBSS);
	...
	} else if (pregistrypriv->wireless_mode ==
		   WIRELESS_MODE_MAX) { /* WIRELESS_11ABGN | WIRELESS_11AC */
		if (pdev_network->Configuration.DSConfig > 14)
...
unsigned char *rtw_get_wpa_ie(unsigned char *pie, int *wpa_ie_len,
			      int limit)
{
	int len;
	u16 val16;
...
unsigned char *rtw_get_wpa2_ie(unsigned char *pie, int *rsn_ie_len,
			       int limit)
{
...
int rtw_parse_wpa2_ie(u8 *rsn_ie, int rsn_ie_len, int *group_cipher,
		      int *pairwise_cipher, int *is_8021x, u8 *mfp_opt)
{
...
u8 *rtw_get_wps_ie_from_scan_queue(u8 *in_ie, uint in_len, u8 *wps_ie,
				   uint *wps_ielen, enum bss_type frame_type)
{
	u8	*wps = NULL;
	...
	case BSS_TYPE_PROB_RSP: {
		...
		wps = rtw_get_wps_ie(in_ie + _PROBERSP_IE_OFFSET_,
				     in_len - _PROBERSP_IE_OFFSET_, wps_ie, wps_ielen);
		break;
	}
	case BSS_TYPE_PROB_REQ: {
		/*	Probe Request */
		wps = rtw_get_wps_ie(in_ie + _PROBEREQ_IE_OFFSET_,
				     in_len - _PROBEREQ_IE_OFFSET_, wps_ie, wps_ielen);
		break;
	}
	default:
	case BSS_TYPE_UNDEF:
 ...
 */
u8 *rtw_get_wps_ie(const u8 *in_ie, uint in_len, u8 *wps_ie,
		   uint *wps_ielen)
{
	uint cnt;
	...
	while (cnt + 1 + 4 < in_len) {
		...
		if (eid == WLAN_EID_VENDOR_SPECIFIC
		    && _rtw_memcmp(&in_ie[cnt + 2], wps_oui, 4) == _TRUE) {
			wpsie_ptr = in_ie + cnt;
...
{
	return (key_char2num(hch) * 100) + (key_char2num(mch) * 10) + key_char2num(
		       lch);
}
...
u32 rtw_get_p2p_merged_ies_len(u8 *in_ie, u32 in_len)
{
	...
	while (i < in_len) {
		...
		if (pIE->ElementID == _VENDOR_SPECIFIC_IE_
		    && _rtw_memcmp(pIE->data, OUI, 4)) {
			len += pIE->Length -
			       4; /* 4 is P2P OUI length, don't count it in this loop */
		}
 ...
 */
u8 *rtw_get_p2p_ie(const u8 *in_ie, int in_len, u8 *p2p_ie,
		   uint *p2p_ielen)
{
	uint cnt;
			     ...
			     u8 *buf_content, uint *len_content)
{
	...
	attr_ptr = rtw_get_p2p_attr(p2p_ie, p2p_ielen, target_attr_id, NULL,
				    &attr_len);
...
inline u8 *rtw_bss_ex_get_p2p_ie(WLAN_BSSID_EX *bss_ex, u8 *p2p_ie,
				 uint *p2p_ielen)
{
	return rtw_get_p2p_ie(BSS_EX_TLV_IES(bss_ex), BSS_EX_TLV_IES_LEN(bss_ex),
			      p2p_ie, p2p_ielen);
}
...
#define DBG_BSS_EX_DEL_P2P_IE 0
	...
	ies_len = rtw_del_p2p_ie(ies, ies_len_ori,
				 DBG_BSS_EX_DEL_P2P_IE ? __func__ : NULL);
 ...
 * Returns: The address of the P2P IE found, or NULL
 */
u8 *rtw_get_wfd_ie(const u8 *in_ie, int in_len, u8 *wfd_ie,
		   uint *wfd_ielen)
{
	uint cnt;
 ...
 * Returns: the address of the specific WPS attribute found, or NULL
 */
u8 *rtw_get_wfd_attr(u8 *wfd_ie, uint wfd_ielen, u8 target_attr_id,
		     u8 *buf_attr, u32 *len_attr)
{
	u8 *attr_ptr = NULL;
	u8 *target_attr_ptr = NULL;
 ...
 */
u8 *rtw_get_wfd_attr_content(u8 *wfd_ie, uint wfd_ielen, u8 target_attr_id,
			     u8 *buf_content, uint *len_content)
{
	u8 *attr_ptr;
	u32 attr_len;
	...
	attr_ptr = rtw_get_wfd_attr(wfd_ie, wfd_ielen, target_attr_id, NULL,
				    &attr_len);
...
inline u8 *rtw_bss_ex_get_wfd_ie(WLAN_BSSID_EX *bss_ex, u8 *wfd_ie,
				 uint *wfd_ielen)
{
	return rtw_get_wfd_ie(BSS_EX_TLV_IES(bss_ex), BSS_EX_TLV_IES_LEN(bss_ex),
			      wfd_ie, wfd_ielen);
}
...
#define DBG_BSS_EX_DEL_WFD_IE 0
	...
	ies_len = rtw_del_wfd_ie(ies, ies_len_ori,
				 DBG_BSS_EX_DEL_WFD_IE ? __func__ : NULL);
	bss_ex->IELength -= ies_len_ori - ies_len;
}
...
int rtw_get_cipher_info(struct wlan_network *pnetwork)
{
	...
	if (pbuf && (wpa_ielen > 0)) {
		if (_SUCCESS == rtw_parse_wpa_ie(pbuf, wpa_ielen + 2, &group_cipher,
...
void rtw_get_bcn_info(struct wlan_network *pnetwork)
{
	...
	/* parsing HT_INFO_IE */
	p = rtw_get_ie(pnetwork->network.IEs + _FIXED_IE_LENGTH_, _HT_ADD_INFO_IE_,
...
u16 rtw_mcs_rate(u8 rf_type, u8 bw_40MHz, u8 short_GI,
		 unsigned char *MCS_rate)
{
	...
	if (MCS_rate[3]) {
		if (MCS_rate[3] & BIT(7))
			max_rate = (bw_40MHz) ? ((short_GI) ? 6000 : 5400) : ((
						short_GI) ? 2889 : 2600);
		else if (MCS_rate[3] & BIT(6))
			max_rate = (bw_40MHz) ? ((short_GI) ? 5400 : 4860) : ((
						short_GI) ? 2600 : 2340);
		else if (MCS_rate[3] & BIT(5))
			max_rate = (bw_40MHz) ? ((short_GI) ? 4800 : 4320) : ((
						short_GI) ? 2311 : 2080);
		else if (MCS_rate[3] & BIT(4))
			max_rate = (bw_40MHz) ? ((short_GI) ? 3600 : 3240) : ((
						short_GI) ? 1733 : 1560);
		else if (MCS_rate[3] & BIT(3))
			max_rate = (bw_40MHz) ? ((short_GI) ? 2400 : 2160) : ((
						short_GI) ? 1156 : 1040);
		else if (MCS_rate[3] & BIT(2))
			max_rate = (bw_40MHz) ? ((short_GI) ? 1800 : 1620) : ((
						short_GI) ? 867 : 780);
		...
		if (MCS_rate[2] & BIT(7))
			max_rate = (bw_40MHz) ? ((short_GI) ? 4500 : 4050) : ((
						short_GI) ? 2167 : 1950);
		else if (MCS_rate[2] & BIT(6))
			max_rate = (bw_40MHz) ? ((short_GI) ? 4050 : 3645) : ((
						short_GI) ? 1950 : 1750);
		else if (MCS_rate[2] & BIT(5))
			max_rate = (bw_40MHz) ? ((short_GI) ? 3600 : 3240) : ((
						short_GI) ? 1733 : 1560);
		else if (MCS_rate[2] & BIT(4))
			max_rate = (bw_40MHz) ? ((short_GI) ? 2700 : 2430) : ((
						short_GI) ? 1300 : 1170);
		...
		if (MCS_rate[1] & BIT(7))
			max_rate = (bw_40MHz) ? ((short_GI) ? 3000 : 2700) : ((
						short_GI) ? 1444 : 1300);
