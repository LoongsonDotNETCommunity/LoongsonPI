...
unsigned int ratetbl2rateset(_adapter *padapter, unsigned char *rateset)
{
	...
	for (i = 0; i < NumRates; i++) {
		...
		if (rtw_get_oper_ch(padapter) > 14
		    && rate < _6M_RATE_) /*5G no support CCK rate*/
			continue;
...
u8 rtw_get_offset_by_chbw(u8 ch, u8 bw, u8 *r_offset)
{
	...
	if (ch >= 1 && ch <= 4)
		...
		if (*r_offset == HAL_PRIME_CHNL_OFFSET_LOWER
		    || *r_offset == HAL_PRIME_CHNL_OFFSET_UPPER)
...
{
	if (adapter->mlmeextpriv.cur_channel == adapter_to_dvobj(
		    adapter)->oper_channel)
		return adapter_to_dvobj(adapter)->on_oper_ch_time;
	else
		return 0;
}
			...
			unsigned char channel_offset, unsigned short bwmode)
{
...
#ifdef CONFIG_DFS_MASTER
	{
		...
		bool ori_overlap_radar_detect_ch = rtw_rfctl_overlap_radar_detect_ch(
				rfctl);
		bool new_overlap_radar_detect_ch = _rtw_rfctl_overlap_radar_detect_ch(
				rfctl, channel, bwmode, channel_offset);
...
int is_IBSS_empty(_adapter *padapter)
{
	...
	for (i = 0; i < macid_ctl->num; i++) {
		...
		if (!GET_H2CCMD_MSRRPT_PARM_OPMODE(&macid_ctl->h2c_msr[i]))
			continue;
		if (GET_H2CCMD_MSRRPT_PARM_ROLE(&macid_ctl->h2c_msr[i]) ==
		    H2C_MSR_ROLE_ADHOC)
			return _FAIL;
	}
...
bool cam_cache_chk(_adapter *adapter, u8 id, u8 *addr, s16 kid, s8 gk)
{
	...
	if (addr && _rtw_memcmp(dvobj->cam_cache[id].mac, addr,
				ETH_ALEN) == _FALSE)
		goto exit;
...
s16 rtw_get_camid(_adapter *adapter, u8 *addr, s16 kid, u8 gk)
{
	...
	for (i = 0; i < cam_ctl->num; i++) {
		...
		if (((i + start_id) % cam_ctl->num) < 4)
			continue;
#endif
		if (_rtw_sec_camid_is_used(cam_ctl,
					   ((i + start_id) % cam_ctl->num)) == _FALSE)
			break;
	}
...
inline void rtw_sec_cam_swap(_adapter *adapter, u8 cam_id_a, u8 cam_id_b)
{
	...
	if (cam_a_used)
		_rtw_memcpy(&cache_a, &dvobj->cam_cache[cam_id_a],
			    sizeof(struct sec_cam_ent));

	if (cam_b_used)
		_rtw_memcpy(&cache_b, &dvobj->cam_cache[cam_id_b],
			    sizeof(struct sec_cam_ent));
...
void rtw_process_wfd_ie(_adapter *adapter, u8 *wfd_ie, u8 wfd_ielen,
			const char *tag)
{
	...
	if (attr_content && attr_contentlen) {
		...
		RTW_INFO("[%s] Peer PORT NUM = %d\n", tag,
			 wdinfo->wfd_info->peer_rtsp_ctrlport);
	}
}
			 ...
			 const char *tag)
{
	...
	while (wfd_ie) {
		...
		wfd_ie = rtw_get_wfd_ie(wfd_ie + wfd_ielen,
					(ies + ies_len) - (wfd_ie + wfd_ielen), NULL, &wfd_ielen);
	}
}
...
int WMM_param_handler(_adapter *padapter, PNDIS_802_11_VARIABLE_IEs	pIE)
{
	...
	if (pregpriv->wifi_spec == 1) {
		if (pmlmeinfo->WMM_enable == 1) {
			...
			pmlmeinfo->WMM_enable = 1;
			_rtw_rtw_memcpy(&(pmlmeinfo->WMM_param), (pIE->data + 6),
					sizeof(struct WMM_para_element));
			return _TRUE;
		}
	} else {
...
static void bwmode_update_check(_adapter *padapter,
...
void HT_caps_handler(_adapter *padapter, PNDIS_802_11_VARIABLE_IEs pIE)
{
	...
	pmlmeinfo->HT_caps.u.HT_cap_element.HT_caps_info = le16_to_cpu(
				pmlmeinfo->HT_caps.u.HT_cap_element.HT_caps_info);
	pmlmeinfo->HT_caps.u.HT_cap_element.HT_ext_caps = le16_to_cpu(
	...
	if (check_fwstate(pmlmepriv, WIFI_AP_STATE)) {
		...
		if (TEST_FLAG(phtpriv->beamform_cap, BEAMFORMING_HT_BEAMFORMER_ENABLE) &&
			...
			SET_FLAG(cur_beamform_cap,
				 GET_HT_CAP_TXBF_CHNL_ESTIMATION_NUM_ANTENNAS(pIE->data) << 6);
		}
		...
		if (TEST_FLAG(phtpriv->beamform_cap, BEAMFORMING_HT_BEAMFORMEE_ENABLE) &&
			...
			SET_FLAG(cur_beamform_cap,
				 GET_HT_CAP_TXBF_COMP_STEERING_NUM_ANTENNAS(pIE->data) << 4);
		}
		     ...
		     struct beacon_keys *recv_beacon)
{
	...
	capability = le16_to_cpu(*(unsigned short *)(pframe + WLAN_HDR_A3_LEN +
				 10));

	/* checking IEs */
	left = packet_len - sizeof(struct rtw_ieee80211_hdr_3addr) -
	       _BEACON_IE_OFFSET_;
...
int rtw_check_bcn_info(ADAPTER *Adapter, u8 *pframe, u32 packet_len)
{
	...
	if (_rtw_memcmp(&recv_beacon, &pmlmepriv->cur_beacon_keys,
			sizeof(recv_beacon)) == _TRUE)
		pmlmepriv->new_beacon_cnts = 0;
	else if ((pmlmepriv->new_beacon_cnts == 0) ||
		 _rtw_memcmp(&recv_beacon, &pmlmepriv->new_beacon_keys,
	...
	if (_rtw_memcmp(bssid->Ssid.Ssid, cur_network->network.Ssid.Ssid,
			32) == _FALSE ||
	...
	rtw_get_sec_ie(bssid->IEs, bssid->IELength, NULL, &rsn_len, NULL,
		       &wpa_len);
	...
	if (encryp_protocol == ENCRYP_PROTOCOL_WPA
		...
		if (pbuf && (wpa_ielen > 0)) {
			rtw_parse_wpa_ie(pbuf, wpa_ielen + 2, &group_cipher, &pairwise_cipher,
					 &is_8021x);
		} else {
			pbuf = rtw_get_wpa2_ie(&bssid->IEs[12], &wpa_ielen, bssid->IELength - 12);

			if (pbuf && (wpa_ielen > 0)) {
				rtw_parse_wpa2_ie(pbuf, wpa_ielen + 2, &group_cipher, &pairwise_cipher,
						  &is_8021x, NULL);
			}
		}
...
void update_beacon_info(_adapter *padapter, u8 *pframe, uint pkt_len,
			struct sta_info *psta)
{
	unsigned int i;
	unsigned int len;
	...
	for (i = 0; i < len;) {
		pIE = (PNDIS_802_11_VARIABLE_IEs)(pframe + (_BEACON_IE_OFFSET_ +
						  WLAN_HDR_A3_LEN) + i);
...
unsigned int is_ap_in_tkip(_adapter *padapter)
{
	...
	if (rtw_get_capability((WLAN_BSSID_EX *)cur_network) &
		...
		for (i = sizeof(NDIS_802_11_FIXED_IEs); i < pmlmeinfo->network.IELength;) {
			...
			case _VENDOR_SPECIFIC_IE_:
				if ((_rtw_memcmp(pIE->data, RTW_WPA_OUI, 4))
				    && (_rtw_memcmp((pIE->data + 12), WPA_TKIP_CIPHER, 4)))
					return _TRUE;
				break;
...
void update_capinfo(PADAPTER Adapter, u16 updateCap)
{
	...
	if (updateCap & cIBSS) {
		...
		if (pmlmeext->cur_wireless_mode & (WIRELESS_11_24N | WIRELESS_11A |
						   WIRELESS_11_5N | WIRELESS_11AC))
...
void update_wireless_mode(_adapter *padapter)
{
	...
	rtw_hal_set_hwreg(padapter, HW_VAR_WIRELESS_MODE,
...
void fire_write_MAC_cmd(_adapter *padapter, unsigned int addr,
			unsigned int value);
void fire_write_MAC_cmd(_adapter *padapter, unsigned int addr,
			unsigned int value)
{
	...
	pwriteMacPara = (struct reg_rw_parm *)rtw_malloc(sizeof(
				struct reg_rw_parm));
	...
	init_h2fwcmd_w_parm_no_rsp(ph2c, pwriteMacPara,
...
void process_addba_req(_adapter *padapter, u8 *paddba_req, u8 *addr)
{
	...
	if (accept == _TRUE)
		...
		rtw_addbarsp_cmd(padapter, addr, tid, 37, size,
				 start_seq); /* reject ADDBA Req */
...
void adaptive_early_32k(struct mlme_ext_priv *pmlmeext, u8 *pframe,
			uint len)
{
	int i;
	u8 *pIE;
...
inline bool rtw_macid_is_iface_specific(struct macid_ctl_t *macid_ctl,
					u8 id, _adapter *adapter)
{
	int i;
	u8 iface_bmp = 0;
...
void rtw_alloc_macid(_adapter *padapter, struct sta_info *psta)
{
	...
	if (_rtw_memcmp(psta->cmn.mac_addr, adapter_mac_addr(padapter),
			ETH_ALEN)) {
	...
	for (i = last_id; i < macid_ctl->num; i++) {
		...
		if (is_bc_sta) {
			...
			if ((!rtw_macid_is_used(macid_ctl, i))
			    && (!rtw_sec_camid_is_used(cam_ctl, i)))
				break;
		} else {
			if (!rtw_macid_is_used(macid_ctl, i))
				break;
...
inline void rtw_macid_ctl_set_h2c_msr(struct macid_ctl_t *macid_ctl, u8 id,
				      u8 h2c_msr)
{
	if (id >= macid_ctl->num) {
		rtw_warn_on(1);
	...
	if (0)
		RTW_INFO("macid:%u, h2c_msr:"H2C_MSR_FMT"\n", id,
			 H2C_MSR_ARG(&macid_ctl->h2c_msr[id]));
}

inline void rtw_macid_ctl_set_bw(struct macid_ctl_t *macid_ctl, u8 id,
				 u8 bw)
{
	if (id >= macid_ctl->num) {
		rtw_warn_on(1);
...
inline void rtw_macid_ctl_set_vht_en(struct macid_ctl_t *macid_ctl, u8 id,
				     u8 en)
{
	if (id >= macid_ctl->num) {
		rtw_warn_on(1);
...
inline void rtw_macid_ctl_set_rate_bmp0(struct macid_ctl_t *macid_ctl,
					u8 id, u32 bmp)
{
	if (id >= macid_ctl->num) {
		rtw_warn_on(1);
...
inline void rtw_macid_ctl_set_rate_bmp1(struct macid_ctl_t *macid_ctl,
					u8 id, u32 bmp)
{
	if (id >= macid_ctl->num) {
		rtw_warn_on(1);
...
inline void rtw_macid_ctl_init_sleep_reg(struct macid_ctl_t *macid_ctl,
		u16 m0, u16 m1, u16 m2, u16 m3)
{
				...
				unsigned char *beacon_frame)
{
	...
	*(fctrl) = 0;
	...
	pframe = rtw_set_ie(pframe, _SSID_IE_, cur_network->Ssid.SsidLength,
			    cur_network->Ssid.Ssid, &len);

	/* supported rates... */
	rate_len = rtw_get_rateset_len(cur_network->SupportedRates);
	pframe = rtw_set_ie(pframe, _SUPPORTEDRATES_IE_,
			    ((rate_len > 8) ? 8 : rate_len), cur_network->SupportedRates, &len);

	/* DS parameter set */
	pframe = rtw_set_ie(pframe, _DSSET_IE_, 1,
			    (unsigned char *) & (cur_network->Configuration.DSConfig), &len);

	/* IBSS Parameter Set... */
	/* ATIMWindow = cur->Configuration.ATIMWindow; */
	ATIMWindow = 0;
	pframe = rtw_set_ie(pframe, _IBSS_PARA_IE_, 2,
			    (unsigned char *)(&ATIMWindow), &len);
	...
	if (rate_len > 8)
		pframe = rtw_set_ie(pframe, _EXT_SUPPORTEDRATES_IE_, (rate_len - 8),
				    (cur_network->SupportedRates + 8), &len);
...
void rtw_dev_pno_debug(struct net_device *net)
{
	...
	for (i = 0 ; i < MAX_PNO_LIST_COUNT ; i++) {
		RTW_INFO("%d SSID (%s) length (%d) cipher(%x) channel(%d)\n",
			 ...
			 pwrctl->pnlo_info->ssid_cipher_info[i],
			 pwrctl->pnlo_info->ssid_channel_info[i]);
	}
