...
#ifdef DBG_MEM_ALLOC
extern bool match_mstat_sniff_rules(const enum mstat_f flags,
				    const size_t size);
struct sk_buff *dbg_rtw_cfg80211_vendor_event_alloc(struct wiphy *wiphy,
		struct wireless_dev *wdev, int len, int event_id, gfp_t gfp
...
struct sk_buff *dbg_rtw_cfg80211_vendor_cmd_alloc_reply_skb(
	struct wiphy *wiphy, int len
	, const enum mstat_f flags, const char *func, const int line)
{
	struct sk_buff *skb;
	unsigned int truesize = 0;
...
struct sk_buff *rtw_cfg80211_vendor_event_alloc(
	struct wiphy *wiphy, struct wireless_dev *wdev, int len, int event_id,
	gfp_t gfp)
{
	struct sk_buff *skb;
				   ...
				   struct net_device *dev, int event_id, const void  *data, int len)
{
	...
	skb = rtw_cfg80211_vendor_event_alloc(wiphy, ndev_to_wdev(dev), len,
					      event_id, kflags);
	if (!skb) {
...
int rtw_dev_get_feature_set(struct net_device *dev)
{
	...
	if (is_supported_5g(adapter_to_regsty(adapter)->wireless_mode)
	    && hal_chk_band_cap(adapter,
				BAND_CAP_5G)) /* v5.3 has no rtw_init_wireless_mode(), need checking hal spec here */
		...
		struct wireless_dev *wdev, const void  *data, int len)
{
	...
	err =  rtw_cfgvendor_send_cmd_reply(wiphy, wdev_to_ndev(wdev), &reply,
					    sizeof(int));
		...
		struct wireless_dev *wdev, const void  *data, int len)
{
	...
	if (unlikely(!skb)) {
		RTW_ERR(FUNC_NDEV_FMT" skb alloc failed",
			FUNC_NDEV_ARG(wdev_to_ndev(wdev)));
		err = -ENOMEM;
		goto exit;
	}
				     ...
				     struct net_device *dev, void  *data, int len, rtw_vendor_event_t event)
{
	...
	while (total > 0) {
		...
		/* Alloc the SKB for vendor_event */
		skb = rtw_cfg80211_vendor_event_alloc(wiphy, ndev_to_wdev(dev), malloc_len,
						      event, kflags);
		if (!skb) {
		...
		struct wireless_dev *wdev, const void  *data, int len)
{
	...
	mem_needed = mem_needed - (SCAN_RESULTS_COMPLETE_FLAG_LEN +
...
static int rtw_cfgvendor_rtt_set_config(struct wiphy *wiphy,
...
static int rtw_cfgvendor_rtt_cancel_config(struct wiphy *wiphy,
		struct wireless_dev *wdev,
		const void *data, int len)
{
	...
	nla_for_each_attr(iter, data, len, rem) {
		...
		if (dhd_dev_rtt_cancel_cfg(bcmcfg_to_prmry_ndev(cfg), mac_list,
					   target_cnt) < 0) {
...
static int rtw_cfgvendor_rtt_get_capability(struct wiphy *wiphy,
		struct wireless_dev *wdev,
		const void *data, int len)
{
	int err = 0;
		...
		struct wireless_dev *wdev, const void  *data, int len)
{
	...
	radio->tx_time_per_levels = (u32 *)(output + sizeof(wifi_radio_stat) +
