...
int rtw_os_recvframe_duplicate_skb(_adapter *padapter,
				   union recv_frame *pcloneframe, _pkt *pskb)
{
	int res = _SUCCESS;
	_pkt	*pkt_copy = NULL;
...
int rtw_os_alloc_recvframe(_adapter *padapter,
			   union recv_frame *precvframe, u8 *pdata, _pkt *pskb)
{
	int res = _SUCCESS;
	u8	shift_sz = 0;
	...
	shift_sz = pattrib->qos ? 6 :
		   0; /*	Qos data, wireless lan header length is 26 */
...
int rtw_os_recv_resource_alloc(_adapter *padapter,
...
int rtw_os_recvbuf_resource_alloc(_adapter *padapter,
				  struct recv_buf *precvbuf)
{
	...
	precvbuf->pdata = precvbuf->phead = precvbuf->ptail = precvbuf->pend =
			NULL;
...
#ifdef CONFIG_USE_USB_BUFFER_ALLOC_RX
	precvbuf->pallocated_buf = rtw_usb_buffer_alloc(pusbd,
				   (size_t)precvbuf->alloc_sz, &precvbuf->dma_transfer_addr);
...
int rtw_os_recvbuf_resource_free(_adapter *padapter,
				 struct recv_buf *precvbuf)
{
	int ret = _SUCCESS;
	...
	rtw_usb_buffer_free(pusbd, (size_t)precvbuf->alloc_sz,
			    precvbuf->pallocated_buf, precvbuf->dma_transfer_addr);
...
_pkt *rtw_os_alloc_msdu_pkt(union recv_frame *prframe, const u8 *da,
			    const u8 *sa, u8 *msdu, u16 msdu_len)
{
	u16	eth_type;
	u8	*data_ptr;
	_pkt *sub_skb;
			      ...
			      union recv_frame *rframe)
{
	...
	if (pkt) {
		...
		if (MLME_IS_AP(padapter)) {
			...
			if (_rtw_memcmp(ehdr->h_dest, adapter_mac_addr(padapter),
					ETH_ALEN) == _FALSE) {
...
void rtw_handle_tkip_mic_err(_adapter *padapter, struct sta_info *sta,
			     u8 bgroup)
{
	...
	cfg80211_michael_mic_failure(padapter->pnetdev, sta->cmn.mac_addr,
...
void rtw_hostapd_mlme_rx(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	*	if(skb == NULL) goto _exit; */
	...
	precv_frame->u.hdr.pkt =
		NULL; /* set pointer to NULL before rtw_free_recvframe() if call rtw_netif_rx() */
}
