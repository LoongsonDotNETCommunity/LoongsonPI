...
static const char *nl80211_channel_type_str(enum nl80211_channel_type
		ctype)
{
	switch (ctype) {
...
static enum nl80211_channel_type rtw_chbw_to_nl80211_channel_type(u8 ch,
		u8 bw, u8 offset, u8 ht)
{
	rtw_warn_on(!ht && (bw >= CHANNEL_WIDTH_40
...
static void rtw_get_chbw_from_nl80211_channel_type(struct ieee80211_channel
		*chan, enum nl80211_channel_type ctype, u8 *ht, u8 *ch, u8 *bw, u8 *offset)
{
	int pri_freq;
...
static u8 rtw_chbw_to_cfg80211_chan_def(struct wiphy *wiphy,
					struct cfg80211_chan_def *chdef, u8 ch, u8 bw, u8 offset, u8 ht)
{
	int freq, cfreq;
...
static void rtw_get_chbw_from_cfg80211_chan_def(struct cfg80211_chan_def
		*chdef, u8 *ht, u8 *ch, u8 *bw, u8 *offset)
{
	int pri_freq;
	...
	case NL80211_CHAN_WIDTH_40:
		*ht = 1;
		*bw = CHANNEL_WIDTH_40;
		*offset = pri_freq > chdef->center_freq1 ? HAL_PRIME_CHNL_OFFSET_UPPER :
...
u8 rtw_cfg80211_ch_switch_notify(_adapter *adapter, u8 ch, u8 bw,
				 u8 offset, u8 ht)
{
...
NDIS_802_11_NETWORK_INFRASTRUCTURE nl80211_iftype_to_rtw_network_type(
...
static int rtw_cfg80211_sync_iftype(_adapter *adapter)
{
	...
	if (!(nl80211_iftype_to_rtw_mlme_state(rtw_wdev->iftype) & MLME_STATE(
		      adapter))) {
...
static int rtw_cfg80211_clear_wps_sr_of_non_target_bss(_adapter *padapter,
		struct wlan_network *pnetwork, struct cfg80211_ssid *req_ssid)
{
...
struct cfg80211_bss *rtw_cfg80211_inform_bss(_adapter *padapter,
		struct wlan_network *pnetwork)
{
...
#ifndef CONFIG_WAPI_SUPPORT
	{
		...
		if (rtw_get_wapi_ie(pnetwork->network.IEs, pnetwork->network.IELength,
				    NULL, &wapi_len) > 0) {
			if (wapi_len > 0) {
	...
	if (0)
		notify_timestamp = le64_to_cpu(*(u64 *)rtw_get_timestampe_from_ie(
						       pnetwork->network.IEs));
	else
		notify_timestamp = rtw_get_systime_us();

	notify_interval = le16_to_cpu(*(u16 *)rtw_get_beacon_interval_from_ie(
					      pnetwork->network.IEs));
	notify_capability = le16_to_cpu(*(u16 *)rtw_get_capability_from_ie(
						pnetwork->network.IEs));
	...
	if (check_fwstate(pmlmepriv, _FW_LINKED) == _TRUE &&
		...
		notify_signal = 100 * translate_percentage_to_dbm(
					pnetwork->network.PhyInfo.SignalStrength); /* dbm */
	}
	...
	*(fctrl) = 0;
	...
	_rtw_memcpy((pbuf + len), pnetwork->network.IEs,
	...
	*((u64 *)(pbuf + len)) = cpu_to_le64(notify_timestamp);
	...
	if (rtw_get_p2p_ie(pnetwork->network.IEs + 12,
			   pnetwork->network.IELength - 12, NULL, NULL))
		RTW_INFO("%s, got p2p_ie\n", __func__);
#endif

#if 1
	bss = cfg80211_inform_bss_frame(wiphy, notify_channel,
					(struct ieee80211_mgmt *)pbuf,
...
int rtw_cfg80211_check_bss(_adapter *padapter)
{
	...
	bss = cfg80211_get_bss(padapter->rtw_wdev->wiphy, notify_channel,
			       ...
			       pnetwork->InfrastructureMode == Ndis802_11Infrastructure ?
			       IEEE80211_BSS_TYPE_ESS : IEEE80211_BSS_TYPE_IBSS,
...
void rtw_cfg80211_indicate_connect(_adapter *padapter)
{
	...
	if (pwdinfo->driver_interface == DRIVER_CFG80211) {
		...
		if (!rtw_p2p_chk_state(pwdinfo, P2P_STATE_NONE)) {
			...
			RTW_INFO("%s, role=%d, p2p_state=%d, pre_p2p_state=%d\n", __func__,
				 rtw_p2p_role(pwdinfo), rtw_p2p_state(pwdinfo), rtw_p2p_pre_state(pwdinfo));
		}
#endif
	}
#endif /* CONFIG_P2P */
	...
	if (check_fwstate(pmlmepriv, WIFI_MONITOR_STATE) != _TRUE) {
		...
		if (_rtw_memcmp(scanned->network.MacAddress, pnetwork->MacAddress,
				sizeof(NDIS_802_11_MAC_ADDRESS)) == _TRUE
		    && _rtw_memcmp(&(scanned->network.Ssid), &(pnetwork->Ssid),
				   sizeof(NDIS_802_11_SSID)) == _TRUE
		   ) {
			if (!rtw_cfg80211_inform_bss(padapter, scanned))
				RTW_INFO(FUNC_ADPT_FMT" inform fail !!\n", FUNC_ADPT_ARG(padapter));
...
check_bss:
	...
	if (rtw_to_roam(padapter) > 0) {
		...
		roam_info.req_ie = pmlmepriv->assoc_req + sizeof(struct
				   rtw_ieee80211_hdr_3addr) + 2;
		roam_info.req_ie_len = pmlmepriv->assoc_req_len - sizeof(
					       struct rtw_ieee80211_hdr_3addr) - 2;
		roam_info.resp_ie = pmlmepriv->assoc_rsp + sizeof(struct
				    rtw_ieee80211_hdr_3addr) + 6;
		roam_info.resp_ie_len = pmlmepriv->assoc_rsp_len - sizeof(
...
void rtw_cfg80211_indicate_disconnect(_adapter *padapter, u16 reason,
				      u8 locally_generated)
{
	...
	if (padapter->ndev_unregistering
	    || !rtw_wdev_not_indic_disco(pwdev_priv)) {
#if LINUX_VERSION_CODE < KERNEL_VERSION(3, 11, 0) || defined(COMPAT_KERNEL_RELEASE)
		RTW_INFO("pwdev->sme_state(b)=%d\n", pwdev->sme_state);
		...
		if (pwdev->sme_state == CFG80211_SME_CONNECTING) {
			...
			RTW_INFO(FUNC_ADPT_FMT" call cfg80211_disconnected\n",
				 FUNC_ADPT_ARG(padapter));
			rtw_cfg80211_disconnected(pwdev, reason, NULL, 0, locally_generated,
						  GFP_ATOMIC);
		}
		...
		struct ieee_param *param)
{
		...
		if (param->u.crypt.set_tx == 0) { /* group key, TX only */
			if (strcmp(param->u.crypt.alg, "WEP") == 0) {
				...
				_rtw_memcpy(psecuritypriv->dot118021XGrpKey[param->u.crypt.idx].skey,
					    param->u.crypt.key, (param->u.crypt.key_len > 16 ? 16 :
								 param->u.crypt.key_len));
				/* set mic key */
				_rtw_memcpy(psecuritypriv->dot118021XGrptxmickey[param->u.crypt.idx].skey,
					    &(param->u.crypt.key[16]), 8);
				_rtw_memcpy(psecuritypriv->dot118021XGrprxmickey[param->u.crypt.idx].skey,
			...
			if (pbcmc_sta) {
				...
				pbcmc_sta->dot118021XPrivacy =
					psecuritypriv->dot118021XGrpPrivacy; /* rx will use bmc_sta's dot118021XPrivacy			 */
			}
			...
			rtw_ap_set_group_key(padapter, param->u.crypt.key,
					     psecuritypriv->dot118021XGrpPrivacy, param->u.crypt.idx);
		}
	...
	if (psecuritypriv->dot11AuthAlgrthm == dot11AuthAlgrthm_8021X
		...
		if (param->u.crypt.set_tx == 1) {
			/* pairwise key */
			_rtw_memcpy(psta->dot118021x_UncstKey.skey,  param->u.crypt.key,
			...
			if (strcmp(param->u.crypt.alg, "CCMP") == 0) {
				...
				_rtw_memcpy(psta->gtk.skey, param->u.crypt.key,
					    (param->u.crypt.key_len > 16 ? 16 : param->u.crypt.key_len));
				...
				RTW_INFO(FUNC_ADPT_FMT" set IGTK of "MAC_FMT", idx:%u, len:%u\n"
					 , FUNC_ADPT_ARG(padapter), MAC_ARG(psta->cmn.mac_addr)
					 , param->u.crypt.idx, param->u.crypt.key_len);
				_rtw_memcpy(psta->igtk.skey, param->u.crypt.key,
					    (param->u.crypt.key_len > 16 ? 16 : param->u.crypt.key_len));
				       ...
				       struct ieee_param *param)
{
	...
	if (padapter->securitypriv.dot11AuthAlgrthm ==
		...
		if (check_fwstate(pmlmepriv,
				  WIFI_STATION_STATE | WIFI_MP_STATE) == _TRUE) { /* sta mode */
#ifdef CONFIG_RTW_80211R
			if (rtw_ft_roam(padapter))
			...
			if (psta == NULL) {
				...
				if (param->u.crypt.set_tx == 1) { /* pairwise key */
					RTW_INFO(FUNC_ADPT_FMT" set %s PTK idx:%u, len:%u\n"
						 , FUNC_ADPT_ARG(padapter), param->u.crypt.alg, param->u.crypt.idx,
						 param->u.crypt.key_len);
					_rtw_memcpy(psta->dot118021x_UncstKey.skey,  param->u.crypt.key,
						    (param->u.crypt.key_len > 16 ? 16 : param->u.crypt.key_len));
...
#ifdef CONFIG_RTW_MESH
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 9, 0))
static enum nl80211_mesh_power_mode rtw_mesh_ps_to_nl80211_mesh_power_mode(
	u8 ps)
{
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 0, 0))
enum nl80211_plink_state rtw_plink_state_to_nl80211_plink_state(
	u8 plink_state)
{
...
u8 nl80211_plink_state_to_rtw_plink_state(enum nl80211_plink_state
		plink_state)
{
...
static void rtw_cfg80211_fill_mesh_only_sta_info(struct mesh_plink_ent
		*plink, struct sta_info *sta, struct station_info *sinfo)
{
	...
	sinfo->plink_state = rtw_plink_state_to_nl80211_plink_state(
				     plink->plink_state);
	if (!sta && plink->scanned) {
		sinfo->filled |= STATION_INFO_SIGNAL;
		sinfo->signal = translate_percentage_to_dbm(
					plink->scanned->network.PhyInfo.SignalStrength);
		sinfo->filled |= STATION_INFO_INACTIVE_TIME;
		if (plink->plink_state == RTW_MESH_PLINK_UNKNOWN)
			sinfo->inactive_time = 0 - 1;
		else
			sinfo->inactive_time = rtw_get_passing_time_ms(
						       plink->scanned->last_scanned);
	}
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 9, 0))
	if (sta) {
		...
		sinfo->nonpeer_pm = rtw_mesh_ps_to_nl80211_mesh_power_mode(
					    sta->nonpeer_mps);
	}
#endif
}
				    ...
				    struct station_info *sinfo)
{
	...
	if (check_fwstate(pmlmepriv, WIFI_STATION_STATE)
		...
		if (_rtw_memcmp((u8 *)mac, cur_network->network.MacAddress,
				ETH_ALEN) == _FALSE) {
...
#endif
		ndev->type =
			ARPHRD_IEEE80211_RADIOTAP; /* IEEE 802.11 + radiotap header : 803 */
		break;
	default:
		ret = -EOPNOTSUPP;
		goto exit;
...
void rtw_cfg80211_unlink_bss(_adapter *padapter,
			     struct wlan_network *pnetwork)
{
	struct wireless_dev *pwdev = padapter->rtw_wdev;
	struct wiphy *wiphy = pwdev->wiphy;
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 1, 0)
			       select_network.InfrastructureMode == Ndis802_11Infrastructure ?
			       IEEE80211_BSS_TYPE_ESS : IEEE80211_BSS_TYPE_IBSS,
...
int rtw_cfg80211_is_target_wps_scan(struct cfg80211_scan_request *scan_req,
...
static void _rtw_cfg80211_surveydone_event_callback(_adapter *padapter,
		struct cfg80211_scan_request *scan_req)
{
	...
	if (scan_req)
		...
		_enter_critical_bh(&pwdev_priv->scan_req_lock, &irqL);
		if (pwdev_priv->scan_request != NULL)
			target_wps_scan = rtw_cfg80211_is_target_wps_scan(pwdev_priv->scan_request,
					  &target_ssid);
		_exit_critical_bh(&pwdev_priv->scan_req_lock, &irqL);
	}
	...
	while (1) {
		...
		if (rtw_chset_search_ch(adapter_to_chset(padapter),
		    ...
		    && rtw_mlme_band_check(padapter,
					   pnetwork->network.Configuration.DSConfig) == _TRUE
		    && _TRUE == rtw_validate_ssid(&(pnetwork->network.Ssid))
		    && target_wps_scan)
			rtw_cfg80211_clear_wps_sr_of_non_target_bss(padapter, pnetwork,
					&target_ssid);
#if 0
...
static int rtw_cfg80211_set_probe_req_wpsp2pie(_adapter *padapter,
		char *buf, int len)
{
	int ret = 0;
	uint wps_ielen = 0;
	...
	if (len > 0) {
		...
		if (p2p_ie) {
			...
			if (rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_LISTEN_CH,
						     (u8 *)listen_ch_attr, (uint *) &attr_contentlen)
			    && attr_contentlen == 5) {
				if (wdinfo->listen_channel !=  listen_ch_attr[4]) {
					RTW_INFO(FUNC_ADPT_FMT" listen channel - country:%c%c%c, class:%u, ch:%u\n",
						 FUNC_ADPT_ARG(padapter), listen_ch_attr[0], listen_ch_attr[1],
						 listen_ch_attr[2],
		...
		if (wfd_ie) {
			...
			if (rtw_mlme_update_wfd_ie_data(pmlmepriv, MLME_PROBE_REQ_IE, wfd_ie,
							wfd_ielen) != _SUCCESS)
				return -EINVAL;
		}
#endif /* CONFIG_WFD */
...
u8 rtw_cfg80211_scan_via_buddy(_adapter *padapter,
...
void rtw_cfg80211_indicate_scan_done_for_buddy(_adapter *padapter,
		bool bscan_aborted)
{
	int i;
	u8 ret = 0;
	...
	for (i = 0; i < dvobj->iface_nums; i++) {
		...
		if ((iface) && rtw_is_adapter_up(iface)) {
			...
			if (wdev_priv->scan_request
			    && mlmepriv->scanning_via_buddy_intf == _TRUE) {
			     ...
			     , struct cfg80211_scan_request *request)
{
	...
	if (check_fwstate(pmlmepriv, WIFI_AP_STATE) == _TRUE) {
		...
		if (check_fwstate(pmlmepriv,
				  WIFI_UNDER_WPS | _FW_UNDER_SURVEY | _FW_UNDER_LINKING) == _TRUE) {
	...
	if (rtw_mi_buddy_check_fwstate(padapter,
				       _FW_UNDER_LINKING | WIFI_UNDER_WPS)) {
	...
	if (request->n_channels == 1) {
		for (i = 1; i < survey_times_for_one_ch; i++)
			_rtw_memcpy(&parm.ch[i], &parm.ch[0],
				    sizeof(struct rtw_ieee80211_channel));
		parm.ch_num = survey_times_for_one_ch;
	} else if (request->n_channels <= 4) {
		for (j = request->n_channels - 1; j >= 0; j--)
			for (i = 0; i < survey_times; i++)
				_rtw_memcpy(&parm.ch[j * survey_times + i], &parm.ch[j],
					    sizeof(struct rtw_ieee80211_channel));
...
static int rtw_cfg80211_set_wpa_version(struct security_priv
					*psecuritypriv, u32 wpa_version)
{
...
static int rtw_cfg80211_set_wpa_ie(_adapter *padapter, u8 *pie,
				   size_t ielen)
{
	...
	if (pwpa2 && wpa2_ielen > 0) {
		if (rtw_parse_wpa2_ie(pwpa2, wpa2_ielen + 2, &group_cipher,
				      &pairwise_cipher, NULL, &mfp_opt) == _SUCCESS) {
			padapter->securitypriv.dot11AuthAlgrthm = dot11AuthAlgrthm_8021X;
			padapter->securitypriv.ndisauthtype = Ndis802_11AuthModeWPA2PSK;
			_rtw_memcpy(padapter->securitypriv.supplicant_ie, &pwpa2[0],
				    wpa2_ielen + 2);
	...
	{/* handle wps_ie */
		...
		if (wps_ie && wps_ielen > 0) {
			...
			padapter->securitypriv.wps_ie_len = wps_ielen < MAX_WPS_IE_LEN ?
							    wps_ielen : MAX_WPS_IE_LEN;
			_rtw_memcpy(padapter->securitypriv.wps_ie, wps_ie,
...
#ifdef CONFIG_WFD
	{
		...
		if (wfd_ie) {
			...
			if (rtw_mlme_update_wfd_ie_data(pmlmepriv, MLME_ASSOC_REQ_IE, wfd_ie,
							wfd_ielen) != _SUCCESS)
				goto exit;
		}
	}
...
static int cfg80211_rtw_join_ibss(struct wiphy *wiphy,
				  struct net_device *ndev,
				  struct cfg80211_ibss_params *params)
{
	...
	rtw_mi_buddy_scan_abort(padapter,
				_TRUE); /* OR rtw_mi_scan_abort(padapter, _TRUE);*/
#endif /*CONFIG_CONCURRENT_MODE*/
				   ...
				   struct net_device *ndev)
{
	...
	if (check_fwstate(&padapter->mlmepriv, _FW_LINKED)) {
		...
		if (rtw_set_802_11_infrastructure_mode(padapter,
						       Ndis802_11Infrastructure) == _FALSE) {
			rtw_wdev->iftype = old_type;
			ret = -EPERM;
			goto leave_ibss;
		}
...
static int cfg80211_rtw_connect(struct wiphy *wiphy,
				struct net_device *ndev,
				struct cfg80211_connect_params *sme)
{
	...
	if (sme->crypto.n_ciphers_pairwise) {
		ret = rtw_cfg80211_set_cipher(psecuritypriv,
					      sme->crypto.ciphers_pairwise[0], _TRUE);
		if (ret < 0)
			goto cancel_ps_deny;
	}
	...
	pwdev_priv->connect_req = (struct cfg80211_connect_params *)rtw_malloc(
					  sizeof(*pwdev_priv->connect_req));
	if (pwdev_priv->connect_req)
		_rtw_memcpy(pwdev_priv->connect_req, sme,
			    sizeof(*pwdev_priv->connect_req));
	else
...
static int cfg80211_rtw_disconnect(struct wiphy *wiphy,
				   struct net_device *ndev,
				   u16 reason_code)
{
	_adapter *padapter = (_adapter *)rtw_netdev_priv(ndev);
	...
	/* if(check_fwstate(&padapter->mlmepriv, _FW_LINKED)) */
	{
		...
		rtw_indicate_disconnect(padapter, 0,
					wiphy->dev.power.is_prepared ? _FALSE : _TRUE);
				  ...
				  struct cfg80211_pmksa *pmksa)
{
	...
	for (index = 0 ; index < NUM_PMKID_CACHE; index++) {
		if (_rtw_memcmp(psecuritypriv->PMKIDList[index].Bssid, (u8 *)pmksa->bssid,
				ETH_ALEN) == _TRUE) {
			/* BSSID is matched, the same AP => rewrite with new PMKID. */
			RTW_INFO(FUNC_NDEV_FMT" BSSID exists in the PMKList.\n",
				 FUNC_NDEV_ARG(ndev));

			_rtw_memcpy(psecuritypriv->PMKIDList[index].PMKID, (u8 *)pmksa->pmkid,
				    WLAN_PMKID_LEN);
...
#ifdef CONFIG_AP_MODE
void rtw_cfg80211_indicate_sta_assoc(_adapter *padapter, u8 *pmgmt_frame,
				     uint frame_len)
{
	s32 freq;
	int channel;
...
void rtw_cfg80211_indicate_sta_disassoc(_adapter *padapter, const u8 *da,
		...
		struct net_device *ndev)
{
	...
	if ((frame_ctl & RTW_IEEE80211_FCTL_FTYPE) == RTW_IEEE80211_FTYPE_DATA) {
		 ...
		 */
		skb_pull(skb, dot11_hdr_len + qos_len + snap_len - sizeof(
				 src_mac_addr) * 2);
	...
	} else if ((frame_ctl & (RTW_IEEE80211_FCTL_FTYPE |
...
static void rtw_cfg80211_monitor_if_set_multicast_list(
	struct net_device *ndev)
{
	RTW_INFO("%s\n", __func__);
}
...
static int rtw_add_beacon(_adapter *adapter, const u8 *head,
			  size_t head_len, const u8 *tail, size_t tail_len)
{
	...
	_rtw_memcpy(pbuf, (void *)head + 24,
		    head_len - 24); /* 24=beacon header len. */
	_rtw_memcpy(pbuf + head_len - 24, (void *)tail, tail_len);

	len = head_len + tail_len - 24;

	/* check wps ie if inclued */
	if (rtw_get_wps_ie(pbuf + _FIXED_IE_LENGTH_, len - _FIXED_IE_LENGTH_, NULL,
			   &wps_ielen))
	...
	if (adapter->wdinfo.driver_interface == DRIVER_CFG80211) {
		/* check p2p if enable */
		if (rtw_get_p2p_ie(pbuf + _FIXED_IE_LENGTH_, len - _FIXED_IE_LENGTH_, NULL,
				   &p2p_ielen)) {
...
static int cfg80211_rtw_add_beacon(struct wiphy *wiphy,
				   struct net_device *ndev,
...
static int cfg80211_rtw_set_beacon(struct wiphy *wiphy,
				   struct net_device *ndev,
				   struct beacon_parameters *info)
{
	_adapter *adapter = (_adapter *)rtw_netdev_priv(ndev);
...
static int	cfg80211_rtw_del_beacon(struct wiphy *wiphy,
					struct net_device *ndev)
{
	_adapter *adapter = (_adapter *)rtw_netdev_priv(ndev);
...
#else
static int cfg80211_rtw_start_ap(struct wiphy *wiphy,
				 struct net_device *ndev,
				 struct cfg80211_ap_settings *settings)
{
	...
	if (settings->ssid && settings->ssid_len) {
		...
		WLAN_BSSID_EX *pbss_network_ext =
		...
		_rtw_memcpy(pbss_network->Ssid.Ssid, (void *)settings->ssid,
			    settings->ssid_len);
		pbss_network->Ssid.SsidLength = settings->ssid_len;
		_rtw_memcpy(pbss_network_ext->Ssid.Ssid, (void *)settings->ssid,
			    settings->ssid_len);
...
static int cfg80211_rtw_set_mac_acl(struct wiphy *wiphy,
				    struct net_device *ndev,
...
void dump_station_parameters(void *sel, struct wiphy *wiphy,
		...
		struct station_parameters *params)
{
	...
	if (MLME_IS_MESH(padapter)) {
		...
		if (!scanned
		    || rtw_get_passing_time_ms(scanned->last_scanned) >=
		    mcfg->peer_sel_policy.scanr_exp_ms
		   ) {
			if (!scanned)
		...
		if (!rtw_bss_is_candidate_mesh_peer(
			    &padapter->mlmepriv.cur_network.network, &scanned->network, 1, 1)) {
				   ...
				   )
{
	...
	if (check_fwstate(pmlmepriv,
			  (_FW_LINKED | WIFI_AP_STATE | WIFI_MESH_STATE)) != _TRUE) {
			...
			if (psta->dot8021xalg == 1 && psta->bpairwise_key_installed == _FALSE)
				...
				if (MLME_IS_AP(padapter))
					updated = ap_free_sta(padapter, psta, _TRUE,
							      WLAN_REASON_PREV_AUTH_NOT_VALID, _TRUE);
				else
		...
		struct station_parameters *params)
{
	...
	if (MLME_IS_MESH(adapter)) {
		...
		plink->plink_state = nl80211_plink_state_to_rtw_plink_state(
					     params->plink_state);
...
struct sta_info *rtw_sta_info_get_by_idx(struct sta_priv *pstapriv,
		...
		, struct ieee80211_channel *chan, enum nl80211_channel_type channel_type)
{
	...
	rtw_set_chbw_cmd(padapter, chan_target, chan_width, chan_offset,
			 RTW_CMDF_WAIT_ACK);
					   ...
					   )
{
	...
	rtw_set_chbw_cmd(padapter, target_channal, target_width, target_offset,
			 RTW_CMDF_WAIT_ACK);
...
void rtw_cfg80211_rx_probe_request(_adapter *adapter,
...
void rtw_cfg80211_rx_action_p2p(_adapter *adapter,
				union recv_frame *rframe)
{
	struct wireless_dev *wdev = adapter->rtw_wdev;
	u8 *frame = get_recvframe_data(rframe);
...
void rtw_cfg80211_rx_p2p_action_public(_adapter *adapter,
				       union recv_frame *rframe)
{
		...
		case P2P_GO_NEGO_CONF:
			if (0) {
				RTW_INFO(FUNC_ADPT_FMT" Nego confirm. state=%u, status=%u, iaddr="MAC_FMT"\n"
					 , FUNC_ADPT_ARG(adapter), pwdev_priv->nego_info.state,
					 pwdev_priv->nego_info.status
					 , MAC_ARG(pwdev_priv->nego_info.iface_addr));
			}
			if (pwdev_priv->nego_info.state == 2
			    && pwdev_priv->nego_info.status == 0
			    && rtw_check_invalid_mac_address(pwdev_priv->nego_info.iface_addr,
							     _FALSE) == _FALSE
			   ) {
				_adapter *intended_iface = dvobj_get_adapter_by_addr(dvobj,
							   pwdev_priv->nego_info.iface_addr);
...
void rtw_cfg80211_rx_action(_adapter *adapter, union recv_frame *rframe,
			    const char *msg)
{
	struct wireless_dev *wdev = adapter->rtw_wdev;
	struct rtw_wdev_priv *pwdev_priv = adapter_wdev_data(adapter);
...
void rtw_cfg80211_rx_rrm_action(_adapter *adapter,
				union recv_frame *rframe)
{
	struct wireless_dev *wdev = adapter->rtw_wdev;
	u8 *frame = get_recvframe_data(rframe);
...
void rtw_cfg80211_rx_mframe(_adapter *adapter, union recv_frame *rframe,
			    const char *msg)
{
	struct wireless_dev *wdev = adapter->rtw_wdev;
	struct rtw_wdev_priv *pwdev_priv = adapter_wdev_data(adapter);
...
#ifdef CONFIG_P2P
void rtw_cfg80211_issue_p2p_provision_request(_adapter *padapter,
		const u8 *buf, size_t len)
{
	...
	struct wifidirect_info *pwdinfo = &(padapter->wdinfo);
	u8 *frame_body = (unsigned char *)(buf + sizeof(struct
					   rtw_ieee80211_hdr_3addr));
	...
	_rtw_memcpy(pwdinfo->tx_prov_disc_info.peerIFAddr, GetAddr1Ptr(buf),
		    ETH_ALEN);
	_rtw_memcpy(pwdinfo->tx_prov_disc_info.peerDevAddr, GetAddr1Ptr(buf),
		    ETH_ALEN);

	pwdinfo->tx_prov_disc_info.wps_config_method_request = WPS_CM_PUSH_BUTTON;

	rtw_get_wps_ie(frame_body + _PUBLIC_ACTION_IE_OFFSET_,
		       frame_body_len - _PUBLIC_ACTION_IE_OFFSET_, wpsie, &wpsielen);
	...
	if (rtw_get_p2p_ie(frame_body + _PUBLIC_ACTION_IE_OFFSET_,
		...
		rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_DEVICE_INFO,
					 devinfo_content, &devinfo_contentlen);
	...
	*(fctrl) = 0;
	...
	_rtw_memcpy(pwlanhdr->addr1, pwdinfo->tx_prov_disc_info.peerDevAddr,
		    ETH_ALEN);
	_rtw_memcpy(pwlanhdr->addr2, adapter_mac_addr(padapter), ETH_ALEN);
	_rtw_memcpy(pwlanhdr->addr3, pwdinfo->tx_prov_disc_info.peerDevAddr,
		    ETH_ALEN);
	...
	*(u16 *)(wpsie + wpsielen) = cpu_to_be16(
	...
	if (wps_devicepassword_id == WPS_DPID_REGISTRAR_SPEC) {
		...
		rtw_msleep_os(
			15000); /* 15 sec for key in PIN CODE, workaround for GS2 before issuing Nego Req. */
	}
#endif
	...
	*cookie = ATOMIC_INC_RETURN(&pcfg80211_wdinfo->ro_ch_cookie_gen);
	...
	if (rtw_mi_check_status(padapter, MI_LINKED)) {
		if (is_p2p_find) /* p2p_find , duration<1000 */
			...
			duration = pwdinfo->ext_listen_interval + (pwdinfo->ext_listen_interval /
					4);
	}
	...
	_rtw_memcpy(&pcfg80211_wdinfo->remain_on_ch_channel, channel,
	...
	p2p_roch_cmd(padapter, *cookie, wdev, channel,
		     pcfg80211_wdinfo->remain_on_ch_type,
		     duration, RTW_CMDF_WAIT_ACK);

	rtw_cfg80211_ready_on_channel(wdev, *cookie, channel, channel_type,
				      duration, GFP_KERNEL);
exit:
	return err;
}
		...
		*/
		...
		if (wdev_data->scan_request
			...
			if (_rtw_memcmp(wdev_data->scan_request->ssids->ssid, "DIRECT-", 7)
			    && rtw_get_p2p_ie((u8 *)wdev_data->scan_request->ie,
					      wdev_data->scan_request->ie_len, NULL, NULL))
				is_p2p_scan = 1;
		}
...
#if defined(RTW_DEDICATED_P2P_DEVICE)
int rtw_pd_iface_alloc(struct wiphy *wiphy, const char *name,
		       struct wireless_dev **pd_wdev)
{
...
static int cfg80211_rtw_start_p2p_device(struct wiphy *wiphy,
		struct wireless_dev *wdev)
{
...
static void cfg80211_rtw_stop_p2p_device(struct wiphy *wiphy,
		struct wireless_dev *wdev)
{
	_adapter *adapter = wiphy_to_adapter(wiphy);
...
inline int rtw_cfg80211_redirect_pd_wdev(struct wiphy *wiphy, u8 *ra,
		struct wireless_dev **wdev)
{
...
static int _cfg80211_rtw_mgmt_tx(_adapter *padapter, u8 tx_ch, u8 no_cck,
				 const u8 *buf, size_t len, int wait_ack)
{
	struct xmit_frame	*pmgntframe;
	struct pkt_attrib	*pattrib;
	unsigned char	*pframe;
	...
	if (rtw_cfg80211_get_is_roch(padapter) == _TRUE) {
		...
		if (!check_fwstate(&padapter->mlmepriv, _FW_LINKED)) {
			...
			_set_timer(&padapter->cfg80211_wdinfo.remain_on_ch_timer,
				...
				u64 *cookie)
{
	...
	if (IS_CH_WAITING(rfctl)) {
#ifdef CONFIG_DFS_MASTER
		if (_rtw_rfctl_overlap_radar_detect_ch(rfctl, tx_ch, CHANNEL_WIDTH_20,
	...
	*cookie = pwdev_priv->mgmt_tx_cookie++;
	...
	frame_styp = le16_to_cpu(((struct rtw_ieee80211_hdr_3addr *)
				  buf)->frame_ctl) & IEEE80211_FCTL_STYPE;
	if (IEEE80211_STYPE_PROBE_RESP == frame_styp) {
#ifdef CONFIG_DEBUG_CFG80211
		RTW_INFO("RTW_Tx: probe_resp tx_ch=%d, no_cck=%u, da="MAC_FMT"\n", tx_ch,
			 no_cck, MAC_ARG(GetAddr1Ptr(buf)));
#endif /* CONFIG_DEBUG_CFG80211 */
		wait_ack = 0;
		goto dump;
	}
...
dump:
	...
	while (1) {
		...
		rtw_mi_scan_abort(padapter, _TRUE);
		tx_ret = rtw_mgnt_tx_cmd(padapter, tx_ch, no_cck, dump_buf, dump_len,
					 wait_ack, RTW_CMDF_WAIT_ACK);
		if (tx_ret == _SUCCESS
		    || (dump_cnt >= dump_limit
	...
	if (is_p2p) {
		...
		case P2P_GO_NEGO_CONF:
			if (0) {
				RTW_INFO(FUNC_ADPT_FMT" Nego confirm. state=%u, status=%u, iaddr="MAC_FMT"\n"
					 , FUNC_ADPT_ARG(padapter), pwdev_priv->nego_info.state,
					 pwdev_priv->nego_info.status
					 , MAC_ARG(pwdev_priv->nego_info.iface_addr));
			}
			if (pwdev_priv->nego_info.state == 2
...
#else
	if ((upd->interface_stypes & rtw_mask) == (pwdev_priv->mgmt_mask &
			rtw_mask))
		return;
				  ...
				  size_t len)
{
	...
	case TDLS_DISCOVERY_RESPONSE:
		issue_tdls_dis_rsp(padapter, &txmgmt,
				   pmlmeinfo->enc_algo ? _TRUE : _FALSE);
		break;
	}
...
int get_legacy_rates_str(struct wiphy *wiphy, enum nl80211_band band,
			 u32 mask, char *buf)
{
	int i;
	int cnt = 0;
...
void dump_mesh_setup(void *sel, struct wiphy *wiphy,
		     const struct mesh_setup *setup)
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 6, 0))
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 8, 0))
	struct cfg80211_chan_def *chdef = (struct cfg80211_chan_def *)(
			&setup->chandef);
#endif
	struct ieee80211_channel *chan;
#endif
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 8, 0))
	RTW_PRINT_SEL(sel,
		      "center_freq:%u, ch:%u, width:%s, cfreq1:%u, cfreq2:%u\n"
		      , chan->center_freq, chan->hw_value, nl80211_chan_width_str(chdef->width),
		      chdef->center_freq1, chdef->center_freq2);
#else
	RTW_PRINT_SEL(sel, "center_freq:%u, ch:%u, channel_type:%s\n"
		      , chan->center_freq, chan->hw_value,
		      nl80211_channel_type_str(setup->channel_type));
#endif
#endif
	...
	if (setup->mcast_rate[chan->band]) {
		RTW_PRINT_SEL(sel, "mcast_rate:%d.%d\n"
			      , wiphy->bands[chan->band]->bitrates[setup->mcast_rate[chan->band] -
					      1].bitrate / 10
			      , wiphy->bands[chan->band]->bitrates[setup->mcast_rate[chan->band] -
					      1].bitrate % 10
			     );
	...
	if (setup->beacon_rate.control[chan->band].legacy) {
		...
		get_legacy_rates_str(wiphy, chan->band,
				     setup->beacon_rate.control[chan->band].legacy, buf);
	...
	if (setup->beacon_rate.control[chan->band].vht_mcs[0]
		...
		for (i = 0; i < 4; i++) {/* parsing up to 4SS */
			...
			RTW_PRINT_SEL(sel, "beacon_rate.vht_mcs[%d]:%s\n", i
				      , mcs_mask == 0x00FF ? "0~7" : mcs_mask == 0x01FF ? "0~8" : mcs_mask ==
				      0x03FF ? "0~9" : "invalid");
		}
	}
	...
	if (setup->beacon_rate.control[chan->band].gi) {
		RTW_PRINT_SEL(sel, "beacon_rate.gi:%s\n"
			      , setup->beacon_rate.control[chan->band].gi == NL80211_TXRATE_FORCE_SGI ?
			      "SGI" :
			      setup->beacon_rate.control[chan->band].gi == NL80211_TXRATE_FORCE_LGI ?
			      "LGI" : "invalid"
			     );
	}
#endif
...
static void rtw_cfg80211_mesh_info_set_profile(struct rtw_mesh_info *minfo,
		const struct mesh_setup *setup)
{
...
static inline bool chk_mesh_attr(enum nl80211_meshconf_params parm,
				 u32 mask)
{
	return (mask >> (parm - 1)) & 0x1;
}

static void rtw_cfg80211_mesh_cfg_set(_adapter *adapter,
				      const struct mesh_config *conf, u32 mask)
{
	...
	if (chk_mesh_attr(NL80211_MESHCONF_HWMP_ACTIVE_PATH_TIMEOUT, mask))
		mcfg->dot11MeshHWMPactivePathTimeout =
			conf->dot11MeshHWMPactivePathTimeout;
	...
	if (chk_mesh_attr(NL80211_MESHCONF_HWMP_NET_DIAM_TRVS_TIME, mask))
		mcfg->dot11MeshHWMPnetDiameterTraversalTime =
			conf->dot11MeshHWMPnetDiameterTraversalTime;
	...
	if (chk_mesh_attr(NL80211_MESHCONF_HWMP_PATH_TO_ROOT_TIMEOUT, mask))
		mcfg->dot11MeshHWMPactivePathToRootTimeout =
			conf->dot11MeshHWMPactivePathToRootTimeout;
	...
	if (chk_mesh_attr(NL80211_MESHCONF_HWMP_CONFIRMATION_INTERVAL, mask))
		mcfg->dot11MeshHWMPconfirmationInterval =
			conf->dot11MeshHWMPconfirmationInterval;
#endif
...
u8 *rtw_cfg80211_construct_mesh_beacon_ies(struct wiphy *wiphy,
		_adapter *adapter
		...
		, uint *ies_len)
{
	...
	*ies_len = 0;
...
#else
	rtw_get_chbw_from_nl80211_channel_type(chan, setup->channel_type, &ht, &ch,
					       &bw, &offset);
#endif
	if (!ch)
		goto exit;
	...
	c = rtw_set_ie(c, WLAN_EID_SUPP_RATES, (n_bitrates >= 8 ? 8 : n_bitrates),
	...
	*(c + 1) = 4;
	...
	if (n_bitrates > 8)
		c = rtw_set_ie(c, WLAN_EID_EXT_SUPP_RATES, n_bitrates - 8,
			       supported_rates + 8, NULL);
...
static int cfg80211_rtw_get_mesh_config(struct wiphy *wiphy,
					...
					, struct mesh_config *conf)
{
	...
	conf->dot11MeshHWMPactivePathTimeout =
		mesh_cfg->dot11MeshHWMPactivePathTimeout;
	conf->dot11MeshHWMPpreqMinInterval =
		mesh_cfg->dot11MeshHWMPpreqMinInterval;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 3, 0))
	conf->dot11MeshHWMPperrMinInterval =
		mesh_cfg->dot11MeshHWMPperrMinInterval;
#endif
	conf->dot11MeshHWMPnetDiameterTraversalTime =
...
#endif
	conf->dot11MeshGateAnnouncementProtocol =
		mesh_cfg->dot11MeshGateAnnouncementProtocol;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 4, 0))
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 6, 0))
	conf->dot11MeshHWMPactivePathToRootTimeout =
		mesh_cfg->dot11MeshHWMPactivePathToRootTimeout;
	conf->dot11MeshHWMProotInterval = mesh_cfg->dot11MeshHWMProotInterval;
	conf->dot11MeshHWMPconfirmationInterval =
		mesh_cfg->dot11MeshHWMPconfirmationInterval;
#endif
...
static void rtw_mbss_info_change_notify(_adapter *adapter,
					bool minfo_changed, bool need_work)
{
	if (need_work)
		rtw_mesh_work(&adapter->mesh_work);
}

static int cfg80211_rtw_update_mesh_config(struct wiphy *wiphy,
		struct net_device *dev
...
static int cfg80211_rtw_join_mesh(struct wiphy *wiphy,
				  struct net_device *dev,
				  const struct mesh_config *conf, const struct mesh_setup *setup)
{
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);
	...
	ies = rtw_cfg80211_construct_mesh_beacon_ies(wiphy, adapter, conf, setup,
			&ies_len);
	if (!ies) {
		ret = -EINVAL;
		goto exit;
...
static int cfg80211_rtw_leave_mesh(struct wiphy *wiphy,
				   struct net_device *dev)
{
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);
	int ret = 0;
...
static int cfg80211_rtw_add_mpath(struct wiphy *wiphy,
				  struct net_device *dev
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 16, 0))
...
static int cfg80211_rtw_del_mpath(struct wiphy *wiphy,
				  struct net_device *dev
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 16, 0))
				  , const u8 *dst
#else
...
static int cfg80211_rtw_change_mpath(struct wiphy *wiphy,
				     struct net_device *dev
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 16, 0))
				     , const u8 *dst, const u8 *next_hop
#else
...
static void rtw_cfg80211_mpath_set_pinfo(struct rtw_mesh_path *mpath,
...
static int cfg80211_rtw_get_mpath(struct wiphy *wiphy,
				  struct net_device *dev, u8 *dst, u8 *next_hop, struct mpath_info *pinfo)
{
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);
	struct rtw_mesh_path *mpath;
	int ret = 0;
...
static int cfg80211_rtw_dump_mpath(struct wiphy *wiphy,
				   struct net_device *dev, int idx, u8 *dst, u8 *next_hop,
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 19, 0))
static void rtw_cfg80211_mpp_set_pinfo(struct rtw_mesh_path *mpath,
...
static int cfg80211_rtw_get_mpp(struct wiphy *wiphy,
				struct net_device *dev, u8 *dst, u8 *mpp, struct mpath_info *pinfo)
{
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);
	struct rtw_mesh_path *mpath;
	int ret = 0;
...
static int cfg80211_rtw_dump_mpp(struct wiphy *wiphy,
				 struct net_device *dev, int idx, u8 *dst, u8 *mpp,
				 struct mpath_info *pinfo)
{
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);
	struct rtw_mesh_path *mpath;
		...
		struct cfg80211_sched_scan_request *request)
{
	...
	ssids = (struct cfg80211_ssid *)rtw_zmalloc(n_ssids * sizeof(
...
int	cfg80211_rtw_resume(struct wiphy *wiphy)
{
	...
	if (pwrpriv->wowlan_last_wake_reason == RX_PNO) {
		...
		rtw_init_sitesurvey_parm(padapter, &parm);
		for (i = 0; i < pwrpriv->pnlo_info->ssid_num
		     && i < RTW_SSID_SCAN_AMOUNT; i++) {
		...
		for (PNOWakeupScanWaitCnt = 0; PNOWakeupScanWaitCnt < 10;
		     PNOWakeupScanWaitCnt++) {
...
static int rtw_cfg80211_set_beacon_wpsp2pie(struct net_device *ndev,
		char *buf, int len)
{
	int ret = 0;
	uint wps_ielen = 0;
	...
	if (len > 0) {
		...
		if (wfd_ie) {
			...
			if (rtw_mlme_update_wfd_ie_data(pmlmepriv, MLME_BEACON_IE, wfd_ie,
							wfd_ielen) != _SUCCESS)
				return -EINVAL;
		}
#endif /* CONFIG_WFD */
	...
	if (len > 0) {
		...
		if (wps_ie) {
			...
			puconfig_method = (u16 *)rtw_get_wps_attr_content(wps_ie, wps_ielen,
					  WPS_ATTR_CONF_METHOD, NULL, &attr_contentlen);
		...
		if (p2p_ie) {
			...
			if (rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_CAPABILITY,
						     (u8 *)&cap_attr, (uint *) &attr_contentlen)) {
				u8 grp_cap = 0;
		...
		if (wfd_ie) {
			...
			if (rtw_mlme_update_wfd_ie_data(pmlmepriv, MLME_PROBE_RESP_IE, wfd_ie,
							wfd_ielen) != _SUCCESS)
				return -EINVAL;
		}
#endif /* CONFIG_WFD */
		...
		char *buf, int len)
{
	...
	if (rtw_mlme_update_wfd_ie_data(pmlmepriv, MLME_ASSOC_RESP_IE, ie,
					ie_len) != _SUCCESS)
		return -EINVAL;
#endif
...
int rtw_cfg80211_set_mgnt_wpsp2pie(struct net_device *net, char *buf,
				   int len,
				   int type)
{
	int ret = 0;
...
#ifdef CONFIG_P2P
				if (ret == 0)
					adapter_wdev_data((_adapter *)rtw_netdev_priv(
								  net))->probe_resp_ie_update_time = rtw_get_current_time();
#endif
				break;
			case 0x4: /* ASSOC_RESP */
		...
		, struct ieee80211_sta_ht_cap *ht_cap, BAND_TYPE band, u8 rf_type)
{
	...
	if (TEST_FLAG(phtpriv->stbc_cap, STBC_HT_ENABLE_RX)) {
		...
		if (stbc_rx_enable) {
			...
			case RF_1T2R:
				ht_cap->cap |=
					IEEE80211_HT_CAP_RX_STBC_1R;/* Only one spatial-stream STBC RX is supported */
				break;
			case RF_3T3R:
			case RF_3T4R:
			case RF_4T4R:
	...
	ht_cap->mcs.rx_highest = cpu_to_le16(
					 rtw_mcs_rate(rf_type
							 ...
							 , hal_is_bw_support(padapter,
									 CHANNEL_WIDTH_40) ? ht_cap->cap &IEEE80211_HT_CAP_SGI_40 : ht_cap->cap &
...
static void rtw_cfg80211_preinit_wiphy(_adapter *adapter,
				       struct wiphy *wiphy)
{
	struct dvobj_priv *dvobj = adapter_to_dvobj(adapter);
	...
	wiphy->flags |=
		WIPHY_FLAG_TDLS_EXTERNAL_SETUP;	/* Driver handles key exchange */
...
static void rtw_cfg80211_set_survey_info_with_find_best_channel(
	struct wiphy *wiphy
	, struct net_device *netdev, int idx, struct survey_info *info)
{
	...
	s8 noise =
		-50;		/*channel noise in dBm. This and all following fields are optional */
	u64 time =
		100;		/*amount of time in ms the radio was turn on (on the channel)*/
...
static void rtw_cfg80211_set_survey_info_with_clm(PADAPTER padapter,
		int idx, struct survey_info *pinfo)
{
	s8 noise =
		-50;			/*channel noise in dBm. This and all following fields are optional */
	u64 time =
		SURVEY_TO;	/*amount of time in ms the radio was turn on (on the channel)*/
	u64 time_busy = 0;		/*amount of time the primary channel was sensed busy*/
	u8 chan = (u8)idx;
...
int rtw_hostapd_acs_dump_survey(struct wiphy *wiphy,
				struct net_device *netdev, int idx, struct survey_info *info)
{
...
#ifdef CONFIG_FIND_BEST_CHANNEL
	rtw_cfg80211_set_survey_info_with_find_best_channel(wiphy, netdev, idx,
			info);
