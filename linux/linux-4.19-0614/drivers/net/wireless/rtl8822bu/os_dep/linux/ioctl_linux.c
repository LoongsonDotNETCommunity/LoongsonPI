...
static int rtw_wx_priv_rrm(struct net_device *dev,
			   struct iw_request_info *a,
			   union iwreq_data *wrqu, char *b)
{
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
			...
			union iwreq_data *wrqu, char *extra)
{
	...
	case 0x7F:
		...
		case 0x09: {
			...
			RTW_INFO_DUMP("sta_dz_bitmap:", pstapriv->sta_dz_bitmap,
	...
	if (strcmp(tmp[0], "wmap") == 0) {
		...
		*extra = 0;
		...
		if (padapter->registrypriv.bFileMaskEfuse != _TRUE
			...
			sprintf(extra,
				"Not load eFuse mask file yet, Please use the efuse_mask CMD, now remove the interface !!!!\n");
