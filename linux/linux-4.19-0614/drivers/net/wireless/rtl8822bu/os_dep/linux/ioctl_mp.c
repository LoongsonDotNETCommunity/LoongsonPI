		...
		struct iw_point *wrqu, char *extra)
{
	...
	sprintf(extra, "Set data rate to %s index %d", input,
		     ...
		     struct iw_point *wrqu, char *extra)
{
	...
	/*RTW_INFO("%s: channel=%d\n", __func__, channel);*/
	_rtw_memset(extra, 0, wrqu->length);
	sprintf(extra, "Change prime channel offset %d to %d",
		   ...
		   struct iw_point *wrqu, char *extra)
{
	...
	if (MsetPower == 0) {
		...
		if (sscanf(input, "patha=%d,pathb=%d,pathc=%d,pathd=%d", &idx_a, &idx_b,
			   &idx_c, &idx_d) < 3)
			 ...
			 struct iw_point *wrqu, char *extra)
{
	...
	RTW_INFO("pattrib->dst:%x %x %x %x %x %x\n", pattrib->dst[0],
		 pattrib->dst[1], pattrib->dst[2], pattrib->dst[3], pattrib->dst[4],
		 pattrib->dst[5]);
	return 0;
}
...
{
	u32 pkTx = 1;
	int countPkTx = 1, cotuTx = 1, CarrSprTx = 1, scTx = 1, sgleTx = 1,
	    stop = 1;
	u32 bStartTest = 1;
	...
	*(extra + wrqu->length) = '\0';
	...
	if (!is_primary_adapter(padapter)) {
		sprintf(extra,
			"Error: MP mode can't support Virtual Adapter, Please to use main Adapter.\n");
		wrqu->length = strlen(extra);
		return 0;
	}
#endif
	       ...
	       struct iw_point *wrqu, char *extra)
{
...
#endif
	bStartRx = (strncmp(input, "start",
			    5) == 0) ? 1 : 0; /* strncmp TRUE is 0*/
	bStopRx = (strncmp(input, "stop", 5) == 0) ? 1 : 0; /* strncmp TRUE is 0*/
	bQueryPhy = (strncmp(input, "phy", 3) == 0) ? 1 : 0; /* strncmp TRUE is 0*/
	bQueryMac = (strncmp(input, "mac", 3) == 0) ? 1 : 0; /* strncmp TRUE is 0*/
	bSetBssid = (strncmp(input, "setbssid=",
			     8) == 0) ? 1 : 0; /* strncmp TRUE is 0*/
	...
	pmppriv->bloopback = (strncmp(input, "loopbk",
				      6) == 0) ? 1 : 0; /* strncmp TRUE is 0*/
		     ...
		     struct iw_point *wrqu, char *extra)
{
	...
	sprintf(extra,
		...
		txfail, rxok, rxfail, rxfilterout);
	      ...
	      union iwreq_data *wrqu, char *extra)
{
	...
	char *pextra = extra;
	u32 bandwidth = 0, sg = 0, channel = 6, txpower = 40, rate = 108, ant = 0,
	    txmode = 1, count = 0;
	...
	if (strncmp(extra, "stop", 3) == 0) {
		...
		if (strncmp(extra, "pmact=", 6) == 0) {
			...
			pMptCtx->PMacTxInfo.NDP_sound =
				FALSE;/*(Adapter.PacketType == NDP_PKT)?TRUE:FALSE;*/
			...
			if (MPT_IS_CCK_RATE(pMptCtx->PMacTxInfo.TX_RATE))
				...
				/* 24 BIT*/
				L_SIG_generator(pMptCtx->PMacPktInfo.N_sym, &pMptCtx->PMacTxInfo,
						&pMptCtx->PMacPktInfo);
			}
			/*	48BIT*/
		...
		if (sscanf(extra, "ch=%d,bw=%d,rate=%d,pwr=%d,ant=%d,tx=%d", &channel,
			   &bandwidth, &rate, &txpower, &ant, &txmode) < 6) {
			...
			RTW_INFO("Got format [ch=%d,bw=%d,rate=%d,pwr=%d,ant=%d,tx=%d]\n", channel,
				 bandwidth, rate, txpower, ant, txmode);
			_rtw_memset(extra, 0, wrqu->data.length);
			sprintf(extra, "Change Current channel %d to channel %d",
				padapter->mppriv.channel, channel);
			padapter->mppriv.channel = channel;
			SetChannel(padapter);
			pHalData->current_channel = channel;
			...
			pextra += sprintf(pextra, "\nChange Current Bandwidth %d to Bandwidth %d",
					  padapter->mppriv.bandwidth, bandwidth);
			padapter->mppriv.bandwidth = (u8)bandwidth;
			padapter->mppriv.preamble = sg;
			SetBandwidth(padapter);
			pHalData->current_channel_bw = bandwidth;
	      ...
	      union iwreq_data *wrqu, char *extra)
{
	...
	} else if (sscanf(extra, "ch=%d,bw=%d,ant=%d", &channel, &bandwidth,
			  &ant) < 3) {
		...
		union iwreq_data *wrqu, char *extra)
{
	...
	_rtw_memcpy((void *)&pMptCtx->PMacTxInfo, (void *)input,
		    sizeof(RT_PMAC_TX_INFO));
			...
			union iwreq_data *wrqu, char *extra)
{
	...
	*(extra + wrqu->data.length) = '\0';
	...
	if (rtw_is_file_readable(rtw_efuse_mask_file_path) == _TRUE) {
		...
		Status = rtw_efuse_file_read(padapter, rtw_efuse_mask_file_path,
					     maskfileBuffer, sizeof(maskfileBuffer));
		if (Status == _TRUE) {
			padapter->registrypriv.bFileMaskEfuse = _TRUE;
		       ...
		       union iwreq_data *wrqu, char *extra)
{
	...
	if (rtw_is_file_readable(rtw_efuse_file_map_path) == _TRUE) {
		RTW_INFO("%s do rtw_efuse_mask_file_read = %s!\n", __func__,
			 rtw_efuse_file_map_path);
		Status = rtw_efuse_file_read(padapter, rtw_efuse_file_map_path,
			  ...
			  union iwreq_data *wrqu, char *extra)
{
	...
	if (rtw_is_file_readable(rtw_efuse_file_map_path) == _TRUE) {
		...
		Status = rtw_efuse_file_read(padapter, rtw_efuse_file_map_path,
					     ...
					     sizeof(pEfuseHal->fakeBTEfuseModifiedMap));
		if (Status == _TRUE) {
		 ...
		 union iwreq_data *wrqu, char *extra)
{
	...
	int testmode = 1, ready = 1, trxparam = 1, setgen = 1, getgen = 1,
	...
	*(extra + wrqu->data.length) = '\0';
	...
	if (strncmp(extra, "dlbt", 4) == 0) {
		...
		RTW_INFO("FirmwareDownloadBT ready = 0x%x 0x%x", pMptCtx->mptOutBuf[4],
			 pMptCtx->mptOutBuf[5]);
