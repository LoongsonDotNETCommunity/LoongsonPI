...
extern void process_wmmps_data(_adapter *padapter,
...
{
	u8 tdls_ch_swithcing_prohibited_bit =
		0x80; /* bit(39); TDLS_channel_switching prohibited */
...
int _issue_nulldata_to_TDLS_peer_STA(_adapter *padapter, unsigned char *da,
 ...
 */
int issue_nulldata_to_TDLS_peer_STA(_adapter *padapter, unsigned char *da,
...
void rtw_tdls_process_ht_cap(_adapter *padapter,
			     struct sta_info *ptdls_sta, u8 *data, u8 Length)
{
	struct mlme_ext_priv	*pmlmeext = &padapter->mlmeextpriv;
	struct mlme_ext_info	*pmlmeinfo = &(pmlmeext->mlmext_info);
	...
	_rtw_memset(&ptdls_sta->htpriv.ht_cap, 0,
		    sizeof(struct rtw_ieee80211_ht_cap));
	if (data && Length >= sizeof(struct rtw_ieee80211_ht_cap)) {
		ptdls_sta->flags |= WLAN_STA_HT;
		ptdls_sta->flags |= WLAN_STA_WME;

		_rtw_memcpy(&ptdls_sta->htpriv.ht_cap, data,
			    sizeof(struct rtw_ieee80211_ht_cap));
	} else {
		ptdls_sta->flags &= ~WLAN_STA_HT;
		return;
	...
	if (ptdls_sta->htpriv.ht_option) {
		...
		if ((phtpriv->sgi_20m == _TRUE)
		    && (ptdls_sta->htpriv.ht_cap.cap_info & cpu_to_le16(
				IEEE80211_HT_CAP_SGI_20)))
			ptdls_sta->htpriv.sgi_20m = _TRUE;

		/* Check if sta support s Short GI 40M */
		if ((phtpriv->sgi_40m == _TRUE)
		    && (ptdls_sta->htpriv.ht_cap.cap_info & cpu_to_le16(
				IEEE80211_HT_CAP_SGI_40)))
			ptdls_sta->htpriv.sgi_40m = _TRUE;

		/* Bwmode would still followed AP's setting */
		if (ptdls_sta->htpriv.ht_cap.cap_info & cpu_to_le16(
...
u8 *rtw_tdls_set_ht_cap(_adapter *padapter, u8 *pframe,
			struct pkt_attrib *pattrib)
{
	...
	rtw_restructure_ht_ie(padapter, NULL, pframe, 0, &(pattrib->pktlen),
...
void rtw_tdls_process_vht_cap(_adapter *padapter,
			      struct sta_info *ptdls_sta, u8 *data, u8 Length)
{
	...
	if (data && Length == 12) {
		...
		if (elems.vht_op_mode_notify && elems.vht_op_mode_notify_len == 1)
			_rtw_memcpy(&pstat->vhtpriv.vht_op_mode_notify, elems.vht_op_mode_notify,
				    1);
	...
	ptdls_sta->vhtpriv.sgi_80m = (GET_VHT_CAPABILITY_ELE_SHORT_GI80M(
					      data) & pvhtpriv->sgi_80m) ? _TRUE : _FALSE;
	...
	ptdls_sta->vhtpriv.ampdu_len = GET_VHT_CAPABILITY_ELE_MAX_RXAMPDU_FACTOR(
					       data);
	...
	ptdls_sta->vhtpriv.vht_highest_rate = rtw_get_vht_highest_rate(
			ptdls_sta->vhtpriv.vht_mcs_map);
}

void rtw_tdls_process_vht_operation(_adapter *padapter,
				    struct sta_info *ptdls_sta, u8 *data, u8 Length)
{
	struct mlme_priv		*pmlmepriv = &padapter->mlmepriv;
	struct mlme_ext_priv	*pmlmeext = &padapter->mlmeextpriv;
	...
	if (GET_VHT_OPERATION_ELE_CHL_WIDTH(data) >= 1) {
		...
		if (hal_is_bw_support(padapter, operation_bw)
		    && REGSTY_IS_BW_5G_SUPPORT(regsty, operation_bw)
...
void rtw_tdls_process_vht_op_mode_notify(_adapter *padapter,
		struct sta_info *ptdls_sta, u8 *data, u8 Length)
{
	struct mlme_priv		*pmlmepriv = &padapter->mlmepriv;
	struct vht_priv		*pvhtpriv = &pmlmepriv->vhtpriv;
	...
	if (target_rxss != current_rxss) {
		...
		rtw_vht_nss_to_mcsmap(target_rxss, vht_mcs_map,
				      ptdls_sta->vhtpriv.vht_mcs_map);
		_rtw_memcpy(ptdls_sta->vhtpriv.vht_mcs_map, vht_mcs_map, 2);
	}
}

u8 *rtw_tdls_set_aid(_adapter *padapter, u8 *pframe,
		     struct pkt_attrib *pattrib)
{
	return rtw_set_ie(pframe, EID_AID, 2,
			  (u8 *) & (padapter->mlmepriv.cur_network.aid), &(pattrib->pktlen));
}

u8 *rtw_tdls_set_vht_cap(_adapter *padapter, u8 *pframe,
			 struct pkt_attrib *pattrib)
{
	u32 ie_len = 0;
...
u8 *rtw_tdls_set_vht_operation(_adapter *padapter, u8 *pframe,
			       struct pkt_attrib *pattrib, u8 channel)
{
	u32 ie_len = 0;
...
u8 *rtw_tdls_set_vht_op_mode_notify(_adapter *padapter, u8 *pframe,
				    struct pkt_attrib *pattrib, u8 bw)
{
	u32 ie_len = 0;
...
u8 *rtw_tdls_set_sup_ch(_adapter *adapter, u8 *pframe,
			struct pkt_attrib *pattrib)
{
	struct rf_ctl_t *rfctl = adapter_to_rfctl(adapter);
	u8 sup_ch[30 * 2] = {0x00}, ch_set_idx = 0, sup_ch_idx = 2;

	while (ch_set_idx < rfctl->max_chan_nums
	       && rfctl->channel_set[ch_set_idx].ChannelNum != 0) {
		if (rfctl->channel_set[ch_set_idx].ChannelNum <= 14) {
			...
			sup_ch[1] =
				rfctl->channel_set[ch_set_idx].ChannelNum;	/* Number of channel */
		} else {
	...
	return rtw_set_ie(pframe, _SUPPORTED_CH_IE_, sup_ch_idx, sup_ch,
			  &(pattrib->pktlen));
}

u8 *rtw_tdls_set_rsnie(struct tdls_txmgmt *ptxmgmt, u8 *pframe,
		       struct pkt_attrib *pattrib,  int init, struct sta_info *ptdls_sta)
{
	u8 *p = NULL;
	int len = 0;
	...
	if (p != NULL)
		...
		return rtw_set_ie(pframe, _RSN_IE_2_, sizeof(ptdls_sta->TDLS_RSNIE),
				  ptdls_sta->TDLS_RSNIE, &(pattrib->pktlen));
}

u8 *rtw_tdls_set_ext_cap(u8 *pframe, struct pkt_attrib *pattrib)
{
	return rtw_set_ie(pframe, _EXT_CAP_IE_, sizeof(TDLS_EXT_CAPIE),
			  TDLS_EXT_CAPIE, &(pattrib->pktlen));
}

u8 *rtw_tdls_set_qos_cap(u8 *pframe, struct pkt_attrib *pattrib)
{
	return rtw_set_ie(pframe, _VENDOR_SPECIFIC_IE_, sizeof(TDLS_WMMIE),
			  TDLS_WMMIE,  &(pattrib->pktlen));
}

u8 *rtw_tdls_set_ftie(struct tdls_txmgmt *ptxmgmt, u8 *pframe,
		      struct pkt_attrib *pattrib, u8 *ANonce, u8 *SNonce)
{
...
u8 *rtw_tdls_set_timeout_interval(struct tdls_txmgmt *ptxmgmt, u8 *pframe,
				  struct pkt_attrib *pattrib, int init, struct sta_info *ptdls_sta)
{
	...
	if (p != NULL)
		return rtw_set_ie(pframe, _TIMEOUT_ITVL_IE_, len, p + 2,
				  &(pattrib->pktlen));
	else {
		/* Timeout interval */
		timeout_itvl[0] = 0x02;
		if (init == _TRUE)
			...
			_rtw_memcpy(timeout_itvl + 1, (u8 *)(&ptdls_sta->TDLS_PeerKey_Lifetime),
				    4);

		return rtw_set_ie(pframe, _TIMEOUT_ITVL_IE_, 5, timeout_itvl,
				  &(pattrib->pktlen));
	}
}

u8 *rtw_tdls_set_bss_coexist(_adapter *padapter, u8 *pframe,
			     struct pkt_attrib *pattrib)
{
	u8 iedata = 0;
	...
	iedata |= BIT(0);
	return rtw_set_ie(pframe, EID_BSSCoexistence, 1, &iedata,
			  &(pattrib->pktlen));
}
...
u8 *rtw_tdls_set_category(u8 *pframe, struct pkt_attrib *pattrib,
			  u8 category)
{
	return rtw_set_fixed_ie(pframe, 1, &(category), &(pattrib->pktlen));
}

u8 *rtw_tdls_set_action(u8 *pframe, struct pkt_attrib *pattrib,
			struct tdls_txmgmt *ptxmgmt)
{
	return rtw_set_fixed_ie(pframe, 1, &(ptxmgmt->action_code),
				&(pattrib->pktlen));
}

u8 *rtw_tdls_set_status_code(u8 *pframe, struct pkt_attrib *pattrib,
			     struct tdls_txmgmt *ptxmgmt)
{
	return rtw_set_fixed_ie(pframe, 2, (u8 *) & (ptxmgmt->status_code),
				&(pattrib->pktlen));
}

u8 *rtw_tdls_set_dialog(u8 *pframe, struct pkt_attrib *pattrib,
			struct tdls_txmgmt *ptxmgmt)
{
	u8 dialogtoken = 1;
	if (ptxmgmt->dialog_token)
		return rtw_set_fixed_ie(pframe, 1, &(ptxmgmt->dialog_token),
					&(pattrib->pktlen));
	else
		return rtw_set_fixed_ie(pframe, 1, &(dialogtoken), &(pattrib->pktlen));
}

u8 *rtw_tdls_set_reg_class(u8 *pframe, struct pkt_attrib *pattrib,
...
u8 *rtw_tdls_set_second_channel_offset(u8 *pframe,
				       struct pkt_attrib *pattrib, u8 ch_offset)
{
	return rtw_set_ie(pframe, EID_SecondaryChnlOffset, 1, &ch_offset,
			  &(pattrib->pktlen));
}

u8 *rtw_tdls_set_capability(_adapter *padapter, u8 *pframe,
			    struct pkt_attrib *pattrib)
{
	struct mlme_ext_priv	*pmlmeext = &padapter->mlmeextpriv;
	struct mlme_ext_info	*pmlmeinfo = &pmlmeext->mlmext_info;
	...
	_rtw_memcpy(cap_from_ie,
		    rtw_get_capability_from_ie(pmlmeinfo->network.IEs), 2);

	return rtw_set_fixed_ie(pframe, 2, cap_from_ie, &(pattrib->pktlen));
}

u8 *rtw_tdls_set_supported_rate(_adapter *padapter, u8 *pframe,
				struct pkt_attrib *pattrib)
{
	...
	rtw_set_supported_rate(bssrate,
			       (padapter->registrypriv.wireless_mode == WIRELESS_MODE_MAX) ?
			       padapter->mlmeextpriv.cur_wireless_mode :
...
{
	return rtw_set_ie(pframe, _SRC_IE_, sizeof(TDLS_SRC), TDLS_SRC,
			  &(pattrib->pktlen));
}

u8 *rtw_tdls_set_linkid(_adapter *padapter, u8 *pframe,
			struct pkt_attrib *pattrib, u8 init)
{
	struct mlme_ext_priv	*pmlmeext = &(padapter->mlmeextpriv);
	struct mlme_ext_info	*pmlmeinfo = &(pmlmeext->mlmext_info);
	...
	return rtw_set_ie(pframe, _LINK_ID_IE_, 18, link_id_addr,
			  &(pattrib->pktlen));
}

#ifdef CONFIG_TDLS_CH_SW
u8 *rtw_tdls_set_target_ch(_adapter *padapter, u8 *pframe,
			   struct pkt_attrib *pattrib)
{
	u8 target_ch = 1;
	if (padapter->tdlsinfo.chsw_info.off_ch_num)
		return rtw_set_fixed_ie(pframe, 1,
					&(padapter->tdlsinfo.chsw_info.off_ch_num), &(pattrib->pktlen));
	else
		return rtw_set_fixed_ie(pframe, 1, &(target_ch), &(pattrib->pktlen));
}

u8 *rtw_tdls_set_ch_sw(u8 *pframe, struct pkt_attrib *pattrib,
		       struct sta_info *ptdls_sta)
{
	...
	u16 switch_timeout = (ptdls_sta->ch_switch_timeout >=
			      TDLS_CH_SWITCH_TIMEOUT * 1000) ?
	...
	return rtw_set_ie(pframe, _CH_SWITCH_TIMING_,  4, ch_switch_timing,
			  &(pattrib->pktlen));
}
...
void rtw_tdls_ch_sw_back_to_base_chnl(_adapter *padapter)
{
	...
	if ((ATOMIC_READ(&pchsw_info->chsw_on) == _TRUE) &&
		...
		rtw_tdls_cmd(padapter, pchsw_info->addr,
...
s32 rtw_tdls_do_ch_sw(_adapter *padapter, struct sta_info *ptdls_sta,
		      u8 chnl_type, u8 channel, u8 channel_offset, u16 bwmode,
		      u16 ch_switch_time)
{
	...
	if (rtw_hal_ch_sw_oper_offload(padapter, channel, channel_offset,
				       bwmode) == _SUCCESS) {
	...
	if (ret == _SUCCESS) {
		ch_sw_time_spent = rtw_systime_to_ms(rtw_get_current_time()) -
				   ch_sw_time_start;
...
u8 *rtw_tdls_set_wmm_params(_adapter *padapter, u8 *pframe,
			    struct pkt_attrib *pattrib)
{
	struct mlme_ext_priv	*pmlmeext = &(padapter->mlmeextpriv);
	struct mlme_ext_info	*pmlmeinfo = &(pmlmeext->mlmext_info);
	...
	if (&pmlmeinfo->WMM_param) {
		...
		if (_rtw_memcmp(&pmlmeinfo->WMM_param, &wmm_param_ele[6], 18) == _TRUE)
			...
			_rtw_memcpy(wmm_param_ele + 6, (u8 *)&TDLS_WMM_PARAM_IE,
				    sizeof(TDLS_WMM_PARAM_IE));
		else
			_rtw_memcpy(wmm_param_ele + 6, (u8 *)&pmlmeinfo->WMM_param,
				    sizeof(pmlmeinfo->WMM_param));
		return rtw_set_ie(pframe, _VENDOR_SPECIFIC_IE_,  24, wmm_param_ele,
				  &(pattrib->pktlen));
	} else
		return pframe;
}
...
#ifdef CONFIG_WFD
void rtw_tdls_process_wfd_ie(struct tdls_info *ptdlsinfo, u8 *ptr,
			     u8 length)
{
	u8 *wfd_ie;
	u32	wfd_ielen = 0;
	...
	while (wfd_ie) {
		...
		attr_content = rtw_get_wfd_attr_content(wfd_ie, wfd_ielen,
							WFD_ATTR_DEVICE_INFO, NULL, &attr_contentlen);
		if (attr_content && attr_contentlen) {
			ptdlsinfo->wfd_info->peer_rtsp_ctrlport = RTW_GET_BE16(attr_content + 2);
			RTW_INFO("[%s] Peer PORT NUM = %d\n", __FUNCTION__,
				 ptdlsinfo->wfd_info->peer_rtsp_ctrlport);
		}

		attr_content = rtw_get_wfd_attr_content(wfd_ie, wfd_ielen,
							WFD_ATTR_LOCAL_IP_ADDR, NULL, &attr_contentlen);
		if (attr_content && attr_contentlen) {
		...
		wfd_ie = rtw_get_wfd_ie(wfd_ie + wfd_ielen,
					(ptr + length) - (wfd_ie + wfd_ielen), NULL, &wfd_ielen);
	}
}
...
int issue_tunneled_probe_rsp(_adapter *padapter,
			     union recv_frame *precv_frame)
{
	struct xmit_frame			*pmgntframe;
	struct pkt_attrib			*pattrib;
...
int issue_tdls_setup_req(_adapter *padapter, struct tdls_txmgmt *ptxmgmt,
			 int wait_ack)
{
	struct tdls_info	*ptdlsinfo = &padapter->tdlsinfo;
	struct xmit_frame			*pmgntframe;
...
int _issue_tdls_teardown(_adapter *padapter, struct tdls_txmgmt *ptxmgmt,
			 struct sta_info *ptdls_sta, u8 wait_ack)
{
	struct xmit_frame			*pmgntframe;
	struct pkt_attrib			*pattrib;
	struct mlme_priv	*pmlmepriv = &padapter->mlmepriv;
...
int issue_tdls_teardown(_adapter *padapter, struct tdls_txmgmt *ptxmgmt,
			u8 wait_ack)
{
	struct sta_info *ptdls_sta = NULL;
	int ret = _FAIL;
...
int issue_tdls_dis_rsp(_adapter *padapter, struct tdls_txmgmt *ptxmgmt,
		       u8 privacy)
{
	struct xmit_frame		*pmgntframe;
	struct pkt_attrib		*pattrib;
...
int issue_tdls_peer_traffic_rsp(_adapter *padapter,
				struct sta_info *ptdls_sta, struct tdls_txmgmt *ptxmgmt)
{
	struct xmit_frame	*pmgntframe;
	struct pkt_attrib	*pattrib;
	struct mlme_priv	*pmlmepriv = &padapter->mlmepriv;
...
int issue_tdls_peer_traffic_indication(_adapter *padapter,
				       struct sta_info *ptdls_sta)
{
	struct xmit_frame			*pmgntframe;
	struct pkt_attrib			*pattrib;
...
#ifdef CONFIG_TDLS_CH_SW
int issue_tdls_ch_switch_req(_adapter *padapter,
			     struct sta_info *ptdls_sta)
{
	struct xmit_frame	*pmgntframe;
	struct pkt_attrib	*pattrib;
...
int issue_tdls_ch_switch_rsp(_adapter *padapter,
			     struct tdls_txmgmt *ptxmgmt, int wait_ack)
{
	struct xmit_frame	*pmgntframe;
	struct pkt_attrib	*pattrib;
	struct mlme_priv	*pmlmepriv = &padapter->mlmepriv;
		       ...
		       struct sta_info *ptdls_sta)
{
	...
	ptr += prx_pkt_attrib->hdrlen + prx_pkt_attrib->iv_len + LLC_HEADER_SIZE +
	       ETH_TYPE_LEN + PAYLOAD_TYPE_LEN;
	...
	if (ptdls_sta) {
			...
			case _RSN_IE_2_:
				...
				if (prx_pkt_attrib->encrypt) {
					...
					for (k = 0; k < pairwise_count; k++) {
						if (_rtw_memcmp(ppairwise_cipher + 4 * k, RSN_CIPHER_SUITE_CCMP,
								4) == _TRUE)
							ccmp_included = 1;
					}
		...
		if (txmgmt.status_code == _STATS_SUCCESSFUL_) {
			...
			if (padapter->wdinfo.wfd_tdls_enable && (rsnie_included
					&& prx_pkt_attrib->encrypt != _AES_))
				txmgmt.status_code = _STATS_SEC_DISABLED_;
#endif /* CONFIG_WFD */
		}
		      ...
		      struct sta_info *ptdls_sta)
{
	...
	u8  *pftie = NULL, *ptimeout_ie = NULL, *plinkid_ie = NULL, *prsnie = NULL,
	     *pftie_mic = NULL, *ppairwise_cipher = NULL;
	...
	for (j = FIXED_IE; j < parsing_length;) {
		...
		case EID_OpModeNotification:
			rtw_tdls_process_vht_op_mode_notify(padapter, ptdls_sta, pIE->data,
							    pIE->Length);
			break;
#endif
		case EID_BSSCoexistence:
	...
	if (prx_pkt_attrib->encrypt) {
		if (verify_ccmp == 1) {
			...
			if (rtw_tdls_is_driver_setup(padapter) == _TRUE) {
				...
				if (tdls_verify_mic(ptdls_sta->tpk.kck, 2, plinkid_ie, prsnie, ptimeout_ie,
						    pftie) == _FAIL) {
		      ...
		      struct sta_info *ptdls_sta)
{
	...
	for (j = FIXED_IE; j < parsing_length;) {
		...
		case EID_VHTOperation:
			rtw_tdls_process_vht_operation(padapter, ptdls_sta, pIE->data,
						       pIE->Length);
			break;
		case EID_OpModeNotification:
			rtw_tdls_process_vht_op_mode_notify(padapter, ptdls_sta, pIE->data,
	...
	if (prx_pkt_attrib->encrypt) {
		...
		if (rtw_tdls_is_driver_setup(padapter) &&
		    (tdls_verify_mic(ptdls_sta->tpk.kck, 3, plinkid_ie, prsnie, ptimeout_ie,
				     pftie) == _FAIL)) {
			rtw_tdls_teardown_pre_hdl(padapter, ptdls_sta);
			rtw_tdls_cmd(padapter, ptdls_sta->cmn.mac_addr,
				     TDLS_TEARDOWN_STA_LOCALLY_POST);
	...
	for (j = FIXED_IE; j < parsing_length;) {
		...
		case _LINK_ID_IE_:
			...
			if (MacAddr_isBcst(dst) == _FALSE
			    && (_rtw_memcmp(adapter_mac_addr(padapter), dst, 6) == _FALSE))
				goto exit;
			break;
		default:
			break;
		     ...
		     struct sta_info *ptdls_sta)
{
	...
	reason = *(ptr + prx_pkt_attrib->hdrlen + prx_pkt_attrib->iv_len +
		   LLC_HEADER_SIZE + ETH_TYPE_LEN + PAYLOAD_TYPE_LEN + 2);
...
int On_TDLS_Peer_Traffic_Indication(_adapter *padapter,
				    union recv_frame *precv_frame, struct sta_info *ptdls_sta)
{
	struct rx_pkt_attrib	*pattrib = &precv_frame->u.hdr.attrib;
	u8 *ptr = precv_frame->u.hdr.rx_data;
	struct tdls_txmgmt txmgmt;

	ptr += pattrib->hdrlen + pattrib->iv_len + LLC_HEADER_SIZE + ETH_TYPE_LEN +
	       PAYLOAD_TYPE_LEN;
	_rtw_memset(&txmgmt, 0x00, sizeof(struct tdls_txmgmt));
...
int On_TDLS_Peer_Traffic_Rsp(_adapter *padapter,
			     union recv_frame *precv_frame, struct sta_info *ptdls_sta)
{
	struct tdls_info *ptdlsinfo = &padapter->tdlsinfo;
	struct mlme_ext_priv *pmlmeext = &padapter->mlmeextpriv;
	...
	if (ptdls_sta->uapsd_vo || ptdls_sta->uapsd_vi || ptdls_sta->uapsd_be
	    || ptdls_sta->uapsd_bk)
		wmmps_ac = 1;
...
#ifdef CONFIG_TDLS_CH_SW
sint On_TDLS_Ch_Switch_Req(_adapter *padapter,
			   union recv_frame *precv_frame, struct sta_info *ptdls_sta)
{
	struct tdls_ch_switch *pchsw_info = &padapter->tdlsinfo.chsw_info;
	struct sta_priv *pstapriv = &padapter->stapriv;
	...
	u16 switch_time = TDLS_CH_SWITCH_TIME * 1000,
	    switch_timeout = TDLS_CH_SWITCH_TIMEOUT * 1000;
	u8 take_care_iqk;
	...
	for (j = FIXED_IE; j < parsing_length;) {
		...
		case _CH_SWITCH_TIMING_:
			ptdls_sta->ch_switch_time = (RTW_GET_LE16(pIE->data) >= TDLS_CH_SWITCH_TIME
						     * 1000) ?
						    RTW_GET_LE16(pIE->data) : TDLS_CH_SWITCH_TIME * 1000;
			ptdls_sta->ch_switch_timeout = (RTW_GET_LE16(pIE->data + 2) >=
							TDLS_CH_SWITCH_TIMEOUT * 1000) ?
	...
	if (take_care_iqk == _TRUE) {
		...
		central_chnl = rtw_get_center_ch(pchsw_info->off_ch_num, bw_mode,
...
sint On_TDLS_Ch_Switch_Rsp(_adapter *padapter,
			   union recv_frame *precv_frame, struct sta_info *ptdls_sta)
{
	struct tdls_ch_switch *pchsw_info = &padapter->tdlsinfo.chsw_info;
	struct sta_priv *pstapriv = &padapter->stapriv;
...
void rtw_build_tdls_setup_req_ies(_adapter *padapter,
				  struct xmit_frame *pxmitframe, u8 *pframe, struct tdls_txmgmt *ptxmgmt,
				  struct sta_info *ptdls_sta)
{
	struct rf_ctl_t *rfctl = adapter_to_rfctl(padapter);
	struct registry_priv	*pregistrypriv = &padapter->registrypriv;
	...
	if (pattrib->encrypt) {
		...
		pframe = rtw_tdls_set_timeout_interval(ptxmgmt, pframe, pattrib, _TRUE,
						       ptdls_sta);
	}
...
void rtw_build_tdls_setup_rsp_ies(_adapter *padapter,
				  struct xmit_frame *pxmitframe, u8 *pframe, struct tdls_txmgmt *ptxmgmt,
				  struct sta_info *ptdls_sta)
{
	struct rf_ctl_t *rfctl = adapter_to_rfctl(padapter);
	...
	if (pattrib->encrypt) {
		...
		ptimeout_ie = pframe;
		pframe = rtw_tdls_set_timeout_interval(ptxmgmt, pframe, pattrib, _FALSE,
						       ptdls_sta);
	}
	...
	if (pattrib->encrypt && rtw_tdls_is_driver_setup(padapter) == _TRUE)
		wpa_tdls_ftie_mic(ptdls_sta->tpk.kck, 2, plinkid_ie, prsnie, ptimeout_ie,
				  pftie, pftie_mic);
	...
	if ((padapter->mlmepriv.htpriv.ht_option == _TRUE)
		...
		pframe = rtw_tdls_set_aid(padapter, pframe, pattrib);
		pframe = rtw_tdls_set_vht_cap(padapter, pframe, pattrib);
		pframe = rtw_tdls_set_vht_op_mode_notify(padapter, pframe, pattrib,
				pmlmeext->cur_bwmode);
	}
#endif
...
void rtw_build_tdls_setup_cfm_ies(_adapter *padapter,
				  struct xmit_frame *pxmitframe, u8 *pframe, struct tdls_txmgmt *ptxmgmt,
				  struct sta_info *ptdls_sta)
{
	struct rf_ctl_t *rfctl = adapter_to_rfctl(padapter);
	...
	if (pattrib->encrypt && (rtw_tdls_is_driver_setup(padapter) == _TRUE))
		wpa_tdls_ftie_mic(ptdls_sta->tpk.kck, 3, plinkid_ie, prsnie, ptimeout_ie,
				  pftie, pftie_mic);
	...
	if ((padapter->mlmepriv.htpriv.ht_option == _TRUE)
		...
		pframe = rtw_tdls_set_vht_operation(padapter, pframe, pattrib,
...
void rtw_build_tdls_teardown_ies(_adapter *padapter,
				 struct xmit_frame *pxmitframe, u8 *pframe, struct tdls_txmgmt *ptxmgmt,
				 struct sta_info *ptdls_sta)
{
	struct pkt_attrib	*pattrib = &pxmitframe->attrib;
	...
	if (pattrib->encrypt && (rtw_tdls_is_driver_setup(padapter) == _TRUE))
		wpa_tdls_teardown_ftie_mic(ptdls_sta->tpk.kck, plinkid_ie,
					   ptxmgmt->status_code, 1, 4, pftie, pftie_mic);
}

void rtw_build_tdls_dis_req_ies(_adapter *padapter,
				struct xmit_frame *pxmitframe, u8 *pframe, struct tdls_txmgmt *ptxmgmt)
{
	struct pkt_attrib *pattrib = &pxmitframe->attrib;
...
void rtw_build_tdls_dis_rsp_ies(_adapter *padapter,
				struct xmit_frame *pxmitframe, u8 *pframe, struct tdls_txmgmt *ptxmgmt,
				u8 privacy)
{
	struct registry_priv	*pregistrypriv = &padapter->registrypriv;
...
#ifdef CONFIG_80211N_HT
	if (pregistrypriv->ht_enable == _TRUE)
		pframe = rtw_tdls_set_ht_cap(padapter, pframe_head - pktlen_index,
					     pattrib);
#endif
...
void rtw_build_tdls_peer_traffic_indication_ies(_adapter *padapter,
		struct xmit_frame *pxmitframe, u8 *pframe, struct tdls_txmgmt *ptxmgmt,
		struct sta_info *ptdls_sta)
{
	...
	if (ptdls_sta->uapsd_vo & BIT(1))
		AC_queue = BIT(3);
	pframe = rtw_set_ie(pframe, _PTI_BUFFER_STATUS_, 1, &AC_queue,
			    &(pattrib->pktlen));

}

void rtw_build_tdls_peer_traffic_rsp_ies(_adapter *padapter,
		struct xmit_frame *pxmitframe, u8 *pframe, struct tdls_txmgmt *ptxmgmt,
		struct sta_info *ptdls_sta)
{
...
#ifdef CONFIG_TDLS_CH_SW
void rtw_build_tdls_ch_switch_req_ies(_adapter *padapter,
				      struct xmit_frame *pxmitframe, u8 *pframe, struct tdls_txmgmt *ptxmgmt,
				      struct sta_info *ptdls_sta)
{
	struct tdls_info *ptdlsinfo = &padapter->tdlsinfo;
...
void rtw_build_tdls_ch_switch_rsp_ies(_adapter *padapter,
				      struct xmit_frame *pxmitframe, u8 *pframe, struct tdls_txmgmt *ptxmgmt,
				      struct sta_info *ptdls_sta)
{
...
#ifdef CONFIG_WFD
void rtw_build_tunneled_probe_req_ies(_adapter *padapter,
				      struct xmit_frame *pxmitframe, u8 *pframe)
{
	u8 i;
	_adapter *iface = NULL;
...
void rtw_build_tunneled_probe_rsp_ies(_adapter *padapter,
				      struct xmit_frame *pxmitframe, u8 *pframe)
{
	u8 i;
	_adapter *iface = NULL;
...
void _tdls_delay_timer_hdl(void *FunctionContext)
{
	...
	RTW_INFO("[TDLS] %s, op_ch:%d, tdls_state:0x%08x\n", __func__,
		 rtw_get_oper_ch(padapter), ptdls_sta->tdls_sta_state);
...
#ifdef CONFIG_TDLS_CH_SW
	rtw_init_timer(&psta->ch_sw_timer, padapter, _tdls_ch_switch_timer_hdl,
		       psta);
	rtw_init_timer(&psta->delay_timer, padapter, _tdls_delay_timer_hdl, psta);
	rtw_init_timer(&psta->stay_on_base_chnl_timer, padapter,
		       _tdls_stay_on_base_chnl_timer_hdl, psta);
	rtw_init_timer(&psta->ch_sw_monitor_timer, padapter,
		       _tdls_ch_switch_monitor_timer_hdl, psta);
#endif
	rtw_init_timer(&psta->handshake_timer, padapter, _tdls_handshake_timer_hdl,
		       psta);
...
void rtw_tdls_teardown_post_hdl(_adapter *padapter, struct sta_info *psta,
				u8 enqueue_cmd)
{
	struct tdls_info *ptdlsinfo = &padapter->tdlsinfo;
