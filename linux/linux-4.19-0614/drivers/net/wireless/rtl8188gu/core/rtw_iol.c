			...
			u32 cmd_len)
{
	...
	_rtw_memcpy(xmit_frame->buf_addr + buf_offset + pattrib->pktlen, IOL_cmds,
		    cmd_len);
...
u8 rtw_IOL_cmd_boundary_handle(struct xmit_frame *pxmit_frame)
{
	...
	if (((pxmit_frame->attrib.pktlen + 32) % 256) + 8 >= 256) {
		...
		pxmit_frame->attrib.pktlen = ((((pxmit_frame->attrib.pktlen + 32) / 256) +
					       1) * 256);
...
#ifdef DBG_IO
int dbg_rtw_IOL_append_WB_cmd(struct xmit_frame *xmit_frame, u16 addr,
			      u8 value, const char *caller, const int line)
{
...
int rtw_IOL_exec_cmd_array_sync(PADAPTER adapter, u8 *IOL_cmds,
				u32 cmd_num, u32 max_wating_ms)
{
	struct xmit_frame	*xmit_frame;
