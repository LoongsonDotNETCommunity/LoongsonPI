		...
		if ((*pold_state == Ndis802_11Infrastructure)
			...
			if (check_fwstate(pmlmepriv, _FW_LINKED) == _TRUE) {
				rtw_indicate_disconnect(padapter, 0,
							_FALSE); /*will clr Linked_state; before this function, we must have checked whether issue dis-assoc_cmd or not*/
			}
		}
				  ...
				  struct rtw_ieee80211_channel *ch, int ch_num)
{
	...
	if ((check_fwstate(pmlmepriv,
		...
		res = rtw_sitesurvey_cmd(padapter, pssid, ssid_max_num, NULL, 0, ch,
					 ch_num);
...
u8 rtw_set_802_11_add_wep(_adapter *padapter, NDIS_802_11_WEP *wep)
{
	...
	bdefaultkey = (wep->KeyIndex & 0x40000000) > 0 ? _FALSE :
		      _TRUE; /* for ??? */
...
u16 rtw_get_cur_max_rate(_adapter *adapter)
{
	...
	if (is_supported_ht(psta->wireless_mode)) {
		...
		max_rate = ((rtw_vht_mcs_to_data_rate(psta->cmn.bw_mode, short_GI,
						      pmlmepriv->vhtpriv.vht_highest_rate) + 1) >> 1) * 10;
