			  ...
			  , u8 *ch, u8 *bw, u8 *offset, u8 *chbw_allow)
{
	...
	if (MSTATE_STA_LD_NUM(&mstate) || MSTATE_AP_NUM(&mstate)
		...
		if (!rtw_chset_is_chbw_valid(adapter_to_chset(adapter), dec_ch, dec_bw,
					     dec_offset)) {
			if (req_ch == -1 || req_bw == -1)
				goto choose_chbw;
		...
		if (rtw_chset_is_ch_non_ocp(adapter_to_chset(adapter), dec_ch, dec_bw,
					    dec_offset) == _FALSE)
...
choose_chbw:
		...
		if (!rtw_odm_dfs_domain_unknown(adapter)) {
			...
			if (adapter_to_rfctl(adapter)->dbg_dfs_master_choose_dfs_ch_first
			    && rtw_choose_shortest_waiting_ch(adapter, req_bw, &dec_ch, &dec_bw,
							      &dec_offset, RTW_CHF_2G | RTW_CHF_NON_DFS) == _TRUE)
				RTW_INFO(FUNC_ADPT_FMT" choose 5G DFS channel for debug\n",
					 FUNC_ADPT_ARG(adapter));
			else if (adapter_to_rfctl(adapter)->dfs_ch_sel_d_flags
				 && rtw_choose_shortest_waiting_ch(adapter, req_bw, &dec_ch, &dec_bw,
						 &dec_offset, adapter_to_rfctl(adapter)->dfs_ch_sel_d_flags) == _TRUE)
			...
			else if (rtw_choose_shortest_waiting_ch(adapter, req_bw, &dec_ch, &dec_bw,
								&dec_offset, 0) == _FALSE) {
			...
			if (rtw_choose_shortest_waiting_ch(adapter, req_bw, &dec_ch, &dec_bw,
							   &dec_offset, RTW_CHF_DFS) == _FALSE) {
				RTW_WARN(FUNC_ADPT_FMT" no available channel\n", FUNC_ADPT_ARG(adapter));
				*chbw_allow = _FALSE;
				goto exit;
			}
