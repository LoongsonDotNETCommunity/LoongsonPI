...
void rtw_mi_update_union_chan_inf(_adapter *adapter, u8 ch, u8 offset,
				  u8 bw)
{
...
int _rtw_mi_get_ch_setting_union(_adapter *adapter, u8 *ch, u8 *bw,
				 u8 *offset, bool include_self)
{
	struct dvobj_priv *dvobj = adapter_to_dvobj(adapter);
	_adapter *iface;
...
inline int rtw_mi_get_ch_setting_union(_adapter *adapter, u8 *ch, u8 *bw,
				       u8 *offset)
{
	return _rtw_mi_get_ch_setting_union(adapter, ch, bw, offset, 1);
}

inline int rtw_mi_get_ch_setting_union_no_self(_adapter *adapter, u8 *ch,
		u8 *bw, u8 *offset)
{
...
void _rtw_mi_status(_adapter *adapter, struct mi_state *mstate,
		    u8 target_sel)
{
	struct dvobj_priv *dvobj = adapter_to_dvobj(adapter);
	_adapter *iface;
	...
	for (i = 0; i < dvobj->iface_nums; i++) {
		...
		} else if (check_fwstate(&iface->mlmepriv,
...
inline void rtw_mi_status_no_self(_adapter *adapter,
				  struct mi_state *mstate)
{
	return _rtw_mi_status(adapter, mstate, MI_STATUS_OTHERS_ONLY);
}

inline void rtw_mi_status_no_others(_adapter *adapter,
				    struct mi_state *mstate)
{
...
void dump_dvobj_mi_status(void *sel, const char *fun_name,
			  _adapter *adapter)
{
	RTW_INFO("\n[ %s ] call %s\n", fun_name, __func__);
...
inline void rtw_mi_update_iface_status(struct mlme_priv *pmlmepriv,
				       sint state)
{
...
u8 rtw_mi_check_status(_adapter *adapter, u8 type)
{
	...
	case MI_LINKED:
		if (MSTATE_STA_LD_NUM(iface_state) || MSTATE_AP_NUM(iface_state)
		    ...
		    || MSTATE_MESH_NUM(iface_state)) /*check_fwstate(&iface->mlmepriv, _FW_LINKED)*/
			ret = _TRUE;
		break;
	case MI_ASSOC:
		if (MSTATE_STA_LD_NUM(iface_state) || MSTATE_AP_LD_NUM(iface_state)
		    || MSTATE_ADHOC_LD_NUM(iface_state) || MSTATE_MESH_LD_NUM(iface_state))
			ret = _TRUE;
		break;
	case MI_UNDER_WPS:
	...
	case MI_STA_NOLINK: /* this is misleading, but not used now */
		if (MSTATE_STA_NUM(iface_state) && (!(MSTATE_STA_LD_NUM(iface_state)
...
static u8 _rtw_mi_process_without_schk(_adapter *padapter,
				       bool exclude_self,
				       void *data, u8(*ops_func)(_adapter *padapter, void *data))
{
	int i;
...
static u8 _rtw_mi_suspend_free_assoc_resource(_adapter *padapter,
		void *data)
{
...
{
	_rtw_mi_process(adapter, _FALSE, NULL,
			_rtw_mi_suspend_free_assoc_resource);
}
...
static u8 _rtw_mi_issue_nulldata(_adapter *padapter, void *data)
{
	...
	if (is_client_associated_to_ap(padapter) == _TRUE) {
		/* TODO: TDLS peers */
		issue_nulldata(padapter, pnulldata_param->da, pnulldata_param->power_mode,
			       pnulldata_param->try_cnt, pnulldata_param->wait_ms);
		return _TRUE;
	}
	return _FALSE;
}

u8 rtw_mi_issue_nulldata(_adapter *padapter, unsigned char *da,
...
u8 rtw_mi_buddy_issue_nulldata(_adapter *padapter, unsigned char *da,
			       unsigned int power_mode, int try_cnt, int wait_ms)
{
	struct nulldata_param nparam;
...
u8 rtw_mi_busy_traffic_check(_adapter *padapter, bool check_sc_interval)
{
	...
	return _rtw_mi_process(padapter, _FALSE, &in_data,
			       _rtw_mi_busy_traffic_check);
}
u8 rtw_mi_buddy_busy_traffic_check(_adapter *padapter,
				   bool check_sc_interval)
{
	bool in_data = check_sc_interval;

	return _rtw_mi_process(padapter, _TRUE, &in_data,
			       _rtw_mi_busy_traffic_check);
}
...
{
	return _rtw_mi_process(padapter, _FALSE, NULL,
			       _rtw_mi_check_miracast_enabled);
}
...
{
	return _rtw_mi_process(padapter, _FALSE, NULL,
			       _rtw_mi_check_pending_xmitbuf);
}
u8 rtw_mi_buddy_check_pending_xmitbuf(_adapter *padapter)
{
	return _rtw_mi_process(padapter, _TRUE, NULL,
			       _rtw_mi_check_pending_xmitbuf);
}
#endif
...
static u8 _rtw_mi_dynamic_check_timer_handlder(_adapter *adapter,
		void *data)
{
...
{
	return _rtw_mi_process(padapter, _FALSE, NULL,
			       _rtw_mi_dynamic_check_timer_handlder);
}
...
u8 rtw_mi_report_survey_event(_adapter *padapter,
			      union recv_frame *precv_frame)
{
	return _rtw_mi_process(padapter, _FALSE, precv_frame,
			       _rtw_mi_report_survey_event);
}
u8 rtw_mi_buddy_report_survey_event(_adapter *padapter,
				    union recv_frame *precv_frame)
{
	return _rtw_mi_process(padapter, _TRUE, precv_frame,
			       _rtw_mi_report_survey_event);
}
...
_adapter *rtw_get_iface_by_macddr(_adapter *padapter, u8 *mac_addr)
{
	...
	for (i = 0; i < dvobj->iface_nums; i++) {
		iface = dvobj->padapters[i];
		if ((iface)
		    && (_rtw_memcmp(mac_addr, adapter_mac_addr(iface), ETH_ALEN))) {
			bmatch = _TRUE;
			break;
		}
	}
...
void rtw_dbg_skb_process(_adapter *padapter, union recv_frame *precvframe,
	...
	*/
	if (precvframe->u.hdr.len != pcloneframe->u.hdr.len)
		RTW_INFO("%s [WARN]  recv_frame length(%d:%d) compare failed\n", __func__,
			 precvframe->u.hdr.len, pcloneframe->u.hdr.len);

	if (_rtw_memcmp(&precvframe->u.hdr.attrib, &pcloneframe->u.hdr.attrib,
			sizeof(struct rx_pkt_attrib)) == _FALSE)
		RTW_INFO("%s [WARN]  recv_frame attrib compare failed\n", __func__);

	if (_rtw_memcmp(precvframe->u.hdr.rx_data, pcloneframe->u.hdr.rx_data,
			precvframe->u.hdr.len) == _FALSE)
		...
		union recv_frame *pcloneframe)
{
	...
	if (pcloneframe) {
		...
		pcloneframe->u.hdr.precvbuf =
			NULL;	/*can't access the precvbuf for new arch.*/
		pcloneframe->u.hdr.len = 0;

		_rtw_memcpy(&pcloneframe->u.hdr.attrib, &precvframe->u.hdr.attrib,
...
#else
		if (rtw_os_recvframe_duplicate_skb(adapter, pcloneframe,
						   precvframe->u.hdr.pkt) == _SUCCESS)
#endif
		{
#ifdef CONFIG_SKB_ALLOCATED
				    ...
				    union recv_frame *precvframe, u8 *pphy_status)
{
	...
	for (i = 0; i < dvobj->iface_nums; i++) {
		...
		if (pcloneframe) {
			ret = _rtw_mi_buddy_clone_bcmc_packet(iface, precvframe, pphy_status,
							      pcloneframe);
			if (_SUCCESS != ret) {
				if (ret == -1)
...
void rtw_mi_update_ap_bmc_camid(_adapter *padapter, u8 camid_a, u8 camid_b)
{
	...
	for (i = 0; i < dvobj->iface_nums; i++) {
		...
		if (macid_ctl->iface_bmc[iface->iface_id] != INVALID_SEC_MAC_CAM_ID) {
			...
			iface->securitypriv.dot118021x_bmc_cam_id  =
				macid_ctl->iface_bmc[iface->iface_id];
		}
	}
#endif
}
