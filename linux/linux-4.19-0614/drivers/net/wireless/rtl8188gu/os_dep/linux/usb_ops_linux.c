...
int usbctrl_vendorreq(struct intf_hdl *pintfhdl, u8 request, u16 value,
...
#define REG_LOCAL_SEC 0x02
	...
	if (current_reg_sec == REG_ON_SEC) {
		...
		status = rtw_usb_control_msg(udev, t_pipe, t_req, t_reqtype, t_reg,
					     t_index, pIo_buf, t_len, RTW_USB_CONTROL_MSG_TIMEOUT);
	...
	if (purb->status == 0) {
		...
		if ((purb->actual_length > MAX_RECVBUF_SZ)
			...
			RTW_INFO("%s()-%d: urb->actual_length:%u, MAX_RECVBUF_SZ:%u, RXDESC_SIZE:%u\n"
				 , __FUNCTION__, __LINE__, purb->actual_length, MAX_RECVBUF_SZ,
				 RXDESC_SIZE);
			rtw_read_port(padapter, precvpriv->ff_hwaddr, 0,
				      (unsigned char *)precvbuf);
		} else {
		...
		if (rtw_inc_and_chk_continual_io_error(adapter_to_dvobj(
				padapter)) == _TRUE)
			rtw_set_surprise_removed(padapter);
...
void usb_read_interrupt_complete(struct urb *purb, struct pt_regs *regs)
{
	...
	if (purb->status == 0) {/*SUCCESS*/
		...
		rtw_hal_interrupt_handler(padapter, purb->actual_length,
