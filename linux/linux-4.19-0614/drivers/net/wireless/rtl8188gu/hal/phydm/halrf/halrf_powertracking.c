...
)
{
	...
	/*  */
	p_adapter->HalFunc.GetHwRegHandler(p_adapter, HW_VAR_RF_STATE,
					   (u8 *)(&rt_state));
	if (p_adapter->bDriverStopped
	    || p_adapter->bDriverIsGoingToPnpSetPowerSleep || rt_state == eRfOff) {
		PHYDM_DBG(p_dm, ODM_COMP_INIT,
			  ("check_pow_status Return false, due to %d/%d/%d\n",
			   p_adapter->bDriverStopped, p_adapter->bDriverIsGoingToPnpSetPowerSleep,
			   rt_state));
		return	false;
	}
#endif
