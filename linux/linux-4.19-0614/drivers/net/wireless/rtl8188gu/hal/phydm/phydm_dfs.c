...
boolean phydm_dfs_is_meteorology_channel(void *p_dm_void)
{
	...
	return ((band_width == CHANNEL_WIDTH_80 && (c_channel) >= 116
		 && (c_channel) <= 128) ||
		(band_width == CHANNEL_WIDTH_40 && (c_channel) >= 116
		 && (c_channel) <= 128) ||
		(band_width == CHANNEL_WIDTH_20 && (c_channel) >= 120
		 && (c_channel) <= 128));
}
...
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct _DFS_STATISTICS	*p_dfs = (struct _DFS_STATISTICS *)
					 phydm_get_structure(p_dm, PHYDM_DFS);
...
)
{
	...
	u8 peak_th_cur = 0, short_pulse_cnt_th_cur = 0, long_pulse_cnt_th_cur = 0,
	   three_peak_opt_cur = 0, three_peak_th2_cur = 0;
	...
	if ((p_dfs->idle_mode == 1)) { /*idle (no traffic)*/
		...
		if (region_domain == PHYDM_DFS_DOMAIN_MKK) {
			...
			long_pulse_cnt_th_cur = 15;
			if (phydm_dfs_is_meteorology_channel(
				    p_dm)) {/*need to add check cac end condition*/
				peak_th_cur = 2;
				nb2wb_th_cur = 3;
				three_peak_opt_cur = 1;
				three_peak_th2_cur = 0;
...
boolean phydm_radar_detect_dm_check(
	void *p_dm_void
)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	...
	u16 i = 0, k = 0, fa_count_cur = 0, fa_count_inc = 0, total_fa_in_hist = 0,
	    pre_post_now_acc_fa_in_hist = 0, max_fa_in_hist = 0,
	    vht_crc_ok_cnt_cur = 0;
	u16 vht_crc_ok_cnt_inc = 0, ht_crc_ok_cnt_cur = 0, ht_crc_ok_cnt_inc = 0,
	    leg_crc_ok_cnt_cur = 0, leg_crc_ok_cnt_inc = 0;
	u16 total_crc_ok_cnt_inc = 0, short_pulse_cnt_cur = 0,
	    short_pulse_cnt_inc = 0, long_pulse_cnt_cur = 0, long_pulse_cnt_inc = 0,
	    total_pulse_count_inc = 0;
	u32 regf98_value = 0, reg918_value = 0, reg91c_value = 0, reg920_value = 0,
	    reg924_value = 0;
	boolean tri_short_pulse = 0, tri_long_pulse = 0, radar_type = 0,
		fault_flag_det = 0, fault_flag_psd = 0, fa_flag = 0, radar_detected = 0;
	...
	if (index == 0)
		pre_post_now_acc_fa_in_hist = p_dfs->fa_inc_hist[index] +
					      p_dfs->fa_inc_hist[index + 1] + p_dfs->fa_inc_hist[4];
	else if (index == 4)
		pre_post_now_acc_fa_in_hist = p_dfs->fa_inc_hist[index] +
					      p_dfs->fa_inc_hist[0] + p_dfs->fa_inc_hist[index - 1];
	else
		pre_post_now_acc_fa_in_hist = p_dfs->fa_inc_hist[index] +
					      p_dfs->fa_inc_hist[index + 1] + p_dfs->fa_inc_hist[index - 1];
	...
	total_crc_ok_cnt_inc = vht_crc_ok_cnt_inc + ht_crc_ok_cnt_inc +
			       leg_crc_ok_cnt_inc;
	...
	if (p_dfs->det_print) {
		...
		PHYDM_DBG(p_dm, DBG_DFS,
			  ("Total_CRC_OK_cnt_inc[%d] VHT_CRC_ok_cnt_inc[%d] HT_CRC_ok_cnt_inc[%d] LEG_CRC_ok_cnt_inc[%d] FA_count_inc[%d]\n",
			   total_crc_ok_cnt_inc, vht_crc_ok_cnt_inc, ht_crc_ok_cnt_inc,
			   leg_crc_ok_cnt_inc, fa_count_inc));
		PHYDM_DBG(p_dm, DBG_DFS,
			  ("Init_Gain[%x] 0x91c[%x] 0xf98[%08x] short_pulse_cnt_inc[%d] long_pulse_cnt_inc[%d]\n",
			   p_dfs->igi_cur, p_dfs->st_l2h_cur, regf98_value, short_pulse_cnt_inc,
			   long_pulse_cnt_inc));
		...
		PHYDM_DBG(p_dm, DBG_DFS,
			  ("0x918[%08x] 0x91c[%08x] 0x920[%08x] 0x924[%08x]\n", reg918_value,
			   reg91c_value, reg920_value, reg924_value));
		PHYDM_DBG(p_dm, DBG_DFS,
			  ("dfs_regdomain = %d, dbg_mode = %d, idle_mode = %d\n", region_domain,
			   p_dfs->dbg_mode, p_dfs->idle_mode));
	}
	...
	if (max_fa_in_hist >= fa_mask_th || total_fa_in_hist >= fa_mask_th
	    || pre_post_now_acc_fa_in_hist >= fa_mask_th || (p_dfs->igi_cur >= 0x30)) {
	...
	if ((st_l2h_new != p_dfs->st_l2h_cur)) {
		...
		p_dfs->pwdb_th = ((int)p_dfs->st_l2h_cur - (int)p_dfs->igi_cur) / 2 +
				 p_dfs->pwdb_scalar_factor;
		p_dfs->pwdb_th = MAX_2(p_dfs->pwdb_th,
				       (int)p_dfs->pwdb_th); /*limit the pwdb value to absoulte lower bound 8*/
		p_dfs->pwdb_th = MIN_2(p_dfs->pwdb_th,
				       0x1f);    /*limit the pwdb value to absoulte upper bound 0x1f*/
...
boolean phydm_radar_detect(void *p_dm_void)
{
	...
	if (p_dfs->igi_pre != p_dfs->igi_cur) {
		p_dfs->pwdb_th = ((int)p_dfs->st_l2h_cur - (int)p_dfs->igi_cur) / 2 +
				 p_dfs->pwdb_scalar_factor;
		p_dfs->pwdb_th = MAX_2(p_dfs->pwdb_th_cur,
				       (int)p_dfs->pwdb_th); /* limit the pwdb value to absoulte lower bound 0xa */
		p_dfs->pwdb_th = MIN_2(p_dfs->pwdb_th_cur,
				       0x1f);    /* limit the pwdb value to absoulte upper bound 0x1f */
		odm_set_bb_reg(p_dm,  0x918, 0x00001f00, p_dfs->pwdb_th);
	}
...
)
{
	...
	struct _DFS_STATISTICS	*p_dfs = (struct _DFS_STATISTICS *)
					 phydm_get_structure(p_dm_odm, PHYDM_DFS);
	u32 used = *_used;
	...
	PHYDM_SNPRINTF((output + used, out_len - used,
			...
			p_dfs->dbg_mode, p_dfs->force_TP_mode, p_dfs->det_print,
			p_dfs->det_print2));
...
boolean phydm_dfs_master_enabled(
	void		*p_dm_void
)
{
