		...
		case 0:
			for (i = 0 ; i < num; i ++)
				PHYDM_DBG(p_dm, ODM_COMP_CALIBRATION, ("[IQK]%-20s %-2d: 0x%x\n",
								       (path == 0) ? "PATH A RX-XYM " : "PATH B RX-XYM", i,
								       p_iqk_info->rx_xym[path][i]));
			break;
		case 1:
			for (i = 0 ; i < num; i ++)
				PHYDM_DBG(p_dm, ODM_COMP_CALIBRATION, ("[IQK]%-20s %-2d: 0x%x\n",
								       (path == 0) ? "PATH A TX-XYM " : "PATH B TX-XYM", i,
								       p_iqk_info->tx_xym[path][i]));
			break;
		case 2:
			for (i = 0 ; i < 6; i ++)
				PHYDM_DBG(p_dm, ODM_COMP_CALIBRATION, ("[IQK]%-20s %-2d: 0x%x\n",
								       (path == 0) ? "PATH A GS1-XYM " : "PATH B GS1-XYM", i,
								       p_iqk_info->gs1_xym[path][i]));
			break;
		case 3:
			for (i = 0 ; i < 6; i ++)
				PHYDM_DBG(p_dm, ODM_COMP_CALIBRATION, ("[IQK]%-20s %-2d: 0x%x\n",
								       (path == 0) ? "PATH A GS2-XYM " : "PATH B GS2-XYM", i,
								       p_iqk_info->gs2_xym[path][i]));
			break;
		case 4:
			for (i = 0 ; i < 6; i ++)
				PHYDM_DBG(p_dm, ODM_COMP_CALIBRATION, ("[IQK]%-20s %-2d: 0x%x\n",
								       (path == 0) ? "PATH A RXK1-XYM " : "PATH B RXK1-XYM", i,
	...
	u32 *_out_len)
{
	...
	PHYDM_SNPRINTF((output + used, out_len - used, "%-20s\n",
			(p_dm->fw_offload_ability & PHYDM_RF_IQK_OFFLOAD) ? "FW-IQK" :
			"Driver-IQK"));
	...
	PHYDM_SNPRINTF((output + used, out_len - used, "%-20s: %llu %s\n",
			"progressing_time", p_dm->rf_calibrate_info.iqk_total_progressing_time,
			"(ms)"));

	tmp = odm_read_4byte(p_dm, 0x1bf0);
	for (rf_path = RF_PATH_A; rf_path <= RF_PATH_B; rf_path++)
		for (j = 0; j < 2; j++)
			iqk_result[0][rf_path][j] = (boolean)(tmp & BIT(rf_path + (j * 4)) >>
							      (rf_path + (j * 4)));
...
void halrf_get_fw_version(void	*p_dm_void)
{
...
#if (DM_ODM_SUPPORT_TYPE & ODM_WIN)
	{
		...
		p_rf->fw_ver = (adapter->MgntInfo.FirmwareVersion << 16) |
...
#elif (DM_ODM_SUPPORT_TYPE & ODM_AP)
	{
		...
		p_rf->fw_ver = (priv->pshare->fw_version << 16) |
...
#else
	{
		...
		p_rf->fw_ver = (p_hal_data->firmware_version << 16) |
			...
			for (i = 0; i < 8; i++) {
				PHYDM_DBG(p_dm, ODM_COMP_CALIBRATION,
					  ("[IQK]%-7s %-3s CFIR_real: %-2d: 0x%x\n",
					   (path == 0) ? "PATH A" : "PATH B", (idx == 0) ? "TX" : "RX", i,
					   p_iqk_info->IQK_CFIR_real[2][path][idx][i]));
			}
			for (i = 0; i < 8; i++) {
				PHYDM_DBG(p_dm, ODM_COMP_CALIBRATION,
					  ("[IQK]%-7s %-3s CFIR_img:%-2d: 0x%x\n",
					   (path == 0) ? "PATH A" : "PATH B", (idx == 0) ? "TX" : "RX", i,
					   p_iqk_info->IQK_CFIR_imag[2][path][idx][i]));
			}
		}
	}
}
...
void halrf_iqk_dbg_cfir_backup_update(
	struct PHY_DM_STRUCT			*p_dm
)
{
	struct _IQK_INFORMATION	*p_iqk_info = &p_dm->IQK_info;
	...
	for (path = 0; path < 2; path++) {
		for (idx = 0; idx < 2; idx++) {
			...
			odm_set_bb_reg(p_dm, 0x1bd4,
				       BIT(20) | BIT(19) | BIT(18) | BIT(17) | BIT(16), 0x10);
			for (i = 0; i < 8; i++) {
				odm_write_4byte(p_dm, 0x1bd8,
						((0xc0000000 >> idx) + 0x3) + (i * 4) +
						(p_iqk_info->IQK_CFIR_real[2][path][idx][i] << 9));
				odm_write_4byte(p_dm, 0x1bd8,
						((0xc0000000 >> idx) + 0x1) + (i * 4) +
						(p_iqk_info->IQK_CFIR_imag[2][path][idx][i] << 9));
...
void halrf_iqk_dbg_cfir_reload(
	struct PHY_DM_STRUCT			*p_dm
)
{
	struct _IQK_INFORMATION	*p_iqk_info = &p_dm->IQK_info;
	...
	for (path = 0; path < 2; path++) {
		for (idx = 0; idx < 2; idx++) {
			...
			for (i = 0; i < 8; i++) {
				...
				odm_write_4byte(p_dm, 0x1bd8,
						((0xc0000000 >> idx) + 0x3) + (i * 4) +
						(p_iqk_info->IQK_CFIR_real[0][path][idx][i] << 9));
				odm_write_4byte(p_dm, 0x1bd8,
						((0xc0000000 >> idx) + 0x1) + (i * 4) +
						(p_iqk_info->IQK_CFIR_imag[0][path][idx][i] << 9));
			}
		}
		odm_set_bb_reg(p_dm, 0x1bd8, MASKDWORD, 0x0);
		odm_set_bb_reg(p_dm, 0x1b0c, BIT(13) | BIT(12), 0x0);
...
)
{
	...
	if (dm_value[0] == 0x0)
		...
		halrf_iqk_dbg_cfir_write(p_dm, 0, dm_value[1], dm_value[2], dm_value[3],
					 dm_value[4]);
	else if (dm_value[0] == 0x11)
		halrf_iqk_dbg_cfir_write(p_dm, 1, dm_value[1], dm_value[2], dm_value[3],
					 dm_value[4]);
...
)
{
	...
	if (!p_dm->rf_calibrate_info.is_iqk_in_progress) {
		...
		p_dm->rf_calibrate_info.iqk_progressing_time = odm_get_progressing_time(
					p_dm, start_time);
...
void halrf_supportability_init_mp(
	void		*p_dm_void
)
{
	struct PHY_DM_STRUCT		*p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
...
)
{
	...
	if (!p_dm->rf_calibrate_info.is_lck_in_progress) {
		...
		p_dm->rf_calibrate_info.lck_progressing_time = odm_get_progressing_time(
					p_dm, start_time);
		ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
