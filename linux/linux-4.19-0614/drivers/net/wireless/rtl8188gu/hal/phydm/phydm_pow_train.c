...
)
{
	...
	if (p_pow_train_t->force_power_training_state == DISABLE_POW_TRAIN) {
	...
	} else if (p_pow_train_t->force_power_training_state ==
		   DYNAMIC_POW_TRAIN) {
		...
		/* Compute score */
		crc_ok_cnt = p_dm->phy_dbg_info.num_qry_phy_status_ofdm +
		...
		/* smoothing */
		p_pow_train_t->pow_train_score = (pt_score_tmp << 4) +
						 (p_pow_train_t->pow_train_score >> 1) + (p_pow_train_t->pow_train_score >>
								 2);
