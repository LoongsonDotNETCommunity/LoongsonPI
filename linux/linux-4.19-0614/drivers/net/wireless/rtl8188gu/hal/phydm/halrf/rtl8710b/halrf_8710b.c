...
)
{
	...
	if ((iqk_result_x != 0) && (*(p_dm->p_band_type) == ODM_BAND_2_4G)) {
		...
		case RF_PATH_A:
			...
			value32 = (odm_get_bb_reg(p_dm, REG_OFDM_0_ECCA_THRESHOLD,
						  MASKDWORD) & (~(BIT(31) | BIT(29) | BIT(28)))) | value32;
		...
		switch (rf_path) {
		case RF_PATH_A:
			odm_set_bb_reg(p_dm, REG_OFDM_0_XA_TX_IQ_IMBALANCE, MASKDWORD,
				       ofdm_swing_table_new[OFDM_index]);
			odm_set_bb_reg(p_dm, REG_OFDM_0_XC_TX_AFE, MASKH4BITS, 0x00);
			value32 = odm_get_bb_reg(p_dm, REG_OFDM_0_ECCA_THRESHOLD,
						 MASKDWORD) & (~(BIT(31) | BIT(29) | BIT(28)));
			odm_set_bb_reg(p_dm, REG_OFDM_0_ECCA_THRESHOLD, MASKDWORD, value32);
			break;

		case RF_PATH_B:
			/*image S1:c80 to S0:Cd0 and Cd4*/
			odm_set_bb_reg(p_dm, 0xcd0, 0x000007FE,
				       ofdm_swing_table_new[OFDM_index] & 0x000003FF);
			odm_set_bb_reg(p_dm, 0xcd0, 0x0007E000,
				       (ofdm_swing_table_new[OFDM_index] & 0x0000FC00) >> 10);
			odm_set_bb_reg(p_dm, 0xcd4, 0x0000007E,
				       (ofdm_swing_table_new[OFDM_index] & 0x003F0000) >> 16);
			odm_set_bb_reg(p_dm, 0xcd4, 0x007FE000,
				       (ofdm_swing_table_new[OFDM_index] & 0xFFC00000) >> 22);
	...
	ODM_RT_TRACE(p_dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD,
		     ("TxPwrTracking path %c: X = 0x%x, Y = 0x%x ele_A = 0x%x ele_C = 0x%x ele_D = 0x%x ele_A_ext = 0x%x ele_C_ext = 0x%x ele_D_ext = 0x%x\n",
		      (rf_path == RF_PATH_A ? 'A' : 'B'), (u32)iqk_result_x, (u32)iqk_result_y,
		      (u32)ele_A, (u32)ele_C, (u32)ele_D, (u32)ele_A_ext, (u32)ele_C_ext,
		      (u32)ele_D_ext));
}
...
{
	odm_set_bb_reg(p_dm, 0xab4, 0x000007FF,
		       cck_swing_table_ch1_ch14_8710b[cck_swing_index]);
}
...
)
{
	...
	if (is_bt_enable) {
		ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
			     ("[IQK]Skip IQK because BT is enable\n"));
		return;
	} else
		ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
			     ("[IQK]Do IQK because BT is disable\n"));
#endif
...
)
{
	...
	if (tx_rate != 0xFF) {
		...
		/* HT*/
			...
			pwr_tracking_limit_ofdm =
				p_rf_calibrate_info->default_ofdm_index;   /*Default OFDM index = 30 */
	}
	...
	if (method == TXAGC) {
		...
		p_rf_calibrate_info->remnant_ofdm_swing_idx[rf_path] =
			p_rf_calibrate_info->absolute_ofdm_swing_idx[rf_path];   /* Remnant index equal to aboslute compensate value. */

#if (DM_ODM_SUPPORT_TYPE & (ODM_WIN | ODM_CE))

#if (MP_DRIVER != 1)
#if 0
		PHY_SetTxPowerLevelByPath8710B(adapter, *p_dm->p_channel,
					       rf_path);   /* Using new set power function */
		...
		if (rf_path == RF_PATH_A) {
			odm_set_tx_power_index_by_rate_section(p_dm, RF_PATH_A, *p_dm->p_channel,
							       CCK);
			odm_set_tx_power_index_by_rate_section(p_dm, RF_PATH_A, *p_dm->p_channel,
							       OFDM);
			odm_set_tx_power_index_by_rate_section(p_dm, RF_PATH_A, *p_dm->p_channel,
			...
			odm_set_tx_power_index_by_rate_section(p_dm, RF_PATH_B, *p_dm->p_channel,
							       CCK);
			odm_set_tx_power_index_by_rate_section(p_dm, RF_PATH_B, *p_dm->p_channel,
							       OFDM);
			odm_set_tx_power_index_by_rate_section(p_dm, RF_PATH_B, *p_dm->p_channel,
		...
		if (rf_path == RF_PATH_A) {
			...
			RT_DISP(FPHY, PHY_TXPWR,
				("odm_tx_pwr_track_set_pwr_8710b: CCK Tx-rf(A) Power = 0x%x\n", tx_agc));

			/*OFDM path S1*/
			pwr = odm_get_bb_reg(p_dm, REG_TX_AGC_A_RATE18_06, 0xFF);
			pwr += (p_rf_calibrate_info->bb_swing_idx_ofdm[RF_PATH_A] -
			...
			RT_DISP(FPHY, PHY_TXPWR,
				("odm_tx_pwr_track_set_pwr_8710b: OFDM Tx-rf(A) Power = 0x%x\n", tx_agc));
			...
			RT_DISP(FPHY, PHY_TXPWR,
				("odm_tx_pwr_track_set_pwr_8710b: CCK Tx-rf(B) Power = 0x%x\n", pwr));


			pwr = odm_get_bb_reg(p_dm, REG_TX_AGC_B_RATE18_06, 0xFF);
			pwr += (p_rf_calibrate_info->bb_swing_idx_ofdm[RF_PATH_B] -
				p_rf_calibrate_info->bb_swing_idx_ofdm_base[RF_PATH_B]);
			tx_agc = ((pwr << 24) | (pwr << 16) | (pwr << 8) | pwr);
			...
			RT_DISP(FPHY, PHY_TXPWR,
				("odm_tx_pwr_track_set_pwr_8710b: OFDM Tx-rf(B) Power = 0x%x\n", tx_agc));
		}
#endif
		...
		final_ofdm_swing_index = p_rf_calibrate_info->default_ofdm_index +
					 p_rf_calibrate_info->absolute_ofdm_swing_idx[rf_path];
		final_cck_swing_index = p_rf_calibrate_info->default_cck_index +
					p_rf_calibrate_info->absolute_ofdm_swing_idx[rf_path];
...
#endif
		ODM_RT_TRACE(p_dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD,
			     ("p_dm->default_ofdm_index=%d,  p_dm->DefaultCCKIndex=%d, p_dm->absolute_ofdm_swing_idx[rf_path]=%d, rf_path = %d\n",
			      p_rf_calibrate_info->default_ofdm_index,
			      p_rf_calibrate_info->default_cck_index,
			      p_rf_calibrate_info->absolute_ofdm_swing_idx[rf_path], rf_path));
		...
		if (rf_path == RF_PATH_A) {
			final_cck_swing_index = p_rf_calibrate_info->default_cck_index +
						p_rf_calibrate_info->absolute_ofdm_swing_idx[rf_path];    /*CCK Follow path-A and lower CCK index means higher power.*/
			...
			if ((p_dm->mp_mode) == 1) {
				...
				ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
					     ("~~~~~beforeAGC~~~~~~0xc80 = 0x%x  0xe14= 0x%x rf0x0 = 0x%x \n",
					      odm_get_bb_reg(p_dm, 0xc80, MASKDWORD), odm_get_bb_reg(p_dm, 0xe14,
							      MASKDWORD), odm_get_rf_reg(p_dm, RF_PATH_A, 0x0, bRFRegOffsetMask)));
				pwr = odm_get_bb_reg(p_dm, REG_TX_AGC_A_RATE18_06, 0xFF);
				pwr += (p_rf_calibrate_info->remnant_ofdm_swing_idx[RF_PATH_A] -
			...
			p_rf_calibrate_info->modify_tx_agc_value_ofdm =
				p_rf_calibrate_info->remnant_ofdm_swing_idx[RF_PATH_A] ;
			...
			if (final_cck_swing_index > pwr_tracking_limit_cck) {
				...
				ODM_RT_TRACE(p_dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD,
					     ("******Path_A CCK Over Limit, pwr_tracking_limit_cck = %d, p_dm->remnant_cck_swing_idx  = %d\n",
					      pwr_tracking_limit_cck, p_rf_calibrate_info->remnant_cck_swing_idx));

				/* Adjust BB swing by CCK filter coefficient*/
				odm_set_bb_reg(p_dm, 0xab4, 0x000007FF,
					       cck_swing_table_ch1_ch14_8710b[pwr_tracking_limit_cck]);
				...
				ODM_RT_TRACE(p_dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD,
					     ("******Path_A CCK Under Limit, pwr_tracking_limit_cck = %d, p_dm->remnant_cck_swing_idx  = %d\n",
				...
				odm_set_bb_reg(p_dm, 0xab4, 0x000007FF,
					       cck_swing_table_ch1_ch14_8710b[final_cck_swing_index]);
			...
			if ((*(p_dm->p_mp_mode)) == 1) {
				...
				pwr += p_rf_calibrate_info->remnant_cck_swing_idx -
			...
			p_rf_calibrate_info->modify_tx_agc_value_cck =
				p_rf_calibrate_info->remnant_cck_swing_idx;
		}
#if 0
		if (rf_path == RF_PATH_B) {
			if (final_ofdm_swing_index > pwr_tracking_limit_ofdm) {
				...
				if (p_rf_calibrate_info->modify_tx_agc_flag_path_b) {
					...
					ODM_RT_TRACE(p_dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD,
						     ("******Path_B p_dm->Modify_TxAGC_Flag = false\n"));
				}
			}
#if (MP_DRIVER == 1)
			if ((*(p_dm->p_mp_mode)) == 1) {
				pwr = odm_get_bb_reg(p_dm, REG_TX_AGC_A_RATE18_06, 0xFF);
				pwr += (p_rf_calibrate_info->remnant_ofdm_swing_idx[RF_PATH_B] -
					p_rf_calibrate_info->modify_tx_agc_value_ofdm);
			...
			p_rf_calibrate_info->modify_tx_agc_value_ofdm =
				p_rf_calibrate_info->remnant_ofdm_swing_idx[RF_PATH_B] ;
		}
#endif
		ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
			     ("~~~~~~~~~~~~~~~0xc80 = 0x%x 0xe14 = 0x%x 0xab4 = 0x%x 0x86c = 0x%x\n",
			      odm_get_bb_reg(p_dm, 0xc80, MASKDWORD), odm_get_bb_reg(p_dm, 0xe14,
					      MASKDWORD), odm_get_bb_reg(p_dm, 0xab4, MASKDWORD), odm_get_bb_reg(p_dm,
							      0x86c, MASKDWORD)));
...
)
{
	...
	ODM_RT_TRACE(p_dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD,
		     ("Power Tracking tx_rate=0x%X\n", tx_rate));

	if (1 <= channel && channel <= 14) {
		if (IS_CCK_RATE(tx_rate)) {
			*temperature_up_a   =
				p_rf_calibrate_info->delta_swing_table_idx_2g_cck_a_p;
			*temperature_down_a =
				p_rf_calibrate_info->delta_swing_table_idx_2g_cck_a_n;
			*temperature_up_b   =
				p_rf_calibrate_info->delta_swing_table_idx_2g_cck_b_p;
			*temperature_down_b =
				p_rf_calibrate_info->delta_swing_table_idx_2g_cck_b_n;
		} else {
...
)
{
	...
	odm_set_bb_reg(p_dm, REG_MAC_PHY_CTRL, 0xFFF000,
		       (crystal_cap | (crystal_cap << 6)));

	ODM_RT_TRACE(p_dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD,
		     ("crystal_cap(0x2c)  0x%X\n", odm_get_bb_reg(p_dm, REG_MAC_PHY_CTRL,
				     0xFFF000)));
...
)
{
	...
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("RF0x1 @S1 TXIQK = 0x%x\n", odm_get_rf_reg(p_dm, RF_PATH_A, 0x1,
				     RFREGOFFSETMASK)));
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("RF0x2 @S1 TXIQK = 0x%x\n", odm_get_rf_reg(p_dm, RF_PATH_A, 0x2,
				     RFREGOFFSETMASK)));
#endif
	...
	original_path = odm_get_mac_reg(p_dm, REG_LTECOEX_PATH_CONTROL,
					MASKDWORD);  /*save 0x70*/
	odm_set_bb_reg(p_dm, REG_LTECOEX_CTRL, MASKDWORD, 0x800f0038);
	ODM_delay_ms(1);
	original_gnt = odm_get_bb_reg(p_dm, REG_LTECOEX_READ_DATA,
				      MASKDWORD);  /*save 0x38*/
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("[IQK]OriginalGNT = 0x%x\n", original_gnt));
	...
	odm_set_bb_reg(p_dm, REG_LTECOEX_CTRL, MASKDWORD,
		       0xc0020038);	/*0x38[15:8] = 0x77*/
	...
	ODM_delay_ms(1);
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("[IQK]GNT_BT @S1 TXIQK = 0x%x\n", odm_get_bb_reg(p_dm,
				     REG_LTECOEX_READ_DATA, MASKDWORD)));
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("[IQK]0x948 @S1 TXIQK = 0x%x\n", odm_get_bb_reg(p_dm, 0x948, MASKDWORD)));
#endif
	...
	*/
	...
	/*save LOK result*/
	p_dm->rf_calibrate_info.lok_result = odm_get_rf_reg(p_dm, RF_PATH_A, 0x8,
					     0xfffff);
	...
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("[IQK]0xeac = 0x%x\n", reg_eac));
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("[IQK]0xe94 = 0x%x, 0xe9c = 0x%x\n", reg_e94, reg_e9c));
	/*monitor image power before & after IQK*/
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("[IQK]0xe90(before IQK)= 0x%x, 0xe98(afer IQK) = 0x%x\n",
		      odm_get_bb_reg(p_dm, 0xe90, MASKDWORD), odm_get_bb_reg(p_dm, 0xe98,
				      MASKDWORD)));
...
)
{
	...
	ODM_delay_ms(1);
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("[IQK]GNT_BT @S1 RXIQK1 = 0x%x\n", odm_get_bb_reg(p_dm,
				     REG_LTECOEX_READ_DATA, MASKDWORD)));
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("[IQK]0x948 @S1 RXIQK1 = 0x%x\n", odm_get_bb_reg(p_dm, 0x948,
				     MASKDWORD)));
#endif
	...
	*/
	...
	u4tmp = 0x80007C00 | (reg_e94 & 0x3FF0000)  | ((reg_e9c & 0x3FF0000) >>
			16);
	odm_set_bb_reg(p_dm, REG_TX_IQK, MASKDWORD, u4tmp);
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("[IQK]0xe40 = 0x%x u4tmp = 0x%x\n", odm_get_bb_reg(p_dm, REG_TX_IQK,
				     MASKDWORD), u4tmp));

	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("[IQK]path S1 RXIQK STEP2!!\n"));
	...
	ODM_delay_ms(1);
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("[IQK]GNT_BT @S1 RXIQK2 = 0x%x\n", odm_get_bb_reg(p_dm,
				     REG_LTECOEX_READ_DATA, MASKDWORD)));
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("[IQK]0x948 @S1 RXIQK2 = 0x%x\n", odm_get_bb_reg(p_dm, 0x948,
				     MASKDWORD)));
#endif
	...
	*/
	...
	/*reload LOK value*/
	odm_set_rf_reg(p_dm, RF_PATH_A, 0x8, RFREGOFFSETMASK,
		       p_dm->rf_calibrate_info.lok_result);
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("[LOK] reg rf 0x8 = 0x%x\n", odm_get_rf_reg(p_dm, RF_PATH_A, 0x8,
				     RFREGOFFSETMASK)));
	...
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("[IQK]0xeac = 0x%x\n", reg_eac));
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("[IQK]0xea4 = 0x%x, 0xeac = 0x%x\n", reg_ea4, reg_eac));

	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("[IQK]0xea0(before IQK)= 0x%x, 0xea8(afer IQK) = 0x%x\n",
		      odm_get_bb_reg(p_dm, 0xea0, MASKDWORD), odm_get_bb_reg(p_dm, 0xea8,
				      MASKDWORD)));
...
{
	u32 reg_eac, reg_e94, reg_e9c, reg_e94_s0, reg_e9c_s0, reg_ea4_s0,
	    reg_eac_s0, tmp, path_sel_bb;
	u8	result = 0x00, ktime;
	u32 original_path, original_gnt;
	...
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("path S0 TXIQK!\n"));
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("[IQK]0x67 @S0 TXIQK = 0x%x\n", odm_get_mac_reg(p_dm, 0x64, MASKBYTE3)));
	path_sel_bb = odm_get_bb_reg(p_dm, 0x948, MASKDWORD);

	odm_set_bb_reg(p_dm, 0x948, MASKDWORD,
		       0x99000280); /*10 od 0x948 0x1 [7] ; WL:S1 to S0;BT:S0 to S1;*/
	...
	ODM_delay_ms(1);
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("[IQK]GNT_BT @S0 TXIQK = 0x%x\n", odm_get_bb_reg(p_dm,
				     REG_LTECOEX_READ_DATA, MASKDWORD)));
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("[IQK]0x948 @S0 TXIQK = 0x%x\n", odm_get_bb_reg(p_dm, 0x948, MASKDWORD)));
	...
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("[IQK]0xeac_s0 = 0x%x\n", reg_eac_s0));
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("[IQK]0xe94_s0 = 0x%x, 0xe9c_s0 = 0x%x\n", reg_e94_s0, reg_e9c_s0));
	/*monitor image power before & after IQK*/
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
...
{
	u32 reg_e94, reg_e9c, reg_ea4, reg_eac, reg_e94_s0, reg_e9c_s0, reg_ea4_s0,
	    reg_eac_s0, tmp, u4tmp, path_sel_bb;
	u8 result = 0x00, ktime;
	u32 original_path, original_gnt;
	...
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("RF0x1 @S0 RXIQK1 = 0x%x\n", odm_get_rf_reg(p_dm, RF_PATH_A, 0x1,
				     RFREGOFFSETMASK)));
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("RF0x2 @S0 RXIQK1 = 0x%x\n", odm_get_rf_reg(p_dm, RF_PATH_A, 0x2,
				     RFREGOFFSETMASK)));
	...
	ODM_delay_ms(1);
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("[IQK]GNT_BT @S0 RXIQK1 = 0x%x\n", odm_get_bb_reg(p_dm,
				     REG_LTECOEX_READ_DATA, MASKDWORD)));
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("[IQK]0x948 @S0 RXIQK1 = 0x%x\n", odm_get_bb_reg(p_dm, 0x948,
				     MASKDWORD)));
	...
	u4tmp = 0x80007C00 | (reg_e94_s0 & 0x3FF0000)  | ((reg_e9c_s0 & 0x3FF0000)
			>> 16);
	odm_set_bb_reg(p_dm, REG_TX_IQK, MASKDWORD, u4tmp);
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("[IQK]0xe40_s0 = 0x%x u4tmp = 0x%x\n", odm_get_bb_reg(p_dm, REG_TX_IQK,
				     MASKDWORD), u4tmp));


	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("[IQK]path S0 RXIQK STEP2!!\n\n"));
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("[IQK]0x67 @S0 RXIQK2 = 0x%x\n", odm_get_mac_reg(p_dm, 0x64, MASKBYTE3)));
	...
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("RF0x1 @S0 RXIQK2 = 0x%x\n", odm_get_rf_reg(p_dm, RF_PATH_A, 0x1,
				     RFREGOFFSETMASK)));
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("RF0x2 @S0 RXIQK2 = 0x%x\n", odm_get_rf_reg(p_dm, RF_PATH_A, 0x2,
				     RFREGOFFSETMASK)));
	/*enter IQK mode*/
	odm_set_bb_reg(p_dm, REG_FPGA0_IQK, 0xffffff00, 0x808000);
	...
	ODM_delay_ms(1);
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("[IQK]GNT_BT @S0 RXIQK2 = 0x%x\n", odm_get_bb_reg(p_dm,
				     REG_LTECOEX_READ_DATA, MASKDWORD)));
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("[IQK]0x948 @S0 RXIQK2 = 0x%x\n", odm_get_bb_reg(p_dm, 0x948,
				     MASKDWORD)));
	...
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("[IQK]0xeac_s0 = 0x%x\n", reg_eac_s0));
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("[IQK]0xea4_s0 = 0x%x, 0xeac_s0 = 0x%x\n", reg_ea4_s0, reg_eac_s0));
...
)
{
	...
	s32	Y, TX1_C;
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("path S1 IQ Calibration %s !\n", (is_iqk_ok) ? "Success" : "Failed"));

	if (final_candidate == 0xFF)
		return;

	else if (is_iqk_ok) {
		oldval_1 = (odm_get_bb_reg(p_dm, REG_OFDM_0_XA_TX_IQ_IMBALANCE,
					   MASKDWORD) >> 22) & 0x3FF;
		...
		odm_set_bb_reg(p_dm, REG_OFDM_0_ECCA_THRESHOLD, BIT(31),
			       ((X * oldval_1 >> 7) & 0x1));
		...
		ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
			     ("Y = 0x%x, TX1_C = 0x%x\n", Y, TX1_C));
		odm_set_bb_reg(p_dm, REG_OFDM_0_XC_TX_AFE, 0xF0000000,
			       ((TX1_C & 0x3C0) >> 6));
		odm_set_bb_reg(p_dm, REG_OFDM_0_XA_TX_IQ_IMBALANCE, 0x003F0000,
			       (TX1_C & 0x3F));

		odm_set_bb_reg(p_dm, REG_OFDM_0_ECCA_THRESHOLD, BIT(29),
			       ((Y * oldval_1 >> 7) & 0x1));

		if (is_tx_only) {
			ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
				     ("_phy_path_s1_fill_iqk_matrix_8710b only Tx OK\n"));
			return;
		}
...
boolean phy_simularity_compare_8710b(
	struct PHY_DM_STRUCT		*p_dm,
	s32		result[][8],
	u8		 c1,
	u8		 c2
)
{
	...
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("===> IQK:phy_simularity_compare_8710b c1 %d c2 %d!!!\n", c1, c2));
	...
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("IQK:phy_simularity_compare_8710b simularity_bit_map   %x !!!\n",
		      simularity_bit_map));
...
)
{
	...
	if (t == 0) {
		_phy_save_adda_registers_8710b(p_dm, ADDA_REG,
					       p_dm->rf_calibrate_info.ADDA_backup, IQK_ADDA_REG_NUM);
		_phy_save_mac_registers_8710b(p_dm, IQK_MAC_REG,
					      p_dm->rf_calibrate_info.IQK_MAC_backup);
		_phy_save_adda_registers_8710b(p_dm, IQK_BB_REG_92C,
					       p_dm->rf_calibrate_info.IQK_BB_backup, IQK_BB_REG_NUM);
	}
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
...
#if 1
	if (t == 0)
		p_dm->rf_calibrate_info.is_rf_pi_enable = (u8)odm_get_bb_reg(p_dm,
				REG_FPGA0_XA_HSSI_PARAMETER1, BIT(8));
	...
	_phy_mac_setting_calibration_8710b(p_dm, IQK_MAC_REG,
					   p_dm->rf_calibrate_info.IQK_MAC_backup);
	...
	for (i = 0 ; i < retry_count ; i++) {
		...
		if (path_s1_ok == 0x01) {
			ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
				     ("path S1 Tx IQK Success!!\n"));
			result[t][0] = (odm_get_bb_reg(p_dm, REG_TX_POWER_BEFORE_IQK_A,
						       MASKDWORD) & 0x3FF0000) >> 16;
			result[t][1] = (odm_get_bb_reg(p_dm, REG_TX_POWER_AFTER_IQK_A,
						       MASKDWORD) & 0x3FF0000) >> 16;
			break;
		} else {
			ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
	...
	for (i = 0 ; i < retry_count ; i++) {
		...
		if (path_s1_ok == 0x03) {
			ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
				     ("path S1 Rx IQK Success!!\n"));
			result[t][2] = (odm_get_bb_reg(p_dm, REG_RX_POWER_BEFORE_IQK_A_2,
						       MASKDWORD) & 0x3FF0000) >> 16;
			result[t][3] = (odm_get_bb_reg(p_dm, REG_RX_POWER_AFTER_IQK_A_2,
						       MASKDWORD) & 0x3FF0000) >> 16;
			break;
		} else {
			ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
	...
	if (is2T) {
		...
		for (i = 0 ; i < retry_count ; i++) {
			...
			if (path_s0_ok == 0x01) {
				...
				result[t][0] = (odm_get_bb_reg(p_dm, REG_TX_POWER_BEFORE_IQK_B,
							       MASKDWORD) & 0x3FF0000) >> 16;
				result[t][1] = (odm_get_bb_reg(p_dm, REG_TX_POWER_AFTER_IQK_B,
							       MASKDWORD) & 0x3FF0000) >> 16;
			}
#endif
		}
	...
	if (t != 0) {
		_phy_reload_adda_registers_8710b(p_dm, ADDA_REG,
						 p_dm->rf_calibrate_info.ADDA_backup, IQK_ADDA_REG_NUM);
		/* Reload MAC parameters*/
		_phy_reload_mac_registers_8710b(p_dm, IQK_MAC_REG,
						p_dm->rf_calibrate_info.IQK_MAC_backup);
		_phy_reload_adda_registers_8710b(p_dm, IQK_BB_REG_92C,
						 p_dm->rf_calibrate_info.IQK_BB_backup, IQK_BB_REG_NUM);
...
)
{
	...
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("==>_phy_ap_calibrate_8710b() delta %d\n", delta));
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
	...
	* root cause, so we remove these actions temporarily. Added by tynli and SD3 Allen. 2010.05.31. */
	...
	_phy_save_adda_registers_8710b(p_adapter, AFE_REG, AFE_backup,
				       IQK_ADDA_REG_NUM);
#else
	_phy_save_mac_registers_8710b(p_dm, MAC_REG, MAC_backup);


	_phy_save_adda_registers_8710b(p_dm, AFE_REG, AFE_backup,
				       IQK_ADDA_REG_NUM);
#endif
	...
	for (path = 0; path < pathbound; path++) {
		...
		if (path == RF_PATH_A) {
			...
			for (index = 0; index < 11; index++) {
				...
				ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
					     ("_phy_ap_calibrate_8710b() offset 0x%x value 0x%x\n", offset,
					      odm_get_bb_reg(p_dm, offset, MASKDWORD)));
		...
		ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
			     ("_phy_ap_calibrate_8710b() offset 0xe70 %x\n", odm_get_bb_reg(p_dm,
		...
		if (path == 0) {
			for (index = 0; index < APK_BB_REG_NUM ; index++) {
				...
				if (index == 0)
					...
					odm_set_bb_reg(p_dm, BB_REG[index], MASKDWORD,
						       BB_backup[index] | BIT(10) | BIT(26));
				else
		...
		ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
			     ("_phy_ap_calibrate_8710b() offset 0x800 %x\n", odm_get_bb_reg(p_dm, 0x800,
					     MASKDWORD)));
		...
		for (index = 0; index < APK_BB_REG_NUM; index++) {
			...
			if (!p_dm->rf_calibrate_info.is_apk_thermal_meter_ignore) {
				...
				ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
					     ("_phy_ap_calibrate_8710b() APK index %d tmp_reg 0x%x delta_V %d delta_offset %d\n",
...
#if !(DM_ODM_SUPPORT_TYPE & ODM_AP)
			ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
				     ("_phy_ap_calibrate_8710b() offset 0xc %x\n", odm_get_rf_reg(p_dm, path,
						     RF_IPA_A, MASKDWORD)));
			odm_set_rf_reg(p_dm, (enum rf_path)path, RF_AC, MASKDWORD,
				       APK_RF_value_0[path][index]);
			ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
				     ("_phy_ap_calibrate_8710b() offset 0x0 %x\n", odm_get_rf_reg(p_dm, path,
						     RF_AC, MASKDWORD)));
			odm_set_rf_reg(p_dm, (enum rf_path)path, RF_TXBIAS_A, MASKDWORD, tmp_reg);
			ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
				     ("_phy_ap_calibrate_8710b() offset 0xd %x\n", odm_get_rf_reg(p_dm, path,
						     RF_TXBIAS_A, MASKDWORD)));
#else
			ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
				     ("_phy_ap_calibrate_8710b() offset 0xc %x\n", odm_get_rf_reg(p_dm, path,
						     RF_IPA_A, MASKDWORD)));
			...
			do {
				odm_set_bb_reg(p_dm, REG_FPGA0_IQK, 0xffffff00, 0x800000);
				{
					...
					ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
						     ("_phy_ap_calibrate_8710b() offset 0x%x value 0x%x\n", APK_offset[path],
						      odm_get_bb_reg(p_dm, APK_offset[path], MASKDWORD)));
					ODM_delay_ms(3);
				...
				ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
					     ("_phy_ap_calibrate_8710b() offset 0xbd8[25:21] %x\n", tmp_reg));
...
#if !(DM_ODM_SUPPORT_TYPE & ODM_AP)
	_phy_reload_adda_registers_8710b(p_adapter, AFE_REG, AFE_backup,
					 IQK_ADDA_REG_NUM);
#else
	_phy_reload_adda_registers_8710b(p_dm, AFE_REG, AFE_backup,
					 IQK_ADDA_REG_NUM);
#endif
	...
	for (path = 0; path < pathbound; path++) {
		...
		if (APK_result[path][1] > 6)
			APK_result[path][1] = 6;
		ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
			     ("apk path %d result %d 0x%x \t", path, 1, APK_result[path][1]));
	}
	...
	for (path = 0; path < pathbound; path++) {
		odm_set_rf_reg(p_dm, (enum rf_path)path, 0x3, MASKDWORD,
			       ((APK_result[path][1] << 15) | (APK_result[path][1] << 10) |
				(APK_result[path][1] << 5) | APK_result[path][1]));
		if (path == RF_PATH_A)
...
)
{
	...
	s32			rege94_s1, rege9c_s1, regea4_s1, regeac_s1, rege94_s0, rege9c_s0,
				regea4_s0, regeac_s0, reg_tmp = 0;
	...
	{
		ODM_RT_TRACE(p_dm, ODM_COMP_INIT, ODM_DBG_LOUD,
			     ("phy_iq_calibrate_8710b: Return due to is_recovery!\n"));
		_phy_reload_adda_registers_8710b(p_dm, IQK_BB_REG_92C,
						 p_dm->rf_calibrate_info.IQK_BB_backup_recover, 9);
		return;
	}
	...
	for (i = 0; i < 4; i++) {
		...
		ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
			     ("[IQK] rege94_s1=%x rege9c_s1=%x regea4_s1=%x regeac_s1=%x rege94_s0=%x rege9c_s0=%x regea4_s0=%x regeac_s0=%x\n ",
			      rege94_s1, rege9c_s1, regea4_s1, regeac_s1, rege94_s0, rege9c_s0,
			      regea4_s0, regeac_s0));
	}
	...
	if (rege94_s1 != 0)
		_phy_path_s1_fill_iqk_matrix_8710b(p_dm, is_path_s1_ok, result,
						   final_candidate, (regea4_s1 == 0));
	if (rege94_s0 != 0)
		_phy_path_s0_fill_iqk_matrix_8710b(p_dm, is_path_s0_ok, result,
						   final_candidate, (regea4_s0 == 0));

	iqk_fail_a = p_dm->n_iqk_fail_cnt;
	if (iqk_fail_a - iqk_fail_b > 0)
		RT_TRACE(COMP_INIT, DBG_LOUD,
			 ("[8710bIQK]n_iqk_fail_cnt+,IQK restore to default value !\n"));
	...
	if (final_candidate < 4) {
		for (i = 0; i < iqk_matrix_reg_num; i++)
			p_dm->rf_calibrate_info.iqk_matrix_reg_setting[indexforchannel].value[0][i]
				= result[final_candidate][i];
		p_dm->rf_calibrate_info.iqk_matrix_reg_setting[indexforchannel].is_iqk_done
			= true;
	}
	...
	_phy_save_adda_registers_8710b(p_dm, IQK_BB_REG_92C,
				       p_dm->rf_calibrate_info.IQK_BB_backup_recover, IQK_BB_REG_NUM);
...
)
{
	...
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("_phy_digital_predistortion_8710b for %s\n", (is2T ? "2T2R" : "1T1R")));
...
#if !(DM_ODM_SUPPORT_TYPE & ODM_AP)
			RF_backup[path][index] = odm_get_rf_reg(p_dm, path, RF_REG[index],
								MASKDWORD);
#else
			RF_backup[path][index] = odm_get_rf_reg(p_dm, path, RF_REG[index],
								MASKDWORD);
	...
	while (retry_count < DP_RETRY_LIMIT
	       && !p_dm->rf_calibrate_info.is_dp_path_aok) {
	...
	if (is2T) {
		...
		while (retry_count < DP_RETRY_LIMIT
		       && !p_dm->rf_calibrate_info.is_dp_path_bok) {
