...
)
{
	...
	odm_set_rf_reg(p_dm, RF_PATH_A, 0xef, 0x80000,
		       0x1);	/*RF mode table write enable*/
	odm_set_rf_reg(p_dm, RF_PATH_B, 0xef, 0x80000,
		       0x1);	/*RF mode table write enable*/
	...
	if (p_beam_info->beamformee_su_cnt > 0) {
		...
		odm_set_rf_reg(p_dm, RF_PATH_A, 0x32, 0xfffff,
		...
		odm_set_rf_reg(p_dm, RF_PATH_B, 0x32, 0xfffff,
			       0xE26BF);	/*Enable TXIQGEN in RX mode*/
	} else {
		/* Paath_A */
		odm_set_rf_reg(p_dm, RF_PATH_A, 0x30, 0x78000, 0x3);		/*Select RX mode*/
		odm_set_rf_reg(p_dm, RF_PATH_A, 0x31, 0xfffff, 0x3F7FF);	/*Set Table data*/
		odm_set_rf_reg(p_dm, RF_PATH_A, 0x32, 0xfffff,
			       0xC26BF);	/*Disable TXIQGEN in RX mode*/
		/* Path_B */
		odm_set_rf_reg(p_dm, RF_PATH_B, 0x30, 0x78000, 0x3);		/*Select RX mode*/
		odm_set_rf_reg(p_dm, RF_PATH_B, 0x31, 0xfffff, 0x3F7FF);	/*Set Table data*/
		odm_set_rf_reg(p_dm, RF_PATH_B, 0x32, 0xfffff,
...
)
{
	...
	struct _RT_BEAMFORMEE_ENTRY	*p_beam_entry = p_beam_info->beamformee_entry +
				idx;
	...
	*p_dm->p_is_fw_dw_rsvd_page_in_progress = true;
	...
	phydm_get_hal_def_var_handler_interface(p_dm, HAL_DEF_TX_PAGE_BOUNDARY,
						(u8 *)&tx_page_bndy);
	...
	odm_write_1byte(p_dm, REG_FWHW_TXQ_CTRL_8812A + 2,
			tmp_reg422 & (~BIT(6)));
	...
	do {
		...
		if (p_beam_entry->beamform_entry_cap & BEAMFORMER_CAP_VHT_SU)
			beamforming_send_vht_ndpa_packet(p_dm, p_beam_entry->mac_addr,
							 p_beam_entry->aid, p_beam_entry->sound_bw, BEACON_QUEUE);
		else
			beamforming_send_ht_ndpa_packet(p_dm, p_beam_entry->mac_addr,
...
)
{
	...
	for (idx = 0; idx < BEAMFORMEE_ENTRY_NUM; idx++) {
		...
		if (p_beam_info->beamformee_entry[idx].is_used
		    && p_beam_info->beamformee_entry[idx].beamform_entry_state ==
	...
	PHYDM_DBG(p_dm, DBG_TXBF,
		  ("[%s] PageNum0 = %d period0 = %d, PageNum1 = %d period1 %d\n", __func__,
		   PageNum0, period0, PageNum1, period1));
}
...
)
{
	...
	if (p_dm->rf_type == RF_2T2R)
		odm_set_bb_reg(p_dm, ODM_REG_CSI_CONTENT_VALUE, MASKDWORD,
			       0x00000000);	/*nc =2*/
	else
		odm_set_bb_reg(p_dm, ODM_REG_CSI_CONTENT_VALUE, MASKDWORD,
			       0x01081008);	/*nc =1*/

	if ((p_beamforming_info->beamformer_su_cnt > 0)
		...
		if (bfer_idx == 0) {
			for (i = 0; i < 6 ; i++)
				odm_write_1byte(p_dm, (REG_BFMER0_INFO_8812A + i),
			...
			for (i = 0; i < 6 ; i++)
				odm_write_1byte(p_dm, (REG_BFMER1_INFO_8812A + i),
						beamformer_entry.mac_addr[i]);
			/*CSI report use legacy ofdm so don't need to fill P_AID.*/
	...
	if ((p_beamforming_info->beamformee_su_cnt > 0)
		...
		if (bfee_idx == 0) {
			...
			odm_write_1byte(p_dm, REG_TXBF_CTRL_8812A + 3, odm_read_1byte(p_dm,
					REG_TXBF_CTRL_8812A + 3) | BIT(4) | BIT(6) | BIT(7));
		} else
			odm_write_2byte(p_dm, REG_TXBF_CTRL_8812A + 2,
					sta_id | BIT(12) | BIT(14) | BIT(15));
...
)
{
	...
	if (beamformee_entry.beamform_entry_cap == BEAMFORMING_CAP_NONE) {
		...
		if (idx == 0) {
			...
			odm_write_2byte(p_dm, REG_TXBF_CTRL_8812A + 2, odm_read_2byte(p_dm,
					REG_TXBF_CTRL_8812A + 2) & 0xF000);
			odm_write_2byte(p_dm, REG_BFMEE_SEL_8812A + 2, odm_read_2byte(p_dm,
					REG_BFMEE_SEL_8812A + 2) & 0x60);
		}
	}
