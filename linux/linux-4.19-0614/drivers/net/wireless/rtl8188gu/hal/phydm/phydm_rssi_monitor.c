...
)
{
	...
	h2c_val[3] = is_rx | (stbc_en << 1) | ((p_dm->noisy_decision & 0x1) << 2)
		     | (bf_en << 6);
	...
	if (p_dm->support_ic_type == ODM_RTL8188E)
		odm_ra_set_rssi_8188e(p_dm, (u8)(p_sta->mac_id & 0xFF),
				      p_sta->rssi_stat.rssi & 0x7F);
	else
#endif
	{
...
)
{
	...
	if ((!p_mgnt_info->bMediaConnect) ||
	    (act_as_ap
	     && (p_hal_data->EntryMinUndecoratedSmoothedPWDB == 0))) {/* We should check AP mode and Entry info.into consideration, revised by Roger, 2013.10.18*/
	...
	if (p_mgnt_info->bMediaConnect) {	/* Default port*/
		...
		if (act_as_ap || p_mgnt_info->mIbss) {
			p_hal_data->MinUndecoratedPWDBForDM =
				p_hal_data->EntryMinUndecoratedSmoothedPWDB;
			/**/
		} else {
...
)
{
	...
	while (p_loop_adapter) {
		...
		if (p_loop_adapter != NULL) {
			...
			cur_tx_ok_cnt = p_loop_adapter->TxStats.NumTxBytesUnicast -
					p_mgnt_info->lastTxOkCnt;
			cur_rx_ok_cnt = p_loop_adapter->RxStats.NumRxBytesUnicast -
					p_mgnt_info->lastRxOkCnt;
		...
		for (i = 0; i < ASSOCIATE_ENTRY_NUM; i++) {
			...
			if (p_entry != NULL) {
				if (p_entry->bAssociated) {
					...
					RT_DISP_ADDR(FDM, DM_PWDB, ("p_entry->mac_addr ="),
...
#if (BEAMFORMING_SUPPORT == 1)
#ifndef BEAMFORMING_VERSION_1
					beamform_cap = phydm_beamforming_get_entry_beam_cap_by_mac_id(p_dm,
							GET_STA_INFO(p_entry).mac_id);
					...
					if ((IS_WIRELESS_MODE_AC(adapter)
					     && TEST_FLAG(p_entry->VHTInfo.STBC, STBC_VHT_ENABLE_TX)) ||
					...
					h2c_parameter[4] = (p_ra_table->RA_threshold_offset & 0x7f) |
							   (p_ra_table->RA_offset_direction << 7);
	...
	if (p_hal_data->bUseRAMask) {
		...
		PlatformEFIOWrite1Byte(adapter, 0x4fe,
				       (u8)p_hal_data->UndecoratedSmoothedPWDB);

	{
		...
		while (p_loop_adapter) {
			local_rssi_min = phydm_find_minimum_rssi(p_dm, p_loop_adapter,
					 p_is_link_temp);
...
)
{
	...
	p_ra_table->PT_collision_pre =
		true;	/*used in odm_dynamic_arfb_select(WIN only)*/
