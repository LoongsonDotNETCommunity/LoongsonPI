...
void phydm_dig_up_bound_lmt_en(
	void			*p_dm_void
)
{
	struct PHY_DM_STRUCT		*p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct phydm_adaptivity_struct	*p_adaptivity = (struct
			phydm_adaptivity_struct *)phydm_get_structure(p_dm, PHYDM_ADAPTIVITY);
...
void phydm_check_adaptivity(
	void			*p_dm_void
)
{
	struct PHY_DM_STRUCT		*p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct phydm_adaptivity_struct	*adaptivity = (struct
			phydm_adaptivity_struct *)phydm_get_structure(p_dm, PHYDM_ADAPTIVITY);
...
#if (DM_ODM_SUPPORT_TYPE == ODM_WIN)
boolean phydm_check_channel_plan(
	void			*p_dm_void
)
{
	struct PHY_DM_STRUCT		*p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	...
	if (p_mgnt_info->RegEnableAdaptivity == 2) {
		if (p_dm->carrier_sense_enable ==
			...
			if ((*p_dm->p_band_type == ODM_BAND_5G) &&
			    !(p_dm->odm_regulation_5g == REGULATION_ETSI
			      || p_dm->odm_regulation_5g == REGULATION_WW)) {
				PHYDM_DBG(p_dm, DBG_ADPTVTY, ("adaptivity skip 5G domain code : %d\n",
							      p_dm->odm_regulation_5g));
				p_dm->adaptivity_enable = false;
				return true;
			} else if ((*p_dm->p_band_type == ODM_BAND_2_4G) &&
				   !(p_dm->odm_regulation_2_4g == REGULATION_ETSI
				     || p_dm->odm_regulation_2_4g == REGULATION_WW)) {
			...
			if ((*p_dm->p_band_type == ODM_BAND_5G) &&
			    !(p_dm->odm_regulation_5g == REGULATION_MKK
			      || p_dm->odm_regulation_5g == REGULATION_WW)) {
				...
				return true;
				 ...
				 !(p_dm->odm_regulation_2_4g == REGULATION_MKK
				   || p_dm->odm_regulation_2_4g == REGULATION_WW)) {
...
void phydm_set_edcca_threshold(
	void	*p_dm_void,
	s8	H2L,
	s8	L2H
)
{
	...
	if (p_dm->support_ic_type & ODM_IC_11N_SERIES)
		odm_set_bb_reg(p_dm, REG_OFDM_0_ECCA_THRESHOLD, MASKBYTE2 | MASKBYTE0,
...
void phydm_set_lna(
	...
	} else if (p_dm->support_ic_type & ODM_RTL8723B)
	{
		if (type == phydm_disable_lna) {
			...
			odm_set_rf_reg(p_dm, RF_PATH_A, 0x43, 0xfffff,
...
void phydm_set_trx_mux(
	...
	if (p_dm->support_ic_type & ODM_IC_11N_SERIES)
	{
		odm_set_bb_reg(p_dm, ODM_REG_CCK_RPT_FORMAT_11N, BIT(3) | BIT(2) | BIT(1),
			       tx_mode);			/*set TXmod to standby mode to remove outside noise affect*/
		odm_set_bb_reg(p_dm, ODM_REG_CCK_RPT_FORMAT_11N,
			       BIT(22) | BIT(21) | BIT(20),
			       rx_mode);		/*set RXmod to standby mode to remove outside noise affect*/
		if (p_dm->rf_type > RF_1T1R) {
			odm_set_bb_reg(p_dm, ODM_REG_CCK_RPT_FORMAT_11N_B,
				       BIT(3) | BIT(2) | BIT(1),
				       tx_mode);		/*set TXmod to standby mode to remove outside noise affect*/
			odm_set_bb_reg(p_dm, ODM_REG_CCK_RPT_FORMAT_11N_B,
				       BIT(22) | BIT(21) | BIT(20),
	...
	{
		odm_set_bb_reg(p_dm, ODM_REG_TRMUX_11AC,
			       BIT(11) | BIT(10) | BIT(9) | BIT(8),
			       tx_mode);				/*set TXmod to standby mode to remove outside noise affect*/
		odm_set_bb_reg(p_dm, ODM_REG_TRMUX_11AC, BIT(7) | BIT(6) | BIT(5) | BIT(4),
			       rx_mode);				/*set RXmod to standby mode to remove outside noise affect*/
		if (p_dm->rf_type > RF_1T1R) {
			odm_set_bb_reg(p_dm, ODM_REG_TRMUX_11AC_B,
				       BIT(11) | BIT(10) | BIT(9) | BIT(8),
				       tx_mode);		/*set TXmod to standby mode to remove outside noise affect*/
			odm_set_bb_reg(p_dm, ODM_REG_TRMUX_11AC_B,
...
void phydm_search_pwdb_lower_bound(
	void		*p_dm_void
)
{
	struct PHY_DM_STRUCT		*p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	...
	if (p_dm->support_ic_type & (ODM_RTL8723B | ODM_RTL8188E | ODM_RTL8192E |
	...
	while (is_adjust) {
		...
		for (cnt = 0; cnt < 20; cnt++) {
			...
			if (value32 & BIT(30)
			    && (p_dm->support_ic_type & (ODM_RTL8723B | ODM_RTL8188E)))
...
boolean phydm_re_search_condition(
	void				*p_dm_void
)
{
	struct PHY_DM_STRUCT		*p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
...
void phydm_adaptivity_info_init(
	void				*p_dm_void,
...
)
{
	...
	adaptivity->adaptivity_dbg_port = (p_dm->support_ic_type &
					   ODM_IC_11N_SERIES) ? 0x208 : 0x209;
	...
	if (p_dm->support_ic_type & ODM_IC_11N_GAIN_IDX_EDCCA) {
		...
		if (p_dm->support_ic_type & ODM_RTL8197F) {
			odm_set_bb_reg(p_dm, ODM_REG_PAGE_B1_97F, BIT(30),
				       0x1);								/*set to page B1*/
			odm_set_bb_reg(p_dm, ODM_REG_EDCCA_DCNF_97F, BIT(27) | BIT(26),
				       0x1);		/*0:rx_dfir, 1: dcnf_out, 2 :rx_iq, 3: rx_nbi_nf_out*/
			odm_set_bb_reg(p_dm, ODM_REG_PAGE_B1_97F, BIT(30), 0x0);
		} else
			odm_set_bb_reg(p_dm, ODM_REG_EDCCA_DCNF_11N, BIT(21) | BIT(20),
				       0x1);		/*0:rx_dfir, 1: dcnf_out, 2 :rx_iq, 3: rx_nbi_nf_out*/
	}
#if (RTL8195A_SUPPORT == 0)
	if (p_dm->support_ic_type &
		...
		odm_set_bb_reg(p_dm, ODM_REG_ACBB_EDCCA_ENHANCE, BIT(29) | BIT(28),
			       0x1);		/*0:rx_dfir, 1: dcnf_out, 2 :rx_iq, 3: rx_nbi_nf_out*/
	}

	if (!(p_dm->support_ic_type & (ODM_IC_11AC_GAIN_IDX_EDCCA |
	...
	adaptivity->adajust_igi_level = (u8)((p_dm->th_l2h_ini + igi_target) -
					     pwdb_upper_bound + dfir_loss);	/*IGI = L2H - PwdB - dfir_loss*/

	/*Check this later on Windows*/
	/*phydm_set_edcca_threshold_api(p_dm, p_dig_t->cur_ig_value);*/

	p_dm->adaptivity_flag = (p_dm->support_ic_type & ODM_IC_GAIN_IDX_EDCCA) ?
				false : true;
...
)
{
	...
	p_adapter->HalFunc.GetHwRegHandler(p_adapter, HW_VAR_FW_PSMODE_STATUS,
					   (u8 *)(&is_fw_current_in_ps_mode));
	...
	if ((!(p_dm->support_ability & ODM_BB_ADAPTIVITY))
		...
		if (p_dm->p_soft_ap_mode != NULL) {
			if (*(p_dm->p_soft_ap_mode) != 0
			    && (p_dm->soft_ap_special_setting & BIT(0)))
				disable_ap_adapt_setting = true;
			PHYDM_DBG(p_dm, DBG_ADPTVTY,
				  ("p_dm->soft_ap_special_setting = %x, *(p_dm->p_soft_ap_mode) = %d, disable_ap_adapt_setting = %d\n",
				   p_dm->soft_ap_special_setting, *(p_dm->p_soft_ap_mode),
				   disable_ap_adapt_setting));
		}
		if (phydm_check_channel_plan(p_dm)
		    || (p_dm->ap_total_num > adaptivity->ap_num_th)
		    || disable_ap_adapt_setting) {
			p_dm->th_l2h_ini = p_dm->th_l2h_ini_mode2;
			p_dm->th_edcca_hl_diff = p_dm->th_edcca_hl_diff_mode2;
		} else {
	...
	PHYDM_DBG(p_dm, DBG_ADPTVTY,
		  ("band_width=%s, igi_target=0x%x, dynamic_link_adaptivity = %d\n",
		   (*p_dm->p_band_width == CHANNEL_WIDTH_80) ? "80M" : ((
					   *p_dm->p_band_width == CHANNEL_WIDTH_40) ? "40M" : "20M"), igi_target,
		   adaptivity->dynamic_link_adaptivity));
	PHYDM_DBG(p_dm, DBG_ADPTVTY,
		  ("adajust_igi_level= 0x%x, adaptivity_flag = %d, adaptivity_enable = %d\n",
		   adaptivity->adajust_igi_level, p_dm->adaptivity_flag,
		   p_dm->adaptivity_enable));

	if ((adaptivity->dynamic_link_adaptivity == true) && (!p_dm->is_linked)
	    && (p_dm->adaptivity_enable == false)) {
...
)
{
	...
	if (is_pasue_edcca) {
		...
		/*Disable EDCCA*/
#if (DM_ODM_SUPPORT_TYPE == ODM_WIN)
		if (odm_is_work_item_scheduled(&(adaptivity->phydm_pause_edcca_work_item))
		    == false)
		...
		/*Resume EDCCA*/
#if (DM_ODM_SUPPORT_TYPE == ODM_WIN)
		if (odm_is_work_item_scheduled(&(adaptivity->phydm_resume_edcca_work_item))
		    == false)
...
)
{
	...
	if (p_dm->support_ic_type & ODM_IC_11N_SERIES)
		odm_set_bb_reg(p_dm, REG_OFDM_0_ECCA_THRESHOLD, MASKBYTE2 | MASKBYTE0,
			       (u32)((u8)adaptivity->backup_l2h | (u8)adaptivity->backup_h2l << 16));
#if (RTL8195A_SUPPORT == 0)
	else if (p_dm->support_ic_type & ODM_IC_11AC_SERIES)
		odm_set_bb_reg(p_dm, REG_FPGA0_XB_LSSI_READ_BACK, MASKLWORD,
			       (u16)((u8)adaptivity->backup_l2h | (u8)adaptivity->backup_h2l << 8));
#endif
...
)
{
	...
	if (dm_value[0] == PHYDM_ADAPT_DEBUG) {
		...
		if (p_dm->support_ic_type & ODM_IC_11N_SERIES) {
			...
			h2l_diff = (s8)(0x000000ff & reg_value32) - (s8)((0x00ff0000 & reg_value32)
					>> 16);
		}
#if (RTL8195A_SUPPORT == 0)
		else if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
			reg_value32 = odm_get_bb_reg(p_dm, 0x8a4, MASKDWORD);
			h2l_diff = (s8)(0x000000ff & reg_value32) - (s8)((0x0000ff00 & reg_value32)
					>> 8);
		}
#endif
		if (h2l_diff == 7)
...
void phydm_set_forgetting_factor(
	void		*p_dm_void
)
{
	struct PHY_DM_STRUCT		*p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
