...
)
{
	...
	if (!(p_dm->support_ic_type & (ODM_RTL8814A | ODM_IC_11N_SERIES |
				       ODM_RTL8822B)))
		return;
#endif
...
)
{
	...
	if (p_dm->support_ic_type == ODM_RTL8188E
	    ...
	    || p_dm->support_ic_type == ODM_RTL8710B) {
		...
		bb_swing = PHY_GetTxBBSwing_8812A(adapter, p_hal_data->CurrentBandType,
						  RF_PATH_A);
	...
	if (p_rf_calibrate_info->default_bb_swing_index_flag != true) {
		...
		if (p_dm->support_ic_type == ODM_RTL8188E
			...
			p_rf_calibrate_info->default_ofdm_index = (default_swing_index >=
					OFDM_TABLE_SIZE) ? 30 : default_swing_index;
			p_rf_calibrate_info->default_cck_index = (default_cck_swing_index >=
					CCK_TABLE_SIZE) ? 20 : default_cck_swing_index;
			...
			p_rf_calibrate_info->default_ofdm_index = (default_swing_index >=
					TXSCALE_TABLE_SIZE) ? 24 : default_swing_index;
