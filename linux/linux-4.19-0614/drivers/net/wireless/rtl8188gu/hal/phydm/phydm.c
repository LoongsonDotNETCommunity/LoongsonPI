...
void phydm_traffic_load_decision(
	void	*p_dm_void
)
{
	...
	p_dm->cur_tx_ok_cnt =  *(p_dm->p_num_tx_bytes_unicast) -
			       p_dm->last_tx_ok_cnt;
	p_dm->cur_rx_ok_cnt =  *(p_dm->p_num_rx_bytes_unicast) -
			       p_dm->last_rx_ok_cnt;
	p_dm->last_tx_ok_cnt =  *(p_dm->p_num_tx_bytes_unicast);
	p_dm->last_rx_ok_cnt =  *(p_dm->p_num_rx_bytes_unicast);

	bit_shift_num = 17 + (PHYDM_WATCH_DOG_PERIOD -
			      1); /*AP:  <<3(8bit), >>20(10^6,M), >>0(1sec)*/
	/*WIN&CE:  <<3(8bit), >>20(10^6,M), >>1(2sec)*/

	p_dm->tx_tp = ((p_dm->tx_tp) >> 1) + (u32)(((p_dm->cur_tx_ok_cnt) >>
			bit_shift_num) >> 1);
	p_dm->rx_tp = ((p_dm->rx_tp) >> 1) + (u32)(((p_dm->cur_rx_ok_cnt) >>
			bit_shift_num) >> 1);
	...
	if (p_dm->cur_tx_ok_cnt > 1875000
	    || p_dm->cur_rx_ok_cnt > 1875000) {		/* ( 1.875M * 8bit ) / 2sec= 7.5M bits /sec )*/

		p_dm->traffic_load = TRAFFIC_HIGH;
		/**/
	} else if (p_dm->cur_tx_ok_cnt > 500000
		   || p_dm->cur_rx_ok_cnt >
		   500000) { /*( 0.5M * 8bit ) / 2sec =  2M bits /sec )*/

		p_dm->traffic_load = TRAFFIC_MID;
		/**/
	} else if (p_dm->cur_tx_ok_cnt > 100000
		   || p_dm->cur_rx_ok_cnt >
...
)
{
	...
	p_dm->is_cck_high_power = (boolean) odm_get_bb_reg(p_dm,
				  ODM_REG(CCK_RPT_FORMAT, p_dm), ODM_BIT(CCK_RPT_FORMAT, p_dm));
	...
	if (p_dm->support_ic_type & (ODM_RTL8703B | ODM_RTL8723D | ODM_RTL8710B)) {
		...
		p_dm->cck_agc_report_type = odm_get_bb_reg(p_dm, 0x950,
					    BIT(11)) ? 1 : 0; /*1: 4bit LNA, 0: 3bit LNA */
	...
	if (p_dm->support_ic_type & ODM_RTL8821C) {
		p_dm->cck_new_agc = odm_get_bb_reg(p_dm, 0xa9c,
						   BIT(17)) ? true : false;          /*1: new agc  0: old agc*/
	...
	if (p_dm->support_ic_type & (ODM_RTL8723D | ODM_RTL8822B | ODM_RTL8197F |
				     ODM_RTL8710B))
		p_dm->cck_new_agc = odm_get_bb_reg(p_dm, 0xa9c,
						   BIT(17)) ? true : false;          /*1: new agc  0: old agc*/
	else
...
void phydm_init_hw_info_by_rfe(
	struct PHY_DM_STRUCT		*p_dm
)
{
...
void phydm_common_info_self_init(
	struct PHY_DM_STRUCT		*p_dm
)
{
	phydm_init_cck_setting(p_dm);
	p_dm->rf_path_rx_enable = (u8) odm_get_bb_reg(p_dm, ODM_REG(BB_RX_PATH,
				  p_dm), ODM_BIT(BB_RX_PATH, p_dm));
...
void phydm_cmn_sta_info_update(
	void	*p_dm_void,
	u8	macid
)
{
...
void phydm_common_info_self_update(
	struct PHY_DM_STRUCT		*p_dm
)
{
	...
	if (sta_cnt == 1) {
		...
		PHYDM_DBG(p_dm, DBG_COMMON_FLOW,
			  ("one_entry_tp=((%d)), pre_one_entry_tp=((%d))\n",
			   p_dm->one_entry_tp, p_dm->pre_one_entry_tp));

		if ((p_dm->one_entry_tp > p_dm->pre_one_entry_tp)
		    && (p_dm->pre_one_entry_tp <= 2)) {
...
void phydm_common_info_self_reset(
	struct PHY_DM_STRUCT		*p_dm
)
{
...
#if (DM_ODM_SUPPORT_TYPE & (ODM_CE))
u64 phydm_supportability_init_ce(
	void		*p_dm_void
)
{
	struct PHY_DM_STRUCT		*p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
...
void phydm_fwoffload_ability_clear(
	struct PHY_DM_STRUCT		*p_dm,
	enum phydm_offload_ability	offload_ability
) {
...
void phydm_supportability_init(
	void		*p_dm_void
)
{
	struct PHY_DM_STRUCT		*p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
...
)
{
	...
	odm_antenna_diversity(
		p_dm); /*enable AntDiv in PS mode, request from SD4 Jeff*/
#endif
	phydm_common_info_self_reset(p_dm);
#endif
}

void phydm_watchdog_mp(
	struct PHY_DM_STRUCT		*p_dm
)
{
#ifdef CONFIG_DYNAMIC_RX_PATH
...
)
{
	...
	PHYDM_DBG(p_dm, ODM_COMP_API, ("[%s][%s] LV=%d, Len=%d\n", __func__,
				       ((pause_type == PHYDM_PAUSE) ? "Pause" : "Resume"),  pause_lv,
				       val_lehgth));
	...
	if (pause_func == F00_DIG) {
		...
		p_dm->phydm_func_handler.pause_phydm_handler =
			phydm_set_dig_val; /*function pointer hook*/
		...
		if (pause_func == F05_CCK_PD) {
			...
			p_dm->phydm_func_handler.pause_phydm_handler =
				phydm_set_cckpd_val; /*function pointer hook*/
			...
			if (pause_func == F06_ANT_DIV) {
				...
				p_dm->phydm_func_handler.pause_phydm_handler =
					phydm_set_antdiv_val; /*function pointer hook*/
				...
				if (pause_func == F13_ADPTVTY) {
					...
					p_dm->phydm_func_handler.pause_phydm_handler =
						phydm_set_edcca_val; /*function pointer hook*/
...
)
{
	...
	if ((strcmp(input[1], help) == 0)) {
		PHYDM_SNPRINTF((output + used, out_len - used,
				"{Func} {1:pause, 2:Resume} {lv} Val[5:0]\n"));
		...
		if (val_length != 0) {
			...
			PHYDM_SNPRINTF((output + used, out_len - used,
					"{%s, lv=%d} val = %d, %d}\n",
					((pause_type == PHYDM_PAUSE) ? "Pause" : "Resume"), pause_lv, var1[3],
					var1[4]));

			set_result = phydm_pause_func(p_dm, func, pause_type, pause_lv, val_length,
						      val_buf);
		}
...
u8 phydm_stop_dm_watchdog_check(
	void					*p_dm_void
)
{
	struct PHY_DM_STRUCT			*p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
...
)
{
	...
	odm_initialize_timer(p_dm,
			     &p_dm->beamforming_info.txbf_info.txbf_fw_ndpa_timer,
...
#define DTC_DWN_BASE       (DTC_BASE-5)	/* RSSI lower than this value, start to increase TX power */
	...
	PHYDM_DBG(p_dm, ODM_COMP_PWR_TRAIN,
...
)
{
	...
	/*DC_Cancellation*/
	odm_set_bb_reg(p_dm, 0xa9c, BIT(20),
		       0x1); /*DC compensation to CCK data path*/
	if (p_dm->support_ic_type & (ODM_RTL8188F | ODM_RTL8710B)) {
		...
		offset_i_hex[0] = (offset_i_hex[0] >= 0x200) ? (0x400 - offset_i_hex[1])
				  : (0x1ff - offset_i_hex[1]);
		offset_q_hex[0] = (offset_q_hex[0] >= 0x200) ? (0x400 - offset_q_hex[1])
...
)
{
	...
	if ((p_dm->support_ic_type & ODM_RTL8188E
	     && ((bb_regf0 & 0xf000) >> 12) < 8) ||
	    p_dm->support_ic_type & ODM_RTL8192E) { /*8188E_T version*/
		if (p_dm->consecutive_idlel_time > 10 && *p_dm->p_mp_mode == false
			...
			if ((bw == CHANNEL_WIDTH_20) && (channel == 1)) {
				...
				p_dm->is_receiver_blocking_en = true;
			} else if (p_dm->is_receiver_blocking_en && channel != 1
				   && channel != 13) {
		    ...
		    && ((bb_regf0 & 0xf000) >> 12) >= 8)) { /*8188E_S version*/
		if (p_dm->consecutive_idlel_time > 10 && *p_dm->p_mp_mode == false
		    && p_dm->adaptivity_enable == true) {
