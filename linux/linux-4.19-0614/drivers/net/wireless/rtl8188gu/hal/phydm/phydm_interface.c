...
)
{
	...
	PlatformInitializeTimer(adapter, p_timer,
				(RT_TIMER_CALL_BACK)call_back_func, p_context, sz_id);
#endif
}
...
)
{
	...
	case ODM_H2C_RSSI_REPORT:
		...
		if (p_dm->support_ic_type == ODM_RTL8881A
		    ...
		    || p_dm->support_ic_type & PHYDM_IC_3081_SERIES)
...
#endif
	{
		GET_HAL_INTERFACE(p_dm->priv)->fill_h2c_cmd_handler(p_dm->priv, h2c_id,
	...
	case PHYDM_C2H_IQK_FINISH:
		...
		if (p_dm->support_ic_type & (ODM_RTL8812 | ODM_RTL8821)) {
			...
			p_dm->rf_calibrate_info.iqk_progressing_time = odm_get_progressing_time(
						p_dm, p_dm->rf_calibrate_info.iqk_start_time);
		}
...
#elif (DM_ODM_SUPPORT_TYPE & ODM_CE)
	phy_set_tx_power_index_by_rate_section(p_dm->adapter, path, Channel,
					       RateSection);
#endif
}
...
)
{
	...
	return PHY_GetTxPowerIndex(p_dm->adapter, path, tx_rate,
	...
	return phy_get_tx_power_index(adapter, (enum rf_path)path, tx_rate,
				      band_width, Channel);
#elif (DM_ODM_SUPPORT_TYPE & ODM_CE)
	return phy_get_tx_power_index(p_dm->adapter, path, tx_rate, band_width,
				      Channel);
#endif
}
