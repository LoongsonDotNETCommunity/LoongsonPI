...
)
{
	...
	/* JJ ADD 20161014 */
	if (p_dm->support_ic_type & (ODM_RTL8703B | ODM_RTL8723D |
				     ODM_RTL8710B))	/*for Xtal Offset*/
		(*c.get_delta_swing_xtal_table)(p_dm, (s8 **)&delta_swing_table_xtal_up,
	...
	p_rf_calibrate_info->thermal_value_avg[p_rf_calibrate_info->thermal_value_avg_index]
		= thermal_value;
	...
	if (delta > 0 && p_rf_calibrate_info->txpowertrack_control) {
		...
		delta = thermal_value > p_hal_data->eeprom_thermal_meter ?
			(thermal_value - p_hal_data->eeprom_thermal_meter) :
			(p_hal_data->eeprom_thermal_meter - thermal_value);
#else
		...
		if (thermal_value > p_dm->priv->pmib->dot11RFEntry.ther) {
			...
			for (p = RF_PATH_A; p < c.rf_path_count; p++) {
				p_rf_calibrate_info->delta_power_index_last[p] =
					p_rf_calibrate_info->delta_power_index[p];	/*recording poer index offset*/
				switch (p) {
				case RF_PATH_B:
					ODM_RT_TRACE(p_dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD,
			...
			for (p = RF_PATH_A; p < c.rf_path_count; p++) {
				...
				case RF_PATH_B:
					...
					p_rf_calibrate_info->delta_power_index[p] = -1 *
							delta_swing_table_idx_tdown_b[delta];
		...
		for (p = RF_PATH_A; p < c.rf_path_count; p++) {
			...
			if (p_rf_calibrate_info->delta_power_index[p] ==
			    p_rf_calibrate_info->delta_power_index_last[p])         /*If Thermal value changes but lookup table value still the same*/
				p_rf_calibrate_info->power_index_offset[p] = 0;
			else
				p_rf_calibrate_info->power_index_offset[p] =
					p_rf_calibrate_info->delta_power_index[p] -
					p_rf_calibrate_info->delta_power_index_last[p];      /*Power index diff between 2 times Power Tracking*/
			...
			p_rf_calibrate_info->OFDM_index[p] =
				p_rf_calibrate_info->bb_swing_idx_ofdm_base[p] +
				p_rf_calibrate_info->power_index_offset[p];
			p_rf_calibrate_info->CCK_index = p_rf_calibrate_info->bb_swing_idx_cck_base
							 + p_rf_calibrate_info->power_index_offset[p];
		...
		ODM_RT_TRACE(p_dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD,
			     ("The thermal meter is unchanged or TxPowerTracking OFF(%d): thermal_value: %d, p_rf_calibrate_info->thermal_value: %d\n",
			      p_rf_calibrate_info->txpowertrack_control, thermal_value,
	...
	ODM_RT_TRACE(p_dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD,
		     ("TxPowerTracking: [CCK] Swing Current index: %d, Swing base index: %d\n",
		      ...
		      p_rf_calibrate_info->bb_swing_idx_cck_base));       /*Print Swing base & current*/
	...
	if ((p_dm->support_ic_type & ODM_RTL8814A)) {
		...
		p_rf_calibrate_info->thermal_value =
		   ...
		   p_rf_calibrate_info->txpowertrack_control
		   && (p_hal_data->eeprom_thermal_meter != 0xff)) {
			...
			for (p = RF_PATH_A; p < c.rf_path_count; p++) {
				ODM_RT_TRACE(p_dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD,
					     ("Temperature Increasing(%d): delta_pi: %d, delta_t: %d, Now_t: %d, EFUSE_t: %d, Last_t: %d\n",
					      p, p_rf_calibrate_info->power_index_offset[p], delta, thermal_value,
					      p_hal_data->eeprom_thermal_meter, p_rf_calibrate_info->thermal_value));
			}
		} else if (thermal_value <
			   p_rf_calibrate_info->thermal_value) {	/*Low temperature*/
			for (p = RF_PATH_A; p < c.rf_path_count; p++) {
				ODM_RT_TRACE(p_dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD,
		...
		p_rf_calibrate_info->bb_swing_idx_cck_base =
			p_rf_calibrate_info->bb_swing_idx_cck;    /*Record last time Power Tracking result as base.*/
		for (p = RF_PATH_A; p < c.rf_path_count; p++)
			p_rf_calibrate_info->bb_swing_idx_ofdm_base[p] =
				p_rf_calibrate_info->bb_swing_idx_ofdm[p];
	...
	if (p_dm->support_ic_type == ODM_RTL8703B
		...
		if (xtal_offset_eanble != 0 && p_rf_calibrate_info->txpowertrack_control
		    && (p_hal_data->eeprom_thermal_meter != 0xff)) {
	...
	if (p_rf_calibrate_info->dpk_thermal[RF_PATH_A] != 0) {
		if (diff_DPK[RF_PATH_A] >= c.threshold_dpk) {
			...
			odm_set_bb_reg(p_dm, 0x82c, BIT(31), 0x1);
			odm_set_bb_reg(p_dm, 0xcc4,
				       BIT(14) | BIT(13) | BIT(12) | BIT(11) | BIT(10), value);
			odm_set_bb_reg(p_dm, 0x82c, BIT(31), 0x0);
		} else {
			odm_set_bb_reg(p_dm, 0x82c, BIT(31), 0x1);
			odm_set_bb_reg(p_dm, 0xcc4,
				       BIT(14) | BIT(13) | BIT(12) | BIT(11) | BIT(10), 0);
			odm_set_bb_reg(p_dm, 0x82c, BIT(31), 0x0);
		}
	}
...
)
{
	...
	if ((p_dm->is_linked) && (!p_iqk_info->rfk_forbidden)) {
		RT_TRACE(COMP_SCAN, ODM_DBG_LOUD, ("interval=%d ch=%d prech=%d scan=%s\n",
						   ...
						   *p_dm->p_channel,  p_dm->pre_channel,
						   *p_dm->p_is_scan_in_process == TRUE ? "TRUE" : "FALSE"));
	...
	RT_TRACE(COMP_SCAN, ODM_DBG_LOUD,
		 ("<=%s interval=%d ch=%d prech=%d scan=%s\n", __FUNCTION__,
		  p_dm->linked_interval,
		  *p_dm->p_channel,  p_dm->pre_channel,
		  *p_dm->p_is_scan_in_process == TRUE ? "TRUE" : "FALSE"));
}
