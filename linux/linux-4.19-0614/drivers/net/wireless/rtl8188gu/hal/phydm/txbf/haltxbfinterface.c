...
)
{
	...
	if (0) {
		...
		if (p_beam_info->first_mu_bfee_index < BEAMFORMEE_ENTRY_NUM) {
			p_tcb->P_AID =
				p_beam_info->beamformee_entry[p_beam_info->first_mu_bfee_index].p_aid;
		...
		if (MacAddr_isBcst(RA) || MacAddr_isMulticast(RA)
		    || p_tcb->macId == MAC_ID_STATIC_FOR_BROADCAST_MULTICAST) {
			p_tcb->G_ID = 63;
			p_tcb->P_AID = 0;
		} else if (ACTING_AS_AP(adapter)) {
			u16	AID = (u16)(MacIdGetOwnerAssociatedClientAID(adapter,
					    p_tcb->macId) & 0x1ff);		/*AID[0:8]*/
			...
			if (AID == 0)		/*A PPDU sent by an AP to a non associated STA*/
				...
				BSSID = ((RA[5] & 0xf0) >> 4) ^ (RA[5] &
								 0xf);	/*BSSID[44:47] xor BSSID[40:43]*/
				p_tcb->P_AID = (AID + BSSID * 32) &
...
enum rt_status beamforming_get_report_frame(
	struct _ADAPTER		*adapter,
	PRT_RFD			p_rfd,
...
)
{
	...
	p_beamform_entry = phydm_beamforming_get_bfee_entry_by_addr(p_dm,
			   Frame_Addr2(*p_pdu_os), &idx);

	if (p_beamform_entry == NULL) {
		PHYDM_DBG(p_dm, DBG_TXBF,
			  ("beamforming_get_report_frame: Cannot find entry by addr\n"));
...
boolean send_fw_ht_ndpa_packet(
	void			*p_dm_void,
	u8			*RA,
	enum channel_width	BW
) {
	...
	if (MgntGetFWBuffer(p_def_adapter, &p_tcb, &p_buf))
	{
		...
		adapter->HalFunc.CmdSendPacketHandler(adapter, p_tcb, p_buf,
						      p_tcb->PacketLength, DESC_PACKET_TYPE_NORMAL, false);
	} else
		ret = false;
...
boolean send_sw_ht_ndpa_packet(
	void			*p_dm_void,
	u8			*RA,
	enum channel_width	BW
) {
	...
	if (MgntGetBuffer(adapter, &p_tcb, &p_buf))
	{
		...
		MgntSendPacket(adapter, p_tcb, p_buf, p_tcb->PacketLength, NORMAL_QUEUE,
			       ndp_tx_rate);
	} else
		ret = false;
...
boolean send_fw_vht_ndpa_packet(
	void			*p_dm_void,
	u8			*RA,
	u16			AID,
	enum channel_width	BW
	...
	if (MgntGetFWBuffer(p_def_adapter, &p_tcb, &p_buf))
	{
		...
		if (phydm_acting_determine(p_dm, phydm_acting_as_ibss)
		    || phydm_acting_determine(p_dm, phydm_acting_as_ap))
			p_tcb->G_ID = 63;
...
boolean send_sw_vht_ndpa_packet(
	void			*p_dm_void,
	u8			*RA,
	u16			AID,
	enum channel_width	BW
 ...
 */
enum rt_status beamforming_get_vht_gid_mgnt_frame(
	struct _ADAPTER		*adapter,
	PRT_RFD			p_rfd,
	POCTET_STRING	p_pdu_os
)
{
	...
	u8			idx;
	struct _RT_BEAMFORMING_INFO	*p_beam_info = &(p_dm->beamforming_info);
	struct _RT_BEAMFORMER_ENTRY	*p_beamform_entry =
			&p_beam_info->beamformer_entry[p_beam_info->mu_ap_index];

	PHYDM_DBG(p_dm, DBG_TXBF, ("[%s] On VHT GID mgnt frame!\n", __func__));

	/* Check length*/
	if (p_pdu_os->length < (FRAME_OFFSET_VHT_GID_MGNT_USER_POSITION_ARRAY +
				16)) {
		PHYDM_DBG(p_dm, DBG_TXBF,
			  ("beamforming_get_vht_gid_mgnt_frame(): Invalid length (%d)\n",
			   p_pdu_os->length));
	...
	if (!eq_mac_addr(p_raddr, adapter->CurrentAddress)) {
		PHYDM_DBG(p_dm, DBG_TXBF,
			  ("beamforming_get_vht_gid_mgnt_frame(): Drop because of RA error.\n"));
	...
	p_buffer = p_pdu_os->Octet +
		   FRAME_OFFSET_VHT_GID_MGNT_MEMBERSHIP_STATUS_ARRAY;
	for (idx = 0; idx < 8; idx++) {
		mem_status[idx] = GET_VHT_GID_MGNT_INFO_MEMBERSHIP_STATUS(p_buffer + idx);
		p_beamform_entry->gid_valid[idx] = GET_VHT_GID_MGNT_INFO_MEMBERSHIP_STATUS(
				p_buffer + idx);
	}
	...
	for (idx = 0; idx < 16; idx++) {
		...
		p_beamform_entry->user_position[idx] = GET_VHT_GID_MGNT_INFO_USER_POSITION(
				p_buffer + idx);
	}
	...
	/* Group ID detail printed*/
	{
		...
		for (i = 0; i < 8; i++) {
			...
			tmp_val2 = ((user_pos[i * 2 + 1] << 8) & 0xFF00) + (user_pos[i * 2] &
					0xFF);
			for (j = 0; j < 8; j++) {
				if ((tmp_val >> j) & BIT(0)) {
	...
	/* Indicate GID frame to IHV service. */
	{
		...
		PlatformMoveMemory(indibuffer + indioffset, p_beamform_entry->gid_valid,
				   8);
		indioffset += 8;
		PlatformMoveMemory(indibuffer + indioffset,
				   p_beamform_entry->user_position, 16);
		indioffset += 16;
...
boolean send_sw_vht_gid_mgnt_frame(
	void			*p_dm_void,
	u8			*RA,
	u8			idx
)
{
	...
	if (ret)
		RT_DISP_DATA(FBEAM, FBEAM_DATA, "", p_buf->Buffer.VirtualAddress,
			     p_tcb->PacketLength);
...
)
{
	...
	SET_80211_HDR_TYPE_AND_SUBTYPE(p_bf_rpt_poll,
...
boolean send_sw_vht_bf_report_poll(
	void			*p_dm_void,
	u8			*RA,
...
boolean send_sw_vht_mu_ndpa_packet(
	void			*p_dm_void,
	enum channel_width	BW
) {
	struct PHY_DM_STRUCT				*p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
...
boolean dbg_send_sw_vht_mundpa_packet(
	void			*p_dm_void,
	enum channel_width	BW
) {
	struct PHY_DM_STRUCT				*p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
...
)
{
	...
	p_beamform_entry = phydm_beamforming_get_bfee_entry_by_addr(p_dm, TA,
			   &idx);
...
)
{
	...
	struct _RT_BEAMFORMER_ENTRY		*p_beamformer_entry =
			NULL;		/*Modified By Jeffery @2014-10-29*/
	...
	p_beamformer_entry = phydm_beamforming_get_bfer_entry_by_addr(p_dm, TA,
			     &idx);		/* Modified By Jeffery @2014-10-29 */
	...
	else if ((p_beamformer_entry->log_success == 1)
		 || (p_beamformer_entry->clock_reset_times == 5)) {
		PHYDM_DBG(p_dm, DBG_TXBF,
			  ("[%s] log_seq=%d, pre_log_seq=%d, log_retry_cnt=%d, log_success=%d, clock_reset_times=%d, clock reset is no longer needed.\n",
			   __func__, p_beamformer_entry->log_seq, p_beamformer_entry->pre_log_seq,
			   p_beamformer_entry->log_retry_cnt, p_beamformer_entry->log_success,
	...
	PHYDM_DBG(p_dm, DBG_TXBF,
		  ("[%s] Start, sequence=%d, log_seq=%d, pre_log_seq=%d, log_retry_cnt=%d, clock_reset_times=%d, log_success=%d\n",
		   __func__, sequence, p_beamformer_entry->log_seq,
		   p_beamformer_entry->pre_log_seq, p_beamformer_entry->log_retry_cnt,
		   p_beamformer_entry->clock_reset_times, p_beamformer_entry->log_success));

	if ((p_beamformer_entry->log_seq != 0)
	    && (p_beamformer_entry->pre_log_seq != 0)) {
		/*Success condition*/
		if ((p_beamformer_entry->log_seq != sequence)
