		...
		for (i = 0; i < DBGPORT_CHK_NUM; i++) {
			...
			if ((p_atd_t->dbg_port_table[i]) == 0) {
				...
				if (p_atd_t->dbg_port_val[i] == curr_dbg_port_val[i]) {
					...
					odm_move_memory(p_dm, &(dbgport_803),  &(p_atd_t->dbg_port_val[i]),
							sizeof(struct n_dbgport_803));

					dbg_print("RSTB{BB, GLB, OFDM}={%d, %d, %d}\n", dbgport_803.bb_rst_b,
						  dbgport_803.glb_rst_b, dbgport_803.ofdm_rst_b);
				...
				if ((p_atd_t->dbg_port_val[i] & BIT(30))
...
void phydm_auto_dbg_engine_init(
	void		*p_dm_void
)
{
#ifdef PHYDM_AUTO_DEGBUG
	...
	odm_move_memory(p_dm, &(p_atd_t->dbg_port_table[0]),  &(dbg_port_table[0]),
			(DBGPORT_CHK_NUM * 2));
