...
void phydm_write_cck_cca_th_new_cs_ratio(
	void			*p_dm_void,
	u8			cca_th,
	u8			cca_th_aaa
)
...
)
{
	...
	en_2rcca = (u8)(odm_get_bb_reg(p_dm, 0xa2c, BIT(18))
			&& odm_get_bb_reg(p_dm, 0xa2c, BIT(22)));
	...
	if (p_dm->is_linked) {
		...
		if ((igi_curr > 0x38) && (p_dm->rssi_min > 32)) {
			...
			cs_2r_offset = 4;
			pd_th = 0xd;
		} else if ((igi_curr > 0x24) || (p_dm->rssi_min > 24
						 && p_dm->rssi_min <= 30)) {
...
#ifdef PHYDM_TDMA_DIG_SUPPORT
	cnt_cck_fail_tmp = (p_dm->original_dig_restore) ? (p_fa_t->cnt_cck_fail)
			   : (p_fa_acc_t->cnt_cck_fail_1sec);
#endif
...
void odm_pause_cck_packet_detection(
	void					*p_dm_void,
