...
)
{
	...
	odm_set_bb_reg(p_dm, p_dm_psd_table->psd_reg, BIT(22),
		       1); /*PSD trigger start*/
	ODM_delay_us(10);
	odm_set_bb_reg(p_dm, p_dm_psd_table->psd_reg, BIT(22),
		       0); /*PSD trigger stop*/
...
)
{
	...
	/*[back up IGI]*/
	p_dm_psd_table->initial_gain_backup = odm_get_bb_reg(p_dm, psd_igi_a_reg,
					      0xff);
	odm_set_bb_reg(p_dm, psd_igi_a_reg, 0xff,
		       0x6e); /*IGI target at 0dBm & make it can't CCA*/
	odm_set_bb_reg(p_dm, psd_igi_b_reg, 0xff,
		       0x6e); /*IGI target at 0dBm & make it can't CCA*/
	ODM_delay_us(10);
	...
	/*[Backup RF Reg]*/
	p_dm_psd_table->rf_0x18_bkp = odm_get_rf_reg(p_dm, RF_PATH_A, 0x18,
				      RFREGOFFSETMASK);
	p_dm_psd_table->rf_0x18_bkp_b = odm_get_rf_reg(p_dm, RF_PATH_B, 0x18,
					RFREGOFFSETMASK);
	...
	/* RF path-a */
	odm_set_rf_reg(p_dm, RF_PATH_A, 0x18, 0xff,
		       psd_fc_channel);     /* Set RF fc*/
	odm_set_rf_reg(p_dm, RF_PATH_A, 0x18, 0x300, rf_reg18_9_8);
	odm_set_rf_reg(p_dm, RF_PATH_A, 0x18, 0xc00,
		       p_dm_psd_table->psd_bw_rf_reg);     /*2b'11: 20MHz, 2b'10: 40MHz, 2b'01: 80MHz */
	odm_set_rf_reg(p_dm, RF_PATH_A, 0x18, 0xf0000,
		       ag_rf_mode_reg);     /* Set RF ag fc mode*/

	/* RF path-b */
	odm_set_rf_reg(p_dm, RF_PATH_B, 0x18, 0xff,
		       psd_fc_channel);     /* Set RF fc*/
	odm_set_rf_reg(p_dm, RF_PATH_B, 0x18, 0x300, rf_reg18_9_8);
	odm_set_rf_reg(p_dm, RF_PATH_B, 0x18, 0xc00,
		       p_dm_psd_table->psd_bw_rf_reg);     /*2b'11: 20MHz, 2b'10: 40MHz, 2b'01: 80MHz */
	odm_set_rf_reg(p_dm, RF_PATH_B, 0x18, 0xf0000,
		       ag_rf_mode_reg);     /* Set RF ag fc mode*/
	...
	for (i = start_point; i <= stop_point; i++) {
		...
		psd_result = (u8)((psd_result_tmp / p_dm_psd_table->sw_avg_time)) -
	...
	odm_set_rf_reg(p_dm, RF_PATH_A, 0x18, RFREGOFFSETMASK,
		       p_dm_psd_table->rf_0x18_bkp);
	odm_set_rf_reg(p_dm, RF_PATH_B, 0x18, RFREGOFFSETMASK,
		       p_dm_psd_table->rf_0x18_bkp_b);
	...
	if ((strcmp(input[1], help) == 0)) {
		...
		if (var1[0] == 0) {
			...
			PHYDM_SNPRINTF((output + used, out_len - used,
					...
					(u8)var1[8]));
			phydm_psd_para_setting(p_dm, (u8)var1[1], (u8)var1[2], (u8)var1[3],
