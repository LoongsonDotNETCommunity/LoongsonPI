...
)
{
	...
	if ((p_entry->wireless_mode & WIRELESS_MODE_AC_5G)
	    || (p_entry->wireless_mode & WIRELESS_MODE_AC_24G)) {
		if (sta_idx != 0)
	...
	odm_move_memory(p_dm, p_entry->my_mac_addr,
			adapter_mac_addr(p_sta->padapter), 6);
	...
	PHYDM_DBG(p_dm, DBG_TXBF,
		  ("p_entry->cur_beamform = 0x%x, p_entry->cur_beamform_vht = 0x%x\n",
...
)
{
	...
	for (i = 0; i < BEAMFORMEE_ENTRY_NUM; i++) {
		if (p_beam_info->beamformee_entry[i].is_used
		    && (eq_mac_addr(RA, p_beam_info->beamformee_entry[i].mac_addr))) {
			*idx = i;
...
)
{
	...
	for (i = 0; i < BEAMFORMER_ENTRY_NUM; i++) {
		if (p_beam_info->beamformer_entry[i].is_used
		    && (eq_mac_addr(TA, p_beam_info->beamformer_entry[i].mac_addr))) {
			*idx = i;
...
)
{
	...
	for (i = 0; i < BEAMFORMEE_ENTRY_NUM; i++) {
		if (p_beam_info->beamformee_entry[i].is_used
		    && (mac_id == p_beam_info->beamformee_entry[i].mac_id)) {
			*idx = i;
			return &(p_beam_info->beamformee_entry[i]);
		}
	}
...
enum beamforming_cap phydm_beamforming_get_entry_beam_cap_by_mac_id(
	void		*p_dm_void,
	u8		mac_id
)
{
...
)
{
	...
	for (idx = 0; idx < BEAMFORMEE_ENTRY_NUM; idx++) {
		if (p_beam_info->beamformee_entry[idx].is_used
		    && p_beam_info->beamformee_entry[idx].is_mu_sta) {
...
)
{
	...
	struct _RT_BEAMFORMEE_ENTRY	*p_entry =
		phydm_beamforming_get_free_bfee_entry(p_dm, idx);
	...
	if (p_entry != NULL) {
		...
		if (phydm_acting_determine(p_dm, phydm_acting_as_ap)) {
			...
			u16 bssid = ((p_sta->my_mac_addr[5] & 0xf0) >> 4) ^ (p_sta->my_mac_addr[5]
					& 0xf);
		...
		if (beamform_cap & BEAMFORMER_CAP_VHT_MU) {
			...
			p_dm->beamforming_info.first_mu_bfee_index =
				phydm_beamforming_get_first_mu_bfee_entry_idx(p_dm);
		} else if (beamform_cap & (BEAMFORMER_CAP_VHT_SU |
...
{
	struct PHY_DM_STRUCT				*p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct _RT_BEAMFORMER_ENTRY	*p_entry =
		phydm_beamforming_get_free_bfer_entry(p_dm, idx);
	...
	if (p_entry != NULL) {
		...
		p_entry->log_success =
			0;	/*log_success is NOT needed to be accumulated, so  LogSuccessCnt->log_success, 2015-04-13, Jeffery*/
		...
		if (beamform_cap & BEAMFORMEE_CAP_VHT_MU) {
			...
			p_entry->aid = p_sta->aid;
		} else if (beamform_cap & (BEAMFORMEE_CAP_VHT_SU |
...
#if 0
boolean beamforming_remove_entry(
...
)
{
	...
	struct _RT_BEAMFORMER_ENTRY	*p_bfer_entry =
		phydm_beamforming_get_bfer_entry_by_addr(p_dm, RA, idx);
	struct _RT_BEAMFORMEE_ENTRY	*p_entry =
		phydm_beamforming_get_bfee_entry_by_addr(p_dm, RA, idx);
	boolean ret = false;
...
)
{
	...
	struct _RT_BEAMFORMEE_ENTRY	*p_entry = &
			(p_beam_info->beamformee_entry[p_beam_info->beamformee_cur_idx]);

	PHYDM_DBG(p_dm, DBG_TXBF, ("[%s] Start!\n", __func__));

	/* 3 TODO  per-client throughput caculation. */

	if ((*(p_dm->p_current_tx_tp) + * (p_dm->p_current_rx_tp) > 2)
	    && ((p_entry->log_status_fail_cnt <= 20) || status)) {
	...
	for (idx = 0; idx < BEAMFORMEE_ENTRY_NUM; idx++) {
		...
		PHYDM_DBG(p_dm, DBG_TXBF, ("[%s] period = %d\n", __func__,
					   sound_period_sw));
		if (p_beamform_entry->beamform_entry_cap & (BEAMFORMER_CAP_HT_EXPLICIT |
				BEAMFORMER_CAP_VHT_SU)) {
			if (p_sound_info->sound_mode == SOUNDING_FW_VHT_TIMER
			    || p_sound_info->sound_mode == SOUNDING_FW_HT_TIMER) {
				if (p_beamform_entry->sound_period != sound_period_fw) {
					p_beamform_entry->sound_period = sound_period_fw;
					is_change_period =
						true;		/*Only FW sounding need to send H2C packet to change sound period. */
				}
...
boolean beamforming_send_ht_ndpa_packet(
	void			*p_dm_void,
	u8			*RA,
	enum channel_width	BW,
	u8			q_idx
...
boolean beamforming_send_vht_ndpa_packet(
	void			*p_dm_void,
	u8			*RA,
	u16			AID,
	enum channel_width	BW,
...
)
{
	...
	if ((p_beam_info->tx_bf_data_rate >= ODM_RATEVHTSS3MCS7)
	    && (p_beam_info->tx_bf_data_rate <= ODM_RATEVHTSS3MCS9)
...
enum beamforming_notify_state phydm_beamfomring_is_sounding(
	void				*p_dm_void,
...
)
{
	...
	struct _RT_BEAMFORMING_OID_INFO	beam_oid_info =
			p_beam_info->beamforming_oid_info;
	struct PHY_DM_STRUCT					*p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	...
	if (beam_oid_info.sound_oid_mode == sounding_stop_all_timer)
		...
		for (i = 0 ; i < BEAMFORMEE_ENTRY_NUM ; i++) {
			PHYDM_DBG(p_dm, DBG_TXBF, ("@%s: BFee Entry %d is_used=%d, is_sound=%d\n",
						   __func__, i, p_beam_info->beamformee_entry[i].is_used,
						   p_beam_info->beamformee_entry[i].is_sound));
			if (p_beam_info->beamformee_entry[i].is_used
			    && (!p_beam_info->beamformee_entry[i].is_sound)) {
...
)
{
	...
	if (beam_oid_info.sound_oid_mode == SOUNDING_SW_HT_TIMER
	    || beam_oid_info.sound_oid_mode == SOUNDING_SW_VHT_TIMER ||
	    beam_oid_info.sound_oid_mode == SOUNDING_HW_HT_TIMER
	    || beam_oid_info.sound_oid_mode == SOUNDING_HW_VHT_TIMER)
		idx = beam_oid_info.sound_oid_idx;
	else {
		u8	i;
		for (i = 0; i < BEAMFORMEE_ENTRY_NUM; i++) {
			if (p_beam_info->beamformee_entry[i].is_used
			    && (false == p_beam_info->beamformee_entry[i].is_sound)) {
				idx = i;
				break;
			}
		}
...
enum sounding_mode phydm_beamforming_sounding_mode(
	void				*p_dm_void,
	struct _RT_BEAMFORMING_INFO	*p_beam_info,
	u8					idx
)
{
	...
	struct _RT_BEAMFORMEE_ENTRY		beam_entry =
			p_beam_info->beamformee_entry[idx];
	struct _RT_BEAMFORMING_OID_INFO	beam_oid_info =
			p_beam_info->beamforming_oid_info;
	enum sounding_mode				mode = beam_oid_info.sound_oid_mode;

	if (beam_oid_info.sound_oid_mode == SOUNDING_SW_VHT_TIMER
	    || beam_oid_info.sound_oid_mode == SOUNDING_HW_VHT_TIMER) {
		if (beam_entry.beamform_entry_cap & BEAMFORMER_CAP_VHT_SU)
			mode = beam_oid_info.sound_oid_mode;
		else
			mode = sounding_stop_all_timer;
	} else if (beam_oid_info.sound_oid_mode == SOUNDING_SW_HT_TIMER
		   || beam_oid_info.sound_oid_mode == SOUNDING_HW_HT_TIMER) {
		...
		if ((support_interface == ODM_ITRF_USB)
		    && !(p_dm->support_ic_type & (ODM_RTL8814A | ODM_RTL8822B)))
...
)
{
	...
	if (mode == SOUNDING_HW_HT_TIMER || mode == SOUNDING_HW_VHT_TIMER)
		...
		/*Modified by David*/
		sounding_time =
			beam_entry.sound_period;	/*beam_oid_info.sound_oid_period;*/
	else
...
enum channel_width phydm_beamforming_sounding_bw(
	void				*p_dm_void,
	struct _RT_BEAMFORMING_INFO	*p_beam_info,
	enum sounding_mode			mode,
	u8					idx
...
boolean phydm_beamforming_select_beam_entry(
	void				*p_dm_void,
	struct _RT_BEAMFORMING_INFO	*p_beam_info
)
{
	...
	p_sound_info->sound_idx = phydm_beamforming_sounding_idx(p_dm,
				  p_beam_info);
	/*p_sound_info->sound_idx = 0;*/

	if (p_sound_info->sound_idx < BEAMFORMEE_ENTRY_NUM)
		p_sound_info->sound_mode = phydm_beamforming_sounding_mode(p_dm,
					   p_beam_info, p_sound_info->sound_idx);
	else
	...
	if (sounding_stop_all_timer == p_sound_info->sound_mode) {
		...
		p_sound_info->sound_bw = phydm_beamforming_sounding_bw(p_dm, p_beam_info,
					 p_sound_info->sound_mode, p_sound_info->sound_idx);
		p_sound_info->sound_period = phydm_beamforming_sounding_time(p_dm,
					     p_beam_info, p_sound_info->sound_mode, p_sound_info->sound_idx);
		return true;
	}
}

/*SU BFee Entry Only*/
boolean phydm_beamforming_start_period(
	void				*p_dm_void
)
{
	struct PHY_DM_STRUCT					*p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	...
	if (p_sound_info->sound_mode == SOUNDING_SW_VHT_TIMER
	    || p_sound_info->sound_mode == SOUNDING_SW_HT_TIMER)
		odm_set_timer(p_dm, &p_beam_info->beamforming_timer,
			      p_sound_info->sound_period);
	else if (p_sound_info->sound_mode == SOUNDING_HW_VHT_TIMER
		 || p_sound_info->sound_mode == SOUNDING_HW_HT_TIMER ||
		 p_sound_info->sound_mode == SOUNDING_AUTO_VHT_TIMER
		 || p_sound_info->sound_mode == SOUNDING_AUTO_HT_TIMER) {
		...
		phydm_set_hw_reg_handler_interface(p_dm, HW_VAR_HW_REG_TIMER_STOP,
						   (u8 *)(&timer_type));
		/* odm_write_1byte(p_dm, 0x15F, 0); */
		/* HW timer init: All IC has the same setting, but 92E & 8812A only write 2 bytes */
		phydm_set_hw_reg_handler_interface(p_dm, HW_VAR_HW_REG_TIMER_INIT,
						   (u8 *)(&val));
		/* odm_write_1byte(p_dm, 0x164, 1); */
		/* odm_write_4byte(p_dm, 0x15C, val); */
		/* HW timer start: All IC has the same setting */
		phydm_set_hw_reg_handler_interface(p_dm, HW_VAR_HW_REG_TIMER_START,
						   (u8 *)(&timer_type));
		/* odm_write_1byte(p_dm, 0x15F, 0x5); */
	} else if (p_sound_info->sound_mode == SOUNDING_FW_VHT_TIMER
		   || p_sound_info->sound_mode == SOUNDING_FW_HT_TIMER)
	...
	PHYDM_DBG(p_dm, DBG_TXBF,
		  ("[%s] sound_idx=%d, sound_mode=%d, sound_bw=%d, sound_period=%d\n",
		   __func__,
		   p_sound_info->sound_idx, p_sound_info->sound_mode, p_sound_info->sound_bw,
		   p_sound_info->sound_period));
...
)
{
	...
	if (is_delete) {
		...
		PHYDM_DBG(p_dm, DBG_TXBF, ("[%s] SW delete BFee entry %d\n", __func__,
					   delete_idx));
		if (p_beamform_entry->beamform_entry_state ==
		    BEAMFORMING_ENTRY_STATE_PROGRESSING) {
			p_beamform_entry->is_beamforming_in_progress = false;
			p_beamform_entry->beamform_entry_state =
				BEAMFORMING_ENTRY_STATE_UNINITIALIZE;
		} else if (p_beamform_entry->beamform_entry_state ==
...
)
{
	...
	case BEAMFORMEE_NOTIFY_DELETE_SU:
		...
		if (p_sound_info->sound_mode == SOUNDING_FW_HT_TIMER
		    || p_sound_info->sound_mode == SOUNDING_FW_VHT_TIMER) {
			phydm_beamforming_clear_entry_fw(p_dm, true, idx);
			if (p_beam_info->beamformee_su_cnt ==
			    0) { /* For 2->1 entry, we should not cancel SW timer */
	...
	case BEAMFORMEE_NOTIFY_ADD_MU:
		...
		if (p_beam_info->beamformee_mu_cnt == 2) {
			...
			odm_set_timer(p_dm, &p_beam_info->beamforming_timer,
				      1000); /*Do MU sounding every 1sec*/
		} else
			PHYDM_DBG(p_dm, DBG_TXBF,
...
boolean beamforming_init_entry(
	void		*p_dm_void,
	u16		sta_idx,
	u8			*bfer_bfee_idx
)
{
	...
	if (BEAMFORMING_CAP_NONE == p_sta->ht_beamform_cap
	    && BEAMFORMING_CAP_NONE == p_sta->vht_beamform_cap) {
		PHYDM_DBG(p_dm, DBG_TXBF,
	...
	if (p_sta->wireless_mode < WIRELESS_MODE_N_24G)
		...
		if (p_sta->wireless_mode & WIRELESS_MODE_N_5G
		    || p_sta->wireless_mode & WIRELESS_MODE_N_24G) {/*HT*/
			if (TEST_FLAG(p_sta->cur_beamform,
				      BEAMFORMING_HT_BEAMFORMER_ENABLE)) {/*We are Beamformee because the STA is Beamformer*/
				beamform_cap = (enum beamforming_cap)(beamform_cap |
								      BEAMFORMEE_CAP_HT_EXPLICIT);
				num_of_sounding_dim = (p_sta->cur_beamform &
						       BEAMFORMING_HT_BEAMFORMEE_CHNL_EST_CAP) >> 6;
			}
			...
			if (TEST_FLAG(p_sta->cur_beamform, BEAMFORMING_HT_BEAMFORMEE_ENABLE) ||
				...
				beamform_cap = (enum beamforming_cap)(beamform_cap |
								      BEAMFORMER_CAP_HT_EXPLICIT);
				comp_steering_num_of_bfer = (p_sta->cur_beamform &
							     BEAMFORMING_HT_BEAMFORMER_STEER_NUM) >> 4;
			}
			PHYDM_DBG(p_dm, DBG_TXBF,
				  ("[%s] HT cur_beamform=0x%X, beamform_cap=0x%X\n", __func__,
				   p_sta->cur_beamform, beamform_cap));
			PHYDM_DBG(p_dm, DBG_TXBF,
				  ("[%s] HT num_of_sounding_dim=%d, comp_steering_num_of_bfer=%d\n",
				   __func__, num_of_sounding_dim, comp_steering_num_of_bfer));
		}
#if (ODM_IC_11AC_SERIES_SUPPORT == 1)
		if (p_sta->wireless_mode & WIRELESS_MODE_AC_5G
		    || p_sta->wireless_mode & WIRELESS_MODE_AC_24G) {	/*VHT*/

			/* We are Beamformee because the STA is SU Beamformer*/
			if (TEST_FLAG(p_sta->cur_beamform_vht,
				      BEAMFORMING_VHT_BEAMFORMER_ENABLE)) {
				beamform_cap = (enum beamforming_cap)(beamform_cap |
								      BEAMFORMEE_CAP_VHT_SU);
				num_of_sounding_dim = (p_sta->cur_beamform_vht &
						       BEAMFORMING_VHT_BEAMFORMEE_SOUND_DIM) >> 12;
			}
			...
			if (TEST_FLAG(p_sta->cur_beamform_vht, BEAMFORMING_VHT_BEAMFORMEE_ENABLE)
				...
				beamform_cap = (enum beamforming_cap)(beamform_cap |
								      BEAMFORMER_CAP_VHT_SU);
				comp_steering_num_of_bfer = (p_sta->cur_beamform_vht &
							     BEAMFORMING_VHT_BEAMFORMER_STS_CAP) >> 8;
			}
			/* We are Beamformee because the STA is MU Beamformer*/
			if (TEST_FLAG(p_sta->cur_beamform_vht,
				      BEAMFORMING_VHT_MU_MIMO_AP_ENABLE)) {
				beamform_cap = (enum beamforming_cap)(beamform_cap |
								      BEAMFORMEE_CAP_VHT_MU);
				num_of_sounding_dim = (p_sta->cur_beamform_vht &
						       BEAMFORMING_VHT_BEAMFORMEE_SOUND_DIM) >> 12;
			}
			/* We are Beamformer because the STA is MU Beamformee*/
			if (phydm_acting_determine(p_dm,
						   phydm_acting_as_ap)) { /* Only AP mode supports to act an MU beamformer */
			...
			PHYDM_DBG(p_dm, DBG_TXBF,
				  ("[%s]VHT cur_beamform_vht=0x%X, beamform_cap=0x%X\n", __func__,
				   p_sta->cur_beamform_vht, beamform_cap));
			PHYDM_DBG(p_dm, DBG_TXBF,
	...
	if (beamform_cap & (BEAMFORMEE_CAP_VHT_MU | BEAMFORMEE_CAP_VHT_SU |
			    BEAMFORMEE_CAP_HT_EXPLICIT)) {
		p_beamformer_entry = phydm_beamforming_get_bfer_entry_by_addr(p_dm,
				     p_sta->ra, &bfer_idx);

		if (p_beamformer_entry == NULL) {
			p_beamformer_entry = beamforming_add_bfer_entry(p_dm, p_sta, beamform_cap,
					     num_of_sounding_dim, &bfer_idx);
			if (p_beamformer_entry == NULL)
	...
	if (beamform_cap & (BEAMFORMER_CAP_VHT_MU | BEAMFORMER_CAP_VHT_SU |
			    BEAMFORMER_CAP_HT_EXPLICIT)) {
		p_beamform_entry = phydm_beamforming_get_bfee_entry_by_addr(p_dm,
				   p_sta->ra, &bfee_idx);
		...
		if (p_beamform_entry == NULL) {
			p_beamform_entry = beamforming_add_bfee_entry(p_dm, p_sta, beamform_cap,
					   num_of_sounding_dim, comp_steering_num_of_bfer, &bfee_idx);
	...
	*bfer_bfee_idx = (bfer_idx << 4) | bfee_idx;
	PHYDM_DBG(p_dm, DBG_TXBF,
		  ("[%s] End: bfer_idx=0x%X, bfee_idx=0x%X, bfer_bfee_idx=0x%X\n", __func__,
		   bfer_idx, bfee_idx, *bfer_bfee_idx));
...
)
{
	...
	struct _RT_BEAMFORMER_ENTRY	*p_bfer_entry =
		phydm_beamforming_get_bfer_entry_by_addr(p_dm, RA, &idx);
	struct _RT_BEAMFORMEE_ENTRY	*p_bfee_entry =
		phydm_beamforming_get_bfee_entry_by_addr(p_dm, RA, &idx);
	boolean ret = false;
...
)
{
	...
	if (p_entry->is_used == false) {
		...
		if (p_entry->beamform_entry_state != BEAMFORMING_ENTRY_STATE_INITIALIZED
	...
	if (mode == 1)
		...
		ret = beamforming_send_vht_ndpa_packet(p_dm, RA, p_entry->aid, BW,
						       NORMAL_QUEUE);
...
boolean beamforming_start_sw(
	...
	} else
	{
		...
		if (p_entry->is_used == false) {
			...
			if (mode == SOUNDING_SW_HT_TIMER || mode == SOUNDING_HW_HT_TIMER
			...
			} else if (mode == SOUNDING_SW_VHT_TIMER || mode == SOUNDING_HW_VHT_TIMER
...
)
{
	...
	if (p_beam_info->is_mu_sounding) {
		...
		hal_com_txbf_set(p_dm, TXBF_SET_SOUNDING_STATUS,
				 (u8 *) & (p_beam_info->beamformee_cur_idx));
	} else {
		...
		if ((p_beam_info->tx_bf_data_rate >= ODM_RATEVHTSS3MCS7)
			...
			p_entry->log_status_fail_cnt++;
			p_entry->beamform_entry_state = BEAMFORMING_ENTRY_STATE_INITIALIZED;
			hal_com_txbf_set(p_dm, TXBF_SET_TX_PATH_RESET,
					 (u8 *) & (p_beam_info->beamformee_cur_idx));
			PHYDM_DBG(p_dm, DBG_TXBF, ("[%s] log_status_fail_cnt %d\n", __func__,
						   p_entry->log_status_fail_cnt));
		}
...
#endif
	if (ret)
		ret = beamforming_start_sw(p_dm, p_sound_info->sound_idx,
					   p_sound_info->sound_mode, p_sound_info->sound_bw);
	else
		PHYDM_DBG(p_dm, DBG_TXBF,
			  ("%s, Error value return from BeamformingStart_V2\n", __func__));

	if ((p_beam_info->beamformee_su_cnt != 0)
	    || (p_beam_info->beamformee_mu_cnt > 1)) {
		if (p_sound_info->sound_mode == SOUNDING_SW_VHT_TIMER
		    || p_sound_info->sound_mode == SOUNDING_SW_HT_TIMER)
			odm_set_timer(p_dm, &p_beam_info->beamforming_timer,
				      p_sound_info->sound_period);
			...
			phydm_set_hw_reg_handler_interface(p_dm, HW_VAR_HW_REG_TIMER_RESTART,
							   (u8 *)(&val));
		}
	}
}
...
)
{
	struct PHY_DM_STRUCT					*p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct _RT_BEAMFORMING_INFO		*p_beam_info = &p_dm->beamforming_info;
	struct _RT_BEAMFORMING_OID_INFO	*p_beam_oid_info = &
			(p_beam_info->beamforming_oid_info);
#if (DM_ODM_SUPPORT_TYPE == ODM_WIN)
	struct _ADAPTER		*adapter = p_dm->adapter;
	HAL_DATA_TYPE	*p_hal_data = GET_HAL_DATA(adapter);
	...
	PHYDM_DBG(p_dm, DBG_TXBF, ("%s mode (%d)\n", __func__,
				   p_beam_oid_info->sound_oid_mode));
...
boolean phydm_acting_determine(
	void			*p_dm_void,
...
void phydm_beamforming_set_txbf_en(
	void		*p_dm_void,
	u8			mac_id,
	boolean			is_txbf
)
{
	...
	PHYDM_DBG(p_dm, DBG_TXBF, ("%s mac_id %d TxBF %d\n", __func__,
				   p_entry->mac_id, p_entry->is_txbf));

	phydm_beamforming_notify(p_dm);
}
#endif

enum beamforming_cap phydm_beamforming_get_beam_cap(
	void						*p_dm_void,
	struct _RT_BEAMFORMING_INFO	*p_beam_info
)
{
...
boolean beamforming_control_v1(
	void			*p_dm_void,
	u8			*RA,
	u8			AID,
	u8			mode,
...
boolean phydm_beamforming_control_v2(
	void		*p_dm_void,
	u8			idx,
	u8			mode,
	enum channel_width	BW,
...
enum beamforming_cap phydm_get_beamform_cap(
	void			*p_dm_void
)
{
	struct PHY_DM_STRUCT                    *p_dm = (struct PHY_DM_STRUCT *)
			p_dm_void;
	struct cmn_sta_info                     *p_sta = NULL;
	struct bf_cmn_info                      *p_bf_info = NULL;
	struct _RT_BEAMFORMING_INFO             *p_beam_info =
			&p_dm->beamforming_info;
	struct _ADAPTER                         *adapter = p_dm->adapter;
	enum beamforming_cap                     beamform_cap =
		BEAMFORMING_CAP_NONE;
	u8                                       macid;
...
#if (DM_ODM_SUPPORT_TYPE == ODM_WIN)
	PMGNT_INFO                              p_MgntInfo = &adapter->MgntInfo;
	PRT_VERY_HIGH_THROUGHPUT                p_vht_info = GET_VHT_INFO(
				p_MgntInfo);
	PRT_HIGH_THROUGHPUT                     p_ht_info  = GET_HT_INFO(
				p_MgntInfo);
	...
	if (TEST_FLAG(ht_curbeamformcap,
		      BEAMFORMING_HT_BEAMFORMER_ENABLE)) /*We are Beamformee because the STA is Beamformer*/
		beamform_cap = (enum beamforming_cap)(beamform_cap |
						      (BEAMFORMEE_CAP_HT_EXPLICIT | BEAMFORMEE_CAP));

	/*We are Beamformer because the STA is Beamformee*/
	if (TEST_FLAG(ht_curbeamformcap, BEAMFORMING_HT_BEAMFORMEE_ENABLE))
		beamform_cap = (enum beamforming_cap)(beamform_cap |
						      (BEAMFORMER_CAP_HT_EXPLICIT | BEAMFORMER_CAP));
