...
boolean phydm_la_buffer_allocate(
	void			*p_dm_void
)
{
...
#if (DM_ODM_SUPPORT_TYPE & ODM_WIN)
		if (PlatformAllocateMemoryWithZero(adapter,
						   (void **) & (adc_smp_buf->octet),
						   adc_smp_buf->buffer_size) != RT_STATUS_SUCCESS) {
#else
...
)
{
	...
	if (p_dm->support_ic_type & ODM_RTL8197F) {
		...
		/*move LA mode content from IMEM to TxPktBuffer
			...
			Length : 64K*/
		GET_HAL_INTERFACE(p_dm->priv)->init_ddma_handler(p_dm->priv, OCPBASE_IMEM,
				OCPBASE_TXBUF, 0x10000);
	}
#endif
#endif
...
void phydm_la_mode_set_mac_iq_dump(
	void		*p_dm_void
)
{
	struct PHY_DM_STRUCT		*p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	...
	if (adc_smp->la_trig_mode == PHYDM_MAC_TRIG) {
		...
		if (adc_smp->la_trig_mode == PHYDM_ADC_MAC_TRIG) {
			...
			odm_set_mac_reg(p_dm, 0x7c0, BIT(3),
			...
			if (adc_smp->la_trig_sig_sel == ADCSMP_TRIG_REG)
				odm_set_mac_reg(p_dm, 0x7c0, BIT(5),
...
)
{
	...
	if (adc_smp->la_count == 0) {
		...
		adc_smp_set(p_dm, adc_smp->la_trig_mode, adc_smp->la_trig_sig_sel,
...
)
{
	...
	if (information_buffer_length != adc_smp_buf->buffer_size) {
		...
		*bytes_written = 0;
		...
		odm_move_memory(p_dm, information_buffer, adc_smp_buf->octet,
				adc_smp_buf->buffer_size);
...
)
{
	...
	if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
		...
		odm_set_bb_reg(p_dm, 0x95C, 0x1f,
			       trig_sig_sel);	/*0x95C[4:0], BB debug port bit*/
		odm_set_bb_reg(p_dm, 0x95C, BIT(31),
		...
		*/
		...
		odm_set_bb_reg(p_dm, 0x9a0, 0x1f,
			       trig_sig_sel);	/*0x9A0[4:0], BB debug port bit*/
		odm_set_bb_reg(p_dm, 0x9A0, BIT(31),
			       is_trigger_edge); /*0: posedge, 1: negedge*/
	...
	if (p_dm->support_ic_type & PHYDM_IC_SUPPORT_LA_MODE) {
		...
		if ((strcmp(input[1], help) == 0)) {
			...
			dbg_print("echo lamode %d %d %d %d %d %d %x %d %d %d\n", var1[0], var1[1],
				  var1[2], var1[3], var1[4], var1[5], var1[6], var1[7], var1[8], var1[9]);
#if (DM_ODM_SUPPORT_TYPE & ODM_WIN)
			RT_TRACE_EX(COMP_LA_MODE, DBG_LOUD,
				    ("echo lamode %d %d %d %d %d %d %x %d %d %d\n", var1[0], var1[1], var1[2],
				     var1[3], var1[4], var1[5], var1[6], var1[7], var1[8], var1[9]));
#endif
			...
			PHYDM_SNPRINTF((output + used, out_len - used,
					...
					trigger_time_mu_sec, adc_smp->la_mac_mask_or_hdr_sel,
					adc_smp->la_dbg_port));
			PHYDM_SNPRINTF((output + used, out_len - used,
					"h.Trig_edge = ((%d)), i.smp rate = ((%d MHz)), j.Cap_num = ((%d))\n",
					adc_smp->la_trigger_edge, (80 >> adc_smp->la_smp_rate),
					adc_smp->la_count));

			adc_smp_set(p_dm, trig_mode, trig_sig_sel, dma_data_sig_sel,
				    trigger_time_mu_sec, 0);
