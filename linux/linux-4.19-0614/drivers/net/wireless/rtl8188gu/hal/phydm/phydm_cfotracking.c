...
)
{
	...
	if (p_dm->support_ic_type & (ODM_RTL8188E | ODM_RTL8188F)) {
		...
		odm_set_bb_reg(p_dm, REG_AFE_XTAL_CTRL, 0x007ff800,
		...
		odm_set_bb_reg(p_dm, REG_MAC_PHY_CTRL, 0x7FF80000,
			       (crystal_cap | (crystal_cap << 6)));

	}
#endif
#if (RTL8703B_SUPPORT == 1) || (RTL8723B_SUPPORT == 1) || (RTL8192E_SUPPORT == 1) || (RTL8821A_SUPPORT == 1) || (RTL8723D_SUPPORT == 1)
	else if ((p_dm->support_ic_type & (ODM_RTL8703B | ODM_RTL8723B |
					   ODM_RTL8192E | ODM_RTL8821 | ODM_RTL8723D))) {

		/* 0x2C[23:18] = 0x2C[17:12] = crystal_cap */
		odm_set_bb_reg(p_dm, REG_MAC_PHY_CTRL, 0x00FFF000,
			       (crystal_cap | (crystal_cap << 6)));
		...
		odm_set_bb_reg(p_dm, REG_MAC_PHY_CTRL, 0x07FF8000,
			       (crystal_cap | (crystal_cap << 6)));

	}
#endif
#if (RTL8822B_SUPPORT == 1) || (RTL8821C_SUPPORT == 1) || (RTL8197F_SUPPORT == 1)
	else if (p_dm->support_ic_type & (ODM_RTL8822B | ODM_RTL8821C |
					  ODM_RTL8197F)) {
		...
		HAL_SetSYSOnReg(p_dm->adapter, REG_SYS_XTAL_CTRL0, 0x3FFC0000,
				(crystal_cap | (crystal_cap << 6)));
#endif
	}
#endif
...
u8 odm_get_default_crytaltal_cap(
	void					*p_dm_void
)
{
	struct PHY_DM_STRUCT					*p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
...
)
{
	...
	odm_set_bb_reg(p_dm, ODM_REG(BB_ATC, p_dm), ODM_BIT(BB_ATC, p_dm),
		       atc_status);
	p_cfo_track->is_atc_status = atc_status;
}

boolean odm_get_atc_status(
	void					*p_dm_void
)
{
	boolean						atc_status;
	...
	atc_status = (boolean)odm_get_bb_reg(p_dm, ODM_REG(BB_ATC, p_dm),
					     ODM_BIT(BB_ATC, p_dm));
	return atc_status;
}

void odm_cfo_tracking_reset(
	void					*p_dm_void
)
{
	struct PHY_DM_STRUCT					*p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
...
)
{
	...
	if (!(p_dm->support_ability & ODM_BB_CFO_TRACKING)) {
		PHYDM_DBG(p_dm, DBG_CFO_TRK,
			  ("odm_cfo_tracking(): Return: support_ability ODM_BB_CFO_TRACKING is disabled\n"));
		return;
	}
	...
	if (!p_dm->is_linked || !p_dm->is_one_entry_only) {
		...
		PHYDM_DBG(p_dm, DBG_CFO_TRK,
			  ("odm_cfo_tracking(): Reset: is_linked = %d, is_one_entry_only = %d\n",
		...
		if (p_cfo_track->packet_count == p_cfo_track->packet_count_pre) {
			PHYDM_DBG(p_dm, DBG_CFO_TRK,
				  ("odm_cfo_tracking(): packet counter doesn't change\n"));
			return;
		}
		...
		for (i = 0; i < p_dm->num_rf_path; i++) {
			...
			valid_path_cnt++;
			CFO_rpt_sum = (u32)((p_cfo_track->CFO_tail[i] < 0) ?
					    (0 - p_cfo_track->CFO_tail[i]) :  p_cfo_track->CFO_tail[i]);
			...
			PHYDM_DBG(p_dm, DBG_CFO_TRK,
				  ("[path %d] CFO_rpt_sum = (( %d )), CFO_cnt = (( %d )) , CFO_avg= (( %s%d )) kHz\n",
				   ...
				   ((p_cfo_track->CFO_tail[i] < 0) ? "-" : " "), cfo_khz_avg[i]));
		}
		...
		if (p_dm->bt_info_table.is_bt_enabled) {
			...
			PHYDM_DBG(p_dm, DBG_CFO_TRK,
				  ("odm_cfo_tracking(): Disable CFO tracking for BT!!\n"));
		}
#if 0
		/* 4 1.6 Big jump */
		if (p_cfo_track->is_adjust) {
			...
			PHYDM_DBG(p_dm, DBG_CFO_TRK,
				  ("odm_cfo_tracking(): Crystal cap offset = %d\n", adjust_xtal));
		}
#endif
#endif
		...
		PHYDM_DBG(p_dm, DBG_CFO_TRK,
			  ("odm_cfo_tracking(): Crystal cap = 0x%x, Default Crystal cap = 0x%x\n",
...
)
{
	...
	struct phydm_perpkt_info_struct		*p_pktinfo = (struct
			phydm_perpkt_info_struct *)p_pktinfo_void;
	struct phydm_cfo_track_struct			*p_cfo_track = (struct
			phydm_cfo_track_struct *)phydm_get_structure(p_dm, PHYDM_CFOTRACK);
	u8					i;
