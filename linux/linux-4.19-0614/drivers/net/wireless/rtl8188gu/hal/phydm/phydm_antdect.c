 ...
 *   */
boolean odm_single_dual_antenna_detection(
	void		*p_dm_void,
	u8			mode
)
{
	...
	u8		n;
	u32		reg88c, regc08, reg874, regc50, reg948, regb2c, reg92c, reg930,
	...
	current_channel = odm_get_rf_reg(p_dm, RF_PATH_A, ODM_CHANNEL,
					 RFREGOFFSETMASK);
	...
	/* Set PSD 128 pts */
	odm_set_bb_reg(p_dm, REG_FPGA0_PSD_FUNCTION, BIT(14) | BIT15,
		       0x0); /* 128 pts */

	/* To SET CH1 to do */
	odm_set_rf_reg(p_dm, RF_PATH_A, ODM_CHANNEL, RFREGOFFSETMASK,
		       0x7401);     /* channel 1 */
	...
	if (p_dm->support_ic_type & ODM_RTL8723B) {
		...
		if ((ant_a_report >= 100) && (ant_b_report >= 100)
		    && (ant_a_report <= 135) && (ant_b_report <= 135)) {
			u8 TH1 = 2, TH2 = 6;

			if ((ant_a_report - ant_b_report < TH1)
			    || (ant_b_report - ant_a_report < TH1)) {
				p_dm_swat_table->ANTA_ON = true;
				p_dm_swat_table->ANTB_ON = true;
				PHYDM_DBG(p_dm, DBG_ANT_DIV,
					  ("odm_single_dual_antenna_detection(): Dual Antenna\n"));
			} else if (((ant_a_report - ant_b_report >= TH1)
				    && (ant_a_report - ant_b_report <= TH2)) ||
				   ((ant_b_report - ant_a_report >= TH1)
				    && (ant_b_report - ant_a_report <= TH2))) {
				p_dm_swat_table->ANTA_ON = false;
				p_dm_swat_table->ANTB_ON = false;
				is_result = false;
				PHYDM_DBG(p_dm, DBG_ANT_DIV,
					  ("odm_single_dual_antenna_detection(): Need to check again\n"));
			} else {
				p_dm_swat_table->ANTA_ON = true;
				p_dm_swat_table->ANTB_ON = false;
				PHYDM_DBG(p_dm, DBG_ANT_DIV,
					  ("odm_single_dual_antenna_detection(): Single Antenna\n"));
			}
...
boolean odm_sw_ant_div_check_before_link(
	void		*p_dm_void
)
{
	...
	u8			tmp_power_diff = 0, power_diff = 0, avg_power_diff = 0,
	...
	/* if(HP id) */
	{
		if (p_dm->dm_swat_table.rssi_ant_dect_result == true
		...
		if (p_dm->support_ic_type == ODM_RTL8723B) {
			if (p_dm_swat_table->swas_no_link_bk_reg948 == 0xff)
				p_dm_swat_table->swas_no_link_bk_reg948 = odm_read_4byte(p_dm,
	...
	if (p_hal_data->eRFPowerState != eRfOn || p_mgnt_info->RFChangeInProgress
	    || p_mgnt_info->bMediaConnect) {
	...
	if (p_dm_swat_table->swas_no_link_state == 0) {
		...
		PlatformMoveMemory((void *)adapter->MgntInfo.tmpbssDesc,
				   (void *)p_mgnt_info->bssDesc, sizeof(RT_WLAN_BSS) * MAX_BSS_DESC);
		...
		if (p_dm->support_ic_type & (ODM_RTL8188E | ODM_RTL8821)) {
			...
			if ((p_dm_swat_table->cur_antenna == MAIN_ANT)
			    && (tmp_swas_no_link_bk_reg948 == 0x200)) {
				odm_set_bb_reg(p_dm, REG_S0_S1_PATH_SWITCH, 0xfff, 0x280);
				odm_set_bb_reg(p_dm, REG_AGC_TABLE_SELECT, BIT(31), 0x1);
			...
			PHYDM_DBG(p_dm, DBG_ANT_DIV,
				  ("odm_sw_ant_div_check_before_link: Change to (( %s-ant))  for testing.\n",
		...
		for (index = 0; index < p_mgnt_info->tmpNumBssDesc; index++) {
			...
			if (PlatformCompareMemory(p_test_bss_desc->bdBssIdBuf,
						  p_tmp_bss_desc->bdBssIdBuf, 6) != 0) {
				PHYDM_DBG(p_dm, DBG_ANT_DIV,
			...
			if (p_dm->support_ic_type != ODM_RTL8723B) {
				if (p_tmp_bss_desc->ChannelNumber == scan_channel) {
					if (p_tmp_bss_desc->RecvSignalPower > p_test_bss_desc->RecvSignalPower) {
						...
						RT_PRINT_STR(COMP_SCAN, DBG_WARNING, "GetScanInfo(): new Bss SSID:",
							     p_tmp_bss_desc->bdSsIdBuf, p_tmp_bss_desc->bdSsIdLen);
					...
					} else if (p_tmp_bss_desc->RecvSignalPower <
						   p_test_bss_desc->RecvSignalPower) {
						PHYDM_DBG(p_dm, DBG_ANT_DIV,
							  ("odm_sw_ant_div_check_before_link: Compare scan entry: score--\n"));
						RT_PRINT_STR(COMP_SCAN, DBG_WARNING, "GetScanInfo(): new Bss SSID:",
				...
				if (p_tmp_bss_desc->ChannelNumber == scan_channel) {
					...
					if (p_tmp_bss_desc->RecvSignalPower >
					    p_test_bss_desc->RecvSignalPower) { /* Pow(Ant1) > Pow(Ant2) */
						counter++;
						tmp_power_diff = (u8)(p_tmp_bss_desc->RecvSignalPower -
								      p_test_bss_desc->RecvSignalPower);
						...
						PlatformMoveMemory(p_test_bss_desc, p_tmp_bss_desc, sizeof(RT_WLAN_BSS));
					} else if (p_test_bss_desc->RecvSignalPower >
						   p_tmp_bss_desc->RecvSignalPower) { /* Pow(Ant1) < Pow(Ant2) */
						counter++;
						tmp_power_diff = (u8)(p_test_bss_desc->RecvSignalPower -
						...
						if (p_test_bss_desc->bdTstamp >
						    p_tmp_bss_desc->bdTstamp) { /* Stamp(Ant1) < Stamp(Ant2) */
		...
		if (p_dm->support_ic_type & (ODM_RTL8188E | ODM_RTL8821)) {
			...
			if (counter == 0) {
				...
				/* 2 [ Dual Antenna ] */
				else if ((p_mgnt_info->NumBssDesc != 0)
			...
			PHYDM_DBG(p_dm, DBG_ANT_DIV,
				  ("dual_ant_counter = (( %d )), single_ant_counter = (( %d )) , retry_counter = (( %d )) , aux_fail_detec_counter = (( %d ))\n\n\n",
				   p_dm->dm_swat_table.dual_ant_counter,
				   p_dm->dm_swat_table.single_ant_counter, p_dm->dm_swat_table.retry_counter,
				   p_dm->dm_swat_table.aux_fail_detec_counter));

			/* 2 recover the antenna setting */

			if (p_dm->dm_swat_table.ANTB_ON == false)
				odm_set_bb_reg(p_dm, REG_S0_S1_PATH_SWITCH, 0xfff,
					       (p_dm_swat_table->swas_no_link_bk_reg948));
...
)
{
	...
	channel_ori = odm_get_rf_reg(p_dm, RF_PATH_A, ODM_CHANNEL,
				     RFREGOFFSETMASK);
	reg948 = odm_get_bb_reg(p_dm, REG_S0_S1_PATH_SWITCH, MASKDWORD);
	regb2c =  odm_get_bb_reg(p_dm, REG_AGC_TABLE_SELECT, MASKDWORD);
	regc50 = odm_get_bb_reg(p_dm, REG_OFDM_0_XA_AGC_CORE1, MASKDWORD);
	...
	odm_set_bb_reg(p_dm, REG_FPGA0_RFMOD, BIT(24),
		       0); /* disable whole CCK block */
	odm_write_1byte(p_dm, REG_TXPAUSE,
			0xFF); /* Turn off TX  ->  Pause TX Queue */
	odm_set_bb_reg(p_dm, 0xC14, MASKDWORD,
	...
	odm_set_rf_reg(p_dm, RF_PATH_A, ODM_CHANNEL, 0x7ff,
		       0x04);     /* Set RF to CH4 & 40M */
	odm_set_bb_reg(p_dm, REG_FPGA0_ANALOG_PARAMETER4, 0xf00000,
	...
	for (i = 0; i < test_num; i++) {
		for (tone_idx = 0; tone_idx < tone_lenth_1; tone_idx++) {
			PSD_report_temp = phydm_get_psd_data(p_dm, tone_idx_1[tone_idx],
							     initial_gain);
	...
	for (i = 0; i < test_num; i++) {
		for (tone_idx = 0; tone_idx < tone_lenth_2; tone_idx++) {
			PSD_report_temp = phydm_get_psd_data(p_dm, tone_idx_2[tone_idx],
							     initial_gain);
			/* if(  PSD_report_temp>psd_report_main[tone_idx]  ) */
	...
	odm_write_1byte(p_dm, REG_TXPAUSE,
			0x0); /* Turn on TX	 */ /* Resume TX Queue */
	odm_set_bb_reg(p_dm, 0xC14, MASKDWORD,
