...
u8 odm_get_auto_channel_select_result(
	void			*p_dm_void,
	u8			band
)
{
...
void odm_auto_channel_select_init(
	void			*p_dm_void
)
{
#if (DM_ODM_SUPPORT_TYPE & (ODM_WIN | ODM_CE))
...
void odm_auto_channel_select_reset(
	void			*p_dm_void
)
{
#if (DM_ODM_SUPPORT_TYPE & (ODM_WIN | ODM_CE))
...
)
{
	...
	if (channel >= 1 && channel <= 14) {
		...
		PHYDM_DBG(p_dm, ODM_COMP_API, ("CH_Info[0][%d]=%d, CH_Info[1][%d]=%d\n",
					       channel_idx, p_acs->channel_info_2g[0][channel_idx], channel_idx,
					       p_acs->channel_info_2g[1][channel_idx]));

		for (search_idx = 0; search_idx < ODM_MAX_CHANNEL_2G; search_idx++) {
			if (p_acs->channel_info_2g[1][search_idx] != 0
			    && p_acs->channel_info_2g[0][search_idx] >= max_score) {
...
)
{
	...
	if ((priv->auto_channel != 0)
	    && (priv->auto_channel != 2)) /* if struct _ACS_ running, do not do FA/CCA counter read */
		return true;
	else
		return false;
#else
...
void phydm_auto_channel_select_setting_ap(
	void   *p_dm_void,
...
)
{
	...
	if (setting == STORE_DEFAULT_NHM_SETTING) {
		...
		if (p_dm->support_ic_type &
		    ODM_IC_11AC_SERIES) {  /* store reg0x990, reg0x994, reg0x998, reg0x99c, Reg0x9a0 */
			p_acs->reg0x990 = odm_read_4byte(p_dm,
							 ODM_REG_CCX_PERIOD_11AC);                /* reg0x990 */
			p_acs->reg0x994 = odm_read_4byte(p_dm,
							 ODM_REG_NHM_TH9_TH10_11AC);           /* reg0x994 */
			p_acs->reg0x998 = odm_read_4byte(p_dm,
							 ODM_REG_NHM_TH3_TO_TH0_11AC);       /* reg0x998 */
			p_acs->reg0x99c = odm_read_4byte(p_dm,
							 ODM_REG_NHM_TH7_TO_TH4_11AC);       /* Reg0x99c */
			p_acs->reg0x9a0 = odm_read_1byte(p_dm,
							 ODM_REG_NHM_TH8_11AC);                   /* Reg0x9a0, u8 */
		} else if (p_dm->support_ic_type & ODM_IC_11N_SERIES) {
			p_acs->reg0x890 = odm_read_4byte(p_dm,
							 ODM_REG_NHM_TH9_TH10_11N);             /* reg0x890 */
			p_acs->reg0x894 = odm_read_4byte(p_dm,
							 ODM_REG_CCX_PERIOD_11N);                  /* reg0x894 */
			p_acs->reg0x898 = odm_read_4byte(p_dm,
							 ODM_REG_NHM_TH3_TO_TH0_11N);         /* reg0x898 */
			p_acs->reg0x89c = odm_read_4byte(p_dm,
							 ODM_REG_NHM_TH7_TO_TH4_11N);         /* Reg0x89c */
			p_acs->reg0xe28 = odm_read_1byte(p_dm,
							 ODM_REG_NHM_TH8_11N);                     /* Reg0xe28, u8 */
		}
	}
	...
	/* 3 struct _ACS_ setting */
		...
		if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
			...
			odm_set_bb_reg(p_dm, ODM_REG_NHM_TH9_TH10_11AC, BIT(8) | BIT(9) | BIT(10),
				       3);

			if (p_acs->acs_step == 0) {
				/* 4 Set IGI */
				odm_set_bb_reg(p_dm, 0xc50,
					       BIT(0) | BIT(1) | BIT(2) | BIT(3) | BIT(4) | BIT(5) | BIT(6), 0x3E);
				if (get_rf_mimo_mode(priv) != RF_1T1R)
					odm_set_bb_reg(p_dm, 0xe50,
						       BIT(0) | BIT(1) | BIT(2) | BIT(3) | BIT(4) | BIT(5) | BIT(6), 0x3E);
				...
				/* 4 Set IGI */
				odm_set_bb_reg(p_dm, 0xc50,
					       BIT(0) | BIT(1) | BIT(2) | BIT(3) | BIT(4) | BIT(5) | BIT(6), 0x2A);
				if (get_rf_mimo_mode(priv) != RF_1T1R)
					odm_set_bb_reg(p_dm, 0xe50,
						       BIT(0) | BIT(1) | BIT(2) | BIT(3) | BIT(4) | BIT(5) | BIT(6), 0x2A);
			...
			odm_write_2byte(p_dm, ODM_REG_CCX_PERIOD_11AC + 2, period);
			/* 4 Set NHM ignore_cca=1, ignore_txon=1, ccx_en=0 */
			odm_set_bb_reg(p_dm, ODM_REG_NHM_TH9_TH10_11N, BIT(8) | BIT(9) | BIT(10),
				       3);
...
void phydm_get_nhm_statistics_ap(
	void       *p_dm_void,
...
)
{
	...
	if (p_dm->support_ic_type & ODM_IC_11N_SERIES) {
		...
		if (p_acs->acs_step == 1) {
			...
			value32 = odm_read_4byte(p_dm,
						 ODM_REG_NHM_CNT_11N);   /* ODM_REG_NHM_CNT3_TO_CNT0_11N */
			...
			p_acs->nhm_cnt[idx][4] = odm_read_1byte(p_dm,
								ODM_REG_NHM_CNT7_TO_CNT4_11N);
		...
		if (p_acs->acs_step == 1) {
			...
			value32 = odm_read_4byte(p_dm,
						 ODM_REG_NHM_CNT_11AC);    /* ODM_REG_NHM_CNT3_TO_CNT0_11AC */
...
)
{
	...
	static u32           score2G[MAX_2G_CHANNEL_NUM],
	       score5G[MAX_5G_CHANNEL_NUM];
	u32                  score[MAX_BSS_NUM], use_nhm = 0;
	u32                  minScore = 0xffffffff;
	u32                  tmpScore, tmpIdx = 0;
	u32                  traffic_check = 0;
	u32                  fa_count_weighting = 1;
	int                     i, j, idx = 0, idx_2G_end = -1, idx_5G_begin = -1,
				      minChan = 0;
...
USE_CLN_CH:
		...
		for (y = ch_begin; y < ch_end; y++)  {
			...
			priv->chnl_ss_load[y] = (((fa_lv > cca_lv) ? fa_lv : cca_lv) * 75 + ((
							 score[y] > 100) ? 100 : score[y]) * 25) / 100;
...
choose_ch:
	...
	if (idx_5G_begin >= 0) {
		for (i = idx_5G_begin; i < priv->available_chnl_num; i++) {
			if (!priv->pmib->dot11DFSEntry.disable_DFS
			    && is_DFS_channel(priv->available_chnl[i])
			    && (score[i] != 0xffffffff))
				score[i] += 1600;
		}
	}
		...
		if (idx_5G_begin >= 0) {
			for (i = idx_5G_begin; i < priv->available_chnl_num; i++) {
				...
				if (priv->available_chnl[i] > 144)
					--ch;
				if ((ch % 4) || ch == 140
				    || ch == 164)	/* mask ch 140, ch 165, ch 184... */
					score[i] = 0xffffffff;
			}
		}
#endif
...
#ifdef _DEBUG_RTL8192CD_
	for (i = 0; i < priv->available_chnl_num; i++)
		len += sprintf(tmpbuf + len, "ch%d:%u ", priv->available_chnl[i],
			       score[i]);
	strcat(tmpbuf, "\n");
	...
	if ((*p_dm->p_band_type == ODM_BAND_5G)
		...
		for (i = 0; i < priv->available_chnl_num; i++) {
			if (is80MChannel(priv->available_chnl, priv->available_chnl_num,
		...
		for (i = 0; i < priv->available_chnl_num; i++) {
			if (is40MChannel(priv->available_chnl, priv->available_chnl_num,
					 priv->available_chnl[i])) {
				tmpScore = 0;
				for (j = 0; j < 2; j++) {
					if ((tmpScore != 0xffffffff) && (score[i + j] != 0xffffffff))
			...
			if (score[i] != 0xffffffff && score[i + 4] != 0xffffffff) {
				...
				if (groupScore[i] < minScore) {
					...
					if (priv->pmib->miscEntry.autoch_1611_enable) {
						if (priv->available_chnl[i] == 1 || priv->available_chnl[i] == 6
						    || priv->available_chnl[i] == 11) {
	...
	if ((priv->pmib->dot11BssType.net_work_type & WIRELESS_11N)
...
#endif
		{
			...
			if ((minChan >= 5) && (minChan <= (ch_max - 5))) {
				if (score[minChan + 4] > score[minChan -
							       4]) { /*  what if some channels were cancelled? */
					GET_MIB(priv)->dot11nConfigEntry.dot11n2ndChOffset = HT_2NDCH_OFFSET_BELOW;
					priv->pshare->offset_2nd_chan	= HT_2NDCH_OFFSET_BELOW;
				} else {
					GET_MIB(priv)->dot11nConfigEntry.dot11n2ndChOffset = HT_2NDCH_OFFSET_ABOVE;
