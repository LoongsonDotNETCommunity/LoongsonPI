...
void phydm_set_noise_data_sum(struct noise_level *noise_data,
			      u8 max_rf_path)
{
	u8 rf_path;
...
s16 odm_inband_noise_monitor_n_series(struct PHY_DM_STRUCT	*p_dm,
				      u8 is_pause_dig, u8 igi_value, u32 max_time)
{
	u32				tmp4b;
	...
	while (1) {
		...
		if ((valid_done == max_rf_path)
		    || (odm_get_progressing_time(p_dm, start) > max_time)) {
	...
	reg_c50 &= ~BIT(7);
	p_dm->noise_level.noise[RF_PATH_A] = (s8)(-110 + reg_c50 +
					     noise_data.sum[RF_PATH_A]);
	...
	if (max_rf_path == 2) {
		...
		reg_c58 &= ~BIT(7);
		p_dm->noise_level.noise[RF_PATH_B] = (s8)(-110 + reg_c58 +
						     noise_data.sum[RF_PATH_B]);
...
s16 phydm_idle_noise_measurement_ac(
...
)
{
	...
	if (max_rf_path == 2) {
		...
		reg_e50 &= ~BIT(7);
		p_dm->noise_level.noise[RF_PATH_B] = (s8)(-110 + reg_e50 +
						     noise_data.sum[RF_PATH_B]);
		p_dm->noise_level.noise_all += p_dm->noise_level.noise[RF_PATH_B];
	}
	p_dm->noise_level.noise_all /= max_rf_path;
...
s16 odm_inband_noise_monitor_ac_series(
	struct PHY_DM_STRUCT	*p_dm,
	u8 is_pause_dig,
	u8 igi_value,
	u32 max_time
)
{
	...
	if (p_dm->support_ic_type & (ODM_RTL8822B | ODM_RTL8821C))
		return phydm_idle_noise_measurement_ac(p_dm, is_pause_dig, igi_value,
						       max_time);
	...
	while (1) {
		...
		if ((!pd_flag) || (rxi_buf_anta != 0x200)) {
			...
			pwdb_A = odm_pwdb_conversion(rxi_buf_anta * rxi_buf_anta + rxq_buf_anta *
						     rxq_buf_anta, 20, 18); /*S(10,9)*S(10,9)=S(20,18)*/

			PHYDM_DBG(p_dm, DBG_ENV_MNTR,
				  ("pwdb_A= %d dB, rxi_buf_anta= 0x%x, rxq_buf_anta= 0x%x\n", pwdb_A,
				   rxi_buf_anta & 0x3FF, rxq_buf_anta & 0x3FF));
		}
		/*Start CK320&CK88*/
...
)
{
	...
	igi_value =
		0x32;	/*since HW ability is about +15~-35, we fix IGI = -60 for maximum coverage*/

	if (p_dm->support_ic_type & ODM_IC_11AC_SERIES)
		return odm_inband_noise_monitor_ac_series(p_dm, is_pause_dig, igi_value,
				max_time);
	else
		return odm_inband_noise_monitor_n_series(p_dm, is_pause_dig, igi_value,
				max_time);
}
...
)
{
	...
	p_dm->noisy_decision_smooth = (p_dm->noisy_decision_smooth >> 1) +
				      (score << 2);

	/* Round the noisy_decision_smooth: +"3" comes from (2^3)/2-1 */
	score_smooth = (total_cca_cnt >= 300) ? ((p_dm->noisy_decision_smooth + 3)
			>> 3) : 0;
	...
	PHYDM_DBG(p_dm, DBG_ENV_MNTR,
		  ("[NoisyDetection] CCA_cnt=%d,FA_cnt=%d, noisy_dec_smooth=%d, score=%d, score_smooth=%d, noisy_dec=%d\n",
		   total_cca_cnt, total_fa_cnt, p_dm->noisy_decision_smooth, score,
		   score_smooth, p_dm->noisy_decision));
