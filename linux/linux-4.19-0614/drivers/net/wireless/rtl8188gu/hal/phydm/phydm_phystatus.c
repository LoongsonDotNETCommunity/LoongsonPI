	...
	if (date_rate <= ODM_RATE54M) {
		...
		if ((p_phy_sta_rpt->gid != 0) && (p_phy_sta_rpt->gid != 63)
			...
			if (p_pktinfo->ppdu_cnt < 4) {
				p_phydm->phy_dbg_info.num_of_ppdu[p_pktinfo->ppdu_cnt] = date_rate | BIT(
							7);
...
void phydm_reset_phystatus_avg(
...
void phydm_reset_phystatus_statistic(
	struct PHY_DM_STRUCT	*p_dm
)
{
...
u8 phydm_query_rx_pwr_percentage(
	s8		ant_power
)
{
...
s32 phydm_signal_scale_mapping_92c_series(
	struct PHY_DM_STRUCT *p_dm,
	s32 curr_sig
)
{
	...
	if ((p_dm->support_interface  == ODM_ITRF_USB)
	    || (p_dm->support_interface  == ODM_ITRF_SDIO)) {
...
s32 phydm_signal_scale_mapping(
	struct PHY_DM_STRUCT *p_dm,
	s32 curr_sig
)
{
...
)
{
	...
	RT_TRACE(COMP_DBG, DBG_TRACE,
		 ("is_cck_rate(%#d), PWDB_ALL(%#d), RSSI(%#d), SQ(%#d)\n", is_cck_rate,
		  PWDB_ALL, RSSI, SQ));
...
)
{
	...
	if (is_cck_rate) {
		...
		if (PWDB_ALL ==
		    0) /* Abnormal case, do not indicate the value above 20 on Win7 */
			SQ = 20;
#endif
...
static u8 phydm_evm_db_to_percentage(
	s8 value
)
{
	/*  */
...
static u8 phydm_evm_dbm_jaguar_series(
	s8 value
)
{
	s8 ret_val = value;
...
void phydm_get_cck_rssi_table_from_reg(
	struct PHY_DM_STRUCT	*p_dm
)
{
	u8	used_lna_idx_tmp;
	u32	reg_0xa80 = 0x7431,
		reg_0xabc =
			0xcbe5edfd; /*example: {-53, -43, -33, -27, -19, -13, -3, 1}*/ /*{0xCB, 0xD5, 0xDF, 0xE5, 0xED, 0xF3, 0xFD, 0x2}*/
	u8	i;
	...
	for (i = 0; i <= 3; i++) {
		...
		p_dm->cck_lna_gain_table[used_lna_idx_tmp] = (s8)((reg_0xabc >>
				(8 * i)) & 0xff);
	}
...
)
{
	...
	struct _phy_status_rpt_8192cd *p_phy_sta_rpt = (struct
			_phy_status_rpt_8192cd *)p_phy_status;
	...
	if (is_cck_rate) {
		...
		if (p_dm->support_ic_type & (ODM_RTL8703B)) {
			...
			if (p_dm->cck_agc_report_type == 1) {  /*4 bit LNA*/
				...
				u8 cck_agc_rpt_b = (p_phy_sta_rpt->cck_rpt_b_ofdm_cfosho_b & BIT(
							    7)) ? 1 : 0;
		...
		for (i = RF_PATH_A; i < ODM_RF_PATH_MAX; i++) {
			...
			p_phy_info->rx_snr[i] = p_dm->phy_dbg_info.rx_snr_db[i] = (s8)(
							p_phy_sta_rpt->path_rxsnr[i] / 2);
			...
			if (i == RF_PATH_A) {
				if (p_dm->iot_table.win_patch_id == RT_CID_819X_LENOVO) {
					p_phy_info->signal_quality = phydm_sq_patch_rt_cid_819x_lenovo(p_dm,
								     is_cck_rate, PWDB_ALL, i, RSSI);
				} else if (p_dm->iot_table.win_patch_id == RT_CID_819X_ACER)
					p_phy_info->signal_quality = phydm_sq_patch_rt_cid_819x_acer(p_dm,
								     is_cck_rate, PWDB_ALL, 0, RSSI);
			}
#endif
		}
		...
		/*  */
		rx_pwr_all = (((p_phy_sta_rpt->cck_sig_qual_ofdm_pwdb_all) >> 1) & 0x7f) -
			     110;
	...
	if (is_cck_rate) {
		...
		p_phy_info->signal_strength = SignalScaleProc(p_dm->adapter, PWDB_ALL,
					      true, true);
#else
		p_phy_info->signal_strength = (u8)(phydm_signal_scale_mapping(p_dm,
						   PWDB_ALL));/*PWDB_ALL;*/
...
#if (DM_ODM_SUPPORT_TYPE == ODM_WIN)
			/* 2012/01/12 MH Use customeris signal strength from HalComRxdDesc.c/ */
			p_phy_info->signal_strength = SignalScaleProc(p_dm->adapter,
						      (total_rssi /= rf_rx_num), true, false);
#else
			p_phy_info->signal_strength = (u8)(phydm_signal_scale_mapping(p_dm,
...
)
{
	...
	u8					cck_highpwr = 0;
	u8					LNA_idx, VGA_idx;
	struct _phy_status_rpt_8812 *p_phy_sta_rpt = (struct _phy_status_rpt_8812
			*)p_phy_status;
	...
	if (is_cck_rate) {
		...
		for (i = RF_PATH_A; i < ODM_RF_PATH_MAX_JAGUAR; i++) {
			...
			/*Get Rx snr value in DB*/
			if (i < RF_PATH_C)
				p_phy_info->rx_snr[i] = p_dm->phy_dbg_info.rx_snr_db[i] =
		...
		/*2012.05.25 LukeLee: Testchip AGC report is wrong, it should be restored back to old formula in MP chip*/
		if ((p_dm->support_ic_type & (ODM_RTL8812 | ODM_RTL8821 | ODM_RTL8881A))
		    && (!p_dm->is_mp_chip))
...
#if (DM_ODM_SUPPORT_TYPE == ODM_WIN)
			/* 2012/01/12 MH Use customeris signal strength from HalComRxdDesc.c/*/
			p_phy_info->signal_strength = SignalScaleProc(p_dm->adapter, avg_rssi,
						      false, false);
#else
			p_phy_info->signal_strength = (u8)(phydm_signal_scale_mapping(p_dm,
							   avg_rssi));
	...
	if (p_pktinfo->is_packet_to_self || p_pktinfo->is_packet_beacon) {
		...
		if (!is_cck_rate) { /* ofdm rate */
			...
			if (p_dm->support_ic_type & (ODM_RTL8814A | ODM_RTL8822B)) {
				...
				if (p_dm->rx_ant_status & BB_PATH_B) {
					...
					RX_count++;
					RSSI_linear += odm_convert_to_linear(
							       p_phy_info->rx_mimo_signal_strength[RF_PATH_B]);
				} else
					p_dm->RSSI_B = 0;
				...
				if (p_dm->rx_ant_status & BB_PATH_C) {
					...
					RX_count++;
					RSSI_linear += odm_convert_to_linear(
							       p_phy_info->rx_mimo_signal_strength[RF_PATH_C]);
				} else
					p_dm->RSSI_C = 0;
				...
				if (p_dm->rx_ant_status & BB_PATH_D) {
					...
					RX_count++;
					RSSI_linear += odm_convert_to_linear(
							       p_phy_info->rx_mimo_signal_strength[RF_PATH_D]);
				} else
					p_dm->RSSI_D = 0;
				...
				case 2:
					...
					break;
				case 3:
					RSSI_linear = ((RSSI_linear) + (RSSI_linear << 1) + (RSSI_linear << 3)) >>
...
#endif
			{
				if (p_phy_info->rx_mimo_signal_strength[RF_PATH_B] == 0) {
					...
					if (p_phy_info->rx_mimo_signal_strength[RF_PATH_A] >
					    p_phy_info->rx_mimo_signal_strength[RF_PATH_B]) {
			...
			if (p_sta->rssi_stat.ofdm_pkt_cnt != 64) {
				i = 63;
				p_sta->rssi_stat.ofdm_pkt_cnt -= (u8)(((p_sta->rssi_stat.packet_map >> i) &
								       BIT(0)) - 1);
			}
			...
			if (undecorated_smoothed_cck <= 0) {	/* initialize */
				...
				p_sta->rssi_stat.cck_sum_power = p_sta->rssi_stat.cck_sum_power +
								 (u16)p_phy_info->rx_pwdb_all;
				undecorated_smoothed_cck = p_sta->rssi_stat.cck_sum_power /
							   p_sta->rssi_stat.cck_pkt_cnt;
		...
		/* if(p_entry) */
		{
			...
			if (p_sta->rssi_stat.ofdm_pkt_cnt ==
				...
				if (p_sta->rssi_stat.valid_bit == 64) {
					weighting = ((p_sta->rssi_stat.ofdm_pkt_cnt) > 4) ? 64 :
						    (p_sta->rssi_stat.ofdm_pkt_cnt << 4);
					undecorated_smoothed_pwdb = (s8)((weighting * undecorated_smoothed_ofdm +
									  (64 - weighting) * undecorated_smoothed_cck) >> 6);
					...
					if (p_sta->rssi_stat.valid_bit != 0)
						undecorated_smoothed_pwdb =
							(p_sta->rssi_stat.ofdm_pkt_cnt * undecorated_smoothed_ofdm +
							 (p_sta->rssi_stat.valid_bit - p_sta->rssi_stat.ofdm_pkt_cnt) *
							 undecorated_smoothed_cck) / p_sta->rssi_stat.valid_bit;
					else
					...
					PHYDM_DBG(p_dm, DBG_RSSI_MNTR,
						  ("PWDB_2[%d] = (( %d )), ofdm_pkt = (( %d )), Valid_Bit = (( %d ))\n",
						   p_pktinfo->station_id, undecorated_smoothed_cck,
						   p_sta->rssi_stat.ofdm_pkt_cnt, p_sta->rssi_stat.valid_bit));
				}
			}
			...
			if (send_rssi_2_fw) { /* Trigger init rate by RSSI */
				...
				PHYDM_DBG(p_dm, DBG_RSSI_MNTR,
					  ("[Send to FW] PWDB = (( %d )), ofdm_pkt = (( %d )), cck_pkt = (( %d ))\n",
					   undecorated_smoothed_pwdb, p_sta->rssi_stat.ofdm_pkt_cnt,
					   p_sta->rssi_stat.cck_pkt_cnt));
...
void phydm_set_per_path_phy_info(
	u8							rx_path,
	s8							rx_pwr,
	s8							rx_evm,
	s8							cfo_tail,
...
)
{
	...
	p_phy_info->cfo_tail[rx_path] = ((p_phy_info->cfo_tail[rx_path] << 5) +
					 (p_phy_info->cfo_tail[rx_path] << 2) +
					 (p_phy_info->cfo_tail[rx_path] << 1) + (p_phy_info->cfo_tail[rx_path])) >>
					9;
	...
	/* if (p_pktinfo->is_packet_match_bssid) */
	{
		...
		dbg_print("evm_dbm = %d, Signal quality = %d\n",
			  p_phy_info->rx_mimo_evm_dbm[rx_path],
			  p_phy_info->rx_mimo_signal_quality[rx_path]);
		dbg_print("CFO = %d, SNR = %d\n", p_phy_info->cfo_tail[rx_path],
			  p_phy_info->rx_snr[rx_path]);
	}
#endif
}

void phydm_set_common_phy_info(
	s8							rx_power,
	u8							channel,
...
)
{
	...
	/* if (p_pktinfo->is_packet_match_bssid) */
	{
		dbg_print("rx_pwdb_all = %d, rx_power = %d, recv_signal_power = %d\n",
			  p_phy_info->rx_pwdb_all, p_phy_info->rx_power,
			  p_phy_info->recv_signal_power);
		dbg_print("signal_quality = %d\n", p_phy_info->signal_quality);
		dbg_print("is_beamformed = %d, is_mu_packet = %d, rx_count = %d\n",
			  p_phy_info->is_beamformed, p_phy_info->is_mu_packet,
			  p_phy_info->rx_count + 1);
...
void phydm_get_rx_phy_status_type0(
	struct PHY_DM_STRUCT		*p_dm,
	u8							*p_phy_status,
	struct phydm_perpkt_info_struct		*p_pktinfo,
	struct phydm_phyinfo_struct	*p_phy_info
)
{
	...
	if (p_dm->cck_new_agc == false) {
		...
		if (p_dm->support_ic_type & ODM_RTL8197F)
			rx_power = phydm_cck_rssi_convert(p_dm, p_phy_sta_rpt->lna_l,
							  p_phy_sta_rpt->vga);
		else
#endif
		{
	...
	if (p_dm->support_ic_type & ODM_RTL8197F) {
		...
		if ((p_phy_sta_rpt->bb_power < bb_pwr_th_l)
		    || (p_phy_sta_rpt->bb_power > bb_pwr_th_h))
	...
	/* Update CCK pwdb */
	phydm_set_per_path_phy_info(RF_PATH_A, rx_power, 0, 0, 0,
...
void phydm_get_rx_phy_status_type1(
	struct PHY_DM_STRUCT		*p_dm,
	u8							*p_phy_status,
	struct phydm_perpkt_info_struct		*p_pktinfo,
	struct phydm_phyinfo_struct	*p_phy_info
)
{
	...
	for (i = RF_PATH_A; i < ODM_RF_PATH_MAX_JAGUAR; i++) {
		if (p_dm->rx_ant_status & BIT(i)) {
			...
			phydm_set_per_path_phy_info(i, rx_path_pwr_db,
	...
	/* count BF packet */
	p_dm->phy_dbg_info.num_qry_bf_pkt = p_dm->phy_dbg_info.num_qry_bf_pkt +
					    p_phy_sta_rpt->beamformed;
	...
	} else if (p_dm->support_ic_type & (ODM_RTL8197F | ODM_RTL8723D |
					    ODM_RTL8710B)) {
	...
	phydm_set_common_phy_info(rx_pwr_db, p_phy_sta_rpt->channel,
				  (boolean)p_phy_sta_rpt->beamformed,
	...
	if (p_pktinfo->is_packet_match_bssid) {
		...
		dbg_print("channel = %d, band = %d, l_rxsc = %d, ht_rxsc = %d, rf_mode = %d\n",
			  p_phy_sta_rpt->channel, p_phy_sta_rpt->band, p_phy_sta_rpt->l_rxsc,
			  p_phy_sta_rpt->ht_rxsc, p_phy_sta_rpt->rf_mode);
		dbg_print("Antidx A = %d, B = %d, C = %d, D = %d\n",
			  p_phy_sta_rpt->antidx_a, p_phy_sta_rpt->antidx_b, p_phy_sta_rpt->antidx_c,
			  p_phy_sta_rpt->antidx_d);
		dbg_print("pwdb A: 0x%x, B: 0x%x, C: 0x%x, D: 0x%x\n",
			  p_phy_sta_rpt->pwdb[0], p_phy_sta_rpt->pwdb[1], p_phy_sta_rpt->pwdb[2],
			  p_phy_sta_rpt->pwdb[3]);
		dbg_print("EVM  A: %d, B: %d, C: %d, D: %d\n", p_phy_sta_rpt->rxevm[0],
			  p_phy_sta_rpt->rxevm[1], p_phy_sta_rpt->rxevm[2], p_phy_sta_rpt->rxevm[3]);
		dbg_print("SNR  A: %d, B: %d, C: %d, D: %d\n", p_phy_sta_rpt->rxsnr[0],
			  p_phy_sta_rpt->rxsnr[1], p_phy_sta_rpt->rxsnr[2], p_phy_sta_rpt->rxsnr[3]);
		dbg_print("CFO  A: %d, B: %d, C: %d, D: %d\n", p_phy_sta_rpt->cfo_tail[0],
			  p_phy_sta_rpt->cfo_tail[1], p_phy_sta_rpt->cfo_tail[2],
			  p_phy_sta_rpt->cfo_tail[3]);
		dbg_print("paid = %d, gid = %d, length = %d\n",
			  (p_phy_sta_rpt->paid + (p_phy_sta_rpt->paid_msb << 8)), p_phy_sta_rpt->gid,
			  p_phy_sta_rpt->lsig_length);
		dbg_print("ldpc: %d, stbc: %d, bf: %d, gnt_bt: %d, antsw: %d\n",
			  p_phy_sta_rpt->ldpc, p_phy_sta_rpt->stbc, p_phy_sta_rpt->beamformed,
			  p_phy_sta_rpt->gnt_bt, p_phy_sta_rpt->hw_antsw_occu);
		dbg_print("NBI: %d, pos: %d\n", p_phy_sta_rpt->nb_intf_flag,
			  (p_phy_sta_rpt->intf_pos + (p_phy_sta_rpt->intf_pos_msb << 8)));
...
void phydm_get_rx_phy_status_type2(
	struct PHY_DM_STRUCT						*p_dm,
	u8							*p_phy_status,
	struct phydm_perpkt_info_struct				*p_pktinfo,
	struct phydm_phyinfo_struct				*p_phy_info
)
{
...
#if (RTL8197F_SUPPORT == 1)
			if ((p_dm->support_ic_type & ODM_RTL8197F)
			    && (p_phy_sta_rpt->pwdb[i] == 0x7f)) { /*for 97f workaround*/
	...
	/* if (p_pktinfo->is_packet_match_bssid) */
	{
		dbg_print("channel = %d, band = %d, l_rxsc = %d, ht_rxsc = %d\n",
			  p_phy_sta_rpt->channel, p_phy_sta_rpt->band, p_phy_sta_rpt->l_rxsc,
			  p_phy_sta_rpt->ht_rxsc);
		dbg_print("pwdb A: 0x%x, B: 0x%x, C: 0x%x, D: 0x%x\n",
			  p_phy_sta_rpt->pwdb[0], p_phy_sta_rpt->pwdb[1], p_phy_sta_rpt->pwdb[2],
			  p_phy_sta_rpt->pwdb[3]);
		dbg_print("Agc table A: 0x%x, B: 0x%x, C: 0x%x, D: 0x%x\n",
			  p_phy_sta_rpt->agc_table_a, p_phy_sta_rpt->agc_table_b,
			  p_phy_sta_rpt->agc_table_c, p_phy_sta_rpt->agc_table_d);
		dbg_print("Gain A: 0x%x, B: 0x%x, C: 0x%x, D: 0x%x\n",
			  p_phy_sta_rpt->gain_a, p_phy_sta_rpt->gain_b, p_phy_sta_rpt->gain_c,
			  p_phy_sta_rpt->gain_d);
		dbg_print("TRSW A: 0x%x, B: 0x%x, C: 0x%x, D: 0x%x\n",
			  p_phy_sta_rpt->trsw_a, p_phy_sta_rpt->trsw_b, p_phy_sta_rpt->trsw_c,
			  p_phy_sta_rpt->trsw_d);
		dbg_print("AAGC step A: 0x%x, B: 0x%x, C: 0x%x, D: 0x%x\n",
			  p_phy_sta_rpt->aagc_step_a, p_phy_sta_rpt->aagc_step_b,
			  p_phy_sta_rpt->aagc_step_c, p_phy_sta_rpt->aagc_step_d);
		dbg_print("HT AAGC gain A: 0x%x, B: 0x%x, C: 0x%x, D: 0x%x\n",
			  p_phy_sta_rpt->ht_aagc_gain[0], p_phy_sta_rpt->ht_aagc_gain[1],
			  p_phy_sta_rpt->ht_aagc_gain[2], p_phy_sta_rpt->ht_aagc_gain[3]);
		dbg_print("DAGC gain A: 0x%x, B: 0x%x, C: 0x%x, D: 0x%x\n",
			  p_phy_sta_rpt->dagc_gain[0], p_phy_sta_rpt->dagc_gain[1],
			  p_phy_sta_rpt->dagc_gain[2], p_phy_sta_rpt->dagc_gain[3]);
		dbg_print("ldpc: %d, stbc: %d, bf: %d, gnt_bt: %d, antsw: %d\n",
			  p_phy_sta_rpt->ldpc, p_phy_sta_rpt->stbc, p_phy_sta_rpt->beamformed,
			  p_phy_sta_rpt->gnt_bt, p_phy_sta_rpt->hw_antsw_occu);
		dbg_print("counter: %d, syn_count: %d\n", p_phy_sta_rpt->counter,
			  p_phy_sta_rpt->syn_count);
		dbg_print("cnt_cca2agc_rdy: %d, cnt_pw2cca: %d, shift_l_map\n",
			  p_phy_sta_rpt->cnt_cca2agc_rdy, p_phy_sta_rpt->cnt_pw2cca,
			  p_phy_sta_rpt->shift_l_map);
		dbg_print("rsvd_0 = %d, rsvd_1 = %d, rsvd_2 = %d, rsvd_3 = %d, rsvd_4 = %d, rsvd_5 = %d\n",
			  p_phy_sta_rpt->rsvd_0, p_phy_sta_rpt->rsvd_1, p_phy_sta_rpt->rsvd_2,
			  p_phy_sta_rpt->rsvd_3, p_phy_sta_rpt->rsvd_4);
		dbg_print("rsvd_5 = %d, rsvd_6 = %d, rsvd_6 = %d\n", p_phy_sta_rpt->rsvd_5,
			  p_phy_sta_rpt->rsvd_6, p_phy_sta_rpt->rsvd_7);
	}
#endif
}

void phydm_get_rx_phy_status_type5(
	u8				*p_phy_status
)
{
	/*
...
)
{
	...
	if (p_pktinfo->is_packet_to_self || p_pktinfo->is_packet_beacon) {
		...
		if (undecorated_smoothed_pwdb <= 0) {
			...
			accumulate_pwdb = accumulate_pwdb - (accumulate_pwdb >> scaling_factor) +
					  rssi_ave;
			undecorated_smoothed_pwdb = (accumulate_pwdb + (1 <<
...
#if (DM_ODM_SUPPORT_TYPE == ODM_WIN)
	p_phy_info->signal_strength = SignalScaleProc(p_dm->adapter,
				      p_phy_info->rx_pwdb_all, false, false);
...
#if	ODM_IC_11AC_SERIES_SUPPORT
		phydm_rx_phy_status_jaguar_series_parsing(p_dm, p_phy_info, p_phy_status,
				p_pktinfo);
		phydm_process_rssi_for_dm(p_dm, p_phy_info, p_pktinfo);
#endif
	} else if (p_dm->support_ic_type & ODM_IC_11N_SERIES) {
#if	ODM_IC_11N_SERIES_SUPPORT
		phydm_rx_phy_status92c_series_parsing(p_dm, p_phy_info, p_phy_status,
						      p_pktinfo);
		phydm_process_rssi_for_dm(p_dm, p_phy_info, p_pktinfo);
#endif
	}
