...
)
{
	...
	if (en == 1) {
		PHYDM_DBG(p_dm, ODM_COMP_API, ("FW fix TX rate[id =%d], %dM, Rate(%d)=",
					       macid, (20 << bw), rate));
...
)
{
	...
	if ((strcmp(input[1], help) == 0)) {
		...
		PHYDM_SNPRINTF((output + used, out_len - used,
				"[FW fix TX Rate] {en, macid,bw,rate}={%d, %d, %d, 0x%x}", var1[1],
	...
	if (para_idx == RADBG_RTY_PENALTY) {
		...
		for (i = 0 ; i < (rate_type_length) ; i++) {
			...
			PHYDM_DBG(p_dm, DBG_RA, ("%8d  %15d\n", (rate_type_start + i),
						 p_ra_table->RTY_P[rate_type_start + i]));
		}
		...
		for (i = 0; i < (rate_type_length); i++) {
			if (p_ra_table->is_ra_dbg_init)
				p_ra_table->RATE_UP_RTY_RATIO_default[rate_type_start + i] = cmd_buf[2 +
						i];

			p_ra_table->RATE_UP_RTY_RATIO[rate_type_start + i] = cmd_buf[2 + i];
			PHYDM_DBG(p_dm, DBG_RA, ("%8d  %15d\n", (rate_type_start + i),
						 p_ra_table->RATE_UP_RTY_RATIO[rate_type_start + i]));
		}
		...
		for (i = 0; i < (rate_type_length); i++) {
			if (p_ra_table->is_ra_dbg_init)
				p_ra_table->RATE_DOWN_RTY_RATIO_default[rate_type_start + i] = cmd_buf[2 +
						i];

			p_ra_table->RATE_DOWN_RTY_RATIO[rate_type_start + i] = cmd_buf[2 + i];
			PHYDM_DBG(p_dm, DBG_RA, ("%8d  %15d\n", (rate_type_start + i),
						 p_ra_table->RATE_DOWN_RTY_RATIO[rate_type_start + i]));
		}
	} else
#endif
		if (para_idx == RADBG_DEBUG_MONITOR1) {
			...
			if (p_dm->support_ic_type & PHYDM_IC_3081_SERIES) {
				...
				PHYDM_DBG(p_dm, DBG_FW_TRACE, ("%5s  %x%x  %x%x  %x%x  %x%x\n", "RA Mask:",
							       cmd_buf[8], cmd_buf[7], cmd_buf[6], cmd_buf[5], cmd_buf[4], cmd_buf[3],
							       cmd_buf[2], cmd_buf[1]));
			}
			PHYDM_DBG(p_dm, DBG_FW_TRACE, ("-------------------------------\n"));
			...
			PHYDM_DBG(p_dm, DBG_FW_TRACE, ("%5s  0x%x%x%x%x%x\n", "RA mask =",
						       cmd_buf[11], cmd_buf[10], cmd_buf[9], cmd_buf[8], cmd_buf[7]));
		}
}

void phydm_ra_dynamic_retry_count(
	void	*p_dm_void
)
{
	struct PHY_DM_STRUCT		*p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
...
)
{
	...
	PHYDM_DBG_F(p_dm, dbg_component, ("( %s%s%s%s%d%s%s)\n",
					  ((rate_idx >= ODM_RATEVHTSS1MCS0)
					   && (rate_idx <= ODM_RATEVHTSS1MCS9)) ? "VHT 1ss  " : "",
					  ((rate_idx >= ODM_RATEVHTSS2MCS0)
					   && (rate_idx <= ODM_RATEVHTSS2MCS9)) ? "VHT 2ss " : "",
					  ((rate_idx >= ODM_RATEVHTSS3MCS0)
					   && (rate_idx <= ODM_RATEVHTSS3MCS9)) ? "VHT 3ss " : "",
					  (rate_idx >= ODM_RATEMCS0) ? "MCS " : "",
					  (vht_en) ? ((rate_idx - ODM_RATEVHTSS1MCS0) % 10) : ((
							  rate_idx >= ODM_RATEMCS0) ? (rate_idx - ODM_RATEMCS0) : ((
									  rate_idx <= ODM_RATE54M) ? legacy_table[rate_idx] : 0)),
					  (b_sgi) ? "-S" : "  ",
					  (rate_idx >= ODM_RATEMCS0) ? "" : "M"));
}

void phydm_c2h_ra_report_handler(
	void	*p_dm_void,
	u8   *cmd_buf,
	u8   cmd_len
)
{
	...
	if (cmd_len >= 6) {
		...
		PHYDM_DBG(p_dm, DBG_RA, ("RA retry ratio: [%d]:",
					 p_ra_table->ra_ratio[macid]));
		/**/
	} else if (cmd_len >= 4) {
		if (cmd_buf[3] == 0) {
...
void odm_ra_post_action_on_assoc(
	void	*p_dm_void
)
{
#if 0
...
void phydm_modify_RA_PCR_threshold(
	void		*p_dm_void,
...
)
{
	...
	PHYDM_DBG(p_dm, DBG_RA, ("Set RA_threshold_offset = (( %s%d ))\n",
				 ((RA_threshold_offset == 0) ? " " : ((RA_offset_direction) ? "+" : "-")),
				 RA_threshold_offset));
}

#if (DM_ODM_SUPPORT_TYPE == ODM_WIN)

void odm_refresh_rate_adaptive_mask_mp(
	void		*p_dm_void
)
{
	struct PHY_DM_STRUCT				*p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	...
	while (p_loop_adapter) {
		...
		if (p_loop_mgnt_info->mAssoc && (!ACTING_AS_AP(p_loop_adapter))) {
			odm_refresh_ldpc_rts_mp(p_loop_adapter, p_dm, p_loop_mgnt_info->mMacId,
						p_loop_mgnt_info->IOTPeer, p_loop_hal_data->UndecoratedSmoothedPWDB);
			/*PHYDM_DBG(p_dm, DBG_RA_MASK, ("Infrasture mode\n"));*/

			ratr_state_new = phydm_rssi_lv_dec(p_dm,
							   p_loop_hal_data->UndecoratedSmoothedPWDB, p_loop_mgnt_info->Ratr_State);

			if ((p_loop_mgnt_info->Ratr_State != ratr_state_new)
			    || (p_ra_table->up_ramask_cnt >= FORCED_UPDATE_RAMASK_PERIOD)) {

				p_ra_table->up_ramask_cnt = 0;
				PHYDM_PRINT_ADDR(p_dm, DBG_RA_MASK, ("Target AP addr :"),
						 p_loop_mgnt_info->Bssid);
				PHYDM_DBG(p_dm, DBG_RA_MASK,
					  ("Update RA Level: ((%x)) -> ((%x)),  RSSI = ((%d))\n\n",
					   p_mgnt_info->Ratr_State, ratr_state_new,
					   p_loop_hal_data->UndecoratedSmoothedPWDB));

				p_loop_mgnt_info->Ratr_State = ratr_state_new;
				p_adapter->HalFunc.UpdateHalRAMaskHandler(p_loop_adapter,
						p_loop_mgnt_info->mMacId, NULL, ratr_state_new);
			} else {
	...
	if (p_target_adapter != NULL) {
		for (i = 0; i < ODM_ASSOCIATE_ENTRY_NUM; i++) {
			...
			if (is_sta_active((&GET_STA_INFO(p_entry)))) {
				...
				odm_refresh_ldpc_rts_mp(p_target_adapter, p_dm,
							GET_STA_INFO(p_entry).mac_id, p_entry->IOTPeer,
							GET_STA_INFO(p_entry).rssi_stat.rssi);

				ratr_state_new = phydm_rssi_lv_dec(p_dm,
								   GET_STA_INFO(p_entry).rssi_stat.rssi,
								   GET_STA_INFO(p_entry).ra_info.rssi_level);

				if ((GET_STA_INFO(p_entry).ra_info.rssi_level != ratr_state_new)
				    || (p_ra_table->up_ramask_cnt >= FORCED_UPDATE_RAMASK_PERIOD)) {

					p_ra_table->up_ramask_cnt = 0;
					PHYDM_PRINT_ADDR(p_dm, DBG_RA_MASK, ("Target AP addr :"),
							 GET_STA_INFO(p_entry).mac_addr);
					PHYDM_DBG(p_dm, DBG_RA_MASK,
					...
					GET_STA_INFO(p_entry).ra_info.rssi_level = ratr_state_new;
					p_adapter->HalFunc.UpdateHalRAMaskHandler(p_target_adapter,
							GET_STA_INFO(p_entry).mac_id, p_entry, ratr_state_new);
				} else {
					PHYDM_DBG(p_dm, DBG_RA_MASK, ("Stay in RA level  = (( %d ))\n\n",
								      ratr_state_new));
					/**/
...
)
{
	...
	for (i = 0; i < ODM_ASSOCIATE_ENTRY_NUM; i++) {
		...
		if (is_sta_active(p_sta)) {
			...
			ratr_state_new = phydm_rssi_lv_dec(p_dm, (u32)p_sta->rssi_stat.rssi,
							   p_sta->ra_info.rssi_level);

			if ((p_sta->ra_info.rssi_level != ratr_state_new)
			    || (p_ra_table->up_ramask_cnt >= FORCED_UPDATE_RAMASK_PERIOD)) {
...
void phydm_rate_adaptive_mask_init(
	void	*p_dm_void
)
{
	struct PHY_DM_STRUCT		*p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
...
void phydm_refresh_rate_adaptive_mask(
	void	*p_dm_void
)
{
	struct PHY_DM_STRUCT		*p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
...
)
{
	...
	for (i = macid_start; i < macid_end; i++) {
		...
		PHYDM_SNPRINTF((output + used, out_len - used, "ADDR:%x-%x-%x-%x-%x-%x\n",
				p_sta->mac_addr[5], p_sta->mac_addr[4], p_sta->mac_addr[3],
				p_sta->mac_addr[2], p_sta->mac_addr[1], p_sta->mac_addr[0]));
		...
		PHYDM_SNPRINTF((output + used, out_len - used,
				...
				p_sta->rssi_stat.rssi, p_sta->rssi_stat.rssi_ofdm,
...
)
{
	...
	if (p_dm->support_ic_type == ODM_RTL8188E)
		/*Driver RA*/
		odm_ra_update_rate_info_8188e(p_dm, macid, p_ra->rate_id, (u32)ra_mask,
					      p_ra->is_support_sgi);
	else
#endif
	{
...
)
{
	...
	for (macid = 0; macid < ODM_ASSOCIATE_ENTRY_NUM; macid++) {
		...
		rssi_lv_new = phydm_rssi_lv_dec(p_dm, (u32)p_sta->rssi_stat.rssi,
						p_ra->rssi_level);
		...
		if ((p_ra->rssi_level != rssi_lv_new) ||
			...
			{
				/*FW RA*/
				phydm_ra_h2c(p_dm, macid, p_ra->disable_ra, p_ra->disable_pt, 1, 0,
					     ra_mask);
			}
		}
	}
	...
	if (p_dm->number_linked_client != 0) {
		for (macid = 0; macid < ODM_ASSOCIATE_ENTRY_NUM; macid++) {
			...
			if (is_sta_active(p_sta)) {
				...
				rate_order_tmp = phydm_rate_order_compute(p_dm,
						 (p_sta->ra_info.curr_tx_rate & 0x7f));
...
)
{
	...
	if (cur_stage != stage) {
		if (cur_stage == 1) {
			...
			phydm_set_hw_reg_handler_interface(p_dm, HW_VAR_BASIC_RATE,
							   (u8 *)&os_rate_set);
		} else if (cur_stage == 3 && (stage == 1 || stage == 2))
			phydm_set_hw_reg_handler_interface(p_dm, HW_VAR_BASIC_RATE,
							   (u8 *)(&p_mgnt_info->mBrates));
	}

	stage = cur_stage;
}

void odm_refresh_ldpc_rts_mp(
...
void odm_rate_adaptive_state_ap_init(
	void		*PADAPTER_VOID,
...
)
{
	...
	for (i = 0; i < ODM_NUM_RATE_IDX; i++) {
		phydm_retry_limit_table_bound(p_dm,
					      &(p_ra_table->per_rate_retrylimit_20M[i]), 0);
		phydm_retry_limit_table_bound(p_dm,
					      &(p_ra_table->per_rate_retrylimit_40M[i]), 0);
	}
}
	...
	if (p_dm->pre_number_active_client == p_dm->number_active_client) {
		...
		if (p_dm->number_active_client == 1) {
			...
			for (i = 0; i < ODM_NUM_RATE_IDX; i++) {
				...
				phydm_retry_limit_table_bound(p_dm,
							      &(p_ra_table->per_rate_retrylimit_20M[i]), retry_offset);
				phydm_retry_limit_table_bound(p_dm,
							      &(p_ra_table->per_rate_retrylimit_40M[i]), retry_offset);
			}
		}
	}
}
...
)
{
	...
	if ((p_dm->rf_type == RF_2T2R) || (p_dm->rf_type == RF_2T3R)
	    || (p_dm->rf_type == RF_2T4R)) {

		if ((p_dm->support_ic_type & (ODM_RTL8812 | ODM_RTL8192E)) &&
		    (wireless_mode & (ODM_WM_N24G | ODM_WM_N5G))
		   ) {
			PHYDM_DBG(p_dm, DBG_RA, ("[ON ASSOC] set N-2SS ARFR5 table\n"));
			odm_set_mac_reg(p_dm, 0x4a4, MASKDWORD,
					0xfc1ffff);	/*N-2SS, ARFR5, rate_id = 0xe*/
			odm_set_mac_reg(p_dm, 0x4a8, MASKDWORD,
					0x0);		/*N-2SS, ARFR5, rate_id = 0xe*/
		} else if ((p_dm->support_ic_type & (ODM_RTL8812)) &&
			   (wireless_mode & (ODM_WM_AC_5G | ODM_WM_AC_24G | ODM_WM_AC_ONLY))
			  ) {
			PHYDM_DBG(p_dm, DBG_RA, ("[ON ASSOC] set AC-2SS ARFR0 table\n"));
			odm_set_mac_reg(p_dm, 0x444, MASKDWORD,
					0x0fff);	/*AC-2SS, ARFR0, rate_id = 0x9*/
			odm_set_mac_reg(p_dm, 0x448, MASKDWORD,
					0xff01f000);		/*AC-2SS, ARFR0, rate_id = 0x9*/
		}
	}
			...
			if (wireless_mode & (ODM_WM_N24G | ODM_WM_N5G)) { /*N mode*/
				if (rate_idx >= ODM_RATEMCS8 && rate_idx <= ODM_RATEMCS15) { /*2SS mode*/
					...
					PHYDM_DBG(p_dm, DBG_RA, ("ARFR_5\n"));
						    ...
						    ODM_WM_AC_ONLY)) {/*AC mode*/
				if (rate_idx >= ODM_RATEVHTSS2MCS0
				    && rate_idx <= ODM_RATEVHTSS2MCS9) {/*2SS mode*/
...
#if (defined(CONFIG_RA_DBG_CMD))
void odm_ra_para_adjust_send_h2c(
	void	*p_dm_void
)
{
	...
	if (p_ra_table->ra_para_feedback_req) { /*h2c_parameter[5]=1 ; ask FW for all RA parameters*/
		...
		if (p_ra_table->para_idx == RADBG_RTY_PENALTY
		    || p_ra_table->para_idx == RADBG_RATE_UP_RTY_RATIO
		    || p_ra_table->para_idx == RADBG_RATE_DOWN_RTY_RATIO) {
...
)
{
	...
	for (i = 0 ; i <= (p_ra_table->rate_length); i++) {
		...
		PHYDM_DBG(p_dm, DBG_RA, ("     [ %d ]  %20d  %25d  %20s\n", i, value[i],
					 value_default[i], ((modify_note[i] == 1) ? "V" : " .  ")));
#else
...
)
{
	...
	u8			rate_size_ht_1ss = 20, rate_size_ht_2ss = 28,
				rate_size_ht_3ss = 36;	 /*4+8+8+8+8 =36*/
	u8			rate_size_vht_1ss = 10, rate_size_vht_2ss = 20,
				rate_size_vht_3ss = 30;	 /*10 + 10 +10 =30*/
#if 0
	/* RTY_PENALTY		=	1,   u8 */
	/* N_HIGH 				=	2, */
	/* N_LOW				=	3, */
	...
	/* JJ ADD 20161014 */
	if (p_dm->support_ic_type & (ODM_RTL8188F | ODM_RTL8195A | ODM_RTL8703B |
				     ODM_RTL8723B | ODM_RTL8188E | ODM_RTL8723D | ODM_RTL8710B))
