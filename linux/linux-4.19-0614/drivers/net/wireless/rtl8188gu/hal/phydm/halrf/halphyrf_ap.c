		...
		if (priv->pshare->rf_ft_var.pwr_track_file) {
			ODM_RT_TRACE(p_dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD,
				     ("diff: (%s)%d ==> get index from table : %d)\n",
				      (is_decrease ? "-" : "+"), delta, get_tx_tracking_index(priv, channel, i,
						      delta, is_decrease, 0)));
...
)
{
	...
	(*c.get_delta_all_swing_table)(p_dm, (u8 **)&delta_swing_table_idx_tup_a,
	...
	thermal_value = (u8)odm_get_rf_reg(p_dm, RF_PATH_A, c.thermal_reg_addr,
	...
	/* calculate average thermal meter */
	p_dm->rf_calibrate_info.thermal_value_avg[p_dm->rf_calibrate_info.thermal_value_avg_index]
		= thermal_value;
	p_dm->rf_calibrate_info.thermal_value_avg_index++;

	if (p_dm->rf_calibrate_info.thermal_value_avg_index ==
			...
			if (is_increase) {			/*thermal is higher than base*/
				for (p = RF_PATH_A; p < c.rf_path_count; p++) {
					...
					case RF_PATH_B:
						...
						p_rf_calibrate_info->absolute_cck_swing_idx[p] =
							delta_swing_table_idx_tup_cck_b[delta];
						ODM_RT_TRACE(p_dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD,
				...
				for (p = RF_PATH_A; p < c.rf_path_count; p++) {
					...
					case RF_PATH_B:
						...
						p_rf_calibrate_info->absolute_ofdm_swing_idx[p] = -1 *
								delta_swing_table_idx_tdown_b[delta];
						p_rf_calibrate_info->absolute_cck_swing_idx[p] = -1 *
								delta_swing_table_idx_tdown_cck_b[delta];
						ODM_RT_TRACE(p_dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD,
...
)
{
	...
	p_rf_calibrate_info->default_ofdm_index =
		priv->pshare->OFDM_index0[RF_PATH_A];
	...
	if (p_dm->support_ic_type & ODM_RTL8814A)	/* for 8814 path C & D */
		(*c.get_delta_swing_table8814only)(p_dm,
						   (u8 **)&delta_swing_table_idx_tup_c, (u8 **)&delta_swing_table_idx_tdown_c,
	...
	is_tssi_enable = (boolean)odm_get_rf_reg(p_dm, RF_PATH_A,
	...
	for (p = RF_PATH_A ; p < c.rf_path_count ; p++) {
		...
		ODM_RT_TRACE(p_dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD,
			     ("0x%x:0x%x ([31:21] = 0x%x)\n", bb_swing_reg[p], odm_get_bb_reg(p_dm,
					     bb_swing_reg[p], MASKDWORD), ele_D));
...
)
{
	...
	for (rf_path = 0 ; rf_path < max_rf_path ; rf_path++) {
		...
		ODM_RT_TRACE(p_dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD,
			     ("0x%x:0x%x ([31:21] = 0x%x)\n", bb_swing_reg[rf_path],
			      phy_query_bb_reg(priv, bb_swing_reg[rf_path], MASKDWORD), ele_D));
	...
	ODM_RT_TRACE(p_dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD,
		     ("0xc1c:0x%x ([31:21] = 0x%x)\n", phy_query_bb_reg(priv, 0xc1c, MASKDWORD),
		      ele_D));
	for (i = 0; i < OFDM_TABLE_SIZE_8812; i++) {/* 4 */
		if (ele_D == ofdm_swing_table_8812[i]) {
			OFDM_index[0] = (unsigned char)i;
	...
	if (rf == 2) {
		...
		ODM_RT_TRACE(p_dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD,
			     ("0xe1c:0x%x ([32:21] = 0x%x)\n", phy_query_bb_reg(priv, 0xe1c, MASKDWORD),
			      ele_D));
		...
		/* if (*p_dm->p_band_type == ODM_BAND_5G) */
		{
			...
			if (priv->pshare->rf_ft_var.pwr_track_file) {
				for (rf_path = 0; rf_path < max_rf_path; rf_path++) {
					ODM_RT_TRACE(p_dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD,
						     ("diff: (%s)%d ==> get index from table : %d)\n",
						      (is_decrease ? "-" : "+"), delta, get_tx_tracking_index(priv, channel,
								      rf_path, delta, is_decrease, 0)));
					if (is_decrease) {
...
)
{
	...
	if (p_dm->support_ic_type & ODM_RTL8812
	    || p_dm->support_ic_type & ODM_RTL8881A) {
	...
	p_dm->rf_calibrate_info.txpowertrack_control =
		p_hal_data->txpowertrack_control; /* <Kordan> We should keep updating the control variable according to HalData.
...
#endif
	ODM_RT_TRACE(p_dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD,
		     ("===>odm_txpowertracking_callback_thermal_meter_8188e, p_dm->bb_swing_idx_cck_base: %d, p_dm->bb_swing_idx_ofdm_base: %d\n",
		      p_rf_calibrate_info->bb_swing_idx_cck_base,
		      p_rf_calibrate_info->bb_swing_idx_ofdm_base));
	/*
	...
	*/
	...
	if (thermal_value_avg_count) {
		...
		p_rf_calibrate_info->thermal_value_delta = thermal_value -
				priv->pmib->dot11RFEntry.ther;
		ODM_RT_TRACE(p_dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD,
			     ("AVG Thermal Meter = 0x%x\n", thermal_value));
	}
	...
	delta	  = (thermal_value > p_dm->rf_calibrate_info.thermal_value) ?
		    (thermal_value - p_dm->rf_calibrate_info.thermal_value) :
		    (p_dm->rf_calibrate_info.thermal_value - thermal_value);
	delta_LCK = (thermal_value > p_dm->rf_calibrate_info.thermal_value_lck) ?
		    (thermal_value - p_dm->rf_calibrate_info.thermal_value_lck) :
		    (p_dm->rf_calibrate_info.thermal_value_lck - thermal_value);
	delta_IQK = (thermal_value > p_dm->rf_calibrate_info.thermal_value_iqk) ?
		    (thermal_value - p_dm->rf_calibrate_info.thermal_value_iqk) :
		    (p_dm->rf_calibrate_info.thermal_value_iqk - thermal_value);
	...
	if (delta > 0 && p_dm->rf_calibrate_info.txpowertrack_control) {
		...
		delta = (thermal_value > p_dm->priv->pmib->dot11RFEntry.ther) ?
			(thermal_value - p_dm->priv->pmib->dot11RFEntry.ther) :
			(p_dm->priv->pmib->dot11RFEntry.ther - thermal_value);
		...
		if (thermal_value > p_dm->priv->pmib->dot11RFEntry.ther) {
			CALCULATE_SWINGTALBE_OFFSET(offset, POWER_INC, index_mapping_NUM_88E,
						    delta);
			p_dm->rf_calibrate_info.delta_power_index_last =
				p_dm->rf_calibrate_info.delta_power_index;
			p_dm->rf_calibrate_info.delta_power_index =
			...
			p_dm->rf_calibrate_info.delta_power_index_last =
				p_dm->rf_calibrate_info.delta_power_index;
			p_dm->rf_calibrate_info.delta_power_index = (-1) *
					delta_swing_table_idx[POWER_DEC][offset];
		}

		if (p_dm->rf_calibrate_info.delta_power_index ==
		    p_dm->rf_calibrate_info.delta_power_index_last)
			p_dm->rf_calibrate_info.power_index_offset = 0;
		else
			p_dm->rf_calibrate_info.power_index_offset =
				p_dm->rf_calibrate_info.delta_power_index -
				p_dm->rf_calibrate_info.delta_power_index_last;

		for (i = 0; i < rf; i++)
			p_dm->rf_calibrate_info.OFDM_index[i] =
				p_rf_calibrate_info->bb_swing_idx_ofdm_base +
				p_dm->rf_calibrate_info.power_index_offset;
		p_dm->rf_calibrate_info.CCK_index =
			p_rf_calibrate_info->bb_swing_idx_cck_base +
			p_dm->rf_calibrate_info.power_index_offset;

		p_rf_calibrate_info->bb_swing_idx_cck = p_dm->rf_calibrate_info.CCK_index;
		p_rf_calibrate_info->bb_swing_idx_ofdm[RF_PATH_A] =
			p_dm->rf_calibrate_info.OFDM_index[RF_PATH_A];
	...
	if (p_dm->rf_calibrate_info.power_index_offset != 0
	    && p_dm->rf_calibrate_info.txpowertrack_control) {
		/* 4 7.2 Configure the Swing Table to adjust Tx Power. */

		p_dm->rf_calibrate_info.is_tx_power_changed =
		...
		if (thermal_value > p_dm->rf_calibrate_info.thermal_value) {
			...
			/*	p_dm->rf_calibrate_info.power_index_offset, delta, thermal_value, p_hal_data->eeprom_thermal_meter, p_dm->rf_calibrate_info.thermal_value)); */
			   ...
			   p_dm->rf_calibrate_info.thermal_value) { /* Low temperature */
			/* ODM_RT_TRACE(p_dm,ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD, */
