...
static	u32 phy_RFSerialRead_8710B(
	IN	PADAPTER			Adapter,
	IN	enum rf_path			eRFPath,
	IN	u32				Offset
)
{
	...
	_enter_critical_mutex(&(adapter_to_dvobj(Adapter)->rf_read_reg_mutex),
			      NULL);
	...
	if (eRFPath == RF_PATH_A)
		RfPiEnable = (u1Byte)phy_query_bb_reg(Adapter,
						      rFPGA0_XA_HSSIParameter1 | MaskforPhySet, BIT(8));
	else if (eRFPath == RF_PATH_B)
		RfPiEnable = (u1Byte)phy_query_bb_reg(Adapter,
						      rFPGA0_XB_HSSIParameter1 | MaskforPhySet, BIT(8));

	if (RfPiEnable) {
		/* Read from BBreg8b8, 12 bits for 8190, 20bits for T65 RF */
		retValue = phy_query_bb_reg(Adapter,
...
#endif
	_exit_critical_mutex(&(adapter_to_dvobj(Adapter)->rf_read_reg_mutex),
			     NULL);
 ...
 *
*/
static	VOID phy_RFSerialWrite_8710B(
	IN	PADAPTER			Adapter,
	IN	enum rf_path			eRFPath,
	IN	u32				Offset,
	IN	u32				Data
...
static	int phy_BB8710b_Config_ParaFile( // Peter, TODO
	IN	PADAPTER	Adapter
)
{
	HAL_DATA_TYPE		*pHalData = GET_HAL_DATA(Adapter);
	...
	/* */
#ifdef CONFIG_LOAD_PHY_PARA_FROM_FILE
	if (phy_ConfigBBWithParaFile(Adapter, PHY_FILE_PHY_REG,
				     CONFIG_BB_PHY_REG) == _FAIL)
#endif
	{
#ifdef CONFIG_EMBEDDED_FWIMG
		if (HAL_STATUS_SUCCESS != odm_config_bb_with_header_file(
			    &pHalData->odmpriv, CONFIG_BB_PHY_REG))
			rtStatus = _FAIL;
#endif
	}
	...
	if (Adapter->registrypriv.mp_mode == 1) {
		...
		phy_set_bb_reg(Adapter, 0xC3C, 0xFF,
			       0xCC);/*make sure low rate sensitivity*/
	}
	...
	/* */
#ifdef CONFIG_LOAD_PHY_PARA_FROM_FILE
	if (phy_ConfigBBWithParaFile(Adapter, PHY_FILE_AGC_TAB,
				     CONFIG_BB_AGC_TAB) == _FAIL)
#endif
	{
#ifdef CONFIG_EMBEDDED_FWIMG
		if (HAL_STATUS_SUCCESS != odm_config_bb_with_header_file(
			    &pHalData->odmpriv, CONFIG_BB_AGC_TAB))
			rtStatus = _FAIL;
#endif
	}
...
)
{
	...
	rtw_write32(Adapter, REG_SYS_FUNC_EN_8710B,
		    RegVal | BIT16 | BIT17 | BIT24 | BIT25 | BIT26);
 ...
 *---------------------------------------------------------------------------*/
int PHY_ConfigRFWithParaFile_8710B(
	IN	PADAPTER			Adapter,
	IN	u8					*pFileName,
	enum rf_path				eRFPath
) {
...
VOID PHY_SetTxPowerIndex_8710B(
	IN	PADAPTER			Adapter,
	IN	u32					PowerIndex,
...
)
{
	...
	base_idx = PHY_GetTxPowerIndexBase(pAdapter, RFPath, Rate, RF_1TX,
					   BandWidth, Channel, &bIn24G);

	by_rate_diff = PHY_GetTxPowerByRate(pAdapter, BAND_ON_2_4G, RF_PATH_A,
					    Rate);
	limit = PHY_GetTxPowerLimit(pAdapter, NULL, (u8)(!bIn24G),
				    pHalData->current_channel_bw, RFPath, Rate, RF_1TX,
...
VOID phy_SpurCalibration_8710B(
	IN	PADAPTER					pAdapter,
	IN	u1Byte						ToChannel,
	IN	u1Byte						threshold
)
{
	...
	if (idx < 0xFF) {
		initial_gain = (u1Byte)(odm_get_bb_reg(pDM_Odm, rOFDM0_XAAGCCore1,
						       bMaskByte0) & 0x7f);
		odm_pause_dig(pDM_Odm, PHYDM_PAUSE, PHYDM_PAUSE_LEVEL_1, 0x30);
		phy_set_bb_reg(pAdapter, rFPGA0_AnalogParameter4, bMaskDWord,
			       0xccf000c0);		/* disable 3-wire */

		phy_set_bb_reg(pAdapter, rFPGA0_PSDFunction, bMaskDWord,
			       freq[idx]);				/* Setup PSD */
		phy_set_bb_reg(pAdapter, rFPGA0_PSDFunction, bMaskDWord,
			       0x400000 | freq[idx]); /* Start PSD	 */
		...
		phy_set_bb_reg(pAdapter, rFPGA0_PSDFunction, bMaskDWord,
			       freq[idx]); /* turn off PSD */
		phy_set_bb_reg(pAdapter, rFPGA0_AnalogParameter4, bMaskDWord,
			       0xccc000c0);	/* enable 3-wire */
		odm_pause_dig(pDM_Odm, PHYDM_RESUME, PHYDM_PAUSE_LEVEL_1, NONE);
	}

	/* --- Notch Filter --- Asked by Rock	 */
	if (b_doNotch) {
		CurrentChannel = odm_get_rf_reg(pDM_Odm, RF_PATH_A, RF_CHNLBW,
						bRFRegOffsetMask);
		wlan_channel   = CurrentChannel & 0x0f;						    /* Get center frequency */

		switch (wlan_channel) {											/* Set notch filter				 */
		case 13:
			odm_set_bb_reg(pDM_Odm, 0xC40,
				       BIT(28) | BIT(27) | BIT(26) | BIT(25) | BIT(24), 0xB);
			odm_set_bb_reg(pDM_Odm, 0xC40, BIT(9),
				       0x1);                    /* enable notch filter */
			...
			odm_set_bb_reg(pDM_Odm, 0xD2C, BIT(28),
				       0x1);                   /* enable CSI mask */
			break;
		case 14:
			odm_set_bb_reg(pDM_Odm, 0xC40,
				       BIT(28) | BIT(27) | BIT(26) | BIT(25) | BIT(24), 0x5);
			odm_set_bb_reg(pDM_Odm, 0xC40, BIT(9),
				       0x1);                   /* enable notch filter */
		...
		default:
			odm_set_bb_reg(pDM_Odm, 0xC40, BIT(9),
				       0x0);						/* disable notch filter */
			odm_set_bb_reg(pDM_Odm, 0xD2C, BIT(28),
				       0x0);                   /* disable CSI mask	function */
			break;
		} /* switch(wlan_channel)	 */
		return;
	}
	...
	odm_set_bb_reg(pDM_Odm, 0xC40,
		       BIT(28) | BIT(27) | BIT(26) | BIT(25) | BIT(24), 0x1f);
	odm_set_bb_reg(pDM_Odm, 0xC40, BIT(9),
		       0x0);                     /* disable notch filter */
	odm_set_bb_reg(pDM_Odm, 0xD40, bMaskDWord, 0x00000000);
	odm_set_bb_reg(pDM_Odm, 0xD44, bMaskDWord, 0x00000000);
	...
	odm_set_bb_reg(pDM_Odm, 0xD4C, bMaskDWord, 0x00000000);
	odm_set_bb_reg(pDM_Odm, 0xD2C, BIT(28),
		       0x0);                    /* disable CSI mask */
}

VOID phy_SetRegBW_8710B(
	IN	PADAPTER		Adapter,
	...
	case CHANNEL_WIDTH_20:
		rtw_write16(Adapter, REG_TRXPTCL_CTL_8710B,
...
)
{
	...
	case CHANNEL_WIDTH_20:
		...
		phy_set_bb_reg(padapter, REG_FPGA0_RFMOD, BIT10 | BIT9 | BIT8,
			       0x7);	//0x800[10:8] = 3'b111
		phy_set_bb_reg(padapter, REG_FPGA0_RFMOD, BIT14 | BIT13 | BIT12,
			       0x5); //0x800[14:12] = 3'b101
		phy_set_bb_reg(padapter, rOFDM0_TxPseudoNoiseWgt, BIT31 | BIT30,
			       0x0); //0xCE4[31:30] = 2'00
		phy_set_bb_reg(padapter, rOFDM0_TxPseudoNoiseWgt, BIT29 | BIT28,
			       0x1); //0xCE4[29:28] = 2'01
		phy_set_bb_reg(padapter, rOFDM0_XARxAFE, BIT29 | BIT28,
			       0x1); //0xC10[29:28] = 2'01
		phy_set_bb_reg(padapter, REG_FPGA0_XB_RF_INTERFACE_OE, BIT30 | BIT29,
			       0x1); //0x864[30:29] = 2'01
		phy_set_bb_reg(padapter, rBBrx_DFIR, BIT19, 1); //0x954[19] = 1
		phy_set_bb_reg(padapter, rBBrx_DFIR, (BIT23 | BIT22 | BIT21 | BIT20),
			       0x8); //0x954[23:20] = 4'1000
		phy_set_bb_reg(padapter, rBBrx_DFIR, (BIT27 | BIT26 | BIT25 | BIT24),
			       0xa); //0x954[27:24] = 4'1010
...
)
{
	...
	if (channelToSW == 14) {
		phy_set_bb_reg(pAdapter, pHalData->RegForRecover[0].offset, bMaskDWord,
			       0x0000B81C);//0xA24
		phy_set_bb_reg(pAdapter, pHalData->RegForRecover[1].offset, bMaskDWord,
			       0x00000000); //0xA28
		phy_set_bb_reg(pAdapter, pHalData->RegForRecover[2].offset, bMaskDWord,
			       0x00003667); //0xAAC
		phy_set_bb_reg(pAdapter, pHalData->RegForRecover[3].offset, bMaskDWord,
			       pHalData->RegForRecover[3].value); //0xA20
...
VOID phy_SwChnlAndSetBwMode8710B(
	IN  PADAPTER		Adapter
)
{
	HAL_DATA_TYPE		*pHalData = GET_HAL_DATA(Adapter);
...
VOID PHY_HandleSwChnlAndSetBW8710B(
	IN	PADAPTER			Adapter,
...
)
{
	...
	PHY_HandleSwChnlAndSetBW8710B(Adapter, _TRUE, _TRUE, channel, Bandwidth,
				      Offset40, Offset80, channel);

	/* RTW_INFO("<==%s()\n",__FUNCTION__); */
}

static VOID _PHY_DumpRFReg_8710B(IN	PADAPTER	pAdapter)
{
	u32 rfRegValue, rfRegOffset;


	for (rfRegOffset = 0x00; rfRegOffset <= 0x30; rfRegOffset++) {
		rfRegValue = PHY_QueryRFReg_8710B(pAdapter, RF_PATH_A, rfRegOffset,
						  bMaskDWord);
	}
}
