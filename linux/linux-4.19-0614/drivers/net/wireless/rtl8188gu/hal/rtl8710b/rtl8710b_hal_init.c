...
static VOID _FWDownloadEnable(PADAPTER padapter, BOOLEAN enable)
{
	...
	if (enable) {
		...
		rtw_write8(padapter, (REG_8051FW_CTRL_V1_8710B + 2) | PAGE0_OFFSET,
			   tmp & 0xf7);
	} else {
		// MCU firmware download enable.
...
static int _BlockWrite(PADAPTER padapter, PVOID buffer, u32 buffSize)
{
...
#if 0
#ifdef CONFIG_USB_HCI
		ret = rtw_writeN(padapter, (FW_8710B_START_ADDRESS + i * blockSize_p1),
				 blockSize_p1, (bufferPtr + i * blockSize_p1));
#else
		ret = rtw_write32(padapter, (FW_8710B_START_ADDRESS + i * blockSize_p1),
	...
	if (remainSize_p1) {
		...
		for (i = 0; i < blockCount_p2; i++) {
			ret = rtw_writeN(padapter,
					 (FW_8710B_START_ADDRESS + offset + i * blockSize_p2), blockSize_p2,
	...
	if (remainSize_p2) {
		...
		for (i = 0 ; i < blockCount_p3 ; i++) {
			ret = rtw_write8(padapter, (FW_8710B_START_ADDRESS + offset + i),
					 *(bufferPtr + offset + i));
...
static int _PageWrite(PADAPTER	padapter, u32 page, PVOID buffer, u32 size)
{
	...
	value8 = (rtw_read8(padapter,
			    REG_8051FW_CTRL_V1_8710B + 2) & 0xF8) | u8Page;
...
void _8051Reset8710(PADAPTER padapter)
{
	...
	rtw_write8(padapter, (REG_8051FW_CTRL_V1_8710B + 3) | PAGE0_OFFSET,
		   value8 & (~BIT0));
...
static s32 polling_fwdl_chksum(_adapter *adapter, u32 min_cnt,
			       u32 timeout_ms)
{
	s32 ret = _FAIL;
	u32 value32;
...
static s32 _FWFreeToGo(_adapter *adapter, u32 min_cnt, u32 timeout_ms)
{
	...
	u32 value_expected = (MCUFWDL_RDY | FWDL_ChkSum_rpt | WINTINI_RDY |
			      RAM_DL_SEL);
...
s32 rtl8710b_FirmwareDownload(PADAPTER padapter, BOOLEAN  bUsedWoWLANFw)
{
	...
	if (rtw_read8(padapter,
		      REG_8051FW_CTRL_V1_8710B | PAGE0_OFFSET) & RAM_DL_SEL) {
		...
		*/
		...
		while (0 != pkt_size_tmp) {
			if (pkt_size_tmp >= FW_DOWNLOAD_SIZE_8710B) {
				...
				new_chk_sum = (u16)(new_chk_sum ^ (((send_pkt_size - 1) << 2) -
								   TXDESC_SIZE));
			} else {
			...
			if (_FALSE == send_fw_packet(padapter, pFirmwareBuf + mem_offset,
						     send_pkt_size)) {
...
DLFW_FAIL:
	if (rtStatus == _FAIL) {
		...
		rtw_write32(padapter, REG_8051FW_CTRL_V1_8710B,
			    0x01000105);  // Can show FW page, suggested by Ben.
		RTW_INFO("DUMP REG 0x90=0x%x \n", rtw_read32(padapter,
				REG_8051FW_CTRL_V1_8710B));//dbg log
...
static void hal_efuse_power_switch(PADAPTER padapter, u8 bWrite,
				   u8 PwrState)
{
...
static void hal_read_efuse_8710b(PADAPTER padapter, u8 efuseType,
				 u16 _offset, u16 _size_byte, u8 *pbuf, BOOLEAN bPseudoTest)
{
#ifdef HAL_EFUSE_MEMORY
	PHAL_DATA_TYPE	pHalData = GET_HAL_DATA(padapter);
	PEFUSE_HAL pEfuseHal = &pHalData->EfuseHal;
	...
	EFUSE_GetEfuseDefinition(padapter, EFUSE_WIFI,
				 TYPE_AVAILABLE_EFUSE_BYTES_TOTAL, &total, bPseudoTest);
	used = eFuse_Addr - 1;
	if (total)
...
error:
	...
	EFUSE_GetEfuseDefinition(padapter, EFUSE_WIFI,
				 TYPE_AVAILABLE_EFUSE_BYTES_TOTAL, &efuse_addr, bPseudoTest);
...
static u8 hal_EfuseCheckIfDatafollowed(
	PADAPTER		padapter,
	u8				word_cnts,
	u16				startAddr,
	u8				bPseudoTest)
	...
	u8				bPseudoTest)
{
	...
	EFUSE_GetEfuseDefinition(padapter, efuseType,
				 TYPE_AVAILABLE_EFUSE_BYTES_TOTAL, &efuse_max_available_len, bPseudoTest);
			...
			if ((curPkt.offset == pTargetPkt->offset) &&
				...
				if (badworden != 0x0F) {
					...
					if (efuseType == EFUSE_WIFI)
						...
						PgWriteSuccess = Efuse_PgPacketWrite_BT(padapter, pTargetPkt->offset,
											badworden, pTargetPkt->data, bPseudoTest);
	...
	u8				bPseudoTest)
{
	...
	/*	RTW_INFO("%s\n", __FUNCTION__); */
	EFUSE_GetEfuseDefinition(padapter, efuseType,
				 TYPE_AVAILABLE_EFUSE_BYTES_BANK, &efuse_max_available_len, bPseudoTest);
...
BOOLEAN hal_efuse_write4(PADAPTER padapter, u16 offset, u8 value)
{
	...
	RT_TRACE(COMP_EFUSE, DBG_LOUD,
		 ("Hal_EfuseOneByteWrite_8710B() offset = 0x%x, value= 0x%x\n", offset,
		  value));
	...
	PlatformEFIOWrite4Byte(padapter, REG_EFUSE_INDIRECT_CTRL_8710B,
			       0x86000000);
	...
	if (u1PollingCnt == 0) {
		RT_TRACE(COMP_EFUSE, DBG_SERIOUS,
			 ("Hal_EfuseOneByteWrite_8710B() Error: Polling 0x6c[31] fail, 0x6c = 0x%x\n",
			  u4Temp));
		bRetVaule = FALSE;
	}
...
u32 indirect_read32_8710b(PADAPTER padapter, u32 regaddr)
{
	...
	rtw_write32(padapter, REG_USB_HOST_INDIRECT_ADDR_8710B, regaddr);
	rtw_write32(padapter, REG_EFUSE_INDIRECT_CTRL_8710B,
		    NORMAL_REG_READ_OFFSET);
...
u32 hal_query_syson_reg_8710b(PADAPTER padapter, u32 regaddr, u32 bitmask)
{
	...
	value32 = indirect_read32_8710b(padapter,
					regaddr | SYSON_REG_BASE_ADDR_8710B);
...
VOID hal_set_syson_reg_8710b(PADAPTER padapter, u32 regaddr, u32 bitmask,
			     u32 data)
{
	u32 value32, bitshift = 0;
	...
	indirect_write32_8710b(padapter, regaddr | SYSON_REG_BASE_ADDR_8710B,
			       data);
	RTW_DBG("<---HAL_SetSYSOnReg_8710B(): RegAddr(%#x), BitMask(%#x), Data(%#x)\n",
		regaddr, bitmask, data);
...
static void read_chip_version_8710b(PADAPTER padapter)
{
	...
	value32 = hal_query_syson_reg_8710b(padapter, REG_SYS_SYSTEM_CFG0,
					    0xFFFFFFFF);
	pHalData->version_id.ICType = CHIP_8710B;
	pHalData->version_id.ChipType = ((value32 & BIT_RTL_ID_8710B) ? TEST_CHIP :
					 NORMAL_CHIP);
	...
	value32 = hal_query_syson_reg_8710b(padapter, REG_SYS_SYSTEM_CFG1,
					    0xFFFFFFFF);
	pHalData->RegulatorMode = ((value32 & BIT_SPSLDO_SEL_8710B) ?
				   RT_LDO_REGULATOR : RT_SWITCHING_REGULATOR);

	value32 = hal_query_syson_reg_8710b(padapter, REG_SYS_SYSTEM_CFG2,
					    0xFFFFFFFF);
...
void rtl8710b_InitBeaconParameters(PADAPTER padapter)
{
	...
	if (check_fwstate(&padapter->mlmepriv, WIFI_STATION_STATE) == _FALSE)
		rtw_write8(padapter, REG_DRVERLYINT,
			   DRIVER_EARLY_INT_TIME_8710B); /* 5ms */
	rtw_write8(padapter, REG_BCNDMATIM,
		   BCN_DMA_ATIME_INT_TIME_8710B); /* 2ms */
...
u8 rtl8710b_MRateIdxToARFRId(PADAPTER padapter, u8 rate_idx)
{
	...
	case RATR_INX_WIRELESS_AC_N:
		if (rftype ==
		    RF_1T1R) /* || padapter->MgntInfo.VHTHighestOperaRate <= MGN_VHT1SS_MCS9) */
			ret = 10;
		else
			ret = 9;
		break;
	...
	u8			bDataFrame)
{
	...
	SET_TX_DESC_PKT_SIZE_8710B(pDesc,
				   BufferLen); /* Buffer size + command header */
	SET_TX_DESC_QUEUE_SEL_8710B(pDesc,
	...
	SET_TX_DESC_USE_RATE_8710B(pDesc,
...
void rtl8710b_init_default_value(PADAPTER padapter)
{
	...
	_rtw_memset(pHalData->EfuseHal.fakeEfuseModifiedMap, 0xFF,
		    EFUSE_MAX_MAP_LEN);
#endif
}
...
u8 GetEEPROMSize8710B(PADAPTER padapter)
{
	...
	value32 = hal_query_syson_reg_8710b(padapter, REG_SYS_EEPROM_CTRL0,
					    0xFFFFFFFF);
	...
	if (_FALSE == pHalData->bautoload_fail_flag) {
		...
		*		if (IS_BOOT_FROM_EEPROM(padapter)) */
		if (_TRUE == pHalData->EepromOrEfuse) {
			...
			_rtw_memcpy((void *)PROMContent, (void *)pHalData->efuse_eeprom_data,
				    HWSET_MAX_SIZE_8710B);
		}
	} else {
		/* autoload fail */
...
void Hal_EfuseParseTxPowerInfo_8710B(
...
)
{
	...
	if (!AutoLoadFail) {
		pHalData->EEPROMRegulatory = (PROMContent[EEPROM_RF_BOARD_OPTION_8710B] &
					      0x7);	/* bit0~2 */
		if (PROMContent[EEPROM_RF_BOARD_OPTION_8710B] == 0xFF)
			pHalData->EEPROMRegulatory = (EEPROM_DEFAULT_BOARD_OPTION &
						      0x7);	/* bit0~2 */
	} else
		pHalData->EEPROMRegulatory = 0;
}

VOID Hal_EfuseParseBoardType_8710B(
	IN	PADAPTER	Adapter,
	IN	u8			*PROMContent,
	IN	BOOLEAN		AutoloadFail
)
{
	...
	if (!AutoloadFail) {
		pHalData->InterfaceSel = (PROMContent[EEPROM_RF_BOARD_OPTION_8710B] & 0xE0)
					 >> 5;
		if (PROMContent[EEPROM_RF_BOARD_OPTION_8710B] == 0xFF)
...
VOID Hal_EfuseParsePackageType_8710B(
	IN	PADAPTER		padapter,
	IN	u8				*hwinfo,
	IN	BOOLEAN		AutoLoadFail
)
...
VOID Hal_EfuseParseChnlPlan_8710B(
	IN	PADAPTER		padapter,
	IN	u8			*hwinfo,
	IN	BOOLEAN			AutoLoadFail
)
...
VOID Hal_EfuseParseCustomerID_8710B(
	IN	PADAPTER		padapter,
	IN	u8			*hwinfo,
	IN	BOOLEAN			AutoLoadFail
)
...
VOID Hal_EfuseParseAntennaDiversity_8710B(
	IN	PADAPTER		padapter,
	IN	u8				*hwinfo,
	IN	BOOLEAN			AutoLoadFail
)
{
	...
	pHalData->TRxAntDivType =
		S0S1_TRX_HW_ANTDIV; /* it's the only diversity-type for 8710B*/
	pHalData->with_extenal_ant_switch = ((hwinfo[EEPROM_RF_BT_SETTING_8710B] &
					      BIT7) >> 7);
	...
	if (!AutoLoadFail) {
		...
		if (pHalData->crystal_cap == 0xFF)
			pHalData->crystal_cap =
				EEPROM_Default_CrystalCap_8710B;	   /* what value should 8812 set? */
	} else
		pHalData->crystal_cap = EEPROM_Default_CrystalCap_8710B;
}


void Hal_EfuseParseThermalMeter_8710B(
	PADAPTER	padapter,
	u8			*PROMContent,
	u8			AutoLoadFail
)
...
static void fill_txdesc_vcs_8710b(PADAPTER padapter,
				  struct pkt_attrib *pattrib, u8 *ptxdesc)
{
	/* RTW_INFO("cvs_mode=%d\n", pattrib->vcs_mode); */
...
static void fill_txdesc_phy_8710b(PADAPTER padapter,
				  struct pkt_attrib *pattrib, u8 *ptxdesc)
{
	...
	u8 *pbuf)
{
	...
	if (pxmitframe->frame_tag == DATA_FRAMETAG) {
		...
		if ((pattrib->ether_type != 0x888e) &&
			...
			if (pHalData->fw_ractrl == _FALSE) {
				...
				SET_TX_DESC_TX_RATE_8710B(pbuf,
							  pHalData->INIDATA_RATE[pattrib->mac_id] & 0x7F);
			}
			if (bmcst)
			...
			RTW_INFO(FUNC_ADPT_FMT ": SP Packet(0x%04X) rate=0x%x\n",
				 FUNC_ADPT_ARG(padapter), pattrib->ether_type,
		...
		if (pxmitframe->ack_report) {
			...
			SET_TX_DESC_SW_DEFINE_8710B(pbuf,
	...
	if (!bmcst && pattrib->psta)
		odm_set_tx_ant_by_tx_info(adapter_to_phydm(padapter), pbuf,
					  pattrib->psta->cmn.mac_id);
#endif
}
...
static void hw_var_set_monitor(PADAPTER Adapter, u8 variable, u8 *val)
{
	...
	if (*((u8 *)val) == _HW_STATE_MONITOR_) {
		...
		/* Receive all type */
		rcr_bits = RCR_AAP | RCR_APM | RCR_AM | RCR_AB | RCR_APWRMGT | RCR_ADF |
			   RCR_ACF | RCR_AMF | RCR_APP_PHYST_RXFF;
	...
	if (padapter->hw_port == HW_PORT1) {
		...
		if ((mode == _HW_STATE_STATION_) || (mode == _HW_STATE_NOLINK_)) {
			if (!rtw_mi_get_ap_num(padapter) && !rtw_mi_get_mesh_num(padapter)) {
				...
				UpdateInterruptMask8710BU(padapter, _TRUE, 0,
							  (IMR_TXBCN0ERR_8710B | IMR_TXBCN0OK_8710B));
			...
			rtw_write8(padapter, REG_BCN_CTRL_1,
				   DIS_TSF_UDT | DIS_ATIM | EN_BCN_FUNCTION);
...
#ifdef CONFIG_INTERRUPT_BASED_TXBCN_BCN_OK_ERR
			UpdateInterruptMask8710BU(padapter, _TRUE,
						  (IMR_TXBCN0ERR_8710B | IMR_TXBCN0OK_8710B), 0);
#endif /* CONFIG_INTERRUPT_BASED_TXBCN_BCN_OK_ERR */
...
static void hw_var_set_mlme_disconnect(PADAPTER padapter, u8 variable,
				       u8 *val)
{
	u8 val8;
...
#ifdef CONFIG_XMIT_ACK
	if (GET_8710B_C2H_TX_RPT_RETRY_OVER(pdata) |
	    GET_8710B_C2H_TX_RPT_LIFE_TIME_OVER(pdata))
...
s32 c2h_handler_8710b(_adapter *adapter, u8 id, u8 seq, u8 plen,
		      u8 *payload)
{
	s32 ret = _SUCCESS;
...
#define RXDMA_IDLE			BIT(17)
	...
	case HW_VAR_NAV_UPPER: {
		...
		usNavUpper = (usNavUpper + HAL_NAV_UPPER_UNIT_8710B - 1) /
			     HAL_NAV_UPPER_UNIT_8710B;
...
void dump_qinfo_8710b(void *sel, struct qinfo_8710b *info, const char *tag)
{
	/* if (info->pkt_num) */
	RTW_PRINT_SEL(sel,
		      "%shead:0x%02x, tail:0x%02x, pkt_num:%u, macid:%u, ac:%u\n"
		      , tag ? tag : "", info->head, info->tail,
		      info->pkt_num, info->macid, info->ac);
}

void dump_bcn_qinfo_8710b(void *sel, struct bcn_qinfo_8710b *info,
			  const char *tag)
{
	/* if (info->pkt_num) */
...
void rtl8710b_read_wmmedca_reg(PADAPTER adapter, u16 *vo_params,
 ...
 */
u8 SetHalDefVar8710B(PADAPTER padapter, HAL_DEF_VARIABLE variable,
		     void *pval)
{
	PHAL_DATA_TYPE pHalData;
	u8 bResult;
...
void hal_ra_info_dump(_adapter *padapter, void *sel)
{
	...
	for (i = 0; i < macid_ctl->num; i++) {
		...
		if (rtw_macid_is_used(macid_ctl, i) && !rtw_macid_is_bmc(macid_ctl, i)) {
			...
			_RTW_PRINT_SEL(sel, "[ ra_info1:0x%08x ] => VHT_EN=0x%02x, ", ra_info1,
				       (ra_info1 >> 24) & 0xFF);
			...
			_RTW_PRINT_SEL(sel,
				       "[ ra_info2:0x%08x ] =>hight_rate=%s, lowest_rate=%s, SGI=0x%02x, RateID=%d\n",
				       ra_info2,
...
void rtl8710b_start_thread(_adapter *padapter)
{
	...
	xmitpriv->SdioXmitThread = kthread_run(rtl8710bs_xmit_thread, padapter,
					       "RTWHALXT");
...
void rtl8710b_set_hal_ops(struct hal_ops *pHalFunc)
{
	...
	pHalFunc->SetBeaconRelatedRegistersHandler =
		&rtl8710b_SetBeaconRelatedRegisters;
...
//	pHalFunc->read_wmmedca_reg = &rtl8710b_read_wmmedca_reg;
	...
	pHalFunc->sreset_linked_status_check  =
		&rtl8710b_sreset_linked_status_check;
