...
struct wifi_display_info {
	...
	enum	SCAN_RESULT_TYPE
	scan_result_type;	/*	Used when P2P is enable. This parameter will impact the scan result. */
	u8 op_wfd_mode;
	u8 stack_wfd_mode;
};
#endif /* CONFIG_WFD */
...
struct wifidirect_info {
	...
	_timer
	reset_ch_sitesurvey2;	/*	Just for resetting the scan limit function by using p2p nego */
	...
	struct profile_info
		profileinfo[P2P_MAX_PERSISTENT_GROUP_NUM];	/*	Store the profile information of persistent group */
	struct tx_invite_resp_info	inviteresp_info;
	struct tx_nego_req_info	nego_req_info;
	struct group_id_info
		groupid_info;	/*	Store the group id information when doing the group negotiation handshake. */
	struct scan_limit_info
		rx_invitereq_info;	/*	Used for get the limit scan channel from the Invitation procedure */
	struct scan_limit_info
		p2p_info;		/*	Used for get the limit scan channel from the P2P negotiation handshake */
#ifdef CONFIG_WFD
	...
	enum	P2P_WPSINFO
	ui_got_wps_info;			/*	This field will store the WPS value (PIN value or PBC) that UI had got from the user. */
...
extern int rtw_init_mlme_priv(_adapter
			      *adapter);/* (struct mlme_priv *pmlmepriv); */

extern void rtw_free_mlme_priv(struct mlme_priv *pmlmepriv);


extern sint rtw_select_and_join_from_scanned_queue(struct mlme_priv
		*pmlmepriv);
extern sint rtw_set_key(_adapter *adapter,
			struct security_priv *psecuritypriv, sint keyid, u8 set_tx, bool enqueue);
extern sint rtw_set_auth(_adapter *adapter,
 ...
 */
extern void rtw_mi_update_iface_status(struct mlme_priv *pmlmepriv,
				       sint state);
...
__inline static void set_scanned_network_val(struct mlme_priv *pmlmepriv,
		sint val)
{
	_irqL irqL;
...
extern void rtw_update_scanned_network(_adapter *adapter,
				       WLAN_BSSID_EX *target);
extern void rtw_disconnect_hdl_under_linked(_adapter *adapter,
		struct sta_info *psta, u8 free_assoc);
extern void rtw_generate_random_ibss(u8 *pibss);
extern struct wlan_network *rtw_find_network(_queue *scanned_queue,
		u8 *addr);
...
extern void rtw_indicate_disconnect(_adapter *adapter, u16 reason,
...
extern int rtw_restruct_sec_ie(_adapter *adapter, u8 *in_ie, u8 *out_ie,
			       uint in_len);
#ifdef CONFIG_WMMPS_STA
void rtw_uapsd_use_default_setting(_adapter *padapter);
bool rtw_is_wmmps_mode(_adapter *padapter);
#endif /* CONFIG_WMMPS_STA */
extern int rtw_restruct_wmm_ie(_adapter *adapter, u8 *in_ie, u8 *out_ie,
...
extern struct wlan_network *_rtw_alloc_network(struct mlme_priv
		*pmlmepriv);
...
extern struct wlan_network *_rtw_find_network(_queue *scanned_queue,
		u8 *addr);
...
void rtw_append_exented_cap(_adapter *padapter, u8 *out_ie,
			    uint *pout_len);
#endif
...
u8 rtw_sta_media_status_rpt_cmd(_adapter *adapter, struct sta_info *sta,
				bool connected);
void rtw_sta_media_status_rpt_cmd_hdl(_adapter *adapter,
...
void rtw_proxim_send_packet(_adapter *padapter, u8 *pbuf, u16 len,
			    u8 m_rate);
