...
extern union recv_frame *_rtw_alloc_recvframe(_queue
			*pfree_recv_queue);   /* get a free recv_frame from pfree_recv_queue */
extern union recv_frame *rtw_alloc_recvframe(_queue
			*pfree_recv_queue);   /* get a free recv_frame from pfree_recv_queue */
...
extern void rtw_free_recvframe_queue(_queue *pframequeue,
				     _queue *pfree_recv_queue);
...
__inline static u8 *recvframe_pull_tail(union recv_frame *precvframe,
					sint sz)
{
...
extern void  mgt_dispatcher(_adapter *padapter,
			    union recv_frame *precv_frame);
