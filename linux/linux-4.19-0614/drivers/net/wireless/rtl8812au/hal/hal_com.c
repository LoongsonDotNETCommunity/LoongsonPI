...
void dump_chip_info(HAL_VERSION	ChipVersion)
{
	...
	if (IS_81XXC(ChipVersion)) {
		cnt += sprintf((buf + cnt), "Chip Version Info: %s_",
			       IS_92C_SERIAL(ChipVersion) ? "CHIP_8192C" : "CHIP_8188C");
...
u8 hal_com_get_channel_plan(					//return the final channel plan decision
	...
	*(buf + 1) = rtw_read8(adapter, REG_C2HEVT_MSG_NORMAL + 1);
	...
	for (i = 0; i < c2h_evt->plen; i++)
		c2h_evt->payload[i] = rtw_read8(adapter,
						REG_C2HEVT_MSG_NORMAL + sizeof(*c2h_evt) + i);
...
u8  rtw_hal_networktype_to_raid(_adapter *adapter,
