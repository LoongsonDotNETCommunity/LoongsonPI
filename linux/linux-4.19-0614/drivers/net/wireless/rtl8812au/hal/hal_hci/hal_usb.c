...
int	usb_init_recv_priv(_adapter *padapter, u16 ini_in_buf_sz)
{
	...
	TASK_INIT(&precvpriv->rx_indicate_tasklet, 0, rtw_rx_indicate_tasklet,
		  padapter);
	...
	mtx_init(&precvpriv->rx_indicate_queue.ifq_mtx, "rx_indicate_queue", NULL,
		 MTX_DEF);
#endif /* CONFIG_RX_INDICATE_QUEUE */
...
static inline void usb_value_remask(struct intf_hdl *pintfhdl, u16 *value)
{
	...
	if ((IS_HARDWARE_TYPE_8192DU(padapter))
		...
		if (*value < 0x1000)
			*value |= 0x4000;
		else if ((*value & MAC1_ACCESS_PHY0)
			 && !(*value & 0x8000)) // MAC1 need to access PHY0
			*value &= 0xFFF;
	}
#endif
}
...
u32 usb_read32(struct intf_hdl *pintfhdl, u32 addr)
{
	...
	len = 4;
#ifdef CONFIG_RTL8192D
	if ((IS_HARDWARE_TYPE_8192DU(pintfhdl->padapter))
	    && ((addr & 0xff000000) >> 24 == 0x66)) {
