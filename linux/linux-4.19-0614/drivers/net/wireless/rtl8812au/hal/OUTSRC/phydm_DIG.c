...
)
{
	...
	if (pDM_Odm->bLinked && (bPerformance || bDFSBand)) {
		if (pDM_Odm->SupportICType == ODM_RTL8192D) {
			...
			// For AP
			if ((RxTp >> 2) > TxTp && RxTp < 10000
			    && RxTp > 500) {		// 10Mbps & 0.5Mbps
				dm_FA_thres[0] = 0x080;
				dm_FA_thres[1] = 0x100;
				dm_FA_thres[2] = 0x200;
			} else {
...
)
{
	...
	if (pFalseAlmCnt->Cnt_all > 10000) {
		...
		if (pDM_DigTable->LargeFAHit >= 3) {
			...
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_DIG, ODM_DBG_LOUD,
				     ("odm_DIG(): Abnormally false alarm case: Recover_cnt = %d \n",
		...
		if (pDM_DigTable->Recover_cnt != 0) {
			pDM_DigTable->Recover_cnt --;
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_DIG, ODM_DBG_LOUD,
				     ("odm_DIG(): Normal Case: Recover_cnt = %d \n",
				      pDM_DigTable->Recover_cnt));
		} else {
			if (pDM_DigTable->LargeFAHit < 3) {
...
)
{
	...
	while (1) {
		...
		if ((sNoise_A < 20 && sNoise_A >= -70) && (sNoise_B < 20
				&& sNoise_B >= -70)) {
			ValidCnt++;
			NoiseRpt_A += sNoise_A;
			NoiseRpt_B += sNoise_B;
...
)
{
	...
	ODM_RT_TRACE(pDM_Odm, ODM_COMP_DIG, ODM_DBG_TRACE,
	...
	if (!pDM_DigTable->bPSDInProgress) {
		if (CurrentIGI > pDM_DigTable->rx_gain_range_max) {
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_DIG, ODM_DBG_TRACE,
				     ("CurrentIGI(0x%02x) is larger than upper bound !!\n", CurrentIGI));
		...
		if (pDM_Odm->SupportAbility & ODM_BB_ADAPTIVITY
			...
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_DIG, ODM_DBG_LOUD,
				     ("odm_write_DIG(): Adaptivity case: Force upper bound to 0x%x !!!!!!\n",
				      CurrentIGI));
		}
	}
	...
	if (pDM_DigTable->CurIGValue != CurrentIGI) {
		...
		if (pDM_Odm->SupportPlatform & (ODM_WIN | ODM_CE)) {
			...
			if ((pDM_Odm->SupportICType & ODM_IC_11AC_SERIES)
				...
				ODM_SetBBReg(pDM_Odm, ODM_REG(IGI_C, pDM_Odm), ODM_BIT(IGI, pDM_Odm),
					     CurrentIGI);
				ODM_SetBBReg(pDM_Odm, ODM_REG(IGI_D, pDM_Odm), ODM_BIT(IGI, pDM_Odm),
					     CurrentIGI);
			}
		} else if (pDM_Odm->SupportPlatform & (ODM_AP | ODM_ADSL)) {
...
)
{
	...
	if (*pDM_DigTable->pbP2pLinkInProgress) {
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_DIG, ODM_DBG_LOUD,
			     ("odm_PauseDIG(): P2P in progress !!\n"));
		return;
	}
#endif

	if (!pDM_DigTable->bPauseDIG && (!(pDM_Odm->SupportAbility & ODM_BB_DIG)
	...
	case ODM_PAUSE_DIG:
		//2 Disable DIG
		ODM_CmnInfoUpdate(pDM_Odm, ODM_CMNINFO_ABILITY,
				  pDM_Odm->SupportAbility & (~ODM_BB_DIG));
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_DIG, ODM_DBG_LOUD,
			     ("odm_PauseDIG(): Pause DIG !!\n"));
		...
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_DIG, ODM_DBG_LOUD,
			     ("odm_PauseDIG(): Backup IGI  = 0x%x\n", pDM_DigTable->IGIBackup));

		//2 Write new IGI value
		ODM_Write_DIG(pDM_Odm, IGIValue);
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_DIG, ODM_DBG_LOUD,
			     ("odm_PauseDIG(): Write new IGI = 0x%x\n", IGIValue));
		break;
	...
	case ODM_RESUME_DIG:
		if (pDM_DigTable->bPauseDIG) {
			...
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_DIG, ODM_DBG_LOUD,
				     ("odm_PauseDIG(): Write original IGI = 0x%x\n", pDM_DigTable->IGIBackup));

			//2 Enable DIG
			ODM_CmnInfoUpdate(pDM_Odm, ODM_CMNINFO_ABILITY,
					  pDM_Odm->SupportAbility | ODM_BB_DIG);
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_DIG, ODM_DBG_LOUD,
				     ("odm_PauseDIG(): Resume DIG !!\n"));
		}
		break;

	default:
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_DIG, ODM_DBG_LOUD,
			     ("odm_PauseDIG(): Wrong  type !!\n"));
		break;
	}
}
...
)
{
	...
	if (!(pDM_Odm->SupportAbility & ODM_BB_FA_CNT)) {
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_DIG, ODM_DBG_LOUD,
			     ("odm_DIG(): Return: SupportAbility ODM_BB_FA_CNT is disabled\n"));
		return	TRUE;
	}
	...
	if (!(pDM_Odm->SupportAbility & ODM_BB_DIG)) {
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_DIG, ODM_DBG_LOUD,
			     ("odm_DIG(): Return: SupportAbility ODM_BB_DIG is disabled\n"));
		return	TRUE;
	}
...
)
{
	...
	BOOLEAN						bPerformance = TRUE, bFirstTpTarget = FALSE,
	...
	if (pDM_Odm->bLinked) {
		...
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_DIG, ODM_DBG_LOUD,
			     ("odm_DIG(): TX TP = %dkbps, RX TP = %dkbps\n", TxTp, RxTp));
	}
	...
	case 2: {
		if (pDM_Odm->bLinked) {
			...
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_DIG, ODM_DBG_LOUD,
				     ("odm_DIG(): TP training mode lower bound = %dkbps\n", TpTrainTH_MIN));
...
#endif
	{
		...
		if (priv->pmib->dot11RFEntry.tx2path && !bDFSBand
		    && (*(pDM_Odm->pWirelessMode) == ODM_WM_B))
			dm_dig_max = 0x2A;
	...
	ODM_RT_TRACE(pDM_Odm, ODM_COMP_DIG, ODM_DBG_LOUD,
		     ("odm_DIG(): Absolutly upper bound = 0x%x, lower bound = 0x%x\n",
	...
	if (0 < *pDM_Odm->pu1ForcedIgiLb) {
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_DIG, ODM_DBG_LOUD,
			     ("odm_DIG(): P2P case: Force IGI lb to: %u !!!!!!\n",
	...
	if (pDM_Odm->bLinked && bPerformance) {
		...
		if ((pDM_Odm->SupportICType & (ODM_RTL8192E | ODM_RTL8723B | ODM_RTL8812 |
					       ODM_RTL8821 | ODM_RTL8723A)) && (pDM_Odm->bBtLimitedDig == 1)) {
			offset = 10;
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_DIG, ODM_DBG_LOUD,
				     ("odm_DIG(): Coex. case: Force upper bound to RSSI + %d !!!!!!\n",
				      offset));
		} else
			offset = 15;
#endif
...
#else
		{
			...
			if (bDFSBand) {
				DIG_Dynamic_MIN = dm_dig_min;
				ODM_RT_TRACE(pDM_Odm, ODM_COMP_DIG, ODM_DBG_LOUD,
					     ("odm_DIG(): DFS band: Force lower bound to 0x%x after link !!!!!!\n",
					      dm_dig_min));
			} else {
				if (pDM_Odm->RSSI_Min < dm_dig_min)
					DIG_Dynamic_MIN = dm_dig_min;
		...
		if (bPerformance && bDFSBand) {
			...
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_DIG, ODM_DBG_LOUD,
				     ("odm_DIG(): DFS band: Force upper bound to 0x%x before link !!!!!!\n",
...
#endif
	{
		if (!pDM_Odm->bLinked) {
			...
			pDM_DigTable->rx_gain_range_min = odm_ForbiddenIGICheck(pDM_Odm,
							  DIG_Dynamic_MIN, CurrentIGI);
	}
	...
	if (pDM_Odm->bLinked && bPerformance) {
		...
		if (bFirstTpTarget || (FirstConnect && bPerformance)) {
			...
			if (bDFSBand) {
				...
				ODM_RT_TRACE(pDM_Odm,	ODM_COMP_DIG, ODM_DBG_LOUD,
					     ("odm_DIG(): DFS band: One-shot to 0x28 upmost!!!!!!\n"));
			} else
#endif
			{
	...
	if (pDM_Odm->SupportICType == ODM_RTL8192D) {
		...
		if (*(pDM_Odm->pMacPhyMode) == ODM_DMSP) {
			ODM_Write_DIG_DMSP(pDM_Odm,
					   CurrentIGI);//ODM_Write_DIG_DMSP(pDM_Odm, pDM_DigTable->CurIGValue);
			if (*(pDM_Odm->pbMasterOfDMSP)) {
	...
	if (pDM_Odm->SupportICType & ODM_IC_11AC_SERIES) {
		...
		if (pDM_Odm->SupportICType == ODM_RTL8881A) {
			...
			if (FalseAlmCnt->Cnt_Ofdm_fail >= FalseAlmCnt->Cnt_Ofdm_fail_pre) {
				...
				FalseAlmCnt->Cnt_Ofdm_fail = FalseAlmCnt->Cnt_Ofdm_fail -
							     Cnt_Ofdm_fail_temp;
			} else
				FalseAlmCnt->Cnt_Ofdm_fail_pre = FalseAlmCnt->Cnt_Ofdm_fail;
		...
		if (CCKenable) { //if(*pDM_Odm->pBandType == ODM_BAND_2_4G)
			FalseAlmCnt->Cnt_all = FalseAlmCnt->Cnt_Ofdm_fail +
					       FalseAlmCnt->Cnt_Cck_fail;
			FalseAlmCnt->Cnt_CCA_all = FalseAlmCnt->Cnt_CCK_CCA +
...
VOID odm_PauseCCKPacketDetection(
	IN		PVOID					pDM_VOID,
...
)
{
	...
	if (!bPaused && (!(pDM_Odm->SupportAbility & ODM_BB_CCK_PD)
			 || !(pDM_Odm->SupportAbility & ODM_BB_FA_CNT))) {
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_CCK_PD, ODM_DBG_LOUD,
			     ("Return: SupportAbility ODM_BB_CCK_PD or ODM_BB_FA_CNT is disabled\n"));
		return;
	}
	...
	case ODM_RESUME_CCKPD:
		if (bPaused) {
			...
			bPaused = FALSE;
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_CCK_PD, ODM_DBG_LOUD,
				     ("Write original CCK packet detection tgreshold = %d\n",
				      pDM_DigTable->CCKPDBackup));
...
)
{
...
#endif
	{
		...
		RXOK_cal = pDM_Odm->PhyDbgInfo.NumQryPhyStatusCCK +
		...
		if (RXOK_cal >= 70 && RxPWDBAve_final <= 40) {
			...
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_DIG, ODM_DBG_LOUD,
				     ("DIG = 0x%x, Cnt_all = %d, Cnt_Ofdm_fail = %d, Cnt_Cck_fail = %d\n",
				      CurrentIGI, pFalseAlmCnt->Cnt_all, pFalseAlmCnt->Cnt_Ofdm_fail,
				      pFalseAlmCnt->Cnt_Cck_fail));
		}
	} else {
		if (pDM_Odm->MPDIG_2G == FALSE) {
			if ((pDM_Odm->SupportPlatform & ODM_WIN)
			    && !(pDM_Odm->SupportICType & (ODM_RTL8814A | ODM_RTL8822B))) {
