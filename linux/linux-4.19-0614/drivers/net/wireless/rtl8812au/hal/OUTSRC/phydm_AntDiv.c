...
)
{
	...
	if (pDM_FatTable->AntDiv_OnOff != swch) {
		...
		if (pDM_Odm->SupportICType & ODM_N_ANTDIV_SUPPORT) {
			...
			ODM_SetBBReg(pDM_Odm, 0xc50, BIT7,
				     swch);  //OFDM AntDiv function block enable
			ODM_SetBBReg(pDM_Odm, 0xa00, BIT15,
			...
			if (pDM_Odm->SupportICType == ODM_RTL8812) {
				...
				ODM_SetBBReg(pDM_Odm, 0x8D4, BIT24,
					     swch);  //OFDM AntDiv function block enable

				if ((pDM_Odm->CutVersion >= ODM_CUT_C)
...
)
{
	...
	if (pDM_FatTable->RxIdleAnt != Ant) {
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
			     ("[ Update Rx-Idle-Ant ] RxIdleAnt =%s\n",
		...
		if (pDM_Odm->SupportICType & ODM_N_ANTDIV_SUPPORT) {
			if (pDM_Odm->SupportICType == ODM_RTL8192E) {
				...
				ODM_SetBBReg(pDM_Odm, 0xB38, BIT8 | BIT7 | BIT6,
					     OptionalAnt); //Optional RX
				ODM_SetBBReg(pDM_Odm, 0x860, BIT14 | BIT13 | BIT12,
					     DefaultAnt); //Default TX
			}
				...
				ODM_SetBBReg(pDM_Odm, 0x864, BIT8 | BIT7 | BIT6,
					     OptionalAnt);	//Optional RX
				ODM_SetBBReg(pDM_Odm, 0x860, BIT14 | BIT13 | BIT12,
					     DefaultAnt);	    //Default TX
			}
			...
			//
			value16 &= ~(BIT11 | BIT10 | BIT9 | BIT8 | BIT7 | BIT6 | BIT5 | BIT4 |
				     BIT3);
		...
		if (pDM_Odm->SupportICType == ODM_RTL8188E) {
			...
			ODM_SetMACReg(pDM_Odm, 0x6D8, BIT10 | BIT9 | BIT8,
				      DefaultAnt);	//PathA Resp Tx
		}

	} else { // pDM_FatTable->RxIdleAnt == Ant
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
			     ("[ Stay in Ori-Ant ]  RxIdleAnt =%s\n",
			      (Ant == MAIN_ANT) ? "MAIN_ANT" : "AUX_ANT"));
		pDM_FatTable->RxIdleAnt = Ant;
	}
}
...
)
{
	...
	if (pDM_Odm->SupportICType == ODM_RTL8192E) {
		...
		ODM_SetBBReg(pDM_Odm, 0x9b0, 0x0FFFFFFF,
			     0x1081008);      //0x9b0[30:0] = 01081008
		ODM_SetBBReg(pDM_Odm, 0x9b4, 0x0FFFFFFF,
...
VOID odm_BDCcoexType_withBferClient(
	IN 		PVOID 		pDM_VOID,
	IN 		u1Byte 			swch
)
{
...
VOID odm_BF_AntDiv_ModeArbitration(
	IN 		PVOID 		pDM_VOID
)
{
	PDM_ODM_T		pDM_Odm = (PDM_ODM_T)pDM_VOID;
	...
	//2 Mode 1
	if ((pDM_BdcTable->num_Txbfee_Client != 0)
	...
	//2 Mode 2
	else if ((pDM_BdcTable->num_Txbfee_Client == 0)
		 && (pDM_BdcTable->num_Txbfer_Client != 0)) {
...
VOID odm_DivTrainState_setting(
	IN		PVOID 		pDM_VOID
)
{
	PDM_ODM_T		pDM_Odm = (PDM_ODM_T)pDM_VOID;
...
VOID odm_BDCcoex_BFeeRxDiv_Arbitration(
	IN		PVOID		pDM_VOID
)
{
	PDM_ODM_T		pDM_Odm = (PDM_ODM_T)pDM_VOID;
	...
	ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
		     ("***{ num_BFee,  num_BFer , num_Client}  = (( %d  ,  %d  ,  %d))  \n",
		      pDM_BdcTable->num_Txbfee_Client, pDM_BdcTable->num_Txbfer_Client,
	...
	//2 Mode 2 & 3
	if (pDM_BdcTable->BDC_Mode == BDC_MODE_2
	    || pDM_BdcTable->BDC_Mode == BDC_MODE_3) {
		...
		if (pDM_BdcTable->num_Txbfer_Client ==
		    pDM_BdcTable->num_Client) { //BFer STA Only?: yes
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
				     ("BFer STA only?  (( Yes ))\n"));
			pDM_BdcTable->BDC_Try_flag = 0;
			pDM_BdcTable->BDC_state = BDC_DIV_TRAIN_STATE;
		...
		//
		if (pDM_BdcTable->bAll_BFSta_Idle == FALSE
			...
			odm_BDCcoexType_withBferClient(pDM_Odm, DIVOFF_CSION);
			return;
		} else if (pDM_BdcTable->bAll_BFSta_Idle == TRUE
		...
		if (pDM_BdcTable->BDC_Try_flag == 0) {
			...
			//2 DECISION_STATE (Mode 2-2)
				...
				ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
					     ("BF_tars exist?  : (( Yes )),  {BF_pass, DIV_pass, StopBF_flag }  = { %d, %d, %d } \n",
				...
				if (StopBF_flag == TRUE) { //DIV_en
					...
					pDM_BdcTable->BDC_state = BDC_DIV_HOLD_STATE;
					ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
						     ("[ StopBF_flag= ((TRUE)),   BDC_DECISION_STATE ] >> [BDC_DIV_HOLD_STATE] \n"));
				} else { //BF_en
					pDM_BdcTable->BDC_Hold_counter = 10; //20
					odm_BDCcoexType_withBferClient(pDM_Odm, DIVOFF_CSION);
					pDM_BdcTable->BDC_state = BDC_BF_HOLD_STATE;
					ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
						     ("[StopBF_flag= ((FALSE)),   BDC_DECISION_STATE ] >> [BDC_BF_HOLD_STATE] \n"));
				}
				//}
			}
...
)
{
	...
	ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
		     ("***8188E AntDiv_Init =>  AntDivType=[CGCS_RX_HW_ANTDIV]\n"));

	//MAC Setting
	value32 = ODM_GetMACReg(pDM_Odm, ODM_REG_ANTSEL_PIN_11N, bMaskDWord);
	ODM_SetMACReg(pDM_Odm, ODM_REG_ANTSEL_PIN_11N, bMaskDWord,
		      value32 | (BIT23 | BIT25)); //Reg4C[25]=1, Reg4C[23]=1 for pin output
	//Pin Settings
	ODM_SetBBReg(pDM_Odm, ODM_REG_PIN_CTRL_11N, BIT9 | BIT8,
		     0); //Reg870[8]=1'b0, Reg870[9]=1'b0 		//antsel antselb by HW
	ODM_SetBBReg(pDM_Odm, ODM_REG_RX_ANT_CTRL_11N, BIT10,
		     0);	//Reg864[10]=1'b0 	//antsel2 by HW
	ODM_SetBBReg(pDM_Odm, ODM_REG_LNA_SWITCH_11N, BIT22,
		     1);	//Regb2c[22]=1'b0 	//disable CS/CG switch
	ODM_SetBBReg(pDM_Odm, ODM_REG_LNA_SWITCH_11N, BIT31,
		     1);	//Regb2c[31]=1'b1	//output at CG only
	//OFDM Settings
	ODM_SetBBReg(pDM_Odm, ODM_REG_ANTDIV_PARA1_11N, bMaskDWord, 0x000000a0);
	//CCK Settings
	ODM_SetBBReg(pDM_Odm, ODM_REG_BB_PWR_SAV4_11N, BIT7,
	...
	ODM_SetBBReg(pDM_Odm, ODM_REG_ANT_MAPPING1_11N, 0xFFFF,
		     0x0001);	//antenna mapping table
}

VOID odm_TRX_HWAntDiv_Init_88E(
	IN		PVOID		pDM_VOID
)
{
	PDM_ODM_T		pDM_Odm = (PDM_ODM_T)pDM_VOID;
	...
	if (pDM_Odm->mp_mode == TRUE) {
		pDM_Odm->AntDivType = CGCS_RX_SW_ANTDIV;
		ODM_SetBBReg(pDM_Odm, ODM_REG_IGI_A_11N, BIT7, 0);  // disable HW AntDiv
		ODM_SetBBReg(pDM_Odm, ODM_REG_RX_ANT_CTRL_11N, BIT5 | BIT4 | BIT3,
			     0); //Default RX   (0/1)
		return;
	}

	ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
		     ("***8188E AntDiv_Init =>  AntDivType=[CG_TRX_HW_ANTDIV (SPDT)]\n"));
	...
	if (!pDM_Odm->bIsMPChip) { //testchip
		ODM_SetBBReg(pDM_Odm, ODM_REG_RX_DEFUALT_A_11N, BIT10 | BIT9 | BIT8,
			     1);	//Reg858[10:8]=3'b001
		ODM_SetBBReg(pDM_Odm, ODM_REG_RX_DEFUALT_A_11N, BIT13 | BIT12 | BIT11,
			     2);	//Reg858[13:11]=3'b010
	} else //MPchip
		ODM_SetBBReg(pDM_Odm, ODM_REG_ANT_MAPPING1_11N, bMaskDWord,
			     0x0001);	//Reg914=3'b010, Reg915=3'b001
}


#if( defined(CONFIG_5G_CG_SMART_ANT_DIVERSITY) ) ||( defined(CONFIG_2G_CG_SMART_ANT_DIVERSITY) )
VOID odm_Smart_HWAntDiv_Init_88E(
	IN		PVOID		pDM_VOID
)
{
	PDM_ODM_T		pDM_Odm = (PDM_ODM_T)pDM_VOID;
	...
	ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
		     ("***8188E AntDiv_Init =>  AntDivType=[CG_TRX_SMART_ANTDIV]\n"));

	if (pDM_Odm->mp_mode == TRUE) {
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_INIT, ODM_DBG_LOUD,
			     ("pDM_Odm->AntDivType: %d\n", pDM_Odm->AntDivType));
		return;
	}
	...
	ODM_SetMACReg(pDM_Odm, 0x4c, bMaskDWord,
		      value32 | (BIT23 | BIT25)); //Reg4C[25]=1, Reg4C[23]=1 for pin output
	value32 = ODM_GetMACReg(pDM_Odm,  0x7B4, bMaskDWord);
	ODM_SetMACReg(pDM_Odm, 0x7b4, bMaskDWord,
		      value32 | (BIT16 | BIT17)); //Reg7B4[16]=1 enable antenna training, Reg7B4[17]=1 enable A2 match
	...
	ODM_SetBBReg(pDM_Odm, 0x870, BIT9 | BIT8,
		     0); //Reg870[8]=1'b0, Reg870[9]=1'b0 		//antsel antselb by HW
	ODM_SetBBReg(pDM_Odm, 0x864, BIT10, 0);	//Reg864[10]=1'b0 	//antsel2 by HW
	ODM_SetBBReg(pDM_Odm, 0xb2c, BIT22,
		     0);	//Regb2c[22]=1'b0 	//disable CS/CG switch
	ODM_SetBBReg(pDM_Odm, 0xb2c, BIT31,
	...
	if (pDM_Odm->fat_comb_a == 2) {
		if (!pDM_Odm->bIsMPChip) { //testchip
			...
			ODM_SetBBReg(pDM_Odm, 0x858, BIT13 | BIT12 | BIT11,
				     2);	//Reg858[13:11]=3'b010
		} else { //MPchip
		...
		if (!pDM_Odm->bIsMPChip) { //testchip
			...
			ODM_SetBBReg(pDM_Odm, 0x858, BIT15 | BIT14,
				     2);	//(Reg878[0],Reg858[14:15])=3'b010
			ODM_SetBBReg(pDM_Odm, 0x878, BIT19 | BIT18 | BIT17,
				     3); //Reg878[3:1]=3b'011
			ODM_SetBBReg(pDM_Odm, 0x878, BIT22 | BIT21 | BIT20,
				     4); //Reg878[6:4]=3b'100
			ODM_SetBBReg(pDM_Odm, 0x878, BIT25 | BIT24 | BIT23,
				     5); //Reg878[9:7]=3b'101
			ODM_SetBBReg(pDM_Odm, 0x878, BIT28 | BIT27 | BIT26,
				     6); //Reg878[12:10]=3b'110
			ODM_SetBBReg(pDM_Odm, 0x878, BIT31 | BIT30 | BIT29,
	...
	//Enter Traing state
	ODM_SetBBReg(pDM_Odm, 0x864, BIT2 | BIT1 | BIT0,
		     (pDM_Odm->fat_comb_a -
...
)
{
	...
	if (pDM_Odm->mp_mode == TRUE) {
		//pDM_Odm->AntDivType = CGCS_RX_SW_ANTDIV;
		odm_AntDiv_on_off(pDM_Odm, ANTDIV_OFF);
		ODM_SetBBReg(pDM_Odm, 0xc50, BIT8,
	...
	ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
		     ("***8192E AntDiv_Init =>  AntDivType=[CGCS_RX_HW_ANTDIV]\n"));

	//Pin Settings
	ODM_SetBBReg(pDM_Odm, 0x870, BIT8,
...
VOID odm_TRX_HWAntDiv_Init_92E(
	IN		PVOID		pDM_VOID
)
{
	PDM_ODM_T		pDM_Odm = (PDM_ODM_T)pDM_VOID;
	...
	ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
		     ("***8192E AntDiv_Init =>  AntDivType=[ Only for DIR605, CG_TRX_HW_ANTDIV]\n"));

	//3 --RFE pin setting---------
	//[MAC]
	ODM_SetMACReg(pDM_Odm, 0x38, BIT11,
	...
	ODM_SetBBReg(pDM_Odm, 0x940, BIT7 | BIT6,
		     0x0); // r_rfe_path_sel_   (RFE_CTRL_3)
	ODM_SetBBReg(pDM_Odm, 0x940, BIT17 | BIT16,
	...
	*/
	...
	//Timming issue
	ODM_SetBBReg(pDM_Odm, 0xE20, BIT23 | BIT22 | BIT21 | BIT20,
		     8); //keep antidx after tx for ACK ( unit x 32 mu sec)
...
#if( defined(CONFIG_5G_CG_SMART_ANT_DIVERSITY) ) ||( defined(CONFIG_2G_CG_SMART_ANT_DIVERSITY) )
VOID odm_Smart_HWAntDiv_Init_92E(
	IN		PVOID		pDM_VOID
)
{
	PDM_ODM_T		pDM_Odm = (PDM_ODM_T)pDM_VOID;
...
#if (RTL8723B_SUPPORT == 1)
VOID odm_TRX_HWAntDiv_Init_8723B(
	IN		PVOID		pDM_VOID
)
{
	PDM_ODM_T		pDM_Odm = (PDM_ODM_T)pDM_VOID;
	ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
		     ("***8723B AntDiv_Init =>  AntDivType=[CG_TRX_HW_ANTDIV(DPDT)]\n"));
	...
	ODM_SetMACReg(pDM_Odm, 0x4C,  BIT24 | BIT23,
...
VOID odm_S0S1_SWAntDiv_Init_8723B(
	IN		PVOID		pDM_VOID
)
{
	PDM_ODM_T		pDM_Odm = (PDM_ODM_T)pDM_VOID;
	...
	ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
		     ("***8723B AntDiv_Init => AntDivType=[ S0S1_SW_AntDiv] \n"));
...
VOID odm_S0S1_SWAntDiv_Reset_8723B(
	IN		PVOID		pDM_VOID
)
{
	PDM_ODM_T		pDM_Odm = (PDM_ODM_T)pDM_VOID;
	...
	ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
		     ("odm_S0S1_SWAntDiv_Reset_8723B(): pDM_FatTable->bBecomeLinked = %d\n",
		      pDM_FatTable->bBecomeLinked));
}

VOID ODM_UpdateRxIdleAnt_8723B(
	IN		PVOID			pDM_VOID,
	IN		u1Byte			Ant,
...
)
{
	...
	if (u1Temp == 0x1) {
		...
		if (u1Temp & BIT0) {
			...
			pAdapter->HalFunc.SetTxPowerLevelHandler(pAdapter,
			...
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
				     ("[ Update Rx-Idle-Ant ] 8723B: Sucess to set RX antenna\n"));
		} else
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
...
VOID odm_TRX_HWAntDiv_Init_8821A(
	IN		PVOID		pDM_VOID
)
{
	PDM_ODM_T		pDM_Odm = (PDM_ODM_T)pDM_VOID;
	...
	ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
		     ("***8821A AntDiv_Init => AntDivType=[ CG_TRX_HW_ANTDIV (DPDT)] \n"));
	...
	ODM_SetMACReg(pDM_Odm, 0x4C, BIT23,
...
VOID odm_S0S1_SWAntDiv_Init_8821A(
	IN		PVOID		pDM_VOID
)
{
	PDM_ODM_T		pDM_Odm = (PDM_ODM_T)pDM_VOID;
	...
	ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
		     ("***8821A AntDiv_Init => AntDivType=[ S0S1_SW_AntDiv] \n"));
...
VOID odm_RX_HWAntDiv_Init_8881A(
	IN		PVOID		pDM_VOID
)
{
	PDM_ODM_T		pDM_Odm = (PDM_ODM_T)pDM_VOID;
	ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
		     ("***8881A AntDiv_Init => AntDivType=[ CGCS_RX_HW_ANTDIV] \n"));

}

VOID odm_TRX_HWAntDiv_Init_8881A(
	IN		PVOID		pDM_VOID
)
{
	PDM_ODM_T		pDM_Odm = (PDM_ODM_T)pDM_VOID;
	...
	ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
		     ("***8881A AntDiv_Init => AntDivType=[ CG_TRX_HW_ANTDIV (SPDT)] \n"));
...
VOID odm_TRX_HWAntDiv_Init_8812A(
	IN		PVOID		pDM_VOID
)
{
	PDM_ODM_T		pDM_Odm = (PDM_ODM_T)pDM_VOID;
	ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
		     ("***8812A AntDiv_Init => AntDivType=[ CG_TRX_HW_ANTDIV (SPDT)] \n"));

	//3 //3 --RFE pin setting---------
	//[BB]
	ODM_SetBBReg(pDM_Odm, 0x900, BIT10 | BIT9 | BIT8,
...
)
{
	...
	if ((pDM_Odm->SupportICType & ODM_EVM_ENHANCE_ANTDIV_SUPPORT_IC)) {
		if (pDM_Odm->bOneEntryOnly) {
			...
			Main_RSSI = (pDM_FatTable->MainAnt_Cnt[i] != 0) ?
				    (pDM_FatTable->MainAnt_Sum[i] / pDM_FatTable->MainAnt_Cnt[i]) : 0;
			Aux_RSSI = (pDM_FatTable->AuxAnt_Cnt[i] != 0) ?
				   (pDM_FatTable->AuxAnt_Sum[i] / pDM_FatTable->AuxAnt_Cnt[i]) : 0;
			...
			if (((Main_RSSI >= Evm_RSSI_TH_High || Aux_RSSI >= Evm_RSSI_TH_High)
				...
				if (((Main_RSSI >= Evm_RSSI_TH_Low) || (Aux_RSSI >= Evm_RSSI_TH_Low))) {
					...
					if (pDM_FatTable->FAT_State == NORMAL_STATE_MIAN) {
						...
						ODM_SetTimer(pDM_Odm, &pDM_Odm->EVM_FastAntTrainingTimer,
							     pDM_Odm->antdiv_intvl);  //m
					}
					//2 [ Normal state Aux ]
						...
						if ((pDM_FatTable->MainCRC32_Ok_Cnt  >= ((pDM_FatTable->AuxCRC32_Ok_Cnt) <<
								1)) || (Diff_RSSI >= 18)) {
							pDM_FatTable->TargetAnt_CRC32 = MAIN_ANT;
							Force_antenna = TRUE;
							ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
								     ("CRC32 Force Main \n"));
						} else if ((pDM_FatTable->AuxCRC32_Ok_Cnt  >= ((
									pDM_FatTable->MainCRC32_Ok_Cnt) << 1)) || (Diff_RSSI >= 18)) {
							...
							if (Main_above1 == TRUE && Aux_above1 == FALSE) {
								...
								Main_CRC_utility = ((pDM_FatTable->MainCRC32_Ok_Cnt) << 7) /
										   pDM_FatTable->MainCRC32_Fail_Cnt;
								Aux_CRC_utility = ((pDM_FatTable->AuxCRC32_Ok_Cnt) << 7) /
										  pDM_FatTable->AuxCRC32_Fail_Cnt;
								pDM_FatTable->TargetAnt_CRC32 = (Main_CRC_utility == Aux_CRC_utility) ?
												(pDM_FatTable->pre_TargetAnt_enhance) : ((Main_CRC_utility >=
								...
								Main_CRC_utility = ((pDM_FatTable->MainCRC32_Fail_Cnt) << 7) /
										   pDM_FatTable->MainCRC32_Ok_Cnt;
								Aux_CRC_utility = ((pDM_FatTable->AuxCRC32_Fail_Cnt) << 7) /
										  pDM_FatTable->AuxCRC32_Ok_Cnt;
								pDM_FatTable->TargetAnt_CRC32 = (Main_CRC_utility == Aux_CRC_utility) ?
												(pDM_FatTable->pre_TargetAnt_enhance) : ((Main_CRC_utility <=
														Aux_CRC_utility) ? MAIN_ANT : AUX_ANT);
						...
						//3 [EVM statistic]
						Main_EVM = (pDM_FatTable->MainAntEVM_Cnt[i] != 0) ?
							   (pDM_FatTable->MainAntEVM_Sum[i] / pDM_FatTable->MainAntEVM_Cnt[i]) : 0;
						Aux_EVM = (pDM_FatTable->AuxAntEVM_Cnt[i] != 0) ?
							  (pDM_FatTable->AuxAntEVM_Sum[i] / pDM_FatTable->AuxAntEVM_Cnt[i]) : 0;
						pDM_FatTable->TargetAnt_EVM = (Main_EVM == Aux_EVM) ?
									      (pDM_FatTable->pre_TargetAnt_enhance) : ((Main_EVM >= Aux_EVM) ? MAIN_ANT :
											      AUX_ANT);
						...
						} else if (diff_EVM <= 2 && (utility_ratio > 4
									     && Force_antenna == FALSE)) {
							pDM_FatTable->TargetAnt_enhance = pDM_FatTable->TargetAnt_CRC32;
						} else if (diff_EVM >= 20) { //
							pDM_FatTable->TargetAnt_enhance = pDM_FatTable->TargetAnt_EVM;
						} else if (utility_ratio >= 6
							   && Force_antenna == FALSE) { // utility_ratio>3
							pDM_FatTable->TargetAnt_enhance = pDM_FatTable->TargetAnt_CRC32;
						} else {
						...
						ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
							     ("*** Client[ %d ] : MainEVM_Cnt = (( %d ))  , Main_EVM= ((  %d )) \n", i,
							      pDM_FatTable->MainAntEVM_Cnt[i], Main_EVM));
						ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
							     ("*** Client[ %d ] : AuxEVM_Cnt   = (( %d ))  , Aux_EVM = ((  %d )) \n", i,
							      pDM_FatTable->AuxAntEVM_Cnt[i], Aux_EVM));
						ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
							     ("*** TargetAnt_EVM = (( %s ))\n",
							      (pDM_FatTable->TargetAnt_EVM  == MAIN_ANT) ? "MAIN_ANT" : "AUX_ANT"));
						ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
							     ("M_CRC_Ok = (( %d ))  , M_CRC_Fail = ((  %d )), Main_CRC_utility = (( %d )) \n",
							      pDM_FatTable->MainCRC32_Ok_Cnt, pDM_FatTable->MainCRC32_Fail_Cnt,
							      Main_CRC_utility));
						ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
							     ("A_CRC_Ok  = (( %d ))  , A_CRC_Fail = ((  %d )), Aux_CRC_utility   = ((  %d )) \n",
							      pDM_FatTable->AuxCRC32_Ok_Cnt, pDM_FatTable->AuxCRC32_Fail_Cnt,
							      Aux_CRC_utility));
						ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
							     ("*** TargetAnt_CRC32 = (( %s ))\n",
...
VOID odm_EVM_FastAntTrainingCallback(
	IN		PVOID		pDM_VOID
)
{
	PDM_ODM_T		pDM_Odm = (PDM_ODM_T)pDM_VOID;
...
)
{
	...
	if (!pDM_Odm->bLinked) { //bLinked==False
		...
		if (pDM_FatTable->bBecomeLinked == FALSE) {
			...
			if (pDM_Odm->SupportICType == ODM_RTL8723B
				...
				ODM_SetBBReg(pDM_Odm, 0x930, 0xF0,
	...
	for (i = 0; i < ODM_ASSOCIATE_ENTRY_NUM; i++) {
		...
		if (IS_STA_VALID(pEntry)) {
			...
			Aux_RSSI = (pDM_FatTable->AuxAnt_Cnt[i] != 0) ?
				   (pDM_FatTable->AuxAnt_Sum[i] / pDM_FatTable->AuxAnt_Cnt[i]) : 0;
			TargetAnt = (Main_RSSI == Aux_RSSI) ? pDM_FatTable->RxIdleAnt : ((
						Main_RSSI >= Aux_RSSI) ? MAIN_ANT : AUX_ANT);

			//ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV,ODM_DBG_LOUD,("*** SupportICType=[%d] \n",pDM_Odm->SupportICType));
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
				     ("*** Client[ %d ] : Main_Cnt = (( %d ))  , Main_RSSI= ((  %d )) \n", i,
				      pDM_FatTable->MainAnt_Cnt[i], Main_RSSI));
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
				     ("*** Client[ %d ] : Aux_Cnt   = (( %d ))  , Aux_RSSI = ((  %d )) \n", i,
				      pDM_FatTable->AuxAnt_Cnt[i], Aux_RSSI));
			...
			if (pDM_BdcTable->BDC_Mode == BDC_MODE_2
				...
				if ((MA_rx_Temp < TH2)   && (MA_rx_Temp > TH1)
				    && (LocalMaxRSSI <= Monitor_RSSI_threshold)) {
					if (pDM_BdcTable->w_BFer_Client[i] == 1) { // Bfer_Target
						pDM_BdcTable->num_BfTar++;

						if (pDM_BdcTable->BDC_state == BDC_DECISION_STATE
						    && pDM_BdcTable->BDC_Try_flag == 0) {
							improve_TP_temp = (pDM_BdcTable->MA_rx_TP_DIV[i] * 9) >> 3 ; //* 1.125
							pDM_BdcTable->BF_pass = (pDM_BdcTable->MA_rx_TP[i] > improve_TP_temp) ?
										TRUE : FALSE;
							ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
								     ("*** Client[ %d ] :  { MA_rx_TP,improve_TP_temp , MA_rx_TP_DIV,  BF_pass}={ %d,  %d, %d , %d }  \n",
								      i, pDM_BdcTable->MA_rx_TP[i], improve_TP_temp,
								      pDM_BdcTable->MA_rx_TP_DIV[i], pDM_BdcTable->BF_pass));
						}
					} else { // DIV_Target
						...
						if (pDM_BdcTable->BDC_state == BDC_DECISION_STATE
							...
							pDM_BdcTable->DIV_pass = (pDM_BdcTable->MA_rx_TP[i] > degrade_TP_temp) ?
										 TRUE : FALSE;
							ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
								     ("*** Client[ %d ] :  { MA_rx_TP, degrade_TP_temp , MA_rx_TP_DIV,  DIV_pass}=\n{ %d,  %d, %d , %d }  \n",
								      i, pDM_BdcTable->MA_rx_TP[i], degrade_TP_temp,
								      pDM_BdcTable->MA_rx_TP_DIV[i], pDM_BdcTable->DIV_pass));
						}
					}
				}
				...
				ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
					     ("*** Client[ %d ] :  { BFmeeCap , BFmerCap}  = { %d , %d } \n", i,
					      pDM_BdcTable->w_BFee_Client[i], pDM_BdcTable->w_BFer_Client[i]));

				if (pDM_BdcTable->BDC_state == BDC_BFer_TRAIN_STATE) {
					ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
						     ("*** Client[ %d ] :    MA_rx_TP_DIV = (( %d ))  \n", i,
						      pDM_BdcTable->MA_rx_TP_DIV[i]));

				} else {
					ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
						     ("*** Client[ %d ] :    MA_rx_TP = (( %d ))  \n", i,
						      pDM_BdcTable->MA_rx_TP[i]));
				}
...
#ifdef BEAMFORMING_SUPPORT
#if(DM_ODM_SUPPORT_TYPE == ODM_AP)
	if (pDM_BdcTable->BDC_Mode == BDC_MODE_1
	    || pDM_BdcTable->BDC_Mode == BDC_MODE_3) {
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
			     ("*** BDC_RxIdleUpdate_counter = (( %d ))\n",
...
)
{
	...
	u1Byte			HighTraffic_TrainTime_U = 0x32, HighTraffic_TrainTime_L = 0,
				Train_time_temp;
	...
	ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
		     ("[%d] { try_flag=(( %d )), Step=(( %d )), Double_chk_flag = (( %d )) }\n",
		      __LINE__, pDM_SWAT_Table->try_flag, Step,
		      pDM_SWAT_Table->Double_chk_flag));
	...
	if (pDM_SWAT_Table->try_flag == 0xff) {
		...
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
			     ("[set try_flag = 0]  Prepare for peak!\n\n"));
		return;
		...
		if (pDM_SWAT_Table->try_flag == 0) {
			...
			if (curTxOkCnt > 1875000
			    || curRxOkCnt > 1875000) { //if(PlatformDivision64(curTxOkCnt+curRxOkCnt, 2) > 1875000)  ( 1.875M * 8bit ) / 2= 7.5M bits /sec )
				...
				ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
					     ("  [HIGH Traffic]  \n"));
			} else if (curTxOkCnt > 125000
				   || curRxOkCnt > 125000) { // ( 0.125M * 8bit ) / 2 =  0.5M bits /sec )
				pDM_SWAT_Table->TrafficLoad = TRAFFIC_LOW;
				Train_time_temp = pDM_SWAT_Table->Train_time ;
			...
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
				     ("TxOkCnt=(( %llu )), RxOkCnt=(( %llu )) \n",
				      curTxOkCnt, curRxOkCnt));

			//-----------------

			ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
				     (" Current MinMaxRSSI is ((%d)) \n", pDM_FatTable->MinMaxRSSI));
		...
		else if (pDM_SWAT_Table->try_flag == 1
			 && pDM_SWAT_Table->Double_chk_flag == 0) {
			nextAnt = (pDM_FatTable->RxIdleAnt  == MAIN_ANT) ? AUX_ANT : MAIN_ANT;
		...
		if ((pDM_SWAT_Table->try_flag == 1)
			...
			for (i = 0; i < ODM_ASSOCIATE_ENTRY_NUM; i++) {
				...
				if (IS_STA_VALID(pEntry)) {
					...
					if (pDM_FatTable->MainAnt_Cnt[i] <= 1
					    && pDM_FatTable->CCK_counter_main >= 1)
						Main_RSSI = 0;

					if (pDM_FatTable->AuxAnt_Cnt[i] <= 1 && pDM_FatTable->CCK_counter_aux >= 1)
						Aux_RSSI = 0;

					TargetAnt = (Main_RSSI == Aux_RSSI) ? pDM_SWAT_Table->PreAntenna : ((
								Main_RSSI >= Aux_RSSI) ? MAIN_ANT : AUX_ANT);
					LocalMaxRSSI = (Main_RSSI >= Aux_RSSI) ? Main_RSSI : Aux_RSSI;
			...
			pkt_cnt_total = pDM_FatTable->CCK_counter_main +
			...
			if (MinMaxRSSI == 0xff
			    || ((pkt_cnt_total < (pDM_SWAT_Table->PktCnt_SWAntDivByCtrlFrame >> 1))
				&& pDM_Odm->PhyDbgInfo.NumQryBeaconPkt < 2)) {
				MinMaxRSSI = 0;
				ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
					     ("Check RSSI of control frame because MinMaxRSSI == 0xff\n"));
				ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
					     ("bByCtrlFrame = %d\n", bByCtrlFrame));

				if (bByCtrlFrame) {
					Main_RSSI = (pDM_FatTable->MainAnt_CtrlFrame_Cnt != 0) ?
						    (pDM_FatTable->MainAnt_CtrlFrame_Sum / pDM_FatTable->MainAnt_CtrlFrame_Cnt)
						    : 0;
					Aux_RSSI = (pDM_FatTable->AuxAnt_CtrlFrame_Cnt != 0) ?
						   (pDM_FatTable->AuxAnt_CtrlFrame_Sum / pDM_FatTable->AuxAnt_CtrlFrame_Cnt) :
						   0;

					if (pDM_FatTable->MainAnt_CtrlFrame_Cnt <= 1
					    && pDM_FatTable->CCK_CtrlFrame_Cnt_main >= 1)
						Main_RSSI = 0;

					if (pDM_FatTable->AuxAnt_CtrlFrame_Cnt <= 1
					    && pDM_FatTable->CCK_CtrlFrame_Cnt_aux >= 1)
						Aux_RSSI = 0;

					if (Main_RSSI != 0 || Aux_RSSI != 0) {
						RxIdleAnt = (Main_RSSI == Aux_RSSI) ? pDM_SWAT_Table->PreAntenna : ((
									Main_RSSI >= Aux_RSSI) ? MAIN_ANT : AUX_ANT);
						LocalMaxRSSI = (Main_RSSI >= Aux_RSSI) ? Main_RSSI : Aux_RSSI;
						LocalMinRSSI = (Main_RSSI >= Aux_RSSI) ? Aux_RSSI : Main_RSSI;
						...
						ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
							     ("RxIdleAnt decided by control frame = %s\n",
			...
			if (pDM_SWAT_Table->Double_chk_flag == 1) {
				...
				if (pDM_FatTable->MinMaxRSSI > SWAntDiv_threshold) {
					ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
						     (" [Double check] MinMaxRSSI ((%d)) > %d again!! \n",
						      pDM_FatTable->MinMaxRSSI, SWAntDiv_threshold));
	...
	ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
		     ("RSSI_Trying = (( %d )),    Ant: (( %s )) >>> (( %s )) \n",
		      pDM_SWAT_Table->RSSI_Trying,
		      (pDM_FatTable->RxIdleAnt  == MAIN_ANT ? "MAIN" : "AUX"),
...
VOID ODM_SW_AntDiv_WorkitemCallback(
	IN PVOID            pContext
)
{
...
VOID ODM_SW_AntDiv_Callback(void *FunctionContext)
{
...
VOID odm_S0S1_SwAntDivByCtrlFrame(
	IN		PVOID			pDM_VOID,
	IN		u1Byte			Step
)
{
	...
	case SWAW_STEP_PEAK:
		...
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
			     ("odm_S0S1_SwAntDivForAPMode(): Start peak and reset counter\n"));
		break;
	case SWAW_STEP_DETERMINE:
		pDM_SWAT_Table->bSWAntDivByCtrlFrame = FALSE;
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
			     ("odm_S0S1_SwAntDivForAPMode(): Stop peak\n"));
		break;
	default:
		pDM_SWAT_Table->bSWAntDivByCtrlFrame = FALSE;
		break;
...
VOID odm_AntselStatisticsOfCtrlFrame(
	IN		PVOID			pDM_VOID,
...
)
{
	...
	isCCKrate = ((pPktinfo->DataRate >= DESC_RATE1M)
		     && (pPktinfo->DataRate <= DESC_RATE11M)) ? TRUE : FALSE;

	if (isCCKrate) {
		pDM_FatTable->antsel_rx_keep_0 = (pDM_FatTable->RxIdleAnt == MAIN_ANT) ?
						 ANT1_2G : ANT2_2G;
		...
		odm_AntselStatisticsOfCtrlFrame(pDM_Odm, pDM_FatTable->antsel_rx_keep_0,
...
#if (DM_ODM_SUPPORT_TYPE & (ODM_AP|ODM_ADSL))
				value32 = (pEntry->hwaddr[3] << 24) | (pEntry->hwaddr[2] << 16) |
					  (pEntry->hwaddr[1] << 8) | pEntry->hwaddr[0];
#else
				value32 = (pEntry->MacAddr[3] << 24) | (pEntry->MacAddr[2] << 16) |
					  (pEntry->MacAddr[1] << 8) | pEntry->MacAddr[0];
#endif
				ODM_SetMACReg(pDM_Odm, 0x7b0, bMaskDWord, value32);//0x7b0~0x7b3

				ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
					     ("pDM_FatTable->TrainIdx=%d\n", pDM_FatTable->TrainIdx));

#if (DM_ODM_SUPPORT_TYPE & (ODM_AP|ODM_ADSL))
				ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
					     ("Training MAC Addr = %x:%x:%x:%x:%x:%x\n",
					      pEntry->hwaddr[5], pEntry->hwaddr[4], pEntry->hwaddr[3], pEntry->hwaddr[2],
					      pEntry->hwaddr[1], pEntry->hwaddr[0]));
#else
				ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
					     ("Training MAC Addr = %x:%x:%x:%x:%x:%x\n",
					      pEntry->MacAddr[5], pEntry->MacAddr[4], pEntry->MacAddr[3],
					      pEntry->MacAddr[2], pEntry->MacAddr[1], pEntry->MacAddr[0]));
#endif
...
)
{
	...
	if (pDM_Odm->SupportICType == ODM_RTL8188E) {
		ODM_SetBBReg(pDM_Odm, 0x864, BIT2 | BIT1 | BIT0,
			     ((pDM_Odm->fat_comb_a) - 1));
	}
#if(RTL8192E_SUPPORT == 1)
	else if (pDM_Odm->SupportICType == ODM_RTL8192E) {
		ODM_SetBBReg(pDM_Odm, 0xB38, BIT2 | BIT1 | BIT0,
			     ((pDM_Odm->fat_comb_a) -
			      1));	     //path-A  // ant combination=regB38[2:0]+1
		ODM_SetBBReg(pDM_Odm, 0xB38, BIT18 | BIT17 | BIT16,
			     ((pDM_Odm->fat_comb_b) -
	...
	if (pDM_FatTable->FAT_State == FAT_TRAINING_STATE) {
		...
		for (i = 0; i < (pDM_Odm->fat_comb_a); i++) { // i : antenna index
			if (pDM_FatTable->antRSSIcnt[i] == 0)
				...
				pDM_FatTable->antAveRSSI[i] = pDM_FatTable->antSumRSSI[i] /
							      pDM_FatTable->antRSSIcnt[i];
		...
		*/
		...
		if (bPktFilterMacth_pathA  == FALSE) {
			...
			if (pDM_Odm->SupportICType == ODM_RTL8188E) {
				...
				ODM_SetBBReg(pDM_Odm, 0xB38, BIT8 | BIT7 | BIT6,
					     TargetAnt_pathA); //Optional RX [pth-A]
			}
			//3 [ update TX ant ]
	...
	if (pDM_FatTable->FAT_State == FAT_NORMAL_STATE) {
		...
		ODM_SetTimer(pDM_Odm, &pDM_Odm->FastAntTrainingTimer,
			     pDM_Odm->antdiv_intvl);  //ms
	}
...
VOID odm_FastAntTrainingWorkItemCallback(
	IN		PVOID		pDM_VOID
)
{
	PDM_ODM_T		pDM_Odm = (PDM_ODM_T)pDM_VOID;
...
)
{
	...
	if (pDM_FatTable->AntDiv_2G_5G == ODM_ANTDIV_2G) {
		...
		if (!(pDM_Odm->SupportICType & ODM_ANTDIV_5G_SUPPORT_IC))
			return;
	} else 	if (pDM_FatTable->AntDiv_2G_5G == (ODM_ANTDIV_2G |
			ODM_ANTDIV_5G)) {
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
...
VOID ODM_Process_RSSIForAntDiv(
...
)
{
	...
	if (pDM_Odm->SupportICType & ODM_RTL8192C | ODM_RTL8192D) {
		if (pPktinfo->bPacketToSelf || pPktinfo->bPacketBeacon) {
			ODM_AntselStatistics_88C(pDM_Odm, pPktinfo->StationID,
						 pPhyInfo->RxPWDBAll, isCCKrate);
		}
	}
#endif
	...
	if (pDM_Odm->AntDivType == CG_TRX_SMART_ANTDIV) {
		if ((pDM_Odm->SupportICType & ODM_SMART_ANT_SUPPORT)
		    && (pPktinfo->bPacketToSelf)
		    && (pDM_FatTable->FAT_State ==
			FAT_TRAINING_STATE)) {  //(pPktinfo->bPacketMatchBSSID && (!pPktinfo->bPacketBeacon))
		...
		if ((pDM_Odm->SupportICType & ODM_ANTDIV_SUPPORT)
		    && (pPktinfo->bPacketToSelf || pPktinfo->bPacketMatchBSSID)) {
			if (pDM_Odm->SupportICType == ODM_RTL8188E
			    || pDM_Odm->SupportICType == ODM_RTL8192E) {
				odm_AntselStatistics(pDM_Odm, pDM_FatTable->antsel_rx_keep_0,
						     pPktinfo->StationID, RxPower_Ant0, RSSI_METHOD);

#ifdef ODM_EVM_ENHANCE_ANTDIV
				if (!isCCKrate) {
					odm_AntselStatistics(pDM_Odm, pDM_FatTable->antsel_rx_keep_0,
							     pPktinfo->StationID, RxEVM_Ant0, EVM_METHOD);
				}
#endif
...
)
{
	...
	if (state == INIT_ANTDIV_TIMMER) {
		...
		ODM_InitializeTimer(pDM_Odm,
				    &pDM_Odm->DM_SWAT_Table.SwAntennaSwitchTimer_8723B,
				    (RT_TIMER_CALL_BACK)ODM_SW_AntDiv_Callback, NULL,
				    "SwAntennaSwitchTimer_8723B");
		...
		ODM_InitializeTimer(pDM_Odm, &pDM_Odm->EVM_FastAntTrainingTimer,
				    (RT_TIMER_CALL_BACK)odm_EVM_FastAntTrainingCallback, NULL,
				    "EVM_FastAntTrainingTimer");
#endif
	} else if (state == CANCEL_ANTDIV_TIMMER) {
#if (RTL8723B_SUPPORT == 1)||(RTL8821A_SUPPORT == 1)
		ODM_CancelTimer(pDM_Odm,
				&pDM_Odm->DM_SWAT_Table.SwAntennaSwitchTimer_8723B);
#elif ( defined(CONFIG_5G_CG_SMART_ANT_DIVERSITY) ) ||( defined(CONFIG_2G_CG_SMART_ANT_DIVERSITY) )
