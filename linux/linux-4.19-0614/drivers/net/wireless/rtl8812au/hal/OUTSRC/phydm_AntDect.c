...
)
{
	...
	u1Byte		n;
	u4Byte		Reg88c, Regc08, Reg874, Regc50, Reg948, Regb2c, Reg92c, Reg930,
			Reg064, AFE_rRx_Wait_CCA;
	...
	if (pDM_Odm->SupportICType == ODM_RTL8723A) {
		...
		//2 Test Ant B based on Ant A is ON
		if ((AntA_report >= 100) && (AntB_report >= 100) && (AntA_report <= 135)
		    && (AntB_report <= 135)) {
			u1Byte TH1 = 2, TH2 = 6;

			if ((AntA_report - AntB_report < TH1)
			    || (AntB_report - AntA_report < TH1)) {
				pDM_SWAT_Table->ANTA_ON = TRUE;
				pDM_SWAT_Table->ANTB_ON = TRUE;
				ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
					     ("ODM_SingleDualAntennaDetection(): Dual Antenna\n"));
			} else if (((AntA_report - AntB_report >= TH1)
				    && (AntA_report - AntB_report <= TH2)) ||
				   ((AntB_report - AntA_report >= TH1)
				    && (AntB_report - AntA_report <= TH2))) {
				pDM_SWAT_Table->ANTA_ON = FALSE;
				pDM_SWAT_Table->ANTB_ON = FALSE;
				bResult = FALSE;
				ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
				...
				pDM_SWAT_Table->ANTB_ON = FALSE;
				ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
					     ("ODM_SingleDualAntennaDetection(): Single Antenna\n"));
			}
			pDM_Odm->AntDetectedInfo.bAntDetected = TRUE;
			pDM_Odm->AntDetectedInfo.dBForAntA = AntA_report;
			pDM_Odm->AntDetectedInfo.dBForAntB = AntB_report;
...
BOOLEAN ODM_SwAntDivCheckBeforeLink(
	IN		PVOID		pDM_VOID
)
{
	...
	u1Byte			tmp_power_diff = 0, power_diff = 0, avg_power_diff = 0,
				max_power_diff = 0, min_power_diff = 0xff;
	...
	//if(HP id)
	{
		if (pDM_Odm->DM_SWAT_Table.RSSI_AntDect_bResult == TRUE
		...
		if (pDM_Odm->SupportICType == ODM_RTL8723B) {
			if (pDM_SWAT_Table->SWAS_NoLink_BK_Reg948 == 0xff)
				pDM_SWAT_Table->SWAS_NoLink_BK_Reg948 = ODM_Read4Byte(pDM_Odm,
									rS0S1_PathSwitch);
		}
	}
	...
	ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
		     ("pDM_SWAT_Table->SWAS_NoLink_State = %d\n",
	...
	if (pDM_SWAT_Table->SWAS_NoLink_State == 0) {
		...
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
			     ("ODM_SwAntDivCheckBeforeLink: Scan one more time\n"));
		...
		if (pDM_Odm->SupportICType & (ODM_RTL8188E | ODM_RTL8821)) {
			...
			if (pDM_Odm->SupportICType == ODM_RTL8192C) {
				...
				if ((pDM_SWAT_Table->CurAntenna = MAIN_ANT)
				    && (tmp_SWAS_NoLink_BK_Reg948 == 0x200)) {
					ODM_SetBBReg(pDM_Odm, rS0S1_PathSwitch, 0xfff, 0x280);
					ODM_SetBBReg(pDM_Odm, rAGC_table_select, BIT31, 0x1);
			...
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
				     ("ODM_SwAntDivCheckBeforeLink: Change to (( %s-ant))  for testing.\n",
		...
		for (index = 0; index < pMgntInfo->tmpNumBssDesc; index++) {
			...
			if (PlatformCompareMemory(pTestBssDesc->bdBssIdBuf,
				...
				ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
					     ("ODM_SwAntDivCheckBeforeLink(): ERROR!! This shall not happen.\n"));
				continue;
			}
			...
			if (pDM_Odm->SupportICType != ODM_RTL8723B) {
				if (pTmpBssDesc->ChannelNumber == ScanChannel) {
					if (pTmpBssDesc->RecvSignalPower > pTestBssDesc->RecvSignalPower) {
						ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
							     ("ODM_SwAntDivCheckBeforeLink: Compare scan entry: Score++\n"));
						RT_PRINT_STR(COMP_SCAN, DBG_WARNING, "GetScanInfo(): new Bss SSID:",
							     pTmpBssDesc->bdSsIdBuf, pTmpBssDesc->bdSsIdLen);
						ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
						...
						if (pTestBssDesc->bdTstamp - pTmpBssDesc->bdTstamp < 5000) {
							...
							ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
								     ("at ch %d, Original: %d, Test: %d\n", pTmpBssDesc->ChannelNumber,
								      pTmpBssDesc->RecvSignalPower, pTestBssDesc->RecvSignalPower));
							ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
								     ("The 2nd Antenna didn't get this AP\n\n"));
						}
					}
				}
			} else { // 8723B
				if (pTmpBssDesc->ChannelNumber == ScanChannel) {
					ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
						     ("ChannelNumber == ScanChannel -> (( %d )) \n",
						      pTmpBssDesc->ChannelNumber));

					if (pTmpBssDesc->RecvSignalPower >
					    pTestBssDesc->RecvSignalPower) { // Pow(Ant1) > Pow(Ant2)
						counter++;
						tmp_power_diff = (u1Byte)(pTmpBssDesc->RecvSignalPower -
						...
						ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
							     ("Original: %d, Test: %d\n", pTmpBssDesc->RecvSignalPower,
							      pTestBssDesc->RecvSignalPower));
						ODM_PRINT_ADDR(pDM_Odm, ODM_COMP_ANT_DIV, DBG_LOUD, ("SSID:"),
							       pTmpBssDesc->bdSsIdBuf);
						ODM_PRINT_ADDR(pDM_Odm, ODM_COMP_ANT_DIV, DBG_LOUD, ("BSSID:"),
							       pTmpBssDesc->bdBssIdBuf);
						...
						PlatformMoveMemory(pTestBssDesc, pTmpBssDesc, sizeof(RT_WLAN_BSS));
					} else if (pTestBssDesc->RecvSignalPower >
						   pTmpBssDesc->RecvSignalPower) { // Pow(Ant1) < Pow(Ant2)
						counter++;
						tmp_power_diff = (u1Byte)(pTestBssDesc->RecvSignalPower -
									  pTmpBssDesc->RecvSignalPower);
						power_diff = power_diff + tmp_power_diff;
						ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
							     ("Original: %d, Test: %d\n", pTmpBssDesc->RecvSignalPower,
						...
						if (pTestBssDesc->bdTstamp >
						    pTmpBssDesc->bdTstamp) { //  Stamp(Ant1) < Stamp(Ant2)
		...
		if (pDM_Odm->SupportICType & (ODM_RTL8188E | ODM_RTL8821)) {
			...
			if (counter == 0) {
				if (pDM_Odm->DM_SWAT_Table.Pre_Aux_FailDetec == FALSE) {
					...
					ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
						     ("ODM_SwAntDivCheckBeforeLink(): Single antenna\n"));
				}
				...
				if (counter == 3) {
					avg_power_diff = ((power_diff - max_power_diff - min_power_diff) >> 1) + ((
								 max_power_diff + min_power_diff) >> 2);
					ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
						     ("counter: (( %d )) ,  power_diff: (( %d )) \n", counter, power_diff));
					ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
						     ("[ counter==3 ] Modified avg_power_diff: (( %d )) , max_power_diff: (( %d )) ,  min_power_diff: (( %d )) \n",
						      avg_power_diff, max_power_diff, min_power_diff));
				} else if (counter >= 4) {
					avg_power_diff = (power_diff - max_power_diff - min_power_diff) /
							 (counter - 2);
					ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
						     ("counter: (( %d )) ,  power_diff: (( %d )) \n", counter, power_diff));
					ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
						     ("[ counter>=4 ] Modified avg_power_diff: (( %d )) , max_power_diff: (( %d )) ,  min_power_diff: (( %d )) \n",
						      avg_power_diff, max_power_diff, min_power_diff));

				} else { //counter==1,2
					avg_power_diff = power_diff / counter;
					ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
						     ("avg_power_diff: (( %d )) , counter: (( %d )) ,  power_diff: (( %d )) \n",
						      avg_power_diff, counter, power_diff));
				}

				//2 [ Retry ]
				if ((avg_power_diff >= power_target_L)
				    && (avg_power_diff <= power_target_H)) {
					...
					if (pDM_Odm->DM_SWAT_Table.Retry_Counter <= 3) {
						...
						pDM_Odm->DM_SWAT_Table.RSSI_AntDect_bResult = TRUE;
						ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
							     ("[[ Still Low confidence result ]]  (( Retry_Counter > 3 )) \n"));
						ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
							     ("ODM_SwAntDivCheckBeforeLink(): Single antenna\n"));
					}
				...
				//2 [ Dual Antenna ]
				else if ((pMgntInfo->NumBssDesc != 0)
					 && (avg_power_diff < power_target_L)) {
					pDM_Odm->DM_SWAT_Table.RSSI_AntDect_bResult = TRUE;
					...
					ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
						     ("ODM_SwAntDivCheckBeforeLink(): Dual antenna\n"));
			...
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
				     ("Dual_Ant_Counter = (( %d )), Single_Ant_Counter = (( %d )) , Retry_Counter = (( %d )) , Aux_FailDetec_Counter = (( %d ))\n\n\n",
				      pDM_Odm->DM_SWAT_Table.Dual_Ant_Counter,
				      pDM_Odm->DM_SWAT_Table.Single_Ant_Counter,
				      pDM_Odm->DM_SWAT_Table.Retry_Counter,
				      pDM_Odm->DM_SWAT_Table.Aux_FailDetec_Counter));
			...
			if (pDM_Odm->DM_SWAT_Table.ANTB_ON == FALSE)
				ODM_SetBBReg(pDM_Odm, rS0S1_PathSwitch, 0xfff,
					     (pDM_SWAT_Table->SWAS_NoLink_BK_Reg948));

			ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD,
				     ("bResult=(( %d )), Recover  Reg[948]= (( %x )) \n\n",
				      pDM_Odm->DM_SWAT_Table.RSSI_AntDect_bResult,
				      pDM_SWAT_Table->SWAS_NoLink_BK_Reg948));
	...
	IN u1Byte 		initial_gain)
{
	...
	ODM_SetBBReg(pDM_Odm, 0x808, BIT22,
		     1);  //Start PSD calculation, Reg808[22]=0->1
	ODM_StallExecution(150);//Wait for HW PSD report
	ODM_SetBBReg(pDM_Odm, 0x808, BIT22,
		     0);//Stop PSD calculation,  Reg808[22]=1->0
	psd_report = ODM_GetBBReg(pDM_Odm, 0x8B4,
				  bMaskDWord) & 0x0000FFFF; //Read PSD report, Reg8B4[15:0]
...
VOID ODM_SingleDualAntennaDetection_PSD(
	IN	 PVOID 	pDM_VOID
)
{
	PDM_ODM_T		pDM_Odm = (PDM_ODM_T)pDM_VOID;
	...
	Channel_ori = ODM_GetRFReg(pDM_Odm, ODM_RF_PATH_A, ODM_CHANNEL,
				   bRFRegOffsetMask);
	Reg948 = ODM_GetBBReg(pDM_Odm, rS0S1_PathSwitch, bMaskDWord);
	Regb2c =  ODM_GetBBReg(pDM_Odm, rAGC_table_select, bMaskDWord);
	Regc50 = ODM_GetBBReg(pDM_Odm, rOFDM0_XAAGCCore1, bMaskDWord);
	...
	ODM_Write1Byte(pDM_Odm, REG_TXPAUSE,
		       0xFF); // Turn off TX  ->  Pause TX Queue
	ODM_SetBBReg(pDM_Odm, 0xC14, bMaskDWord,
	...
	ODM_SetRFReg(pDM_Odm, ODM_RF_PATH_A, ODM_CHANNEL, 0x7ff,
		     0x04);     // Set RF to CH4 & 40M
	ODM_SetBBReg(pDM_Odm, rFPGA0_AnalogParameter4, 0xf00000,
		     0xf);	// 3 wire Disable    88c[23:20]=0xf
	ODM_SetBBReg(pDM_Odm, rFPGA0_PSDFunction, BIT14 | BIT15,
		     0x0); //128 pt	//Set PSD 128 ptss
	...
	for (i = 0; i < test_num; i++) {
		for (tone_idx = 0; tone_idx < Tone_lenth_1; tone_idx++) {
			PSD_report_temp = odm_GetPSDData(pDM_Odm, Tone_idx_1[tone_idx],
							 initial_gain);
	...
	for (i = 0; i < test_num; i++) {
		for (tone_idx = 0; tone_idx < Tone_lenth_2; tone_idx++) {
			PSD_report_temp = odm_GetPSDData(pDM_Odm, Tone_idx_2[tone_idx],
							 initial_gain);
			//if(  PSD_report_temp>PSD_report_Main[tone_idx]  )
	...
	ODM_SetBBReg(pDM_Odm, 0xC14, bMaskDWord,
		     Regc14); // [ Set IQK Matrix = 0 ] equivalent to [ Turn on CCA]
