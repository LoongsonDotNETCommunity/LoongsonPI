...
//======================================================================
VOID ODM_ClearTxPowerTrackingState(
	IN PDM_ODM_T		pDM_Odm
)
{
...
)
{
	...
	pDM_Odm->RFCalibrateInfo.TxPowerTrackControl =
		pHalData->TxPowerTrackControl; // <Kordan> We should keep updating the control variable according to HalData.
#endif
	...
	ODM_RT_TRACE(pDM_Odm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD,
		 ...
		 \n pDM_Odm->BbSwingIdxCckBase: %d, pDM_Odm->BbSwingIdxOfdmBase[A]: %d, pDM_Odm->DefaultOfdmIndex: %d\n",
		      pDM_Odm->BbSwingIdxCckBase, pDM_Odm->BbSwingIdxOfdmBase[ODM_RF_PATH_A],
	...
	pDM_Odm->RFCalibrateInfo.ThermalValue_AVG[pDM_Odm->RFCalibrateInfo.ThermalValue_AVG_index]
		= ThermalValue;
	...
	if ((delta_LCK >=
	     c.Threshold_IQK)) { // Delta temperature is equal to or larger than 20 centigrade.
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD,
	...
	if (delta > 0 && pDM_Odm->RFCalibrateInfo.TxPowerTrackControl) {
...
#else
		delta = (ThermalValue > pDM_Odm->priv->pmib->dot11RFEntry.ther) ?
			(ThermalValue - pDM_Odm->priv->pmib->dot11RFEntry.ther) :
			(pDM_Odm->priv->pmib->dot11RFEntry.ther - ThermalValue);
#endif
		...
		if (ThermalValue > pDM_Odm->priv->pmib->dot11RFEntry.ther) {
			...
			pDM_Odm->Aboslute_OFDMSwingIdx[ODM_RF_PATH_A] =
				deltaSwingTableIdx_TUP_A[delta];        // Record delta swing for mix mode power tracking
			...
			if (c.RfPathCount > 1) {
				...
				pDM_Odm->Aboslute_OFDMSwingIdx[ODM_RF_PATH_B] =
					deltaSwingTableIdx_TUP_B[delta];       // Record delta swing for mix mode power tracking
			...
			pDM_Odm->RFCalibrateInfo.DeltaPowerIndexLast[ODM_RF_PATH_A] =
				pDM_Odm->RFCalibrateInfo.DeltaPowerIndex[ODM_RF_PATH_A];
			pDM_Odm->RFCalibrateInfo.DeltaPowerIndex[ODM_RF_PATH_A] = -1 *
					deltaSwingTableIdx_TDOWN_A[delta];

			pDM_Odm->Aboslute_OFDMSwingIdx[ODM_RF_PATH_A] =  -1 *
					deltaSwingTableIdx_TDOWN_A[delta];        // Record delta swing for mix mode power tracking
			...
			if (c.RfPathCount > 1) {
				...
				pDM_Odm->RFCalibrateInfo.DeltaPowerIndexLast[ODM_RF_PATH_B] =
					pDM_Odm->RFCalibrateInfo.DeltaPowerIndex[ODM_RF_PATH_B];
				pDM_Odm->RFCalibrateInfo.DeltaPowerIndex[ODM_RF_PATH_B] = -1 *
						deltaSwingTableIdx_TDOWN_B[delta];

				pDM_Odm->Aboslute_OFDMSwingIdx[ODM_RF_PATH_B] =  -1 *
						deltaSwingTableIdx_TDOWN_B[delta];       // Record delta swing for mix mode power tracking
		...
		for (p = ODM_RF_PATH_A; p < c.RfPathCount; p++) {
			...
			if (pDM_Odm->RFCalibrateInfo.DeltaPowerIndex[p] ==
			    pDM_Odm->RFCalibrateInfo.DeltaPowerIndexLast[p])         // If Thermal value changes but lookup table value still the same
				pDM_Odm->RFCalibrateInfo.PowerIndexOffset[p] = 0;
			else
			...
			pDM_Odm->RFCalibrateInfo.OFDM_index[p] = pDM_Odm->BbSwingIdxOfdmBase[p] +
					pDM_Odm->RFCalibrateInfo.PowerIndexOffset[p];
			...
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD,
				     ("The 'OFDM' final index(%d) = BaseIndex[%c](%d) + PowerIndexOffset(%d)\n",
				      pDM_Odm->BbSwingIdxOfdm[p], (p == ODM_RF_PATH_A ? 'A' : 'B'),
	...
	ODM_RT_TRACE(pDM_Odm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD,
		     ("TxPowerTracking: [CCK] Swing Current Index: %d, Swing Base Index: %d\n",
		      ...
		      pDM_Odm->BbSwingIdxCckBase));       //Print Swing base & current
	for (p = ODM_RF_PATH_A; p < c.RfPathCount; p++) {
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD,
			     ("TxPowerTracking: [OFDM] Swing Current Index: %d, Swing Base Index[%c]: %d\n",
			      pDM_Odm->RFCalibrateInfo.OFDM_index[p], (p == ODM_RF_PATH_A ? 'A' : 'B'),
			      pDM_Odm->BbSwingIdxOfdmBase[p]));
	}
	...
	if ((pDM_Odm->RFCalibrateInfo.PowerIndexOffset[ODM_RF_PATH_A] != 0 ||
		...
		if (ThermalValue > pDM_Odm->RFCalibrateInfo.ThermalValue) {
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD,
				     ("Temperature Increasing(A): delta_pi: %d , delta_t: %d, Now_t: %d, EFUSE_t: %d, Last_t: %d\n",
				      pDM_Odm->RFCalibrateInfo.PowerIndexOffset[ODM_RF_PATH_A], delta,
				      ThermalValue, pHalData->EEPROMThermalMeter,
				      pDM_Odm->RFCalibrateInfo.ThermalValue));
			...
			if (c.RfPathCount > 1)
				ODM_RT_TRACE(pDM_Odm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD,
					     ("Temperature Increasing(B): delta_pi: %d , delta_t: %d, Now_t: %d, EFUSE_t: %d, Last_t: %d\n",
					      pDM_Odm->RFCalibrateInfo.PowerIndexOffset[ODM_RF_PATH_B], delta,
					      ThermalValue, pHalData->EEPROMThermalMeter,
					      pDM_Odm->RFCalibrateInfo.ThermalValue));

		} else if (ThermalValue <
			   pDM_Odm->RFCalibrateInfo.ThermalValue) { // Low temperature
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD,
		...
		pDM_Odm->BbSwingIdxCckBase =
			pDM_Odm->BbSwingIdxCck;   // Record last time Power Tracking result as base.
		for (p = ODM_RF_PATH_A; p < c.RfPathCount; p++)
		...
		pDM_Odm->RFCalibrateInfo.ThermalValue =
			ThermalValue;         //Record last Power Tracking Thermal Value

	}
#if !(DM_ODM_SUPPORT_TYPE & ODM_AP)
	if ((delta_IQK >=
	     c.Threshold_IQK)) // Delta temperature is equal to or larger than 20 centigrade (When threshold is 8).
...
)
{
	...
	ODM_RT_TRACE(pDM_Odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
		     ("PHY_ResetIQKResult:: settings regs %d default regs %d\n",
		      (u32)(sizeof(pDM_Odm->RFCalibrateInfo.IQKMatrixRegSetting) / sizeof(
				    IQK_MATRIX_REGS_SETTING)), IQK_Matrix_Settings_NUM));
