...
extern	void	DBG_PrintAllComp(void)
{
	...
	ODM_RT_TRACE(pDM_Odm, COMP_CMD, DBG_LOUD,
		     ("GlobalDebugComponents = %"i64fmt"x\n", GlobalDebugComponents));
			    ...
			    u2Byte Len)
{
	...
	for (i = 0; i < ((Len >> 3) + 1); i++) {
		ODM_RT_TRACE(pDM_Odm, DbgComp, DbgLevel,
			     ("%02X %02X %02X %02X %02X %02X %02X %02X\n",
			      *(pMem + (i * 8)), *(pMem + (i * 8 + 1)), *(pMem + (i * 8 + 2)),
			      *(pMem + (i * 8 + 3)),
			      *(pMem + (i * 8 + 4)), *(pMem + (i * 8 + 5)), *(pMem + (i * 8 + 6)),
			      *(pMem + (i * 8 + 7))));
