...
void Init_ODM_ComInfo(_adapter *adapter)
{
...

	{
		...
		if (!IS_HARDWARE_TYPE_OLDER_THAN_8723A(adapter)) {
			...
			if (pHalData->InterfaceSel == INTF_SEL1_USB_High_Power
			    || pHalData->BoardType ==
			    BOARD_USB_High_PA	/* This is legacy code for hal_data.BoardType */
			   ) {
				ODM_CmnInfoInit(pDM_Odm, ODM_CMNINFO_EXT_LNA, 1);
				ODM_CmnInfoInit(pDM_Odm, ODM_CMNINFO_EXT_PA, 1);
			} else
	...
	ODM_CmnInfoInit(pDM_Odm, ODM_CMNINFO_DOMAIN_CODE_2G,
			pHalData->Regulation2_4G);
	ODM_CmnInfoInit(pDM_Odm, ODM_CMNINFO_DOMAIN_CODE_5G,
