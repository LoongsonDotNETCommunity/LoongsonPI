...
//============================================================
u1Byte halbtc8188c2ant_WifiRssiState(
...
u1Byte halbtc8188c2ant_ActionAlgorithm(
	IN	PBTC_COEXIST		pBtCoexist
)
{
...
VOID halbtc8188c2ant_SetFwBalance(
	IN	PBTC_COEXIST		pBtCoexist,
...
)
{
	...
	BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE_FW_EXEC,
		  ("[BTCoex], Balance=[%s:%dms:%dms], write 0xc=0x%x\n",
...
VOID halbtc8188c2ant_SetFwDiminishWifi(
	IN	PBTC_COEXIST		pBtCoexist,
	IN	BOOLEAN 		bDacOn,
...
)
{
	...
	if ((pBtCoexist->stackInfo.minBtRssi <= -5) && (fwDacSwingLvl == 0x20)) {
		BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE_FW_EXEC,
			  ("[BTCoex], DiminishWiFi 0x20 original, but set 0x18 for Low RSSI!\n"));
		fwDacSwingLvl = 0x18;
	}
	...
	BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE_FW_EXEC,
		  ("[BTCoex], bDacOn=%s, bInterruptOn=%s, bNavOn=%s, write 0xe=0x%x\n",
...
VOID halbtc8188c2ant_DiminishWifi(
	IN	PBTC_COEXIST		pBtCoexist,
	IN	BOOLEAN			bForceExec,
	IN	BOOLEAN			bDacOn,
	IN	BOOLEAN			bInterruptOn,
...
{
	BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE_FW,
		  ("[BTCoex], %s set Diminish Wifi, bDacOn=%s, bInterruptOn=%s, fwDacSwingLvl=%d, bNavOn=%s\n",
		   (bForceExec ? "force to" : ""), (bDacOn ? "ON" : "OFF"),
		   (bInterruptOn ? "ON" : "OFF"), fwDacSwingLvl, (bNavOn ? "ON" : "OFF")));
	...
	if (!bForceExec) {
		BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE_FW_DETAIL,
			  ("[BTCoex], bPreDacOn=%d, bCurDacOn=%d!!\n",
			   pCoexDm->bPreDacOn, pCoexDm->bCurDacOn));
		BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE_FW_DETAIL,
			  ("[BTCoex], bPreInterruptOn=%d, bCurInterruptOn=%d!!\n",
			   pCoexDm->bPreInterruptOn, pCoexDm->bCurInterruptOn));
		BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE_FW_DETAIL,
			  ("[BTCoex], preFwDacSwingLvl=%d, curFwDacSwingLvl=%d!!\n",
			   pCoexDm->preFwDacSwingLvl, pCoexDm->curFwDacSwingLvl));
		BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE_FW_DETAIL,
			  ("[BTCoex], bPreNavOn=%d, bCurNavOn=%d!!\n",
	...
	halbtc8188c2ant_SetFwDiminishWifi(pBtCoexist, bDacOn, bInterruptOn,
					  fwDacSwingLvl, bNavOn);
...
VOID halbtc8188c2ant_SetSwRfRxLpfCorner(
	IN	PBTC_COEXIST		pBtCoexist,
...
)
{
	if (bRxRfShrinkOn) {
		...
		if (pBtCoexist->bInitilized) {
			...
			pBtCoexist->fBtcSetRfReg(pBtCoexist, BTC_RF_A, 0x1e, 0xf0,
						 pCoexDm->btRf0x1eBackup);
		}
	}
}

VOID halbtc8188c2ant_RfShrink(
	IN	PBTC_COEXIST		pBtCoexist,
	IN	BOOLEAN			bForceExec,
	IN	BOOLEAN			bRxRfShrinkOn
)
...
VOID halbtc8188c2ant_SetSwPenaltyTxRateAdaptive(
	IN	PBTC_COEXIST		pBtCoexist,
...
VOID halbtc8188c2ant_LowPenaltyRa(
	IN	PBTC_COEXIST		pBtCoexist,
	IN	BOOLEAN			bForceExec,
	IN	BOOLEAN			bLowPenaltyRa
)
{
	...
	halbtc8188c2ant_SetSwPenaltyTxRateAdaptive(pBtCoexist,
			pCoexDm->bCurLowPenaltyRa);

	pCoexDm->bPreLowPenaltyRa = pCoexDm->bCurLowPenaltyRa;
}

VOID halbtc8188c2ant_SetSwFullTimeDacSwing(
	IN	PBTC_COEXIST		pBtCoexist,
...
)
{
	...
	delay_ms(30);
	halbtc8188c2ant_SetSwFullTimeDacSwing(pBtCoexist, bDacSwingOn,
					      dacSwingLvl);
...
VOID halbtc8188c2ant_SetAdcBackOff(
	IN	PBTC_COEXIST		pBtCoexist,
...
VOID halbtc8188c2ant_AdcBackOff(
	IN	PBTC_COEXIST		pBtCoexist,
	IN	BOOLEAN			bForceExec,
	IN	BOOLEAN			bAdcBackOff
)
...
VOID halbtc8188c2ant_SetAgcTable(
	IN	PBTC_COEXIST		pBtCoexist,
...
VOID halbtc8188c2ant_SetCoexTable(
	IN	PBTC_COEXIST	pBtCoexist,
	IN	u4Byte		val0x6c4,
	IN	u4Byte		val0x6c8,
	IN	u4Byte		val0x6cc
...
VOID halbtc8188c2ant_CoexTable(
	IN	PBTC_COEXIST		pBtCoexist,
	IN	BOOLEAN			bForceExec,
	IN	u4Byte			val0x6c4,
	IN	u4Byte			val0x6c8,
...
{
	BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE_SW,
		  ("[BTCoex], %s write Coex Table 0x6c4=0x%x, 0x6c8=0x%x, 0x6cc=0x%x\n",
	...
	if (!bForceExec) {
		BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE_SW_DETAIL,
			  ("[BTCoex], preVal0x6c4=0x%x, preVal0x6c8=0x%x, preVal0x6cc=0x%x !!\n",
			   pCoexDm->preVal0x6c4, pCoexDm->preVal0x6c8, pCoexDm->preVal0x6cc));
		BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE_SW_DETAIL,
			  ("[BTCoex], curVal0x6c4=0x%x, curVal0x6c8=0x%x, curVal0x6cc=0x%x !!\n",
...
VOID halbtc8188c2ant_CoexAllOff(
	IN	PBTC_COEXIST		pBtCoexist
)
{
	// fw mechanism
	halbtc8188c2ant_Balance(pBtCoexist, NORMAL_EXEC, FALSE, 0, 0);
	halbtc8188c2ant_DiminishWifi(pBtCoexist, NORMAL_EXEC, FALSE, FALSE, 0,
				     FALSE);
...
VOID halbtc8188c2ant_InitCoexDm(
	IN	PBTC_COEXIST		pBtCoexist
)
{
}
...
VOID halbtc8188c2ant_MonitorBtState(
	IN	PBTC_COEXIST			pBtCoexist
)
{
	...
	pBtCoexist->fBtcSet(pBtCoexist, BTC_SET_BL_BT_TRAFFIC_BUSY,
			    &pCoexSta->bBtBusy);
...
VOID halbtc8188c2ant_ActionA2dpBc4(
	IN	PBTC_COEXIST		pBtCoexist
)
{
	...
	if (pCoexSta->bBtBusy) {
		if (BTC_WIFI_BW_HT40 == wifiBw) {
			...
			if (BTC_WIFI_TRAFFIC_TX == wifiTrafficDir) {
				...
				halbtc8188c2ant_DiminishWifi(pBtCoexist, NORMAL_EXEC, FALSE, FALSE, 0x20,
							     FALSE);
...
VOID halbtc8188c2ant_ActionA2dpBc8(
	IN	PBTC_COEXIST		pBtCoexist
)
{
	u1Byte			wifiRssiState;
	...
	if (pCoexSta->bA2dpBusy && bWifiBusy) {
		...
		if (BTC_WIFI_TRAFFIC_TX == wifiTrafficDir) {
			halbtc8188c2ant_Balance(pBtCoexist, NORMAL_EXEC, TRUE, 0xc, 0x18);
			halbtc8188c2ant_DiminishWifi(pBtCoexist, NORMAL_EXEC, TRUE, FALSE, 0x20,
						     FALSE);
		} else if (BTC_WIFI_TRAFFIC_RX == wifiTrafficDir) {
		...
		// fw mechanism first
		halbtc8188c2ant_Balance(pBtCoexist, NORMAL_EXEC, FALSE, 0, 0);
		halbtc8188c2ant_DiminishWifi(pBtCoexist, NORMAL_EXEC, TRUE, TRUE, 0x18,
					     FALSE);
...
VOID halbtc8188c2ant_ActionA2dp(
	IN	PBTC_COEXIST		pBtCoexist
)
{
	if (BTC_CHIP_CSR_BC4 == pBtCoexist->boardInfo.btChipType) {
...
VOID halbtc8188c2ant_ActionPanBc4(
	IN	PBTC_COEXIST		pBtCoexist
)
{
	...
	if (bBtHsOn) {
		halbtc8188c2ant_Balance(pBtCoexist, NORMAL_EXEC, FALSE, 0, 0);
		halbtc8188c2ant_DiminishWifi(pBtCoexist, NORMAL_EXEC, FALSE, FALSE, 0x0,
					     FALSE);
	} else {
...
VOID halbtc8188c2ant_ActionPanBc8(
	IN	PBTC_COEXIST		pBtCoexist
)
{
	BOOLEAN		bBtHsOn = FALSE, bWifiBusy = FALSE;
...
VOID halbtc8188c2ant_ActionPan(
	IN	PBTC_COEXIST		pBtCoexist
)
{
	if (BTC_CHIP_CSR_BC4 == pBtCoexist->boardInfo.btChipType) {
...
VOID halbtc8188c2ant_ActionHid(
	IN	PBTC_COEXIST		pBtCoexist
)
{
	u4Byte		wifiBw, wifiTrafficDir;
	...
	if (BTC_WIFI_BW_LEGACY == wifiBw) {
		...
		halbtc8188c2ant_CoexTable(pBtCoexist, NORMAL_EXEC, 0x55555555, 0x000000f0,
					  0x40000010);
...
VOID halbtc8188c2ant_ActionSco(
	IN	PBTC_COEXIST		pBtCoexist
)
{
	u1Byte	wifiRssiState;
...
VOID halbtc8188c2ant_ActionHidA2dpBc4(
	IN	PBTC_COEXIST		pBtCoexist
)
{
	u1Byte		wifiRssiState;
...
VOID halbtc8188c2ant_ActionHidA2dpBc8(
	IN	PBTC_COEXIST		pBtCoexist
)
{
	u1Byte		wifiRssiState;
...
VOID halbtc8188c2ant_ActionHidA2dp(
	IN	PBTC_COEXIST		pBtCoexist
)
{
	if (BTC_CHIP_CSR_BC4 == pBtCoexist->boardInfo.btChipType) {
...
VOID halbtc8188c2ant_ActionHidPanBc4(
	IN	PBTC_COEXIST		pBtCoexist
)
{
	BOOLEAN		bBtHsOn = FALSE, bWifiBusy = FALSE;
...
VOID halbtc8188c2ant_ActionHidPanBc8(
	IN	PBTC_COEXIST		pBtCoexist
)
{
	BOOLEAN		bBtHsOn = FALSE, bWifiBusy = FALSE;
...
VOID halbtc8188c2ant_ActionHidPan(
	IN	PBTC_COEXIST		pBtCoexist
)
{
	if (BTC_CHIP_CSR_BC4 == pBtCoexist->boardInfo.btChipType) {
...
VOID halbtc8188c2ant_ActionPanA2dpBc4(
	IN	PBTC_COEXIST		pBtCoexist
)
{
	BOOLEAN		bBtHsOn = FALSE, bWifiBusy = FALSE;
...
VOID halbtc8188c2ant_ActionPanA2dpBc8(
	IN	PBTC_COEXIST		pBtCoexist
)
{
	BOOLEAN		bBtHsOn = FALSE, bWifiBusy = FALSE;
...
VOID halbtc8188c2ant_ActionPanA2dp(
	IN	PBTC_COEXIST		pBtCoexist
)
{
	if (BTC_CHIP_CSR_BC4 == pBtCoexist->boardInfo.btChipType) {
...
//============================================================
VOID EXhalbtc8188c2ant_PowerOnSetting(
	IN	PBTC_COEXIST		pBtCoexist
)
{
}

VOID EXhalbtc8188c2ant_InitHwConfig(
	IN	PBTC_COEXIST		pBtCoexist,
...
)
{
	...
	if ((BTC_CHIP_CSR_BC4 == pBtCoexist->boardInfo.btChipType) ||
		...
		halbtc8188c2ant_CoexTable(pBtCoexist, FORCE_EXEC, 0xaaaa9aaa, 0xffbd0040,
					  0x40000010);
	}
}

VOID EXhalbtc8188c2ant_InitCoexDm(
	IN	PBTC_COEXIST		pBtCoexist
)
{
	BTC_PRINT(BTC_MSG_INTERFACE, INTF_INIT,
...
VOID EXhalbtc8188c2ant_DisplayCoexInfo(
	IN	PBTC_COEXIST		pBtCoexist
)
{
...
VOID EXhalbtc8188c2ant_IpsNotify(
	IN	PBTC_COEXIST		pBtCoexist,
	IN	u1Byte			type
)
{
...
VOID EXhalbtc8188c2ant_LpsNotify(
	IN	PBTC_COEXIST		pBtCoexist,
	IN	u1Byte			type
)
{
...
VOID EXhalbtc8188c2ant_ScanNotify(
	IN	PBTC_COEXIST		pBtCoexist,
	IN	u1Byte			type
)
{
...
VOID EXhalbtc8188c2ant_ConnectNotify(
	IN	PBTC_COEXIST		pBtCoexist,
	IN	u1Byte			type
)
{
...
VOID EXhalbtc8188c2ant_MediaStatusNotify(
	IN	PBTC_COEXIST			pBtCoexist,
	IN	u1Byte				type
)
{
...
VOID EXhalbtc8188c2ant_SpecialPacketNotify(
	IN	PBTC_COEXIST			pBtCoexist,
	IN	u1Byte				type
)
{
...
VOID EXhalbtc8188c2ant_BtInfoNotify(
	IN	PBTC_COEXIST		pBtCoexist,
	IN	pu1Byte			tmpBuf,
	IN	u1Byte			length
)
...
VOID EXhalbtc8188c2ant_HaltNotify(
	IN	PBTC_COEXIST			pBtCoexist
)
{
...
VOID EXhalbtc8188c2ant_Periodical(
	IN	PBTC_COEXIST			pBtCoexist
)
{
	u1Byte	algorithm;
	...
	if ((BTC_CHIP_CSR_BC4 == pBtCoexist->boardInfo.btChipType) ||
		...
		case BT_8188C_2ANT_COEX_ALGO_PAN:
			BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE,
				  ("[BTCoex], Action 2-Ant, algorithm = PAN\n"));
		...
		case BT_8188C_2ANT_COEX_ALGO_HID_PAN:
			BTC_PRINT(BTC_MSG_ALGORITHM, ALGO_TRACE,
				  ("[BTCoex], Action 2-Ant, algorithm = PAN+HID\n"));
