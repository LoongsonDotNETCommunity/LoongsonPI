...
void UpdateEarlyModeInfo8812(struct xmit_priv *pxmitpriv,
			     struct xmit_buf *pxmitbuf)
{
	...
	for (index = 0; index < pframe->agg_num; index++) {
		...
		for (j = 0; j < eminfo.EMPktNum ; j++) {
			eminfo.EMPktLen[j] = pxmitpriv->agg_pkt[index + 1 + j].pkt_len +
					     4; // 4 bytes CRC
		}
	...
	_rtw_memset(pxmitpriv->agg_pkt, 0,
		    sizeof(struct agg_pkt_info)*MAX_AGG_PKT_NUM);
...
void rtl8812a_fill_txdesc_vcs(PADAPTER padapter,
...
void rtl8812a_fill_txdesc_phy(PADAPTER padapter,
			      struct pkt_attrib *pattrib, u8 *ptxdesc)
{
