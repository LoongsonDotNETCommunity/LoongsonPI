...
void interrupt_handler_8812au(_adapter *padapter, u16 pkt_len, u8 *pbuf)
{
	...
	// HISR
	_rtw_memcpy(&(pHalData->IntArray[0]),
		    &(pbuf[USB_INTR_CONTENT_HISR_OFFSET]), 4);
	_rtw_memcpy(&(pHalData->IntArray[1]),
	...
	// C2H Event
	if (pbuf[0] != 0) {
		_rtw_memcpy(&(pHalData->C2hArray[0]), &(pbuf[USB_INTR_CONTENT_C2H_OFFSET]),
			    16);
	...
	if (IS_MCAST(paddr1) == _FALSE) { //unicast packets
		...
		if (_TRUE == clone) {
			...
			if (precvframe_if2) {
				...
				if (rtw_os_alloc_recvframe(secondary_padapter, precvframe_if2, pbuf,
							   NULL) == _SUCCESS) {
