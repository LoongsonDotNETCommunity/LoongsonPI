	...
	if (RT_GetInterfaceSelection(padapter) == INTF_SEL2_MINICARD ||
		...
		case LED_PIN_LED0:
			...
			rtw_write8(padapter, REG_LEDCFG2,
				   (LedCfg & 0xf0) | BIT5 | BIT6); // SW control led0 on.
			break;

		case LED_PIN_LED1:
			rtw_write8(padapter, REG_LEDCFG2,
				   (LedCfg & 0x0f) | BIT5); // SW control led1 on.
			break;
		...
		case LED_PIN_LED0:
			if (pHalData->AntDivCfg == 0) {
				...
				rtw_write8(padapter, REG_LEDCFG0,
					   (LedCfg & 0x70) | BIT5); // SW control led0 on.
				RT_TRACE(_module_rtl8712_led_c_, _drv_info_, ("SwLedOn LED0 0x%x\n",
						rtw_read32(padapter, REG_LEDCFG0)));
			} else {
				LedCfg = rtw_read8(padapter, REG_LEDCFG2);
				rtw_write8(padapter, REG_LEDCFG2,
					   (LedCfg & 0xe0) | BIT7 | BIT6 | BIT5); // SW control led0 on.
				RT_TRACE(_module_rtl8712_led_c_, _drv_info_,
					 ("SwLedOn LED0 Addr 0x%x,  0x%x\n", REG_LEDCFG2, rtw_read32(padapter,
							 REG_LEDCFG2)));
			}
			break;
		...
		case LED_PIN_LED1:
			...
			rtw_write8(padapter, (REG_LEDCFG1),
				   (LedCfg & 0x70) | BIT5); // SW control led1 on.
	...
	if (RT_GetInterfaceSelection(Adapter) == INTF_SEL2_MINICARD ||
		...
		case LED_PIN_LED0:
			...
			RT_TRACE(_module_rtl8712_led_c_, _drv_info_,
				 ("In SwLedOn,LedAddr:%X LEDPIN=%d\n", REG_LEDCFG2, pLed->LedPin));
			LedCfg = rtw_read8(Adapter, REG_LEDCFG2);
			rtw_write8(Adapter, REG_LEDCFG2,
				   (LedCfg & 0xf0) | BIT5 | BIT6); // SW control led0 on.
			break;
		...
		case LED_PIN_LED2:
			if (IS_HARDWARE_TYPE_8821U(Adapter)) {
				LedCfg = rtw_read8(Adapter, REG_LEDCFG2);
				rtw_write8(Adapter, REG_LEDCFG2,
					   ((LedCfg & 0x20) & (~BIT3)) | BIT5); // SW control led0 on.
