...
void Hal_mpt_SwitchRfSetting(PADAPTER pAdapter)
{
	...
	pmp->MptCtx.backup0x52_RF_A = (u1Byte)PHY_QueryRFReg(pAdapter, RF_PATH_A,
				      RF_0x52, 0x000F0);
	pmp->MptCtx.backup0x52_RF_B = (u1Byte)PHY_QueryRFReg(pAdapter, RF_PATH_B,
				      RF_0x52, 0x000F0);
...
//	SelectChannel(pAdapter, pmp->channel);
	set_channel_bwmode(pAdapter, pmp->channel, pmp->channel_offset,
			   pmp->bandwidth);
#else
	u8 		eRFPath;
	...
	if (pHalData->CurrentChannel == 14
	    && !pDM_Odm->RFCalibrateInfo.bCCKinCH14) {
		pDM_Odm->RFCalibrateInfo.bCCKinCH14 = _TRUE;
		Hal_MPT_CCKTxPowerAdjust(pAdapter, pDM_Odm->RFCalibrateInfo.bCCKinCH14);
	} else if (pHalData->CurrentChannel != 14
		   && pDM_Odm->RFCalibrateInfo.bCCKinCH14) {
...
void Hal_SetCCKTxPower(PADAPTER pAdapter, u8 *TxPower)
{
	...
	tmpval = (TxPower[RF_PATH_A] << 16) | (TxPower[RF_PATH_A] << 8) |
		 TxPower[RF_PATH_A];
	write_bbreg(pAdapter, rTxAGC_B_CCK11_A_CCK2_11, 0xffffff00, tmpval);

	// rf-B cck tx power
	write_bbreg(pAdapter, rTxAGC_B_CCK11_A_CCK2_11, bMaskByte0,
		    TxPower[RF_PATH_B]);
	tmpval = (TxPower[RF_PATH_B] << 16) | (TxPower[RF_PATH_B] << 8) |
		 TxPower[RF_PATH_B];
...
void Hal_SetTxPower(PADAPTER pAdapter)
{
	...
	path = (pAdapter->mppriv.antenna_tx == ANTENNA_A) ? (ODM_RF_PATH_A) :
	       (ODM_RF_PATH_B);
...
void Hal_SetAntenna(PADAPTER pAdapter)
{
	...
	case ANTENNA_A:
		...
		if (pHalData->RFEType == 3)
			PHY_SetBBReg(pAdapter, r_ANTSEL_SW_Jaguar, bMask_AntselPathFollow_Jaguar,
				     0x0);
		break;
	...
	case ANTENNA_A:
		...
		PHY_SetRFReg(pAdapter, ODM_RF_PATH_B, RF_AC_Jaguar, 0xF0000,
			     0x1); // RF_B_0x0[19:16] = 1, Standby mode
		PHY_SetBBReg(pAdapter, rCCK_RX_Jaguar, bCCK_RX_Jaguar, 0x0);
		PHY_SetRFReg(pAdapter, ODM_RF_PATH_A, RF_AC_Jaguar,
	...
	case ANTENNA_B:
		...
		PHY_SetRFReg(pAdapter, ODM_RF_PATH_A, RF_AC_Jaguar, 0xF0000,
			     0x1); // RF_A_0x0[19:16] = 1, Standby mode
		PHY_SetBBReg(pAdapter, rCCK_RX_Jaguar, bCCK_RX_Jaguar, 0x1);
		PHY_SetRFReg(pAdapter, ODM_RF_PATH_B, RF_AC_Jaguar,
			     BIT19 | BIT18 | BIT17 | BIT16, 0x3);
...
s32 Hal_SetThermalMeter(PADAPTER pAdapter, u8 target_ther)
{
	...
	if (!netif_running(pAdapter->pnetdev)) {
		RT_TRACE(_module_mp_, _drv_warning_,
			 ("SetThermalMeter! Fail: interface not opened!\n"));
		return _FAIL;
	}

	if (check_fwstate(&pAdapter->mlmepriv, WIFI_MP_STATE) == _FALSE) {
		RT_TRACE(_module_mp_, _drv_warning_,
			 ("SetThermalMeter: Fail! not in MP mode!\n"));
		return _FAIL;
	}
...
u8 Hal_ReadRFThermalMeter(PADAPTER pAdapter)
{
	...
	ThermalValue = (u1Byte)PHY_QueryRFReg(pAdapter, ODM_RF_PATH_A,
					      RF_T_METER_8812A, 0xfc00);	// 0x42: RF Reg[15:10]
...
void Hal_SetSingleCarrierTx(PADAPTER pAdapter, u8 bStart)
{
	...
	if (bStart) { // Start Single Carrier.
		...
		if (IS_HARDWARE_TYPE_JAGUAR(pAdapter))
			PHY_SetBBReg(pAdapter, rSingleTone_ContTx_Jaguar, BIT18 | BIT17 | BIT16,
				     OFDM_SingleCarrier);
		else
			PHY_SetBBReg(pAdapter, rOFDM1_LSTF, BIT30 | BIT29 | BIT28,
				     OFDM_SingleCarrier);
...
void Hal_SetSingleToneTx(PADAPTER pAdapter, u8 bStart)
{
	...
	if (bStart) { // Start Single Tone.
		if (IS_HARDWARE_TYPE_JAGUAR(pAdapter)) {
			...
			regRF0x0 = PHY_QueryRFReg(pAdapter, ODM_RF_PATH_A, RF_AC_Jaguar,
						  bRFRegOffsetMask);
			...
			PHY_SetBBReg(pAdapter, rOFDMCCKEN_Jaguar, BIT29 | BIT28,
				     0x0); // Disable CCK and OFDM

			if (pMptCtx->MptRfPath == RF_PATH_AB) {
				for (p = ODM_RF_PATH_A; p <= ODM_RF_PATH_B; ++p) {
					PHY_SetRFReg(pAdapter, p, RF_AC_Jaguar, 0xF0000,
						     0x2); // Tx mode: RF0x00[19:16]=4'b0010
					PHY_SetRFReg(pAdapter, p, RF_AC_Jaguar, 0x1F,
						     0x0); // Lowest RF gain index: RF_0x0[4:0] = 0
					PHY_SetRFReg(pAdapter, p, LNA_Low_Gain_3, BIT1, 0x1); // RF LO enabled
				}
			} else {
				PHY_SetRFReg(pAdapter, pMptCtx->MptRfPath, RF_AC_Jaguar, 0xF0000,
					     0x2); // Tx mode: RF0x00[19:16]=4'b0010
				PHY_SetRFReg(pAdapter, pMptCtx->MptRfPath, RF_AC_Jaguar, 0x1F,
					     0x0); // Lowest RF gain index: RF_0x0[4:0] = 0
				PHY_SetRFReg(pAdapter, pMptCtx->MptRfPath, LNA_Low_Gain_3, BIT1,
					     0x1); // RF LO enabled
			}


			PHY_SetBBReg(pAdapter, rA_RFE_Pinmux_Jaguar, 0xFF00F0,
				     0x77007);  // 0xCB0[[23:16, 7:4] = 0x77007
			PHY_SetBBReg(pAdapter, rB_RFE_Pinmux_Jaguar, 0xFF00F0,
				     0x77007);  // 0xCB0[[23:16, 7:4] = 0x77007

			if (pHalData->ExternalPA_5G) {

				PHY_SetBBReg(pAdapter, rA_RFE_Pinmux_Jaguar + 4, 0xFF00000,
					     0x12); // 0xCB4[23:16] = 0x12
				PHY_SetBBReg(pAdapter, rB_RFE_Pinmux_Jaguar + 4, 0xFF00000,
...
void Hal_SetContinuousTx(PADAPTER pAdapter, u8 bStart)
{
	...
	if (bStart) {
		write_bbreg(pAdapter, rRx_Wait_CCCA, read_bbreg(pAdapter,
