...
)
{
	...
	if ((!pHalData->bSupportUSB3)
	    && (IS_TEST_CHIP(pHalData->VersionID))) { // USB 2.0 or older
...
static	VOID phy_InitBBRFRegisterDefinition(
	IN	PADAPTER		Adapter
)
{
	HAL_DATA_TYPE		*pHalData = GET_HAL_DATA(Adapter);
...
#ifdef CONFIG_EMBEDDED_FWIMG
		if (HAL_STATUS_SUCCESS != ODM_ConfigRFWithHeaderFile(&pHalData->odmpriv,
				CONFIG_RF_TXPWR_LMT, 0))
			rtStatus = _FAIL;
#else
		rtStatus = PHY_ConfigBBWithPowerLimitTableParaFile(Adapter,
				pszRFTxPwrLmtFile);
#endif
...
#ifdef CONFIG_EMBEDDED_FWIMG
	if (HAL_STATUS_SUCCESS != ODM_ConfigBBWithHeaderFile(&pHalData->odmpriv,
			CONFIG_BB_PHY_REG))
		rtStatus = _FAIL;
#else
...
u8 phy_getPowerByRateBaseIndex(
	IN	BAND_TYPE			Band,
	IN	u8					Rate
)
{
...
VOID PHY_ConvertPowerLimitToPowerIndex(
	IN	PADAPTER			Adapter
)
{
	HAL_DATA_TYPE		*pHalData = GET_HAL_DATA(Adapter);
	...
	for (regulation = 0; regulation < MAX_REGULATION_NUM; ++regulation) {
		for (bw = 0; bw < MAX_2_4G_BANDWITH_NUM; ++bw) {
			for (group = 0; group < MAX_2_4G_CHANNEL_NUM; ++group) {
				...
				for (rateSection = 0; rateSection < MAX_2_4G_RATE_SECTION_NUM;
				     ++rateSection) {
					...
					// all the path
					tempPwrLmt =
						pHalData->TxPwrLimit_2_4G[regulation][bw][rateSection][group][ODM_RF_PATH_A];
					...
					for (rfPath = 0; rfPath < MAX_RF_PATH_NUM; ++rfPath) {
						...
						if (tempPwrLmt != MAX_POWER_INDEX) {
							...
							pHalData->TxPwrLimit_2_4G[regulation][bw][rateSection][group][rfPath] =
								tempValue;
						}

						DBG_871X("TxPwrLimit_2_4G[regulation %d][bw %d][rateSection %d][group %d] %d=\n\
							(TxPwrLimit in dBm %d - BW40PwrLmt2_4G[channel %d][rfPath %d] %d) \n",
							 regulation, bw, rateSection, group,
							 pHalData->TxPwrLimit_2_4G[regulation][bw][rateSection][group][rfPath],
		...
		for (regulation = 0; regulation < MAX_REGULATION_NUM; ++regulation) {
			for (bw = 0; bw < MAX_5G_BANDWITH_NUM; ++bw) {
				for (group = 0; group < MAX_5G_CHANNEL_NUM; ++group) {
					...
					for (rateSection = 1; rateSection < MAX_5G_RATE_SECTION_NUM;
					     ++rateSection) {
						if (pHalData->odmpriv.PhyRegPgValueType == PHY_REG_PG_EXACT_VALUE) {
						...
						// all the path
						tempPwrLmt =
							pHalData->TxPwrLimit_5G[regulation][bw][rateSection][group][ODM_RF_PATH_A];
						if (tempPwrLmt == MAX_POWER_INDEX) {
						...
						for (rfPath = ODM_RF_PATH_B; rfPath < MAX_RF_PATH_NUM; ++rfPath) {
							...
							if (tempPwrLmt != MAX_POWER_INDEX) {
								...
								pHalData->TxPwrLimit_5G[regulation][bw][rateSection][group][rfPath] =
									tempValue;
							}
		...
		for (regulation = 0; regulation < MAX_REGULATION_NUM; ++regulation) {
			for (bw = 0; bw < MAX_5G_BANDWITH_NUM; ++bw) {
				for (group = 0; group < MAX_5G_CHANNEL_NUM; ++group) {
					...
					for (rateSection = 0; rateSection < MAX_5G_RATE_SECTION_NUM;
					     ++rateSection) {
						if (pHalData->odmpriv.PhyRegPgValueType == PHY_REG_PG_EXACT_VALUE) {
							// obtain the base dBm values in 5G band
							// OFDM => 54M, HT 1T => MCS7, HT 2T => MCS15,
...
VOID PHY_SetPowerLimitTableValue(
	IN	PDM_ODM_T		pDM_Odm,
	IN	s8				*Regulation,
	IN	s8				*Band,
	IN	s8				*Bandwidth,
...
)
{
	...
	if (eqNByte(Band, "2.4G", 4)) {
		...
		pHalData->TxPwrLimit_2_4G[regulation][bandwidth][rateSection][channelGroup][ODM_RF_PATH_A]
			= powerLimit;
		...
		pHalData->TxPwrLimit_5G[regulation][bandwidth][rateSection][channelGroup][ODM_RF_PATH_A]
			= powerLimit;
	} else {
...
)
{
	...
	if ((Adapter->registrypriv.RegEnableTxPowerLimit == 0
	     && pHalData->EEPROMRegulatory != 1) ||
	    pHalData->EEPROMRegulatory == 2)
...
// 2012/10/18
//
VOID PHY_StorePwrByRateIndexVhtSeries(
	IN	PADAPTER	Adapter,
	IN	u32			RegAddr,
	IN	u32			BitMask,
	IN	u32			Data
)
{
	...
	pHalData->TxPwrByRateOffset[pHalData->TxPwrByRateBand][rf_path][rate_section]
		= Data;
...
VOID phy_ChangePGDataFromExactToRelativeValue(
	IN	u32	*pData,
	IN	u8		Start,
	IN	u8		End,
	IN	u8		BaseValue
)
{
	...
	for (i = 3; i >= 0; --i) {
		if (i >= Start && i <= End) {
			...
			TempValue = (TempValue > BaseValue) ? TempValue - BaseValue : BaseValue -
				    TempValue;
		} else {
...
)
{
	...
	case 0xE44:
		...
		BaseValue = ((u8)(
				     pHalData->TxPwrByRateOffset[pHalData->TxPwrByRateBand][rf_path][rate_section
						     - 1] >> 28) & 0xF) * 10 +
			    ((u8)(pHalData->TxPwrByRateOffset[pHalData->TxPwrByRateBand][rf_path][rate_section
					    - 1] >> 24) & 0xF);
...
VOID phy_PreprocessPGDataFromExactToRelativeValue(
	IN	PADAPTER	Adapter,
	IN	u4Byte		RegAddr,
	IN	u4Byte		BitMask,
	IN	pu4Byte		pData
...
VOID phy_StorePwrByRateIndexBase(
	IN	PADAPTER	Adapter,
	IN	u32			RegAddr,
	IN	u32			Data
)
{
	...
	if (pHalData->TxPwrByRateTable == 1
	    && pHalData->TxPwrByRateBand == 0) {  // 2.4G
		if (pHalData->odmpriv.PhyRegPgValueType == PHY_REG_PG_EXACT_VALUE) {
	...
	} else if (pHalData->TxPwrByRateTable == 3
		   && pHalData->TxPwrByRateBand == 1) {  // 5G
		if (pHalData->odmpriv.PhyRegPgValueType == PHY_REG_PG_EXACT_VALUE) {
			Base = (((u8)(Data >> 28) & 0xF) * 10 +
				((u8)(Data >> 24) & 0xF));
	...
	} else if (pHalData->TxPwrByRateTable == 2
		   && pHalData->TxPwrByRateBand == 0) {  // 2.4G
		if (pHalData->odmpriv.PhyRegPgValueType == PHY_REG_PG_EXACT_VALUE) {
			Base = (((u8)(Data >> 28) & 0xF) * 10 +
				((u8)(Data >> 24) & 0xF));
...
)
{
	...
	if (pHalData->odmpriv.PhyRegPgValueType == PHY_REG_PG_EXACT_VALUE) {
		...
		phy_PreprocessPGDataFromExactToRelativeValue(Adapter, RegAddr, BitMask,
				&Data);
...
)
{
	...
	case WIRELESS_MODE_B:
		...
		TxPwrIdx = (u8)((pHalData->OriginalCckTxPwrIdx * (PowerInDbm -
				 pHalData->MinCCKDbm)) / (pHalData->MaxCCKDbm - pHalData->MinCCKDbm));
		break;

	case WIRELESS_MODE_G:
	case WIRELESS_MODE_N_24G:
		Offset = -8;
		TxPwrIdx = (u8)((pHalData->OriginalOfdm24GTxPwrIdx *
				 (PowerInDbm - pHalData->MinHOFDMDbm)) / (pHalData->MaxHOFDMDbm -
						 pHalData->MinHOFDMDbm));
		break;
...
)
{
	...
	pHalData->MaxLOFDMDbm = phy_TxPwrIdxToDbm(Adapter, WIRELESS_MODE_G,
				TxPwrLevel);
	...
	pHalData->MaxHOFDMDbm = phy_TxPwrIdxToDbm(Adapter, WIRELESS_MODE_G,
				TxPwrLevel);
...
)
{
	...
	if (pMgntInfo->OpMode == RT_OP_MODE_INFRASTRUCTURE &&
		...
		u1Byte	CckCellPwrIdx = phy_DbmToTxPwrIdx(Adapter, WIRELESS_MODE_B,
					pCcxInfo->CcxCellPwr);
		u1Byte	LegacyOfdmCellPwrIdx = phy_DbmToTxPwrIdx(Adapter, WIRELESS_MODE_G,
					       pCcxInfo->CcxCellPwr);
		u1Byte	OfdmCellPwrIdx = phy_DbmToTxPwrIdx(Adapter, WIRELESS_MODE_N_24G,
					 pCcxInfo->CcxCellPwr);

		RT_TRACE(COMP_TXAGC, DBG_LOUD,
			 ("CCX Cell Limit: %d dbm => CCK Tx power index : %d, Legacy OFDM Tx power index : %d, OFDM Tx power index: %d\n",
			  pCcxInfo->CcxCellPwr, CckCellPwrIdx, LegacyOfdmCellPwrIdx,
			  OfdmCellPwrIdx));
		RT_TRACE(COMP_TXAGC, DBG_LOUD,
			 ("EEPROM channel(%d) => CCK Tx power index: %d, Legacy OFDM Tx power index : %d, OFDM Tx power index: %d\n",
			  channel, cckPowerLevel[0],
			  ofdmPowerLevel[0] + pHalData->LegacyHTTxPowerDiff, ofdmPowerLevel[0]));

		// CCK
		if (cckPowerLevel[0] > CckCellPwrIdx)
			cckPowerLevel[0] = CckCellPwrIdx;
		// Legacy OFDM, HT OFDM
		if (ofdmPowerLevel[0] + pHalData->LegacyHTTxPowerDiff >
		...
		RT_TRACE(COMP_TXAGC, DBG_LOUD,
			 ("Altered CCK Tx power index : %d, Legacy OFDM Tx power index: %d, OFDM Tx power index: %d\n",
			  cckPowerLevel[0], ofdmPowerLevel[0] + pHalData->LegacyHTTxPowerDiff,
			  ofdmPowerLevel[0]));
	}
#else
	// Add or not ???
...
//
u32 phy_GetTxPwrByRateOffset_8812(
...
)
{
	...
	tx_pwr_diff = (pHalData->TxPwrByRateOffset[Band][Rf_Path][Rate_Section] >>
		       shift) & 0xff;
...
)
{
	...
	if (bIn24G) { //3 ============================== 2.4 G ==============================
		...
		// BW20-1S, BW20-2S
		if (BandWidth == CHANNEL_WIDTH_20) {
			if ((MGN_MCS0 <= Rate && Rate <= MGN_MCS15) || (MGN_VHT2SS_MCS0 <= Rate
					&& Rate <= MGN_VHT2SS_MCS9))
				txPower += pHalData->BW20_24G_Diff[RFPath][TX_1S];
			if ((MGN_MCS8 <= Rate && Rate <= MGN_MCS15) || (MGN_VHT2SS_MCS0 <= Rate
					&& Rate <= MGN_VHT2SS_MCS9))
		...
		// BW40-1S, BW40-2S
			...
			if ((MGN_MCS0 <= Rate && Rate <= MGN_MCS15) || (MGN_VHT1SS_MCS0 <= Rate
					&& Rate <= MGN_VHT2SS_MCS9))
		...
		// BW80-1S, BW80-2S
			...
			if ((MGN_MCS0 <= Rate && Rate <= MGN_MCS15)  || (MGN_VHT1SS_MCS0 <= Rate
				...
				txPower = pHalData->Index5G_BW80_Base[RFPath][chnlIdx] +
					  pHalData->BW80_5G_Diff[RFPath][TX_1S];
			if ((MGN_MCS8 <= Rate && Rate <= MGN_MCS15) || (MGN_VHT2SS_MCS0 <= Rate
					&& Rate <= MGN_VHT2SS_MCS9))
				txPower = pHalData->Index5G_BW80_Base[RFPath][chnlIdx] +
					  pHalData->BW80_5G_Diff[RFPath][TX_1S] +
					  pHalData->BW80_5G_Diff[RFPath][TX_2S];
	...
	// HT=3/4/5/6 			MCS0-3 MCS4-7 MCS8-11 MCS12-15
	// VHT=7/8/9/10/11		1SSMCS0-3 1SSMCS4-7 2SSMCS1/0/1SSMCS/9/8 2SSMCS2-5
	if (pregistrypriv->RegPwrByRate == _FALSE
	    && pHalData->EEPROMRegulatory != 2) {
		powerDiffByRate = phy_GetTxPwrByRateOffset_8812(pAdapter, (u8)(!bIn24G),
				  RFPath, Rate);

		if ((pregistrypriv->RegEnableTxPowerLimit == 1
		     && pHalData->EEPROMRegulatory != 2) ||
		    pHalData->EEPROMRegulatory == 1) {
			u8 limit = 0;
			limit = PHY_GetPowerLimitValue(pAdapter, pregistrypriv->RegPwrTblSel,
						       (u8)(!bIn24G) ? BAND_ON_5G : BAND_ON_2_4G, BandWidth,
						       (ODM_RF_RADIO_PATH_E)RFPath, Rate, Channel);
	...
	if (pDM_Odm->Modify_TxAGC_Flag_PathA
	    || pDM_Odm->Modify_TxAGC_Flag_PathB) {  //20130424 Mimic whether path A or B has to modify TxAGC
...
VOID PHY_SetTxPowerIndex_8812A(
	IN	PADAPTER			Adapter,
...
)
{
	...
	if ((PowerIndex % 2 == 1) && IS_HARDWARE_TYPE_JAGUAR(Adapter)
	    && IS_TEST_CHIP(pHalData->VersionID))
		PowerIndex -= 1;
	...
	if (RFPath == RF_PATH_A) {
		...
		case MGN_6M:
			PHY_SetBBReg(Adapter, rTxAGC_A_Ofdm18_Ofdm6_JAguar, bMaskByte0,
				     PowerIndex);
			break;
		case MGN_9M:
			PHY_SetBBReg(Adapter, rTxAGC_A_Ofdm18_Ofdm6_JAguar, bMaskByte1,
				     PowerIndex);
			break;
		case MGN_12M:
			PHY_SetBBReg(Adapter, rTxAGC_A_Ofdm18_Ofdm6_JAguar, bMaskByte2,
				     PowerIndex);
			break;
		case MGN_18M:
			PHY_SetBBReg(Adapter, rTxAGC_A_Ofdm18_Ofdm6_JAguar, bMaskByte3,
				     PowerIndex);
			break;

		case MGN_24M:
			PHY_SetBBReg(Adapter, rTxAGC_A_Ofdm54_Ofdm24_JAguar, bMaskByte0,
				     PowerIndex);
			break;
		case MGN_36M:
			PHY_SetBBReg(Adapter, rTxAGC_A_Ofdm54_Ofdm24_JAguar, bMaskByte1,
				     PowerIndex);
			break;
		case MGN_48M:
			PHY_SetBBReg(Adapter, rTxAGC_A_Ofdm54_Ofdm24_JAguar, bMaskByte2,
				     PowerIndex);
			break;
		case MGN_54M:
			PHY_SetBBReg(Adapter, rTxAGC_A_Ofdm54_Ofdm24_JAguar, bMaskByte3,
				     PowerIndex);
			break;
		...
		case MGN_VHT1SS_MCS0:
			PHY_SetBBReg(Adapter, rTxAGC_A_Nss1Index3_Nss1Index0_JAguar, bMaskByte0,
				     PowerIndex);
			break;
		case MGN_VHT1SS_MCS1:
			PHY_SetBBReg(Adapter, rTxAGC_A_Nss1Index3_Nss1Index0_JAguar, bMaskByte1,
				     PowerIndex);
			break;
		case MGN_VHT1SS_MCS2:
			PHY_SetBBReg(Adapter, rTxAGC_A_Nss1Index3_Nss1Index0_JAguar, bMaskByte2,
				     PowerIndex);
			break;
		case MGN_VHT1SS_MCS3:
			PHY_SetBBReg(Adapter, rTxAGC_A_Nss1Index3_Nss1Index0_JAguar, bMaskByte3,
				     PowerIndex);
			break;

		case MGN_VHT1SS_MCS4:
			PHY_SetBBReg(Adapter, rTxAGC_A_Nss1Index7_Nss1Index4_JAguar, bMaskByte0,
				     PowerIndex);
			break;
		case MGN_VHT1SS_MCS5:
			PHY_SetBBReg(Adapter, rTxAGC_A_Nss1Index7_Nss1Index4_JAguar, bMaskByte1,
				     PowerIndex);
			break;
		case MGN_VHT1SS_MCS6:
			PHY_SetBBReg(Adapter, rTxAGC_A_Nss1Index7_Nss1Index4_JAguar, bMaskByte2,
				     PowerIndex);
			break;
		case MGN_VHT1SS_MCS7:
			PHY_SetBBReg(Adapter, rTxAGC_A_Nss1Index7_Nss1Index4_JAguar, bMaskByte3,
				     PowerIndex);
			break;

		case MGN_VHT1SS_MCS8:
			PHY_SetBBReg(Adapter, rTxAGC_A_Nss2Index1_Nss1Index8_JAguar, bMaskByte0,
				     PowerIndex);
			break;
		case MGN_VHT1SS_MCS9:
			PHY_SetBBReg(Adapter, rTxAGC_A_Nss2Index1_Nss1Index8_JAguar, bMaskByte1,
				     PowerIndex);
			break;
		case MGN_VHT2SS_MCS0:
			PHY_SetBBReg(Adapter, rTxAGC_A_Nss2Index1_Nss1Index8_JAguar, bMaskByte2,
				     PowerIndex);
			break;
		case MGN_VHT2SS_MCS1:
			PHY_SetBBReg(Adapter, rTxAGC_A_Nss2Index1_Nss1Index8_JAguar, bMaskByte3,
				     PowerIndex);
			break;

		case MGN_VHT2SS_MCS2:
			PHY_SetBBReg(Adapter, rTxAGC_A_Nss2Index5_Nss2Index2_JAguar, bMaskByte0,
				     PowerIndex);
			break;
		case MGN_VHT2SS_MCS3:
			PHY_SetBBReg(Adapter, rTxAGC_A_Nss2Index5_Nss2Index2_JAguar, bMaskByte1,
				     PowerIndex);
			break;
		case MGN_VHT2SS_MCS4:
			PHY_SetBBReg(Adapter, rTxAGC_A_Nss2Index5_Nss2Index2_JAguar, bMaskByte2,
				     PowerIndex);
			break;
		case MGN_VHT2SS_MCS5:
			PHY_SetBBReg(Adapter, rTxAGC_A_Nss2Index5_Nss2Index2_JAguar, bMaskByte3,
				     PowerIndex);
			break;

		case MGN_VHT2SS_MCS6:
			PHY_SetBBReg(Adapter, rTxAGC_A_Nss2Index9_Nss2Index6_JAguar, bMaskByte0,
				     PowerIndex);
			break;
		case MGN_VHT2SS_MCS7:
			PHY_SetBBReg(Adapter, rTxAGC_A_Nss2Index9_Nss2Index6_JAguar, bMaskByte1,
				     PowerIndex);
			break;
		case MGN_VHT2SS_MCS8:
			PHY_SetBBReg(Adapter, rTxAGC_A_Nss2Index9_Nss2Index6_JAguar, bMaskByte2,
				     PowerIndex);
			break;
		case MGN_VHT2SS_MCS9:
			PHY_SetBBReg(Adapter, rTxAGC_A_Nss2Index9_Nss2Index6_JAguar, bMaskByte3,
				     PowerIndex);
			break;
		...
		case MGN_6M:
			PHY_SetBBReg(Adapter, rTxAGC_B_Ofdm18_Ofdm6_JAguar, bMaskByte0,
				     PowerIndex);
			break;
		case MGN_9M:
			PHY_SetBBReg(Adapter, rTxAGC_B_Ofdm18_Ofdm6_JAguar, bMaskByte1,
				     PowerIndex);
			break;
		case MGN_12M:
			PHY_SetBBReg(Adapter, rTxAGC_B_Ofdm18_Ofdm6_JAguar, bMaskByte2,
				     PowerIndex);
			break;
		case MGN_18M:
			PHY_SetBBReg(Adapter, rTxAGC_B_Ofdm18_Ofdm6_JAguar, bMaskByte3,
				     PowerIndex);
			break;

		case MGN_24M:
			PHY_SetBBReg(Adapter, rTxAGC_B_Ofdm54_Ofdm24_JAguar, bMaskByte0,
				     PowerIndex);
			break;
		case MGN_36M:
			PHY_SetBBReg(Adapter, rTxAGC_B_Ofdm54_Ofdm24_JAguar, bMaskByte1,
				     PowerIndex);
			break;
		case MGN_48M:
			PHY_SetBBReg(Adapter, rTxAGC_B_Ofdm54_Ofdm24_JAguar, bMaskByte2,
				     PowerIndex);
			break;
		case MGN_54M:
			PHY_SetBBReg(Adapter, rTxAGC_B_Ofdm54_Ofdm24_JAguar, bMaskByte3,
				     PowerIndex);
			break;
		...
		case MGN_VHT1SS_MCS0:
			PHY_SetBBReg(Adapter, rTxAGC_B_Nss1Index3_Nss1Index0_JAguar, bMaskByte0,
				     PowerIndex);
			break;
		case MGN_VHT1SS_MCS1:
			PHY_SetBBReg(Adapter, rTxAGC_B_Nss1Index3_Nss1Index0_JAguar, bMaskByte1,
				     PowerIndex);
			break;
		case MGN_VHT1SS_MCS2:
			PHY_SetBBReg(Adapter, rTxAGC_B_Nss1Index3_Nss1Index0_JAguar, bMaskByte2,
				     PowerIndex);
			break;
		case MGN_VHT1SS_MCS3:
			PHY_SetBBReg(Adapter, rTxAGC_B_Nss1Index3_Nss1Index0_JAguar, bMaskByte3,
				     PowerIndex);
			break;

		case MGN_VHT1SS_MCS4:
			PHY_SetBBReg(Adapter, rTxAGC_B_Nss1Index7_Nss1Index4_JAguar, bMaskByte0,
				     PowerIndex);
			break;
		case MGN_VHT1SS_MCS5:
			PHY_SetBBReg(Adapter, rTxAGC_B_Nss1Index7_Nss1Index4_JAguar, bMaskByte1,
				     PowerIndex);
			break;
		case MGN_VHT1SS_MCS6:
			PHY_SetBBReg(Adapter, rTxAGC_B_Nss1Index7_Nss1Index4_JAguar, bMaskByte2,
				     PowerIndex);
			break;
		case MGN_VHT1SS_MCS7:
			PHY_SetBBReg(Adapter, rTxAGC_B_Nss1Index7_Nss1Index4_JAguar, bMaskByte3,
				     PowerIndex);
			break;

		case MGN_VHT1SS_MCS8:
			PHY_SetBBReg(Adapter, rTxAGC_B_Nss2Index1_Nss1Index8_JAguar, bMaskByte0,
				     PowerIndex);
			break;
		case MGN_VHT1SS_MCS9:
			PHY_SetBBReg(Adapter, rTxAGC_B_Nss2Index1_Nss1Index8_JAguar, bMaskByte1,
				     PowerIndex);
			break;
		case MGN_VHT2SS_MCS0:
			PHY_SetBBReg(Adapter, rTxAGC_B_Nss2Index1_Nss1Index8_JAguar, bMaskByte2,
				     PowerIndex);
			break;
		case MGN_VHT2SS_MCS1:
			PHY_SetBBReg(Adapter, rTxAGC_B_Nss2Index1_Nss1Index8_JAguar, bMaskByte3,
				     PowerIndex);
			break;

		case MGN_VHT2SS_MCS2:
			PHY_SetBBReg(Adapter, rTxAGC_B_Nss2Index5_Nss2Index2_JAguar, bMaskByte0,
				     PowerIndex);
			break;
		case MGN_VHT2SS_MCS3:
			PHY_SetBBReg(Adapter, rTxAGC_B_Nss2Index5_Nss2Index2_JAguar, bMaskByte1,
				     PowerIndex);
			break;
		case MGN_VHT2SS_MCS4:
			PHY_SetBBReg(Adapter, rTxAGC_B_Nss2Index5_Nss2Index2_JAguar, bMaskByte2,
				     PowerIndex);
			break;
		case MGN_VHT2SS_MCS5:
			PHY_SetBBReg(Adapter, rTxAGC_B_Nss2Index5_Nss2Index2_JAguar, bMaskByte3,
				     PowerIndex);
			break;

		case MGN_VHT2SS_MCS6:
			PHY_SetBBReg(Adapter, rTxAGC_B_Nss2Index9_Nss2Index6_JAguar, bMaskByte0,
				     PowerIndex);
			break;
		case MGN_VHT2SS_MCS7:
			PHY_SetBBReg(Adapter, rTxAGC_B_Nss2Index9_Nss2Index6_JAguar, bMaskByte1,
				     PowerIndex);
			break;
		case MGN_VHT2SS_MCS8:
			PHY_SetBBReg(Adapter, rTxAGC_B_Nss2Index9_Nss2Index6_JAguar, bMaskByte2,
				     PowerIndex);
			break;
		case MGN_VHT2SS_MCS9:
			PHY_SetBBReg(Adapter, rTxAGC_B_Nss2Index9_Nss2Index6_JAguar, bMaskByte3,
				     PowerIndex);
			break;
...
)
{
	...
	for (i = 0; i < RateArraySize; ++i) {
		powerIndex = PHY_GetTxPowerIndex_8812A(pAdapter, RFPath, Rates[i],
						       BandWidth, Channel);
...
VOID PHY_GetTxPowerIndexByRateArray_8812A(
	IN	PADAPTER			pAdapter,
	IN	u8  					RFPath,
	IN	CHANNEL_WIDTH	BandWidth,
	IN	u8					Channel,
...
)
{
	...
	for (i = 0 ; i < ArraySize; i++) {
		PowerIndex[i] = (u8)PHY_GetTxPowerIndex_8812A(pAdapter, RFPath, Rate[i],
				BandWidth, Channel);
		if ((PowerIndex[i] % 2 == 1) && IS_HARDWARE_TYPE_JAGUAR(pAdapter)
		    && ! IS_NORMAL_CHIP(pHalData->VersionID))
			PowerIndex[i] -= 1;
	}

}

VOID phy_TxPowerTrainingByPath_8812(
	IN	PADAPTER			Adapter,
	IN	CHANNEL_WIDTH		BandWidth,
	IN	u8					Channel,
	IN	u8					RfPath
)
{
	...
	if (RfPath == ODM_RF_PATH_A) {
		PowerLevel = PHY_GetTxPowerIndex_8812A(Adapter, ODM_RF_PATH_A, MGN_MCS7,
						       BandWidth, Channel);
		writeOffset =  rA_TxPwrTraing_Jaguar;
	} else {
		PowerLevel = PHY_GetTxPowerIndex_8812A(Adapter, ODM_RF_PATH_B, MGN_MCS7,
						       BandWidth, Channel);
...
VOID PHY_SetTxPowerLevelByPath8812(
	IN	PADAPTER		Adapter,
	IN	u8				channel,
	IN	u8				path
)
{
	...
	//if(pMgntInfo->RegNByteAccess == 0)
	{
		if (pHalData->CurrentBandType == BAND_ON_2_4G)
			phy_SetTxPowerIndexByRateArray(Adapter, path, pHalData->CurrentChannelBW,
						       channel,
	...
	phy_TxPowerTrainingByPath_8812(Adapter, pHalData->CurrentChannelBW,
				       channel, path);
...
BOOLEAN PHY_UpdateTxPowerDbm8812(
	IN	PADAPTER	Adapter,
	IN	int		powerInDbm
)
{
...
s32 PHY_SwitchWirelessBand8812(
	IN PADAPTER		Adapter,
	IN u8			Band
)
{
	...
	if (Band == BAND_ON_2_4G) {
		...
		// STOP Tx/Rx
		PHY_SetBBReg(Adapter, rOFDMCCKEN_Jaguar, bOFDMEN_Jaguar | bCCKEN_Jaguar,
			     0x00);

		if (IS_HARDWARE_TYPE_8821(Adapter)) {
			// Turn off RF PA and LNA
			PHY_SetBBReg(Adapter, rA_RFE_Pinmux_Jaguar, 0xF000,
				     0x7); // 0xCB0[15:12] = 0x7 (LNA_On)
			PHY_SetBBReg(Adapter, rA_RFE_Pinmux_Jaguar, 0xF0,
				     0x7); // 0xCB0[7:4] = 0x7 (PAPE_A)
		}
		...
		if (IS_HARDWARE_TYPE_8821(Adapter)) {
			PHY_SetBBReg(Adapter, rA_RFE_Pinmux_Jaguar, 0xF000,
				     0x5); // 0xCB0[15:12] = 0x5 (LNA_On)
			PHY_SetBBReg(Adapter, rA_RFE_Pinmux_Jaguar, 0xF0,
				     0x4); // 0xCB0[7:4] = 0x4 (PAPE_A)
		}
	...
	if (IS_NORMAL_CHIP(pHalData->VersionID)
		...
		PHY_SetBBReg(Adapter, rA_TxScale_Jaguar, 0xFFE00000,
			     PHY_GetTxBBSwing_8812A(Adapter, (BAND_TYPE)Band,
						    ODM_RF_PATH_A)); // 0xC1C[31:21]
		PHY_SetBBReg(Adapter, rB_TxScale_Jaguar, 0xFFE00000,
			     PHY_GetTxBBSwing_8812A(Adapter, (BAND_TYPE)Band,
						    ODM_RF_PATH_B)); // 0xE1C[31:21]

		// <20121005, Kordan> When TxPowerTrack is ON, we should take care of the change of BB swing.
		// That is, reset all info to trigger Tx power tracking.
		{
			if (Band != currentBand) {
				BBDiffBetweenBand = (pRFCalibrateInfo->BBSwingDiff2G -
						     pRFCalibrateInfo->BBSwingDiff5G);
				BBDiffBetweenBand = (Band == BAND_ON_2_4G) ? BBDiffBetweenBand :
						    (-1 * BBDiffBetweenBand);
...
)
{
	...
	case CHANNEL_WIDTH_20:
		rtw_write16(Adapter, REG_WMAC_TRXPTCL_CTL,
...
)
{
	...
	if (IS_VENDOR_8812A_C_CUT(pAdapter)) {
		if (Bandwidth == CHANNEL_WIDTH_40 && Channel == 11)
			PHY_SetBBReg(pAdapter, rRFMOD_Jaguar, 0xC00,
...
)
{
	...
	case CHANNEL_WIDTH_20:
		PHY_SetBBReg(Adapter, rRFMOD_Jaguar, 0x003003C3,
			     0x00300200); // 0x8ac[21,20,9:6,1,0]=8'b11100000
		PHY_SetBBReg(Adapter, rADC_Buf_Clk_Jaguar, BIT30, 0);			// 0x8c4[30] = 1'b0

		PHY_SetBBReg(Adapter, rFPGA0_XB_RFInterfaceOE, 0x001C0000,
			     4);	// 0x864[20:18] = 3'b4

		if (pHalData->rf_type == RF_2T2R)
			PHY_SetBBReg(Adapter, rL1PeakTH_Jaguar, 0x03C00000,
	...
	phy_FixSpur_8812A(Adapter, pHalData->CurrentChannelBW,
	...
	for (eRFPath = 0; eRFPath < pHalData->NumTotalRFPath; eRFPath++) {
		...
		if (IS_VENDOR_8812A_TEST_CHIP(pAdapter)) {
			if (1 <= channelToSW && channelToSW <= 7)
				PHY_SetRFReg(pAdapter, eRFPath, RF_TxLCTank_Jaguar, bLSSIWrite_data_Jaguar,
					     0x0017e);
		...
		if (36 <= channelToSW && channelToSW <= 64)
			PHY_SetRFReg(pAdapter, eRFPath, RF_CHNLBW_Jaguar,
				     BIT18 | BIT17 | BIT16 | BIT9 | BIT8, 0x101); //5'b00101);
		else if (100 <= channelToSW && channelToSW <= 140)
			PHY_SetRFReg(pAdapter, eRFPath, RF_CHNLBW_Jaguar,
				     BIT18 | BIT17 | BIT16 | BIT9 | BIT8, 0x301); //5'b01101);
		else if (140 < channelToSW)
			PHY_SetRFReg(pAdapter, eRFPath, RF_CHNLBW_Jaguar,
				     BIT18 | BIT17 | BIT16 | BIT9 | BIT8, 0x501); //5'b10101);
		else
			PHY_SetRFReg(pAdapter, eRFPath, RF_CHNLBW_Jaguar,
				     BIT18 | BIT17 | BIT16 | BIT9 | BIT8, 0x000); //5'b00000);
		...
		if (IS_HARDWARE_TYPE_8811AU(pAdapter)
		    && (!IS_NORMAL_CHIP(pHalData->VersionID)) && channelToSW > 14) {
...
)
{
	...
	PHY_HandleSwChnlAndSetBW8812(Adapter, _FALSE, _TRUE,
				     pHalData->CurrentChannel, Bandwidth, Offset, Offset,
				     pHalData->CurrentChannel);
...
)
{
	...
	PHY_HandleSwChnlAndSetBW8812(Adapter, _TRUE, _FALSE, channel, 0, 0, 0,
				     channel);

	//DBG_871X("<==%s()\n",__FUNCTION__);
}

VOID PHY_SetSwChnlBWMode8812(
	IN	PADAPTER			Adapter,
	IN	u8					channel,
	IN	CHANNEL_WIDTH		Bandwidth,
	IN	u8					Offset40,
...
)
{
	...
	PHY_HandleSwChnlAndSetBW8812(Adapter, _TRUE, _TRUE, channel, Bandwidth,
