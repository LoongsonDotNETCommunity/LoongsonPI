...
#define BT_INFO_LENGTH 8
	...
	DBG_871X("%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x\n"
		 , pcmd[4], pcmd[5], pcmd[6], pcmd[7], pcmd[8], pcmd[9], pcmd[10],
		 pcmd[11]);
...
void rtw_btcoex_recvmsgbynetlink(struct sk_buff *skb)
{
	...
	if (RX_INVITE_REQ == parse_res) {
		...
		if (_SUCCESS == res) {
			...
			res = rtw_btcoex_sendmsgbysocket(pbtcoexadapter, rsp_msg,
							 sizeof(rsp_msg)); //inform BT to close netlink socket
			if (res >= 0) {
		...
		res = sendmsgbynetlink(pbtcoexadapter, ack_msg,
				       sizeof(ack_msg));// Tx leave_ack;
...
s8 rtw_btcoex_sendmsgbynetlink(_adapter *padapter, u8 *msg, u8 msg_size)
{
	...
	DBG_871X("===> send\n");
	if (pcoex_info->nl_sk) {
		res = netlink_unicast(pcoex_info->nl_sk, skb_out, pcoex_info->pid,
				      MSG_WAITALL);
		msleep(100);
	}
	DBG_871X("send <===\n");
	...
	if (_TRUE == pcoex_info->BT_attend) {
		...
		while (len > 0) {
			...
			if (RX_ATTEND_ACK == parse_res) { //attend ack
				...
				rtw_btcoex_sendmsgbysocket(pbtcoexadapter, tx_msg,
							   sizeof(tx_msg)); // no ack
				pcoex_info ->BT_attend = _FALSE;
...
void rtw_btcoex_create_nl_socket(_adapter *padapter)
{
	...
	pcoex_info->nl_sk = netlink_kernel_create(&init_net, NETLINK_USER, 0,
			    rtw_btcoex_recvmsgbynetlink, NULL, THIS_MODULE);
#elif(LINUX_VERSION_CODE == KERNEL_VERSION(3,6,0))
	pcoex_info->nl_sk = netlink_kernel_create(&init_net, NETLINK_USER,
			    THIS_MODULE, &nl_cfg);
...
u8 rtw_btcoex_create_kernel_socket(_adapter *padapter, u8 is_invite)
{
	...
	if (kernel_socket_err < 0) {
		...
		kernel_socket_err = pcoex_info->udpsock->ops->bind(pcoex_info->udpsock,
				    (struct sockaddr *)&pcoex_info->sin, sizeof(pcoex_info->sin));
