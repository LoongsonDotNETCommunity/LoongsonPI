...
u32	beamforming_get_report_frame(PADAPTER	 Adapter,
...
void	beamforming_get_ndpa_frame(PADAPTER	 Adapter,
				   union recv_frame *precv_frame)
{
	u8	*ta;
	u8	idx, Sequence;
	...
	DBG_871X("%s LogSeq %d LogRetryCnt %d LogSuccessCnt %d\n",
		 __FUNCTION__, pBeamformEntry->LogSeq, pBeamformEntry->LogRetryCnt,
		 pBeamformEntry->LogSuccessCnt);
}

BOOLEAN	issue_ht_ndpa_packet(PADAPTER Adapter, u8 *ra, CHANNEL_WIDTH bw,
			     u8 qidx)
{
			      ...
			      CHANNEL_WIDTH bw, u8 qidx)
{
	...
	*(fctrl) = 0;
	...
	if (IsSupported5G(pmlmeext->cur_wireless_mode)
	    || IsSupportedHT(pmlmeext->cur_wireless_mode))
		aSifsTime = 16;
	else
		aSifsTime = 10;
...
CHANNEL_WIDTH	beamforming_sounding_bw(struct beamforming_info *pBeamInfo,
					SOUNDING_MODE mode, u8 idx)
{
