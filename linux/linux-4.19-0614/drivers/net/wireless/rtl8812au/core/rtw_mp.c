...
)
{
	...
	_init_timer(&pMptCtx->MPh2c_timeout_timer, pAdapter->pnetdev,
	...
	PHY_SetRFPathSwitch(pAdapter,
			    1/*pHalData->bDefaultAntenna*/);	//Wifi default use Main
#else
	...
	pMptCtx->backup0xc50 = (u1Byte)PHY_QueryBBReg(pAdapter, rOFDM0_XAAGCCore1,
			       bMaskByte0);
	pMptCtx->backup0xc58 = (u1Byte)PHY_QueryBBReg(pAdapter, rOFDM0_XBAGCCore1,
			       bMaskByte0);
	pMptCtx->backup0xc30 = (u1Byte)PHY_QueryBBReg(pAdapter, rOFDM0_RxDetector1,
			       bMaskByte0);
#ifdef CONFIG_RTL8188E
	pMptCtx->backup0x52_RF_A = (u1Byte)PHY_QueryRFReg(pAdapter, RF_PATH_A,
				   RF_0x52, 0x000F0);
	pMptCtx->backup0x52_RF_B = (u1Byte)PHY_QueryRFReg(pAdapter, RF_PATH_A,
				   RF_0x52, 0x000F0);
#endif
		...
		u8 Channel, u8 BandWidthID)
{
	...
	if (RateIdx < MPT_RATE_6M) {	// CCK rate,for 88cu
		...
		if ((4 == Channel) || (8 == Channel) || (12 == Channel))
			...
			rfReg0x26 = 0x4f200;
	} else if ((RateIdx >= MPT_RATE_MCS0)
		   && (RateIdx <= MPT_RATE_MCS15)) { // MCS 20M ,for 88cu // MCS40M rate,for 88cu
...
//ndef CONFIG_RTL8723A
		pHalData->ReceiveConfig = AAP | APM | AM | AB | APP_ICV | ADF | AMF |
					  HTC_LOC_CTRL | APP_MIC | APP_PHYSTS;
...
void _rtw_mp_xmit_priv(struct xmit_priv *pxmitpriv)
{
	...
	for (i = 0; i < num_xmit_extbuf; i++) {
		...
		if ((res = rtw_os_xmit_resource_alloc(padapter, pxmitbuf,
						      max_xmit_extbuf_size + XMITBUF_ALIGN_SZ, _FALSE)) == _FAIL) {
			res = _FAIL;
			goto exit;
		}
...
)
{
...

	{
		...
		case MPT_RATE_6M:	//0xe00 [31:0] = 18M,12M,09M,06M
			TxPower += ((pHalData->MCSTxPowerLevelOriginalOffset[PwrGroup][0 +
					Pathmapping]) & 0xff);
		...
		case MPT_RATE_24M:	//0xe04[31:0] = 54M,48M,36M,24M
			TxPower += ((pHalData->MCSTxPowerLevelOriginalOffset[PwrGroup][1 +
					Pathmapping]) & 0xff);
		...
		case MPT_RATE_MCS0: //0xe10[31:0]= MCS=03,02,01,00
			TxPower += ((pHalData->MCSTxPowerLevelOriginalOffset[PwrGroup][2 +
					Pathmapping]) & 0xff);
		...
		case MPT_RATE_MCS4: //0xe14[31:0]= MCS=07,06,05,04
			TxPower += ((pHalData->MCSTxPowerLevelOriginalOffset[PwrGroup][3 +
					Pathmapping]) & 0xff);
		...
		case MPT_RATE_MCS8: //0xe18[31:0]= MCS=11,10,09,08
			TxPower += ((pHalData->MCSTxPowerLevelOriginalOffset[PwrGroup][4 +
					Pathmapping]) & 0xff);
		...
		case MPT_RATE_MCS12:	//0xe1c[31:0]= MCS=15,14,13,12
			TxPower += ((pHalData->MCSTxPowerLevelOriginalOffset[PwrGroup][5 +
					Pathmapping]) & 0xff);
...
static	ULONG mpt_ProQueryCalTxPower_8188E(
	IN	PADAPTER		pAdapter,
	IN	u1Byte			RfPath
)
{
	...
	ULONG				TxPowerCCK = 1, TxPowerOFDM = 1, TxPowerBW20 = 1,
					TxPowerBW40 = 1 ;
	PMPT_CONTEXT		pMptCtx = &(pAdapter->mppriv.MptCtx);
...
)
{
	...
	if (IS_HARDWARE_TYPE_8188E(pAdapter)
	    || IS_HARDWARE_TYPE_8192E(
		    pAdapter)) { //|| IS_HARDWARE_TYPE_8723B(pAdapter))
...
void Hal_ProSetCrystalCap(PADAPTER pAdapter, u32 CrystalCapVal)
{
	...
	if (IS_HARDWARE_TYPE_8192D(pAdapter)) {
		...
		PHY_SetBBReg(pAdapter, REG_AFE_PLL_CTRL, 0xF0000000,
			     (CrystalCapVal & 0xF0) >> 4);
		...
		PHY_SetBBReg(pAdapter, REG_MAC_PHY_CTRL, 0x7FF80000,
			     (CrystalCapVal | (CrystalCapVal << 6)));
	} else if (IS_HARDWARE_TYPE_8821(pAdapter)
		   || IS_HARDWARE_TYPE_8192E(pAdapter)) {
		// write 0x2C[23:18] = 0x2C[17:12] = CrystalCap
		PHY_SetBBReg(pAdapter, REG_MAC_PHY_CTRL, 0xFFF000,
			     (CrystalCapVal | (CrystalCapVal << 6)));
	} else {
		PHY_SetBBReg(pAdapter, 0x2c, 0xFFF000,
			     (CrystalCapVal | (CrystalCapVal << 6)));
	}
}
