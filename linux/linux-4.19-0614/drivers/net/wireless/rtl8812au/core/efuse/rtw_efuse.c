			...
			IN	BOOLEAN		bPseudoTest)
{
	...
	ret =  pAdapter->HalFunc.Efuse_PgPacketWrite(pAdapter, offset, word_en,
			data, bPseudoTest);
			   ...
			   IN	BOOLEAN		bPseudoTest)
{
	...
	ret =  pAdapter->HalFunc.Efuse_PgPacketWrite_BT(pAdapter, offset, word_en,
			data, bPseudoTest);
	...
	IN BOOLEAN	bPseudoTest)
{
	...
	if (pEEPROM->bautoload_fail_flag == _TRUE) {
		...
		if (_SUCCESS != retriveAdaptorInfoFile(
			    pAdapter->registrypriv.adaptor_info_caching_file_path, pEEPROM)) {
#endif
...
int retriveAdaptorInfoFile(char *path, struct eeprom_priv *eeprom_priv)
{
	...
	if (path && eeprom_priv) {
		...
		ret = rtw_retrive_from_file(path, eeprom_priv->efuse_eeprom_data,
					    EEPROM_MAX_SIZE);
