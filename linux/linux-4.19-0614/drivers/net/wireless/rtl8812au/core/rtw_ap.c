		...
		if (psta->expire_to > 0) {
			...
			if (psta->expire_to == 0) {
				...
				DBG_871X("auth expire %02X%02X%02X%02X%02X%02X\n",
					 psta->hwaddr[0], psta->hwaddr[1], psta->hwaddr[2], psta->hwaddr[3],
...
void add_RATid(_adapter *padapter, struct sta_info *psta, u8 rssi_level)
{
	...
	if (get_rf_mimo_mode(padapter) == RTL8712_RF_2T2R) {
		//is this a 2r STA?
		if ((pstat->tx_ra_bitmap & 0x0ff00000) != 0
			...
			if ((priv->pmib->dot11BssType.net_work_type & WIRELESS_11N)
			    && pstat->ht_cap_len && priv->pshare->has_2r_sta == 0) {
	...
	if ((!(pstat->tx_ra_bitmap & 0x8000000) && (priv->pshare->has_2r_sta > 0)
	     && (get_rf_mimo_mode(padapter) == RTL8712_RF_2T2R)) ||
...
static void update_bmc_sta(_adapter *padapter)
{
	...
	if (psta) {
		...
		_rtw_memcpy(psta->bssrateset, &pcur_network->SupportedRates,
			    supportRateNum);
...
void update_sta_info_apmode(_adapter *padapter, struct sta_info *psta)
{
	...
	if (phtpriv_sta->ht_option) {
		...
		if ((phtpriv_sta->ht_cap.cap_info & phtpriv_ap->ht_cap.cap_info) &
		    cpu_to_le16(IEEE80211_HT_CAP_SGI_20 | IEEE80211_HT_CAP_SGI_40)) {
...
static void start_bss_network(_adapter *padapter, u8 *pbuf)
{
	...
	} else if (check_buddy_fwstate(padapter,
				       _FW_LINKED) == _TRUE) { //only second adapter can enter AP Mode
		_adapter *pbuddy_adapter = padapter->pbuddy_adapter;
...
#ifdef CONFIG_P2P
	_rtw_memcpy(pwdinfo->p2p_group_ssid, pnetwork->Ssid.Ssid,
...
int rtw_check_beacon_data(_adapter *padapter, u8 *pbuf,  int len)
{
	...
	_rtw_memcpy(pbss_network->MacAddress, myid(&(padapter->eeprompriv)),
		    ETH_ALEN);
	...
	if (p && ie_len > 0) {
		...
		pht_cap->ampdu_params_info |= (IEEE80211_HT_CAP_AMPDU_FACTOR &
					       0x03); //set  Max Rx AMPDU size  to 64K
...
//#ifdef CONFIG_80211AC_VHT
	...
	if ((pbss_network->Configuration.DSConfig > 14) &&
		...
		cap_len = rtw_build_vht_cap_ie(padapter,
					       pbss_network->IEs + pbss_network->IELength);
	       ...
	       u16 reason)
{
	...
	rtw_clearstakey_cmd(padapter, (u8 *)psta, (u8)rtw_get_camid(psta->mac_id),
			    _TRUE);
