...
unsigned int OnAssocReq(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if (elems.vht_capabilities && elems.vht_capabilities_len == 12) {
		...
		if (elems.vht_op_mode_notify && elems.vht_op_mode_notify_len == 1) {
			pstat->vhtpriv.bwmode = GET_VHT_OPERATING_MODE_FIELD_CHNL_WIDTH(
							elems.vht_op_mode_notify);
		}
	} else {
		pstat->flags &= ~WLAN_STA_HT;
	}
...
void issue_p2p_GO_confirm(_adapter *padapter, u8 *raddr, u8 result)
{
	...
	*(u16 *)(p2pie + p2pielen) = cpu_to_le16(pwdinfo->channel_list_attr_len);
	...
	//	Value:
	_rtw_memcpy(p2pie + p2pielen, pwdinfo->channel_list_attr,
...
void issue_probersp_p2p(_adapter *padapter, unsigned char *da)
{
	...
	u8 listen_channel = (u8) ieee80211_frequency_to_channel(
				    ieee_ch->center_freq);
#endif //CONFIG_IOCTL_CFG80211
	...
	*(fctrl) = 0;
	...
	if (wdev_to_priv(padapter->rtw_wdev)->p2p_enabled && listen_channel != 0
		...
		pframe = rtw_set_ie(pframe, _DSSET_IE_, 1,
				    (unsigned char *)&listen_channel, &pattrib->pktlen);
	} else
#endif //CONFIG_IOCTL_CFG80211
	{
		pframe = rtw_set_ie(pframe, _DSSET_IE_, 1,
...
#endif //CONFIG_IOCTL_CFG80211
	{
		...
		case P2P_INVIT_REQ: {
			...
			if ((p2p_ie = rtw_get_p2p_ie(frame_body + _PUBLIC_ACTION_IE_OFFSET_,
				...
				rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_INVITATION_FLAGS,
							 &invitation_flag, &attr_contentlen);
				if (attr_contentlen) {

					rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_GROUP_BSSID,
	...
	if (padapter->mlmepriv.htpriv.ht_option == _TRUE) {
		...
		if ((p != NULL) && (!(is_ap_in_tkip(padapter)))) {
			...
			if (BT_1Ant(padapter) == _TRUE) {
				// set to 8K
				pmlmeinfo->HT_caps.u.HT_cap_element.AMPDU_para &= (u8)
		    ...
		    WLAN_BSSID_EX *bssid)
{
	...
	bssid->Rssi =
		precv_frame->u.hdr.attrib.phy_info.RecvSignalPower; // in dBM.raw data
	bssid->PhyInfo.SignalQuality =
		precv_frame->u.hdr.attrib.phy_info.SignalQuality;//in percentage
...
static void process_80211d(PADAPTER padapter, WLAN_BSSID_EX *bssid)
{
	...
	if (pregistrypriv->enable80211d &&
		...
		while ((i < MAX_CHANNEL_NUM) && (chplan_sta[i].ChannelNum != 0)) {
			DBG_871X("%02d(%c),", chplan_sta[i].ChannelNum,
...
void mlmeext_joinbss_event_callback(_adapter *padapter, int join_res)
{
	...
	if (psta) { //only for infra. mode
		...
		rtw_hal_set_hwreg(padapter, HW_VAR_H2C_MEDIA_STATUS_RPT,
				  (u8 *)&media_status);
	}
			 ...
			 u32 out_num, struct rtw_ieee80211_channel *in, u32 in_num)
{
	...
	if (padapter->setband == GHZ_24) {				// 2.4G
		...
		for (i = 0; i < j ; i++) {
			if (out[i].hw_value > 35) {
				_rtw_memcpy(&out[scan_ch_num++], &out[i],
...
u8 set_stakey_hdl(_adapter *padapter, u8 *pbuf)
{
	...
	if ((pmlmeinfo->state & 0x03) == WIFI_FW_AP_STATE) {
		...
		if (psta) {
			...
			cam_id = (u8)rtw_get_camid(
					 psta->mac_id);//0~3 for default key, cmd_id=macid + 3;
...
int concurrent_chk_start_clnt_join(_adapter *padapter)
{
	...
	if ((pbuddy_pmlmeinfo->state & 0x03) == WIFI_FW_AP_STATE) { //for AP MODE
		...
		if (!rtw_p2p_chk_state(pbuddy_wdinfo, P2P_STATE_NONE)
		    && pbuddy_wdinfo->driver_interface == DRIVER_CFG80211) {
		...
		if ((pmlmeext->cur_bwmode == CHANNEL_WIDTH_40) &&
			...
			DBG_871X("start_clnt_join(bwmode=%d, ch_offset=%d), but bwmode & ch_offset mismatch with buddy(bwmode=%d, ch_offset=%d) interface\n",
				 pmlmeext->cur_bwmode, pmlmeext->cur_ch_offset, pbuddy_mlmeext->cur_bwmode,
				 pbuddy_mlmeext->cur_ch_offset);
