...
int const rtw_usb_id_len = sizeof(rtw_usb_id_tbl) / sizeof(
...
static inline int RT_usb_endpoint_dir_in(const struct
		usb_endpoint_descriptor *epd)
{
	return ((epd->bEndpointAddress & USB_ENDPOINT_DIR_MASK) == USB_DIR_IN);
}

static inline int RT_usb_endpoint_dir_out(const struct
		usb_endpoint_descriptor *epd)
{
...
static u8 rtw_init_intf_priv(struct dvobj_priv *dvobj)
{
	...
	dvobj->usb_vendor_req_buf  =
		(u8 *)N_BYTE_ALIGMENT((SIZE_PTR)(dvobj->usb_alloc_vendor_req_buf),
				      ALIGNMENT_UNIT);
exit:
#endif
...
static struct dvobj_priv *usb_dvobj_init(struct usb_interface *usb_intf)
{
	...
	if ((pdvobjpriv = (struct dvobj_priv *)rtw_zmalloc(sizeof(
				  *pdvobjpriv))) == NULL) {
		goto exit;
	}
	...
	for (i = 0; i < pdvobjpriv->nr_endpoint; i++) {
		...
		if (phost_endp) {
			...
			if (RT_usb_endpoint_is_bulk_in(pendp_desc)) {
				...
				pdvobjpriv->RtInPipe[pdvobjpriv->RtNumInPipes] = RT_usb_endpoint_num(
							pendp_desc);
				...
				DBG_871X("RT_usb_endpoint_is_bulk_out = %x\n",
					 RT_usb_endpoint_num(pendp_desc));
				pdvobjpriv->RtOutPipe[pdvobjpriv->RtNumOutPipes] = RT_usb_endpoint_num(
							pendp_desc);
...
static void rtw_decide_chip_type_by_usb_info(_adapter *padapter,
		const struct usb_device_id *pdid)
{
...
static int rtw_suspend(struct usb_interface *pusb_intf,
		       pm_message_t message)
{
	...
	if (pwrpriv->bInternalAutoSuspend) {
		...
		if ((padapter->bFWReady) && (padapter->pwrctrlpriv.bHWPwrPindetect)
...
int rtw_resume_process(_adapter *padapter)
{
	...
	DBG_871X("%s...pm_usage_cnt(%d)  pwrpriv->bAutoResume=%x.  ....\n",
		 __func__, atomic_read(&(adapter_to_dvobj(
				 padapter)->pusbintf->pm_usage_cnt)), pwrpriv->bAutoResume);
	pm_cnt = atomic_read(&(adapter_to_dvobj(
				       padapter)->pusbintf->pm_usage_cnt));
#else
			   ...
			   struct usb_interface *pusb_intf, const struct usb_device_id *pdid)
{
	...
	if (padapter->registrypriv.power_mgnt != PS_MODE_ACTIVE) {
		if (padapter->registrypriv.usbss_enable) { 	/* autosuspend (2s delay) */
			...
			dvobj->pusbdev->dev.power.autosuspend_delay = 0 *
					HZ;//15 * HZ; idle-delay time
#else
...
static int __init rtw_drv_entry(void)
{
	...
	tmp &= 0xffffff00;
	tmp |= 0x55;
	writel(tmp, (volatile unsigned int *)
	       0xb801a608); //write dummy register for 1055
#endif
#ifdef CONFIG_PLATFORM_ARM_SUNxI
#ifndef CONFIG_RTL8723A
	int ret = 0;
	/* ----------get usb_wifi_usbc_num------------- */
	ret = script_parser_fetch("usb_wifi_para", "usb_wifi_usbc_num",
