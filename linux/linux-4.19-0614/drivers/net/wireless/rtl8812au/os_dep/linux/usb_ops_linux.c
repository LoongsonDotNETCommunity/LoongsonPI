...
int usb_write_async(struct usb_device *udev, u32 addr, void *pdata,
		    u16 len)
{
	u8 request;
	u8 requesttype;
	...
	ret = _usbctrl_vendorreq_async_write(udev, request, wvalue, index, pdata,
					     len, requesttype);
	...
	*/
	...
	if (padapter->bSurpriseRemoved || padapter->bDriverStopped
		...
		DBG_8192C("%s(): TX Warning! bDriverStopped(%d) OR bSurpriseRemoved(%d) bWritePortCancel(%d) pxmitbuf->buf_tag(%x) \n",
			  ...
			  padapter->bReadPortCancel, pxmitbuf->buf_tag);
