	...
	if (res == _SUCCESS) {
		...
		if ((padapter->pbuddy_adapter->mlmeextpriv.mlmext_info.state & 0x03) ==
			...
			_set_timer(&pmlmepriv->scan_to_timer,
				   SURVEY_TO * (padapter->mlmeextpriv.max_chan_nums +
						(padapter->mlmeextpriv.max_chan_nums  / RTW_SCAN_NUM_OF_CH) *
...
u8 rtw_createbss_cmd_ex(_adapter  *padapter, unsigned char *pbss,
			unsigned int sz)
{
	struct cmd_obj	*pcmd;
	struct cmd_priv 	*pcmdpriv = &padapter->cmdpriv;
...
u8 rtw_joinbss_cmd(_adapter  *padapter, struct wlan_network *pnetwork)
{
	...
	*/
	...
	if (pmlmepriv->assoc_by_bssid == _FALSE) {
		_rtw_memcpy(&pmlmepriv->assoc_bssid[ 0 ],
...
u8 rtw_set_ch_cmd(_adapter *padapter, u8 ch, u8 bw, u8 ch_offset,
		  u8 enqueue)
{
...
u8 rtw_set_chplan_cmd(_adapter *padapter, u8 chplan, u8 enqueue)
{
	...
	//prepare cmd parameter
	setChannelPlan_param = (struct	SetChannelPlan_param *)rtw_zmalloc(sizeof(
...
#define TRAFFIC_PROTECT_PERIOD_MS 4500
		...
		if (link_detect->NumTxOkInPeriod > TX_ACTIVE_TH
			...
			LOG_LEVEL(_drv_info_,
				  FUNC_ADPT_FMT" acqiure wake_lock for %u ms(tx:%d,rx_unicast:%d)\n",
				  FUNC_ADPT_ARG(padapter),
		...
		if ((ptdlsinfo->watchdog_count % TDLS_WATCHDOG_PERIOD) ==
		    0)	//TDLS_WATCHDOG_PERIOD * 2sec, periodically sending
...
{
	rtw_hal_set_hwreg(padapter, HW_VAR_ANTENNA_DIVERSITY_SELECT,
			  (u8 *)(&antenna));
}
...
void rtw_createbss_cmd_callback(_adapter *padapter, struct cmd_obj *pcmd)
{
	...
	if (check_fwstate(pmlmepriv, WIFI_AP_STATE)) {
		...
		_rtw_memcpy(&tgt_network->network, pnetwork,
