	...
	*/
	...
	for (i = 0; i < NR_XMITBUFF; i++) {
		...
		*/
		...
		if ((res = rtw_os_xmit_resource_alloc(padapter, pxmitbuf,
						      (MAX_XMITBUF_SZ + XMITBUF_ALIGN_SZ))) == _FAIL) {
			res = _FAIL;
			goto exit;
		}
	...
	for (i = 0; i < NR_XMIT_EXTBUFF; i++) {
		...
		*/
		...
		if ((res = rtw_os_xmit_resource_alloc(padapter, pxmitbuf,
						      MAX_XMIT_EXTBUF_SZ + XMITBUF_ALIGN_SZ)) == _FAIL) {
			res = _FAIL;
			goto exit;
		}
				   ...
				   struct xmit_frame *pxmitframe)
{
	...
	if (pmlmeext->cur_wireless_mode < WIRELESS_11_24N
		...
		while (_TRUE) {
			...
			//IOT action
			if ((pmlmeinfo->assoc_AP_vendor == atherosAP)
			    ...
			    struct xmit_frame *pxmitframe)
{
	...
	if (pattrib->encrypt ==
		...
		if (stainfo != NULL) {
			...
			pframe = pxmitframe->buf_addr + TXDESC_SIZE + (pxmitframe->pkt_offset *
					PACKET_OFFSET_SZ);
#else
			...
			if (bmcst) {
				...
				if (_rtw_memcmp(&stainfo->dot11tkiptxmickey.skey[0], null_key,
						16) == _TRUE) {
					//DbgPrint("\nxmitframe_addmic:stainfo->dot11tkiptxmickey==0\n");
					//rtw_msleep_os(10);
					return _FAIL;
...
#ifdef CONFIG_USB_TX_AGGREGATION
	mem_start = pbuf_start + TXDESC_SIZE + (pxmitframe->pkt_offset *
						PACKET_OFFSET_SZ);
#else
	mem_start = pbuf_start + TXDESC_OFFSET;
#endif
