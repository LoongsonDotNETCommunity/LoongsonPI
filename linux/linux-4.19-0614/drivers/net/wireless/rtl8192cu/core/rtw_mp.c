...
{
	return rtw_hal_read_rfreg(padapter, (RF_RADIO_PATH_E)rfpath, addr,
				  bitmask);
}
...
{
	return _read_rfreg(padapter, (RF_RADIO_PATH_E)rfpath, addr,
			   bRFRegOffsetMask);
}

void write_rfreg(PADAPTER padapter, u8 rfpath, u32 addr, u32 val)
{
	_write_rfreg(padapter, (RF_RADIO_PATH_E)rfpath, addr, bRFRegOffsetMask,
		     val);
}
...
static int init_mp_priv_by_os(struct mp_priv *pmp_priv)
{
	...
	pmp_priv->pmp_xmtframe_buf = pmp_priv->pallocated_mp_xmitframe_buf + 4 - ((
					     uint)(pmp_priv->pallocated_mp_xmitframe_buf) & 3);
...
#else
	if (pAdapter->HalFunc.GetInterfaceSelectionHandler(pAdapter) ==
	    INTF_SEL2_MINICARD)
		PHY_SetRFPathSwitch(Adapter,
				    pAdapter->MgntInfo.bDefaultAntenna); //default use Main
#endif
