		     ...
		     int *eof, void *data)
{
	...
	if (psta) {
		...
		len += snprintf(page + len, count - len,
				"can't get sta's macaddr, cur_network's macaddr:" MAC_FMT "\n",
		      ...
		      int *eof, void *data)
{
	...
	len += snprintf(page + len, count - len,
			"read_port_complete_EINPROGRESS_cnt=%d\n",
			precvpriv->read_port_complete_EINPROGRESS_cnt);
	len += snprintf(page + len, count - len,
			"read_port_complete_other_urb_err_cnt=%d\n",
			   ...
			   int *eof, void *data)
{
	...
	for (i = 0x0; i < 0x300; i += 4) {
		...
		len += snprintf(page + len, count - len, " 0x%08x ", rtw_read32(padapter,
				i));
		if ((j++) % 4 == 0)
			len += snprintf(page + len, count - len, "\n");
	}
			  ...
			  int *eof, void *data)
{
	...
	for (i = 0; pmlmeext->channel_set[i].ChannelNum != 0; i++) {
		...
		if (pmlmeext->channel_set[i].ChannelNum == 6) {
			if (pmlmeext->channel_set[i].rx_count <
			    pmlmeext->channel_set[index_24G].rx_count) {
				index_24G = i;
...
int proc_get_sreset(char *page, char **start, off_t offset, int count,
		    int *eof, void *data)
{
	struct net_device *dev = data;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
...
int proc_set_sreset(struct file *file, const char *buffer,
		    unsigned long count, void *data)
{
	struct net_device *dev = (struct net_device *)data;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
			   ...
			   unsigned long count, void *data)
{
	...
	if (buffer && !copy_from_user(tmp, buffer, sizeof(tmp))) {
		...
		int num = sscanf(tmp, "%x %hhd %x %d %hhu %hhu",
				 &TH_L2H_ini, &TH_EDCCA_HL_diff, &IGI_Base, &ForceEDCCA, &AdapEn_RSSI,
				 &IGI_LowerBound);

		if (num != 6)
			return count;

		dm_adaptivity_set_parm(padapter, (s8)TH_L2H_ini, TH_EDCCA_HL_diff,
				       (s8)IGI_Base, (bool)ForceEDCCA, AdapEn_RSSI, IGI_LowerBound);
	}
