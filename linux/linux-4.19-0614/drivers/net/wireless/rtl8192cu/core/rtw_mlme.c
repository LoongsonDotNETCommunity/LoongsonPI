...
struct	wlan_network *_rtw_dequeue_network(_queue *queue)
{
	...
	if (_rtw_queue_empty(queue) == _TRUE)
		...
		pnetwork = LIST_CONTAINOR(get_next(&queue->queue), struct wlan_network,
					  list);
...
int is_same_network(WLAN_BSSID_EX *src, WLAN_BSSID_EX *dst)
{
	...
	if (((uint)dst) <= 0x7fffffff ||
		...
		KeBugCheckEx(0x87110000, (ULONG_PTR)dst, (ULONG_PTR)src, (ULONG_PTR)&s_cap,
			     (ULONG_PTR)&d_cap);
				   ...
				   WLAN_BSSID_EX *pnetwork)
{
	...
	if ((unsigned long)(&(pmlmepriv->cur_network.network)) < 0x7ffffff) {
		KeBugCheckEx(0x87111c1c, (ULONG_PTR)(&(pmlmepriv->cur_network.network)), 0,
			     0, 0);
	}
#endif
...
void rtw_surveydone_event_callback(_adapter	*adapter, u8 *pbuf)
{
	...
	if (pmlmepriv->to_join == _TRUE) {
		if ((check_fwstate(pmlmepriv, WIFI_ADHOC_STATE) == _TRUE)) {
			...
			set_fwstate(pmlmepriv, _FW_UNDER_LINKING);
			pmlmepriv->to_join = _FALSE;
			if (_SUCCESS == (s_ret = rtw_select_and_join_from_scanned_queue(
							 pmlmepriv))) {
				_set_timer(&pmlmepriv->assoc_timer, MAX_JOIN_TIMEOUT);
	...
	if (pwlan) {
		...
		if (!rtw_p2p_chk_state(&adapter->wdinfo, P2P_STATE_NONE)) {
			...
			if ((p2p_ie = rtw_get_p2p_ie(pwlan->network.IEs + _FIXED_IE_LENGTH_,
						     pwlan->network.IELength - _FIXED_IE_LENGTH_, NULL, &p2p_ielen))) {
				pcap = rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_CAPABILITY,
								NULL, &capability_len);
				if (pcap && capability_len == 2) {
					u16 cap = *(u16 *)pcap ;
					*(u16 *)pcap = cap &
						       0x00ff; //clear group capability when free this network
				}
			}
...
void rtw_indicate_disconnect(_adapter *padapter)
{
	...
	if (check_fwstate(&padapter->mlmepriv, _FW_LINKED)
		...
		padapter->pwrctrlpriv.ips_deny_time = rtw_get_current_time() +
						      rtw_ms_to_systime(3000);
	...
	if (pnetwork->join_res > 0) {
		...
		if (check_fwstate(pmlmepriv, _FW_UNDER_LINKING)) {
			...
			if (check_fwstate(pmlmepriv, WIFI_STATION_STATE) == _TRUE) {
				...
				RT_TRACE(_module_rtl871x_mlme_c_, _drv_info_, ("adhoc mode, fw_state:%x",
						get_fwstate(pmlmepriv)));
			}
				    ...
				    , struct wlan_network **candidate, struct wlan_network *competitor)
{
	...
	if (pmlmepriv->assoc_ssid.Ssid && pmlmepriv->assoc_ssid.SsidLength) {
		if (competitor->network.Ssid.SsidLength != pmlmepriv->assoc_ssid.SsidLength
		    || _rtw_memcmp(competitor->network.Ssid.Ssid, pmlmepriv->assoc_ssid.Ssid,
				   pmlmepriv->assoc_ssid.SsidLength) == _FALSE
		   )
			goto exit;
	}
	...
	if (rtw_to_roaming(adapter) > 0) {
		if (rtw_get_passing_time_ms((u32)competitor->last_scanned) >=
		    ...
		    || is_same_ess(&competitor->network,
				   &pmlmepriv->cur_network.network) == _FALSE
		   )
			goto exit;
	}
#endif
	...
	if (pmlmepriv->assoc_by_bssid == _TRUE) { // associate with bssid
		if ((*candidate == NULL
		     || (*candidate)->network.Rssi < competitor->network.Rssi)
		    && _rtw_memcmp(competitor->network.MacAddress, pmlmepriv->assoc_bssid,
				   ETH_ALEN) == _TRUE
		   ) {
			*candidate = competitor;
			updated = _TRUE;
		}
	} else  if (pmlmepriv->assoc_ssid.SsidLength ==
		    0) {  // associate with ssid, but ssidlength is 0
		if ((*candidate == NULL
		     || (*candidate)->network.Rssi < competitor->network.Rssi)) {
			*candidate = competitor;
			updated = _TRUE;
		...
		if (rtw_to_roaming(adapter)) { // roaming
			if ((*candidate == NULL
			    ...
			    && rtw_get_passing_time_ms((u32)competitor->last_scanned) <
			    RTW_SCAN_RESULT_EXPIRE
...
#endif
		{
			...
			if ((*candidate == NULL
			     || (*candidate)->network.Rssi < competitor->network.Rssi)
			    && (competitor->network.Ssid.SsidLength ==
				pmlmepriv->assoc_ssid.SsidLength)
			    && ((_rtw_memcmp(competitor->network.Ssid.Ssid, pmlmepriv->assoc_ssid.Ssid,
					     pmlmepriv->assoc_ssid.SsidLength)) == _TRUE)
			    && rtw_is_desired_network(adapter, competitor)
			   ) {
				*candidate = competitor;
				updated = _TRUE;
...
int rtw_select_and_join_from_scanned_queue(struct mlme_priv *pmlmepriv)
{
	...
	while (!rtw_end_of_queue_search(phead, pmlmepriv->pscanned)) {
		...
		if (pnetwork == NULL) {
			RT_TRACE(_module_rtl871x_mlme_c_, _drv_err_,
				 ("(2)rtw_select_and_join_from_scanned_queue return _FAIL:(pnetwork==NULL)\n"));
			_exit_critical_bh(&(pmlmepriv->scanned_queue.lock), &irqL);
			return _FAIL;
		}
		...
		if (pmlmepriv->assoc_by_bssid == _TRUE) {
			if (_rtw_memcmp(pnetwork->network.MacAddress, pmlmepriv->assoc_bssid,
					ETH_ALEN) == _TRUE) {
			...
			if ((roaming_candidate == NULL
			     || roaming_candidate->network.Rssi < pnetwork->network.Rssi)
				...
				("roaming_candidate???: %s("MAC_FMT")\n",
				 ...
				 MAC_ARG(roaming_candidate->network.MacAddress))
				//)
				;
			}
			continue;
		...
		} else if ((pnetwork->network.Ssid.SsidLength ==
			    pmlmepriv->assoc_ssid.SsidLength)
			   && ((_rtw_memcmp(dst_ssid, src_ssid,
					    pmlmepriv->assoc_ssid.SsidLength)) == _TRUE)
			  ) {
			RT_TRACE(_module_rtl871x_mlme_c_, _drv_err_,
				 ("dst_ssid=%s, src_ssid=%s \n", dst_ssid, src_ssid));
#ifdef CONFIG_ANTENNA_DIVERSITY
			rtw_hal_get_def_var(adapter, HAL_DEF_CURRENT_ANTENNA, &(CurrentAntenna));
			...
			if (pmlmepriv->assoc_by_rssi ==
			    _TRUE) { //if the ssid is the same, select the bss which has the max rssi
				if (NULL == pnetwork_max_rssi
				    || pnetwork->network.Rssi > pnetwork_max_rssi->network.Rssi)
...
static int SecIsInPMKIDList(_adapter *Adapter, u8 *bssid)
{
	...
	do {
		if ((psecuritypriv->PMKIDList[i].bUsed) &&
		    (_rtw_memcmp(psecuritypriv->PMKIDList[i].Bssid, bssid,
				 ETH_ALEN) == _TRUE)) {
			break;
		} else {
			i++;
...
void _rtw_roaming(_adapter *padapter, struct wlan_network *tgt_network)
{
	...
	if (0 < rtw_to_roaming(padapter)) {
		DBG_871X("roaming from %s("MAC_FMT"), length:%d\n",
			 pnetwork->network.Ssid.Ssid, MAC_ARG(pnetwork->network.MacAddress),
			 pnetwork->network.Ssid.SsidLength);
		_rtw_memcpy(&pmlmepriv->assoc_ssid, &pnetwork->network.Ssid,
			    sizeof(NDIS_802_11_SSID));
