...
void dm_adaptivity_set_parm(_adapter *pAdapter, s8 TH_L2H_ini,
			    s8 TH_EDCCA_HL_diff,
...
void dm_adaptivity_init(_adapter *pAdapter)
{
	...
	PHY_SetBBReg(pAdapter, 0x524, BIT11,
...
void dm_adaptivity(_adapter *pAdapter)
{
	...
	LOG_LEVEL(_drv_info_,
		  "ForceEDCCA=%d, IGI_Base=0x%x, TH_L2H_ini = %d, TH_EDCCA_HL_diff = %d, AdapEn_RSSI = %d\n",
		  dmpriv->ForceEDCCA, dmpriv->IGI_Base, dmpriv->TH_L2H_ini,
		  dmpriv->TH_EDCCA_HL_diff, dmpriv->AdapEn_RSSI);

	/*if(pDM_Odm->SupportICType & ODM_IC_11AC_SERIES)
		PHY_SetBBReg(0x800, BIT10, 0); //ADC_mask enable
	*/

	if (!dm_linked(pAdapter)
	    || pHalData->CurrentChannel > 149) { /* Band4 doesn't need adaptivity */
		/*if(pDM_Odm->SupportICType & ODM_IC_11N_SERIES)*/
		{
	...
	LOG_LEVEL(_drv_info_, "BandWidth=%s, IGI_target=0x%x, EDCCA_State=%d\n",
		  (BandWidth == HT_CHANNEL_WIDTH_40) ? "40M" : "20M", IGI_target,
		  EDCCA_State);
