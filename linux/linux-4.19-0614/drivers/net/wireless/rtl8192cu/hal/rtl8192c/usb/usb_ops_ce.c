...
BOOL GetTransferStatus(
	...
	USB_TRANSFER   hTransfer,
	LPDWORD
	pBytesTransferred,  // OPTIONAL returns number of bytes transferred
...
)
{
	...
	if (pUsbFuncs->lpGetTransferStatus(hTransfer, pBytesTransferred,
					   pUsbError)) {
		if (USB_NO_ERROR != *pUsbError) {
			RT_TRACE(_module_hci_ops_os_c_, _drv_err_,
				 ("*** CloseTransfer ERROR:%d ***\n", GetLastError()));
			RT_TRACE(_module_hci_ops_os_c_, _drv_err_,
				 ("GetTransferStatus (BytesTransferred:%d, UsbError:0x%x)\n",
				  pBytesTransferred ? *pBytesTransferred : -1, pUsbError ? *pUsbError : -1));
		}
	} else {
		RT_TRACE(_module_hci_ops_os_c_, _drv_err_,
			 ("*** GetTransferStatus ERROR:%d ***\n", GetLastError()));
...
//	    }
		...
		RT_TRACE(_module_hci_ops_os_c_, _drv_err_,
			 ("usb_read_port:precv_frame ==NULL\n"));
	}
...
DWORD usb_read_port_complete(PVOID context)
{
	...
	(*usb_funcs_vp->lpGetTransferStatus)(precvbuf->usb_transfer_read_port,
					     &dwBytesTransferred, &dwErr);
	fComplete = (*usb_funcs_vp->lpIsTransferComplete)(
			    precvbuf->usb_transfer_read_port);
	if (fComplete != _TRUE) {
		RT_TRACE(_module_hci_ops_os_c_, _drv_err_,
			 ("usb_read_port_complete CloseTransfer before complete\n"));
	}
	...
	if (USB_NO_ERROR != dwErr)
		RT_TRACE(_module_hci_ops_os_c_, _drv_err_,
			 ("usb_read_port_complete Fail :%d\n", dwErr));

	{

		if (dwBytesTransferred > MAX_RECVBUF_SZ
		    || dwBytesTransferred < RXDESC_SIZE) {
			RT_TRACE(_module_hci_ops_os_c_, _drv_err_,
...
DWORD usb_write_mem_complete(LPVOID Context)
{
	...
	(*usb_funcs_vp->lpGetTransferStatus)(pio_req->usb_transfer_write_mem,
					     &dwBytes, &dwErr);
	fComplete = (*usb_funcs_vp->lpIsTransferComplete)(
			    pio_req->usb_transfer_write_mem);
	if (fComplete != _TRUE) {
		RT_TRACE(_module_hci_ops_os_c_, _drv_err_,
			 ("usb_write_mem_complete CloseTransfer before complete\n"));
	}
...
void usb_write_mem(struct intf_hdl *pintfhdl, u32 addr, u32 cnt, u8 *wmem)
{
	...
	struct intf_priv 	*pintfpriv	= pintfhdl->pintfpriv;
	struct dvobj_priv    *pdvobj_priv   = (struct dvobj_priv *)
					      pintfpriv->intf_dev;
	...
	_func_enter_;
	RT_TRACE(_module_hci_ops_os_c_, _drv_info_,
		 ("usb_write_mem(%u) pintfhdl %p wmem %p\n", __LINE__, pintfhdl, wmem));
	...
	if ((pio_req == NULL) || (adapter->bSurpriseRemoved)) {
		RT_TRACE(_module_hci_ops_os_c_, _drv_err_,
			 ("async_irp_write32 : pio_req =0x%x adapter->bSurpriseRemoved=0x%x",
			  pio_req, adapter->bSurpriseRemoved));
		goto exit;
	}
...
USB_PIPE ffaddr2pipehdl(struct dvobj_priv *pNdisCEDvice, u32 addr)
{
	...
	if (pNdisCEDvice->nr_endpoint == 11) {
		...
		RT_TRACE(_module_hci_ops_os_c_, _drv_err_,
			 ("ffaddr2pipehdl():nr_endpoint=%d error!\n", pNdisCEDvice->nr_endpoint));
	}
...
{
	struct zero_bulkout_context *pcontext = (struct zero_bulkout_context *)
						pZeroContext;
	...
	(*usb_funcs_vp->lpGetTransferStatus)(pxmitpriv->usb_transfer_write_port,
					     &dwBytesTransferred, &dwErr);
	fComplete = (*usb_funcs_vp->lpIsTransferComplete)(
			    pxmitpriv->usb_transfer_write_port);
	if (fComplete != _TRUE) {
		RT_TRACE(_module_hci_ops_os_c_, _drv_err_,
			 ("usb_bulkout_zero_complete CloseTransfer before complete\n"));
	}
...
u32 usb_bulkout_zero(struct intf_hdl *pintfhdl, u32 addr)
{
	...
	pcontext = (struct zero_bulkout_context *)rtw_zmalloc(sizeof(
				struct zero_bulkout_context));
...
u32 usb_write_port(struct intf_hdl *pintfhdl, u32 addr, u32 cnt, u8 *wmem)
{
	...
	RT_TRACE(_module_hci_ops_os_c_, _drv_info_,
		 ("usb_write_port: pipe handle convert\n"));
	...
	RT_TRACE(_module_hci_ops_os_c_, _drv_info_,
		 ("usb_write_port(%u): pxmitframe %X  pxmitframe->padapter %X\n", __LINE__,
		  pxmitframe, pxmitframe->padapter));
...
//    u8 *ptr;
	...
	if (pxmitpriv->txirp_cnt == 0) {
		RT_TRACE(_module_hci_ops_os_c_, _drv_err_,
			 ("usb_write_port_complete: txirp_cnt== 0, set allrxreturnevt!\n"));
	...
	if (fComplete != _TRUE) {
		RT_TRACE(_module_hci_ops_os_c_, _drv_err_,
			 ("usb_write_port_complete CloseTransfer before complete\n"));
	}
	(*usb_funcs_vp->lpCloseTransfer)(pxmitpriv->usb_transfer_write_port);

#else

	if ((*usb_funcs_vp->lpIsTransferComplete)(
		    pxmitpriv->usb_transfer_write_port)) {
		(*usb_funcs_vp->lpCloseTransfer)(pxmitpriv->usb_transfer_write_port);
	}
...
{
#ifndef PLATFORM_OS_CE
	struct SCSI_BUFFER_ENTRY *psb_entry = (struct SCSI_BUFFER_ENTRY *)
					      pTxContext;
	...
	RT_TRACE(_module_hci_ops_os_c_, _drv_info_, ("%s(%u): circ_space = %d\n",
			__FUNCTION__, __LINE__, CIRC_SPACE(psb->head, psb->tail,
					SCSI_BUFFER_NUMBER)));

#if 1

	(*lpUsbFuncs->lpGetTransferStatus)(psb_entry->usb_transfer_scsi_txcmd,
					   &dwBytesTransferred, &dwErr);
	fComplete = (*lpUsbFuncs->lpIsTransferComplete)(
			    psb_entry->usb_transfer_scsi_txcmd);
	if (fComplete != _TRUE) {
		RT_TRACE(_module_hci_ops_os_c_, _drv_err_,
			 ("usb_write_scsi_complete CloseTransfer before complete\n"));
	}
	(*lpUsbFuncs->lpCloseTransfer)(psb_entry->usb_transfer_scsi_txcmd);

#else

	if ((*lpUsbFuncs->lpIsTransferComplete)(
		    psb_entry->usb_transfer_scsi_txcmd))
		(*lpUsbFuncs->lpCloseTransfer)(psb_entry->usb_transfer_scsi_txcmd);
#endif
	...
	if (CIRC_CNT(psb->head, psb->tail, SCSI_BUFFER_NUMBER) == 0) {
		RT_TRACE(_module_hci_ops_os_c_, _drv_info_,
			 ("write_txcmd_scsififo_callback: up_sema\n"));
...
uint usb_write_scsi(struct intf_hdl *pintfhdl, u32 cnt, u8 *wmem)
{
	...
	struct SCSI_BUFFER       *psb      = padapter->pscsi_buf;
	struct SCSI_BUFFER_ENTRY *psb_entry = LIST_CONTAINOR(wmem,
					      struct SCSI_BUFFER_ENTRY, entry_memory);
	...
	RT_TRACE(_module_hci_ops_os_c_, _drv_info_, ("%s(%u)\n", __FUNCTION__,
			__LINE__));
	psb_entry->usb_transfer_scsi_txcmd =
		pdev->pUsbExtension->_lpUsbFuncs->lpIssueBulkTransfer(
...
uint usb_init_intf_priv(struct intf_priv *pintfpriv)
{
	...
	struct dvobj_priv *pNdisCEDvice = (struct dvobj_priv *)
					  pintfpriv->intf_dev;
	...
	if (pintfpriv->allocated_io_rwmem == NULL) {
		...
		pintfpriv->io_rwmem = pintfpriv->allocated_io_rwmem + 4 - ((u32)(
					      pintfpriv->allocated_io_rwmem) & 3);
	}
	...
	if (pintfpriv->allocated_io_r_mem == NULL) {
		...
		// word align the io_rwmem
		pintfpriv->io_r_mem = pintfpriv->allocated_io_r_mem + 4 - ((u32)(
					      pintfpriv->allocated_io_r_mem) & 3);
	}
#endif
...
DWORD usbctrl_vendorreq_complete(LPVOID lpvNotifyParameter)
{
	...
	RT_TRACE(_module_hci_ops_os_c_, _drv_debug_,
		 ("+usbctrl_vendorreq_complete\n"));

	return STATUS_SUCCESS;
}


int usbctrl_vendorreq(struct intf_priv *pintfpriv, u8 request, u16 value,
...
//	LPCUSB_DEVICE		lpDeviceInfo;
	...
	if (0x01 == requesttype) {
		usb_device_req.bmRequestType = USB_REQUEST_DEVICE_TO_HOST |
					       USB_REQUEST_VENDOR | USB_REQUEST_FOR_DEVICE;
	} else {
		usb_device_req.bmRequestType = USB_REQUEST_HOST_TO_DEVICE |
					       USB_REQUEST_VENDOR | USB_REQUEST_FOR_DEVICE;
	}
...
//	rtw_usleep_os(10);
	...
	if (usbTrans) {
		...
		if (fComplete == _TRUE) {
			...
			RT_TRACE(_module_hci_ops_os_c_, _drv_debug_,
				 ("usbctrl_vendorreq lpCloseTransfer\n"));
		}

		if (dwErr != USB_NO_ERROR || fComplete != _TRUE) {
			RT_TRACE(_module_hci_ops_os_c_, _drv_err_,
				 ("usbctrl_vendorreq lpCloseTransfer without complete\n"));
			ret = _FALSE;
			goto exit;
		}
	} else {
		RT_TRACE(_module_hci_ops_os_c_, _drv_err_,
			 ("usbctrl_vendorreq without usbTrans\n"));
		ret = _FALSE;
		goto exit;
