...
static BOOLEAN hal_EfusePgPacketWrite2ByteHeader(
	IN	PADAPTER		pAdapter,
	IN	u8				efuseType,
	IN	u16				*pAddr,
	IN	PPGPKT_STRUCT	pTargetPkt,
	IN	BOOLEAN			bPseudoTest);
static BOOLEAN hal_EfusePgPacketWrite1ByteHeader(
	IN	PADAPTER		pAdapter,
	IN	u8				efuseType,
	IN	u16				*pAddr,
	IN	PPGPKT_STRUCT	pTargetPkt,
	IN	BOOLEAN			bPseudoTest);
static BOOLEAN hal_EfusePgPacketWriteData(
	IN	PADAPTER		pAdapter,
	IN	u8				efuseType,
	IN	u16				*pAddr,
	IN	PPGPKT_STRUCT	pTargetPkt,
	IN	BOOLEAN			bPseudoTest);
static BOOLEAN hal_EfusePgPacketWrite_BT(
	IN	PADAPTER		pAdapter,
	IN	u8				offset,
	IN	u8				word_en,
	IN	u8				*pData,
...
)
{
	...
	for (i = 0 ; i < blockCount ; i++) {
		offset = i * blockSize;
		rtw_write32(Adapter, (FW_8192C_START_ADDRESS + offset),
			    cpu_to_le32(*(pu4BytePtr + i)));
	}
	...
	if (remainSize) {
		...
		case 1:
			remainFW[0] = *(p);
			ret = rtw_write32(Adapter,
					  (FW_8192C_START_ADDRESS + blockCount * blockSize),
	...
	for (i = 0 ; i < blockCount ; i++) {
		offset = i * blockSize;
#ifdef SUPPORTED_BLOCK_IO
		ret = rtw_writeN(Adapter, (FW_8192C_START_ADDRESS + offset), blockSize,
				 (bufferPtr + offset));
#else
		ret = rtw_write32(Adapter, (FW_8192C_START_ADDRESS + offset),
	...
	if (remainSize) {
		...
		ret = rtw_writeN(Adapter, (FW_8192C_START_ADDRESS + offset), remainSize,
				 (bufferPtr + offset));
		goto exit;
#endif
...
#ifdef SUPPORTED_BLOCK_IO
			ret = rtw_writeN(Adapter, (FW_8192C_START_ADDRESS + offset), blockSize2,
					 (bufferPtr + offset));
#else
			ret = rtw_write8(Adapter, (FW_8192C_START_ADDRESS + offset),
					 *(bufferPtr + offset));
#endif
		...
		if (remainSize2) {
			...
			for (i = 0 ; i < remainSize2 ; i++) {
				ret = rtw_write8(Adapter, (FW_8192C_START_ADDRESS + offset + i),
						 *(bufferPtr + i));
...
VOID rtl8192c_FirmwareSelfReset(
	IN		PADAPTER		Adapter
)
{
	u8	u1bTmp;
...
#endif
			rtw_write8(Adapter, REG_SYS_FUNC_EN + 1, (rtw_read8(Adapter,
					REG_SYS_FUNC_EN + 1) & ~BIT2));
		}
...
)
{
...
#ifdef CONFIG_FILE_FWIMG
		rtStatus = rtw_retrive_from_file(rtw_fw_file_path, FwBuffer8192C,
						 FW_8192C_SIZE);
	...
	DBG_8192C("fw_ver=v%d, fw_subver=%d, sig=0x%x\n",
		  pHalData->FirmwareVersion, pHalData->FirmwareSubVersion,
		  le16_to_cpu(pFwHdr->Signature) & 0xFFF0);
...
VOID InitializeFirmwareVars92C(
	IN	PADAPTER		Adapter
)
{
	HAL_DATA_TYPE		*pHalData	= GET_HAL_DATA(Adapter);
...
// 2011.04.12 by tynli.
//
VOID SetFwRelatedForWoWLAN8192CU(
	IN	PADAPTER			padapter,
...
static void _update_bt_param(_adapter *padapter)
{
	...
	if (2 != registry_par->bt_iso)
		pbtpriv->BT_Ant_isolation =
			registry_par->bt_iso;// 0:Low, 1:High, 2:From Efuse

	if (registry_par->bt_sco ==
	    1) // 0:Idle, 1:None-SCO, 2:SCO, 3:From Counter, 4.Busy, 5.OtherBusy
...
)
{
	...
	pbtpriv->BT_Coexist = (((PROMContent[EEPROM_RF_OPT1] &
				 BOARD_TYPE_NORMAL_MASK) >> 5) == BOARD_USB_COMBO) ? _TRUE :
			      _FALSE;	// bit [7:5]
...
VERSION_8192C rtl8192c_ReadChipVersion(
	IN	PADAPTER	Adapter
)
{
	u32			value32;
	...
	if (value32 & TRP_VAUX_EN) {
		...
		if (IS_HARDWARE_TYPE_8723A(Adapter)) {
			...
			ChipVersion |= ((value32 & BT_FUNC) ? CHIP_8723 :
					0); // RTL8723 with BT function.
		} else	 {
			version = (value32 & TYPE_ID) ? VERSION_TEST_CHIP_92C :
				  VERSION_TEST_CHIP_88C;
		}
#else
		// tynli_test. 2011.01.10.
		if (IS_HARDWARE_TYPE_8192C(Adapter)) {
			ChipVersion = (value32 & TYPE_ID) ? VERSION_TEST_CHIP_92C :
				      VERSION_TEST_CHIP_88C;
		} else {
			ChipVersion |= ((value32 & VENDOR_ID) ? CHIP_VENDOR_UMC : 0);
			ChipVersion |= ((value32 & BT_FUNC) ? CHIP_8723 :
...
//#endif
		...
		if (IS_HARDWARE_TYPE_8192C(Adapter)) {
			...
			if ((!IS_CHIP_VENDOR_UMC(ChipVersion)) && (value32 & CHIP_VER_RTL_MASK)) {
				...
				ChipVersion |= ((value32 & CHIP_VER_RTL_MASK) |
						CHIP_VENDOR_UMC); // IC version (CUT)
		...
		if (IS_92C_SERIAL(ChipVersion)) {
			...
			ChipVersion |= ((CHIP_BONDING_IDENTIFIER(value32) == CHIP_BONDING_92C_1T2R)
					? RF_TYPE_1T2R : 0);
	...
	if (IS_8723_SERIES(ChipVersion)) {
		...
		pHalData->MultiFunc = (RT_MULTI_FUNC)(pHalData->MultiFunc | ((
				value32 & WL_FUNC_EN) ?  RT_MULTI_FUNC_WIFI : 0));
		pHalData->MultiFunc = (RT_MULTI_FUNC)(pHalData->MultiFunc | ((
				value32 & BT_FUNC_EN) ?  RT_MULTI_FUNC_BT : 0));
		pHalData->MultiFunc = (RT_MULTI_FUNC)(pHalData->MultiFunc | ((
...
void rtl8192c_EfuseParseChnlPlan(
	IN	PADAPTER		padapter,
	IN	u8			*hwinfo,
	IN	BOOLEAN			AutoLoadFail
)
...
u8 GetEEPROMSize8192C(PADAPTER Adapter)
{
	...
	size = (curRCR & BOOT_FROM_EEPROM) ? 6 :
...
static VOID hal_EfusePowerSwitch_RTL8192C(
	IN	PADAPTER	pAdapter,
	IN	u8		bWrite,
	IN	u8		PwrState)
{
...
static VOID hal_EfusePowerSwitch_RTL8723(
	IN	PADAPTER	pAdapter,
	IN	u8		bWrite,
	IN	u8		PwrState)
{
...
static VOID rtl8192c_EfusePowerSwitch(
	IN	PADAPTER	pAdapter,
	IN	u8		bWrite,
	IN	u8		PwrState)
{
...
static VOID ReadEFuse_RTL8192C(
	PADAPTER	Adapter,
	u16		 _offset,
	u16 		_size_byte,
	u8      	*pbuf,
...
static BOOLEAN Hal_EfuseSwitchToBank(
	IN		PADAPTER	pAdapter,
	IN		u8			bank,
	IN		BOOLEAN		bPseudoTest
)
...
)
{
	...
	if (bPseudoTest) {
		fakeBTEfuseUsedBytes = (EFUSE_REAL_CONTENT_LEN * (bank - 1)) + eFuse_Addr -
				       1;
	} else {
...
static VOID rtl8192c_ReadEFuse(
	PADAPTER	Adapter,
	u8		efuseType,
	u16		_offset,
	u16 		_size_byte,
...
{
	if (bPseudoTest) {
		ReadEFuse_Pseudo(Adapter, efuseType, _offset, _size_byte, pbuf,
				 bPseudoTest);
	} else {
...
static VOID Hal_EFUSEGetEfuseDefinition(
	IN		PADAPTER	pAdapter,
	IN		u8		efuseType,
	IN		u8		type,
	OUT		PVOID		*pOut
)
{
	...
	case TYPE_AVAILABLE_EFUSE_BYTES_TOTAL: {
		...
		if (efuseType == EFUSE_WIFI)
			...
			*pu2Tmp = (u16)(EFUSE_BT_REAL_CONTENT_LEN - (EFUSE_PROTECT_BYTES_BANK *
					3));
	}
	break;
...
static VOID Hal_EFUSEGetEfuseDefinition_Pseudo(
	IN		PADAPTER	pAdapter,
	IN		u8		efuseType,
	IN		u8		type,
	OUT		PVOID		*pOut
...
static VOID rtl8192c_EFUSE_GetEfuseDefinition(
	IN		PADAPTER	pAdapter,
	IN		u8		efuseType,
	IN		u8		type,
	OUT		PVOID		*pOut,
...
static u8 Hal_EfuseWordEnableDataWrite(IN	PADAPTER	pAdapter,
				       IN	u16		efuse_addr,
				       IN	u8		word_en,
				       IN	u8		*data,
				       IN	BOOLEAN		bPseudoTest)
...
static u8 Hal_EfuseWordEnableDataWrite_Pseudo(IN	PADAPTER	pAdapter,
		IN	u16		efuse_addr,
		IN	u8		word_en,
		IN	u8		*data,
		IN	BOOLEAN		bPseudoTest)
{
	...
	ret = Hal_EfuseWordEnableDataWrite(pAdapter, efuse_addr, word_en, data,
					   bPseudoTest);

	return ret;
}

static u8 rtl8192c_Efuse_WordEnableDataWrite(IN	PADAPTER	pAdapter,
		IN	u16		efuse_addr,
		IN	u8		word_en,
		IN	u8		*data,
		IN	BOOLEAN		bPseudoTest)
{
	...
	if (bPseudoTest) {
		ret = Hal_EfuseWordEnableDataWrite_Pseudo(pAdapter, efuse_addr, word_en,
				data, bPseudoTest);
	} else {
		ret = Hal_EfuseWordEnableDataWrite(pAdapter, efuse_addr, word_en, data,
						   bPseudoTest);
...
static u16 hal_EfuseGetCurrentSize_8192C(IN	PADAPTER	pAdapter,
		IN		BOOLEAN		bPseudoTest)
{
...
static u16 Hal_EfuseGetCurrentSize_BT(IN	PADAPTER	pAdapter,
				      IN		BOOLEAN			bPseudoTest)
{
	int	bContinual = _TRUE;
	u16	efuse_addr = 0;
	...
	for (bank = startBank; bank < EFUSE_MAX_BANK; bank++) {
		if (!Hal_EfuseSwitchToBank(pAdapter, bank, bPseudoTest))
			...
			bContinual = _TRUE;
			if (bank !=
			    startBank)	// only when bank is switched we have to reset the efuse_addr.
				efuse_addr = 0;
		}
...
static u16 hal_EfuseGetCurrentSize_8723(IN	PADAPTER	pAdapter,
					IN		BOOLEAN			bPseudoTest)
{
	int	bContinual = _TRUE;
...
static u16 Hal_EfuseGetCurrentSize_Pseudo(IN	PADAPTER	pAdapter,
		IN		BOOLEAN			bPseudoTest)
{
	u16	ret = 0;
...
static u16 rtl8192c_EfuseGetCurrentSize(
	IN	PADAPTER	pAdapter,
	IN	u8			efuseType,
	IN	BOOLEAN		bPseudoTest)
{
...
static int hal_EfusePgPacketRead_8192C(IN	PADAPTER	pAdapter,
				       IN	u8			offset,
				       IN	u8			*data,
				       IN	BOOLEAN			bPseudoTest)
{
...
static int hal_EfusePgPacketRead_8723(IN	PADAPTER	pAdapter,
				      IN	u8			offset,
				      IN	u8			*data,
				      IN	BOOLEAN			bPseudoTest)
{
...
static int Hal_EfusePgPacketRead(IN	PADAPTER	pAdapter,
				 IN	u8			offset,
				 IN	u8			*data,
				 IN	BOOLEAN			bPseudoTest)
{
...
static int Hal_EfusePgPacketRead_Pseudo(IN	PADAPTER	pAdapter,
					IN	u8			offset,
					IN	u8			*data,
					IN	BOOLEAN		bPseudoTest)
{
...
static int rtl8192c_Efuse_PgPacketRead(IN	PADAPTER	pAdapter,
				       IN	u8			offset,
				       IN	u8			*data,
				       IN	BOOLEAN		bPseudoTest)
{
...
static BOOLEAN hal_EfuseFixHeaderProcess(
	IN		PADAPTER			pAdapter,
	IN		u8					efuseType,
...
)
{
	...
	if (Efuse_PgPacketRead(pAdapter, pFixPkt->offset, originaldata,
		...
		if (badworden != 0xf) {	// write fail
			if (efuseType == EFUSE_WIFI)
				PgWriteSuccess = Efuse_PgPacketWrite(pAdapter, pFixPkt->offset, badworden,
								     originaldata, bPseudoTest);
			else
				PgWriteSuccess = hal_EfusePgPacketWrite_BT(pAdapter, pFixPkt->offset,
						 badworden, originaldata, bPseudoTest);
			if (!PgWriteSuccess)
				return _FALSE;
			else
	...
	IN			BOOLEAN			bPseudoTest)
{
	...
	EFUSE_GetEfuseDefinition(pAdapter, efuseType,
				 TYPE_AVAILABLE_EFUSE_BYTES_BANK, (PVOID)&efuse_max_available_len,
				 bPseudoTest);
	...
	while (efuse_addr < efuse_max_available_len) {
		...
		if (tmp_header == pg_header) {
			...
			if ((tmp_header & 0x0F) == 0x0F) {	//word_en PG fail
				...
				fixPkt.offset = ((pg_header_temp & 0xE0) >> 5) | ((tmp_header & 0xF0) >>
						1);
	...
	if (badworden == 0x0F) {
		...
		if (efuseType == EFUSE_WIFI)
			PgWriteSuccess = Efuse_PgPacketWrite(pAdapter, pTargetPkt->offset,
							     badworden, pTargetPkt->data, bPseudoTest);
		else
			PgWriteSuccess = hal_EfusePgPacketWrite_BT(pAdapter, pTargetPkt->offset,
					 badworden, pTargetPkt->data, bPseudoTest);
		if (!PgWriteSuccess)
			return _FALSE;
		else
...
static BOOLEAN hal_EfusePgPacketWriteHeader(
	IN			PADAPTER		pAdapter,
	IN			u8				efuseType,
	IN			u16				*pAddr,
	IN			PPGPKT_STRUCT	pTargetPkt,
...
static BOOLEAN hal_EfusePgCheckAvailableAddr(
	IN	PADAPTER	pAdapter,
	IN	u8			efuseType,
	IN	BOOLEAN		bPseudoTest
)
{
	...
	EFUSE_GetEfuseDefinition(pAdapter, efuseType,
				 TYPE_AVAILABLE_EFUSE_BYTES_TOTAL, (PVOID)&efuse_max_available_len,
				 bPseudoTest);
	//RTPRINT(FEEPROM, EFUSE_PG, ("efuse_max_available_len = %d\n", efuse_max_available_len));

	if (Efuse_GetCurrentSize(pAdapter, efuseType,
				 bPseudoTest) >= efuse_max_available_len) {
...
)
{
	...
	for (i = 0; i < (word_cnts * 2) ; i++) {
		if (efuse_OneByteRead(pAdapter, (startAddr + i), &efuse_data, bPseudoTest)
		    && (efuse_data != 0xFF))
			bRet = _TRUE;
	}
...
static BOOLEAN hal_EfusePartialWriteCheck(
	IN	PADAPTER		pAdapter,
	IN	u8				efuseType,
	IN	u16				*pAddr,
	IN	PPGPKT_STRUCT	pTargetPkt,
...
)
{
	...
	EFUSE_GetEfuseDefinition(pAdapter, efuseType,
				 TYPE_AVAILABLE_EFUSE_BYTES_BANK, (PVOID)&efuse_max_available_len,
				 bPseudoTest);
	EFUSE_GetEfuseDefinition(pAdapter, efuseType, TYPE_EFUSE_REAL_CONTENT_LEN,
				 (PVOID)&efuse_max, bPseudoTest);
	...
	while (1) {
		...
		if (efuse_OneByteRead(pAdapter, startAddr, &efuse_data, bPseudoTest)
		    && (efuse_data != 0xFF)) {
			if (EXT_HEADER(efuse_data)) {
			...
			if ((curPkt.offset == pTargetPkt->offset) &&
				...
				badworden = Efuse_WordEnableDataWrite(pAdapter, startAddr + 1,
								      matched_wden, pTargetPkt->data, bPseudoTest);
				if (badworden != 0x0F) {
					u32	PgWriteSuccess = 0;
...
static BOOLEAN hal_EfusePgPacketWrite_8723(
	IN	PADAPTER		pAdapter,
	IN	u8 			offset,
	IN	u8			word_en,
	IN	u8			*pData,
...
static int hal_EfusePgPacketWrite_8192C(IN	PADAPTER	pAdapter,
					IN	u8 			offset,
					IN	u8			word_en,
					IN	u8			*data,
					IN	BOOLEAN		bPseudoTest)
{
	...
	//
	if (Efuse_GetCurrentSize(pAdapter, efuseType,
				 bPseudoTest) >= (EFUSE_REAL_CONTENT_LEN - EFUSE_OOB_PROTECT_BYTES)) {
	...
	//
	while (bContinual
	       && (efuse_addr  < (EFUSE_REAL_CONTENT_LEN - EFUSE_OOB_PROTECT_BYTES))) {
...
static int Hal_EfusePgPacketWrite_Pseudo(IN	PADAPTER	pAdapter,
		IN	u8 			offset,
		IN	u8			word_en,
		IN	u8			*data,
		IN	BOOLEAN		bPseudoTest)
{
	...
	ret = hal_EfusePgPacketWrite_8723(pAdapter, offset, word_en, data,
					  bPseudoTest);

	return ret;
}

static int Hal_EfusePgPacketWrite(IN	PADAPTER	pAdapter,
				  IN	u8 			offset,
				  IN	u8			word_en,
				  IN	u8			*data,
				  IN	BOOLEAN		bPseudoTest)
{
	...
	if (IS_HARDWARE_TYPE_8192C(pAdapter)) {
		ret = hal_EfusePgPacketWrite_8192C(pAdapter, offset, word_en, data,
						   bPseudoTest);
	} else if (IS_HARDWARE_TYPE_8723A(pAdapter)) {
		ret = hal_EfusePgPacketWrite_8723(pAdapter, offset, word_en, data,
						  bPseudoTest);
...
static int rtl8192c_Efuse_PgPacketWrite(IN	PADAPTER	pAdapter,
					IN	u8 			offset,
					IN	u8			word_en,
					IN	u8			*data,
					IN	BOOLEAN		bPseudoTest)
{
	...
	if (bPseudoTest) {
		ret = Hal_EfusePgPacketWrite_Pseudo(pAdapter, offset, word_en, data,
						    bPseudoTest);
	} else {
...
VOID rtl8192c_EfuseParseIDCode(
	IN	PADAPTER	pAdapter,
	IN	u8			*hwinfo
)
{
...
static s32 c2h_handler_8192c(_adapter *padapter,
...
void rtl8192c_set_hal_ops(struct hal_ops *pHalFunc)
{
	...
	pHalFunc->sreset_linked_status_check  =
		&rtl8192c_sreset_linked_status_check;
