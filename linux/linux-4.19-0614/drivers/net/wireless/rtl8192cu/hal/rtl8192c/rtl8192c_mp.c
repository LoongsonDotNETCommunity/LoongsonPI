	...
	if (IS_92C_SERIAL(pHalData->VersionID)) {
		...
		if (BOARD_MINICARD == pHalData->BoardType) {
			...
			if (ChannelToSw == 6 || ChannelToSw == 8) {
				...
				write_bbreg(pAdapter, rOFDM0_RxDetector1, bMaskByte0,
					    pMptCtx->backup0xc30);
			}
		} else {
			if (ulRateIdx < MPT_RATE_6M) {	// CCK rate
...
//	SelectChannel(pAdapter, pmp->channel);
	...
	for (eRFPath = 0; eRFPath < pHalData->NumTotalRFPath; eRFPath++) {
		if (IS_HARDWARE_TYPE_8192D(pAdapter))
			_write_rfreg(pAdapter, (RF_RADIO_PATH_E)eRFPath, rRfChannel, 0xFF,
				     channel);
		else
...
//				PHY_SetBBReg(pAdapter, rFPGA0_XB_RFInterfaceOE, BIT10, 0);
	...
	case ANTENNA_AB:	// For 8192S
		...
		//if (IS_HARDWARE_TYPE_8192S(pAdapter))
		{
			...
			if ((pHalData->VersionID ==
			     VERSION_8192S_ACUT)) { // For RTL8192SU A-Cut only, by Roger, 2008.11.07.
...
//				PHY_SetBBReg(pAdapter, rFPGA0_XB_RFInterfaceOE, BIT10, 0);
	...
	if (chgTx && chgRx) {
		...
		case RF_6052:
			...
			PHY_SetBBReg(pAdapter, rFPGA1_TxInfo, 0x7fffffff,
				     r_ant_select_ofdm_val);	//OFDM Tx
			PHY_SetBBReg(pAdapter, rFPGA0_TxInfo, 0x0000000f,
				     r_ofdm_tx_en_val);		//OFDM Tx
			PHY_SetBBReg(pAdapter, rOFDM0_TRxPathEnable, 0x0000000f,
				     r_rx_antenna_ofdm);	//OFDM Rx
			PHY_SetBBReg(pAdapter, rOFDM1_TRxPathEnable, 0x0000000f,
				     r_rx_antenna_ofdm);	//OFDM Rx
			PHY_SetBBReg(pAdapter, rCCK0_AFESetting, bMaskByte3,
...
u8 Hal_ReadRFThermalMeter(PADAPTER pAdapter)
{
	...
	ThermalValue = _read_rfreg(pAdapter, RF_PATH_A, RF_T_METER,
...
void Hal_SetCCKContinuousTx(PADAPTER pAdapter, u8 bStart)
{
	...
	if (bStart) {
		...
		write_bbreg(pAdapter, rCCK0_System, bCCKBBMode, 0x2);	//transmit mode
		write_bbreg(pAdapter, rCCK0_System, bCCKScramble,
			    bEnable);	//turn on scramble setting
