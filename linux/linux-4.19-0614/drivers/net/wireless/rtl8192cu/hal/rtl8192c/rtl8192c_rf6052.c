...
)
{
	...
	for (rf = 0; rf < 2; rf++) {
		...
		case 0:	// Realtek better performance
			...
			writeVal = pHalData->MCSTxPowerLevelOriginalOffset[chnlGroup][index +
					(rf ? 8 : 0)] +
		...
		case 3:	// Customer defined power diff.
			...
			for (i = 0; i < 4; i++) {
				pwr_diff_limit[i] = (u8)((
								 pHalData->MCSTxPowerLevelOriginalOffset[chnlGroup][index +
										 (rf ? 8 : 0)] & (0x7f << (i * 8))) >> (i * 8));
				if (pHalData->CurrentChannelBW == HT_CHANNEL_WIDTH_40) {
			...
			writeVal = customer_limit + ((index < 2) ? powerBase0[rf] :
						     powerBase1[rf]);
	...
	IN	u8		Channel)
{
	...
	getPowerBase(Adapter, pPowerLevel, Channel, &powerBase0[0],
		     &powerBase1[0]);
...
static VOID phy_RF6052_Config_HardCode(
	IN	PADAPTER		Adapter
)
{
...
static int phy_RF6052_Config_ParaFile(
	IN	PADAPTER		Adapter
)
{
	u32					u4RegValue;
	...
	if (IS_HARDWARE_TYPE_8192C(Adapter)) {
		if (IS_92C_SERIAL(
			    pHalData->VersionID)) { // 88c's IPA  is different from 92c's
	...
	for (eRFPath = 0; eRFPath < pHalData->NumTotalRFPath; eRFPath++) {
		...
		case RF_PATH_A:
			...
			rtStatus = rtl8192c_PHY_ConfigRFWithHeaderFile(Adapter,
					(RF_RADIO_PATH_E)eRFPath);
#else
			rtStatus = rtl8192c_PHY_ConfigRFWithParaFile(Adapter, pszRadioAFile,
					(RF_RADIO_PATH_E)eRFPath);
#endif
			break;
		case RF_PATH_B:
			...
			rtStatus = rtl8192c_PHY_ConfigRFWithHeaderFile(Adapter,
					(RF_RADIO_PATH_E)eRFPath);
#else
			rtStatus = rtl8192c_PHY_ConfigRFWithParaFile(Adapter, pszRadioBFile,
					(RF_RADIO_PATH_E)eRFPath);
#endif
			break;
		case RF_PATH_C:
...
VOID PHY_RFShadowCompareFlagSetAll(
	IN	PADAPTER			Adapter)
{
	u32		eRFPath;
	u32		Offset;
	...
	for (eRFPath = 0; eRFPath < RF6052_MAX_PATH; eRFPath++) {
		for (Offset = 0; Offset <= RF6052_MAX_REG; Offset++) {
			...
			if (Offset != 0x26 && Offset != 0x27)
				PHY_RFShadowCompareFlagSet(Adapter, (RF_RADIO_PATH_E)eRFPath, Offset,
							   _FALSE);
			else
				PHY_RFShadowCompareFlagSet(Adapter, (RF_RADIO_PATH_E)eRFPath, Offset,
							   _TRUE);
	...
	IN	PADAPTER			Adapter)
{
	...
	for (eRFPath = 0; eRFPath < RF6052_MAX_PATH; eRFPath++) {
		for (Offset = 0; Offset <= RF6052_MAX_REG; Offset++) {
			// 2008/11/20 MH For S3S4 test, we only check reg 26/27 now!!!!
			if (Offset != 0x26 && Offset != 0x27)
				PHY_RFShadowRecorverFlagSet(Adapter, (RF_RADIO_PATH_E)eRFPath, Offset,
							    _FALSE);
			else
				PHY_RFShadowRecorverFlagSet(Adapter, (RF_RADIO_PATH_E)eRFPath, Offset,
							    _TRUE);
