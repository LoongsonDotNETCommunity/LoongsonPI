...
)
{
	...
	pDigTable->CurSTAConnectState = pDigTable->PreSTAConnectState =
						DIG_STA_DISCONNECT;
	...
	if ((pDigTable->CurMultiSTAConnectState == DIG_MultiSTA_CONNECT) &&
		...
		if (pdmpriv->EntryMinUndecoratedSmoothedPWDB != 0)
			...
			Rssi_val_min  = (pdmpriv->UndecoratedSmoothedPWDB +
					 pdmpriv->EntryMinUndecoratedSmoothedPWDB) / 2;
#else
...
static VOID dm_FalseAlarmCounterStatistics(
	IN	PADAPTER	Adapter
)
{
	u32 ret_value;
...
static VOID dm_CtrlInitGainByFA(
	IN	PADAPTER	pAdapter
)
{
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(pAdapter);
...
VOID dm_CtrlInitGainByRssi(IN PADAPTER pAdapter)
{
	...
	u8 Adap_IGI_Upper = pdmpriv->IGI_target + 30 + (u8) pdmpriv->TH_L2H_ini -
			    (u8) pdmpriv->TH_EDCCA_HL_diff;
#endif
	...
	if ((FalseAlmCnt->Cnt_all < 500)
	    && (pdmpriv->DIG_Dynamic_MIN > DM_DIG_MIN))
		pdmpriv->DIG_Dynamic_MIN--;
	if ((pDigTable->Rssi_val_min < 8)
	    && (pdmpriv->DIG_Dynamic_MIN > DM_DIG_MIN))
		pdmpriv->DIG_Dynamic_MIN--;
	...
	if (FalseAlmCnt->Cnt_all < 250) {
		...
		pDigTable->CurIGValue = pDigTable->Rssi_val_min + 10 -
					pDigTable->BackoffVal;
...
static VOID dm_initial_gain_Multi_STA(
	IN	PADAPTER	pAdapter)
{
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(pAdapter);
	struct dm_priv	*pdmpriv = &pHalData->dmpriv;
	...
	if (check_fwstate(pmlmepriv,
			  WIFI_AP_STATE | WIFI_ADHOC_STATE | WIFI_ADHOC_MASTER_STATE) == _TRUE) {
		bMulti_STA = _TRUE;
	}
...
static VOID dm_initial_gain_STA_beforelinked(
	IN	PADAPTER	pAdapter)
{
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(pAdapter);
	struct dm_priv	*pdmpriv = &pHalData->dmpriv;
	...
	printk("%s ==> FalseAlmCnt->Cnt_all:%d CurIGValue:0x%02x \n", __FUNCTION__,
	       pFalseAlmCnt->Cnt_all, pDigTable->CurIGValue);
}

static VOID dm_initial_gain_STA(
	IN	PADAPTER	pAdapter)
{
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(pAdapter);
	struct dm_priv	*pdmpriv = &pHalData->dmpriv;
...
static	void dm_CtrlInitGainByTwoPort(
	IN	PADAPTER	pAdapter)
{
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(pAdapter);
	struct dm_priv	*pdmpriv = &pHalData->dmpriv;
	...
	if (check_fwstate(pmlmepriv,
		...
		if ((is_IBSS_empty(pAdapter) == _FAIL)
		    && (pAdapter->stapriv.asoc_sta_count > 2))
	...
	IN	PADAPTER	pAdapter)
{
	...
	pDigTable->PreIGValue = (u8)PHY_QueryBBReg(pAdapter, rOFDM0_XAAGCCore1,
				bMaskByte0);
...
static VOID DM_ChangeDynamicInitGainThresh(
	IN	PADAPTER	pAdapter,
	IN	u32		DM_Type,
	IN	u32		DM_Value)
{
...
)
{
	...
	if ((bbtchange) || (!precvpriv->bIsAnyNonBEPkts)) {
		...
		if ((pmlmeinfo->assoc_AP_vendor == ralinkAP)
		    || (pmlmeinfo->assoc_AP_vendor == atherosAP)) {
...
static	VOID dm_TXPowerTrackingCallback_ThermalMeter_92C(
	IN PADAPTER	Adapter)
{
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(Adapter);
	struct dm_priv	*pdmpriv = &pHalData->dmpriv;
...
#endif
	u8			OFDM_min_index = 6,
				rf; //OFDM BB Swing should be less than +3.0dB, which is required by Arthur
#if 0
	...
	rtl8192c_PHY_APCalibrate(Adapter,
...
//		if(!pHalData->ThermalValue)
		{
			...
			for (i = 0 ; i < CCK_TABLE_SIZE ; i++) {
				if (pdmpriv->bCCKinCH14) {
					if (_rtw_memcmp((void *)&TempCCk, (void *)&CCKSwingTable_Ch14[i][2],
							4) == _TRUE) {
						CCK_index_old = (u8)i;
					...
					if (_rtw_memcmp((void *)&TempCCk, (void *)&CCKSwingTable_Ch1_Ch13[i][2],
							4) == _TRUE) {
						CCK_index_old = (u8)i;
		...
		delta = (ThermalValue > pdmpriv->ThermalValue) ? (ThermalValue -
				pdmpriv->ThermalValue) : (pdmpriv->ThermalValue - ThermalValue);
#ifdef CONFIG_USB_HCI
		if (pHalData->BoardType == BOARD_USB_High_PA) {
			if (pdmpriv->bDoneTxpower)
				delta_HP = (ThermalValue > pdmpriv->ThermalValue) ? (ThermalValue -
						pdmpriv->ThermalValue) : (pdmpriv->ThermalValue - ThermalValue);
			else
		...
		delta_LCK = (ThermalValue > pdmpriv->ThermalValue_LCK) ?
			    (ThermalValue - pdmpriv->ThermalValue_LCK) : (pdmpriv->ThermalValue_LCK -
					    ThermalValue);
		delta_IQK = (ThermalValue > pdmpriv->ThermalValue_IQK) ?
			    (ThermalValue - pdmpriv->ThermalValue_IQK) : (pdmpriv->ThermalValue_IQK -
					    ThermalValue);
		...
		if ((delta > 0 || delta_HP > 0) && pdmpriv->TxPowerTrackControl) {
...
#endif
			{
				...
				for (i = 0; i < rf; i++) {
					...
					} else if (TxPwrLevel[i] >= 27 && TxPwrLevel[i] <= 32
						   && ThermalValue > pHalData->EEPROMThermalMeter) {
						if (delta < 5)
							OFDM_index[i] -= 1;
						else
							OFDM_index[i] -= 2;
					} else if (TxPwrLevel[i] >= 32 && TxPwrLevel[i] <= 38
						   && ThermalValue > pHalData->EEPROMThermalMeter && delta > 5) {
						OFDM_index[i] -= 1;
					}
				}
		...
		if (pdmpriv->TxPowerTrackControl && (delta != 0 || delta_HP != 0)) {
			...
			if (X != 0) {
				...
				PHY_SetBBReg(Adapter, rOFDM0_XATxIQImbalance, bMaskDWord,
					     OFDMSwingTable[OFDM_index[0]]);
			...
			if (is2T) {
				...
				if (X != 0) {
					...
					PHY_SetBBReg(Adapter, rOFDM0_XBTxIQImbalance, bMaskDWord,
						     OFDMSwingTable[OFDM_index[1]]);
...
static	VOID dm_InitializeTXPowerTracking_ThermalMeter(
	IN	PADAPTER		Adapter)
{
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(Adapter);
	struct dm_priv	*pdmpriv = &pHalData->dmpriv;
...
static VOID DM_InitializeTXPowerTracking(
	IN	PADAPTER		Adapter)
{
...
//
static VOID DM_TXPowerTracking92CDirectCall(
	IN	PADAPTER		Adapter)
{
	dm_TXPowerTrackingCallback_ThermalMeter_92C(Adapter);
}

static VOID dm_CheckTXPowerTracking_ThermalMeter(
	IN	PADAPTER		Adapter)
{
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(Adapter);
	struct dm_priv	*pdmpriv = &pHalData->dmpriv;
...
static BOOLEAN BT_BTStateChange(PADAPTER Adapter)
{
	...
	*/
	...
	if (BT_Tx == 0xffffffff && BT_PRI == 0xffffffff && Polling == 0xffffffff
	    && BT_State == 0xff)
		return _FALSE;
...
static BOOLEAN BT_WifiConnectChange(
	IN	PADAPTER	Adapter
)
{
	struct mlme_priv *pmlmepriv = &(Adapter->mlmepriv);
...
)
{
	...
	if (check_fwstate(pmlmepriv, _FW_LINKED) == _TRUE) {
		...
		if (pdmpriv->EntryMinUndecoratedSmoothedPWDB == 0)
			UndecoratedSmoothedPWDB =
				100;	// No any RSSI information. Assume to be MAX.
		else
			UndecoratedSmoothedPWDB = pdmpriv->EntryMinUndecoratedSmoothedPWDB;
	}
...
static void dm_BTCoexist(PADAPTER Adapter)
{
	...
	if ((pbtpriv->BT_Coexist) && (pbtpriv->BT_CoexistType == BT_CSR_BC4)
	    && (check_fwstate(pmlmepriv, WIFI_AP_STATE) == _FALSE)) {
				...
				if (pbtpriv->BT_Ampdu) { // 0:Disable BT control A-MPDU, 1:Enable BT control A-MPDU.
					...
					if (pHalData->bt_coexist.BtRssiState & BT_RSSI_STATE_AMDPU_OFF) {
						if (pMgntInfo->bBT_Ampdu && pHTInfo->bAcceptAddbaReq) {
							...
							if (GetTs(Adapter, (PTS_COMMON_INFO *)(&pRxTs), pMgntInfo->Bssid, 0,
								  RX_DIR, FALSE))
...
)
{
	...
	*/
	...
	if (BT_Tx == 0xffffffff && BT_PRI == 0xffffffff && Polling == 0xffffffffff
	    && BT_State == 0xff)
		return FALSE;
...
BOOLEAN BT_RSSIChangeWithAMPDU(
	IN	PADAPTER	Adapter
)
{
	PMGNT_INFO		pMgntInfo = &Adapter->MgntInfo;
	...
	if (!Adapter->pNdisCommon->bRegBT_Ampdu
	    || !Adapter->pNdisCommon->bRegAcceptAddbaReq)
		return FALSE;
	...
	if ((pHalData->UndecoratedSmoothedPWDB <= 32)
	    && pMgntInfo->pHTInfo->bAcceptAddbaReq) {
...
)
{
	...
	if ((pHalData->bt_coexist.BluetoothCoexist) &&
		...
		RTPRINT(FBT, BT_TRACE,
			("bWifiConnectChange %d, bBtStateChange  %d,LastTxPowerLvl  %x,  DynamicTxHighPowerLvl  %x\n",
			 bWifiConnectChange, bBtStateChange, LastTxPowerLvl,
			 pHalData->DynamicTxHighPowerLvl));
		if (bWifiConnectChange || bBtStateChange  ||
		    (LastTxPowerLvl != pHalData->DynamicTxHighPowerLvl)
		    || bRSSIChangeWithAMPDU) {
			LastTxPowerLvl = pHalData->DynamicTxHighPowerLvl;
 ...
 *
 *---------------------------------------------------------------------------*/
static VOID dm_CheckRfCtrlGPIO(
	IN	PADAPTER	Adapter
)
{
#if 0
...
static VOID dm_InitRateAdaptiveMask(
	IN	PADAPTER	Adapter
)
{
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(Adapter);
...
static VOID dm_RefreshRateAdaptiveMask(IN	PADAPTER	pAdapter)
{
	...
	if (pAdapter->MgntInfo.mAssoc && (!ACTING_AS_AP(pAdapter))) {
		...
		if (pRA->PreRATRState != pRA->RATRState) {
			RT_PRINT_ADDR(COMP_RATR, DBG_LOUD, ("Target AP addr : "),
				      pMgntInfo->Bssid);
	...
	if (pTargetAdapter != NULL) {
		...
		for (i = 0; i < ASSOCIATE_ENTRY_NUM; i++) {
			if (pTargetAdapter->MgntInfo.AsocEntry[i].bUsed
			    && pTargetAdapter->MgntInfo.AsocEntry[i].bAssociated) {
				...
				if (pEntry->rssi_stat.UndecoratedSmoothedPWDB > (s4Byte)
				    HighRSSIThreshForRA)
					pEntryRA->RATRState = DM_RATR_STA_HIGH;
				else if (pEntry->rssi_stat.UndecoratedSmoothedPWDB >
					 (s4Byte)LowRSSIThreshForRA)
...
static VOID dm_CheckProtection(
	IN	PADAPTER	Adapter
)
{
#if 0
...
static VOID dm_CheckStatistics(
	IN	PADAPTER	Adapter
)
{
#if 0
	...
	rtw_hal_get_hwreg(Adapter, HW_VAR_INIT_TX_RATE,
			  (pu1Byte)(&Adapter->TxStats.CurrentInitTxRate));
	...
	rtw_hal_get_hwreg(Adapter, HW_VAR_RETRY_COUNT,
			  (pu1Byte)(&Adapter->TxStats.NumTxRetryCount));
#endif
}
...
static void dm_CheckPbcGPIO(_adapter *padapter)
{
	...
	do {
		...
		rtw_write8(padapter, GPIO_IO_SEL,
			   tmp1byte);	//enable GPIO[2] as output mode

		tmp1byte &= ~(HAL_8192C_HW_GPIO_WPS_BIT);
		rtw_write8(padapter,  GPIO_IN,
			   tmp1byte);		//reset the floating voltage level
...
//
static void dm_InitGPIOSetting(
	IN	PADAPTER	Adapter
)
{
	PHAL_DATA_TYPE		pHalData = GET_HAL_DATA(Adapter);
...
static void update_EDCA_param(_adapter *padapter)
{
	...
	if (pdmpriv->prv_traffic_idx != trafficIndex) {
		...
		if (_TRUE == bbtchange)
			...
			rtw_write32(padapter, REG_EDCA_BE_PARAM,
				    EDCAParam[pmlmeinfo->assoc_AP_vendor][trafficIndex]);
	...
	if (pPSTable->PreCCAState != pPSTable->CurCCAState) {
		if (pPSTable->CurCCAState == CCA_1R) {
			if (pHalData->rf_type == RF_2T2R) {
				...
				PHY_SetBBReg(pAdapter, 0xe70, 0x7fc00000,
					     0x10c); // Set RegE70[30:22] = 9b'100001100
			}
		} else {
...
)
{
	...
	if (pdmpriv->initialize == 0) {
		pdmpriv->rf_saving_Reg874 = (PHY_QueryBBReg(pAdapter,
					     rFPGA0_XCD_RFInterfaceSW, bMaskDWord) & 0x1CC000) >> 14;
		pdmpriv->rf_saving_RegC70 = (PHY_QueryBBReg(pAdapter, rOFDM0_AGCParameter1,
					     bMaskDWord)&BIT3) >> 3;
		pdmpriv->rf_saving_Reg85C = (PHY_QueryBBReg(pAdapter,
					     rFPGA0_XCD_SwitchControl, bMaskDWord) & 0xFF000000) >> 24;
		pdmpriv->rf_saving_RegA74 = (PHY_QueryBBReg(pAdapter, 0xa74,
	...
	if (pPSTable->PreRFState != pPSTable->CurRFState) {
		if (pPSTable->CurRFState == RF_Save) {
			PHY_SetBBReg(pAdapter, rFPGA0_XCD_RFInterfaceSW, 0x1C0000,
				     0x2);   //Reg874[20:18]=3'b010
			PHY_SetBBReg(pAdapter, rOFDM0_AGCParameter1, BIT3, 0); //RegC70[3]=1'b0
			PHY_SetBBReg(pAdapter, rFPGA0_XCD_SwitchControl, 0xFF000000,
				     0x63); //Reg85C[31:24]=0x63
			PHY_SetBBReg(pAdapter, rFPGA0_XCD_RFInterfaceSW, 0xC000,
			...
			PHY_SetBBReg(pAdapter, rFPGA0_XCD_RFInterfaceSW, 0x1CC000,
				     pdmpriv->rf_saving_Reg874);
			PHY_SetBBReg(pAdapter, rOFDM0_AGCParameter1, BIT3,
				     pdmpriv->rf_saving_RegC70);
			PHY_SetBBReg(pAdapter, rFPGA0_XCD_SwitchControl, 0xFF000000,
				     pdmpriv->rf_saving_Reg85C);
...
static void dm_DynamicBBPowerSaving(
	IN	PADAPTER	pAdapter
)
{
...
void	SwAntDivCompare8192C(PADAPTER Adapter, WLAN_BSSID_EX *dst,
			     WLAN_BSSID_EX *src)
{
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(Adapter);
...
//
void SwAntDivRestAfterLink8192C(
	IN	PADAPTER	Adapter
)
{
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(Adapter);
...
//
static VOID dm_SW_AntennaSwitch(
	PADAPTER	Adapter,
	u8			Step
)
{
	...
	if (pDM_SWAT_Table->try_flag == 0xff) {
		...
		if (pMgntInfo->mAssoc && !ACTING_AS_AP(Adapter)) {
			...
			RT_TRACE(COMP_SWAS, DBG_LOUD,
				 ("dm_SW_AntennaSwitch(): RSSI_target is DEF AP!\n"));
		} else {
			u8			index = 0;
			...
			if (pEntry == NULL) {
				...
				RT_TRACE(COMP_SWAS, DBG_LOUD,
					 ("dm_SW_AntennaSwitch(): RSSI_target is PEER STA\n"));
			}
		}
		...
		if (pDM_SWAT_Table->try_flag == 1) {
			...
			nextAntenna = (pDM_SWAT_Table->CurAntenna == Antenna_A) ? Antenna_B :
				      Antenna_A;
			pDM_SWAT_Table->RSSI_Trying--;
			//DBG_8192C("RSSI_Trying = %d\n",pDM_SWAT_Table->RSSI_Trying);

			if (pDM_SWAT_Table->RSSI_Trying == 0) {
				CurByteCnt = (pDM_SWAT_Table->CurAntenna == Antenna_A) ?
					     (pdmpriv->TXByteCnt_A + pdmpriv->RXByteCnt_A) : (pdmpriv->TXByteCnt_B +
							     pdmpriv->RXByteCnt_B);
				PreByteCnt = (pDM_SWAT_Table->CurAntenna == Antenna_A) ?
					     (pdmpriv->TXByteCnt_B + pdmpriv->RXByteCnt_B) : (pdmpriv->TXByteCnt_A +
							     pdmpriv->RXByteCnt_A);
				...
				pDM_SWAT_Table->PreRSSI = (pDM_SWAT_Table->CurAntenna == Antenna_A) ?
							  RSSI_B : RSSI_A;
			...
			if (pHalData->RSSI_cnt_B > 0)
				...
				RSSI_B = 0;
			curRSSI = (pDM_SWAT_Table->CurAntenna == Antenna_A) ? RSSI_A : RSSI_B;
			pDM_SWAT_Table->PreRSSI = (pDM_SWAT_Table->PreAntenna == Antenna_A) ?
						  RSSI_A : RSSI_B;
...
static VOID dm_SW_AntennaSwitchInit(
	IN	PADAPTER	Adapter
)
{
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(Adapter);
...
void rtl8192c_init_dm_priv(IN PADAPTER Adapter)
{
	...
	_init_timer(&(pdmpriv->SwAntennaSwitchTimer),  Adapter->pnetdev,
		    dm_SW_AntennaSwitchCallback, Adapter);
#endif
#endif
}
...
void dm_InitHybridAntDiv(IN PADAPTER Adapter)
{
	...
	PHY_SetBBReg(Adapter, 0x870, BIT9 | BIT8,
		     0); //Enable hardware control of "ANT_SEL" & "ANT_SELB"
	PHY_SetBBReg(Adapter, 0xCA4, BIT11,
		     0); //Switch to another antenna by checking pwdb threshold
	PHY_SetBBReg(Adapter, 0xCA4, 0x7FF, 0x080); //Pwdb threshold=8dB
	PHY_SetBBReg(Adapter, 0xC54, BIT23,
	...
	PHY_SetBBReg(Adapter, 0xA0C, BIT4,
		     0); //Antenna diversity decision period = 32 sample
	PHY_SetBBReg(Adapter, 0xA0C, 0xf,
	...
	PHY_SetBBReg(Adapter, 0xA14, 0x1f,
...
)
{
	...
	for (i = 0 ; i < 32 ; i++) {
		pdmpriv->INIDATA_RATE[i] = rtw_read8(Adapter,
						     REG_INIDATA_RATE_SEL + i) & 0x3f;
	}
...
static void FindMinimumRSSI(PADAPTER Adapter)
{
	...
	if ((check_fwstate(pmlmepriv, WIFI_AP_STATE) == _TRUE
	    ...
	    (check_fwstate(pmlmepriv, WIFI_STATION_STATE)
	     ...
	     && check_buddy_fwstate(Adapter, _FW_LINKED))) {
		if (pdmpriv->UndecoratedSmoothedPWDB >
		    pbuddy_dmpriv->UndecoratedSmoothedPWDB)
	...
	if (check_fwstate(pmlmepriv, WIFI_AP_STATE) == _TRUE
		...
		pbuddy_dmpriv->EntryMinUndecoratedSmoothedPWDB = 0;
		 ...
		 && pbuddy_adapter->stapriv.asoc_sta_count > 2) {
		pdmpriv->EntryMinUndecoratedSmoothedPWDB =
			pbuddy_dmpriv->EntryMinUndecoratedSmoothedPWDB;
