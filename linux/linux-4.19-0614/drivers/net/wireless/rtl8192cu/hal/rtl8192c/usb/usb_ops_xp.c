...
uint usb_init_intf_priv(struct intf_priv *pintfpriv)
{
	...
	_init_timer(&pintfpriv->io_timer, padapter->hndis_adapter,
		    io_irp_timeout_handler, pintfpriv);
	...
	NextDeviceStackSize = (u8)
			      pdev->nextdevstacksz;//pintfpriv->pUsbDevObj->StackSize + 1;
	...
	_rtw_init_sema(&(pintfpriv->io_retevt),
		       0);//NdisInitializeEvent(&pintfpriv->io_irp_return_evt);
...
u32 usb_bulkout_zero(struct intf_hdl *pintfhdl, u32 addr)
{
	...
	pcontext = (struct zero_bulkout_context *)ExAllocatePool(NonPagedPool,
...
NTSTATUS usb_write_mem_complete(PDEVICE_OBJECT	pUsbDevObj,
				PIRP piowrite_irp, PVOID pusb_cnxt)
{
...
void usb_write_mem(struct intf_hdl *pintfhdl, u32 addr, u32 cnt, u8 *wmem)
{
	...
	if ((NtStatus != STATUS_PENDING) || USBD_HALTED(usbdstatus)) {
		...
		if (USBD_HALTED(usbdstatus)) {
			...
			RT_TRACE(_module_hci_ops_os_c_, _drv_err_,
				 ("usb_write_mem():USBD_HALTED(usbdstatus)=%X!\n",
...
NTSTATUS usb_read_port_complete(PDEVICE_OBJECT pUsbDevObj, PIRP pIrp,
				PVOID context)
{
	uint isevt, *pbuf;
	...
	if (precvpriv->rx_pending_cnt == 0) {
		RT_TRACE(_module_hci_ops_os_c_, _drv_err_,
			 ("usb_read_port_complete: rx_pending_cnt== 0, set allrxreturnevt!\n"));
	...
	if (pIrp->Cancel == _TRUE) {
		...
		RT_TRACE(_module_hci_ops_os_c_, _drv_err_,
			 ("usb_read_port_complete: One IRP has been cancelled succesfully\n"));
		return STATUS_MORE_PROCESSING_REQUIRED;
	}
	...
	case STATUS_SUCCESS:
		...
		if ((pbulkurb->TransferBufferLength > (MAX_RECVBUF_SZ))
		    || (pbulkurb->TransferBufferLength < RXDESC_SIZE)) {
			RT_TRACE(_module_hci_ops_os_c_, _drv_err_,
...
u32 usb_read_port(struct intf_hdl *pintfhdl, u32 addr, u32 cnt, u8 *rmem)
{
	...
	if (precvbuf != NULL) {
		...
		if (ntstatus == STATUS_PENDING) {
			...
			RT_TRACE(_module_hci_ops_os_c_, _drv_err_,
				 ("usb_read_port(): IoCallDriver failed!!! IRP STATUS: %X\n", ntstatus));
			RT_TRACE(_module_hci_ops_os_c_, _drv_err_,
				 ("usb_read_port(): IoCallDriver failed!!! USB STATUS: %X\n", usbdstatus));
		}
	...
	if (precvpriv->rx_pending_cnt) {
		...
		for (i = 0; i < NR_RECVBUFF; i++) {
			if (precvbuf->irp_pending == _TRUE) {
				...
				RT_TRACE(_module_hci_ops_os_c_, _drv_err_,
					 ("usb_read_port_cancel() :IoCancelIrp\n"));
			}
...
#if 0
	pxmitframe->fragcnt--;
	if (pxmitframe->fragcnt ==
	    0) { // if((pxmitframe->fragcnt == 0) && (pxmitframe->irpcnt == 8)){
...
//	}
	...
	if (irp == NULL) {
		RT_TRACE(_module_hci_ops_os_c_, _drv_err_,
			 ("SyncCallUSBD: memory alloc for irp failed\n"));
	...
	if (nextstack == NULL)
		RT_TRACE(_module_hci_ops_os_c_, _drv_err_,
			 ("IoGetNextIrpStackLocation fail\n"));
	...
	if (ntstatus == STATUS_PENDING) {
		...
		ntstatus = KeWaitForSingleObject(&kevent, Executive, KernelMode, _FALSE,
						 &waittime); //8150 code
		      ...
		      u16 index, void *pdata, u16 len, u8 requesttype)
{
	...
	if (purb == NULL) {
		...
		RT_TRACE(_module_hci_ops_os_c_, _drv_err_,
			 ("usbctrl_vendorreq(): Failed to allocate urb !!!\n"));
		ret = _FALSE;
		goto exit;
	}
	...
	UsbBuildVendorRequest(
		...
		sizeof(struct
		       _URB_CONTROL_VENDOR_OR_CLASS_REQUEST),  //Specifies the length, in bytes, of the URB.
	...
	if (!NT_SUCCESS(ntstatus)) {
		ExFreePool(purb);
		RT_TRACE(_module_hci_ops_os_c_, _drv_err_,
			 (" usbctrl_vendorreq() : SOMETHING WRONG\n"));
		ret = _FALSE;
		goto exit;
	}
