...
void rtl8192cu_interface_configure(_adapter *padapter)
{
	...
	pHalData->UsbRxAggPageCount	=
		48; //uint :128 b //0x0A;	// 10 = MAX_RX_DMA_BUFFER_SIZE/2/pHalData->UsbBulkOutSize
...
//	For hardware power on sequence.
	...
	*/
	...
	rtw_udelay_os(
		100);//PlatformSleepUs(150);//this is not necessary when initially power on
	...
	if (0 == (value8 & LDV12_EN)) {
		...
		rtw_udelay_os(
			100);//PlatformSleepUs(100);//this is not necessary when initially power on
	...
	if (pHalData->BoardType == BOARD_USB_COMBO) {
		if (pHalData->SlimComboDbg == _TRUE) {
			...
			rtw_write8(padapter, REG_SYS_ISO_CTRL, rtw_read8(padapter,
					REG_SYS_ISO_CTRL) | BIT7);
			...
			rtw_write8(padapter, REG_APS_FSMCO, rtw_read8(padapter,
					REG_APS_FSMCO) | BIT6);
			...
			rtw_write16(padapter, REG_GPIO_MUXCFG, (rtw_read16(padapter,
								REG_GPIO_MUXCFG) & 0x07FF) | BIT14);
			rtw_write16(padapter, REG_GPIO_MUXCFG, rtw_read16(padapter,
					REG_GPIO_MUXCFG) & 0x07FF);
		}
	}
	...
	if (IS_HARDWARE_TYPE_8192C(padapter)
		...
		rtw_write32(padapter, rFPGA0_XCD_RFParameter, rtw_read32(padapter,
				rFPGA0_XCD_RFParameter) & (~BIT1));
	return ret;
...
static VOID _InitQueueReservedPage(
	IN  PADAPTER Adapter
)
{
...
static VOID _InitTxBufferBoundary(
	IN  PADAPTER Adapter
)
{
	struct registry_priv *pregistrypriv = &Adapter->registrypriv;
...
static VOID _InitNormalChipQueuePriority(
	IN	PADAPTER Adapter
)
{
	HAL_DATA_TYPE	*pHalData	= GET_HAL_DATA(Adapter);
...
static VOID _InitQueuePriority(
	IN  PADAPTER Adapter
)
{
...
static VOID _InitHardwareDropIncorrectBulkOut(
	IN  PADAPTER Adapter
)
{
...
static VOID _InitTransferPageSize(
	IN  PADAPTER Adapter
)
{
...
static VOID _InitDriverInfoSize(
	IN  PADAPTER	Adapter,
	IN	u8		drvInfoSize
)
{
...
)
{
	...
	pHalData->ReceiveConfig = RCR_APM | RCR_AM | RCR_AB | RCR_CBSSID_DATA |
				  RCR_CBSSID_BCN | RCR_APP_ICV | RCR_AMF | RCR_HTC_LOC_CTRL | RCR_APP_MIC |
				  RCR_APP_PHYSTS;
...
static VOID _InitBeaconMaxError(
	IN  PADAPTER	Adapter,
...
static VOID _InitRetryFunction(
	IN  PADAPTER Adapter
)
{
	u8	value8;
...
)
{
	...
	if (pHalData->UsbTxAggMode) {
		...
		value32 |= ((pHalData->UsbTxAggDescNum & BLK_DESC_NUM_MASK) <<
			    BLK_DESC_NUM_SHIFT);
...
static VOID InitUsbAggregationSetting(
	IN  PADAPTER Adapter
)
{
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(Adapter);
...
static VOID _InitOperationMode(
	IN	PADAPTER			Adapter
)
{
#if 0//gtest
	...
	PlatformEFIOWrite1Byte(Adapter, REG_AMPDU_MIN_SPACE,
			       Adapter->MgntInfo.MinSpaceCfg);
#endif
}


static VOID _InitBeaconParameters(
	IN  PADAPTER Adapter
)
{
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(Adapter);
...
)
{
	...
	if (pMgntInfo->RegRfOff == TRUE) { // User disable RF via registry.
		RT_TRACE((COMP_INIT | COMP_RF), DBG_LOUD,
			 ("InitializeAdapter8192CUsb(): Turn off RF for RegRfOff.\n"));
		MgntActSet_RF_State(Adapter, eRfOff, RF_CHANGE_BY_SW);
		// Those action will be discard in MgntActSet_RF_State because off the same state
		for (eRFPath = 0; eRFPath < pHalData->NumTotalRFPath; eRFPath++)
			PHY_SetRFReg(Adapter, (RF_RADIO_PATH_E)eRFPath, 0x4, 0xC00, 0x0);
	} else if (pMgntInfo->RfOffReason >
		   RF_CHANGE_BY_PS) { // H/W or S/W RF OFF before sleep.
		RT_TRACE((COMP_INIT | COMP_RF), DBG_LOUD,
			 ("InitializeAdapter8192CUsb(): Turn off RF for RfOffReason(%ld).\n",
		...
		RT_TRACE((COMP_INIT | COMP_RF), DBG_LOUD,
			 ("InitializeAdapter8192CUsb(): RF is on.\n"));
	}
#endif
}
...
static VOID _InitAntenna_Selection(IN	PADAPTER Adapter)
{
	...
	if ((RF_1T1R == pHalData->rf_type)) {
		rtw_write32(Adapter, REG_LEDCFG0, rtw_read32(Adapter,
				REG_LEDCFG0) | BIT23);
		...
		DBG_8192C("%s,Cur_ant:(%x)%s\n", __FUNCTION__, pHalData->CurAntenna,
			  (pHalData->CurAntenna == Antenna_A) ? "Antenna_A" : "Antenna_B");
...
//
static BOOLEAN HalDetectPwrDownMode(
	IN PADAPTER				Adapter
)
{
	u8	tmpvalue;
...
// slim card.
//
static VOID HalDetectSelectiveSuspendMode(
	IN PADAPTER				Adapter
)
{
	u8	tmpvalue;
...
rt_rf_power_state RfOnOffDetect(IN	PADAPTER pAdapter)
{
	...
	if (pAdapter->pwrctrlpriv.bHWPowerdown) {
		...
		rtw_write8(pAdapter, REG_MAC_PINMUX_CFG, rtw_read8(pAdapter,
				REG_MAC_PINMUX_CFG) & ~(BIT3));
...
u32 rtl8192cu_hal_init(PADAPTER Adapter)
{
	...
	for (hal_init_profiling_i = 0; hal_init_profiling_i < HAL_INIT_STAGES_NUM;
...
#define HAL_INIT_PROFILE_TAG(stage) do {} while(0)
	...
	rtw_write16(Adapter, REG_PKT_VO_VI_LIFE_TIME,
		    0x0400);	// unit: 256us. 256ms
	rtw_write16(Adapter, REG_PKT_BE_BK_LIFE_TIME,
		    0x0400);	// unit: 256us. 256ms
	...
	if (IS_VENDOR_UMC_A_CUT(pHalData->VersionID)
	...
	//
	pHalData->RfRegChnlVal[0] = PHY_QueryRFReg(Adapter, (RF_RADIO_PATH_E)0,
				    RF_CHNLBW, bRFRegOffsetMask);
	pHalData->RfRegChnlVal[1] = PHY_QueryRFReg(Adapter, (RF_RADIO_PATH_E)1,
				    RF_CHNLBW, bRFRegOffsetMask);
...
//_RfPowerSave(Adapter);
	...
	pwrctrlpriv->rfoff_reason |= eRfPowerStateToSet == rf_on ?
				     RF_CHANGE_BY_INIT : RF_CHANGE_BY_HW;
	pwrctrlpriv->rfoff_reason |= (pwrctrlpriv->reg_rfoff) ? RF_CHANGE_BY_SW :
				     0;
	...
	if (pwrctrlpriv->reg_rfoff == _TRUE) {
		...
		if (pHalData->BoardType ==
		    BOARD_MINICARD /*&& (Adapter->MgntInfo.PowerSaveControl.bGpioRfSw)*/) {
...
//fixed USB interface interference issue
	...
	if (!IS_VENDOR_UMC_A_CUT(
		    pHalData->VersionID)) { //&& !IS_92C_SERIAL(pHalData->VersionID))// TSMC , 8188
		// 0xE6=0x94
	...
	rtw_write32(Adapter, REG_FWHW_TXQ_CTRL, rtw_read32(Adapter,
...
exit:
	...
	for (hal_init_profiling_i = 0;
	     hal_init_profiling_i < HAL_INIT_STAGES_NUM - 1; hal_init_profiling_i++) {
...
)
{
	...
	case rf_off:
		...
		if (bRegSSPwrLvl == 1) {
			...
			Adapter->pwrctrlpriv.PS_BBRegBackup[PSBBREG_RF0] = PHY_QueryBBReg(Adapter,
					rFPGA0_XAB_RFParameter, bMaskDWord);
			Adapter->pwrctrlpriv.PS_BBRegBackup[PSBBREG_RF1] = PHY_QueryBBReg(Adapter,
					rOFDM0_TRxPathEnable, bMaskDWord);
			Adapter->pwrctrlpriv.PS_BBRegBackup[PSBBREG_RF2] = PHY_QueryBBReg(Adapter,
					rFPGA0_RFMOD, bMaskDWord);
			if (pHalData->rf_type ==  RF_2T2R) {
			...
			Adapter->pwrctrlpriv.PS_BBRegBackup[PSBBREG_AFE0] = PHY_QueryBBReg(Adapter,
					rRx_Wait_CCA, bMaskDWord);
			if (pHalData->rf_type ==  RF_2T2R)
...
static VOID _DisableMAC_AFE_PLL(
	IN PADAPTER			Adapter
)
{
	u32	value32;
...
static VOID _AutoPowerDownToHostOff(
	IN	PADAPTER		Adapter
)
{
	u32			value32;
...
static VOID _DisableRFAFEAndResetBB(
	IN PADAPTER			Adapter
)
{
	/**************************************
	...
	if (pHalData->FirmwareVersion <=  0x20) {
		...
		******************************/
		...
		rtw_write16(Adapter, REG_SYS_FUNC_EN,
			    (valu16 & (~FEN_CPUEN)));//reset MCU ,8051
		...
		valu16 = rtw_read16(Adapter, REG_SYS_FUNC_EN);
		rtw_write16(Adapter, REG_SYS_FUNC_EN,
			    (valu16 | FEN_CPUEN));//enable MCU ,8051
		...
		if (rtw_read8(Adapter, REG_MCUFWDL) & BIT1) {
			...
			if (Adapter->bFWReady) {
				...
				while ((retry_cnts++ < 100)
				       && (FEN_CPUEN & rtw_read16(Adapter, REG_SYS_FUNC_EN))) {
...
static VOID _ResetDigitalProcedure2(
	IN 	PADAPTER			Adapter
)
{
	/*****************************
...
static int CardDisableHWSM( // HW Auto state machine
	IN	PADAPTER		Adapter,
	IN	BOOLEAN			resetMCU
)
{
...
static int CardDisableWithoutHWSM( // without HW Auto state machine
	IN	PADAPTER		Adapter
)
{
	int		rtStatus = _SUCCESS;
...
unsigned int rtl8192cu_inirp_init(PADAPTER Adapter)
{
	...
	for (i = 0; i < NR_RECVBUFF; i++) {
		if (_read_port(pintfhdl, precvpriv->ff_hwaddr, 0,
			       (unsigned char *)precvbuf) == _FALSE) {
			RT_TRACE(_module_hci_hal_init_c_, _drv_err_,
...
static VOID _ReadPowerValueFromPROM(
	...
	for (group = 0 ; group < CHANNEL_GROUP_MAX ; group++) {
		for (rfPath = 0 ; rfPath < RF_PATH_MAX ; rfPath++) {
			pwrInfo->HT40_2SIndexDiff[rfPath][group] =
				(PROMContent[EEPROM_HT40_2S_TX_PWR_INX_DIFF + group] >>
				 (rfPath * 4)) & 0xF;

#if 1
			pwrInfo->HT20IndexDiff[rfPath][group] =
				(PROMContent[EEPROM_HT20_TX_PWR_INX_DIFF + group] >> (rfPath * 4)) & 0xF;
			if (pwrInfo->HT20IndexDiff[rfPath][group] &
...
)
{
	...
	if ((pHalData->EEPROMVID == 0x103C)
	    && (pHalData->EEPROMVID == 0x1629)) // HP Lite-On for RTL8188CUS Slim Combo.
...
)
{
	...
	pHalData->bLedOpenDrain =
		_TRUE;// Support Open-drain arrangement for controlling the LED. Added by Roger, 2009.10.16.
#else // HW LED
	pledpriv->LedStrategy = HW_LED;
#endif //CONFIG_SW_LED
}

static VOID _ReadThermalMeter(
	IN	PADAPTER	Adapter,
	IN	u8 	*PROMContent,
	IN	BOOLEAN 	AutoloadFail
)
...
#if 0
	if (pHalData->EEPROMThermalMeter < 0x06
	    || pHalData->EEPROMThermalMeter > 0x1c)
...
static VOID readAntennaDiversity(
	IN	PADAPTER	pAdapter,
	IN	u8			*hwinfo,
	IN	BOOLEAN		AutoLoadFail
)
...
)
{
	...
	if (_FALSE == pEEPROM->bautoload_fail_flag) {
		...
		if (_TRUE == pEEPROM->EepromOrEfuse) {
			...
			_rtw_memcpy((void *)PROMContent, (void *)pEEPROM->efuse_eeprom_data,
				    HWSET_MAX_SIZE);
		}
	} else {
		//autoload fail
...
static void _ReadPSSetting(IN PADAPTER Adapter, IN u8 *PROMContent,
			   IN u8	AutoloadFail)
{
	if (AutoloadFail) {
		...
		Adapter->pwrctrlpriv.bSupportRemoteWakeup =
			(PROMContent[EEPROM_TEST_USB_OPT] & BIT1) ? _TRUE : _FALSE;
#endif //CONFIG_WOWLAN
		...
		DBG_8192C("%s...bHWPwrPindetect(%x)-bHWPowerdown(%x) ,bSupportRemoteWakeup(%x)\n",
			  __FUNCTION__,
			  Adapter->pwrctrlpriv.bHWPwrPindetect, Adapter->pwrctrlpriv.bHWPowerdown,
			  Adapter->pwrctrlpriv.bSupportRemoteWakeup);
...
static VOID readAdapterInfo_8192CU(
	IN	PADAPTER	Adapter
)
{
	EEPROM_EFUSE_PRIV *pEEPROM = GET_EEPROM_EFUSE_PRIV(Adapter);
	...
	//
	rtl8192c_ReadBluetoothCoexistInfo(Adapter, PROMContent,
					  pEEPROM->bautoload_fail_flag);
#endif

	rtl8192c_EfuseParseChnlPlan(Adapter, PROMContent,
				    pEEPROM->bautoload_fail_flag);
...
static VOID _InitOtherVariable(
	IN PADAPTER 		Adapter
)
{
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(Adapter);
...
static void rtl8192cu_trigger_gpio_0(_adapter *padapter)
{
	...
	gpioctrl = (BIT(GPIO_DEBUG_PORT_NUM) << 24) | (BIT(GPIO_DEBUG_PORT_NUM) <<
			16);
	...
	if (Adapter->iface_type == IFACE_PORT1) {
		...
		if ((mode == _HW_STATE_STATION_) || (mode == _HW_STATE_NOLINK_)) {
			...
			rtw_write8(Adapter, REG_BCN_CTRL_1,
				   (DIS_TSF_UDT0_NORMAL_CHIP | EN_BCN_FUNCTION | EN_TXBCN_RPT | BIT(1)));
			...
			DBG_871X("%s()-%d: REG_BCN_CTRL_1 = %02x\n", __FUNCTION__, __LINE__,
				 rtw_read8(Adapter, REG_BCN_CTRL_1));
...
static void hw_var_set_bcn_func(PADAPTER Adapter, u8 variable, u8 *val)
{
...
#endif
	{
		...
		if (*((u8 *)val)) {
			...
			rtw_write8(Adapter, bcn_ctrl_reg, (rtw_read8(Adapter,
							   bcn_ctrl_reg) & (~(EN_TXBCN_RPT))) | DIS_TSF_UDT0_NORMAL_CHIP);
		}
	}
...
static int rtw_wowlan_set_pattern(_adapter *padapter, u8 *pbuf)
{
	...
	u32 content = 0, cmd = 0;
	u32 *pdata;
	u8 config, crc, mc, bc, uc, idx, pattern_len, packet[200], packet_len,
	valid;
	u16 crc_val = 0, i;
...
void rtw_wowlan_reload_pattern(_adapter *padapter)
{
	...
	for (idx = 0; idx < 8; idx ++) {
		if (pwrpriv->wowlan_pattern_idx & BIT(idx)) {
			//offset 0
			rtw_write32(padapter, REG_WKFMCAM_RWD,
				    pwrpriv->wowlan_pattern_context[idx][0]);
			cmd = BIT(31) | BIT(16) | (idx + 0);
			rtw_write32(padapter, REG_WKFMCAM_CMD, cmd);

			//offset 4
			rtw_write32(padapter, REG_WKFMCAM_RWD,
				    pwrpriv->wowlan_pattern_context[idx][1]);
			cmd = BIT(31) | BIT(16) | (idx + 1);
			rtw_write32(padapter, REG_WKFMCAM_CMD, cmd);

			//offset 8
			rtw_write32(padapter, REG_WKFMCAM_RWD,
				    pwrpriv->wowlan_pattern_context[idx][2]);
			cmd = BIT(31) | BIT(16) | (idx + 2);
			rtw_write32(padapter, REG_WKFMCAM_CMD, cmd);

			//offset 12
			rtw_write32(padapter, REG_WKFMCAM_RWD,
				    pwrpriv->wowlan_pattern_context[idx][3]);
			cmd = BIT(31) | BIT(16) | (idx + 3);
			rtw_write32(padapter, REG_WKFMCAM_CMD, cmd);

			//offset 16
			rtw_write32(padapter, REG_WKFMCAM_RWD,
				    pwrpriv->wowlan_pattern_context[idx][4]);
			cmd = BIT(31) | BIT(16) | (idx + 4);
			rtw_write32(padapter, REG_WKFMCAM_CMD, cmd);
	...
	if (Adapter->iface_type == IFACE_PORT1) {
		...
		*/
		...
		for (i = 0; i < 10; i++) {
			...
			DBG_871X("%s()-%d: reg_bcn_disable_cnt=%02x\n", __FUNCTION__, __LINE__,
				 rtw_read8(Adapter, REG_FW_BCN_DIS_CNT));
			DBG_871X("%s()-%d: REG_BCN_CTRL=%02x\n", __FUNCTION__, __LINE__,
				 rtw_read8(Adapter, REG_BCN_CTRL));
			DBG_871X("%s()-%d: FWISR=%08x\n", __FUNCTION__, __LINE__,
				 rtw_read32(Adapter, REG_FWISR));
			rtw_msleep_os(100);
		}
		DBG_871X("%s()-%d: reg_bcn_disable_cnt=%02x\n", __FUNCTION__, __LINE__,
			 rtw_read8(Adapter, REG_FW_BCN_DIS_CNT));
...
void SetHwReg8192CU(PADAPTER Adapter, u8 variable, u8 *val)
{
	...
	case HW_VAR_ON_RCR_AM:
		...
		DBG_871X("%s, %d, RCR= %x \n", __FUNCTION__, __LINE__, rtw_read32(Adapter,
				REG_RCR));
		break;
	...
	case HW_VAR_SEC_CFG:
#ifdef CONFIG_CONCURRENT_MODE
		rtw_write8(Adapter, REG_SECCFG,
			   0x0c | BIT(5)); // enable tx enc and rx dec engine, and no key search for MC/BC
#else
	...
	case HW_VAR_AMPDU_MIN_SPACE: {
		...
		if (MinSpacingToSet <= 7) {
			...
			rtw_write8(Adapter, REG_AMPDU_MIN_SPACE, (rtw_read8(Adapter,
					REG_AMPDU_MIN_SPACE) & 0xf8) | MinSpacingToSet);
		}
	}
	break;
...
#define RXDMA_IDLE			BIT17
		...
		if (pwrpriv->bkeepfwalive != _TRUE) {
			//RX DMA stop
			rtw_write32(Adapter, REG_RXPKT_NUM, (rtw_read32(Adapter,
							     REG_RXPKT_NUM) | RW_RELEASE_EN));
			do {
		...
		case WOWLAN_PATTERN_MATCH:
			...
			if (poidparam->subcode_value == 1) {
				...
				DBG_8192C("%s Adapter->pwrctrlpriv.wowlan_pattern=%x\n", __FUNCTION__,
					  Adapter->pwrctrlpriv.wowlan_pattern);
			} else {
				//rtw_write8(Adapter, REG_WOW_CTRL, (rtw_read8(Adapter, REG_WOW_CTRL)&~BIT(1)));
		...
		case WOWLAN_MAGIC_PACKET:
			...
			if (poidparam->subcode_value == 1) {
				...
				DBG_8192C("%s Adapter->pwrctrlpriv.wowlan_magic=%x\n", __FUNCTION__,
					  Adapter->pwrctrlpriv.wowlan_magic);
			} else {
				//rtw_write8(Adapter, REG_WOW_CTRL, (rtw_read8(Adapter, REG_WOW_CTRL)&~BIT(2)));
		...
		case WOWLAN_UNICAST:
			...
			if (poidparam->subcode_value == 1) {
				...
				DBG_8192C("%s Adapter->pwrctrlpriv.wowlan_unicast=%x\n", __FUNCTION__,
					  Adapter->pwrctrlpriv.wowlan_unicast);
			}
			break;
...
//							rtw_write8(Adapter, REG_WOW_CTRL, (rtw_read8(Adapter, REG_WOW_CTRL)|BIT(1)|BIT(2)|BIT(3)));
		...
		if (Adapter->pwrctrlpriv.wowlan_unicast
		    || Adapter->pwrctrlpriv.wowlan_magic
		    || Adapter->pwrctrlpriv.wowlan_pattern)
	...
	case HW_VAR_BCN_VALID:
		...
		rtw_write8(Adapter, REG_TDECTRL + 2, rtw_read8(Adapter,
				REG_TDECTRL + 2) | BIT0);
		break;
...
#ifdef CONFIG_ANTENNA_DIVERSITY
		*((u8 *)pValue) = (IS_92C_SERIAL(pHalData->VersionID)
				   || (pHalData->AntDivCfg == 0)) ? _FALSE : _TRUE;
#endif
		break;
...
void UpdateHalRAMask8192CUsb(PADAPTER padapter, u32 mac_id)
{
	...
	case 2:// first station uses macid=0, second station uses macid=2
		...
		mask |= (pmlmeinfo->HT_enable) ? update_MSC_rate(&(pmlmeinfo->HT_caps)) :
			0;
...
static u8 rtl8192cu_ps_func(PADAPTER Adapter, HAL_INTF_PS_FUNC efunc_id,
			    u8 *val)
{
	u8 bResult = _TRUE;
	switch (efunc_id) {
	...
	case HAL_USB_SELECT_SUSPEND: {
		u8 bfwpoll = *((u8 *)val);
		rtl8192c_set_FwSelectSuspend_cmd(Adapter, bfwpoll,
						 500); //note fw to support hw power down ping detect
	}
	break;
