...
static int rtw_cfg80211_inform_bss(_adapter *padapter,
	...
	if (wdev_to_priv(wdev)->scan_request != NULL) {
		...
		if (sr != 0) {
			if (request->n_ssids == 1
			    && request->n_channels == 1) { // it means under processing WPS
				...
				if (pssid->SsidLength == ssids[0].ssid_len &&
					...
					if (pselect_network->Configuration.DSConfig <= RTW_CH_MAX_2G_CHANNEL)
						freq = rtw_ieee80211_channel_to_frequency(
							       pselect_network->Configuration.DSConfig, IEEE80211_BAND_2GHZ);
					else
						freq = rtw_ieee80211_channel_to_frequency(
							       pselect_network->Configuration.DSConfig, IEEE80211_BAND_5GHZ);
...
int rtw_cfg80211_check_bss(_adapter *padapter)
{
	...
	if (pnetwork->Configuration.DSConfig <= RTW_CH_MAX_2G_CHANNEL)
		freq = rtw_ieee80211_channel_to_frequency(pnetwork->Configuration.DSConfig,
				IEEE80211_BAND_2GHZ);
	else
		freq = rtw_ieee80211_channel_to_frequency(pnetwork->Configuration.DSConfig,
				IEEE80211_BAND_5GHZ);
				    ...
				    u8 *mac, struct station_info *sinfo)
{
	...
	if (check_fwstate(pmlmepriv, WIFI_STATION_STATE)
		...
		if (_rtw_memcmp(mac, cur_network->network.MacAddress,
				ETH_ALEN) == _FALSE) {
...
//#endif
	...
	if (pwdinfo->driver_interface == DRIVER_CFG80211) {
		if (ssids->ssid != NULL
			...
			if (rtw_p2p_chk_state(pwdinfo, P2P_STATE_NONE)) {
				...
				padapter->HalFunc.SetHwRegHandler(padapter, HW_VAR_INITIAL_GAIN,
								  (u8 *) & (initialgain));
				padapter->HalFunc.SetHwRegHandler(padapter, HW_VAR_INITIAL_GAIN,
								  (u8 *) & (initialgain));
			} else {
...
#ifdef CONFIG_CONCURRENT_MODE
	if (pbuddy_mlmepriv
	    && (pbuddy_mlmepriv->LinkDetectInfo.bBusyTraffic == _TRUE)) {
	...
	if (request->n_channels == 1) {
		...
		_status = rtw_sitesurvey_cmd(padapter, ssid, RTW_SSID_SCAN_AMOUNT, NULL,
					     0);
	}
	_exit_critical_bh(&pmlmepriv->lock, &irqL);
				   ...
				   size_t ielen)
{
	...
	{/* handle wps_ie */
		...
		if (wps_ie && wps_ielen > 0) {
			...
			padapter->securitypriv.wps_ie_len = wps_ielen < MAX_WPS_IE_LEN ? wps_ielen
							    : MAX_WPS_IE_LEN;
				  ...
				  struct cfg80211_pmksa *pmksa)
{
	...
	for (index = 0 ; index < NUM_PMKID_CACHE; index++) {
		if (_rtw_memcmp(psecuritypriv->PMKIDList[index].Bssid, pmksa->bssid,
				ETH_ALEN) == _TRUE) {
			// BSSID is matched, the same AP => rewrite with new PMKID.
			DBG_871X(FUNC_NDEV_FMT" BSSID exists in the PMKList.\n",
				 FUNC_NDEV_ARG(netdev));

			_rtw_memcpy(psecuritypriv->PMKIDList[index].PMKID, pmksa->pmkid,
				    WLAN_PMKID_LEN);
...
{
	struct rtw_wdev_priv *pwdev_priv = (struct rtw_wdev_priv *)wiphy_priv(
			wiphy);
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,6,0))
	struct net_device *ndev;
...
void rtw_cfg80211_rx_action_p2p(_adapter *padapter, u8 *pmgmt_frame,
				uint frame_len)
{
	int type;
	s32 freq;
...
indicate:
	...
	rtw_cfg80211_rx_mgmt(padapter, freq, 0, pmgmt_frame, frame_len,
			     GFP_ATOMIC);
#else
	cfg80211_rx_action(padapter->pnetdev, freq, pmgmt_frame, frame_len,
			   GFP_ATOMIC);
#endif
}

void rtw_cfg80211_rx_p2p_action_public(_adapter *padapter, u8 *pmgmt_frame,
				       uint frame_len)
{
	int type;
	s32 freq;
...
void rtw_cfg80211_rx_action(_adapter *adapter, u8 *frame, uint frame_len,
			    const char *msg)
{
	s32 freq;
	int channel;
		...
		if (ready_on_channel == _TRUE) {
...
#endif
			{
				if (!padapter->mlmepriv.LinkDetectInfo.bBusyTraffic)
					set_channel_bwmode(padapter, remain_ch, HAL_PRIME_CHNL_OFFSET_DONT_CARE,
							   HT_CHANNEL_WIDTH_20);
			}
		}
...
static int _cfg80211_rtw_mgmt_tx(_adapter *padapter, u8 tx_ch,
				 const u8 *buf, size_t len)
{
	struct xmit_frame	*pmgntframe;
	struct pkt_attrib	*pattrib;
		...
		char *buf, int len)
{
	...
	if (len > 0) {
		if ((wps_ie = rtw_get_wps_ie(buf, len, NULL, &wps_ielen))) {
			...
			if ((puconfig_method = (u16 *)rtw_get_wps_attr_content(wps_ie, wps_ielen,
					       WPS_ATTR_CONF_METHOD, NULL, &attr_contentlen)) != NULL) {
#ifdef CONFIG_DEBUG_CFG80211
...
static void rtw_cfg80211_preinit_wiphy(_adapter *padapter,
				       struct wiphy *wiphy)
{
	...
	*/
	...
	/* if (padapter->registrypriv.wireless_mode & WIRELESS_11G) */
	wiphy->bands[IEEE80211_BAND_2GHZ] = rtw_spt_band_alloc(
			IEEE80211_BAND_2GHZ);
	/* if (padapter->registrypriv.wireless_mode & WIRELESS_11A) */
	wiphy->bands[IEEE80211_BAND_5GHZ] = rtw_spt_band_alloc(
			IEEE80211_BAND_5GHZ);
...
int rtw_wdev_alloc(_adapter *padapter, struct device *dev)
{
	...
	wdev->iftype =
		NL80211_IFTYPE_MONITOR; // for rtw_setopmode_cmd() in cfg80211_rtw_change_iface()
