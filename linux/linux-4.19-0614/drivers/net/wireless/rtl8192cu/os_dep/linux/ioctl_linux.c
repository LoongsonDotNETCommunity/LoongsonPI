			  ...
			  unsigned short ielen)
{
	...
	if (ielen) {
		...
		if (rtw_parse_wpa_ie(buf, ielen, &group_cipher,
		...
		if (rtw_parse_wpa2_ie(buf, ielen, &group_cipher,
				      &pairwise_cipher) == _SUCCESS) {
			padapter->securitypriv.dot11AuthAlgrthm = dot11AuthAlgrthm_8021X;
		...
		case 0x05: { //blink LED test
...

			{
				...
				if (_SUCCESS != rtw_IOL_exec_cmds_sync(padapter, xmit_frame,
								       (blink_delay_ms * blink_num * 2) + 200))
					ret = -EPERM;
			}
		}
		break;
			    ...
			    union iwreq_data *wdata, char *extra)
{
	...
	u16 addr = 0, cnts = 0, max_available_size = 0, raw_cursize = 0,
	    raw_maxsize = 0;
	...
	if (strcmp(tmp[0], "realmap") == 0) {
		...
		sprintf(extra, "%s : [ available raw size] = %d", extra,
	...
	do {
		...
		if ((ploopback->bstop == _TRUE) ||
			...
			sprintf(ploopback->msg,
				"loopback result: ok=%d%%(%d/%d),error=%d%%(%d/%d)", ok_rate, ok, cnt,
				fail_rate, fail, cnt);
			break;
		}
	} while (1);
