...
static s32 rnpvf_stop_hw_vf(struct rnpvf_hw *hw)
{
	...
	for (i = 0; i < adapter->num_tx_queues; i++) {
		...
		wr32(hw, RNP_DMA_TX_START(ring->rnpvf_queue_idx), 0);
	}
	*/
...
u8 *rnpvf_addr_list_itr(struct rnpvf_hw __maybe_unused *hw, u8 **mc_addr_ptr)
{
					...
					struct net_device *netdev)
{
        ...
        for (i = 0; i < addr_count; i++) {
                vector_list[i++] = rnpvf_mta_vector(hw, rnpvf_addr_list_itr(hw, &addr_list));
        }
