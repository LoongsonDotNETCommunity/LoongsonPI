...
s32 rnpm_read_pba_string_generic(struct rnpm_hw *hw,
								 u8 *pba_num,
								 u32 pba_num_size);
...
s32 rnpm_read_eerd_buffer_generic(struct rnpm_hw *hw,
								  u16 offset,
								  u16 words,
								  u16 *data);
s32 rnpm_write_eewr_generic(struct rnpm_hw *hw, u16 offset, u16 data);
s32 rnpm_write_eewr_buffer_generic(struct rnpm_hw *hw,
								   u16 offset,
								   u16 words,
								   u16 *data);
s32 rnpm_read_eeprom_bit_bang_generic(struct rnpm_hw *hw,
									  u16 offset,
									  u16 *data);
...
s32 rnpm_set_rar_generic(
	struct rnpm_hw *hw, u32 index, u8 *addr, u32 vmdq, u32 enable_addr);
s32 rnpm_set_rar_mac(
	struct rnpm_hw *hw, u32 index, u8 *addr, u32 vmdq, u32 port);
...
s32 rnpm_device_supports_autoneg_fc(struct rnpm_hw *hw);
...
s32 rnpm_set_vfta_mac_generic(struct rnpm_hw *hw,
							  u32 vlan,
							  u32 vind,
							  bool vlan_on);
s32 rnpm_clear_vfta_generic(struct rnpm_hw *hw);
s32 rnpm_check_mac_link_generic(struct rnpm_hw *hw,
								rnpm_link_speed *speed,
								bool *link_up,
								bool link_up_wait_to_complete);
s32 rnpm_get_wwn_prefix_generic(struct rnpm_hw *hw,
								u16 *wwnn_prefix,
								u16 *wwpn_prefix);
...
s32 rnpm_set_fw_drv_ver_generic(
	struct rnpm_hw *hw, u8 maj, u8 min, u8 build, u8 ver);
...
#else
/* for debug software */
static inline unsigned int rd32(void *hw, u32 off)
{
	u32 value = 0;

	switch (off) {
		case rnpm_DMA_STATUS:
			value = 16 << 24;
			break;
		case rnpm_TOP_MAC_OUI:
			value = 0xb49691;
			break;
		case rnpm_TOP_MAC_SN:
			value = 0xb2a348;
			break;
		case rnpm_TOP_NIC_VERSION:
			value = 0x20210707;
			break;
		default:
			value = 0;
	...
	printk(KERN_DEBUG "readreg %x--> %x\n", off, value);
	return value;
}
#define wr32(hw, off, val)                                  \
	do {                                                    \
		;                                                   \
		printk(KERN_DEBUG "writereg %x<-- %x\n", off, val); \
		while (0)

#define ring_rd32(ring, off)	  rd32((ring)->dma_hw_addr, off)
#define ring_wr32(ring, off, val) wr32((ring)->dma_hw_addr, off, val)
#endif
...
static inline void
hw_queue_strip_rx_vlan(struct rnpm_hw *hw, u8 ring_num, bool enable)
{
...
#define DPRINTK(nlevel, klevel, fmt, args...)                                 \
	((NETIF_MSG_##nlevel & adapter->msg_enable)                               \
		 ? (void)(netdev_printk(KERN_##klevel, adapter->netdev, fmt, ##args)) \
		 : NULL)
...
static inline void buf_dump_line(const char *msg, int line, void *buf, int len)
{
	...
	offset += snprintf(msg_buf + offset,
					   msg_len,
					   "=== %s #%d line:%d buf:%p==\n000: ",
					   msg,
					   len,
...
static inline void buf_dump(const char *msg, void *buf, int len)
{
	...
	offset +=
		snprintf(msg_buf + offset, msg_len, "=== %s #%d ==\n000: ", msg, len);
...
#define rx_dbg(fmt, args...) \
	printk(KERN_DEBUG "KERN_DEBUG [ %s:%d ] " fmt, __func__, __LINE__, ##args)
...
#define desc_hex_dump(msg, buf, len) \
	print_hex_dump(                  \
		KERN_DEBUG, msg, DUMP_PREFIX_OFFSET, 16, 1, (buf), (len), false)
...
static inline int rnpm_recalculate_err_pkts(u64 now, u64 init)
{
	if (now >= init) {
		return now - init;
	} else {
		return (u64)-1 - init + now;
	}

	return 0;
}

#define rnpm_logd_if(bit, input...)   \
	if ((1 << (bit)) & rnpm_loglevel) \
	##input
