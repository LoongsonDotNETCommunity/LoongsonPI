...
static s32 rnpgbe_check_for_msg_pf(struct rnpgbe_hw *hw, enum MBX_ID mbx_id)
{
	...
	if (mbx_id == MBX_CM3CPU) {
		...
		if (mbx->mbx_feature & MBX_FEATURE_NO_ZERO) {
			//printk("hw %d , sw %d \n", rnpgbe_mbx_get_req(hw, CPU2PF_COUNTER(mbx)), hw->mbx.cpu_req);
			if ((hw_req_count != 0) &&
			       ...
			       enum MBX_ID mbx_id)
{
	...
	if (ret_val) {
		printk(KERN_DEBUG "%s: get mbx:%d wlock failed. ret:%d. req:0x%08x-0x%08x\n",
...
static void rnpgbe_mbx_reset(struct rnpgbe_hw *hw)
{
	...
	if (PF_VF_MBOX_MASK_LO(mbx))
		wr32(hw, PF_VF_MBOX_MASK_LO(mbx), 0); // allow vf to vectors
	if (PF_VF_MBOX_MASK_HI(mbx))
		wr32(hw, PF_VF_MBOX_MASK_HI(mbx), 0); // enable irq
				   ...
				   bool enable)
{
	...
	if (enable) {
		...
		wr32(hw, CPU2PF_MBOX_VEC(mbx),
		     nr_vec); // cm3 and VF63 share #63 irq
		// allow CM3CPU to PF MBX IRQ
		wr32(hw, CPU_PF_MBOX_MASK(mbx), 0);
