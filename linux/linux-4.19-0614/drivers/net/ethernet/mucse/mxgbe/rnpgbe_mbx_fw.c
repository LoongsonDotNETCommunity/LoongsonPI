...
int wait_mbx_init_done(struct rnpgbe_hw *hw)
{
	...
	while (count) {
		...
		printk(KERN_DEBUG "waiting fw up\n");
		count--;
	}
	printk(KERN_DEBUG "fw init ok %x\n", v);
...
rnpgbe_nic_mode_convert_to_adapter_cnt(struct phy_abilities *ability)
{
	int adapter_cnt = 0;
