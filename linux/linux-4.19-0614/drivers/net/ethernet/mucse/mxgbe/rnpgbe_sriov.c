			   ...
			   enum PF_STATUS status)
{
	...
	for (vf = 0; vf < adapter->num_vfs; vf++) {
		...
		if (!test_bit(__RNP_IN_IRQ, &adapter->state)) {
			if (test_and_set_bit(__VF_MBX_USED, &adapter->vfinfo[vf]
						.status)) {
				adapter->miss_time++;
				printk(KERN_DEBUG "send miss \n");
				return -1;
			}
			err |= rnpgbe_msg_post_status_signle(adapter,
					status, vf);
			clear_bit(__VF_MBX_USED, &adapter->vfinfo[vf].status);
		}
	}
...
#endif /* HAVE_NDO_SET_VF_MIN_MAX_TX_RATE */
{
	...
	// n500 only 1 ring
	//vf_ring = rnpgbe_get_vf_ringnum(hw, vf, 1);
	//rnpgbe_setup_ring_maxrate(adapter, vf_ring, real_rate);
	return 0;
}
...
__maybe_unused static int rnpgbe_link_mbps(struct rnpgbe_adapter *adapter)
{
	switch (adapter->link_speed) {
	case RNP_LINK_SPEED_100_FULL:
		return 100;
