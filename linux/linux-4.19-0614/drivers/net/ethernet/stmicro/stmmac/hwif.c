...
static void stmmac_dwmac_mode_quirk(struct stmmac_priv *priv)
{
	...
	if (priv->chain_mode) {
		...
		mac->mode = priv->plat->dma_cfg->dma64 ? &chain_mode64_ops : &chain_mode_ops;
	} else {
		dev_info(priv->device, "Ring mode enabled\n");
		priv->mode = STMMAC_RING_MODE;
		mac->mode = priv->plat->dma_cfg->dma64 ? &ring_mode64_ops : &ring_mode_ops;
	}
}
...
static int stmmac_dwmac1_quirks(struct stmmac_priv *priv)
{
	...
	if (priv->plat->enh_desc) {
		...
		if (readl(priv->ioaddr + GMAC_VERSION) & 0x00008000) {
			priv->extend_desc = 1;
			priv->plat->dma_cfg->dma64 = true;
