				       ...
				       struct ttm_mem_reg *mem)
{
	...
	DRM_DEBUG_DRIVER("mem->num_pages=%lu\n", mem->num_pages);

	switch (mem->mem_type) {
	case TTM_PL_SYSTEM:
		/* system memory */
		DRM_DEBUG_DRIVER("ttm pl system memory\n");
		return 0;
	case TTM_PL_VRAM:
		if (ldev->gpu_pdev)
...
int loongson_bo_create2(struct drm_device *dev,
			int size,
			int align,
			uint32_t flags,
			struct sg_table *sg,
			...
			struct loongson_bo **pplbo)
{
	...
	lbo = kzalloc(sizeof(struct loongson_bo), GFP_KERNEL);
	if (!lbo)
		return -ENOMEM;

	ret = drm_gem_object_init(dev, &lbo->gem, size);
	if (ret) {
		kfree(lbo);
		return ret;
	}

	lbo->bo.bdev = &ldev->ttm.bdev;
	loongson_ttm_placement(lbo, TTM_PL_FLAG_VRAM | TTM_PL_FLAG_SYSTEM);

	acc_size = ttm_bo_dma_acc_size(&ldev->ttm.bdev,
				       size,
				       sizeof(struct loongson_bo));
...
unsigned long loongson_bo_size(struct loongson_bo *lbo)
{
...
int loongson_bo_push_sysram(struct loongson_bo *bo)
{
	...
	DRM_DEBUG_DRIVER("push the previous fb to system RAM");
...
int loongson_drm_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	DRM_DEBUG_DRIVER("loongson_drm_mmap()");
