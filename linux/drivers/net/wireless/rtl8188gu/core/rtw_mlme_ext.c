...
bool rtw_choose_shortest_waiting_ch(_adapter *adapter, u8 req_bw,
...
void init_mlme_ext_timer(_adapter *padapter)
{
	...
	rtw_init_timer(&pmlmeext->ft_link_timer, padapter, ft_link_timer_hdl,
		       padapter);
	rtw_init_timer(&pmlmeext->ft_roam_timer, padapter, ft_roam_timer_hdl,
		       padapter);
#endif
			  ...
			  union recv_frame *precv_frame)
{
	...
	if (go2asoc) {
		...
		if ((rtw_to_roam(padapter) > 0)
		    && rtw_chk_ft_flags(padapter, RTW_FT_SUPPORTED)) {
			...
			rtw_buf_update(&pmlmepriv->auth_rsp, &pmlmepriv->auth_rsp_len, pframe,
				       pkt_len);
			pftpriv->ft_event.ies = pmlmepriv->auth_rsp + sizeof(
							struct rtw_ieee80211_hdr_3addr) + 6;
			pftpriv->ft_event.ies_len = pmlmepriv->auth_rsp_len - sizeof(
							    struct rtw_ieee80211_hdr_3addr) - 6;
...
unsigned int OnAction_ft(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	case RTW_WLAN_ACTION_FT_RESPONSE:
		...
		_rtw_memcpy(pftpriv->ft_action, pframe, rtw_min(frame_len,
				RTW_MAX_FTIE_SZ));
		ret = _SUCCESS;
		break;
...
int _issue_probereq(_adapter *padapter, NDIS_802_11_SSID *pssid, u8 *da,
		    u8 ch, bool append_wps, int wait_ack)
{
	int ret = _FAIL;
	struct xmit_frame		*pmgntframe;
	struct pkt_attrib		*pattrib;
...
inline void issue_probereq(_adapter *padapter, NDIS_802_11_SSID *pssid,
			   u8 *da)
{
	...
	*(fctrl) = 0;
	...
	if (psta) { /* for AP mode */
		...
		if (is_ft_roaming == _TRUE) {
			pie = rtw_get_ie(pftpriv->updated_ft_ies, EID_WPA2, &ft_ie_len,
					 pftpriv->updated_ft_ies_len);
			if (pie)
				pframe = rtw_set_ie(pframe, EID_WPA2, ft_ie_len, pie + 2,
						    &(pattrib->pktlen));
			else
				is_ft_roaming_with_rsn_ie = _FALSE;

			pie = rtw_get_ie(pftpriv->updated_ft_ies, _MDIE_, &ft_ie_len,
					 pftpriv->updated_ft_ies_len);
			if (pie)
				pframe = rtw_set_ie(pframe, _MDIE_, ft_ie_len, pie + 2,
						    &(pattrib->pktlen));

			pie = rtw_get_ie(pftpriv->updated_ft_ies, _FTIE_, &ft_ie_len,
					 pftpriv->updated_ft_ies_len);
			if (pie && is_ft_roaming_with_rsn_ie)
				pframe = rtw_set_ie(pframe, _FTIE_, ft_ie_len, pie + 2,
						    &(pattrib->pktlen));
		}
#endif
...
void _issue_assocreq(_adapter *padapter, u8 is_reassoc)
{
	...
	*(fctrl) = 0;
	...
	for (i = sizeof(NDIS_802_11_FIXED_IEs); i < pmlmeinfo->network.IELength;) {
		...
		case EID_WPA2:
#ifdef CONFIG_RTW_80211R
			if ((is_reassoc == _TRUE) && (rtw_to_roam(padapter) > 0)
			    && rtw_chk_ft_flags(padapter, RTW_FT_SUPPORTED)) {
				pie = rtw_get_ie(pftpriv->updated_ft_ies, EID_WPA2, &ft_ie_len,
						 pftpriv->updated_ft_ies_len);
				if (pie)
		   ...
		   unsigned int power_mode, int try_cnt, int wait_ms)
{
	...
	if (psta) {
		...
		RTW_INFO(FUNC_ADPT_FMT ": Can't find sta info for " MAC_FMT
			 ", skip macid %s!!\n",
	...
	if (caps & cap_ESS) {
		...
		if ((rtw_to_roam(padapter) > 0)
			...
			if (rtw_chk_ft_flags(padapter, RTW_FT_OVER_DS_SUPPORTED)) {
				...
				pftpriv->ft_event.ies =  pftpriv->ft_action + sizeof(
								 struct rtw_ieee80211_hdr_3addr) + 16;
				pftpriv->ft_event.ies_len = pftpriv->ft_action_len - sizeof(
								    struct rtw_ieee80211_hdr_3addr);
...
#ifdef CONFIG_RTW_80211R
		if ((rtw_to_roam(padapter) > 0)
		    && !rtw_chk_ft_status(padapter, RTW_FT_REQUESTED_STA))
...
void ft_link_timer_hdl(void *ctx)
{
	...
	if (rtw_chk_ft_status(padapter, RTW_FT_REQUESTING_STA)) {
		if (pftpriv->ft_req_retry_cnt < FT_ACTION_REQ_LIMIT) {
			...
			issue_action_ft_request(padapter,
...
void report_ft_event(_adapter *padapter)
{
	...
	if (ft_evt_parms.ies)
		_rtw_memcpy((void *)ft_evt_parms.ies, pftpriv->ft_event.ies,
		...
		for (i = 0; i < dvobj->iface_nums; i++) {
			...
			if ((MLME_IS_AP(iface) || MLME_IS_MESH(iface))
				...
				if (is_grouped == _FALSE) {
					...
					rtw_sync_chbw(&mlmeext->cur_channel, &mlmeext->cur_bwmode,
