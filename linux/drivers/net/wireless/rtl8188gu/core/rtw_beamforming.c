...
static void _get_txvector_parameter(PADAPTER adapter, struct sta_info *sta,
				    u8 *g_id, u16 *p_aid)
{
...
static u8 _send_ht_ndpa_packet(PADAPTER adapter, u8 *ra,
			       enum channel_width bw)
{
	...
	txrate = beamforming_get_htndp_tx_rate(GET_PDM_ODM(adapter),
					       bfee->comp_steering_num_of_bfer);
...
static u8 _send_vht_ndpa_packet(PADAPTER adapter, u8 *ra, u16 aid,
				enum channel_width bw)
{
	/* General */
	struct xmit_priv		*pxmitpriv;
	...
	txrate = beamforming_get_vht_ndp_tx_rate(GET_PDM_ODM(adapter),
			bfee->comp_steering_num_of_bfer);
	...
	if (is_supported_5g(pmlmeext->cur_wireless_mode)
	    || is_supported_ht(pmlmeext->cur_wireless_mode))
		aSifsTime = 16;
	else
		aSifsTime = 10;
...
static void _sounding_update_min_period(PADAPTER adapter, u16 period,
					u8 leave)
{
	struct beamforming_info *info;
	struct beamformee_entry *bfee;
...
static struct beamformer_entry *_bfer_get_entry_by_addr(PADAPTER adapter,
		u8 *ra)
{
	u8 i = 0;
	struct beamforming_info *info;
		...
		struct sta_info *sta, u8 bf_cap, u8 sounding_dim, u8 comp_steering)
{
	...
	} else if (TEST_FLAG(bf_cap,
			     BEAMFORMER_CAP_VHT_SU | BEAMFORMER_CAP_HT_EXPLICIT)) {
...
static void _bfer_remove_entry(PADAPTER adapter,
...
static u8 _bfer_set_entry_gid(PADAPTER adapter, u8 *addr, u8 *gid,
			      u8 *position)
{
	...
	rtw_bf_cmd(adapter, BEAMFORMING_CTRL_SET_GID_TABLE, (u8 *)&bfer,
		   sizeof(struct beamformer_entry *), 1);
...
static struct beamformee_entry *_bfee_get_entry_by_addr(PADAPTER adapter,
		u8 *ra)
{
	u8 i = 0;
	struct beamforming_info *info;
...
static u8 _bfee_get_first_su_entry_idx(PADAPTER adapter,
				       struct beamformee_entry *ignore)
{
	struct beamforming_info *info;
	struct beamformee_entry *bfee;
	u8 i;
	...
	for (i = 0; i < MAX_BEAMFORMEE_ENTRY_NUM; i++) {
		...
		if ((!TEST_FLAG(bfee->cap, BEAMFORMEE_CAP_VHT_MU))
		    && TEST_FLAG(bfee->cap,
				 BEAMFORMEE_CAP_VHT_SU | BEAMFORMEE_CAP_HT_EXPLICIT))
			return i;
	}
 ...
 *
 */
static u8 _bfee_get_first_mu_entry_idx(PADAPTER adapter,
				       struct beamformee_entry *ignore)
{
	struct beamforming_info *info;
	struct beamformee_entry *bfee;
		...
		struct sta_info *sta, u8 bf_cap, u8 sounding_dim, u8 comp_steering)
{
	...
	bfee->SoundCnt = GetInitSoundCnt(bfee->sound_period,
					 info->sounding_info.min_sounding_period);
	bfee->cap = bf_cap;
	...
	if (TEST_FLAG(bf_cap, BEAMFORMEE_CAP_VHT_MU)) {
		...
		if (_TRUE == info->bEnableSUTxBFWorkAround) {
			...
			if ((info->beamformee_mu_cnt == 1) && (info->beamformee_su_cnt > 0)) {
				...
				rtw_bf_cmd(adapter, BEAMFORMING_CTRL_SET_CSI_REPORT,
					   (u8 *)&info->TargetCSIInfo, sizeof(struct _RT_CSI_INFO), 0);
			}
		}
...
static void _bfee_remove_entry(PADAPTER adapter,
...
static enum beamforming_cap _bfee_get_entry_cap_by_macid(PADAPTER adapter,
		u8 macid)
{
	struct beamforming_info *info;
	struct beamformee_entry *bfee;
...
static void _beamforming_enter(PADAPTER adapter, void *p)
{
	...
	if (sta_bf_cap & BEAMFORMER_CAP) {
		...
		bfer = _bfer_add_entry(adapter, sta, sta_bf_cap, sounding_dim,
				       comp_steering_num);
		if (!bfer)
	...
	if (sta_bf_cap & BEAMFORMEE_CAP) {
		...
		bfee = _bfee_add_entry(adapter, sta, sta_bf_cap, sounding_dim,
				       comp_steering_num);
		if (!bfee)
 ...
 */
enum beamforming_cap rtw_bf_bfee_get_entry_cap_by_macid(void *mlme,
		u8 macid)
{
	PADAPTER adapter;
...
struct beamformer_entry *rtw_bf_bfer_get_entry_by_addr(PADAPTER adapter,
		u8 *ra)
{
	return _bfer_get_entry_by_addr(adapter, ra);
}

struct beamformee_entry *rtw_bf_bfee_get_entry_by_addr(PADAPTER adapter,
		u8 *ra)
{
	return _bfee_get_entry_by_addr(adapter, ra);
}

void rtw_bf_get_ndpa_packet(PADAPTER adapter,
			    union recv_frame *precv_frame)
{
	RTW_DBG("+%s\n", __FUNCTION__);
}

u32 rtw_bf_get_report_packet(PADAPTER adapter,
			     union recv_frame *precv_frame)
{
	u32 ret = _SUCCESS;
	struct beamforming_info *info;
	...
	if ((_TRUE == info->bEnableSUTxBFWorkAround)
		...
		if ((info->TargetCSIInfo.Nc != Nc) || (info->TargetCSIInfo.Nr != Nr) ||
			...
			rtw_bf_cmd(adapter, BEAMFORMING_CTRL_SET_CSI_REPORT,
				   (u8 *)&info->TargetCSIInfo, sizeof(struct _RT_CSI_INFO), 1);
		}
	}
...
u8 rtw_bf_send_vht_gid_mgnt_packet(PADAPTER adapter, u8 *ra, u8 *gid,
				   u8 *position)
{
	/* General */
 ...
 */
void rtw_bf_get_vht_gid_mgnt_packet(PADAPTER adapter,
				    union recv_frame *precv_frame)
{
	u8 *pframe;
	u8 *ta, *gid, *position;
...
void rtw_bf_init(PADAPTER adapter)
{
	...
	*/
	...
	_sounding_init(&info->sounding_info);
	rtw_init_timer(&info->sounding_timer, adapter, _sounding_timer_handler,
		       adapter);
	rtw_init_timer(&info->sounding_timeout_timer, adapter,
		       _sounding_timeout_timer_handler, adapter);
...
void rtw_bf_update_attrib(PADAPTER adapter, struct pkt_attrib *attrib,
			  struct sta_info *sta)
{
	if (sta) {
...
void rtw_bf_update_traffic(PADAPTER adapter)
{
	...
	sounding_idx = phydm_get_beamforming_sounding_info(GET_PDM_ODM(adapter),
			tp, MAX_BEAMFORMEE_ENTRY_NUM, tx_rate);
...
struct beamforming_entry	*beamforming_get_entry_by_addr(
	struct mlme_priv *pmlmepriv, u8 *ra, u8 *idx)
{
	u8	i = 0;
...
BEAMFORMING_CAP beamforming_get_entry_beam_cap_by_mac_id(PVOID pmlmepriv,
		u8 mac_id)
{
	u8	i = 0;
	struct beamforming_info	*pBeamInfo = GET_BEAMFORM_INFO((
...
struct beamforming_entry	*beamforming_get_free_entry(
	struct mlme_priv *pmlmepriv, u8 *idx)
{
	u8	i = 0;
	struct beamforming_info	*pBeamInfo = GET_BEAMFORM_INFO(pmlmepriv);
...
struct beamforming_entry	*beamforming_add_entry(PADAPTER adapter, u8 *ra,
		u16 aid,
		u16 mac_id, enum channel_width bw, BEAMFORMING_CAP beamfrom_cap, u8 *idx)
{
	struct mlme_priv			*pmlmepriv = &(adapter->mlmepriv);
	struct beamforming_entry	*pEntry = beamforming_get_free_entry(pmlmepriv,
			idx);
	...
	if (pEntry != NULL) {
		...
		if (check_fwstate(pmlmepriv, WIFI_AP_STATE)) {
			u16	BSSID = ((*(adapter_mac_addr(adapter) + 5) & 0xf0) >> 4) ^
					(*(adapter_mac_addr(adapter) + 5) &
					 0xf); /* BSSID[44:47] xor BSSID[40:43] */
			pEntry->p_aid = (aid + BSSID * 32) &
					0x1ff;		/* (dec(A) + dec(B)*32) mod 512 */
			pEntry->g_id = 63;
		...
		pEntry->LogSuccess =
			0;	/*LogSuccess is NOT needed to be accumulated, so  LogSuccessCnt->LogSuccess, 2015-04-13, Jeffery*/
...
BOOLEAN	beamforming_remove_entry(struct mlme_priv *pmlmepriv, u8 *ra,
				 u8 *idx)
{
	struct beamforming_entry	*pEntry = beamforming_get_entry_by_addr(pmlmepriv,
			ra, idx);
...
void beamforming_dym_period(PADAPTER Adapter)
{
	...
	if (pdvobjpriv->traffic_stat.cur_tx_tp + pdvobjpriv->traffic_stat.cur_rx_tp
	    > 2) {
	...
	for (Idx = 0; Idx < BEAMFORMING_ENTRY_NUM; Idx++) {
		...
		if (pBeamformEntry->beamforming_entry_cap & (BEAMFORMER_CAP_HT_EXPLICIT |
				BEAMFORMER_CAP_VHT_SU)) {
			if (pSoundInfo->sound_mode == SOUNDING_FW_VHT_TIMER
			    || pSoundInfo->sound_mode == SOUNDING_FW_HT_TIMER) {
				if (pBeamformEntry->sound_period != SoundPeriod_FW) {
					pBeamformEntry->sound_period = SoundPeriod_FW;
					bChangePeriod =
						_TRUE;	/* Only FW sounding need to send H2C packet to change sound period. */
				}
...
BOOLEAN	issue_ht_sw_ndpa_packet(PADAPTER Adapter, u8 *ra,
				enum channel_width bw, u8 qidx)
{
	struct xmit_frame		*pmgntframe;
...
BOOLEAN	issue_ht_ndpa_packet(PADAPTER Adapter, u8 *ra,
			     enum channel_width bw, u8 qidx)
{
	struct xmit_frame		*pmgntframe;
	struct pkt_attrib		*pattrib;
...
BOOLEAN	beamforming_send_ht_ndpa_packet(PADAPTER Adapter, u8 *ra,
					enum channel_width bw, u8 qidx)
{
	return issue_ht_ndpa_packet(Adapter, ra, bw, qidx);
}
BOOLEAN	issue_vht_sw_ndpa_packet(PADAPTER Adapter, u8 *ra, u16 aid,
				 enum channel_width bw, u8 qidx)
{
	struct xmit_frame		*pmgntframe;
	struct pkt_attrib		*pattrib;
...
BOOLEAN	issue_vht_ndpa_packet(PADAPTER Adapter, u8 *ra, u16 aid,
			      enum channel_width bw, u8 qidx)
{
	struct xmit_frame		*pmgntframe;
	struct pkt_attrib		*pattrib;
...
BOOLEAN	beamforming_send_vht_ndpa_packet(PADAPTER Adapter, u8 *ra, u16 aid,
		enum channel_width bw, u8 qidx)
{
...
SOUNDING_MODE	beamforming_sounding_mode(struct beamforming_info *pBeamInfo,
		u8 idx)
{
...
u16	beamforming_sounding_time(struct beamforming_info *pBeamInfo,
				  SOUNDING_MODE mode, u8 idx)
{
...
enum channel_width	beamforming_sounding_bw(struct beamforming_info
		*pBeamInfo, SOUNDING_MODE mode, u8 idx)
{
...
BOOLEAN	beamforming_select_beam_entry(struct beamforming_info *pBeamInfo)
{
	...
	if (pSoundInfo->sound_idx < BEAMFORMING_ENTRY_NUM)
		pSoundInfo->sound_mode = beamforming_sounding_mode(pBeamInfo,
					 pSoundInfo->sound_idx);
	else
	...
	if (SOUNDING_STOP_All_TIMER == pSoundInfo->sound_mode)
		...
		pSoundInfo->sound_bw = beamforming_sounding_bw(pBeamInfo,
				       pSoundInfo->sound_mode, pSoundInfo->sound_idx);
		pSoundInfo->sound_period = beamforming_sounding_time(pBeamInfo,
					   pSoundInfo->sound_mode, pSoundInfo->sound_idx);
		return _TRUE;
	}
}
...
BOOLEAN	beamforming_start_period(PADAPTER adapter)
{
	...
	RTW_INFO("%s Idx %d Mode %d BW %d Period %d\n", __FUNCTION__,
		 pSoundInfo->sound_idx, pSoundInfo->sound_mode, pSoundInfo->sound_bw,
		 pSoundInfo->sound_period);
...
void	beamforming_notify(PADAPTER adapter)
{
	...
	if (pBeamInfo->beamforming_state == BEAMFORMING_STATE_IDLE) {
		...
		RTW_INFO("%s BeamformState %d\n", __FUNCTION__,
			 pBeamInfo->beamforming_state);

	RTW_INFO("%s BeamformState %d bSounding %d\n", __FUNCTION__,
		 pBeamInfo->beamforming_state, bSounding);
}

BOOLEAN	beamforming_init_entry(PADAPTER	adapter, struct sta_info *psta,
			       u8 *idx)
{
	struct mlme_priv	*pmlmepriv = &(adapter->mlmepriv);
	...
	if (is_supported_ht(wireless_mode) || is_supported_vht(wireless_mode)) {
		...
		if (TEST_FLAG(cur_beamform, BEAMFORMING_HT_BEAMFORMER_ENABLE))
			beamform_cap = (BEAMFORMING_CAP)(beamform_cap |
							 BEAMFORMEE_CAP_HT_EXPLICIT);
		...
		if (pBeamformEntry == NULL) {
			pBeamformEntry = beamforming_add_entry(adapter, ra, aid, mac_id, bw,
							       beamform_cap, idx);
			if (pBeamformEntry == NULL)
				return _FALSE;
			else
				pBeamformEntry->beamforming_entry_state =
					BEAMFORMING_ENTRY_STATE_INITIALIZEING;
		} else {
			/* Entry has been created. If entry is initialing or progressing then errors occur. */
			if (pBeamformEntry->beamforming_entry_state !=
...
void	beamforming_reset(PADAPTER adapter)
{
	...
	for (idx = 0; idx < BEAMFORMING_ENTRY_NUM; idx++) {
		if (pBeamInfo->beamforming_entry[idx].bUsed == _TRUE) {
			...
			pBeamInfo->beamforming_entry[idx].beamforming_entry_cap =
				BEAMFORMING_CAP_NONE;
			pBeamInfo->beamforming_entry[idx].beamforming_entry_state =
...
void beamforming_sounding_fail(PADAPTER Adapter)
{
	...
	struct beamforming_info	*pBeamInfo = GET_BEAMFORM_INFO(pmlmepriv);
	struct beamforming_entry	*pEntry = &
			(pBeamInfo->beamforming_entry[pBeamInfo->beamforming_cur_idx]);

	pEntry->bSound = _FALSE;
	rtw_hal_set_hwreg(Adapter, HW_VAR_SOUNDING_FW_NDPA,
			  (u8 *)&pBeamInfo->beamforming_cur_idx);
...
u32	rtw_beamforming_get_report_frame(PADAPTER	 Adapter,
		union recv_frame *precv_frame)
{
	u32	ret = _SUCCESS;
	...
	else if (pBeamformEntry->beamforming_entry_cap &
		 BEAMFORMER_CAP_HT_EXPLICIT)
	...
	if (_rtw_memcmp(pBeamformEntry->PreCsiReport + offset, pframe + offset,
			frame_len - offset) == _FALSE)
...
void	rtw_beamforming_get_ndpa_frame(PADAPTER	 Adapter,
				       union recv_frame *precv_frame)
{
#if (BEAMFORMING_SUPPORT == 1)
	PHAL_DATA_TYPE	pHalData = GET_HAL_DATA(Adapter);
	...
	else if ((pBeamformEntry->LogSuccess == 1)
		 || (pBeamformEntry->ClockResetTimes == 5)) {
		RTW_INFO("[%s] LogSeq=%d, PreLogSeq=%d\n", __func__,
			 pBeamformEntry->LogSeq, pBeamformEntry->PreLogSeq);
		return;
	}

	Sequence = (pframe[16]) >> 2;
	RTW_INFO("[%s] Start, Sequence=%d, LogSeq=%d, PreLogSeq=%d, LogRetryCnt=%d, ClockResetTimes=%d, LogSuccess=%d\n",
		 __func__, Sequence, pBeamformEntry->LogSeq, pBeamformEntry->PreLogSeq,
		 pBeamformEntry->LogRetryCnt, pBeamformEntry->ClockResetTimes,
		 pBeamformEntry->LogSuccess);
...
u8	beamforming_wk_cmd(_adapter *padapter, s32 type, u8 *pbuf, s32 size,
			   u8 enqueue)
{
	struct cmd_obj	*ph2c;
	struct drvextra_cmd_parm	*pdrvextra_cmd_parm;
...
void update_attrib_txbf_info(_adapter *padapter,
