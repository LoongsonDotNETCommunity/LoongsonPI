			...
			if (_SUCCESS == s_ret)
				...
				if (rtw_to_roam(adapter) != 0) {
					if (rtw_dec_to_roam(adapter) == 0
					    || _SUCCESS != rtw_sitesurvey_cmd(adapter, &pmlmepriv->assoc_ssid, 1, NULL,
									      0)
					   ) {
...
#ifdef CONFIG_LAYER2_ROAMING
#ifdef CONFIG_RTW_80211R
		if (reason == WLAN_REASON_EXPIRATION_CHK
				       ...
				       , struct wlan_network **candidate, struct wlan_network *competitor)
{
	...
	if (rtw_chk_ft_flags(adapter, RTW_FT_SUPPORTED)) {
		ptmp = rtw_get_ie(&competitor->network.IEs[12], _MDIE_, &mdie_len,
				  competitor->network.IELength - 12);
		if (ptmp) {
			...
			if (rtw_chk_ft_flags(adapter, RTW_FT_STA_OVER_DS_SUPPORTED)) {
				if ((rtw_chk_ft_flags(adapter, RTW_FT_OVER_DS_SUPPORTED)
				     && !(*(ptmp + 4) & 0x01)) ||
				    (!rtw_chk_ft_flags(adapter, RTW_FT_OVER_DS_SUPPORTED)
				     && (*(ptmp + 4) & 0x01))) {
				   ...
				   u8 *out_ie, uint in_len, uint *pout_len, u8 channel)
{
	...
	u8	cbw40_enable = 0, rf_type = 0, operation_bw = 0, rf_num = 0,
		rx_stbc_nss = 0, rx_nss = 0;
	struct registry_priv *pregistrypriv = &padapter->registrypriv;
	struct mlme_priv	*pmlmepriv = &padapter->mlmepriv;
	struct ht_priv		*phtpriv = &pmlmepriv->htpriv;
	struct mlme_ext_priv	*pmlmeext = &padapter->mlmeextpriv;
