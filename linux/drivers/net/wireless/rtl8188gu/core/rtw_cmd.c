...
sint	_rtw_init_cmd_priv(struct	cmd_priv *pcmdpriv)
{
	...
	pcmdpriv->rsp_buf = pcmdpriv->rsp_allocated_buf  +  4 - ((SIZE_PTR)(
				    pcmdpriv->rsp_allocated_buf) & 3);
...
static void c2h_wk_callback(_workitem *work)
{
	...
	while (!rtw_cbuf_empty(evtpriv->c2h_queue)) {
		...
		if (!rtw_hal_c2h_valid(adapter, c2h_evt)
		    || rtw_hal_c2h_reg_hdr_parse(adapter, c2h_evt, &id, &seq, &plen,
						 &payload) != _SUCCESS
		   ) {
			rtw_mfree(c2h_evt, C2H_REG_LEN);
			continue;
...
sint _rtw_init_evt_priv(struct evt_priv *pevtpriv)
{
	...
	pevtpriv->evt_buf = pevtpriv->evt_allocated_buf  +  4 - ((unsigned int)(
				    pevtpriv->evt_allocated_buf) & 3);
	...
	pevtpriv->pc2h_mdl = IoAllocateMdl((u8 *)pevtpriv->c2h_mem, C2H_MEM_SZ,
					   FALSE, FALSE, NULL);
...
sint _rtw_enqueue_cmd(_queue *queue, struct cmd_obj *obj, bool to_head)
{
	...
	if (dump_cmd_id) {
		...
		if (obj->cmdcode == GEN_CMD_CODE(_Set_MLME_EVT)) {
			if (obj->parmbuf) {
				struct C2HEvent_Header *pc2h_evt_hdr = (struct C2HEvent_Header *)(
						obj->parmbuf);
		...
		if (obj->cmdcode == GEN_CMD_CODE(_Set_Drv_Extra)) {
			if (obj->parmbuf) {
				struct drvextra_cmd_parm *pdrvextra_cmd_parm = (struct drvextra_cmd_parm *)
						(obj->parmbuf);
	...
	if (queue->queue.prev->next != &queue->queue) {
		RTW_INFO("[%d] head %p, tail %p, tail->prev->next %p[tail], tail->next %p[head]\n",
			 __LINE__,
			 &queue->queue, queue->queue.prev, queue->queue.prev->prev->next,
			 queue->queue.prev->next);
...
{
	u8 bAllow =
		_FALSE; /* set to _TRUE to allow enqueuing cmd when hw_init_completed is _FALSE */
...
u32 rtw_enqueue_cmd(struct cmd_priv *pcmdpriv, struct cmd_obj *cmd_obj)
{
	...
	if ((_FAIL == res) || (cmd_obj->cmdsz > MAX_CMDSZ)) {
		...
		if (cmd_obj->cmdcode == GEN_CMD_CODE(_Set_Drv_Extra)) {
			struct drvextra_cmd_parm *extra_parm = (struct drvextra_cmd_parm *)
							       cmd_obj->parmbuf;
...
thread_return rtw_cmd_thread(thread_context context)
{
	...
	while (1) {
		if (_rtw_down_sema(&pcmdpriv->cmd_queue_sema) == _FAIL) {
			RTW_PRINT(
				FUNC_ADPT_FMT" _rtw_down_sema(&pcmdpriv->cmd_queue_sema) return _FAIL, break\n",
...
_next:
		...
		if (DBG_CMD_EXECUTE)
			RTW_INFO(ADPT_FMT" "CMD_FMT" %sexecute\n", ADPT_ARG(pcmd->padapter),
				 CMD_ARG(pcmd)
				 , pcmd->res == H2C_ENQ_HEAD ? "ENQ_HEAD " : (pcmd->res == H2C_ENQ_HEAD_FAIL
						 ? "ENQ_HEAD_FAIL " : ""));
...
post_process:
		...
		if (cmd_process_time > 1000) {
			RTW_INFO(ADPT_FMT" "CMD_FMT" process_time=%d\n", ADPT_ARG(pcmd->padapter),
				 CMD_ARG(pcmd), cmd_process_time);
			if (0)
				rtw_warn_on(1);
		}

		/* call callback function for post-processed */
		if (pcmd->cmdcode < (sizeof(rtw_cmd_callback) / sizeof(
			...
			if (pcmd_callback == NULL) {
				...
				pcmd_callback(pcmd->padapter,
					      pcmd);/* need conider that free cmd_obj in rtw_cmd_callback */
			}
		} else {
			rtw_free_cmd_obj(pcmd);
		}
...
u8 rtw_setstandby_cmd(_adapter *padapter, uint action)
{
	...
	psetusbsuspend = (struct usb_suspend_parm *)rtw_zmalloc(sizeof(
...
*/
u8 rtw_sitesurvey_cmd(_adapter  *padapter, NDIS_802_11_SSID *ssid,
		      int ssid_num,
	...
	if (ch) {
		...
		for (i = 0; i < ch_num && i < RTW_CHANNEL_SCAN_AMOUNT; i++) {
			if (ch[i].hw_value && !(ch[i].flags & RTW_IEEE80211_CHAN_DISABLED)) {
				_rtw_memcpy(&psurveyPara->ch[i], &ch[i],
...
u8 rtw_setdatarate_cmd(_adapter *padapter, u8 *rateset)
{
	...
	pbsetdataratepara = (struct setdatarate_parm *)rtw_zmalloc(sizeof(
...
u8 rtw_setbasicrate_cmd(_adapter *padapter, u8 *rateset)
{
	...
	pssetbasicratepara = (struct setbasicrate_parm *)rtw_zmalloc(sizeof(
...
u8 rtw_setphy_cmd(_adapter *padapter, u8 modem, u8 ch)
{
	...
	psetphypara = (struct setphy_parm *)rtw_zmalloc(sizeof(
				struct setphy_parm));
...
u8 rtw_getmacreg_cmd(_adapter *padapter, u8 len, u32 addr)
{
	...
	preadmacparm = (struct readMAC_parm *)rtw_zmalloc(sizeof(
				struct readMAC_parm));
...
u8 rtw_setbbreg_cmd(_adapter *padapter, u8 offset, u8 val)
{
	...
	pwritebbparm = (struct writeBB_parm *)rtw_zmalloc(sizeof(
				struct writeBB_parm));
...
u8 rtw_setrfreg_cmd(_adapter  *padapter, u8 offset, u32 val)
{
	...
	pwriterfparm = (struct writeRF_parm *)rtw_zmalloc(sizeof(
				struct writeRF_parm));
...
void rtw_getbbrfreg_cmdrsp_callback(_adapter	*padapter,
				    struct cmd_obj *pcmd)
{
...
void rtw_readtssi_cmdrsp_callback(_adapter	*padapter,
				  struct cmd_obj *pcmd)
{
			    ...
			    , s16 req_ch, s8 req_bw, s8 req_offset)
{
	...
	if (req_ch > 0 && req_bw >= 0 && req_offset >= 0) {
		if (!rtw_chset_is_chbw_valid(adapter_to_chset(adapter), req_ch, req_bw,
					     req_offset)) {
			res = _FAIL;
			goto exit;
		}
...
inline u8 rtw_change_bss_chbw_cmd(_adapter *adapter, int flags, s16 req_ch,
				  s8 req_bw, s8 req_offset)
{
	return rtw_createbss_cmd(adapter, flags
				 , 0
...
u8 rtw_joinbss_cmd(_adapter  *padapter, struct wlan_network *pnetwork)
{
	...
	t_len = sizeof(ULONG) + sizeof(NDIS_802_11_MAC_ADDRESS) + 2 +
		...
		sizeof(NDIS_802_11_RATES_EX) + sizeof(WLAN_PHY_INFO) + sizeof(
			ULONG) + MAX_IE_SZ;
#endif
	...
	*/
	...
	_rtw_memcpy(psecnetwork, &pnetwork->network,
	...
	if ((psecnetwork->IELength - 12) < (256 - 1))
		_rtw_memcpy(&psecuritypriv->authenticator_ie[1], &psecnetwork->IEs[12],
	...
	if (pmlmepriv->assoc_by_bssid == _FALSE)
		_rtw_memcpy(&pmlmepriv->assoc_bssid[0], &pnetwork->network.MacAddress[0],
			    ETH_ALEN);
	...
	if (pregistrypriv->wmm_enable) {
		...
		tmp_len = rtw_restruct_wmm_ie(padapter, &pnetwork->network.IEs[0],
					      &psecnetwork->IEs[0], pnetwork->network.IELength, psecnetwork->IELength);
	...
	ptmp = rtw_get_ie(&pnetwork->network.IEs[12], _HT_CAPABILITY_IE_, &tmp_len,
	...
	if (pregistrypriv->ht_enable && ptmp && tmp_len > 0) {
		...
		if ((padapter->securitypriv.dot11PrivacyAlgrthm != _WEP40_) &&
			...
			rtw_restructure_ht_ie(padapter, &pnetwork->network.IEs[12],
					      &psecnetwork->IEs[0],
	...
	if (phtpriv->ht_option
		...
		rtw_restructure_vht_ie(padapter, &pnetwork->network.IEs[0],
				       &psecnetwork->IEs[0],
	...
	if ((rtw_chk_ft_flags(padapter, RTW_FT_STA_SUPPORTED)) &&
		...
		ptmp = rtw_get_ie(&pnetwork->network.IEs[12], _MDIE_, &tmp_len,
				  pnetwork->network.IELength - 12);
		if (ptmp) {
			...
			if ((rtw_chk_ft_flags(padapter, RTW_FT_STA_OVER_DS_SUPPORTED))
			    && (pftpriv->ft_roam_on_expired == _FALSE) && (pftpriv->ft_cap & 0x01))
	...
	if (psecnetwork->IELength < (256 - 1))
		_rtw_memcpy(&psecuritypriv->supplicant_ie[1], &psecnetwork->IEs[0],
			    psecnetwork->IELength);
	else
		_rtw_memcpy(&psecuritypriv->supplicant_ie[1], &psecnetwork->IEs[0],
			    (256 - 1));
...
u8 rtw_disassoc_cmd(_adapter *padapter, u32 deauth_timeout_ms,
		    int flags) /* for sta_mode */
{
...
u8 rtw_setopmode_cmd(_adapter  *padapter,
		     NDIS_802_11_NETWORK_INFRASTRUCTURE networktype, bool enqueue)
{
	...
	psetop = (struct setopmode_parm *)rtw_zmalloc(sizeof(
...
u8 rtw_setstakey_cmd(_adapter *padapter, struct sta_info *sta, u8 key_type,
		     bool enqueue)
{
	struct cmd_obj			*ph2c;
	...
	psetstakey_para = (struct set_stakey_parm *)rtw_zmalloc(sizeof(
	...
	if (key_type == GROUP_KEY)
		_rtw_memcpy(&psetstakey_para->key,
	...
	if (enqueue) {
		...
		psetstakey_rsp = (struct set_stakey_rsp *)rtw_zmalloc(sizeof(
...
u8 rtw_clearstakey_cmd(_adapter *padapter, struct sta_info *sta,
		       u8 enqueue)
{
	struct cmd_obj			*ph2c;
	struct set_stakey_parm	*psetstakey_para;
	...
	if (!enqueue) {
		while ((cam_id = rtw_camid_search(padapter, sta->cmn.mac_addr, -1,
						  -1)) >= 0) {
...
u8 rtw_setrttbl_cmd(_adapter  *padapter,
		    struct setratable_parm *prate_table)
{
	...
	psetrttblparm = (struct setratable_parm *)rtw_zmalloc(sizeof(
...
u8 rtw_getrttbl_cmd(_adapter  *padapter, struct getratable_rsp *pval)
{
	...
	pgetrttblparm = (struct getratable_parm *)rtw_zmalloc(sizeof(
...
u8 rtw_setassocsta_cmd(_adapter  *padapter, u8 *mac_addr)
{
	...
	psetassocsta_para = (struct set_assocsta_parm *)rtw_zmalloc(sizeof(
	...
	psetassocsta_rsp = (struct set_stakey_rsp *)rtw_zmalloc(sizeof(
...
u8 rtw_addbareq_cmd(_adapter *padapter, u8 tid, u8 *addr)
{
	...
	paddbareq_parm = (struct addBaReq_parm *)rtw_zmalloc(sizeof(
...
u8 rtw_addbarsp_cmd(_adapter *padapter, u8 *addr, u16 tid, u8 status,
		    u8 size, u16 start_seq)
{
	...
	paddBaRsp_parm = (struct addBaRsp_parm *)rtw_zmalloc(sizeof(
...
u8 rtw_set_chbw_cmd(_adapter *padapter, u8 ch, u8 bw, u8 ch_offset,
		    u8 flags)
{
...
u8 _rtw_set_chplan_cmd(_adapter *adapter, int flags, u8 chplan,
...
inline u8 rtw_set_chplan_cmd(_adapter *adapter, int flags, u8 chplan,
			     u8 swconfig)
{
	return _rtw_set_chplan_cmd(adapter, flags, chplan, NULL, swconfig);
}

inline u8 rtw_set_country_cmd(_adapter *adapter, int flags,
			      const char *country_code, u8 swconfig)
{
	...
	return _rtw_set_chplan_cmd(adapter, flags, RTW_CHPLAN_UNSPECIFIED, ent,
				   swconfig);
}
...
u8 rtw_led_blink_cmd(_adapter *padapter, PVOID pLed)
{
	...
	ledBlink_param = (struct	LedBlink_param *)rtw_zmalloc(sizeof(
...
u8 rtw_set_csa_cmd(_adapter *padapter, u8 new_ch_no)
{
	...
	setChannelSwitch_param = (struct SetChannelSwitch_param *)rtw_zmalloc(
					 sizeof(struct	SetChannelSwitch_param));
...
u8 rtw_tdls_cmd(_adapter *padapter, u8 *addr, u8 option)
{
	...
	TDLSoption = (struct TDLSoption_param *)rtw_zmalloc(sizeof(
...
u8 rtw_antenna_select_cmd(_adapter *padapter, u8 antenna, u8 enqueue)
{
	...
	rtw_hal_get_def_var(padapter, HAL_DEF_IS_SUPPORT_ANT_DIV,
			    &(bSupportAntDiv));
		       ...
		       )
{
	...
	roch_parm = (struct p2p_roch_parm *)rtw_zmalloc(sizeof(
	...
	if (flags & RTW_CMDF_DIRECTLY) {
		/* no need to enqueue, do the cmd hdl directly and free cmd parameter */
		if (H2C_SUCCESS != p2p_protocol_wk_hdl(adapter,
						       cancel ? P2P_CANCEL_RO_CH_WK : P2P_RO_CH_WK, (u8 *)roch_parm))
			res = _FAIL;
		rtw_mfree((u8 *)roch_parm, sizeof(*roch_parm));
	} else {
		/* need enqueue, prepare cmd_obj and enqueue */
		parm = (struct drvextra_cmd_parm *)rtw_zmalloc(sizeof(
					struct drvextra_cmd_parm));
		if (parm == NULL) {
			rtw_mfree((u8 *)roch_parm, sizeof(*roch_parm));
			res = _FAIL;
...
inline u8 p2p_cancel_roch_cmd(_adapter *adapter, u64 cookie,
			      struct wireless_dev *wdev, u8 flags)
{
			  ...
			  const u8 *buf, size_t len, int wait_ack, u8 flags)
{
	...
	mgnt_parm = (struct mgnt_tx_parm *)rtw_zmalloc(sizeof(
				struct mgnt_tx_parm));
	if (mgnt_parm == NULL) {
		res = _FAIL;
		goto exit;
...
static void rtw_chk_hi_queue_hdl(_adapter *padapter)
{
	...
	while (_FALSE == empty
	       && rtw_get_passing_time_ms(start) < rtw_get_wait_hiq_empty_ms()) {
		rtw_msleep_os(100);
		rtw_hal_get_hwreg(padapter, HW_VAR_CHK_HI_QUEUE_EMPTY, &empty);
	}
	...
	if (psta_bmc->sleepq_len == 0) {
		if (empty == _SUCCESS) {
			...
			if (update_tim == _TRUE)
				_update_beacon(padapter, _TIM_IE_, NULL, _TRUE,
					       "bmc sleepq and HIQ empty");
		} else /* re check again */
...
u8 rtw_dfs_master_hdl(_adapter *adapter)
{
	...
	if (rfctl->dbg_dfs_master_radar_detect_trigger_non) {
		...
		if (i >= dvobj->iface_nums) {
			...
			rtw_change_bss_chbw_cmd(dvobj->padapters[i], RTW_CMDF_DIRECTLY, -1,
						dvobj->padapters[i]->mlmepriv.ori_bw, -1);
		}
...
cac_status_chk:
	...
	if (!IS_CH_WAITING(rfctl) && !IS_CAC_STOPPED(rfctl)) {
		...
		if (rtw_mi_check_fwstate(adapter,
					 WIFI_UNDER_LINKING | WIFI_SITE_MONITOR) == _FALSE) {
...
void rtw_dfs_master_disable(_adapter *adapter, u8 ch, u8 bw, u8 offset,
			    bool by_others)
{
	struct rf_ctl_t *rfctl = adapter_to_rfctl(adapter);
...
void rtw_dfs_master_status_apply(_adapter *adapter, u8 self_action)
{
	...
	if (sync_ch == _TRUE) {
		if (!rtw_is_chbw_grouped(mlmeext->cur_channel, mlmeext->cur_bwmode,
					 mlmeext->cur_ch_offset, u_ch, u_bw, u_offset)) {
			RTW_INFO(FUNC_ADPT_FMT" can't sync %u,%u,%u with %u,%u,%u\n",
				 FUNC_ADPT_ARG(adapter)
				 , mlmeext->cur_channel, mlmeext->cur_bwmode, mlmeext->cur_ch_offset, u_ch,
				 u_bw, u_offset);
			goto apply;
		}
...
u8 rtw_test_h2c_cmd(_adapter *adapter, u8 *buf, u8 len)
{
	...
	init_h2fwcmd_w_parm_no_rsp(pcmdobj, pdrvextra_cmd_parm,
				   GEN_CMD_CODE(_Set_Drv_Extra));
...
static s32 rtw_customer_str_cmd_hdl(_adapter *adapter, u8 write,
				    const u8 *cstr)
{
	int ret = H2C_SUCCESS;
...
{
	return rtw_c2h_wk_cmd(adapter, c2h_evt, c2h_evt ? C2H_REG_LEN : 0,
			      C2H_TYPE_REG);
}
#endif
...
u8 rtw_run_in_thread_cmd(PADAPTER padapter, void (*func)(void *),
			 void *context)
{
	...
	parm = (struct RunInThread_param *)rtw_zmalloc(sizeof(
...
u8 session_tracker_cmd(_adapter *adapter, u8 cmd, struct sta_info *sta,
		       u8 *local_naddr, u8 *local_port, u8 *remote_naddr, u8 *remote_port)
{
...
{
	return session_tracker_cmd(adapter, ST_CMD_CHK, sta, NULL, NULL, NULL,
				   NULL);
}

inline u8 session_tracker_add_cmd(_adapter *adapter, struct sta_info *sta,
				  u8 *local_naddr, u8 *local_port, u8 *remote_naddr, u8 *remote_port)
{
	return session_tracker_cmd(adapter, ST_CMD_ADD, sta, local_naddr,
				   local_port, remote_naddr, remote_port);
}

inline u8 session_tracker_del_cmd(_adapter *adapter, struct sta_info *sta,
				  u8 *local_naddr, u8 *local_port, u8 *remote_naddr, u8 *remote_port)
{
	return session_tracker_cmd(adapter, ST_CMD_DEL, sta, local_naddr,
				   local_port, remote_naddr, remote_port);
}
	...
	while (rtw_end_of_queue_search(phead, plist) == _FALSE) {
		...
		if (0)
			RTW_INFO(FUNC_ADPT_FMT" local:%u, remote:%u, rtsp:%u, %u, %u\n",
				 FUNC_ADPT_ARG(adapter)
				 , ntohs(st->local_port), ntohs(st->remote_port),
				 adapter->wfd_info.rtsp_ctrlport, adapter->wfd_info.tdls_rtsp_ctrlport
...
u8 rtw_drvextra_cmd_hdl(_adapter *padapter, unsigned char *pbuf)
{
	...
	case STA_MSTATUS_RPT_WK_CID:
		rtw_sta_media_status_rpt_cmd_hdl(padapter,
						 (struct sta_media_status_rpt_cmd_parm *)pdrvextra_cmd->pbuf);
		break;
	...
	case P2P_PROTO_WK_CID:
		...
		*/
		ret = p2p_protocol_wk_hdl(padapter, pdrvextra_cmd->type,
					  pdrvextra_cmd->pbuf);
		break;
#endif
#ifdef CONFIG_AP_MODE
	...
	case SESSION_TRACKER_WK_CID:
		session_tracker_cmd_hdl(padapter,
					(struct st_cmd_parm *)pdrvextra_cmd->pbuf);
		break;
	case EN_HW_UPDATE_TSF_WK_CID:
		rtw_hal_set_hwreg(padapter, HW_VAR_EN_HW_UPDATE_TSF, NULL);
		break;
	case TEST_H2C_CID:
		rtw_hal_fill_h2c_cmd(padapter, pdrvextra_cmd->pbuf[0],
				     pdrvextra_cmd->size - 1, &pdrvextra_cmd->pbuf[1]);
		break;
	case MP_CMD_WK_CID:
#ifdef CONFIG_MP_INCLUDED
	...
	case CUSTOMER_STR_WK_CID:
		ret = rtw_customer_str_cmd_hdl(padapter, pdrvextra_cmd->type,
					       pdrvextra_cmd->pbuf);
		break;
#endif
...
void rtw_getmacreg_cmdrsp_callback(_adapter *padapter,
				   struct cmd_obj *pcmd)
{
...
void rtw_create_ibss_post_hdl(_adapter *padapter, int status)
{
...

	{
		...
		_rtw_memcpy(&mlme_cur_network->network, pdev_network,
			    (get_WLAN_BSSID_EX_sz(pdev_network)));

#if 0
		/* reset DSConfig */
		mlme_cur_network->network.Configuration.DSConfig = (u32)rtw_ch2freq(
...
void rtw_setstaKey_cmdrsp_callback(_adapter	*padapter,
				   struct cmd_obj *pcmd)
{
...
void rtw_setassocsta_cmdrsp_callback(_adapter	*padapter,
				     struct cmd_obj *pcmd)
{
	_irqL	irqL;
	struct sta_priv *pstapriv = &padapter->stapriv;
	struct mlme_priv	*pmlmepriv = &padapter->mlmepriv;
	struct set_assocsta_parm *passocsta_parm = (struct set_assocsta_parm *)(
				pcmd->parmbuf);
...
void rtw_getrttbl_cmd_cmdrsp_callback(_adapter	*padapter,
				      struct cmd_obj *pcmd);
void rtw_getrttbl_cmd_cmdrsp_callback(_adapter	*padapter,
				      struct cmd_obj *pcmd)
{
