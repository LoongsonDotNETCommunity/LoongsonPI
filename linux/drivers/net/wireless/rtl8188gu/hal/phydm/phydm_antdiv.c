	...
	if (p_dm_fat_table->ant_div_on_off != swch) {
		...
		if (p_dm->support_ic_type & ODM_N_ANTDIV_SUPPORT) {
			...
			if (p_dm->support_ic_type & ODM_RTL8812) {
				odm_set_bb_reg(p_dm, 0xc50, BIT(7),
					       swch); /* OFDM AntDiv function block enable */
				odm_set_bb_reg(p_dm, 0xa00, BIT(15),
					       swch); /* CCK AntDiv function block enable */
			} else {
				odm_set_bb_reg(p_dm, 0x8D4, BIT(24),
					       swch); /* OFDM AntDiv function block enable */

				if ((p_dm->cut_version >= ODM_CUT_C)
...
)
{
	...
	if (p_dm->support_ic_type == ODM_RTL8188E) {
		...
		odm_set_bb_reg(p_dm, 0xB34, BIT(28),
	...
	if (p_dm_fat_table->rx_idle_ant != ant) {
		...
		if (p_dm->support_ic_type & ODM_N_ANTDIV_SUPPORT) {
			if (p_dm->support_ic_type == ODM_RTL8192E) {
				odm_set_bb_reg(p_dm, 0xB38, BIT(5) | BIT(4) | BIT(3),
					       default_ant); /* Default RX */
				odm_set_bb_reg(p_dm, 0xB38, BIT(8) | BIT(7) | BIT(6),
					       optional_ant); /* Optional RX */
				odm_set_bb_reg(p_dm, 0x860, BIT(14) | BIT(13) | BIT(12),
				...
				*/
				...
				odm_set_bb_reg(p_dm, 0x864, BIT(5) | BIT(4) | BIT(3),
					       default_ant);		/*Default RX*/
				odm_set_bb_reg(p_dm, 0x864, BIT(8) | BIT(7) | BIT(6),
					       optional_ant);	/*Optional RX*/
				odm_set_bb_reg(p_dm, 0x860, BIT(14) | BIT(13) | BIT(12),
...
#if 0
			odm_set_bb_reg(p_dm, ODM_REG_TRMUX_11AC, BIT(21) | BIT20 | BIT19,
				       default_ant);	 /* Default RX */
			odm_set_bb_reg(p_dm, ODM_REG_TRMUX_11AC, BIT(24) | BIT23 | BIT22,
				       optional_ant); /* Optional RX */
			odm_set_bb_reg(p_dm, ODM_REG_TRMUX_11AC, BIT(27) | BIT26 | BIT25,
				       default_ant);	 /* Default TX */
#endif
		}
		...
		if (p_dm->support_ic_type & (ODM_RTL8821C | ODM_RTL8822B | ODM_RTL8814A)) {
			...
			odm_set_mac_reg(p_dm, 0x6D8, BIT(7) | BIT(6),
					default_tx_ant);		/*PathA Resp Tx*/
			/**/
		} else {
			odm_set_mac_reg(p_dm, 0x6D8, BIT(10) | BIT(9) | BIT(8),
					default_tx_ant);	/*PathA Resp Tx*/
			/**/
		}
...
)
{
	...
	if (p_dm_fat_table->b_fix_tx_ant != NO_FIX_TX_ANT)
		ant = (p_dm_fat_table->b_fix_tx_ant == FIX_TX_AT_MAIN) ? MAIN_ANT :
		      AUX_ANT;
...
)
{
	...
	if (p_dm->support_ic_type == ODM_RTL8192E) {
		...
		odm_set_bb_reg(p_dm, 0x9b0, 0x0FFFFFFF,
			       0x1081008);     /* 0x9b0[30:0] = 01081008 */
		odm_set_bb_reg(p_dm, 0x9b4, 0x0FFFFFFF,
...
)
{
	...
	/* 2 mode 1 */
	if ((p_dm_bdc_table->num_txbfee_client != 0)
	...
	/* 2 mode 2 */
	else if ((p_dm_bdc_table->num_txbfee_client == 0)
		 && (p_dm_bdc_table->num_txbfer_client != 0)) {
...
)
{
	...
	PHYDM_DBG(p_dm, DBG_ANT_DIV,
		  ("***{ num_BFee,  num_BFer, num_client}  = (( %d  ,  %d  ,  %d))\n",
		   p_dm_bdc_table->num_txbfee_client, p_dm_bdc_table->num_txbfer_client,
	...
	/* 2 mode 2 & 3 */
	if (p_dm_bdc_table->bdc_mode == BDC_MODE_2
	    || p_dm_bdc_table->bdc_mode == BDC_MODE_3) {
		...
		if (p_dm_bdc_table->num_txbfer_client ==
		    p_dm_bdc_table->num_client) { /* BFer STA Only?: yes */
		...
		/*  */
		if (p_dm_bdc_table->is_all_bf_sta_idle == false
			...
			odm_bd_ccoex_type_with_bfer_client(p_dm, DIVOFF_CSION);
			return;
		} else if (p_dm_bdc_table->is_all_bf_sta_idle == true
...
)
{
	...
	odm_set_mac_reg(p_dm, ODM_REG_ANTSEL_PIN_11N, MASKDWORD,
			value32 | (BIT(23) | BIT(
					   25))); /* Reg4C[25]=1, Reg4C[23]=1 for pin output */
	/* Pin Settings */
	odm_set_bb_reg(p_dm, ODM_REG_PIN_CTRL_11N, BIT(9) | BIT(8),
		       0);/* reg870[8]=1'b0, reg870[9]=1'b0		 */ /* antsel antselb by HW */
	odm_set_bb_reg(p_dm, ODM_REG_RX_ANT_CTRL_11N, BIT(10),
		       0);	/* reg864[10]=1'b0	 */ /* antsel2 by HW */
	odm_set_bb_reg(p_dm, ODM_REG_LNA_SWITCH_11N, BIT(22),
		       1);	/* regb2c[22]=1'b0	 */ /* disable CS/CG switch */
	odm_set_bb_reg(p_dm, ODM_REG_LNA_SWITCH_11N, BIT(31),
		       1);	/* regb2c[31]=1'b1	 */ /* output at CG only */
	/* OFDM Settings */
	odm_set_bb_reg(p_dm, ODM_REG_ANTDIV_PARA1_11N, MASKDWORD, 0x000000a0);
	/* CCK Settings */
	odm_set_bb_reg(p_dm, ODM_REG_BB_PWR_SAV4_11N, BIT(7),
		       1); /* Fix CCK PHY status report issue */
	odm_set_bb_reg(p_dm, ODM_REG_CCK_ANTDIV_PARA2_11N, BIT(4),
		       1); /* CCK complete HW AntDiv within 64 samples */

	odm_set_bb_reg(p_dm, ODM_REG_ANT_MAPPING1_11N, 0xFFFF,
...
)
{
	...
	if (*(p_dm->p_mp_mode) == true) {
		odm_set_bb_reg(p_dm, ODM_REG_IGI_A_11N, BIT(7), 0); /* disable HW AntDiv */
		odm_set_bb_reg(p_dm, ODM_REG_RX_ANT_CTRL_11N, BIT(5) | BIT4 | BIT3,
			       0); /* Default RX   (0/1) */
		return;
	}
#endif
	...
	if (!p_dm->is_mp_chip) { /* testchip */
		odm_set_bb_reg(p_dm, ODM_REG_RX_DEFUALT_A_11N, BIT(10) | BIT(9) | BIT(8),
			       1);	/* Reg858[10:8]=3'b001 */
		odm_set_bb_reg(p_dm, ODM_REG_RX_DEFUALT_A_11N, BIT(13) | BIT(12) | BIT(11),
			       2);	/* Reg858[13:11]=3'b010 */
	} else /* MPchip */
		odm_set_bb_reg(p_dm, ODM_REG_ANT_MAPPING1_11N, MASKDWORD,
...
)
{
	...
	odm_set_bb_reg(p_dm, 0x870, BIT(9) | BIT(8),
		       0);/* reg870[8]=1'b0, reg870[9]=1'b0		 */ /* antsel antselb by HW */
	odm_set_bb_reg(p_dm, 0x864, BIT(10),
		       0);	/* reg864[10]=1'b0	 */ /* antsel2 by HW */
	odm_set_bb_reg(p_dm, 0xb2c, BIT(22),
		       0);	/* regb2c[22]=1'b0	 */ /* disable CS/CG switch */
	odm_set_bb_reg(p_dm, 0xb2c, BIT(31),
	...
	if (p_dm->fat_comb_a == 2) {
		if (!p_dm->is_mp_chip) { /* testchip */
			odm_set_bb_reg(p_dm, 0x858, BIT(10) | BIT(9) | BIT(8),
				       1);	/* Reg858[10:8]=3'b001 */
			odm_set_bb_reg(p_dm, 0x858, BIT(13) | BIT(12) | BIT(11),
				       2);	/* Reg858[13:11]=3'b010 */
		} else { /* MPchip */
		...
		if (!p_dm->is_mp_chip) { /* testchip */
			...
			odm_set_bb_reg(p_dm, 0x858, BIT(15) | BIT(14),
				       2);	/* (Reg878[0],Reg858[14:15])=3'b010 */
			odm_set_bb_reg(p_dm, 0x878, BIT(19) | BIT(18) | BIT(17),
				       3); /* Reg878[3:1]=3b'011 */
			odm_set_bb_reg(p_dm, 0x878, BIT(22) | BIT(21) | BIT(20),
				       4); /* Reg878[6:4]=3b'100 */
			odm_set_bb_reg(p_dm, 0x878, BIT(25) | BIT(24) | BIT(23),
				       5); /* Reg878[9:7]=3b'101 */
			odm_set_bb_reg(p_dm, 0x878, BIT(28) | BIT(27) | BIT(26),
				       6); /* Reg878[12:10]=3b'110 */
			odm_set_bb_reg(p_dm, 0x878, BIT(31) | BIT(30) | BIT(29),
	...
	/* Enter Traing state */
	odm_set_bb_reg(p_dm, 0x864, BIT(2) | BIT(1) | BIT(0),
		       (p_dm->fat_comb_a -
...
)
{
	...
	if (*(p_dm->p_mp_mode) == true) {
		odm_ant_div_on_off(p_dm, ANTDIV_OFF);
		odm_set_bb_reg(p_dm, 0xc50, BIT(8),
	...
	/* Pin Settings */
	odm_set_bb_reg(p_dm, 0x870, BIT(8),
	...
	/* CCK Settings */
	odm_set_bb_reg(p_dm, 0xa04, 0xF000000,
		       0); /* Select which path to receive for CCK_1 & CCK_2 */
	odm_set_bb_reg(p_dm, 0xb34, BIT(30),
		       0); /* (92E) ANTSEL_CCK_opt = r_en_antsel_cck? ANTSEL_CCK: 1'b0 */
	odm_set_bb_reg(p_dm, 0xa74, BIT(7),
		       1); /* Fix CCK PHY status report issue */
	odm_set_bb_reg(p_dm, 0xa0c, BIT(4),
		       1); /* CCK complete HW AntDiv within 64 samples */
...
)
{
	...
	/* 3 --RFE pin setting--------- */
	/* [MAC] */
	odm_set_mac_reg(p_dm, 0x38, BIT(11),
			1);            /* DBG PAD Driving control (GPIO 8) */
	odm_set_mac_reg(p_dm, 0x4c, BIT(23),
			0);            /* path-A, RFE_CTRL_3 */
	odm_set_mac_reg(p_dm, 0x4c, BIT(29),
			1);            /* path-A, RFE_CTRL_8 */
	/* [BB] */
	odm_set_bb_reg(p_dm, 0x944, BIT(3), 1);              /* RFE_buffer */
	odm_set_bb_reg(p_dm, 0x944, BIT(8), 1);
	odm_set_bb_reg(p_dm, 0x940, BIT(7) | BIT(6),
		       0x0); /* r_rfe_path_sel_   (RFE_CTRL_3) */
	odm_set_bb_reg(p_dm, 0x940, BIT(17) | BIT(16),
	...
	odm_set_bb_reg(p_dm, 0x930, 0xF000,
		       0x8);           /* path-A, RFE_CTRL_3 */
	...
	if (p_dm->priv->pshare->rf_ft_var.use_ext_pa
	    || p_dm->priv->pshare->rf_ft_var.use_ext_lna)
		odm_set_bb_reg(p_dm, 0xC50, BIT(9), 1);	/* path-A 	output at CS */
	else
		odm_set_bb_reg(p_dm, 0xC50, BIT(9),
...
)
{
	...
	odm_set_mac_reg(p_dm, 0x4C,  BIT(24) | BIT(23),
...
)
{
	...
	if (u1_temp == 0x1) {
		...
		if (u1_temp & BIT(0)) {
			...
			odm_set_bb_reg(p_dm, 0x864, BIT(5) | BIT4 | BIT3,
				       default_ant);	/* Default RX */
			odm_set_bb_reg(p_dm, 0x864, BIT(8) | BIT7 | BIT6,
				       optional_ant);	/* Optional RX */
			odm_set_bb_reg(p_dm, 0x860, BIT(14) | BIT13 | BIT12,
				       default_ant); /* Default TX */
			p_dm_fat_table->rx_idle_ant = ant;
...
#if (DM_ODM_SUPPORT_TYPE == ODM_WIN)
			p_adapter->hal_func.set_tx_power_level_handler(p_adapter,
					*p_dm->p_channel);
#elif (DM_ODM_SUPPORT_TYPE == ODM_CE)
			rtw_hal_set_tx_power_level(p_adapter, *p_dm->p_channel);
#endif
...
)
{
	...
	odm_set_bb_reg(p_dm, 0x800, BIT(25),
		       0); /* ANTSEL_CCK sent to the smart_antenna circuit */
	odm_set_bb_reg(p_dm, 0xA00, BIT(15),
		       0); /* CCK AntDiv function block enable */

	/* BT Coexistence */
	odm_set_bb_reg(p_dm, 0xCAC, BIT(9),
		       1); /* keep antsel_map when GNT_BT = 1 */
	odm_set_bb_reg(p_dm, 0x804, BIT(4),
		       1); /* Disable hw antsw & fast_train.antsw when GNT_BT=1 */

	odm_set_bb_reg(p_dm, 0x8CC, BIT(20) | BIT(19) | BIT(18),
...
)
{
	...
	/* Timming issue */
	odm_set_bb_reg(p_dm, 0x818, BIT(23) | BIT(22) | BIT(21) | BIT(20),
...
)
{
	...
	odm_set_bb_reg(p_dm, 0x900, BIT(18),
		       0); /* TX ant  by Reg */ /* A-cut bug */
}
...
)
{
	...
	/* 3 */ /* 3 --RFE pin setting--------- */
	/* [BB] */
	odm_set_bb_reg(p_dm, 0x900, BIT(10) | BIT(9) | BIT(8),
...
)
{
	...
	if (p_dm->support_ic_type == ODM_RTL8188F) {
		if (p_dm->support_interface == ODM_ITRF_USB)
			odm_set_mac_reg(p_dm, 0x44, BIT(20) | BIT(19),
	...
	if (p_dm->support_ic_type == ODM_RTL8188F) {
		if (p_dm->support_interface == ODM_ITRF_USB) {
			if (default_ant == ANT1_2G)
				...
				codeword = 2;/*2'b10*/
			odm_set_mac_reg(p_dm, 0x44, (BIT(12) | BIT(11)),
...
)
{
	...
	if ((p_dm->support_ic_type & ODM_EVM_ENHANCE_ANTDIV_SUPPORT_IC)) {
		if (p_dm->is_one_entry_only) {
			...
			main_rssi = (p_dm_fat_table->main_ant_cnt[i] != 0) ?
				    (p_dm_fat_table->main_ant_sum[i] / p_dm_fat_table->main_ant_cnt[i]) : 0;
			aux_rssi = (p_dm_fat_table->aux_ant_cnt[i] != 0) ?
				   (p_dm_fat_table->aux_ant_sum[i] / p_dm_fat_table->aux_ant_cnt[i]) : 0;
			...
			if (((main_rssi >= evm_rssi_th_high || aux_rssi >= evm_rssi_th_high)
				...
				if (((main_rssi >= evm_rssi_th_low) || (aux_rssi >= evm_rssi_th_low))) {
					...
					if (p_dm_fat_table->fat_state_cnt < ((p_dm->antdiv_train_num) << 1)) {
						...
						if (p_dm_fat_table->fat_state_cnt == 0) {
							...
							if ((*p_dm->p_band_width == CHANNEL_WIDTH_20)
								...
								curr_rssi = (u8)((p_dm_fat_table->rx_idle_ant == MAIN_ANT) ? main_rssi :
										 aux_rssi);
								rssi_diff = (curr_rssi > p_dm_fat_table->pre_antdiv_rssi) ?
									    (curr_rssi - p_dm_fat_table->pre_antdiv_rssi) :
									    (p_dm_fat_table->pre_antdiv_rssi - curr_rssi);

								PHYDM_DBG(p_dm, DBG_ANT_DIV,
									  ("[1] rssi_return, curr_rssi=((%d)), pre_rssi=((%d))\n", curr_rssi,
									   p_dm_fat_table->pre_antdiv_rssi));
							...
							odm_set_mac_reg(p_dm, 0x608, BIT(8),
									1); /*RCR accepts CRC32-Error packets*/
						...
						odm_set_timer(p_dm, &p_dm->evm_fast_ant_training_timer,
							      p_dm->antdiv_intvl); //ms
					...
					/*Decision state: 4==============================================================*/
						...
						/* 3 [CRC32 statistic] */
#if 0
						if ((p_dm_fat_table->main_crc32_ok_cnt > ((
									p_dm_fat_table->aux_crc32_ok_cnt) << 1)) || ((diff_rssi >= 40)
											&& (rssi_larger_ant == MAIN_ANT))) {
							p_dm_fat_table->target_ant_crc32 = MAIN_ANT;
							force_antenna = true;
							PHYDM_DBG(p_dm, DBG_ANT_DIV, ("CRC32 Force Main\n"));
						} else if ((p_dm_fat_table->aux_crc32_ok_cnt > ((
									p_dm_fat_table->main_crc32_ok_cnt) << 1)) || ((diff_rssi >= 40)
											&& (rssi_larger_ant == AUX_ANT))) {
...
#endif
						{
							...
							if (main_above1 == true && aux_above1 == false) {
								...
								main_crc_utility = ((p_dm_fat_table->main_crc32_ok_cnt) << 7) /
										   p_dm_fat_table->main_crc32_fail_cnt;
								aux_crc_utility = ((p_dm_fat_table->aux_crc32_ok_cnt) << 7) /
										  p_dm_fat_table->aux_crc32_fail_cnt;
								p_dm_fat_table->target_ant_crc32 = (main_crc_utility == aux_crc_utility)
												   ? (p_dm_fat_table->pre_target_ant_enhance) : ((main_crc_utility >=
														   aux_crc_utility) ? MAIN_ANT : AUX_ANT);
								...
								main_crc_utility = ((p_dm_fat_table->main_crc32_fail_cnt) << 7) /
										   p_dm_fat_table->main_crc32_ok_cnt;
								aux_crc_utility = ((p_dm_fat_table->aux_crc32_fail_cnt) << 7) /
										  p_dm_fat_table->aux_crc32_ok_cnt;
								p_dm_fat_table->target_ant_crc32 = (main_crc_utility == aux_crc_utility)
												   ? (p_dm_fat_table->pre_target_ant_enhance) : ((main_crc_utility <=
														   aux_crc_utility) ? MAIN_ANT : AUX_ANT);
						...
						odm_set_mac_reg(p_dm, 0x608, BIT(8),
								0);/* NOT Accept CRC32 Error packets. */
						PHYDM_DBG(p_dm, DBG_ANT_DIV,
							  ("MAIN_CRC: Ok=((%d)), Fail = ((%d)), Utility = ((%d))\n",
							   p_dm_fat_table->main_crc32_ok_cnt, p_dm_fat_table->main_crc32_fail_cnt,
							   main_crc_utility));
						PHYDM_DBG(p_dm, DBG_ANT_DIV,
							  ("AUX__CRC: Ok=((%d)), Fail = ((%d)), Utility = ((%d))\n",
							   p_dm_fat_table->aux_crc32_ok_cnt, p_dm_fat_table->aux_crc32_fail_cnt,
							   aux_crc_utility));
						...
						/* 3 [EVM statistic] */
						/*1SS EVM*/
						main_1ss_evm = (p_dm_fat_table->main_ant_evm_cnt[i] != 0) ?
							       (p_dm_fat_table->main_ant_evm_sum[i] / p_dm_fat_table->main_ant_evm_cnt[i])
							       : 0;
						aux_1ss_evm = (p_dm_fat_table->aux_ant_evm_cnt[i] != 0) ?
							      (p_dm_fat_table->aux_ant_evm_sum[i] / p_dm_fat_table->aux_ant_evm_cnt[i]) :
							      0;
						target_ant_evm_1ss = (main_1ss_evm == aux_1ss_evm) ?
								     (p_dm_fat_table->pre_target_ant_enhance) : ((main_1ss_evm >= aux_1ss_evm) ?
										     MAIN_ANT : AUX_ANT);
						...
						/*2SS EVM*/
						main_2ss_evm[0] = (p_dm_fat_table->main_ant_evm_2ss_cnt[i] != 0) ?
								  (p_dm_fat_table->main_ant_evm_2ss_sum[i][0] /
								   p_dm_fat_table->main_ant_evm_2ss_cnt[i]) : 0;
						main_2ss_evm[1] = (p_dm_fat_table->main_ant_evm_2ss_cnt[i] != 0) ?
								  (p_dm_fat_table->main_ant_evm_2ss_sum[i][1] /
								   p_dm_fat_table->main_ant_evm_2ss_cnt[i]) : 0;
						main_2ss_evm_sum = main_2ss_evm[0] + main_2ss_evm[1];

						aux_2ss_evm[0] = (p_dm_fat_table->aux_ant_evm_2ss_cnt[i] != 0) ?
								 (p_dm_fat_table->aux_ant_evm_2ss_sum[i][0] /
								  p_dm_fat_table->aux_ant_evm_2ss_cnt[i]) : 0;
						aux_2ss_evm[1] = (p_dm_fat_table->aux_ant_evm_2ss_cnt[i] != 0) ?
								 (p_dm_fat_table->aux_ant_evm_2ss_sum[i][1] /
								  p_dm_fat_table->aux_ant_evm_2ss_cnt[i]) : 0;
						aux_2ss_evm_sum = aux_2ss_evm[0] + aux_2ss_evm[1];

						target_ant_evm_2ss = (main_2ss_evm_sum == aux_2ss_evm_sum) ?
								     (p_dm_fat_table->pre_target_ant_enhance) : ((main_2ss_evm_sum >=
										     aux_2ss_evm_sum) ? MAIN_ANT : AUX_ANT);

						PHYDM_DBG(p_dm, DBG_ANT_DIV,
							  ("Cnt = ((%d)), Main2ss_EVM{A,B,Sum} = {%d, %d, %d}\n",
							   p_dm_fat_table->main_ant_evm_2ss_cnt[i], main_2ss_evm[0], main_2ss_evm[1],
							   main_2ss_evm_sum));
						PHYDM_DBG(p_dm, DBG_ANT_DIV,
							  ("Cnt = ((%d)), Aux_2ss_EVM{A,B,Sum} = {%d, %d, %d}\n",
							   p_dm_fat_table->aux_ant_evm_2ss_cnt[i], aux_2ss_evm[0], aux_2ss_evm[1],
							   aux_2ss_evm_sum));
						...
						PHYDM_DBG(p_dm, DBG_ANT_DIV, ("***2.TargetAnt_EVM((%d-ss)) = ((%s))\n",
									      decision_evm_ss, (p_dm_fat_table->target_ant_evm == MAIN_ANT) ?
									      "MAIN_ANT" : "AUX_ANT"));


						//3 [TP statistic]
						antdiv_tp_main_avg = (p_dm_fat_table->antdiv_tp_main_cnt != 0) ?
								     (p_dm_fat_table->antdiv_tp_main / p_dm_fat_table->antdiv_tp_main_cnt) : 0;
						antdiv_tp_aux_avg = (p_dm_fat_table->antdiv_tp_aux_cnt != 0) ?
								    (p_dm_fat_table->antdiv_tp_aux / p_dm_fat_table->antdiv_tp_aux_cnt) : 0;
						p_dm_fat_table->target_ant_tp = (antdiv_tp_main_avg == antdiv_tp_aux_avg)
										? (p_dm_fat_table->pre_target_ant_enhance) : ((antdiv_tp_main_avg >=
						...
						if (p_dm_fat_table->target_ant_evm == p_dm_fat_table->target_ant_crc32) {
							...
							if ((utility_ratio < 2 && force_antenna == false) && diff_EVM <= 30)
								p_dm_fat_table->target_ant_enhance =
									p_dm_fat_table->pre_target_ant_enhance;
							else
						...
						PHYDM_DBG(p_dm, DBG_ANT_DIV, ("*** 4.TargetAnt_enhance = (( %s ))******\n",
									      (p_dm_fat_table->target_ant_enhance == MAIN_ANT) ? "MAIN_ANT" :
									      "AUX_ANT"));
	...
	if (!p_dm->is_linked) { /* is_linked==False */
		...
		if (p_dm_fat_table->is_become_linked == false) {
			...
			if (p_dm->support_ic_type == ODM_RTL8723B
				...
				odm_set_bb_reg(p_dm, 0x930, 0xF0,
	...
	for (i = 0; i < ODM_ASSOCIATE_ENTRY_NUM; i++) {
		...
		if (is_sta_active(p_sta)) {
			...
			if ((p_dm_fat_table->main_ant_cnt[i] != 0)
				...
				main_rssi = (mian_cnt != 0) ? (p_dm_fat_table->main_ant_sum_cck[i] /
							       mian_cnt) : 0;
				aux_rssi = (aux_cnt != 0) ? (p_dm_fat_table->aux_ant_sum_cck[i] / aux_cnt)
					   : 0;
				target_ant = (main_rssi == aux_rssi) ? p_dm_fat_table->rx_idle_ant : ((
			...
			PHYDM_DBG(p_dm, DBG_ANT_DIV,
				  ("*** Client[ %d ] : Main_Cnt = (( %d ))  ,  CCK_Main_Cnt = (( %d )) ,  main_rssi= ((  %d ))\n",
				   i, p_dm_fat_table->main_ant_cnt[i], p_dm_fat_table->main_ant_cnt_cck[i],
				   main_rssi));
			PHYDM_DBG(p_dm, DBG_ANT_DIV,
				  ("*** Client[ %d ] : Aux_Cnt   = (( %d ))  , CCK_Aux_Cnt   = (( %d )) ,  aux_rssi = ((  %d ))\n",
				   i, p_dm_fat_table->aux_ant_cnt[i], p_dm_fat_table->aux_ant_cnt_cck[i],
				   aux_rssi));
			...
			if (p_dm_bdc_table->bdc_mode == BDC_MODE_2
				...
				if ((ma_rx_temp < TH2)   && (ma_rx_temp > TH1)
					...
					if (p_dm_bdc_table->w_bfer_client[i] == 1) { /* Bfer_Target */
						...
						if (p_dm_bdc_table->BDC_state == BDC_DECISION_STATE
						    && p_dm_bdc_table->bdc_try_flag == 0) {
							improve_TP_temp = (p_dm_bdc_table->MA_rx_TP_DIV[i] * 9) >> 3
									  ; /* * 1.125 */
							p_dm_bdc_table->BF_pass = (p_dm_bdc_table->MA_rx_TP[i] > improve_TP_temp) ?
										  true : false;
							PHYDM_DBG(p_dm, DBG_ANT_DIV,
								  ("*** Client[ %d ] :  { MA_rx_TP,improve_TP_temp, MA_rx_TP_DIV,  BF_pass}={ %d,  %d, %d , %d }\n",
								   i, p_dm_bdc_table->MA_rx_TP[i], improve_TP_temp,
								   p_dm_bdc_table->MA_rx_TP_DIV[i], p_dm_bdc_table->BF_pass));
						}
					} else { /* DIV_Target */
						...
						if (p_dm_bdc_table->BDC_state == BDC_DECISION_STATE
							...
							p_dm_bdc_table->DIV_pass = (p_dm_bdc_table->MA_rx_TP[i] > degrade_TP_temp)
										   ? true : false;
							PHYDM_DBG(p_dm, DBG_ANT_DIV,
								  ("*** Client[ %d ] :  { MA_rx_TP, degrade_TP_temp, MA_rx_TP_DIV,  DIV_pass}=\n{ %d,  %d, %d , %d }\n",
								   i, p_dm_bdc_table->MA_rx_TP[i], degrade_TP_temp,
								   p_dm_bdc_table->MA_rx_TP_DIV[i], p_dm_bdc_table->DIV_pass));
						}
					}
				}
				...
				PHYDM_DBG(p_dm, DBG_ANT_DIV,
					  ("*** Client[ %d ] :  { BFmeeCap, BFmerCap}  = { %d , %d }\n", i,
					   p_dm_bdc_table->w_bfee_client[i], p_dm_bdc_table->w_bfer_client[i]));

				if (p_dm_bdc_table->BDC_state == bdc_bfer_train_state)
					PHYDM_DBG(p_dm, DBG_ANT_DIV,
						  ("*** Client[ %d ] :    MA_rx_TP_DIV = (( %d ))\n", i,
						   p_dm_bdc_table->MA_rx_TP_DIV[i]));

				else
					PHYDM_DBG(p_dm, DBG_ANT_DIV,
						  ("*** Client[ %d ] :    MA_rx_TP = (( %d ))\n", i,
						   p_dm_bdc_table->MA_rx_TP[i]));
...
#if (BEAMFORMING_SUPPORT == 1)
#if (DM_ODM_SUPPORT_TYPE == ODM_AP)
	if (p_dm_bdc_table->bdc_mode == BDC_MODE_1
	    || p_dm_bdc_table->bdc_mode == BDC_MODE_3) {
		PHYDM_DBG(p_dm, DBG_ANT_DIV,
...
)
{
	...
	PHYDM_DBG(p_dm, DBG_ANT_DIV,
		  ("[%d] { try_flag=(( %d )), step=(( %d )), double_chk_flag = (( %d )) }\n",
		   __LINE__, p_dm_swat_table->try_flag, step,
		   p_dm_swat_table->double_chk_flag));
	...
	if (p_dm_swat_table->try_flag == SWAW_STEP_INIT) {
		...
		if (p_dm_swat_table->try_flag == SWAW_STEP_PEEK) {
			...
			} else if ((p_dm->traffic_load == TRAFFIC_MID)
			...
			if ((p_dm_fat_table->min_max_rssi > RSSI_CHECK_THRESHOLD)
				...
				odm_set_timer(p_dm, &(p_dm_swat_table->phydm_sw_antenna_switch_timer),
					      p_dm_swat_table->train_time); /*ms*/
				return;
			}
		...
		} else if ((p_dm_swat_table->try_flag == SWAW_STEP_DETERMINE)
			   && (p_dm_swat_table->double_chk_flag == 0)) {
		...
		/* 1 Decision state */
		if ((p_dm_swat_table->try_flag == SWAW_STEP_DETERMINE)
		    && (p_dm_swat_table->rssi_trying == 0)) {
			...
			for (i = 0; i < ODM_ASSOCIATE_ENTRY_NUM; i++) {
				...
				if (is_sta_active(p_entry)) {
					...
					if ((p_dm_fat_table->main_ant_cnt[i] != 0)
						...
						main_rssi = (main_ant_cnt != 0) ? (p_dm_fat_table->main_ant_sum[i] /
										   main_ant_cnt) : 0;
						aux_rssi = (aux_ant_cnt != 0) ? (p_dm_fat_table->aux_ant_sum[i] /
										 aux_ant_cnt) : 0;
						if (p_dm->support_ic_type == ODM_RTL8723D) {
							if (p_dm_swat_table->pre_antenna == MAIN_ANT) {
								if (main_ant_cnt == 0) {
									...
									target_ant = ((aux_ant_cnt > main_ant_cnt) && ((main_rssi - aux_rssi < 5)
											|| (aux_rssi > main_rssi))) ? AUX_ANT : p_dm_swat_table->pre_antenna;
								}
							} else {
								if (aux_ant_cnt == 0) {
									target_ant = (main_ant_cnt != 0) ? MAIN_ANT : p_dm_swat_table->pre_antenna;
								} else {
									target_ant = ((main_ant_cnt > aux_ant_cnt) && ((aux_rssi - main_rssi < 5)
											|| (main_rssi > aux_rssi))) ? MAIN_ANT : p_dm_swat_table->pre_antenna;
								}
							}
						} else {
							if (p_dm_swat_table->pre_antenna == MAIN_ANT) {
								target_ant = ((aux_ant_cnt > 20)
									      && (aux_rssi >= main_rssi)) ? AUX_ANT : p_dm_swat_table->pre_antenna;
							} else {
								target_ant = ((main_ant_cnt > 20)
									      && (main_rssi >= aux_rssi)) ? MAIN_ANT : p_dm_swat_table->pre_antenna;
							}
						}
					} else {	/*CCK only case*/
						...
						main_rssi = (main_ant_cnt != 0) ? (p_dm_fat_table->main_ant_sum_cck[i] /
										   main_ant_cnt) : 0;
						aux_rssi = (aux_ant_cnt != 0) ? (p_dm_fat_table->aux_ant_sum_cck[i] /
										 aux_ant_cnt) : 0;
						target_ant = (main_rssi == aux_rssi) ? p_dm_fat_table->rx_idle_ant : ((
									main_rssi >= aux_rssi) ? MAIN_ANT :
								AUX_ANT); /*Use RSSI for CCK only case*/
			...
			if (min_max_rssi == 0xff
			    || ((pkt_cnt_total < (p_dm_swat_table->pkt_cnt_sw_ant_div_by_ctrl_frame >>
						  1)) && p_dm->phy_dbg_info.num_qry_beacon_pkt < 2)) {
				min_max_rssi = 0;
				PHYDM_DBG(p_dm, DBG_ANT_DIV,
				...
				if (is_by_ctrl_frame) {
					main_rssi = (p_dm_fat_table->main_ant_ctrl_frame_cnt != 0) ?
						    (p_dm_fat_table->main_ant_ctrl_frame_sum /
						     p_dm_fat_table->main_ant_ctrl_frame_cnt) : 0;
					aux_rssi = (p_dm_fat_table->aux_ant_ctrl_frame_cnt != 0) ?
						   (p_dm_fat_table->aux_ant_ctrl_frame_sum /
						    p_dm_fat_table->aux_ant_ctrl_frame_cnt) : 0;

					if (p_dm_fat_table->main_ant_ctrl_frame_cnt <= 1
					    && p_dm_fat_table->cck_ctrl_frame_cnt_main >= 1)
						main_rssi = 0;

					if (p_dm_fat_table->aux_ant_ctrl_frame_cnt <= 1
					    && p_dm_fat_table->cck_ctrl_frame_cnt_aux >= 1)
						aux_rssi = 0;

					if (main_rssi != 0 || aux_rssi != 0) {
						rx_idle_ant = (main_rssi == aux_rssi) ? p_dm_swat_table->pre_antenna : ((
	...
	PHYDM_DBG(p_dm, DBG_ANT_DIV,
		  ("rssi_trying = (( %d )),    ant: (( %s )) >>> (( %s ))\n",
		   p_dm_swat_table->rssi_trying,
		   (p_dm_fat_table->rx_idle_ant  == MAIN_ANT ? "MAIN" : "AUX"),
...
)
{
	...
	struct _sw_antenna_switch_			*p_dm_swat_table =
			&p_hal_data->DM_OutSrc.dm_swat_table;
...
)
{
	...
	is_cck_rate = ((p_pktinfo->data_rate >= DESC_RATE1M)
		       && (p_pktinfo->data_rate <= DESC_RATE11M)) ? true : false;

	if (is_cck_rate) {
		p_dm_fat_table->antsel_rx_keep_0 = (p_dm_fat_table->rx_idle_ant ==
						    MAIN_ANT) ? ANT1_2G : ANT2_2G;
		...
		odm_antsel_statistics_of_ctrl_frame(p_dm, p_dm_fat_table->antsel_rx_keep_0,
						    p_phy_info->rx_mimo_signal_strength[RF_PATH_A]);
	} else {
		p_dm_fat_table->antsel_rx_keep_0 = (p_dm_fat_table->rx_idle_ant ==
						    MAIN_ANT) ? ANT1_2G : ANT2_2G;
	...
	if (p_dm->is_linked) {
		for (i = 0; i < ODM_ASSOCIATE_ENTRY_NUM; i++) {
			...
			if (is_sta_active(p_entry)) {
				...
				value32 = (p_entry->mac_addr[3] << 24) | (p_entry->mac_addr[2] << 16) |
					  (p_entry->mac_addr[1] << 8) | p_entry->mac_addr[0];
				...
				PHYDM_DBG(p_dm, DBG_ANT_DIV, ("Training MAC addr = %x:%x:%x:%x:%x:%x\n",
							      p_entry->mac_addr[5], p_entry->mac_addr[4], p_entry->mac_addr[3],
							      p_entry->mac_addr[2], p_entry->mac_addr[1], p_entry->mac_addr[0]));
...
)
{
	...
	if (p_dm->support_ic_type == ODM_RTL8188E)
		...
		odm_set_bb_reg(p_dm, 0xB38, BIT(2) | BIT(1) | BIT(0),
			       ((p_dm->fat_comb_a) -
				1));	   /* path-A  */ /* ant combination=regB38[2:0]+1 */
		odm_set_bb_reg(p_dm, 0xB38, BIT(18) | BIT(17) | BIT(16),
			       ((p_dm->fat_comb_b) -
	...
	if (p_dm_fat_table->fat_state == FAT_TRAINING_STATE) {
		...
		for (i = 0; i < (p_dm->fat_comb_a); i++) { /* i : antenna index */
			if (p_dm_fat_table->ant_rssi_cnt[i] == 0)
				...
				p_dm_fat_table->ant_ave_rssi[i] = p_dm_fat_table->ant_sum_rssi[i] /
								  p_dm_fat_table->ant_rssi_cnt[i];
		...
		for (i = 0; i < (p_dm->fat_comb_b); i++) {
			if (p_dm_fat_table->antRSSIcnt_pathB[i] == 0)
				...
				p_dm_fat_table->antAveRSSI_pathB[i] = p_dm_fat_table->antSumRSSI_pathB[i] /
								      p_dm_fat_table->antRSSIcnt_pathB[i];
			...
			if (p_dm->fat_print_rssi == 1) {
				PHYDM_DBG(p_dm, DBG_ANT_DIV,
					  ("***{path-B}: Sum RSSI[%d] = (( %d )),      cnt RSSI [%d] = (( %d )),     Avg RSSI[%d] = (( %d ))\n",
					   i, p_dm_fat_table->antSumRSSI_pathB[i], i,
					   p_dm_fat_table->antRSSIcnt_pathB[i], i,
					   p_dm_fat_table->antAveRSSI_pathB[i]));
			}
		}
#endif
#endif
		...
		if (is_pkt_filter_macth_path_b == false) {
			...
			odm_set_bb_reg(p_dm, 0xB38, BIT(21) | BIT20 | BIT19,
				       target_ant_path_b);	/* Default RX is Omni, Optional RX is the best decision by FAT */
			odm_set_bb_reg(p_dm, 0x80c, BIT(21),
				       1); /* Reg80c[21]=1'b1		//from TX Info */

			p_dm_fat_table->antsel_pathB[p_dm_fat_table->train_idx] =
				target_ant_path_b;
		}
#endif
#endif
	...
	if (p_dm_fat_table->fat_state == FAT_PREPARE_STATE) {
		...
		odm_set_timer(p_dm, &p_dm->fast_ant_training_timer,
			      p_dm->antdiv_intvl); /* ms */
	}
...
)
{
	...
	if (p_dm_fat_table->ant_div_2g_5g == ODM_ANTDIV_2G) {
		...
		if (!(p_dm->support_ic_type & ODM_ANTDIV_5G_SUPPORT_IC))
			return;
	} else	if (p_dm_fat_table->ant_div_2g_5g == (ODM_ANTDIV_2G |
			ODM_ANTDIV_5G))
		PHYDM_DBG(p_dm, DBG_ANT_DIV,
	...
	p_dm_fat_table->rx_idle_ant =
		0xff; /*to make RX-idle-antenna will be updated absolutly*/
	odm_update_rx_idle_ant(p_dm, MAIN_ANT);
	phydm_keep_rx_ack_ant_by_tx_ant_time(p_dm,
	...
	if (p_dm_fat_table->p_force_tx_ant_by_desc == NULL) {
		...
		p_dm_fat_table->p_force_tx_ant_by_desc = &
				(p_dm_fat_table->force_tx_ant_by_desc);
	}
...
)
{
...

	{
		...
		if (!p_dm->is_bt_continuous_turn) {
			if ((beamform_cap & BEAMFORMEE_CAP)
			    && (!(*p_dm_fat_table->is_no_csi_feedback))) { /* BFmee On  &&   Div On->Div Off */
				PHYDM_DBG(p_dm, DBG_ANT_DIV,
	...
	/* 2 [--8821C---] */
		...
		if (!p_dm->is_bt_continuous_turn) {
			p_dm->ant_div_type = S0S1_SW_ANTDIV;
			ODM_RT_TRACE(p_dm, DBG_ANT_DIV, ODM_DBG_LOUD,
...
)
{
	...
	if (method == RSSI_METHOD) {
		...
		if (is_cck_rate) {
			if (antsel_tr_mux == ANT1_2G) {
				if (p_dm_fat_table->main_ant_sum_cck[mac_id] >
		...
		if (p_dm->rate_ss == 1) {
			...
			if (antsel_tr_mux == ANT1_2G) {
				p_dm_fat_table->main_ant_evm_sum[mac_id] += ((
							p_phy_info->rx_mimo_evm_dbm[0]) << 5);
				p_dm_fat_table->main_ant_evm_cnt[mac_id]++;
			} else {
				p_dm_fat_table->aux_ant_evm_sum[mac_id] += ((
							p_phy_info->rx_mimo_evm_dbm[0]) << 5);
			...
			if (antsel_tr_mux == ANT1_2G) {
				...
				p_dm_fat_table->main_ant_evm_2ss_sum[mac_id][0] +=
					(p_phy_info->rx_mimo_evm_dbm[0] << 5);
				p_dm_fat_table->main_ant_evm_2ss_sum[mac_id][1] +=
					(p_phy_info->rx_mimo_evm_dbm[1] << 5);
				p_dm_fat_table->main_ant_evm_2ss_cnt[mac_id]++;

			} else {

				p_dm_fat_table->aux_ant_evm_2ss_sum[mac_id][0] +=
					(p_phy_info->rx_mimo_evm_dbm[0] << 5);
				p_dm_fat_table->aux_ant_evm_2ss_sum[mac_id][1] +=
					(p_phy_info->rx_mimo_evm_dbm[1] << 5);
...
)
{
	...
	if ((p_dm->ant_div_type == HL_SW_SMART_ANT_TYPE2)
	    && (p_dm_fat_table->fat_state == FAT_TRAINING_STATE))
		phydm_process_rssi_for_hb_smtant_type2(p_dm, p_phy_info, p_pktinfo,
						       rssi_avg);	/*for 8822B*/
	else
#endif
		...
		if ((p_dm->ant_div_type == HL_SW_SMART_ANT_TYPE1)
			...
			if (p_pktinfo->is_packet_beacon) {
				...
				if (pdm_sat_table->beacon_counter >= pdm_sat_table->pre_beacon_counter +
				    2) {
					...
					PHYDM_DBG(p_dm, DBG_ANT_DIV,
						  ("pre_beacon_counter = ((%d)), pkt_counter = ((%d)), update_beam_idx = ((%d))\n",
						   pdm_sat_table->pre_beacon_counter, pdm_sat_table->pkt_counter,
						   pdm_sat_table->update_beam_idx));
					...
					*/
					PHYDM_DBG(p_dm, DBG_ANT_DIV,
						  ("ID[%d][pkt_cnt = %d]: {ANT, Beam} = {%d, %d}, RSSI = ((%d))\n",
						   p_pktinfo->station_id, pdm_sat_table->pkt_counter,
						   p_dm_fat_table->antsel_rx_keep_0, pdm_sat_table->fast_training_beam_num,
						   rx_power_ant0));
...
#if 1
					train_pkt_number =
						pdm_sat_table->beam_train_cnt[p_dm_fat_table->rx_idle_ant -
									      1][pdm_sat_table->fast_training_beam_num];
#else
			...
			if (pdm_sat_table->update_beam_idx == pdm_sat_table->ant_num) {
				...
				if (pdm_sat_table->fast_training_beam_num >=
				    (pdm_sat_table->beam_patten_num_each_ant - 1)) {
					...
					*/
					PHYDM_DBG(p_dm, DBG_ANT_DIV,
						  ("StaID[%d]:  antsel_pathA = ((%d)), is_packet_to_self = ((%d)), Beam_num = ((%d)), RSSI = ((%d))\n",
						   p_pktinfo->station_id, p_dm_fat_table->antsel_rx_keep_0,
						   p_pktinfo->is_packet_to_self, pdm_sat_table->fast_training_beam_num,
						   rx_power_ant0));
...
#endif
			if (p_dm->ant_div_type == CG_TRX_SMART_ANTDIV) {
				if ((p_dm->support_ic_type & ODM_SMART_ANT_SUPPORT)
				    && (p_pktinfo->is_packet_to_self)
				    && (p_dm_fat_table->fat_state ==
					FAT_TRAINING_STATE)) { /* (p_pktinfo->is_packet_match_bssid && (!p_pktinfo->is_packet_beacon)) */
					u8	antsel_tr_mux;
					antsel_tr_mux = (p_dm_fat_table->antsel_rx_keep_2 << 2) |
							(p_dm_fat_table->antsel_rx_keep_1 << 1) | p_dm_fat_table->antsel_rx_keep_0;
				...
				if ((p_dm->support_ic_type & ODM_ANTDIV_SUPPORT)
				    && (p_pktinfo->is_packet_to_self
					|| p_dm_fat_table->use_ctrl_frame_antdiv)) {
					...
					if (p_dm->ant_div_type == S0S1_SW_ANTDIV) {
						...
						odm_antsel_statistics(p_dm, p_phy_info, p_dm_fat_table->antsel_rx_keep_0,
								      p_pktinfo->station_id, rx_power_ant0, RSSI_METHOD, is_cck_rate);
						...
						if (p_dm->support_ic_type == ODM_RTL8192E) {
							if (!is_cck_rate) {
								odm_antsel_statistics(p_dm, p_phy_info, p_dm_fat_table->antsel_rx_keep_0,
										      p_pktinfo->station_id, rx_evm_ant0, EVM_METHOD, is_cck_rate);
								odm_antsel_statistics(p_dm, p_phy_info, p_dm_fat_table->antsel_rx_keep_0,
										      p_pktinfo->station_id, rx_evm_ant0, TP_METHOD, is_cck_rate);
							}
...
)
{
	...
	if (state == INIT_ANTDIV_TIMMER) {
		...
		odm_initialize_timer(p_dm,
				     &(p_dm->dm_swat_table.phydm_sw_antenna_switch_timer),
