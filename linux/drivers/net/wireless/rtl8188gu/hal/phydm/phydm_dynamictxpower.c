...
)
{
	...
	p_dm->tx_agc_ofdm_18_6 = odm_get_bb_reg(p_dm, 0xC24,
						MASKDWORD); /*TXAGC {18M 12M 9M 6M}*/
...
)
{
	...
	for (index = 0; index < 6; index++)
		p_hal_data->PowerIndex_backup[index] = PlatformEFIORead1Byte(adapter,
...
)
{
	...
	for (index = 0; index < 6; index++)
		PlatformEFIOWrite1Byte(adapter, power_index_reg[index],
				       p_hal_data->PowerIndex_backup[index]);
...
void odm_dynamic_tx_power_nic_ce(
	void					*p_dm_void
)
{
	...
	if (p_dm->last_dtp_lvl != p_dm->dynamic_tx_high_power_lvl) {
		...
		if (p_dm->support_ic_type & (ODM_RTL8821)) {
			...
			if (p_dm->dynamic_tx_high_power_lvl == tx_high_pwr_level_level2) {
				...
				odm_set_mac_reg(p_dm, 0x6D8, BIT(20) | BIT19 | BIT18,
...
)
{
	...
	if (p_dm->support_ic_type == ODM_RTL8814A)
		...
		if (p_mgnt_info->RegRspPwr == 1)	{
			if (p_dm->rssi_min > 60)
				odm_set_mac_reg(p_dm, ODM_REG_RESP_TX_11AC, BIT(20) | BIT19 | BIT18,
						1); /*Resp TXAGC offset = -3dB*/
...
)
{
	...
	PHYDM_DBG(p_dm, DBG_DYN_TXPWR,
		  ("TxLevel=%d p_mgnt_info->iot_action=%x p_mgnt_info->is_dynamic_tx_power_enable=%d\n",
		   p_hal_data->DynamicTxHighPowerLvl, p_mgnt_info->IOTAction,
		   p_mgnt_info->bDynamicTxPowerEnable));
	...
	if ((p_mgnt_info->bDynamicTxPowerEnable != true)
	    || p_mgnt_info->IOTAction & HT_IOT_ACT_DISABLE_HIGH_POWER)
		p_hal_data->DynamicTxHighPowerLvl = tx_high_pwr_level_normal;
	else {
		...
		} else if ((undecorated_smoothed_pwdb < (TX_POWER_NEAR_FIELD_THRESH_LVL2 -
				3)) &&
		...
		} else if (undecorated_smoothed_pwdb < (TX_POWER_NEAR_FIELD_THRESH_LVL1 -
							5)) {
			p_hal_data->DynamicTxHighPowerLvl = tx_high_pwr_level_normal;
	...
	if (p_hal_data->DynamicTxHighPowerLvl != p_hal_data->LastDTPLvl) {
		PHYDM_DBG(p_dm, DBG_DYN_TXPWR,
			  ("odm_dynamic_tx_power_8814a() channel = %d\n",
			   p_hal_data->CurrentChannel));
		odm_set_tx_power_level8814(adapter, p_hal_data->CurrentChannel,
					   p_hal_data->DynamicTxHighPowerLvl);
	}


	PHYDM_DBG(p_dm, DBG_DYN_TXPWR,
		  ("odm_dynamic_tx_power_8814a() channel = %d  TXpower lvl=%d/%d\n",
		   p_hal_data->CurrentChannel, p_hal_data->LastDTPLvl,
		   p_hal_data->DynamicTxHighPowerLvl));
...
)
{
	...
	for (path = RF_PATH_A; path <= RF_PATH_D; ++path) {
		...
		for (i = 0; i <= 16; i++) {
			for (j = 0; j <= 3; j++) {
				...
				power_index = (u32) PHY_GetTxPowerIndex(adapter, (u8)path,
									jaguar2_rates[i][j], p_hal_data->CurrentChannelBW, channel);

				/*for Query bus type to recude tx power.*/
				if (usb_host != USB_MODE_U3 && usb_rfset == 1
				    && IS_HARDWARE_TYPE_8814AU(adapter) && usb_rf_type == RF_3T3R) {
					if (channel <= 14) {
						if (power_index >= 16)
							power_index -= 16;
						else
				...
				txagc_table_wd |= (path << 8) | MRateToHwRate(jaguar2_rates[i][j]) |
						  (power_index << 24);

				PHY_SetTxPowerIndexShadow(adapter, (u8)power_index, (u8)path,
							  jaguar2_rates[i][j]);
	...
	if (adapter->MgntInfo.bScanInProgress == false
	    &&  adapter->MgntInfo.RegFWOffload == 2)
