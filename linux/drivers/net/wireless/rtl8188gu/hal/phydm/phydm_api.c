...
)
{
	...
	} else if (p_dm->support_ic_type & (ODM_RTL8723D | ODM_RTL8821C |
...
)
{
	...
	if (p_dm->support_ic_type & (ODM_RTL8192E)) {
		...
		odm_set_bb_reg(p_dm, 0x808, MASKBYTE0,
...
)
{
	...
	if (set_type == PHYDM_SET) {
		...
		if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
			...
			odm_set_bb_reg(p_dm, 0x88c, 0xf00000,
...
)
{
	...
	if (set_type == PHYDM_SET) {
		...
		odm_set_bb_reg(p_dm, 0x520, 0xff0000,
...
)
{
	...
	if (enable == FUNC_DISABLE) {
		...
		/*calculate fc*/
		if (phydm_calculate_fc(p_dm, channel, bw, second_ch,
				       &fc) == PHYDM_SET_FAIL)
			set_result = PHYDM_SET_FAIL;

		else {
			/*calculate interference distance*/
			if (phydm_calculate_intf_distance(p_dm, bw, fc, f_interference,
...
)
{
	...
	if (function_map == PHYDM_API_NBI) {
		...
		if (dm_value[0] == 100) {
			...
			PHYDM_SNPRINTF((output + used, out_len - used,
					...
					channel, bw, f_interference, ((second_ch == PHYDM_DONT_CARE) || (bw == 20)
							|| (channel > 14)) ? "Don't care" : ((second_ch == PHYDM_ABOVE) ? "H" :
									"L")));
			set_result = phydm_nbi_setting(p_dm, FUNC_ENABLE, channel, bw,
						       f_interference, second_ch);

		} else if (dm_value[0] == FUNC_DISABLE) {

			PHYDM_SNPRINTF((output + used, out_len - used, "[Disable NBI]\n"));
			set_result = phydm_nbi_setting(p_dm, FUNC_DISABLE, channel, bw,
						       f_interference, second_ch);
	...
	/*-------------------------------------------------------------------------------------------------------------------------------*/
		...
		if (dm_value[0] == 100) {
			...
			set_result = phydm_csi_mask_setting(p_dm,	FUNC_ENABLE, channel, bw,
							    f_interference, second_ch);

		} else if (dm_value[0] == FUNC_DISABLE) {

			PHYDM_SNPRINTF((output + used, out_len - used, "[Disable CSI MASK]\n"));
			set_result = phydm_csi_mask_setting(p_dm, FUNC_DISABLE, channel, bw,
							    f_interference, second_ch);
...
)
{
	...
	if (phy_status == true) {
		...
		if ((p_dm->rx_pkt_type == type_block_ack)
		    || (p_dm->rx_pkt_type == type_rts) || (p_dm->rx_pkt_type == type_cts))
			seq_num = 0;
		else
			seq_num = p_rt_rfd_status->seq_num;

		PHYDM_DBG_F(p_dm, ODM_COMP_SNIFFER,
			    ("%04d , %01s, rate=0x%02x, L=%04d , %s , %s",
			     seq_num,
			     /*p_rt_rfd_status->mac_id,*/
			     ((p_rt_rfd_status->is_crc) ? "C" : (p_rt_rfd_status->is_ampdu) ? "A" :
			      "_"),
			     p_rt_rfd_status->data_rate,
			     p_rt_rfd_status->length,
			     ((p_rt_rfd_status->band_width == 0) ? "20M" : ((p_rt_rfd_status->band_width
					     == 1) ? "40M" : "80M")),
