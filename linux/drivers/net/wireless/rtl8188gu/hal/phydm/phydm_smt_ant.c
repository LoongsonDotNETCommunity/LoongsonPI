...
)
{
	...
	path_a_0to3_idx = ((table_path_a[3] & 0xf) << 24) | ((
				  table_path_a[2] & 0xf) << 16)
			  | ((table_path_a[1] & 0xf) << 8) | (table_path_a[0] & 0xf);

	path_b_0to3_idx = ((table_path_b[3] & 0xf) << 28) | ((
				  table_path_b[2] & 0xf) << 20)
			  | ((table_path_b[1] & 0xf) << 12) | ((table_path_b[0] & 0xf) << 4);

	path_a_4to7_idx = ((table_path_a[7] & 0xf) << 24) | ((
				  table_path_a[6] & 0xf) << 16)
			  | ((table_path_a[5] & 0xf) << 8) | (table_path_a[4] & 0xf);

	path_b_4to7_idx = ((table_path_b[7] & 0xf) << 28) | ((
				  table_path_b[6] & 0xf) << 20)
...
)
{
	...
	struct	smt_ant_cumitek		*p_cumi_smtant_table = &
			(p_dm->smtant_table.cumi_smtant_table);
	u32		value32;
...
)
{
	...
	/*path-A*/
	p_cumi_smtant_table->tx_ant_idx[0][mac_id] =
		tx_ant_idx_path_a; /*fill this value into TXDESC*/

	/*path-B*/
	p_cumi_smtant_table->tx_ant_idx[1][mac_id] =
		tx_ant_idx_path_b; /*fill this value into TXDESC*/
}
...
)
{
	...
	if (p_cumi_smtant_table->rx_default_ant_idx[0] != rx_ant_idx_path_a) {
		...
		if (p_dm->support_ic_type == ODM_RTL8822B) {
			...
			odm_set_bb_reg(p_dm, 0xc08, BIT(21) | BIT(20) | BIT(19),
				       rx_ant_idx_path_a); /*default RX antenna*/
			odm_set_mac_reg(p_dm, 0x6d8, BIT(2) | BIT(1) | BIT(0),
					rx_ant_idx_path_a); /*default response TX antenna*/
		}
#endif
	...
	if (p_cumi_smtant_table->rx_default_ant_idx[1] != rx_ant_idx_path_b) {
		...
		if (p_dm->support_ic_type == ODM_RTL8822B) {
			...
			odm_set_bb_reg(p_dm, 0xe08, BIT(21) | BIT(20) | BIT(19),
				       rx_ant_idx_path_b); /*default antenna*/
			odm_set_mac_reg(p_dm, 0x6d8, BIT(5) | BIT(4) | BIT(3),
					rx_ant_idx_path_b); /*default response TX antenna*/
		}
#endif
...
)
{
	...
	if (strcmp(input[1], help) == 0) {
		...
		phydm_cumitek_smt_rx_default_ant_update(p_dm, (u8)dm_value[1],
							(u8)dm_value[2]);
		...
		phydm_cumitek_smt_tx_ant_update(p_dm, (u8)dm_value[1], (u8)dm_value[2],
						(u8)dm_value[3]);
		...
		PHYDM_SNPRINTF((output + used, out_len - used,
				...
				table_path_a[7], table_path_a[6], table_path_a[5], table_path_a[4],
				table_path_a[3], table_path_a[2], table_path_a[1], table_path_a[0]));
		PHYDM_SNPRINTF((output + used, out_len - used,
				"Ant_Table_B[7:0]={0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x}\n",
				table_path_b[7], table_path_b[6], table_path_b[5], table_path_b[4],
				table_path_b[3], table_path_b[2], table_path_b[1], table_path_b[0]));

		phydm_cumitek_smt_ant_mapping_table_8822b(p_dm, &(table_path_a[0]),
				&(table_path_b[0]));
...
)
{
	...
	odm_move_memory(p_dm, pdm_sat_table->rfu_codeword_table_2g,
			rfu_codeword_table_init_2g,
			(SUPPORT_BEAM_SET_PATTERN_NUM * MAX_PATH_NUM_8822B));
	odm_move_memory(p_dm, pdm_sat_table->rfu_codeword_table_5g,
			rfu_codeword_table_init_5g,
			(SUPPORT_BEAM_SET_PATTERN_NUM * MAX_PATH_NUM_8822B));

	/*GPIO setting*/
	odm_set_mac_reg(p_dm, 0x64, (BIT(18) | BIT(17) | BIT(16)), 0);
	odm_set_mac_reg(p_dm, 0x44, BIT(25) | BIT(24),
			0);	/*config P_GPIO[3:2] to data port*/
	odm_set_mac_reg(p_dm, 0x44, BIT(17) | BIT(16),
			0x3);	/*enable_output for P_GPIO[3:2]*/
	...
	for (i = 0; i <= (codeword_length - 1); i++) {
		...
		if (p_dm->debug_components & DBG_ANT_DIV) {
			...
			if (i == (codeword_length - 1)) {
				...
				dbg_print("Start sending codeword[1:%d] ---> [ %d ", codeword_length,
					  beam_ctrl_signal);
				/**/
...
)
{
	...
	pdm_sat_table->update_beam_codeword =
		phydm_construct_hb_rfu_codeword_type2(p_dm,
	...
	phydm_update_beam_pattern_type2(p_dm, pdm_sat_table->update_beam_codeword,
					pdm_sat_table->rfu_codeword_total_bit_num);
#else
...
)
{
	...
	if (strcmp(input[1], help) == 0) {
		...
		if (pdm_sat_table->fix_beam_pattern_en == 1) {
			...
			for (i = 0; i <= (codeword_length - 1); i++) {
				beam_ctrl_signal = (boolean)((pdm_sat_table->update_beam_codeword & BIT(
								      i)) >> i);
				...
				if (i == (codeword_length - 1)) {
					...
					PHYDM_SNPRINTF((output + used, out_len - used,
							...
							pdm_sat_table->rfu_codeword_total_bit_num, beam_ctrl_signal));
					/**/
				} else if ((i % devide_num) == (devide_num - 1)) {
		...
		if (pdm_sat_table->fix_training_num_en == 1) {
			...
			PHYDM_SNPRINTF((output + used, out_len - used,
					...
					pdm_sat_table->fix_training_num_en,
		...
		if (dm_value[1] == 1) { /*2G*/
			...
			for (i = 0; i < pdm_sat_table->total_beam_set_num_2g; i++) {
				PHYDM_SNPRINTF((output + used, out_len - used,
						...
						i, pdm_sat_table->rfu_codeword_table_2g[i][0],
						pdm_sat_table->rfu_codeword_table_2g[i][1]));
			}
			PHYDM_SNPRINTF((output + used, out_len - used,
			...
			for (i = 0; i < pdm_sat_table->total_beam_set_num_5g; i++) {
				PHYDM_SNPRINTF((output + used, out_len - used,
						...
						i, pdm_sat_table->rfu_codeword_table_5g[i][0],
						pdm_sat_table->rfu_codeword_table_5g[i][1]));
			}
		}
...
)
{
	...
	if (p_dm_fat_table->fat_state == FAT_DECISION_STATE) {
		...
		for (j = 0; j < (pdm_sat_table->total_beam_set_num); j++) {
			...
			/*[Decision1: RSSI]-------------------------------------------------------------------*/
			if (pdm_sat_table->statistic_pkt_cnt[j] ==
			    0) {	/*if new RSSI = 0 -> MA_RSSI-=2*/
				...
				avg_rssi_tmp = (u8)((pdm_sat_table->beam_set_rssi_avg_sum[j]) /
						    (pdm_sat_table->statistic_pkt_cnt[j]));
				avg_rssi_tmp_ma = (avg_rssi_tmp + pdm_sat_table->beam_set_avg_rssi_pre[j])
						  >> 1;
			}
			...
			if (pdm_sat_table->beam_path_evm_2ss_cnt[j] != 0) {
				avg_evm2ss[0] =  pdm_sat_table->beam_path_evm_2ss_sum[j][0] /
						 pdm_sat_table->beam_path_evm_2ss_cnt[j];
				avg_evm2ss[1] =  pdm_sat_table->beam_path_evm_2ss_sum[j][1] /
						 pdm_sat_table->beam_path_evm_2ss_cnt[j];
			...
			if (pdm_sat_table->beam_path_evm_1ss_cnt[j] != 0) {
				avg_evm1ss =  pdm_sat_table->beam_path_evm_1ss_sum[j] /
					      pdm_sat_table->beam_path_evm_1ss_cnt[j];
			...
			PHYDM_DBG(p_dm, DBG_ANT_DIV,
				  ("Beam[%d] Pkt_cnt=(( %d )), avg{MA,rssi}={%d, %d}, EVM1={%d}, EVM2={%d, %d, %d}\n",
				   ...
				   avg_evm1ss, avg_evm2ss[0], avg_evm2ss[1], avg_evm2ss_sum));
		...
		for (j = 0; j < (pdm_sat_table->total_beam_set_num); j++) {
			...
			if (decision_type == 1) {
				per_beam_val_diff_tmp = target_beam_max_rssi -
							pdm_sat_table->beam_set_avg_rssi_pre[j];

			} else if (decision_type == 2) {
				per_beam_val_diff_tmp = ((u8)target_beam_max_evm2ss -
							 pdm_sat_table->beam_set_avg_evm_2ss_pre[j]) >> 1;
			} else if (decision_type == 3) {
				per_beam_val_diff_tmp = (u8)target_beam_max_evm1ss -
							pdm_sat_table->beam_set_avg_evm_1ss_pre[j];
			}
	...
	/* [TRAINING STATE] */
		...
		if (pdm_sat_table->fast_training_beam_num >
			...
			if (pdm_sat_table->fast_training_beam_num >= ((u32)
					pdm_sat_table->total_beam_set_num - 1)) {
	...
	/*=======================================================================================*/
		...
		PHYDM_DBG(p_dm, DBG_ANT_DIV,
			  ("TrafficLoad = (( %d )), Fix_beam = (( %d )), per_beam_training_pkt_num = (( %d )), decision_holding_period = ((%d))\n",
			   p_dm->traffic_load, pdm_sat_table->fix_training_num_en,
			   pdm_sat_table->per_beam_training_pkt_num,
			   pdm_sat_table->decision_holding_period));
		...
		for (j = 0; j < (pdm_sat_table->total_beam_set_num); j++) {
			...
			if ((pdm_sat_table->per_beam_training_pkt_num) > training_pkt_num_offset)
				pdm_sat_table->beam_set_train_cnt[j] =
					pdm_sat_table->per_beam_training_pkt_num - training_pkt_num_offset;
			else
				pdm_sat_table->beam_set_train_cnt[j] = 1;

			PHYDM_DBG(p_dm, DBG_ANT_DIV,
				  ("Beam_Set[ %d ] training_pkt_offset = ((%d)), training_pkt_num = ((%d))\n",
				   j, pdm_sat_table->beam_set_train_val_diff[j],
				   pdm_sat_table->beam_set_train_cnt[j]));
		}
		...
		if (pdm_sat_table->pkt_skip_statistic_en == 0) {
			...
			PHYDM_DBG(p_dm, DBG_ANT_DIV,
				  ("ID[%d] pkt_cnt=((%d)): Beam_set = ((%d)), RSSI{A,B,avg} = {%d, %d, %d}\n",
				   ...
				   pdm_sat_table->fast_training_beam_num, rx_power_ant0, rx_power_ant1,
				   rssi_avg));

			PHYDM_DBG(p_dm, DBG_ANT_DIV,
				  ("Rate_ss = ((%d)), EVM{A,B} = {%d, %d}, RX Rate =", rate_ss,  rx_evm_ant0,
				   rx_evm_ant1));
			phydm_print_rate(p_dm, p_dm->rx_rate, DBG_ANT_DIV);


			if (pdm_sat_table->pkt_counter >= 1) { /*packet skip count*/
				pdm_sat_table->beam_set_rssi_avg_sum[pdm_sat_table->fast_training_beam_num]
				+= rssi_avg;
				pdm_sat_table->statistic_pkt_cnt[pdm_sat_table->fast_training_beam_num]++;

				pdm_sat_table->beam_path_rssi_sum[pdm_sat_table->fast_training_beam_num][0]
				+= rx_power_ant0;
				pdm_sat_table->beam_path_rssi_sum[pdm_sat_table->fast_training_beam_num][1]
				+= rx_power_ant1;

				if (rate_ss == 2) {
					pdm_sat_table->beam_path_evm_2ss_sum[pdm_sat_table->fast_training_beam_num][0]
					+= rx_evm_ant0;
					pdm_sat_table->beam_path_evm_2ss_sum[pdm_sat_table->fast_training_beam_num][1]
					+= rx_evm_ant1;
					pdm_sat_table->beam_path_evm_2ss_cnt[pdm_sat_table->fast_training_beam_num]++;
				} else {
					pdm_sat_table->beam_path_evm_1ss_sum[pdm_sat_table->fast_training_beam_num]
					+= rx_evm_ant0;
			...
			train_pkt_number =
				pdm_sat_table->beam_set_train_cnt[pdm_sat_table->fast_training_beam_num];
...
)
{
	...
	odm_set_mac_reg(p_dm, 0x44, BIT(27) | BIT(26), 0);
	odm_set_mac_reg(p_dm, 0x44, BIT(19) | BIT(18),
			0x3);	/*enable_output for P_GPIO[3:2]*/
	...
	/*[BB] FAT setting*/
	odm_set_bb_reg(p_dm, 0xc08, BIT(18) | BIT(17) | BIT(16),
		       pdm_sat_table->ant_num);
	odm_set_bb_reg(p_dm, 0xc08, BIT(31),
		       0); /*increase ant num every FAT period 0:+1, 1+2*/
	odm_set_bb_reg(p_dm, 0x8c4, BIT(2) | BIT(1),
		       1); /*change cca antenna timming threshold if no CCA occurred: 0:200ms / 1:100ms / 2:no use / 3: 300*/
	...
	odm_set_mac_reg(p_dm, 0x7b4, MASKDWORD,
			value32 | (BIT(16) | BIT(17)));	/*Reg7B4[16]=1 enable antenna training */
...
)
{
	...
	if (ant_num < 8) {
		for (i = 0; i < (pdm_sat_table->ant_num_total); i++) {
			...
			if ((i < (pdm_sat_table->first_train_ant - 1))
			    || (break_counter >= (pdm_sat_table->ant_num))) {
				data_tmp = 0;
				/**/
			} else {
	...
	for (i = 0; i <= (codeword_length - 1); i++) {
		...
		if (p_dm->debug_components & DBG_ANT_DIV) {
			...
			if (i == (codeword_length - 1)) {
				...
				dbg_print("Send codeword[1:%d] ---> [ %d ", codeword_length,
					  beam_ctrl_signal);
				/**/
			} else if ((i % devide_num) == (devide_num - 1)) {
				dbg_print("%d  |  ", beam_ctrl_signal);
...
)
{
	...
	pdm_sat_table->update_beam_codeword = phydm_construct_hl_beam_codeword(
			p_dm, &(pdm_sat_table->rx_idle_beam[0]), pdm_sat_table->ant_num);
	PHYDM_DBG(p_dm, DBG_ANT_DIV,
...
#if DEV_BUS_TYPE == RT_PCI_INTERFACE
	phydm_update_beam_pattern(p_dm, pdm_sat_table->update_beam_codeword,
				  pdm_sat_table->rfu_codeword_total_bit_num);
#else
	odm_schedule_work_item(&pdm_sat_table->hl_smart_antenna_workitem);
	/*odm_stall_execution(1);*/
		...
		if (pdm_sat_table->fix_beam_pattern_en == 1) {
			...
			for (i = 0; i <= (codeword_length - 1); i++) {
				...
				if (i == (codeword_length - 1)) {
					...
					PHYDM_SNPRINTF((output + used, out_len - used,
							"Send Codeword[1:24] to RFU -> [%d", beam_ctrl_signal));
					/**/
				} else if ((i % devide_num) == (devide_num - 1)) {
					PHYDM_SNPRINTF((output + used, out_len - used, "%d|", beam_ctrl_signal));
					/**/
...
)
{
	...
	if (p_dm_fat_table->fat_state == FAT_DECISION_STATE) {
		...
		break_counter = 0;
		/*compute target beam in each antenna*/
		for (i = (pdm_sat_table->first_train_ant - 1);
		     i < pdm_sat_table->ant_num_total; i++) {
			for (j = 0; j < (pdm_sat_table->beam_patten_num_each_ant); j++) {
				...
				if (pdm_sat_table->pkt_rssi_cnt[i][j] == 0) {
					...
					avg_rssi_tmp = (pdm_sat_table->pkt_rssi_sum[i][j]) /
						       (pdm_sat_table->pkt_rssi_cnt[i][j]);
		...
		for (i = (pdm_sat_table->first_train_ant - 1);
			...
			for (j = 0; j < (pdm_sat_table->beam_patten_num_each_ant); j++) {
				...
				per_beam_rssi_diff_tmp = (u8)(max_beam_ant_rssi -
							      pdm_sat_table->pkt_rssi_pre[i][j]);
	...
	/*=======================================================================================*/
		...
		for (i = (pdm_sat_table->first_train_ant - 1);
			...
			for (j = 0; j < (pdm_sat_table->beam_patten_num_each_ant); j++) {
				...
				if ((pdm_sat_table->per_beam_training_pkt_num) > training_pkt_num_offset)
					pdm_sat_table->beam_train_cnt[i][j] =
						pdm_sat_table->per_beam_training_pkt_num - training_pkt_num_offset;
				else
