...
u4Byte odm_tx_gain_gap_psd_8197f(
	void	*p_dm_void,
	u1Byte	rf_path,
	u4Byte	rf56
)
{
	...
	odm_set_bb_reg(p_dm_odm, iqk_ctl_addr[rf_path][0], 0xffffffff,
		       psd_cntl_value[rf_path][0]);
	odm_set_bb_reg(p_dm_odm, iqk_ctl_addr[rf_path][1], 0xffffffff,
		       psd_cntl_value[rf_path][1]);
	odm_set_bb_reg(p_dm_odm, iqk_ctl_addr[rf_path][2], 0xffffffff,
		       psd_cntl_value[rf_path][0]);
	odm_set_bb_reg(p_dm_odm, iqk_ctl_addr[rf_path][3], 0xffffffff,
		       psd_cntl_value[rf_path][0]);
	...
	for (i = 0; i < psd_avg_time; i++) {
		...
		for (j = 0; j < 1000 ; j++) {
			...
			while (!odm_get_bb_reg(p_dm_odm, 0xeac,
					       psd_finish_bit[rf_path]));	/*wait finish bit*/

			if (!odm_get_bb_reg(p_dm_odm, 0xeac,
					    psd_fail_bit[rf_path])) {	/*check fail bit*/

				psd_vaule[i] = odm_get_bb_reg(p_dm_odm, psd_report_addr[rf_path],
							      0xffffffff);
...
)
{
	...
	u1Byte rf_path, rf0_idx, rf0_idx_current, rf0_idx_next, i,
	       delta_gain_retry = 3;
	...
	for (rf_path = RF_PATH_A; rf_path <= RF_PATH_B; rf_path++) {
		...
		if (rf_path == RF_PATH_A)
			odm_set_bb_reg(p_dm_odm, 0x88c, (BIT(21) | BIT(20)),
				       0x3);	/*disable 3-wire*/
		else if (rf_path == RF_PATH_B)
			odm_set_bb_reg(p_dm_odm, 0x88c, (BIT(23) | BIT(22)),
				       0x3);	/*disable 3-wire*/
		...
		for (rf0_idx = 1; rf0_idx <= 10; rf0_idx++) {
			...
			ODM_delay_us(100);
			rf56_current_temp[rf_path][rf0_idx] = odm_get_rf_reg(p_dm_odm, rf_path,
							      0x56, 0xfff);
			...
			if ((rf56_current >> 5) == (rf56_next >> 5)) {
				...
				ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
					     ("[TGGC] rf56_current[11:5] == rf56_next[%d][%d][11:5]=0x%x delta_gain_gap[%d][%d]=%d\n",
					      rf_path, rf0_idx, (rf56_next >> 5), rf_path, rf0_idx,
					      delta_gain_gap[rf_path][rf0_idx]));
			} else {
				ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
					     ("[TGGC] rf56_current[%d][%d][11:5]=0x%x != rf56_next[%d][%d][11:5]=0x%x\n",
					      rf_path, rf0_idx, (rf56_current >> 5), rf_path, rf0_idx,
					      (rf56_next >> 5)));


				for (i = 0; i < delta_gain_retry; i++) {
					psd_value_current = odm_tx_gain_gap_psd_8197f(p_dm_odm, rf_path,
							    rf56_current);

					psd_value_next = odm_tx_gain_gap_psd_8197f(p_dm_odm, rf_path,
							 rf56_next - 2);
					...
					ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
						     ("[TGGC] psd_value_current=0x%x psd_value_next=0x%x psd_value_next/psd_value_current=%d delta_gain_gap[%d][%d]=%d\n",
						      psd_value_current, psd_value_next, psd_gap, rf_path, rf0_idx,
						      delta_gain_gap[rf_path][rf0_idx]));
						...
						if (delta_gain_gap_pre != delta_gain_gap[rf_path][rf0_idx]) {
							...
							ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
								     ("[TGGC] delta_gain_gap_pre(%d) != delta_gain_gap[%d][%d](%d) time=%d\n",
								      delta_gain_gap_pre, rf_path, rf0_idx, delta_gain_gap[rf_path][rf0_idx],
								      i));
	...
	for (rf_path = RF_PATH_A; rf_path <= RF_PATH_B; rf_path++) {
		...
		for (rf0_idx = 1; rf0_idx <= 10; rf0_idx++) {
			...
			rf33[rf_path][rf0_idx] = rf33[rf_path][rf0_idx] +
						 (rf56_current_temp[rf_path][rf0_idx] & 0x1f);
			...
			rf33[rf_path][rf0_idx] = rf33[rf_path][rf0_idx] + ((rf0_idx - 1) * 0x4000)
						 + (rf56_current_temp[rf_path][rf0_idx] & 0xfffe0);

			ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD,
				     ("[TGGC] rf56[%d][%d]=0x%05x rf33[%d][%d]=0x%05x\n", rf_path, rf0_idx,
				      rf56_current_temp[rf_path][rf0_idx], rf_path, rf0_idx,
				      rf33[rf_path][rf0_idx]));

			odm_set_rf_reg(p_dm_odm, rf_path, 0x33, bRFRegOffsetMask,
				       rf33[rf_path][rf0_idx]);
		}
