...
)
{
	...
	if (GET_CHIP_VER(priv) == VERSION_8197F) {
		bb_swing = phy_query_bb_reg(priv, REG_OFDM_0_XA_TX_IQ_IMBALANCE,
					    MASKOFDM_D);
...
)
{
	...
	ODM_RT_TRACE(p_dm, COMP_POWER_TRACKING, DBG_LOUD,
		     ("p_mgnt_info->is_txpowertracking = %d\n",
...
#else
	{
		...
		/* if(IS_HARDWARE_TYPE_8192C(p_hal_data)) */
		{
			...
			if (*(p_dm->p_mp_mode) ==
			    false)		/* for mp driver, turn off txpwrtracking as default */
...
#if (RTL8197F_SUPPORT == 1)
	if (GET_CHIP_VER(priv) == VERSION_8197F) {
		p_rf_calibrate_info->default_ofdm_index = (default_swing_index >=
				(OFDM_TABLE_SIZE_92D - 1)) ? 30 : default_swing_index;
	...
	if (GET_CHIP_VER(priv) == VERSION_8822B) {
		p_rf_calibrate_info->default_ofdm_index = (default_swing_index >=
				(TXSCALE_TABLE_SIZE - 1)) ? 24 : default_swing_index;
	...
	for (p = 0; p < MAX_RF_PATH; p++) {
		p_dm->rf_calibrate_info.OFDM_index[p] =
			p_rf_calibrate_info->default_ofdm_index;
	...
	ODM_RT_TRACE(p_dm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD,
		     ("p_rf_calibrate_info->default_ofdm_index=%d p_rf_calibrate_info->default_cck_index=%d\n",
		      p_rf_calibrate_info->default_ofdm_index,
		      p_rf_calibrate_info->default_cck_index));
...
)
{
	...
	if (!adapter->is_slave_of_dmsp
	    || adapter->dual_mac_smart_concurrent == false)
...
)
{
	...
	if (p_dm->support_ic_type & (ODM_RTL8188E | ODM_RTL8192E | ODM_RTL8812 |
				     ODM_RTL8881A | ODM_RTL8814A | ODM_RTL8197F | ODM_RTL8822B | ODM_RTL8821C))
