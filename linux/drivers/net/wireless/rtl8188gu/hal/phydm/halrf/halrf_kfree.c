...
)
{
	...
	odm_efuse_one_byte_read(p_dm, PPG_BB_GAIN_2G_TXAB_OFFSET_8821C, &pg_power,
				false);

	if (pg_power != 0xff) {
		p_power_trim_info->bb_gain[0][0] = pg_power;
		odm_efuse_one_byte_read(p_dm, PPG_BB_GAIN_5GL1_TXA_OFFSET_8821C, &pg_power,
					false);
		p_power_trim_info->bb_gain[1][0] = pg_power;
		odm_efuse_one_byte_read(p_dm, PPG_BB_GAIN_5GL2_TXA_OFFSET_8821C, &pg_power,
					false);
		p_power_trim_info->bb_gain[2][0] = pg_power;
		odm_efuse_one_byte_read(p_dm, PPG_BB_GAIN_5GM1_TXA_OFFSET_8821C, &pg_power,
					false);
		p_power_trim_info->bb_gain[3][0] = pg_power;
		odm_efuse_one_byte_read(p_dm, PPG_BB_GAIN_5GM2_TXA_OFFSET_8821C, &pg_power,
					false);
		p_power_trim_info->bb_gain[4][0] = pg_power;
		odm_efuse_one_byte_read(p_dm, PPG_BB_GAIN_5GH1_TXA_OFFSET_8821C, &pg_power,
					false);
		p_power_trim_info->bb_gain[5][0] = pg_power;
		p_power_trim_info->flag = p_power_trim_info->flag | KFREE_FLAG_ON |
...
)
{
	...
	ODM_RT_TRACE(p_dm, ODM_COMP_CALIBRATION, ODM_DBG_TRACE,
		     ("[kfree] 8821c 0x55[19:14]=0x%X 0x65[19:14]=0x%X\n",
		      odm_get_rf_reg(p_dm, e_rf_path, 0x55,
				     (BIT(19) | BIT(18) | BIT(17) | BIT(16) | BIT(15) | BIT(14))),
		      odm_get_rf_reg(p_dm, e_rf_path, 0x65,
				     (BIT(19) | BIT(18) | BIT(17) | BIT(16) | BIT(15) | BIT(14)))
		     ));
}
...
)
{
	...
	ODM_RT_TRACE(p_dm, ODM_COMP_MP, ODM_DBG_LOUD,
		     ("[kfree] 8822b 0xc8:0x%2x\n", thermal_trim_enable));

	thermal_trim_enable = (thermal_trim_enable & BIT(5)) >> 5;

	ODM_RT_TRACE(p_dm, ODM_COMP_MP, ODM_DBG_LOUD,
		     ("[kfree] 8822b thermal trim Enable:%d\n", thermal_trim_enable));

	if ((p_rf_calibrate_info->reg_rf_kfree_enable == 0
	     && thermal_trim_enable == 1) ||
...
)
{
	...
	ODM_RT_TRACE(p_dm, ODM_COMP_MP, ODM_DBG_LOUD,
		     ("[kfree] 8822b power trim Enable:%d\n", power_trim_enable));
	...
	if ((p_rf_calibrate_info->reg_rf_kfree_enable == 0
		...
		if (pg_power != 0xff) {
			...
			odm_efuse_one_byte_read(p_dm, PPG_BB_GAIN_5GL1_TXA_OFFSET, &pg_power,
						false);
			p_power_trim_info->bb_gain[1][0] = pg_power;
			odm_efuse_one_byte_read(p_dm, PPG_BB_GAIN_5GL2_TXA_OFFSET, &pg_power,
						false);
			p_power_trim_info->bb_gain[2][0] = pg_power;
			odm_efuse_one_byte_read(p_dm, PPG_BB_GAIN_5GM1_TXA_OFFSET, &pg_power,
						false);
			p_power_trim_info->bb_gain[3][0] = pg_power;
			odm_efuse_one_byte_read(p_dm, PPG_BB_GAIN_5GM2_TXA_OFFSET, &pg_power,
						false);
			p_power_trim_info->bb_gain[4][0] = pg_power;
			odm_efuse_one_byte_read(p_dm, PPG_BB_GAIN_5GH1_TXA_OFFSET, &pg_power,
						false);
			p_power_trim_info->bb_gain[5][0] = pg_power;

			/*Path B*/
			odm_efuse_one_byte_read(p_dm, PPG_BB_GAIN_5GL1_TXB_OFFSET, &pg_power,
						false);
			p_power_trim_info->bb_gain[1][1] = pg_power;
			odm_efuse_one_byte_read(p_dm, PPG_BB_GAIN_5GL2_TXB_OFFSET, &pg_power,
						false);
			p_power_trim_info->bb_gain[2][1] = pg_power;
			odm_efuse_one_byte_read(p_dm, PPG_BB_GAIN_5GM1_TXB_OFFSET, &pg_power,
						false);
			p_power_trim_info->bb_gain[3][1] = pg_power;
			odm_efuse_one_byte_read(p_dm, PPG_BB_GAIN_5GM2_TXB_OFFSET, &pg_power,
						false);
			p_power_trim_info->bb_gain[4][1] = pg_power;
			odm_efuse_one_byte_read(p_dm, PPG_BB_GAIN_5GH1_TXB_OFFSET, &pg_power,
						false);
...
)
{
	...
	ODM_RT_TRACE(p_dm, ODM_COMP_MP, ODM_DBG_LOUD,
		     ("[kfree] 8822b 2g tx pa bias rf_0x3f(0x%X) path=%d\n",
		      odm_get_rf_reg(p_dm, e_rf_path, 0x3f,
...
)
{
	...
	if (pg_pa_bias != 0xff) {
		...
		/*paht b*/
		odm_efuse_one_byte_read(p_dm, PPG_PA_BIAS_2G_TXB_OFFSET, &pg_pa_bias,
					false);
		pg_pa_bias = pg_pa_bias & 0xf;
...
)
{
	...
	if (p_dm->support_ic_type & ODM_RTL8814A)
		max_rf_path = 4;	/*0~3*/
	else if (p_dm->support_ic_type & (ODM_RTL8812 | ODM_RTL8192E |
					  ODM_RTL8822B))
		max_rf_path = 2;	/*0~1*/
		...
		if (p_power_trim_info->flag & KFREE_FLAG_ON) {
			...
			if (*p_dm->p_band_type == ODM_BAND_2_4G
				...
				for (rfpath = RF_PATH_A;  rfpath < max_rf_path; rfpath++) {
					...
					phydm_set_kfree_to_rf(p_dm, rfpath,
							      p_power_trim_info->bb_gain[channel_idx][rfpath]);
				}
				...
				for (rfpath = RF_PATH_A;  rfpath < max_rf_path; rfpath++)
					phydm_clear_kfree_to_rf(p_dm, rfpath,
								p_power_trim_info->bb_gain[channel_idx][rfpath]);
			}
		} else {
			ODM_RT_TRACE(p_dm, ODM_COMP_MP, ODM_DBG_LOUD,
