...
)
{
	...
	odm_config_rf_reg_8710b(p_dm, addr, data, RF_PATH_A,
...
)
{
	...
	odm_config_rf_reg_8710b(p_dm, addr, data, RF_PATH_B,
				addr | maskfor_phy_set);
...
)
{
	if (addr == 0xfe || addr == 0xffe)
		...
		phy_store_tx_power_by_rate(p_dm->adapter, band, rf_path, tx_num, addr,
					   bitmask, data);
#elif (DM_ODM_SUPPORT_TYPE & ODM_WIN)
		PHY_StoreTxPowerByRate(p_dm->adapter, band, rf_path, tx_num, addr, bitmask,
				       data);
#endif
	}
	PHYDM_DBG(p_dm, ODM_COMP_INIT,
