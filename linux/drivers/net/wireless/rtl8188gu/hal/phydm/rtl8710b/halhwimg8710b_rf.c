...
void odm_read_and_config_mp_8710b_radioa(
	struct	PHY_DM_STRUCT *p_dm
)
{
	u32	i = 0;
...
void odm_read_and_config_mp_8710b_txpowertrack_pcie(
	struct PHY_DM_STRUCT	 *p_dm
)
{
#if DEV_BUS_TYPE == RT_PCI_INTERFACE
...
void odm_read_and_config_mp_8710b_txpowertrack_sdio(
	struct PHY_DM_STRUCT	 *p_dm
)
{
#if DEV_BUS_TYPE == RT_SDIO_INTERFACE
...
void odm_read_and_config_mp_8710b_txpowertrack_usb(
	struct PHY_DM_STRUCT	 *p_dm
)
{
#if DEV_BUS_TYPE == RT_USB_INTERFACE
	...
	odm_move_memory(p_dm, p_rf_calibrate_info->delta_swing_table_idx_2ga_p,
			g_delta_swing_table_idx_mp_2ga_p_txpowertrack_usb_8710b,
			DELTA_SWINGIDX_SIZE);
	odm_move_memory(p_dm, p_rf_calibrate_info->delta_swing_table_idx_2ga_n,
			g_delta_swing_table_idx_mp_2ga_n_txpowertrack_usb_8710b,
			DELTA_SWINGIDX_SIZE);
	odm_move_memory(p_dm,
			p_rf_calibrate_info->delta_swing_table_idx_2g_cck_a_p,
			g_delta_swing_table_idx_mp_2g_cck_a_p_txpowertrack_usb_8710b,
			DELTA_SWINGIDX_SIZE);
	odm_move_memory(p_dm,
			p_rf_calibrate_info->delta_swing_table_idx_2g_cck_a_n,
			g_delta_swing_table_idx_mp_2g_cck_a_n_txpowertrack_usb_8710b,
			DELTA_SWINGIDX_SIZE);
...
void odm_read_and_config_mp_8710b_txpwr_lmt(
	struct PHY_DM_STRUCT	*p_dm
)
{
	u32	i = 0;
	...
	PlatformZeroMemory(p_hal_data->BufOfLinesPwrLmt,
	...
	for (i = 0; i < array_len; i += 7) {
		...
		odm_config_bb_txpwr_lmt_8710b(p_dm, regulation, band, bandwidth, rate,
					      rf_path, chnl, val);
#if (DM_ODM_SUPPORT_TYPE == ODM_WIN)
		rsprintf((char *)p_hal_data->BufOfLinesPwrLmt[i / 7], 100,
			 "\"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\",",
...
void odm_read_and_config_mp_8710b_txxtaltrack(
	struct	PHY_DM_STRUCT *p_dm
)
{
	struct odm_rf_calibration_structure	*p_rf_calibrate_info = &
	...
	odm_move_memory(p_dm, p_rf_calibrate_info->delta_swing_table_xtal_p,
			g_delta_swing_table_xtal_mp_p_txxtaltrack_8710b, DELTA_SWINGIDX_SIZE);
	odm_move_memory(p_dm, p_rf_calibrate_info->delta_swing_table_xtal_n,
			g_delta_swing_table_xtal_mp_n_txxtaltrack_8710b, DELTA_SWINGIDX_SIZE);
}
...
void odm_read_and_config_mp_8710b_txpowertrack_qfn48m_smic(
...
void odm_read_and_config_mp_8710b_txpowertrack_qfn48m_umc(
	struct PHY_DM_STRUCT *dm)
{
#ifdef CONFIG_8710B_QFN48M_UMC
