	...
	if (primary_cca->mf_state != curr_mf_state) {
		...
		if (p_dm->support_ic_type & ODM_IC_11N_SERIES) {
			...
			if (curr_mf_state == MF_USC_LSC) {
				...
				odm_set_bb_reg(p_dm, 0xc84, 0xf0000000,
					       primary_cca->cca_th_40m_bkp); /*40M OFDM MF CCA threshold*/
			} else {
		...
		PHYDM_DBG(p_dm, DBG_PRI_CCA,
			  ("Set CCA at ((%s SB)), 0xc6c[8:7]=((%d))\n",
			   ((curr_mf_state == MF_USC_LSC) ? "D" : ((curr_mf_state == MF_LSC) ? "L" :
					   "U")), curr_mf_state));
	}
}
...
)
{
	...
	if (curr_bw != p_primary_cca->pre_bw) {
		...
		if (curr_bw == CHANNEL_WIDTH_40) {
			...
			if ((*(p_dm->p_sec_ch_offset)) ==
			    SECOND_CH_AT_LSB) {/* Primary CH @ upper sideband*/
...
void odm_dynamic_primary_cca_8188e(
	void			*p_dm_void
)
{
	struct PHY_DM_STRUCT		*p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	...
	struct phydm_pricca_struct		*primary_cca = &(p_dm->dm_pri_cca);
	boolean		client_40mhz = false,
			client_tmp = false;      /* connected client BW */
	...
	sec_ch_offset = sec_ch_offset % 2 +
			1; /* NIC's definition is reverse to AP   1:secondary below,  2: secondary above */
#endif
	...
	if (ACTING_AS_AP(
		    p_dm->adapter))   /* primary cca process only do at AP mode */
#endif
	{
		...
		for (i = 0; i < ASSOCIATE_ENTRY_NUM; i++) {
			if (IsAPModeExist(p_dm->adapter)
			    && GetFirstExtAdapter(p_dm->adapter) != NULL)
			...
			if (p_entry != NULL) {
				...
				if (client_tmp > client_40mhz)
					client_40mhz =
						client_tmp;     /* 40M/20M coexist => 40M priority is High */
		...
		if (primary_cca->monitor_flag == 1) {
			if (sec_ch_offset ==
				...
				if ((false_alm_cnt->cnt_ofdm_cca > 500)
				    && (false_alm_cnt->cnt_bw_lsc > false_alm_cnt->cnt_bw_usc + 500)) {
				   ...
				   2) { /* secondary channel is above the primary channel */
				if ((false_alm_cnt->cnt_ofdm_cca > 500)
				    && (false_alm_cnt->cnt_bw_usc > false_alm_cnt->cnt_bw_lsc + 500)) {
					if (false_alm_cnt->cnt_ofdm_fail > false_alm_cnt->cnt_ofdm_cca >> 1) {
						primary_cca->intf_type = 1;
						primary_cca->pri_cca_flag = 1;
		...
		if (primary_cca->monitor_flag == 0) {
			if (*(p_dm->p_band_width) ==
			    CHANNEL_WIDTH_40) {		/* if RFBW==40M mode which require to process primary cca */
				/* 2 STA is NOT Connected */
				if (!is_connected) {
					PHYDM_DBG(p_dm, DBG_PRI_CCA, ("STA NOT Connected!!!!\n"));

					if (primary_cca->pri_cca_flag ==
					    1) {	/* reset primary cca when STA is disconnected */
						primary_cca->pri_cca_flag = 0;
						odm_set_bb_reg(p_dm, 0xc6c, BIT(8) | BIT(7), 0);
					}
					if (primary_cca->dup_rts_flag ==
					    1)		/* reset Duplicate RTS when STA is disconnected */
						primary_cca->dup_rts_flag = 0;

					if (sec_ch_offset ==
					    1) { /* secondary channel is below the primary channel */
						if ((false_alm_cnt->cnt_ofdm_cca > 800)
						    && (false_alm_cnt->cnt_bw_lsc * 5 > false_alm_cnt->cnt_bw_usc * 9)) {
							primary_cca->intf_flag =
								1;    /* secondary channel interference is detected!!! */
							if (false_alm_cnt->cnt_ofdm_fail > false_alm_cnt->cnt_ofdm_cca >> 1)
						   ...
						   2) { /* secondary channel is above the primary channel */
						if ((false_alm_cnt->cnt_ofdm_cca > 800)
						    && (false_alm_cnt->cnt_bw_usc * 5 > false_alm_cnt->cnt_bw_lsc * 9)) {
							primary_cca->intf_flag =
								1;    /* secondary channel interference is detected!!! */
							if (false_alm_cnt->cnt_ofdm_fail > false_alm_cnt->cnt_ofdm_cca >> 1)
								primary_cca->intf_type = 1;   	/* interference is shift */
				...
				/* 2 STA is Connected */
					...
					if (client_40mhz == 0) {	/* 3 */
						...
						if (primary_cca->intf_flag == 1) { /* interference is detected!! */
							...
							if ((cur_tx_ok_cnt + cur_rx_ok_cnt) <
							    1) { /* idle mode or TP traffic is very low */
								if (sec_ch_offset == 1) {
									if ((false_alm_cnt->cnt_ofdm_cca > 800)
									    && (false_alm_cnt->cnt_bw_lsc * 5 > false_alm_cnt->cnt_bw_usc * 9)) {
								...
								if (sec_ch_offset == 1) {
									if (false_alm_cnt->cnt_bw_lsc > (false_alm_cnt->cnt_bw_usc + 500)) {
										if (delay ==
										    0) { /* add delay to avoid interference occurring abruptly, jump one time */
											primary_cca->intf_flag = 1;
											if (false_alm_cnt->cnt_ofdm_fail > false_alm_cnt->cnt_ofdm_cca >> 1)
												primary_cca->intf_type = 1;   	/* interference is shift */
											else
		...
		if ((primary_cca->pri_cca_flag == 1) || (primary_cca->dup_rts_flag == 1)) {
			if (client_40mhz ==
			    0) {  /* client=20M no need to monitor primary cca flag */
			...
			if ((counter == 30)
			    || ((client_40mhz - client_40mhz_pre) == 1)) { /* Every 60 sec to monitor one time */
...
void odm_dynamic_primary_cca_mp_8192e(
	void			*p_dm_void
)
{
	struct PHY_DM_STRUCT		*p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	...
	sec_ch_offset = *
			(p_dm->p_sec_ch_offset);		/* NIC: 2: sec is below,  1: sec is above */
	...
	if (primary_cca->pri_cca_flag == 0) {
		...
		if (sec_ch_offset ==
		    SECOND_CH_AT_LSB) {  /* Primary channel is above   NOTE: duplicate CTS can remove this condition */
...
)
{
	...
	if ((false_alm_cnt->cnt_ofdm_cca > OFDMCCA_TH)
	    && (false_alm_cnt->cnt_bw_lsc > (false_alm_cnt->cnt_bw_usc +
					     bw_ind_bias))) {
		...
		if (false_alm_cnt->cnt_ofdm_fail > (false_alm_cnt->cnt_ofdm_cca >> 1))
		   ...
		   && (false_alm_cnt->cnt_bw_usc > (false_alm_cnt->cnt_bw_lsc +
				   bw_ind_bias))) {
...
void odm_dynamic_primary_cca_ap_8192e(
	void			*p_dm_void
)
{
	struct PHY_DM_STRUCT		*p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	...
	sec_ch_offset = *
			(p_dm->p_sec_ch_offset);		/* AP: 1: sec is below,  2: sec is above */
...
boolean odm_dynamic_primary_cca_dup_rts(
	void			*p_dm_void
)
{
#ifdef PHYDM_PRIMARY_CCA
