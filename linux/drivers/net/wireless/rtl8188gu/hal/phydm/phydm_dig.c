...
)
{
	...
	if (total_nhm_cnt != 0) {
		if (p_dig_t->dig_go_up_check_level == DIG_GOUPCHECK_LEVEL_0) {
			...
			ret = (p_dig_t->dig_level1_ratio_reciprocal * over_dig_cover_cnt <
			       total_nhm_cnt) ? true : false;

			if (!ret) {
				/* update p_dig_t->rx_gain_range_max */
				p_dig_t->rx_gain_range_max = (rx_gain_range_max >= max_cover_bond - 6) ?
							     (max_cover_bond - 6) : rx_gain_range_max;
...
)
{
	...
	if (p_dig_t->is_dbg_fa_th) {
		...
		} else if ((p_dm->rx_tp >> 2) > p_dm->tx_tp && p_dm->rx_tp < 10
...
)
{
	...
	if (p_dig_t->cur_ig_value != current_igi) {
		...
		if (p_dm->support_ic_type & PHYDM_IC_ABOVE_2SS)
			odm_set_bb_reg(p_dm, ODM_REG(IGI_B, p_dm), ODM_BIT(IGI, p_dm),
				       current_igi);
#endif

#if (defined(PHYDM_COMPILE_ABOVE_4SS))
		if (p_dm->support_ic_type & PHYDM_IC_ABOVE_4SS) {
			odm_set_bb_reg(p_dm, ODM_REG(IGI_C, p_dm), ODM_BIT(IGI, p_dm),
				       current_igi);
			odm_set_bb_reg(p_dm, ODM_REG(IGI_D, p_dm), ODM_BIT(IGI, p_dm),
				       current_igi);
		}
#endif
...
)
{
	...
	case PHYDM_PAUSE: {
		...
		p_dig_t->pause_lv_bitmap |= BIT(
						    pause_level); /* Update pause level bit-map*/
...
boolean phydm_dig_performance_mode_decision(
...
)
{
	...
	first_dis_connect = (!p_dm->is_linked)
			    && (p_dig_t->is_media_connect == true);
 ...
 * 3 FASLE ALARM CHECK
 * 3============================================================ */
void phydm_false_alarm_counter_reg_reset(
	void					*p_dm_void
)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
...
void phydm_false_alarm_counter_reg_hold(
	void					*p_dm_void
)
{
	struct PHY_DM_STRUCT *p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
...
void odm_false_alarm_counter_statistics(
	void		*p_dm_void
)
{
	struct PHY_DM_STRUCT					*p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	...
	if (p_dm->support_ic_type & ODM_IC_11N_SERIES) {
		...
		false_alm_cnt->cnt_cck_crc32_error = odm_get_bb_reg(p_dm,
						     ODM_REG_CCK_CRC32_ERROR_CNT_11N, MASKDWORD);
		false_alm_cnt->cnt_cck_crc32_ok = odm_get_bb_reg(p_dm,
						  ODM_REG_CCK_CRC32_OK_CNT_11N, MASKDWORD);
...

		{
			...
			false_alm_cnt->cnt_cck_cca = ((ret_value & 0xFF) << 8) | ((
							     ret_value & 0xFF00) >> 8);
		}
	...
	if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
		...
		false_alm_cnt->cnt_ofdm_fail = odm_get_bb_reg(p_dm, ODM_REG_OFDM_FA_11AC,
					       MASKLWORD);

		/* Read CCK FA counter */
		false_alm_cnt->cnt_cck_fail = odm_get_bb_reg(p_dm, ODM_REG_CCK_FA_11AC,
					      MASKLWORD);
		...
		if (p_dm->support_ic_type == ODM_RTL8881A) {
			...
			if (false_alm_cnt->cnt_ofdm_fail >= false_alm_cnt->cnt_ofdm_fail_pre) {
				...
				false_alm_cnt->cnt_ofdm_fail = false_alm_cnt->cnt_ofdm_fail -
							       cnt_ofdm_fail_temp;
			} else
				false_alm_cnt->cnt_ofdm_fail_pre = false_alm_cnt->cnt_ofdm_fail;
			PHYDM_DBG(p_dm, DBG_FA_CNT,
				  ("odm_false_alarm_counter_statistics(): cnt_ofdm_fail=%d\n",
				   false_alm_cnt->cnt_ofdm_fail_pre));
			PHYDM_DBG(p_dm, DBG_FA_CNT,
				  ("odm_false_alarm_counter_statistics(): cnt_ofdm_fail_pre=%d\n",
				   cnt_ofdm_fail_temp));
		}
#endif
		...
		if (cck_enable) { /* if(*p_dm->p_band_type == ODM_BAND_2_4G) */
			false_alm_cnt->cnt_all = false_alm_cnt->cnt_ofdm_fail +
						 false_alm_cnt->cnt_cck_fail;
			false_alm_cnt->cnt_cca_all = false_alm_cnt->cnt_cck_cca +
	...
	if (p_dm->support_ic_type != ODM_RTL8723D) {
		...
		if (phydm_set_bb_dbg_port(p_dm, BB_DBGPORT_PRIORITY_1,
			...
			if (p_dm->support_ic_type & (ODM_RTL8723B | ODM_RTL8188E))
				false_alm_cnt->edcca_flag = (boolean)((phydm_get_bb_dbg_port_value(
						p_dm) & BIT(30)) >> 30);
			else
				false_alm_cnt->edcca_flag = (boolean)((phydm_get_bb_dbg_port_value(
						p_dm) & BIT(29)) >> 29);
		...
		false_alm_cnt->edcca_flag = (boolean)(odm_get_bb_reg(p_dm, 0x9a0,
						      BIT(29)));
	}
	...
	false_alm_cnt->cnt_crc32_error_all = false_alm_cnt->cnt_vht_crc32_error +
					     false_alm_cnt->cnt_ht_crc32_error + false_alm_cnt->cnt_ofdm_crc32_error +
					     false_alm_cnt->cnt_cck_crc32_error;
	false_alm_cnt->cnt_crc32_ok_all = false_alm_cnt->cnt_vht_crc32_ok +
					  false_alm_cnt->cnt_ht_crc32_ok + false_alm_cnt->cnt_ofdm_crc32_ok +
...
#if (DM_ODM_SUPPORT_TYPE == (ODM_AP))
	if (p_dm->support_ic_type & ODM_RTL8197F)
		phydm_add_interrupt_mask_handler(p_dm,
						 HAL_INT_TYPE_PSTIMEOUT2);	/*HAL_INT_TYPE_PSTIMEOUT2*/
...
)
{
	...
	/* fa_end_tsf = fa_start_tsf = TSF */
		...
		/*1sec = 1000000us*/
		if (p_dig_t->fa_acc_1sec_timestamp >= (u32)(1000000 /
				p_dig_t->sec_factor)) {
			rssi_dump_en = 1;
...
)
{
	...
	if ((p_dm->lna_sat_chk_period_ms > 0)
	    && (p_dm->lna_sat_chk_period_ms <= ONE_SEC_MS))
		max_check_time = lna_sat_chk_cnt * (ONE_SEC_MS /
						    (p_dm->lna_sat_chk_period_ms)) * 5;
	else
	...
	odm_set_timer(p_dm, &p_lna_info->phydm_lna_sat_chk_timer,
		      p_dm->lna_sat_chk_period_ms);
}
