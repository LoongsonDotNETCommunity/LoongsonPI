...
enum hal_status odm_config_rf_with_header_file(
...
#if RTL8188E_SUPPORT
	if (p_dm->support_ic_type == ODM_RTL8188E) {
		if (odm_get_mac_reg(p_dm, 0xF0,
				    0xF000) >= 8) {		/*if 0xF0[15:12] >= 8, SMIC*/
			if (p_dm->support_interface == ODM_ITRF_PCIE)
...
enum hal_status odm_config_bb_with_header_file(
	struct PHY_DM_STRUCT		*p_dm,
...
enum hal_status odm_config_mac_with_header_file(
	struct PHY_DM_STRUCT	*p_dm
)
{
	enum hal_status	result = HAL_STATUS_SUCCESS;
...
u32 query_phydm_stbc_capability(
	struct PHY_DM_STRUCT					*p_dm
)
{
	u32 value32 = 0xFFFFFFFF;
...
u32 query_phydm_ldpc_capability(
	struct PHY_DM_STRUCT					*p_dm
)
{
	u32 value32 = 0xFFFFFFFF;
...
u32 query_phydm_txbf_parameters(
	struct PHY_DM_STRUCT					*p_dm
)
{
	u32 value32 = 0xFFFFFFFF;
...
u32 query_phydm_txbf_capability(
	struct PHY_DM_STRUCT					*p_dm
)
{
	u32 value32 = 0xFFFFFFFF;
