...
void phydm_bb_dbg_port_header_sel(
	void			*p_dm_void,
	u32			header_idx
)
{
	...
	if (p_dm->support_ic_type & ODM_IC_11AC_SERIES) {
		...
		odm_set_bb_reg(p_dm, 0x8f8, (BIT(25) | BIT(24) | BIT(23) | BIT(22)),
			       header_idx);
...
void phydm_bb_dbg_port_clock_en(
	void			*p_dm_void,
	u8			enable
)
{
...
void phydm_release_bb_dbg_port(
	void			*p_dm_void
)
{
	struct PHY_DM_STRUCT		*p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
...
u32 phydm_get_bb_dbg_port_value(
	void			*p_dm_void
)
{
	struct PHY_DM_STRUCT		*p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
...
)
{
	...
	PHYDM_SNPRINTF((output + used, out_len - used,
			"\r\n %-35s = %d / %d / %d / %d", "OFDM RX RF Gain(A/B/C/D)", rf_gain_a,
			rf_gain_b, rf_gain_c, rf_gain_d));
	...
	PHYDM_SNPRINTF((output + used, out_len - used,
			"\r\n %-35s = %d / %d / %d / %d", "RXSNR(A/B/C/D, dB)", rx_snr_a, rx_snr_b,
			rx_snr_c, rx_snr_d));
...
)
{
	...
	static u8	MCSS, tail, parity, rsv, vrsv, idx, smooth, htsound, agg, stbc,
		     vstbc, fec, fecext, sgi, sgiext, htltf, vgid, v_nsts, vtxops, vrsv2, vbrsv,
		     bf, vbcrc;
	...
	PHYDM_SNPRINTF((output + used, out_len - used,
			...
			rf_gain_path_a, rf_gain_path_b, rf_gain_path_c, rf_gain_path_d));
	...
	PHYDM_SNPRINTF((output + used, out_len - used, "\r\n %-35s = %d / %d",
			"HT/VHT MCS NOT SUPPORT counter", ((value32_2 & 0xFFFF0000) >> 16),
			value32_1 & 0xFFFF));
	...
	PHYDM_SNPRINTF((output + used, out_len - used, "\r\n %-35s = %d / %d / %d",
			"RXEVM (1ss/2ss/3ss)", RXEVM_0, RXEVM_1, RXEVM_2));
	PHYDM_SNPRINTF((output + used, out_len - used,
			"\r\n %-35s = %d / %d / %d / %d", "RXSNR(A/B/C/D, dB)", rx_snr_path_a,
			rx_snr_path_b, rx_snr_path_c, rx_snr_path_d));

	value32 = odm_get_bb_reg(p_dm, 0xF8C, MASKDWORD);
	PHYDM_SNPRINTF((output + used, out_len - used, "\r\n %-35s = %d / %d",
			"CSI_1st /CSI_2nd", value32 & 0xFFFF, ((value32 & 0xFFFF0000) >> 16)));
	...
	PHYDM_SNPRINTF((output + used, out_len - used, "\r\n %-35s",
			"CFO Report Info"));
	PHYDM_SNPRINTF((output + used, out_len - used,
			"\r\n %-35s = %d / %d / %d /%d", "Short CFO(Hz) <A/B/C/D>", SFO_A, SFO_B,
			SFO_C, SFO_D));
	PHYDM_SNPRINTF((output + used, out_len - used,
			"\r\n %-35s = %d / %d / %d /%d", "Long CFO(Hz) <A/B/C/D>", LFO_A, LFO_B,
			LFO_C, LFO_D));
	...
	PHYDM_SNPRINTF((output + used, out_len - used,
			"\r\n %-35s = %d / %d / %d /%d", "value SCFO(Hz) <A/B/C/D>", SFO_A, SFO_B,
			SFO_C, SFO_D));
	PHYDM_SNPRINTF((output + used, out_len - used,
			"\r\n %-35s = %d / %d / %d /%d", "ACQ CFO(Hz) <A/B/C/D>", LFO_A, LFO_B,
			LFO_C, LFO_D));
	...
	PHYDM_SNPRINTF((output + used, out_len - used,
			"\r\n %-35s = %d / %d / %d /%d", "End CFO(Hz) <A/B/C/D>", LFO_A, LFO_B,
			LFO_C, LFO_D));
	...
	PHYDM_SNPRINTF((output + used, out_len - used, "\r\n %-35s : %s", "rate",
			L_rate[idx]));
	PHYDM_SNPRINTF((output + used, out_len - used, "\r\n %-35s = %x / %x / %x",
			"Rsv/length/parity", rsv, RX_BW, length));
	...
	PHYDM_SNPRINTF((output + used, out_len - used, "\r\n %-35s", "HT-SIG1"));
	PHYDM_SNPRINTF((output + used, out_len - used, "\r\n %-35s = %x / %x / %x",
			"MCS/BW/length", HMCSS, HRX_BW, h_length));
	...
	PHYDM_SNPRINTF((output + used, out_len - used,
			"\r\n %-35s = %x / %x / %x / %x / %x / %x",
			"Smooth/NoSound/Rsv/Aggregate/STBC/LDPC", smooth, htsound, rsv, agg, stbc,
			fec));
	PHYDM_SNPRINTF((output + used, out_len - used,
			"\r\n %-35s = %x / %x / %x / %x", "SGI/E-HT-LTFs/CRC/tail", sgi, htltf,
			htcrc8, tail));
	...
	PHYDM_SNPRINTF((output + used, out_len - used,
			"\r\n %-35s = %x / %x / %x / %x / %x / %x / %x / %x",
			"BW/Rsv1/STBC/GID/Nsts/PAID/TXOPPS/Rsv2", v_rx_bw, vrsv, vstbc, vgid,
			v_nsts, vpaid, vtxops, vrsv2));
	...
	PHYDM_SNPRINTF((output + used, out_len - used,
			"\r\n %-35s = %x / %x / %x / %x / %x / %x / %x",
			"SGI/FEC/MCS/BF/Rsv/CRC/tail", sgiext, fecext, v_mcss, bf, vrsv, vhtcrc8,
			v_tail));
	...
	PHYDM_SNPRINTF((output + used, out_len - used, "\r\n %-35s", "VHT-SIG-B"));
	PHYDM_SNPRINTF((output + used, out_len - used,
			"\r\n %-35s = %x / %x / %x / %x", "length/Rsv/tail/CRC", v_length, vbrsv,
			vb_tail, vbcrc));
	...
	if (p_dm->support_ic_type & ODM_RTL8822B) {
		...
		odm_set_bb_reg(p_dm, 0x1988, BIT(22),
		...
		PHYDM_SNPRINTF((output + used, out_len - used,
				"\r\n %-35s = %d (factor = %s)", "Condition number", condition_num,
				factor));
...
void phydm_reset_rx_rate_distribution(
...
)
{
	...
	p_dbg_avg->rssi_cck_avg = (u8)((p_dbg_statistic->rssi_cck_cnt != 0) ?
				       (p_dbg_statistic->rssi_cck_sum / p_dbg_statistic->rssi_cck_cnt) : 0);
	...
	if (p_dm->support_ic_type & (PHYDM_IC_ABOVE_2SS)) {
		...
		if (p_dbg_statistic->rssi_2ss_cnt != 0) {
			p_dbg_avg->rssi_2ss_avg[0] = (u8)(p_dbg_statistic->rssi_2ss_sum[0] /
							  p_dbg_statistic->rssi_2ss_cnt);
			p_dbg_avg->rssi_2ss_avg[1] = (u8)(p_dbg_statistic->rssi_2ss_sum[1] /
							  p_dbg_statistic->rssi_2ss_cnt);

			p_dbg_avg->evm_2ss_avg[0] = (u8)(p_dbg_statistic->evm_2ss_sum[0] /
							 p_dbg_statistic->rssi_2ss_cnt);
			p_dbg_avg->evm_2ss_avg[1] = (u8)(p_dbg_statistic->evm_2ss_sum[1] /
							 p_dbg_statistic->rssi_2ss_cnt);

			p_dbg_avg->snr_2ss_avg[0] = (u8)(p_dbg_statistic->snr_2ss_sum[0] /
							 p_dbg_statistic->rssi_2ss_cnt);
			p_dbg_avg->snr_2ss_avg[1] = (u8)(p_dbg_statistic->snr_2ss_sum[1] /
							 p_dbg_statistic->rssi_2ss_cnt);
		}
	...
	if (p_dm->support_ic_type & (PHYDM_IC_ABOVE_3SS)) {
		...
		if (p_dbg_statistic->rssi_3ss_cnt != 0) {
			p_dbg_avg->rssi_3ss_avg[0] = (u8)(p_dbg_statistic->rssi_3ss_sum[0] /
							  p_dbg_statistic->rssi_3ss_cnt);
			p_dbg_avg->rssi_3ss_avg[1] = (u8)(p_dbg_statistic->rssi_3ss_sum[1] /
							  p_dbg_statistic->rssi_3ss_cnt);
			p_dbg_avg->rssi_3ss_avg[2] = (u8)(p_dbg_statistic->rssi_3ss_sum[2] /
							  p_dbg_statistic->rssi_3ss_cnt);

			p_dbg_avg->evm_3ss_avg[0] = (u8)(p_dbg_statistic->evm_3ss_sum[0] /
							 p_dbg_statistic->rssi_3ss_cnt);
			p_dbg_avg->evm_3ss_avg[1] = (u8)(p_dbg_statistic->evm_3ss_sum[1] /
							 p_dbg_statistic->rssi_3ss_cnt);
			p_dbg_avg->evm_3ss_avg[2] = (u8)(p_dbg_statistic->evm_3ss_sum[2] /
							 p_dbg_statistic->rssi_3ss_cnt);

			p_dbg_avg->snr_3ss_avg[0] = (u8)(p_dbg_statistic->snr_3ss_sum[0] /
							 p_dbg_statistic->rssi_3ss_cnt);
			p_dbg_avg->snr_3ss_avg[1] = (u8)(p_dbg_statistic->snr_3ss_sum[1] /
							 p_dbg_statistic->rssi_3ss_cnt);
			p_dbg_avg->snr_3ss_avg[2] = (u8)(p_dbg_statistic->snr_3ss_sum[2] /
							 p_dbg_statistic->rssi_3ss_cnt);
		}
		...
		PHYDM_DBG(p_dm, ODM_COMP_COMMON,
			  ("* 3-ss Cnt= ((%d)) RSSI:{%d, %d, %d} EVM:{%d, %d, %d} SNR:{%d, %d, %d}\n",
			   p_dbg_statistic->rssi_3ss_cnt,
			   p_dbg_avg->rssi_3ss_avg[0], p_dbg_avg->rssi_3ss_avg[1],
			   p_dbg_avg->rssi_3ss_avg[2],
			   p_dbg_avg->evm_3ss_avg[0], p_dbg_avg->evm_3ss_avg[1],
			   p_dbg_avg->evm_3ss_avg[2],
			   p_dbg_avg->snr_3ss_avg[0], p_dbg_avg->snr_3ss_avg[1],
			   p_dbg_avg->snr_3ss_avg[2]));
	}
#endif
	...
	if (p_dm->support_ic_type & PHYDM_IC_ABOVE_4SS) {
		...
		if (p_dbg_statistic->rssi_4ss_cnt != 0) {
			p_dbg_avg->rssi_4ss_avg[0] = (u8)(p_dbg_statistic->rssi_4ss_sum[0] /
							  p_dbg_statistic->rssi_4ss_cnt);
			p_dbg_avg->rssi_4ss_avg[1] = (u8)(p_dbg_statistic->rssi_4ss_sum[1] /
							  p_dbg_statistic->rssi_4ss_cnt);
			p_dbg_avg->rssi_4ss_avg[2] = (u8)(p_dbg_statistic->rssi_4ss_sum[2] /
							  p_dbg_statistic->rssi_4ss_cnt);
			p_dbg_avg->rssi_4ss_avg[3] = (u8)(p_dbg_statistic->rssi_4ss_sum[3] /
							  p_dbg_statistic->rssi_4ss_cnt);

			p_dbg_avg->evm_4ss_avg[0] = (u8)(p_dbg_statistic->evm_4ss_sum[0] /
							 p_dbg_statistic->rssi_4ss_cnt);
			p_dbg_avg->evm_4ss_avg[1] = (u8)(p_dbg_statistic->evm_4ss_sum[1] /
							 p_dbg_statistic->rssi_4ss_cnt);
			p_dbg_avg->evm_4ss_avg[2] = (u8)(p_dbg_statistic->evm_4ss_sum[2] /
							 p_dbg_statistic->rssi_4ss_cnt);
			p_dbg_avg->evm_4ss_avg[3] = (u8)(p_dbg_statistic->evm_4ss_sum[3] /
							 p_dbg_statistic->rssi_4ss_cnt);

			p_dbg_avg->snr_4ss_avg[0] = (u8)(p_dbg_statistic->snr_4ss_sum[0] /
							 p_dbg_statistic->rssi_4ss_cnt);
			p_dbg_avg->snr_4ss_avg[1] = (u8)(p_dbg_statistic->snr_4ss_sum[1] /
							 p_dbg_statistic->rssi_4ss_cnt);
			p_dbg_avg->snr_4ss_avg[2] = (u8)(p_dbg_statistic->snr_4ss_sum[2] /
							 p_dbg_statistic->rssi_4ss_cnt);
			p_dbg_avg->snr_4ss_avg[3] = (u8)(p_dbg_statistic->snr_4ss_sum[3] /
							 p_dbg_statistic->rssi_4ss_cnt);
		}
		...
		PHYDM_DBG(p_dm, ODM_COMP_COMMON,
			  ("* 4-ss Cnt= ((%d)) RSSI:{%d, %d, %d, %d} EVM:{%d, %d, %d, %d} SNR:{%d, %d, %d, %d}\n",
			   p_dbg_statistic->rssi_4ss_cnt,
			   p_dbg_avg->rssi_4ss_avg[0], p_dbg_avg->rssi_4ss_avg[1],
			   p_dbg_avg->rssi_4ss_avg[2], p_dbg_avg->rssi_4ss_avg[3],
			   p_dbg_avg->evm_4ss_avg[0], p_dbg_avg->evm_4ss_avg[1],
			   p_dbg_avg->evm_4ss_avg[2], p_dbg_avg->evm_4ss_avg[3],
			   p_dbg_avg->snr_4ss_avg[0], p_dbg_avg->snr_4ss_avg[1],
			   p_dbg_avg->snr_4ss_avg[2], p_dbg_avg->snr_4ss_avg[3]));
	}
#endif
...
)
{
	...
	struct PHY_DM_STRUCT		*p_dm = (struct PHY_DM_STRUCT *)p_dm_void;
	struct phydm_fa_struct *false_alm_cnt = (struct phydm_fa_struct *)
						phydm_get_structure(p_dm, PHYDM_FALSEALMCNT);
	struct phydm_cfo_track_struct				*p_cfo_track = (struct
			phydm_cfo_track_struct *)phydm_get_structure(p_dm, PHYDM_CFOTRACK);
	...
	if (p_dm->is_linked) {
		...
		if ((p_dm->support_ic_type & ODM_IC_PHY_STATUE_NEW_TYPE)
			...
			PHYDM_DBG(p_dm, ODM_COMP_COMMON, ("[AGC Idx] {0x%x, 0x%x, 0x%x, 0x%x}\n",
							  p_dm->ofdm_agc_idx[0], p_dm->ofdm_agc_idx[1], p_dm->ofdm_agc_idx[2],
							  p_dm->ofdm_agc_idx[3]));
		} else {
		...
		tmp_val_u1 = (p_cfo_track->crystal_cap > p_cfo_track->def_x_cap) ?
...
)
{
...
#if (DM_ODM_SUPPORT_TYPE & ODM_WIN)
	{
		...
		PHYDM_SNPRINTF((output + used, out_len - used,
				"  %-35s: %d (Subversion: %d)\n", "FW Ver",
				adapter->MgntInfo.FirmwareVersion, adapter->MgntInfo.FirmwareSubVersion));
	}
#elif (DM_ODM_SUPPORT_TYPE & ODM_AP)
	{
		struct rtl8192cd_priv *priv = p_dm->priv;
		PHYDM_SNPRINTF((output + used, out_len - used,
				"  %-35s: %d (Subversion: %d)\n", "FW Ver", priv->pshare->fw_version,
...
#elif (DM_ODM_SUPPORT_TYPE == ODM_CE) && defined(DM_ODM_CE_MAC80211)
	{
		...
		PHYDM_SNPRINTF((output + used, out_len - used,
				"  %-35s: %d (Subversion: %d)\n", "FW Ver", rtlhal->fw_version,
				rtlhal->fw_subversion));
	}
#else
	{
		struct _ADAPTER		*adapter = p_dm->adapter;
		HAL_DATA_TYPE		*p_hal_data = GET_HAL_DATA(adapter);
		PHYDM_SNPRINTF((output + used, out_len - used,
				"  %-35s: %d (Subversion: %d)\n", "FW Ver", p_hal_data->firmware_version,
				p_hal_data->firmware_sub_version));
	}
#endif
	...
	if (p_dm->support_ic_type & (ODM_RTL8822B | ODM_RTL8197F | ODM_RTL8821C)) {
		if (dm_value[1] <= 1) {
			if ((u8)dm_value[2] != 0xff) {
				if (phydm_api_set_txagc(p_dm, dm_value[3], (enum rf_path) dm_value[1],
							(u8)dm_value[2], true))
					PHYDM_SNPRINTF((output + used, out_len - used, "  %s%d   %s%x%s%x\n",
							"Write path-", dm_value[1], "rate index-0x", dm_value[2], " = 0x",
							dm_value[3]));
				else
					PHYDM_SNPRINTF((output + used, out_len - used, "  %s%d   %s%x%s\n",
							"Write path-", (dm_value[1] & 0x1), "rate index-0x", (dm_value[2] & 0x7f),
							" fail"));
			} else {
				u8	i;
				u32	power_index;
				...
				if (p_dm->support_ic_type & (ODM_RTL8822B | ODM_RTL8821C)) {
					power_index = (power_index << 24) | (power_index << 16) |
						      (power_index << 8) | (power_index);

					for (i = 0; i < ODM_RATEVHTSS2MCS9; i += 4)
						status = (status & phydm_api_set_txagc(p_dm, power_index,
										       (enum rf_path) dm_value[1], i, false));
				} else if (p_dm->support_ic_type & ODM_RTL8197F) {
...
)
{
	...
	if (dm_value[0] == 100) {
		...
		if (p_dm->fw_debug_components == 0) {
			...
			phydm_fw_trace_en_h2c(p_dm, false, p_dm->fw_debug_components, dm_value[2],
					      dm_value[3]); /*H2C to enable C2H Msg*/
		} else {
			p_dm->debug_components |= DBG_FW_TRACE;
			phydm_fw_trace_en_h2c(p_dm, true, p_dm->fw_debug_components, dm_value[2],
					      dm_value[3]); /*H2C to enable C2H Msg*/
		}
	}
}
...
)
{
	...
	if (state == false) {
		...
		odm_set_bb_reg(p_dm, 0x8c8, 0xfe,
			       ((p_dig_t->big_jump_step3 << 5) | (p_dig_t->big_jump_step2 << 3) |
...
)
{
	...
	u8 id = 0;
	int var1[10] = {0};
	int i, input_idx = 0, phydm_ary_size = sizeof(phy_dm_ary) / sizeof(
			struct _PHYDM_COMMAND);
	char help[] = "-h";
	...
	case PHYDM_SMART_ANT:
		...
		if (input_idx >= 1) {
			...
			phydm_hl_smart_ant_debug_type2(p_dm, &input[0], &used, output, &out_len,
						       input_num);
#elif (defined(CONFIG_HL_SMART_ANTENNA_TYPE1))
			phydm_hl_smart_ant_debug(p_dm, &input[0], &used, output, &out_len,
						 input_num);
#endif
			...
			phydm_cumitek_smt_ant_debug(p_dm, &input[0], &used, output, &out_len,
						    input_num);
#endif
		}
	...
	case PHYDM_TRX_PATH:
		...
		if (p_dm->support_ic_type & (ODM_RTL8822B | ODM_RTL8197F)) {
			...
			if (is_enable_dbg_mode) {
				p_dm->is_disable_phy_api = false;
				phydm_api_trx_mode(p_dm, (enum bb_path) tx_path, (enum bb_path) rx_path,
						   is_tx2_path);
				p_dm->is_disable_phy_api = true;
				PHYDM_SNPRINTF((output + used, out_len - used,
	...
	case PHYDM_LA_MODE:
		...
		phydm_lamode_trigger_setting(p_dm, &input[0], &used, output, &out_len,
					     input_num);
#else
		PHYDM_SNPRINTF((output + used, out_len - used,
	...
	case PHYDM_AUTO_DBG:
		...
		phydm_auto_dbg_console(p_dm, &input[0], &used, output, &out_len,
				       input_num);
#endif
		break;
	...
	case PHYDM_CSI_MASK_EN:
		...
		if (input_idx >= 1) {
			...
			phydm_api_debug(p_dm, PHYDM_API_CSI_MASK, (u32 *)var1, &used, output,
					&out_len);
			/**/
		}
	...
	case PHYDM_NHM: {
		...
		if ((var1[0] <= 2) && (var1[0] != 0)) {
			...
			ccx_info->nhm_inexclude_cca = (var1[0] == 1) ? NHM_EXCLUDE_CCA :
						      NHM_INCLUDE_CCA;
			...
			for (i = 0; i <= 10; i++) {
				...
				if (i == 5)
					PHYDM_SNPRINTF((output + used, out_len - used,
							"\r\n NHM_th[%d] = 0x%x, echo_igi = 0x%x", i, ccx_info->nhm_th[i],
							ccx_info->echo_igi));
				else if (i == 10)
					PHYDM_SNPRINTF((output + used, out_len - used, "\r\n NHM_th[%d] = 0x%x\n",
							i, ccx_info->nhm_th[i]));
		...
		/*Get NHM results*/
			...
			for (i = 0; i <= 11; i++) {
				...
				if (i == 5)
					PHYDM_SNPRINTF((output + used, out_len - used,
							"\r\n nhm_result[%d] = %d, echo_igi = 0x%x", i, ccx_info->nhm_result[i],
							ccx_info->echo_igi));
				else if (i == 11)
					PHYDM_SNPRINTF((output + used, out_len - used,
	...
	case PHYDM_PAUSE_FUNC:
		phydm_pause_func_console(p_dm, &input[0], &used, output, &out_len,
					 input_num);
		break;
...
void phydm_fw_trace_handler_code(
	void	*p_dm_void,
	u8	*buffer,
	u8	cmd_len
)
	...
	/*--------------------------------------------*/
		...
		if (dbg_num == 1) {
			if (content_0 == 0) {
				...
				if (content_0 == 1) {
					PHYDM_DBG(p_dm, DBG_FW_TRACE,
						  ("[FW][H2C=0x40]  MACID=((%d)), rate ID=((%d)),  SGI=((%d)),  BW=((%d))\n",
						   content_1, content_2, content_3, content_4));
					/**/
				} else if (content_0 == 2) {
					PHYDM_DBG(p_dm, DBG_FW_TRACE,
						  ("[FW][H2C=0x40]   VHT_en=((%d)), Disable_PowerTraining=((%d)),  Disable_RA=((%d)),  No_Update=((%d))\n",
						   content_1, content_2, content_3, content_4));
					/**/
				} else if (content_0 == 3) {
					PHYDM_DBG(p_dm, DBG_FW_TRACE,
						  ("[FW][H2C=0x40]   RA_MSK=[%x | %x | %x | %x ]\n", content_1, content_2,
						   content_3, content_4));
					/**/
				}
			}
	...
	/*--------------------------------------------*/
		...
		PHYDM_DBG(p_dm, DBG_FW_TRACE,
			  ("[FW][general][%d, %d, %d] = {%d, %d, %d, %d}\n", function, dbg_num,
			   content_0, content_1, content_2, content_3, content_4));
		/**/
	}
#else
	PHYDM_DBG(p_dm, DBG_FW_TRACE,
...
#if (DM_ODM_SUPPORT_TYPE == ODM_WIN)
	RT_DISP(FC2H, C2H_Summary,
		("[Extend C2H packet], Extend_c2hSubId=0x%x, extend_c2h_dbg_len=%d\n",
		 extend_c2h_sub_id, extend_c2h_dbg_len));

	RT_DISP_DATA(FC2H, C2H_Summary, "[Extend C2H packet], Content Hex:",
		     extend_c2h_dbg_content, cmd_len - 2);
#endif
