...
static u8 _halmac_mac_reg_page0_chk(const char *func,
				    struct dvobj_priv *dvobj, u32 offset)
{
...
static u8 _halmac_buff_print(void *p, u32 msg_type, u8 msg_level, s8 *buf,
			     u32 size)
{
...
static inline u8 is_valid_id_status(HALMAC_FEATURE_ID id,
				    HALMAC_CMD_PROCESS_STATUS status)
{
	switch (id) {
...
static int init_halmac_event_with_waittime(struct dvobj_priv *d,
		HALMAC_FEATURE_ID id, u8 *buf, u32 size, u32 time)
{
...
static inline int init_halmac_event(struct dvobj_priv *d,
				    HALMAC_FEATURE_ID id, u8 *buf, u32 size)
{
	return init_halmac_event_with_waittime(d, id, buf, size,
 ...
 */
static u8 _halmac_event_indication(void *p, HALMAC_FEATURE_ID feature_id,
				   HALMAC_CMD_PROCESS_STATUS process_status, u8 *buf, u32 size)
{
	struct dvobj_priv *d;
	PADAPTER adapter;
	PHAL_DATA_TYPE hal;
	...
	if (!indicator->sctx) {
		RTW_WARN("%s: No feature id(%d, %s) waiting!!\n", __FUNCTION__, feature_id,
			 RTW_HALMAC_FEATURE_NAME[feature_id]);
		goto exit;
	}
	sctx = indicator->sctx;
...
static void _read_register(struct dvobj_priv *d, u32 addr, u32 cnt,
			   u8 *buf)
{
#if 1
	struct _ADAPTER *a;
...
#ifdef CONFIG_SDIO_HCI
static int _sdio_read_local(struct dvobj_priv *d, u32 addr, u32 cnt,
			    u8 *buf)
{
...
void rtw_halmac_read_mem(struct intf_hdl *pintfhdl, u32 addr, u32 cnt,
			 u8 *pmem)
{
	struct dvobj_priv *d;
...
#ifdef CONFIG_SDIO_HCI
static enum _HALMAC_SDIO_SPEC_VER _sdio_ver_drv2halmac(
	struct dvobj_priv *d)
{
	bool v3;
...
int rtw_halmac_init_adapter(struct dvobj_priv *d,
			    PHALMAC_PLATFORM_API pf_api)
{
...
static u8 _network_type_halmac2drv(enum _HALMAC_NETWORK_TYPE_SELECT
				   network)
{
...
int rtw_halmac_get_hw_value(struct dvobj_priv *d, HALMAC_HW_ID hw_id,
			    VOID *pvalue)
{
	PHALMAC_ADAPTER mac;
	PHALMAC_API api;
...
int rtw_halmac_get_tx_agg_align_size(struct dvobj_priv *d, u16 *size)
{
	...
	status = api->halmac_get_hw_value(halmac, HALMAC_HW_TX_AGG_ALIGN_SIZE,
					  &val);
	if (status != HALMAC_RET_SUCCESS)
		return -1;
...
int rtw_halmac_get_rx_agg_align_size(struct dvobj_priv *d, u8 *size)
{
	...
	status = api->halmac_get_hw_value(halmac, HALMAC_HW_RX_AGG_ALIGN_SIZE,
					  &val);
	if (status != HALMAC_RET_SUCCESS)
		return -1;
 ...
 * Rteurn 0 for OK, otherwise fail.
 */
int rtw_halmac_get_mac_address(struct dvobj_priv *d, enum _hw_port hwport,
			       u8 *addr)
{
	struct _HALMAC_ADAPTER *halmac;
	struct _HALMAC_API *api;
 ...
 * Rteurn 0 for OK, otherwise fail.
 */
int rtw_halmac_get_network_type(struct dvobj_priv *d, enum _hw_port hwport,
				u8 *type)
{
#if 0
	struct _HALMAC_ADAPTER *halmac;
 ...
 * Rteurn 0 for OK, otherwise fail.
 */
int rtw_halmac_set_mac_address(struct dvobj_priv *d, enum _hw_port hwport,
			       u8 *addr)
{
	PHALMAC_ADAPTER halmac;
	PHALMAC_API api;
 ...
 * Rteurn 0 for OK, otherwise fail.
 */
int rtw_halmac_set_bssid(struct dvobj_priv *d, enum _hw_port hwport,
			 u8 *addr)
{
	PHALMAC_ADAPTER halmac;
	PHALMAC_API api;
 ...
 * Rteurn 0 for OK, otherwise fail.
 */
int rtw_halmac_set_tx_address(struct dvobj_priv *d, enum _hw_port hwport,
			      u8 *addr)
{
	struct _HALMAC_ADAPTER *halmac;
	struct _HALMAC_API *api;
 ...
 * Rteurn 0 for OK, otherwise fail.
 */
int rtw_halmac_set_network_type(struct dvobj_priv *d, enum _hw_port hwport,
				u8 type)
{
	struct _HALMAC_ADAPTER *halmac;
	struct _HALMAC_API *api;
...
int rtw_halmac_set_bandwidth(struct dvobj_priv *d, u8 channel,
			     u8 pri_ch_idx, u8 bw)
{
	PHALMAC_ADAPTER mac;
	PHALMAC_API api;
 ...
 */
int rtw_halmac_set_edca(struct dvobj_priv *d, u8 queue, u8 aifs, u8 cw,
			u16 txop)
{
	PHALMAC_ADAPTER mac;
	PHALMAC_API api;
...
#ifdef CONFIG_SDIO_HCI
	status = api->halmac_sdio_cmd53_4byte(halmac,
...
static inline HALMAC_RX_FIFO_EXPANDING_MODE _trx_share_mode_drv2halmac(
	u8 trx_share_mode)
{
	if (0 == trx_share_mode)
...
static HALMAC_RX_FIFO_EXPANDING_MODE _rtw_get_trx_share_mode(
	_adapter *adapter)
{
...
static u8 _rsvd_page_num_halmac2drv(HALMAC_DRV_RSVD_PG_NUM
				    rsvd_page_number)
{
	u8 num = 0;
...
#ifdef CONFIG_SUPPORT_TRX_SHARED
	status = api->halmac_cfg_rx_fifo_expanding_mode(halmac,
...
static enum _HALMAC_DLFW_MEM _fw_mem_drv2halmac(enum fw_mem mem,
		u8 tx_stop)
{
...
int rtw_halmac_dlfw_mem(struct dvobj_priv *d, u8 *fw, u32 fwsize,
			enum fw_mem mem)
{
	struct _HALMAC_ADAPTER *mac;
	struct _HALMAC_API *api;
...
int rtw_halmac_dlfw_mem_from_file(struct dvobj_priv *d, u8 *fwpath,
				  enum fw_mem mem)
{
	u8 *fw = NULL;
	u32 fwmaxsize = 0, size = 0;
...
static u8 _is_fw_read_cmd_down(PADAPTER adapter, u8 msgbox_num)
{
	...
	if (_FALSE == read_down)
		RTW_WARN("%s, reg_1cc(%x), msg_box(%d)...\n", __func__, rtw_read8(adapter,
				REG_HMETFR), msgbox_num);
...
int rtw_halmac_read_physical_efuse_map(struct dvobj_priv *d, u8 *map,
				       u32 size)
{
	PHALMAC_ADAPTER mac;
	PHALMAC_API api;
...
int rtw_halmac_read_physical_efuse(struct dvobj_priv *d, u32 offset,
				   u32 cnt, u8 *data)
{
	PHALMAC_ADAPTER mac;
	PHALMAC_API api;
...
int rtw_halmac_write_physical_efuse(struct dvobj_priv *d, u32 offset,
				    u32 cnt, u8 *data)
{
	PHALMAC_ADAPTER mac;
	PHALMAC_API api;
...
int rtw_halmac_read_logical_efuse_map(struct dvobj_priv *d, u8 *map,
...
int rtw_halmac_write_logical_efuse_map(struct dvobj_priv *d, u8 *map,
				       u32 size, u8 *maskmap, u32 masksize)
{
	PHALMAC_ADAPTER mac;
	PHALMAC_API api;
...
int rtw_halmac_read_logical_efuse(struct dvobj_priv *d, u32 offset,
				  u32 cnt, u8 *data)
{
	PHALMAC_ADAPTER mac;
	PHALMAC_API api;
...
int rtw_halmac_write_logical_efuse(struct dvobj_priv *d, u32 offset,
				   u32 cnt, u8 *data)
{
	PHALMAC_ADAPTER mac;
	PHALMAC_API api;
...
int rtw_halmac_write_bt_physical_efuse(struct dvobj_priv *d, u32 offset,
				       u32 cnt, u8 *data)
{
	PHALMAC_ADAPTER mac;
	PHALMAC_API api;
...
int rtw_halmac_read_bt_physical_efuse_map(struct dvobj_priv *d, u8 *map,
		u32 size)
{
	PHALMAC_ADAPTER mac;
	PHALMAC_API api;
...
int rtw_halmac_dump_fifo(struct dvobj_priv *d, u8 fifo_sel, u32 addr,
			 u32 size, u8 *buffer)
{
	PHALMAC_ADAPTER mac;
	PHALMAC_API api;
	...
	status = api->halmac_dump_fifo(mac, halmac_fifo_sel, addr, fifo_size,
				       pfifo_map);
	if (HALMAC_RET_SUCCESS != status) {
		ret = -1;
		goto _exit;
...
int rtw_halmac_download_rsvd_page(struct dvobj_priv *dvobj, u8 pg_offset,
				  u8 *pbuf, u32 size)
{
 ...
 *	others	Fail
 */
int rtw_halmac_fill_hal_spec(struct dvobj_priv *dvobj,
			     struct hal_spec_t *spec)
{
	HALMAC_RET_STATUS status;
	PHALMAC_ADAPTER halmac;
	PHALMAC_API api;
...
int rtw_halmac_p2pps(struct dvobj_priv *dvobj,
		     PHAL_P2P_PS_PARA pp2p_ps_para)
{
	HALMAC_RET_STATUS status = HALMAC_RET_SUCCESS;
	PHALMAC_ADAPTER halmac = dvobj_to_halmac(dvobj);
	PHALMAC_API api = HALMAC_GET_API(halmac);
	...
	(&halmac_p2p_ps)->p2p_port_id = _hw_port_drv2halmac(
						pp2p_ps_para->p2p_port_id);
...
static int _phy_parameter_drv2halmac(struct rtw_phy_parameter *para,
 ...
 */
int rtw_halmac_cfg_phy_para(struct dvobj_priv *d,
...
int rtw_halmac_query_tx_page_num(struct dvobj_priv *d)
{
	...
	if (status != HALMAC_RET_SUCCESS)
		return -1;
	status = api->halmac_get_hw_value(halmac, HALMAC_HW_TXFF_ALLOCATION,
					  &fifosize);
	if (status != HALMAC_RET_SUCCESS)
		return -1;
 ...
 *	others	Fail, "page" is invalid.
 */
int rtw_halmac_get_tx_queue_page_num(struct dvobj_priv *d, u8 queue,
				     u32 *page)
{
	*page = 0;
...
int rtw_halmac_usb_get_txagg_desc_num(struct dvobj_priv *d, u8 *num)
{
	...
	status = api->halmac_get_hw_value(halmac, HALMAC_HW_USB_TXAGG_DESC_NUM,
					  &val);
	if (status != HALMAC_RET_SUCCESS)
		return -1;
...
static inline HALMAC_USB_MODE _usb_mode_drv2halmac(enum RTW_USB_SPEED
		usb_mode)
{
...
u8 rtw_halmac_switch_usb_mode(struct dvobj_priv *d,
			      enum RTW_USB_SPEED usb_mode)
{
	PHALMAC_ADAPTER mac;
	PHALMAC_API api;
	HALMAC_RET_STATUS status;
	...
	status = api->halmac_set_hw_value(mac, HALMAC_HW_USB_MODE,
					  (void *)&halmac_usb_mode);
