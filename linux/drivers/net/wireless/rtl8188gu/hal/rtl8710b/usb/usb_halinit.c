...
static BOOLEAN HalUsbSetQueuePipeMapping8710BUsb(PADAPTER padapter,
...
void HalSetOutPutGPIO(PADAPTER padapter, u8 index, u8 OutPutValue)
{
	if (index <= 7) {
		...
		/* set output value */
		if (OutPutValue)
			rtw_write8(padapter, REG_GPIO_PIN_CTRL + 1, rtw_read8(padapter,
					REG_GPIO_PIN_CTRL + 1) | BIT(index));
		else
			rtw_write8(padapter, REG_GPIO_PIN_CTRL + 1, rtw_read8(padapter,
					REG_GPIO_PIN_CTRL + 1) & ~BIT(index));
	} else {
		...
		/* config GPIO mode */
		rtw_write8(padapter, REG_GPIO_PIN_CTRL_2 + 3, rtw_read8(padapter,
				REG_GPIO_PIN_CTRL_2 + 3) & ~BIT(index));

		/* config GPIO Sel */
		/* 0: input */
		/* 1: output */
		rtw_write8(padapter, REG_GPIO_PIN_CTRL_2 + 2, rtw_read8(padapter,
				REG_GPIO_PIN_CTRL_2 + 2) | BIT(index));

		/* set output value */
		if (OutPutValue)
			rtw_write8(padapter, REG_GPIO_PIN_CTRL_2 + 1, rtw_read8(padapter,
					REG_GPIO_PIN_CTRL_2 + 1) | BIT(index));
		else
			rtw_write8(padapter, REG_GPIO_PIN_CTRL_2 + 1, rtw_read8(padapter,
					REG_GPIO_PIN_CTRL_2 + 1) & ~BIT(index));
	}
...
static u32 _InitPowerOn_8710bu(PADAPTER padapter)
{
	...
	if (!HalPwrSeqCmdParsing(padapter, PWR_CUT_ALL_MSK, PWR_FAB_ALL_MSK,
				 PWR_INTF_USB_MSK, rtl8710B_card_enable_flow))
		return _FALSE;

	// Enable MAC DMA/WMAC/SCHEDULE/SEC block
	// Set CR bit10 to enable 32k calibration. Suggested by SD1 Gimmy. Added by tynli. 2011.08.31.
	rtw_write16(padapter, REG_CR_8710B,
...
static void _InitQueueReservedPage(PADAPTER padapter)
{
	...
	if (pHalData->OutEpQueueSel & TX_SELE_HQ)
		numHQ = bWiFiConfig ? WMM_NORMAL_PAGE_NUM_HPQ_8710B :
			NORMAL_PAGE_NUM_HPQ_8710B;

	if (pHalData->OutEpQueueSel & TX_SELE_LQ)
		numLQ = bWiFiConfig ? WMM_NORMAL_PAGE_NUM_LPQ_8710B :
			NORMAL_PAGE_NUM_LPQ_8710B;

	/* NOTE: This step shall be proceed before writing REG_RQPN. */
	if (pHalData->OutEpQueueSel & TX_SELE_NQ)
		numNQ = bWiFiConfig ? WMM_NORMAL_PAGE_NUM_NPQ_8710B :
			NORMAL_PAGE_NUM_NPQ_8710B;
...
u32 rtl8710bu_hal_init(PADAPTER padapter)
{
	...
	rtw_write8(padapter, REG_USB_ACCESS_TIMEOUT,
		   0x80);  //set usb timeout to fix read mac register fail before power on
	...
	hal_set_syson_reg_8710b(padapter, 0x138, BIT5,
				0x0);//add by ylb , clear the bit to prevent CM4 Suspend
	...
	rtw_write8(padapter, REG_LIFETIME_CTRL, rtw_read8(padapter,
			REG_LIFETIME_CTRL) | 0x0f);
	...
	rtw_write32(padapter, REG_FWHW_TXQ_CTRL, rtw_read32(padapter,
	...
	*/
	...
	rtw_write32(padapter, REG_WL_RF_PSS_8710B,
		    value32 | (BIT0 | BIT1 | BIT2 | BIT8)); //0x5c = 0x107

	if (padapter->registrypriv.mp_mode == 1) {
		//This is Only for 8710B MP , suggest by RF jerry_fan
		PlatformEFIOWrite4Byte(padapter, REG_EDCA_VO_PARAM_8710B,
				       0x00000080); // interval 128us
		PlatformEFIOWrite4Byte(padapter, REG_EDCA_VI_PARAM_8710B,
				       0x00000080); // interval 128us
		PlatformEFIOWrite4Byte(padapter, REG_EDCA_BE_PARAM_8710B,
				       0x00000080); // interval 128us
		PlatformEFIOWrite4Byte(padapter, REG_EDCA_BK_PARAM_8710B,
				       0x00000080); // interval 128us
	}

	if (padapter->registrypriv.mp_mode == 1) {
		value32 = indirect_read32_8710b(padapter, 0x10000318);
		indirect_write32_8710b(padapter, 0x10000318,
				       value32 & (~BIT19)); //clear 0x10000318[19]=0
		RTW_DBG("%s : REG 0x10000318 = 0x%x!!\n", __FUNCTION__,
			indirect_read32_8710b(padapter, 0x10000318));
	}
...
void card_disable_8710bu(PADAPTER padapter)
{
	...
	hal_set_syson_reg_8710b(padapter, 0x138, BIT5,
				0x1); //add by ylb , set the bit to allow CM4 Suspend
	...
	HalPwrSeqCmdParsing(padapter, PWR_CUT_ALL_MSK, PWR_FAB_ALL_MSK,
			    PWR_INTF_USB_MSK, rtl8710B_enter_lps_flow);
	...
	HalPwrSeqCmdParsing(padapter, PWR_CUT_ALL_MSK, PWR_FAB_ALL_MSK,
			    PWR_INTF_USB_MSK, rtl8710B_card_disable_flow);
 ...
 *-------------------------------------------------------------------
 */
static void hal_EfuseParseLEDSetting(
	IN PADAPTER padapter,
	IN u8 *PROMContent,
...
static void hal_EfuseParseRFSetting(
	IN PADAPTER padapter,
	IN u8 *PROMContent,
	IN BOOLEAN AutoloadFail
)
...
static void hal_EfuseParsePowerSavingSetting(
	IN PADAPTER padapter,
	IN u8 *PROMContent,
	IN u8 AutoloadFail
)
{
	...
	if (AutoloadFail) {
		...
		pwrctl->bSupportRemoteWakeup = (PROMContent[EEPROM_USB_OPTIONAL_FUNCTION0]
						& BIT(1)) ? _TRUE : _FALSE;
		...
		RTW_INFO("%s...bHWPwrPindetect(%x)-bHWPowerdown(%x) ,bSupportRemoteWakeup(%x)\n",
			 __func__,
			 pwrctl->bHWPwrPindetect, pwrctl->bHWPowerdown,
...
static u8 InitpadapterVariablesByPROM_8710bu(
	IN PADAPTER padapter
)
{
	PHAL_DATA_TYPE pHalData = GET_HAL_DATA(padapter);
	...
	Hal_EfuseParseEEPROMVer_8710B(padapter, hwinfo,
	...
	hal_EfuseParsePowerSavingSetting(padapter, hwinfo,
					 pHalData->bautoload_fail_flag);

	Hal_EfuseParseTxPowerInfo_8710B(padapter, hwinfo,
					pHalData->bautoload_fail_flag);
	Hal_EfuseParseBoardType_8710B(padapter, hwinfo,
				      pHalData->bautoload_fail_flag);

	Hal_EfuseParseBTCoexistInfo_8710B(padapter, hwinfo,
					  pHalData->bautoload_fail_flag);

	Hal_EfuseParseChnlPlan_8710B(padapter, hwinfo,
				     pHalData->bautoload_fail_flag);
	Hal_EfuseParseXtal_8710B(padapter, hwinfo, pHalData->bautoload_fail_flag);
	Hal_EfuseParseThermalMeter_8710B(padapter, hwinfo,
					 pHalData->bautoload_fail_flag);
	Hal_EfuseParseAntennaDiversity_8710B(padapter, hwinfo,
					     pHalData->bautoload_fail_flag);
	Hal_EfuseParseCustomerID_8710B(padapter, hwinfo,
				       pHalData->bautoload_fail_flag);
...
void Hal_EfuseParsePackageType(PADAPTER pAdapter, u8 *map, u8 mapvalid)
{
	...
	pAdapter->hal_func.efuse_indirect_read4(pAdapter,
						EEPROM_PACKAGE_TYPE_8710B, &package);
	...
	RTW_INFO("PackageType = 0x%X, %s\n", pHalData->PackageType,
		 (pHalData->PackageType == PACKAGE_QFN48M_S) ? "QFN48M_S" : "QFN48M_U");
}
				      ...
				      u8 mapvalid)
{
	...
	if (mapvalid) {
		pHalData->EEPROMRegulatory = (map[EEPROM_RF_BOARD_OPTION_8710B] &
					      0x7);	/* bit0~2 */
		...
		u8 mapvalid)
{
	...
	if (hal->EEPROMBluetoothAntNum == Ant_x1)
		...
		if (registry_par->antdiv_cfg == 2)/* 0:OFF , 1:ON, 2:By EFUSE */
			hal->AntDivCfg = (map[EEPROM_RF_BOARD_OPTION_8710B] & BIT3) ? _TRUE :
					 _FALSE;
		else
			hal->AntDivCfg = registry_par->antdiv_cfg;
	}
	/*hal->TRxAntDivType = S0S1_TRX_HW_ANTDIV;*/
	hal->with_extenal_ant_switch = ((map[EEPROM_RF_BT_SETTING_8710B] & BIT7) >>
					7);

	RTW_INFO("%s:EEPROM AntDivCfg=%d, AntDivType=%d, extenal_ant_switch:%d\n",
		 __func__, hal->AntDivCfg, hal->TRxAntDivType,
		 hal->with_extenal_ant_switch);
...
static void hal_ReadPAType(PADAPTER adapter, u8 *map, u8 mapvalid)
{
	...
	if (mapvalid) {
		...
		if (GetRegAmplifierType2G(adapter) == 0) {
			...
			hal_data->ExternalPA_2G  = (GetRegAmplifierType2G(adapter) &
						    ODM_BOARD_EXT_PA)  ? 1 : 0;
			hal_data->ExternalLNA_2G = (GetRegAmplifierType2G(adapter) &
 ...
 */
u8 GetHalDefVar8710bu(PADAPTER padapter, HAL_DEF_VARIABLE eVariable,
		      PVOID pValue)
{
	HAL_DATA_TYPE *pHalData = GET_HAL_DATA(padapter);
	u8 bResult = _SUCCESS;
 ...
 */
u8 SetHalDefVar8710bu(PADAPTER padapter, HAL_DEF_VARIABLE eVariable,
		      PVOID pValue)
{
	HAL_DATA_TYPE *pHalData = GET_HAL_DATA(padapter);
	u8 bResult = _SUCCESS;
...
static u8 rtl8710bu_ps_func(PADAPTER padapter, HAL_INTF_PS_FUNC efunc_id,
			    u8 *val)
{
	u8 bResult = _TRUE;
	...
	case HAL_USB_SELECT_SUSPEND: {
		...
		rtl8710b_set_FwSelectSuspend_cmd(padapter, bfwpoll,
...
extern void rtl8710bu_interrupt_handler(_adapter *padapter, u16 pkt_len,
					u8 *pbuf);
#endif
