...
void _dbg_dump_tx_info(PADAPTER padapter, int frame_tag,
...
int urb_zero_packet_chk(PADAPTER padapter, int sz)
{
	...
	blnSetTxDescOffset = (((sz + TXDESC_SIZE) % pHalData->UsbBulkOutSize) == 0)
			     ? 1 : 0;
...
s32 rtl8710bu_xmitframe_complete(PADAPTER padapter,
...
s32 rtl8710bu_hal_xmitframe_enqueue(PADAPTER padapter,
				    struct xmit_frame *pxmitframe)
{
	struct xmit_priv *pxmitpriv = &padapter->xmitpriv;
	s32 err;
