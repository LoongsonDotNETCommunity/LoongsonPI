...
*
******************************************/
s32 FillH2CCmd8710B(PADAPTER padapter, u8 ElementID, u32 CmdLen,
		    u8 *pCmdBuffer)
{
	u8 h2c_box_num;
	...
	do {
		...
		msgbox_ex_addr = REG_HMEBOX_EXT0_8710B + (h2c_box_num *
				 RTL8710B_EX_MESSAGE_BOX_SIZE);
...
static void rtl8710b_set_FwRsvdPage_cmd(PADAPTER padapter,
					PRSVDPAGE_LOC rsvdpageloc)
{
	...
	FillH2CCmd8710B(padapter, H2C_8710B_RSVD_PAGE, H2C_RSVDPAGE_LOC_LEN,
			u1H2CRsvdPageParm);
}

static void rtl8710b_set_FwAoacRsvdPage_cmd(PADAPTER padapter,
		PRSVDPAGE_LOC rsvdpageloc)
{
	...
	if (check_fwstate(pmlmepriv, _FW_LINKED)) {
		...
		FillH2CCmd8710B(padapter, H2C_8710B_AOAC_RSVD_PAGE,
				H2C_AOAC_RSVDPAGE_LOC_LEN, u1H2CAoacRsvdPageParm);
	} else {
		...
		if (!pwrpriv->wowlan_in_resume) {
			...
			SET_H2CCMD_AOAC_RSVDPAGE_LOC_NLO_INFO(u1H2CAoacRsvdPageParm,
							      rsvdpageloc->LocPNOInfo);
			FillH2CCmd8710B(padapter, H2C_AOAC_RSVDPAGE3, H2C_AOAC_RSVDPAGE_LOC_LEN,
...
void rtl8710b_set_FwPwrMode_cmd(PADAPTER padapter, u8 psmode)
{
	...
	if (psmode == PS_MODE_MIN) {
		...
		smart_ps = pwrpriv->smart_ps;
		   ...
		   PS_MODE_DTIM) { /* For WOWLAN LPS, DTIM = (awake_intvl - 1) */
	...
	SET_8710B_H2CCMD_PWRMODE_PARM_ALL_QUEUE_UAPSD(u1H2CPwrModeParm,
			allQueueUAPSD);
	...
	FillH2CCmd8710B(padapter, H2C_8710B_SET_PWR_MODE, H2C_PWRMODE_LEN,
			u1H2CPwrModeParm);
}
...
void rtl8710b_set_BcnEarly_C2H_Rpt_cmd(PADAPTER padapter, u8 enable)
{
	...
	FillH2CCmd8710B(padapter, H2C_8710B_SET_PWR_MODE, sizeof(u1H2CSetPwrMode),
			u1H2CSetPwrMode);
}
#endif
#endif
...
void rtl8710b_set_FwPsTuneParam_cmd(PADAPTER padapter)
{
	...
	FillH2CCmd8710B(padapter, H2C_8710B_PS_TUNING_PARA, H2C_PSTUNEPARAM_LEN,
			u1H2CPsTuneParm);
}
...
static void SetFwRsvdPagePkt_BTCoex(PADAPTER padapter)
{
	...
	rtw_hal_construct_NullFunctionData(
		...
		_TRUE, 0, 0, _FALSE);
	rtl8710b_fill_fake_txdesc(padapter,
				  &ReservedPagePacket[BufIndex - TxDescLen], BTQosNullLength, _FALSE, _TRUE,
				  _FALSE);
	...
	if (TotalPacketLen > MaxRsvdPageBufSize) {
		RTW_INFO(FUNC_ADPT_FMT
			 ": ERROR: The rsvd page size is not enough!!TotalPacketLen %d, MaxRsvdPageBufSize %d\n",
...
void rtl8710b_set_p2p_ps_offload_cmd(_adapter *padapter, u8 p2p_ps_state)
{
	...
	struct P2P_PS_Offload_t	*p2p_ps_offload = (struct P2P_PS_Offload_t *)(
				&pHalData->p2p_ps_offload);
	u8	i;
	...
	FillH2CCmd8710B(padapter, H2C_8710B_P2P_PS_OFFLOAD, 1,
			(u8 *)p2p_ps_offload);
#endif
