		...
		if (param->u.crypt.set_tx == 1) {
			...
			list_for_each_entry(pWapiSta, &pWapiInfo->wapiSTAUsedList, list) {
				if (_rtw_memcmp(pWapiSta->PeerMacAddr, get_bssid(pmlmepriv), 6)) {
					...
					_rtw_memcpy(pWapiSta->lastRxMulticastPN, WapiAEMultiCastPNInitialValueSrc,
						    16);
			     ...
			     , struct cfg80211_scan_request *request)
{
	...
	_rtw_memset(ch, 0, sizeof(struct rtw_ieee80211_channel) *
	...
	if (request->n_channels == 1) {
		...
		for (j = request->n_channels - 1; j >= 0; j--)
			for (i = 0; i < survey_times; i++)
				_rtw_memcpy(&ch[j * survey_times + i], &ch[j],
	...
	_enter_critical_bh(&pwdev_priv->scan_req_lock, &irqL);
	_enter_critical_bh(&pmlmepriv->lock, &irqL);
	_status = rtw_sitesurvey_cmd(padapter, ssid, RTW_SSID_SCAN_AMOUNT, pch,
				     chan_num);
	if (_status == _SUCCESS)
		pwdev_priv->scan_request = request;
	else
...
void rtw_cfg80211_indicate_sta_disassoc(_adapter *padapter,
...
struct sta_info *rtw_sta_info_get_by_idx(const int idx,
				      ...
				      struct cfg80211_update_ft_ies_params *ftie)
{
	...
	if ((rtw_to_roam(padapter) > 0)
	    && rtw_chk_ft_status(padapter, RTW_FT_AUTHENTICATED_STA)) {
...
dump:
	...
	while (1) {
		...
		rtw_mi_scan_abort(padapter, _TRUE);
		tx_ret = rtw_mgnt_tx_cmd(padapter, tx_ch, no_cck, buf, len, wait_ack,
					 RTW_CMDF_WAIT_ACK);
...
int	cfg80211_rtw_resume(struct wiphy *wiphy)
{
	...
	if (pwrpriv->wowlan_last_wake_reason == RX_PNO) {
		...
		rtw_sitesurvey_cmd(padapter,
				   (NDIS_802_11_SSID *)&pwrpriv->pno_ssid_list->node,
				   pwrpriv->pnlo_info->ssid_num, NULL, 0);
		_exit_critical_bh(&pmlmepriv->lock, &irqL);
...
static void rtw_cfg80211_init_ht_capab_ex(_adapter *padapter,
		struct ieee80211_sta_ht_cap *ht_cap, enum nl80211_band band, u8 rf_type)
{
	struct registry_priv *pregistrypriv = &padapter->registrypriv;
	struct mlme_priv	*pmlmepriv = &padapter->mlmepriv;
...
static void rtw_cfg80211_init_vht_capab(_adapter *padapter,
					struct ieee80211_sta_vht_cap *vht_cap, u8 rf_type)
{
	struct registry_priv *pregistrypriv = &padapter->registrypriv;
	struct mlme_priv	*pmlmepriv = &padapter->mlmepriv;
	...
	/* MCS map */
	vht_cap->vht_mcs.tx_mcs_map = pvhtpriv->vht_mcs_map[0] |
				      (pvhtpriv->vht_mcs_map[1] << 8);
	...
	if (pregistrypriv->ampdu_factor != 0xFE)
		vht_cap->cap |= pregistrypriv->ampdu_factor <<
	...
	HighestRate = VHT_MCS_DATA_RATE[bw][pvhtpriv->sgi_80m][((
...
void rtw_cfg80211_init_wiphy(_adapter *padapter)
{
	...
	if (IsSupported24G(padapter->registrypriv.wireless_mode)) {
		...
		if (bands)
			rtw_cfg80211_init_ht_capab(padapter, &bands->ht_cap, NL80211_BAND_2GHZ,
						   rf_type);
	}
...
#ifdef CONFIG_VHT_EXTRAS
		if (bands) {
			rtw_cfg80211_init_ht_capab(padapter, &bands->ht_cap, NL80211_BAND_5GHZ,
						   rf_type);
