...
static int rtw_drv_init(struct pci_dev *pdev,
...
static u8 rtw_pci_platform_switch_device_pci_aspm(_adapter *padapter,
		u8 value)
{
...
void rtw_pci_disable_aspm(_adapter *padapter)
{
	...
	if ((pcipriv->busnumber == 0xff &&
		...
		NdisRawWritePortUlong(PCI_CONF_ADDRESS,
...
void rtw_pci_enable_aspm(_adapter *padapter)
{
	...
	u_pcibridge_aspmsetting = pcipriv->pcibridge_linkctrlreg |
				  pdvobjpriv->const_hostpci_aspm_setting;
	...
	rtw_pci_platform_switch_device_pci_aspm(padapter,
						u_device_aspmsetting);	//(priv->linkctrl_reg | ASPMLevel));

	if (pwrpriv->reg_rfps_level & RT_RF_OFF_LEVL_CLK_REQ) {
		rtw_pci_switch_clk_req(padapter,
				       (pwrpriv->reg_rfps_level & RT_RF_OFF_LEVL_CLK_REQ) ? 1 : 0);
...
//
static u8 rtw_get_link_control_field(_adapter *padapter, u8 busnum,
				     u8 devnum,
				     u8 funcnum)
{
	struct dvobj_priv	*pdvobjpriv = adapter_to_dvobj(padapter);
	...
	pcicfg_addrport = (busnum << 16) | (devnum << 11) | (funcnum << 8) |
			  (1 << 31);
...
//
static u8 rtw_get_pci_bus_info(_adapter *padapter,
			       u16 vendorid,
			       u16 deviceid,
			       ...
			       u8 *busnum, u8 *devnum, u8 *funcnum)
{
	...
	*funcnum = 0xFF;
	...
	for (busnum_idx = 0; busnum_idx < PCI_MAX_BRIDGE_NUMBER;
	     busnum_idx++) {	//255
		for (devicenum_idx = 0; devicenum_idx < PCI_MAX_DEVICES;
		     devicenum_idx++) {	//32
			b_singlefunc = _FALSE;
			for (functionnum_idx = 0; functionnum_idx < PCI_MAX_FUNCTION;
			     functionnum_idx++) {	//8
				//
				...
				if (functionnum_idx == 0) {
					...
					pcicfg_addrport = (busnum_idx << 16) | (devicenum_idx << 11) |
							  (functionnum_idx << 8) | (1 << 31);
				...
				regoffset = 0x3C;
				pcicfg_addrport = (busnum_idx << 16) | (devicenum_idx << 11) |
						  (functionnum_idx << 8) | (1 << 31) | (regoffset & 0xFFFFFFFC);
				NdisRawWritePortUlong(PCI_CONF_ADDRESS, pcicfg_addrport);
				...
				if (b_bridgechk && (venId != vendorid) && (basec == basecode)
				    && (subc == subclass))
					return _TRUE;
				...
				if ((basec == basecode) && (subc == subclass) && ((field19 == filed19val)
						|| (filed19val == U1DONTCARE))) {
					*busnum = busnum_idx;
...
static u8 rtw_get_pci_brideg_info(_adapter *padapter,
				  u8 basecode,
				  u8 subclass,
...
static u8 rtw_get_amd_l1_patch(_adapter *padapter, u8 busnum, u8 devnum,
			       u8 funcnum)
{
	u8	status = _FALSE;
	u8	offset_e0;
...
void rtw_pci_disable_aspm(_adapter *padapter)
{
	...
	if ((pcipriv->busnumber == 0xff && pcipriv->devnumber == 0xff
	     && pcipriv->funcnumber == 0xff) ||
	    (pcipriv->pcibridge_busnum == 0xff && pcipriv->pcibridge_devnum == 0xff
	     && pcipriv->pcibridge_funcnum == 0xff)) {
		// Do Nothing!!
	} else {
		...
		//NdisRawWritePortUchar(PCI_CONF_DATA, pcibridge_linkctrlreg);
		pci_write_config_byte(bridge_pdev,
				      pcipriv->pcibridge_pciehdr_offset + 0x10, pcibridge_linkctrlreg);
...
//Added by sherry 20100803
	if (IS_HARDWARE_TYPE_8192DE(padapter)) {
		...
		pci_read_config_dword(bridge_pdev,
				      (pcipriv->pcibridge_pciehdr_offset + 0x0C), &u_device_aspmsupportsetting);
		DBG_871X("rtw_pci_enable_aspm(): Bridge ASPM support %x \n",
			 u_device_aspmsupportsetting);
		if (((u_device_aspmsupportsetting & BIT(11)) != BIT(11))
		    || ((u_device_aspmsupportsetting & BIT(10)) != BIT(10))) {
	...
	//NdisRawWritePortUchar(PCI_CONF_DATA, u_pcibridge_aspmsetting);
	pci_write_config_byte(bridge_pdev,
			      (pcipriv->pcibridge_pciehdr_offset + 0x10), u_pcibridge_aspmsetting);
...
static void rtw_pci_get_linkcontrol_field(struct dvobj_priv *dvobj)
{
	...
	pci_read_config_byte(bridge_pdev, capabilityoffset + PCI_EXP_LNKCTL,
			     &linkctrl_reg);
...
static irqreturn_t rtw_pci_interrupt(int irq, void *priv,
				     struct pt_regs *regs)
{
...
int pci_alloc_irq(struct dvobj_priv *dvobj)
{
	...
	err = request_irq(pdev->irq, &rtw_pci_interrupt, IRQF_SHARED, DRV_NAME,
			  dvobj);
#else
	err = request_irq(pdev->irq, &rtw_pci_interrupt, SA_SHIRQ, DRV_NAME,
			  dvobj);
#endif
	if (err) {
...
static struct dvobj_priv *pci_dvobj_init(struct pci_dev *pdev)
{
...
#ifdef RTK_DMP_PLATFORM
	dvobj->pci_mem_start = (unsigned long)ioremap_nocache(pmem_start,
			       pmem_len);
#else
	dvobj->pci_mem_start = (unsigned long)pci_iomap(pdev, 2,
			       pmem_len); /* shared mem start */
#endif
	...
	if (bridge_pdev) {
		...
		pcipriv->pcibridge_pciehdr_offset = pci_find_capability(bridge_pdev,
						    PCI_CAP_ID_EXP);
#else
...
static void decide_chip_type_by_pci_device_id(_adapter *padapter,
		struct pci_dev *pdev)
{
	...
	pci_read_config_byte(pdev, PCI_REVISION_ID,
			     &revisionid); // PCI_REVISION_ID 0x08
#else
	revisionid = pdev->revision;
#endif
	pci_read_config_word(pdev, PCI_INTERRUPT_LINE,
			     &irqline); // PCI_INTERRUPT_LINE 0x3c
	...
	if (padapter->bSurpriseRemoved == _FALSE) {
		...
		*/
		if (padapter->pbuddy_adapter) {
			...
			tasklet_disable(&
					(padapter->pbuddy_adapter->recvpriv.irq_prepare_beacon_tasklet));
			tasklet_disable(&(padapter->pbuddy_adapter->xmitpriv.xmit_tasklet));
		}
#endif
		RT_TRACE(_module_hci_intfs_c_, _drv_err_,
			 ("pci_intf_stop: SurpriseRemoved==_FALSE\n"));
	} else {
...
static void rtw_dev_unload(_adapter *padapter)
{
	...
	if (padapter->bup == _TRUE) {
		...
		RT_TRACE(_module_hci_intfs_c_, _drv_err_,
			 ("r871x_dev_unload():padapter->bup == _FALSE\n"));
	}
...
static void disable_ht_for_spec_devid(const struct pci_device_id *pdid)
{
	...
	u32 flags;
	int i;
	int num = sizeof(specific_device_id_tbl) / sizeof(struct
			specific_device_id);
	...
	for (i = 0; i < num; i++) {
		...
		if ((pdid->vendor == vid) && (pdid->device == pid)
			   ...
			   const struct pci_device_id *pdid)
{
	...
	if (status == _FAIL) {
		RT_TRACE(_module_hci_intfs_c_, _drv_err_,
			 ("Initialize PCI desc ring Failed!\n"));
		goto free_drv_sw;
	}
