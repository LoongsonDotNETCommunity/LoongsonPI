...
#else
	timer_setup(&precvpriv->signal_stat_timer, RTW_TIMER_HDL_NAME(signal_stat),
		    0);
#endif
#elif defined(PLATFORM_OS_CE) || defined(PLATFORM_WINDOWS)
	_init_timer(&precvpriv->signal_stat_timer, padapter->hndis_adapter,
		    RTW_TIMER_HDL_NAME(signal_stat), padapter);
#endif
	...
	if (prxattrib->encrypt == _TKIP_) {
		...
		RT_TRACE(_module_rtl871x_recv_c_, _drv_info_,
			 ("\n recvframe_chkmic:da=0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x\n",
			  prxattrib->ra[0], prxattrib->ra[1], prxattrib->ra[2], prxattrib->ra[3],
		...
		if (stainfo != NULL) {
			...
			if (bmic_err == _TRUE) {
				...
				RT_TRACE(_module_rtl871x_recv_c_, _drv_err_,
					  ...
					  *(pframemic - 8), *(pframemic - 7), *(pframemic - 6), *(pframemic - 5),
					  *(pframemic - 4), *(pframemic - 3), *(pframemic - 2), *(pframemic - 1)));
				RT_TRACE(_module_rtl871x_recv_c_, _drv_err_,
					 ("\n *(pframemic-16)-*(pframemic-9)=0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x\n",
					  *(pframemic - 16), *(pframemic - 15), *(pframemic - 14), *(pframemic - 13),
					  *(pframemic - 12), *(pframemic - 11), *(pframemic - 10),
					  *(pframemic - 9)));
...
#ifdef CONFIG_CONCURRENT_MODE
		if (!IS_MCAST(
			    prxattrib->ra)) //bc/mc packets use sw decryption for concurrent mode
#endif
	...
	struct sta_info **psta)
{
	...
	if ((check_fwstate(pmlmepriv, WIFI_STATION_STATE) == _TRUE)
		...
		if (_rtw_memcmp(pattrib->bssid, "\x0\x0\x0\x0\x0\x0", ETH_ALEN) ||
			...
			if (!bmcast) {
				DBG_871X("issue_deauth to the nonassociated ap=" MAC_FMT
					 " for the reason(7)\n", MAC_ARG(pattrib->bssid));
...

	{
		...
		if (psta) {
			...
			if (GetFrameSubType(precv_frame->u.hdr.rx_data) == WIFI_BEACON)
				...
				if (_rtw_memcmp(padapter->eeprompriv.mac_addr,
			      ...
			      union recv_frame *precv_frame)
{
	...
	if (recv_decache(precv_frame, bretry,
...
sint validate_recv_frame(_adapter *adapter, union recv_frame *precv_frame)
{
	...
	if (pmlmeext->sitesurvey_res.state == SCAN_PROCESS) {
		int ch_set_idx = rtw_ch_set_search_ch(pmlmeext->channel_set,
...
int amsdu_to_msdu(_adapter *padapter, union recv_frame *prframe)
{
	...
	while (a_len > ETH_HLEN) {
		...
		sub_m = m_devget(pdata, nSubframe_Length + ETH_HLEN, ETHER_ALIGN,
				 padapter->pifp, NULL);
	...
	for (i = 0; i < nr_subframes; i++) {
		...
		if (sub_m) {
			...
			if (((u32)(mtod(sub_m, caddr_t) + 14) % 4) != 0)
				printf("%s()-%d: mtod(sub_m) = %p\n", __FUNCTION__, __LINE__, mtod(sub_m,
						caddr_t));
...
exit:
	...
	do {
		...
		if (eth_type == 0x0800) { //ip
			...
			VlanPriInfo.TagHeader.CanonicalFormatId = 0; // Should be zero.
			VlanPriInfo.TagHeader.Reserved = 0; // Should be zero.
			NDIS_PER_PACKET_INFO_FROM_PACKET(pnrframe->u.hdr.pkt,
							 Ieee8021QInfo) = VlanPriInfo.Value;
		...
		if (a_len > ETH_HLEN) {
			...
			if (pnrframe_new) {
				...
				pnrframe_new->u.hdr.rx_head = pnrframe_new->u.hdr.rx_data =
								      pnrframe_new->u.hdr.rx_tail = pdata;
...
int recv_indicatepkt_reorder(_adapter *padapter,
			     union recv_frame *prframe);
...
int process_recv_indicatepkts(_adapter *padapter,
			      union recv_frame *prframe);
int process_recv_indicatepkts(_adapter *padapter,
			      union recv_frame *prframe)
{
...
#endif //CONFIG_TDLS
	{
		...
		if (recv_indicatepkt_reorder(padapter,
					     prframe) != _SUCCESS) { // including perform A-MPDU Rx Ordering Buffer Control
#ifdef DBG_RX_DROP_FRAME
...
int recv_func_prehandle(_adapter *padapter, union recv_frame *rframe)
{
	...
	if ((check_fwstate(pmlmepriv,
			   WIFI_MP_STATE) == _TRUE)) { //&&(padapter->mppriv.check_mp_pkt == 0))
