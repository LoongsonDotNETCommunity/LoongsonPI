...
u32 build_beacon_wfd_ie(struct wifidirect_info *pwdinfo, u8 *pbuf)
{
	...
	if (P2P_ROLE_GO == pwdinfo->role) {
		if (is_any_client_associated(pwdinfo->padapter)) {
			...
			RTW_PUT_BE16(wfdie + wfdielen,
				     pwfd_info->wfd_device_type | WFD_DEVINFO_WSD);
		} else {
		...
		if (P2P_ROLE_GO == pwdinfo->role) {
			if (is_any_client_associated(pwdinfo->padapter)) {
				if (pwdinfo->wfd_tdls_enable) {
					...
					RTW_PUT_BE16(wfdie + wfdielen,
						     pwfd_info->wfd_device_type | WFD_DEVINFO_WSD | WFD_DEVINFO_PC_TDLS |
						     WFD_DEVINFO_HDCP_SUPPORT);
				} else {
					//	WiFi Direct mode + WSD ( WFD Service Discovery )
					RTW_PUT_BE16(wfdie + wfdielen,
						     pwfd_info->wfd_device_type | WFD_DEVINFO_WSD | WFD_DEVINFO_HDCP_SUPPORT);
				}
			} else {
				if (pwdinfo->wfd_tdls_enable) {
...
#ifdef CONFIG_TDLS
	if ((tunneled == 0)
	    && (padapter->pbuddy_adapter->wdinfo.wfd_tdls_enable == 1)) {
		...
		_rtw_memcpy(wfdie + wfdielen,
			    &padapter->pbuddy_adapter->eeprompriv.mac_addr[ 0 ], ETH_ALEN);
...
u32 build_nego_req_wfd_ie(struct wifidirect_info *pwdinfo, u8 *pbuf)
{
	...
	RTW_PUT_BE16(wfdie + wfdielen,
		     pwfd_info->wfd_device_type | WFD_DEVINFO_WSD | WFD_DEVINFO_SESSION_AVAIL);
	wfdielen += 2;
			     ...
			     uint len)
{
	...
	if (rtw_p2p_chk_role(pwdinfo, P2P_ROLE_DEVICE)
		...
		if ((p2pie = rtw_get_p2p_ie(pframe + WLAN_HDR_A3_LEN +
					    _PROBEREQ_IE_OFFSET_, len - WLAN_HDR_A3_LEN - _PROBEREQ_IE_OFFSET_, NULL,
					    &p2pielen))) {
			if ((p != NULL)
...
u8 rtw_p2p_ch_inclusion(struct mlme_ext_priv *pmlmeext, u8 *peer_ch_list,
				    ...
				    u8 *pframe, uint len)
{
	...
	if ((wpsie = rtw_get_wps_ie(pframe + _PUBLIC_ACTION_IE_OFFSET_,
				    len - _PUBLIC_ACTION_IE_OFFSET_, NULL, &wps_ielen))) {
	...
	while (p2p_ie) {
		...
		attr_contentlen = 0;
		if (rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_INTENTED_IF_ADDR,
		...
		if (rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_CH_LIST,
			...
			peer_ch_num = rtw_p2p_get_peer_ch_list(pwdinfo, ch_content, ch_cnt,
							       peer_ch_list);
			ch_num_inclusioned = rtw_p2p_ch_inclusion(&padapter->mlmeextpriv,
					     peer_ch_list, peer_ch_num, ch_list_inclusioned);
	...
	if (rtw_get_wfd_ie(pframe + _PUBLIC_ACTION_IE_OFFSET_,
			   len - _PUBLIC_ACTION_IE_OFFSET_, wfd_ie, &wfd_ielen)) {
	...
	while (p2p_ie) {	//	Found the P2P IE.
		...
		if (rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_GROUP_ID, groupid,
			...
			DBG_871X("[%s] Ssid = %s, ssidlen = %d\n", __FUNCTION__,
				 &groupid[ETH_ALEN], (u32)strlen(&groupid[ETH_ALEN]));
			_rtw_memcpy(pwdinfo->groupid_info.go_device_addr, &groupid[0], ETH_ALEN);
			_rtw_memcpy(pwdinfo->groupid_info.ssid, &groupid[6],
				    attr_contentlen - ETH_ALEN);
		}
			...
			if (rtw_p2p_chk_state(pwdinfo, P2P_STATE_IDLE)) {
				...
				if (pbuddy_mlmeext->cur_channel != pwdinfo->listen_channel) {
					set_channel_bwmode(padapter, pbuddy_mlmeext->cur_channel,
							   pbuddy_mlmeext->cur_ch_offset, pbuddy_mlmeext->cur_bwmode);
					val8 = 0;
					padapter->HalFunc.SetHwRegHandler(padapter, HW_VAR_MLME_SITESURVEY,
									  (u8 *)(&val8));
...
static void ro_ch_handler(_adapter *padapter)
{
	...
	} else if (wdev_to_priv(padapter->rtw_wdev)->p2p_enabled
				   ...
				   const u8 *frame_body, u32 len, u8 ch)
{
	...
	while (p2p_ie) {
		...
		attr_contentlen = 0;
		pattr = NULL;
		if ((pattr = rtw_get_p2p_attr_content(p2p_ie, p2p_ielen,
						      P2P_ATTR_OPERATING_CH, NULL, (uint *)&attr_contentlen)) != NULL) {
			*(pattr + 4) = ch;
		}
				     ...
				     const u8 *frame_body, u32 len, u8 ch)
{
	...
	while (p2p_ie) {
		...
		if ((pattr = rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_CH_LIST,
						      NULL, (uint *)&attr_contentlen)) != NULL) {
			int i;
			u32 num_of_ch;
	...
	while (p2p_ie) {
		...
		if ((pattr = rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_CH_LIST,
			...
			while (attr_contentlen > 0) {
				...
				for (i = 0; i < num_of_ch; i++)
					*(pattr_temp + 2 + i) =
						pbuddy_mlmeext->cur_channel; //forcing to the same channel
		...
		if (action == ACT_PUBLIC_VENDOR
			...
			case P2P_GO_NEGO_REQ: {
				...
				if ((cont = rtw_get_p2p_attr_content(p2p_ie, p2p_ielen,
					...
					op_ch = *(cont + 4);
				if ((cont = rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_LISTEN_CH,
								     NULL, &cont_len)))
					listen_ch = *(cont + 4);
				if ((cont = rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_GO_INTENT,
								     NULL, &cont_len)))
					intent = *cont;
				...
				_rtw_memcpy(nego_info->peer_mac, tx ? GetAddr1Ptr(buf) : GetAddr2Ptr(buf),
					    ETH_ALEN);
			...
			case P2P_GO_NEGO_RESP: {
				...
				if ((cont = rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_GO_INTENT,
								     NULL, &cont_len)))
					intent = *cont;
				if ((cont = rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_STATUS,
								     NULL, &cont_len)))
					status = *cont;

				if (nego_info->token == dialogToken && nego_info->state == 0
				    && _rtw_memcmp(nego_info->peer_mac,
						   tx ? GetAddr1Ptr(buf) : GetAddr2Ptr(buf), ETH_ALEN) == _TRUE
				   ) {
			...
			case P2P_INVIT_REQ: {
				...
				_rtw_memcpy(invit_info->peer_mac, tx ? GetAddr1Ptr(buf) : GetAddr2Ptr(buf),
					    ETH_ALEN);
...
void rtw_init_cfg80211_wifidirect_info(_adapter	*padapter)
{
	...
	_init_timer(&pcfg80211_wdinfo->remain_on_ch_timer, padapter->pnetdev,
		...
		if (padapter->pwrctrlpriv.bFwCurrentInPSMode == _TRUE) {
			if (pwrpriv->smart_ps == 0) {
				...
				rtw_hal_set_hwreg(padapter, HW_VAR_H2C_FW_PWRMODE,
						  (u8 *)(&(padapter->pwrctrlpriv.pwr_mode)));
			}
		}
		break;
	case P2P_PS_ENABLE:
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 15, 0)
	_init_timer(&pwdinfo->find_phase_timer, padapter->pnetdev,
		    find_phase_timer_process, padapter);
	_init_timer(&pwdinfo->restore_p2p_state_timer, padapter->pnetdev,
		    restore_p2p_state_timer_process, padapter);
	_init_timer(&pwdinfo->pre_tx_scan_timer, padapter->pnetdev,
		    pre_tx_scan_timer_process, padapter);
	_init_timer(&pwdinfo->reset_ch_sitesurvey, padapter->pnetdev,
		    reset_ch_sitesurvey_timer_process, padapter);
	_init_timer(&pwdinfo->reset_ch_sitesurvey2, padapter->pnetdev,
		    reset_ch_sitesurvey_timer_process2, padapter);
#ifdef CONFIG_CONCURRENT_MODE
	_init_timer(&pwdinfo->ap_p2p_switch_timer, padapter->pnetdev,
...
void init_wifidirect_info(_adapter *padapter, enum P2P_ROLE role)
{
	...
	if ((check_buddy_fwstate(padapter, _FW_LINKED) == _TRUE) &&
	    ((pbuddy_mlmeext->cur_channel == 1) || (pbuddy_mlmeext->cur_channel == 6)
