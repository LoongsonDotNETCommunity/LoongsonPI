			   ...
			   RT_CHANNEL_INFO *channel_set)
{
	...
	if (b2_4GBand) {
		for (index = 0; index < RTW_ChannelPlan2G[Index2G].Len; index++) {
			...
			if (RT_CHANNEL_DOMAIN_GLOBAL_DOAMIN ==
			    ChannelPlan) { //Channel 1~11 is active, and 12~14 is passive
		...
		for (index = 0; index < RTW_ChannelPlan5G[Index5G].Len; index++) {
			...
			if (channel_set[chanset_size].ChannelNum <= 48
				...
				if (RT_CHANNEL_DOMAIN_WORLD_WIDE_5G ==
				    ChannelPlan) //passive scan for all 5G channels
					channel_set[chanset_size].ScanType = SCAN_PASSIVE;
				else
					channel_set[chanset_size].ScanType = SCAN_ACTIVE;
			} else {
			...
			if (RTW_ChannelPlan5G[Index5G].Channel[index] <= 48
				...
				DBG_871X("%s(): channel_set[%d].ChannelNum = %d\n", __FUNCTION__,
					 chanset_size, channel_set[chanset_size].ChannelNum);
				chanset_size++;
			}
#endif /* CONFIG_DFS */
		}
...
int	init_mlme_ext_priv(_adapter *padapter)
{
	...
	pmlmeext->max_chan_nums = init_channel_set(padapter,
				  pmlmepriv->ChannelPlan, pmlmeext->channel_set);
	init_channel_list(padapter, pmlmeext->channel_set, pmlmeext->max_chan_nums,
			    ...
			    struct mlme_handler *ptable, union recv_frame *precv_frame)
{
	...
	if (ptable->func) {
		...
		if (!_rtw_memcmp(GetAddr1Ptr(pframe), myid(&padapter->eeprompriv),
				 ETH_ALEN) &&
...
unsigned int OnProbeReq(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if (!rtw_p2p_chk_state(pwdinfo, P2P_STATE_NONE) &&
		...
		if (wifi_test_chk_rate == 1) {
			if ((is_valid_p2p_probereq = process_probe_req_p2p_ie(pwdinfo, pframe,
						     len)) == _TRUE) {
...
_continue:
	...
	if (check_fwstate(pmlmepriv, _FW_LINKED) == _FALSE &&
	    check_fwstate(pmlmepriv,
			  WIFI_ADHOC_MASTER_STATE | WIFI_AP_STATE) == _FALSE) {
		return _SUCCESS;
	}
...
unsigned int OnProbeRsp(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if (rtw_p2p_chk_state(pwdinfo, P2P_STATE_TX_PROVISION_DIS_REQ)) {
		if (_TRUE == pwdinfo->tx_prov_disc_info.benable) {
			if (_rtw_memcmp(pwdinfo->tx_prov_disc_info.peerIFAddr, GetAddr2Ptr(pframe),
					ETH_ALEN)) {
		...
		if (_TRUE == pwdinfo->nego_req_info.benable) {
			...
			if (_rtw_memcmp(pwdinfo->nego_req_info.peerDevAddr, GetAddr2Ptr(pframe),
					ETH_ALEN)) {
		...
		if (_TRUE == pwdinfo->invitereq_info.benable) {
			...
			if (_rtw_memcmp(pwdinfo->invitereq_info.peer_macaddr, GetAddr2Ptr(pframe),
					ETH_ALEN)) {
	...
	if (_rtw_memcmp(GetAddr3Ptr(pframe), get_my_bssid(&pmlmeinfo->network),
		...
		if (((pmlmeinfo->state & 0x03) == WIFI_FW_STATION_STATE)
			...
			if ((psta = rtw_get_stainfo(pstapriv, GetAddr2Ptr(pframe))) != NULL) {
				...
				//get supported rate
				if (update_sta_support_rate(padapter,
							    (pframe + WLAN_HDR_A3_LEN + _BEACON_IE_OFFSET_),
							    (len - WLAN_HDR_A3_LEN - _BEACON_IE_OFFSET_), cam_idx) == _FAIL) {
...
unsigned int OnAuth(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if (GetPrivacy(pframe)) {
		...
		if (SWCRYPTO) {
			...
			if (status == FALSE) {
				...
				RT_TRACE(_module_rtl871x_mlme_c_, _drv_err_,
					 "wep-decrypt a Auth frame error!\n");
				status = _STATS_CHALLENGE_FAIL_;
				goto auth_fail;
			}
		}

		seq = cpu_to_le16(*(unsigned short *)((unsigned int)pframe +
						      WLAN_HDR_A3_LEN + 4 + 2));
		algorithm = cpu_to_le16(*(unsigned short *)((unsigned int)pframe +
					WLAN_HDR_A3_LEN + 4));
#endif
	}
	...
	//check sa
	if (acl_mode ==
	    1)		// 1: positive check, only those on acl_list can be connected.
		res = FAIL;
	else
		res = SUCCESS;
			  ...
			  union recv_frame *precv_frame)
{
	...
	seq 	= le16_to_cpu(*(unsigned short *)((SIZE_PTR)pframe + WLAN_HDR_A3_LEN +
			      offset + 2));
	status 	= le16_to_cpu(*(unsigned short *)((SIZE_PTR)pframe +
...
unsigned int OnAssocReq(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	p = rtw_get_ie(pframe + WLAN_HDR_A3_LEN + ie_offset, _SUPPORTEDRATES_IE_,
	...
	if (wpa_ie == NULL) {
		...
		if (elems.wps_ie) {
			...
			copy_len = ((wpa_ie_len + 2) > sizeof(pstat->wpa_ie)) ? (sizeof(
						pstat->wpa_ie)) : (wpa_ie_len + 2);
		}
	...
	if (elems.ht_capabilities
		...
		_rtw_memcpy(&pstat->htpriv.ht_cap, elems.ht_capabilities,
	...
	if (rtw_p2p_chk_role(pwdinfo, P2P_ROLE_GO)) {
		if ((p2pie = rtw_get_p2p_ie(pframe + WLAN_HDR_A3_LEN + ie_offset,
					    pkt_len - WLAN_HDR_A3_LEN - ie_offset, NULL, &p2pielen))) {
			pstat->is_p2p_device = _TRUE;
			if ((p2p_status_code = (u8)process_assoc_req_p2p_ie(pwdinfo, pframe,
					       pkt_len, pstat)) > 0) {
...
#ifdef CONFIG_WFD
		if (rtw_get_wfd_ie(pframe + WLAN_HDR_A3_LEN + ie_offset,
				   pkt_len - WLAN_HDR_A3_LEN - ie_offset, wfd_ie, &wfd_ielen)) {
			u8	attr_content[ 10 ] = { 0x00 };
			u32	attr_contentlen = 0;

			DBG_8192C("[%s] WFD IE Found!!\n", __FUNCTION__);
			rtw_get_wfd_attr_content(wfd_ie, wfd_ielen, WFD_ATTR_DEVICE_INFO,
...
unsigned int OnAssocRsp(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	//status
	if ((status = le16_to_cpu(*(unsigned short *)(pframe + WLAN_HDR_A3_LEN +
				  2))) > 0) {
...
unsigned int OnDeAuth(_adapter *padapter, union recv_frame *precv_frame)
{
...
#endif
	{
		...
		DBG_871X("%s, STA:" MAC_FMT ", ignore = %d\n", __FUNCTION__,
			 MAC_ARG(GetAddr3Ptr(pframe)), ignore_received_deauth);
			   ...
			   union recv_frame *precv_frame)
{
	...
	if (!_rtw_memcmp(myid(&(padapter->eeprompriv)), GetAddr1Ptr(pframe),
	...
	if (category == RTW_WLAN_CATEGORY_BACK) { // representing Block Ack
		...
		case RTW_WLAN_ACTION_ADDBA_REQ: //ADDBA request
			...
			if (pmlmeinfo->bAcceptAddbaReq == _TRUE) {
				...
				issue_action_BA(padapter, addr, RTW_WLAN_ACTION_ADDBA_RESP,
						37);//reject ADDBA Req
			}
...
static void get_channel_cnt_24g_5gl_5gh(struct mlme_ext_priv *pmlmeext,
					u8 *p24g_cnt, u8 *p5gl_cnt, u8 *p5gh_cnt)
{
	int	i = 0;
...
void issue_p2p_invitation_request(_adapter *padapter, u8 *raddr)
{
	...
	*(u16 *)(p2pie + p2pielen) = cpu_to_le16(0x0005);
	...
	if (_rtw_memcmp(myid(&padapter->eeprompriv),
...
void issue_probersp_p2p(_adapter *padapter, unsigned char *da)
{
	...
	*(fctrl) = 0;
...
#endif //CONFIG_IOCTL_CFG80211
	{
		...
		//WFD IE
		_rtw_memcpy(pframe, pmlmepriv->wfd_probe_resp_ie,
...
int _issue_probereq_p2p(_adapter *padapter, u8 *da, int wait_ack)
{
	...
	*(fctrl) = 0;
...
#endif
	{
		...
		//WFD IE
		_rtw_memcpy(pframe, pmlmepriv->wfd_probe_req_ie,
...
#endif //CONFIG_IOCTL_CFG80211
	{
		...
		case P2P_GO_NEGO_REQ: {
			...
			if (_rtw_memcmp(pwdinfo->rx_prov_disc_info.peerDevAddr, empty_addr,
					ETH_ALEN));
			_rtw_memcpy(pwdinfo->rx_prov_disc_info.peerDevAddr, GetAddr2Ptr(pframe),
				    ETH_ALEN);

			result = process_p2p_group_negotation_req(pwdinfo, frame_body, len);
			issue_p2p_GO_response(padapter, GetAddr2Ptr(pframe), frame_body, len,
					      result);
#ifdef CONFIG_INTEL_WIDI
			...
			if ((p2p_ie = rtw_get_p2p_ie(frame_body + _PUBLIC_ACTION_IE_OFFSET_,
				...
				if (attr_contentlen) {
					...
					if (invitation_flag & P2P_INVITATION_FLAGS_PERSISTENT) {
						...
						if (attr_contentlen) {
							if (_rtw_memcmp(group_id.go_device_addr, myid(&padapter->eeprompriv),
									ETH_ALEN)) {
								...
								if (is_matched_in_profilelist(pwdinfo->p2p_peer_interface_addr,
									...
									if (rtw_get_p2p_attr_content(merged_p2pie, merged_p2p_ielen,
												     P2P_ATTR_OPERATING_CH, operatingch_info, &attr_contentlen)) {
				...
				issue_p2p_invitation_response(padapter, GetAddr2Ptr(pframe),
		    ...
		    int wait_ack)
{
	...
	*(fctrl) = 0;
	...
	if (pmlmepriv->probereq_wpsie_len > 0
	    && pmlmepriv->probereq_wpsie_len < MAX_WPS_IE_LEN) {
		_rtw_memcpy(pframe, pmlmepriv->probereq_wpsie,
		   ...
		   struct sta_info *pstat, int pkt_type)
{
	...
	*(fctrl) = 0;
	...
	_rtw_memcpy((void *)GetAddr2Ptr(pwlanhdr), myid(&(padapter->eeprompriv)),
		    ETH_ALEN);
	...
	pframe = rtw_set_fixed_ie(pframe, _STATUS_CODE_, (unsigned char *)&status,
				  &(pattrib->pktlen));
...
void issue_assocreq(_adapter *padapter)
{
	...
	*(fctrl) = 0;
	...
	if (pmlmeext->cur_channel > 14) {
		...
		pframe = rtw_set_ie(pframe, _POW_CAP_IE_, 2, pow_cap_ele,
				    &(pattrib->pktlen));
		...
		do {
			if (pmlmeext->channel_set[sup_ch_idx].ChannelNum <= 14) {
				...
				sup_ch[1] =
					pmlmeext->channel_set[sup_ch_idx].ChannelNum;	//Number of channel
			} else {
		...
		pframe = rtw_set_ie(pframe, _SUPPORTED_CH_IE_, idx_5g, sup_ch,
				    &(pattrib->pktlen));
	}
#endif //CONFIG_DFS

	//RSN
	p = rtw_get_ie((pmlmeinfo->network.IEs + sizeof(NDIS_802_11_FIXED_IEs)),
		       _RSN_IE_2_, &ie_len, (pmlmeinfo->network.IELength - sizeof(
				       NDIS_802_11_FIXED_IEs)));
	if (p != NULL) {
		pframe = rtw_set_ie(pframe, _RSN_IE_2_, ie_len, (p + 2),
				    &(pattrib->pktlen));
	}
	...
	if (padapter->mlmepriv.htpriv.ht_option == _TRUE) {
		p = rtw_get_ie((pmlmeinfo->network.IEs + sizeof(NDIS_802_11_FIXED_IEs)),
			       ...
			       (pmlmeinfo->network.IELength - sizeof(NDIS_802_11_FIXED_IEs)));
		if ((p != NULL) && (!(is_ap_in_tkip(padapter)))) {
			_rtw_memcpy(&(pmlmeinfo->HT_caps), (p + 2),
			...
			default:
				...
				if ((pregpriv->rx_stbc == 0x3) || //enable for 2.4/5 GHz
				    ((pmlmeext->cur_wireless_mode & WIRELESS_11_24N)
				     && (pregpriv->rx_stbc == 0x1)) || //enable for 2.4GHz
				    ((pmlmeext->cur_wireless_mode & WIRELESS_11_5N)
				     && (pregpriv->rx_stbc == 0x2)) || //enable for 5GHz
				    (pregpriv->wifi_spec == 1)) {
					DBG_871X("declare supporting RX STBC\n");
					pmlmeinfo->HT_caps.u.HT_cap_element.HT_caps_info |= cpu_to_le16(
								0x0200);//RX STBC two spatial stream
				}
#ifdef CONFIG_DISABLE_MCS13TO15
				if (pmlmeext->cur_bwmode == HT_CHANNEL_WIDTH_40
				    && (pregpriv->wifi_spec != 1))
					_rtw_memcpy(pmlmeinfo->HT_caps.u.HT_cap_element.MCS_rate,
	...
	for (i = sizeof(NDIS_802_11_FIXED_IEs); i < pmlmeinfo->network.IELength;) {
		...
		case _VENDOR_SPECIFIC_IE_:
			if ((_rtw_memcmp(pIE->data, RTW_WPA_OUI, 4)) ||
				...
				pframe = rtw_set_ie(pframe, _VENDOR_SPECIFIC_IE_, pIE->Length, pIE->data,
						    &(pattrib->pktlen));
			}
			break;
...
#endif //CONFIG_IOCTL_CFG80211
	{
		...
		//WFD IE
		_rtw_memcpy(pframe, pmlmepriv->wfd_assoc_req_ie,
				 ...
				 u8 ch_offset)
{
	...
	*(fctrl) = 0;
	...
	_rtw_memcpy(pwlanhdr->addr2, myid(&(padapter->eeprompriv)),
		    ETH_ALEN); /* TA */
...
void issue_action_BA(_adapter *padapter, unsigned char *raddr,
	...
	*(fctrl) = 0;
	...
	if (category == 3) {
		...
		case 2://DELBA
			...
			pframe = rtw_set_fixed_ie(pframe, 2, (unsigned char *)(&(reason_code)),
						  &(pattrib->pktlen));
			break;
		default:
			break;
...
//#endif
	...
	} while (_FALSE == bxmitok && issue < 100 && !padapter->bSurpriseRemoved
...
void site_survey(_adapter *padapter)
{
	...
	if ((pwdinfo->rx_invitereq_info.scan_op_ch_only)
		...
		if (pwdinfo->rx_invitereq_info.scan_op_ch_only) {
			survey_channel =
				pwdinfo->rx_invitereq_info.operation_ch[pmlmeext->sitesurvey_res.channel_idx];
		} else {
			survey_channel =
				pwdinfo->p2p_info.operation_ch[pmlmeext->sitesurvey_res.channel_idx];
		}
...
#endif //CONFIG_P2P
	{
		...
		if (pmlmeext->sitesurvey_res.channel_idx <
			...
			ScanType = (ch->flags & RTW_IEEE80211_CHAN_PASSIVE_SCAN) ? SCAN_PASSIVE :
				   SCAN_ACTIVE;
		}
	}
	...
	if (survey_channel != 0) {
...
#ifdef CONFIG_CONCURRENT_MODE
#ifdef CONFIG_STA_MODE_SCAN_UNDER_AP_MODE
		if ((padapter->pbuddy_adapter->mlmeextpriv.mlmext_info.state & 0x03) ==
		    WIFI_FW_AP_STATE) {
		...
		if (pmlmeext->sitesurvey_res.channel_idx == 0) {
			set_channel_bwmode(padapter, survey_channel,
					   HAL_PRIME_CHNL_OFFSET_DONT_CARE, HT_CHANNEL_WIDTH_20);
		} else {
		...
		if (stay_buddy_ch == 1)
			set_survey_timer(pmlmeext,
					 pmlmeext->chan_scan_time * RTW_STAY_AP_CH_MILLISECOND);
		else
#endif //CONFIG_STA_MODE_SCAN_UNDER_AP_MODE
		...
		if (rtw_get_ch_setting_union(padapter, &cur_channel, &cur_bwmode,
					     &cur_ch_offset) != 0) {
			if (0)
		    ...
		    WLAN_BSSID_EX *bssid)
{
	...
	bssid->PhyInfo.SignalQuality =
		precv_frame->u.hdr.attrib.signal_qual;//in percentage
	bssid->PhyInfo.SignalStrength =
		precv_frame->u.hdr.attrib.signal_strength;//in percentage
	...
	rtw_hal_get_def_var(padapter, HAL_DEF_CURRENT_ANTENNA,
			    &bssid->PhyInfo.Optimum_antenna);
#endif

	// checking SSID
	if ((p = rtw_get_ie(bssid->IEs + _FIXED_IE_LENGTH_, _SSID_IE_, &len,
			    bssid->IELength - _FIXED_IE_LENGTH_)) == NULL) {
	...
	//checking rate info...
	i = 0;
	p = rtw_get_ie(bssid->IEs + _FIXED_IE_LENGTH_, _SUPPORTEDRATES_IE_, &len,
	...
	if (p) {
		...
		p = rtw_get_ie(bssid->IEs + _FIXED_IE_LENGTH_, _HT_ADD_INFO_IE_, &len,
			       bssid->IELength - _FIXED_IE_LENGTH_);
		if (p) {
	...
	if ((pregistrypriv->wifi_spec == 1)
		...
		p = rtw_get_ie(bssid->IEs + _FIXED_IE_LENGTH_, _HT_CAPABILITY_IE_, &len,
			       bssid->IELength - _FIXED_IE_LENGTH_);
		if (p && len > 0) {
...
//				chplan_new[k].ScanType = chplan_sta[i].ScanType;
		...
		while ((k < MAX_CHANNEL_NUM) && (chplan_new[k].ChannelNum != 0)) {
			printk("%02d(%c),", chplan_new[k].ChannelNum,
...
void report_survey_event(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if ((pcmd_obj = (struct cmd_obj *)rtw_zmalloc(sizeof(
				struct cmd_obj))) == NULL) {
		return;
	}
...
void mlmeext_joinbss_event_callback(_adapter *padapter, int join_res)
{
	...
	//turn on dynamic functions
	Switch_DM_Func(padapter,
		       DYNAMIC_FUNC_DIG | DYNAMIC_FUNC_HP | DYNAMIC_FUNC_SS, _TRUE);
...
void _linked_rx_signal_strehgth_display(_adapter *padapter)
{
	...
	DBG_871X(" DIG PATH-A(0x%02x), PATH-B(0x%02x)\n", rtw_read8(padapter,
			0xc50), rtw_read8(padapter, 0xc58));
	DBG_871X(" OFDM -Alarm DA2(0x%04x),DA4(0x%04x),DA6(0x%04x),DA8(0x%04x)\n",
		 rtw_read16(padapter, 0xDA2), rtw_read16(padapter, 0xDA4),
		 rtw_read16(padapter, 0xDA6), rtw_read16(padapter, 0xDA8));

	DBG_871X(" CCK -Alarm A5B(0x%02x),A5C(0x%02x)\n", rtw_read8(padapter,
			0xA5B), rtw_read8(padapter, 0xA5C));
...
void linked_status_chk(_adapter *padapter)
{
	...
	if (is_client_associated_to_ap(padapter)) {
		...
		if ((psta = rtw_get_stainfo(pstapriv,
...
void survey_timer_hdl(_adapter *padapter)
{
	...
	if (pmlmeext->sitesurvey_res.state > SCAN_START) {
		...
		if ((ph2c = (struct cmd_obj *)rtw_zmalloc(sizeof(struct cmd_obj))) ==
		    NULL) {
			goto exit_survey_timer_hdl;
		}

		if ((psurveyPara = (struct sitesurvey_parm *)rtw_zmalloc(sizeof(
					   struct sitesurvey_parm))) == NULL) {
...
int rtw_scan_ch_decision(_adapter *padapter,
			 ...
			 u32 out_num, struct rtw_ieee80211_channel *in, u32 in_num)
{
	...
	if (j == 0) {
		for (i = 0; i < pmlmeext->max_chan_nums; i++) {
			...
			if (rtw_mlme_band_check(padapter,
						pmlmeext->channel_set[i].ChannelNum) == _TRUE) {
	...
	if (pmlmeext->sitesurvey_res.state == SCAN_DISABLE) {
		...
		for (i = 0; i < RTW_SSID_SCAN_AMOUNT; i++) {
			if (pparm->ssid[i].SsidLength) {
				_rtw_memcpy(pmlmeext->sitesurvey_res.ssid[i].Ssid, pparm->ssid[i].Ssid,
					    IW_ESSID_MAX_SIZE);
...
u8 set_tx_beacon_cmd(_adapter *padapter)
{
	...
	if ((ptxBeacon_parm = (struct Tx_Beacon_param *)rtw_zmalloc(sizeof(
				      struct Tx_Beacon_param))) == NULL) {
		rtw_mfree((unsigned char *)ph2c, sizeof(struct	cmd_obj));
		res = _FAIL;
		goto exit;
	}
...
void dc_SetBWMode(_adapter *padapter, unsigned short bwmode,
		...
		if (((pbuddy_mlmeinfo->state & 0x03) == WIFI_FW_AP_STATE) &&
			...
			if (join_res >= 0) {
				...
				if (pbuddy_mlmeext->cur_bwmode == HT_CHANNEL_WIDTH_40) {
					p = rtw_get_ie((pbuddy_network_mlmeext->IEs + sizeof(
								NDIS_802_11_FIXED_IEs)), _HT_ADD_INFO_IE_, &ie_len,
						       (pbuddy_network_mlmeext->IELength - sizeof(NDIS_802_11_FIXED_IEs)));
					if (p && ie_len) {
				...
				p = rtw_get_ie((pbuddy_network_mlmeext->IEs + sizeof(
							NDIS_802_11_FIXED_IEs)), _DSSET_IE_, &ie_len,
					       (pbuddy_network_mlmeext->IELength - sizeof(NDIS_802_11_FIXED_IEs)));
				if (p && ie_len > 0)
				...
				_rtw_memcpy(&pbuddy_mlmepriv->cur_network.network, pbuddy_network_mlmeext,
...
void	dc_report_survey_event(_adapter *padapter,
			       union recv_frame *precv_frame)
{
	if (padapter->pbuddy_adapter != NULL &&
	    padapter->DualMacConcurrent == _TRUE) {
...
void dc_set_ap_channel_bandwidth(_adapter *padapter, u8 channel,
				 u8 channel_offset, u8 bwmode)
{
	...
	struct registry_priv	*pregpriv = &padapter->registrypriv;
	struct mlme_priv		*pmlmepriv = &(padapter->mlmepriv);
	WLAN_BSSID_EX		*pnetwork = (WLAN_BSSID_EX *)
	...
	p = rtw_get_ie((pnetwork->IEs + sizeof(NDIS_802_11_FIXED_IEs)),
	...
	if (pbuddy_adapter != NULL &&
		...
		if (!check_fwstate(pbuddy_mlmepriv,
			...
			p = rtw_get_ie((pnetwork->IEs + sizeof(NDIS_802_11_FIXED_IEs)), _DSSET_IE_,
				       &ie_len, (pnetwork->IELength - sizeof(NDIS_802_11_FIXED_IEs)));
			if (p && ie_len > 0)
				*(p + 2) = cur_channel;
...
int rtw_chk_start_clnt_join(_adapter *padapter)
{
	...
	if ((pbuddy_pmlmeinfo->state & 0x03) == WIFI_FW_AP_STATE) { //for AP MODE
		DBG_871X("start_clnt_join: "ADPT_FMT"(ch=%d, bw=%d, ch_offset=%d)"
			 ...
			 ADPT_ARG(padapter), pmlmeext->cur_channel, pmlmeext->cur_bwmode,
			 pmlmeext->cur_ch_offset,
			 ADPT_ARG(pbuddy_adapter), pbuddy_mlmeext->cur_channel,
			 pbuddy_mlmeext->cur_bwmode, pbuddy_mlmeext->cur_ch_offset);
		...
		if (chbw_allow == _FALSE) {
#ifdef CONFIG_SPCT_CH_SWITCH
			if (1) {
				rtw_ap_inform_ch_switch(pbuddy_adapter, pmlmeext->cur_channel,
							pmlmeext->cur_ch_offset);
			} else
#endif
			{
		   ...
		   check_fwstate(pbuddy_mlmepriv,
				 WIFI_STATION_STATE) == _TRUE) { //for Client Mode/p2p client
		DBG_871X("start_clnt_join: "ADPT_FMT"(ch=%d, bw=%d, ch_offset=%d)"
...
int rtw_get_ch_setting_union(_adapter *adapter, u8 *ch, u8 *bw, u8 *offset)
{
	...
	for (i = 0; i < dvobj->iface_nums; i++) {
		...
		if (bw_ret < mlmeext->cur_bwmode) {
			bw_ret = mlmeext->cur_bwmode;
			offset_ret = mlmeext->cur_ch_offset;
		} else if (bw_ret == mlmeext->cur_bwmode
			   && offset_ret != mlmeext->cur_ch_offset) {
			num = 0;
			break;
		}
...
u8 set_csa_hdl(_adapter *padapter, unsigned char *pbuf)
{
	...
	if (((new_ch_no >= 52) && (new_ch_no <= 64)) || ((new_ch_no >= 100)
			&& (new_ch_no <= 140))) {
...
u8 tdls_hdl(_adapter *padapter, unsigned char *pbuf)
{
	...
	case TDLS_DONE_CH_SEN:
		survey_channel =
			pmlmeext->channel_set[pmlmeext->sitesurvey_res.channel_idx].ChannelNum;
		if (survey_channel) {
			SelectChannel(padapter, survey_channel);
