...
void dump_txrpt_ccx_8192c(void *buf)
{
	...
	DBG_871X("%s:\n"
		 ...
		 txrpt_ccx->rsvd_1
		 , txrpt_ccx_qtime_8192c(txrpt_ccx), txrpt_ccx->missed_pkt_num,
		 txrpt_ccx->rsvd_4
		 , txrpt_ccx->mac_id, txrpt_ccx->des1_fragssn
		 , txrpt_ccx->rpt_pkt_num, txrpt_ccx->pkt_drop, txrpt_ccx->lifetime_over,
		 txrpt_ccx->retry_over
		 , txrpt_ccx->edca_tx_queue, txrpt_ccx->rsvd_7, txrpt_ccx->bmc,
		 txrpt_ccx->pkt_ok, txrpt_ccx->int_ccx
		);
}
