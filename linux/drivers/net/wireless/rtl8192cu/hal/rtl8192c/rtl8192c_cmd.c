...
*
******************************************/
int rtl8192c_FillH2CCmd(_adapter *padapter, u8 ElementID, u32 CmdLen,
			u8 *pCmdBuffer)
{
	u8	bcmd_down = _FALSE;
	int 	retry_cnts = 100;
	...
	do {
		...
		if (CmdLen <= 3) {
			...
			_rtw_memcpy((u8 *)(&h2c_cmd) + 1, pCmdBuffer + 2,
				    (CmdLen - EX_MESSAGE_BOX_SIZE));
			*(u8 *)(&h2c_cmd) |= BIT(7);
		}
...
u8 rtl8192c_set_FwSelectSuspend_cmd(_adapter *padapter, u8 bfwpoll,
				    u16 period)
{
	u8	res = _SUCCESS;
	...
	rtl8192c_FillH2CCmd(padapter, SELECTIVE_SUSPEND_ROF_CMD, sizeof(param),
			    (u8 *)(&param));
	return res;
}
...
void rtl8192c_set_FwPwrMode_cmd(_adapter *padapter, u8 Mode)
{
	...
	rtl8192c_FillH2CCmd(padapter, SET_PWRMODE_EID, sizeof(H2CSetPwrMode),
			    (u8 *)&H2CSetPwrMode);
...
void ConstructNullFunctionData(_adapter *padapter, u8 *pframe,
			       u32 *pLength, u8 *StaAddr, BOOLEAN bForcePowerSave)
{
	struct rtw_ieee80211_hdr	*pwlanhdr;
	u16					*fctrl;
	u32					pktlen;
...
//
static VOID FillFakeTxDescriptor92C(
	IN PADAPTER		Adapter,
	IN u8			*pDesc,
	IN u32			BufferLen,
	IN BOOLEAN		IsPsPoll
)
{
	...
	//offset 0
	ptxdesc->txdw0 |= cpu_to_le32(OWN | FSG |
				      LSG);  //own, bFirstSeg, bLastSeg;

	ptxdesc->txdw0 |= cpu_to_le32(((TXDESC_SIZE + OFFSET_SZ) << OFFSET_SHT) &
				      0x00ff0000); //32 bytes for TX Desc

	ptxdesc->txdw0 |= cpu_to_le32(BufferLen &
				      0x0000ffff); // Buffer size + command header

	//offset 4
	ptxdesc->txdw1 |= cpu_to_le32((QSLT_MGNT << QSEL_SHT) &
	...
	if (IsPsPoll) {
		...
		ptxdesc->txdw3 |= cpu_to_le32((8 <<
					       28)); //set bit3 to 1. Suugested by TimChen. 2009.12.29.
	}
...
//--------------------------------------------------------------------
	...
	FillFakeTxDescriptor92C(Adapter, &ReservedPagePacket[BufIndex - TxDescLen],
				PSPollLength, _TRUE);
...
//------------------------------------------------------------------
	...
	if (bDLOK) {
		...
		rtl8192c_FillH2CCmd(Adapter, RSVD_PAGE_EID, sizeof(RsvdPageLoc),
				    (u8 *)&RsvdPageLoc);
	}
...
void rtl8192c_set_FwJoinBssReport_cmd(_adapter *padapter, u8 mstatus)
{
	...
	rtl8192c_FillH2CCmd(padapter, JOINBSS_RPT_EID, sizeof(JoinBssRptParm),
			    (u8 *)&JoinBssRptParm);
...
#ifdef CONFIG_IOL
#include <rtw_iol.h>
int rtl8192c_IOL_exec_cmds_sync(ADAPTER *adapter,
				struct xmit_frame *xmit_frame, u32 max_wating_ms)
{
	...
	if (_SUCCESS != rtl8192c_FillH2CCmd(adapter, H2C_92C_IO_OFFLOAD,
					    sizeof(IO_OFFLOAD_LOC), (u8 *)&IoOffloadLoc))
		goto exit;

	//polling if the IO offloading is done
	while ((passing_time_ms = rtw_get_passing_time_ms(start_time)) <=
	       max_wating_ms) {
#if 0 //C2H
	...
	if (pwrpriv->wowlan_mode == _TRUE) {
		...
		//printk("\n %s 5.pwowlan_parm.mode=0x%x \n",__FUNCTION__,pwowlan_parm.mode );
		{
			...
			printk("\n %s H2C_WO_WLAN=%x %02x:%02x:%02x:%02x:%02x \n", __FUNCTION__,
			       H2C_WO_WLAN_CMD, ptr[0], ptr[1], ptr[2], ptr[3], ptr[4]);
		}
		...
		pwowlan_parm.mode = 3;
		pwowlan_parm.gpio_index = 3;
		rtl8192c_FillH2CCmd(padapter, KEEP_ALIVE_CONTROL_CMD, 2,
				    (u8 *)&pwowlan_parm);
		printk("%s after KEEP_ALIVE_CONTROL_CMD register 0x81=%x \n", __FUNCTION__,
		       rtw_read8(padapter, 0x81));

		pwowlan_parm.mode = 1;
		pwowlan_parm.gpio_index = 0;
		pwowlan_parm.gpio_duration = 0;
		rtl8192c_FillH2CCmd(padapter, DISCONNECT_DECISION_CTRL_CMD, 3,
				    (u8 *)&pwowlan_parm);
		...
		pwowlan_parm.gpio_index = 0;
		pwowlan_parm.gpio_duration = 0;
		rtl8192c_FillH2CCmd(padapter, REMOTE_WAKE_CTRL_CMD, 3,
				    (u8 *)&pwowlan_parm);
	} else
