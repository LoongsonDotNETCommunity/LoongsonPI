...
static int usbctrl_vendorreq(struct intf_hdl *pintfhdl, u8 request,
			     u16 value, u16 index, void *pdata, u16 len, u8 requesttype)
{
	...
	pIo_buf = (tmp_buf == NULL) ? NULL : tmp_buf + ALIGNMENT_UNIT - ((
				SIZE_PTR)(tmp_buf) & 0x0f);
#endif
	...
	while (++vendorreq_times <= MAX_USBCTRL_VENDORREQ_TIMES) {
		...
		status = rtw_usb_control_msg(udev, pipe, request, reqtype, value, index,
					     pIo_buf, len
					     , ((value >= FW_8192C_START_ADDRESS && value <= FW_8192C_END_ADDRESS)
						|| value != 0x1000) ? RTW_USB_CONTROL_MSG_TIMEOUT :
		...
		if (status == len) {  // Success this control transfer.
			...
			DBG_8192C("reg 0x%x, usb %s %u fail, status:%d value=0x%x, vendorreq_times:%d\n"
				  , value, (requesttype == 0x01) ? "read" : "write", len, status,
				  *(u32 *)pdata, vendorreq_times);
		...
		if ((value >= FW_8192C_START_ADDRESS && value <= FW_8192C_END_ADDRESS)
		    || status == len)
			break;
...
static u8 usb_read8(struct intf_hdl *pintfhdl, u32 addr)
{
	...
	usbctrl_vendorreq(pintfhdl, request, wvalue, index, &data, len,
			  requesttype);
...
static int usb_write8(struct intf_hdl *pintfhdl, u32 addr, u8 val)
{
	...
	ret = usbctrl_vendorreq(pintfhdl, request, wvalue, index, &data, len,
				requesttype);
...
static int usb_writeN(struct intf_hdl *pintfhdl, u32 addr, u32 length,
		      u8 *pdata)
{
	u8 request;
	u8 requesttype;
...
static s32 pre_recv_entry(union recv_frame *precvframe,
	...
	if (IS_MCAST(paddr1) == _FALSE) { //unicast packets
		...
		case WIFI_MGT_TYPE: //Handle BC/MC mgnt Packets
			if (subtype == WIFI_BEACON) {
				...
				if (check_fwstate(&primary_padapter->mlmepriv,
						  _FW_UNDER_SURVEY | _FW_UNDER_LINKING) ||
				    check_fwstate(&secondary_padapter->mlmepriv,
						  _FW_UNDER_SURVEY | _FW_UNDER_LINKING)) {
					clone = _TRUE;
					precvframe->u.hdr.adapter = primary_padapter;
				}
		...
		case WIFI_DATA_TYPE: //Handle BC/MC data Packets
			...
			if (_rtw_memcmp(paddr2, get_bssid(&secondary_padapter->mlmepriv),
					ETH_ALEN)) {
				//change to secondary interface
				precvframe->u.hdr.adapter = secondary_padapter;
				clone = _FALSE;
		...
		if (_TRUE == clone) {
			...
			if (precvframe_if2) {
				...
				precvframe_if2->u.hdr.precvbuf =
					NULL;	//can't access the precvbuf for new arch.
				precvframe_if2->u.hdr.len = 0;

				_rtw_memcpy(&precvframe_if2->u.hdr.attrib, &precvframe->u.hdr.attrib,
...
static int recvbuf2recvframe(_adapter *padapter, struct recv_buf *precvbuf)
{
	...
	if ((pkt_cnt > 10) || (pkt_cnt < 1) || (transfer_len < RXDESC_SIZE)
	    || (pkt_len <= 0)) {
		return _FAIL;
	}
#endif
	...
	do {
		...
		pkt_offset = RXDESC_SIZE + pattrib->drvinfo_sz + pattrib->shift_sz +
			     pattrib->pkt_len;
		...
		if (pkt_copy) {
			...
			skb_reserve(pkt_copy,
				    shift_sz);  //force ip_hdr at 8-byte alignment address according to shift_sz.
			_rtw_memcpy(pkt_copy->data,
				    (pbuf + pattrib->shift_sz + pattrib->drvinfo_sz + RXDESC_SIZE), skb_len);
...
void rtl8192cu_recv_tasklet(void *priv)
{
	...
	while (NULL != (precvbuf = rtw_dequeue_recvbuf(
...
static void usb_read_port_complete(struct urb *purb, struct pt_regs *regs)
{
	...
	if (purb->status == 0) { //SUCCESS
		...
		if (rtw_inc_and_chk_continual_urb_error(adapter_to_dvobj(
				padapter)) == _TRUE) {
			padapter->bSurpriseRemoved = _TRUE;
		}
			 ...
			 u8 *rmem)
{
	...
	if (precvbuf != NULL) {
		...
		if (precvbuf->pbuf) {
			...
			if ((err) && (err != (-EPERM))) {
				RT_TRACE(_module_hci_ops_os_c_, _drv_err_,
					 ("cannot submit rx in-token(err=0x%.8x), URB_STATUS =0x%.8x", err,
					  purb->status));
			 ...
			 u8 *rmem)
{
	...
	if ((precvbuf->reuse == _FALSE) || (precvbuf->pskb == NULL)) {
		if (NULL != (precvbuf->pskb = skb_dequeue(
