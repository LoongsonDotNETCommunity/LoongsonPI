...
static void query_rx_phy_status(union recv_frame *prframe,
				struct phy_stat *pphy_stat)
{
	...
	PHY_RX_DRIVER_INFO_8192CD *pDrvInfo = ((PHY_RX_DRIVER_INFO_8192CD *)
					       pphy_stat);
	...
	if (bcck_rate) { //CCK
		...
		//
		rx_pwr_all = (((pOfdm_buf->pwdb_all) >> 1) & 0x7f) -
			     110;//for OFDM Average RSSI
		pwdb_all = query_rx_pwr_percentage(rx_pwr_all);
	...
	if (bcck_rate) {
		...
		if (rf_rx_num != 0) {
			pattrib->signal_strength = (u8)(signal_scale_mapping(padapter,
					...
					struct phy_stat *pphy_info)
{
	...
	if ((pattrib->physt) && (pphy_info != NULL)) {
		bPacketMatchBSSID = ((!IsFrameTypeCtrl(precvframe->u.hdr.rx_data))
				     ...
				     _rtw_memcmp(get_hdr_bssid(precvframe->u.hdr.rx_data),
						 get_my_bssid(&padapter->mlmeextpriv.mlmext_info.network), ETH_ALEN));


		bPacketToSelf = bPacketMatchBSSID
				&& (_rtw_memcmp(get_da(precvframe->u.hdr.rx_data),
						myid(&padapter->eeprompriv), ETH_ALEN));

		bPacketBeacon = bPacketMatchBSSID
				&& (GetFrameSubType(precvframe->u.hdr.rx_data) ==  WIFI_BEACON);
				   ...
				   struct recv_stat *pdesc)
{
	...
	pattrib->drvinfo_sz = (u8)((le32_to_cpu(pdesc->rxdw0) >> 16) & 0xf) *
			      8;//uint 2^3 = 8 bytes
	...
	pattrib->qos = (u8)((le32_to_cpu(pdesc->rxdw0) >> 23) &
			    0x1);   // Qos data, wireless lan header length is 26
	pattrib->bdecrypted = (le32_to_cpu(pdesc->rxdw0) & BIT(27)) ? 0 : 1;

	//Offset 4
	pattrib->mfrag = (u8)((le32_to_cpu(pdesc->rxdw1) >> 27) &
			      0x1);//more fragment bit

	//Offset 8
	pattrib->frag_num = (u8)((le32_to_cpu(pdesc->rxdw2) >> 12) &
				 0xf);//fragmentation number
