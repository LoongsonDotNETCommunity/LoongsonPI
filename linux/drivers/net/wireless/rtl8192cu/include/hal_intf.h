...
struct hal_ops {
	...
	void	(*set_bwmode_handler)(PADAPTER Adapter, HT_CHANNEL_WIDTH Bandwidth,
				      u8 Offset);
	...
	u8(*GetHalDefVarHandler)(PADAPTER Adapter, HAL_DEF_VARIABLE eVariable,
				 PVOID pValue);
	u8(*SetHalDefVarHandler)(PADAPTER Adapter, HAL_DEF_VARIABLE eVariable,
				 PVOID pValue);
	...
	void	(*AntDivCompareHandler)(PADAPTER Adapter, WLAN_BSSID_EX *dst,
					WLAN_BSSID_EX *src);
#endif
	u8(*interface_ps_func)(PADAPTER Adapter, HAL_INTF_PS_FUNC efunc_id,
			       u8 *val);
	...
	void	(*write_rfreg)(PADAPTER Adapter, u32 eRFPath, u32 RegAddr,
	...
	void (*ReadEFuse)(PADAPTER Adapter, u8 efuseType, u16 _offset,
			  u16 _size_byte, u8 *pbuf, BOOLEAN bPseudoTest);
	void (*EFUSEGetEfuseDefinition)(PADAPTER pAdapter, u8 efuseType, u8 type,
					PVOID *pOut, BOOLEAN bPseudoTest);
	u16(*EfuseGetCurrentSize)(PADAPTER pAdapter, u8 efuseType,
				  BOOLEAN bPseudoTest);
	int	(*Efuse_PgPacketRead)(PADAPTER pAdapter, u8 offset, u8 *data,
				      BOOLEAN bPseudoTest);
	int	(*Efuse_PgPacketWrite)(PADAPTER pAdapter, u8 offset, u8 word_en,
				       u8 *data, BOOLEAN bPseudoTest);
	u8(*Efuse_WordEnableDataWrite)(PADAPTER pAdapter, u16 efuse_addr,
...
#ifdef CONFIG_IOL
	int (*IOL_exec_cmds_sync)(ADAPTER *adapter, struct xmit_frame *xmit_frame,
				  u32 max_wating_ms);
#endif
...
void rtw_hal_set_bwmode(_adapter *padapter, HT_CHANNEL_WIDTH Bandwidth,
			u8 Offset);
