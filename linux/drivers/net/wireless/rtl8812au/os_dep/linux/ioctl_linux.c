			    ...
			    char *start, char *stop)
{
		...
		if (!rtw_p2p_chk_state(pwdinfo, P2P_STATE_NONE)) {
			...
			if (_rtw_memcmp(pnetwork->network.Ssid.Ssid, pwdinfo->p2p_wildcard_ssid,
				...
				if (rtw_get_p2p_ie(&pnetwork->network.IEs[12],
						   pnetwork->network.IELength - 12, NULL, &p2pielen)) {
					blnGotP2PIE = _TRUE;
				}
			}
	...
	if (SCAN_RESULT_WFD_TYPE == pwdinfo->wfd_info->scan_result_type) {
		...
		if (rtw_get_wfd_ie(&pnetwork->network.IEs[12],
				   pnetwork->network.IELength - 12,  wfd_ie, &wfd_ielen)) {
	...
	//parsing VHT_CAP_IE
	p = rtw_get_ie(&pnetwork->network.IEs[12], EID_VHTCapability, &vht_ielen,
		       pnetwork->network.IELength - 12);
	if (p && vht_ielen > 0) {
		u8	mcs_map[2];
		...
		vht_data_rate = rtw_vht_data_rate(CHANNEL_WIDTH_80, short_GI,
						  vht_highest_rate);
	}
#endif
		...
		_clr_fwstate_(&padapter->mlmepriv, WIFI_UNDER_WPS);
		{
			...
			while (cnt < ielen) {
				...
				if ((eid == _VENDOR_SPECIFIC_IE_)
					...
					padapter->securitypriv.wps_ie_len = ((buf[cnt + 1] + 2) <
									     (MAX_WPA_IE_LEN << 2)) ? (buf[cnt + 1] + 2) : (MAX_WPA_IE_LEN << 2);
...
//	range->max_r_time;	/* Maximal retry lifetime */
	...
	for (i = 0, val = 0; i < MAX_CHANNEL_NUM; i++) {
		...
		if (pmlmeext->channel_set[i].ChannelNum != 0) {
			...
			range->freq[val].m = rtw_ch2freq(pmlmeext->channel_set[i].ChannelNum) *
					     100000;
			range->freq[val].e = 1;
			val++;
		}
		...
		while (1) {
			if (rtw_end_of_queue_search(phead, pmlmepriv->pscanned) == _TRUE) {
				...
				if (check_fwstate(pmlmepriv, WIFI_ADHOC_STATE) == _TRUE) {
					...
					RT_TRACE(_module_rtl871x_ioctl_os_c, _drv_info_,
						 ("rtw_wx_set_ssid(): scanned_queue is empty\n"));
					ret = -EINVAL;
					goto exit;
				}
#endif
 ...
 */
static  int rtw_drvext_hdl(struct net_device *dev,
			   struct iw_request_info *info,
			   union iwreq_data *wrqu, char *extra)
{
	...
	while (1) {
		...
		if (_rtw_memcmp(pnetwork->network.MacAddress, peerMAC, ETH_ALEN)) {
			...
			if ((wpsie = rtw_get_wps_ie(&pnetwork->network.IEs[12],
						    pnetwork->network.IELength - 12, NULL, &wpsie_len))) {
		...
		union iwreq_data *wrqu, char *extra, char *subcmd)
{
	...
	while (1) {
		...
		if (_rtw_memcmp(pnetwork->network.MacAddress, peerMAC, ETH_ALEN)) {
			...
			if ((p2pie = rtw_get_p2p_ie(&pnetwork->network.IEs[12],
						    pnetwork->network.IELength - 12, NULL, &p2pielen))) {
				while (p2pie) {
					...
					p2pie = rtw_get_p2p_ie(p2pie + p2pielen,
							       pnetwork->network.IELength - 12 - (p2pie - &pnetwork->network.IEs[12] +
									       p2pielen), NULL, &p2pielen);
				}
			}
		}
			     ...
			     union iwreq_data *wrqu, char *extra)
{
	...
	while (1) {
		...
		if ((p2pie = rtw_get_p2p_ie(&pnetwork->network.IEs[ie_offset],
					    pnetwork->network.IELength - ie_offset, NULL, &p2pielen))) {
			while (p2pie) {
				//	The P2P Device ID attribute is included in the Beacon frame.
				//	The P2P Device Info attribute is included in the probe response frame.
				...
				//Get the next P2P IE
				p2pie = rtw_get_p2p_ie(p2pie + p2pielen,
						       pnetwork->network.IELength - ie_offset - (p2pie -
								       &pnetwork->network.IEs[ie_offset] + p2pielen), NULL, &p2pielen);
			}
			       ...
			       struct ieee_param *param, int len)
{
	...
	if (ssid_ie && ssid_len) {
		...
		ssid[ssid_len > NDIS_802_11_LENGTH_SSID ? NDIS_802_11_LENGTH_SSID :
			      ssid_len] = 0x0;
			    ...
			    union iwreq_data *wdata, char *extra)
{
	...
	if (strcmp(tmp[0], "wmap") == 0) {
		...
		if (rtw_efuse_map_read(padapter, 0, EFUSE_BT_MAX_MAP_LEN,
		...
		if (rtw_efuse_map_write(padapter, 0x00, EFUSE_MAX_MAP_LEN,
...
static int rtw_mp_ctx(struct net_device *dev,
		      ...
		      struct iw_point *wrqu, char *extra)
{
	u32 pkTx = 1, countPkTx = 1, cotuTx = 1, CarrSprTx = 1, scTx = 1,
	    sgleTx = 1, stop = 1;
	u32 bStartTest = 1;
	u32 count = 0;
	...
	if (bStartRx) {
		...
		*/
		...
		sprintf(extra, "Phy Received packet OK:%d CRC error:%d FA Counter: %d",
			cckok + ofdmok + htok, cckcrc + ofdmcrc + htcrc, OFDM_FA + CCK_FA);
	}
	wrqu->length = strlen(extra) + 1;
	return 0;
}
			    ...
			    struct iw_point *wrqu, char *extra)
{
	...
	sprintf(extra, "Tx OK:%d, Tx Fail:%d, Rx OK:%d, CRC error:%d ", txok,
		txfail, rxok, rxfail);
...
//extern u8 _InitPowerOn(PADAPTER padapter);
extern s32 FillH2CCmd8723B(PADAPTER padapter, u8 ElementID, u32 CmdLen,
			   u8 *pCmdBuffer);
#endif
