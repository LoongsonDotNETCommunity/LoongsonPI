...
struct sk_buff *dbg_rtw_cfg80211_vendor_event_alloc(struct wiphy *wiphy,
		int len, int event_id, gfp_t gfp
				   ...
				   struct net_device *dev, int event_id, const void  *data, int len)
{
	...
	if (!skb) {
		DBG_871X_LEVEL(_drv_err_, FUNC_NDEV_FMT" skb alloc failed",
			       FUNC_NDEV_ARG(dev));
		return -ENOMEM;
	}
...
int rtw_dev_get_feature_set(struct net_device *dev)
{
	...
	if (IS_92D(*hal_ver) || IS_8812_SERIES(*hal_ver)
		...
		struct wireless_dev *wdev, const void  *data, int len)
{
	...
	if (unlikely(err))
		DBG_871X_LEVEL(_drv_err_,
			       FUNC_NDEV_FMT" Vendor Command reply failed ret:%d \n"
...
void wl_cfgvendor_rtt_evt(void *ctx, void *rtt_data)
{
	...
	kflags = in_atomic() ? GFP_ATOMIC : GFP_KERNEL;
	/* Alloc the SKB for vendor_event */
	skb = rtw_cfg80211_vendor_event_alloc(wiphy, tot_len,
	...
	list_for_each_entry(rtt_result, rtt_list, list) {
		...
		if (rtt_result->TOF_type == TOF_TYPE_ONE_WAY) {
			...
			rtt_report->type = (rtt_result->TOF_type == TOF_TYPE_ONE_WAY) ? RTT_ONE_WAY
					   : RTT_TWO_WAY;
		...
		struct wireless_dev *wdev, const void  *data, int len)
{
	...
	if (strncmp((char *)data, BRCM_VENDOR_SCMD_CAPA,
		    strlen(BRCM_VENDOR_SCMD_CAPA)) == 0) {
