			     ...
			     , struct cfg80211_scan_request *request)
{
	...
	if (request->n_channels == 1) {
		...
		_rtw_memcpy(&ch[survey_times], &ch[1],
			    sizeof(struct rtw_ieee80211_channel));
		for (i = 1; i < survey_times; i++) {
			_rtw_memcpy(&ch[i], &ch[0], sizeof(struct rtw_ieee80211_channel));
			_rtw_memcpy(&ch[i + survey_times], &ch[survey_times],
				    sizeof(struct rtw_ieee80211_channel));
		}
		_status = rtw_sitesurvey_cmd(padapter, ssid, RTW_SSID_SCAN_AMOUNT, ch,
					     survey_times * 2);
				...
				struct cfg80211_connect_params *sme)
{
	...
	while (1) {
		...
		if (sme->bssid)  {
			if (_rtw_memcmp(pnetwork->network.MacAddress, sme->bssid,
					ETH_ALEN) == _FALSE)
				continue;
		}
		...
		if (sme->ssid && sme->ssid_len) {
			if (pnetwork->network.Ssid.SsidLength != sme->ssid_len
			    || _rtw_memcmp(pnetwork->network.Ssid.Ssid, sme->ssid,
					   sme->ssid_len) == _FALSE
			   )
				continue;
		}
		...
		if (sme->bssid) {
			...
			if ((_rtw_memcmp(dst_bssid, src_bssid, ETH_ALEN)) == _TRUE) {
				...
				_rtw_memcpy(ndis_ssid.Ssid, pnetwork->network.Ssid.Ssid,
