...
typedef ssize_t (*read_proc_t)(struct file *, char __user *, ssize_t,
			       loff_t *);
...
read:
	...
	struct proc_dir_entry *res = proc_create_data(name, mode, base, &fops,
				     data);
	return res;
}
typedef ssize_t (*write_proc_t)(struct file *, const char __user *,
				ssize_t, loff_t *);
static inline struct proc_dir_entry *create_proc_read_write_entry(
	const char *name,
	mode_t mode, struct proc_dir_entry *base,
...
uint loadparam(_adapter *padapter,  _nic_hdl	pnetdev)
{
	...
	if (IsSupported24G(registry_par->wireless_mode)
	    && (!IsSupported5G(registry_par->wireless_mode))
	    && (registry_par->channel > 14)) {
		registry_par->channel = 1;
	} else if (IsSupported5G(registry_par->wireless_mode)
