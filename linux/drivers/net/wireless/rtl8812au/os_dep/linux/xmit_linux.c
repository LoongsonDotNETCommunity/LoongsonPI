...
{
	return (pfile->buf_len - ((SIZE_PTR)(pfile->cur_addr) - (SIZE_PTR)(
					  pfile->buf_start)));
}
...
int rtw_os_xmit_resource_alloc(_adapter *padapter,
			       struct xmit_buf *pxmitbuf, u32 alloc_sz, u8 flag)
{
	if (alloc_sz > 0) {
		...
		pxmitbuf->pallocated_buf = rtw_usb_buffer_alloc(pusbd, (size_t)alloc_sz,
					   &pxmitbuf->dma_transfer_addr);
		...
		pxmitbuf->pbuf = (u8 *)N_BYTE_ALIGMENT((SIZE_PTR)(
...
void rtw_os_xmit_resource_free(_adapter *padapter,
			       struct xmit_buf *pxmitbuf, u32 free_sz, u8 flag)
{
	...
	if (free_sz > 0) {
		...
		rtw_usb_buffer_free(pusbd, (size_t)free_sz, pxmitbuf->pallocated_buf,
				    pxmitbuf->dma_transfer_addr);
...
void rtw_os_xmit_schedule(_adapter *padapter)
{
	...
	if (_rtw_queue_empty(&pri_adapter->xmitpriv.pending_xmitbuf_queue) ==
	    _FALSE)
