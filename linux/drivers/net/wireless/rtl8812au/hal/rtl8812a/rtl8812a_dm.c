...
static void dm_CheckPbcGPIO(_adapter *padapter)
{
	...
	if (IS_HARDWARE_TYPE_8812(padapter)) {
		...
		tmp1byte |= (BIT4);
		rtw_write8(padapter, GPIO_IO_SEL_8811A,
			   tmp1byte);	//enable GPIO[2] as output mode

		tmp1byte &= ~(BIT4);
		rtw_write8(padapter,  GPIO_IN_8811A,
			   tmp1byte);		//reset the floating voltage level
...
static void Init_ODM_ComInfo_8812(PADAPTER	Adapter)
{
	...
	ODM_CmnInfoInit(pDM_Odm,	ODM_CMNINFO_MP_TEST_CHIP,
...
void	AntDivCompare8812(PADAPTER Adapter, WLAN_BSSID_EX *dst,
			  WLAN_BSSID_EX *src)
{
