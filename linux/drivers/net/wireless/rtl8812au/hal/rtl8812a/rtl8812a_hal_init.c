...
static VOID efuse_phymap_to_logical(u8 *phymap, u16 _offset,
				    u16 _size_byte, u8  *pbuf)
{
	...
	eFuseWord = (u16 **)rtw_malloc2d(EFUSE_MAX_SECTION_JAGUAR,
	...
	//
	efuse_usage = (u1Byte)((efuse_utilized * 100) /
...
exit:
	...
	if (eFuseWord)
		rtw_mfree2d((void *)eFuseWord, EFUSE_MAX_SECTION_JAGUAR,
			    EFUSE_MAX_WORD_UNIT, sizeof(u16));
}
	...
	while (1) {
		...
		while (!(reg_0x143 = rtw_read8(adapter, REG_TXPKTBUF_DBG)) //dbg
			...
			DBG_871X("%s polling reg_0x143:0x%02x, reg_0x106:0x%02x\n", __FUNCTION__,
				 reg_0x143, rtw_read8(adapter, 0x106));
			rtw_usleep_os(100);
		}
...
)
{
	...
	if (status == _SUCCESS)
		efuse_read_phymap_from_txpktbuf(padapter, txpktbuf_bndy, physical_map,
						&size);
...
static s32 _LLTWrite(PADAPTER padapter, u32 address, u32 data)
{
	s32	status = _SUCCESS;
	s32	count = 0;
	u32	value = _LLT_INIT_ADDR(address) | _LLT_INIT_DATA(data) | _LLT_OP(
				_LLT_WRITE_ACCESS);
...
static VOID _FWDownloadEnable_8812(
...
)
{
...
#ifdef CONFIG_USB_HCI
		ret = rtw_writeN(padapter, (FW_START_ADDRESS + i * blockSize_p1),
				 blockSize_p1, (bufferPtr + i * blockSize_p1));
#else
		ret = rtw_write32(padapter, (FW_START_ADDRESS + i * blockSize_p1),
	...
	if (remainSize_p1) {
		...
		case 1:
			...
			ret = rtw_write32(padapter,
					  (FW_START_ADDRESS + blockCount_p1 * blockSize_p1),
	...
	if (remainSize_p1) {
		...
		for (i = 0; i < blockCount_p2; i++) {
			ret = rtw_writeN(padapter, (FW_START_ADDRESS + offset + i * blockSize_p2),
					 blockSize_p2, (bufferPtr + offset + i * blockSize_p2));
	...
	if (remainSize_p2) {
		...
		for (i = 0 ; i < blockCount_p3 ; i++) {
			ret = rtw_write8(padapter, (FW_START_ADDRESS + offset + i),
					 *(bufferPtr + offset + i));
...
//		_FillDummy(bufferPtr, &size);
	...
	for (page = 0; page < pageNums; page++) {
		...
		ret = _PageWrite_8812(padapter, page, bufferPtr + offset,
				      MAX_DLFW_PAGE_SIZE);
...
)
{
...
#ifdef CONFIG_FILE_FWIMG
		rtStatus = rtw_retrive_from_file(rtw_fw_file_path, FwBuffer8812,
						 FW_SIZE_8812);
	...
	case FW_SOURCE_HEADER_FILE:
		...
		ODM_ConfigFWWithHeaderFile(&pHalData->odmpriv, CONFIG_FW_WoWLAN,
					   (u8 *) & (pFirmware->szWoWLANFwBuffer), &(pFirmware->ulWoWLANFwLength));
	...
	DBG_871X("%s: fw_ver=%d fw_subver=%d sig=0x%x\n",
		 __FUNCTION__, pHalData->FirmwareVersion, pHalData->FirmwareSubVersion,
	...
	while (1) {
		...
		rtw_write8(Adapter, REG_MCUFWDL, rtw_read8(Adapter,
...
// 2011.04.12 by tynli.
//
VOID SetFwRelatedForWoWLAN8812(
	IN		PADAPTER			padapter,
...
static void hal_ReadPowerValueFromPROM8812A(
	IN	PADAPTER 		Adapter,
	...
	for (rfPath = 0 ; rfPath < MAX_RF_PATH ; rfPath++) {
		...
		for (TxCount = 0; TxCount < MAX_TX_COUNT; TxCount++) {
			if (TxCount == 0) {
...

				{
					...
					if (pwrInfo24G->BW20_Diff[rfPath][TxCount] &
					    BIT3)		//4bit sign number to 8 bit sign number
...

				{
					...
					if (pwrInfo24G->OFDM_Diff[rfPath][TxCount] &
					    BIT3)		//4bit sign number to 8 bit sign number
...

				{
					...
					if (pwrInfo24G->BW40_Diff[rfPath][TxCount] &
					    BIT3)		//4bit sign number to 8 bit sign number
...

				{
					...
					if (pwrInfo24G->CCK_Diff[rfPath][TxCount] &
					    BIT3)		//4bit sign number to 8 bit sign number
		...
		for (TxCount = 0; TxCount < MAX_TX_COUNT; TxCount++) {
			if (TxCount == 0) {
...

				{
					...
					if (pwrInfo5G->BW20_Diff[rfPath][TxCount] &
					    BIT3)		//4bit sign number to 8 bit sign number
...

				{
					...
					if (pwrInfo5G->OFDM_Diff[rfPath][TxCount] &
					    BIT3)		//4bit sign number to 8 bit sign number
...

				{
					...
					if (pwrInfo5G->BW40_Diff[rfPath][TxCount] &
					    BIT3)		//4bit sign number to 8 bit sign number
		...
		for (TxCount = 0; TxCount < MAX_TX_COUNT; TxCount++) {

			{
				...
				if (pwrInfo5G->BW80_Diff[rfPath][TxCount] &
				    BIT3)		//4bit sign number to 8 bit sign number
...

			{
				...
				if (pwrInfo5G->BW160_Diff[rfPath][TxCount] &
				    BIT3)		//4bit sign number to 8 bit sign number
...
)
{
	...
	hal_ReadPowerValueFromPROM8812A(Adapter, &pwrInfo24G, &pwrInfo5G,
					PROMContent, AutoLoadFail);
	...
	for (rfPath = 0 ; rfPath < MAX_RF_PATH ; rfPath++) {
		...
		for (ch = 0 ; ch < CHANNEL_MAX_NUMBER_5G; ch++) {
			...
			pHalData->Index5G_BW40_Base[rfPath][ch] =
				pwrInfo5G.IndexBW40_Base[rfPath][group];
			//DBG_871X("======= Path %d, ChannelIndex %d, Group %d=======\n",rfPath,ch, group);
		...
		for (TxCount = 0; TxCount < MAX_TX_COUNT; TxCount++) {
			...
			pHalData->OFDM_5G_Diff[rfPath][TxCount] =
				pwrInfo5G.OFDM_Diff[rfPath][TxCount];
			pHalData->BW20_5G_Diff[rfPath][TxCount] =
				pwrInfo5G.BW20_Diff[rfPath][TxCount];
			pHalData->BW40_5G_Diff[rfPath][TxCount] =
				pwrInfo5G.BW40_Diff[rfPath][TxCount];
			pHalData->BW80_5G_Diff[rfPath][TxCount] =
				pwrInfo5G.BW80_Diff[rfPath][TxCount];
//#if DBG
#if 0
			DBG_871X("--------------------------------------- 2.4G ---------------------------------------\n");
	...
	if (!AutoLoadFail) {
		...
		if (registry_par->regulatory_tid == 0xff) {
			...
			if (PROMContent[EEPROM_RF_BOARD_OPTION_8812] == 0xFF)
				...
				pHalData->EEPROMRegulatory = (PROMContent[EEPROM_RF_BOARD_OPTION_8812] &
							      0x7);	//bit0~2
		} else {
		...
		pHalData->TxPwrCalibrateRate =
			PROMContent[EEPROM_TX_PWR_CALIBRATE_RATE_8812];
	} else {
...
)
{
	...
	if (!AutoloadFail) {
		pHalData->InterfaceSel = (PROMContent[EEPROM_RF_BOARD_OPTION_8812] & 0xE0)
					 >> 5;
		if (PROMContent[EEPROM_RF_BOARD_OPTION_8812] == 0xFF)
...
)
{
	...
	if (!AutoLoadFail) {
		...
		if (pHalData->CrystalCap == 0xFF)
			pHalData->CrystalCap =
				EEPROM_Default_CrystalCap_8812;	 //what value should 8812 set?
	} else {
...
VOID Hal_ReadAntennaDiversity8812A(
	IN	PADAPTER		pAdapter,
	IN	u8				*PROMContent,
	IN	BOOLEAN			AutoLoadFail
)
{
	...
	if (!AutoLoadFail) {
		...
		if (registry_par->antdiv_cfg == 2) {
			pHalData->AntDivCfg = (PROMContent[EEPROM_RF_BOARD_OPTION_8812] & 0x18) >>
					      3;
			if (PROMContent[EEPROM_RF_BOARD_OPTION_8812] == 0xFF)
		...
		if (pHalData->EEPROMBluetoothCoexist != 0
		    && pHalData->EEPROMBluetoothAntNum == Ant_x1)
			pHalData->AntDivCfg = 0;

		pHalData->TRxAntDivType =
			PROMContent[EEPROM_RF_ANTENNA_OPT_8812];  //todo by page
		if (pHalData->TRxAntDivType == 0xFF)
			pHalData->TRxAntDivType =
				FIXED_HW_ANTDIV; // For 88EE, 1Tx and 1RxCG are fixed.(1Ant, Tx and RxCG are both on aux port)
	} else {
		pHalData->AntDivCfg = 0;
...
)
{
	...
	if (! AutoloadFail) {
		if (GetRegAmplifierType2G(Adapter) == 0) { // AUTO
			...
			pHalData->LNAType_2G = EF1Byte(*(u8 *)
						       &PROMContent[EEPROM_LNA_TYPE_2G_8812AU]);
			if (pHalData->PAType_2G == 0xFF && pHalData->LNAType_2G == 0xFF) {
				pHalData->PAType_2G = 0;
				pHalData->LNAType_2G = 0;
			}
			pHalData->ExternalPA_2G = ((pHalData->PAType_2G & BIT5)
						   && (pHalData->PAType_2G & BIT4)) ? 1 : 0;
			pHalData->ExternalLNA_2G = ((pHalData->LNAType_2G & BIT7)
						    && (pHalData->LNAType_2G & BIT3)) ? 1 : 0;
		} else {
			pHalData->ExternalPA_2G  = (GetRegAmplifierType2G(Adapter)
						    &ODM_BOARD_EXT_PA)  ? 1 : 0;
			pHalData->ExternalLNA_2G = (GetRegAmplifierType2G(Adapter)
						    &ODM_BOARD_EXT_LNA) ? 1 : 0;
		}

		if (GetRegAmplifierType5G(Adapter) == 0) { // AUTO
			pHalData->PAType_5G = EF1Byte(*(u8 *)&PROMContent[EEPROM_PA_TYPE_8812AU]);
			pHalData->LNAType_5G = EF1Byte(*(u8 *)
						       &PROMContent[EEPROM_LNA_TYPE_5G_8812AU]);
			if (pHalData->PAType_5G == 0xFF && pHalData->LNAType_5G == 0xFF) {
				pHalData->PAType_5G = 0;
				pHalData->LNAType_5G = 0;
			}
			pHalData->ExternalPA_5G = ((pHalData->PAType_5G & BIT1)
						   && (pHalData->PAType_5G & BIT0)) ? 1 : 0;
			pHalData->ExternalLNA_5G = ((pHalData->LNAType_5G & BIT7)
						    && (pHalData->LNAType_5G & BIT3)) ? 1 : 0;
		} else {
			pHalData->ExternalPA_5G  = (GetRegAmplifierType5G(Adapter)
						    &ODM_BOARD_EXT_PA_5G)  ? 1 : 0;
			pHalData->ExternalLNA_5G = (GetRegAmplifierType5G(Adapter)
						    &ODM_BOARD_EXT_LNA_5G) ? 1 : 0;
		}
	} else {
...
static VOID Hal_EfusePowerSwitch8812A(
	IN	PADAPTER	pAdapter,
	IN	u8		bWrite,
	IN	u8		PwrState)
{
...
static VOID rtl8812_EfusePowerSwitch(
	IN	PADAPTER	pAdapter,
	IN	u8		bWrite,
	IN	u8		PwrState)
{
...
static BOOLEAN Hal_EfuseSwitchToBank8812A(
	IN		PADAPTER	pAdapter,
	IN		u1Byte		bank,
...
static VOID Hal_EfuseReadEFuse8812A(
	PADAPTER		Adapter,
	u16			_offset,
	u16 			_size_byte,
	u8      		*pbuf,
...
#ifdef DBG_IOL_READ_EFUSE_MAP
	if (_rtw_memcmp(logical_map, Adapter->eeprompriv.efuse_eeprom_data,
			0x130) == _FALSE) {
		int i;
...
//Do not support BT
VOID Hal_EFUSEGetEfuseDefinition8812A(
	IN		PADAPTER	pAdapter,
	IN		u1Byte		efuseType,
	IN		u1Byte		type,
	OUT		PVOID		pOut
)
{
	...
	case TYPE_AVAILABLE_EFUSE_BYTES_BANK: {
		u16 *pu2Tmp;
		pu2Tmp = (u16 *)pOut;
		*pu2Tmp = (u16)(EFUSE_REAL_CONTENT_LEN_JAGUAR -
...
VOID Hal_EFUSEGetEfuseDefinition_Pseudo8812A(
	IN		PADAPTER	pAdapter,
	IN		u8			efuseType,
	IN		u8			type,
	OUT		PVOID		pOut
)
{
	...
	case TYPE_AVAILABLE_EFUSE_BYTES_BANK: {
		u16 *pu2Tmp;
		pu2Tmp = (pu2Byte)pOut;
		*pu2Tmp = (u2Byte)(EFUSE_REAL_CONTENT_LEN_JAGUAR -
				   EFUSE_OOB_PROTECT_BYTES_JAGUAR);
	}
	break;
	case TYPE_AVAILABLE_EFUSE_BYTES_TOTAL: {
...
static VOID rtl8812_EFUSE_GetEfuseDefinition(
	IN		PADAPTER	pAdapter,
	IN		u8		efuseType,
	IN		u8		type,
	OUT		void		*pOut,
...
static u8 Hal_EfuseWordEnableDataWrite8812A(IN	PADAPTER	pAdapter,
		IN	u16		efuse_addr,
		IN	u8		word_en,
		IN	u8		*data,
		IN	BOOLEAN		bPseudoTest)
...
static u8 rtl8812_Efuse_WordEnableDataWrite(IN	PADAPTER	pAdapter,
		IN	u16		efuse_addr,
		IN	u8		word_en,
		IN	u8		*data,
		IN	BOOLEAN		bPseudoTest)
{
	...
	ret = Hal_EfuseWordEnableDataWrite8812A(pAdapter, efuse_addr, word_en,
						data, bPseudoTest);
...
static u16 hal_EfuseGetCurrentSize_8812A(IN	PADAPTER	pAdapter,
		IN		BOOLEAN			bPseudoTest)
{
...
static int hal_EfusePgPacketRead_8812A(
	IN	PADAPTER	pAdapter,
	IN	u8			offset,
	IN	u8			*data,
	IN	BOOLEAN		bPseudoTest)
...
static int rtl8812_Efuse_PgPacketRead(IN	PADAPTER	pAdapter,
				      IN	u8			offset,
				      IN	u8			*data,
				      IN	BOOLEAN		bPseudoTest)
{
...
int hal_EfusePgPacketWrite_8812A(IN	PADAPTER	pAdapter,
				 IN	u8 			offset,
				 IN	u8			word_en,
				 IN	u8			*data,
				 IN	BOOLEAN		bPseudoTest)
{
	...
	// 2009.02.19.
	//
	if (Efuse_GetCurrentSize(pAdapter, efuseType,
				 bPseudoTest) >= (EFUSE_REAL_CONTENT_LEN_JAGUAR -
						 EFUSE_OOB_PROTECT_BYTES_JAGUAR)) {
		DBG_871X("hal_EfusePgPacketWrite_8812A() error: %x >= %x\n",
			 Efuse_GetCurrentSize(pAdapter, efuseType, bPseudoTest),
			 (EFUSE_REAL_CONTENT_LEN_JAGUAR - EFUSE_OOB_PROTECT_BYTES_JAGUAR));
		return _FALSE;
	}
			...
			if (efuse_OneByteRead(pAdapter, efuse_addr, &efuse_data, bPseudoTest) &&
				...
				if ((efuse_data & 0x1F) == 0x0F) {	//extended header
					...
					if ((efuse_data & 0x0F) == 0x0F) { //wren fail
						...
						if (next == 0xFF
						    && next_next == 0xFF) { // Have enough space to make fake data to recover bad header.
							switch (s) {
							case 0x0:
							case 0x2:
							case 0x4:
					...
					if (tmp_pkt.word_en == 0xF) {
						...
						efuse_OneByteRead(pAdapter, efuse_addr + 1, &next, bPseudoTest);
						if (next ==
						    0xFF) { // Have enough space to make fake data to recover bad header.
				...
				if (tmp_pkt.offset  != target_pkt.offset) {
					...
					for (tmpindex = 0 ; tmpindex < (tmp_word_cnts * 2) ; tmpindex++) {
						if (efuse_OneByteRead(pAdapter, (efuse_addr + 1 + tmpindex), &efuse_data,
					...
					if (bDataEmpty == _FALSE) {
						...
						if ((match_word_en & 0x0F) != 0x0F) {
							badworden = Efuse_WordEnableDataWrite(pAdapter, efuse_addr + 1,
											      tmp_pkt.word_en, target_pkt.data, bPseudoTest);
...
//								efuse_addr = efuse_addr + (2*tmp_word_cnts) +1;//next pg packet addr
				...
				if (target_pkt.offset >= EFUSE_MAX_SECTION_BASE) {
					...
					if (tmp_header == pg_header) {
						...
						if ((tmp_header & 0x0F) == 0x0F) { //wren PG fail
							...
							tmp_pkt.offset = ((pg_header_temp & 0xE0) >> 5) | ((tmp_header & 0xF0) >>
									 1);
							tmp_pkt.word_en =  tmp_header & 0x0F;
							tmp_word_cnts =  Efuse_CalculateWordCnts(tmp_pkt.word_en);
						}
				...
				if (tmp_header == pg_header) {
					...
					if (Efuse_PgPacketRead(pAdapter, tmp_pkt.offset, originaldata,
						...
						//efuse_reg_ctrl(pAdapter,_TRUE);//power on
						badworden = Efuse_WordEnableDataWrite(pAdapter, efuse_addr + 1,
										      tmp_pkt.word_en, originaldata, bPseudoTest);
						//############################
						if (0x0F != (badworden & 0x0F)) {
		...
		//write data state
			...
			badworden = 0x0f;
			badworden = Efuse_WordEnableDataWrite(pAdapter, efuse_addr + 1,
							      target_pkt.word_en, target_pkt.data, bPseudoTest);
...
static int rtl8812_Efuse_PgPacketWrite(IN	PADAPTER	pAdapter,
				       IN	u8 			offset,
				       IN	u8			word_en,
				       IN	u8			*data,
				       IN	BOOLEAN		bPseudoTest)
	...
	BOOLEAN					bSet)
{
	...
	case HAL_ODM_STA_INFO: {
		...
		if (bSet) {
			...
			ODM_CmnInfoPtrArrayHook(podmpriv, ODM_CMNINFO_STA_STATUS, psta->mac_id,
						psta);
...
void rtl8812_clone_haldata(_adapter *dst_adapter, _adapter *src_adapter)
{
	...
	_rtw_memcpy(&temp_SdioXmitTerminateSema,
		    &(pHalData->SdioXmitTerminateSema), sizeof(_sema));
#endif
	_rtw_memcpy(&temp_SdioTxFIFOFreePageLock,
		    &(pHalData->SdioTxFIFOFreePageLock), sizeof(_lock));

	_rtw_memcpy(dst_adapter->HalData, src_adapter->HalData,
		    dst_adapter->hal_data_sz);

#ifndef CONFIG_SDIO_TX_TASKLET
	_rtw_memcpy(&(pHalData->SdioXmitSema), &temp_SdioXmitSema, sizeof(_sema));
	_rtw_memcpy(&(pHalData->SdioXmitTerminateSema),
		    &temp_SdioXmitTerminateSema, sizeof(_sema));
#endif
	_rtw_memcpy(&(pHalData->SdioTxFIFOFreePageLock),
		    &temp_SdioTxFIFOFreePageLock, sizeof(_lock));
...
)
{
	...
	size = (curRCR & EEPROMSEL) ? 6 :
	       4; // 6: EEPROM used is 93C46, 4: boot from E-Fuse.
...
void InitPGData8812A(PADAPTER padapter)
{
...
#ifdef CONFIG_EFUSE_CONFIG_FILE
	{
		...
		_rtw_memcpy(&pEEPROM->efuse_eeprom_data[addr], pEEPROM->mac_addr,
			    ETH_ALEN);
	}
...
)
{
	...
	pHalData->PolarityCtl = ((value32 & WL_HWPDN_SL) ? RT_POLARITY_HIGH_ACT :
				 RT_POLARITY_LOW_ACT);
...
void UpdateHalRAMask8812A(PADAPTER padapter, u32 mac_id, u8 rssi_level)
{
	...
	case 2:// first station uses macid=0, second station uses macid=2
		...
		networkType = judge_network_type(padapter, cur_network->SupportedRates,
						 supportRateNum);
...
#endif
		{
			mask |= (pmlmeinfo->HT_enable) ? update_MCS_rate(&(pmlmeinfo->HT_caps)) :
				0;
...
static VOID _BeaconFunctionEnable(
	IN	PADAPTER		Adapter,
	IN	BOOLEAN			Enable,
	IN	BOOLEAN			Linked
)
...
void SetBeaconRelatedRegisters8812A(PADAPTER padapter)
{
	...
	rtw_write8(padapter, bcn_ctrl_reg, rtw_read8(padapter,
			bcn_ctrl_reg) | BIT(1));
...
static void hw_var_set_opmode(PADAPTER Adapter, u8 variable, u8 *val)
{
	...
	if (Adapter->iface_type == IFACE_PORT1) {
		...
		rtw_write8(Adapter, REG_BCN_CTRL_1, rtw_read8(Adapter,
		...
		if ((mode == _HW_STATE_STATION_) || (mode == _HW_STATE_NOLINK_)) {
			if (!check_buddy_mlmeinfo_state(Adapter, WIFI_FW_AP_STATE)) {
				...
				UpdateInterruptMask8812AU(Adapter, _TRUE, 0,
							  (IMR_TXBCN0ERR_8812 | IMR_TXBCN0OK_8812));
...
#ifdef CONFIG_INTERRUPT_BASED_TXBCN_BCN_OK_ERR
			UpdateInterruptMask8812AU(Adapter, _TRUE,
						  (IMR_TXBCN0ERR_8812 | IMR_TXBCN0OK_8812), 0);
#endif//CONFIG_INTERRUPT_BASED_TXBCN_BCN_OK_ERR
			...
			if (IS_HARDWARE_TYPE_8821(Adapter)) {
				...
				rtw_write8(Adapter, REG_CCK_CHECK_8812,	 rtw_read8(Adapter,
						REG_CCK_CHECK_8812) | BIT(5));
			}
...
static void hw_var_set_bcn_func(PADAPTER Adapter, u8 variable, u8 *val)
{
	...
	if (*((u8 *)val)) {
		...
		rtw_write8(Adapter, bcn_ctrl_reg, rtw_read8(Adapter,
...
static void hw_var_set_mlme_disconnect(PADAPTER Adapter, u8 variable,
				       u8 *val)
{
#ifdef CONFIG_CONCURRENT_MODE
...
static void hw_var_set_mlme_sitesurvey(PADAPTER Adapter, u8 variable,
				       u8 *val)
{
	...
	if (*((u8 *)val)) { //under sitesurvey
		...
		if (check_fwstate(pmlmepriv,
			...
			//disable update TSF
			rtw_write8(Adapter, reg_bcn_ctl, rtw_read8(Adapter,
					reg_bcn_ctl) | DIS_TSF_UDT);
		}
...
static void hw_var_set_mlme_join(PADAPTER Adapter, u8 variable, u8 *val)
{
	...
	if (type == 0) { // prepare to join
		...
		if (check_buddy_mlmeinfo_state(Adapter, WIFI_FW_AP_STATE)) {
			rtw_write32(Adapter, REG_RCR, rtw_read32(Adapter,
...
//				rtw_write32(padapter, REG_RCR, val32);
	...
	case HW_VAR_MLME_JOIN:
...
#else // !CONFIG_CONCURRENT_MODE
		{
			...
			val16 = RetryLimit << RETRY_LIMIT_SHORT_SHIFT | RetryLimit <<
				RETRY_LIMIT_LONG_SHIFT;
...
//					BT_WifiMediaStatusNotify(padapter, RT_MEDIA_CONNECT);
	...
	case HW_VAR_ON_RCR_AM:
		...
		val32 |= RCR_AM;
		rtw_write32(padapter, REG_RCR, val32);
		DBG_8192C("%s, %d, RCR= %x\n", __FUNCTION__, __LINE__, rtw_read32(padapter,
				REG_RCR));
		break;
 ...
 */
u8 SetHalDefVar8812A(PADAPTER padapter, HAL_DEF_VARIABLE variable,
		     void *pval)
{
	PHAL_DATA_TYPE pHalData;
	u8 bResult;
 ...
 */
u8 GetHalDefVar8812A(PADAPTER padapter, HAL_DEF_VARIABLE variable,
		     void *pval)
{
	PHAL_DATA_TYPE pHalData;
	u8 bResult;
...
void rtl8812_set_hal_ops(struct hal_ops *pHalFunc)
{
	...
	pHalFunc->sreset_linked_status_check  =
		&rtl8812_sreset_linked_status_check;
	...
	pHalFunc->SetBeaconRelatedRegistersHandler =
		&SetBeaconRelatedRegisters8812A;
}
