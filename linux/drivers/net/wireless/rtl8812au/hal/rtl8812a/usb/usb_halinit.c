...
static VOID _ConfigChipOutEP_8812(
...
void rtl8812au_interface_configure(_adapter *padapter)
{
	...
	pHalData->UsbRxAggPageCount	=
		16; //uint :128 b //0x0A;	// 10 = MAX_RX_DMA_BUFFER_SIZE/2/pHalData->UsbBulkOutSize
...
static VOID _InitBurstPktLen(IN PADAPTER Adapter)
{
	...
	rtw_write16(Adapter, REG_RXDMA_STATUS,
	...
	if (speedvalue & BIT7) { //USB2/1.1 Mode
		...
		if (((temp >> 4) & 0x03) == 0) {
			...
			rtw_write8(Adapter, REG_RXDMA_PRO_8812,
				   ((provalue | BIT(4)) & (~BIT(5)))); //set burst pkt len=512B
			rtw_write16(Adapter, REG_RXDMA_PRO_8812, 0x1e);
		} else {
			pHalData->UsbBulkOutSize = 64;
			provalue = rtw_read8(Adapter, REG_RXDMA_PRO_8812);
			rtw_write8(Adapter, REG_RXDMA_PRO_8812,
				   ((provalue | BIT(5)) & (~BIT(4)))); //set burst pkt len=64B
		}
		...
		provalue = rtw_read8(Adapter, REG_RXDMA_PRO_8812);
		rtw_write8(Adapter, REG_RXDMA_PRO_8812,
			   provalue & (~(BIT5 | BIT4))); //set burst pkt len=1k
	...
	rtw_write8(Adapter, REG_HT_SINGLE_AMPDU_8812, rtw_read8(Adapter,
			REG_HT_SINGLE_AMPDU_8812) | BIT(7)); //enable single pkt ampdu
	...
	if (IS_HARDWARE_TYPE_8821U(Adapter)
	    && (Adapter->registrypriv.wifi_spec == _FALSE)) {
		...
		rtw_write8(Adapter, REG_FWHW_TXQ_CTRL, rtw_read8(Adapter,
				REG_FWHW_TXQ_CTRL) & (~BIT(7)));
	}
	...
	rtw_write8(Adapter, 0x1c, rtw_read8(Adapter,
					    0x1c) | BIT(5) | BIT(6)); //to prevent mac is reseted by bus. 20111208, by Page
...
static u32 _InitPowerOn8812AU(_adapter *padapter)
{
	...
	if (IS_VENDOR_8821A_MP_CHIP(padapter)) {
		...
		if (!HalPwrSeqCmdParsing(padapter, PWR_CUT_A_MSK, PWR_FAB_ALL_MSK,
					 PWR_INTF_USB_MSK, Rtl8821A_NIC_ENABLE_FLOW)) {
			DBG_871X(KERN_ERR "%s: run power on flow fail\n", __func__);
			return _FAIL;
		}
	} else if (IS_HARDWARE_TYPE_8821U(padapter)) {
		if (!HalPwrSeqCmdParsing(padapter, PWR_CUT_TESTCHIP_MSK, PWR_FAB_ALL_MSK,
					 PWR_INTF_USB_MSK, Rtl8821A_NIC_ENABLE_FLOW)) {
			DBG_871X(KERN_ERR "%s: run power on flow fail\n", __func__);
			return _FAIL;
		}
		...
		if (!HalPwrSeqCmdParsing(padapter, PWR_CUT_ALL_MSK, PWR_FAB_ALL_MSK,
					 PWR_INTF_USB_MSK, Rtl8812_NIC_ENABLE_FLOW)) {
			DBG_871X(KERN_ERR "%s: run power on flow fail\n", __func__);
			return _FAIL;
		}
	}
...
static VOID _InitInterrupt_8812AU(
	IN  PADAPTER Adapter
)
{
	HAL_DATA_TYPE	*pHalData	= GET_HAL_DATA(Adapter);
...
static VOID _InitQueueReservedPage_8821AUsb(
	IN  PADAPTER Adapter
)
{
	HAL_DATA_TYPE		*pHalData = GET_HAL_DATA(Adapter);
...
static VOID _InitQueueReservedPage_8812AUsb(
	IN  PADAPTER Adapter
)
{
	HAL_DATA_TYPE		*pHalData = GET_HAL_DATA(Adapter);
...
static VOID _InitTxBufferBoundary_8821AUsb(
	IN PADAPTER Adapter
)
{
	struct registry_priv *pregistrypriv = &Adapter->registrypriv;
	...
	rtw_write8(Adapter, REG_TDECTRL1_8812 + 2, rtw_read8(Adapter,
			REG_TDECTRL1_8812 + 2) | BIT1);
#endif

}

static VOID _InitTxBufferBoundary_8812AUsb(
	IN PADAPTER Adapter
)
{
	struct registry_priv *pregistrypriv = &Adapter->registrypriv;
...
static VOID _InitPageBoundary_8812AUsb(
	IN  PADAPTER Adapter
)
{
	//u2Byte 			rxff_bndy;
...
//	rxff_bndy = (Offset*256)-1;
	...
	if (IS_HARDWARE_TYPE_8812(Adapter))
		rtw_write16(Adapter, (REG_TRXFF_BNDY + 2),
			    MAX_RX_DMA_BUFFER_SIZE_8812 - 1);
	else
		rtw_write16(Adapter, (REG_TRXFF_BNDY + 2),
			    MAX_RX_DMA_BUFFER_SIZE_8821 - 1);
...
static VOID _InitNormalChipTwoOutEpPriority_8812AUsb(
	IN	PADAPTER Adapter
)
{
	HAL_DATA_TYPE	*pHalData	= GET_HAL_DATA(Adapter);
	...
	_InitNormalChipRegPriority_8812AUsb(Adapter, beQ, bkQ, viQ, voQ, mgtQ,
					    hiQ);

}

static VOID _InitNormalChipThreeOutEpPriority_8812AUsb(
	IN	PADAPTER Adapter
)
{
	struct registry_priv *pregistrypriv = &Adapter->registrypriv;
...
static VOID _InitQueuePriority_8812AUsb(
	IN	PADAPTER Adapter
)
{
	HAL_DATA_TYPE	*pHalData	= GET_HAL_DATA(Adapter);
...
static VOID _InitHardwareDropIncorrectBulkOut_8812A(
	IN  PADAPTER Adapter
)
{
...
static VOID _InitNetworkType_8812A(
	IN  PADAPTER Adapter
)
{
	u32	value32;
...
static VOID _InitTransferPageSize_8812AUsb(
	IN  PADAPTER Adapter
)
{
...
static VOID _InitDriverInfoSize_8812A(
	IN  PADAPTER	Adapter,
	IN	u8		drvInfoSize
)
{
...
static VOID _InitWMACSetting_8812A(
	IN  PADAPTER Adapter
)
{
	//u4Byte			value32;
	...
	pHalData->ReceiveConfig =
		RCR_APM | RCR_AM | RCR_AB | RCR_CBSSID_DATA | RCR_CBSSID_BCN | RCR_APP_ICV
...
static VOID _InitAdaptiveCtrl_8812AUsb(
	IN  PADAPTER Adapter
)
{
	u16	value16;
...
static VOID _InitEDCA_8812AUsb(
	IN  PADAPTER Adapter
)
{
...
static VOID _InitBeaconMaxError_8812A(
	IN  PADAPTER	Adapter,
...
static VOID _InitRDGSetting_8812A(
	IN	PADAPTER Adapter
)
{
...
static VOID _InitRxSetting_8812AU(
	IN	PADAPTER Adapter
)
{
...
static VOID _InitRetryFunction_8812A(
	IN  PADAPTER Adapter
)
{
	u8	value8;
 ...
 *
 *---------------------------------------------------------------------------*/
static VOID usb_AggSettingTxUpdate_8812A(
	IN	PADAPTER			Adapter
)
{
#ifdef CONFIG_USB_TX_AGGREGATION
 ...
 *
 *---------------------------------------------------------------------------*/
static VOID usb_AggSettingRxUpdate_8812A(
	IN	PADAPTER			Adapter
)
{
#ifdef CONFIG_USB_RX_AGGREGATION
...
static VOID init_UsbAggregationSetting_8812A(
	IN  PADAPTER Adapter
)
{
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(Adapter);
...
static VOID _InitOperationMode_8812A(
	IN	PADAPTER			Adapter
)
{
#if 0//gtest
...
static VOID _InitAntenna_Selection_8812A(IN	PADAPTER Adapter)
{
	...
	DBG_8192C("%s,Cur_ant:(%x)%s\n", __FUNCTION__, pHalData->CurAntenna,
		  (pHalData->CurAntenna == MAIN_ANT) ? "MAIN_ANT" : "AUX_ANT");
 ...
 *---------------------------------------------------------------------------*/
static VOID HwSuspendModeEnable_8812AU(
	IN	PADAPTER	pAdapter,
	IN	u8			Type
)
{
...
#define HAL_INIT_PROFILE_TAG(stage) do {} while(0)
...
#endif  //#if (MP_DRIVER == 1)
	{
		...
		rtw_write8(Adapter, REG_QUEUE_CTRL, rtw_read8(Adapter,
				REG_QUEUE_CTRL) & 0xF7);
		...
		rtw_write8(Adapter, REG_EARLY_MODE_CONTROL_8812 + 3,
...
)
{
	...
	if (IS_HARDWARE_TYPE_8821U(Adapter))
		HalPwrSeqCmdParsing(Adapter, PWR_CUT_ALL_MSK, PWR_FAB_ALL_MSK,
				    PWR_INTF_USB_MSK, Rtl8821A_NIC_LPS_ENTER_FLOW);
	else
		HalPwrSeqCmdParsing(Adapter, PWR_CUT_ALL_MSK, PWR_FAB_ALL_MSK,
				    PWR_INTF_USB_MSK, Rtl8812_NIC_LPS_ENTER_FLOW);
	...
	if (IS_HARDWARE_TYPE_8821U(Adapter))
		HalPwrSeqCmdParsing(Adapter, PWR_CUT_ALL_MSK, PWR_FAB_ALL_MSK,
				    PWR_INTF_USB_MSK, Rtl8821A_NIC_DISABLE_FLOW);
	else
		HalPwrSeqCmdParsing(Adapter, PWR_CUT_ALL_MSK, PWR_FAB_ALL_MSK,
				    PWR_INTF_USB_MSK, Rtl8812_NIC_DISABLE_FLOW);
}
...
#endif
	{
		rtw_write16(Adapter, REG_GPIO_MUXCFG, rtw_read16(Adapter,
				REG_GPIO_MUXCFG) & (~BIT12));
	}
...
)
{
	...
	if ((pHalData->EEPROMVID == 0x050D)
	    && (pHalData->EEPROMPID == 0x1106)) // SerComm for Belkin.
		pEEPROM->CustomerID = RT_CID_819x_Sercomm_Belkin;
	else if ((pHalData->EEPROMVID == 0x0846)
		 && (pHalData->EEPROMPID == 0x9051))// SerComm for Netgear.
		pEEPROM->CustomerID = RT_CID_819x_Sercomm_Netgear;
	else if ((pHalData->EEPROMVID == 0x2001)
		 && (pHalData->EEPROMPID ==
		     0x330e))//add by ylb 20121012 for customer led for alpha
		pEEPROM->CustomerID = RT_CID_819x_ALPHA_Dlink;
	else if ((pHalData->EEPROMVID == 0x0B05)
		 && (pHalData->EEPROMPID == 0x17D2))//Edimax for ASUS
...
)
{
	...
	if (_FALSE == AutoloadFail) {
		if (IS_HARDWARE_TYPE_8812AU(Adapter)) {
			...
			_rtw_memcpy(pEEPROM->mac_addr, &PROMContent[EEPROM_MAC_ADDR_8812AU],
				    ETH_ALEN);
		} else if (IS_HARDWARE_TYPE_8821U(Adapter)) {
			//Read Permanent MAC address and set value to hardware
			_rtw_memcpy(pEEPROM->mac_addr, &PROMContent[EEPROM_MAC_ADDR_8821AU],
				    ETH_ALEN);
		}
	} else {
...
VOID hal_CustomizedBehavior_8812AU(
	IN	PADAPTER	Adapter
)
{
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(Adapter);
...
static void hal_CustomizeByCustomerID_8812AU(
	IN	PADAPTER		pAdapter
)
{
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(pAdapter);
	...
	if ((pHalData->EEPROMVID == 0x103C)
	    && (pHalData->EEPROMPID == 0x1629)) // HP Lite-On for RTL8188CUS Slim Combo.
		pEEPROM->CustomerID = RT_CID_819x_HP;
	else if ((pHalData->EEPROMVID == 0x9846)
		 && (pHalData->EEPROMPID == 0x9041))
		pEEPROM->CustomerID = RT_CID_NETGEAR;
	else if ((pHalData->EEPROMVID == 0x2019)
		 && (pHalData->EEPROMPID == 0x1201))
		pEEPROM->CustomerID = RT_CID_PLANEX;
	else if ((pHalData->EEPROMVID == 0x0BDA)
		 && (pHalData->EEPROMPID == 0x5088))
	...
	case EEPROM_CID_DEFAULT:
		if ((pHalData->EEPROMVID == 0x2001) && (pHalData->EEPROMPID == 0x3308))
			...
			pEEPROM->CustomerID = RT_CID_DLINK;
		else if ((pHalData->EEPROMVID == 0x0BFF)
			 && (pHalData->EEPROMPID == 0x8160)) {
...
VOID hal_ReadUsbModeSwitch_8812AU(
	IN	PADAPTER	Adapter,
	IN	u8			*PROMContent,
	IN	BOOLEAN		AutoloadFail
)
...
static VOID ReadLEDSetting_8812AU(
	IN	PADAPTER	Adapter,
	IN	u8		*PROMContent,
	IN	BOOLEAN		AutoloadFail
)
...
VOID InitAdapterVariablesByPROM_8812AU(
	IN	PADAPTER	Adapter
)
{
	EEPROM_EFUSE_PRIV *pEEPROM = GET_EEPROM_EFUSE_PRIV(Adapter);
	...
	Hal_ReadPROMVersion8812A(Adapter, pEEPROM->efuse_eeprom_data,
				 pEEPROM->bautoload_fail_flag);
	hal_ReadIDs_8812AU(Adapter, pEEPROM->efuse_eeprom_data,
			   pEEPROM->bautoload_fail_flag);
	hal_ReadMACAddress_8812AU(Adapter, pEEPROM->efuse_eeprom_data,
				  pEEPROM->bautoload_fail_flag);
	Hal_ReadTxPowerInfo8812A(Adapter, pEEPROM->efuse_eeprom_data,
				 pEEPROM->bautoload_fail_flag);
	Hal_ReadBoardType8812A(Adapter, pEEPROM->efuse_eeprom_data,
			       pEEPROM->bautoload_fail_flag);

	//
	// Read Bluetooth co-exist and initialize
	//
	Hal_EfuseParseBTCoexistInfo8812A(Adapter, pEEPROM->efuse_eeprom_data,
					 pEEPROM->bautoload_fail_flag);

	Hal_ReadChannelPlan8812A(Adapter, pEEPROM->efuse_eeprom_data,
				 pEEPROM->bautoload_fail_flag);
	Hal_EfuseParseXtal_8812A(Adapter, pEEPROM->efuse_eeprom_data,
				 pEEPROM->bautoload_fail_flag);
	Hal_ReadThermalMeter_8812A(Adapter, pEEPROM->efuse_eeprom_data,
				   pEEPROM->bautoload_fail_flag);
	Hal_ReadAntennaDiversity8812A(Adapter, pEEPROM->efuse_eeprom_data,
				      pEEPROM->bautoload_fail_flag);

	if (IS_HARDWARE_TYPE_8821U(Adapter)) {
		Hal_ReadPAType_8821A(Adapter, pEEPROM->efuse_eeprom_data,
				     pEEPROM->bautoload_fail_flag);
	} else {
		Hal_ReadPAType_8812A(Adapter, pEEPROM->efuse_eeprom_data,
				     pEEPROM->bautoload_fail_flag);
		Hal_ReadRFEType_8812A(Adapter, pEEPROM->efuse_eeprom_data,
				      pEEPROM->bautoload_fail_flag);
	}

	hal_ReadUsbModeSwitch_8812AU(Adapter, pEEPROM->efuse_eeprom_data,
				     pEEPROM->bautoload_fail_flag);
	hal_CustomizeByCustomerID_8812AU(Adapter);

	ReadLEDSetting_8812AU(Adapter, pEEPROM->efuse_eeprom_data,
			      pEEPROM->bautoload_fail_flag);

	// 2013/04/15 MH Add for different board type recognize.
	hal_ReadUsbType_8812AU(Adapter, pEEPROM->efuse_eeprom_data,
			       pEEPROM->bautoload_fail_flag);
}
...
VOID hal_CustomizedBehavior_8812AUsb(
	IN PADAPTER 		Adapter
)
{
#if 0
...
void UpdateInterruptMask8812AU(PADAPTER padapter, u8 bHIMR0, u32 AddMSR,
			       u32 RemoveMSR)
{
...
static u8 rtl8812au_ps_func(PADAPTER Adapter, HAL_INTF_PS_FUNC efunc_id,
			    u8 *val)
{
	u8 bResult = _TRUE;
	switch (efunc_id) {
