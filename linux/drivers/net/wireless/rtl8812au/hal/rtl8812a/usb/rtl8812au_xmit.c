	...
	if ((pxmitframe->frame_tag & 0x0f) == DATA_FRAMETAG) {
		...
		if ((pattrib->ether_type != 0x888e) &&
			...
			if (pHalData->fw_ractrl == _FALSE) {
				...
				SET_TX_DESC_TX_RATE_8812(ptxdesc,
							 (pdmpriv->INIDATA_RATE[pattrib->mac_id] & 0x7F));
			}
...
static s32 rtw_dump_xframe(_adapter *padapter,
...
//			rtw_free_xmitframe(pxmitpriv, pxmitframe);
		...
		pxmitframe->pkt_offset =
			2; // first frame of aggregation, reserve one offset for EM info ,another for usb bulk-out block check
#else
...
static s32 xmitframe_direct(_adapter *padapter,
			    struct xmit_frame *pxmitframe)
{
	s32 res = _SUCCESS;
//DBG_8192C("==> %s \n",__FUNCTION__);
