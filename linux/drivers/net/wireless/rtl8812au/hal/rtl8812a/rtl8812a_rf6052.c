 ...
 *---------------------------------------------------------------------------*/
VOID PHY_RF6052SetBandwidth8812(
	IN	PADAPTER				Adapter,
...
VOID PHY_RF6052SetCckTxPower8812(
	IN	PADAPTER	Adapter,
	IN	u8			*pPowerlevel)
{
	HAL_DATA_TYPE		*pHalData = GET_HAL_DATA(Adapter);
...
VOID PHY_RF6052SetOFDMTxPower8812(
	IN	PADAPTER	Adapter,
	IN	u8		*pPowerLevelOFDM,
	IN	u8		*pPowerLevelBW20,
	IN	u8		*pPowerLevelBW40,
	IN	u8		Channel)
{
	...
	getPowerBase8812(Adapter, pPowerLevelOFDM, pPowerLevelBW20,
...
static int phy_RF6052_Config_ParaFile_8812(
	IN	PADAPTER		Adapter
)
{
	u8					eRFPath;
	...
	char					*pszRadioAFile = NULL, *pszRadioBFile = NULL,
						 *pszTxPwrTrack = NULL;
	...
	for (eRFPath = 0; eRFPath < pHalData->NumTotalRFPath; eRFPath++) {
		...
		case RF_PATH_A:
			...
			if (HAL_STATUS_FAILURE == ODM_ConfigRFWithHeaderFile(&pHalData->odmpriv,
					CONFIG_RF_RADIO, (ODM_RF_RADIO_PATH_E)eRFPath))
				rtStatus = _FAIL;
#else
