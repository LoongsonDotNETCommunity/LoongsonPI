...
)
{
	...
	u32 uLedBlinkNoLinkInterval =
		LED_BLINK_NO_LINK_INTERVAL_ALPHA; //add by ylb 20121012 for customer led for alpha
	...
	case LED_BLINK_SLOWLY:
		...
		_set_timer(&(pLed->BlinkTimer),
			   uLedBlinkNoLinkInterval);//change by ylb 20121012 for customer led for alpha
		break;
...
)
{
	...
	if (pLed->BlinkingLedState == RTW_LED_ON) {
		SwLedOn(Adapter, pLed);
		RT_TRACE(_module_rtl8712_led_c_, _drv_info_,
			 ("Blinktimes blink8(%d): turn on\n", pLed->BlinkTimes));
	} else {
		SwLedOff(Adapter, pLed);
		RT_TRACE(_module_rtl8712_led_c_, _drv_info_,
			 ("Blinktimes blink8(%d): turn off\n", pLed->BlinkTimes));
	}
	...
	case LED_BLINK_ALWAYS_ON:
		...
		if (bStopBlinking) {
			...
			if (Adapter->pwrctrlpriv.rf_pwrstate != rf_on
				...
				if (IS_HARDWARE_TYPE_8192DU(Adapter) || IS_HARDWARE_TYPE_8723AU(Adapter)
				    || IS_HARDWARE_TYPE_8812AU(Adapter)) {
					pLed->BlinkingLedState = RTW_LED_ON;
				} else {
					if (pLed->bLedOn)
						pLed->BlinkingLedState = RTW_LED_OFF;
...
)
{
		...
		if (bStopBlinking) {
			...
			if (Adapter->pwrctrlpriv.rf_pwrstate != rf_on
				...
				if (pLed->bLedOn) {
					...
					pLed->BlinkingLedState = RTW_LED_ON;
					_set_timer(&(pLed->BlinkTimer),
						   LED_BLINK_LINK_SLOWLY_INTERVAL_NETGEAR + LED_BLINK_LINK_INTERVAL_NETGEAR);
				}
			}
		}
		break;
	...
	case LED_BLINK_WPS:
		if (pLed->bLedOn) {
			...
			pLed->BlinkingLedState = RTW_LED_ON;
			_set_timer(&(pLed->BlinkTimer),
				   LED_BLINK_NORMAL_INTERVAL + LED_BLINK_LINK_INTERVAL_NETGEAR);
		}
		break;
...
)
{
	...
	RT_TRACE(_module_rtl8712_led_c_, _drv_info_,
		 ("!!! SwLedBlink13 CurrLedState %d, bLedWPSBlinkInProgress %d, bLedBlinkInProgress %d\n",
...
)
{
	...
	case LED_CTL_SITE_SURVEY:
		if ((pmlmepriv->LinkDetectInfo.bBusyTraffic)
			...
			if (padapter->pwrctrlpriv.rf_pwrstate != rf_on
			    && padapter->pwrctrlpriv.rfoff_reason == RF_CHANGE_BY_IPS)
...
)
{
	...
	RT_TRACE(_module_rtl8712_led_c_, _drv_info_,
		 ("LEd control mode 7 CurrLedState %d\n", pLed->CurrLedState));
}

void SwLedControlMode8(
	PADAPTER			Adapter,
	LED_CTL_MODE		LedAction
)
{
...
)
{
	...
	RT_TRACE(_module_rtl8712_led_c_, _drv_info_,
		 ("Led control mode 10 Led %d\n", pLed->CurrLedState));
}

//Edimax-ASUS, added by Page, 20121221
void SwLedControlMode11(
	PADAPTER			Adapter,
	LED_CTL_MODE		LedAction
)
{
	...
	RT_TRACE(_module_rtl8712_led_c_, _drv_info_,
		 ("Led mode 1 CurrLedState %d\n", pLed->CurrLedState));
}

// page added for NEC

VOID SwLedControlMode12(
	PADAPTER			Adapter,
	LED_CTL_MODE		LedAction
)
{
...
)
{
	...
	RT_TRACE(_module_rtl8712_led_c_, _drv_info_,
		 ("Led control mode 13 CurrLedState %d, LedAction %d\n", pLed->CurrLedState,
		  LedAction));
	switch (LedAction) {
	case LED_CTL_LINK:
		if (pLed->bLedWPSBlinkInProgress) {
...
void ResetLedStatus(PLED_USB pLed)
{
	...
	pLed->BlinkingLedState =
		LED_UNKNOWN; // Next state for blinking, either RTW_LED_ON or RTW_LED_OFF are.
