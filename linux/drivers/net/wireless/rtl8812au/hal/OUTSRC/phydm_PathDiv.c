...
)
{
	PDM_ODM_T		pDM_Odm = (PDM_ODM_T)pDM_VOID;
	pPATHDIV_T		pDM_PathDiv = &pDM_Odm->DM_PathDiv;
	u4Byte	rssi_avg_a = 0, rssi_avg_b = 0, rssi_avg_c = 0, rssi_avg_d = 0,
		rssi_avg_bcd = 0;
	u4Byte	rssi_total_a = 0, rssi_total_b = 0, rssi_total_c = 0,
		rssi_total_d = 0;

	//2 Default Path Selection By RSSI

	rssi_avg_a = (pDM_PathDiv->path_a_cnt_all > 0) ?
		     (pDM_PathDiv->path_a_sum_all / pDM_PathDiv->path_a_cnt_all) : 0 ;
	rssi_avg_b = (pDM_PathDiv->path_b_cnt_all > 0) ?
		     (pDM_PathDiv->path_b_sum_all / pDM_PathDiv->path_b_cnt_all) : 0 ;
	rssi_avg_c = (pDM_PathDiv->path_c_cnt_all > 0) ?
		     (pDM_PathDiv->path_c_sum_all / pDM_PathDiv->path_c_cnt_all) : 0 ;
	rssi_avg_d = (pDM_PathDiv->path_d_cnt_all > 0) ?
		     (pDM_PathDiv->path_d_sum_all / pDM_PathDiv->path_d_cnt_all) : 0 ;
...
)
{
	...
	memcpy(&(pDM_PathDiv->search_space_2[0]), &(search_space_2[0]),
	       NUM_CHOOSE2_FROM4);
	memcpy(&(pDM_PathDiv->search_space_3[0]), &(search_space_3[0]),
	       NUM_CHOOSE3_FROM4);
	...
	if (pPktinfo->bPacketToSelf || pPktinfo->bPacketMatchBSSID) {
		if (pPktinfo->DataRate > ODM_RATE11M) {
			if (pDM_PathDiv->path_div_type == PHYDM_4R_PATH_DIV) {
				...
				pDM_PathDiv->PathA_Sum[pPktinfo->StationID] +=
					pPhyInfo->RxMIMOSignalStrength[0];
				pDM_PathDiv->PathA_Cnt[pPktinfo->StationID]++;

				pDM_PathDiv->PathB_Sum[pPktinfo->StationID] +=
					pPhyInfo->RxMIMOSignalStrength[1];
...
)
{
	...
	if ((!(pHalData->CVID_Version == VERSION_1_BEFORE_8703B
	       && IS_92C_SERIAL(pHalData->VersionID))) || (pHalData->PathDivCfg != 1)
	...
	if (pDM_PDTable->PathDiv_NoLink_State == 0) {
		...
		for (index = 0; index < Adapter->MgntInfo.tmpNumBssDesc; index++) {
			...
			if (pTmpBssDesc->RecvSignalPower > pTestBssDesc->RecvSignalPower) {
				...
				RT_PRINT_STR(COMP_INIT, DBG_LOUD, "SSID: ", pTestBssDesc->bdSsIdBuf,
...
u1Byte odm_SwAntDivSelectScanChnl(
	IN	PADAPTER	Adapter
)
{
#if (RT_MEM_SIZE_LEVEL != RT_MEM_SIZE_MINIMUM)
	...
	if (EachChannelSTAs[ScanChannel] == 0) {
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, DBG_LOUD,
			     ("odm_SwAntDivSelectScanChnl(): Scan List is empty.\n"));
		return 0;
	}
...
VOID odm_SwAntDivConstructScanChnl(
	IN	PADAPTER	Adapter,
	IN	u1Byte		ScanChnl
)
{
	...
	if (ScanChnl == 0) {
		...
		RT_TRACE_F(COMP_SCAN, DBG_TRACE,
			   (" RT_CHNL_LIST_ACTION_CONSTRUCT chnl %d \n", ScanChnl));
