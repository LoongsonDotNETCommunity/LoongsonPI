...
s2Byte odm_InbandNoise_Monitor_NSeries(PDM_ODM_T	pDM_Odm, u8 bPauseDIG,
				       u8 IGIValue, u32 max_time)
{
	u4Byte				tmp4b;
	...
	while (1) {
		...
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_COMMON, ODM_DBG_LOUD,
			     ("value_a = 0x%x(%d), value_b = 0x%x(%d)\n",
			      noise_data.value[ODM_RF_PATH_A], noise_data.value[ODM_RF_PATH_A],
			      noise_data.value[ODM_RF_PATH_B], noise_data.value[ODM_RF_PATH_B]));
		...
		if ((valid_done == max_rf_path)
		    || (ODM_GetProgressingTime(pDM_Odm, start) > max_time)) {
			for (rf_path = ODM_RF_PATH_A; rf_path < max_rf_path; rf_path++) {
	...
	pDM_Odm->noise_level.noise[ODM_RF_PATH_A] = -110 + reg_c50 +
			noise_data.sum[ODM_RF_PATH_A];
	...
	if (max_rf_path == 2) {
		...
		pDM_Odm->noise_level.noise[ODM_RF_PATH_B] = -110 + reg_c58 +
				noise_data.sum[ODM_RF_PATH_B];
		pDM_Odm->noise_level.noise_all +=
...
s2Byte ODM_InbandNoise_Monitor(PVOID pDM_VOID, u8 bPauseDIG, u8 IGIValue,
			       u32 max_time)
{
	...
	if (pDM_Odm->SupportICType & ODM_IC_11AC_SERIES) {
		...
		return odm_InbandNoise_Monitor_NSeries(pDM_VOID, bPauseDIG, IGIValue,
						       max_time);
	}
}
