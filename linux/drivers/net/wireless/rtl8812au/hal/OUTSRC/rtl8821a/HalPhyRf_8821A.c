...
)
{
	...
	if ((IqkResult_X != 0) && (*(pDM_Odm->pBandType) == ODM_BAND_2_4G)) {
		...
		switch (RFPath) {
		case ODM_RF_PATH_A:
			ODM_SetBBReg(pDM_Odm, rOFDM0_XATxIQImbalance, bMaskDWord,
	...
	ODM_RT_TRACE(pDM_Odm, ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD,
		     ("TxPwrTracking path B: X = 0x%x, Y = 0x%x ele_A = 0x%x ele_C = 0x%x ele_D = 0x%x 0xeb4 = 0x%x 0xebc = 0x%x\n",
		      (u4Byte)IqkResult_X, (u4Byte)IqkResult_Y, (u4Byte)ele_A, (u4Byte)ele_C,
		      (u4Byte)ele_D, (u4Byte)IqkResult_X, (u4Byte)IqkResult_Y));
}
...
VOID ODM_TxPwrTrackSetPwr8821A(
	PDM_ODM_T			pDM_Odm,
		...
		case ODM_RF_PATH_A: {
			...
			if (VDF_enable == 1) {
				...
				for (k = 0; k <= 2; k++) {
					...
					case 1: {
						ODM_Write4Byte(pDM_Odm, 0xc80,
							       0x08008c38);// TX_Tone_idx[9:0], TxK_Mask[29] TX_Tone = 16
						ODM_Write4Byte(pDM_Odm, 0xc84,
							       0x28008c38);// RX_Tone_idx[9:0], RxK_Mask[29]
						ODM_SetBBReg(pDM_Odm, 0xce8, BIT(30), 0x0);
					}
					break;
					case 2: {
					...
					ODM_SetBBReg(pDM_Odm, 0x978, 0x03FF8000,
						     (TX_X0_RXK[cal]) >> 21 & 0x000007ff);
					ODM_SetBBReg(pDM_Odm, 0x978, 0x000007FF,
...
VOID phy_IQCalibrate_By_FW_8821A(
...
)
{
	...
	_IQK_BackupMacBB_8821A(pDM_Odm, MACBB_backup, Backup_MACBB_REG,
			       MACBB_REG_NUM);
	_IQK_BackupAFE_8821A(pDM_Odm, AFE_backup, Backup_AFE_REG, AFE_REG_NUM);
	_IQK_BackupRF_8821A(pDM_Odm, RFA_backup, RFB_backup, Backup_RF_REG,
			    RF_REG_NUM);

	_IQK_ConfigureMAC_8821A(pDM_Odm);
	_IQK_Tx_8821A(pDM_Odm, ODM_RF_PATH_A);
	_IQK_RestoreRF_8821A(pDM_Odm, ODM_RF_PATH_A, Backup_RF_REG, RFA_backup,
			     RF_REG_NUM);

	_IQK_RestoreAFE_8821A(pDM_Odm, AFE_backup, Backup_AFE_REG, AFE_REG_NUM);
	_IQK_RestoreMacBB_8821A(pDM_Odm, MACBB_backup, Backup_MACBB_REG,
				MACBB_REG_NUM);
