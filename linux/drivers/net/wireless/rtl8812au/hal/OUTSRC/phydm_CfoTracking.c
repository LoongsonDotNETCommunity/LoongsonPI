...
VOID odm_SetCrystalCap(
	IN		PVOID					pDM_VOID,
	IN		u1Byte					CrystalCap
)
{
	PDM_ODM_T					pDM_Odm = (PDM_ODM_T)pDM_VOID;
	PCFO_TRACKING				pCfoTrack = (PCFO_TRACKING)PhyDM_Get_Structure(pDM_Odm,
			PHYDM_CFOTRACK);
	...
	if (pDM_Odm->SupportICType & ODM_RTL8192D) {
		...
		ODM_SetBBReg(pDM_Odm, REG_AFE_PLL_CTRL, 0xF0000000,
			     ((CrystalCap & 0xF0) >> 4));
	} else if (pDM_Odm->SupportICType & ODM_RTL8188E) {
		// write 0x24[22:17] = 0x24[16:11] = CrystalCap
		CrystalCap = CrystalCap & 0x3F;
		ODM_SetBBReg(pDM_Odm, REG_AFE_XTAL_CTRL, 0x007ff800,
			     (CrystalCap | (CrystalCap << 6)));
	} else if (pDM_Odm->SupportICType & ODM_RTL8812) {
		// write 0x2C[30:25] = 0x2C[24:19] = CrystalCap
		CrystalCap = CrystalCap & 0x3F;
		ODM_SetBBReg(pDM_Odm, REG_MAC_PHY_CTRL, 0x7FF80000,
			     (CrystalCap | (CrystalCap << 6)));
		...
		CrystalCap = CrystalCap & 0x3F;
		ODM_SetBBReg(pDM_Odm, REG_MAC_PHY_CTRL, 0x00FFF000,
			     (CrystalCap | (CrystalCap << 6)));
		...
		CrystalCap = CrystalCap & 0x3F;
		ODM_SetBBReg(pDM_Odm, REG_MAC_PHY_CTRL, 0x07FF8000,
			     (CrystalCap | (CrystalCap << 6)));
	} else {
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_CFO_TRACKING, ODM_DBG_LOUD,
			     ("odm_SetCrystalCap(): Use default setting.\n"));
		ODM_SetBBReg(pDM_Odm, REG_MAC_PHY_CTRL, 0xFFF000,
			     (CrystalCap | (CrystalCap << 6)));
	}
...
u1Byte odm_GetDefaultCrytaltalCap(
	IN		PVOID					pDM_VOID
)
{
	PDM_ODM_T					pDM_Odm = (PDM_ODM_T)pDM_VOID;
...
)
{
	...
	ODM_SetBBReg(pDM_Odm, ODM_REG(BB_ATC, pDM_Odm), ODM_BIT(BB_ATC, pDM_Odm),
		     ATCStatus);
	pCfoTrack->bATCStatus = ATCStatus;
}

BOOLEAN odm_GetATCStatus(
	IN		PVOID					pDM_VOID
)
{
	BOOLEAN						ATCStatus;
	...
	ATCStatus = (BOOLEAN)ODM_GetBBReg(pDM_Odm, ODM_REG(BB_ATC, pDM_Odm),
					  ODM_BIT(BB_ATC, pDM_Odm));
	return ATCStatus;
}

VOID ODM_CfoTrackingReset(
	IN		PVOID					pDM_VOID
)
{
	PDM_ODM_T					pDM_Odm = (PDM_ODM_T)pDM_VOID;
	...
	if (pCfoTrack->CrystalCap > pCfoTrack->DefXCap) {
		for (CrystalCap = pCfoTrack->CrystalCap; CrystalCap >= pCfoTrack->DefXCap;
		     CrystalCap--)
...
)
{
	...
	pCfoTrack->DefXCap = pCfoTrack->CrystalCap = odm_GetDefaultCrytaltalCap(
				     pDM_Odm);
...
)
{
	...
	if (!pDM_Odm->bLinked || !pDM_Odm->bOneEntryOnly) {
		...
		CFO_ave_diff = (pCfoTrack->CFO_ave_pre >= CFO_ave) ?
			       (pCfoTrack->CFO_ave_pre - CFO_ave) : (CFO_ave - pCfoTrack->CFO_ave_pre);
		if (CFO_ave_diff > 20 && pCfoTrack->largeCFOHit == 0
		    && !pCfoTrack->bAdjust) {
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_CFO_TRACKING, ODM_DBG_LOUD,
				     ("ODM_CfoTracking(): first large CFO hit\n"));
