...
u1Byte odm_QueryRxPwrPercentage(
	IN		s1Byte		AntPower
)
{
...
//
s4Byte odm_SignalScaleMapping_92CSeries_patch_RT_CID_819x_Lenovo(
...
s4Byte odm_SignalScaleMapping_92CSeries_patch_RT_CID_819x_Netcore(
	IN OUT PDM_ODM_T pDM_Odm,
	s4Byte CurrSig
)
{
...
s4Byte odm_SignalScaleMapping_92CSeries(
	IN OUT PDM_ODM_T pDM_Odm,
	IN s4Byte CurrSig
)
{
...
)
{
	if ((pDM_Odm->SupportPlatform == ODM_WIN) &&
		...
		return odm_SignalScaleMapping_92CSeries_patch_RT_CID_819x_Netcore(pDM_Odm,
				CurrSig);
		...
		return odm_SignalScaleMapping_92CSeries_patch_RT_CID_819x_Lenovo(pDM_Odm,
				CurrSig);
	} else {
...
)
{
	...
	if (isCCKrate) {
		...
		if (IS_HARDWARE_TYPE_8723AE(pDM_Odm->Adapter)) {
			...
			RT_TRACE(COMP_DBG, DBG_WARNING,
				 ("odm_SQ_process_patch_RT_CID_819x_Lenovo\n"));
	...
	RT_TRACE(COMP_DBG, DBG_TRACE,
		 ("isCCKrate(%#d), PWDB_ALL(%#d), RSSI(%#d), SQ(%#d)\n", isCCKrate,
		  PWDB_ALL, RSSI, SQ));
...
static u1Byte odm_EVMdbToPercentage(
	IN		s1Byte Value
)
{
	//
...
static u1Byte odm_EVMdbm_JaguarSeries(
	IN  s1Byte Value
)
{
...
VOID odm_RxPhyStatus92CSeries_Parsing(
	IN OUT	PDM_ODM_T					pDM_Odm,
...
)
{
	...
	if (isCCKrate) {
		...
		if (pPktinfo->bPacketMatchBSSID) {
			...
			if ((pDM_Odm->SupportPlatform == ODM_WIN) &&
				...
				SQ = odm_SQ_process_patch_RT_CID_819x_Lenovo(pDM_Odm, isCCKrate, PWDB_ALL,
						0, 0);
		...
		for (i = ODM_RF_PATH_A; i < ODM_RF_PATH_MAX; i++) {
			...
			pPhyInfo->RxSNR[i] = pDM_Odm->PhyDbgInfo.RxSNRdB[i] = (s4Byte)(
						     pPhyStaRpt->path_rxsnr[i] / 2);
#endif
			...
			if (pPktinfo->bPacketMatchBSSID) {
				if ((pDM_Odm->SupportPlatform == ODM_WIN) &&
				    (pDM_Odm->PatchID == RT_CID_819x_Lenovo)) {
					if (i == ODM_RF_PATH_A)
						pPhyInfo->SignalQuality = odm_SQ_process_patch_RT_CID_819x_Lenovo(pDM_Odm,
									  isCCKrate, PWDB_ALL, i, RSSI);
		...
		// (2)PWDB, Average PWDB cacluated by hardware (for rate adaptive)
		//
		rx_pwr_all = (((pPhyStaRpt->cck_sig_qual_ofdm_pwdb_all) >> 1) & 0x7f) -
			     110;
		...
		if ((pDM_Odm->SupportPlatform == ODM_WIN) && (pDM_Odm->PatchID == 19)) {
			...
			// (3)EVM of HT rate
			//
			if (pPktinfo->DataRate >= DESC92C_RATEMCS8
			    && pPktinfo->DataRate <= DESC92C_RATEMCS15)
	...
	if (isCCKrate) {
		...
		pPhyInfo->SignalStrength = (u1Byte)(SignalScaleMapping(pDM_Odm->Adapter,
						    PWDB_ALL));//PWDB_ALL;
#else
		pPhyInfo->SignalStrength = (u1Byte)(odm_SignalScaleMapping(pDM_Odm,
						    PWDB_ALL));//PWDB_ALL;
#endif
	} else {
		if (rf_rx_num != 0) {
...
VOID odm_RxPhyStatusJaguarSeries_Parsing(
	IN OUT	PDM_ODM_T					pDM_Odm,
	OUT		PODM_PHY_INFO_T			pPhyInfo,
	IN 		pu1Byte						pPhyStatus,
	IN		PODM_PACKET_INFO_T			pPktinfo
...
#if (DM_ODM_SUPPORT_TYPE &  (ODM_WIN|ODM_CE|ODM_AP|ODM_ADSL))
			//Get Rx snr value in DB
			pPhyInfo->RxSNR[i] = pDM_Odm->PhyDbgInfo.RxSNRdB[i] = pPhyStaRpt->rxsnr[i]
					     / 2;
#endif
	...
	if (pDM_Odm->SupportICType == ODM_RTL8188E) {
		...
		if (pDM_Odm->AntDivType == CG_TRX_SMART_ANTDIV) {
			...
			if (pPktinfo->bPacketToSelf || pPktinfo->bPacketMatchBSSID) {
				...
				ODM_AntselStatistics_88E(pDM_Odm, antsel_tr_mux, pPktinfo->StationID,
							 pPhyInfo->RxPWDBAll);
			}
		}
	...
	if (pDM_Odm->SupportICType == ODM_RTL8821) {
		...
		if (pPktinfo->bPacketToSelf || pPktinfo->bPacketMatchBSSID) {
			if (pPktinfo->DataRate > DESC8812_RATE11M)
				ODM_AntselStatistics_8821A(pDM_Odm, pDM_FatTable->antsel_rx_keep_0,
	...
	if (pDM_Odm->SupportICType == ODM_RTL8723B) { //not CCK rate
		...
		if (pPktinfo->bPacketToSelf || pPktinfo->bPacketMatchBSSID) {
			if (pPktinfo->DataRate > DESC92C_RATE11M)
				ODM_AntselStatistics_8723B(pDM_Odm, pDM_FatTable->antsel_rx_keep_0,
							   pPktinfo->StationID, pPhyInfo->RxPWDBAll);
			else //CCK rate
				ODM_AntselStatistics_8723B(pDM_Odm, pDM_FatTable->antsel_rx_keep_0,
							   pPktinfo->StationID, pPhyInfo->RxPWDBAll);
	...
	if (pPktinfo->bPacketToSelf || pPktinfo->bPacketBeacon) {
		...
		if (!isCCKrate) { //ofdm rate
			if (pPhyInfo->RxMIMOSignalStrength[ODM_RF_PATH_B] == 0) {
				...
				if (pPhyInfo->RxMIMOSignalStrength[ODM_RF_PATH_A] >
		...
		//if(pEntry)
		{
			...
			if (pEntry->rssi_stat.ValidBit == 64) {
				...
				UndecoratedSmoothedPWDB = (Weighting * UndecoratedSmoothedOFDM +
							   (64 - Weighting) * UndecoratedSmoothedCCK) >> 6;
			} else {
				if (pEntry->rssi_stat.ValidBit != 0)
					UndecoratedSmoothedPWDB = (OFDM_pkt * UndecoratedSmoothedOFDM +
								   (pEntry->rssi_stat.ValidBit - OFDM_pkt) * UndecoratedSmoothedCCK) /
								  pEntry->rssi_stat.ValidBit;
				else
...
//
VOID ODM_PhyStatusQuery_92CSeries(
	IN OUT	PDM_ODM_T					pDM_Odm,
	OUT		PODM_PHY_INFO_T				pPhyInfo,
	IN 		pu1Byte						pPhyStatus,
	IN		PODM_PACKET_INFO_T			pPktinfo
...
// Endianness before calling this API
//
VOID ODM_PhyStatusQuery_JaguarSeries(
	IN OUT	PDM_ODM_T					pDM_Odm,
	OUT		PODM_PHY_INFO_T			pPhyInfo,
	IN 		pu1Byte						pPhyStatus,
	IN		PODM_PACKET_INFO_T			pPktinfo
...
HAL_STATUS ODM_ConfigRFWithTxPwrTrackHeaderFile(
	IN 	PDM_ODM_T	        	pDM_Odm
)
{
	ODM_RT_TRACE(pDM_Odm, ODM_COMP_INIT, ODM_DBG_LOUD,
...
)
{
	...
	if (pDM_Odm->SupportICType == ODM_RTL8812) {
		...
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_INIT, ODM_DBG_LOUD,
			     (" ===> phy_ConfigBBWithHeaderFile() agc:Rtl8812PHY_REGArray\n"));
	}
#endif
	...
	if (pDM_Odm->SupportICType == ODM_RTL8821) {
		...
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_INIT, ODM_DBG_LOUD,
			     (" ===> phy_ConfigBBWithHeaderFile() agc:Rtl8821PHY_REGArray\n"));
	}
#endif
#if (RTL8723B_SUPPORT == 1)
	if (pDM_Odm->SupportICType == ODM_RTL8723B) {
...
HAL_STATUS ODM_ConfigMACWithHeaderFile(
	IN 	PDM_ODM_T	pDM_Odm
)
{
