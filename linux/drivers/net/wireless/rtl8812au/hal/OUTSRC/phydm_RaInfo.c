...
VOID ODM_C2HRaParaReportHandler(
	IN	PVOID	pDM_VOID,
	IN pu1Byte   CmdBuf,
	IN u1Byte   CmdLen
)
{
	...
	ODM_RT_TRACE(pDM_Odm, PHYDM_COMP_RA_DBG, ODM_DBG_LOUD,
		     ("[ From FW C2H RA Para ]  CmdBuf[0]= (( %d ))\n", CmdBuf[0]));

	if (para_idx == RADBG_RTY_PENALTY) {
		ODM_RT_TRACE(pDM_Odm, PHYDM_COMP_RA_DBG, ODM_DBG_LOUD,
			     (" |Rate Index|   |RTY Penality Index| \n"));
		...
		ODM_RT_TRACE(pDM_Odm, PHYDM_COMP_RA_DBG, ODM_DBG_LOUD,
			     (" |Rate Index|   |Rate Down RTY Ratio| \n"));
...
VOID odm_RA_ParaAdjust_Send_H2C(
	IN	PVOID	pDM_VOID
)
{
	...
	if (pRA_Table->RA_Para_feedback_req) { //H2C_Parameter[5]=1 ; ask FW for all RA parameters
		...
		ODM_RT_TRACE(pDM_Odm, PHYDM_COMP_RA_DBG, ODM_DBG_LOUD,
			     ("[H2C] Send H2C to FW for modifying RA parameter \n"));

		H2C_Parameter[1] =  pRA_Table->para_idx;
		H2C_Parameter[2] =  pRA_Table->rate_idx;
		//1 [8 bit]
		if (pRA_Table->para_idx == RADBG_RTY_PENALTY
		    || pRA_Table->para_idx == RADBG_RATE_UP_RTY_RATIO
		    || pRA_Table->para_idx == RADBG_RATE_DOWN_RTY_RATIO) {
	...
	ODM_RT_TRACE(pDM_Odm, PHYDM_COMP_RA_DBG, ODM_DBG_LOUD,
		     (" H2C_Parameter[1] = 0x%x  \n", H2C_Parameter[1]));
	ODM_RT_TRACE(pDM_Odm, PHYDM_COMP_RA_DBG, ODM_DBG_LOUD,
		     (" H2C_Parameter[2] = 0x%x  \n", H2C_Parameter[2]));
	ODM_RT_TRACE(pDM_Odm, PHYDM_COMP_RA_DBG, ODM_DBG_LOUD,
		     (" H2C_Parameter[3] = 0x%x  \n", H2C_Parameter[3]));
	ODM_RT_TRACE(pDM_Odm, PHYDM_COMP_RA_DBG, ODM_DBG_LOUD,
		     (" H2C_Parameter[4] = 0x%x  \n", H2C_Parameter[4]));
	ODM_RT_TRACE(pDM_Odm, PHYDM_COMP_RA_DBG, ODM_DBG_LOUD,
		     (" H2C_Parameter[5] = 0x%x  \n", H2C_Parameter[5]));
...
)
{
	...
	ODM_RT_TRACE(pDM_Odm, PHYDM_COMP_RA_DBG, ODM_DBG_LOUD,
		     (" |Rate index| |Current-value| |Default-value| |Modify?| \n"));
	for (i = 0 ; i <= (pRA_Table->rate_length); i++) {
#if(DM_ODM_SUPPORT_TYPE & (ODM_WIN))
		ODM_RT_TRACE(pDM_Odm, PHYDM_COMP_RA_DBG, ODM_DBG_LOUD,
			     ("     [ %d ]  %20d  %25d  %20s \n", i, value[i], value_default[i],
			      ((modify_note[i] == 1) ? "V" : " .  ")));
#else
		ODM_RT_TRACE(pDM_Odm, PHYDM_COMP_RA_DBG, ODM_DBG_LOUD,
			     ("     [ %d ]  %10d  %14d  %14s \n", i, value[i], value_default[i],
	...
	*/
	...
	if (pDM_Odm->SupportICType == ODM_RTL8192E)
		...
		pRA_Table->rate_length = ODM_RATEMCS7;
		 ...
		 || (pDM_Odm->SupportICType == ODM_RTL8881A))
...
VOID odm_Set_RA_DM_ARFB_by_Noisy(
...
u4Byte Set_RA_DM_Ratrbitmap_by_Noisy(
	IN	PVOID			pDM_VOID,
...
#if(DM_ODM_SUPPORT_TYPE & (ODM_WIN))
	pRA_Table->PT_collision_pre =
		TRUE;   //used in ODM_DynamicARFBSelect(WIN only)
#endif
#endif
}

VOID ODM_RAPostActionOnAssoc(
	IN	PVOID	pDM_VOID
)
{
	PDM_ODM_T	pDM_Odm = (PDM_ODM_T)pDM_VOID;
	...
	*/
	...
	while (pLoopAdapter) {
		...
		if (pLoopAdapter != NULL) {
			...
			curTxOkCnt = pLoopAdapter->TxStats.NumTxBytesUnicast -
				     pMgntInfo->lastTxOkCnt;
			curRxOkCnt = pLoopAdapter->RxStats.NumRxBytesUnicast -
				     pMgntInfo->lastRxOkCnt;
		...
		for (i = 0; i < ASSOCIATE_ENTRY_NUM; i++) {
			...
			if (pEntry != NULL) {
				if (pEntry->bAssociated) {
					...
					//2 BF_en
#if (BEAMFORMING_SUPPORT)
					Beamform_cap = Beamforming_GetEntryBeamCapByMacId(Adapter,
					...
					if ((IS_WIRELESS_MODE_AC(Adapter)
					     && TEST_FLAG(pEntry->VHTInfo.STBC, STBC_VHT_ENABLE_TX)) ||
	...
	if (pHalData->bUseRAMask) {
		...
		//2 STBC_en
		if ((IS_WIRELESS_MODE_AC(Adapter)
		     && TEST_FLAG(pVHTInfo->VhtCurStbc, STBC_VHT_ENABLE_TX)) ||
			...
			if (IS_STA_VALID(psta = pDM_Odm->pODM_StaInfo[i])) {
				...
				if (psta->rssi_stat.UndecoratedSmoothedPWDB != (-1)) {
					...
					if (pDM_Odm->SupportICType == ODM_RTL8192E
						...
						BEAMFORMING_CAP Beamform_cap = beamforming_get_entry_beam_cap_by_mac_id(
										       &Adapter->mlmepriv, psta->mac_id);
					...
					if (pDM_Odm->SupportICType == ODM_RTL8192D)
						...
						PWDB_rssi[sta_cnt++] = (((u8)(psta->mac_id & 0xFF)) | ((
									...
									(STBC_TX << 25) | (FirstConnect << 29) | (TxBF_EN << 30));
					else
						PWDB_rssi[sta_cnt++] = (psta->mac_id |
									(psta->rssi_stat.UndecoratedSmoothedPWDB << 16));
				}
...
)
{
	...
	for (i = 0; i < ODM_ASSOCIATE_ENTRY_NUM; i++) {
		...
		if (IS_STA_VALID(pstat)) {
			...
			//2 BF_en
#ifdef BEAMFORMING_SUPPORT
			BEAMFORMING_CAP Beamform_cap = Beamforming_GetEntryBeamCapByMacId(priv,
						       pstat->aid);
			...
			if ((Beamform_cap & BEAMFORMEE_CAP_HT_EXPLICIT)
			    || (Beamform_cap & BEAMFORMEE_CAP_VHT_SU)) {
...
)
{
	...
	if (pMgntInfo->mAssoc && (!ACTING_AS_AP(pAdapter))) {
		odm_RefreshLdpcRtsMP(pAdapter, pDM_Odm, pMgntInfo->mMacId,
				     pMgntInfo->IOTPeer, pHalData->UndecoratedSmoothedPWDB);
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_RA_MASK, ODM_DBG_LOUD,
		...
		if (ODM_RAStateCheck(pDM_Odm, pHalData->UndecoratedSmoothedPWDB,
			...
			pAdapter->HalFunc.UpdateHalRAMaskHandler(pAdapter, pMgntInfo->mMacId, NULL,
					pMgntInfo->Ratr_State);
		} else if (pDM_Odm->bChangeState) {
			ODM_PRINT_ADDR(pDM_Odm, ODM_COMP_RA_MASK, ODM_DBG_LOUD,
				       ("Target AP addr : "), pMgntInfo->Bssid);
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_RA_MASK, ODM_DBG_LOUD,
				     ("Change Power Training State, bDisablePowerTraining = %d\n",
	...
	if (pTargetAdapter != NULL) {
		...
		for (i = 0; i < ODM_ASSOCIATE_ENTRY_NUM; i++) {
			...
			if (NULL != pEntry) {
				if (pEntry->bAssociated) {
					odm_RefreshLdpcRtsMP(pAdapter, pDM_Odm, pEntry->AssociatedMacId,
							     pEntry->IOTPeer, pEntry->rssi_stat.UndecoratedSmoothedPWDB);
					...
					if (ODM_RAStateCheck(pDM_Odm, pEntry->rssi_stat.UndecoratedSmoothedPWDB,
						...
						ODM_RT_TRACE(pDM_Odm, ODM_COMP_RA_MASK, ODM_DBG_LOUD,
							     ("RSSI:%d, RSSI_LEVEL:%d\n", pEntry->rssi_stat.UndecoratedSmoothedPWDB,
							      pEntry->Ratr_State));
						pAdapter->HalFunc.UpdateHalRAMaskHandler(pTargetAdapter,
								pEntry->AssociatedMacId, pEntry, pEntry->Ratr_State);
					} else if (pDM_Odm->bChangeState) {
						ODM_RT_TRACE(pDM_Odm, ODM_COMP_RA_MASK, ODM_DBG_LOUD,
							     ("Change Power Training State, bDisablePowerTraining = %d\n",
							      pDM_Odm->bDisablePowerTraining));
...
#endif
					GET_HAL_INTERFACE(priv)->UpdateHalRAMaskHandler(priv, pstat,
							pstat->rssi_level);
				} else
#endif
