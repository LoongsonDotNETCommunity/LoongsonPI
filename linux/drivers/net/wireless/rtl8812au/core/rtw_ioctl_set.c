...
u8 rtw_do_join(_adapter *padapter)
{
	...
	if (_rtw_queue_empty(queue) == _TRUE) {
		...
		int select_ret;
		_exit_critical_bh(&(pmlmepriv->scanned_queue.lock), &irqL);
		if ((select_ret = rtw_select_and_join_from_scanned_queue(
					  pmlmepriv)) == _SUCCESS) {
			pmlmepriv->to_join = _FALSE;
			...
			if (check_fwstate(pmlmepriv, WIFI_ADHOC_STATE) == _TRUE) {
				...
				if ((check_fwstate(pmlmepriv, WIFI_STATION_STATE) == _TRUE)) {
					if (_rtw_memcmp(pmlmepriv->cur_network.network.Ssid.Ssid,
...
u8 rtw_set_802_11_reload_defaults(_adapter *padapter,
...
u8 set_802_11_test(_adapter *padapter, NDIS_802_11_TEST *test)
{
	...
	case 2:
		NdisMIndicateStatus(padapter->hndis_adapter,
				    NDIS_STATUS_MEDIA_SPECIFIC_INDICATION, (PVOID)&test->RssiTrigger,
...
u8 rtw_set_802_11_bssid(_adapter *padapter, u8 *bssid)
{
	...
	if (check_fwstate(pmlmepriv,
		...
		RT_TRACE(_module_rtl871x_ioctl_set_c_, _drv_info_,
			 ("set_bssid: _FW_LINKED||WIFI_ADHOC_MASTER_STATE\n"));

		if (_rtw_memcmp(&pmlmepriv->cur_network.network.MacAddress, bssid,
				ETH_ALEN) == _TRUE) {
...
u8 rtw_set_802_11_ssid(_adapter *padapter, NDIS_802_11_SSID *ssid)
{
	...
	if (check_fwstate(pmlmepriv,
		...
		if ((pmlmepriv->assoc_ssid.SsidLength == ssid->SsidLength) &&
		    (_rtw_memcmp(&pmlmepriv->assoc_ssid.Ssid, ssid->Ssid,
				 ssid->SsidLength) == _TRUE)) {
			if ((check_fwstate(pmlmepriv, WIFI_STATION_STATE) == _FALSE)) {
				RT_TRACE(_module_rtl871x_ioctl_set_c_, _drv_err_,
				      ...
				      NDIS_802_11_NETWORK_INFRASTRUCTURE networktype)
{
	...
	NDIS_802_11_NETWORK_INFRASTRUCTURE *pold_state = &
		...
		if ((*pold_state == Ndis802_11Infrastructure)
			...
			if (check_fwstate(pmlmepriv, _FW_LINKED) == _TRUE) {
				rtw_indicate_disconnect(
					padapter); //will clr Linked_state; before this function, we must have chked whether  issue dis-assoc_cmd or not
			}
		}
...
u8 rtw_set_802_11_authentication_mode(_adapter *padapter,
...
u8 rtw_set_802_11_add_wep(_adapter *padapter, NDIS_802_11_WEP *wep)
{
	...
	btransmitkey = (wep->KeyIndex & 0x80000000) > 0 ? _TRUE  :
		       _FALSE;	//for ???
	...
	default:
		...
		RT_TRACE(_module_rtl871x_ioctl_set_c_, _drv_info_,
			 ("MgntActrtw_set_802_11_add_wep:wep->KeyLength!=5 or 13\n"));
		break;
	}
	...
	_rtw_memcpy(&(psecuritypriv->dot11DefKey[keyid].skey[0]),
...
u8 rtw_set_802_11_add_key(_adapter *padapter, NDIS_802_11_KEY *key)
{
	...
	if (key->KeyIndex & 0x40000000) {
		...
		if ((check_fwstate(&padapter->mlmepriv, WIFI_ADHOC_STATE) == _TRUE)
		    && (IS_MAC_ADDRESS_BROADCAST(key->BSSID) == _FALSE)) {
			RT_TRACE(_module_rtl871x_ioctl_set_c_, _drv_err_,
		...
		} else if (encryptionalgo == _AES_ && (key->KeyLength != 16
						       && key->KeyLength != 32)) {
	...
	if ((padapter->securitypriv.dot11AuthAlgrthm != dot11AuthAlgrthm_8021X)
		...
		_rtw_memcpy(&(padapter->securitypriv.dot11DefKey[keyindex].skey[0]),
	...
	if (bgroup == _TRUE) { // Group transmit key
		...
		_rtw_memset(&padapter->securitypriv.dot118021XGrpKey[(u8)((
					key->KeyIndex) & 0x03)], 0, 16);
		_rtw_memset(&padapter->securitypriv.dot118021XGrptxmickey[(u8)((
					key->KeyIndex) & 0x03)], 0, 16);
		_rtw_memset(&padapter->securitypriv.dot118021XGrprxmickey[(u8)((
					key->KeyIndex) & 0x03)], 0, 16);

		if ((key->KeyIndex & 0x10000000)) {
			_rtw_memcpy(&padapter->securitypriv.dot118021XGrptxmickey[(u8)((
						key->KeyIndex) & 0x03)], key->KeyMaterial + 16, 8);
			_rtw_memcpy(&padapter->securitypriv.dot118021XGrprxmickey[(u8)((
						key->KeyIndex) & 0x03)], key->KeyMaterial + 24, 8);

			RT_TRACE(_module_rtl871x_ioctl_set_c_, _drv_err_,
				 ("\n rtw_set_802_11_add_key:rx mic :0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x\n",
				  padapter->securitypriv.dot118021XGrprxmickey[(u8)((key->KeyIndex) &
						  0x03)].skey[0], padapter->securitypriv.dot118021XGrprxmickey[(u8)((
								  key->KeyIndex - 1) & 0x03)].skey[1],
				  padapter->securitypriv.dot118021XGrprxmickey[(u8)((key->KeyIndex) &
						  0x03)].skey[2], padapter->securitypriv.dot118021XGrprxmickey[(u8)((
								  key->KeyIndex - 1) & 0x03)].skey[3],
				  padapter->securitypriv.dot118021XGrprxmickey[(u8)((key->KeyIndex) &
						  0x03)].skey[4], padapter->securitypriv.dot118021XGrprxmickey[(u8)((
								  key->KeyIndex - 1) & 0x03)].skey[5],
				  padapter->securitypriv.dot118021XGrprxmickey[(u8)((key->KeyIndex) &
						  0x03)].skey[6], padapter->securitypriv.dot118021XGrprxmickey[(u8)((
								  key->KeyIndex - 1) & 0x03)].skey[7]));
			RT_TRACE(_module_rtl871x_ioctl_set_c_, _drv_err_,
		...
		_rtw_memcpy(&padapter->securitypriv.dot118021XGrpKey[(u8)((
					key->KeyIndex) & 0x03)], key->KeyMaterial, key->KeyLength);
...
u8 rtw_set_802_11_remove_key(_adapter	*padapter,
	...
	if (IsSupportedTxHT(pmlmeext->cur_wireless_mode)) {
		...
		if (p && ht_ielen > 0) {
			...
			bw_40MHz = (pmlmeext->cur_bwmode
				    && (HT_INFO_HT_PARAM_REC_TRANS_CHNL_WIDTH & pmlmeinfo->HT_info.infos[0])) ?
				   1 : 0;

			//short_GI = (pht_capie->cap_info&(IEEE80211_HT_CAP_SGI_20|IEEE80211_HT_CAP_SGI_40)) ? 1:0;
			short_GI_20 = (pmlmeinfo->HT_caps.u.HT_cap_element.HT_caps_info &
				       IEEE80211_HT_CAP_SGI_20) ? 1 : 0;
			short_GI_40 = (pmlmeinfo->HT_caps.u.HT_cap_element.HT_caps_info &
		...
		max_rate = ((rtw_vht_data_rate(pvhtpriv->bwmode, pvhtpriv->sgi,
					       pvhtpriv->vht_highest_rate) + 1) >> 1) * 10;
	}
