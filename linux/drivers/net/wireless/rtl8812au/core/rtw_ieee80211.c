			      ...
			      int limit)
{
	...
	while (1) {
		...
		if (pbuf) {
			...
			if (_rtw_memcmp((pbuf + 2), wpa_oui_type,
					sizeof(wpa_oui_type)) == _FALSE) {
			...
			if (_SUCCESS == rtw_parse_wpa2_ie(pbuf, wpa_ielen + 2, &group_cipher,
				...
				RT_TRACE(_module_rtl871x_mlme_c_, _drv_info_,
					 ("%s: pnetwork->pairwise_cipher: %d,"
					  "pnetwork->group_cipher is %d, is_8021x is %d",	__func__,
