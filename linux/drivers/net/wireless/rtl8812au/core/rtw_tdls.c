...
extern s32 rtw_dump_xframe(_adapter *padapter,
...
void rtw_reset_tdls_info(_adapter *padapter)
{
	...
	ptdlsinfo->candidate_ch =
		1;	//when inplement channel switching, default candidate channel is 1
...
void issue_nulldata_to_TDLS_peer_STA(_adapter *padapter,
				     struct sta_info *ptdls_sta, unsigned int power_mode)
{
...
void rtw_tdls_set_mac_id(struct tdls_info *ptdlsinfo,
...
void rtw_tdls_set_key(_adapter *adapter,
		      struct rx_pkt_attrib *prx_pkt_attrib, struct sta_info *ptdls_sta)
{
...
void rtw_tdls_process_ht_cap(_adapter *adapter, struct sta_info *ptdls_sta,
			     u8 *data, u8 Length)
{
			...
			struct pkt_attrib *pattrib)
{
...

	{
		...
		padapter->HalFunc.GetHalDefVarHandler(padapter, HAL_DEF_RX_PACKET_OFFSET,
						      &rx_packet_offset);
		padapter->HalFunc.GetHalDefVarHandler(padapter, HAL_DEF_MAX_RECVBUF_SZ,
						      &max_recvbuf_sz);
	...
	padapter->HalFunc.GetHwRegHandler(padapter, HW_VAR_RF_TYPE,
					  (u8 *)(&rf_type));
	switch (rf_type) {
	case RF_1T1R:
	...
	return (rtw_set_ie(pframe, _HT_CAPABILITY_IE_,
			   sizeof(struct rtw_ieee80211_ht_cap), (unsigned char *)&ht_capie,
			   &(pattrib->pktlen)));
}

u8 *rtw_tdls_set_sup_ch(struct mlme_ext_priv *pmlmeext, u8 *pframe,
			struct pkt_attrib *pattrib)
{
	...
	return (rtw_set_ie(pframe, _SUPPORTED_CH_IE_, idx_5g, sup_ch,
			   &(pattrib->pktlen)));
}
			     ...
			     u8 length)
{
	...
	wfd_offset = 0;
	wfd_offset = rtw_get_wfd_ie(ptr + wfd_offset, length - wfd_offset, wfd_ie,
				    &wfd_ielen);
	while (wfd_offset) {
		...
		attr_contentlen = 0;
		rtw_get_wfd_attr_content(wfd_ie, wfd_ielen, WFD_ATTR_LOCAL_IP_ADDR,
					 attr_content, &attr_contentlen);
		if (attr_contentlen) {
...
void issue_tunneled_probe_req(_adapter *padapter)
{
	...
	if (rtw_xmit_tdls_coalesce(padapter, pmgntframe,
				   TUNNELED_PROBE_REQ) != _SUCCESS) {
...
void issue_tunneled_probe_rsp(_adapter *padapter,
			      union recv_frame *precv_frame)
{
	struct xmit_frame			*pmgntframe;
	struct pkt_attrib			*pattrib;
...
void issue_tdls_setup_req(_adapter *padapter, u8 *mac_addr)
{
	...
	u32 timeout_interval = TPK_RESEND_COUNT *
			       1000;	//retry timer should set at least 301 sec, using TPK_count counting 301 times.
...
void issue_tdls_dis_req(_adapter *padapter, u8 *mac_addr)
{
	...
	pattrib->qsel = pattrib->priority;
	if (rtw_xmit_tdls_coalesce(padapter, pmgntframe,
				   TDLS_DISCOVERY_REQUEST) != _SUCCESS) {
		rtw_free_xmitbuf(pxmitpriv, pmgntframe->pxmitbuf);
		rtw_free_xmitframe(pxmitpriv, pmgntframe);
		goto exit;
	}
...
void issue_tdls_setup_rsp(_adapter *padapter,
			  union recv_frame *precv_frame)
{
	struct xmit_frame			*pmgntframe;
	struct pkt_attrib			*pattrib;
...
void issue_tdls_setup_cfm(_adapter *padapter,
			  union recv_frame *precv_frame)
{
	struct tdls_info *ptdlsinfo = &padapter->tdlsinfo;
	struct xmit_frame			*pmgntframe;
...
void issue_tdls_peer_traffic_indication(_adapter *padapter,
					struct sta_info *ptdls_sta)
{
	struct xmit_frame			*pmgntframe;
	struct pkt_attrib			*pattrib;
	...
	pattrib->qsel = pattrib->priority;
	if (rtw_xmit_tdls_coalesce(padapter, pmgntframe,
				   TDLS_PEER_TRAFFIC_INDICATION) != _SUCCESS) {
		rtw_free_xmitbuf(pxmitpriv, pmgntframe->pxmitbuf);
		rtw_free_xmitframe(pxmitpriv, pmgntframe);
		goto exit;
	}
...
void issue_tdls_ch_switch_req(_adapter *padapter, u8 *mac_addr)
{
	...
	pattrib->qsel = pattrib->priority;
	if (rtw_xmit_tdls_coalesce(padapter, pmgntframe,
				   TDLS_CHANNEL_SWITCH_REQUEST) != _SUCCESS) {
		rtw_free_xmitbuf(pxmitpriv, pmgntframe->pxmitbuf);
		rtw_free_xmitframe(pxmitpriv, pmgntframe);
		goto exit;
	}
...
void issue_tdls_ch_switch_rsp(_adapter *padapter, u8 *mac_addr)
{
	...
	*/
	if (rtw_xmit_tdls_coalesce(padapter, pmgntframe,
				   TDLS_CHANNEL_SWITCH_RESPONSE) != _SUCCESS) {
		rtw_free_xmitbuf(pxmitpriv, pmgntframe->pxmitbuf);
		rtw_free_xmitframe(pxmitpriv, pmgntframe);
		goto exit;
	}
...
sint On_TDLS_Dis_Rsp(_adapter *adapter, union recv_frame *precv_frame)
{
	...
	if (ptdls_sta != NULL) {
		...
		adapter->HalFunc.GetHalDefVarHandler(adapter,
...
#ifdef CONFIG_WFD
		rtw_tdls_process_wfd_ie(ptdlsinfo, ptr + FIXED_IE,
					parsing_length - FIXED_IE);
#endif // CONFIG_WFD
...
sint On_TDLS_Setup_Cfm(_adapter *adapter, union recv_frame *precv_frame)
{
	...
	*ppairwise_cipher;
	...
	if (prx_pkt_attrib->encrypt) {
		...
		if (tdls_verify_mic(ptdls_sta->tpk.kck, 3, plinkid_ie, prsnie, ptimeout_ie,
				    pftie) == 0) {	//0: Invalid, 1: Valid
			free_tdls_sta(adapter, ptdls_sta);
			return _FAIL;
		}
	...
	for (j = FIXED_IE; j < parsing_length;) {
		...
		case _LINK_ID_IE_:
			...
			if ((MacAddr_isBcst(dst) == _FALSE)
			    && (_rtw_memcmp(myid(&(adapter->eeprompriv)), dst, 6) == _FALSE)) {
				goto exit;
			}
			break;
		default:
...
sint On_TDLS_Peer_Traffic_Rsp(_adapter *adapter,
			      union recv_frame *precv_frame)
{
	struct tdls_info *ptdlsinfo = &adapter->tdlsinfo;
	struct mlme_ext_priv *pmlmeext = &adapter->mlmeextpriv;
...
sint On_TDLS_Ch_Switch_Req(_adapter *adapter,
			   union recv_frame *precv_frame)
{
	struct sta_info *ptdls_sta = NULL;
	struct sta_priv *pstapriv = &adapter->stapriv;
	...
	if ((ptdls_sta->tdls_sta_state & TDLS_CH_SWITCH_ON_STATE)
	    && ptdls_sta->off_ch == pmlmeext->cur_channel) {
...
sint On_TDLS_Ch_Switch_Rsp(_adapter *adapter,
			   union recv_frame *precv_frame)
{
	struct sta_info *ptdls_sta = NULL;
	struct sta_priv *pstapriv = &adapter->stapriv;
...
void wfd_ie_tdls(_adapter *padapter, u8 *pframe, u32 *pktlen)
{
	...
	RTW_PUT_BE16(wfdie + wfdielen,
		     pwfd_info->wfd_device_type | WFD_DEVINFO_SESSION_AVAIL
				  ...
				  struct xmit_frame *pxmitframe, u8 *pframe)
{
	...
	struct sta_info *ptdls_sta = rtw_get_stainfo((&padapter->stapriv),
				     pattrib->dst);
	...
	pframe = rtw_set_fixed_ie(pframe, 1, &(ptdls_sta->dialog),
				  &(pattrib->pktlen));
	...
	//RSNIE
	if (pattrib->encrypt)
		pframe = rtw_set_ie(pframe, _RSN_IE_2_, 20, TDLS_RSNIE,
				    &(pattrib->pktlen));

	//extended capabilities
	pframe = rtw_set_ie(pframe, _EXT_CAP_IE_, 5, TDLS_EXT_CAPIE,
			    &(pattrib->pktlen));

	//QoS capability(WMM_IE)
	pframe = rtw_set_ie(pframe, _VENDOR_SPECIFIC_IE_, 7, TDLS_WMMIE,
			    &(pattrib->pktlen));
	...
	pframe = rtw_set_ie(pframe, _LINK_ID_IE_,  18, link_id_addr,
			    &(pattrib->pktlen));
				  ...
				  struct xmit_frame *pxmitframe, u8 *pframe)
{
	...
	pframe = rtw_set_fixed_ie(pframe, 1, &(category), &(pattrib->pktlen));
	pframe = rtw_set_fixed_ie(pframe, 1, &(action), &(pattrib->pktlen));
	pframe = rtw_set_fixed_ie(pframe, 2, (u8 *)&ptdls_sta->stat_code,
				  &(pattrib->pktlen));
				  ...
				  struct xmit_frame *pxmitframe, u8 *pframe)
{
	...
	if (pattrib->encrypt) {
		...
		_set_timer(&ptdls_sta->TPK_timer,
				 ...
				 struct xmit_frame *pxmitframe, u8 *pframe)
{
	...
	pframe = rtw_set_fixed_ie(pframe, 1, &(category), &(pattrib->pktlen));
	pframe = rtw_set_fixed_ie(pframe, 1, &(action), &(pattrib->pktlen));
	pframe = rtw_set_fixed_ie(pframe, 1, (u8 *)&ptdls_sta->stat_code,
				  &(pattrib->pktlen));
...
void rtw_build_tdls_dis_rsp_ies(_adapter *padapter,
				struct xmit_frame *pxmitframe, u8 *pframe, u8 dialog)
{
	struct mlme_ext_priv	*pmlmeext = &padapter->mlmeextpriv;
	struct mlme_ext_info	*pmlmeinfo = &pmlmeext->mlmext_info;
	struct pkt_attrib	*pattrib = &pxmitframe->attrib;
				      ...
				      struct xmit_frame *pxmitframe, u8 *pframe)
{
	...
	pframe = rtw_set_fixed_ie(pframe, 1, &(category), &(pattrib->pktlen));
	pframe = rtw_set_fixed_ie(pframe, 1, &(action), &(pattrib->pktlen));
	pframe = rtw_set_fixed_ie(pframe, 1, &(ptdlsinfo->candidate_ch),
				  &(pattrib->pktlen));
	...
	pframe = rtw_set_ie(pframe, _CH_SWITCH_TIMING_,  4, ch_switch_timing,
			    &(pattrib->pktlen));
...
{
	psta->padapter = padapter;
	_init_timer(&psta->option_timer, padapter->pnetdev, _ch_switch_timer_hdl,
		    psta);
}
...
{
	psta->padapter = padapter;
	_init_timer(&psta->base_ch_timer, padapter->pnetdev, _base_ch_timer_hdl,
		    psta);
}
...
{
	psta->padapter = padapter;
	_init_timer(&psta->off_ch_timer, padapter->pnetdev, _off_ch_timer_hdl,
		    psta);
}
...
{
	psta->padapter = padapter;
	_init_timer(&psta->handshake_timer, padapter->pnetdev,
		    _tdls_handshake_timer_hdl, psta);
}
...
{
	psta->padapter = padapter;
	_init_timer(&psta->alive_timer1, padapter->pnetdev,
		    _tdls_alive_timer_phase1_hdl, psta);
	_init_timer(&psta->alive_timer2, padapter->pnetdev,
		    _tdls_alive_timer_phase2_hdl, psta);
}
