...
u8 networktype_to_raid_ex(_adapter *adapter, unsigned char network_type)
{
	...
	case WIRELESS_11_24AC:
		if ((rf_type == RF_1T1R)
		    || (adapter->mlmepriv.vhtpriv.vht_highest_rate <= MGN_VHT1SS_MCS9))
...
unsigned char get_highest_mcs_rate(struct HT_caps_element *pHT_caps)
{
	...
	mcs_rate = (pHT_caps->u.HT_cap_element.MCS_rate[0] |
		    (pHT_caps->u.HT_cap_element.MCS_rate[1] << 8));
...
void update_capinfo(PADAPTER Adapter, u16 updateCap)
{
	...
	if (updateCap & cIBSS) {
		...
		} else if (pmlmeext->cur_wireless_mode & (WIRELESS_11_24N | WIRELESS_11A |
			...
			unsigned int value)
{
	...
	if ((pwriteMacPara = (struct reg_rw_parm *)rtw_malloc(sizeof(
				     struct reg_rw_parm))) == NULL) {
...
void update_bmc_sta_support_rate(_adapter *padapter, u32 mac_id)
{
	...
	if (pmlmeext->cur_wireless_mode & WIRELESS_11B) {
		...
		_rtw_memcpy((pmlmeinfo->FW_sta_info[mac_id].SupportedRates),
			    ...
			    uint var_ie_len, int cam_idx)
{
	...
	pIE = (PNDIS_802_11_VARIABLE_IEs)rtw_get_ie(pvar_ie, _SUPPORTEDRATES_IE_,
			&ie_len, var_ie_len);
	if (pIE == NULL) {
		return _FAIL;
	}

	_rtw_memcpy(pmlmeinfo->FW_sta_info[cam_idx].SupportedRates, pIE->data,
		    ie_len);
	supportRateNum = ie_len;

	pIE = (PNDIS_802_11_VARIABLE_IEs)rtw_get_ie(pvar_ie,
			_EXT_SUPPORTEDRATES_IE_, &ie_len, var_ie_len);
	if (pIE) {
		_rtw_memcpy((pmlmeinfo->FW_sta_info[cam_idx].SupportedRates +
...
void process_addba_req(_adapter *padapter, u8 *paddba_req, u8 *addr)
{
	...
	if (psta) {
		...
		preorder_ctrl->enable = (pmlmeinfo->bAcceptAddbaReq == _TRUE) ? _TRUE :
					_FALSE;
	}
