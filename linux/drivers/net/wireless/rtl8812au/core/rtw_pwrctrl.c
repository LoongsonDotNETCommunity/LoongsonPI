	...
	if ((pwrpriv->rf_pwrstate == rf_off) && (!pwrpriv->bips_processing)) {
		...
		if ((_WEP40_ == psecuritypriv->dot11PrivacyAlgrthm)
			...
			DBG_871X("==>%s,channel(%d),processing(%x)\n", __FUNCTION__,
				 padapter->mlmeextpriv.cur_channel, pwrpriv->bips_processing);
		...
		if (pwrpriv->LpsIdleCount >= 2) { //  4 Sec
			if (pwrpriv->pwr_mode == PS_MODE_ACTIVE) {
				...
				rtw_set_ps_mode(padapter, pwrpriv->power_mgnt,
						padapter->registrypriv.smart_ps, 0);
			}
		} else
		    ...
		    const char *caller)
{
	...
	if (pwrpriv->ips_deny_time < rtw_get_current_time() + rtw_ms_to_systime(
		    ips_deffer_ms))
	...
	if ((pwrpriv->bInternalAutoSuspend == _FALSE)
