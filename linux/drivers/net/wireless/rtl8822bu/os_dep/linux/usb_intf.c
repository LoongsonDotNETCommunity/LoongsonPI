...
{
	struct usb_interface *usb_intf = container_of(dev, struct usb_interface,
					 dev);
...
static void rtw_decide_chip_type_by_usb_info(struct dvobj_priv *pdvobjpriv,
		const struct usb_device_id *pdid)
{
...
u8 rtw_set_hal_ops(_adapter *padapter)
{
	...
	if (rtw_get_chip_type(padapter) == RTL8812
	    || rtw_get_chip_type(padapter) == RTL8821)
		       ...
		       pm_message_t message)
{
	...
	if ((padapter->bup) || !rtw_is_drv_stopped(padapter)
		...
		if (pwrpriv->bInternalAutoSuspend) {
			...
			if ((GET_HAL_DATA(padapter)->bFWReady) && (pwrpriv->bHWPwrPindetect)
...
#else
		dvobj->pusbdev->autosuspend_disabled =
