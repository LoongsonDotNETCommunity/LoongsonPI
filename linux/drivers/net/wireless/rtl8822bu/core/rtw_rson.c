...
*/
inline u8 rtw_cal_rson_score(struct rtw_rson_struct *cand_rson_data,
...
*/
int rtw_rson_choose(struct wlan_network **candidate,
...
*/
int rtw_get_rson_struct(WLAN_BSSID_EX *bssid,
			struct  rtw_rson_struct *rson_data)
{
	...
	rson_data->loading = 0;
	/*	fake root		*/
	if (is_match_bssid(bssid->MacAddress, rtw_rson_root_bssid,
			   rtw_rson_root_bssid_idx) == _TRUE) {
			      ...
			      , struct wlan_network **candidate, struct wlan_network *competitor)
{
	...
	if ((!mlme->cur_network_scanned)
	    ...
	    || (rtw_get_rson_struct(&(mlme->cur_network_scanned->network),
				    &rson_curr)) != _TRUE)
		return _FALSE;
	...
	comp_score = rtw_cal_rson_score(&rson_comp, competitor->network.Rssi);
	curr_score = rtw_cal_rson_score(&rson_curr,
	...
	if (rtw_get_rson_struct(&((*candidate)->network), &rson_cand) != _TRUE) {
		RTW_ERR("%s : Unable to get rson_struct from candidate(%s -- "
			MAC_FMT")\n",
			__func__, (*candidate)->network.Ssid.Ssid,
...
void rtw_rson_show_survey_info(struct seq_file *m, _list *plist,
			       _list *phead)
{
	...
	RTW_PRINT_SEL(m, "%5s  %-17s  %3s  %5s %14s  %10s  %-3s  %5s %32s\n",
		      "index", "bssid", "ch", "id", "hop_cnt", "loading", "RSSI", "score",
		      "ssid");
	while (1) {
...
*/
u8 rtw_rson_ap_check_sta(_adapter *padapter, u8 *pframe, uint pkt_len,
			 unsigned short ie_offset)
{
	...
	for (p = pframe + WLAN_HDR_A3_LEN + ie_offset; ; p += (len + 2)) {
		p = rtw_get_ie(p, _VENDOR_SPECIFIC_IE_, &len,
