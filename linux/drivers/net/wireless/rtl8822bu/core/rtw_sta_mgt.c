...
{
	_rtw_memset(st_ctl->reg, 0,
		    sizeof(struct st_register) * SESSION_TRACKER_REG_ID_NUM);
...
inline void rtw_st_ctl_register(struct st_ctl_t *st_ctl, u8 st_reg_id,
				struct st_register *reg)
{
...
inline bool rtw_st_ctl_chk_reg_rule(struct st_ctl_t *st_ctl,
				    _adapter *adapter, u8 *local_naddr, u8 *local_port, u8 *remote_naddr,
				    u8 *remote_port)
{
	bool ret = _FALSE;
	int i;
	...
	for (i = 0; i < SESSION_TRACKER_REG_ID_NUM; i++) {
		...
		if (rule && rule(adapter, local_naddr, local_port, remote_naddr,
				 remote_port) == _TRUE) {
			ret = _TRUE;
			break;
		}
...
void rtw_st_ctl_rx(struct sta_info *sta, u8 *ehdr_pos)
{
	...
	if (ntohs(etherhdr->h_proto) == ETH_P_IP) {
		...
		if (GET_IPV4_PROTOCOL(ip) == 0x06  /* TCP */
			...
			if (rtw_st_ctl_chk_reg_rule(&sta->st_ctl, adapter, IPV4_DST(ip),
						    TCP_DST(tcp), IPV4_SRC(ip), TCP_SRC(tcp)) == _TRUE) {
...
u32	_rtw_init_sta_priv(struct	sta_priv *pstapriv)
{
	...
	pstapriv->pallocated_stainfo_buf = rtw_zvmalloc(sizeof(
			struct sta_info) * NUM_STA + 4);
	...
	pstapriv->sta_aid = rtw_zmalloc(pstapriv->max_aid * sizeof(
						struct sta_info *));
	if (!pstapriv->sta_aid)
		goto exit;
...
#else
	pstapriv->expire_to =
		60;/* 60*2 = 120 sec = 2 min, expire after no any traffic. */
#endif
...
inline int rtw_stainfo_offset(struct sta_priv *stapriv,
			      struct sta_info *sta)
{
	int offset = (((u8 *)sta) - stapriv->pstainfo_buf) / sizeof(
			     struct sta_info);
		...
		int offset)
{
	...
	return (struct sta_info *)(stapriv->pstainfo_buf + offset * sizeof(
					   struct sta_info));
}
...
static void	_rtw_free_sta_recv_priv_lock(struct sta_recv_priv
		*psta_recvpriv)
{
...
struct	sta_info *rtw_alloc_stainfo(struct	sta_priv *pstapriv,
				    const u8 *hwaddr)
{
	_irqL irqL, irqL2;
	s32	index;
	...
	if (_rtw_queue_empty(pfree_sta_queue) == _TRUE) {
		...
		psta = LIST_CONTAINOR(get_next(&pfree_sta_queue->queue), struct sta_info,
				      list);
		...
		for (i = 0; i < 16; i++) {
			_rtw_memcpy(&psta->sta_recvpriv.rxcache.tid_rxseq[i], &wRxSeqInitialValue,
				    2);
			_rtw_memcpy(&psta->sta_recvpriv.bmc_tid_rxseq[i], &wRxSeqInitialValue, 2);
			_rtw_memset(&psta->sta_recvpriv.rxcache.iv[i], 0,
				    sizeof(psta->sta_recvpriv.rxcache.iv[i]));
		}

		rtw_init_timer(&psta->addba_retry_timer, psta->padapter, addba_timer_hdl,
			       psta);
#ifdef CONFIG_IEEE80211W
		rtw_init_timer(&psta->dot11w_expire_timer, psta->padapter,
...
struct sta_info *rtw_get_stainfo(struct sta_priv *pstapriv,
				 const u8 *hwaddr)
{
