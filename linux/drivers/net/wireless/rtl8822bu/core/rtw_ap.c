...
static void update_BCNTIM(_adapter *padapter)
{
	...
	if (_TRUE) {
		...
		/* append TIM IE */
		dst_ie += rtw_set_tim_ie(0, 1, pstapriv->tim_bitmap, pstapriv->aid_bmp_len,
					 dst_ie);
...
void rtw_remove_bcn_ie(_adapter *padapter, WLAN_BSSID_EX *pnetwork,
		       u8 index)
{
...
void	expire_timeout_chk(_adapter *padapter)
{
	...
	while ((rtw_end_of_queue_search(phead, plist)) == _FALSE) {
		...
		if (_rtw_memcmp((void *)(pstapriv->atmel_rc_pattern),
				(void *)(psta->cmn.mac_addr), ETH_ALEN) == _TRUE)
			continue;
	...
	while ((rtw_end_of_queue_search(phead, plist)) == _FALSE) {
		...
		RTW_INFO("%s:%d  psta=%p, %02x,%02x||%02x,%02x  \n\n", __func__,  __LINE__,
			 psta, pstapriv->atmel_rc_pattern[0], pstapriv->atmel_rc_pattern[5],
			 psta->cmn.mac_addr[0], psta->cmn.mac_addr[5]);
		if (_rtw_memcmp((void *)pstapriv->atmel_rc_pattern,
				(void *)(psta->cmn.mac_addr), ETH_ALEN) == _TRUE)
			continue;
		if (psta->flag_atmel_rc)
...
#ifdef CONFIG_TX_MCAST2UNI
		if ((psta->flags & WLAN_STA_HT) && (psta->htpriv.agg_enable_bitmap
			...
			if (psta->expire_to <= (pstapriv->expire_to - 50)) {
				...
				psta->under_exist_checking = 0;
				psta->expire_to = 0;
			} else if (psta->expire_to <= (pstapriv->expire_to - 3)
				   && (psta->under_exist_checking == 0)) {
...
#define KEEP_ALIVE_TRYCNT (3)
			...
			if (psta->keep_alive_trycnt > 0
			    && psta->keep_alive_trycnt <= KEEP_ALIVE_TRYCNT) {
				if (psta->state & WIFI_STA_ALIVE_CHK_STATE)
					psta->state ^= WIFI_STA_ALIVE_CHK_STATE;
				else
					psta->keep_alive_trycnt = 0;

			} else if ((psta->keep_alive_trycnt > KEEP_ALIVE_TRYCNT)
				   && !(psta->state & WIFI_STA_ALIVE_CHK_STATE))
				psta->keep_alive_trycnt = 0;
			...
			if (psta->sleepq_len > (NR_XMITFRAME / pstapriv->asoc_list_cnt)
				...
				RTW_INFO("%s sta:"MAC_FMT", sleepq_len:%u, free_xmitframe_cnt:%u, asoc_list_cnt:%u, clear sleep_q\n",
					 __func__
					 , MAC_ARG(psta->cmn.mac_addr)
					 , psta->sleepq_len, padapter->xmitpriv.free_xmitframe_cnt,
	...
	if (chk_alive_num) {
		...
		if (pmlmeext->active_keep_alive_check) {
			...
			if (switch_channel_by_drv == _TRUE
			    && rtw_get_oper_ch(padapter) != pmlmeext->cur_channel) {
...
#ifdef CONFIG_ATMEL_RC_PATCH
			if (_rtw_memcmp(pstapriv->atmel_rc_pattern, psta->cmn.mac_addr,
					ETH_ALEN) == _TRUE)
				continue;
			if (psta->flag_atmel_rc)
				continue;
...
void rtw_ap_update_sta_ra_info(_adapter *padapter, struct sta_info *psta)
{
	...
	u64 tx_ra_bitmap = 0;
	struct mlme_priv *pmlmepriv = &(padapter->mlmepriv);
	WLAN_BSSID_EX *pcur_network = (WLAN_BSSID_EX *)
...
void rtw_init_bmc_sta_tx_rate(_adapter *padapter, struct sta_info *psta)
{
	...
	if (padapter->bmc_tx_rate != MGN_UNKNOWN)
		...
		if (IsEnableHWOFDM(pmlmeext->cur_wireless_mode)
			...
			rate_idx = get_lowest_rate_idx_ex(psta->cmn.ra_info.ramask,
							  4); /*from basic rate*/
		else
			rate_idx = get_lowest_rate_idx(
					   psta->cmn.ra_info.ramask); /*from basic rate*/
#else
		rate_idx = get_highest_rate_idx(
				   psta->cmn.ra_info.ramask); /*from basic rate*/
#endif
		if (rate_idx < 12)
...
void update_bmc_sta(_adapter *padapter)
{
	...
	if (psta) {
		...
		network_type = rtw_check_network_type((u8 *)&pcur_network->SupportedRates,
						      supportRateNum, pcur_network->Configuration.DSConfig);
...
void update_sta_info_apmode_ht_bf_cap(_adapter *padapter,
				      struct sta_info *psta)
{
	...
	if (TEST_FLAG(phtpriv_ap->beamform_cap, BEAMFORMING_HT_BEAMFORMEE_ENABLE)
		...
		SET_FLAG(cur_beamform_cap,
			 GET_HT_CAP_TXBF_CHNL_ESTIMATION_NUM_ANTENNAS((u8 *)(&phtpriv_sta->ht_cap))
			 << 6);
	}
	...
	if (TEST_FLAG(phtpriv_ap->beamform_cap, BEAMFORMING_HT_BEAMFORMER_ENABLE)
		...
		SET_FLAG(cur_beamform_cap,
			 GET_HT_CAP_TXBF_COMP_STEERING_NUM_ANTENNAS((u8 *)(&phtpriv_sta->ht_cap)) <<
			 4);
	}
	if (cur_beamform_cap)
...
static void update_ap_info(_adapter *padapter, struct sta_info *psta)
{
	...
	psta->state |=
		WIFI_AP_STATE; /* Aries, add,fix bug of flush_cam_entry at STOP AP mode , 0724 */
}
		...
		if (rtw_chset_search_ch(adapter_to_chset(padapter),
			...
			if (delta_time < lifetime) {
				...
				pbuf = rtw_get_ie(ie + _BEACON_IE_OFFSET_, _HT_CAPABILITY_IE_, &ie_len,
						  (pbss->IELength - _BEACON_IE_OFFSET_));
				if (pbuf == NULL) {
...
#define DUMP_ADAPTERS_STATUS 0
	...
	WLAN_BSSID_EX *pnetwork = (WLAN_BSSID_EX *)
				  &pmlmepriv->cur_network.network; /* used as input */
	struct mlme_ext_priv	*pmlmeext = &(padapter->mlmeextpriv);
	struct mlme_ext_info	*pmlmeinfo = &(pmlmeext->mlmext_info);
	WLAN_BSSID_EX *pnetwork_mlmeext = &(pmlmeinfo->network);
	struct dvobj_priv *pdvobj = padapter->dvobj;
	s16 req_ch = REQ_CH_NONE, req_bw = REQ_BW_NONE,
	...
	if (NULL == rtw_get_wps_ie(pnetwork->IEs + _FIXED_IE_LENGTH_,
	...
	if (pmlmepriv->cur_network.join_res !=
	    _TRUE) { /* setting only at  first time */
	...
	/* Set Security */
	val8 = (psecuritypriv->dot11AuthAlgrthm == dot11AuthAlgrthm_8021X) ? 0xcc :
	       0xcf;
...
chbw_decision:
	ch_setting_changed = rtw_ap_chbw_decision(padapter, req_ch, req_bw,
			     req_offset
	...
	if (ch_to_set != 0) {
		...
		rtw_mi_update_union_chan_inf(padapter, ch_to_set, offset_to_set,
					     bw_to_set);
	}
	...
	if (ch_setting_changed == _TRUE
	    && (MLME_IS_GO(padapter)
		|| MLME_IS_MESH(padapter)) /* pure AP is not needed*/
	   ) {
...
int rtw_check_beacon_data(_adapter *padapter, u8 *pbuf,  int len)
{
	...
	struct security_priv *psecuritypriv = &padapter->securitypriv;
	struct mlme_priv *pmlmepriv = &(padapter->mlmepriv);
	WLAN_BSSID_EX *pbss_network = (WLAN_BSSID_EX *)
				      &pmlmepriv->cur_network.network;
	struct sta_priv *pstapriv = &padapter->stapriv;
	...
	_rtw_memcpy(pbss_network->MacAddress, adapter_mac_addr(padapter),
		    ETH_ALEN);

	/* beacon interval */
	p = rtw_get_beacon_interval_from_ie(
		    ie);/* ie + 8;	 */ /* 8: TimeStamp, 2: Beacon Interval 2:Capability */
...
#ifdef CONFIG_P2P
		_rtw_memcpy(padapter->wdinfo.p2p_group_ssid, pbss_network->Ssid.Ssid,
			    pbss_network->Ssid.SsidLength);
	...
	/* Mesh ID */
	if (MLME_IS_MESH(padapter)) {
		p = rtw_get_ie(ie + _BEACON_IE_OFFSET_, WLAN_EID_MESH_ID, &ie_len,
			       (pbss_network->IELength - _BEACON_IE_OFFSET_));
		if (p && ie_len > 0) {
	...
	if (p && ie_len > 0) {
		...
		struct rtw_ieee80211_ht_cap *pht_cap = (struct rtw_ieee80211_ht_cap *)(
				p + 2);
		...
		_rtw_memcpy(&(pmlmeinfo->HT_caps), pht_cap,
		...
		/* Update Supported MCS Set field */
		{
			...
			for (i = 0; i < 10; i++)
				*(HT_CAP_ELE_RX_MCS_MAP(pht_cap) + i) &=
					padapter->mlmeextpriv.default_supported_mcs_set[i];
		}
		...
		/* HT Beamformer */
		if (TEST_FLAG(pmlmepriv->htpriv.beamform_cap,
	...
	if (p && ie_len > 0) {
		pHT_info_ie = p;
		if (channel == 0)
			pbss_network->Configuration.DSConfig = GET_HT_OP_ELE_PRI_CHL(
					pHT_info_ie + 2);
		else if (channel != GET_HT_OP_ELE_PRI_CHL(pHT_info_ie + 2)) {
			RTW_INFO(FUNC_ADPT_FMT" ch inconsistent, DSSS:%u, HT primary:%u\n"
				 , FUNC_ADPT_ARG(padapter), channel,
				 GET_HT_OP_ELE_PRI_CHL(pHT_info_ie + 2));
		}
	}
#endif /* CONFIG_80211N_HT */
	...
	if (pregistrypriv->ht_enable && ht_cap == _TRUE) {
		...
		pmlmepriv->htpriv.ampdu_enable = pregistrypriv->ampdu_enable ? _TRUE :
						 _FALSE;
	...
	/* Parsing VHT CAP IE */
	p = rtw_get_ie(ie + _BEACON_IE_OFFSET_, EID_VHTCapability, &ie_len,
		       (pbss_network->IELength - _BEACON_IE_OFFSET_));
	if (p && ie_len > 0)
		vht_cap = _TRUE;
	...
	if ((pbss_network->Configuration.DSConfig > 14)
		...
		if (vht_cap == _TRUE
		    && MLME_IS_MESH(
			    padapter) /* allow only mesh temporarily before VHT IE checking is ready */
		   ) {
	...
	if (pbss_network->Configuration.DSConfig <= 14
		...
		rtw_set_ie(pbss_network->IEs + pbss_network->IELength, EID_EXTCapability,
			   8, pmlmepriv->ext_capab_ie_data, &len);
		pbss_network->IELength += pmlmepriv->ext_capab_ie_len;
	}

	pbss_network->Length = get_WLAN_BSSID_EX_sz((WLAN_BSSID_EX *)pbss_network);

	rtw_ies_get_chbw(pbss_network->IEs + _BEACON_IE_OFFSET_,
			 pbss_network->IELength - _BEACON_IE_OFFSET_
	...
	pmlmepriv->cur_network.join_res =
		_TRUE;/* for check if already set beacon */
...
static void _rtw_macaddr_acl_deinit(_adapter *adapter, u8 period,
				    bool clear_only)
{
	struct sta_priv *stapriv = &adapter->stapriv;
	struct wlan_acl_pool *acl;
...
u8 rtw_ap_set_sta_key(_adapter *adapter, const u8 *addr, u8 alg,
		      const u8 *key, u8 keyid, u8 gk)
{
	...
	param = (struct set_stakey_parm *)rtw_zmalloc(sizeof(
...
static int rtw_ap_set_key(_adapter *padapter, u8 *key, u8 alg, int keyid,
			  u8 set_tx)
{
	u8 keylen;
	struct cmd_obj *pcmd;
...
int rtw_ap_set_wep_key(_adapter *padapter, u8 *key, u8 keylen, int keyid,
		       u8 set_tx)
{
	u8 alg;
...
static void associated_stainfo_update(_adapter *padapter,
				      struct sta_info *psta, u32 sta_info_type)
{
	struct mlme_priv *pmlmepriv = &(padapter->mlmepriv);

	RTW_INFO("%s: "MAC_FMT", updated_type=0x%x\n", __func__,
		 MAC_ARG(psta->cmn.mac_addr), sta_info_type);
...
static void update_bcn_ext_capab_ie(_adapter *padapter)
{
	...
	if ((pmlmepriv->ext_capab_ie_len > 0) &&
	    (_rtw_memcmp(pmlmepriv->ext_capab_ie_data, null_extcap_data,
			 sizeof(null_extcap_data)) == _FALSE))
		rtw_add_bcn_ie(padapter, pnetwork, _EXT_CAP_IE_,
			       pmlmepriv->ext_capab_ie_data, pmlmepriv->ext_capab_ie_len);
...
static void update_bcn_htinfo_ie(_adapter *padapter)
{
	...
	*/
	...
	RTW_INFO("num_sta_40mhz_intolerant(%d), 20mhz_width_req(%d), intolerant_ch_rpt(%d), olbc(%d)\n",
		 pmlmepriv->num_sta_40mhz_intolerant, pmlmepriv->ht_20mhz_width_req,
...
void _update_beacon(_adapter *padapter, u8 ie_id, u8 *oui, u8 tx,
		    const char *tag)
{
	_irqL irqL;
	...
	case WLAN_EID_MESH_CONFIG:
		updated = rtw_mesh_update_bss_peering_status(padapter,
				&(pmlmeext->mlmext_info.network));
		updated |= rtw_mesh_update_bss_formation_info(padapter,
				&(pmlmeext->mlmext_info.network));
		updated |= rtw_mesh_update_bss_forwarding_state(padapter,
				&(pmlmeext->mlmext_info.network));
		break;
#endif
...
void rtw_process_public_act_bsscoex(_adapter *padapter, u8 *pframe,
				    uint frame_len)
{
	struct sta_info *psta;
	struct sta_priv *pstapriv = &padapter->stapriv;
...
u8 bss_cap_update_on_sta_leave(_adapter *padapter, struct sta_info *psta)
{
	...
	associated_clients_update(padapter,  beacon_updated,
				  STA_INFO_UPDATE_ALL); /* move it to avoid deadlock */
#endif
...
u8 ap_free_sta(_adapter *padapter, struct sta_info *psta, bool active,
	       u16 reason, bool enqueue)
{
	_irqL irqL;
	u8 beacon_updated = _FALSE;
	struct mlme_priv *pmlmepriv = &(padapter->mlmepriv);
...
#ifdef CONFIG_BEAMFORMING
	beamforming_wk_cmd(padapter, BEAMFORMING_CTRL_LEAVE, psta->cmn.mac_addr,
			   ETH_ALEN, 1);
#endif
	...
	report_del_sta_event(padapter, psta->cmn.mac_addr, reason, enqueue,
			     _FALSE);
...
int rtw_ap_inform_ch_switch(_adapter *padapter, u8 new_ch, u8 ch_offset)
{
	...
	while ((rtw_end_of_queue_search(phead, plist)) == _FALSE) {
		...
		issue_action_spct_ch_switch(padapter, psta->cmn.mac_addr, new_ch,
					    ch_offset);
...
void sta_info_update(_adapter *padapter, struct sta_info *psta)
{
	...
	if (WLAN_STA_HT & flags) {
		...
		psta->htpriv.smps_cap = (psta->htpriv.ht_cap.cap_info &
...
void start_ap_mode(_adapter *padapter)
{
	...
	_rtw_memset(pmlmepriv->ext_capab_ie_data, 0,
		    sizeof(pmlmepriv->ext_capab_ie_data));
...
void rtw_ap_bcmc_sta_flush(_adapter *padapter)
{
	...
	if (cam_id != INVALID_SEC_MAC_CAM_ID) {
		...
		rtw_iface_bcmc_id_set(padapter,
				      INVALID_SEC_MAC_CAM_ID);	/*init default value*/
	}
#else
...
void stop_ap_mode(_adapter *padapter)
{
	...
	_rtw_memset((unsigned char *)&padapter->securitypriv, 0,
...
void rtw_ap_update_bss_chbw(_adapter *adapter, WLAN_BSSID_EX *bss, u8 ch,
			    u8 bw, u8 offset)
{
...
#define UPDATE_HT_CAP 1
	...
	if (vhtpriv->vht_option == _TRUE) {
		...
		vht_cap_ie = rtw_get_ie((bss->IEs + sizeof(NDIS_802_11_FIXED_IEs)),
					EID_VHTCapability, &vht_cap_ielen,
					(bss->IELength - sizeof(NDIS_802_11_FIXED_IEs)));
		vht_op_ie = rtw_get_ie((bss->IEs + sizeof(NDIS_802_11_FIXED_IEs)),
				       EID_VHTOperation, &vht_op_ielen,
				       (bss->IELength - sizeof(NDIS_802_11_FIXED_IEs)));
...
#ifdef CONFIG_80211N_HT
	{
		...
		ht_cap_ie = rtw_get_ie((bss->IEs + sizeof(NDIS_802_11_FIXED_IEs)),
				       EID_HTCapability, &ht_cap_ielen,
				       (bss->IELength - sizeof(NDIS_802_11_FIXED_IEs)));
		ht_op_ie = rtw_get_ie((bss->IEs + sizeof(NDIS_802_11_FIXED_IEs)),
				      EID_HTInfo, &ht_op_ielen, (bss->IELength - sizeof(NDIS_802_11_FIXED_IEs)));
...
*/
bool rtw_ap_chbw_decision(_adapter *adapter, s16 req_ch, s8 req_bw,
			  s8 req_offset
			  , u8 *ch, u8 *bw, u8 *offset, u8 *chbw_allow)
{
	...
	if (MSTATE_STA_LD_NUM(&mstate) || MSTATE_AP_NUM(&mstate)
		...
		rtw_warn_on(!rtw_mi_get_ch_setting_union_no_self(adapter, &u_ch, &u_bw,
				&u_offset));
...
#ifdef CONFIG_MCC_MODE
		if (MCC_EN(adapter)) {
			if (!rtw_is_chbw_grouped(u_ch, u_bw, u_offset, dec_ch, dec_bw,
						 dec_offset)) {
		...
		while (!rtw_chset_is_chbw_valid(chset, dec_ch, dec_bw, dec_offset)
			   ...
			   && rtw_is_dfs_chbw(dec_ch, dec_bw, dec_offset))
...
#define BEACON_EARLY_TIME		20	/* unit:TU*/
	...
	if (rtw_is_list_empty(&pdvobj->ap_if_q.queue) || (pdvobj->nr_ap_if == 0)) {
		...
		padapter = LIST_CONTAINOR(get_next(&(pdvobj->ap_if_q.queue)),
					  struct _ADAPTER, list);
	_exit_critical_bh(&pdvobj->ap_if_q.lock, &irqL);
	...
	if (nr_vap > 0) {
		inter_bcn_space_us = pdvobj->inter_bcn_space *
				     NET80211_TU_TO_US; /* beacon_interval / (nr_vap+1); */
		...
		if (((idx + 2 == nr_vap + 1) && (idx < nr_vap + 1)) || (0 == bcn_idx)) {
			time_offset = bcn_interval_us - cur_tick - BEACON_EARLY_TIME *
				      NET80211_TU_TO_US;
		...
		/*#endif*/ { /* MBSSID */
		time_offset = 2 * bcn_interval_us - cur_tick - BEACON_EARLY_TIME *
			      NET80211_TU_TO_US;
...
u16 rtw_ap_parse_sta_supported_rates(_adapter *adapter,
				     struct sta_info *sta, u8 *tlv_ies, u16 tlv_ies_len)
{
	u8 rate_set[16];
	u8 rate_num;
	int i;
...
#endif
		if ((sec->mfp_opt == MFP_REQUIRED && mfp_opt == MFP_NO)
		    || mfp_opt == MFP_INVALID)
	...
	if (wpa_ie == NULL) {
		...
		if ((sec->wpa_psk > 0)
			...
			if (mlme->wps_beacon_ie) {
				...
				rtw_get_wps_attr_content(mlme->wps_beacon_ie, mlme->wps_beacon_ie_len,
		...
		if (elems->wps_ie) {
			...
			copy_len = ((wpa_ie_len + 2) > sizeof(sta->wpa_ie)) ? (sizeof(
						sta->wpa_ie)) : (wpa_ie_len + 2);
...
void rtw_ap_parse_sta_wmm_ie(_adapter *adapter, struct sta_info *sta,
			     u8 *tlv_ies, u16 tlv_ies_len)
{
	...
	p = rtw_get_ie_ex(tlv_ies, tlv_ies_len, WLAN_EID_VENDOR_SPECIFIC, WMM_IE,
			  6, NULL, NULL);
	if (!p)
		goto exit;
			     ...
			     struct rtw_ieee802_11_elems *elems)
{
	...
	if (elems->vht_capabilities
		...
		if (elems->vht_op_mode_notify && elems->vht_op_mode_notify_len == 1) {
			_rtw_memcpy(&sta->vhtpriv.vht_op_mode_notify, elems->vht_op_mode_notify,
				    1);
