...
#ifdef CONFIG_RTW_CFGVEDNOR_LLSTATS
		pwrpriv->pwr_saving_time += rtw_get_passing_time_ms(
						    pwrpriv->pwr_saving_start_time);
#endif /* CONFIG_RTW_CFGVEDNOR_LLSTATS */
...
bool rtw_pwr_unassociated_idle(_adapter *adapter)
{
	...
	if (rtw_time_after(adapter_to_pwrctl(adapter)->ips_deny_time,
...
void	traffic_check_for_leave_lps(PADAPTER padapter, u8 tx, u32 tx_packets)
{
	...
	if (tx) { /* from tx */
		...
		if (rtw_get_passing_time_ms(start_time) >
	...
	if (bLeaveLPS) {
		...
		rtw_lps_ctrl_wk_cmd(padapter,
				    tx ? LPS_CTRL_TX_TRAFFIC_LEAVE : LPS_CTRL_RX_TRAFFIC_LEAVE, tx ? 0 : 1);
	}
		...
		if (1
			...
			for (i = 0; i < NUM_STA; i++) {
				...
				while ((rtw_end_of_queue_search(phead, plist)) == _FALSE) {
					...
					if (ptdls_sta->tdls_sta_state & TDLS_LINKED_STATE)
						issue_nulldata_to_TDLS_peer_STA(padapter, ptdls_sta->cmn.mac_addr, 0, 0,
										0);
			...
			if (pwrpriv->wowlan_mode == _TRUE ||
				...
				do {
					...
					if (!(val8 & BIT(
						      4))) { /* 0x08 bit4 =1 --> in 32k, bit4 = 0 --> leave 32k */
						pwrpriv->cpwm = PS_STATE_S4;
						break;
					}
		...
		if ((PS_RDY_CHECK(padapter)
			...
			for (i = 0; i < NUM_STA; i++) {
				...
				while ((rtw_end_of_queue_search(phead, plist)) == _FALSE) {
					...
					if (ptdls_sta->tdls_sta_state & TDLS_LINKED_STATE)
						issue_nulldata_to_TDLS_peer_STA(padapter, ptdls_sta->cmn.mac_addr, 1, 0,
										0);
					plist = get_next(plist);
				}
			}
	...
	if (rtw_mi_check_status(Adapter, MI_LINKED)) { /*connect*/
		...
		if (pwrpriv->rf_pwrstate == rf_off) {
			...
			if (Adapter->registrypriv.usbss_enable) {
				...
				adapter_to_dvobj(Adapter)->pusbdev->autosuspend_disabled =
					Adapter->bDisableAutosuspend;/* autosuspend disabled by the user */
#endif
			} else
#endif
			{
...
{
	struct pwrctrl_priv *pwrpriv = container_of(work, struct pwrctrl_priv,
				       cpwm_event);
...
__inline static void register_task_alive(struct pwrctrl_priv *pwrctrl,
		u32 tag)
{
	pwrctrl->alives |= tag;
}

__inline static void unregister_task_alive(struct pwrctrl_priv *pwrctrl,
		u32 tag)
{
	pwrctrl->alives &= ~tag;
}
...
void rtw_init_pwrctrl_priv(PADAPTER padapter)
{
	...
	_init_workitem(&pwrctrlpriv->rpwmtimeoutwi, rpwmtimeout_workitem_callback,
		       NULL);
	rtw_init_timer(&pwrctrlpriv->pwr_rpwm_timer, padapter,
		       pwr_rpwm_timeout_handler, padapter);
#endif /* CONFIG_LPS_RPWM_TIMER */
#endif /* CONFIG_LPS_LCLK */

	rtw_init_timer(&pwrctrlpriv->pwr_state_check_timer, padapter,
		       pwr_state_check_handler, padapter);
...
inline void rtw_set_do_late_resume(struct pwrctrl_priv *pwrpriv,
				   bool enable)
{
...
{
	struct pwrctrl_priv *pwrpriv = container_of(h, struct pwrctrl_priv,
				       early_suspend);
	RTW_INFO("%s\n", __FUNCTION__);
...
void rtw_register_early_suspend(struct pwrctrl_priv *pwrpriv)
{
	...
	/* jeff: set the early suspend level before blank screen, so we wll do late resume after scree is lit */
	pwrpriv->early_suspend.level = ANDROID_EARLY_SUSPEND_LEVEL_BLANK_SCREEN -
				       20;
	pwrpriv->early_suspend.suspend = rtw_early_suspend;
	pwrpriv->early_suspend.resume = rtw_late_resume;
...
u8 rtw_interface_ps_func(_adapter *padapter, HAL_INTF_PS_FUNC efunc_id,
			 u8 *val)
{
	u8 bResult = _TRUE;
...
int _rtw_pwr_wakeup(_adapter *padapter, u32 ips_deffer_ms,
		    const char *caller)
{
	struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);
	struct pwrctrl_priv *pwrpriv = dvobj_to_pwrctl(dvobj);
	...
	if (rtw_time_after(rtw_get_current_time() + rtw_ms_to_systime(
				   ips_deffer_ms), pwrpriv->ips_deny_time))
		pwrpriv->ips_deny_time = rtw_get_current_time() + rtw_ms_to_systime(
						 ips_deffer_ms);
	...
	if (rtw_hal_sreset_inprogress(padapter)) {
		...
		while (rtw_hal_sreset_inprogress(padapter)
		       && rtw_get_passing_time_ms(start) <= 4000)
			rtw_msleep_os(10);
		if (rtw_hal_sreset_inprogress(padapter))
	...
	if ((pwrpriv->bInternalAutoSuspend == _TRUE)
