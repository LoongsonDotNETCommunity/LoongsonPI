...
static void arcfour_init(struct arc4context	*parc4ctx, u8 *key,
			 u32	key_len)
{
	u32	t, u;
	u32	keyindex;
...
static void phase1(u16 *p1k, const u8 *tk, const u8 *ta, u32 iv32)
{
	...
	for (i = 0; i < PHASE1_LOOP_CNT ; i++) {
		...
		p1k[4] += (unsigned short)
			  i;                     /* avoid "slide attacks" */
	}
}
...
)
{
	...
	for (i = 2; i < 8; i++)
		mic_iv[i] = mpdu[i +
				 8];                    /* mic_iv[2:7] = A2[0:5] = mpdu[10:15] */
...
#endif
	for (i = 2; i < 8; i++)
		ctr_preload[i] = mpdu[i +
				      8];                       /* ctr_preload[2:7] = A2[0:5] = mpdu[10:15] */
#ifdef CONSISTENT_PN_ORDER
	for (i = 8; i < 14; i++)
		ctr_preload[i] =    pn_vector[i -
					      8];           /* ctr_preload[8:13] = PN[0:5] */
#else
	for (i = 8; i < 14; i++)
		ctr_preload[i] =    pn_vector[13 -
					      i];          /* ctr_preload[8:13] = PN[5:0] */
#endif
		       ...
		       u8 *pframe, uint plen)
{
	...
	if (payload_remainder > 0) {
		...
		for (j = 0; j < payload_remainder; j++) {
			padded_buffer[j] =
				pframe[payload_index++];/* padded_buffer[j] = message[payload_index++]; */
		}
	...
	for (j = 0; j < 8; j++)
		pframe[payload_index + j] =
			mic[j];	/* message[payload_index+j] = mic[j]; */
	...
	for (i = 0; i < num_blocks; i++) {
		...
		for (j = 0; j < 16; j++)
			pframe[payload_index++] =
				chain_buffer[j];/* for (j=0; j<16;j++) message[payload_index++] = chain_buffer[j]; */
	}

	if (payload_remainder >
	    0) {        /* If there is a short final block, then pad it,*/
		...
		for (j = 0; j < payload_remainder; j++)
			pframe[payload_index++] =
				chain_buffer[j];/* for (j=0; j<payload_remainder;j++) message[payload_index++] = chain_buffer[j]; */
	}
	...
	for (j = 0; j < 16; j++)
		padded_buffer[j] = 0x00;
	for (j = 0; j < 8; j++) {
		padded_buffer[j] = pframe[j + hdrlen + 8 +
	...
	for (j = 0; j < 8; j++)
		pframe[payload_index++] =
			chain_buffer[j];/* for (j=0; j<8;j++) message[payload_index++] = chain_buffer[j]; */
	return _SUCCESS;
}
			 ...
			 u8 *pframe, uint plen)
{
	...
	/* compare the mic */
	for (i = 0; i < 8; i++) {
		if (pframe[hdrlen + 8 + plen - 8 + i] != message[hdrlen + 8 + plen - 8 +
				i]) {
		       ...
		       , const u8 *key, u16 keyid, u64 *ipn)
{
	...
	_rtw_memcpy(BIP_AAD + BIP_AAD_SIZE, whdr_pos + WLAN_HDR_A3_LEN,
		    flen - WLAN_HDR_A3_LEN);
...
void wpa_tdls_generate_tpk(_adapter *padapter, PVOID sta)
{
	...
	if (os_memcmp(adapter_mac_addr(padapter), psta->cmn.mac_addr,
		      ETH_ALEN) < 0) {
...
u8 rtw_handle_tkip_countermeasure(_adapter *adapter, const char *caller)
{
	...
	if (securitypriv->btkip_countermeasure == _TRUE) {
		u32 passing_ms = rtw_get_passing_time_ms(
