...
static void rtw_mask_map_read(PADAPTER padapter, u16 addr, u16 cnts,
			      u8 *data)
{
	u16 i = 0;
...
void rtw_efuse_analyze(PADAPTER	padapter, u8 Type, u8 Fake)
{
	...
	u16	 i, j;
	u8	u1temp = 0;
	u8	efuseHeader = 0, efuseExtHdr = 0, efuseData[EFUSE_MAX_WORD_UNIT * 2] = {0},
			dataCnt = 0;
	...
	EFUSE_GetEfuseDefinition(padapter, efuseType,
				 TYPE_EFUSE_PROTECT_BYTES_BANK, (PVOID)&protectBytesBank, bPseudoTest);
	...
	for (bank = startBank; bank <= endBank; bank++) {
		...
		while ((efuseHeader != 0xFF) && ((efuseType == EFUSE_WIFI
				...
				for (i = 0; i < EFUSE_MAX_WORD_UNIT; i++) {
					...
					if (!(wden & (0x01 << i))) {
						if (!((efuseType == EFUSE_WIFI && (eFuse_Addr + 2 < maprawlen)) ||
						      (efuseType == EFUSE_BT
						       && (eFuse_Addr + 2 < (endBank + 1) * EFUSE_MAX_BANK_SIZE)))) {
							RTW_INFO("eFuse_Addr exceeds, break\n");
							break;
						}
						efuse_OneByteRead(padapter, eFuse_Addr++, &efuseData[dataCnt++],
								  bPseudoTest);
			...
			if (bExtHeader) {
				...
				RTW_INFO("Header=0x%x, ExtHeader=0x%x, ", ParseEfuseHeader,
					 ParseEfuseExtHdr);
			} else {
			...
			for (i = 0; i < EFUSE_MAX_WORD_UNIT; i++) {
				if (!(wden & (0x01 << i))) {
					RTW_INFO("Map[ %02X ] = %02X %02X\n",
						 ((offset * EFUSE_MAX_WORD_UNIT * 2) + (i * 2)), efuseData[wd_cnt * 2 + 0],
	...
	for (i = 0; i < max_section; i++) {
		for (j = 0; j < EFUSE_MAX_WORD_UNIT; j++) {
			pEfuseHal->fakeEfuseInitMap[(i * 8) + (j * 2)] = (eFuseWord[(i * 8) +
					(j * 2)]);
			      ...
			      void *out, BOOLEAN test)
{
	...
	if (adapter->hal_func.EFUSEGetEfuseDefinition) {
		adapter->hal_func.EFUSEGetEfuseDefinition(adapter, efusetype, type, out,
				test);
		return;
	}
 ...
 */
u8 rtw_efuse_access(PADAPTER adapter, u8 write, u16 addr, u16 cnts,
		    u8 *data)
{
	struct dvobj_priv *d;
	u8 *efuse = NULL;
 ...
 * read/write raw efuse data
 */
u8 rtw_efuse_bt_access(PADAPTER adapter, u8 write, u16 addr, u16 cnts,
		       u8 *data)
{
	struct dvobj_priv *d;
	u8 *efuse = NULL;
...
u8 rtw_efuse_map_write(PADAPTER adapter, u16 addr, u16 cnts, u8 *data)
{
	...
	if (adapter->registrypriv.boffefusemask == 0) {
		...
		if (mask_len != 0) {
			...
			err = rtw_halmac_write_logical_efuse_map(d, efuse, size, mask_buf,
					mask_len);
		} else
	...
	for (bank = 1; bank <= total; bank++) { /* 8723d Max bake 0~2 */
		...
		while (AVAILABLE_EFUSE_ADDR(eFuse_Addr)) {
			...
			RTW_INFO("%s: efuse[%#X]=0x%02x (header)\n", __FUNCTION__,
				 (((bank - 1) * EFUSE_BT_REAL_CONTENT_LEN) + eFuse_Addr - 1), efuseHeader);
			...
			if (EXT_HEADER(efuseHeader)) {	/* extended header */
				...
				RTW_INFO("%s: efuse[%#X]=0x%02x (ext header)\n", __FUNCTION__,
					 (((bank - 1) * EFUSE_BT_REAL_CONTENT_LEN) + eFuse_Addr - 1), efuseExtHdr);
				if (ALL_WORDS_DISABLED(efuseExtHdr))
					continue;
	...
	u8				bPseudoTest)
{
	...
	while (1) {
		...
		if (rtw_efuse_bt_access(padapter, _FALSE, startAddr, 1, &efuse_data)
		    && (efuse_data != 0xFF)) {
			bRet = _FALSE;
...
u8 efuse_OneByteRead(struct _ADAPTER *a, u16 addr, u8 *data,
		     u8 bPseudoTest)
{
	struct dvobj_priv *d;
	int err;
...
static u16 hal_EfuseGetCurrentSize_BT(
	PADAPTER	padapter,
	u8			bPseudoTest)
{
#ifdef HAL_EFUSE_MEMORY
...
static u8 hal_BT_EfusePgCheckAvailableAddr(
	PADAPTER	pAdapter,
	u8		bPseudoTest)
{
...
BOOLEAN Efuse_Read1ByteFromFakeContent(
...
BOOLEAN Efuse_Write1ByteToFakeContent(
	IN		PADAPTER	pAdapter,
	IN		u16		Offset,
	IN		u8		Value);
BOOLEAN Efuse_Write1ByteToFakeContent(
...
{
	Adapter->hal_func.ReadEFuse(Adapter, efuseType, _offset, _size_byte, pbuf,
				    bPseudoTest);
}
...
{
	pAdapter->hal_func.EFUSEGetEfuseDefinition(pAdapter, efuseType, type, pOut,
			bPseudoTest);
}
...
int Efuse_PgPacketRead(IN	PADAPTER	pAdapter,
		       IN	u8			offset,
		       IN	u8			*data,
		       IN	BOOLEAN		bPseudoTest)
{
...
int Efuse_PgPacketWrite(IN	PADAPTER	pAdapter,
			IN	u8			offset,
			IN	u8			word_en,
			IN	u8			*data,
			IN	BOOLEAN		bPseudoTest)
...
int Efuse_PgPacketWrite_BT(IN	PADAPTER	pAdapter,
			   IN	u8			offset,
			   IN	u8			word_en,
			   IN	u8			*data,
			   IN	BOOLEAN		bPseudoTest)
{
	...
	ret =  pAdapter->hal_func.Efuse_PgPacketWrite_BT(pAdapter, offset, word_en,
			data, bPseudoTest);
...
u8 Efuse_WordEnableDataWrite(IN	PADAPTER	pAdapter,
			     IN	u16		efuse_addr,
			     IN	u8		word_en,
			     IN	u8		*data,
			     IN	BOOLEAN		bPseudoTest)
{
	...
	ret =  pAdapter->hal_func.Efuse_WordEnableDataWrite(pAdapter, efuse_addr,
			word_en, data, bPseudoTest);
...
u16 efuse_GetMaxSize(PADAPTER padapter)
{
	...
	max_size = 0;
	EFUSE_GetEfuseDefinition(padapter, EFUSE_WIFI,
				 TYPE_AVAILABLE_EFUSE_BYTES_TOTAL, (PVOID)&max_size, _FALSE);
	return max_size;
}
/* ------------------------------------------------------------------------------ */
...
#define RT_ASSERT_RET(expr)												\
	...
	EFUSE_GetEfuseDefinition(padapter, EFUSE_WIFI, TYPE_EFUSE_MAP_LEN,
				 (PVOID)&mapLen, _FALSE);
	EFUSE_GetEfuseDefinition(padapter, EFUSE_WIFI,
				 TYPE_AVAILABLE_EFUSE_BYTES_TOTAL, &efuse_max_available_len, _FALSE);
	...
	while (idx < cnts) {
		...
		if (word_en != 0xF) {
			...
			if (offset >=
			    EFUSE_MAX_SECTION_BASE) /* Over EFUSE_MAX_SECTION 16 for 2 ByteHeader */
				chk_total_byte += 2;
			else
				chk_total_byte += 1;
		}
					...
					if (IS_C_CUT(pHalData->version_id) || IS_B_CUT(pHalData->version_id)) {
						if (pHalData->adjuseVoltageVal == 6) {
							...
							RTW_INFO(" %s ,\n adjuseVoltageVal = %d ,newdata[%d] = %x\n", __func__,
								 pHalData->adjuseVoltageVal, i, newdata[i]);
						}
					}
				}
#endif
 ...
 *---------------------------------------------------------------------------*/
#ifdef PLATFORM
static VOID efuse_ShadowWrite1Byte(
	IN	PADAPTER	pAdapter,
	IN	u16		Offset,
	IN	u8		Value);
#endif /* PLATFORM */
...
/* ---------------Write Two Bytes */
static VOID efuse_ShadowWrite2Byte(
	IN	PADAPTER	pAdapter,
	IN	u16		Offset,
	IN	u16		Value)
{
...
/* ---------------Write Four Bytes */
static VOID efuse_ShadowWrite4Byte(
	IN	PADAPTER	pAdapter,
	IN	u16		Offset,
	IN	u32		Value)
{
 ...
 *
 *---------------------------------------------------------------------------*/
static VOID efuse_ShadowRead1Byte(
	IN	PADAPTER	pAdapter,
	IN	u16		Offset,
	IN OUT	u8		*Value)
{
...
/* ---------------Read Two Bytes */
static VOID efuse_ShadowRead2Byte(
	IN	PADAPTER	pAdapter,
	IN	u16		Offset,
	IN OUT	u16		*Value)
{
	...
	IN BOOLEAN	bPseudoTest)
{
	...
	if (pHalData->bautoload_fail_flag == _FALSE) {
		err = rtw_halmac_read_logical_efuse_map(adapter_to_dvobj(pAdapter),
							efuse_map, mapLen, NULL, 0);
		if (err)
	...
	if (pHalData->bautoload_fail_flag == _TRUE)
		...
		if (_SUCCESS != retriveAdaptorInfoFile(
...
#ifdef CONFIG_ADAPTOR_INFO_CACHING_FILE
			storeAdaptorInfoFile(pAdapter->registrypriv.adaptor_info_caching_file_path,
					     pHalData->efuse_eeprom_data);
		}
#endif
	}
...
u32 rtw_read_efuse_from_file(const char *path, u8 *buf, int map_size)
{
	...
	for (i = 0 ; i < map_size ; i++) {
		...
		while (1) {
			...
			if (temp_i == 0 && (is_eol(c) == _TRUE || is_space(c) == _TRUE
					    || is_null(c) == _TRUE))
				continue;
