...
sint _rtw_init_recv_priv(struct recv_priv *precvpriv, _adapter *padapter)
{
	...
	precvpriv->pallocated_frame_buf = rtw_zvmalloc(NR_RECVFRAME * sizeof(
			union recv_frame) + RXFRAME_ALIGN_SZ);
	...
	precvpriv->precv_frame_buf = (u8 *)N_BYTE_ALIGMENT((SIZE_PTR)(
	...
	rtw_init_timer(&precvpriv->signal_stat_timer, padapter,
		       rtw_signal_stat_timer_hdl, padapter);
...
#define DBG_RFRAME_DEL_WFD_IE 0
	u8 *ies = rframe->u.hdr.rx_data + sizeof(struct rtw_ieee80211_hdr_3addr) +
		  ies_offset;
	uint ies_len_ori = rframe->u.hdr.len - (ies - rframe->u.hdr.rx_data);
	uint ies_len;

	ies_len = rtw_del_wfd_ie(ies, ies_len_ori,
				 DBG_RFRAME_DEL_WFD_IE ? __func__ : NULL);
	...
	if (prxattrib->encrypt == _TKIP_) {
		...
		if (stainfo != NULL) {
			...
			if (bmic_err == _TRUE) {
				...
				if ((IS_MCAST(prxattrib->ra) == _TRUE)
				...
				/* mic checked ok */
				if ((psecuritypriv->bcheck_grpkey == _FALSE)
				    && (IS_MCAST(prxattrib->ra) == _TRUE)) {
...
union recv_frame *decryptor(_adapter *padapter,
...
#ifdef CONFIG_CONCURRENT_MODE
		    && !IS_MCAST(
			    prxattrib->ra) /* bc/mc packets may use sw decryption for concurrent mode */
#endif
		   )
...
union recv_frame *portctrl(_adapter *adapter,
				   union recv_frame *precv_frame);
union recv_frame *portctrl(_adapter *adapter,
				   union recv_frame *precv_frame)
{
	...
	if (psta->state & WIFI_SLEEP_STATE) {
		...
		if (wmmps_ac) {
			if (psta->sleepq_ac_len > 0) {
				...
				issue_qos_nulldata(padapter, psta->cmn.mac_addr, (u16)pattrib->priority, 0,
						   0, 0);
			}
		}
...
sint OnTDLS(_adapter *adapter, union recv_frame *precv_frame)
{
	...
	RTW_INFO("[TDLS] Recv %s from "MAC_FMT" with SeqNum = %d\n",
		 ...
		 GetSequence(get_recvframe_data(precv_frame)));
	...
	struct sta_info **psta)
{
	...
	if ((check_fwstate(pmlmepriv, WIFI_STATION_STATE) == _TRUE)
		...
		if (_rtw_memcmp(pattrib->bssid, "\x0\x0\x0\x0\x0\x0", ETH_ALEN) ||
			...
			if (!bmcast) {
				RTW_INFO(ADPT_FMT" -issue_deauth to the nonassociated ap=" MAC_FMT
...
sint validate_recv_ctrl_frame(_adapter *padapter,
			      union recv_frame *precv_frame);
sint validate_recv_ctrl_frame(_adapter *padapter,
			      union recv_frame *precv_frame)
{
	...
	if (get_frame_sub_type(pframe) == WIFI_PSPOLL) {
		...
		if ((psta->state & WIFI_SLEEP_STATE)
			...
			if ((rtw_end_of_queue_search(xmitframe_phead,
...
static sint validate_mgmt_protect(_adapter *adapter,
				  union recv_frame *precv_frame)
{
...
#define DBG_VALIDATE_MGMT_DEC 0
	...
	if (!pattrib->privacy) {
		...
		if (is_bmc) {
			...
			if (subtype == WIFI_DEAUTH || subtype == WIFI_DISASSOC) {
				if (!MLME_IS_MESH(adapter)) {
					unsigned short reason = le16_to_cpu(*(unsigned short *)(
							ptr + WLAN_HDR_A3_LEN));

#if DBG_VALIDATE_MGMT_PROTECT
					RTW_INFO(FUNC_ADPT_FMT" unicast %s, reason=%d w/o encrypt\n"
						 , FUNC_ADPT_ARG(adapter), subtype == WIFI_DEAUTH ? "deauth" : "disassoc",
						 reason);
#endif
					if (reason == 6 || reason == 7) {
						/* issue sa query request */
						issue_action_SA_Query(adapter, psta->cmn.mac_addr, 0, 0,
...
union recv_frame *recvframe_chk_defrag(PADAPTER padapter,
					       union recv_frame *precv_frame);

sint validate_recv_mgnt_frame(PADAPTER padapter,
			      union recv_frame *precv_frame)
{
	struct sta_info *psta = precv_frame->u.hdr.psta
				= rtw_get_stainfo(&padapter->stapriv,
						get_addr2_ptr(precv_frame->u.hdr.rx_data));
	...
	if (psta) {
		...
		if (get_frame_sub_type(precv_frame->u.hdr.rx_data) == WIFI_BEACON)
			...
			if (_rtw_memcmp(adapter_mac_addr(padapter),
					GetAddr1Ptr(precv_frame->u.hdr.rx_data), ETH_ALEN) == _TRUE)
	...
	if (padapter->proximity.proxim_on == _TRUE) {
		struct rx_pkt_attrib *pattrib = &precv_frame->u.hdr.attrib;
		struct recv_stat *prxstat = (struct recv_stat *)
...
sint validate_recv_data_frame(_adapter *adapter,
			      union recv_frame *precv_frame)
{
	u8 bretry, a4_shift;
	struct sta_info *psta = NULL;
...
pre_validate_status_chk:
	if (ret == _FAIL) {
		...
		RTW_INFO("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" case:%d, res:%d, ra="MAC_FMT", ta="MAC_FMT"\n"
			 ...
			 MAC_ARG(GetAddr1Ptr(ptr)), MAC_ARG(get_addr2_ptr(ptr)));
#endif
		goto exit;
	} else if (ret == RTW_RX_HANDLED)
		goto exit;
	...
	if (psta == NULL) {
		...
		RTW_INFO("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" psta == NULL, ra="MAC_FMT", ta="MAC_FMT"\n"
			 , FUNC_ADPT_ARG(adapter), MAC_ARG(GetAddr1Ptr(ptr)),
	...
	/* parsing QC field */
	if (pattrib->qos == 1) {
		pattrib->priority = GetPriority((ptr + WLAN_HDR_A3_LEN +
						 a4_shift)); /* point to Qos field*/
	...
	if (!IS_MCAST(pattrib->ra)) {
		...
		if (recv_decache(precv_frame,
				 psta->sta_recvpriv.rxcache.tid_rxseq) == _FAIL) {
#ifdef DBG_RX_DROP_FRAME
	...
	case WIFI_DATA_TYPE: /* data */
		...
		if (retval == _FAIL) {
			...
			if (!pattrib->encrypt || pattrib->bdecrypted) {
				...
				rtw_hal_get_def_var(adapter, HAL_DEF_DBG_DUMP_RXPKT, &(bDumpRxPkt));
				/* get ether_type */
				_rtw_memcpy(&eth_type, ptr + pattrib->hdrlen + pattrib->iv_len +
					    RATTRIB_GET_MCTRL_LEN(pattrib) + LLC_HEADER_SIZE, 2);
...
sint wlanhdr_to_ethhdr(union recv_frame *precvframe)
{
	...
	psnap = (struct ieee80211_snap_hdr *)(ptr + pattrib->hdrlen +
					      pattrib->iv_len + RATTRIB_GET_MCTRL_LEN(pattrib));
	psnap_type = ptr + pattrib->hdrlen + pattrib->iv_len +
		     RATTRIB_GET_MCTRL_LEN(pattrib) + SNAP_SIZE;
...
sint wlanhdr_to_ethhdr(union recv_frame *precvframe)
{
	...
	if (psnap->dsap == 0xaa && psnap->ssap == 0xaa && psnap->ctrl == 0x03) {
		...
		else if (_rtw_memcmp(psnap->oui, SNAP_HDR_APPLETALK_DDP,
				     WLAN_IEEE_OUI_LEN) &&
	...
	if (eth_type == 0x0800) { /* ip */
		...
		NDIS_PER_PACKET_INFO_FROM_PACKET(precvframe->u.hdr.pkt,
						 Ieee8021QInfo) = VlanPriInfo.Value;
#endif
	}
...
static int rtw_recv_indicatepkt_check(union recv_frame *rframe,
				      u8 *ehdr_pos, u32 pkt_len)
{
...
#define PAYLOAD_LEN_LOC_OF_IP_HDR 0x10 /*ethernet payload length location of ip header (DA + SA+eth_type+(version&hdr_len)) */
	...
	if (ntohs(ehdr->h_proto) == ETH_P_ARP) {
		...
		if (be16_to_cpu(*((u16 *)(ehdr_pos + PAYLOAD_LEN_LOC_OF_IP_HDR))) !=
		    (pkt_len) - ETH_HLEN) {
			RTW_INFO("Error !!%s,Payload length not correct\n", __func__);
			RTW_INFO("%s, IP header describe Total length=%u\n", __func__,
				 be16_to_cpu(*((u16 *)(ehdr_pos + PAYLOAD_LEN_LOC_OF_IP_HDR))));
...
static void recv_free_fwd_resource(_adapter *adapter,
...
int amsdu_to_msdu(_adapter *padapter, union recv_frame *prframe)
{
	...
	while (a_len > ETH_HLEN) {
		...
		if (MLME_IS_MESH(padapter)) {
			...
			struct rtw_ieee80211s_hdr *mctrl = (struct rtw_ieee80211s_hdr *)(
					pdata + ETH_HLEN);
			int v_ret;
		...
		if (rtw_recv_indicatepkt_check(prframe, rtw_os_pkt_data(sub_pkt),
					       rtw_os_pkt_len(sub_pkt)) == _SUCCESS)
	...
	if (pattrib->amsdu) {
		...
		if (MLME_IS_MESH(padapter) && pattrib->mesh_ctrl_present) {
			act = rtw_mesh_rx_msdu_act_check(prframe
							 ...
							 , (struct rtw_ieee80211s_hdr *)(get_recvframe_data(prframe) +
...
static int enqueue_reorder_recvframe(struct recv_reorder_ctrl
				     *preorder_ctrl, union recv_frame *prframe)
{
	struct rx_pkt_attrib *pattrib = &prframe->u.hdr.attrib;
...
static void recv_indicatepkts_pkt_loss_cnt(_adapter *padapter,
				      ...
				      struct recv_reorder_ctrl *preorder_ctrl, int bforced)
{
	...
	if (bforced == _TRUE) {
		...
		RTW_INFO("DBG_RX_SEQ "FUNC_ADPT_FMT" tid:%u FORCE indicate_seq:%d, seq_num:%d\n"
			 ...
			 pattrib->seq_num);
#endif
		recv_indicatepkts_pkt_loss_cnt(padapter, preorder_ctrl->indicate_seq,
					       pattrib->seq_num);
...
static int recv_indicatepkt_reorder(_adapter *padapter,
				    union recv_frame *prframe)
{
	_irqL irql;
	int retval = _SUCCESS;
	...
	_queue *ppending_recvframe_queue = preorder_ctrl ?
					   &preorder_ctrl->pending_recvframe_queue : NULL;
	struct recv_priv  *precvpriv = &padapter->recvpriv;
...
static void recv_set_iseq_before_mpdu_process(union recv_frame *rframe,
		u16 seq_num, const char *caller)
{
#if defined(CONFIG_80211N_HT) && defined(CONFIG_RECV_REORDERING_CTRL)
...
static void recv_set_iseq_after_mpdu_process(union recv_frame *rframe,
		u16 seq_num, const char *caller)
{
#if defined(CONFIG_80211N_HT) && defined(CONFIG_RECV_REORDERING_CTRL)
	struct recv_reorder_ctrl *reorder_ctrl = rframe->u.hdr.preorder_ctrl;
...
#ifdef CONFIG_MP_INCLUDED
int validate_mp_recv_frame(_adapter *adapter,
			   union recv_frame *precv_frame)
{
	int ret = _SUCCESS;
	u8 *ptr = precv_frame->u.hdr.rx_data;
	...
	if (pmppriv->bloopback) {
		if (_rtw_memcmp(ptr + 24, pmptx->buf + 24,
				precv_frame->u.hdr.len - 24) == _FALSE) {
	...
	if (_rtw_memcmp(get_addr2_ptr(ptr), adapter->mppriv.mac_filter,
			ETH_ALEN) == _FALSE)
		ret = _FAIL;
...
static sint MPwlanhdr_to_ethhdr(union recv_frame *precvframe)
{
	...
	if (adapter->mppriv.bRTWSmbCfg == _TRUE) {
		...
		if (_rtw_memcmp(mcastheadermac, pattrib->bssid,
				3) == _TRUE) /* SimpleConfig Dest. */
...
int mp_recv_frame(_adapter *padapter, union recv_frame *rframe)
{
	...
	if ((check_fwstate(pmlmepriv,
			   _FW_LINKED) == _TRUE)) { /* &&(padapter->mppriv.check_mp_pkt == 0)) */
...
static sint fill_radiotap_hdr(_adapter *padapter,
			      union recv_frame *precvframe, u8 *buf)
{
...
int recv_func_prehandle(_adapter *padapter, union recv_frame *rframe)
{
	...
	if (padapter->registrypriv.mp_mode == 1
	    || padapter->mppriv.bRTWSmbCfg == _TRUE) {
...
int recv_func_posthandle(_adapter *padapter, union recv_frame *prframe)
{
	...
	if (IS_MCAST(pattrib->ra))
		RTW_INFO("%s =>"ADPT_FMT" Rx BC/MC from "MAC_FMT"\n", __func__,
			 ADPT_ARG(padapter), MAC_ARG(pattrib->ta));
#endif
...
#ifdef CONFIG_TDLS
	/* check TDLS frame */
	psnap_type = get_recvframe_data(orig_prframe) + pattrib->hdrlen +
		     pattrib->iv_len + SNAP_SIZE;
...
static void rtw_signal_stat_timer_hdl(void *ctx)
{
	...
	u8 ratio_pre_stat = 0, ratio_curr_stat = 0, ratio_total = 0,
	   ratio_profile = SIGNAL_STAT_CALC_PROFILE_0;
	...
	if (adapter->recvpriv.is_signal_dbg) {
		...
		if (num_signal_strength == 0) {
			if (rtw_get_on_cur_ch_time(adapter) == 0
			    || rtw_get_passing_time_ms(rtw_get_on_cur_ch_time(adapter)) < 2 *
			    adapter->mlmeextpriv.mlmext_info.bcn_interval
			   )
				goto set_timer;
		}
		...
		tmp_s = (ratio_curr_stat * avg_signal_strength + ratio_pre_stat *
		...
		tmp_q = (ratio_curr_stat * avg_signal_qual + ratio_pre_stat *
		...
		RTW_INFO(FUNC_ADPT_FMT" signal_strength:%3u, rssi:%3d, signal_qual:%3u"
			 ...
			 , rtw_get_on_cur_ch_time(adapter) ? rtw_get_passing_time_ms(
				 rtw_get_on_cur_ch_time(adapter)) : 0
			);
#endif
	}
...
static void rx_process_rssi(_adapter *padapter, union recv_frame *prframe)
{
	...
	/* if(pRfd->Status.bPacketToSelf || pRfd->Status.bPacketBeacon) */
	{
		...
		if (padapter->recvpriv.signal_strength_data.total_num++ >=
			...
			last_rssi =
				padapter->recvpriv.signal_strength_data.elements[padapter->recvpriv.signal_strength_data.index];
...
static void rx_process_link_qual(_adapter *padapter,
				 union recv_frame *prframe)
{
	...
	if (pattrib->phy_info.signal_quality != 0) {
		...
		padapter->recvpriv.signal_qual_data.total_val +=
			pattrib->phy_info.signal_quality;
	...
	u8 *pphy_status)
{
	...
	pkt_info.is_packet_match_bssid = (!IsFrameTypeCtrl(wlanhdr))
					 ...
					 && ((!MLME_IS_MESH(padapter)
					      && _rtw_memcmp(get_hdr_bssid(wlanhdr), get_bssid(&padapter->mlmepriv),
							      ETH_ALEN))
	...
	odm_phy_status_query(&pHalData->odmpriv, p_phy_info, pphy_status,
			     &pkt_info);
...
* @return _FALSE:
*/
int rtw_inc_and_chk_continual_no_rx_packet(struct sta_info *sta,
		int tid_index)
{
...
u8 adapter_allow_bmc_data_rx(_adapter *adapter)
{
	if (check_fwstate(&adapter->mlmepriv,
			  WIFI_MONITOR_STATE | WIFI_MP_STATE) == _TRUE)
		return 1;
...
void rx_bh_tk_set_buf(struct recv_priv *recv, void *buf, void *data,
		      u32 dlen)
{
	if (recv->rx_bh_cbuf)
...
{
	RTW_PRINT_SEL(sel,
		      "[RXBHTK]s:%u, buf_dqc:%u, lbuf:%p, cbuf:%p, dlen:%u, pos:%u, cframe:%p\n"
		      , recv->rx_bh_stage
		      , recv->rx_bh_buf_dq_cnt
		      , recv->rx_bh_lbuf
		      , recv->rx_bh_cbuf
