...
void rtw_init_sitesurvey_parm(_adapter *padapter,
...
inline u8 rtw_create_ibss_cmd(_adapter *adapter, int flags)
{
	return rtw_createbss_cmd(adapter, flags
				 , 1
				 , 0, REQ_BW_NONE,
				 REQ_OFFSET_NONE /* for now, adhoc doesn't support ch,bw,offset request */
				);
}
...
u8 rtw_joinbss_cmd(_adapter  *padapter, struct wlan_network *pnetwork)
{
	...
	*/
	...
	psecnetwork->IELength += rtw_restruct_sec_ie(padapter,
				 psecnetwork->IEs + psecnetwork->IELength);
...
u8 rtw_setopmode_cmd(_adapter  *adapter,
...
u8 rtw_dfs_master_hdl(_adapter *adapter)
{
	...
	rtw_change_bss_chbw_cmd(adapter, RTW_CMDF_DIRECTLY, -1,
...
u8 rtw_drvextra_cmd_hdl(_adapter *padapter, unsigned char *pbuf)
{
	...
	case MCC_SET_DURATION_WK_CID:
		ret = rtw_set_mcc_duration_hdl(padapter, pdrvextra_cmd->type,
					       pdrvextra_cmd->pbuf);
		break;
#endif /* CONFIG_MCC_MODE */
#if defined(CONFIG_RTW_MESH) && defined(RTW_PER_CMD_SUPPORT_FW)
