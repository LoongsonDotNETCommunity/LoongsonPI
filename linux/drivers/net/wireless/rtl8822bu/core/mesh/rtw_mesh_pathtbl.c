 ...
 */
void rtw_mesh_path_assign_nexthop(struct rtw_mesh_path *mpath,
				  struct sta_info *sta)
{
...
static struct rtw_mesh_path *rtw_mpath_lookup(struct rtw_mesh_table *tbl,
		const u8 *dst)
{
	struct rtw_mesh_path *mpath;
...
static struct rtw_mesh_path *__rtw_mesh_path_lookup_by_idx(
	struct rtw_mesh_table *tbl, int idx)
{
	int i = 0, ret;
 ...
 * @mpath: gate mpath
 */
void rtw_mesh_gate_del(struct rtw_mesh_table *tbl,
		       struct rtw_mesh_path *mpath)
{
	struct rtw_mesh_cfg *mcfg;
	struct rtw_mesh_info *minfo;
					...
					const u8 *dst)
{
	...
	if (ATOMIC_INC_UNLESS(&adapter->mesh_info.mpaths,
			      RTW_MESH_MAX_MPATHS) == 0)
...
static void __rtw_mesh_path_del(struct rtw_mesh_table *tbl,
				struct rtw_mesh_path *mpath)
{
	rtw_rhashtable_remove_fast(&tbl->rhead, &mpath->rhash,
				   rtw_mesh_rht_params);
	rtw_mesh_path_free_rcu(tbl, mpath);
}
	...
	while ((mpath = rtw_rhashtable_walk_next(&iter))) {
		...
		if (mpath->is_gate &&  /* need not to deal with non-gate case */
			...
			if (mpath->gate_asked) { /* asked gate before */
				...
				mpath->gate_timeout = rtw_get_current_time() + rtw_ms_to_systime(
							      mpath->gate_ann_int);
				exit_critical_bh(&mpath->state_lock);
				rtw_mesh_queue_preq(mpath, RTW_PREQ_Q_F_START | RTW_PREQ_Q_F_REFRESH);
				RTW_MPATH_DBG(
					FUNC_ADPT_FMT"mpath [%pM] ask mesh gate existence (is_root=%d)\n",
