...
static inline u32 rtw_u32_field_get(const u8 *preq_elem, int shift,
				    BOOLEAN ae)
{
	if (ae)
		shift += 6;
...
static inline u16 rtw_u16_field_get(const u8 *preq_elem, int shift,
				    BOOLEAN ae)
{
	if (ae)
		shift += 6;
...
static inline BOOLEAN rtw_ether_addr_equal(const u8 *addr1,
		const u8 *addr2)
{
...
static int rtw_mesh_path_sel_frame_tx(enum rtw_mpath_frame_type
...
static u32 rtw_airtime_link_metric_get(_adapter *adapter,
				       struct sta_info *sta)
{
	struct dm_struct *dm = adapter_to_phydm(adapter);
	int device_constant = phydm_get_plcp(dm,
					     sta->cmn.mac_id) << RTW_ARITH_SHIFT;
...
static inline struct sta_info *rtw_next_hop_deref_protected(
					...
					const u8 *prep_elem, u32 metric)
{
	...
	_rtw_memcpy(next_hop, rtw_next_hop_deref_protected(path)->cmn.mac_addr,
		    ETH_ALEN);
	exit_critical_bh(&path->state_lock);
	--ttl;
	flags = RTW_PREP_IE_FLAGS(prep_elem);
	...
	rtw_mesh_path_sel_frame_tx(RTW_MPATH_PREP, flags, originator_addr,
				   originator_sn, 0,
...
void rtw_mesh_rx_path_sel_frame(_adapter *adapter,
				union recv_frame *rframe)
{
	...
	struct rtw_ieee80211_hdr_3addr *frame_hdr = (struct rtw_ieee80211_hdr_3addr
			*)pframe;
	...
	if (elems.rann)
		rtw_hwmp_rann_frame_process(adapter, frame_hdr,
					    (struct rtw_ieee80211_rann_ie *)elems.rann);
}
...
void rtw_mesh_queue_preq(struct rtw_mesh_path *path, u8 flags)
{
	...
	if (rtw_time_after(rtw_get_current_time(),
			   minfo->last_preq + rtw_min_preq_int_jiff(adapter)))
		rtw_mesh_work(&adapter->mesh_work);

	else if (rtw_time_before(rtw_get_current_time(), minfo->last_preq)) {
		/* systime wrapped around issue */
		minfo->last_preq = rtw_get_current_time() - rtw_min_preq_int_jiff(
					   adapter) - 1;
		rtw_mesh_work(&adapter->mesh_work);
	} else
...
#endif
	rtw_mesh_path_sel_frame_tx(RTW_MPATH_PREQ, flags,
				   adapter_mac_addr(adapter), minfo->sn,
				   target_flags, path->dst, path->sn, da, 0,
				   ttl, lifetime, 0, minfo->preq_id++, adapter);
	rtw_mod_timer(&path->timer,
		      rtw_get_current_time() + path->discovery_timeout);
...
void rtw_mesh_path_tx_root_frame(_adapter *adapter)
{
	...
	case RTW_IEEE80211_PROACTIVE_RANN:
		rtw_mesh_path_sel_frame_tx(RTW_MPATH_RANN, flags,
...
static void rtw_ieee80211_mesh_rootpath(_adapter *adapter)
{
	...
	if (adapter->mesh_cfg.dot11MeshHWMPRootMode ==
	    RTW_IEEE80211_PROACTIVE_RANN)
...
void rtw_mesh_work_hdl(_workitem *work)
{
	...
	if (adapter->mesh_info.preq_queue_len &&
	    rtw_time_after(rtw_get_current_time(),
			   adapter->mesh_info.last_preq + rtw_ms_to_systime(
