...
inline u8 *rtw_set_ie_mesh_id(u8 *buf, u32 *buf_len, const char *mesh_id,
			      u8 id_len)
{
...
inline u8 *rtw_set_ie_mesh_config(u8 *buf, u32 *buf_len
				  , u8 path_sel_proto, u8 path_sel_metric, u8 congest_ctl_mode,
				  u8 sync_method, u8 auth_proto
...
int rtw_bss_is_same_mbss(WLAN_BSSID_EX *a, WLAN_BSSID_EX *b)
{
	...
	if (a->InfrastructureMode != Ndis802_11_mesh)
		goto exit;
	a_mconf_ie = rtw_get_ie(BSS_EX_TLV_IES(a), WLAN_EID_MESH_CONFIG,
				&a_mconf_ie_len, BSS_EX_TLV_IES_LEN(a));
	if (!a_mconf_ie || a_mconf_ie_len != 7)
		goto exit;
	if (b->InfrastructureMode != Ndis802_11_mesh)
		goto exit;
	b_mconf_ie = rtw_get_ie(BSS_EX_TLV_IES(b), WLAN_EID_MESH_CONFIG,
				&b_mconf_ie_len, BSS_EX_TLV_IES_LEN(b));
	if (!b_mconf_ie || b_mconf_ie_len != 7)
		goto exit;

	if (a->mesh_id.SsidLength != b->mesh_id.SsidLength
	    || _rtw_memcmp(a->mesh_id.Ssid, b->mesh_id.Ssid,
			   a->mesh_id.SsidLength) == _FALSE)
		goto exit;
...
int rtw_bss_is_candidate_mesh_peer(WLAN_BSSID_EX *self,
				   WLAN_BSSID_EX *target, u8 ch, u8 add_peer)
{
	int ret = 0;
	u8 *mconf_ie;
	sint mconf_ie_len;
	...
	if (add_peer) {
		...
		mconf_ie = rtw_get_ie(BSS_EX_TLV_IES(target), WLAN_EID_MESH_CONFIG,
				      &mconf_ie_len, BSS_EX_TLV_IES_LEN(target));
...
void dump_mesh_peer_blacklist_settings(void *sel, _adapter *adapter)
{
	struct mesh_peer_sel_policy *peer_sel_policy =
			&adapter->mesh_cfg.peer_sel_policy;
...
u8 rtw_mesh_cto_mgate_network_filter(_adapter *adapter,
				     struct wlan_network *scanned)
{
	...
	return !rtw_mesh_cto_mgate_required(adapter)
		   ...
		   && !rtw_mesh_cto_mgate_blacklist_search(adapter,
...
void rtw_chk_candidate_peer_notify(_adapter *adapter,
				   struct wlan_network *scanned)
{
	...
	if (rtw_get_passing_time_ms(scanned->last_scanned) >=
	    mcfg->peer_sel_policy.scanr_exp_ms
	    || (mcfg->rssi_threshold && mcfg->rssi_threshold > scanned->network.Rssi)
	    || !rtw_bss_is_candidate_mesh_peer(&mlme->cur_network.network,
					       &scanned->network, 1, 1)
...
void rtw_mesh_peer_status_chk(_adapter *adapter)
{
	...
	if (rtw_mesh_cto_mgate_required(adapter)) {
		...
		issue_probereq_ex(adapter, &adapter->mlmepriv.cur_network.network.mesh_id,
				  NULL, 0, 0, 0, 0);
	}
#endif
	...
	while (rtw_end_of_queue_search(head, list) == _FALSE) {
		...
		if (!rtw_bss_is_candidate_mesh_peer(&mlme->cur_network.network,
						    &plink->scanned->network, 1, 0)
#if CONFIG_RTW_MACADDR_ACL
		...
		if (IS_CTO_MGATE_CONF_DISABLED(plink)) {
			if (cto_mgate && !forwarding && !mgate)
				SET_CTO_MGATE_CONF_END_TIME(plink,
							    mcfg->peer_sel_policy.cto_mgate_conf_timeout_ms);
			else
...
u8 rtw_mesh_offch_candidate_accepted(_adapter *adapter)
{
	...
	if (ret) {
		...
		if (MSTATE_STA_LD_NUM(&mstate_no_self) || MSTATE_AP_LD_NUM(&mstate_no_self)
		    || MSTATE_ADHOC_LD_NUM(&mstate_no_self)
		    || MSTATE_MESH_LD_NUM(&mstate_no_self))
			ret = 0;
	}
#endif
...
u8 rtw_mesh_select_operating_ch(_adapter *adapter)
{
	...
	while (!rtw_end_of_queue_search(head, pos)) {
		...
		if (rtw_get_passing_time_ms(scanned->last_scanned) <
			...
			int ch_set_idx = rtw_chset_search_ch(rfctl->channel_set,
							     scanned->network.Configuration.DSConfig);
...
void dump_mesh_networks(void *sel, _adapter *adapter)
{
	...
	while (rtw_end_of_queue_search(head, list) == _FALSE) {
		...
		mesh_conf_ie = rtw_get_ie(BSS_EX_TLV_IES(&network->network),
	...
	for (i = 0; i < mesh_network_cnt; i++) {
		...
		if (MLME_IS_MESH(adapter) && MLME_IS_ASOC(adapter)) {
			...
			if (plink && plink->plink_state == RTW_MESH_PLINK_ESTAB)
				...
				;
			else if (rtw_bss_is_candidate_mesh_peer(&mlme->cur_network.network,
								&network->network, 0, 1))
				candidate = 1;
			else if (rtw_bss_is_same_mbss(&mlme->cur_network.network,
						      &network->network))
				same_mbss = 1;
		}

		RTW_PRINT_SEL(sel, "%c "MAC_FMT" %3d %4ld %5d %-32s %c%2u %3u %c%c \n"
			      , established ? 'E' : (blocked ? 'B' : (plink ? 'N' : (candidate ? 'C' :
						     (same_mbss ? 'S' : ' '))))
...
static int rtw_mpm_tx_ies_sync_bss(_adapter *adapter,
				   struct mesh_plink_ent *plink
				   , u8 *fhead, size_t flen, u8 *fbody, u8 tlv_ies_offset, u8 *mpm_ie,
				   u8 *mic_ie
				   , u8 **nbuf, size_t *nlen)
{
	int ret = _FAIL;
	...
	if (mic_ie) {
		...
		if (rtw_mpm_ampe_dec(adapter, plink, fhead, flen, fbody, mic_ie,
				     ampe_buf) != _SUCCESS)
			goto exit;
	...
	/* build new frame  */
	_rtw_memcpy(new_buf, fhead,
		    sizeof(struct rtw_ieee80211_hdr_3addr) + tlv_ies_offset);
	new_fhead = new_buf;
	new_flen = new_len;
	...
	/* encode */
	if (mic_ie) {
		int enc_ret = rtw_mpm_ampe_enc(adapter, plink, new_fbody, new_mic_ie,
					       ampe_buf, 0);
		if (enc_ret != _SUCCESS)
			goto exit;
	}
...
static void rtw_mpm_info_msg(struct mpm_frame_info *mpm_info,
			     u8 *mpm_log_buf)
{
	int cnt = 0;

	if (mpm_info->aid) {
		cnt += snprintf(mpm_log_buf + cnt, MPM_LOG_BUF_LEN - cnt - 1,
				"aid:0x%04x ", mpm_info->aid_v);
		if (cnt >= MPM_LOG_BUF_LEN - 1)
			goto exit;
	}
	if (mpm_info->pid) {
		cnt += snprintf(mpm_log_buf + cnt, MPM_LOG_BUF_LEN - cnt - 1,
				"pid:0x%04x ", mpm_info->pid_v);
		if (cnt >= MPM_LOG_BUF_LEN - 1)
			goto exit;
	}
	if (mpm_info->llid) {
		cnt += snprintf(mpm_log_buf + cnt, MPM_LOG_BUF_LEN - cnt - 1,
				"llid:0x%04x ", mpm_info->llid_v);
		if (cnt >= MPM_LOG_BUF_LEN - 1)
			goto exit;
	}
	if (mpm_info->plid) {
		cnt += snprintf(mpm_log_buf + cnt, MPM_LOG_BUF_LEN - cnt - 1,
				"plid:0x%04x ", mpm_info->plid_v);
		if (cnt >= MPM_LOG_BUF_LEN - 1)
			goto exit;
	}
	if (mpm_info->reason) {
		cnt += snprintf(mpm_log_buf + cnt, MPM_LOG_BUF_LEN - cnt - 1,
				"reason:0x%04x ", mpm_info->reason_v);
		if (cnt >= MPM_LOG_BUF_LEN - 1)
			goto exit;
	}
	if (mpm_info->chosen_pmk) {
		cnt += snprintf(mpm_log_buf + cnt, MPM_LOG_BUF_LEN - cnt - 1,
				"chosen_pmk:0x"KEY_FMT, KEY_ARG(mpm_info->chosen_pmk));
		if (cnt >= MPM_LOG_BUF_LEN - 1)
			goto exit;
	}
...
static int rtw_mpm_check_frames(_adapter *adapter, u8 action,
				const u8 **buf, size_t *len, u8 tx)
{
	struct rtw_mesh_info *minfo = &adapter->mesh_info;
	struct mesh_plink_pool *plink_ctl = &minfo->plink_ctl;
	...
	if (!plink && (tx == _TRUE
		       || action == RTW_ACT_SELF_PROTECTED_MESH_CONF)) {
	...
	mpm_ie = rtw_get_ie(fhead + sizeof(struct rtw_ieee80211_hdr_3addr) +
			    tlv_ies_offset
	...
	if (mpm_info.pid_v == 1) {
		mic_ie = rtw_get_ie(fhead + sizeof(struct rtw_ieee80211_hdr_3addr) +
				    tlv_ies_offset
...
bypass_sync_bss:
	...
	if (action == RTW_ACT_SELF_PROTECTED_MESH_OPEN) {
		...
		if (action == RTW_ACT_SELF_PROTECTED_MESH_CONF) {
			...
			if (ies) {
				_rtw_memcpy(ies, fhead + sizeof(struct rtw_ieee80211_hdr_3addr) + 2,
					    ies_len);
				if (tx == _FALSE) {
...
static int rtw_mesh_check_frames(_adapter *adapter, const u8 **buf,
				 size_t *len, u8 tx)
{
	int is_mesh_frame = -1;
	const u8 *frame_body;
...
int rtw_mesh_check_frames_tx(_adapter *adapter, const u8 **buf,
			     size_t *len)
{
...
unsigned int on_action_self_protected(_adapter *adapter,
...
bool rtw_mesh_update_bss_peering_status(_adapter *adapter,
					WLAN_BSSID_EX *bss)
{
	struct sta_priv *stapriv = &adapter->stapriv;
	struct rtw_mesh_cfg *mcfg = &adapter->mesh_cfg;
...
bool rtw_mesh_update_bss_formation_info(_adapter *adapter,
					WLAN_BSSID_EX *bss)
{
	struct rtw_mesh_cfg *mcfg = &adapter->mesh_cfg;
	struct rtw_mesh_info *minfo = &adapter->mesh_info;
	u8 cto_mgate = (minfo->num_gates
...
bool rtw_mesh_update_bss_forwarding_state(_adapter *adapter,
		WLAN_BSSID_EX *bss)
{
	struct rtw_mesh_cfg *mcfg = &adapter->mesh_cfg;
...
struct mesh_plink_ent *_rtw_mesh_plink_get(_adapter *adapter,
		const u8 *hwaddr)
{
	struct rtw_mesh_info *minfo = &adapter->mesh_info;
	struct mesh_plink_pool *plink_ctl = &minfo->plink_ctl;
...
struct mesh_plink_ent *rtw_mesh_plink_get(_adapter *adapter,
		const u8 *hwaddr)
{
	struct rtw_mesh_info *minfo = &adapter->mesh_info;
	struct mesh_plink_pool *plink_ctl = &minfo->plink_ctl;
...
struct mesh_plink_ent *rtw_mesh_plink_get_no_estab_by_idx(
	_adapter *adapter, u8 idx)
{
	struct rtw_mesh_info *minfo = &adapter->mesh_info;
	struct mesh_plink_pool *plink_ctl = &minfo->plink_ctl;
	struct mesh_plink_ent *ent = NULL;
...
#ifdef CONFIG_RTW_MESH_AEK
int rtw_mesh_plink_set_aek(_adapter *adapter, const u8 *hwaddr,
			   const u8 *aek)
{
	struct rtw_mesh_info *minfo = &adapter->mesh_info;
	struct mesh_plink_pool *plink_ctl = &minfo->plink_ctl;
...
#if CONFIG_RTW_MESH_PEER_BLACKLIST
int rtw_mesh_plink_set_peer_conf_timeout(_adapter *adapter,
		const u8 *hwaddr)
{
	struct rtw_mesh_cfg *mcfg = &adapter->mesh_cfg;
	struct rtw_mesh_info *minfo = &adapter->mesh_info;
...
void dump_mesh_plink_ctl(void *sel, _adapter *adapter)
{
	...
	for (i = 0; i < RTW_MESH_MAX_PEER_CANDIDATES; i++)  {
		...
		RTW_PRINT_SEL(sel, "l_nonce:"KEY_FMT KEY_FMT"\n", KEY_ARG(ent->l_nonce),
			      KEY_ARG(ent->l_nonce + 16));
		RTW_PRINT_SEL(sel, "p_nonce:"KEY_FMT KEY_FMT"\n", KEY_ARG(ent->p_nonce),
			      KEY_ARG(ent->p_nonce + 16));
#endif
...
int rtw_mesh_peer_establish(_adapter *adapter,
			    struct mesh_plink_ent *plink, struct sta_info *sta)
{
...
#define DBG_RTW_MESH_PEER_ESTABLISH 0
	...
	if (rtw_ieee802_11_parse_elems(tlv_ies, tlv_ieslen, &elems,
				       1) == ParseFailed) {
	...
	if (rtw_ap_parse_sta_supported_rates(adapter, sta, tlv_ies,
					     tlv_ieslen) != _STATS_SUCCESSFUL_)
		goto exit;

	if (rtw_ap_parse_sta_security_ie(adapter, sta,
					 &elems) != _STATS_SUCCESSFUL_)
		goto exit;
...
static u8 *rtw_mesh_construct_peer_mesh_close(_adapter *adapter,
		struct mesh_plink_ent *plink, u16 reason, u32 *len)
{
	struct rtw_mesh_info *minfo = &adapter->mesh_info;
	u8 *frame = NULL, *pos;
	u32 flen;
...
void _rtw_mesh_expire_peer_ent(_adapter *adapter,
			       struct mesh_plink_ent *plink)
{
	...
	if (plink->plink_state == RTW_MESH_PLINK_ESTAB)
		frame = rtw_mesh_construct_peer_mesh_close(adapter, plink,
				WLAN_REASON_MESH_CLOSE, &flen);
...
#ifdef DBG_MEM_ALLOC
inline void *dbg_rtw_mcache_alloc(rtw_mcache *cachep,
...
inline void dbg_rtw_mcache_free(rtw_mcache *cachep, void *pbuf,
				const enum mstat_f flags, const char *func, const int line)
{
	u32 sz = cachep->size;
...
static int rtw_mrc_init(_adapter *adapter)
{
	...
	minfo->mrc->cache = rtw_mcache_create(cache_name,
					      sizeof(struct rtw_mrc_entry));
...
static int rtw_mrc_check(_adapter *adapter, const u8 *msa, u32 seq)
{
	...
	rtw_hlist_for_each_entry_safe(p, np, n, &mrc->bucket[idx], list) {
		...
		if (timeout || entries == RTW_MRC_QUEUE_MAX_LEN) {
			...
			--entries;
			   ...
			   && _rtw_memcmp(msa, p->msa, ETH_ALEN) == _TRUE)
			return -1;
	}
	...
	p->seqnum = seq;
	p->exp_time = rtw_get_current_time() + rtw_ms_to_systime(
			      RTW_MRC_TIMEOUT_MS);
...
void rtw_mesh_cfg_init_peer_sel_policy(struct rtw_mesh_cfg *mcfg)
{
	...
	sel_policy->cto_mgate_blacklist_timeout_ms =
		RTW_MESH_CTO_MGATE_BLACKLIST_TIMEOUT_MS;
#endif
}
...
void rtw_mesh_cfg_init(_adapter *adapter)
{
	...
	mcfg->dot11MeshHWMPconfirmationInterval =
		RTW_MESH_ROOT_CONFIRMATION_INTERVAL;
...
void rtw_mesh_init_mesh_info(_adapter *adapter)
{
	...
	rtw_init_timer(&adapter->mesh_path_timer, adapter,
		       rtw_ieee80211_mesh_path_timer, adapter);
	rtw_init_timer(&adapter->mesh_path_root_timer, adapter,
		       rtw_ieee80211_mesh_path_root_timer, adapter);
	rtw_init_timer(&adapter->mesh_atlm_param_req_timer, adapter,
		       rtw_mesh_atlm_param_req_timer, adapter);
			     ...
			     struct xmit_frame *xframe)
{
	...
	rtw_rcu_read_lock();
	err = rtw_mesh_nexthop_lookup(adapter, target_addr, attrib->msa,
				      attrib->ra);
	if (!err)
		goto endlookup;
	...
	if (mpath->frame_queue_len >= RTW_MESH_FRAME_QUEUE_LEN) {
		xframe_to_free = LIST_CONTAINOR(get_next(get_list_head(
							&mpath->frame_queue)), struct xmit_frame, list);
...
int rtw_mesh_addr_resolve(_adapter *adapter, struct xmit_frame *xframe,
			  _pkt *pkt, _list *b2u_list)
{
	...
	if (!is_da_mcast) {
		...
		ae_need = _rtw_memcmp(adapter_mac_addr(adapter), etherhdr.h_source,
				      ETH_ALEN) == _FALSE
			  || (mppath
			      && _rtw_memcmp(mppath->mpp, etherhdr.h_dest, ETH_ALEN) == _FALSE);
	} else {
		ae_need = _rtw_memcmp(adapter_mac_addr(adapter), etherhdr.h_source,
				      ETH_ALEN) == _FALSE;

#if CONFIG_RTW_MESH_DATA_BMC_TO_UC
		if (rtw_msrc_b2u_policy_chk(adapter->mesh_cfg.b2u_flags_msrc,
					    etherhdr.h_dest)) {
	...
	if (is_da_mcast) {
		...
		attrib->mesh_frame_mode = ae_need ? MESH_UCAST_PX_DATA : MESH_UCAST_DATA;
		_rtw_memcpy(attrib->mda, (mppath
					  && ae_need) ? mppath->mpp : attrib->dst, ETH_ALEN);
		_rtw_memcpy(attrib->msa, adapter_mac_addr(adapter), ETH_ALEN);
...
s8 rtw_mesh_tx_set_whdr_mctrl_len(u8 mesh_frame_mode,
				  struct pkt_attrib *attrib)
{
	u8 ret = 0;
...
void rtw_mesh_tx_build_mctrl(_adapter *adapter, struct pkt_attrib *attrib,
			     u8 *buf)
{
...
int rtw_mesh_rx_data_validate_hdr(_adapter *adapter,
				  union recv_frame *rframe, struct sta_info **sta)
{
	...
	case 1:
		...
		*sta = rtw_get_stainfo(stapriv, get_addr2_ptr(whdr));
		...
		_rtw_memcpy(rattrib->msa, GetAddr3Ptr(whdr),
			    ETH_ALEN); /* may change after checking AMSDU subframe header */
		_rtw_memcpy(rattrib->dst, GetAddr1Ptr(whdr), ETH_ALEN);
		_rtw_memcpy(rattrib->src, GetAddr3Ptr(whdr),
			    ETH_ALEN); /* may change after checking mesh ctrl field */
	...
	case 3:
		...
		*sta = rtw_get_stainfo(stapriv, get_addr2_ptr(whdr));
		...
		_rtw_memcpy(rattrib->ra, GetAddr1Ptr(whdr), ETH_ALEN);
		_rtw_memcpy(rattrib->ta, get_addr2_ptr(whdr), ETH_ALEN);
		_rtw_memcpy(rattrib->mda, GetAddr3Ptr(whdr),
			    ETH_ALEN); /* may change after checking AMSDU subframe header */
		_rtw_memcpy(rattrib->msa, GetAddr4Ptr(whdr),
			    ETH_ALEN); /* may change after checking AMSDU subframe header */
		_rtw_memcpy(rattrib->dst, GetAddr3Ptr(whdr),
			    ETH_ALEN); /* may change after checking mesh ctrl field */
		_rtw_memcpy(rattrib->src, GetAddr4Ptr(whdr),
			    ETH_ALEN); /* may change after checking mesh ctrl field */
		_rtw_memcpy(rattrib->bssid, get_addr2_ptr(whdr), ETH_ALEN);
		a4_shift = ETH_ALEN;
		break;
	...
	ps = GetPwrMgt(whdr);
	mps_mode = ps ? (is_ra_bmc
			 || (get_mps_lv(qc)) ? RTW_MESH_PS_DSLEEP : RTW_MESH_PS_LSLEEP) :
		   RTW_MESH_PS_ACTIVE;
	...
	if (is_ra_bmc)
		...
		if (mps_mode != RTW_MESH_PS_ACTIVE
		    && (*sta)->nonpeer_mps == RTW_MESH_PS_ACTIVE)
...
int rtw_mesh_rx_data_validate_mctrl(_adapter *adapter,
				    union recv_frame *rframe
				    ...
				    , const u8 **da, const u8 **sa)
{
	...
	if (ret == _FAIL) {
		...
		RTW_INFO("DBG_RX_DROP_FRAME "FUNC_ADPT_FMT" invalid tfDS:%u AE:%u combination ra="MAC_FMT" ta="MAC_FMT"\n"
			 , FUNC_ADPT_ARG(adapter), rattrib->to_fr_ds, ae, MAC_ARG(rattrib->ra),
			 MAC_ARG(rattrib->ta));
#endif
		*mctrl_len = 0;
	} else
...
inline int rtw_mesh_rx_validate_mctrl_non_amsdu(_adapter *adapter,
		union recv_frame *rframe)
{
	struct rx_pkt_attrib *rattrib = &rframe->u.hdr.attrib;
	const u8 *da, *sa;
	...
	ret = rtw_mesh_rx_data_validate_mctrl(adapter, rframe
					      , (struct rtw_ieee80211s_hdr *)(get_recvframe_data(rframe) +
							      rattrib->hdrlen + rattrib->iv_len)
			       ...
			       , struct xmit_frame **fwd_frame, _list *b2u_list)
{
	...
	u8 is_mda_self = !is_mda_bmc
			 && _rtw_memcmp(mda, adapter_mac_addr(adapter), ETH_ALEN);
	struct xmit_frame *xframe;
	...
	if (is_mda_bmc) {
		...
		if (mppath
		    && _rtw_memcmp(mppath->mpp, adapter_mac_addr(adapter),
				   ETH_ALEN) == _FALSE) {
...
fwd_chk:
...
#if CONFIG_RTW_MESH_DATA_BMC_TO_UC
	if (is_mda_bmc
	    && rtw_mfwd_b2u_policy_chk(mcfg->b2u_flags_mfwd, mda,
				       rattrib->to_fr_ds == 3)
	   ) {
		bmc_need = rtw_mesh_data_bmc_to_uc(adapter
						   , da, sa, mda, msa, ae_need, rframe->u.hdr.psta->cmn.mac_addr,
						   mctrl->ttl - 1
...
#endif
	{
		...
		if (is_mda_bmc) {
			xattrib->mesh_frame_mode = ae_need ? MESH_BMCAST_PX_DATA :
						   MESH_BMCAST_DATA;
