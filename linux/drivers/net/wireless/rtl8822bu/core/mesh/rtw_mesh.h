...
u8 *rtw_set_ie_mesh_id(u8 *buf, u32 *buf_len, const char *mesh_id,
		       u8 id_len);
...
int rtw_mesh_plink_set_state(_adapter *adapter, const u8 *hwaddr,
			     u8 state);
#ifdef CONFIG_RTW_MESH_AEK
...
int rtw_mesh_plink_set_peer_conf_timeout(_adapter *adapter,
		const u8 *hwaddr);
#endif
void _rtw_mesh_plink_del_ent(_adapter *adapter,
...
int rtw_mesh_rx_validate_mctrl_non_amsdu(_adapter *adapter,
		union recv_frame *rframe);
