...
void rtw_hal_read_sta_dk_key(_adapter *adapter, u8 key_id)
{
	...
	if (_rtw_camid_is_gk(adapter, key_id)) {
		...
		RTW_INFO("[cam_cache KEY] - Key-id:%d "KEY_FMT"\n", key_id,
			 KEY_ARG(&dvobj->cam_cache[key_id].key));
	}
...
void rtw_hal_hw_port_enable(_adapter *adapter)
{
	...
	/*rtw_halmac_get_bcn_ctrl(struct dvobj_priv *d, enum _hw_port hwport,
				struct rtw_halmac_bcn_ctrl *bcn_ctrl)*/
	if (rtw_halmac_set_bcn_ctrl(dvobj, get_hw_port(adapter),
				    &bcn_ctrl) == -1) {
...
s32 c2h_evt_read_88xx(_adapter *adapter, u8 *buf)
{
	...
	for (i = 0; i < C2H_PLEN_88XX(buf); i++)
		*(C2H_PAYLOAD_88XX(buf) + i) = rtw_read8(adapter,
					       REG_C2HEVT_MSG_NORMAL + 2 + i);
...
void rtw_hal_c2h_pkt_pre_hdl(_adapter *adapter, u8 *buf, u16 len)
{
	...
	if (rtw_hal_c2h_pkt_hdr_parse(adapter, buf, len, &id, &seq, &plen,
				      &payload) != _SUCCESS) {
		parse_fail = 1;
		goto exit;
	}

	hdl_here = rtw_hal_c2h_id_handle_directly(adapter, id, seq, plen,
			payload) == _TRUE ? 1 : 0;
	if (hdl_here)
...
mac_hidden_rpt_hdl:
	...
	c2h_mac_hidden_rpt_2_hdl(adapter, mac_hidden_rpt + MAC_HIDDEN_RPT_LEN,
				 MAC_HIDDEN_RPT_2_LEN);
...
exit:
	...
	RTW_INFO("%s %s! (%u, %dms), fwdl:%d, id:0x%02x\n", __func__
		 , (ret == _SUCCESS) ? "OK" : "Fail", cnt, rtw_get_passing_time_ms(start),
		 ret_fwdl, id);
...
s32 rtw_hal_h2c_customer_str_req(_adapter *adapter)
{
	...
	return rtw_hal_fill_h2c_cmd(adapter, H2C_CUSTOMER_STR_REQ,
				    H2C_CUSTOMER_STR_REQ_LEN, h2c_data);
}
	...
	if (dvobj->customer_str_sctx != NULL) {
		if (dvobj->customer_str_sctx->status != RTW_SCTX_SUBMITTED)
			RTW_WARN("%s invalid sctx.status:%d\n", __func__,
				 dvobj->customer_str_sctx->status);
		_rtw_memcpy(dvobj->customer_str,  C2H_CUSTOMER_STR_RPT_BYTE0(data),
			    CUSTOMER_STR_RPT_LEN);
...
int c2h_customer_str_rpt_2_hdl(_adapter *adapter, u8 *data, u8 len)
{
	...
	if (dvobj->customer_str_sctx != NULL) {
		...
		_rtw_memcpy(dvobj->customer_str + CUSTOMER_STR_RPT_LEN,
			    C2H_CUSTOMER_STR_RPT_2_BYTE8(data), CUSTOMER_STR_RPT_2_LEN);
...
s32 rtw_hal_h2c_customer_str_write(_adapter *adapter, const u8 *cs)
{
	...
	ret = rtw_hal_fill_h2c_cmd(adapter, H2C_CUSTOMER_STR_W1,
				   H2C_CUSTOMER_STR_W1_LEN, h2c_data_w1);
	if (ret != _SUCCESS) {
		RTW_WARN("%s w1 fail\n", __func__);
		goto exit;
	}

	ret = rtw_hal_fill_h2c_cmd(adapter, H2C_CUSTOMER_STR_W2,
				   H2C_CUSTOMER_STR_W2_LEN, h2c_data_w2);
	if (ret != _SUCCESS) {
		RTW_WARN("%s w2 fail\n", __func__);
		goto exit;
	}

	ret = rtw_hal_fill_h2c_cmd(adapter, H2C_CUSTOMER_STR_W3,
				   H2C_CUSTOMER_STR_W3_LEN, h2c_data_w3);
	if (ret != _SUCCESS) {
		RTW_WARN("%s w3 fail\n", __func__);
		goto exit;
	}
...
void rtw_hal_update_sta_wset(_adapter *adapter, struct sta_info *psta)
{
	...
	if ((psta->wireless_mode & WIRELESS_11G)
	    || (psta->wireless_mode & WIRELESS_11A))
		w_set |= WIRELESS_OFDM;
	...
	if ((psta->wireless_mode & WIRELESS_11AC)
	    || (psta->wireless_mode & WIRELESS_11_5N))
		w_set |= WIRELESS_VHT;
...
void rtw_hal_update_sta_rate_mask(PADAPTER padapter, struct sta_info *psta)
{
	...
	for (i = 0; i < sizeof(psta->bssrateset); i++) {
		if (psta->bssrateset[i])
			tx_ra_bitmap |= rtw_get_bit_value_from_ieee_value(psta->bssrateset[i] &
					0x7f);
	}
	...
	if (psta->vhtpriv.vht_option) {
		/* AC mode ra_bitmap */
		tx_ra_bitmap |= (rtw_vht_mcs_map_to_bitmap(psta->vhtpriv.vht_mcs_map,
				 tx_nss) << 12);
	} else
		...
		if (psta->htpriv.ht_option) {
			...
			tx_ra_bitmap |= (rtw_ht_mcs_set_to_bitmap(psta->htpriv.ht_cap.supp_mcs_set,
					 tx_nss) << 12);
		}
#endif /* CONFIG_80211N_HT */
...
u32 rtw_sec_read_cam(_adapter *adapter, u8 addr)
{
	...
	if (DBG_SEC_CAM_ACCESS || timeout) {
		RTW_INFO(FUNC_ADPT_FMT" addr:0x%02x, rdata:0x%08x, to:%u, polling:%u, %d ms\n"
			 , FUNC_ADPT_ARG(adapter), addr, rdata, timeout, cnt,
			 rtw_get_time_interval_ms(start, end));
	}
...
void rtw_sec_write_cam(_adapter *adapter, u8 addr, u32 wdata)
{
	...
	if (DBG_SEC_CAM_ACCESS || timeout) {
		RTW_INFO(FUNC_ADPT_FMT" addr:0x%02x, wdata:0x%08x, to:%u, polling:%u, %d ms\n"
			 , FUNC_ADPT_ARG(adapter), addr, wdata, timeout, cnt,
			 rtw_get_time_interval_ms(start, end));
	}
}

void rtw_sec_read_cam_ent(_adapter *adapter, u8 id, u8 *ctrl, u8 *mac,
			  u8 *key)
{
...
void rtw_sec_write_cam_ent(_adapter *adapter, u8 id, u16 ctrl, u8 *mac,
			   u8 *key)
{
	unsigned int i;
	int j;
	...
	for (; j >= 0; j--) {
		...
		default:
			i = (j - 2) << 2;
			wdata = (key[i] | (key[i + 1] << 8) | (key[i + 2] << 16) |
				 (key[i + 3] << 24));
			break;
		}
...
static u8 _rtw_mbid_cam_search_by_macaddr(_adapter *adapter, u8 *mac_addr)
{
	...
	for (i = 0; i < TOTAL_MBID_CAM_NUM; i++) {
		if (mac_addr
		    && _rtw_memcmp(dvobj->mbid_cam_cache[i].mac_addr, mac_addr,
				   ETH_ALEN) == _TRUE) {
			cam_id = i;
			break;
		}
...
static u8 _rtw_mbid_cam_search_by_ifaceid(_adapter *adapter, u8 iface_id)
{
	...
	if (cam_id != INVALID_CAM_ID)
		RTW_INFO("%s iface_id:%d mac:"MAC_FMT" - cam_id:%d\n",
			 __func__, iface_id, MAC_ARG(dvobj->mbid_cam_cache[cam_id].mac_addr),
			 cam_id);
...
static inline void mbid_cam_cache_init(_adapter *adapter,
				       struct mbid_cam_cache *pmbid_cam, u8 *mac_addr)
{
...
int rtw_mbid_cam_cache_dump(void *sel, const char *fun_name,
			    _adapter *adapter)
{
	_irqL irqL;
	u8 i;
	...
	for (i = 0; i < TOTAL_MBID_CAM_NUM; i++) {
		...
		if (mbid_cam_ctl->bitmap & BIT(i)) {
			...
			RTW_PRINT_SEL(sel, "MAC Addr:"MAC_FMT"\t",
				      MAC_ARG(dvobj->mbid_cam_cache[i].mac_addr));
...
static void read_mbssid_cam(_adapter *padapter, u8 cam_addr, u8 *mac)
{
	...
	rtw_write32(padapter, REG_MBIDCAMCFG_2,
		    BIT_MBIDCAM_POLL | ((cam_addr & MBIDCAM_ADDR_MASK) << MBIDCAM_ADDR_SHIFT));
...
static void write_mbssid_cam(_adapter *padapter, u8 cam_addr, u8 *mac)
{
	...
	cam_val[1] = ((cam_addr & MBIDCAM_ADDR_MASK) << MBIDCAM_ADDR_SHIFT)  |
		     (mac[5] << 8) | mac[4];
...
#ifdef RTW_HALMAC
	rtw_halmac_set_mac_address(adapter_to_dvobj(adapter), adapter->hw_port,
				   val);
#else /* !RTW_HALMAC */
...
static void rtw_hal_get_macaddr_port(_adapter *adapter, u8 *mac_addr)
{
	...
	_rtw_memset(mac_addr, 0, ETH_ALEN);
#ifdef RTW_HALMAC
	rtw_halmac_get_mac_address(adapter_to_dvobj(adapter), adapter->hw_port,
				   mac_addr);
#else /* !RTW_HALMAC */
	switch (adapter->hw_port) {
	case HW_PORT0:
...
void rtw_hal_rcr_set_chk_bssid(_adapter *adapter, u8 self_action)
{
	...
	if ((MSTATE_AP_NUM(&mstate)
	     && adapter->registrypriv.wifi_spec) /* for 11n Logo 4.2.31/4.2.32 */
...
static u8 rtw_hal_net_type_decision(_adapter *adapter, u8 net_type)
{
	if ((adapter->hw_port == HW_PORT0)
	    && (rtw_get_mbid_cam_entry_num(adapter))) {
...
void hw_var_port_switch(_adapter *adapter)
{
	...
	*/
	...
	rtw_write8(adapter, REG_BCN_CTRL,
		   (bcn_ctrl & (~EN_BCN_FUNCTION)) | DIS_TSF_UDT);
	rtw_write8(adapter, REG_BCN_CTRL_1,
		   (bcn_ctrl_1 & (~EN_BCN_FUNCTION)) | DIS_TSF_UDT);
...
s32 rtw_hal_set_default_port_id_cmd(_adapter *adapter, u8 mac_id)
{
	...
	RTW_INFO("%s port_id :%d, mad_id:%d\n", __func__, adapter->hw_port,
		 mac_id);

	ret = rtw_hal_fill_h2c_cmd(adapter, H2C_DEFAULT_PORT_ID,
				   H2C_DEFAULT_PORT_ID_LEN, parm);
...
void rtw_set_p2p_ps_offload_cmd(_adapter *adapter, u8 p2p_ps_state)
{
	...
	_rtw_memcpy((&p2p_ps_para), &hal->p2p_ps_offload,
		    sizeof(hal->p2p_ps_offload));
	...
	_rtw_memcpy(&hal->p2p_ps_offload, (&p2p_ps_para),
		    sizeof(hal->p2p_ps_offload));
...
* @macid_end:
*/
s32 rtw_hal_set_FwMediaStatusRpt_cmd(_adapter *adapter, bool opmode,
				     bool miracast, bool miracast_sink, u8 role, u8 macid, bool macid_ind,
				     u8 macid_end)
{
	...
	ret = rtw_hal_fill_h2c_cmd(adapter, H2C_MEDIA_STATUS_RPT,
				   H2C_MEDIA_STATUS_RPT_LEN, parm);
	if (ret != _SUCCESS)
		goto exit;
...
inline s32 rtw_hal_set_FwMediaStatusRpt_single_cmd(_adapter *adapter,
		bool opmode, bool miracast, bool miracast_sink, u8 role, u8 macid)
{
	return rtw_hal_set_FwMediaStatusRpt_cmd(adapter, opmode, miracast,
						miracast_sink, role, macid, 0, 0);
}

inline s32 rtw_hal_set_FwMediaStatusRpt_range_cmd(_adapter *adapter,
		bool opmode, bool miracast, bool miracast_sink, u8 role, u8 macid,
		u8 macid_end)
{
	return rtw_hal_set_FwMediaStatusRpt_cmd(adapter, opmode, miracast,
						miracast_sink, role, macid, 1, macid_end);
}

void rtw_hal_set_FwRsvdPage_cmd(PADAPTER padapter,
				PRSVDPAGE_LOC rsvdpageloc)
{
	...
	SET_H2CCMD_RSVDPAGE_LOC_PROBE_RSP(u1H2CRsvdPageParm,
					  rsvdpageloc->LocProbeRsp);
	SET_H2CCMD_RSVDPAGE_LOC_PSPOLL(u1H2CRsvdPageParm, rsvdpageloc->LocPsPoll);
	SET_H2CCMD_RSVDPAGE_LOC_NULL_DATA(u1H2CRsvdPageParm,
					  rsvdpageloc->LocNullData);
	SET_H2CCMD_RSVDPAGE_LOC_QOS_NULL_DATA(u1H2CRsvdPageParm,
					      rsvdpageloc->LocQosNull);
	SET_H2CCMD_RSVDPAGE_LOC_BT_QOS_NULL_DATA(u1H2CRsvdPageParm,
			rsvdpageloc->LocBTQosNull);
...
void rtw_hal_switch_gpio_wl_ctrl(_adapter *padapter, u8 index, u8 enable)
{
	...
	if (IS_8723D_SERIES(pHalData->version_id)
	    || IS_8822B_SERIES(pHalData->version_id))
...
void rtw_hal_set_FwAoacRsvdPage_cmd(PADAPTER padapter,
				    PRSVDPAGE_LOC rsvdpageloc)
{
	struct	hal_ops *pHalFunc = &padapter->hal_func;
	...
	if (check_fwstate(pmlmepriv, _FW_LINKED)) {
		SET_H2CCMD_AOAC_RSVDPAGE_LOC_REMOTE_WAKE_CTRL_INFO(u1H2CAoacRsvdPageParm,
				rsvdpageloc->LocRemoteCtrlInfo);
		SET_H2CCMD_AOAC_RSVDPAGE_LOC_ARP_RSP(u1H2CAoacRsvdPageParm,
		...
		SET_H2CCMD_AOAC_RSVDPAGE_LOC_NDP_INFO(u1H2CAoacRsvdPageParm,
						      rsvdpageloc->LocNDPInfo);
#ifdef CONFIG_GTK_OL
		SET_H2CCMD_AOAC_RSVDPAGE_LOC_GTK_RSP(u1H2CAoacRsvdPageParm,
						     rsvdpageloc->LocGTKRsp);
		SET_H2CCMD_AOAC_RSVDPAGE_LOC_GTK_INFO(u1H2CAoacRsvdPageParm,
						      rsvdpageloc->LocGTKInfo);
		SET_H2CCMD_AOAC_RSVDPAGE_LOC_GTK_EXT_MEM(u1H2CAoacRsvdPageParm,
...
void rtw_hal_set_fw_dbg_msg_pkt_rsvd_page_cmd(PADAPTER padapter,
		PRSVDPAGE_LOC rsvdpageloc)
{
	struct	hal_ops *pHalFunc = &padapter->hal_func;
	...
	SET_H2CCMD_RSVDPAGE_LOC_FW_DBG_MSG_PKT(u1H2C_fw_dbg_msg_pkt_parm,
					       rsvdpageloc->loc_fw_dbg_msg_pkt);
	ret = rtw_hal_fill_h2c_cmd(padapter,
				   H2C_FW_DBG_MSG_PKT,
...
#ifdef RTW_HALMAC
	if (rtw_halmac_dump_fifo(adapter_to_dvobj(adapter), 2, addr, size,
				 buffer) < 0)
		rst = _FALSE;
	else
		rst = _TRUE;
...
void rtw_dump_rsvd_page(void *sel, _adapter *adapter, u8 page_offset,
			u8 page_num)
{
	u32 page_size = 0;
	u8 *buffer = NULL;
...
void rtw_dump_fifo(void *sel, _adapter *adapter, u8 fifo_sel,
		   u32 fifo_addr, u32 fifo_size)
{
	u8 *buffer = NULL;
	u8 buff_size = 0;
	...
	rtw_halmac_dump_fifo(adapter_to_dvobj(adapter), fifo_sel, fifo_addr,
			     buff_size, buffer);
...
static u8 rtw_hal_check_wow_ctrl(_adapter *adapter, u8 chk_type)
{
	...
	if (IS_HARDWARE_TYPE_JAGUAR2(adapter)) {
		if (chk_type) {
			...
			while (((fe1_imr & BIT_FS_RXDONE_INT_EN)
				|| (rxpkt_num & BIT_RW_RELEASE_EN)) && trycnt > 1) {
				rtw_msleep_os(20);
...
static u8 rtw_hal_set_keep_alive_cmd(_adapter *adapter, u8 enable,
				     u8 pkt_type)
{
...
static u8 rtw_hal_set_disconnect_decision_cmd(_adapter *adapter, u8 enable)
{
	...
	SET_H2CCMD_DISCONDECISION_PARM_CHECK_PERIOD(u1H2CDisconDecisionParm,
			check_period);
	SET_H2CCMD_DISCONDECISION_PARM_TRY_PKT_NUM(u1H2CDisconDecisionParm,
			trypkt_num);
#ifdef CONFIG_FW_MULTI_PORT_SUPPORT
...
static u8 rtw_hal_set_global_info_cmd(_adapter *adapter, u8 group_alg,
				      u8 pairwise_alg)
{
	struct hal_ops *pHalFunc = &adapter->hal_func;
	u8 ret = _FAIL;
...
static void rtw_hal_set_fw_ap_wow_related_cmd(_adapter *padapter,
		u8 enable)
{
...
static void rtw_hal_construct_P2PBeacon(_adapter *padapter, u8 *pframe,
					u32 *pLength)
{
...
static void rtw_hal_construct_P2PProbeRsp(_adapter *padapter, u8 *pframe,
		u32 *pLength)
{
	/* struct xmit_frame			*pmgntframe; */
	/* struct pkt_attrib			*pattrib; */
...
static void rtw_hal_construct_P2PNegoRsp(_adapter *padapter, u8 *pframe,
		u32 *pLength)
{
...
static void rtw_hal_construct_P2PInviteRsp(_adapter *padapter, u8 *pframe,
		u32 *pLength)
{
	unsigned char category = RTW_WLAN_CATEGORY_PUBLIC;
	u8			action = P2P_PUB_ACTION_ACTION;
...
static void rtw_hal_construct_P2PProvisionDisRsp(_adapter *padapter,
...
u8 rtw_hal_set_FwP2PRsvdPage_cmd(_adapter *adapter,
				 PRSVDPAGE_LOC rsvdpageloc)
{
	...
	SET_H2CCMD_RSVDPAGE_LOC_P2P_BCN(u1H2CP2PRsvdPageParm,
					rsvdpageloc->LocProbeRsp);
	SET_H2CCMD_RSVDPAGE_LOC_P2P_PROBE_RSP(u1H2CP2PRsvdPageParm,
					      rsvdpageloc->LocPsPoll);
	SET_H2CCMD_RSVDPAGE_LOC_P2P_NEGO_RSP(u1H2CP2PRsvdPageParm,
					     rsvdpageloc->LocNullData);
	SET_H2CCMD_RSVDPAGE_LOC_P2P_INVITE_RSP(u1H2CP2PRsvdPageParm,
					       rsvdpageloc->LocQosNull);
	SET_H2CCMD_RSVDPAGE_LOC_P2P_PD_RSP(u1H2CP2PRsvdPageParm,
					   rsvdpageloc->LocBTQosNull);
...
u8 rtw_hal_set_p2p_wowlan_offload_cmd(_adapter *adapter)
{
	...
	struct wifidirect_info	*pwdinfo = &(adapter->wdinfo);
	struct P2P_WoWlan_Offload_t *p2p_wowlan_offload = (struct
			P2P_WoWlan_Offload_t *)offload_cmd;
	struct hal_ops *pHalFunc = &adapter->hal_func;
	u8 ret = _FAIL;
...
void rtw_hal_construct_ProbeRsp(_adapter *padapter, u8 *pframe,
				u32 *pLength,
				       ...
				       u32 *pLength, pno_ssid_t *ssid)
{
	...
	*(fctrl) = 0;
	...
	if (ssid == NULL)
		...
		pframe = rtw_set_ie(pframe, _SSID_IE_, ssid->SSID_len, ssid->SSID,
				    &pktlen);
	}
		...
		*total_pkt_len = index + rc_len;
		...
		if (IS_HARDWARE_TYPE_8188E(adapter) || IS_HARDWARE_TYPE_8812(adapter)) {
			...
			_rtw_memcpy(pframe + index - tx_desc + 2 + RTW_KCK_LEN,
				    kek, RTW_KEK_LEN);
			CurtPktPageNum = (u8)PageNum(tx_desc + 2 + RTW_KCK_LEN + RTW_KEK_LEN,
						     page_size);
		} else {
...
static u8 rtw_hal_wow_pattern_generate(_adapter *adapter, u8 idx,
				       struct rtl_wow_pattern *pwow_pattern)
{
	struct pwrctrl_priv *pwrctl = adapter_to_pwrctl(adapter);
	u8 *pattern;
	u8 len = 0;
...
static u32 _rtw_wow_pattern_read_cam(_adapter *adapter, u8 addr)
{
	...
	rtw_write32(adapter, REG_WKFMCAM_CMD,
		    BIT_WKFCAM_POLLING_V1 | BIT_WKFCAM_ADDR_V2(addr));
...
void rtw_wow_pattern_read_cam_ent(_adapter *adapter, u8 id,
				  struct  rtl_wow_pattern *context)
{
	int i;
	u32 rdata;
...
static void _rtw_wow_pattern_write_cam(_adapter *adapter, u8 addr,
				       u32 wdata)
{
	struct pwrctrl_priv *pwrpriv = adapter_to_pwrctl(adapter);
	_mutex *mutex = &pwrpriv->wowlan_pattern_cam_mutex;
	...
	rtw_write32(adapter, REG_WKFMCAM_CMD,
		    BIT_WKFCAM_POLLING_V1 | BIT_WKFCAM_WE | BIT_WKFCAM_ADDR_V2(addr));
...
void rtw_wow_pattern_write_cam_ent(_adapter *adapter, u8 id,
				   struct  rtl_wow_pattern *context)
{
	int j;
	u8 addr;
...
static u8 _rtw_wow_pattern_clean_cam(_adapter *adapter)
{
	...
	_enter_critical_mutex(mutex, NULL);
	rtw_write32(adapter, REG_WKFMCAM_CMD,
		    BIT_WKFCAM_POLLING_V1 | BIT_WKFCAM_CLR_V1);
...
void rtw_dump_wow_pattern(void *sel, struct rtl_wow_pattern *pwow_pattern,
			  u8 idx)
{
	int j;
...
#ifdef CONFIG_P2P_WOWLAN
void rtw_hal_set_p2p_wow_fw_rsvd_page(_adapter *adapter, u8 *pframe,
				      u16 index,
				      u8 tx_desc, u32 page_size, u8 *page_num, u32 *total_pkt_len,
				      RSVDPAGE_LOC *rsvd_page_loc)
{
...
{
	struct pwrctrl_priv *pwrpriv = adapter_to_pwrctl(adapter);
	struct sta_info	*psta = rtw_get_stainfo(&adapter->stapriv,
						get_bssid(&adapter->mlmepriv));
	struct dvobj_priv *dvobj = adapter_to_dvobj(adapter);
	...
	RTW_INFO("  %02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X\n",
		 ...
		 *(lps_pg_info + 4), *(lps_pg_info + 5), *(lps_pg_info + 6),
		 *(lps_pg_info + 7));
	RTW_INFO("  %02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X\n",
		 *(lps_pg_info + 8), *(lps_pg_info + 9), *(lps_pg_info + 10),
		 *(lps_pg_info + 11),
		 *(lps_pg_info + 12), *(lps_pg_info + 13), *(lps_pg_info + 14),
		 *(lps_pg_info + 15));
	RTW_INFO("==== DBG_LPSPG_INFO_RSVD_PAGE_DUMP====\n");
#endif

	rtw_halmac_download_rsvd_page(dvobj, pwrpriv->lpspg_rsvd_page_locate,
				      lps_pg_info, LPS_PG_INFO_RSVD_LEN);

#ifdef DBG_LPSPG_INFO_DUMP
	RTW_INFO("Get LPS-PG INFO from rsvd page_offset:%d\n",
		 pwrpriv->lpspg_rsvd_page_locate);
	rtw_dump_rsvd_page(RTW_DBGDUMP, adapter, pwrpriv->lpspg_rsvd_page_locate,
			   1);
#endif
}
...
void rtw_hal_lps_pg_rssi_lv_decide(_adapter *adapter, struct sta_info *sta)
{
	...
	RTW_INFO("%s mac-id:%d, rssi:%d, rssi_level:%d, lps_pg_rssi_lv:%d\n",
		 __func__, sta->cmn.mac_id, sta->cmn.rssi_stat.rssi,
		 sta->cmn.ra_info.rssi_level, sta->lps_pg_rssi_lv);
}
...
static void _rtw_hal_set_fw_rsvd_page(_adapter *adapter, bool finished,
				      u8 *page_num)
{
	PHAL_DATA_TYPE pHalData;
...
static void hw_var_set_mlme_sitesurvey(_adapter *adapter, u8 variable,
				       u8 *val)
{
	struct dvobj_priv *dvobj = adapter_to_dvobj(adapter);
	HAL_DATA_TYPE *hal_data = GET_HAL_DATA(adapter);
	...
	if (*((u8 *)val)) { /* under sitesurvey */
		...
		*/
		...
		for (i = 0; i < dvobj->iface_nums; i++) {
			...
			if (rtw_linked_check(iface)
				...
				if (iface->hw_port == HW_PORT1)
					rtw_write8(iface, REG_BCN_CTRL_1, rtw_read8(iface,
							REG_BCN_CTRL_1) | DIS_TSF_UDT);
				else
					rtw_write8(iface, REG_BCN_CTRL, rtw_read8(iface,
							REG_BCN_CTRL) | DIS_TSF_UDT);
		...
		if (IS_8812_SERIES(hal_data->version_id)
		    || IS_8821_SERIES(hal_data->version_id)) {
			/* set 718[1:0]=2'b00 to avoid BF scan hang */
			hal_data->backup_snd_ptcl_ctrl = rtw_read8(adapter,
							 REG_SND_PTCL_CTRL_8812A);
			rtw_write8(adapter, REG_SND_PTCL_CTRL_8812A,
				   (hal_data->backup_snd_ptcl_ctrl & 0xfc));
		}
#endif
		...
		*/
		if (rtw_mi_check_fwstate(adapter,
					 _FW_LINKED | WIFI_AP_STATE | WIFI_MESH_STATE)) {
...
static void rtw_hal_set_hw_update_tsf(PADAPTER padapter)
{
	...
	if (rtw_linked_check(padapter)
		...
		if (padapter->hw_port == HW_PORT1)
			rtw_write8(padapter, REG_BCN_CTRL_1, rtw_read8(padapter,
					REG_BCN_CTRL_1) & (~DIS_TSF_UDT));
		else
#endif
...
static void hw_var_set_correct_tsf(_adapter *adapter)
{
	...
	tsf = mlmeext->TSFValue - rtw_modular64(mlmeext->TSFValue,
						(mlmeinfo->bcn_interval * 1024)) - 1024; /*us*/
...
s32 rtw_hal_ch_sw_oper_offload(_adapter *padapter, u8 channel,
			       u8 channel_offset, u16 bwmode)
{
	PHAL_DATA_TYPE	pHalData =  GET_HAL_DATA(padapter);
	...
	return rtw_hal_fill_h2c_cmd(padapter, H2C_CHNL_SWITCH_OPER_OFFLOAD,
				    sizeof(ch_sw_h2c_buf), ch_sw_h2c_buf);
}
#endif
#endif
...
s32 rtw_hal_set_wifi_port_id_cmd(_adapter *adapter)
{
	...
	return rtw_hal_fill_h2c_cmd(adapter, H2C_BTC_WL_PORT_ID,
				    H2C_BTC_WL_PORT_ID_LEN, h2c_buf);
}
#endif
...
u8 SetHwReg(_adapter *adapter, u8 variable, u8 *val)
{
	...
	case HW_VAR_MBSSID_CAM_WRITE: {
		...
		cmd = BIT_MBIDCAM_POLL | BIT_MBIDCAM_WT_EN | BIT_MBIDCAM_VALID |
		      cam_val[1];
	...
	case HW_VAR_MBSSID_CAM_CLEAR: {
		...
		cmd = BIT_MBIDCAM_POLL | BIT_MBIDCAM_WT_EN | ((entry_id &
				MBIDCAM_ADDR_MASK) << MBIDCAM_ADDR_SHIFT);
		rtw_write32(adapter, REG_MBIDCAMCFG_2, cmd);
	}
	break;
	...
	case HW_VAR_SEC_CFG: {
		...
		if (_rtw_camctl_chk_flags(adapter,
...
u8 rtw_hal_query_txbfer_rf_num(_adapter *adapter)
{
	...
	if ((pregistrypriv->beamformer_rf_num)
	    && (IS_HARDWARE_TYPE_8814AE(adapter) || IS_HARDWARE_TYPE_8814AU(adapter)
		|| IS_HARDWARE_TYPE_8822BU(adapter) || IS_HARDWARE_TYPE_8821C(adapter)))
		return pregistrypriv->beamformer_rf_num;
	else if (IS_HARDWARE_TYPE_8814AE(adapter)
#if 0
#if defined(CONFIG_USB_HCI)
		 || (IS_HARDWARE_TYPE_8814AU(adapter) && (pUsbModeMech->CurUsbMode == 2
				 || pUsbModeMech->HubUsbMode == 2))  /* for USB3.0 */
#endif
#endif
		) {
...
u8 rtw_hal_query_txbfee_rf_num(_adapter *adapter)
{
	...
	if ((pregistrypriv->beamformee_rf_num)
	    && (IS_HARDWARE_TYPE_8814AE(adapter) || IS_HARDWARE_TYPE_8814AU(adapter)
		|| IS_HARDWARE_TYPE_8822BU(adapter) || IS_HARDWARE_TYPE_8821C(adapter)))
...
BOOLEAN GetFractionValueFromString(
	IN		char			*szStr,
	IN OUT	u8				*pInteger,
	IN OUT	u8				*pFraction,
	IN OUT	u32			*pu4bMove
...
BOOLEAN GetU1ByteIntegerFromStringInDecimal(
	IN		char	*Str,
	IN OUT	u8		*pInt
)
{
...
void rtw_hal_check_rxfifo_full(_adapter *adapter)
{
	...
	if (regsty->check_hw_status == 1) {
		...
		if (IS_8188E(pHalData->version_id) ||
			...
			rtw_write8(adapter, REG_RXERR_RPT + 3, rtw_read8(adapter,
					REG_RXERR_RPT + 3) | 0xa0);
			save_cnt = _TRUE;
		} else {
			/* todo: other chips */
		}
		...
		if (save_cnt) {
			...
			pdbgpriv->dbg_rx_fifo_diff_overflow = pdbgpriv->dbg_rx_fifo_curr_overflow -
...
void rtw_get_raw_rssi_info(void *sel, _adapter *padapter)
{
	...
	RTW_PRINT_SEL(sel, "RxRate = %s, PWDBALL = %d(%%), rx_pwr_all = %d(dBm)\n",
		      HDATA_RATE(psample_pkt_rssi->data_rate), psample_pkt_rssi->pwdball,
		      psample_pkt_rssi->pwr_all);
			...
			while ((rtw_end_of_queue_search(phead, plist)) == _FALSE) {
				...
				if (psta) {
					if ((_rtw_memcmp(psta->cmn.mac_addr, bc_addr, 6)  !=   _TRUE)
					    ...
					    && (_rtw_memcmp(psta->cmn.mac_addr, adapter_mac_addr(padapter),
							    6)  !=  _TRUE)) {
						...
						for (j = 0; j < DBG_RX_DFRAME_RAW_DATA_TYPES; j++) {
							...
							isCCKrate = (psta_dframe_info->sta_data_rate <= DESC_RATE11M) ? TRUE :
								    FALSE;
							...
							for (rf_path = 0; rf_path < pHalData->NumTotalRFPath; rf_path++) {
								if (!isCCKrate) {
									RTW_PRINT_SEL(sel, "RF_PATH_%d RSSI:%d(dBm)", rf_path,
										      psta_dframe_info->sta_RxPwr[rf_path]);
									_RTW_PRINT_SEL(sel, ",rx_ofdm_snr:%d(dB)\n",
										       psta_dframe_info->sta_ofdm_snr[rf_path]);
								} else
									RTW_PRINT_SEL(sel, "RF_PATH_%d RSSI:%d(dBm)\n", rf_path,
										      (psta_dframe_info->sta_mimo_signal_strength[rf_path]) - 100);
							}
						}
	...
	if (precvpriv->store_law_data_flag) {
		...
		for (rf_path = 0; rf_path < pHalData->NumTotalRFPath; rf_path++) {
			psample_pkt_rssi->mimo_signal_strength[rf_path] =
				p_phy_info->rx_mimo_signal_strength[rf_path];
			psample_pkt_rssi->mimo_signal_quality[rf_path] =
				p_phy_info->rx_mimo_signal_quality[rf_path];
			if (!isCCKrate) {
			...
			if (psta) {
				...
				if ((_rtw_memcmp(psta->cmn.mac_addr, bc_addr, ETH_ALEN) != _TRUE)
					...
					for (rf_path = 0; rf_path < pHalData->NumTotalRFPath; rf_path++) {
						...
						psta_dframe_info->sta_mimo_signal_strength[rf_path] =
							(p_phy_info->rx_mimo_signal_strength[rf_path]);/*Percentage to dbm*/
...
void rtw_dump_cur_efuse(PADAPTER padapter)
{
	...
	if (hal_data->efuse_file_status == EFUSE_FILE_LOADED)
		RTW_MAP_DUMP_SEL(RTW_DBGDUMP, "EFUSE FILE", hal_data->efuse_eeprom_data,
				 mapsize);
	else
		RTW_MAP_DUMP_SEL(RTW_DBGDUMP, "HW EFUSE", hal_data->efuse_eeprom_data,
				 mapsize);
#endif
}
...
u32 Hal_readPGDataFromConfigFile(PADAPTER padapter)
{
	...
	hal_data->efuse_file_status = ((ret == _FAIL) ? EFUSE_FILE_FAILED :
				       EFUSE_FILE_LOADED);
...
void rtw_bb_rf_gain_offset(_adapter *padapter)
{
	...
	if (value & BIT4 && (registry_par->RegPwrTrimEnable == 1)) {
		...
		if (pHalData->EEPROMRFGainVal != 0xff) {
			...
			phy_set_rf_reg(padapter, RF_PATH_A, REG_RF_BB_GAIN_OFFSET,
	...
	if (kfree_data->flag & KFREE_FLAG_ON)
		rtw_rf_apply_tx_gain_offset(padapter,
					    6); /* input ch6 to select BB_GAIN_2G */
#endif
...
void rtw_set_usb_agg_by_mode_normal(_adapter *padapter,
				    u8 cur_wireless_mode)
{
	...
	if (cur_wireless_mode < WIRELESS_11_24N
		...
		if (0x6 != pHalData->rxagg_usb_size
...
#else /* !CONFIG_PREALLOC_RX_SKB_BUFFER */
		if ((0x5 != pHalData->rxagg_usb_size)
...
void rtw_set_usb_agg_by_mode_customer(_adapter *padapter,
...
#else /* !RTW_HALMAC */
	if (IS_HARDWARE_TYPE_8821U(
		    padapter)) { /* || IS_HARDWARE_TYPE_8192EU(padapter)) */
		/* This AGG_PH_TH only for UsbRxAggMode == USB_RX_AGG_USB */
		if ((pHalData->rxagg_mode == RX_AGG_USB)
		    && (check_fwstate(pmlmepriv, _FW_LINKED) == _TRUE)) {
			if (pdvobjpriv->traffic_stat.cur_tx_tp > 2
			    && pdvobjpriv->traffic_stat.cur_rx_tp < 30)
				rtw_write16(padapter, REG_RXDMA_AGG_PG_TH, 0x1010);
			else if (pdvobjpriv->traffic_stat.last_tx_bytes > 220000
				 && pdvobjpriv->traffic_stat.cur_rx_tp < 30)
...
inline u8 rtw_hal_busagg_qsel_check(_adapter *padapter, u8 pre_qsel,
				    u8 next_qsel)
{
	struct mlme_priv *pmlmepriv = &(padapter->mlmepriv);
	u8 chk_rst = _SUCCESS;
...
void dump_TX_FIFO(_adapter *padapter, u8 page_num, u16 page_size)
{
	...
	for (i = 0 ; i < page_num * count ; i += 2) {
		...
		printk(" %08x %08x ", rtw_read32(padapter, 0x144), rtw_read32(padapter,
				0x148));
		rtw_write32(padapter, 0x140, addr + i + 1);
		printk(" %08x %08x\n", rtw_read32(padapter, 0x144), rtw_read32(padapter,
				0x148));
	}
}
...
u8 rtw_hal_get_gpio(_adapter *adapter, u8 gpio_num)
{
	...
	/* Read GPIO Direction */
	direction = (rtw_read8(adapter,
			       gpio_pin_output_en) & BIT(gpio_num_to_set)) >> gpio_num_to_set;

	/* According the direction to read register value */
	if (direction)
		value = (rtw_read8(adapter,
				   gpio_pin_output_val) & BIT(gpio_num_to_set)) >> gpio_num_to_set;
	else
		value = (rtw_read8(adapter,
				   gpio_pin_input_val) & BIT(gpio_num_to_set)) >> gpio_num_to_set;
...
int  rtw_hal_set_gpio_output_value(_adapter *adapter, u8 gpio_num,
				   bool isHigh)
{
	u8 direction = 0;
	u8 res = -1;
	...
	if (direction) {
		if (isHigh)
			rtw_write8(adapter, gpio_pin_output_val, rtw_read8(adapter,
					gpio_pin_output_val) | BIT(gpio_num_to_set));
		else
			rtw_write8(adapter, gpio_pin_output_val, rtw_read8(adapter,
					gpio_pin_output_val) & ~BIT(gpio_num_to_set));
...
int rtw_hal_config_gpio(_adapter *adapter, u8 gpio_num, bool isOutput)
{
	...
	if (isOutput)
		rtw_write8(adapter, gpio_ctrl_reg_to_set, rtw_read8(adapter,
				gpio_ctrl_reg_to_set) | BIT(gpio_num_to_set));
	else
		rtw_write8(adapter, gpio_ctrl_reg_to_set, rtw_read8(adapter,
				gpio_ctrl_reg_to_set) & ~BIT(gpio_num_to_set));
...
int rtw_hal_register_gpio_interrupt(_adapter *adapter, int gpio_num,
				    void(*callback)(u8 level))
{
	...
	/* Read GPIO direction */
	direction = (rtw_read8(adapter,
			       REG_GPIO_PIN_CTRL + 2) & BIT(gpio_num)) >> gpio_num;
	if (direction) {
	...
	/* Config GPIO Mode */
	rtw_write8(adapter, REG_GPIO_PIN_CTRL + 3, rtw_read8(adapter,
			REG_GPIO_PIN_CTRL + 3) | BIT(gpio_num));
	...
	adapter->gpiointpriv.interrupt_mode = rtw_read8(adapter,
					      REG_HSIMR + 2) ^ value;
	rtw_write8(adapter, REG_GPIO_INTM, adapter->gpiointpriv.interrupt_mode);

	/* Enable GPIO interrupt */
	adapter->gpiointpriv.interrupt_enable_mask = rtw_read8(adapter,
			REG_HSIMR + 2) | BIT(gpio_num);
	rtw_write8(adapter, REG_HSIMR + 2,
		   adapter->gpiointpriv.interrupt_enable_mask);
...
s8 rtw_hal_ch_sw_iqk_info_search(_adapter *padapter, u8 central_chnl,
				 u8 bw_mode)
{
	HAL_DATA_TYPE *pHalData = GET_HAL_DATA(padapter);
	u8 i;
...
void rtw_hal_ch_sw_iqk_info_backup(_adapter *padapter)
{
	...
	res = rtw_hal_ch_sw_iqk_info_search(padapter, pHalData->current_channel,
					    pHalData->current_channel_bw);
	if ((res >= 0) && (res < MAX_IQK_INFO_BACKUP_CHNL_NUM)) {
		rtw_hal_set_hwreg(padapter, HW_VAR_CH_SW_IQK_INFO_BACKUP,
	...
	for (i = 1; i < MAX_IQK_INFO_BACKUP_CHNL_NUM; i++)
		_rtw_memcpy(&(pHalData->iqk_reg_backup[i - 1]),
			    &(pHalData->iqk_reg_backup[i]), sizeof(struct hal_iqk_reg_backup));

	rtw_hal_set_hwreg(padapter, HW_VAR_CH_SW_IQK_INFO_BACKUP,
			  (u8 *) & (pHalData->iqk_reg_backup[MAX_IQK_INFO_BACKUP_CHNL_NUM - 1]));
}

void rtw_hal_ch_sw_iqk_info_restore(_adapter *padapter, u8 ch_sw_use_case)
{
	rtw_hal_set_hwreg(padapter, HW_VAR_CH_SW_IQK_INFO_RESTORE,
			  &ch_sw_use_case);
}

void rtw_dump_mac_rx_counters(_adapter *padapter,
			      struct dbg_rx_counter *rx_counter)
{
	...
	if (IS_HARDWARE_TYPE_JAGUAR(padapter)
	    || IS_HARDWARE_TYPE_JAGUAR2(padapter))
		phy_set_mac_reg(padapter, REG_RXERR_RPT, BIT26, 0x0);/*clear bit-26*/

	phy_set_mac_reg(padapter, REG_RXERR_RPT, BIT28 | BIT29 | BIT30 | BIT31,
			0x3);
	mac_cck_ok	= phy_query_mac_reg(padapter, REG_RXERR_RPT,
					    bMaskLWord);/* [15:0]	  */
	phy_set_mac_reg(padapter, REG_RXERR_RPT, BIT28 | BIT29 | BIT30 | BIT31,
			0x0);
	mac_ofdm_ok	= phy_query_mac_reg(padapter, REG_RXERR_RPT,
					    bMaskLWord);/* [15:0]	 */
	phy_set_mac_reg(padapter, REG_RXERR_RPT, BIT28 | BIT29 | BIT30 | BIT31,
			0x6);
	mac_ht_ok	= phy_query_mac_reg(padapter, REG_RXERR_RPT,
					    bMaskLWord);/* [15:0]	 */
	mac_vht_ok	= 0;
	if (IS_HARDWARE_TYPE_JAGUAR(padapter)
	    || IS_HARDWARE_TYPE_JAGUAR2(padapter)) {
		...
		mac_vht_ok	= phy_query_mac_reg(padapter, REG_RXERR_RPT,
						    bMaskLWord);/* [15:0]*/
		phy_set_mac_reg(padapter, REG_RXERR_RPT, BIT26, 0x0);/*clear bit-26*/
	}
	...
	phy_set_mac_reg(padapter, REG_RXERR_RPT, BIT28 | BIT29 | BIT30 | BIT31,
			0x4);
	mac_cck_err	= phy_query_mac_reg(padapter, REG_RXERR_RPT,
					    bMaskLWord);/* [15:0]	 */
	phy_set_mac_reg(padapter, REG_RXERR_RPT, BIT28 | BIT29 | BIT30 | BIT31,
			0x1);
	mac_ofdm_err	= phy_query_mac_reg(padapter, REG_RXERR_RPT,
					    bMaskLWord);/* [15:0]	 */
	phy_set_mac_reg(padapter, REG_RXERR_RPT, BIT28 | BIT29 | BIT30 | BIT31,
			0x7);
	mac_ht_err	= phy_query_mac_reg(padapter, REG_RXERR_RPT,
					    bMaskLWord);/* [15:0]		 */
	mac_vht_err	= 0;
	if (IS_HARDWARE_TYPE_JAGUAR(padapter)
	    || IS_HARDWARE_TYPE_JAGUAR2(padapter)) {
		phy_set_mac_reg(padapter, REG_RXERR_RPT, BIT28 | BIT29 | BIT30 | BIT31,
				0x1);
		phy_set_mac_reg(padapter, REG_RXERR_RPT, BIT26, 0x1);
		mac_vht_err	= phy_query_mac_reg(padapter, REG_RXERR_RPT,
						    bMaskLWord);/* [15:0]*/
		phy_set_mac_reg(padapter, REG_RXERR_RPT, BIT26, 0x0);/*clear bit-26*/
	}
	...
	phy_set_mac_reg(padapter, REG_RXERR_RPT, BIT28 | BIT29 | BIT30 | BIT31,
			0x5);
	mac_cck_fa	= phy_query_mac_reg(padapter, REG_RXERR_RPT,
					    bMaskLWord);/* [15:0]	 */
	phy_set_mac_reg(padapter, REG_RXERR_RPT, BIT28 | BIT29 | BIT30 | BIT31,
			0x2);
	mac_ofdm_fa	= phy_query_mac_reg(padapter, REG_RXERR_RPT,
					    bMaskLWord);/* [15:0]	 */
	phy_set_mac_reg(padapter, REG_RXERR_RPT, BIT28 | BIT29 | BIT30 | BIT31,
			0x9);
	mac_ht_fa	= phy_query_mac_reg(padapter, REG_RXERR_RPT,
					    bMaskLWord);/* [15:0]		 */

	/* Mac_DropPacket */
	rtw_write32(padapter, REG_RXERR_RPT, (rtw_read32(padapter,
					      REG_RXERR_RPT) & 0x0FFFFFFF) | Mac_DropPacket);
	DropPacket = rtw_read32(padapter, REG_RXERR_RPT) & 0x0000FFFF;

	rx_counter->rx_pkt_ok = mac_cck_ok + mac_ofdm_ok + mac_ht_ok + mac_vht_ok;
	rx_counter->rx_pkt_crc_error = mac_cck_err + mac_ofdm_err + mac_ht_err +
				       mac_vht_err;
...
void rtw_dump_phy_rx_counters(_adapter *padapter,
			      struct dbg_rx_counter *rx_counter)
{
	u32 cckok = 0, cckcrc = 0, ofdmok = 0, ofdmcrc = 0, htok = 0, htcrc = 0,
	    OFDM_FA = 0, CCK_FA = 0, vht_ok = 0, vht_err = 0;
	if (!rx_counter) {
		rtw_warn_on(1);
		return;
	}
	if (IS_HARDWARE_TYPE_JAGUAR(padapter)
		...
		vht_err	= 0;
		OFDM_FA = phy_query_bb_reg(padapter, 0xCF0,
					   bMaskLWord) + phy_query_bb_reg(padapter, 0xCF2, bMaskLWord) +
			  phy_query_bb_reg(padapter, 0xDA2, bMaskLWord) + phy_query_bb_reg(padapter,
					  0xDA4, bMaskLWord) +
			  phy_query_bb_reg(padapter, 0xDA6, bMaskLWord) + phy_query_bb_reg(padapter,
					  0xDA8, bMaskLWord);
...
void rtw_dump_drv_rx_counters(_adapter *padapter,
			      struct dbg_rx_counter *rx_counter)
{
	struct recv_priv *precvpriv = &padapter->recvpriv;
	if (!rx_counter) {
...
void rtw_dump_phy_rxcnts_preprocess(_adapter *padapter, u8 rx_cnt_mode)
{
	...
	if ((!(padapter->dump_rx_cnt_mode & DUMP_PHY_RX_COUNTER))
	    && (rx_cnt_mode & DUMP_PHY_RX_COUNTER)) {
	...
	} else if ((padapter->dump_rx_cnt_mode & DUMP_PHY_RX_COUNTER)
		   && (!(rx_cnt_mode & DUMP_PHY_RX_COUNTER))) {
...
void hal_set_crystal_cap(_adapter *adapter, u8 crystal_cap)
{
	...
	case RTL8188F:
		...
		phy_set_bb_reg(adapter, REG_AFE_XTAL_CTRL, 0x007FF800,
	...
	case RTL8812:
		...
		phy_set_bb_reg(adapter, REG_MAC_PHY_CTRL, 0x7FF80000,
			       (crystal_cap | (crystal_cap << 6)));
		break;
#endif
	...
	case RTL8192E:
		...
		phy_set_bb_reg(adapter, REG_MAC_PHY_CTRL, 0x00FFF000,
			       (crystal_cap | (crystal_cap << 6)));
		break;
#endif
#if defined(CONFIG_RTL8814A)
	case RTL8814A:
		...
		phy_set_bb_reg(adapter, REG_MAC_PHY_CTRL, 0x07FF8000,
			       (crystal_cap | (crystal_cap << 6)));
		break;
#endif
...
void rtw_hal_correct_tsf(_adapter *padapter, u8 hw_port, u64 tsf)
{
	if (hw_port == HW_PORT0) {
		...
		/*disable related TSF function*/
		rtw_write8(padapter, REG_BCN_CTRL_1, rtw_read8(padapter,
				REG_BCN_CTRL_1) & (~EN_BCN_FUNCTION));
...
void hw_var_set_opmode_mbid(_adapter *Adapter, u8 mode)
{
	...
	rtw_write8(Adapter, REG_BCN_CTRL, rtw_read8(Adapter,
			REG_BCN_CTRL) | DIS_TSF_UDT);
	...
	if ((mode == _HW_STATE_STATION_) || (mode == _HW_STATE_NOLINK_)) {
		...
		rtw_write8(Adapter, REG_BCN_CTRL,
			   DIS_TSF_UDT | EN_BCN_FUNCTION | DIS_ATIM);/*disable atim wnd*/
		...
		rtw_write8(Adapter, REG_CCK_CHECK, rtw_read8(Adapter,
				REG_CCK_CHECK) | BIT_EN_BCN_PKT_REL);
#endif

		if (IS_HARDWARE_TYPE_8821(Adapter)
		    || IS_HARDWARE_TYPE_8192E(Adapter))/* select BCN on port 0 for DualBeacon*/
			rtw_write8(Adapter, REG_CCK_CHECK, rtw_read8(Adapter,
					REG_CCK_CHECK) & (~BIT_BCN_PORT_SEL));
...
void rtw_dump_phy_cap_by_phydmapi(void *sel, _adapter *adapter)
{
	...
	RTW_PRINT_SEL(sel, "[PHY SPEC] VHT STBC Tx : %s\n",
		      ((phy_spec->stbc_cap >> 24) & 0xFF) ? "Supported" :
		      "N/A"); /*VHT STBC Tx [31:24]*/
	/*VHT STBC Rx [23:16]
	0 = not support
	...
	RTW_PRINT_SEL(sel, "[PHY SPEC] HT STBC Tx : %s\n",
		      ((phy_spec->stbc_cap >> 8) & 0xFF) ? "Supported" :
		      "N/A"); /*HT STBC Tx [15:8]*/
	/*HT STBC Rx [7:0]
	0 = not support
	1 = support for 1 spatial stream
	...
	RTW_PRINT_SEL(sel, "[PHY SPEC] VHT LDPC Tx : %s\n",
		      ((phy_spec->ldpc_cap >> 24) & 0xFF) ? "Supported" :
		      "N/A"); /*VHT LDPC Tx [31:24]*/
	RTW_PRINT_SEL(sel, "[PHY SPEC] VHT LDPC Rx : %s\n",
		      ((phy_spec->ldpc_cap >> 16) & 0xFF) ? "Supported" :
		      "N/A"); /*VHT LDPC Rx [23:16]*/
	RTW_PRINT_SEL(sel, "[PHY SPEC] HT LDPC Tx : %s\n",
		      ((phy_spec->ldpc_cap >> 8) & 0xFF) ? "Supported" :
		      "N/A"); /*HT LDPC Tx [15:8]*/
	...
	RTW_PRINT_SEL(sel, "[PHY SPEC] VHT MU Bfer : %s\n",
		      ((phy_spec->txbf_cap >> 28) & 0xF) ? "Supported" :
		      "N/A"); /*VHT MU Bfer [31:28]*/
	RTW_PRINT_SEL(sel, "[PHY SPEC] VHT MU Bfee : %s\n",
		      ((phy_spec->txbf_cap >> 24) & 0xF) ? "Supported" :
		      "N/A"); /*VHT MU Bfee [27:24]*/
	RTW_PRINT_SEL(sel, "[PHY SPEC] VHT SU Bfer : %s\n",
		      ((phy_spec->txbf_cap >> 20) & 0xF) ? "Supported" :
		      "N/A"); /*VHT SU Bfer [23:20]*/
	RTW_PRINT_SEL(sel, "[PHY SPEC] VHT SU Bfee : %s\n",
		      ((phy_spec->txbf_cap >> 16) & 0xF) ? "Supported" :
		      "N/A"); /*VHT SU Bfee [19:16]*/
	RTW_PRINT_SEL(sel, "[PHY SPEC] HT Bfer : %s\n",
		      ((phy_spec->txbf_cap >> 4) & 0xF)  ? "Supported" :
		      "N/A"); /*HT Bfer [7:4]*/
...
void rtw_hal_switch_chnl_and_set_bw_offload(_adapter *adapter,
		u8 central_ch, u8 pri_ch_idx, u8 bw)
{
	...
	rtw_hal_fill_h2c_cmd(adapter, H2C_SINGLE_CHANNELSWITCH_V2,
			     H2C_SINGLE_CHANNELSWITCH_V2_LEN, h2c);
