...
#endif
static void usb_write_port_complete_not_xmitframe(struct urb *purb,
		struct pt_regs *regs)
{
...
static u32 usb_write_port_not_xmitframe(struct dvobj_priv *d, u8 addr,
					u32 cnt, u8 *wmem)
{
...
static u8 usb_write_data_not_xmitframe(void *d, u8 *pBuf, u32 size,
				       u8 qsel)
{
	struct dvobj_priv *pobj = (struct dvobj_priv *)d;
	PADAPTER padapter = dvobj_get_primary_adapter(pobj);
...
static u8 usb_write_data_rsvd_page(void *d, u8 *pBuf, u32 size)
{
	...
	if (pHalData->not_xmitframe_fw_dl)
		return usb_write_data_not_xmitframe(d, pBuf, size,
