...
static u8 recvbuf2recvframe_proccess_c2h(PADAPTER padapter, u8 *pbuf,
		s32 transfer_len)
{
	u8 ret = _SUCCESS;
...
(PADAPTER padapter, u8 *pbuf, struct rx_pkt_attrib *pattrib,
 union recv_frame *precvframe, _pkt *pskb)
{
	u8 ret = _SUCCESS;
...
int recvbuf2recvframe(PADAPTER padapter, void *ptr)
{
	...
	do {
		...
		if ((pattrib->pkt_len <= 0) || (pkt_offset > transfer_len)) {
			...
			if (pkt_offset > transfer_len)
				RTW_INFO("%s()-%d: RX Warning!,RXDESC_SIZE(%d), drvinfo_sz(%d), shift_sz(%d),pkt_len(%d)\n"
					 , __func__, __LINE__, RXDESC_SIZE, pattrib->drvinfo_sz, pattrib->shift_sz,
					 pattrib->pkt_len);
		...
		case NORMAL_RX:
		default:
			if (recvbuf2recvframe_proccess_normal_rx(padapter, pbuf, pattrib,
					precvframe, pskb) == _FAIL)
				goto _exit_recvbuf2recvframe;
			break;
		}
