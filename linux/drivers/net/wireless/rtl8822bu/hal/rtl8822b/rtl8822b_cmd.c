...
static void rtl8822b_set_FwRsvdPage_cmd(PADAPTER adapter,
...
static void rtl8822b_set_FwAoacRsvdPage_cmd(PADAPTER adapter,
		PRSVDPAGE_LOC rsvdpageloc)
{
#ifdef CONFIG_WOWLAN
	...
	RTW_INFO(FUNC_ADPT_FMT
		 ": RWC=%d ArpRsp=%d NbrAdv=%d GtkRsp=%d GtkInfo=%d ProbeReq=%d NetworkList=%d\n",
		 FUNC_ADPT_ARG(adapter),
	...
	if (check_fwstate(pmlmepriv, _FW_LINKED) == _TRUE) {
		...
		AOAC_RSVD_PAGE_SET_LOC_REMOTE_CTRL_INFO(h2c,
...
void rtl8822b_set_FwMediaStatusRpt_cmd(PADAPTER	adapter, u8 mstatus,
				       u8 macid)
{
	u8 h2c[RTW_HALMAC_H2C_MAX_SIZE] = {0};
	u8 macid_end = 0;
...
static void rtl8822b_set_FwKeepAlive_cmd(PADAPTER adapter, u8 benable,
		u8 pkt_type)
{
	u8 h2c[RTW_HALMAC_H2C_MAX_SIZE] = {0};
	u8 adopt = 1;
...
void rtl8822b_set_FwPwrMode_cmd(PADAPTER adapter, u8 psmode)
{
	...
	if (pwrpriv->dtim > 0)
		RTW_INFO(FUNC_ADPT_FMT ": dtim=%d, HW port id=%d\n",
			 FUNC_ADPT_ARG(adapter),
			 pwrpriv->dtim, psmode == PS_MODE_ACTIVE ? pwrpriv->current_lps_hw_port_id :
			 get_hw_port(adapter));
	else
		RTW_INFO(FUNC_ADPT_FMT ": HW port id=%d\n", FUNC_ADPT_ARG(adapter),
			 psmode == PS_MODE_ACTIVE ? pwrpriv->current_lps_hw_port_id : get_hw_port(
				 adapter));
...
static void SetFwRsvdPagePkt_BTCoex(PADAPTER adapter)
{
	...
	rtw_hal_fill_fake_txdesc(adapter,
				 &ReservedPagePacket[BufIndex - desc_size], BTQosNullLength, _FALSE, _TRUE,
				 _FALSE);
...
static VOID C2HTxRPTHandler_8822b(
	IN	PADAPTER	Adapter,
	IN	u8			*CmdBuf,
	IN	u8			CmdLen
)
{
	...
	adapter_ognl = rtw_get_iface_by_id(GET_PRIMARY_ADAPTER(Adapter),
					   pstapriv->c2h_adapter_id);
	if (!adapter_ognl) {
...
)
{
	...
	psta->sta_stats.tx_retry_cnt = (C2H_SPECIAL_STATISTICS_GET_DATA3(
						CmdBuf) << 8) | C2H_SPECIAL_STATISTICS_GET_DATA2(CmdBuf);
