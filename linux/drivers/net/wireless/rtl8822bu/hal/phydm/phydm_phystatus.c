...
)
{
	...
	struct phy_status_rpt_jaguar2_type1	*phy_sta_rpt = (struct
			phy_status_rpt_jaguar2_type1 *)phy_status_inf;
...
void phydm_process_signal_strength(
	struct dm_struct				*dm,
	struct phydm_phyinfo_struct			*phy_info,
	struct phydm_perpkt_info_struct			*pktinfo
)
{
	...
	avg_rssi = (pktinfo->rate_ss == 1) ? best_rssi : ((best_rssi + second_rssi)
			>> 1);
	...
	if (dm->support_ic_type & PHYSTS_2ND_TYPE_IC) {
		...
		phy_info->signal_strength = SignalScaleProc((PADAPTER)dm->adapter,
					    phy_info->rx_pwdb_all, false, false);
#elif (DM_ODM_SUPPORT_TYPE == ODM_CE)
		phy_info->signal_strength = (u8)(phydm_signal_scale_mapping(dm,
		...
		if (pktinfo->is_cck_rate) {
			...
			phy_info->signal_strength = SignalScaleProc((PADAPTER)dm->adapter,
						    phy_info->rx_pwdb_all, false, true);
#else
			phy_info->signal_strength = (u8)(phydm_signal_scale_mapping(dm,
		...
		if (pktinfo->is_cck_rate) {
			...
			/* 2012/01/12 MH Use customeris signal strength from HalComRxdDesc.c/ */
			phy_info->signal_strength = SignalScaleProc((PADAPTER)dm->adapter,
						    phy_info->rx_pwdb_all, true, true);
#else
			phy_info->signal_strength = (u8)(phydm_signal_scale_mapping(dm,
							 phy_info->rx_pwdb_all));/*pwdb_all;*/
#endif
...
)
{
	...
	for (i = 0; i <= 3; i++) {
		...
		dm->cck_lna_gain_table[used_lna_idx_tmp] = (s8)((reg_0xabc >>
				(8 * i)) & 0xff);
	}
...
)
{
	...
	u8				sq;
	struct phy_status_rpt_8192cd *phy_sta_rpt = (struct phy_status_rpt_8192cd
			*)phy_status_inf;
	...
	if (pktinfo->is_cck_rate) {
		...
		if (dm->iot_table.win_patch_id == RT_CID_819X_LENOVO)
			sq = phydm_sq_patch_rt_cid_819x_lenovo(dm, pktinfo->is_cck_rate, pwdb_all,
							       0, 0);
		else if (dm->iot_table.win_patch_id == RT_CID_819X_ACER)
			sq = phydm_sq_patch_rt_cid_819x_acer(dm, pktinfo->is_cck_rate, pwdb_all, 0,
							     0);
		else
#endif
		...
		for (i = RF_PATH_A; i < PHYDM_MAX_RF_PATH_N; i++) {
			...
			if (i == RF_PATH_A) {
				if (dm->iot_table.win_patch_id == RT_CID_819X_LENOVO) {
					phy_info->signal_quality = phydm_sq_patch_rt_cid_819x_lenovo(dm,
								   pktinfo->is_cck_rate, pwdb_all, i, RSSI);
				} else if (dm->iot_table.win_patch_id == RT_CID_819X_ACER)
					phy_info->signal_quality = phydm_sq_patch_rt_cid_819x_acer(dm,
								   pktinfo->is_cck_rate, pwdb_all, 0, RSSI);
			}
#endif
		}

		/* (2)PWDB, Average PWDB calculated by hardware (for rate adaptive) */
		rx_pwr_all = (((phy_sta_rpt->cck_sig_qual_ofdm_pwdb_all) >> 1) & 0x7f) -
			     110;
		...
		phydm_parsing_cfo(dm, pktinfo, phy_sta_rpt->path_cfotail,
				  pktinfo->rate_ss);
...
)
{
	...
	u8					cck_highpwr = 0;
	u8					LNA_idx, VGA_idx;
	struct phy_status_rpt_8812 *phy_sta_rpt = (struct phy_status_rpt_8812 *)
			phy_status_inf;
	...
	if (pktinfo->is_cck_rate) {
		...
		/*2012.05.25 LukeLee: Testchip AGC report is wrong, it should be restored back to old formula in MP chip*/
		if ((dm->support_ic_type & (ODM_RTL8812 | ODM_RTL8821 | ODM_RTL8881A))
		    && (!dm->is_mp_chip))
...
)
{
	...
	if (pktinfo->is_packet_to_self || pktinfo->is_packet_beacon) {
		if (!pktinfo->is_cck_rate) { /* ofdm rate */
			...
			if (dm->support_ic_type & (ODM_RTL8814A)) {
				...
				if (dm->rx_ant_status & BB_PATH_B) {
					rx_count++;
					rssi_linear += odm_convert_to_linear(
							       phy_info->rx_mimo_signal_strength[RF_PATH_B]);
				}

				if (dm->rx_ant_status & BB_PATH_C) {
					rx_count++;
					rssi_linear += odm_convert_to_linear(
							       phy_info->rx_mimo_signal_strength[RF_PATH_C]);
				}

				if (dm->rx_ant_status & BB_PATH_D) {
					rx_count++;
					rssi_linear += odm_convert_to_linear(
							       phy_info->rx_mimo_signal_strength[RF_PATH_D]);
				}
...
#endif
			{
				if (phy_info->rx_mimo_signal_strength[RF_PATH_B] == 0) {
					...
					if (phy_info->rx_mimo_signal_strength[RF_PATH_A] >
					    phy_info->rx_mimo_signal_strength[RF_PATH_B]) {
			...
			if (sta->rssi_stat.ofdm_pkt_cnt != 64) {
				i = 63;
				sta->rssi_stat.ofdm_pkt_cnt -= (u8)(((sta->rssi_stat.packet_map >> i) &
								     BIT(0)) - 1);
			}
			...
			if (undecorated_smoothed_cck <= 0) {	/* initialize */
				...
				sta->rssi_stat.cck_sum_power = sta->rssi_stat.cck_sum_power +
							       (u16)phy_info->rx_pwdb_all;
				undecorated_smoothed_cck = sta->rssi_stat.cck_sum_power /
							   sta->rssi_stat.cck_pkt_cnt;
			...
			i = 63;
			sta->rssi_stat.ofdm_pkt_cnt -= (u8)((sta->rssi_stat.packet_map >> i) & BIT(
					0));
		...
		if (sta->rssi_stat.ofdm_pkt_cnt ==
			...
			if (sta->rssi_stat.valid_bit == 64) {
				weighting = ((sta->rssi_stat.ofdm_pkt_cnt) > 4) ? 64 :
					    (sta->rssi_stat.ofdm_pkt_cnt << 4);
				...
				if (sta->rssi_stat.valid_bit != 0)
					undecorated_smoothed_pwdb =
						(sta->rssi_stat.ofdm_pkt_cnt * undecorated_smoothed_ofdm +
						 (sta->rssi_stat.valid_bit - sta->rssi_stat.ofdm_pkt_cnt) *
						 undecorated_smoothed_cck) / sta->rssi_stat.valid_bit;
				else
					undecorated_smoothed_pwdb = 0;

				PHYDM_DBG(dm, DBG_RSSI_MNTR,
					  "PWDB_2[%d] = (( %d )), ofdm_pkt = (( %d )), Valid_Bit = (( %d ))\n",
					  pktinfo->station_id, undecorated_smoothed_cck, sta->rssi_stat.ofdm_pkt_cnt,
					  sta->rssi_stat.valid_bit);
			}
		}


		if ((sta->rssi_stat.ofdm_pkt_cnt >= 1 || sta->rssi_stat.cck_pkt_cnt >= 5)
		    && (sta->rssi_stat.is_send_rssi == RA_RSSI_STATE_INIT)) {
			send_rssi_2_fw = 1;
		...
		if (send_rssi_2_fw) { /* Trigger init rate by RSSI */
			...
			PHYDM_DBG(dm, DBG_RSSI_MNTR,
				  ...
				  undecorated_smoothed_pwdb, sta->rssi_stat.ofdm_pkt_cnt,
				  sta->rssi_stat.cck_pkt_cnt);
...
void phydm_print_phy_status_jarguar2(
	struct 	dm_struct			*dm,
	u8								*phy_status_inf,
	struct 	phydm_perpkt_info_struct	*pktinfo,
	struct 	phydm_phyinfo_struct		*phy_info,
...
{
	struct phy_status_rpt_jaguar2_type0	*rpt0 = (struct
			phy_status_rpt_jaguar2_type0 *)phy_status_inf;
	struct phy_status_rpt_jaguar2_type1	*rpt = (struct
			phy_status_rpt_jaguar2_type1 *)phy_status_inf;
	struct phy_status_rpt_jaguar2_type2	*rpt2 = (struct
			phy_status_rpt_jaguar2_type2 *)phy_status_inf;
	struct odm_phy_dbg_info		*dbg = &dm->phy_dbg_info;
	u32	phy_status[PHY_STATUS_JRGUAR2_DW_LEN] = {0};
	u8	i;

	odm_move_memory(dm, phy_status, phy_status_inf,
	...
	pr_debug("StaID=%d, RxRate = 0x%x match_bssid=%d\n", pktinfo->station_id,
		 pktinfo->data_rate, pktinfo->is_packet_match_bssid);
	...
	if (phy_status_page_num == 0) {
		...
		pr_debug("[4] band=%d, CH=%d, agc_table = %d, rxsc = %d\n", rpt0->band,
			 rpt0->channel, rpt0->agc_table, rpt0->rxsc);
		pr_debug("[8] AntIdx[D:A]={%d, %d, %d, %d}, LSIG_len=%d\n",
			 rpt0->antidx_d, rpt0->antidx_c, rpt0->antidx_b, rpt0->antidx_a,
			 rpt0->length);
		...
		pr_debug("[8] AntIdx[D:A]={%d, %d, %d, %d}, LSIG_len=%d\n",
			 rpt->antidx_d, rpt->antidx_c, rpt->antidx_b, rpt->antidx_a,
			 rpt->lsig_length);
		...
		pr_debug("[20] CFO[D:A]={%d, %d, %d, %d}\n", rpt->cfo_tail[3],
			 rpt->cfo_tail[2], rpt->cfo_tail[1], rpt->cfo_tail[0]);
		pr_debug("[24] SNR[D:A]={%d, %d, %d, %d}\n\n", rpt->rxsnr[3],
			 rpt->rxsnr[2], rpt->rxsnr[1], rpt->rxsnr[0]);
		...
		pr_debug("[4] BF: %d, ldpc=%d, stbc=%d, g_bt=%d, antsw=%d, band=%d, CH=%d, rxsc[ht, l]={%d, %d}\n",
			 ...
			 rpt2->hw_antsw_occu, rpt2->band, rpt2->channel, rpt2->ht_rxsc,
			 rpt2->l_rxsc);
		...
		pr_debug("[12] (TRSW|Gain)[D:A]={%d %d, %d %d, %d %d, %d %d}, cnt_cca2agc_rdy=%d\n",
			 ...
			 rpt2->trsw_b, rpt2->gain_b, rpt2->trsw_a, rpt2->gain_a,
		...
		pr_debug("[16] AAGC step[D:A]={%d, %d, %d, %d} HT AAGC gain[D:A]={%d, %d, %d, %d}\n",
			 ...
			 rpt2->ht_aagc_gain[3], rpt2->ht_aagc_gain[2], rpt2->ht_aagc_gain[1],
			 rpt2->ht_aagc_gain[0]);
...
)
{
	...
	phy_info->cfo_tail[rx_path] = (cfo_tail * 5) >>
				      1; /* CFO(kHz) = CFO_tail * 312.5(kHz) / 2^7 ~= CFO tail * 5/2 (kHz)*/
	phy_info->rx_mimo_evm_dbm[rx_path] = evm_dbm;
	phy_info->rx_mimo_signal_strength[rx_path] = phydm_query_rx_pwr_percentage(
				rx_pwr);
...
)
{
	...
	phy_info->rx_pwdb_all = phydm_query_rx_pwr_percentage(
...
)
{
	...
	struct phy_status_rpt_jaguar2_type0	*phy_sta_rpt = (struct
			phy_status_rpt_jaguar2_type0 *)phy_status_inf;
	u8							sq = 0;
	...
	if (dm->cck_new_agc == false) {
		...
		if (dm->support_ic_type & ODM_RTL8197F)
			rx_power = phydm_cck_rssi_convert(dm, phy_sta_rpt->lna_l,
							  phy_sta_rpt->vga);
		else
#endif
		{
	...
	if (dm->support_ic_type & ODM_RTL8197F) {
		...
		if ((phy_sta_rpt->bb_power < bb_pwr_th_l)
		    || (phy_sta_rpt->bb_power > bb_pwr_th_h))
	...
	phydm_set_per_path_phy_info(RF_PATH_A, rx_power, 0, 0, 0,
				    phy_info);					/* Update per-path information */
...
)
{
	...
	phydm_set_common_phy_info(rx_pwr_db, phy_sta_rpt->channel,
				  (boolean)phy_sta_rpt->beamformed,
...
{
	struct phy_status_rpt_jaguar2_type2	*phy_sta_rpt = (struct
			phy_status_rpt_jaguar2_type2 *)phy_status_inf;
...
#if (RTL8197F_SUPPORT == 1)
			if ((dm->support_ic_type & ODM_RTL8197F)
			    && (phy_sta_rpt->pwdb[i] == 0x7f)) { /*for 97f workaround*/
	...
	/* Update packet information */
	phydm_set_common_phy_info(rx_pwr_db_max, phy_sta_rpt->channel,
				  (boolean)phy_sta_rpt->beamformed,
...
)
{
	...
	if (pktinfo->is_packet_to_self || pktinfo->is_packet_beacon) {
		...
		if (rssi_pre <= 0) {
			...
			sta->rssi_stat.rssi_acc = sta->rssi_stat.rssi_acc -
						  (sta->rssi_stat.rssi_acc >> RSSI_MA_FACTOR) + rssi_avg_db;
			sta->rssi_stat.rssi = (s8)((sta->rssi_stat.rssi_acc + (1 <<
						    (RSSI_MA_FACTOR - 1))) >> RSSI_MA_FACTOR);
		}
...
#if (RTL8822B_SUPPORT)
	if (dm->support_ic_type & ODM_RTL8822B)
		phydm_print_phy_status_jarguar2(dm, phy_status_inf, pktinfo, phy_info,
						phy_status_type);
#endif
...
#if	ODM_IC_11AC_SERIES_SUPPORT
		phydm_rx_phy_status_jaguar_series_parsing(dm, phy_info, phy_status_inf,
				pktinfo);
		phydm_process_rssi_for_dm(dm, phy_info, pktinfo);
#endif
	} else if (dm->support_ic_type & ODM_IC_11N_SERIES) {
#if	ODM_IC_11N_SERIES_SUPPORT
		phydm_rx_phy_status92c_series_parsing(dm, phy_info, phy_status_inf,
						      pktinfo);
		phydm_process_rssi_for_dm(dm, phy_info, pktinfo);
#endif
	}
