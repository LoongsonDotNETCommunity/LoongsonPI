...
)
{
	...
	current_channel = odm_get_rf_reg(dm, RF_PATH_A, ODM_CHANNEL,
					 RFREGOFFSETMASK);
	...
	/* Set PSD 128 pts */
	odm_set_bb_reg(dm, REG_FPGA0_PSD_FUNCTION, BIT(14) | BIT15,
		       0x0); /* 128 pts */

	/* To SET CH1 to do */
	odm_set_rf_reg(dm, RF_PATH_A, ODM_CHANNEL, RFREGOFFSETMASK,
		       0x7401);     /* channel 1 */
...
)
{
	...
	/* if(HP id) */
	{
		if (dm->dm_swat_table.rssi_ant_dect_result == true
		...
		if (dm->support_ic_type == ODM_RTL8723B) {
			if (dm_swat_table->swas_no_link_bk_reg948 == 0xff)
				dm_swat_table->swas_no_link_bk_reg948 = odm_read_4byte(dm,
	...
	if (hal_data->eRFPowerState != eRfOn || mgnt_info->RFChangeInProgress
	    || mgnt_info->bMediaConnect) {
	...
	if (dm_swat_table->swas_no_link_state == 0) {
		...
		PlatformMoveMemory((void *)mgnt_info->tmpbssDesc,
				   (void *)mgnt_info->bssDesc, sizeof(RT_WLAN_BSS) * MAX_BSS_DESC);
		...
		if (dm->support_ic_type & (ODM_RTL8188E | ODM_RTL8821)) {
			...
			if ((dm_swat_table->cur_antenna == MAIN_ANT)
		...
		if (dm->support_ic_type & (ODM_RTL8188E | ODM_RTL8821)) {
			...
			if (counter == 0) {
				...
				/* 2 [ Dual Antenna ] */
				else if ((mgnt_info->NumBssDesc != 0)
			...
			PHYDM_DBG(dm, DBG_ANT_DIV,
				  ...
				  dm->dm_swat_table.dual_ant_counter, dm->dm_swat_table.single_ant_counter,
				  dm->dm_swat_table.retry_counter, dm->dm_swat_table.aux_fail_detec_counter);

			/* 2 recover the antenna setting */

			if (dm->dm_swat_table.ANTB_ON == false)
				odm_set_bb_reg(dm, REG_S0_S1_PATH_SWITCH, 0xfff,
...
)
{
	...
	odm_set_bb_reg(dm, REG_FPGA0_RFMOD, BIT(24),
		       0); /* disable whole CCK block */
	odm_write_1byte(dm, REG_TXPAUSE,
			0xFF); /* Turn off TX  ->  Pause TX Queue */
	odm_set_bb_reg(dm, 0xC14, MASKDWORD,
	...
	odm_set_rf_reg(dm, RF_PATH_A, ODM_CHANNEL, 0x7ff,
		       0x04);     /* Set RF to CH4 & 40M */
	odm_set_bb_reg(dm, REG_FPGA0_ANALOG_PARAMETER4, 0xf00000,
	...
	for (i = 0; i < test_num; i++) {
		for (tone_idx = 0; tone_idx < tone_lenth_1; tone_idx++) {
			PSD_report_temp = phydm_get_psd_data(dm, tone_idx_1[tone_idx],
							     initial_gain);
	...
	for (i = 0; i < test_num; i++) {
		for (tone_idx = 0; tone_idx < tone_lenth_2; tone_idx++) {
			PSD_report_temp = phydm_get_psd_data(dm, tone_idx_2[tone_idx],
							     initial_gain);
			/* if(  PSD_report_temp>psd_report_main[tone_idx]  ) */
