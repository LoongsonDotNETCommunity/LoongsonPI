...
void halrf_rf_lna_setting_8822b(
...
boolean get_mix_mode_tx_agc_bb_swing_offset_8822b(
	void				*dm_void,
...
)
{
	...
	if (cali_info->absolute_ofdm_swing_idx[rf_path] >= 0
	    && cali_info->absolute_ofdm_swing_idx[rf_path] <= tx_power_index_offest) {
...
void odm_tx_pwr_track_set_pwr8822b(
	void				*dm_void,
	enum pwrtrack_method	method,
	u8				rf_path,
...
)
{
	...
	PHALMAC_PWR_TRACKING_OPTION p_pwr_tracking_opt = &
			(cali_info->HALMAC_PWR_TRACKING_INFO);
	...
	if (*(dm->mp_mode) == true) {
		...
		if (!rate) { /*auto rate*/
#if (DM_ODM_SUPPORT_TYPE & ODM_WIN)
			tx_rate = ((PADAPTER)adapter)->HalFunc.GetHwRateFromMRateHandler(
					  dm->tx_rate);
#elif (DM_ODM_SUPPORT_TYPE & ODM_CE)
	...
	tx_power_index = phy_get_tx_power_index(adapter, (enum rf_path) rf_path,
	...
	PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
		  ...
		  method, tx_power_index, cali_info->absolute_ofdm_swing_idx[rf_path],
	...
	p_pwr_tracking_opt->pwr_tracking_para[rf_path].tx_pwr_index =
		tx_power_index;
	p_pwr_tracking_opt->pwr_tracking_para[rf_path].pwr_tracking_offset_value =
		cali_info->absolute_ofdm_swing_idx[rf_path];
...
#if (DM_ODM_SUPPORT_TYPE & ODM_WIN)
	tx_power_index = odm_get_tx_power_index(dm, (enum rf_path) rf_path,
						tx_rate, (enum channel_width)band_width, channel);
#elif (DM_ODM_SUPPORT_TYPE & ODM_CE)
	tx_power_index = odm_get_tx_power_index(dm, (enum rf_path) rf_path,
						tx_rate, band_width, channel);
#else
	tx_power_index = config_phydm_read_txagc_8822b(dm, rf_path,
			 0x04); /*0x04(TX_AGC_OFDM_6M)*/
#endif
	...
	PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
		  ...
		  tx_power_index_offest, rf_path);

	if (method == BBSWING) {	/*use for mp driver clean power tracking status*/
		switch (rf_path) {
		case RF_PATH_A:
			odm_set_bb_reg(dm, 0xC94,
				       (BIT(29) | BIT(28) | BIT(27) | BIT(26) | BIT(25)),
				       cali_info->absolute_ofdm_swing_idx[rf_path]);
			odm_set_bb_reg(dm, REG_A_TX_SCALE_JAGUAR, 0xFFE00000,
				       tx_scaling_table_jaguar[cali_info->bb_swing_idx_ofdm[rf_path]]);
			break;
		case RF_PATH_B:
			odm_set_bb_reg(dm, 0xE94,
				       (BIT(29) | BIT(28) | BIT(27) | BIT(26) | BIT(25)),
				       cali_info->absolute_ofdm_swing_idx[rf_path]);
			odm_set_bb_reg(dm, REG_B_TX_SCALE_JAGUAR, 0xFFE00000,
				       tx_scaling_table_jaguar[cali_info->bb_swing_idx_ofdm[rf_path]]);
			break;
		...
		switch (rf_path) {
		case RF_PATH_A:
			get_mix_mode_tx_agc_bb_swing_offset_8822b(dm, method, rf_path,
					tx_power_index_offest);
			odm_set_bb_reg(dm, 0xC94,
				       (BIT(29) | BIT(28) | BIT(27) | BIT(26) | BIT(25)),
				       cali_info->absolute_ofdm_swing_idx[rf_path]);
