...
)
{
	...
	/* JJ ADD 20161014 */
	if (dm->support_ic_type & (ODM_RTL8703B | ODM_RTL8723D |
				   ODM_RTL8710B))	/*for Xtal Offset*/
		(*c.get_delta_swing_xtal_table)(dm, (s8 **)&delta_swing_table_xtal_up,
	...
	delta	= (thermal_value > cali_info->thermal_value) ?
		  (thermal_value - cali_info->thermal_value) : (cali_info->thermal_value -
				  thermal_value);
	...
	if (!(dm->support_ic_type &
		...
		if (cali_info->thermal_value_lck == 0xff) {
			...
			if (!(dm->support_ic_type & ODM_RTL8814A)
			    && !(dm->support_ic_type & ODM_RTL8822B) && c.phy_lc_calibrate)
	...
	if (delta > 0 && cali_info->txpowertrack_control) {
...
#else
		delta = thermal_value > hal_data->eeprom_thermal_meter ?
			(thermal_value - hal_data->eeprom_thermal_meter) :
			(hal_data->eeprom_thermal_meter - thermal_value);
#endif
#else
		...
		if (thermal_value > dm->priv->pmib->dot11RFEntry.ther) {
			...
			for (p = RF_PATH_A; p < c.rf_path_count; p++) {
				cali_info->delta_power_index_last[p] =
					cali_info->delta_power_index[p];	/*recording poer index offset*/
				switch (p) {
				case RF_PATH_B:
					PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
			...
			if (dm->support_ic_type & (ODM_RTL8703B | ODM_RTL8723D | ODM_RTL8710B)) {
				...
				cali_info->xtal_offset_last =
					cali_info->xtal_offset;	/*recording last Xtal offset*/
				PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
					  "[Xtal] delta_swing_table_xtal_up[%d] = %d\n", delta,
					  delta_swing_table_xtal_up[delta]);
				cali_info->xtal_offset = delta_swing_table_xtal_up[delta];
				xtal_offset_eanble = (cali_info->xtal_offset_last !=
			...
			for (p = RF_PATH_A; p < c.rf_path_count; p++) {
				...
				case RF_PATH_B:
					...
					cali_info->delta_power_index[p] = -1 *
									  delta_swing_table_idx_tdown_b[delta];
		...
		for (p = RF_PATH_A; p < c.rf_path_count; p++) {
			...
			if (cali_info->delta_power_index[p] ==
			    cali_info->delta_power_index_last[p])         /*If Thermal value changes but lookup table value still the same*/
				cali_info->power_index_offset[p] = 0;
			else
				cali_info->power_index_offset[p] = cali_info->delta_power_index[p] -
								   cali_info->delta_power_index_last[p];      /*Power index diff between 2 times Power Tracking*/

			PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
				  "[path-%d] power_index_offset(%d) = delta_power_index(%d) - delta_power_index_last(%d)\n",
				  p, cali_info->power_index_offset[p], cali_info->delta_power_index[p],
				  cali_info->delta_power_index_last[p]);

			cali_info->OFDM_index[p] = cali_info->bb_swing_idx_ofdm_base[p] +
			...
			PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
				  ...
				  cali_info->bb_swing_idx_ofdm[p], p, cali_info->bb_swing_idx_ofdm_base[p],
				  cali_info->power_index_offset[p]);
	...
	if ((dm->support_ic_type & ODM_RTL8814A)) {
		...
		if (thermal_value > cali_info->thermal_value) {
			for (p = RF_PATH_A; p < c.rf_path_count; p++) {
				...
				PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
					  ...
					  p, cali_info->power_index_offset[p], delta, thermal_value,
					  rtlefu->eeprom_thermalmeter, cali_info->thermal_value);
#else
				PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
					  "Temperature Increasing(%d): delta_pi: %d, delta_t: %d, Now_t: %d, EFUSE_t: %d, Last_t: %d\n",
					  p, cali_info->power_index_offset[p], delta, thermal_value,
					  hal_data->eeprom_thermal_meter, cali_info->thermal_value);
#endif
			}
...
#endif
		{
			...
			if (dm->support_ic_type == ODM_RTL8188E
			    ...
			    dm->support_ic_type == ODM_RTL8703B || dm->support_ic_type == ODM_RTL8188F
			    || dm->support_ic_type == ODM_RTL8822B ||
			    dm->support_ic_type == ODM_RTL8723D || dm->support_ic_type == ODM_RTL8821C
		...
		cali_info->bb_swing_idx_cck_base =
			cali_info->bb_swing_idx_cck;    /*Record last time Power Tracking result as base.*/
		for (p = RF_PATH_A; p < c.rf_path_count; p++)
...
#if (DM_ODM_SUPPORT_TYPE == ODM_CE) && defined(DM_ODM_CE_MAC80211)
		if (xtal_offset_eanble != 0 && cali_info->txpowertrack_control
		    && (rtlefu->eeprom_thermalmeter != 0xff)) {
#else
		if (xtal_offset_eanble != 0 && cali_info->txpowertrack_control
		    && (hal_data->eeprom_thermal_meter != 0xff)) {
#endif
	...
	if (cali_info->dpk_thermal[RF_PATH_A] != 0) {
		if (diff_DPK[RF_PATH_A] >= c.threshold_dpk) {
			...
			odm_set_bb_reg(dm, 0xcc4, BIT(14) | BIT(13) | BIT(12) | BIT(11) | BIT(10),
	...
	if (cali_info->dpk_thermal[RF_PATH_B] != 0) {
		if (diff_DPK[RF_PATH_B] >= c.threshold_dpk) {
			odm_set_bb_reg(dm, 0x82c, BIT(31), 0x1);
			odm_set_bb_reg(dm, 0xec4, BIT(14) | BIT(13) | BIT(12) | BIT(11) | BIT(10),
