...
)
{
	...
	odm_set_bb_reg(dm, iqk_ctl_addr[rf_path][0], 0xffffffff,
		       psd_cntl_value[rf_path][0]);
	odm_set_bb_reg(dm, iqk_ctl_addr[rf_path][1], 0xffffffff,
		       psd_cntl_value[rf_path][1]);
	odm_set_bb_reg(dm, iqk_ctl_addr[rf_path][2], 0xffffffff,
		       psd_cntl_value[rf_path][0]);
	odm_set_bb_reg(dm, iqk_ctl_addr[rf_path][3], 0xffffffff,
		       psd_cntl_value[rf_path][0]);
...
)
{
	...
	for (rf_path = RF_PATH_A; rf_path <= RF_PATH_B; rf_path++) {
		...
		for (rf0_idx = 1; rf0_idx <= 10; rf0_idx++) {
			...
			ODM_delay_us(100);
			rf56_current_temp[rf_path][rf0_idx] = odm_get_rf_reg(dm, rf_path, 0x56,
							      0xfff);
	...
	for (rf_path = RF_PATH_A; rf_path <= RF_PATH_B; rf_path++) {
		...
		for (rf0_idx = 1; rf0_idx <= 10; rf0_idx++) {
			...
			odm_set_rf_reg(dm, rf_path, 0x33, bRFRegOffsetMask,
				       rf33[rf_path][rf0_idx]);
		}
