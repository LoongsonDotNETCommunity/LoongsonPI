...
void odm_txpowertracking_callback_thermal_meter_92e(
	void		*dm_void
)
{
	struct dm_struct		*dm = (struct dm_struct *)dm_void;
	struct dm_iqk_info	*iqk_info = &dm->IQK_info;
	u8	thermal_value = 0, delta, delta_IQK, delta_LCK, channel, is_decrease,
		rf_mimo_mode;
	u8	thermal_value_avg_count = 0;
	u8     OFDM_min_index =
		10; /* OFDM BB Swing should be less than +2.5dB, which is required by Arthur */
	...
	thermal_value = (unsigned char)odm_get_rf_reg(dm, RF_PATH_A,
			ODM_RF_T_METER_92E, 0xfc00);	/* 0x42: RF Reg[15:10] 88E */
	...
	for (i = 0; i < OFDM_TABLE_SIZE_92E; i++) {
		if (ele_D == (ofdm_swing_table_92e[i] >> 22)) {
			...
			PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
				  "PathA 0xC80[31:22] = 0x%x, OFDM_index=%d\n", ele_D, OFDM_index[0]);
			break;
		}
	}
	...
	if (rf_mimo_mode == RF_2T2R) {
		...
		for (i = 0; i < OFDM_TABLE_SIZE_92E; i++) {
			if (ele_D == (ofdm_swing_table_92e[i] >> 22)) {
				...
				PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
					  "PathB 0xC88[31:22] = 0x%x, OFDM_index=%d\n", ele_D, OFDM_index[1]);
				break;
			}
		}
	}
	...
	{
		priv->pshare->thermal_value_avg_88xx[priv->pshare->thermal_value_avg_index_88xx]
			= thermal_value;
		...
		if (thermal_value_avg_count) {
			thermal_value = (unsigned char)(thermal_value_avg /
							thermal_value_avg_count);
	...
	if (thermal_value != priv->pshare->thermal_value) {
		...
		if (priv->pshare->rf_ft_var.pwr_track_file) {
			PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
				  "diff: (%s)%d ==> get index from table : %d)\n", (is_decrease ? "-" : "+"),
				  delta, get_tx_tracking_index(priv, channel, i, delta, is_decrease, 0));

			if (is_decrease) {
				for (i = 0; i < rf; i++) {
					OFDM_index[i] = priv->pshare->OFDM_index0[i] + get_tx_tracking_index(priv,
							channel, i, delta, is_decrease, 0);
					OFDM_index[i] = ((OFDM_index[i] > (OFDM_TABLE_SIZE_92E- 1)) ?
							 (OFDM_TABLE_SIZE_92E - 1) : OFDM_index[i]);
					PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
						  ">>> decrese power ---> new OFDM_INDEX:%d (%d + %d)\n", OFDM_index[i],
						  priv->pshare->OFDM_index0[i], get_tx_tracking_index(priv, channel, i,
								  delta, is_decrease, 0));
					CCK_index = priv->pshare->CCK_index0 + get_tx_tracking_index(priv, channel,
							i, delta, is_decrease, 1);
					CCK_index = ((CCK_index > (CCK_TABLE_SIZE_92E - 1)) ?
						     (CCK_TABLE_SIZE_92E - 1) : CCK_index);
					PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
						  ">>> Decrese power ---> new CCK_INDEX:%d (%d + %d)\n",  CCK_index,
						  priv->pshare->CCK_index0, get_tx_tracking_index(priv, channel, i, delta,
								  is_decrease, 1));
				}
			} else {
				for (i = 0; i < rf; i++) {
					OFDM_index[i] = priv->pshare->OFDM_index0[i] - get_tx_tracking_index(priv,
							channel, i, delta, is_decrease, 0);
					OFDM_index[i] = ((OFDM_index[i] < OFDM_min_index) ?  OFDM_min_index :
							 OFDM_index[i]);
					PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
						  ">>> Increse power ---> new OFDM_INDEX:%d (%d - %d)\n", OFDM_index[i],
						  priv->pshare->OFDM_index0[i], get_tx_tracking_index(priv, channel, i,
								  delta, is_decrease, 0));
					CCK_index = priv->pshare->CCK_index0 - get_tx_tracking_index(priv, channel,
							i, delta, is_decrease, 1);
					...
					PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
						  ">>> Increse power ---> new CCK_INDEX:%d (%d - %d)\n", CCK_index,
						  priv->pshare->CCK_index0, get_tx_tracking_index(priv, channel, i, delta,
								  is_decrease, 1));
				}
			}
		...
		PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
			  "ofdm_swing_table_92e[(unsigned int)OFDM_index[0]] = %x\n",
			  ofdm_swing_table_92e[(unsigned int)OFDM_index[0]]);
		PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
			  "ofdm_swing_table_92e[(unsigned int)OFDM_index[1]] = %x\n",
			  ofdm_swing_table_92e[(unsigned int)OFDM_index[1]]);

		/* Adujst OFDM Ant_A according to IQK result */
		ele_D = (ofdm_swing_table_92e[(unsigned int)OFDM_index[0]] & 0xFFC00000) >>
			22;
		...
		if (X != 0) {
			...
			phy_set_bb_reg(priv, REG_OFDM_0_XA_TX_IQ_IMBALANCE, MASKDWORD,
				       ofdm_swing_table_92e[(unsigned int)OFDM_index[0]]);
		...
		if (rf == 2) {
			ele_D = (ofdm_swing_table_92e[(unsigned int)OFDM_index[1]] & 0xFFC00000) >>
				22;
			...
			if (X != 0) {
				...
				phy_set_bb_reg(priv, REG_OFDM_0_XB_TX_IQ_IMBALANCE, MASKDWORD,
					       ofdm_swing_table_92e[(unsigned int)OFDM_index[1]]);
...
void odm_txpowertracking_callback_thermal_meter_jaguar_series3(
	void		*dm_void
)
{
#if 1
	...
	u8			*delta_swing_table_idx_tup_a = NULL,
				 *delta_swing_table_idx_tdown_a = NULL;
	u8			*delta_swing_table_idx_tup_b = NULL,
				 *delta_swing_table_idx_tdown_b = NULL;
	u8			*delta_swing_table_idx_tup_cck_a = NULL,
				 *delta_swing_table_idx_tdown_cck_a = NULL;
	u8			*delta_swing_table_idx_tup_cck_b = NULL,
				 *delta_swing_table_idx_tdown_cck_b = NULL;
	/*for 8814 add by Yu Chen*/
	u8			*delta_swing_table_idx_tup_c = NULL,
				 *delta_swing_table_idx_tdown_c = NULL;
	u8			*delta_swing_table_idx_tup_d = NULL,
				 *delta_swing_table_idx_tdown_d = NULL;
	u8			*delta_swing_table_idx_tup_cck_c = NULL,
				 *delta_swing_table_idx_tdown_cck_c = NULL;
	u8			*delta_swing_table_idx_tup_cck_d = NULL,
				 *delta_swing_table_idx_tdown_cck_d = NULL;
	...
	(*c.get_delta_all_swing_table)(dm, (u8 **)&delta_swing_table_idx_tup_a,
				       (u8 **)&delta_swing_table_idx_tdown_a,
				       (u8 **)&delta_swing_table_idx_tup_b, (u8 **)&delta_swing_table_idx_tdown_b,
				       (u8 **)&delta_swing_table_idx_tup_cck_a,
				       (u8 **)&delta_swing_table_idx_tdown_cck_a,
				       (u8 **)&delta_swing_table_idx_tup_cck_b,
				       (u8 **)&delta_swing_table_idx_tdown_cck_b);

	thermal_value = (u8)odm_get_rf_reg(dm, RF_PATH_A, c.thermal_reg_addr,
	...
	PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
		  ...
		  , thermal_value, thermal_value, priv->pmib->dot11RFEntry.ther,
	...
	/* calculate average thermal meter */
	dm->rf_calibrate_info.thermal_value_avg[dm->rf_calibrate_info.thermal_value_avg_index]
		= thermal_value;
	dm->rf_calibrate_info.thermal_value_avg_index++;

	if (dm->rf_calibrate_info.thermal_value_avg_index ==
	...
	delta = RTL_ABS(thermal_value, priv->pmib->dot11RFEntry.ther);
	delta_LCK = RTL_ABS(thermal_value,
			    dm->rf_calibrate_info.thermal_value_lck);
	delta_IQK = RTL_ABS(thermal_value,
				...
				for (p = RF_PATH_A; p < c.rf_path_count; p++) {
					...
					case RF_PATH_B:
						PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
							  ...
							  delta, delta_swing_table_idx_tup_b[delta], delta,
							  delta_swing_table_idx_tup_cck_b[delta]);
					...
					case RF_PATH_C:
						PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
							  ...
							  delta, delta_swing_table_idx_tup_c[delta], delta,
							  delta_swing_table_idx_tup_cck_c[delta]);
					...
					case RF_PATH_D:
						PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
							  ...
							  delta, delta_swing_table_idx_tup_d[delta], delta,
							  delta_swing_table_idx_tup_cck_d[delta]);
					...
					default:
						PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
							  ...
							  delta, delta_swing_table_idx_tup_a[delta], delta,
							  delta_swing_table_idx_tup_cck_a[delta]);
				...
				for (p = RF_PATH_A; p < c.rf_path_count; p++) {
					...
					case RF_PATH_B:
						PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
							  ...
							  delta, delta_swing_table_idx_tdown_b[delta], delta,
							  delta_swing_table_idx_tdown_cck_b[delta]);
						cali_info->absolute_ofdm_swing_idx[p] = -1 *
											delta_swing_table_idx_tdown_b[delta];
						cali_info->absolute_cck_swing_idx[p] = -1 *
										       delta_swing_table_idx_tdown_cck_b[delta];
						PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
					...
					case RF_PATH_C:
						PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
							  ...
							  delta, delta_swing_table_idx_tdown_c[delta], delta,
							  delta_swing_table_idx_tdown_cck_c[delta]);
						cali_info->absolute_ofdm_swing_idx[p] = -1 *
					...
					case RF_PATH_D:
						PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
							  ...
							  delta, delta_swing_table_idx_tdown_d[delta], delta,
							  delta_swing_table_idx_tdown_cck_d[delta]);
						cali_info->absolute_ofdm_swing_idx[p] = -1 *
					...
					default:
						PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
							  ...
							  delta, delta_swing_table_idx_tdown_a[delta], delta,
							  delta_swing_table_idx_tdown_cck_a[delta]);
						cali_info->absolute_ofdm_swing_idx[p] = -1 *
...
)
{
	...
	if (dm->support_ic_type & ODM_RTL8814A)	/* for 8814 path C & D */
		(*c.get_delta_swing_table8814only)(dm, (u8 **)&delta_swing_table_idx_tup_c,
	...
	is_tssi_enable = (boolean)odm_get_rf_reg(dm, RF_PATH_A,
			 REG_RF_TX_GAIN_OFFSET, BIT(7));	/* check TSSI enable */
	...
	for (p = RF_PATH_A ; p < c.rf_path_count ; p++) {
		...
		for (bb_swing_idx = 0; bb_swing_idx < TXSCALE_TABLE_SIZE;
		     bb_swing_idx++) {/* 4 */
			if (ele_D == tx_scaling_table_jaguar[bb_swing_idx]) {
				...
				PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK, "OFDM_index[%d]=%d\n", p,
					  dm->rf_calibrate_info.OFDM_index[p]);
				break;
			}
		}
		PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK, "kfree_offset[%d]=%d\n", p,
			  cali_info->kfree_offset[p]);
			...
			if (is_increase) {		/* thermal is higher than base */
				for (p = RF_PATH_A; p < c.rf_path_count; p++) {
					...
					case RF_PATH_B:
						...
						cali_info->absolute_ofdm_swing_idx[p] =
							delta_swing_table_idx_tup_b[delta];       /* Record delta swing for mix mode power tracking */
						PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
							  "******Temp is higher and dm->absolute_ofdm_swing_idx[RF_PATH_B] = %d\n",
							  cali_info->absolute_ofdm_swing_idx[p]);
						break;
					...
					case RF_PATH_C:
						...
						cali_info->absolute_ofdm_swing_idx[p] =
							delta_swing_table_idx_tup_c[delta];       /* Record delta swing for mix mode power tracking */
						PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
							  "******Temp is higher and dm->absolute_ofdm_swing_idx[RF_PATH_C] = %d\n",
							  cali_info->absolute_ofdm_swing_idx[p]);
						break;
					...
					case RF_PATH_D:
						...
						cali_info->absolute_ofdm_swing_idx[p] =
							delta_swing_table_idx_tup_d[delta];       /* Record delta swing for mix mode power tracking */
						PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
							  "******Temp is higher and dm->absolute_ofdm_swing_idx[RF_PATH_D] = %d\n",
							  cali_info->absolute_ofdm_swing_idx[p]);
						break;
					...
					default:
						...
						cali_info->absolute_ofdm_swing_idx[p] =
							delta_swing_table_idx_tup_a[delta];        /* Record delta swing for mix mode power tracking */
						PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
							  "******Temp is higher and dm->absolute_ofdm_swing_idx[RF_PATH_A] = %d\n",
							  cali_info->absolute_ofdm_swing_idx[p]);
						break;
					}
				}
				...
				for (p = RF_PATH_A; p < c.rf_path_count; p++) {
					...
					case RF_PATH_B:
						PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
							  ...
							  delta_swing_table_idx_tdown_b[delta]);
						cali_info->absolute_ofdm_swing_idx[p] = -1 *
											delta_swing_table_idx_tdown_b[delta];        /* Record delta swing for mix mode power tracking */
						PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
							  "******Temp is lower and dm->absolute_ofdm_swing_idx[RF_PATH_B] = %d\n",
							  cali_info->absolute_ofdm_swing_idx[p]);
						break;
					...
					case RF_PATH_C:
						PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
							  ...
							  delta_swing_table_idx_tdown_c[delta]);
						cali_info->absolute_ofdm_swing_idx[p] = -1 *
											delta_swing_table_idx_tdown_c[delta];        /* Record delta swing for mix mode power tracking */
						PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
							  "******Temp is lower and dm->absolute_ofdm_swing_idx[RF_PATH_C] = %d\n",
							  cali_info->absolute_ofdm_swing_idx[p]);
						break;
					...
					case RF_PATH_D:
						PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
							  ...
							  delta_swing_table_idx_tdown_d[delta]);
						cali_info->absolute_ofdm_swing_idx[p] = -1 *
											delta_swing_table_idx_tdown_d[delta];        /* Record delta swing for mix mode power tracking */
						PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
							  "******Temp is lower and dm->absolute_ofdm_swing_idx[RF_PATH_D] = %d\n",
							  cali_info->absolute_ofdm_swing_idx[p]);
						break;
					...
					default:
						PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
							  ...
							  delta_swing_table_idx_tdown_a[delta]);
						cali_info->absolute_ofdm_swing_idx[p] = -1 *
											delta_swing_table_idx_tdown_a[delta];        /* Record delta swing for mix mode power tracking */
						PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
							  "******Temp is lower and dm->absolute_ofdm_swing_idx[RF_PATH_A] = %d\n",
							  cali_info->absolute_ofdm_swing_idx[p]);
						break;
					}
				}
...
void odm_txpowertracking_callback_thermal_meter_jaguar_series(
	void		*dm_void
)
{
	struct dm_struct		*dm = (struct dm_struct *)dm_void;
	...
	struct rtl8192cd_priv		*priv = dm->priv;
	unsigned char			OFDM_min_index =
		7; /* OFDM BB Swing should be less than +2.5dB, which is required by Arthur and Mimic */
	struct dm_iqk_info	*iqk_info = &dm->IQK_info;
	...
	if (dm->support_ic_type == ODM_RTL8881A) {
		...
		if ((get_bonding_type_8881A() == BOND_8881AM
		     || get_bonding_type_8881A() == BOND_8881AN)
		    && priv->pshare->rf_ft_var.use_intpa8881A
		    && (*dm->band_type == ODM_BAND_2_4G))
			OFDM_min_index =
				6;		/* intPA - upper bond set to +3 dB (base: -2 dB)ot11RFEntry.phy_band_select == PHY_BAND_2G)) */
		else
			OFDM_min_index =
				10;		/* OFDM BB Swing should be less than +1dB, which is required by Arthur and Mimic */
	}
#endif


	thermal_value = (unsigned char)phy_query_rf_reg(priv, RF_PATH_A, 0x42,
			0xfc00, 1); /* 0x42: RF Reg[15:10] 88E */
	PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
		  "\nReadback Thermal Meter = 0x%x pre thermal meter 0x%x EEPROMthermalmeter 0x%x\n",
		  thermal_value, priv->pshare->thermal_value, priv->pmib->dot11RFEntry.ther);
	...
	for (rf_path = 0 ; rf_path < max_rf_path ; rf_path++) {
		...
		PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK, "0x%x:0x%x ([31:21] = 0x%x)\n",
			  bb_swing_reg[rf_path], phy_query_bb_reg(priv, bb_swing_reg[rf_path],
					  MASKDWORD), ele_D);
	...
	/* calculate average thermal meter */
	{
		priv->pshare->thermal_value_avg_8812[priv->pshare->thermal_value_avg_index_8812]
			= thermal_value;
		...
		/* if (*dm->band_type == ODM_BAND_5G) */
		{
			...
			if (priv->pshare->rf_ft_var.pwr_track_file) {
				for (rf_path = 0; rf_path < max_rf_path; rf_path++) {
					PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
						  "diff: (%s)%d ==> get index from table : %d)\n", (is_decrease ? "-" : "+"),
						  delta, get_tx_tracking_index(priv, channel, rf_path, delta, is_decrease,
									       0));
					if (is_decrease) {
						OFDM_index[rf_path] = priv->pshare->OFDM_index0[rf_path] +
								      get_tx_tracking_index(priv, channel, rf_path, delta, is_decrease, 0);
						OFDM_index[rf_path] = ((OFDM_index[rf_path] > (OFDM_TABLE_SIZE_8812 - 1))
								       ? (OFDM_TABLE_SIZE_8812 - 1) : OFDM_index[rf_path]);
						PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
							  ">>> decrese power ---> new OFDM_INDEX:%d (%d + %d)\n",
							  OFDM_index[rf_path], priv->pshare->OFDM_index0[rf_path],
							  get_tx_tracking_index(priv, channel, rf_path, delta, is_decrease, 0));
						...
						if (dm->support_ic_type == ODM_RTL8881A) {
							if (priv->pshare->rf_ft_var.pwrtrk_tx_agc_enable) {
								if (OFDM_index[i] < OFDM_min_index) {
									priv->pshare->add_tx_agc_index = (OFDM_min_index - OFDM_index[i]) /
													 2; /* Calculate Remnant tx_agc value,  2 index for 1 tx_agc */
...
#else
						OFDM_index[rf_path] = ((OFDM_index[rf_path] < OFDM_min_index) ?
								       OFDM_min_index : OFDM_index[rf_path]);
#endif
						PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
							  ">>> increse power ---> new OFDM_INDEX:%d (%d - %d)\n",
							  OFDM_index[rf_path], priv->pshare->OFDM_index0[rf_path],
			...
			for (rf_path = 0; rf_path < max_rf_path; rf_path++) {
				phy_set_bb_reg(priv, bb_swing_reg[rf_path], 0xffe00000,
					       ofdm_swing_table_8812[(unsigned int)OFDM_index[rf_path]]);
				PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
					  "Readback 0x%x[31:21] = 0x%x, OFDM_index:%d\n", bb_swing_reg[rf_path],
					  phy_query_bb_reg(priv, bb_swing_reg[rf_path], 0xffe00000),
					  OFDM_index[rf_path]);
			}
...
)
{
	...
	if (dm->support_ic_type & ODM_RTL8812
	    || dm->support_ic_type & ODM_RTL8881A) {
	...
	u8			OFDM_max_index = 34,
				rf = (is2T) ? 2 : 1; /* OFDM BB Swing should be less than +3.0dB, which is required by Arthur */
	u8			indexforchannel =
		0;/*get_right_chnl_place_for_iqk(hal_data->current_channel)*/
	...
	dm->rf_calibrate_info.txpowertrack_control =
		hal_data->txpowertrack_control; /* <Kordan> We should keep updating the control variable according to HalData.
...
#endif
	PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
		  "===>odm_txpowertracking_callback_thermal_meter_8188e, dm->bb_swing_idx_cck_base: %d, dm->bb_swing_idx_ofdm_base: %d\n",
	...
	*/
	...
	if (thermal_value_avg_count) {
		...
		cali_info->thermal_value_delta = thermal_value -
						 priv->pmib->dot11RFEntry.ther;
		PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK, "AVG Thermal Meter = 0x%x\n",
			  thermal_value);
	}
	...
	delta	  = (thermal_value > dm->rf_calibrate_info.thermal_value) ?
		    (thermal_value - dm->rf_calibrate_info.thermal_value) :
		    (dm->rf_calibrate_info.thermal_value - thermal_value);
	delta_LCK = (thermal_value > dm->rf_calibrate_info.thermal_value_lck) ?
		    (thermal_value - dm->rf_calibrate_info.thermal_value_lck) :
		    (dm->rf_calibrate_info.thermal_value_lck - thermal_value);
	delta_IQK = (thermal_value > dm->rf_calibrate_info.thermal_value_iqk) ?
		    (thermal_value - dm->rf_calibrate_info.thermal_value_iqk) :
		    (dm->rf_calibrate_info.thermal_value_iqk - thermal_value);
	...
	if (delta > 0 && dm->rf_calibrate_info.txpowertrack_control) {
		...
		delta = (thermal_value > dm->priv->pmib->dot11RFEntry.ther) ?
			(thermal_value - dm->priv->pmib->dot11RFEntry.ther) :
			(dm->priv->pmib->dot11RFEntry.ther - thermal_value);

		/* 4 7.1 The Final Power index = BaseIndex + power_index_offset */

		if (thermal_value > dm->priv->pmib->dot11RFEntry.ther) {
			CALCULATE_SWINGTALBE_OFFSET(offset, POWER_INC, index_mapping_NUM_88E,
						    delta);
			dm->rf_calibrate_info.delta_power_index_last =
				dm->rf_calibrate_info.delta_power_index;
			dm->rf_calibrate_info.delta_power_index =
				delta_swing_table_idx[POWER_INC][offset];

		} else {

			CALCULATE_SWINGTALBE_OFFSET(offset, POWER_DEC, index_mapping_NUM_88E,
						    delta);
			dm->rf_calibrate_info.delta_power_index_last =
				dm->rf_calibrate_info.delta_power_index;
			dm->rf_calibrate_info.delta_power_index = (-1) *
					delta_swing_table_idx[POWER_DEC][offset];
		}

		if (dm->rf_calibrate_info.delta_power_index ==
		    dm->rf_calibrate_info.delta_power_index_last)
			dm->rf_calibrate_info.power_index_offset = 0;
		else
			dm->rf_calibrate_info.power_index_offset =
				dm->rf_calibrate_info.delta_power_index -
				dm->rf_calibrate_info.delta_power_index_last;

		for (i = 0; i < rf; i++)
			dm->rf_calibrate_info.OFDM_index[i] = cali_info->bb_swing_idx_ofdm_base +
							      dm->rf_calibrate_info.power_index_offset;
		dm->rf_calibrate_info.CCK_index = cali_info->bb_swing_idx_cck_base +
						  dm->rf_calibrate_info.power_index_offset;

		cali_info->bb_swing_idx_cck = dm->rf_calibrate_info.CCK_index;
		cali_info->bb_swing_idx_ofdm[RF_PATH_A] =
			dm->rf_calibrate_info.OFDM_index[RF_PATH_A];

		PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
			  "The 'CCK' final index(%d) = BaseIndex(%d) + power_index_offset(%d)\n",
			  cali_info->bb_swing_idx_cck, cali_info->bb_swing_idx_cck_base,
			  dm->rf_calibrate_info.power_index_offset);
		PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
			  "The 'OFDM' final index(%d) = BaseIndex(%d) + power_index_offset(%d)\n",
			  cali_info->bb_swing_idx_ofdm[RF_PATH_A], cali_info->bb_swing_idx_ofdm_base,
			  dm->rf_calibrate_info.power_index_offset);
		...
		PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
			  "The thermal meter is unchanged or TxPowerTracking OFF: thermal_value: %d, dm->rf_calibrate_info.thermal_value: %d)\n",
			  thermal_value, dm->rf_calibrate_info.thermal_value);
		dm->rf_calibrate_info.power_index_offset = 0;
	}
	PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
	...
	PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK,
		  ...
		  dm->rf_calibrate_info.OFDM_index[RF_PATH_A],
		  cali_info->bb_swing_idx_ofdm_base);

	if (dm->rf_calibrate_info.power_index_offset != 0
	    && dm->rf_calibrate_info.txpowertrack_control) {
		/* 4 7.2 Configure the Swing Table to adjust Tx Power. */

		dm->rf_calibrate_info.is_tx_power_changed =
		...
		if (thermal_value > dm->rf_calibrate_info.thermal_value) {
			...
			/*	dm->rf_calibrate_info.power_index_offset, delta, thermal_value, hal_data->eeprom_thermal_meter, dm->rf_calibrate_info.thermal_value); */
			   ...
			   dm->rf_calibrate_info.thermal_value) { /* Low temperature */
			/* PHYDM_DBG(dm,ODM_COMP_TX_PWR_TRACK, */
