...
)
{
	...
	odm_move_memory(dm, entry->my_mac_addr, adapter_mac_addr(sta->padapter),
			6);
#endif
...
)
{
	...
	for (i = 0; i < BEAMFORMEE_ENTRY_NUM; i++) {
		if (beam_info->beamformee_entry[i].is_used
		    && (eq_mac_addr(RA, beam_info->beamformee_entry[i].mac_addr))) {
			*idx = i;
...
)
{
	...
	for (i = 0; i < BEAMFORMER_ENTRY_NUM; i++) {
		if (beam_info->beamformer_entry[i].is_used
		    && (eq_mac_addr(TA, beam_info->beamformer_entry[i].mac_addr))) {
			*idx = i;
...
)
{
	...
	for (i = 0; i < BEAMFORMEE_ENTRY_NUM; i++) {
		if (beam_info->beamformee_entry[i].is_used
		    && (mac_id == beam_info->beamformee_entry[i].mac_id)) {
			*idx = i;
			return &beam_info->beamformee_entry[i];
		}
	}
...
)
{
	...
	for (idx = 0; idx < BEAMFORMEE_ENTRY_NUM; idx++) {
		if (beam_info->beamformee_entry[idx].is_used
		    && beam_info->beamformee_entry[idx].is_mu_sta) {
...
{
	struct dm_struct				*dm = (struct dm_struct *)dm_void;
	struct _RT_BEAMFORMEE_ENTRY	*entry = phydm_beamforming_get_free_bfee_entry(
				dm, idx);
	...
	if (entry != NULL) {
		...
		if (phydm_acting_determine(dm, phydm_acting_as_ap)) {
			...
			u16 bssid = ((sta->my_mac_addr[5] & 0xf0) >> 4) ^ (sta->my_mac_addr[5] &
					0xf);
		...
		if (beamform_cap & BEAMFORMER_CAP_VHT_MU) {
			...
			dm->beamforming_info.first_mu_bfee_index =
				phydm_beamforming_get_first_mu_bfee_entry_idx(dm);
...
{
	struct dm_struct				*dm = (struct dm_struct *)dm_void;
	struct _RT_BEAMFORMER_ENTRY	*entry = phydm_beamforming_get_free_bfer_entry(
				dm, idx);
...
)
{
	...
	struct _RT_BEAMFORMER_ENTRY	*bfer_entry =
		phydm_beamforming_get_bfer_entry_by_addr(dm, RA, idx);
	struct _RT_BEAMFORMEE_ENTRY	*entry =
		phydm_beamforming_get_bfee_entry_by_addr(dm, RA, idx);
	boolean ret = false;
...
)
{
	...
	struct _RT_BEAMFORMEE_ENTRY	*entry =
			&beam_info->beamformee_entry[beam_info->beamformee_cur_idx];

	PHYDM_DBG(dm, DBG_TXBF, "[%s] Start!\n", __func__);

	/* 3 TODO  per-client throughput caculation. */

	if ((*dm->current_tx_tp + *dm->current_rx_tp > 2)
	    && ((entry->log_status_fail_cnt <= 20) || status)) {
	...
	for (idx = 0; idx < BEAMFORMEE_ENTRY_NUM; idx++) {
		...
		if ((beamform_entry->beamform_entry_cap & (BEAMFORMER_CAP_HT_EXPLICIT |
				BEAMFORMER_CAP_VHT_SU)) == 0)
			continue;

		if (sound_info->sound_mode == SOUNDING_FW_VHT_TIMER
		    || sound_info->sound_mode == SOUNDING_FW_HT_TIMER) {
...
)
{
	...
	if ((beam_info->tx_bf_data_rate >= ODM_RATEVHTSS3MCS7)
	    && (beam_info->tx_bf_data_rate <= ODM_RATEVHTSS3MCS9)
	    && (!beam_info->snding3ss))
...
)
{
	...
	struct _RT_BEAMFORMING_OID_INFO	beam_oid_info =
			beam_info->beamforming_oid_info;
	struct dm_struct					*dm = (struct dm_struct *)dm_void;
	u8 i;
...
)
{
	...
	if (beam_oid_info.sound_oid_mode == SOUNDING_SW_VHT_TIMER
		...
		if ((support_interface == ODM_ITRF_USB)
		    && !(dm->support_ic_type & (ODM_RTL8814A | ODM_RTL8822B)))
...
)
{
	...
	if (sound_info->sound_idx < BEAMFORMEE_ENTRY_NUM)
		sound_info->sound_mode = phydm_beamforming_sounding_mode(dm, beam_info,
					 sound_info->sound_idx);
	else
	...
	if (sounding_stop_all_timer == sound_info->sound_mode) {
		...
		sound_info->sound_bw = phydm_beamforming_sounding_bw(dm, beam_info,
				       sound_info->sound_mode, sound_info->sound_idx);
		sound_info->sound_period = phydm_beamforming_sounding_time(dm, beam_info,
					   sound_info->sound_mode, sound_info->sound_idx);
		return true;
	}
}
...
)
{
	...
	if (sound_info->sound_mode == SOUNDING_SW_VHT_TIMER
	    || sound_info->sound_mode == SOUNDING_SW_HT_TIMER)
		odm_set_timer(dm, &beam_info->beamforming_timer, sound_info->sound_period);
	else if (sound_info->sound_mode == SOUNDING_HW_VHT_TIMER
		 || sound_info->sound_mode == SOUNDING_HW_HT_TIMER ||
		 sound_info->sound_mode == SOUNDING_AUTO_VHT_TIMER
		 || sound_info->sound_mode == SOUNDING_AUTO_HT_TIMER) {
		...
		phydm_set_hw_reg_handler_interface(dm, HW_VAR_HW_REG_TIMER_STOP,
						   (u8 *)(&timer_type));
		/* odm_write_1byte(dm, 0x15F, 0); */
		/* HW timer init: All IC has the same setting, but 92E & 8812A only write 2 bytes */
		phydm_set_hw_reg_handler_interface(dm, HW_VAR_HW_REG_TIMER_INIT,
						   (u8 *)(&val));
		/* odm_write_1byte(dm, 0x164, 1); */
		/* odm_write_4byte(dm, 0x15C, val); */
		/* HW timer start: All IC has the same setting */
		phydm_set_hw_reg_handler_interface(dm, HW_VAR_HW_REG_TIMER_START,
						   (u8 *)(&timer_type));
		/* odm_write_1byte(dm, 0x15F, 0x5); */
	} else if (sound_info->sound_mode == SOUNDING_FW_VHT_TIMER
		   || sound_info->sound_mode == SOUNDING_FW_HT_TIMER)
	...
	PHYDM_DBG(dm, DBG_TXBF,
		  ...
		  __func__,
		  sound_info->sound_idx, sound_info->sound_mode, sound_info->sound_bw,
		  sound_info->sound_period);
...
)
{
	...
	if (is_delete) {
		...
		} else if (beamform_entry->beamform_entry_state ==
...
)
{
	...
	case BEAMFORMEE_NOTIFY_DELETE_SU:
		...
		if (sound_info->sound_mode == SOUNDING_FW_HT_TIMER
		    || sound_info->sound_mode == SOUNDING_FW_VHT_TIMER) {
			phydm_beamforming_clear_entry_fw(dm, true, idx);
			if (beam_info->beamformee_su_cnt ==
	...
	case BEAMFORMEE_NOTIFY_ADD_MU:
		...
		if (beam_info->beamformee_mu_cnt == 2) {
			/*if (sound_info->sound_mode == SOUNDING_SW_VHT_TIMER || sound_info->sound_mode == SOUNDING_SW_HT_TIMER)
				odm_set_timer(dm, &beam_info->beamforming_timer, sound_info->sound_period);*/
			odm_set_timer(dm, &beam_info->beamforming_timer,
...
)
{
	...
	if (BEAMFORMING_CAP_NONE == sta->ht_beamform_cap
	    && BEAMFORMING_CAP_NONE == sta->vht_beamform_cap) {
		PHYDM_DBG(dm, DBG_TXBF,
	...
	if (!(cmn_sta->support_wireless_set & (WIRELESS_VHT | WIRELESS_HT)))
		...
		if (cmn_sta->support_wireless_set & WIRELESS_HT) {/*HT*/
			if (TEST_FLAG(sta->cur_beamform,
				...
				beamform_cap = (enum beamforming_cap)(beamform_cap |
								      BEAMFORMEE_CAP_HT_EXPLICIT);
				num_of_sounding_dim = (sta->cur_beamform &
			...
			if (TEST_FLAG(sta->cur_beamform, BEAMFORMING_HT_BEAMFORMEE_ENABLE) ||
				...
				beamform_cap = (enum beamforming_cap)(beamform_cap |
								      BEAMFORMER_CAP_HT_EXPLICIT);
				comp_steering_num_of_bfer = (sta->cur_beamform &
			...
			PHYDM_DBG(dm, DBG_TXBF,
				  ...
				  num_of_sounding_dim, comp_steering_num_of_bfer);
		}
		...
		if (cmn_sta->support_wireless_set & WIRELESS_VHT) {	/*VHT*/
			...
			if (TEST_FLAG(sta->cur_beamform_vht, BEAMFORMING_VHT_BEAMFORMER_ENABLE)) {
				...
				num_of_sounding_dim = (sta->cur_beamform_vht &
			...
			if (TEST_FLAG(sta->cur_beamform_vht, BEAMFORMING_VHT_BEAMFORMEE_ENABLE) ||
				...
				comp_steering_num_of_bfer = (sta->cur_beamform_vht &
			...
			PHYDM_DBG(dm, DBG_TXBF,
				  ...
				  sta->cur_beamform_vht, beamform_cap);
			PHYDM_DBG(dm, DBG_TXBF,
	...
	if (beamform_cap & (BEAMFORMEE_CAP_VHT_MU | BEAMFORMEE_CAP_VHT_SU |
			    BEAMFORMEE_CAP_HT_EXPLICIT)) {
		beamformer_entry = phydm_beamforming_get_bfer_entry_by_addr(dm, sta->ra,
				   &bfer_idx);

		if (beamformer_entry == NULL) {
			beamformer_entry = beamforming_add_bfer_entry(dm, sta, beamform_cap,
	...
	if (beamform_cap & (BEAMFORMER_CAP_VHT_MU | BEAMFORMER_CAP_VHT_SU |
			    BEAMFORMER_CAP_HT_EXPLICIT)) {
		beamform_entry = phydm_beamforming_get_bfee_entry_by_addr(dm, sta->ra,
				 &bfee_idx);
		...
		if (beamform_entry == NULL) {
			beamform_entry = beamforming_add_bfee_entry(dm, sta, beamform_cap,
...
)
{
	...
	struct _RT_BEAMFORMER_ENTRY	*bfer_entry =
		phydm_beamforming_get_bfer_entry_by_addr(dm, RA, &idx);
	struct _RT_BEAMFORMEE_ENTRY	*bfee_entry =
		phydm_beamforming_get_bfee_entry_by_addr(dm, RA, &idx);
	boolean ret = false;
...
)
{
	...
	if (entry->is_used == false) {
		...
		if (entry->beamform_entry_state != BEAMFORMING_ENTRY_STATE_INITIALIZED
	...
	if (mode == 1)
		...
		ret = beamforming_send_vht_ndpa_packet(dm, RA, entry->aid, BW,
						       NORMAL_QUEUE);
...
#endif
	if (ret)
		ret = beamforming_start_sw(dm, sound_info->sound_idx,
					   sound_info->sound_mode, sound_info->sound_bw);
	else
		PHYDM_DBG(dm, DBG_TXBF,
			  "%s, Error value return from BeamformingStart_V2\n", __func__);

	if ((beam_info->beamformee_su_cnt != 0)
	    || (beam_info->beamformee_mu_cnt > 1)) {
		if (sound_info->sound_mode == SOUNDING_SW_VHT_TIMER
		    || sound_info->sound_mode == SOUNDING_SW_HT_TIMER)
			odm_set_timer(dm, &beam_info->beamforming_timer, sound_info->sound_period);
		else {
			u32	val = (sound_info->sound_period << 16) | HAL_TIMER_TXBF;
			phydm_set_hw_reg_handler_interface(dm, HW_VAR_HW_REG_TIMER_RESTART,
							   (u8 *)(&val));
		}
	}
}
...
#if (DM_ODM_SUPPORT_TYPE == ODM_WIN)
	PMGNT_INFO                              p_MgntInfo = &(((PADAPTER)(
				adapter))->MgntInfo);
