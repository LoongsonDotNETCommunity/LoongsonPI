...
)
{
	...
	if ((beamforming_info->beamformee_su_cnt > 0)
	    || (beamforming_info->beamformee_mu_cnt > 0)) {
		for (i = RF_PATH_A; i < RF_PATH_B; i++) {
	...
	if (beamforming_info->beamformee_su_cnt > 0) {
		...
		odm_set_bb_reg(dm, REG_BB_TXBF_ANT_SET_BF1_8822B, BIT(28) | BIT29,
			       0x2);			/*enable BB TxBF ant mapping register*/
		...
		odm_set_bb_reg(dm, REG_BB_TX_PATH_SEL_1_8822B, 0xfff00000,
			       0x1); /*1SS by path-A*/
		odm_set_bb_reg(dm, REG_BB_TX_PATH_SEL_2_8822B, MASKLWORD,
			       0x430); /*2SS by path-A,B*/
	}
...
#if 0
void hal_txbf_8822b_download_ndpa(
	void			*adapter,
	u8				idx
)
{
	...
	u16			tx_page_bndy =
		LAST_ENTRY_OF_TX_PKT_BUFFER_8814A; /*default reseved 1 page for the IC type which is undefined.*/
	struct _RT_BEAMFORMING_INFO	*beam_info = GET_BEAMFORM_INFO(adapter);
	struct _RT_BEAMFORMEE_ENTRY	*p_beam_entry = beam_info->beamformee_entry +
				idx;

	hal_data->is_fw_dw_rsvd_page_in_progress = true;
	phydm_get_hal_def_var_handler_interface(dm, HAL_DEF_TX_PAGE_BOUNDARY,
						(u16 *)&tx_page_bndy);
	...
	tmp_reg422 = platform_efio_read_1byte(adapter,
					      REG_FWHW_TXQ_CTRL_8814A + 2);
	platform_efio_write_1byte(adapter, REG_FWHW_TXQ_CTRL_8814A + 2,
				  tmp_reg422 & (~BIT(6)));

	if (tmp_reg422 & BIT(6)) {
		RT_TRACE(COMP_INIT, DBG_LOUD,
			 ("SetBeamformDownloadNDPA_8814A(): There is an adapter is sending beacon.\n"));
		is_send_beacon = true;
	}
	...
	do {
		...
		bcn_valid_reg = platform_efio_read_1byte(adapter,
				REG_FIFOPAGE_CTRL_2_8814A + 1);
		platform_efio_write_1byte(adapter, REG_FIFOPAGE_CTRL_2_8814A + 1,
					  (bcn_valid_reg | BIT(7)));

		/*download NDPA rsvd page.*/
		if (p_beam_entry->beamform_entry_cap & BEAMFORMER_CAP_VHT_SU)
			beamforming_send_vht_ndpa_packet(dm, p_beam_entry->mac_addr,
							 p_beam_entry->AID, p_beam_entry->sound_bw, BEACON_QUEUE);
		else
			beamforming_send_ht_ndpa_packet(dm, p_beam_entry->mac_addr,
							p_beam_entry->sound_bw, BEACON_QUEUE);
	...
	/*0x204[11:0]	Beacon Head for TXDMA*/
	platform_efio_write_2byte(adapter, REG_FIFOPAGE_CTRL_2_8814A,
				  tx_page_bndy);
	...
	u1b_tmp = platform_efio_read_1byte(adapter, REG_CR_8814A + 1);
	platform_efio_write_1byte(adapter, REG_CR_8814A + 1,
				  (u1b_tmp & (~BIT(0))));
...
void hal_txbf_8822b_fw_txbf_cmd(
	void	*adapter
)
{
	u8	idx, period = 0;
	...
	for (idx = 0; idx < BEAMFORMEE_ENTRY_NUM; idx++) {
		if (beam_info->beamformee_entry[idx].is_used
		    && beam_info->beamformee_entry[idx].beamform_entry_state ==
	...
	RT_DISP(FBEAM, FBEAM_FUN,
		("@%s End, PageNum0 = 0x%x, PageNum1 = 0x%x period = %d", __func__,
		 PageNum0, PageNum1, period));
}
#endif

#if 0
void hal_txbf_8822b_init(
	void			*dm_void
)
{
	struct dm_struct	*dm = (struct dm_struct *)dm_void;
	...
	odm_set_bb_reg(dm, 0x14c0, BIT(16),
		       1); /*Enable P1 aggr new packet according to P0 transfer time*/
	odm_set_bb_reg(dm, 0x14c0, BIT(15) | BIT14 | BIT13 | BIT12,
		       10); /*MU Retry Limit*/
	odm_set_bb_reg(dm, 0x14c0, BIT(7),
		       0); /*Disable Tx MU-MIMO until sounding done*/
	...
	/*Temp Settings*/
	odm_set_bb_reg(dm, 0x6DC, 0x3F000000,
		       4); /*STA2's CSI rate is fixed at 6M*/
	odm_set_bb_reg(dm, 0x1C94, MASKDWORD,
		       0xAFFFAFFF); /*Grouping bitmap parameters*/
	...
	if (dm->rf_type == RF_2T2R) { /*2T2R*/
		...
		config_phydm_trx_mode_8822b(dm, (enum bb_path)3, (enum bb_path)3,
					    true);/*Tx2path*/
	}
	...
	odm_set_bb_reg(dm, 0xB54, BIT(30),
		       1); /* RegB54[30] = 1 (force user position) */
	odm_set_bb_reg(dm, 0xB54, (BIT(29) | BIT28),
		       adapter->MgntInfo.sniff_user_position); /* RegB54[29:28] = user position (0~3) */
...
)
{
	...
	u8					nc_index = 0, nr_index = 0, grouping = 0, codebookinfo = 0,
						coefficientsize = 0;
	...
	RT_DISP(FBEAM, FBEAM_FUN,
		("%s: bfer_bfee_idx=%d, bfer_idx=%d, bfee_idx=%d\n", __func__,
		 bfer_bfee_idx, bfer_idx, bfee_idx));

	/*************SU BFer Entry Init*************/
	if ((beamforming_info->beamformer_su_cnt > 0)
	    && (bfer_idx < BEAMFORMER_ENTRY_NUM)) {
		...
		if (beamformer_entry->su_reg_index == 0) {
			for (i = 0; i < 6 ; i++)
				odm_write_1byte(dm, (REG_ASSOCIATED_BFMER0_INFO_8822B + i),
						beamformer_entry->mac_addr[i]);
		} else {
			for (i = 0; i < 6 ; i++)
				odm_write_1byte(dm, (REG_ASSOCIATED_BFMER1_INFO_8822B + i),
						beamformer_entry->mac_addr[i]);
		}

		/*CSI report parameters of Beamformer*/
		nc_index = hal_txbf_8822b_get_nrx(
				   dm);	/*for 8814A nrx = 3(4 ant), min=0(1 ant)*/
		nr_index =
			beamformer_entry->num_of_sounding_dim;	/*0x718[7] = 1 use Nsts, 0x718[7] = 0 use reg setting. as Bfee, we use Nsts, so nr_index don't care*/
	...
	if ((beamforming_info->beamformee_su_cnt > 0)
		...
		if (p_beamformee_entry->su_reg_index == 0) {
			...
			odm_write_1byte(dm, REG_TXBF_CTRL_8822B + 3, odm_read_1byte(dm,
					REG_TXBF_CTRL_8822B + 3) | BIT(4) | BIT(6) | BIT(7));
		} else
			odm_write_2byte(dm, REG_TXBF_CTRL_8822B + 2,
					sta_id | BIT(14) | BIT(15) | BIT(12));
		...
		if (p_beamformee_entry->su_reg_index == 0) {
			...
			odm_write_2byte(dm, REG_ASSOCIATED_BFMEE_SEL_8822B + 2,
					sta_id | 0xE200);	/*Set BIT25*/

		phydm_beamforming_notify(dm);
	}

	/*************MU BFer Entry Init*************/
	if ((beamforming_info->beamformer_mu_cnt > 0)
	    && (bfer_idx < BEAMFORMER_ENTRY_NUM)) {
		beamformer_entry = &beamforming_info->beamformer_entry[bfer_idx];
		...
		/* Set partial AID */
		odm_write_2byte(dm, (REG_ASSOCIATED_BFMER0_INFO_8822B + 6),
				beamformer_entry->p_aid);
		...
		nr_index =
			1;	/*0x718[7] = 1 use Nsts, 0x718[7] = 0 use reg setting. as Bfee, we use Nsts, so nr_index don't care*/
	...
	if ((beamforming_info->beamformee_mu_cnt > 0)
		...
		if (p_beamformee_entry->mu_reg_index == 0xFF) {
			...
			RT_DISP(FBEAM, FBEAM_FUN,
				("%s: ERROR! There is no valid bit in beamformee_mu_reg_maping!\n",
				 __func__));
			return;
		}
		...
		for (i = 0; i < 16; i++) {
			if (i < 4)
				p_beamformee_entry->user_position[i] = (u8)((user_position_l >>
								       (i * 8)) & 0xFF);
			else if (i < 8)
				p_beamformee_entry->user_position[i] = (u8)((user_position_h >> ((
						i - 4) * 8)) & 0xFF);
			else
		...
		beamforming_info->reg_mu_tx_ctrl |= (p_beamformee_entry->mu_reg_index << 8)
						    & (BIT(8) | BIT(9) | BIT(10));
		...
		beamforming_info->reg_mu_tx_ctrl |=
			beamforming_info->beamformee_mu_reg_maping & 0x3F;
		odm_write_4byte(dm, 0x14c0, beamforming_info->reg_mu_tx_ctrl);

		PHYDM_DBG(dm, DBG_TXBF,
			  "@%s, reg_mu_tx_ctrl = 0x%x, user_position_l = 0x%x, user_position_h = 0x%x\n",
			  __func__, beamforming_info->reg_mu_tx_ctrl, user_position_l,
			  user_position_h);
...
#if 1
		{
			...
			odm_write_4byte(dm, ctrl_info_offset,
					(p_beamformee_entry->mu_reg_index + 1) << 16);
			odm_write_1byte(dm, 0x81, 0x80); /*RPTBUF ready*/

			PHYDM_DBG(dm, DBG_TXBF,
				  "@%s, mac_id = %d, ctrl_info_offset = 0x%x, mu_reg_index = %x\n",
				  __func__, p_beamformee_entry->mac_id, ctrl_info_offset,
...
)
{
	...
	if (beamformer_entry->beamform_entry_cap == BEAMFORMING_CAP_NONE) {
		...
		if (beamformer_entry->is_mu_ap == 0) { /*SU BFer */
			...
			beamforming_info->beamformer_su_reg_maping &= ~(BIT(
						beamformer_entry->su_reg_index));
	...
	if (p_beamformee_entry->beamform_entry_cap == BEAMFORMING_CAP_NONE) {
		...
		if (p_beamformee_entry->is_mu_sta == 0) { /*SU BFee*/
			if (p_beamformee_entry->su_reg_index == 0) {
				...
				odm_write_2byte(dm, REG_TXBF_CTRL_8822B + 2,
						0x0 | BIT(14) | BIT(15) | BIT(12));

				odm_write_2byte(dm, REG_ASSOCIATED_BFMEE_SEL_8822B + 2,
						odm_read_2byte(dm, REG_ASSOCIATED_BFMEE_SEL_8822B + 2) & 0x60);
			}
			beamforming_info->beamformee_su_reg_maping &= ~(BIT(
						p_beamformee_entry->su_reg_index));
			...
			p_beamformee_entry->is_mu_sta = false;
			beamforming_info->beamformee_mu_reg_maping &= ~(BIT(
						p_beamformee_entry->mu_reg_index));
...
)
{
	...
	boolean	is_mu_sounding = beamforming_info->is_mu_sounding,
	...
	if (is_mu_sounding == false) {
		...
		if (1) { /* (beamform_entry->beamform_entry_state == BEAMFORMING_ENTRY_STATE_PROGRESSED) { */
			...
			PHYDM_DBG(dm, DBG_TXBF,
				  ...
				  __func__, is_sounding_success[0], is_sounding_success[1],
				  is_sounding_success[2], is_sounding_success[3], is_sounding_success[4],
			...
			for (idx = 0; idx < 6; idx++) {
				beamforming_info->reg_mu_tx_ctrl &= ~(BIT(8) | BIT(9) | BIT(10));
				beamforming_info->reg_mu_tx_ctrl |= ((idx << 8) & (BIT(8) | BIT(9) | BIT(
						10)));
				odm_write_4byte(dm, 0x14c0, beamforming_info->reg_mu_tx_ctrl);
				odm_set_mac_reg(dm, 0x14C4, MASKDWORD,
						gid_valid[idx]); /*set MU STA gid valid table*/
			}
...
)
{
	...
	if (beamforming_info->mu_ap_index < BEAMFORMER_ENTRY_NUM)
		beamformer_entry =
			&beamforming_info->beamformer_entry[beamforming_info->mu_ap_index];
	else
		return;
...
void haltxbf8822b_gtab_translation(
	struct dm_struct			*dm
)
{
	u8 idx, gid;
...
)
{
	...
	if (p_beam_entry->beamform_entry_state ==
...
)
{
	...
	if ((su_bfee_cnt > 0) || (mu_bfee_cnt > 0)) {
		for (i = RF_PATH_A; i <= RF_PATH_B; i++) {
			odm_set_rf_reg(dm, (enum rf_path)i, 0xEF, BIT(19),
				       0x1); /*RF mode table write enable*/
			odm_set_rf_reg(dm, (enum rf_path)i, 0x33, 0xF, 3); /*Select RX mode*/
			odm_set_rf_reg(dm, (enum rf_path)i, 0x3E, 0xfffff,
				       0x00036); /*Set Table data*/
			odm_set_rf_reg(dm, (enum rf_path)i, 0x3F, 0xfffff,
				       0x5AFCE); /*Set Table data*/
			odm_set_rf_reg(dm, (enum rf_path)i, 0xEF, BIT(19),
				       0x0); /*RF mode table write disable*/
		}
	}

	odm_set_bb_reg(dm, REG_BB_TXBF_ANT_SET_BF1_8822B, BIT(30),
		       1);			/*if Nsts > Nc, don't apply V matrix*/

	if (su_bfee_cnt > 0 || mu_bfee_cnt > 0) {
		/*for 8814 19ac(idx 1), 19b4(idx 0), different Tx ant setting*/
		odm_set_bb_reg(dm, REG_BB_TXBF_ANT_SET_BF1_8822B, BIT(28) | BIT29,
			       0x2);	/*enable BB TxBF ant mapping register*/
		odm_set_bb_reg(dm, REG_BB_TXBF_ANT_SET_BF1_8822B, BIT(31),
			       1);			/*ignore user since 8822B only 2Tx*/
		...
		odm_set_bb_reg(dm, REG_BB_TXBF_ANT_SET_BF1_8822B, BIT(28) | BIT29,
			       0x0);	/*enable BB TxBF ant mapping register*/
		odm_set_bb_reg(dm, REG_BB_TXBF_ANT_SET_BF1_8822B, BIT(31),
			       0);			/*ignore user since 8822B only 2Tx*/
...
)
{
	...
	if (enable_su_bfer) {
		...
		if (is_vht)
			odm_set_bb_reg(dm, 0x19f0,
				       BIT(5) | BIT(4) | BIT(3) | BIT(2) | BIT(1) | BIT(0), 0x1f);
		else
			odm_set_bb_reg(dm, 0x19f0,
				       BIT(5) | BIT(4) | BIT(3) | BIT(2) | BIT(1) | BIT(0), 0x22);
