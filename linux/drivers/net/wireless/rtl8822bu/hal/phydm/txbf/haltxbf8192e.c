...
)
{
	...
	odm_set_rf_reg(dm, RF_PATH_A, RF_WE_LUT, 0x80000,
		       0x1); /*RF mode table write enable*/
	odm_set_rf_reg(dm, RF_PATH_B, RF_WE_LUT, 0x80000,
		       0x1); /*RF mode table write enable*/

	if (beam_info->beamformee_su_cnt > 0) {
		/*Path_A*/
		odm_set_rf_reg(dm, RF_PATH_A, 0x30, 0xfffff,
			       0x18000);	/*Select RX mode  0x30=0x18000*/
		odm_set_rf_reg(dm, RF_PATH_A, 0x31, 0xfffff, 0x0000f);	/*Set Table data*/
		odm_set_rf_reg(dm, RF_PATH_A, 0x32, 0xfffff,
			       0x77fc2);	/*Enable TXIQGEN in RX mode*/
		/*Path_B*/
		...
		odm_set_rf_reg(dm, RF_PATH_A, 0x31, 0xfffff, 0x0000f);	/*Set Table data*/
		odm_set_rf_reg(dm, RF_PATH_A, 0x32, 0xfffff,
			       0x77f82);	/*Disable TXIQGEN in RX mode*/
		/*Path_B*/
		odm_set_rf_reg(dm, RF_PATH_B, 0x30, 0xfffff, 0x18000);	/*Select RX mode*/
		odm_set_rf_reg(dm, RF_PATH_B, 0x31, 0xfffff, 0x0000f);	/*Set Table data*/
		odm_set_rf_reg(dm, RF_PATH_B, 0x32, 0xfffff,
...
)
{
	...
	if ((beamforming_info->beamformer_su_cnt > 0)
		...
		if (bfer_idx == 0) {
			for (i = 0; i < 6 ; i++)
				odm_write_1byte(dm, (REG_ASSOCIATED_BFMER0_INFO_8192E+i),
						beamformer_entry.mac_addr[i]);
		} else {
			for (i = 0; i < 6 ; i++)
				odm_write_1byte(dm, (REG_ASSOCIATED_BFMER1_INFO_8192E+i),
						beamformer_entry.mac_addr[i]);
		}
	...
	if ((beamforming_info->beamformee_su_cnt > 0)
		...
		if (bfee_idx == 0) {
			...
			odm_write_1byte(dm, REG_TXBF_CTRL_8192E+3, odm_read_1byte(dm,
					REG_TXBF_CTRL_8192E+3) | BIT(4) | BIT(6) | BIT(7));
		} else
			odm_write_2byte(dm, REG_TXBF_CTRL_8192E+2,
...
)
{
	...
	*/
	...
	if (idx == 0) {
		...
		odm_write_2byte(dm, REG_TXBF_CTRL_8192E+2, odm_read_1byte(dm,
				REG_TXBF_CTRL_8192E+2) & 0xF000);
		odm_write_4byte(dm, REG_ASSOCIATED_BFMER1_INFO_8192E, 0);
		odm_write_2byte(dm, REG_ASSOCIATED_BFMER1_INFO_8192E+4, 0);
		odm_write_2byte(dm, REG_ASSOCIATED_BFMEE_SEL_8192E+2, odm_read_2byte(dm,
				REG_ASSOCIATED_BFMEE_SEL_8192E+2) & 0x60);
	}
