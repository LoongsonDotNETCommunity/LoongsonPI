...
)
{
	...
	if (0) {
		...
		if (beam_info->first_mu_bfee_index < BEAMFORMEE_ENTRY_NUM) {
			tcb->P_AID =
				beam_info->beamformee_entry[beam_info->first_mu_bfee_index].p_aid;
		...
		if (MacAddr_isBcst(RA) || MacAddr_isMulticast(RA)
		    || tcb->macId == MAC_ID_STATIC_FOR_BROADCAST_MULTICAST) {
			tcb->G_ID = 63;
			tcb->P_AID = 0;
		} else if (ACTING_AS_AP((PADAPTER)adapter)) {
			u16	AID = (u16)(MacIdGetOwnerAssociatedClientAID((PADAPTER)adapter,
					    tcb->macId) & 0x1ff);		/*AID[0:8]*/
...
)
{
	...
	beamform_entry = phydm_beamforming_get_bfee_entry_by_addr(dm,
...
boolean send_fw_ht_ndpa_packet(
	...
	if (MgntGetFWBuffer((PADAPTER)p_def_adapter, &tcb, &p_buf))
	{
		...
		desc_len = ((PADAPTER)adapter)->HWDescHeadLength -
		hal_data->USBALLDummyLength;
#endif
		...
		adapter->HalFunc.CmdSendPacketHandler(adapter, tcb, p_buf,
						      tcb->PacketLength, DESC_PACKET_TYPE_NORMAL, false);
	} else
		ret = false;
...
boolean send_sw_ht_ndpa_packet(
	...
	if (MgntGetBuffer(adapter, &tcb, &p_buf))
	{
		...
		MgntSendPacket(adapter, tcb, p_buf, tcb->PacketLength, NORMAL_QUEUE,
			       ndp_tx_rate);
	} else
		ret = false;
...
boolean send_fw_vht_ndpa_packet(
	...
	if (MgntGetFWBuffer((PADAPTER)p_def_adapter, &tcb, &p_buf))
	{
		...
		if (phydm_acting_determine(dm, phydm_acting_as_ibss)
		    || phydm_acting_determine(dm, phydm_acting_as_ap))
			tcb->G_ID = 63;
...
)
{
	...
	u8			idx;
	struct _RT_BEAMFORMING_INFO	*beam_info = &(dm->beamforming_info);
	struct _RT_BEAMFORMER_ENTRY	*beamform_entry =
			&beam_info->beamformer_entry[beam_info->mu_ap_index];
	...
	for (idx = 0; idx < 8; idx++) {
		...
		beamform_entry->gid_valid[idx] = GET_VHT_GID_MGNT_INFO_MEMBERSHIP_STATUS(
				p_buffer + idx);
	}
	...
	for (idx = 0; idx < 16; idx++) {
		...
		beamform_entry->user_position[idx] = GET_VHT_GID_MGNT_INFO_USER_POSITION(
				p_buffer + idx);
	}
	...
	/* Indicate GID frame to IHV service. */
	{
		...
		indioffset += 8;
		PlatformMoveMemory(indibuffer + indioffset, beamform_entry->user_position,
				   16);
		indioffset += 16;
...
)
{
	...
	struct _RT_BEAMFORMING_INFO	*beam_info = &(dm->beamforming_info);
	struct _RT_BEAMFORMEE_ENTRY	*beamform_entry =
			&beam_info->beamformee_entry[idx];
	void				*adapter = beam_info->source_adapter;
...
)
{
	...
	beamformer_entry = phydm_beamforming_get_bfer_entry_by_addr(dm, TA,
	...
	else if ((beamformer_entry->log_success == 1)
		 || (beamformer_entry->clock_reset_times == 5)) {
		PHYDM_DBG(dm, DBG_TXBF,
			  "[%s] log_seq=%d, pre_log_seq=%d, log_retry_cnt=%d, log_success=%d, clock_reset_times=%d, clock reset is no longer needed.\n",
			  __func__, beamformer_entry->log_seq, beamformer_entry->pre_log_seq,
			  beamformer_entry->log_retry_cnt, beamformer_entry->log_success,
	...
	PHYDM_DBG(dm, DBG_TXBF,
		  ...
		  beamformer_entry->pre_log_seq, beamformer_entry->log_retry_cnt,
		  beamformer_entry->clock_reset_times, beamformer_entry->log_success);

	if ((beamformer_entry->log_seq != 0)
	    && (beamformer_entry->pre_log_seq != 0)) {
		/*Success condition*/
		if ((beamformer_entry->log_seq != sequence)
