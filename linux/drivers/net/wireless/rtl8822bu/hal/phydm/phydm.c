...
)
{
	...
	dm->tx_tp = ((dm->tx_tp) >> 1) + (u32)(((dm->cur_tx_ok_cnt) >>
						bit_shift_num) >> 1);
	dm->rx_tp = ((dm->rx_tp) >> 1) + (u32)(((dm->cur_rx_ok_cnt) >>
						bit_shift_num) >> 1);
	...
	if (dm->cur_tx_ok_cnt > 1875000
	    || dm->cur_rx_ok_cnt > 1875000) {		/* ( 1.875M * 8bit ) / 2sec= 7.5M bits /sec )*/

		dm->traffic_load = TRAFFIC_HIGH;
		/**/
	} else if (dm->cur_tx_ok_cnt > 500000
		   || dm->cur_rx_ok_cnt > 500000) { /*( 0.5M * 8bit ) / 2sec =  2M bits /sec )*/

		dm->traffic_load = TRAFFIC_MID;
		/**/
	} else if (dm->cur_tx_ok_cnt > 100000
		   || dm->cur_rx_ok_cnt > 100000)  { /*( 0.1M * 8bit ) / 2sec =  0.4M bits /sec )*/
...
)
{
	...
	dm->is_cck_high_power = (boolean) odm_get_bb_reg(dm,
				ODM_REG(CCK_RPT_FORMAT, dm), ODM_BIT(CCK_RPT_FORMAT, dm));
	...
	if (dm->support_ic_type & (ODM_RTL8703B | ODM_RTL8723D | ODM_RTL8710B)) {
		dm->cck_agc_report_type = odm_get_bb_reg(dm, 0x950,
	...
	if (dm->support_ic_type & ODM_RTL8821C) {
		dm->cck_new_agc = odm_get_bb_reg(dm, 0xa9c,
...
)
{
	...
	if (pause_func == F00_DIG) {
		...
		dm->phydm_func_handler.pause_phydm_handler =
...
)
{
	...
	if ((strcmp(input[1], help) == 0)) {
		...
		if (val_length != 0) {
			...
			set_result = phydm_pause_func(dm, func, pause_type, pause_lv, val_length,
						      val_buf);
		}
...
)
{
	...
	odm_initialize_timer(dm,
			     &dm->beamforming_info.txbf_info.txbf_fw_ndpa_timer,
...
)
{
	...
	/*DC_Cancellation*/
	odm_set_bb_reg(dm, 0xa9c, BIT(20),
...
)
{
	...
	if ((dm->support_ic_type & ODM_RTL8188E
		...
		if (dm->consecutive_idlel_time > 10 && *dm->mp_mode == false
