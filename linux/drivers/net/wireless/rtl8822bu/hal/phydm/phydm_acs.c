...
)
{
	...
	if (channel >= 1 && channel <= 14) {
		...
		PHYDM_DBG(dm, ODM_COMP_API, "CH_Info[0][%d]=%d, CH_Info[1][%d]=%d\n",
			  channel_idx, acs->channel_info_2g[0][channel_idx], channel_idx,
			  acs->channel_info_2g[1][channel_idx]);
...
)
{
	...
	if ((priv->auto_channel != 0)
	    && (priv->auto_channel != 2)) /* if struct acs_info running, do not do FA/CCA counter read */
		return true;
	else
		return false;
#else
...
)
{
	...
	/* 3 struct acs_info setting */
		...
		if (dm->support_ic_type & ODM_IC_11AC_SERIES) {
			...
			odm_set_bb_reg(dm, ODM_REG_NHM_TH9_TH10_11AC, BIT(8) | BIT(9) | BIT(10),
				       3);

			if (acs->acs_step == 0) {
				/* 4 Set IGI */
				odm_set_bb_reg(dm, 0xc50,
					       BIT(0) | BIT(1) | BIT(2) | BIT(3) | BIT(4) | BIT(5) | BIT(6), 0x3E);
				if (get_rf_mimo_mode(priv) != RF_1T1R)
					odm_set_bb_reg(dm, 0xe50,
						       BIT(0) | BIT(1) | BIT(2) | BIT(3) | BIT(4) | BIT(5) | BIT(6), 0x3E);
