...
void phydm_dynamic_ant_weighting(
	void		*dm_void
)
{
...
void phydm_pathb_q_matrix_rotate_en(
	void		*dm_void
)
{
#ifdef PHYDM_COMPILE_IC_2SS
...
void phydm_pathb_q_matrix_rotate(
	void		*dm_void,
	u16		phase_idx
)
{
...
#if (ODM_IC_11AC_SERIES_SUPPORT)
	if (dm->support_ic_type & ODM_IC_11AC_SERIES) {
		odm_set_bb_reg(dm, 0x1954, 0xffffff,
...
void phydm_init_trx_antenna_setting(
	void		*dm_void
)
{
	struct dm_struct	*dm = (struct dm_struct *)dm_void;
...
void phydm_config_ofdm_rx_path(
	void			*dm_void,
	u32			path
)
{
...
void phydm_config_cck_rx_antenna_init(
	void		*dm_void
)
{
	struct dm_struct	*dm = (struct dm_struct *)dm_void;
...
)
{
	...
	if (set_type == PHYDM_SET) {
		...
		if (phydm_set_bb_dbg_port(dm, BB_DBGPORT_PRIORITY_3,
					  0x0) == false) /*set debug port to 0x0*/
		...
		odm_set_bb_reg(dm, 0x520, 0xff0000,
			       (u32)(api->tx_queue_bitmap)); /*Release all TX queue*/
		...
		if (dm->support_ic_type & ODM_IC_11AC_SERIES) {
			...
			odm_set_bb_reg(dm, 0xc14, MASKDWORD,
				       api->rx_iqc_reg_1); /* [ Set IQK Matrix = 0 ] equivalent to [ Turn off CCA] */
...
u8 phydm_calculate_intf_distance(
	void		*dm_void,
	u32		bw,
	u32		fc,
	u32		f_interference,
...
)
{
	...
	if ((f_interference >= bw_low) && (f_interference <= bw_up)) {
		...
		PHYDM_DBG(dm, ODM_COMP_API,
			  ...
			  (tone_idx_tmp / 10), (tone_idx_tmp % 10));
...
)
{
	...
	if (enable == FUNC_DISABLE) {
		...
		PHYDM_DBG(dm, ODM_COMP_API,
			  ...
			  channel, bw, f_interference, (((bw == 20)
							|| (channel > 14)) ? "Don't care" : (second_ch == PHYDM_ABOVE) ? "H" :
							"L"));
		...
		if (phydm_calculate_fc(dm, channel, bw, second_ch, &fc) == PHYDM_SET_FAIL)
			...
			if (phydm_calculate_intf_distance(dm, bw, fc, f_interference,
...
)
{
	...
	if (enable == FUNC_DISABLE)
		...
		PHYDM_DBG(dm, ODM_COMP_API,
			  ...
			  channel, bw, f_interference, (((second_ch == PHYDM_DONT_CARE)
							|| (bw == 20)
							|| (channel > 14)) ? "Don't care" : (second_ch == PHYDM_ABOVE) ? "H" :
							"L"));
...
)
{
	...
	if (function_map == PHYDM_API_NBI) {
		if (dm_value[0] == 100) {
			...
			PDM_SNPF(out_len, used, output + used,
				 ...
				 ((second_ch == PHYDM_DONT_CARE) || (bw == 20)
				  || (channel > 14)) ? "Don't care" : ((second_ch == PHYDM_ABOVE) ? "H" :
						  "L"));
			set_result = phydm_nbi_setting(dm, FUNC_ENABLE, channel, bw,
						       f_interference, second_ch);

		} else if (dm_value[0] == FUNC_DISABLE) {
			PDM_SNPF(out_len, used, output + used,
				 out_len - used, "[Disable NBI]\n");
			set_result = phydm_nbi_setting(dm, FUNC_DISABLE, channel, bw,
						       f_interference, second_ch);
		...
		PDM_SNPF(out_len, used, output + used, out_len - used,
			 "[NBI set result: %s]\n",
			 (set_result == PHYDM_SET_SUCCESS) ? "Success" : ((set_result ==
					 PHYDM_SET_NO_NEED) ? "No need" : "Error"));
	...
	/*-------------------------------------------------------------------------------------------------------------------------------*/
		...
		if (dm_value[0] == 100) {
			...
			PDM_SNPF(out_len, used, output + used,
				 ...
				 channel, bw, f_interference,
				 (channel > 14) ? "Don't care" : (((second_ch == PHYDM_DONT_CARE)
						 || (bw == 20) || (channel > 14)) ? "H" : "L"));
			set_result = phydm_csi_mask_setting(dm,	FUNC_ENABLE, channel, bw,
							    f_interference, second_ch);

		} else if (dm_value[0] == FUNC_DISABLE) {
			PDM_SNPF(out_len, used, output + used,
				 out_len - used, "[Disable CSI MASK]\n");
			set_result = phydm_csi_mask_setting(dm, FUNC_DISABLE, channel, bw,
							    f_interference, second_ch);
...
boolean phydm_set_bb_txagc_offset(
	void			*dm_void,
...
)
{
	...
	if (phy_status == true) {
		if ((dm->rx_pkt_type == type_block_ack) || (dm->rx_pkt_type == type_rts)
		    || (dm->rx_pkt_type == type_cts))
			seq_num = 0;
		else
		...
		PHYDM_DBG_F(dm, ODM_COMP_SNIFFER,
			    ...
			    ((rt_rfd_status->band_width == 0) ? "20M" : ((rt_rfd_status->band_width ==
					    1) ? "40M" : "80M")),
