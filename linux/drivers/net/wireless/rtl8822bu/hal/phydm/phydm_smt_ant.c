...
)
{
	...
	struct	smt_ant_cumitek		*cumi_smtant_table =
			&dm->smtant_table.cumi_smtant_table;
	u32		value32;
...
)
{
	...
	/*path-A*/
	cumi_smtant_table->tx_ant_idx[0][mac_id] =
		tx_ant_idx_path_a; /*fill this value into TXDESC*/

	/*path-B*/
	cumi_smtant_table->tx_ant_idx[1][mac_id] =
...
)
{
	...
	if (cumi_smtant_table->rx_default_ant_idx[0] != rx_ant_idx_path_a) {
		...
		if (dm->support_ic_type == ODM_RTL8822B) {
			...
			odm_set_bb_reg(dm, 0xc08, BIT(21) | BIT(20) | BIT(19),
				       rx_ant_idx_path_a); /*default RX antenna*/
			odm_set_mac_reg(dm, 0x6d8, BIT(2) | BIT(1) | BIT(0),
	...
	if (cumi_smtant_table->rx_default_ant_idx[1] != rx_ant_idx_path_b) {
		...
		if (dm->support_ic_type == ODM_RTL8822B) {
			...
			odm_set_bb_reg(dm, 0xe08, BIT(21) | BIT(20) | BIT(19),
				       rx_ant_idx_path_b); /*default antenna*/
			odm_set_mac_reg(dm, 0x6d8, BIT(5) | BIT(4) | BIT(3),
...
)
{
	...
	if (strcmp(input[1], help) == 0) {
		...
		phydm_cumitek_smt_rx_default_ant_update(dm, (u8)dm_value[1],
							(u8)dm_value[2]);
		PDM_SNPF(out_len, used, output + used, out_len - used,
			 "RX Ant{A, B}={%d, %d}\n", dm_value[1],
			 dm_value[2]);
		...
		phydm_cumitek_smt_tx_ant_update(dm, (u8)dm_value[1], (u8)dm_value[2],
						(u8)dm_value[3]);
	...
	for (i = 0; i <= (codeword_length - 1); i++) {
		...
		if (dm->debug_components & DBG_ANT_DIV) {
			if (i == (codeword_length - 1)) {
				...
				pr_debug("Start sending codeword[1:%d] ---> [ %d ", codeword_length,
					 beam_ctrl_signal);
				/**/
...
)
{
	...
	sat_tab->update_beam_codeword = phydm_construct_hb_rfu_codeword_type2(dm,
	...
	phydm_update_beam_pattern_type2(dm, sat_tab->update_beam_codeword,
...
)
{
	...
	if (strcmp(input[1], help) == 0) {
		...
		if (sat_tab->fix_beam_pattern_en == 1) {
			...
			for (i = 0; i <= (codeword_length - 1); i++) {
				beam_ctrl_signal = (boolean)((sat_tab->update_beam_codeword & BIT(
								      i)) >> i);
		...
		if (dm_value[1] == 0) {
			...
			for (i = 0; i < sat_tab->total_beam_set_num_5g; i++) {
				PDM_SNPF((output + used, out_len - used,
					  ...
					  i, sat_tab->rfu_codeword_table_5g[i][0],
					  sat_tab->rfu_codeword_table_5g[i][1]));
			}
		}
#endif
...
)
{
	...
	if (fat_tab->fat_state == FAT_DECISION_STATE) {
		...
		for (j = 0; j < (sat_tab->total_beam_set_num); j++) {
			...
			if (sat_tab->statistic_pkt_cnt[j] == 0) {	/*if new RSSI = 0 -> MA_RSSI-=2*/
				...
				avg_rssi_tmp = (u8)((sat_tab->beam_set_rssi_avg_sum[j]) /
						    (sat_tab->statistic_pkt_cnt[j]));
			...
			if (sat_tab->beam_path_evm_2ss_cnt[j] != 0) {
				avg_evm2ss[0] =  sat_tab->beam_path_evm_2ss_sum[j][0] /
						 sat_tab->beam_path_evm_2ss_cnt[j];
				avg_evm2ss[1] =  sat_tab->beam_path_evm_2ss_sum[j][1] /
						 sat_tab->beam_path_evm_2ss_cnt[j];
			...
			if (sat_tab->beam_path_evm_1ss_cnt[j] != 0) {
				avg_evm1ss =  sat_tab->beam_path_evm_1ss_sum[j] /
					      sat_tab->beam_path_evm_1ss_cnt[j];
			...
			PHYDM_DBG(dm, DBG_ANT_DIV,
				  ...
				  j, sat_tab->statistic_pkt_cnt[j], avg_rssi_tmp_ma, avg_rssi_tmp,
	...
	/* [TRAINING STATE] */
		...
		if (sat_tab->fast_training_beam_num > sat_tab->pre_fast_training_beam_num)
			...
			if (sat_tab->fast_training_beam_num >= ((u32)sat_tab->total_beam_set_num -
								1)) {
				PHYDM_DBG(dm, DBG_ANT_DIV,
	...
	/*=======================================================================================*/
		...
		PHYDM_DBG(dm, DBG_ANT_DIV,
			  ...
			  sat_tab->per_beam_training_pkt_num, sat_tab->decision_holding_period);
		...
		for (j = 0; j < (sat_tab->total_beam_set_num); j++) {
			...
			if ((sat_tab->per_beam_training_pkt_num) > training_pkt_num_offset)
				sat_tab->beam_set_train_cnt[j] = sat_tab->per_beam_training_pkt_num -
								 training_pkt_num_offset;
			else
	...
	/*[data]*/
		...
		if (sat_tab->pkt_skip_statistic_en == 0) {
			PHYDM_DBG(dm, DBG_ANT_DIV,
				  ...
				  sat_tab->fast_training_beam_num, rx_power_ant0, rx_power_ant1, rssi_avg);

			PHYDM_DBG(dm, DBG_ANT_DIV,
				  "Rate_ss = ((%d)), EVM{A,B} = {%d, %d}, RX Rate =", pktinfo->rate_ss,
				  rx_evm_ant0, rx_evm_ant1);
			phydm_print_rate(dm, dm->rx_rate, DBG_ANT_DIV);


			if (sat_tab->pkt_counter >= 1) { /*packet skip count*/
				sat_tab->beam_set_rssi_avg_sum[sat_tab->fast_training_beam_num] +=
					rssi_avg;
				sat_tab->statistic_pkt_cnt[sat_tab->fast_training_beam_num]++;

				sat_tab->beam_path_rssi_sum[sat_tab->fast_training_beam_num][0] +=
					rx_power_ant0;
				sat_tab->beam_path_rssi_sum[sat_tab->fast_training_beam_num][1] +=
					rx_power_ant1;

				if (pktinfo->rate_ss == 2) {
					sat_tab->beam_path_evm_2ss_sum[sat_tab->fast_training_beam_num][0] +=
						rx_evm_ant0;
					sat_tab->beam_path_evm_2ss_sum[sat_tab->fast_training_beam_num][1] +=
						rx_evm_ant1;
					sat_tab->beam_path_evm_2ss_cnt[sat_tab->fast_training_beam_num]++;
				} else {
					sat_tab->beam_path_evm_1ss_sum[sat_tab->fast_training_beam_num] +=
						rx_evm_ant0;
			...
			train_pkt_number =
				sat_tab->beam_set_train_cnt[sat_tab->fast_training_beam_num];
...
)
{
	...
	odm_set_mac_reg(dm, 0x44, BIT(27) | BIT(26), 0);
	odm_set_mac_reg(dm, 0x44, BIT(19) | BIT(18),
			0x3);	/*enable_output for P_GPIO[3:2]*/
	...
	odm_set_bb_reg(dm, 0xc08, BIT(31),
		       0); /*increase ant num every FAT period 0:+1, 1+2*/
	odm_set_bb_reg(dm, 0x8c4, BIT(2) | BIT(1),
...
)
{
	...
	if (ant_num < 8) {
		for (i = 0; i < (sat_tab->ant_num_total); i++) {
			...
			if ((i < (sat_tab->first_train_ant - 1))
			    || (break_counter >= (sat_tab->ant_num))) {
				data_tmp = 0;
				/**/
			} else {
				break_counter++;
	...
	for (i = 0; i <= (codeword_length - 1); i++) {
		...
		if (dm->debug_components & DBG_ANT_DIV) {
			if (i == (codeword_length - 1)) {
				...
				pr_debug("Send codeword[1:%d] ---> [ %d ", codeword_length,
					 beam_ctrl_signal);
				/**/
			} else if ((i % devide_num) == (devide_num - 1)) {
				pr_debug("%d  |  ", beam_ctrl_signal);
...
)
{
		...
		for (i = (sat_tab->first_train_ant - 1); i < sat_tab->ant_num_total; i++) {
			for (j = 0; j < (sat_tab->beam_patten_num_each_ant); j++) {
				if (sat_tab->pkt_rssi_cnt[i][j] == 0) {
					...
					avg_rssi_tmp = (sat_tab->pkt_rssi_sum[i][j]) /
						       (sat_tab->pkt_rssi_cnt[i][j]);
	...
	/*=======================================================================================*/
		...
		PHYDM_DBG(dm, DBG_ANT_DIV,
			  ...
			  sat_tab->fix_training_num_en, sat_tab->per_beam_training_pkt_num,
			  sat_tab->decision_holding_period);
		...
		for (i = (sat_tab->first_train_ant - 1); i < sat_tab->ant_num_total; i++) {
			for (j = 0; j < (sat_tab->beam_patten_num_each_ant); j++) {
				...
				if ((sat_tab->per_beam_training_pkt_num) > training_pkt_num_offset)
					sat_tab->beam_train_cnt[i][j] = sat_tab->per_beam_training_pkt_num -
									training_pkt_num_offset;
				else
