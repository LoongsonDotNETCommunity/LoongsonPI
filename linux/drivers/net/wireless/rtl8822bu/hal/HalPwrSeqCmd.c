		...
		if ((GET_PWR_CFG_FAB_MASK(PwrCfgCmd) & FabVersion) &&
			...
			case PWR_CMD_WRITE:
...
#endif
				{
					...
					value = value | (GET_PWR_CFG_VALUE(PwrCfgCmd) & GET_PWR_CFG_MASK(
								 PwrCfgCmd));
					...
					if (pollingCount++ > maxPollingCnt) {
						...
						if (bHWICSupport && offset == 0x06  && flag == 0) {
							...
							if (IS_HARDWARE_TYPE_8723DE(padapter))
								PlatformEFIOWrite1Byte(padapter, 0x40, (PlatformEFIORead1Byte(padapter,
													0x40)) & (~BIT3));

							PlatformEFIOWrite1Byte(padapter, 0x04, PlatformEFIORead1Byte(padapter,
									       0x04) | BIT3);
							PlatformEFIOWrite1Byte(padapter, 0x04, PlatformEFIORead1Byte(padapter,
									       0x04) & ~BIT3);
