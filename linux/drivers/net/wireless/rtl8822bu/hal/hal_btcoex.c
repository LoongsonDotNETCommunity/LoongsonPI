...
void halbtcoutsrc_AggregationCheck(PBTC_COEXIST pBtCoexist)
{
	...
	if (pBtCoexist->bt_info.reject_agg_pkt) {
		bNeedToAct = _TRUE;
		pBtCoexist->bt_info.pre_reject_agg_pkt =
			pBtCoexist->bt_info.reject_agg_pkt;
	} else {
		...
		if (pBtCoexist->bt_info.pre_bt_ctrl_agg_buf_size !=
			...
			bNeedToAct = _TRUE;
			pBtCoexist->bt_info.pre_bt_ctrl_agg_buf_size =
				pBtCoexist->bt_info.bt_ctrl_agg_buf_size;
		}
...
u8 halbtcoutsrc_IsDualBandConnected(PADAPTER padapter)
{
	...
	if (MCC_EN(padapter)
		...
		u8 band0 = mccobjpriv->iface[0]->mlmeextpriv.cur_channel > 14 ?
			   BAND_ON_5G : BAND_ON_2_4G;
		u8 band1 = mccobjpriv->iface[1]->mlmeextpriv.cur_channel > 14 ?
			   BAND_ON_5G : BAND_ON_2_4G;
 ...
 */
static u8 _btmpoper_cmd(PBTC_COEXIST pBtCoexist, u8 opcode, u8 opcodever,
			u8 *cmd, u8 size)
{
	PADAPTER padapter;
...
#if 0
		*pu8 = (rtw_mi_check_fwstate(padapter,
					     WIFI_SITE_MONITOR)) ? _TRUE : _FALSE;
#else
...
u16 halbtcoutsrc_LnaConstrainLvl(void *pBtcContext,
				 u8 *lna_constrain_level)
{
	...
	if (halbtcoutsrc_IsHwMailboxExist(pBtCoexist) == _TRUE) {
		...
		ret = _btmpoper_cmd(pBtCoexist, BT_OP_SET_BT_LANCONSTRAIN_LEVEL, 0,
...
void halbtcoutsrc_DisplayCoexStatistics(PBTC_COEXIST pBtCoexist)
{
	...
	if (gl_coex_offload.cnt_h2c_sent) {
		...
		CL_SPRINTF(cliBuf, BT_TMP_BUF_SIZE, "\r\n %-35s = H2c(%d)/Ack(%d)",
			   "Coex h2c/c2h overall statistics",
			   gl_coex_offload.cnt_h2c_sent, gl_coex_offload.cnt_c2h_ack);
		for (j = 0; j < COL_STATUS_MAX; j++) {
			if (gl_coex_offload.status[j]) {
				CL_SPRINTF(tmpbuf, BTC_TMP_BUF_SHORT, ", %s:%d", coexH2cResultString[j],
					   gl_coex_offload.status[j]);
	...
	for (i = 0; i < COL_OP_WIFI_OPCODE_MAX; i++) {
		if (gl_coex_offload.h2c_record[i].count) {
			...
			/*==========================================*/
			CL_SPRINTF(cliBuf, BT_TMP_BUF_SIZE, "\r\n %-35s = total:%d",
				   coexOpcodeString[i], gl_coex_offload.h2c_record[i].count);
			for (j = 0; j < COL_STATUS_MAX; j++) {
				if (gl_coex_offload.h2c_record[i].status[j]) {
					CL_SPRINTF(tmpbuf, BTC_TMP_BUF_SHORT, ", %s:%d", coexH2cResultString[j],
						   gl_coex_offload.h2c_record[i].status[j]);
					CL_STRNCAT(cliBuf, BT_TMP_BUF_SIZE, tmpbuf, BTC_TMP_BUF_SHORT);
				}
			}
			CL_PRINTF(cliBuf);
			...
			for (j = 0; j < gl_coex_offload.h2c_record[i].h2c_len; j++) {
				CL_SPRINTF(tmpbuf, BTC_TMP_BUF_SHORT, "%02x ",
					   gl_coex_offload.h2c_record[i].h2c_buf[j]);
			...
			if (gl_coex_offload.h2c_record[i].c2h_ack_len) {
				...
				for (j = 0; j < gl_coex_offload.h2c_record[i].c2h_ack_len; j++) {
					CL_SPRINTF(tmpbuf, BTC_TMP_BUF_SHORT, "%02x ",
						   gl_coex_offload.h2c_record[i].c2h_ack_buf[j]);
					CL_STRNCAT(cliBuf, BT_TMP_BUF_SIZE, tmpbuf, 3);
				}
			}
			CL_PRINTF(cliBuf);
	...
	for (i = 0; i < COL_IND_MAX; i++) {
		if (gl_coex_offload.c2h_ind_record[i].count) {
			...
			/*==========================================*/
			CL_SPRINTF(cliBuf, BT_TMP_BUF_SIZE, "\r\n %-35s = total:%d",
				   coexIndTypeString[i], gl_coex_offload.c2h_ind_record[i].count);
			for (j = 0; j < COL_STATUS_MAX; j++) {
				if (gl_coex_offload.c2h_ind_record[i].status[j]) {
					CL_SPRINTF(tmpbuf, BTC_TMP_BUF_SHORT, ", %s:%d", coexH2cResultString[j],
						   gl_coex_offload.c2h_ind_record[i].status[j]);
					CL_STRNCAT(cliBuf, BT_TMP_BUF_SIZE, tmpbuf, BTC_TMP_BUF_SHORT);
				}
			}
			CL_PRINTF(cliBuf);
			...
			/*==========================================*/
			CL_SPRINTF(cliBuf, BT_TMP_BUF_SIZE, "\r\n %-35s = ",
				   "C2H indication content");
			for (j = 0; j < gl_coex_offload.c2h_ind_record[i].ind_len; j++) {
				CL_SPRINTF(tmpbuf, BTC_TMP_BUF_SHORT, "%02x ",
					   gl_coex_offload.c2h_ind_record[i].ind_buf[j]);
				CL_STRNCAT(cliBuf, BT_TMP_BUF_SIZE, tmpbuf, 3);
			}
			CL_PRINTF(cliBuf);
			/*==========================================*/
	...
	if (pHalData->EEPROMBluetoothCoexist == 1) {
		...
		CL_PRINTF(cliBuf);
		CL_SPRINTF(cliBuf, BT_TMP_BUF_SIZE, "\r\n %-35s = %d ", "IsBtDisabled",
			   rtw_btcoex_IsBtDisabled(padapter));
		CL_PRINTF(cliBuf);
		CL_SPRINTF(cliBuf, BT_TMP_BUF_SIZE, "\r\n %-35s = %d ", "IsBtControlLps",
...
void halbtcoutsrc_DisplayWifiStatus(PBTC_COEXIST pBtCoexist)
{
	...
	CL_SPRINTF(cliBuf, BT_TMP_BUF_SIZE, "\r\n %-35s = %d/ %d/ %d %s %s",
		   ...
		   ((halbtcoutsrc_is_autoload_fail(pBtCoexist) == _TRUE) ? "fail" : "ok"),
		   ((halbtcoutsrc_is_fw_ready(pBtCoexist) == _TRUE) ? "ok" : "fail"));
	CL_PRINTF(cliBuf);

	if (wifiLinkStatus & WIFI_STA_CONNECTED) {
		CL_SPRINTF(cliBuf, BT_TMP_BUF_SIZE, "\r\n %-35s = %s", "IOT Peer",
			   GLBtcIotPeerString[padapter->mlmeextpriv.mlmext_info.assoc_AP_vendor]);
		CL_PRINTF(cliBuf);
	}

	pBtCoexist->btc_get(pBtCoexist, BTC_GET_S4_WIFI_RSSI, &wifiRssi);
	pBtCoexist->btc_get(pBtCoexist, BTC_GET_U1_WIFI_DOT11_CHNL, &wifiChnl);
	pBtCoexist->btc_get(pBtCoexist, BTC_GET_U2_BEACON_PERIOD,
			    &wifiBcnInterval);
	if ((wifiLinkStatus & WIFI_P2P_GO_CONNECTED)
	    || (wifiLinkStatus & WIFI_P2P_GC_CONNECTED))
		pBtCoexist->btc_get(pBtCoexist, BTC_GET_U1_WIFI_P2P_CHNL, &wifiP2PChnl);
	CL_SPRINTF(cliBuf, BT_TMP_BUF_SIZE, "\r\n %-35s = %d dBm/ %d/ %d/ %d",
		   "RSSI/ STA_Chnl/ P2P_Chnl/ BI",
	...
	pBtCoexist->btc_get(pBtCoexist, BTC_GET_U4_WIFI_TRAFFIC_DIRECTION,
			    &wifiTrafficDir);
	pBtCoexist->btc_get(pBtCoexist, BTC_GET_BL_WIFI_UNDER_B_MODE,
			    &bWifiUnderBMode);
	pBtCoexist->btc_get(pBtCoexist, BTC_GET_U1_AP_NUM, &nScanAPNum);
	CL_SPRINTF(cliBuf, BT_TMP_BUF_SIZE, "\r\n %-35s = %s / %s/ %s/ %d ",
		   "Band/ BW/ Traffic/ APCnt",
...
void halbtcoutsrc_SetRfReg(void *pBtcContext, enum rf_path eRFPath,
...
u32 halbtcoutsrc_GetRfReg(void *pBtcContext, enum rf_path eRFPath,
...
u16 halbtcoutsrc_SetBtReg(void *pBtcContext, u8 RegType, u32 RegAddr,
			  u32 Data)
{
	PBTC_COEXIST pBtCoexist;
	u16 ret = BT_STATUS_BT_OP_SUCCESS;
...
BOOLEAN halbtcoutsrc_SetBtTRXMASK(
...
)
{
	...
	if (IS_HARDWARE_TYPE_8723B(pBtCoexist->Adapter)
	    || IS_HARDWARE_TYPE_8723D(pBtCoexist->Adapter)
...
u16 halbtcoutsrc_GetBtReg_with_status(void *pBtcContext, u8 RegType,
...
u32 halbtcoutsrc_GetBtReg(void *pBtcContext, u8 RegType, u32 RegAddr)
{
	...
	return (BT_STATUS_BT_OP_SUCCESS == halbtcoutsrc_GetBtReg_with_status(
			pBtcContext, RegType, RegAddr, &regVal)) ? regVal : 0xffffffff;
}
...
static COL_H2C_STATUS halbtcoutsrc_send_h2c(PADAPTER Adapter,
		PCOL_H2C pcol_h2c, u16 h2c_cmd_len)
{
	...
	reinit_completion(
		&gl_coex_offload.c2h_event[pcol_h2c->req_num]);		/* set event to un signaled state */
#else
	...
	if (TRUE) {
		...
		if (RT_STATUS_SUCCESS == HAL_MAC_Send_BT_COEX(&GET_HAL_MAC_INFO(Adapter),
				(pu1Byte)(pcol_h2c), (u4Byte)h2c_cmd_len, 1)) {
			if (!wait_for_completion_timeout(
				    &gl_coex_offload.c2h_event[pcol_h2c->req_num], 20)) {
...
static COL_H2C_STATUS halbtcoutsrc_check_c2h_ack(PADAPTER Adapter,
		PCOL_SINGLE_H2C_RECORD pH2cRecord)
{
	COL_H2C_STATUS	c2h_status = COL_STATUS_C2H_OK;
	PCOL_H2C		p_h2c_cmd = (PCOL_H2C)&pH2cRecord->h2c_buf[0];
	u8			req_num = p_h2c_cmd->req_num;
	PCOL_C2H_ACK	p_c2h_ack = (PCOL_C2H_ACK)
				    &gl_coex_offload.c2h_ack_buf[req_num];


	if ((COL_C2H_ACK_HDR_LEN + p_c2h_ack->ret_len) >
	    gl_coex_offload.c2h_ack_len[req_num]) {
		c2h_status = COL_STATUS_COEX_DATA_OVERFLOW;
		return c2h_status;
	}
	/* else */
	{
		_rtw_memmove(&pH2cRecord->c2h_ack_buf[0],
			     &gl_coex_offload.c2h_ack_buf[req_num],
			     gl_coex_offload.c2h_ack_len[req_num]);
		...
		u8 opcode, u8 opcode_ver, u8 *ph2c_par, u8 h2c_par_len)
{
	...
	col_h2c_len = h2c_par_len +
		      2;	/* 2=sizeof(OPCode, OPCode_version and  Request number) */
	...
	_rtw_memmove((PVOID)&gl_coex_offload.h2c_record[opcode].h2c_buf[0],
		     (PVOID)pcol_h2c, col_h2c_len);
	...
	if (COL_STATUS_C2H_OK == h2c_status) {
		...
		c2h_status = halbtcoutsrc_check_c2h_ack(Adapter,
							&gl_coex_offload.h2c_record[opcode]);
...
u8 halbtcoutsrc_GetAntDetValFromBt(void *pBtcContext)
{
	...
	RT_TRACE(COMP_DBG, DBG_LOUD,
		 ("$$$ halbtcoutsrc_GetAntDetValFromBt(): status = %d, feature = %x\n",
		  status, AntDetVal));
...
u8 halbtcoutsrc_GetBtAFHMapFromBt(void *pBtcContext, u8 mapType,
				  u8 *afhMap)
{
	struct btc_coexist *pBtCoexist = (struct btc_coexist *)pBtcContext;
	u8 buf[2] = {0};
...
void halbtcoutsrc_phydm_modify_RA_PCR_threshold(void *pBtcContext,
		u8 RA_offset_direction, u8 RA_threshold_offset)
{
	struct btc_coexist *pBtCoexist = (struct btc_coexist *)pBtcContext;

	/* switch to #if 0 in case the phydm version does not provide the function */
#if 1
	phydm_modify_RA_PCR_threshold(pBtCoexist->odm_priv, RA_offset_direction,
				      RA_threshold_offset);
#endif
}
...
u32 halbtcoutsrc_phydm_query_PHY_counter(void *pBtcContext, u8 info_type)
{
	...
	/* switch to #if 0 in case the phydm version does not provide the function */
#if 1
	return phydm_cmn_info_query((struct dm_struct *)pBtCoexist->odm_priv,
...
static void BT_CoexOffloadC2hAckCheck(PADAPTER	Adapter, u8 *tmpBuf,
				      u8 length)
{
	PADAPTER		pDefaultAdapter = GetDefaultAdapter(Adapter);
	...
	if (length <
	    COL_C2H_ACK_HDR_LEN) {		/* c2h ack length must >= 3 (status, opcode_ver, req_num and ret_len) */
		gl_coex_offload.status[COL_STATUS_INVALID_C2H_LEN]++;
	} else {
...
static void BT_CoexOffloadC2hIndCheck(PADAPTER Adapter, u8 *tmpBuf,
				      u8 length)
{
	PADAPTER		pDefaultAdapter = GetDefaultAdapter(Adapter);
	...
	if (length <
	    COL_C2H_IND_HDR_LEN) {		/* c2h indication length must >= 3 (type, version and length) */
		...
		_rtw_memmove(&gl_coex_offload.c2h_ind_record[ind_type].ind_buf[0], tmpBuf,
			     length);
...
{
#if 0 /*(USE_HAL_MAC_API == 1)*/
	u8	c2hSubCmdId = 0, c2hAckLen = 0, h2cCmdId = 0, h2cSubCmdId = 0,
		c2hIndLen = 0;
...
void EXhalbtcoutsrc_AntInfoSetting(void *padapter)
{
	...
	pBtCoexist->board_info.tfbga_package = rtw_btcoex_is_tfbga_package_type((
			PADAPTER)padapter);

	pBtCoexist->board_info.rfe_type = rtw_btcoex_get_pg_rfe_type((
			PADAPTER)padapter);

	pBtCoexist->board_info.ant_div_cfg = rtw_btcoex_get_ant_div_cfg((
			PADAPTER)padapter);
...
u8 EXhalbtcoutsrc_InitlizeVariables(void *padapter)
{
	...
	pBtCoexist->btc_get_bt_coex_supported_feature =
		halbtcoutsrc_GetBtCoexSupportedFeature;
	pBtCoexist->btc_get_bt_coex_supported_version =
		halbtcoutsrc_GetBtCoexSupportedVersion;
	pBtCoexist->btc_get_ant_det_val_from_bt = halbtcoutsrc_GetAntDetValFromBt;
	pBtCoexist->btc_get_ble_scan_type_from_bt =
		halbtcoutsrc_GetBleScanTypeFromBt;
	pBtCoexist->btc_get_ble_scan_para_from_bt =
		halbtcoutsrc_GetBleScanParaFromBt;
	...
	pBtCoexist->btc_phydm_modify_RA_PCR_threshold =
		halbtcoutsrc_phydm_modify_RA_PCR_threshold;
	pBtCoexist->btc_phydm_query_PHY_counter =
		halbtcoutsrc_phydm_query_PHY_counter;
	pBtCoexist->btc_phydm_modify_ANTDIV_HwSw =
		halbtcoutsrc_phydm_modify_AntDiv_HwSw;
...
void EXhalbtcoutsrc_media_status_notify(PBTC_COEXIST pBtCoexist,
...
void EXhalbtcoutsrc_specific_packet_notify(PBTC_COEXIST pBtCoexist,
		u8 pktType)
{
	u8	packetType;
...
void EXhalbtcoutsrc_bt_info_notify(PBTC_COEXIST pBtCoexist, u8 *tmpBuf,
				   u8 length)
{
	if (!halbtcoutsrc_IsBtCoexistAvailable(pBtCoexist))
		return;
...
void EXhalbtcoutsrc_WlFwDbgInfoNotify(PBTC_COEXIST pBtCoexist, u8 *tmpBuf,
				      u8 length)
{
	if (!halbtcoutsrc_IsBtCoexistAvailable(pBtCoexist))
		return;
...
void EXhalbtcoutsrc_rx_rate_change_notify(PBTC_COEXIST pBtCoexist,
		u8 is_data_frame, u8 btc_rate_id)
{
	if (!halbtcoutsrc_IsBtCoexistAvailable(pBtCoexist))
		return;
...
void EXhalbtcoutsrc_dbg_control(PBTC_COEXIST pBtCoexist, u8 opCode,
				u8 opLen, u8 *pData)
{
	if (!halbtcoutsrc_IsBtCoexistAvailable(pBtCoexist))
		return;
...
)
{
	...
	if (IS_HARDWARE_TYPE_8723B(pBtCoexist->Adapter)) {
		if (pBtCoexist->board_info.btdm_ant_num == 1)
			ex_halbtc8723b1ant_AntennaDetection(pBtCoexist, centFreq, offset, span,
							    seconds);
	}
...
void EXhalbtcoutsrc_SetAntNum(u8 type, u8 antNum)
{
	if (BT_COEX_ANT_TYPE_PG == type) {
		...
		if ((GLBtCoexist.board_info.pg_ant_num == 2)
		    && (GLBtCoexist.board_info.btdm_ant_num == 1))
...
void ex_halbtcoutsrc_pta_off_on_notify(PBTC_COEXIST pBtCoexist, u8 bBTON)
{
	...
	if (IS_HARDWARE_TYPE_8812(pBtCoexist->Adapter)) {
		if (pBtCoexist->board_info.btdm_ant_num == 2)
			ex_halbtc8812a2ant_pta_off_on_notify(pBtCoexist,
							     (bBTON == _TRUE) ? BTC_BT_ON : BTC_BT_OFF);
	}
#endif
}
...
void EXhalbtcoutsrc_switchband_notify(struct btc_coexist *pBtCoexist,
				      u8 type)
{
	if (!halbtcoutsrc_IsBtCoexistAvailable(pBtCoexist))
		return;
...
)
{
				...
				while (i < strlen(szLine)) {
					if (szLine[i] != '"')
						...
						if (!ParseQualifiedString(szLine, &i, param_value_string, '"', '"')) {
							...
							return _FAIL;
						} else if (!GetU1ByteIntegerFromStringInDecimal(param_value_string,
	...
	if (ant_num == 3 && ant_distance >= 50)
		...
		anttype = 2;
		 ...
		 && rfe_type == 1)
		anttype = 2;
	else if ((ant_num == 2 && ant_distance < 15 && rfe_type == 1)
		 || (ant_num == 3 && ant_distance < 15))
		anttype = 1;
...
void hal_btcoex_switchband_notify(u8 under_scan, u8 band_type)
{
	...
	case BAND_ON_2_4G:
		if (under_scan)
			...
			EXhalbtcoutsrc_switchband_notify(&GLBtCoexist,
...
void hal_btcoex_rx_rate_change_notify(PADAPTER padapter, u8 is_data_frame,
				      u8 rate_id)
{
	EXhalbtcoutsrc_rx_rate_change_notify(&GLBtCoexist, is_data_frame,
					     EXhalbtcoutsrc_rate_id_to_btc_rate_id(rate_id));
}
