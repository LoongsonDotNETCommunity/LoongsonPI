...
static void _rtw_bss_nums_count(_adapter *adapter, u8 *pbss_nums)
{
	...
	while (1) {
		...
		if (!pnetwork)
			break;
		chan_idx = rtw_chset_search_ch(adapter_to_chset(adapter),
...
u8 rtw_phydm_clm_ratio(_adapter *adapter)
{
	...
	return phydm_cmn_info_query(phydm,
				    (enum phydm_info_query) PHYDM_INFO_CLM_RATIO);
}
u8 rtw_phydm_nhm_ratio(_adapter *adapter)
{
	...
	return phydm_cmn_info_query(phydm,
				    (enum phydm_info_query) PHYDM_INFO_NHM_RATIO);
}
...
void rtw_acs_adv_setting(_adapter *adapter, RT_SCAN_TYPE scan_type,
			 u16 scan_time, u8 igi, u8 bw)
{
	HAL_DATA_TYPE *hal_data = GET_HAL_DATA(adapter);
	struct auto_chan_sel *pacs = &hal_data->acs;
		     ...
		     enum NHM_PID pid)
{
	...
	hal_data->acs.trig_rst = phydm_env_mntr_trigger(phydm, &nhm_para,
				 &clm_para, &trig_rpt);
...
void rtw_acs_get_rst(_adapter *adapter)
{
	...
	RTW_INFO("[ACS] Result CH:%d, CLM:%d NHM:%d\n",
		 cur_chan, hal_data->acs.clm_ratio[chan_idx],
		 hal_data->acs.nhm_ratio[chan_idx]);
#endif
}
...
#ifdef CONFIG_RTW_ACS_DBG
	_RTW_PRINT_SEL(sel,
		       "Advanced setting - scan_type:%c, ch_ms:%d(ms), igi:0x%02x, bw:%d\n",
		       hal_data->acs.scan_type ? 'A' : 'P', hal_data->acs.scan_time,
		       hal_data->acs.igi, hal_data->acs.bw);
...
void rtw_noise_info_dump(void *sel, _adapter *adapter)
{
	...
	_RTW_PRINT_SEL(sel, "%5s  %3s  %3s  %10s", "Index", "CH", "BSS",
		       "Noise(dBm)\n");
...
void rtw_noise_measure(_adapter *adapter, u8 chan, u8 is_pause_dig,
		       u8 igi_value, u32 max_time)
{
	...
	noise = odm_inband_noise_monitor(phydm, is_pause_dig, igi_value,
					 max_time); /*dBm*/
