...
void rtw_hal_def_value_init(_adapter *padapter)
{
	if (is_primary_adapter(padapter)) {
...

		{
			...
			dvobj->cam_ctl.num = rtw_min(hal_spec->sec_cam_ent_num,
...
u8 rtw_hal_get_def_var(_adapter *padapter, HAL_DEF_VARIABLE eVariable,
		       PVOID pValue)
{
	return padapter->hal_func.get_hal_def_var_handler(padapter, eVariable,
			pValue);
}
...
{
	padapter->hal_func.GetHalODMVarHandler(padapter, eVariable, pValue1,
					       pValue2);
}
...
s32 rtw_hal_fw_mem_dl(_adapter *padapter, enum fw_mem mem)
{
	...
	RTW_INFO("%s in %dms\n", __func__,
		 rtw_get_passing_time_ms(dlfw_start_time));
...
u8 rtw_hal_pci_l1off_nic_support(_adapter *padapter)
{
	...
	padapter->hal_func.GetHwRegHandler(padapter, HW_VAR_L1OFF_NIC_SUPPORT,
					   &l1off);
	return l1off;
}
...
u8 rtw_hal_pci_l1off_capability(_adapter *padapter)
{
	...
	padapter->hal_func.GetHwRegHandler(padapter, HW_VAR_L1OFF_CAPABILITY,
					   &l1off);
	return l1off;
}
	...
	if (padapter->hal_func.read_rfreg) {
		...
		if (match_rf_read_sniff_ranges(eRFPath, RegAddr, BitMask)) {
			RTW_INFO("DBG_IO rtw_hal_read_rfreg(%u, 0x%04x, 0x%08x) read:0x%08x(0x%08x)\n"
				 , eRFPath, RegAddr, BitMask, (data << PHY_CalculateBitShift(BitMask)),
				 data);
		}
	}
			 ...
			 u32 RegAddr, u32 BitMask, u32 Data)
{
	if (padapter->hal_func.write_rfreg) {
		...
		if (!IS_HARDWARE_TYPE_JAGUAR_AND_JAGUAR2(
			    padapter)) /*For N-Series IC, suggest by Jenyu*/
			rtw_udelay_os(2);
#endif
	}
}
			    ...
			    enum channel_width Bandwidth, u8 Offset40, u8 Offset80)
{
	...
	padapter->hal_func.set_chnl_bw_handler(padapter, channel, Bandwidth,
					       Offset40, Offset80);
}
...
void	rtw_hal_dm_watchdog_in_lps(_adapter *padapter)
{
	...
	if (adapter_to_pwrctl(padapter)->bFwCurrentInPSMode == _TRUE)
		rtw_phydm_watchdog_in_lps_lclk(
...
int rtw_hal_iol_cmd(ADAPTER *adapter, struct xmit_frame *xmit_frame,
		    u32 max_waiting_ms, u32 bndy_cnt)
{
	if (adapter->hal_func.IOL_exec_cmds_sync)
		return adapter->hal_func.IOL_exec_cmds_sync(adapter, xmit_frame,
				max_waiting_ms, bndy_cnt);
	return _FAIL;
}
#endif
...
s32 c2h_handler(_adapter *adapter, u8 id, u8 seq, u8 plen, u8 *payload)
{
	...
	// fallthrough
	default:
		if (phydm_c2H_content_parsing(adapter_to_phydm(adapter), id, plen,
					      payload) != TRUE)
			ret = _FAIL;
		break;
	}
...
static s32 _rtw_hal_macid_bmp_sleep(_adapter *adapter,
				    struct macid_bmp *bmp, u8 sleep)
{
	struct macid_ctl_t *macid_ctl = adapter_to_macidctl(adapter);
	u16 reg_sleep;
	u32 *m = &bmp->m0;
			 ...
			 u8 *pCmdBuffer)
{
	...
	if (GET_HAL_DATA(pri_adapter)->bFWReady == _TRUE)
		return padapter->hal_func.fill_h2c_cmd(padapter, ElementID, CmdLen,
						       pCmdBuffer);
	else if (padapter->registrypriv.mp_mode == 0)
		RTW_PRINT(
			FUNC_ADPT_FMT" FW doesn't exit when no MP mode, by pass H2C id:0x%02x\n"
...
{
	padapter->hal_func.fill_fake_txdesc(padapter, pDesc, BufferLen, IsPsPoll,
					    IsBTQosNull, bDataFrame);
...
void rtw_hal_set_tx_power_index(PADAPTER padapter, u32 powerindex,
				enum rf_path rfpath, u8 rate)
{
	return padapter->hal_func.set_tx_power_index_handler(padapter, powerindex,
			rfpath, rate);
}

u8 rtw_hal_get_tx_power_index(PADAPTER padapter, enum rf_path rfpath,
			      u8 rate, u8 bandwidth, u8 channel, struct txpwr_idx_comp *tic)
{
	return padapter->hal_func.get_tx_power_index_handler(padapter, rfpath,
			rate, bandwidth, channel, tic);
}
