...
typedef struct rtw_if_operations {
	...
	int __must_check(*write)(struct dvobj_priv *d, unsigned int addr,
				 void *buf,
				 size_t len, bool fixed);
...
struct macid_ctl_t {
	...
	struct sta_info
		*sta[MACID_NUM_SW_LIMIT]; /* corresponding stainfo when macid is not shared */
...
struct dvobj_priv {
	...
	pconfig_descriptor;/* UsbConfigurationDescriptor; */
#endif
...
struct _ADAPTER {
	...
	ERROR_CODE
	LastError; /* <20130613, Kordan> Only the functions associated with MP records the error code by now. */
...
int rtw_parse_ssid_list_tlv(char **list_str, pno_ssid_t *ssid, int max,
			    int *bytes_left);
