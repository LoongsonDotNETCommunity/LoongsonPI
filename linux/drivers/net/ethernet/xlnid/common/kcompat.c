...
int xlnid_dcb_netlink_register(void)
{
	return 0;
}

int xlnid_dcb_netlink_unregister(void)
{
	return 0;
}

int xlnid_copy_dcb_cfg(struct xlnid_adapter __always_unused *adapter, int __always_unused tc_max)
{
	return 0;
}
#endif /* < 2.6.23 */
		      ...
		      u16 num_tx_queues)
{
	...
	if (netdev_get_num_tc(dev)) {
		...
		if (skb->priority == TC_PRIO_CONTROL) {
			...
			qoffset &= XLNID_TX_FLAGS_VLAN_PRIO_MASK;
			qoffset >>= 13;
		}
...
{
	struct adapter_struct *kc_adapter = netdev_priv(dev);
	if (kc_adapter->flags & XLNID_FLAG_DCB_ENABLED)
...
int _kc_netdev_set_num_tc(struct net_device *dev, u8 num_tc)
{
	...
	if (num_tc > XLNID_DCB_MAX_TRAFFIC_CLASS)
		return -EINVAL;
...
u8 _kc_netdev_get_prio_tc_map(struct net_device __maybe_unused *dev, u8 __maybe_unused up)
{
	...
	return xlnid_dcb_get_tc_from_up(&kc_adapter->dcb_cfg, 0, up);
}
...
static inline int kc_get_xps_queue(struct net_device *dev, struct sk_buff *skb)
{
	...
	if (kc_adapter->flags & XLNID_FLAG_FDIR_HASH_CAPABLE) {
