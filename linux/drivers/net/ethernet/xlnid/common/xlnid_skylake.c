...
STATIC s32 xlnid_read_eeprom_buffer_skylake(struct xlnid_hw *hw, u16 offset,
					  u16 words, u8 *data);
STATIC s32 xlnid_write_eeprom_buffer_skylake(struct xlnid_hw *hw, u16 offset,
                      u16 bytes, u8 *data);

extern u32 xlnid_read_top_reg(u32 reg);
extern void xlnid_write_top_reg(u32 reg, u32 data);
...
void xlnid_init_mac_link_ops_skylake(struct xlnid_hw *hw)
{
	...
	if ((mac->ops.get_media_type(hw) == xlnid_media_type_fiber) &&
		...
		mac->ops.disable_tx_laser =
				       xlnid_disable_tx_laser_skylake;
		mac->ops.enable_tx_laser =
					xlnid_enable_tx_laser_skylake;
		mac->ops.flap_tx_laser = xlnid_flap_tx_laser_skylake;
...
s32 xlnid_init_ops_skylake(struct xlnid_hw *hw)
{
	...
	mac->ops.check_link = xlnid_check_mac_link_skylake;
    ...
    eeprom->ops.write_buffer = xlnid_write_eeprom_buffer_skylake;
...
static u32 xlnid_read_gephy_skylake(struct xlnid_hw *hw, u32 reg)
{
    u32 regval = (XLNID_MDIO_CL22_READ
                | ((hw->phy.addr & XLNID_MDIO_ADDR_MASK) << 8)
                | (reg & XLNID_MDIO_ADDR_MASK));

    XLNID_WRITE_REG_MAC(hw, SKYLAKE_MAS_TXBUS_CTL, regval);

    XLNID_WRITE_REG_MAC(hw, SKYLAKE_MAS_SEND_EN, 0x1);
    usec_delay(60);
    XLNID_WRITE_REG_MAC(hw, SKYLAKE_MAS_SEND_EN, 0x0);

    return XLNID_READ_REG_MAC(hw, SKYLAKE_MAS_RECV_DATA);
}

static u32 xlnid_read_gmac_skylake(struct xlnid_hw *hw, u32 reg)
{
    XLNID_WRITE_REG_MAC(hw, SKYLAKE_MCI_CMD_DATA, (XLNID_MAC_CMD_RD | (reg & 0xFFF)));

    return XLNID_READ_REG_MAC(hw, SKYLAKE_MCI_RDATA_O);
}

static void xlnid_write_gmac_skylake(struct xlnid_hw *hw, u32 reg, u32 data)
{
    XLNID_WRITE_REG_MAC(hw, SKYLAKE_MCI_WDATA_I, data);
    XLNID_WRITE_REG_MAC(hw, SKYLAKE_MCI_CMD_DATA, (XLNID_MAC_CMD_WR | (reg & 0xFFF)));
}
#endif
...
/**
 *  xlnid_check_mac_link_skylake - Determine link and speed status
 *  @hw: pointer to hardware structure
 *  @speed: pointer to link speed
 ...
 **/
s32 xlnid_check_mac_link_skylake(struct xlnid_hw *hw, xlnid_link_speed *speed,
               bool *link_up, bool link_up_wait_to_complete)
{
#ifdef XLNID_GEPHY_SUPPORT
    u32 bmcr = 0;
    u32 bmsr = 0;
    u32 top_speed, gmac_config;
    u32 i = 0;

    /* clear the old state */
    bmsr = xlnid_read_gephy_skylake(hw, MII_BMSR);

    bmsr = xlnid_read_gephy_skylake(hw, MII_BMSR);
	bmcr = xlnid_read_gephy_skylake(hw, MII_BMCR);
    ...
    top_speed = xlnid_read_top_reg(SKYLAKE_TOP_MPI_LINK_SPEED);
    top_speed &= ~(0x3 << (hw->bus.lan_id << 1));
    
    gmac_config = xlnid_read_gmac_skylake(hw, 0x0);
    gmac_config &= ~(0x0000c000);

    bmcr = xlnid_read_gephy_skylake(hw, MII_BMCR);
    if (bmcr & BMCR_SPEED1000)
    {
        *speed = XLNID_LINK_SPEED_1GB_FULL;
        top_speed |= (0x2 << (hw->bus.lan_id << 1));
    }
    else if (bmcr & BMCR_SPEED100)
    {
        *speed = (bmcr & BMCR_FULLDPLX) ? XLNID_LINK_SPEED_100_FULL : XLNID_LINK_SPEED_100_HALF;
        top_speed |= (0x1 << (hw->bus.lan_id << 1));
        gmac_config |= 0x0000c000;
    }
    else
    {
        *speed = (bmcr & BMCR_FULLDPLX) ? XLNID_LINK_SPEED_10_FULL : XLNID_LINK_SPEED_10_HALF;
        gmac_config |= 0x00008000;
    }

    xlnid_write_top_reg(SKYLAKE_TOP_MPI_LINK_SPEED, top_speed);
    xlnid_write_gmac_skylake(hw, 0x0, gmac_config);
#endif

#ifdef XLNID_1000BASEX_SUPPORT
    u32 pcs_link = 0;

    pcs_link = xlnid_read_top_reg(SKYLAKE_PCS_LINK_STATUS);
    if (pcs_link & (1 << hw->bus.lan_id))
    {
        *link_up = true;
        *speed = XLNID_LINK_SPEED_1GB_FULL;
    }
    ...
    xpcs_link = xlnid_read_top_reg(SKYLAKE_PCS_LINK_STATUS);
    if (xpcs_link & (0x1000 << hw->bus.lan_id))
    {
        *link_up = true;
        *speed = XLNID_LINK_SPEED_10GB_FULL;
    }
...
/**
 *  xlnid_disable_tx_laser_skylake - Disable Tx laser
 *  @hw: pointer to hardware structure
 *
 ...
 **/
void xlnid_disable_tx_laser_skylake(struct xlnid_hw *hw)
{
    return;
}

/**
 *  xlnid_enable_tx_laser_skylake - Enable Tx laser
 *  @hw: pointer to hardware structure
 *
 *  The base drivers may require better control over SFP+ module
 ...
 **/
void xlnid_enable_tx_laser_skylake(struct xlnid_hw *hw)
{
    return;
}

/**
 *  xlnid_flap_tx_laser_skylake - Flap Tx laser
 *  @hw: pointer to hardware structure
 *
 ...
 **/
void xlnid_flap_tx_laser_skylake(struct xlnid_hw *hw)
{
	DEBUGFUNC("xlnid_flap_tx_laser_skylake");

	if (hw->mac.autotry_restart) {
		xlnid_disable_tx_laser_skylake(hw);
		xlnid_enable_tx_laser_skylake(hw);
...
#ifdef XLNID_GEPHY_SUPPORT
    hw->phy.addr = XLNID_READ_REG_MAC(hw, SKYLAKE_PI_PHYADDR);
#endif
