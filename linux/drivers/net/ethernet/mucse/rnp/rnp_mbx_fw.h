...
/*
	@ret:
		...
		!=0 : error
 */
__maybe_unused static int cmd_get_readreg_value(struct mbx_fw_cmd_reply *reply,
												int *value)
{
...
/*
static inline void dev_build_send_to_pf_cmd(struct mbx_fw_cmd_req *req, void
*cookie, void *buf, int buflen)
{
	req->flags   = 0;
	req->opcode  = SEND_TO_PF;
	req->datalen = buflen;
}
*/
									  ...
									  int bytes)
{
	...
	// printk("bytes:%d lo:%x,hi:%x\n", req->get_dump.bytes,
	// req->get_dump.bin_phy_lo, req->get_dump.bin_phy_hi);
...
static inline void build_mbx_sfp_write(
	struct mbx_fw_cmd_req *req, unsigned nr_lane, int sfp_addr, int reg, int v)
{
	req->flags = 0;
...
static inline void build_set_phy_reg(struct mbx_fw_cmd_req *req, void *cookie,
                                     enum PHY_INTERFACE phy_inf, char phy_addr, int reg, int w_data,
                                     int recall_qsfp_page)
{
	...
	/*
	if (phy_inf != PHY_INTERNAL_PHY) {
		req->set_phy_reg.external_phy_addr = phy_addr;
	}
	*/
...
static inline void build_get_phy_reg(struct mbx_fw_cmd_req *req, void *cookie,
                                     enum PHY_INTERFACE phy_inf, char phy_addr, int reg)
{
    req->flags   = 0;
	...
	/*
	if (phy_inf != PHY_INTERNAL_PHY) {
		req->get_phy_reg.external_phy_addr = phy_addr;
	}
	*/
