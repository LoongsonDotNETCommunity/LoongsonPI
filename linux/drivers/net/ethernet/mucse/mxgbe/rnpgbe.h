...
extern void rnpgbe_unmap_and_free_tx_resource(struct rnpgbe_ring *ring,
		struct rnpgbe_tx_buffer *tx_buffer_info);
...
extern void rnpgbe_setup_dma_rx(struct rnpgbe_adapter *adapter,
		int count_in_dw);
...
extern u32 rnpgbe_tx_desc_unused_sw(struct rnpgbe_ring *tx_ring);
extern u32 rnpgbe_tx_desc_unused_hw(struct rnpgbe_hw *hw,
		struct rnpgbe_ring *tx_ring);
extern s32 rnpgbe_disable_rxr_maxrate(struct net_device *netdev,
		u8 queue_index);
extern s32 rnpgbe_enable_rxr_maxrate(struct net_device *netdev, u8 queue_index,
		u32 maxrate);
extern u32 rnpgbe_rx_desc_used_hw(struct rnpgbe_hw *hw,
					  ...
					  struct sk_buff *skb)
{
	...
	//__rnpgbe_ptp_rx_hwtstamp(rx_ring->q_vector, skb);
