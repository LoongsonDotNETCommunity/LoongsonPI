...
// OPTM_WITH_LPAGE should never define along with CONFIG_RNP_DISABLE_PACKET_SPLIT
...
struct rnpgbe_eth_operations {
	...
	void (*set_min_max_packet)(struct rnpgbe_eth_info *eth, int min, int max);
	void (*set_vlan_strip)(struct rnpgbe_eth_info *eth, u16 queue, bool enable);
	...
	void (*set_vf_vlan_mode)(struct rnpgbe_eth_info *eth, u16 vlan,
			int vf, bool enable);
};
...
struct rnpgbe_hw_operations {
	...
	void (*set_vlan_filter)(struct rnpgbe_hw *hw, u16 vid, bool enable,
			bool sriov_flag);
	int (*set_veb_vlan_mask)(struct rnpgbe_hw *hw, u16 vid, int vf, bool enable);
	...
	void (*set_rx_mode)(struct rnpgbe_hw *hw, struct net_device *netdev,
			    bool sriov_flag);
	void (*set_rar_with_vf)(struct rnpgbe_hw *hw, u8 *mac, int idx, u32 vfnum, bool enable);
	...
	void (*set_mac_speed)(struct rnpgbe_hw *hw, bool link, u32 speed, bool duplex);
	...
	s32 (*check_link)(struct rnpgbe_hw *hw, rnpgbe_link_speed *speed, bool *link_up,
			  bool *duplex, bool link_up_wait_to_complete);
	s32 (*setup_link)(struct rnpgbe_hw *hw, rnpgbe_link_speed adv, u32 autoneg,
			u32 speed, u32 duplex);
	void (*clean_link)(struct rnpgbe_hw *hw);
	s32 (*get_link_capabilities)(struct rnpgbe_hw *hw, rnpgbe_link_speed *speed,
				     bool *autoneg);
	s32 (*init_rx_addrs)(struct rnpgbe_hw *hw);

	// ntuple function
	void (*set_layer2_remapping)(struct rnpgbe_hw *hw,
				     union rnpgbe_atr_input *input, u16 pri_id, u8 queue,
				     bool prio_flag);
	...
	void (*set_tcp_sync_remapping)(struct rnpgbe_hw *hw, int queue, bool flag,
				       bool prio);
	void (*set_rx_skip)(struct rnpgbe_hw *hw, int count, bool flag);
	void (*set_outer_vlan_type)(struct rnpgbe_hw *hw, int type);

	void (*update_hw_status)(struct rnpgbe_hw *hw, struct rnpgbe_hw_stats *hw_stats,
	...
	s32 (*phy_read_reg)(struct rnpgbe_hw *hw, u32 reg_addr,
			u32 device_type, u16 *phy_data);
	s32 (*phy_write_reg)(struct rnpgbe_hw *hw, u32 reg_addr,
			u32 device_type, u16 phy_data);
	void (*setup_wol)(struct rnpgbe_hw *hw, u32 mode);
	void (*set_vf_vlan_mode)(struct rnpgbe_hw *hw, u16 vlan, int vf, bool enable);
...
struct rnpgbe_mac_operations {
	...
	void (*set_mac_speed)(struct rnpgbe_mac_info *mac, bool link, u32 speed, bool duplex);
	void (*set_mac_fcs)(struct rnpgbe_mac_info *mac, bool status);
	s32 (*set_fc_mode)(struct rnpgbe_mac_info *mac);
	void (*check_link)(struct rnpgbe_mac_info *mac, rnpgbe_link_speed *speed,
	...
	int (*mdio_read)(struct rnpgbe_mac_info *mac, u32 phyreg, u32 *regvalue);
	void (*pmt)(struct rnpgbe_mac_info *mac, u32 mode);
...
struct rnpgbe_phy_operations {
	...
	s32 (*read_reg)(struct rnpgbe_hw *hw, u32 reg_addr, u32 device_type, u16 *phy_data);
	s32 (*write_reg)(struct rnpgbe_hw *hw, u32 reg_addr, u32 device_type, u16 phy_data);
	s32 (*setup_link)(struct rnpgbe_hw *hw);
	s32 (*setup_link_speed)(struct rnpgbe_hw *hw, rnpgbe_link_speed speed, bool wait_to_complete);
	s32 (*check_link)(struct rnpgbe_hw *hw, rnpgbe_link_speed *speed, bool *flag);
	s32 (*get_firmware_version)(struct rnpgbe_hw *hw, u16 *version);
	s32 (*read_i2c_byte)(struct rnpgbe_hw *hw, u8 byte_offset, u8 dev_addr, u8 *data);
	s32 (*write_i2c_byte)(struct rnpgbe_hw *hw, u8 byte_offset, u8 dev_addr, u8 data);
...
struct rnpgbe_dma_operations {
	void (*set_tx_maxrate)(struct rnpgbe_dma_info *dma, u16 queue, u32 max_rate);
	void (*set_veb_mac)(struct rnpgbe_dma_info *dma, u8 *mac, u32 vfnum, u32 ring);
	/* only set own vlan */
	void (*set_veb_vlan)(struct rnpgbe_dma_info *dma, u16 vlan, u32 vfnum);
	void (*set_veb_vlan_mask)(struct rnpgbe_dma_info *dma, u16 vlan, u16 mask, int entry);
...
struct rnpgbe_mbx_operations {
	...
	s32 (*read_posted)(struct rnpgbe_hw *hw, u32 *msg, u16 size, enum MBX_ID);
	s32 (*write_posted)(struct rnpgbe_hw *hw, u32 *msg, u16 size, enum MBX_ID);
