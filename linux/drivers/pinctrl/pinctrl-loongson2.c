...
static int loongson2_pinctrl_probe(struct platform_device *pdev)
{
	...
	pctrl->saved_regs = devm_kcalloc(&pdev->dev, ARRAY_SIZE(loongson2_pmx_groups),
...
#ifdef CONFIG_PM_SLEEP
static int loongson2_pinctrl_suspend(struct device *dev)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct loongson2_pinctrl *pctrl = platform_get_drvdata(pdev);
	unsigned int ngroups = ARRAY_SIZE(loongson2_pmx_groups);
	void * __iomem reg;
	unsigned long flags;
	int i, mux_bit;
	...
	spin_lock_irqsave(&pctrl->lock, flags);
	for (i = 0; i < ngroups; i++) {
		reg = pctrl->reg_base + loongson2_pmx_groups[i].reg;
		mux_bit = loongson2_pmx_groups[i].bit;
...
static int loongson2_pinctrl_resume(struct device *dev)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct loongson2_pinctrl *pctrl = platform_get_drvdata(pdev);
	unsigned int ngroups = ARRAY_SIZE(loongson2_pmx_groups);
...
static const struct dev_pm_ops loongson2_pinctrl_pm_ops = {
	SET_LATE_SYSTEM_SLEEP_PM_OPS(loongson2_pinctrl_suspend,
				loongson2_pinctrl_resume)
};
#endif
